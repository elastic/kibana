...
All JavaScript and TypeScript code is formatted with [oxfmt](https://oxc.rs/docs/guide/usage/formatter),
configured in `.oxfmtrc.json`. Run `node scripts/oxfmt [paths...]` to format files
(`--check` only reports) and `node scripts/eslint --fix` to fix linting issues. We recommend
enabling the oxfmt editor extension (format on save) and running ESLint via your IDE; ESLint no
longer formats code.

Whenever possible we are trying to use the formatter and linting, instead of maintaining a set of written style guide rules.
Consider every linting rule and every formatting rule to be also part of our style guide
