...
- **TypeScript does not catch circular dependencies between files at compile time.** TypeScript (and ES6 modules) allow circular imports between files, so circular dependencies will compile successfully but can cause runtime issues.

- **TypeScript project references can detect circular dependencies between projects** when building with `tsc -b`. If you're using TypeScript project references (see [TypeScript documentation](docs/extend/typescript.md)), circular dependencies between projects will be caught during the build process.

- **The Kibana plugin system detects circular dependencies at runtime.** When plugins are loaded, the system performs topological sorting and will throw an error if circular dependencies are detected between plugins, preventing the application from starting.

- **For TypeScript project references**, circular dependencies between projects must be resolved before migration, as they prevent proper type declaration generation.
