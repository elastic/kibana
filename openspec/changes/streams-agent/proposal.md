...
### New Capabilities
- `stream-querying`: Natural language read operations — listing streams, inspecting stream details (retention, storage, ingestion, processors, schema, partitions, data quality, features/systems), and cross-stream analysis (e.g. "which streams have data quality issues?"). Implemented as Agent Builder tools that call existing Streams APIs.
...
- **Tool registration**: New Streams-scoped tools (e.g. `streams.list_streams`, `streams.get_stream`, `streams.set_retention`, `streams.suggest_partitions`, etc.) registered via `agentBuilder.tools.register()`.
