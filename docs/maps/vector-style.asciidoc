...
Quantitative data driven styling symbolizes non-numeric fields like strings and ip addresses by category.

Quantitative data driven styling is available for the following styling properties:
* *Fill color*
* *Border color*
* *Label color*
* *Label border color*

Quantitative data driven styling uses {ref}/search-aggregations-bucket-terms-aggregation.html[terms aggregation] to retrieve the top nine categories for the field. Feature values within the top categories set are assigned a unique color. Feature values outside of the top categories set are grouped into the *Other* category. A feature is assigned the *Other* category when the property value is undefined.
