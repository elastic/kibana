...
* *Interpolate (default)*. Color ramp is divided into 8 bands. Values are interpolated between min and max to a color band on a linear scale.
* *Percentiles*. Use percentiles to divide color ramp into bands that map to values
