...
*Maps* is your place for analyzing geospatial data. For example, a map might 
show the origin, destination,
and average time of flights or web log requests by country.
...
Your best bet to getting started with *Maps* is to add one of 
the sample data sets that ship with {kib}. Adding a sample data set also 
loads a map to go with the data.

. In the upper left corner of your browser, click the {kib} logo to navigate
to the {kib} home page.
. Click *Load a data set and a {kib} dashboard*.
. Add the *Sample web logs* data set.

. In the side navigation, click *Maps*. 

. In the maps list, click *[Logs] Total Requests and Bytes*.
+
This loads the map for the web logs data set. Check out each of the three layers of the map. As you move your mouse over the map,
note how the tooltips show property values. 

You'll re-create the *[Logs] Total Requests and Bytes* map in this tutorial. 
You'll learn how to:
...
. In the application header, click *Maps* to return to the list of maps.
. Click *Create map*.
+
...
===== Join the vector layer with the {es} index
