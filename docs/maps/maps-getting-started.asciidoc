...
Add your saved map to a {kibana-ref}/dashboard.html[dashboard] to view geospatial data along side bar charts, pie charts, and other visualizations.

. In the side navigation, click *Dashboard*.
. Click *Create new dashboard*.
. Set the time range to *Last 7 days*.
. Click *Add*.
. To filter *Add panels* to only show maps, set the *Types* select to *Map*.
. To add a map to the dashboard, click on the name your saved map or the *[Logs] Total Requests and Bytes* map included with the sample data set.
. To filter *Add panels* to show visualizations included with the same data set, set the *Types* select to *Visualization*.
...
. In the map panel, move your mouse over the United States vector and click the mouse.
