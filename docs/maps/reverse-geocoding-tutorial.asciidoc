...
*Maps* comes with a lot of https://maps.elastic.co/#file[predefined regions] allowing you to quicky build visualize regions by metrics. *Maps* also offers the ability to easily map your own regions. You can use any region data you'd like, as long as your source data contains an identifier for the corresponding region. How can you map regions when your source data does not contain a region identifier? This is where reverse geocoding comes in. Reverse geocoding is the process of assigning a region identifer to a feature based on its location.
