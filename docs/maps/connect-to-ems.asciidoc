...
With Elastic Maps Server running, add the `map.emsUrl` configuration key in your `kibana.yml` file pointing to the root of the service. This setting will point Kibana to request EMS basemaps and boundaries from the new instance.
...
Elastic Maps Server runs a status check periodically that is exposed in three different forms:
