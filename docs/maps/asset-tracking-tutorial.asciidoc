...
Add a layer for construction zones. You will draw the construction zones on a map. This construction zones will be used as your geofence boundary or threshold that serves as the basis for triggering alerts.
...
.. Select bounding box icon image:maps/images/asset-tracking-tutorial/bounding_box_icon.png[bounding box icon] in the toolbar on the left side of the map.
.. Move your mouse to an area of the map where you would like to draw a construction zone bounding box.
.. Click to start rectangle.
.. Move mouse to adjust rectangle size.
.. Click again to finish.
. When finished drawing construction zones, click *Exit* under the layer name in the legend.
. In *Layer settings*, set *Name* to *Construction zones*.
. Scroll to *Layer Style*, set *Fill color* to yellow.
. Click *Save & close*.
. *Save* the map. Give the map a title, and under *Add to dashboard*, select *None*. Click *Save and add to library*.
...
[float]
====
Step 2. Create a new index for the alerts

Every time a bus enters the construction zone, it will cause an alert. In this case, each alert will write a new document to the `bus_alerts` index.

. Open the main menu, and click *Dev Tools*.
. To create the `bus_alerts` index, run:
+
[source,js]
----------------------------------
PUT bus_alerts
...
. To configure the `bus_alerts` index mappings, run:
+
[source,js]
----------------------------------
PUT bus_alerts/_mapping
{
  "properties": {
    "entityId": {
      "type": "keyword"
...
. Click *Create index pattern*.
. Give the index pattern a name: *bus_alerts**.
. Click *Next step*.
. Set the *Time field* to *detectionDateTime*.
. Click *Create index pattern*.

[float]
==== Step 3. Configure an alert

Create a new alert by defining a rule and a connector. The rule includes the conditions that will trigger the alert, and the connector defines what action takes place once the alert is triggered.
...
. Scroll to *Tracking containment* and select.
. Set *Select entity*:
...
. In the *Index connector*, set:
.. *Connector name* to *Bus alert connector*
.. Index to *bus-alerts*.
.. Click Save.
. Complete the *Actions* configuration.
.. *Run when*: Tracking containment met
.. *Index connector*: Bus alert connector
.. *Document to index*:
+
[source,js]
----------------------------------
...
[float]
==== Step 4. Visualize the alerts on the operational map

Add a layer to the existing map based on the alert index. The end result will show a map with active bus locations, their direction, their routes, the construction zones, and the alerts for when a bus entered a construction zone.

. Go to the map that you built in this tutorial.
. Click *Add layer*, and then select *Documents*.
. Select the *bus_alerts** index pattern.
. Accept the defaults and click *Add layer*.
. Click *Save & close*.
...
You have successfully completed the asset tracking tutorial. You now have the recipe. Now it’s your turn to try replicating the same analysis with your data. Have fun!
