...
. In *Console*, create the `tri_met_tracks` index lifecyle policy. This policy will keep the events in the hot data phase for 7 days at the end of which moves the data to the warm phase, after 365 days in the warm phase the data is delete. 
+
[source,js]
----------------------------------
