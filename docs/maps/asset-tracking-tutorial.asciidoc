...
Its hard to get an overview of Portland buses by looking at individual events. Lets create a map to show the bus routes and current location for each bus along with the direction buses are headed.
...
Lets make TriMet Portland bus data actionable and alert when buses enter construction zones.
...
Congratulations! You have completed the tutorial and have the recipe for tracking assets. Now try replicating this same analysis with your own data.
