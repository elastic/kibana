...
Use aggregated layers with document layers to show aggregated views when the map shows larger amounts of the globe and individual documents when the  map shows smaller regions. See the <<maps-add-elasticsearch-layer, Getting started>> tutorial for more details.
...
The user starts at zoom level 3 and only sees aggregated data. As the user zooms in, the aggregated data transitions to show individual documents.
