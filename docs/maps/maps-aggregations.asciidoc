...
Terms joins use a shared key to combine vector features, your left source, with the results of an {es} terms aggregation, your right source.
...
The cloropeth example links World countries and web log traffic on the shared key, https://wikipedia.org/wiki/ISO_3166-1_alpha-2[ISO 3166-1 alpha-2 code].
...
The cloropeth example uses https://www.elastic.co/elastic-maps-service[Elastic Maps Service (EMS)] World Countries for the left source. The cloropeth example uses vector features provided by EMS. You can also use your own vector features.

The following shows an example feature from World Countries. Note the *iso2* property, which defines the shared key for the left source.
--------------------------------------------------
{
  geometry: {
    coordinates: [...],
...
After configuring the join, the join adds metrics for each terms aggregation bucket are added to the world country feature with the corresponding ISO 3166-1 alpha-2 code. Features that do not have a corresponding terms aggregation bucket are not visible on the map.
