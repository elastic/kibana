...
Use aggregated layers with document layers to show aggregated views when the map shows larger amounts of the globe and individual documents when the  map shows smaller regions. See the <<maps-add-elasticsearch-layer, Getting started>> tutorial for more details on configuring the layers.

In the following example, the *Grid aggregation* layer is only visible when the map is at zoom levels 0 through 5.
The *Documents* layer is only visible when the map is at zoom levels 4 through 24.
The map is at zoom level 3 and is only showing the aggregated layer. As the user zooms in, the map transitions from showing the aggregated layer to showing the individual documents layer.
