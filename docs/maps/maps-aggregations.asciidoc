...
Use top hits per entity to display the most relevent documents per entity.
For example, you could display the most recent GPS tracks per flight.

Top hits per entity uses {es} {ref}/search-aggregations-bucket-terms-aggregation.html[terms aggregation] to group your documents by entity.
...
[role="screenshot"]
image::maps/images/top_hits.gif[]
