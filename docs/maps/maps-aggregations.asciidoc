...
Use aggregated layers in conjunction with document layers to show aggregated views when the map shows larger amounts of the globe and individual documents when the  map shows smaller regions. See <<maps-add-elasticsearch-layer, gettig started>> tutorial for more details.
