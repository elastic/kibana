...
You can run {es} on your laptop or on hundreds of servers.
How do you plot documents from hundreds of servers onto a single map?
...
Aggregations group your documents into buckets and then caluclate metrics for each bucket.
...
The *Grid aggregation* source uses {ref}/search-aggregations-bucket-geotilegrid-aggregation.html[GeoTile grid aggregation] to group {es} documents into grids. You can calculate metrics for each gridded cell.
...
*Grid rectangles*:: Creates a <<vector-layer, vector layer>> with the bounding box polygon for each gridded cell.
...
Most recent entities uses {ref}/search-aggregations-metrics-top-hits-aggregation[Top hits aggregation]

Configure the *Documents* source to that plot most recent entities.
