...
*Clusters* uses {ref}/search-aggregations-bucket-geotilegrid-aggregation.html[GeoTile grid aggregation] to group your documents into grids. You can calculate metrics for each gridded cell.

You can symbolize grid aggregation metrics as:

*Circles*:: Creates a <<vector-layer, vector layer>> with a circle for each gridded cell.
The circle is centered at the weighted centroid for all geo-points in the gridded cell.
...
. Click *Add layer* button and select *Grid aggregation* source.

To enable a blended layer that dynamically shows clusters or documents:

. Click *Add layer* button and select *Documents* source.
. Configure *Index pattern* and *Geospatial field*. To enable clustering, *Geospatial field* must be set to a field mapped as {ref}/geo-point.html[geo_point].
. In *Scaling*, select the *Show clusters when results exceed 10000*.
...
. In *Scaling*, select the *Show top hits per entity*.
