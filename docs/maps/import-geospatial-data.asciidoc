...
* [ ] `create`, `create_index`, and `read` index privileges for destination indices.
Its recommended that users also have `view_index_metadata` index privilege for destination indices.
