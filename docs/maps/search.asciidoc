...
* Clear the *Apply global filter to source* checkbox in *Source settings* to prevent the global search context from applying to the layer source.

* Clear the *Apply global filter to join* checkbox in *Term joins* to prevent the global search context from applying to a <<terms-join, term join>>.
