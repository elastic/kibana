...
Layers that request data from {es} using an <<index-patterns, {data-source}>> with a configured time field are narrowed by the <<set-time-filter, global time filter>>.
