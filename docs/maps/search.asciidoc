...
Layers narrowed by the time filter contain the clock icon image:maps/images/clock_icon.png[] next to the layer name in the legend. Use the timeslider to quickly select time slices within the global time filter range.
Click previous and next buttons to advance the timeslice backward or forward. Click play to animate your spatial temporal data.
...
You can create three types of filters by interacting with your map:
...
Filter dashboard panels by map extent to show uniform data across all panels.
...
* Click the gear icon image:maps/images/gear_icon.png[] to open the map panel menu.
