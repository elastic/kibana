...
`locations` are hosted by Elastic. You can specify a Private Locations using the location's name.
+
...
[NOTE]
====
You may provide either `locations` or `private_locations`, and you may also provide both if needed. At least one of them is required, but providing both is optional.
====
...
`source.inline.script`:: (Required, string): The inline script.
...
POST /api/synthetics/monitors
{
  ...
  "source.inline.script": "step('Go to https://google.com.co', async () => {\n  await page.goto('https://www.google.com');\n});"
}
--------------------------------------------------
