...
`PUT <kibana host>:<port>/api/synthetics/monitors/<monitor_id>`

`PUT <kibana host>:<port>/s/<space_id>/api/synthetics/monitors/<monitor_id>`
...
`type`:: (Required, string): The monitor type (e.g., `http` , `tcp` , `icmp` or `browser`) can't be changed after the monitor is created.
