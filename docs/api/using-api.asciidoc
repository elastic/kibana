...
For all non whitelisted APIs, you must use a header.
...
`Content-Type: application/json`::
  {kib} APIs use JSON for requests and responses. To make an API call, configure the `Content-Type` accordingly.
