...
For all non whitelisted APIs, you must use a header. For information on how to whitelist APIs, refer to the <<settings, `server.xsrf.whitelist`>> documentation.

{kib} APIs support the following headers:

`kbn-xsrf: true`::
  By default, {kib} requires the `kbn-xsrf` header for all API interactions.

`Content-Type: application/json`::
  {kib} APIs use JSON for the request and response bodies. To interact with the APIs, configure the `Content-Type` accordingly.
