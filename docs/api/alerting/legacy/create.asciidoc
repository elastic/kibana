...
deprecated::[7.15.0,"Ability to specify `<id>` as a path parameter is deprecated in 7.15.0. As part of the {kibana-ref}/sharing-saved-objects.html[Sharing Saved Objects] effort, all rule IDs will be regenerated in 8.0.0. Because we cannot guarantee that a rule created with a specified ID will retain that ID, we are deprecating this path parameter."]
