...
WARNING: As part of the {kibana-ref}/sharing-saved-objects.html[Sharing Saved Objects] effort, rule IDs for all rules in a custom space will be regenerated in 8.0.0. This means that all rules created using this API that specifies both the `id` and `space_id` path parameters will be re-assigned a randomly generated ID in 8.0.0.
