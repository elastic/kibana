...
`allowPartialAuthorization`::
  (Optional, boolean) When enabled, the API will return any spaces that the user is partially authorized to access. This only has an effect
  when _not_ using the `any` purpose. Any partially authorized results (for instance, spaces that the user has read-only access to) will be
