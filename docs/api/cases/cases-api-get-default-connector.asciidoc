...
== Get default case connector API
++++
<titleabbrev>Get default connector</titleabbrev>
++++

Retrieves information about the default connector for cases.
...
connector to a specific set of applications. Valid values are: `cases`,
`observability`, and `securitySolution`. If this parameter is omitted, the
response contains the default connector for all applications that the user has
access to read.
