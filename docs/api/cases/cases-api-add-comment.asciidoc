...
only when `type` is `alert`. If you are adding multiple alerts to a case, use an
array of strings; `index` must also be an array with the same length or number
of elements. Adding multiple alerts in this manner is recommended rather than
calling the API multiple times. preview:[]
...
of strings; `alertId` must also be an array with the same length or number of
elements. preview:[]
