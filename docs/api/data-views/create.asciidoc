...
`allowNoIndex`::
(Optional, boolean) Allows the data view saved object to exist before data is present.

`namespaces`::
(Optional, string[]) Array of {kibana-ref}/xpack-spaces.html[space] IDs, for sharing the data view between multiple spaces.
