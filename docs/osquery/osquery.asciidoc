...
If you choose the *Policy* option, you must also define policies to run the pack. To do this, configure one or both of the following settings: 
...
NOTE: If you only want _some_ {agents} within a policy to run the pack, configure these details in the *Partial deployment (shards)* field. 
+
** *Partial deployment (shards)*: In the *Policy* field, enter the name of a policy to run the pack. Next, use the *Shard* slider to specify a percentage of {agents} to run pack. For example, if you choose 50% on the slider, the pack will be scheduled to run on half of the policy's registered agents.
