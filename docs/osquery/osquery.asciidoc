...
that represent a point in time with a set of results. You can modify the result type so that query results are returned as
https://osquery.readthedocs.io/en/stable/deployment/logging/#differential-logs[differentials] or differentials without removals (this will only show data that has been added).
