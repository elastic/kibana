...
+
TIP: Pack deployment details are stored within the <<osquery-custom-config,Osquery configuration>>. The `shard` field value shows the percentage of agents within the policy to which the pack has been deployed.  
+
If you choose the *Policy* option, you must also configure one or both of the following fields: 
+
...
** *Partial deployment (shards)*: Allows you to partially deploy the pack to specified agent policies. After defining a policy, use the *Shard* slider to set the amount of agents to which the pack is deployed. For example, after specifying a policy, you can choose to deploy the pack to half of the policy's agents by selecting 50% on the slider.
