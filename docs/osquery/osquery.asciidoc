...
that represent a point in time with a set of results. You can modify the result type to be https://osquery.readthedocs.io/en/stable/deployment/logging/#differential-logs[differential logs] or differential logs without removals (results only show data that has been added) type.
