...
The *Live queries history* section on the *Live queries* tab shows a log of queries run over the last 30 days. From the Live queries table, you can do the following:

* Click image:images/play-icon.png[Right-pointing triangle] to rerun a single query or a query pack.

* Click image:images/table-icon.png[Table icon] to examine the <<osquery-results,results>> for a single query or a query pack. From the results table, you can also find the query <<osquery-status,status>>.
+
[role="screenshot"]
