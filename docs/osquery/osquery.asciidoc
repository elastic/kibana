...
. Provide a name for the pack. A short description is optional. 
...
+
** *Partial deployment (shards): In the *Policy* field, enter the name of a policy to run the pack. Next, use the *Shard* slider to specify a percentage of policy {agents} to run pack. 
