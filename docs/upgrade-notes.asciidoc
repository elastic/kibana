...
[discrete]
[[known-issue-204384]]
.Make HTTP/2 the default protocol when TLS is enabled and add a deprecation warning if HTTP/2 is not enabled or TLS is not configured (9.0.0)
[%collapsible]
====
*Details* +
...
*Action* +
1. Verify that TLS is properly configured by enabling it and providing valid certificates in the settings.
2. Test your system to ensure that connections are established securely over HTTP/2.
