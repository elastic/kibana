...
*Action* +
Existing user roles are automatically migrated to include the `savedQueryManagement` privilege as required to prevent breaking changes. No action is required for existing roles. For new roles, ensure that the `savedQueryManagement` privilege is set as needed.
====
