...
For security reasons, Code allows only a few <<#clone-url-management, trusted hostnames>>, such as github.com. To clone private repositories <<#clone-private-repo-with-ssh-key, add an SSH key>>.

To delete a repository, find the go to the **Repositories** tab, find the name of the repo and click *Delete*.

[float]
==== Clone private repo with SSH key
Clones of private repos requires an SSH key for authentication. The username associated with your host must have write access to the repository you want to clone.
...
- Your disk might not have enough space to clone repositories due to {kib} watermark settings. To update you're watermark settings, contact your system administrator and request additional disk space.
