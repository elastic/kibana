...
`items:{ name:banana and stock:9 }` does not match because there isn't a single nested document that 
matches the entire query in the nested group. 

`items:{ name:banana } and items:{ stock:9 }` is a similar looking query but returns different results. Since the 
subqueries are in separate nested groups they can match different nested documents. `name:banana` will match the first
document in the array and `stock:9` will match the third document in the array. This is the second way of querying a 
nested field, mentioned above.

You can combine these two strategies to create complex queries. What if you wanted to find a store with more than 10 
