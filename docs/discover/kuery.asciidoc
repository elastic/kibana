...
free text search or field-based search. It is only used for filtering data, and has
no role in sorting or aggregating the data.

KQL is able to suggest field names, values, and operators as you type, if you have the
https://www.elastic.co/subscriptions[Basic tier] or above. The performance of the suggestions
are controlled by <<settings, {kib} settings>>:
...
To query using *exact search terms*, type the field name followed by `:` and
then the values with spaces separating:
