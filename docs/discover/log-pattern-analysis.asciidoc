...
. Click the `message` field in the **Available fields** list on the left 
sidebar and click **Run pattern analysis**.
+
--
[role="screenshot"]
...
You can apply filters to one or more patterns. By doing so, Discover only 
display documents that match the selected patterns. Additionally, you can remove 
selected patterns from the Discover view, resulting in the display of only those 
