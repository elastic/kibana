...
The in-table search looks for all matching values in all results and pages currently loaded in the table. The number of results loaded depends on the <<document-explorer-sample-size,Sample size>> option value. If you load more results, the search automatically updates and reflects the new number of matching values, if any more are found.
