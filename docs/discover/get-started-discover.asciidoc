...
[[filter-in-discover]]
With the query field, you can filter data using the KQL or Lucene languages. You can also use the **Add filter** function available next to the query field to build your filters manually or edit them as Query DSL.
...
This can happen with runtime fields and regular keyword fields when the string exceeds the value set for the {ref}/ignore-above.html[ignore_above] setting used when indexing the data into {es}.