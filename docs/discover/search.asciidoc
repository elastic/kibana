...
=== Save a query
A filter set is a portable collection of query text and filters that you can reuse in <<discover, Discover>> and <<dashboard, Dashboard>>. Save a query when you want to:
...
Filter sets don't include information specific to Discover, such as the currently selected columns in the document table, the sort order, and the index pattern. If you want to save your current view of Discover for later retrieval and reuse, create a <<save-open-search, saved search>> instead.

[role="xpack"]
==== Read-only access
If you have insufficient privileges to save queries, the *Save current query* button isn't visible in the filter set management popover. For more information, see <<xpack-security-authorization, Granting access to Kibana>>
...
+
[role="screenshot"]
image::discover/images/saved-query-management-component-all-privileges.png["Example of the filter set management popover with a list of filter sets with write access",width="80%"]
+
...
+
[role="screenshot"]
image::discover/images/saved-query-save-form-default-filters.png["Example of the filter set management save form with the filters option included and the time filter option excluded",width="80%"]
. Click *Save*.

==== Load a query
To load a filter set into Discover or Dashboard:
...
+
[role="screenshot"]
image::discover/images/saved-query-management-component-save-as-new-query.png["Example of the filter set management popover when a query is loaded and we have made changes to the query",width="80%"]
...
+
[role="screenshot"]
image::discover/images/saved-query-management-component-delete-query-button.png["Example of the filter set management popover when a query is hovered over and we are about to delete a query",width="80%"]

You can import, export, and delete filter sets from <<managing-saved-objects, Saved Objects in Management>>.
