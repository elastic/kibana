...
which features autocomplete and a simple, easy to use syntax or you can use the
full JSON-based {ref}/query-dsl.html[Elasticsearch Query DSL]. Kibana's legacy query
language (based on Lucene https://lucene.apache.org/core/2_9_4/queryparsersyntax.html[query syntax])
is still available for the time being under the options menu in the Query Bar. 
