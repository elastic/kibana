...
[float]
=== Query languages
{kib}'s <<kuery-query, standard query language>> (KQL)
features autocomplete and a simple, easy-to-use syntax. {kib}'s legacy query
language, based on the
Lucene https://lucene.apache.org/core/2_9_4/queryparsersyntax.html[query syntax],
is still available from the KQL popup in the query bar. When you enable the
legacy query language, you can also use the full
JSON-based {ref}/query-dsl.html[Elasticsearch Query DSL].

[float]
=== Search your data

You can search the indices that match the current <<index-patterns, index pattern>>
by entering your search criteria in the query bar.
The query bar is designed to help you find what you’re looking
for as quickly as possible. With KQL enabled, you'll get suggestions for
...
long-running queries are required.  Consider a threat hunting scenario
