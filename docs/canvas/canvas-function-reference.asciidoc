...
|`fields`
|`string`
|A comma-separated list of fields. Fewer fields will perform better

|`count`
|`number`
|The number of docs to retrieve. Smaller numbers perform better 
...
Adds a column calculated as the result of other columns, or not. See also <<alterColumn_fn>> and <<staticColumn_fn>>.
...
|`emptyImage`
|`string`
|Fill the difference between the _context_ and the `max` parameter with this image. Provide an image asset as a `base64` data URL, or pass in a sub-expression.
...
|`origin`
|`string`
|Where to start from. For example, `"top"`, `"left"`, `"bottom"`, or `"right"` 
