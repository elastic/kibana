...
|`backgroundColor`
|`string`
|A valid CSS background color string.

|`backgroundImage`
|`string`
|A valid CSS background image string.

|`backgroundSize`
|`string`
|A valid CSS background size string.

Default: `"contain"`

|`backgroundRepeat`
|`string`
|A valid CSS background repeat string.
...
Format a valid date string or number of milliseconds since epoch using https://momentjs.com/[MomentJS]
...
Returns whether the _context_ is greater than or equal to the argument.
...
Alias: `condition`
|`boolean`
|A `true` or `false`, usually returned by a sub-expression. 
If this is not supplied, then the input _context_ will be used.
...
then the input _context_ to the function will be returned.
|===
...
Aliases: `dataurl`, `url`
|`string`, `null`
|An image asset can be provided as a base64 dataurl, or passed in as a sub-expression
(see example below)
...
|`image`
|`string`
|The image to reveal. An image asset can be provided as a base64 dataurl, or passed in as a sub-expression 
(see example below) 
