...
|`format`
|`string`
|The MomentJS format used to parse the specified date string. See https://momentjs.com/docs/#/displaying/.
|===
...
Configures a drop-down filter control element.
...
Return whether the _context_ is equal to the argument.
...
Formats a number into a formatted number string using NumeralJS. See http://numeraljs.com/#format.
...
Find your current location using the Geolocation API of the browser. Performance can vary, but is fairly accurate. See https://developer.mozilla.org/en-US/docs/Web/API/Navigator/geolocation.
