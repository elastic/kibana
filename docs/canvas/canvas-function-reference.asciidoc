...
<<a_fns>> | B | <<c_fns>> | <<d_fns>> | <<e_fns>> | <<f_fns>> | <<g_fns>> | <<h_fns>> | <<i_fns>> | J | K | <<l_fns>> | <<m_fns>> | <<n_fns>> | O | <<p_fns>> | Q | <<r_fns>> | <<s_fns>> | <<t_fns>> | <<u_fns>> | V | W | X | Y | Z
...
Alias: `when`
|`any`
|This value is compared to the _context_ to see if the condition 
...
Alias: `this`, `b`
|`any`
|The value to compare the _context_ to, usually returned by a sub-expression.
|===
...
|`borderRadius`
|`string`
|The number of pixels to use when rounding the border

|`padding`
|`string`
|Content distance, in pixels, from the border.
...
Runs multiple sub-expressions and returns the original _context_ after executing the sub-expressions. Use for running functions that produce an action.
...
available in the root pipeline as this function simply returns the _context_.
|===
...
|`valueColumn` ***
|`string`
|The column or field to extract the unique values for the drop-down filter.
...
|The value to compare to the _context_.
|===
...
Queries {es} for the number of hits matching a query.
...
Queries {es} for raw documents. Specify the fields you want to receive, 
...
Format a `number` into a formatted `string` using NumeralJS. See http://numeraljs.com/#format.
...
Alias: `format`
|`string`
|A NumeralJS format string. See http://numeraljs.com/#format.
|===
...
Aliases: `column`, `c`
|`string`
|The name of the column to fetch the value from. If not provided, the value will be retrieved from the first column.
...
Alias: `value`
|`number`, `string`
|The value to compare the _context_ to
|===
...
Gets the first N rows from the `datatable`. See also <<tail_fn>>.
...
Alias: `count`
|`number`
|The number of rows to retrieve from the beginning of the `datatable`. See also <<tail_fn>>.
...
|`then`
|`any`
|The return value if the condition is `true`. When unspecified and the condition is met, the original _context_ is returned.

|`else`
|`any`
|The return value if the condition is `false`. When unspecified and the condition is not met, the original _context_ is returned.
|===
...
Aliases: `dataurl`, `url`
|`string`, `null`
|An image asset can be provided as a `base64` data URL, or passed in as a sub-expression
...
Returns whether the _context_ is less than the argument passed in.
...
Returns whether the _context_ is less than or equal to the argument passed in.
...
|`font`
|`style`
|Set the CSS font properties for the content. For exmaple, `font-family` or `font-weight`.
...
Interprets a `TinyMath` math expression using a `number` or `datatable` as _context_. Datatable columns are available by their column name. If you pass in a `number`, it is available as `value`.
...
|`metricFont`
|`style`
|Set the CSS font properties for the metric. For example, `font-family` or `font-weight`

Default: `{<<font_fn>> size=48 family="'Open Sans', Helvetica, Arial, sans-serif" color="#000000" align=center lHeight=48}`.

|`labelFont`
|`style`
|Set the CSS font properties for the label. For example, `font-family` or `font-weight`.
...
Alias: `color`
|`string`
|Palette colors, rgba, hex, or HTML color string
...
To automatically set radius, use `"auto"`.
...
a different row count, pipe into another instance of `ply`.				
...
|`barWeight`
|`number`
|Thickness of the background bar
...
specialized function instead, such as `<<plot_fn>>` or `debug`.
...
Alias: `format`
|`string`
|The MomentJS Format to use for bucketing. For example, `"YYYY-MM"` rounds to the month. See 
...
Use <<seriesStyle_fn>> inside of a charting function, like <<plot_fn>> or <<pie_fn>>.
...
Alias: `column`
|`string`
|The column to sort on. When unspecified, the `datatable` is sorted by the first column.
...
Adds a column with a static value. See also <<alterColumn_fn>> and <<mapColumn_fn>>.
...
|The columns values. TIP: To roll up other columns into a static value, use a sub-expression.	 
...
sub-expressions that output a string, or something castable to `string`.
...
|One or more values to join together into one `string`. Include spaces where needed.
|===
...
Retrieves the URL parameters, then uses the parameters in expressions. The <<urlparam_fn>> function always returns a `string`. For example, you can retrieve the value of `"20"` from the parameter `myVar` in the URL `https://localhost:5601/app/canvas?myVar=20`. 
