...
.. Click *Save*.
+
The query selects the total price field and sets it to the sum_total_price field. All fields are pulled from the sample eCommerce orders {data-source}.
...
.. From the *X-axis* drop-down lists, make sure *Value* and *order_date* are selected.
