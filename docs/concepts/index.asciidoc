...
[float]
=== Accessing data with {data-sources}

{kib} requires an {data-source} to tell it which {es} data you want to access,
and whether the data is time-based. {A-data-source} can point to one or more {es}
...
{Data-sources} are typically created by an administrator when sending data to {es}.

You can <<index-patterns,create or update {data-sources}>> in *Stack Management*, or by using a script
that accesses the {kib} API.

{kib} uses the {data-source} to show you a list of fields, such as
...
In most cases, the time filter applies to the time field in the {data-source},
