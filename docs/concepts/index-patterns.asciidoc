...
+
** To match multiple indices, use a wildcard (*). `filebeat-*` matches
...
+
** To exclude an index, use a minus sign (-), for example, `-test3`.
...
When you delete an index pattern, you cannot to recover the associated field formatters, runtime fields, source filters,
