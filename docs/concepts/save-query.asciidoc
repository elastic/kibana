...
With filter sets, you can save your query text, filters, and time range for
...
Filter sets are different than <<save-open-search,saved searches>>,
...
the *Save* button isn't visible in the filter set management popover.
...
. Save the query.
. To load a filter set, select it in the *Filter set* menu.
+
The query text, filters, and time range are updated and your data refreshed.
If you’re loading a filter set that did not include the filters or time range, those components remain as-is.
. To add filters and clear filter sets, use the *Filter set* menu.
