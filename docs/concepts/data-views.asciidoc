...
Excluding a cluster avoids sending any network calls to that cluster. For more information, see
{ref}/modules-cross-cluster-search.html#exclude-problematic-clusters[Excluding clusters or indicies from cross-cluster search]. To exclude a cluster with the name `cluster_one`:
