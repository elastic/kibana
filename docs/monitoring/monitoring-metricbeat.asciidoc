...
You must specify the following settings in the `modules.d/kibana-xpack.yml` file:
...
----------------------------------
<1> Replace `KIBANA_URL` with the appropriate host and port number that you use
to access {kib}. The default value is `localhost:5601`.
...
If you configured {kib} to use encrypted communications, you must access it via
HTTPS. For example, use a `hosts` setting like `https://localhost:5601`.
