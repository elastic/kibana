...
After you enable monitoring in one or more products in the {stack}, you can use 
{kib} to view their health and performance. 
...
. If the Elastic {security-features} are enabled on the monitoring cluster: 
...
. If the Elastic {security-features} are enabled on the {kib} server, identify 
or create users that have access to the {kib} indices and permission to read 
from the monitoring indices.
+
--
...
//TBD: What permissions/roles are required to enable data collection?
////
If you have permission to turn on data collection and there is data in the cluster,
{kib} displays the monitoring dashboards.  
////
...
. (Optional) Define index patterns so that you can explore your monitoring data. 
+
--
For example, use the pattern `metricbeat*` to match the indices that are sent to 
your monitoring cluster from {metricbeat}. For more information, see <<index-patterns>>. 
--
