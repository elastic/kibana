...
. To view a subset of the documents, such as the orders for women's clothing that are $60 or more, use the <<kuery-query,*KQL*>> search field:
+
[source,text]
products.taxless_price >= 60 and category : Women's Clothing
+
[role="screenshot"]
image::images/tutorial-discover-4.png[This needs to be updated]

. Hover over the list of *Available fields*, then click *+* next to the fields you want to view in the table.
+
To view the product categories that contain orders, click *+* next to *category*.
+
[role="screenshot"]
image::images/tutorial-discover-3.png[Discover table that displays only the product categories that contain orders]
+
Ready to learn more? Go to <<discover, *Discover*>>.
...
To view a subset of the data on the dashboard, apply filters to all of the dashboard panels. 

. View only the women's clothing data from the Gnomehouse manufacturer.

.. On the *[eCommerce] Controls* panel, select *Gnomehouse* from the *Manufacturer* dropdown.
...
. View only the women's clothing data generated on Wednesday from the Gnomehouse manufacturer. 
...
. To remove the filters, click *x* next to each filter.
...
+
Ready to learn more? Go to <<dashboard,Dashboard>>.
