...
The Agent Key API allows you to configure agent keys to authorize requests from agents to the APM Server.
...
Create an APM agent key. Specify privileges in the request body at creation time. If privileges are not specified at creation time, the created key will have all privileges.
...
`event`::
(optional, boolean) Adds the `event:write` privilege. Required for ingesting agent events.

`agentConfig`::
(optional, boolean) Adds the `config_agent:read` privilege. Required for agents to read agent configuration remotely.

`sourcemap`::
(optional, boolean) Adds the `sourcemap:write` privilege. Required for uploading sourcemaps.
...
Once created, you can copy the API key (Base64 encoded) and use it to to authorize requests from agents to the APM Server.