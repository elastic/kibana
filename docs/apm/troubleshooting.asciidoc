...
you might still see errors in the APM app related to transaction group limit reached (`_other`) or too many transaction names.
For transaction group limit reached, you could configure the value of `aggregation.transaction.max_groups` in {apm-guide-ref}/transaction-metrics.html#transactions-max_groups[APM Server].
For too many transaction names, you could use the query bar to narrow down your results.
