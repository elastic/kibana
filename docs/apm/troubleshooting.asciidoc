...
`The transaction group limit in APM Server has been reached. Excess groups have been captured in **_other**. Please see the {apm-guide-ref}/transaction-metrics.html#transactions-max_groups[APM Server docs] for **aggregation.transaction.max_groups** to increase this limit`

You will see this warning if an agent is creating too many transaction groups. This could indicate incorrect instrumentation which will have to be fixed in your application. Alternatively you can increase the limit in {apm-guide-ref}/transaction-metrics.html#transactions-max_groups[APM Server docs].

`The transaction group limit in Kibana has been reached. Excess groups have been omitted. Try narrowing down your results using the query bar.`
