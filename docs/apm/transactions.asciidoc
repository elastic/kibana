...
This graph is an easy way to visualize how much time each span is taking within your application.
The time a transaction took to complete is also recorded and displayed on the chart under the "app" label.
"App" indicates there was something happening within the application, but we're not sure what.
This could be a sign that the agent does not have auto-instrumentation for whatever was happening during that time.

It's important to note that if you have asynchronous spans, the total time of all spans might exceed 100%.
