...
The Kibana Query bar functionality known from Discover has now been added to APM. This enables you to pass advanced queries on your data to filter on particular pieces of information that you're interested in. It comes with a handy autocomplete that helps find the fields and even provides suggestions to data they include. The query bar is available on Service, Transaction and Errors views and any input will persist through those views.

Please see the {kibana-ref}/kuery-query.html[Kibana Query Language Enhancements] page to learn more about the capabilities within Kibana Query.
