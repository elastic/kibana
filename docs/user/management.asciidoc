...
| <<xpack-ml-anomalies, Machine Learning Jobs>>
| Create jobs that model the normal behavior of your time series data &mdash;
learning trends, periodicity, and more &mdash; so you can detect anomalies,
...
the actions that a user with the role can perform, both on a cluster and an index level.
