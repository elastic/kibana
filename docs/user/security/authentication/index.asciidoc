...
If using username and password credentials isn't desired or feasible then you can create a dedicated <<api-keys, API key>> for the anonymous service account. In this case your `kibana.yml` can look like this:
...
The configuration snippet above uses an API key string that is the result of base64-encoding of `id` and `api_key` fields returned from the {es} API joined by a colon. You can also specify these fields separately and {kib} will do concatenation and base64-encoding for you:
...
Here is how your `kibana.yml` can look like if you want to use {es} anonymous access to impersonate anonymous users in {kib}:
...
<1> The `elasticsearch_anonymous_user` is a special constant that indicates you wish to use {es}'s anonymous user.
