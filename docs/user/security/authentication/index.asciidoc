...
TIP: Login Selector screen can also be explicitly disabled or enabled with `xpack.security.authc.selector.enabled` setting. 
...
It's also possible to configure multiple SAML authentication providers at the same time. In this case users will need to choose which provider to use for login at the Login Selector screen:
...
It's also possible to configure multiple OpenID Connect authentication providers at the same time. In this case users will need to choose which provider to use for login at the Login Selector screen:
