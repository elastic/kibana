...
Enable OIDC authentication by specifying which OIDC realm in {es} should be used:
...
If you want to use Third Party initiated SSO, configure your OpenID Provider to use `/api/security/oidc/initiate_login` as `Initiate Login URI`.
