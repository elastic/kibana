...
and users. You can also configure <<elasticsearch-mutual-tls,mutual TLS>>
between {kib} and {es} where requests are authenticated by both applications.

NOTE: With {security-features} enabled, if you load a {kib} dashboard that
accesses data in an index that you are not authorized to view, you get an error
that indicates the index does not exist. The {security-features} do not
currently provide a way to control which users can load which dashboards.

[discrete]
[[security-configure-settings]]
=== Configure security settings

You can configure the following security settings to provide additional
encryption.
