...
The {es} API key service is automatically enabled. For instructions on disabling it, see {ref}/security-settings.html#api-key-service-settings[API key service settings].

If the {es} API key service is disabled, you cannot create or manage API keys, and you will get an error message when trying to use API keys in {kib}.
