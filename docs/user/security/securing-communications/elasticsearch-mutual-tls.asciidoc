...
If {kib} has `elasticsearch.username` and `elasticsearch.password` configured,
it attempts to use these credentials to authenticate to {es} via the
{ref}/native-realm.html[native realm]. However, {kib} also supports mutual TLS
authentication with {es} via a
...
When you {ref}/configuring-stack-security.html[start the {stack} with security enabled],
password authentication for {kib} is configured and TLS encryption is enabled
between {kib} and {es} automatically. For steps to manually set up TLS
encryption, refer to
{ref}/security-basic-setup-https.html#encrypt-kibana-elasticsearch[encrypting traffic between {kib} and {es}].
