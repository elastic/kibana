...
The `manage_security` {ref}/security-privileges.htmlprivileges-list-cluster[cluster privilege] is required to access role management.

[[adding_cluster_privileges]]
==== Adding cluster privileges

Cluster privileges grant access to monitoring and management features within {es}. They also enable certain <<management, Stack Management>> capabilities within {kib}.

Refer to {ref}/security-privileges.htmlprivileges-list-cluster[cluster privileges] for a complete description of available options.

[[adding_index_privileges]]
==== Adding index privileges

Each role can grant access to multiple data indices, and each index can have a different set of privileges.
We recommend granting the `read` and `view_index_metadata` privileges to each index that you expect your users to work with inside of {kib}.

Refer to {ref}/security-privileges.htmlprivileges-list-indices[index privileges] for a complete description of available options.

Document-level and field-level security affords you even more granularity when it comes to granting access to your data.
With document-level security ("DLS"), you can write an {es} query to describe which documents this role should grant access to.
...
===== Example 1: Grant access to indices matching the `filebeat-*` pattern

. Enter `filebeat-*` in the **Index** field under the **Index Privileges** section.
. Enter `read` and `view_index_metadata` in the **Privileges** field under the **Index Privileges** section.

[role="screenshot"]
image::user/security/images/create-role-index-example.png[Create role with index privileges]

[[index_privilege_dls_example]]
===== Example 2: Grant read access to specific documents matching the `filebeat-*` pattern

. Enter `filebeat-*` in the **Indices** field under the **Index Privileges** section.
. Enter `read` and `view_index_metadata` in the **Privileges** field under the **Index Privileges** section.
. Select the **Grant read privileges to selected documents** option.
. Enter an {es} query describing the documents you wish to grant access to. In this example, we are writing a query which allows access to documents that have a `category` field equal to `click`:
+
[source,sh]
--------------------------------------------------
{
...
--------------------------------------------------
+
NOTE: {kib} automatically surrounds your DLS query with a `query` block, so it is not necessary to provide your own.
