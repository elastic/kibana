...
To assign {kib} privileges to the role, click **Add {kib} privilege** in the Kibana section.
...
[role="screenshot"]
image::user/security/images/create-space-privilege.png[Add Kibana privilege]
...
Using the same role, it’s possible to assign different privileges to different spaces. After you’ve added space privileges, click **Add {kib} privilege**. If you’ve already added privileges for either *** Global (all spaces)** or an individual space, you will not be able to select these in the **Spaces** selection control.
