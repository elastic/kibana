...
With document-level security ("DLS"), you can write an {es} query to describe which documents your users can access.
...
. Enter an {es} query describing the documents for which you want to grant access. In this example, we are writing a query which allows access to documents that have a `category` field equal to `click`:
+
[source,sh]
--------------------------------------------------
{
