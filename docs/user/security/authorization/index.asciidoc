...
===== Example: Grant read access to specific documents that match the `filebeat-*` pattern

. Enter `filebeat-*` in the **Indices** field under the **Index Privileges** section.
. Enter `read` and `view_index_metadata` in the **Privileges** field under the **Index Privileges** section.
. Select **Grant read privileges to selected documents**.
. Enter an {es} query that describes the documents you want to grant access to. This example writes a query that allows access to documents that have a `category` field equal to `click`:
+
[source,sh]
--------------------------------------------------
{
