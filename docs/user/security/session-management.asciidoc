...
To manage user sessions programmatically, {kib} exposes <<session-management-api, session management APIs>>. For more settings, see <<security-settings-kb.html#security-session-and-cookie-settings,session and cookie settings>>.
