...
To check the plugin status, go to the status endpoint. For example, `localhost:5601/status`. For JSON-formatted status details, use the `localhost:5601/api/status` API endpoint.
