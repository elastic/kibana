...
* *Auto fit rows to content* &mdash; Wraps long text strings and displays full images in table cells.
...
To view and download the data included in the visualization and the requests that collected the data. When you download visualization panels with multiple layers, each layer produces a CSV file, and the file names contain the visualization and layer index names.
