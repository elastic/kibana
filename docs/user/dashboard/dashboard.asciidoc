...
| Use one of the editors to create visualizations of your data. The capabilities for each editor vary to support all levels of data analysts.
...
[float]
[[download-csv]]
== View and download panel data

View and download panel data, and view the requests that collected the data. When you download visualization panels with multiple layers, each layer produces a CSV file, and the file names contain the visualization and layer index names.

. Open the panel menu, then select *Inspect*.

. Open the *View* dropdown, then click *Data*.

. To download the data, click *Download CSV*, then select a format type.
