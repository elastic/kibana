...
These additional interactions allow users to open *Discover* from the dashboard. Administrators can choose
to enable:

<<settings-explore-data-in-context,`xpack.discoverEnhanced.actions.exploreDataInContextMenu.enabled`>>::
  Adds an action to each panel's context menu. This action does not carry the filters from your dashboard to
  *Discover*.

<<settings-explore-data-in-chart,`xpack.discoverEnhanced.actions.exploreDataInChart.enabled`>>::
  Adds an action to each series in a panel.
