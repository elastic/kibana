...
To add panels to the dashboard, use one of the editors to create the panel, 
add a panel from the *Visualize Library*, a table of live streaming logs, or the results from a machine learning anomaly detection job.
...
+
When panels are saved in the *Visualize Library*, the panel displays image:dashboard/images/visualize-library-icon.png[Visualize Library icon] in the header.
...
. To unlink the panel from the *Visualize Library*, open the panel menu, then click *More > Unlink from library*.
...
* *Sync color pallettes across panels* &mdash; Specifies the color pallette that is applied to all panels.
...
Add filters to a dashboard, or pin filters to multiple places in {kib}. To add filters, use a basic editor or an advanced JSON editor for the {es} {ref}/query-dsl.html[query DSL]. 
