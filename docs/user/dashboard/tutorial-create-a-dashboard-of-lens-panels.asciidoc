...
If you don't, drag and drop the field onto the workspace. Skip this step if you are
...
. From the *Available fields* list, drag and drop `clientip` onto the workspace.
...
If you drag and drop a numeric field onto the workspace, *Lens* adds the default
time field from the index pattern. When the *Date histogram* function is used, you can
replace the time field by dragging and dropping the field onto the workspace.

To visualize the `bytes` field over time without choosing a visualization type or function:

. From the *Available fields* list, drag and drop `bytes` onto the workspace to have *Lens* automatically
...
When you drag and drop a text or IP address field onto the workspace,
...
. From the *Available fields* list, drag and drop `clientip` onto the *Vertical axis* field in the editor. 
+
*Lens* automatically chooses the *Unique count* function. If you drag and drop `clientip` onto the workspace, *Lens* adds the field to the incorrect axis.

. Drag and drop `request.keyword` onto the workspace. *Lens* adds *Top values of request.keyword*
...
. From the *Available fields* list, drag and drop *geo.src* onto the workspace.
