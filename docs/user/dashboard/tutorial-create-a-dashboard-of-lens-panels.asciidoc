...
*Lens* will add the default time field if you drag a numeric field into the main panel.
If your data has multiple time fields, drag and drop your
specific time field to replace the default.

To see bytes over time, drag and drop *bytes* from the field list to the visualization builder. 
...
=== View the distribution of frequent values as a percentage

The *top values* function finds the most frequent values of a field and ranks them.
The values must be exact matches, so the terms `win 10` and `win xp` are distinct.
...
. Open *Lens* and select a proportion chart like *Donut* from the *Chart Type* dropdown.
...
. From the *Chart Type* dropdown, select *Bar vertical percentage*.
