...
NOTE: VEGA supports the `interval` paramater although is not supported by {es} since v8.0. It is recommended though to use the `fixed_interval` or `calendar_interval` instead.
