...
| *Event*
| 
| Variables that depend on user interaction. These variables are dynamically extracted from the interaction context when the drilldown is executed. Please note that in *preview* these variables are replaced by <<values-in-preview, dummy values>>.
