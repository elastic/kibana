...
[%collapsible]
====
You can inspect the requests sent by the visualization to Elasticsearch by using the Inspector. It can be accessed within the editor or in the dashboard.
====
...
[%collapsible]
====
Press the `shift` key and click on the item in the chart legend to unselect all the other series.
====
