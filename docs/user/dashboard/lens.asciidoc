...
. Click *Event*, or drag a field to the *Add an annotation* field in the *Annotations* layer pane.
...
. To add a field to the tooltip, click *Add field*, then select the field you want to display.

. To close, click *X*.

. To use global filters in the annotation, click image:dashboard/images/lens_layerActions_8.5.0.png[Actions menu for the annotations layer] in the *Annotations* layer pane, then select *Keep global filters* from the dropdown.
...
You can use the <<semi-structured-search, query bar>> to focus on a data set for the entire visualization, or use the filter options from the layer pane or legend.
