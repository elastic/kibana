...
[%collapsible]
====
Using the Inspector within the editor or in the dashboard it is possible to select the specific request sent by the visualization to Elasticsearch.
====

[discrete]
[[is-it-possible-to-isolate-a-single-series-in-a-chart]]
.*Is it possible to isolate a single series in a chart?*
[%collapsible]
====
Using the `shift` keyboard key and clicking on the item to isolate in the chart legend will unselect all the other series.
====