...
To configure the visualization, use the editor. For each visualization type, the editor options are different. 

For area, bar, and line charts, you can add multiple layers. To display multiple index patterns and visualization types, you can specify an index pattern and visualization type for each layer. Each layer is made of dimensions that include the field you want to visualize, the bucket and metrics aggregation functions, and the display options. Dimensions are grouped by *Vertical axis*, *Hortizontal axis*, and *Break down by*. The *Vertical axis* and *Hortizontal axis* groups support multiple dimensions.

All other visualization types support a single set of dimensions. The dimension groups are different for each visualization type, but also include the field, aggregation functions, and display options.
...
. Drag and drop the fields you want to visualize on to the visualization builder. 
...
To configure the visualization components, use the dropdown menus. The visualization components include visual options, data labels, legend, and axis. The dropdown menus differ depending on the visualization type.

[role="screenshot"]
image:images/lens_dropdown_menus.gif[Dropdown menus in Lens for configuring the visualization components, width=50%]

For example, you can add data labels to a subset of visualizations:
