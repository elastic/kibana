...
* Add mappings between terms and colors.
...
. To change the display options, click a *Metric* field in the layer pane, then configure the following options:
...
[float]
[[create-term-to-color-mapping]]
==== Create term to color mapping

preview::[]

Visualizations in Lens now support a color to term mapping system. These color mappings provide a visual guide for understanding the data presented in our visualizations. The visualization types that use this feature are bar charts, line and area graphs, and proportion charts. The feature is enabled by default and a maximum of 10 colors can be mapped to key terms. 

. from the *Visualize Library* click *Create visualization* and select *Lens*.
. Drag the fields you want to visualize to the workspace. 
. In the layer pane, underneath *Breakdown*, click the field name.
. In the *Breakdown* pane, under *Appearance*, click on the color palette labelled *Color mapping*.
. To edit the default color mapping, turn off the *Auto assign* slider. 
+
[role="screenshot"]
image::images/lens-color-mapping.png[A screenshot of the new Lens color mapping feature]
+
. Once you are happy with your edits, click *Back* and then *Close*.
