...
At the moment it is not possible to change the scale of the Y axis (for instance, making it start from another value than 0, or use a logarithmic scale.) in *Lens*.

[float]
[[is-it-possible-to-have-pagination-for-datatable]]
===== Is it possible to have pagination in the datatable visualization?

At the moment it is not possible to have pagination on a *Lens* datatable. However the <<types-of-visualizations,aggregation based data table>> does support this feature.
