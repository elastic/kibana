...
[float]
==== Tutorial: Create charts with Vega and Vega-lite
...
To create the stacked area chart, edit the Vega-Lite spec. The Vega-Lite spec uses https://hjson.github.io/[HJSON], which is similar to JSON, but optimized for editing. 
...
When you are finished with the tutorial, you'll have a stacked area chart that displays the sales data.
...
Add a data field from the *kibana_sample_data_ecommerce* index. 

. In the *Vega-Lite* spec, replace the following:
...
[float]
===== Add the encoding block

In the *Vega-Lite* spec, add an `encoding` block:
...
The stacked area chart appears. 

[float]
===== Add hover states and tooltips

With the *Vega-Lite* spec, you can add hover states and tooltips with a `selection` block.
...
Add a valid {es} search query with the `data` block, then click *Update*:
...
Show the clickable points to filter for a specific date.
