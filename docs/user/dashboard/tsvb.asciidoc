...
Time series::
  The Y axis defaults to showing the full range of data, including zero. To scale the axis from
  the minimum to maximum values of the data automatically, go to *Series > Options > Fill* and set *Fill to 0*.
  Annotations can be added to the X axis from based on timestamped documents in a separate {es} index.
