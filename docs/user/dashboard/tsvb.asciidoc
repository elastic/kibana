...
IMPORTANT: Creating *TSVB* visualizations with an {es} index string is deprecated, disabled by default and will be removed in a future release. To enable this feature, contact your administrator, or go to <<advanced-options, Advanced Settings>>, and set `metrics:allowStringIndices` to `true`.
