...
To analyze multiple visualization types, create an area chart that displays the average order prices, then overlay with a line chart that displays the number of customers. 
...
By default, the visualization editors displays time series data with stacked chart visualizations, which shows how the different document sets change over time. 
...
+
To use custom time shifts, enter the time value and incrememnt, then press Enter. For example, enter *1w* to use the *1 week ago* time shift.
+
[role="screenshot"]
...
Use tables when you want to display field values.
