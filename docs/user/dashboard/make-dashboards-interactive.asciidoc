...
.. To select multiple options in the dropdown, select *Allow multiple selections in dropdown*.

.. To exclude options from appearing on the dashboard, select *Allow selections to be excluded*.

.. To create an exists query, select *Allow exists query*.

.. To populate the entire list of options, even when the Options list takes longer to populate than expected, select *Ignore timeout for results*.
