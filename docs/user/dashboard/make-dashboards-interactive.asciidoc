...
+
[role="screenshot"]
image::images/dashboard_controlsOptionsList_8.7.0.png[Options list control for the `machine.os.keyword` field with the `ios` option selected]
...
. On the *Edit control* flyout, change the options.

. If you are editing an *Options list*, specify the additional settings:
...
* To allow the options to be sorted, select *Allow dynamic sorting of suggestions*, then select the default sort options. 
