...
Each panel type in {kib} has a built-in interaction which applies a filter to the dashboard, for example by dragging a time range or clicking a pie slice. Drilldowns let you customize this behavior while keeping the context of the interaction. For example, if you add a drilldown to a Lens panel you navigate from a summary dashboard to a dashboard with a filter for the specific host name you filtered. You could also navigate to an external website with the specific host name as a parameter. You can create multiple drilldowns for a single panel.
