...
Panels have built-in interactive capabilties that apply filters to the dashboard data. For example, when you drag a time range or click a pie slice, a filter for the time range or pie slice is applied. Each dashboard panel can support multiple drilldowns. The drilldowns you create apply only to the panel that you created the drilldown on, and are not shared across panels.

Drilldowns let you customize the interactive behavior while keeping the context of the interaction. {kib} supports two types of drilldowns:
