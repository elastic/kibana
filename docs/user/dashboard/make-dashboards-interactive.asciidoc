...
You can add interactions to panels that allow you to open *Discover* and explore the panel data. To use the *Discover* interactions, the panel must use only one {data source}, and you must enable <<settings-explore-data-in-context,`xpack.discoverEnhanced.actions.`>> and <<settings-explore-data-in-chart,`xpack.discoverEnhanced.actions.exploreDataInChart.enabled`>> in kibana.yml. 
...
Panel interactions:: 
Opens the panel data in *Discover*, including the dashboard-level filters, but not the panel-level filters.
+
To use panel interactions, open the panel menu, then click one of the following options: 

* *Explore underlying data* &mdash; Opens the panel data in *Discover*.

* *Open in Discover* &mdash; Opens the <<save-your-search,saved search table>> in *Discover*.

Panel series data  interactions::
Opens the panel series data in *Discover*.
+
