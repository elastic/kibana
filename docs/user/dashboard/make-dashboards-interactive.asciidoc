...
For example, when you want to further explore the data in a *Lens* visualization, you can create a drilldown that opens the visualization data in *Discover*.

[role="screenshot"]
image:images/dashboard_discoverDrilldown_8.3.gif[Drilldown on bar vertical stacked chart that navigates to Discover]

Discover drilldowns are supported only by *Lens* panels. 
...
. To take the time range, filters, and other parameters with you, select *Apply filter*.
