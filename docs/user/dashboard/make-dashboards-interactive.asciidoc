...
Panels have built-in interactive capabilities that apply filters to the dashboard data. For example, when you drag a time range or click a pie slice, a filter for the time range or pie slice is applied. Drilldowns let you customize the interactive behavior while keeping the context of the interaction. For example, you can create a drilldown for a *Lens* panel that navigates you from a summary dashboard to a dashboard with a filter for a specific host name. You can also create drilldowns that navigate you to an external website with the specific host name as a parameter. Each dashboard panel can support multiple drilldowns. The drilldowns you create apply only to the panel that you created the drilldown on, and are not shared across panels.
