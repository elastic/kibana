...
With *Timelion*, you manually configure the <<aggregation-reference,aggregations>> and data fields, then format the fields to best display the data.
...
[float]
==== Tutorial: Create visualization with Timelion 
...
. To set up Metricbeat, go to link:{metricbeat-ref}/current/metricbeat-installation-configuration.html[Metricbeat quick start: installation and configuration]
