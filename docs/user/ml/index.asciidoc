...
You can attach selected charts to a dashboard or a case by using the context 
menu. You can either preserve the applied time range or use the time bound from 
the page date picker, configure a specific partition, or set the maximum number 
of top change points to plot. You can also add or edit change point charts 
directly from the **Dashboard** app.