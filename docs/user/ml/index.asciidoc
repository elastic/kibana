...
10,000, sorted by document count, are analyzed. You can select multiple metric 
fields and split fields to identify change points.

When a change point is detected, a row displays basic information including the 
time, a preview chart, the type of change point, p-value, field name, and value. 
You can further examine the selected change point in a detailed view. A chart 
...
displays the type of change point, its value, and the time of occurrence. The 
corresponding `p-value` indicates the magnitude of the change; lower values 
indicate more significant changes. You can use the change point type selector to 
filter the results by specific types of change points.
