...
spikes by using the investigative workflow view. Examine the histogram chart of 
the log rates for a given {data-source}, and find the reason behind a particular 
change possibly in millions of log events across multiple fields and values.
...
Sliders in the chart show the baseline values and the deviation in the analyzed 
data. You can move the sliders to redefine both the baseline and the deviation 
and rerun the analysis with the modified values.
