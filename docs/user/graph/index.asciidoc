...
+
If you're new to {kib}, and don't yet have any data, follow the links to add sample data.
...
. Add more fields, or click an existing field filter to edit, disable or deselect it.
+
[role="screenshot"]
...
particular pages from different locations, search for the 200
...
+
[role="screenshot"]
image::user/graph/images/graph-control-bar.png["Graph control bar", width=50%]

. Use the toolbar in the *Selections* pane to display additional connections:
+
