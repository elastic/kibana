...
An alert specifies a background task that runs on the {kib} server to check for specfic conditions. It consists of three main parts: 
...
See <<alert-types>> for the types of alerts provided by Kibana and how they express their conditions.
...
See <<action-types>> for details on the types of actions provided by Kibana. 

[float]
[[alerting-concepts-alert-instances]]
=== Alert instances

When checking for a condition, an alert might identify multiple occurences of the condition. {kib} tracks each of these *alert instances* separately and takes action per instance. 
...
* *Alert instance*: state tracked by {kib} for every occurence of a detected condition. Actions as well as controls like muting and re-notification are controlled at the instance level. 
...
* Actions are linked to *alert instances* in {kib} alerting. Actions are fired for each occurence of a detected condition, rather than for the entire alert. 
...
Pre-packaged *alert types* simplify setup, hide the details complex domain-specific detections, while providing a consistent interface across Kibana.
...
* TLS must be configured for communication <<configuring-tls-kib-es, between Elasticsearch and Kibana>>. {kib} alerting uses <<api-keys, API keys>> to secure background alert checks and actions, and API keys require {ref}/configuring-tls.html#tls-http[TLS on the HTTP interface].
