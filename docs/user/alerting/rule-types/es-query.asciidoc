...
The rule type checks for duplication of document matches across rule executions. 
If you configure the rule with a schedule interval smaller than the time window, 
and a document matches a query in multiple rule executions, it is alerted on 
only once. The rule uses the timestamp of the matches for checking duplications.
