...
[role="screenshot"]
image::user/alerting/images/rule-types-es-query-conditions-2.png[Six clauses define the condition to detect]
...
detection.
Exclude the hits from previous run:: Defines if the documents that have already been
detected by the previous rule run should be excluded (to avoid duplication) or not.
