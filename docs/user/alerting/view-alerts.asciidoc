...
In *{stack-manage-app}* > *Alerts*, you can filter the list by alert status or time period.
By default, the list contains all the alerts that you have authority to view except those associated with Security rules.
To view alerts for Security rules, click the query menu and select *Security rule types*.

NOTE: To view or manage alerts for Security rules, you must have the appropriate {kib} feature privileges and index privileges.
Refer to {security-guide}/detections-permissions-section.html#enable-detections-ui[Enable and access detections].
