...
Actions are {kib} services or integrations with third-party systems that run as background tasks on the {kib} server when rule conditions are met. Actions use connectors, which provide a central place to store connection information for reusable services and third party integrations. {kib} provides the following types of actions and connectors:
