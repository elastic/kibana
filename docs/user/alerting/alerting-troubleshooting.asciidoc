...
| The other {kib} instance might be trying to run the rule using a different encryption key than what the rule was created with. Ensure the encryption keys among all the {kib} instances are the same, and setting <<encryption-key-rotation, decryption only keys>> for previously used encryption keys.
