...
The following query can help you identify rules that are taking a long time to execute and may be impacting the overall health of your deployment.
...
<2> Execution durations are stored as nanoseconds. This interval buckets the duration into 10 second intervals. Update this value to change the granularity of the buckets.
<3> This retrieves the top 10 rule ids for this duration interval. Update this value to retrieve more rule ids.
...
<2> A single rule with id `41893910-6bca-11eb-9e0d-85d233e3ee35` has taken between 30 and 40 seconds to execute.

We can use the <<get-rule-api,Get Rule API>> to retrieve additional information about rules that are taking a long time to execute.