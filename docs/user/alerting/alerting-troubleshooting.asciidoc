...
--------------------------------------------------
Some of the resources, such as saved objects and API keys, may no longer be available or valid, yielding error messages about those missing resources. If {kib} logs are not clearly describing the problem, best to ask someone from the alerting team to determine whether errors/warnings are of which class.
