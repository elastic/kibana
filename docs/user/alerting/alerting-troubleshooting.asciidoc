...
[float]
[[alerting-stack-monitoring]]
=== Use Stack Monitoring

Starting in 8.4, you can use Stack Monitoring to gain insight into the performance of your rules and actions. This integration requires monitoring through Metricbeat (https://www.elastic.co/guide/en/kibana/current/monitoring-metricbeat.html) and requires no additional setup - simply ensure you are monitoring Kibana through Metricbeat and rule and action metrics will show up in the Stack Monitoring UI.
