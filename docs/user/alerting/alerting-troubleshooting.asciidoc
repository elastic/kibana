...
7.10+
<<testing-connectors,Test connector>> functionality in the UI.

7.11+
Better webhook error messages.
Better overall debug logging for actions/connectors.
Task manager <<task-manager-diagnosing-root-cause,diagnostics endpoint>>.
...
Investigating the underlying Task can help you gauge whether the problem you’re seeing is rooted in the Alert not running at all, whether it’s running and failing or whether it is running, but exhibiting behaviour that is different than what was expected (at which point you should focus on the Rule itself, rather than the Task).
