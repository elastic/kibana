...
| Ensure any previous encryption key is part of the <<encryption-key-rotation, decryption only key>>.

| If another {kib} instance with a different encryption key connects to the cluster.
| It might be trying to run the rule using a different encryption key than what the rule was created with. Ensure the encryption keys between all the {kib} instances are the same is necessary, and setting <<encryption-key-rotation, decryption only keys>> for previously used encryption keys.

| If other scenarios don't apply.
| Generate a new API key for the rule by disabling/enabling the rule.
