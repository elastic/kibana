...
Currently {kib} provides two stack alert: the <<alert-type-index-threshold>> type and the <<alert-type-es-query>> type.
...
Threshold:: This clause defines a threshold value and a comparison operator  (one of `is above`, `is above or equals`, `is below`, `is below or equals`, or `is between`). The number of documents that match the specified query is compared to this threshold. 
...
matched the query will be displayed.

[role="screenshot"]
image::images/alert-types-es-query-valid.png[Test ES query returns number of matches when valid]

When your query is invalid:: An error message with reason will be shown in the case of invalid queries.
