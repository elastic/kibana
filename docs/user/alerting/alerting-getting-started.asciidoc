...
* To have emails with an automated footer with a link back to Kibana, add the <<server-publicBaseUrl, `server.publicBaseUrl`>> setting in the kibana.yml configuration file.
