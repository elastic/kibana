...
For example, an <<alert-types, index threshold rule type>> lets you specify the index to query, an aggregation field, and a time window, but the details of the underlying {es} query are hidden.

See <<alert-types>> for the types of rules provided by {kib} and how they express their conditions.
