...
Next you must specify when the action is to be run, by choosing an _action group_ under the **Run When** field. When an alert instance is detected as matching a condition it is marked as _Active_, assigned an action group and triggers the actions in that group.
