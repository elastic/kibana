...
* `xpack.actions.enabledActionTypes`: Set to an array of action types that are enabled. An * indicates all action types registered are enabled. The action types that {kib} provides are: .server-log, .slack, .email, .index, .pagerduty, .webhook. Default: [ {asterisk} ]
