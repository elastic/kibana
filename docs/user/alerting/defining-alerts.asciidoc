...
When an alert instance is detected as matching a condition it is marked as _Active_, assigned an action group and triggers the actions in that group.
When the condition is no longer detected for an _Active_ alert instance, it is assigned the _Recovered_ action group, triggering any actions assigned to that group.

**Run When** allows you to assign an action to an _action group_, ensuring you are notified about alert instances in that group by triggering the action in accordance with your **Notify every** setting.
