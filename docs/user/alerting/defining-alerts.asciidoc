...
**Run When** allows you to assign an action to an _action group_. This will trigger the action in accordance with your **Notify** setting.
...
[float]
==== Action variables
Using the https://mustache.github.io/[Mustache] template syntax `{{variable name}}`, you can pass alert values at the time a condition is detected to an action. The list of available variables can be accessed using the "add variable" button. While available variables differ by alert type, all alert types pass the following variables:
...
`kibanaBaseUrl`:: The configured `server.publicBaseUrl` value. If not configured, this will be empty.
