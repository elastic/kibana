...
Notify:: Defines how often to repeat actions while alerts are active. Options
include generating actions at each check interval, only when the alert status
changes, or at a custom action interval. 
+
--
