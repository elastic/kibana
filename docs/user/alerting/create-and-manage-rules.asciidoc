...
There are could be a multiple status values for the rule:

`active`:: The conditions for the rule have been met, and the associated actions should be invoked.
`ok`:: The conditions for the rule had previously been met, but no longer are being met. Changed to `recovered` in a 7.14 release.
