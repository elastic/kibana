...
The *Rules* UI provides a cross-app view of alerting. Different {kib} apps like {observability-guide}/create-alerts.html[*Observability*], {security-guide}/prebuilt-rules.html[*Security*],  geo-alerting, *Maps*>> and *Machine Learning* can offer their own rules. The *Rules* UI provides a central place to:
...
Each action type exposes different properties. For example, an email action allows you to set the recipients, the subject, and a message body in markdown format. See Action and connector types for details on the types of actions provided by {kib} and their properties.
