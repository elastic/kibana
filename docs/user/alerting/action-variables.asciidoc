...
`context`:: The context of the alert, which varies by rule type and typically contains multiple variables. 

[float]
[[rule-mustache-tips]]
=== Mustache tips

Rule actions pass variable values into alert notifications by using https://mustache.github.io/mustache.5.html[Mustache] template syntax (`{{variable name}}`).

In Mustache templates, all variables are HTML escaped by default.
Triple braces (`{{{variable name}}}`) indicate no escaping should be done at all.
Use this form with caution, since it could end up rendering the variable content such that the resulting parameter is invalid or formatted incorrectly. Some connectors automatically escape variable values in specific scenarios. For example:
...
For diagnostic or exploratory purposes, rule action variables whose values are objects (such as `rule.params` and `context`) can be referenced directly as variables.
// TBD: Is context an object (per above) or an array (per below) or a "list" (per mustache syntax)?
The resulting value will be a JSON representation of the object.
For example, if the message in an action includes `{{context}}`, it will expand to the JSON representation of all the variables and values provided by the rule type.

For situations where your rule response returns arrays of data, you can use the Mustache implicit iterator (`{{.}}`):
...
You can enhance the values contained in Mustache variables when the Mustache template is rendered by rendering objects as JSON or by using Mustache lambdas, as described in the subsequent sections.
...
The template references the following rule action variables:
...
//TBD: "Available as Mustache variables" is unclear.
