...
// TBD: Is context an "object" (per above), "array" (per example below), or "list"?
...
The following example makes use of an implicit iterator (`{{.}}`) and a section, which starts with a pound (`#`) and ends with a slash (`/`).
If your rule response returns arrays of data, you can use this syntax to loop through all the context items:
...
// TBD: Are these the only lambdas that we support?
...
//TBD: "Available as Mustache variables" is unclear. How does someone know which are available?
