...
Below is a Mustache template for an email action sent from an elasticsearch query rule.
...
- `# {{context.title}}` will render the value of the `context.title` variable as a level 1 heading.
- `{{#FormatDate}} ... {{/FormatDate}}` will render the value of the `date` variable as a formatted date in the America/New_York time zone.
- `**documents** _[view in Discover]({{{context.link}}})_` show examples of `**bold**`, `_italic_`, and `[text](url)` links
- The last section is a table with three columns, with one row per element in the `context.hits` array.  From each of those elements, we access the `provider`, `action`, and `duration` fields of the `_source.event` object.  The `duration` field is rendered as a number of seconds, rounded to the nearest second - it's stored as nanoseconds so needs to be divided by a billion to yield seconds.  The duration field is optional, so we use a `{{#duration}} ... {{/duration}}` section to render the duration if it's present, and `-n/a-` if it's not present.

When rendered into Markdown, and then HTML, and viewed in an email client, it will look like this:
...
Some of the relevant data available as mustache variables:
