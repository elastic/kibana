...
Numbers can be formatted with any supported locales and `Intl.NumberFormat` options.

To use it, surround the date and formatting parameters with `{{#FormatNumber}}...{{/FormatNumber}}`.

The format of the text passed to the lambda is: `<number>; <locales>; <options>`, where semicolons (`;`) separate each parameter.
The `<number>` parameter is required; the `<locales>` and `<options>` parameters are optional, but the semicolons must be provided - the values may be empty strings.
...
The default locale is `en-US` and the default options are `{}`.

[source]
----
    {{#FormatNumber}} {{{numberProperty}}};; {{/FormatNumber}}
    {{#FormatNumber}} {{{numberProperty}}} ; en-US ; {{/FormatNumber}}
    {{#FormatNumber}} {{{numberProperty}}} ; de-DE ; style: currency, currency: EUR  {{/FormatNumber}}
----
