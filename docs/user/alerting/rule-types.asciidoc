...
| Aggregate field values from documents using {es} queries, compares them to threshold values, and schedules actions to run when the thresholds are met.

| <<rule-type-es-query>>
| Run a user-configured {es} query, compares the number of matches to a configured threshold, and schedules actions to run when the threshold condition is met.
