...
{ml-docs}/ml-configuring-alerts.html[{ml-cap} rules] run scheduled checks on an {anomaly-job} to detect anomalies with certain conditions. If an anomaly meets the conditions, an alert is created and the associated action is triggered.

[float]
[[security-rules]]
=== Security rules

{security-guide}/prebuilt-rules.html[Security rules] detect suspicious source events with pre-built or custom rules and create alerts when a rule’s conditions are met.
