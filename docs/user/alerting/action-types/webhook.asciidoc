...
Note that mustache template variables are escaped so that the final JSON will be valid JSON (example: escaping double quote characters) For more information on mustache template variables, refer to
