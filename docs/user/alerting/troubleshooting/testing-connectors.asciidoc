...
By using Kibana Management UI you can test you newly created Connector by getting to the Test tab of Connector Edit flyout, by clicking "Save & test" button on Create flyout:
[role="screenshot"]
image::user/alerting/images/connector-save-and-test.png[Rule management page with the errors banner]

or by direct opnening the proper connector Edit flyout:
[role="screenshot"]
...
[float]
==== Troubleshooting Connectors with `kbn-action` tool
