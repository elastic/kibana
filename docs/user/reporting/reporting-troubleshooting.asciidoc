...
Reporting comes with a built-in utility to try and find common issues automatically. When Kibana is running, navigate to the Report Listing page, and click the "Run reporting diagnostics..." button. This will open up a diagnostic tool that allows you to check various parts of reportings technology stack, and see where problems might be.
