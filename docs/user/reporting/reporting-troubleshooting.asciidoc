...
If the system is missing dependencies, then Reporting will fail in a non-deterministic way. Kibana runs a self-test at server startup, and
if it encouters errors they will logged in the Kibana console. Unfortunately, the error message of this test won't include very much
information about why Chromium failed to run. For example, the most common error message here is `Error: connect ECONNREFUSED`. It indicates
that when Kibana tried to connect to the Chromium process, it couldn't.

To troubleshoot the problem you can start the Kibana server with environment variables that tell Chromium to print verbose logs. See the
<<_puppeteer_debug_logs, Puppeteer debug method>> for more information.
...
=== System requirements
In Elastic cloud, the Kibana instances that most configurations provide by default is for 1GB of RAM for the instance. That is enough for
Kibana Reporting when the dashboard or visualization is relatively simple: for example, a single pie chart visualization or a dashboard with
a few visualizations. However, certain visualization types incur more load than others, for example TSVB panel incurs a lot of network
requests to render.

If the Kibana instance doesn't have enough memory to run the report, the report will fail with an error such as `Error: Page crashed!` in
the message. In those cases, try increasing the memory for the Kibana instance to 2GB.
