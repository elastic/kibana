...
1. Click the "info" icon next to the report.
2. In the footer of the side panel, click the "Actions" button.
3. Click the "Inspect query in Console" action that appears in the actions menu.
4. This will open the *Console* application, pre-filled with the queries used to generate the CSV export.

[role="screenshot"]
image::user/reporting/images/inspect-query-from-csv-export.gif["Inspecting the query used for CSV export"]
