...
To automatically generate PDF and CSV reports, generate a POST URL, then submit an HTTP `POST` request using {watcher} or a script. In {stack}, you can also use {kib} to generate reports on a recurring schedule and share them with a list of emails that you specify.
...
. In the toolbar, the **Export** icon, then **CSV**. In the export flyout, copy the POST URL.
...
. (Optional) To share generated reports outside of Kibana, enable **Send by email** and enter a list of email addresses. Recipients will receive emails with the generated reports attached and on the schedule that you specified.
+
