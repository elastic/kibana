...
Elastic is turning on the {kibana-ref}/document-explorer.html[Document Explorer] experience by default, and in preparation,
we call attention in the product to try it out. Document Explorer is faster, more robust,
and offers industry leading multi-row analysis. Try Document Explorer,
...
This helps you to determine the combinations that contribute to your metrics' performance.
...
visualization is now configurable. Keeping the default to a smaller size allows for maximize space for
labels. You can change the option in *Lens*.
