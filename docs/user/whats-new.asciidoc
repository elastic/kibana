...
[discrete]
==== {Data-sources} &mdash; use without saving

You can now explore your data in *Discover*, *Lens*, and *Maps*
...
The {kibana-ref}/access-agreement.html[access agreement]
...
You can do so either while creating a new Case or in an existing by using
...
{kib} users from all authentication realms are now searchable, if
they have accessed {kib} at least once. On first access,
