...
*ServiceNow additional field* - You can now create enriched ServiceNow tickets based on detected alerts with a more comprehensive structure that matches the ServiceNow ticket scheme.  A new JSON field is now available as part of the ServiceNow action and lets you define any field to be sent from Kibana Alerts to ServiceNow tickets. 
