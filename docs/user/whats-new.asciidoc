...
[float]
[[index-patterns-refresh]]
=== Index patterns automatically refresh

Updates to index patterns are now automatic, so you'll no see longer
the refresh icon in **Stack Management > Index Patterns**. When a new
field is added to the index, the index pattern field list is
updated the next time the index pattern is loaded, for example, when you load
the page or move between {kib} apps.
