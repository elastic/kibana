...
A new action is now supported to allow users to create cases automatically based on alerting rules when alerts are detected.
With this action users are able to automate alert aggregation which is a manual process today or managed by external services.The case action allows to aggregate alerts and group them by any alert field and time window. So as an example:  “All alerts that are detected by a certain alerting rule in a given time window (for example 7 days) with the same user will be assigned automatically to the same case”.
...
A new Alerts page is now available to manage alerts as part of the Stack Management menu. The new Alerts page allows users to filter by rule types and solution and get a unified view of all generated alerts within the space based on user access. 
...
With this Jira connector enhancement, users are allowed to create enriched Jira issues based on detected alerts with a more comprehensive structure that matches the Jira ticket scheme.  A new JSON field is now supported as part of the Jira action so users can define any field to be sent from Kibana Alerts to Jira tickets. 
