...
Creating runtime fields is now easier and more reliable. When a script context supports the scripting fields API, use the new shortcut for the field method, which replaces `field('fieldname').get(defaultValue)` with `$('fieldname', defaultValue)`.
...
from *Lens*, *TSVB*, *Timelion*, and *Aggregation-based* editors.
...
To help you create workpads with exactly the right behavior, use the new *Filters* on any element or object to view the applied object filters. 
...
To view the top combinations in *Lens*, combine multiple fields with drag and drop, which helps you to determine the combinations that contribute to your metric performance.
...
You can now take advantage of the {es} rare terms aggregations in *Lens* by viewing the top values by rarity, making it easy to ask "What's not normal here?"
...
most dashboards are created. Since waffles display the data in 1% blocks, they are best for low cardinality breakdowns, and are not recommended for small partitions. 
...
You can now enable users with view-only dashboard privileges to sort data tables on the fly, similar to the existing visualization editor behavior.
