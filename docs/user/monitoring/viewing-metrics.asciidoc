...
What do we recommend here for viewing data collected by Elastic Agent? Looks like the
monitoring_user role has read and read_cross_cluster on .monitoring-* and 
metricbeat-* indices, not the ones created by the agent.
