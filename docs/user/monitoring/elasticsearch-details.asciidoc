...
This is calculated by subtracting the delta of `follower_global_checkpoint` from the delta of 
`leader_max_seq_no` for each shard, and displaying the maximum value.
...
- **Ops synced**: The number of operations indexed into the follower index from the leader index 
in the selected time period. This is the sum of the delta between the maximum and minimum number of 
operations indexed across all of the shards in the follower index during the time period.
- **Error**: Any exceptions returned for the most recent document in the time period.

For more information on the properties used to calculate these metrics, refer to the {ref}/ccr-get-follow-stats.html[get follower stats API] documentation.
