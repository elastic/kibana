...
Start with the home page, which provides access to *Enterprise Search*,
*Observability*, *Security*, and *Analytics*. For more options, open the main menu.
...
You can also use *Graph* to explore relationships.

| *4*
| *Advanced analysis.*  With <<canvas, *Canvas*>>, you can display your data on a visually
compelling, pixel-perfect workpad and give your data
the “wow” factor for display on a big screen.
...
your daily workflow: email, Slack, PagerDuty, and ServiceNow, to name a few.
...
Use the search field in the global header to
