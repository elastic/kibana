...
. Drag the field into the document table, and **Discover** creates a column for the field.


. Find the `customer_first_name` and `customer_last_name` fields and drag
them into the document table. Your table should look similar to this:
+
[role="screenshot"]
