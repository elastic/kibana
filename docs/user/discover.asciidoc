...
[role="screenshot"]
image:images/discover.png[View of discover showing available fileds, histogram, and documents table.]
...
+
[role="screenshot"]
image:images/discover-filter-field.png[Fields list that displays the top five search results]

. Click image:images/add-icon.png[Add icon] to toggle the field into the document table.

. Find the `day of week` field and add it to your document table.

. To rearrange the table columns, hover the mouse over a
column header, and then use the move and sort actions in the dropdown menu.
+
[role="screenshot"]
...
Saving a search saves the query text, filters, and the
...
If your documents contain geo point fields
(image:images/geoip-icon.png[Geo point field icon, width=20px]), you can visualize them in **Maps**.
From the **Available fields** list, click the geo point field, and then click **Visualize**.
