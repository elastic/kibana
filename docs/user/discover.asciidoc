...
want to combine two fields and treat them as one? This is where runtime fields come into play.
...
. In *Create field*, enter `hello` for the name.

. Turn on *Set value*.

. Define the field with the Painless scripting language:
+
```ts
emit("Hello World!");
```
