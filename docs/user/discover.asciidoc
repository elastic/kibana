...
column header for move and sort controls.
...
. To view documents that occurred closest in time this event, click View surrounding documents.

. To view document data as a separate page, click View single document.
+
You can bookmark and share the link for direct access to a particular document.
...
. In the side navigation, click  `day_of_week` and then click **Visualize**.
+
...
. Customize the visualization and then save it  for use in a dashboard.
