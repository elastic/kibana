...
. Expand *Available fields* and click `manufacturer` to view its most popular values.
+
...
. Select *:* for equals some value and *US*, and then refresh the query.
...
. In the *Add filter* pop-up, set the field to *day_of_week* field, the operator to *is not*,
