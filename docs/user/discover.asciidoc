...
to define the data to work with. The current index pattern is in the upper left.
...
<<lucene-query, Lucene query syntax>>, you can switch to it from the KQL menu.
...
Now you want to to see the documents that occurred just before and after the
