...
**Discover** includes a table, also called the Document Explorer,
that shows all the documents that match your search.
By default, the Document Explorer includes columns for the time field and the document `_source`.
You’ll modify the Document Explorer to display your fields of interest.
...
. Click image:images/add-icon.png[Add icon] to toggle the field into the Document Explorer.

. Find the `customer_first_name` and `customer_last_name` fields and add
them to the Document Explorer. Your table should look similar to this:
+
[role="screenshot"]
image:images/document-table.png[Document Explorer with fields for manufacturer, customer_first_name, and customer_last_name]
...
image:images/chart-icon.png[icon button for opening Show/Hide chart menu, width=50%]
...
. In the fields list, search for the *hello* field, and then add it to the Document Explorer to view it's value.
+
[role="screenshot"]
image:images/hello-field.png[hello field in the Document Explorer]
...
```
. Remove `customer_first_name` and `customer_last_name` from the Document Explorer and add `customer` in their place.
+
[role="screenshot"]
...
. In the Document Explorer, expand any document.
+
[role="screenshot"]
...
click the ellipsis (...) in the *Actions* column for filters and other controls.

. To view documents that occurred before or after the event you are looking at, click
<<discover-view-surrounding-documents, Surrounding documents>>.
...
the Document Explorer, the sort order, and the {data-source}.
