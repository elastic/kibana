...
[WARNING]
====
Running more than one {kib} version against the same {es} index is unsupported.
...
set `xpack.security.cookieName` explicitly to the same value in the {kib} configuration
of each {kib} cluster.
...
Depending on the load balancing configuration, you might need to specify 
`xpack.reporting.kibanaServer.*` settings in case you want to use Reporting.
