...
[IMPORTANT]
==============================================
Task definitions are stored in the index specified by `xpack.task_manager.index`, which defaults to `.kibana_task_manager`.

You must have at least one replica of this index for production deployments. If you lose this index, all scheduled tasks are lost.
==============================================
...
* An {es} task index is polled for overdue tasks at 3-second intervals.  To change this interval, use the `xpack.task_manager.poll_interval` setting.
