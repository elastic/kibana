...
To generate reports, {kib} uses the Chromium web browser, which runs on the {kib} server in headless mode to load {kib} and capture the rendered {kib} visualizations as images. Chromium is an open-source project not related to Elastic, and is embedded into {kib} to work with minimal setup. The operating system that the {kib} server uses can require additional dependencies for Chromium.

[NOTE]
============
Chromium is not embedded into {kib} for the Darwin (Mac OS) architecture. When
running Kibana on Darwin, Reporting will download Chromium into proper area of
