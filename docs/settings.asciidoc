...
|Number of replicas the Kibana index should be setup to have when created in Elasticsearch. This setting is only
applied if no index with the name specified by `kibana_index` exists in Elasticsearch and a new index is created.
Changing this setting will have no effect on an existing Kibana index. Default: `kibana_index_replicas: 1`.
