...
... make changes, then update it with:
...
IMPORTANT: We recommend only changing the `settings` section as other changes are
...
Marvel relies on Elasticsearch's apability to automatically create new indices
