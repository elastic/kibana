...
| `kibana.diff` | Field-level diff of a saved object create, update, or delete. Only present when `xpack.security.audit.savedObjectDiff.enabled` is `true` and the saved object type is listed in `xpack.security.audit.savedObjectDiff.typesToInclude`. An object with `format` (`json_patch_extended`), `ops[]` (RFC 6902 `add`/`remove`/`replace` operations with `path`, `value`, and `oldValue`), and `noOps[]` (paths of unchanged attributes). Encrypted attribute values are replaced with `[redacted]`. Saved object types written by plugins that manage their own saved objects authorization and auditing (for example connectors, alerting rules, cases, and Fleet) never carry this field. With the `otel` appender, the whole `kibana.diff` object is serialized to a single JSON string attribute, because OTel attribute values cannot hold arrays of objects; parse it before reading `ops` or `noOps`.<br>Example: `{"format":"json_patch_extended","ops":[{"op":"replace","path":"/title","value":"New","oldValue":"Old"}],"noOps":[{"path":"/description"}]}` |
