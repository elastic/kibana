...
| `rule_alert_unsnooze` | `unknown` | User is unsnoozing an alert of a rule (per-alert conditional snooze). |
| | `success` | An alert was auto-unsnoozed because its conditions were met. |
| | `failure` | User is not authorized to unsnooze an alert. |
