...
Slack connectors support the following authentication methods:
...
Incoming Webhook URL {applies_to}`serverless:` {applies_to}`stack: ga 9.6`
...
:   Send a text or Block Kit message.
    ...
    - `blocks` (optional) {applies_to}`serverless:` {applies_to}`stack: ga 9.6`: Up to 50 structured Slack Block Kit blocks. Either `text` or `blocks` is required.
