...
Message
:   The message for the alert (required). {{jsm}} limits this field to 130 characters. {{kib}} truncates longer values, including values expanded from Mustache templates, instead of failing the action. Values longer than 15,000 characters are still rejected.
