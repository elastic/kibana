...
* **A missing result is data, not an error.** `getResult` returns `found: false` while a scan is still processing, and reports `deleted: true` for a scan that has been removed. `getScreenshot` and `getDom` return `found: false` when no artifact was stored. Check the `found` field rather than relying on an error.
...
* **Every action reports its rate-limit budget.** URLScan.io limits requests per minute, per hour, and per day, separately for each kind of action, and answers HTTP 429 when a window is exhausted. Each action returns a `rateLimit` block read from the response headers, and `getQuota` gives the full picture before a batch.
