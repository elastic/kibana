...
2. Click on your account name in the top right corner and select **Security Credentials** from the dropdown menu.
   - **note**: If you do not see the **Security Credentials** link, you may need to talk to your AWS administrator to get access.
...
8. To create a new access key, click **Create access key** and choose the appropriate use case (e.g., **Application running outside AWS**).
   - Download the `.csv` file or copy both the **Access Key ID** and **Secret Access Key** immediately - the secret key cannot be retrieved later.
9.  For the **AWS region**, select the region where your S3 buckets are located (e.g., `us-east-1`, `eu-west-1`). You can find the region in the AWS console URL or in the S3 bucket details.

:::caution
Store your Secret Access Key securely. It will only be displayed once during creation and cannot be recovered. If lost, you must create a new access key.
:::

:::tip
Ensure the IAM user has the necessary S3 permissions (e.g., `s3:ListBucket`, `s3:GetObject`) on the buckets you want the credentials to access for the connector to function properly.
:::
