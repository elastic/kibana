...
Project ID
:   (Optional) Your Google Cloud project ID. Required when using the **List Buckets** action. Find it in the [Google Cloud Console](https://console.cloud.google.com/).

Bearer Token
:   A Google OAuth 2.0 access token with Google Cloud Storage API scopes. Refer to [Get API credentials](#google-cloud-storage-api-credentials) for instructions.
...
List buckets
:   List all Google Cloud Storage buckets accessible to the configured credentials. Requires **Project ID** to be set in the connector configuration.
...
To use the Google Cloud Storage connector, you need a Google OAuth 2.0 access token with Cloud Storage API scopes. You can obtain one using the [Google OAuth 2.0 Playground](https://developers.google.com/oauthplayground/):

1. Open the [OAuth 2.0 Playground](https://developers.google.com/oauthplayground/).
2. In the list of APIs, select **Cloud Storage JSON API v1** and select the `https://www.googleapis.com/auth/devstorage.read_only` scope (or `https://www.googleapis.com/auth/devstorage.full_control` for full access).
...
Alternatively, you can use a [Google Cloud service account](https://cloud.google.com/iam/docs/service-account-overview) and generate a short-lived access token using the [Google Cloud CLI](https://cloud.google.com/sdk/gcloud):
