...
Environment URL
:   Base URL of your Dynatrace environment, without a trailing `/api/v2`. SaaS example: `https://your-environment-id.live.dynatrace.com`. Managed example: `https://your-domain/e/your-environment-id`. Environment ActiveGate example: `https://your-activegate-domain:9999/e/your-environment-id`.

Authentication
:   API token sent as the `Authorization` header. Enter the value as `Api-Token YOUR_TOKEN` (include the `Api-Token` prefix). Required scopes: `problems.read`, `problems.write`, `events.read`, `events.ingest`, `metrics.read`, `entities.read`, `settings.read`, `settings.write`.
...
1. Open your Dynatrace environment and note the environment URL from the browser address bar (SaaS form `https://{environment-id}.live.dynatrace.com`).
...
3. When creating the connector in {{kib}}, paste the token as `Api-Token YOUR_TOKEN` (include the prefix) and set the Environment URL to the base URL from step 1.
