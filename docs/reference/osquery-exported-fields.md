...
* *deb_package_files.admindir* - libdpkg admindir. Defaults to /var/lib/dpkg
...
**data_type** - keyword, text.text

* *system_profiler.data_type* - The system profiler data type (e.g., SPHardwareDataType)
...
**entitlements** - keyword, text.text

* *signature.entitlements* - JSON representation of the code signing entitlements
...
**memory_inactive_file** - keyword, number.long

* *docker_container_stats.memory_inactive_file* - Memory inactive file
...
**memory_total_inactive_file** - keyword, number.long

* *docker_container_stats.memory_total_inactive_file* - Memory total inactive file
...
* *recent_files.mtime* - Last modification time of the shortcut (usually corresponds to last opened time for the file)
...
* *dns_lookup_events.name* - Name being queried in lookup
...
* *jetbrains_plugins.name* - Name of the plugin (Title Case)
...
* *deb_package_files.package* - DEB package name
...
**package_family_name** - keyword, text.text

* *programs.package_family_name* - A combination of PackageName and PublisherHash that is used to uniquely identify applications across versions and architectures.
...
* *deb_package_files.path* - File path within the package
* *device_file.path* - A logical path within the device node
* *disk_events.path* - Path of the DMG file accessed
* *dns_lookup_events.path* - Path to binary of process making the lookup (sometimes unavailable for very short-lived processes)
...
* *jetbrains_plugins.path* - The path on the filesystem for the plugin. This may be a folder or a jar filename
...
* *recent_files.path* - The full path of the file
...
* *dns_lookup_events.pid* - Process ID of process making the lookup
...
**product_type** - keyword, text.text

* *jetbrains_plugins.product_type* - The product type (Valid values: CLion, DataGrip, GoLand, IntelliJIdea, IntelliJIdeaCommunityEdition, PhpStorm, PyCharm, PyCharmCommunityEdition, ReSharper, Rider, RubyMine, RustRover, WebStorm)
...
**response** - keyword, text.text

* *dns_lookup_events.response* - Results returned by lookup
...
* *connected_displays.rotation* - The rotation of the display (0, 90, 180, or 270 degrees). This field is -1 if display rotation is not supported.
...
* *block_devices.serial* - Disk serial number
...
**shortcut_path** - keyword, text.text

* *recent_files.shortcut_path* - Path to the shortcut where Windows stores the recent file data
...
* *dns_lookup_events.status* - Response status code
...
**symlink_target_path** - keyword, text.text

* *file.symlink_target_path* - Full path of the symlink target if any
...
* *dns_lookup_events.time* - Event timestamp in Unix format
...
* *dns_lookup_events.time_windows* - Event timestamp in Windows format
...
* *dns_lookup_events.type* - DNS record type of lookup as string
...
* *recent_files.type* - Display type for the file
...
**type_id** - keyword, number.long

* *dns_lookup_events.type_id* - Integer type ID for record type
...
* *jetbrains_plugins.uid* - The local user that owns the plugin
...
* *python_packages.uid* - The local user that owns the python package
* *recent_files.uid* - The local user ID
...
**upgrade_code** - keyword, text.text

* *programs.upgrade_code* - Specific to MSI applications, a GUID used to identify a product suite across multiple versions.
...
* *dns_lookup_events.username* - User rights - primary token username
...
* *system_profiler.value* - A JSON representation of the full result dictionary for the data type
...
* *jetbrains_plugins.vendor* - The vendor name or organization id that authored the plugin
...
* *jetbrains_plugins.version* - Version of the plugin
...
* *vscode_extensions.vscode_edition* - The VSCode edition (vscode, vscode_insiders, vscodium, vscodium_insiders, cursor, windsurf, trae)
