...
for any API interactions.

`Content-Type: application/json`:: The Kibana API uses JSON for the request and
response bodies. Interaction with the API requires that the Content-Type is set accordingly.
