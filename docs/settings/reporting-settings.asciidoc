...
[NOTE]
============
We recommend using CSV reports to export moderate amounts of data only. The feature enables analysis of data in external tools, but it's not intended for bulk export or to backup {es} data. If you are considering increasing `xpack.reporting.csv.maxSizeBytes` to larger than the default, please first consider creating smaller reports, or using {es} APIs directly.
