...
to configure Reporting to communicate with Kibana. 

If you use a reverse-proxy to access Kibana, you can set the proxy port, protocol, 
...
make them work with Kibana. For example, a reverse proxy can remove a 
base from requests so Kibana will properly serve the requests.
