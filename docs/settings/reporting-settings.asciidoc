...
[NOTE]
============
We recommend using CSV reports to export moderate amounts of data only. The feature enables analysis of data in external tools, but is not designed for bulk export or to backup {es} data. Setting `xpack.reporting.csv.maxSizeBytes` larger than the default limit hat the potential to negatively affect the performance of your {es} cluster. 
...
For exporting large amounts of data we recommend using {es} APIs directly, or Logstash CSV export. See {{search}} https://www.elastic.co/guide/en/elasticsearch/reference/current/scroll-api.html and https://www.elastic.co/guide/en/elasticsearch/reference/current/point-in-time-api.html, {ref}/sql-rest-format.html#_csv[sql] 

and {{logstash}} https://www.elastic.co/guide/en/elasticsearch/reference/current/point-in-time-api.html.
============
...
[NOTE]
============
If the default size of 500 covers a large number of shards in {es}, you may need to lower this setting. 
============
...
[NOTE]
============
If search latency in {es} is sufficiently high, such as if you are using cross-cluster search, you may need to increase the setting. 
============
...
[NOTE]
============
This setting exists for backwards compatibility, but is unused and hardcoded to `true`. CSV export from a saved search on a dashboard is enabled when Reporting is enabled.
============
