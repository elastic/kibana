...
| `xpack.reporting.queue.pollInterval`
  | (time value) Specifies the time that the reporting poller waits between polling the
  index for any pending Reporting jobs. Defaults to `3s`.

| [[xpack-reporting-q-timeout]] `xpack.reporting.queue.timeout`
  | (time value) How long each worker has to produce a report. If your machine is slow or under
  heavy load, you might need to increase this timeout.
...
| `xpack.reporting.capture.timeouts.openUrl`
  | (time value) Specify how long to allow the Reporting browser to wait for the "Loading..." screen
...
| `xpack.reporting.capture.timeouts.waitForElements`
  ...
  is captured showing the current state, and the download link shows a warning message. Defaults to `30s`.
...
| `xpack.reporting.capture.loadDelay`
  | (time value) When visualizations are not evented, this is the amount of time before
...
| `xpack.reporting.csv.scroll.duration`
  | (time value) Amount of time allowed before {kib} cleans the scroll context during a CSV export.
  Defaults to `30s`.
