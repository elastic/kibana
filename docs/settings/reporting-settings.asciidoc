...
When `true`, enables the {report-features}. The {report-features} are automatically enabled in {kib}. Set this
to `false` to disable {report-features} entirely. The default is `true`.

[NOTE]
============
Disabling Reporting entirely is discouraged. Rather than disabling the entire Reporting plugin, please use
`xpack.reporting.queue.pollEnabled: false` instead. This rule is mandatory if you intend to enable Reporting on
a portion of Kibana instances in a deployment. If you need to disable Kibana features from the UI, you can do
so on a per-role and per-space basis with <<kibana-privileges, {kib} application privileges>>.
============
...
you prefer the headless browser to connect to Kibana using a specific hostname, Reporting has a set of
experimental settings which can allow Reporting to connect to Kibana through a proxy.

[NOTE]
============
The Reporting server settings are entirely optional. Take caution when editing these settings. Adding these
settings brings a new category of reasons that Reporting might not work. If the host is not routable or has
encryption that can not be validated by Kibana, finding clues to the reason will require deep inspection of the
server logs: the full Kibana URL that Reporting is attempting to open is logged during report execution.
============

`xpack.reporting.kibanaServer.port`:: experimental[]
The port for accessing {kib}, if different from the <<server-port, `server.port`>> value.

`xpack.reporting.kibanaServer.protocol`:: experimental[]
The protocol for accessing {kib}, typically `http` or `https`.

[[xpack-kibanaServer-hostname]] `xpack.reporting.kibanaServer.hostname`:: experimental[]
