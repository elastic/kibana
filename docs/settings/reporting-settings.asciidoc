...
[NOTE]
============
Setting `xpack.reporting.csv.maxSizeBytes` much larger than the default 10mB limit has the potential to negatively affect the
...
For information about how Kibana memory limits work, see <<production, using {kib} in a production environment>>.
============
