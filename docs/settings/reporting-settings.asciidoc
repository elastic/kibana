...
deprecated:[7.9.0,This setting has no effect.] Enables CSV export from a discover view on a dashboard. This action is available in the dashboard panel menu for the discover view.
[NOTE]
============
This setting exists for backwards compatibility, and is hardcoded to `true`. CSV export from a discover view on a dashboard is enabled when Reporting is enabled.
============
