...
failure and no download will be available. Can be specified as number of milliseconds. Defaults to `2m`.
...
| `xpack.reporting.capture.timeouts.openUrl`
  ...
  to dismiss and find the initial data for the page. If the time is exceeded, a page screenshot is captured showing the current
...
| `xpack.reporting.capture.timeouts.waitForElements`
  ...
  to load on the page. If the time is exceeded, a page screenshot is captured showing the current state, and the download link shows
...
| `xpack.reporting.capture.loadDelay`
  ...
  instead of visualizations, try increasing this value. Defaults to `3s`.
