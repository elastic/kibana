...
| `xpack.apm.searchAggregatedTransactions`
  | experimental[] Enables Transaction histogram metrics. Defaults to `never` and the UI will use metric indices over transaction indices for transactions if aggregated transactions are found. When set to `always`, additional configuration in APM Server is required. When set to `never`, aggregated transactions are not used.
