...
A list of action types that are enabled. It defaults to `[*]`, enabling all types. The names for built-in {kib} action types are prefixed with a `.` and include: `.server-log`, `.slack`, `.email`, `.index`, `.pagerduty`, `.webhook`,'.jira','.swimlane','.servicenow','.servicenow-sir','.resilient' and `.teams`.. An empty list `[]` will disable all action types.
+
