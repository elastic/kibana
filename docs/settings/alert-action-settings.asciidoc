...
Specifies the minimum schedule interval allowed for the rules. This minimum is applied to all rules created or updated after the introduction of this setting. The time is formatted as:
+
`<count>[s,m,h,d]` 
+
