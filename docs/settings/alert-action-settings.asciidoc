...
Overrides the configs under `xpack.alerting.rules.execution.actions` for the connector type with the given ID. The value is array of objects consists of the key(s) that are wanted to be overridden under actions object and the id of the specific connector type that is wanted to be modified.
+
For example:
```
