...
cluster. {kib} will always use the credentials of the logged user to access the 
dedicated monitoring cluster.

`xpack.monitoring.elasticsearch.username`::
Specifies the user ID that {kib} uses for authentication when it sends monitoring data
to the monitoring cluster. If not set, {kib} uses the value of the
`elasticsearch.username` setting.

`xpack.monitoring.elasticsearch.password`::
Specifies the password that {kib} uses for authentication when it sends monitoring data
to the monitoring cluster. If not set, {kib} uses the value of the
