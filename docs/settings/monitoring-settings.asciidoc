...
Specifies a {ref}/security-api-create-service-token.html[service account token] to establish a persistent connection in {kib} to the {es} monitoring cluster and to verify licensing status on the {es} monitoring cluster.  This setting is an alternative to `monitoring.ui.elasticsearch.username` and `monitoring.ui.elasticsearch.password`.
+
If not set, {kib} uses the value of <<elasticsearch-service-account-token, `elasticsearch.serviceAccountToken`>> setting.
