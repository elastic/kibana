...
Specifies a {ref}/security-api-create-service-token.html[service account token] for the {es} cluster where your monitoring data is stored when using `monitoring.ui.elasticsearch.hosts`.  This setting is an alternative to `monitoring.ui.elasticsearch.username` and `monitoring.ui.elasticsearch.password`. If not set, {kib} uses the value of <<elasticsearch-service-account-token, `elasticsearch.serviceAccountToken`>> setting.
