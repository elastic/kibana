...
| Ensures that user sessions will expire after a period of inactivity. By default, this setting is equal to <<xpack-session-idleTimeout, `xpack.security.session.idleTimeout`>>.
...
| Ensures that user sessions will expire after the defined time period. This behavior also known as an "absolute timeout". By default, this setting is equal to <<xpack-session-lifespan, `xpack.security.session.lifespan`>>.
...
highly recommended. You can also specify this setting for <<xpack-security-provider-session-idleTimeout, every provider separately>>. By default, this setting is not set.
