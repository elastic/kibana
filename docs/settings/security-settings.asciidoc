...
List of values matched against the `event.kibana.space_id` field of an audit event. This represents the space id in which the event took place.
...
List of values matched against the `event.user.name` field of an audit event. This represents the `username` associated with the audit event.
