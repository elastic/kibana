...
| Set to `true` and configure where the audit log should be written to using `xpack.security.audit.appender` to enable audit logging. *Default:* `false`
...
[NOTE]
============
deprecated:[7.15.0,"In 8.0 and later, the legacy audit logger will be removed, and this setting alone will instead enable the ECS audit logger with a default appender."] To enable the legacy audit logger only specify:
