...
The valid settings in the `xpack.security.authc.providers` namespace vary depending on the authentication provider type. For more information, refer to {kib} authentication.
...
For more details and a reference of audit events, refer to {kib} audit logging.
...
List of values matched against the `event.action` field of an audit event. Refer to {kib} audit logging for a list of available events.
