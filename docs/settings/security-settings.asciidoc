...
xpack.security.audit.appender:
  ...
  layout:
    kind: json
----------------------------------------
<1> Rotate log files every 24 hours
<2> Keep maximum of 10 log files before deleting older ones
...
Refer to <<audit-logging-file-appender, file appender>> and <<audit-logging-rolling-file-appender, rolling file appender>> for appender specific settings.
...
Refer to <<audit-logging-pattern-layout, pattern layout>> for layout specific settings.
...
The `file` appender logs to a file and can be configured using the following settings:
...
2+a| 
[float]
[[audit-logging-rolling-file-appender]]
===== Rolling file appender

The `rolling-file` appender logs to a file and rotates it using a rolling strategy, when a particular policy is triggered:
...
Refer to <<audit-logging-size-limit-policy, size limit policy>> and <<audit-logging-time-interval-policy, time interval policy>> for policy specific settings.
...
Refer to <<audit-logging-numeric-strategy, numeric strategy>> for strategy specific settings.

2+a| 
[float]
[[audit-logging-size-limit-policy]]
===== Size limit triggering policy

The `size-limit` triggering policy will suffix the log file with a given pattern when rolling over, and will limit the number of rolled files to a fixed amount:
...
2+a| 
[float]
[[audit-logging-time-interval-policy]]
...
2+a| 
[float]
[[audit-logging-numeric-strategy]]
