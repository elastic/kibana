...
<2> Specifies the order of the provider in the authentication chain and at the Login Selector UI. This setting is mandatory.
...
The valid settings in `xpack.security.authc.providers` namespace vary depending on the authentication provider type. For more information, refer to <<kibana-authentication>>.
