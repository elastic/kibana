...
The _all_ and _off_ levels can only be used in configuration and are handy shortcuts that allow you to log every log record or disable logging entirely or for a specific logger. These levels can also be specified using <<logging-cli-migration,cli arguments>>.
