...
[WARNING]
====
Upgrading from 6.8 to 7.0 is *not* supported.
You must upgrade directly to 7.1 or later.
We recommend upgrading the latest 7.n release. 
====
...
6.8 before you upgrade to 7.1 or later.
...
to 7.x, you must reindex or delete any indices created in 5.x or earlier.
...
The recommended path is to upgrade to 6.8 before upgrading to 7.n. This makes it
easier to identify the required changes, and enables you to use the Upgrade
Assistant to prepare for your upgrade.

IMPORTANT: Upgrading from 6.8 to 7.0 is not supported. 
You must upgrade directly to 7.1 or later. 
We recommend upgrading to the latest 7.n release.
...
[[upgrade-67]]
=== Upgrade from 6.8
To help you prepare for your upgrade to 7.n, 6.8 includes an https://www.elastic.co/guide/en/kibana/6.8/upgrade-assistant.html[Upgrade Assistant]
...
Assistant, <<upgrade-standard,upgrade to 7.n>>.
