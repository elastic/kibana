...
When you upgrade the Elastic Stack, you can use a minor {es} version that is higher than the {kib} version when you upgrade {es} first. For example, Kibana 5.0 and Elasticsearch 5.1. The Kibana server startup logs a temporary warning until you upgrade to the same {es} version.
endif::[]
