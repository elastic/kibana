...
Please see 
*{stack-ov}/upgrading-elastic-stack.html[Upgrading the Elastic Stack]* for a 
...
See the {ref}/upgrading-elastic-stack.html#oss-stack-upgrade[Stack upgrade guide] 
for more information.

Once your reindex is complete, you can follow the <<upgrade-standard, Standard upgrade>> 
instructions to do a manual upgrade. The recommended path is to upgrade to 6.7 before upgrading to 7.0.
...
[float]
==== Upgrading from 6.7
6.7 includes an <<upgrade-assistant, Upgrade Assistant>> to help you prepare for your upgrade to 7.0. 
To access the assistant, go to *Management > 7.0 Upgrade Assistant*. 

You can also use the <<upgrade-standard, Standard upgrade>> instructions to do a manual upgrade to 7.0.
