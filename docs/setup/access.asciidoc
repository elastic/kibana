...
----
These {kib}-backing indices must also not have https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-get-settings.html[index settings] flagging `read_only_allow_delete` or `write` https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-blocks.html[index blocks]. 
