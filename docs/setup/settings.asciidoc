...
| The `root` logger has is a <<dedicated-loggers, dedicated logger>> and is pre-configured and will log at an `info` level if not specifically declared. If any other logging configuration is specified, `root` _must_ also be explicitly configured.
