...
Instead, configure the `elasticsearch.query` logger and set an approriate appender for the logger.
+
...
[WARNING]
============================================================================
{kib} supports both deprecated and current logging systems being enabled at the same time. 
We strongly suggest not mixing the configurations and rather switch over completely to avoid unforeseen ciscumstances.
...
The `root` logger has is a <<dedicated-loggers, dedicated logger>> and is pre-configured. The `root` logger logs at `info` level by default. If any other logging configuration is specified, `root` _must_ also be explicitly configured with an appropriate appender. 
...
Specific appender format to apply for a particular logger context. If not configured, falls back to the `root` logger appender if 
