...
| `csp.unsafe_eval`
| experimental[] Set this to `false` to remove the https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src#unsafe_eval_expressions[`unsafe-eval`] source expression from the `script-src` directive. *Default: `true`*

By disabling `csp.unsafe_eval`, Kibana will use a custom version of the Handlebars template library which doesn't support https://handlebarsjs.com/guide/partials.html#inline-partials[inline partials].
...
If you experience any issues rendering Handlebars templates after turning off `csp.unsafe_eval`, or if you rely on inline partials, please revert this setting to `true` and https://github.com/elastic/kibana/issues/new/choose[open an issue] in the Kibana GitHub repository.
