...
`exploreDataInContextMenu.enabled`
 | Enables the *Explore underlying data* option that allows you to open *Discover* from a dashboard panel and view the panel data. *Default: `false`*
  ...
  When you create visualizations using the *Lens* drag-and-drop editor, you can use the toolbar to open and explore your data in *Discover*. For more information, check out <<open-lens-data-in-discover, Open the data in Discover>>. 
