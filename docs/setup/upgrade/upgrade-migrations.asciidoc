...
. Remove the write block which the migration system has placed on the index:
+
[source,sh]
--------------------------------------------
...
Please be aware the Dashboard with ID `e3c5fc71-ac71-4805-bcab-2bcc9cc93275` that belongs to the space `marketing_space` **will no longer be available**.
