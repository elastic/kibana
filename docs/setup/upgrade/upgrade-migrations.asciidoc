...
As part of upgrade migrations, {kib} creates indices with an auto-expanded replica. When routing allocation is disabled or restricted (`cluster.routing.allocation.enable: none/primaries/new_primaries`) the replica shard won't get assigned causing the upgrade to fail. 
