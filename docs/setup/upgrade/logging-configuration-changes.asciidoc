...
WARNING: {kib} 8.0 and later uses a logging system. Be sure to read the documentation for your version of {kib} before proceeding.

[[logging-pattern-format-old-and-new-example]]
[options="header"]
|===

| Parameter | Platform log record in **pattern** format | Legacy Platform log record **tedocs/xt** format
