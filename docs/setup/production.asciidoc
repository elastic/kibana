...
In each cluster, {kib} instances should have the same `cookieName` value in order to achieve seamless high availability and keep the session active in case of failure from the currently used instance.
