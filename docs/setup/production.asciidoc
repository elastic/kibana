...
Multiple Kibana installations can be served behind a load balancer with configuration changes.  Descriptions can be referenced on the {kibana-ref}/settings.html[settings page].

Settings unique across each Kibana instances:
--------
server.uuid
server.name
--------

Settings unique across each host (e.g. running multiple installations on the same virtual machine):
--------
logging.dest
path.data
pid.file
