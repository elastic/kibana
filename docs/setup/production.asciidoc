...
To be able to access multiple load-balanced {kib} clusters from the same browser, the setting `xpack.security.cookieName` must be set in the configuration to avoid having conflicts between cookies from the different {kib} instances.
