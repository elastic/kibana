...
For security, you will need to grant users access to the {report-features}, and secure the reporting endpoints
with TLS/SSL encryption. Additionally, you may need to install graphical packages into the operating system in
order for the {kib} server to have screenshotting capability.
...
.. Next each application listed, click *All* or customize the privileges more granularly by clicking *Read* and enabling the *Customize sub-feature privileges* checkbox.
+
NOTE: If you have a *Basic* license, sub-feature privileges are not available for your subscription. Refer to the screenshot in <<grant-user-access-basic>>.
[role="screenshot"]
...
+
NOTE: If *Reporting* options for application features are not available, and the cluster has higher than a *Basic* license, contact your administrator, or <<reporting-advanced-settings,check that xpack.reporting.roles.enabled is set to false in kibana.yml>>.
...
With <<grant-user-access, {kib} application privileges>> enabled in Reporting, you can also use the {ref}/security-api-put-role.html[role API] to grant access to the {report-features}, using *All* privileges, or granular sub-feature privileges.

NOTE: If you have a *Basic* license, sub-feature privileges are not available for your subscription. Refer to the API command to grant *All* privileges shown in <<grant-user-access-basic>>.

Grant custom reporting roles to users in combination with other roles that grant read access to the data in {es}, and at least read access in the applications where users can generate reports.
...
With a Basic license and enabled, you are still able to grant users access with custom roles to {report-features} with <<kibana-privileges, {kib} application privileges>>. However, with a Basic license, sub-feature privileges are not available. You will need to create a role <<grant-user-access, as desribed above>>, but select *All* privileges for the applications for which to allow users to create reports.
...
You can also use the {ref}/security-api-put-role.html[role API] to grant access to the {report-features}. With
a *Basic* license, sub-feature application privileges are not available, so the API call must be 
...
<2> Grants access to download CSV from saved search panels in *Dashboard*.
