...
NOTE: To generate reports in a cross-cluster search environment, your role requires different privileges, based on whether you are using an API key for authentication or directly authenticating as the user. For more information, refer to {ref}/remote-clusters-cert.html#remote-clusters-privileges-ccr[Configure privileges for cross-cluster replication].
