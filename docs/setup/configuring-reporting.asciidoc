...
When security is enabled, granting users privilege to generate reports can be controlled by <<kibana-privileges, {kib} application privileges>>. This lets you create custom roles that control the spaces and applications where users are allowed to generate reports.

. Enable **application privileges** in Reporting. To do this, you must turn off the default user access control features in `kibana.yml` with:
+
[source,yaml]
------------------------------------
...
------------------------------------
+
NOTE: If you keep the default settings, you can still create a custom role that grants reporting privilege: the default role is `reporting_user`. This behavior is being deprecated as it does not allow application-level access control for {report-features}, and it does not allow API keys or authentication tokens to authorize report generation. See <<reporting-advanced-settings, reporting security settings>> for information and caveats about the deprecated access control features.
