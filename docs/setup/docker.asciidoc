...
The images are available in two different configurations or "flavours". The
`x-pack` flavour, which is the default, ships with X-Pack features
pre-installed. The `oss` flavour does not include X-Pack, and contains only
open-source Kibana.
