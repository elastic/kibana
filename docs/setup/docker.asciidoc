...
In general, any setting listed in <<settings>> can be configured with this technique. If you are looking to configure settings not listed there, please refer to using <<bind-mount-config,a bind mounted `kibana.yml`>>.
