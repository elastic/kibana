...
* Upload geo data the with GeoJSON Upload feature.

* Set up a Beats module to index logs, metrics, events, or application data.
...
create index patterns that match the names of the indices you want to explore.
When you add data with the File Data Visualizer, GeoJSON Upload,
or an built-in tutorial, an index pattern is created for you.
