...
=== System Dependencies
Reporting launches a "headless" web browser called Chromium on the Kibana server, and it has dependencies on some system packages. Make sure
the system has the appropriate packages installed for the distribution.
...
Also, ensure that the `headless_shell` binary located in your Kibana data directory is owned by the user who is running Kibana, that the
