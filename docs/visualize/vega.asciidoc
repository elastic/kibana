...
For Vega visualization two different views was implemented: *Request* view and *Vega debug* view.

===== Inspect Elasticsearch requests

Vega visualization uses the {ref}/search-search.html[{es} search API] to get documents and aggregation 
results from {es}.To troubleshoot these requests, open the *Inspector*, which shows the most recent requests. 
In case your specification has more than one request, you can switch between them using the *Request* dropdown.
...
Vega specs unless you can share a dataset. To do this, open the *Inspector > Vega debug* view 
and navigate to *Spec* tab:

[role="screenshot"]
image::visualize/images/vega_tutorial_getting_help.png[]

To copy the response, click the button "Copy to clipboard". Paste the copied data to 
