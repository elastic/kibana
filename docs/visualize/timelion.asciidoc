...
[float]
[[customizing-data-series-y-axis]]
==== Customize data series y-axis

Use different y-axis scales and ranges for your data series.
The `.yaxis()` function allows the following parameters:

[horizontal]
yaxis :: The numbered y-axis to plot this series on, eg .yaxis(2) for a 2nd y-axis.
min :: Min value for y-axis range
max :: Max value for y-axis range
position :: `left` or `right`  
label :: Label for axis  
color :: Color of axis label
units :: The function to use for formatting y-axis labels. One of: `bits`, `bits/s`, `bytes`, `bytes/s`, `currency(:ISO 4217 currency code)`, `percent`, `custom(:prefix:suffix)`
tickDecimals :: Tick decimal precision
...
<1> `.yaxis(1,units=bytes,position=left)` allows to set a first Y axis for the first series, changing the units, on the left
<2> `.yaxis(2,units=bytes,position=left)` allows to set a second Y axis for the second series, changing the units, on the right
