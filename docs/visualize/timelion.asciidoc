...
[horizontal]
yaxis :: The numbered y-axis to plot this series on, eg .yaxis(2) for a second y-axis.
min :: Minimum value for y-axis range
max :: Maximum value for y-axis range
...
<1> `.yaxis(1,units=bytes,position=left)` sets the first y-axis for the first data series, and changes the units on the left
