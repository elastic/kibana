...
Ready to create a visualization? Create a visualization on your own, or use the following tutorials:

* <<area-chart-tutorial, Area chart>>
* <<controls-tutorial, Controls>>
* <<data-table-tutorial, Data table>>
* <<gauge-tutorial, Gauge>>
* <<goal-tutorial, Goal>>
* <<bar-chart-tutorial, Bar chart>>
* <<line-chart-tutorial, Line chart>>
* <<markdown-tutorial, Markdown>>
* <<metric-tutorial, Metric>>
* <<pie-chart-tutorial, Pie chart>>
* <<tsvb-tutorial, TSVB>>
* <<tag-cloud-tutorial, Tag cloud>>
* <<timelion-tutorials, Timelion>>

[float]
[[area-chart-tutorial]]
=== Area chart tutorial

Create an area chart that shows the sum flight distance in the last 7 days.

[float]
[[area-chart-before-you-beging]]
==== Before you begin
...
. On the *New Area/Choose a source* window, select *[Flights] Flight Log*.

. From the time filter, select *Last 7 days*.

[float]
[[area-chart-metric-aggregation]]
=== Choose the aggregations

Specify the options for the metric and bucket aggregations.

. In the *Metrics* area, expand the *Y-axis* options.

. From the *Aggregation* dropdown, select *Average*.

. From the *Field* dropdown, select *DistanceMiles*.

. Add a *Custom label*. For example, `Distance (Miles)`.
...
. Add a *Custom label*. For example, `Date and time`.

. Click *Apply changes* image:images/apply-changes-button.png[].

image:images/visualize-area-chart-tutorial.png[Area chart with flight sample data]

[float]
[[area-chart-lines]]
=== Make the area chart easier to read

To make the area chart easier to read, add background lines.

. Select *Panel Settings*.

. To make the area chart easier to read, select *Show x-axis lines*.

. From the *Y-axis lines* dropdown, select *LeftAxis-1*.

. Click *Apply changes* image:images/apply-changes-button.png[].

image:images/visualize-area-chart-tutorial02.png[Area chart with flight sample data]

[[controls-tutorial]]
=== Controls tutorial
experimental[]

Want some more Controls inspiration? Check out the link:https://www.elastic.co/blog/interactive-inputs-on-kibana-dashboards[Interactive Inputs on Kibana Dashboards] blog.

[[data-table-tutorial]]
=== Data table tutorial

Create a data table to display the status code and request resource from web logs.

[float]
[[data-table-before-you-begin]]
==== Before you begin

For this tutorial, you'll need to add the <<add-sample-data, Sample Web Logs data>>.

[float]
[[create-the-data-table]]
==== Create the data table

To create the data table visualization, use the visualization builder.
...
. On the *New Data Table/Choose a source* window, select *kibana_sample_data_logs*.
...
Use these values to create the table, accept the defaults for all other settings.
...
*** *Order*: Descending
*** *Size*: 8
*** Toggle *Group other values in separate bucket*
...
[role="screenshot"]
image:images/visualize-data-table-tutorial01.png[Data table with web logs sample data]

[float]
[[data-table-split-table]]
==== Split the data table into multiple tables

If you wanted to view all of the requests that had a certain status code. You could use the split table option to see results grouped by status code in separate tables.
...
[role="screenshot"]
image:images/visualize-data-table-tutorial02.png[Split data table with web logs sample data]
...
=== Create a visualization with mathematical functions
...
=== Create a visualization with conditional logic and tracking trends
