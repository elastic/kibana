...
NOTE: By default, the *place markers off grid (use geocentroid) box is checked. When this box is checked, the markers are
placed in the center of all the documents in that bucket. When it is not checked, the markers are placed in the center
of the geohash grid cell. Leaving this box checked generally results in a more accurate visualization.
