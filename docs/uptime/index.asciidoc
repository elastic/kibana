...
For example, version 6.7 of Kibana will expect an index of `heartbeat-6*`, while Kibana
7.0 requires an index of `heartbeat-7*` (containing documents from Heartbeat 7.0).
Kibana 7.0 does not support older versions of Heartbeat data because there are breaking
changes in the schema mappings between the major versions.
