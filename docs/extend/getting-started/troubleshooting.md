...
If `pnpm kbn bootstrap` fails, try the following in order:

1. Clear caches and force reinstall dependencies:

```bash
pnpm store prune && pnpm kbn clean
...
If you experience minified React errors and want to expand them to their full error messages you will currently need to rebuild the `@kbn/ui-shared-deps-npm` package using "development" mode instead of "production", which can be done by modifying the corresponding line in `packages/kbn-ui-shared-deps-npm/webpack.config.js` and make sure to run `pnpm kbn bootstrap` afterwards:
