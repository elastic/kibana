  ...
  "compounds": [
    {
      ...
      "description": "Attaches to Kibana server in debug mode. Requires first starting Kibana with `pnpm debug`",
...
2. Start Elasticsearch with `pnpm es snapshot`.
3. Start Kibana with `pnpm debug` or `node --inspect scripts/kibana --dev`.
...
2. Start Elasticsearch with `pnpm es snapshot`, but don't start Kibana and kill any running instances of Kibana.
...
```jsonc
  // self managed
  "jest.jestCommandLine": "pnpm test:jest --runInBand"
```
