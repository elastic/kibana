...
Judge suite size by **spec duration**, not a test-count cap. Most Scout specs finish within about a minute; longer files become a single-worker bottleneck. A small `for` loop or shared describe helper that is still one role + one flow can stay in one file — split when the generated suite is large or the rows are different flows.
