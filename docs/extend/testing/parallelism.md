...
- **Sequential** (`tests/`, one worker): suites that require a “clean” cluster state or need global mutations that aren’t space-scoped. They run in the default space with `test`; `spaceTest` isn’t needed there, and switching a sequential suite from `spaceTest` to `test` doesn’t weaken its isolation.
