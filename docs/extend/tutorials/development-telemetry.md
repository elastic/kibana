...
* Total execution time of `@kbn/optimizer` runs as well as the following metadata about the runs: whether the build succeeded, the number of bundle entries created, usage of `--watch`, `--dist` and `--no-cache` flags, and the count of themes being built.
