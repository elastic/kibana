...
There are several ways to configure an advanced setting:
 - [Through the Advanced Settings UI](#configuration-with-advanced-settings-ui)
 - [Locked via `kibana.yml`](#configuration-with-ui-settings-overrides)
 - [Through the client-side `uiSettings` service](#client-side-usage)
 - [Through the server-side `uiSettings` service](#server-side-usage)

`uiSettings` are registered synchronously during `core`'s setup lifecycle phase. This means that once you add a new advanced setting, you cannot change or remove it without [registering a migration in core](#registering-migrations).
