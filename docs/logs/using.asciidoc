...
<<xpack-logs-configuring,Configure the logs source data>> to change the index pattern and other settings.
...
[[logs-customize]]
=== Customize your view
Click *Customize* to customize the output view.
...
To quickly filter the logs stream by one of the field values here, click the *View event with filter* icon image:logs/images/logs-view-event-with-filter.png[View event icon] beside the field.
This automatically adds a search filter to the logs stream to filter the entries by this field and value.

Click *Actions* to see the other actions related to that log event.
