...
In some cases, you might want to compare the results of two separate queries. Kibana can handle multiple queries by joining them with a logical
`OR` and using the knowledge that they should be treated as separate to
...
tab. Give the tab a click and we can see that a filter was added to the
