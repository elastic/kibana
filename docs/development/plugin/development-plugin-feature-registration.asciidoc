...
|`string`
|`"sample_feature"`
|A unique identifier for your feature. Usually, the name of your plugin is sufficient.
...
- Users with read/write access (`all` privilege) need to be able to read/write `canvas-workdpad` saved objects, and they need read-only access to `index-pattern` saved objects.
...
The `all` privileges defines a single “save” UI Capability. To access this in the UI, Canvas could:
