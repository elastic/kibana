...
|  [asSystemApi](./kibana-plugin-public.httpfetchoptions.assystemapi.md) | <code>boolean</code> | Whether or not the request should include the "system API" header to differentiate an end user request from Kibana internal request. Can be read on the server-side using KibanaRequest\#isSystemApi. Defaults to <code>false</code>. |
