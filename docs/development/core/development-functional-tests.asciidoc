...
* Popular methods:
** `retry.try(fn, onFailureBlock)` - execute `fn` in a loop until it succeeds or the default try timeout elapses
** `retry.tryForTime(ms, fn, onFailureBlock)` execute fn in a loop until it succeeds or `ms` milliseconds elapses
