...
** When running against cloud deployment, some tests are not applicable use --exclude-tag to skip those tests.  An example shell file can be found at: {blob}test/scripts/jenkins_cloud.sh[test/scripts/jenkins_cloud.sh]
+
["source", "shell"]
----------
...
**Tags**:::
We use tags to group functional tests:
* `ciGroup{id}` tag to assign test suite to specific CI worker
* `skipCloud` and `skipFirefox` to exclude test suite from running on Cloud or Firefox browser
* `smoke` to group tests that we run both on Chrome and Firefox
Tags are supported for `describe()` function only.

**Cross-browser testing**:::
On CI all the functional tests are executed in Chrome by default. To run tests also on Firefox `smoke` tag should be assigned:
...
To run tests on FIrefox locally, `config.firefox.js` should be provided:
