...
We use a browser automation tool to automate the launch and interact with the browser of choice (currently we support https://www.google.com/chrome/[Chrome] and https://www.mozilla.org/en-US/firefox/new/[Firefox] through their respective drivers.) We then use different services (`browser`, `find`, `testSubjects`, etc.) to perform interations with Kibana and report the results. 
...
[float]
===== Functional Test Runner API Docs

The Kibana functional tests define the vast majority of the actual functionality used by tests.

**browser**:::
* Source {blob}/test/functional/services/browser.js
* Handles interactions that have to do with the browser directly. 
* Methods
** `browser.refresh()` - Refreshes the page that we are on in the browser.
** `browser.dragAndDrop(from, to)` - Drags an element from one point to another. 
*** args
**** `from`
...
***** {element: WebElementWrapper} 
***** {{ x: number, y: number}, offset: { x: number, y: number }}
** `browser.moveMouseTo(element, xOffset, yOffset)` - Moves the remote environment's mouse cursor to the specified element or relative position.
*** args
**** `element`
***** { element: WebElementWrapper } 
**** `xOffset`
***** number: number of pixels from the left to offset mouse
**** `yOffset`
***** number: number of pixels from the top to offset mouse
** `browser.getWindowSize()` - Retrieves the a rect describing the current top-level window's size and position.
*** returns
**** {`height`: number, `width`: number, `x`: number, `y`: number}
** `browser.setWindowSize(...args)` - Sets the dimensions of a window.
*** args
**** `width` 
***** number : number of pixels wide that the window should be.
**** `height` 
***** number : number of pixels tall that the window should be. 
** `browser.getCurrentUrl()` - Gets the URL that is loaded in the focused window/frame.
*** returns
**** string : Current URL
** `browser.get(url)`
*** `args`
**** `url`
***** string : Well formed URL to navigate to in the browser.
** `browser.goBack()` - Navigates the focused window/frame back one page using the browser’s navigation history.
** `browser.pressKeys(...args)` - Sends a sequence of keyboard keys. For each key, this will perform a pair of keyDown and keyUp actions.
*** args
**** `keys`
***** string : A string with the keys that should be want pressed.
***** string[] : An array of the strings that should be pressed. 
** `browser.clickMouseButton(...args)` - Performs a left click at a given origin being either the center of an element, the viewport or the mouse's current position. 
*** args
**** `element`
***** { WebElementWrapper } 
**** `xOffset`
***** number : number of pixels from left of origin to target click
**** `yOffset`
***** number : number of pixels from top of origin to target click
** `browser.getPageSource()` - Gets the HTML loaded in the focused window/frame.
*** returns
**** string : HTML source
** `browser.getLogsFor(...args)` - Get's all logs from the remote environment of the given type. 
*** args
**** `type`
***** { logging.Type } - Type Object from Selenium Library to indicate what type of logs to return.
*** returns
**** { LogEntry[] } : Array of LogEntry items; Logs are cleared after return. 
** `browser.takeScreenshot()` - Gets a screenshot of the focused window/frame and returns it as a base-64 encoded PNG.
*** returns
**** Buffer : The PNG
** `browser.doubleClick(element)` - Performs a double left-click with the mouse at a given element. If no element specified, mouse will double-click at its current location.
**** `element`
***** { WebElementWrapper } 
** `browser.switchWindowTo(...args)` - Changes the focus to a new window of the specified handle. All actions will be performed on this window going forward.
*** args 
**** `handle`
***** string : Identifier for the window
** `browser.getAllWindowHandles()` - Get the handles of all currently open windows.
*** returns
**** string[] : Array of all open window handles.
** `browser.closeCurrentWindow()` - Closes the currently open window.
** `browser.execute(fn, ...args)` - Executes the given JavaScript function within the focused window/frame.
*** `function`
**** string : String representation of desired function to execute.
**** function : JavaScript function to execute.
*** `args`
**** any[] - An array of any type containing arguments to pass into given function.
** `browser.setLocalStorageItem(key, value)` - Sets a value in local storage for the focused window/frame.
*** `key`
**** string : Key to set in local storage.
*** `value`
**** string : Value for given key to set in local storage.

**retry:**:::
* Source: {blob}test/functional/services/retry.js[test/functional/services/retry.js]
* **A Last Resort For Retrying Operations**
* Up until recently, `retry` was our go-to operation for waiting for a condition to be true before continuing.
* This has been replaced by the waiting mechanism within the `find` service. Within this waiting mechanism, retries are 
* already included. Calling `retry` directly should be avoided unless it's the only option.
* Methods
