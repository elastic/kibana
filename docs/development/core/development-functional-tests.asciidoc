...
* `ciGroup{id}` to assign test suite to specific CI worker
...
On CI, all the functional tests are executed in Chrome by default. To run tests on Firefox, assign the `smoke` tag:
