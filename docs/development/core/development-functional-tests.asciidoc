...
Use the following command to start the server required for the `node scripts/functional_test_runner` tasks. Once the server is started `node scripts/functional_test_runner` can be run multiple times without waiting for the server to start.
...
Execute the `FunctionalTestRunner`'s script with node.js to run the tests with Kibana's default configuration. This requires the server started by the `test:ui:server` task.
