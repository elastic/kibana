...
Default response handler will convert pure elasticsearch response to tabular format. Its the recommended response
handler to use. The response object will contain a table property, which is an array of all the tables produced (if 
you use the split chart for example). Each of the table objects has two properties:
...
- *vis.API.queryFilter*: allows you to query filter
- *vis.API.queryManager*: allows you to query manager
- *vis.API.filterManager*: allows you to filter manager
- *vis.API.kuery*: allows you to access to kuery language API
...
Few times we mentioned the AggConfig object and you will probably have to handle with it in every visualization you 
will create. Here we provide short description of some of the methods on it, however the best reference would be 
to actually check the code and other visualizations.
