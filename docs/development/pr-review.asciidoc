...
This document only seeks to describe the high level goals of the PR review process. It is not expected nor intended for a PR review to take the shape of this document, where the topmost guidelines are always applied first and the bottommost guidelines are always applied last. This entire document should be considered as a whole rather than as a series of consecutive todos that must be completed in order.
...
[float]
=== Review feedback that is never appropriate

Whether or not a bit of feedback is appropriate for a pull request is really dependent on the motivation for giving the feedback in the first place. Reviewers should not be reviewing code with the mindset of "how would I write this code?". The reviewer didn't write the code, and their goal isn't to craft the contribution into a form that is simply whatever they would have written if they had.

Feedback such as "this is crap" isn't feedback at all. It's both mean and unhelpful, and it is never appropriate.
