...
Two cases return HTTP 400 Bad Request rather than 403. A `runSavedQueries`-only caller who runs a saved query containing `{{parameter}}` placeholders without alert context is rejected, because there is nothing to substitute and the literal placeholder is not dispatched to the agent. A `saved_query_id` that resolves to nothing on a request that requires stored content is likewise a 400.
