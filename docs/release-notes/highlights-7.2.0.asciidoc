...
[float]
=== Simplified creation of {ml} jobs for sample data sets and {metribeat} data
...
For example, you can select a supplied configuration to detect unusual increases
in disk utilization:

[role="screenshot"]
image::release-notes/images/7.2-metricbeat-ml-modules.png[Embedded map in dark mode]

These wizards expedite the creation of customized jobs, dashboards, searches,
