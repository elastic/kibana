...
[float]
=== View recent logs alongside your monitoring data

You can now see your cluster's recent logs within the *Stack Monitoring*
application:

[role="screenshot"]
image::release-notes/images/7.2-monitoring-logs.png["Monitoring recent {es} logs"]

See <<elasticsearch-metrics>>. If this subset of logs is not sufficient, you can
follow a link to the <<xpack-logs,Logs UI>> and continue your investigation there.
