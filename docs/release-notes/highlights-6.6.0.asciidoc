...
help keep your clusters tidy. You can create and manage index lifecycle policies, 
which perform actions such as rolling over an index, relocating shards to warm 
...
{kib} also has two new user interfaces for cross cluster replication, available
in *Management*. 
*Remote clusters* helps you manage remote clusters for use with cross cluster 
...
choose from a light and a dark themed workpad. Look for the *Templates* tab on 
...
For more information on these settings, see {kibana-ref}/settings.html[Configure Kibana].
