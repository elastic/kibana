...
The token authentication provider that the login form uses can also support the `Authorization` request header with the `Basic` scheme. This enables applications like `curl`. Set the following in your `kibana.yml`:
