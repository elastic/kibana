...
. {es} can potentially have multiple different SAML realms configured at the same time, and the one that will be used by a particular {kib}
instance is determined by the `Assertion Consumer Service` URL that {kib} constructs and sends to {es} during SAML handshake. But there are
cases when it'd make more sense to change this behaviour and force {kib} to use a specific SAML realm by defining the following setting:
+
[source,yaml]
--------------------------------------------------------------------------------
...
Relying on specific realm name can also help when `xpack.security.public.port` is set to the value that is considered as default for
a particular protocol and hence omitted by some systems (e.g. `443`) that can lead to various misconfiguration issues.
