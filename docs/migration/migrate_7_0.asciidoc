...
Support for time-based index patterns will be removed in 8.0.

*Impact:* If you query a time-based index pattern, that query will now be performed 
on the root wildcard term. For example, a query on `logstash-2019-04-18` will now 
query all indices that match `logstash-*`. Support for time-based index patterns
will be removed in 8.0, you can migrate your index pattern to a wildcard pattern
