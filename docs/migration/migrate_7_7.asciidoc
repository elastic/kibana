...
* Adds ability to embeddable to report what triggers it executes through
`.supportedTriggers()` method.
...
This PR makes trigger context type typed in TypeScript and linked to `Trigger` type.
Now when you execute a trigger or when an action receives trigger context it
can be well typed. This functionality was added to visualization embeddable
in the place where it executes a trigger.
