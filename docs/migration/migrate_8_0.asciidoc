...
*Impact:* To restore the previous behavior, in kibana.yml use the pattern layout, with a <<date-format, date modifier>>:
[source,yaml]
-------------------
logging:
  appenders:
...
*Impact:* To restore the previous behavior, in `kibana.yml` use the <<rolling-file-appender,`rolling-file`>> appender.
