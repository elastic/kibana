...
*Details:* The `kibana_user` role has been renamed to `kibana_admin` to better reflect its intended use. This role
continues to grant all access to every Kibana feature. If you wish to restrict access to specific features,
then you should create custom roles with <<kibana-privileges>>.

*Impact:* Any users currently assigned the `kibana_user` role will need to instead be assigned the `kibana_admin`
role in order to maintain their current access level.
