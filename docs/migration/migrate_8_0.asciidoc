...
*Impact:* Users who relied on changing these settings to achieve multitenancy should use Spaces, cross-cluster replication or cross-cluster search instead. To migrate to Spaces, users are encouraged to use saved-object management to export their saved-objects from a tenant into the default tenant in a Space. There are improvements planned to improve on this workflow. See https://github.com/elastic/kibana/issues/82020 for more details.   
