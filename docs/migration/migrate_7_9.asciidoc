...
You can add now templates through `SavedObject` management.
...
and will be deleted without any notice. Use the core start API instead.
...
The API reference docs for `state_sync` and `state_containers` are available:
...
.Saved objects now include support fo hidden types
[%collapsible]
====
...
.OSS features registration moved to Kibana platform
[%collapsible]
====
...
If needed, you can disable this behavior by adding the `data-disable-core-navigation`
