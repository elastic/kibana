...
* In the migration function we then migrate the value of `consumer` to the value we want (`alerts` or `unknown`, depending on the current value). In this function we can assume that only documents with a `consumer` of `alerting` or `undefined` will be passed in, but it's still safest not to, and so we use the current `consumer` as the default when needed.
...
As we said above, an EncryptedSavedObject migration is a normal SavedObjects migration,
and so we can plug it into the underlying SavedObject just like any other kind of migration:
...
If your migration needs to change the type by, for example,
removing an encrypted field, you will have to specify the legacy type for the input.
...
This example shows how we provide a legacy type that describes the _input_ that needs to be decrypted.
...
Previously, workpad templates could be added through the Canvas API client side.
Workpad templates are now stored as saved objects, so there is no API needed for adding them.
Any additional templates can simply be added through `SavedObject` management.
...
This PR deprecates the front end search strategy concept and removes the
following API methods from the `data.search` plugin:
...
.DocLinks API exposed from start only
[%collapsible]
====
...
Plugins may now customize the logging configuration for their plugin's
logging context on the fly.
...
* Moves CONTRIBUTING content into our developer guide
* Cleans up the developer guide, removing outdated content, creating
a https://github.com/elastic/kibana/issues/41833#issuecomment-646195319[structure proposed
in this issue]
* Fixes https://github.com/elastic/kibana/issues/67782[this issue]
...
and will be deleted without any notice. Please, switch to the **core start** API instead.
...
The API reference docs for `state_sync` and `state_containers` available here:
...
The Elasticsearch service no longer provides separate `data` & `admin` clients.
The Elasticsearch service client is marked as deprecated and will superseded by a new one.
...
.Licensing uses {es} from start contract
[%collapsible]
====
...
The Actions SavedObject type `action` is now a hidden type.
Interaction with it requires that you tell your SavedObjectsClient to include
the `action` hidden type which can be done as follows:

```
core.savedObjects.getScopedClient(request, { includedHiddenTypes: ['action'] })
```

We prefer that you do not and use the `ActionsClient` as
that is the only way to ensure the correct authorization of user interactions with actions.
...
.Saved objects can accept a list of types
[%collapsible]
====
...
This can be used to create a client that has access to hidden types, like so:

```
core.savedObjects.getScopedClient(request, { includedHiddenTypes: ['hiddenType'] })
```

This would create a SavedObjects client scoped to a user by the specified
request with access to a hidden type called `hiddenType`.

====

[[breaking_79_eso_plugin]]
.The EncryptedSavedObject plugin no longer exposes a single client
[%collapsible]
====
...
This `getClient` can now also specify a list of hidden types to gain access to which are hidden by default.

For example, given a {kib} Platform plugin which has specified `encryptedSavedObjects` as a `Setup` dependency:
...
.The alerting API changed
[%collapsible]
====

The alerting API endpoints urls, bodies and params were changed to follow the {kib} styleguide.

Renamed `alerting` plugin to `alerts`.

* Changed actions `BASE_ALERT_API_PATH` to ` '/api/alerts'` - according to styleguide it should keep structure `/api/plugin_id`
* Changed endpoint `/api/alert/_find` just to `/api/alerts/_find`
* Changed `/types` to `/list_alert_types`
* Changes POST `/api/alert` to POST `/api/alerts/alert`
* Changed GET `/api/alert/{id} ` to GET   ` /api/alerts/alert/{id}`
* Changed PUT  `/api/alert/{id} `  PUT  `/api/alerts/alert/{id}`
...
.Implements New Platform API in Management
[%collapsible]
====
...
* Implements management landing page and sidebar in Management plugin,
its currently implemented here - src/legacy/core_plugins/kibana/public/management/index.js
* Removes legacy API from src/plugins/management/public/plugin.ts and related code
...
The Alerting SavedObject type `alert` is now a hidden type.
Interaction with it requires that you tell your `SavedObjectsClient` to include the `alert` hidden
type, which can be done as follows:

```
core.savedObjects.getScopedClient(request, { includedHiddenTypes: ['alert'] })
```

We prefer that you do not and use the `AlertsClient` as that is the only way to
ensure the correct authorization of user interactions with alerts.
...
.OSS features registration moved to KP
[%collapsible]
====

{kib} now allows the `Features` plugin `getFeatures` method to be called within the Start lifecycle.
...
SavedObject registration in the legacy platform is no longer supported.
Your plugin must be moved to the {kib} platform in order to use SavedObjects.
...
Links from an application to another are now automatically handled by the {kib} platform
...
If needed, this behavior can be disabled by adding the `data-disable-core-navigation`
attribute on the link (`a`) element or any of it's parent.
...
This makes the legacy bridge consistent with how true KP applications are mounted.
