...
your snapshots in the *Snapshots* view. You can drill down 
...
cluster or a cluster name. This structure enables you to
...
Use a {ref}/snapshot-lifecycle-management-api.html[snapshot lifecycle policy] to automate the creation and deletion 
of the snapshots of your cluster. Taking automatic snapshots:
...
*Create policy* wizard.  It walks you through defining the 
snapshots and repository, when to take snapshots, the settings you want, 
...
. Create the snapshot by entering the following in the editor pane. 
+
[source,js]
PUT /_snapshot/my_backup/2019-04-25_snapshot
...
. As you step through the wizard, enter the following values:
+
|===
|*Logistics* |
...
or leave in place to keep existing settings. are backed up.
...
then you’re navigated to *Restore Status*, 
