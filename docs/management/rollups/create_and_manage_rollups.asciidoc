...
First, create the rollup index pattern in *Management > Index Patterns* so you will be
able to select your rolled up data for visualizations.

. Select Create index pattern and choose *Rollup index pattern*.
...
Using the information from the example rollup configuration, create a vertical
bar chart.  You can use `rollup_logstash` to match the rolled up index pattern,
...
For this example, choose `rollup_logstash,kibana_sample_data_logs` to see both
the raw and rolled up data.
