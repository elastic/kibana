...
* All started {dfeeds} are stopped. All open {anomaly-jobs} are closed.
...
* Pipelines cannot be created, edited, viewed, listed, or deleted from the UI.
* Cannot list or view existing pipelines from the UI.
* Logstash instances that are registered to listen to existing pipelines cannot run.
