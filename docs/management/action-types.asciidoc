...
also enables you to filter to a subset of connector types. For example:
...
New connectors can be created by clicking the *Create connector* button, which
will guide you to select the type of connector and configure its properties. For
example:

[role="screenshot"]
image::images/connector-select-type.png[Connector select type]

Refer to <<action-types>> for the types of connectors available and how to
configure them. After you create a connector, it is available for use any time
you set up an action in the current space.
