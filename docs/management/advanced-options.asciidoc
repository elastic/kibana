...
`courier:ignoreFilterIfFieldNotInIndex`:: Useful when dashboards consist of visualizations from multiple index patterns, this setting will skip filters that apply to fields that don't exist in the visualization's index.
...
`timelion:showTutorial`:: When enabled, the Timelion tutorial will be shown when a user first opens Timelion.
