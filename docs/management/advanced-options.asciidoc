...
`search:queryLanguage`:: Default is `lucene`. Query language used by the query bar. Choose between the lucene query syntax and KQL, an experimental new language built specifically for Kibana.
