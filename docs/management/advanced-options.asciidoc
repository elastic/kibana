...
[horizontal]
`courier:batchSearches`:: Determines whether to send multiple concurrent search requests (such as from a dashboard from
multiple panels) as a single `_msearch` request, or multiple `_search` requests. Note that cancellation of queries (for
example, when a user navigates away from the page before the search has returned) is only supported when using the
`_search` Elasticsearch endpoint.
