...
When enabled, documents will be loaded and displayed using `_source`. This is soon going to be deprecated. 
When disabled, the new `fields` parameter of Elasticsearch search service is being used. This way runtime, alias and 
multi fields can be displayed.
