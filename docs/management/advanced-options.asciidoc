...
Limits the density of histograms in *Lens* and aggregation-based visualizations
for better performance by making a test query before the main query.
If the test query would produce more buckets than this setting allows,
the interval between buckets will be increased. If multiple aggregations are
used in each chart, this setting is applied separately to each one.
...
Limits the density of the *TSVB* histogram for better performance by making a
test query before the main query. If the test query would produce more buckets
than this setting allows, the interval between buckets will be increased.
To find the maximum value of this setting, divide the {es} `search.max_buckets`
value by the maximum number of "group by" buckets that you will allow in *TSVB*.
