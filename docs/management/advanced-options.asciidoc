...
[horizontal]
`securitySolution:defaultAnomalyScore`:: The threshold above which Machine Learning job anomalies are displayed in the SIEM app.
`securitySolution:defaultIndex`:: A comma-delimited list of Elasticsearch indices from which the SIEM app collects events.
...
`securitySolution:enableNewsFeed`:: Enables the security news feed on the SIEM *Overview*
page.
`securitySolution:newsFeedUrl`:: The URL from which the security news feed content is
retrieved.
`securitySolution:refreshIntervalDefaults`:: The default refresh interval for the SIEM time filter, in milliseconds.
`securitySolution:timeDefaults`:: The default period of time in the SIEM time filter.
