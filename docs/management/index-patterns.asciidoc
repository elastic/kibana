...
When creating a name for an index pattern be sure that the name is unique. Index
patterns can not have the exact same name.
...
index pattern for rolled up data the same way you do for any data.  The name 
must be unique.
