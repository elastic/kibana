...
This tells {kib} which {es} indices contain the data that you want to work with. 
An index pattern can match a single index, multiple indices, and a rollup index. 

[float]
[[index-patterns-read-only-access]]
=== [xpack]#Read-only access#
If you have insufficient privileges to create or save index patterns, the following 
indicator in Kibana is displayed. The buttons to create new index patterns or save
...
To get started, go to *Management > Kibana > Index Patterns*.  Your index 
patterns show up here, including any that were created when you added a sample data set. 

You can create a standard index pattern, or if a rollup index is detected in the 
...
You can match multiple indices to create a combination index pattern.  
...
* Present your data in charts, tables, gauges, tag clouds, and more in Visualize.
* Show off your data in a Canvas presentation.
* If your data includes geo data, visualize it using Maps.

For a walkthrough of creating an index pattern and visualizing the data, 
see <<getting-started, Getting Started>>. 

[float]
==== Rolled up index pattern

Clicking *Create index pattern* includes an item for creating a rollup 
index pattern, if a rollup index is detected in the cluster. You create an 
...
only one rolled up index, not multiple. There is no restriction on the 
...
[float]
=== Managing your index patterns

Once you’ve created an index pattern, you’re presented a list of all fields 
and associated data type in the index.  

[role="screenshot"]
image:management/index-patterns/images/new-index-pattern.png["Index files and data types"]

From this page, you can perform actions on the index pattern using the 
controls in the upper right. You can:

* *Make it the default index pattern.* {kib} uses a badge to make users 
...
you’ve used most often within {kib} and are used to sort fields within lists.
