...
and associated data types in the index.  Click a column header to sort the table by that column. 
Use the field dropdown menu to limit to 
display to a specific field. 
...
[float]
=== Set as default, reload, and delete

From the index page, you can perform actions on the index pattern
