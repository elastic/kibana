...
You can optionally configure a repeating schedule.

//TO-DO: Add automated screenshot

A maintenance window can have any one of the following statuses:

- `Upcoming`: It will run at the scheduled date and time.
- `Running`: It is running.
- `Finished`: It ended and does not have a future repeating schedule.
- `Archived`: It ended and is queued for deletion.