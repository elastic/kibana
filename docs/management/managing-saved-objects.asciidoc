...
or indicate that you want the object to be shared to _All spaces_, which includes those that exist now and any new ones that are creatd in
the future.

Note that not all saved object types are shareable. If an object is shareable, the Spaces column will show which space(s) it exists in. You
can also click those space icons to open the Share UI.
