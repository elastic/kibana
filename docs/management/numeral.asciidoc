[[numeral]]
== Elastic Numeral Formatting

Number formatting in {kib} is done through a pattern syntax that {kib}
maintains. These patterns are originally based on Numeral.js, but this
page contains complete documentation for use in {kib}.

The simplest pattern format is `0`, and the default {kib} pattern is `0,0.[000]`.
These highlight the composability of the pattern syntax, which gives you control
over:

Number of decimal places:: The pattern `0.0` will always show one decimal in the locale-specific way. The patterns `0[.]0`, `0.[0]`, `0.0[0]` allow for optional display of decimal places.
Thousands separator:: The thousands separator `,` will be displayed as a locale-specific
separator.
...
The display of these patterns is affect by the <<kibana-general-settings, advanced setting>> `format:number:defaultLocale`.
...
| 10000.23 | 0.0 | en (English) | 10000.0
| 10000.23 | 0,0.0 | fr (French) | 10 000,0
| 10000.23 | 0,0[.]0 | en (English) | 10,000
| 10000.23 | 0.0[0] | en (English) | 10,000.23
...
The bytes and bits formatter take a number representing bytes or bits and
display scaled values. The base number formatting options are the same as above.
