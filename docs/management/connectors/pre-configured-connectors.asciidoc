...
The following example creates a <<xmatters-action-type,xMatters connector>> with basic authentication:
...
<3> A user name for authentication, whic his required when `usesBasic` is `true`.
