...
On the Azure portal, open *Manage > API permissions*, and then define the permissions for the registered application to send emails. Refer to the https://docs.microsoft.com/en-us/graph/api/user-sendmail?view=graph-rest-1.0&tabs=http#permissions[documentation] for the Microsoft Graph API.
...
Confirm the grant. The status for the "Mail.Send" permission changes to granted.
[role="screenshot"]
...
To configure the Client secret , open *Manage > Certificates & secrets* on the Azure portal.
[role="screenshot"]
image::management/connectors/images/exchange-secrets.png[MS Exchange secrets configuration]

Add a new client secret, then copy the value and put it to the proper field in Microsoft Exchange email connector.

[float]
[[exchange-client-tenant-id]]
===== Configure Microsoft Exchange Client ID and Tenant ID
To find application Client ID,  open the *Overview* page on the Azure portal.
[role="screenshot"]
