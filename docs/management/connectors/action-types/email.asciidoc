...
New Elastic Cloud deployments will contain a preconfigured connector
for the Elastic Cloud email service, named **Elastic-Cloud-SMTP**, with the
settings below.  If your deployment is older and does not contain this 
preconfigured connector, you can create a connector manually using the 
following settings:

Sender::
`alerts-noreply@elastic.co`
