...
* (If using OAuth) Create an RSA keypair and add an X.509 Certificate.
* (If using OAuth) Create an OAuth JWT API endpoint for external clients with a JWT Verifiers Map.
...
. Use OpenSSL[https://www.openssl.org/docs/man1.0.2/man1/genrsa.html] to generate an RSA private key:
...
----
<1> Use the `passout` option to set a password on your private key. This is optional but remember your password if you use one!

. Use OpenSSL[https://www.openssl.org/docs/man1.0.2/man1/req.html] to generate the matching public key:
...
. In your {sn} instance, go to *Certificates*.
. Click *New*.
...
. In your {sn} instance, go to *Application Registry*.
. Click *New*.
...
. Retrieve the *Client ID*, *Client Secret* and *JWT Key ID*. You will need these values to create your {sn} connector.
...
Private Key Password:: The password for the RSA private key generated during setup, if any.
