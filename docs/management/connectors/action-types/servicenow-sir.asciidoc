...
NOTE: Using the default configuration of {{ruleID}}:{{alert ID}} ensures that ServiceNow will create a separate incident for every generated alert that uses a unique alert ID. In other words, if the rule generates multiple alerts that all use the same alert IDs, ServiceNow will create a single ticket for that alert and continually update it.
