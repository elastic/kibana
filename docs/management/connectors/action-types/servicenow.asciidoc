...
==== Prerequisites
After upgrading from {stack} version \<=7.15 to >= 7.16, you must complete the following within your {sn} instance before creating a new {sn} ITSM connector or <<servicenow-itsm-connector-update, updating an existing one>>:
...
. Click *New*.
. Configure the rule settings to use the Elastic ITSM API and the Kibana URL:
...
{sn} ITSM connectors created in {stack} version \<=7.15 are marked as deprecated after you upgrade to >=7.16. Deprecated connectors have a yellow icon after their name and display a warning message when selected.
