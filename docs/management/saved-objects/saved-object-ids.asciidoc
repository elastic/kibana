...
Every time you upgrade the {kib} server to a new version, <<saved-object-migrations,saved objects are migrated to a new format>>. When you
...
After you upgrade, or if you set up a brand new {kib} server using 8.x, there are a few more things that behave differently.

[[saved-object-ids-impact-when-using-legacy-urls]]
===== Accessing saved objects using old URLs

When you upgrade {kib} and saved object IDs change, the "deep link" URLs to access those saved objects will change too. To reduce the impact
...
NOTE: When you import a saved object and it is created with a different ID, it will not be 
