...
Use the following command to install a plugin from `download.elastic.co`

[source,shell]
bin/kibana plugin --install <package>/<version>

You can also use `-i` instead of `--install`, as in the following example:

[source,shell]
bin/kibana plugin -i marvel/latest

This will attempt to download the plugin from `download.elastic.co`.


[float]
=== Installing Plugins from an Arbitrary, or Github URL
