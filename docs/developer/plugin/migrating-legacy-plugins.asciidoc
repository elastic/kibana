...
|===
|Legacy Platform |{kib} Platform |Notes
|`chrome.addBasePath`
...
|Deprecations from {kib} Platform are not applied to legacy configuration
...
|`publicDir` |`N/A`. {kib} Platform serves static assets from `/public/assets`
folder under `/plugins/{id}/assets/{path*}` URL.

|`preInit`, `init`, `postInit` |`N/A`. Use {kib} Platform <<plugin-lifecycles,plugin-lifecycles>>
|===
