...
=== Create a {kib} Platform plugin

For example, if wanted to move the legacy `demoplugin` plugin's
configuration to the {kib} Platform, we could create {kib} Platform plugin with the
...
In the {kib} Platform, all these registrations are to be performed
