...
If your plugin isn't server only and contains `ui` in order for Kibana to pick the browser bundles you need to run `yarn dev --watch` in the plugin root folder.

After, in another terminal, run `yarn start` in the {kib} root folder. Make sure {kib} found and bootstrapped your plugin:
["source","shell"]
-----------
