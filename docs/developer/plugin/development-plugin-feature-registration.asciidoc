...
|The set of sub-features to enable granular access control beyond the `all` and `read` feature privileges. These options will only be available to clusters running a Gold license or better.
...
Discover takes advantage of sub-feature privileges to allow fine-grained access control. In this example,
a single "Create Short URLs" sub-feature privilege is defined, which allows users to grant access to this feature without having to grant the "All" privilege to Discover. To say another way, you can grant "Read" access to Discover, but _additionally_ grant the ability to create Short URLs.
