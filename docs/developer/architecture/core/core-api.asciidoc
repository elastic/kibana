...
Kibana Platform provides a set of low-level API required to run all the Kibana plugins.
API is provided as a part of plugin lifecycle contract and may be invoked during an appropriate lifecycle only:
...
Access to Kibana config in Kibana Platform has been subject to significant refactoring.
Config service does not provide access to the whole config anymore.
Kibana platform plugin cannot read configuration parameters of the core services nor other plugins directly.
...
if not specified) in `kibana.json` manifest file.
...
If your plugin has deprecated properties, you can describe them using
...
include::saved-objects-service.asciidoc
...
==== UI settings service
The program interface to <<advanced-options>>
It makes it possible for Kibana plugins to extend Kibana UI Settings Management with custom settings.

See {kib-repo}blob/{branch}/docs/development/core/server/kibana-plugin-core-server.uisettingsservicesetup.register.md[UI settings service Setup API docs]
See {kib-repo}blob/{branch}/docs/development/core/server/kibana-plugin-core-server.uisettingsservicestart.register.md[UI settings service Start API docs]
...
=== Client-side
==== Application service
Kibana has migrated to be a Single Page Application. Plugins should use `Application service` API to instruct Kibana what an application should be loaded & rendered in UI in response to user interactions.
[source,typescript]
----
...
Note that you are free to use any UI library to render a plugin application in DOM.
...
==== Elasticsearch service
Is not available in the browser. Use {kib-repo}blob/{branch}/docs/development/plugins/data/server/kibana-plugin-plugins-data-public.md[Data plugin] instead.
