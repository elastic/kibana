...
When upgrading to a new major version of Node.js, the following extra steps must be performed:
  ...
  - Compare the new Node.js versions list of supported platforms with the https://www.elastic.co/support/matrix#matrix_os[Kibana Support Matrix].
    ...
    If Node.js has dropped support for platform still supported by Kibana, appropriate steps must be taken as soon as possible to deprecate support for this platform, so that support for it can be dropped before the current major version of Node.js reaches End-of-Life.
