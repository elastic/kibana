...
[TIP]
====
You can also mark your object as
https://github.com/elastic/kibana/blob/{branch}/docs/development/core/server/kibana-plugin-core-server.savedobjectstypemanagementdefinition.md[importable and exportable]:

```json
management: { isImportableAndExportable: true },
```
...
- Change usage of saved object delete API, and notify users of affected spaces and include the `force: true` option

[[sharing-saved-objects-step-8]]
==== Step 8

> *Update your _client-side code_ to add reusable components*

WIP

- Spaces list
- Share to space flyout
- Nothing else?
