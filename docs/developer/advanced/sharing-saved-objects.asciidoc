...
will not affect end-users _if_ consumers implement the required steps below.*
...
Note that some URLs may contain deep links to multiple objects, for example, a Dashboard _and_ a filter for an Index Pattern.
...
==== 1. Why do object IDs need to be changed?
...
==== 3. Why are there three different resolve outcomes?
...
If an object was converted in the Default space, then you tried to resolve it with its ID, you would have an `'exactMatch'`:

image::images/sharing-saved-objects-faq-resolve-outcomes-1.png["Sharing Saved Objects resolve outcome 1 (exactMatch)"]

*Outcome 2*

If an object was converted in a non-Default space, then you tried to resolve it with its old ID, you would have an `'aliasMatch'`:

image::images/sharing-saved-objects-faq-resolve-outcomes-2.png["Sharing Saved Objects resolve outcome 2 (aliasMatch)"]

*Outcome 3*

If two objects were converted in the Default space and a non-Default space, _then_ you shared one from the Default space to a non-Default
space, _then_ you tried to resolve it in the non-Default space with its old ID, you would have a `'conflict'`:

image::images/sharing-saved-objects-faq-resolve-outcomes-3.png["Sharing Saved Objects resolve outcome 3 (conflict)"]

This last scenario is contrived, and we actually have controls in place to prevent this from happening when you share, import, or copy
...
==== 4. Why are there both "share-capable" and "shareable" object types?
...
==== 5. What if one page has deep links to multiple objects?
