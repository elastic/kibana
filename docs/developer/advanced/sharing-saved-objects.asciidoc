...
edit the object's assigned spaces. You may want to follow the example of the
https://github.com/elastic/kibana/blob/{main}/src/plugins/data_view_management/public/components/index_pattern_table/spaces_list.tsx[data
view management page] and combine these into a single component so that the space list can be clicked to show the flyout:
+
```tsx
...
const shareToSpaceFlyoutProps: ShareToSpaceFlyoutProps = {
  ...
  onUpdate: () => { /* callback when object is updated */ },
  onClose: () => {
    setShowFlyout(false);
  },
};
