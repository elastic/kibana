...
Enable pnpm via corepack (bundled with Node.js) and activate the version pinned in `package.json` `engines.pnpm`:
...
In case you don't have an internet connection, the `pnpm kbn bootstrap` command will 
...
If you have failures during `pnpm kbn bootstrap` you may have some
corrupted packages in your pnpm store which you can clean with:
