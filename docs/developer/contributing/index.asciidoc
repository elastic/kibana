...
Whether you want to fix a bug, implement a feature, add an improvement, add APIs, the following sections will 
...
. Label the PR with the appropriate GitHub labels:
  ...
  * `release_note:fix` — Fixed bugs from the previous release. 
  * `release_note:deprecation` — Deprecated functionality from the previous release.
