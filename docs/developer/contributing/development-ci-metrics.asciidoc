...
The number of separate modules included in each bundle/plugin. This is the best indicator we have for how long a specific bundle will take to be built by the `@kbn/optimizer`, so we report it to help people know when they've imported a module which might be include a surprising number of modules.
...
[[ci-metric-oss-distributable-size]] `oss distributable size` (not reported) ::
