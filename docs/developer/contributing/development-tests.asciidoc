...
|`pnpm test:jest [test path]`

|Jest (integration) |`**/integration_tests/**/*.test.{js,mjs,ts,tsx}`
|`pnpm test:jest_integration [test path]`

|Functional
|`test/**/config.js` `x-pack/platform/test/**/config.js`
...
[source,bash]
----
pnpm exec jest --config src/platform/plugins/shared/dashboard/jest.config.js
----

A script is available to provide a better user experience when testing while navigating throughout the repository. To run the tests within your current working directory, use `pnpm test:jest`. Like the Jest CLI, you can also supply a path to determine which tests to run.

[source,bash]
----
kibana/src/platform/plugins/shared/dashboard/server$ pnpm test:jest #or
kibana/src/platform/plugins/shared/dashboard$ pnpm test:jest server #or
kibana$ pnpm test:jest src/platform/plugins/shared/dashboard/server
----
...
[source,bash]
----
kibana/src/platform/plugins/shared/dashboard/server$ pnpm test:jest --coverage
...
* Run {kib} with `pnpm start --host=computer.local` (substituting
your computer name).
