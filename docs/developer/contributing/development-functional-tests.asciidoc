...
The tests are written in https://mochajs.org[mocha] using https://github.com/elastic/kibana/tree/main/src/dev/packages/expect[@kbn/expect] for assertions.
...
In order to achieve this a new service was introduced called `createTestUserService` (see `src/dev/packages/ftr-common-functional-ui-services/services/security/test_user.ts`). The purpose of this test user service is to create roles defined in the test config files and setRoles() or restoreDefaults().
...
This annotated example file shows the basic structure every test suite uses. It starts by importing https://github.com/elastic/kibana/tree/main/src/dev/packages/expect[`@kbn/expect`] and defining its default export: an anonymous Test Provider. The test provider then destructures the Provider API for the `getService()` and `getPageObjects()` functions. It uses these functions to collect the dependencies of this suite. The rest of the test file will look pretty normal to mocha.js users. `describe()`, `it()`, `before()` and the lot are used to define suites that happen to automate a browser via services and objects of type `PageObject`.
...
**log:**:::
// * Source: {kibana-blob}src/dev/packages/dev-utils/src/tooling_log/tooling_log.js[src/dev/packages/dev-utils/src/tooling_log/tooling_log.js]
...
**retry:**:::
// * Source: {kibana-blob}src/dev/packages/ftr-common-functional-services/services/retry/retry.ts[src/dev/packages/ftr-common-functional-services/services/retry/retry.ts]
...
**kibanaServer:**:::
// * Source: {kibana-blob}src/dev/packages/ftr-common-functional-services/services/kibana_server/kibana_server.ts[src/dev/packages/ftr-common-functional-services/services/kibana_server/kibana_server.ts]
...
**esArchiver:**:::
// * Source: {kibana-blob}src/dev/packages/ftr-common-functional-services/services/es_archiver.ts[src/dev/packages/ftr-common-functional-services/services/es_archiver.ts]
...
**Low-level utilities:**:::
* es
// ** Source: {kibana-blob}src/dev/packages/ftr-common-functional-services/services/es.ts[src/dev/packages/ftr-common-functional-services/services/es.ts]
** {es} client
