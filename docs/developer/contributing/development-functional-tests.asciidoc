...
The `FunctionalTestRunner` is very bare bones and gets most of its functionality from its config file, located at {kibana-blob}test/functional/config.js[test/functional/config.js] or {kibana-blob}x-pack/test/functional/config.js[x-pack/test/functional/config.js]. If you’re writing a plugin outside the {kib} repo, you will have your own config file.
