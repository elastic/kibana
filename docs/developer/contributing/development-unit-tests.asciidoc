...
Each plugin and package contains it's own `jest.config.js` file to define it's root and any overrides 
to the jest-preset provided by the `@kbn/test` package. When working on a single plugin or package, you will 
find it's more efficient to supply its Jest configuration file when running.
