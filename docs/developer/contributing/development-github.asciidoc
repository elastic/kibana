...
* `main` is the next major version.
...
* Breaking changes do not get backported and only go into `main`.
...
Here's how you should rebase `main` onto your branch, and how to fix
...
Then, check out your branch and rebase `main` on top of it, which will
apply all of the new commits on `main` to your branch, and then apply
...
that doesn't destroy work that's been done in `main`. Refer to the `main`
