...
* Changes backported to a `<major>.<minor>` branch must be justified based on the severity and confidence of the change and the reasoning included in the pull request description.
