...
* Bugs can be backported to a `<major>.<minor>` branch if the changes are safe and appropriate. This is a judgement call you make based on factors like the bug's severity, test coverage, your confidence in the changes, etc.  Your reasoning should be included in the pull request description.
