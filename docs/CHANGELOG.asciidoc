...
Stack connectors::
The email connector now verifies TLS certificates on HTTPS proxies and OAuth2 token endpoints by default. Connections to hosts with self-signed, expired, or hostname-mismatched certificates that previously succeeded will now fail. To opt out, set `rejectUnauthorized: false` for the affected host in `xpack.actions.customHostSettings`. Hosted SMTP with public certificate authorities is unaffected ({kibana-pull}283150[#283150]).
...
* Fix Fleet to restrict cluster privileges in package manifests to an allowlist before writing agent API key permissions ({kibana-pull}286084[#286084]).
* Fix Fleet Kafka output to accept multi-format-string patterns entered directly in the dynamic topic free-text field ({kibana-pull}285581[#285581]).
Developer tools::
* Fix duplicate Cut, Copy, and Paste actions in the Dev Tools Console context menu ({kibana-pull}286704[#286704]).
...
Kibana platform::
* Fix PKI session invalidation when in-flight HTTP/2 requests are cancelled ({kibana-pull}285153[#285153]).
* Fix Kibana to detect a misconfigured audit log file path on startup and report a degraded state instead of crashing ({kibana-pull}282347[#282347]).
Machine Learning::
