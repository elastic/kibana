...
.Kibana server crashes if you use model management features without ingest pipelines
[%collapsible]
====
*Details* +
If you access *{ml-app} > Model Management* and there are no ingest pipelines
defined, the Kibana server crashes.

*Impact* +
To avoid this issue, create a simple {ref}/ingest.html[ingest pipeline] before
accessing the *Model Management* page. The ingest pipeline does not need to be
used; it just needs to exist to prevent the crash. This problem will be fixed in
8.0.0-rc1.
====
...
[discrete]
[[breaking-113516]]
.Removes fontAwesome usage
[%collapsible]
====
*Details* +
fontAwesome has been removed from the vislib plugin. For more information, refer to {kibana-pull}113516[#113516].

*Impact* +
The *Display warnings* option is no longer available in the legacy Gauge visualization.
====
...
====
*Details* +
All deprecated visEditors plugin settings have been removed. For more information, refer to {kibana-pull}112643[#112643].

*Impact* +
Use the new settings in kibana.yml.
====
...
[discrete]
[[breaking-110985]]
.Removes less support from markdown editor
[%collapsible]
====
*Details* +
`markdown_less` has been removed from saved_object, and markdown id has been removed from `markdown_css`. For more information, refer to {kibana-pull}110985[#110985].

*Impact* +
A migration script is used for `markdown_less` and `markdown_css`, and `@emotion/react` is now used to encapsulate styles inside markdown instead of using the ID.
====
...
[discrete]
[[breaking-113653]]
.Removes unused and renamed deprecated core settings and deprecated settings from core plugins
[%collapsible]
====
*Details* +
...
*Impact* +
* `newsfeed.defaultLanguage` -- Newsfeed items are retrieved based on the browser locale and default to English
...
*Impact* +
Before you upgrade to 8.0.0, remove `{plugin_name}.enabled` from kibana.yml. If you use the setting to control user access to {kib} applications, use <<tutorial-secure-access-to-kibana,*Features* controls>> instead.
====
...
====
*Details* +
The `so/server/es` settings have been removed. If your configuration uses the following settings, {kib} fails to start:
...
*Impact* +
Use spaces, cross-cluster replication, or cross-cluster search. To migrate to <<xpack-spaces,spaces>>, export your <<managing-saved-objects, saved objects>> from a tenant into the default tenant in a space. For more details, refer to link:https://github.com/elastic/kibana/issues/82020[#82020].
====
...
*Impact* +
To audit {kib} events, enable the <<xpack-security-ecs-audit-logging, ECS audit logger>>.
====
...
Machine Learning::
* Enhancements for the Model Management page {kibana-pull}115772[#115772]
Platform::
...
Machine Learning::
* Adds support for installing ML models {kibana-pull}107710[#107710]
