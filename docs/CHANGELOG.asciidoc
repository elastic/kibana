...
.<<search-sessions,Search Sessions>> is deprecated in 8.15.0 and will be removed in a future version.
[%collapsible]
====
*Details* +
Search Sessions is now deprecated and will be removed in a future version. By default, queries that take longer than 10 minutes (the default for the advanced setting `search:timeout`) will be canceled. To allow queries to run longer, consider increasing `search:timeout` or setting it to `0` which will allow queries to continue executing as long as a user is waiting on-screen for results.
====
