...
.*Discover* now uses the fields API
[%collapsible]
====
*Details* +
In 7.12.0, *Discover* uses the {ref}/search-fields.html#search-fields-param[fields API] by default.
Reading from `_source` is still supported through a switch in *Advanced Settings*.
This change stems from updates made to {es} in 7.11 with the extension of
the search API to include the new {ref}/search-fields.html#search-fields-param[fields] parameter.

*Impact* +
Refer to https://www.elastic.co/blog/discover-uses-fields-api-in-7-12[this blog]
for more information on runtime fields in *Discover*,
including the switch to using the fields API.
====
