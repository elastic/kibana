...
=== Fixes
Alerting::
* Fix alerting resource installation using unbounded concurrency, which could exhaust {kib} heap when many contexts and namespaces install at once ({kibana-pull}287643[#287643]).
* Fix the email connector HTTP test sending caller-supplied subject and message content. The test now sends fixed content ({kibana-pull}288664[#288664]).
Dashboards and Visualizations::
* Increase the TinyMath expression length limit so longer visualization formulas no longer fail ({kibana-pull}290314[#290314]).
* Fix Canvas functions API creating data tables with more than 50 columns per row ({kibana-pull}287935[#287935]).
Data ingestion and Fleet::
* Fix deleting a managed integration policy leaving the agent listed as active ({kibana-pull}290181[#290181]).
* Fix Elastic Defend metadata transforms accumulating as duplicates in {es} across stack upgrades ({kibana-pull}288379[#288379]) ({kibana-pull}288902[#288902]).
* Fix comment-only values in {fleet} agent policy advanced YAML settings leaving agents stuck on an outdated policy ({kibana-pull}288430[#288430]).
Data management::
* Fix Index Management moving component templates listed in `ignore_missing_component_templates` to the end of `composed_of` when you edit or clone an index template ({kibana-pull}290171[#290171]).
Developer tools::
* Fix Console treating request-like lines inside an unterminated triple-quoted string as standalone requests ({kibana-pull}287547[#287547]).
* Fix Console body autocomplete preferring a same-name global rule over an explicit field rule ({kibana-pull}286703[#286703]).
Elastic Observability solution::
* Add a confirmation modal in Synthetics **Alerting defaults** settings when disabling an active default status or TLS rule. Custom synthetics rules are not affected ({kibana-pull}287914[#287914]).
* Add a **Can manage rules** Synthetics sub-feature privilege so users with Synthetics **Read** can create and manage Synthetics and Uptime alert rules without monitor write access ({kibana-pull}289952[#289952]).
* Add a **Can run tests manually** Synthetics sub-feature privilege so users with Synthetics **Read** can run a test without monitor write access ({kibana-pull}282149[#282149]).
* Fix Custom threshold and Metric threshold grouped alerts recovering when **If there is no data** is set to **Do nothing** ({kibana-pull}289623[#289623]).
* Fix Observability AI Assistant hanging on **Help me understand this alert** for custom threshold alerts that have no entity filters ({kibana-pull}290244[#290244]).
* Fix a 500 error when hovering over AWS RDS instances in the **Infrastructure Inventory** map tooltip, caused by unquoted ARN identifiers containing `:` characters being misinterpreted as KQL field-value separators ({kibana-pull}288209[#288209]).
* Fix Synthetics global parameters shared across all spaces being dropped from private location monitor configurations during all-spaces sync ({kibana-pull}282575[#282575]).
Elastic Security solution::
For the Elastic Security 8.19.22 release information, refer to {security-guide}/release-notes.html[_Elastic Security release notes_].
Elasticsearch solution::
* Add an **Expand role members** toggle to ServiceNow connectors for compact document-level security ({kibana-pull}288071[#288071]).
% !!DEFERRED!! Not yet confirmed in build candidate 8.19.22-022cc937 (backport merged after BC cutoff). Re-verify against the next BC before publishing.
% * Add an **Expand site group members** toggle to SharePoint Online connectors for compact document-level security ({kibana-pull}288070[#288070]).
Kibana platform::
* Hide the **Activate user** action on deactivated users when you have read-only access ({kibana-pull}289801[#289801]).
* Fix PKI authentication sessions being invalidated when in-flight HTTP/2 requests are canceled, which logged you out. This affects deployments with `server.protocol: http2` ({kibana-pull}285153[#285153]).
* Fix {kib} failing to start when the file-based audit log appender cannot write. {kib} reports a degraded state instead ({kibana-pull}282347[#282347]).
* Fix a failed audit log write after startup crashing {kib}. {kib} now reports a degraded state and turns the audit logger off instead ({kibana-pull}287360[#287360]).
% !!DEFERRED!! Not yet confirmed in build candidate 8.19.22-022cc937 (backport merged after BC cutoff). Re-verify against the next BC before publishing.
% * Fix {kib} shutting down when the product documentation installer downloads a corrupt or incomplete artifact ({kibana-pull}291142[#291142]).
...
The 8.19.20 release includes the following fixes.

[float]
[[fixes-v8.19.20]]
=== Fixes
Data ingestion and Fleet::
* Fix Kafka outputs incorrectly accepting a proxy configuration, which Kafka doesn't support ({kibana-pull}282313[#282313]).
* Fix preconfigured Fleet proxies being rewritten on every {kib} startup even when unchanged, which needlessly incremented dependent agent policies ({kibana-pull}281059[#281059]).
* Fix Fleet package policy bulk creation to store secrets in the secrets index instead of inline on the saved object, matching the existing create, update, and bulk update behavior ({kibana-pull}282213[#282213]).
* Fix the create enrollment API key request to preserve an existing expiration value when creating the key in {es} security ({kibana-pull}281324[#281324]).
Data management::
* Fix ILM Explain in Index Management to include hidden and system indices when filtering by lifecycle phase ({kibana-pull}282137[#282137]).
* Fix Cross-Cluster Replication to clear selected follower indices after a pause, resume, or unfollow action, so the Manage menu reflects the updated state ({kibana-pull}273576[#273576]).
Elastic Observability solution::
* Fix the Synthetics monitors overview failing to load maintenance window status for users with read-only access ({kibana-pull}281894[#281894]).
Elastic Security solution::
For the Elastic Security 8.19.20 release information, refer to {security-guide}/release-notes.html[_Elastic Security release notes_].
Machine Learning::
* Fix the trained model infer, update, and stop APIs to prevent a deployment ID from being used to access a different trained model ({kibana-pull}280511[#280511]).
* Fix the annotation and anomaly result search APIs to enforce job-level space access checks, and require a job ID on anomaly searches to prevent bypassing those checks ({kibana-pull}277197[#277197]).
* Fix {ml} saved objects APIs to accept any one of the anomaly detection, data frame analytics, or trained models privileges instead of requiring all three, and correctly check trained models access when removing a saved object from a space ({kibana-pull}276936[#276936]).
...
=== Enhancements
Elastic Observability solution::
* Ensures APM integration policy updates retain API keys when clients omit them ({kibana-pull}274647[#274647]).
* Speeds up loading Synthetics journey screenshots and step details ({kibana-pull}273513[#273513]).
* Honors the `observability:searchExcludedDataTiers` advanced setting in Synthetics searches so operators can exclude slow data tiers such as `data_frozen` ({kibana-pull}273418[#273418]).
Elasticsearch solution::
* Adds a MongoDB connector `datetime_conversion` setting so syncs can clamp, convert, or error on dates outside the supported range instead of always aborting ({kibana-pull}277685[#277685]).

[float]
[[fixes-v8.19.19]]
=== Fixes
Alerting::
* Defaults a missing reporting job `version` parameter to the current {kib} version instead of showing a hardcoded `7.14.0` value ({kibana-pull}275469[#275469]).
Dashboards and Visualizations::
* Limits the Canvas `createTable` expression `rowCount` to a maximum of `10000`, matching the default {es} `index.max_result_window` ({kibana-pull}276502[#276502]).
Data ingestion and Fleet::
* Fixes resetting preconfigured agent policies aborting with a 404 on deployments where space awareness is disabled ({kibana-pull}276742[#276742]).
Developer tools::
* Fixes Console autocomplete so nested object fields and multi-fields in index mapping bodies suggest mapping options again ({kibana-pull}278508[#278508]).
* Fixes Console requests that combine comments with triple-quote strings failing with `x_content_e_o_f_exception` ({kibana-pull}277259[#277259]).
* Fixes a Windows-specific code editor issue that could corrupt multi-line edited text after refresh ({kibana-pull}277007[#277007]).
* Fixes the Console **Copy** button silently failing to copy response output ({kibana-pull}276365[#276365]).
* Fixes Console autocomplete matching an incorrect API endpoint when a URL matches both a literal path and a parameterized path ({kibana-pull}276239[#276239]).
* Improves Console {esql} source autocomplete by reusing a fresh source list instead of refetching it on every keystroke ({kibana-pull}275535[#275535]).
Elastic Observability solution::
* Fixes Synthetics monitor updates ignoring `retest_on_failure: false`, which left retests enabled on existing monitors ({kibana-pull}277701[#277701]).
Elastic Security solution::
For the Elastic Security 8.19.19 release information, refer to {security-guide}/release-notes.html[_Elastic Security release notes_].
Kibana platform::
* Fixes `kibana-frontend` page-load transaction names to use low-cardinality `/app/{appId}` labels instead of raw URLs ({kibana-pull}275650[#275650]).
Machine Learning::
* Prevents removing all spaces from a {ml} job or trained model through the spaces update APIs ({kibana-pull}276490[#276490]).
* Restricts the clear audit messages API so users can only clear messages for jobs they can access, and only against valid notification indices ({kibana-pull}276419[#276419]).
* Adds request size limits to the file upload and `index_exists` endpoints ({kibana-pull}274566[#274566]).

[[release-notes-8.19.18]]
== {kib} 8.19.18

The 8.19.18 release includes the following fixes.

[float]
[[fixes-v8.19.18]]
=== Fixes
Alerting and cases::
* Fixes case pushes only stamping a subset of attachments, which caused the alert count comment to be duplicated in the external system on every push ({kibana-pull}274451[#274451]).
Dashboards and Visualizations::
* Fixes **Canvas** autoplay not advancing past the first page ({kibana-pull}272619[#272619]).
Data ingestion and Fleet::
* Improves `POST /api/fleet/setup` performance for deployments with a large number of configured outputs by no longer decrypting every output on each call ({kibana-pull}273848[#273848]).
* Fixes a crash in the **Integrations** view when a package has no icon ({kibana-pull}273191[#273191]).
Developer tools::
* Fixes **Dev Tools Console** autocomplete in query filter context to suggest query DSL instead of the obsolete Elasticsearch 1.x filter DSL ({kibana-pull}272907[#272907]).
Elastic Observability solution::
* Fixes deleting a label from a Synthetics monitor not persisting on edit ({kibana-pull}274404[#274404]).
Elastic Security solution::
For the Elastic Security 8.19.18 release information, refer to {security-guide}/release-notes.html[_Elastic Security release notes_].
Kibana platform::
* Fixes the Files plugin blob download to return the requested file type ({kibana-pull}274783[#274783]).
* Fixes an issue where the Kibana JSON logger could produce JSON objects with a large number of numbered keys ({kibana-pull}256233[#256233]).

[[release-notes-8.19.17]]
== {kib} 8.19.17

The 8.19.17 release includes the following known issues and fixes.

[float]
[[known-issues-8.19.17]]
=== Known issues

// tag::known-issue-268398[]
.Canvas Autoplay stops advancing after one page.
[%collapsible]
====
*Details* +
...
*Workaround* +
Manually advance to the next page or upgrade to 8.19.18 or 9.3.7.

*Resolved* + 
This issue is resolved in 8.19.18 and 9.3.7.
====
// end::known-issue-268398[]

[float]
[[fixes-v8.19.17]]
=== Fixes
Alerting::
* Fixes the **Alerts** page fetching alerts twice on initial load, which briefly showed all alerts before the filtered set ({kibana-pull}271441[#271441]).
Dashboards and Visualizations::
* Fixes the autoplay timer in **Canvas** jumping to the wrong page when pages are added or removed ({kibana-pull}268398[#268398]).
Data ingestion and Fleet::
* Fixes data stream selectors in input packages with multiple policy templates incorrectly sharing or duplicating variables between templates ({kibana-pull}273364[#273364]).
* Improves response time of the agent policies list by computing counts in a single aggregation instead of multiple queries per policy ({kibana-pull}272429[#272429]).
* Fixes missing security rule names in the **Integrations → Assets** tab ({kibana-pull}272089[#272089]).
Developer tools::
* Fixes autocomplete so `GET` is shown before `DELETE` when suggesting HTTP methods on an empty request line in **Console** ({kibana-pull}270787[#270787]).
* Marks invalid HTTP methods (for example, `GETT` and `POSTS`) as errors in **Console** ({kibana-pull}270479[#270479]).
Discover::
* Fixes slow parsing when KQL queries contain long runs of whitespace ({kibana-pull}266512[#266512]).
Elastic Observability solution::
* Fixes overlapping messages in the **Logs → Anomalies** table by restoring single-line truncation with an ellipsis ({kibana-pull}273221[#273221]).
* Fixes status tiles in the SLO Overview so that newly created SLOs are counted consistently with the per-row **No Data** badge ({kibana-pull}266315[#266315]).
Elastic Security solution::
For the Elastic Security 8.19.17 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
...
=== Fixes
Alerting::
* Fixes alert index template updates failing when auto-increasing `total_fields.limit` due to system-managed read-only fields ({kibana-pull}262534[#262534]).
Data ingestion and Fleet::
* Fixes integration package `elasticsearch` settings (`index_mode`, `source_mode`, `dynamic_namespace`, and `dynamic_dataset`) changing unexpectedly on package upgrade or reinstall ({kibana-pull}269080[#269080]).
* Fixes Fleet background tasks failing when paginating agent policies sorted on the non-existent `created_at` field ({kibana-pull}267285[#267285]).
* Fixes the **IP Location** processor in **Ingest pipelines** selecting the wrong database when a local `.mmdb` filename matches a managed database display name ({kibana-pull}265740[#265740]).
Elastic Security solution::
For the Elastic Security 8.19.16 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Elasticsearch solution::
* Fixes behavioral analytics APIs to reject collection names longer than 1024 characters ({kibana-pull}266225[#266225]).

[[release-notes-8.19.15]]
== {kib} 8.19.15

The 8.19.15 release includes the following enhancements and fixes.

[float]
[[enhancement-v8.19.15]]
=== Enhancements
Elastic Security solution::
For the Elastic Security 8.19.15 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Remembers the pagination state when navigating back from an edit on the **Users** page, instead of always returning to page 1 and resetting the search ({kibana-pull}261152[#261152]).
...
* Fixes missing sort-state announcements for screen readers in the **Ingest pipelines** list ({kibana-pull}262226[#262226]).
* Fixes screen readers announcing duplicate row indices in the **Ingest pipelines** list ({kibana-pull}261369[#261369]).
* Fixes the inference processor form to accept the `input_output` configuration shape ({kibana-pull}260517[#260517]).
* Fixes screen readers announcing icon labels twice in the **Ingest pipelines** list ({kibana-pull}259185[#259185]).
Data management::
* Fixes the transforms detail summary to show all `source_index` entries when `source_index` is an array ({kibana-pull}261875[#261875]).
* Fixes screen readers announcing duplicate row indices on the data streams table ({kibana-pull}261366[#261366]).
* Fixes screen readers announcing icon labels twice in the **Edit policy** flyout ({kibana-pull}261324[#261324]).
* Fixes screen readers announcing policy actions tooltip text twice in the index lifecycle policies list ({kibana-pull}261322[#261322]).
* Fixes screen readers announcing copy button labels twice ({kibana-pull}261311[#261311]).
* Fixes screen readers announcing grey color badge text twice in the transforms list ({kibana-pull}261307[#261307]).
* Fixes screen readers not announcing invalid field validation errors ({kibana-pull}260673[#260673]).
Elastic Observability solution::
For the Elastic Observability 8.19.15 release information, refer to {observability-guide}/release-notes.html[_Elastic Observability Release Notes_].
Elastic Security solution::
For the Elastic Security 8.19.15 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes screen readers announcing data stream and index options twice in the **Create policy** and **Restore snapshot** selectable lists ({kibana-pull}261786[#261786]).
Machine Learning::
* Fixes the single metric chart in Anomaly Explorer not showing anomaly actions correctly ({kibana-pull}263925[#263925]).
...
=== Fixes
Dashboards and Visualizations::
*  Fixes Lens panels appearing to revert after inline edits until you refresh the page, even though changes are saved ({kibana-pull}256984[#256984]).
Data ingestion and Fleet::
* Fixes Fleet bulk agent actions (unenroll, upgrade, reassign to policy) not respecting the current space when selection uses a kuery filter ({kibana-pull}258582[#258582]).
Data management::
* Restores keyboard focus to the **Create a transform** button on the **Transforms** page ({kibana-pull}258095[#258095]).
Elastic Observability solution::
* Fixes alert tags on inventory threshold, metric threshold, and custom threshold rules when tags are provided as a string instead of an array ({kibana-pull}259729[#259729]).
Elastic Security solution::
For the Elastic Security 8.19.14 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Elasticsearch solution::
* Fixes the query rules UI rejecting case variants as duplicate values ({kibana-pull}259506[#259506]).
* Fixes only passing filtered rules to the API and deleting rules not matching the filter in the Query Rule Set editor ({kibana-pull}259503[#259503]).
Machine Learning::
* Fixes the anomaly swim lane dashboard panel not refreshing ({kibana-pull}259962[#259962]). 
...
=== Fixes
Alerting and cases::
* Fixes an issue where webhook connector updates failed when the external service returned a 204 (no content) response ({kibana-pull}251090[#251090]).
Data ingestion and Fleet::
* Filters out unenrolled agents in the cleanup policy revisions task ({kibana-pull}254899[#254899]).
Discover::
* Fixes a rendering issue in the **Attributes** tab on the trace details flyout that caused the UI to become unresponsive ({kibana-pull}255173[#255173]).
Elastic Observability solution::
* Passes alerts data view to `buildEsQuery` for correct wildcard queries on keyword fields ({kibana-pull}255225[#255225]).
* Fixes a crash in APM apps when invalid values were passed to `rangeFrom` and `rangeTo` URL query parameters ({kibana-pull}256887[#256887]).
* Fixes an error on the SLO burn rate alert details page for suppressed alerts ({kibana-pull}256435[#256435]).
* Fixes a 500 error and infinite loading on the APM **Error group details** page when the related transaction document is missing the `transaction.sampled` field ({kibana-pull}255788[#255788]).
Elastic Security solution::
For the Elastic Security 8.19.13 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Management::
* Fixes an issue in **Dev Tools Console** where syntax highlighting broke when queries contained accented or non-ASCII characters ({kibana-pull}255649[#255649]).
* Fixes an issue in **Dev Tools Console** where closing nested braces broke syntax highlighting for subsequent elements ({kibana-pull}255426[#255426]).
Search::
* Fixes focus behavior when there are errors in the connector flyout form ({kibana-pull}255770[#255770]).

[[release-notes-8.19.12]]
== {kib} 8.19.12

The 8.19.12 release includes the following enhancements and fixes.

IMPORTANT: This release contains fixes for potential security vulnerabilities. Check our link:https://discuss.elastic.co/c/announcements/security-announcements/31[security advisory] for more details.

[float]
[[enhancement-v8.19.12]]
=== Enhancements
Elastic Security solution::
For the Elastic Security 8.19.12 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].

[float]
[[fixes-v8.19.12]]
=== Fixes
Elastic Observability solution::
* Fixes SLO filters that contain spaces and wildcard characters ({kibana-pull}251033[#251033]).
Elastic Security solution::
For the Elastic Security 8.19.12 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
...
The 8.19.11 release includes the following fixes.

[float]
[[fixes-v8.19.11]]
=== Fixes
Alerting::
* Fixes timestamp override for ES|QL CSV scheduled reports with relative time ranges ({kibana-pull}248169[#248169]).
Elastic Observability solution::
* Fixes incorrect dependencies stats ({kibana-pull}249434[#249434]).
* Adds `maxQueue` backpressure to anonymization regex worker pool ({kibana-pull}249108[#249108]).
* Uses monitor query ID for project monitors package policies ({kibana-pull}248762[#248762]).
* Fixes an issue where synthetics test results showed up as duplicated on the monitor status heat map, for higher granularity columns ({kibana-pull}248761[#248761]).
* Adds more granular control over how alerts behave when data stops being reported for metric threshold rules ({kibana-pull}247669[#247669]).
Elastic Security solution::
For the Elastic Security 8.19.11 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Kibana platform::
* Fixes an issue with the Share menu where all time ranges were being shared as absolute ({kibana-pull}248804[#248804]).
* Fixes an issue with certain properties causing failures when adding a policy to an index template ({kibana-pull}249168[#249168]).
* Fixes an issue with duplicated managed ILM policies still appearing as managed ({kibana-pull}248586[#248586]).
* Fixes Stack Monitoring Recent Log Entries' timestamps to respect Kibana's time zone setting (`dateFormat:tz`) ({kibana-pull}249016[#249016]).
