...
+
After upgrading from an older version of {kib} while using {security}, 
if you get a permission error when you start {kib} for the first time, do the following steps to recover:
+
. Stop {kib}.
. Delete the `.kibana_1` and `.kibana_2` indices that were created.
. Create a new role in Elasticsearch that has the `create_index`, `create`, and 
`read`  permission for the `.tasks` index.
. Create a new user in {es} that has the `kibana_system` role as well as the new role you just created.
. Update `elasticsearch.username` and `elasticsearch.password` in `kibana.yml` with the details from that new user.
. If using a {kib} secure settings keystore, remove keys `elasticsearch.username` and `elasticsearch.password`
from the keystore using the kibana-keystore tool. Add these keys back to the keystore using the new user 
