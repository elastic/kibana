...
// ====
// *Details* +
// !!TODO!! For more information, refer to ({kibana-pull}184036[#184036]).
...
Now we allow the decimals value in the threshold filed for the Failed transaction rate threshold rule e.g. `30.50`% or `0.1` %
////
Cases::
* Allow users to create case using templates ({kibana-pull}187138[#187138]).
Dashboard::
* Redesign the "Add Panel" Experience ({kibana-pull}183764[#183764]).
Discover::
* Creating where clause filters from the table, sidebar and table row viewer ({kibana-pull}181399[#181399]).
Elastic Security::
...
////
Lens & Visualizations::
* Legend Statistics feature in *Lens* ({kibana-pull}182357[#182357]).
Machine Learning::
* AIOps: Adds AI Assistant contextual insight to Log Rate Analysis page in ML plugin in Observability serverless ({kibana-pull}186509[#186509]).
* Adds Field statistics embeddable as panel in Dashboard ({kibana-pull}184030[#184030]).
* Adds ES|QL support for field statistics table in Discover ({kibana-pull}180849[#180849]).
* AIOps: Adds Pattern analysis tab in Discover ({kibana-pull}178916[#178916]).
Management::
* Display results instead of errors after search timeout ({kibana-pull}179679[#179679]).
////
!!TODO!! The above PR had a lengthy release note description:
When a long-running search times out, instead of showing an error, any results that are available will be shown (along with a warning that the results are incomplete).
////
SharedUX::
* Cloud ID copy input now has a information "?" icon with a popup and a link ({kibana-pull}183388[#183388]).
* Now, when clicking on the "Endpoints and API keys" button in the header, it will open the new flyout ({kibana-pull}183236[#183236]).
* Adds "Give feedback" button to Serverless header ({kibana-pull}180942[#180942]).
* Connection Details Flyout ({kibana-pull}180912[#180912]).
////
!!TODO!! The above PR had a lengthy release note description:
The new *Connection details* overlay features a cleaner UI for Elasticsearch endpoint URL and Cloud ID token retrieval. As well as the new *"API key"* tab, which is a simplified interface for creating an API key with all the default settings, except the expiration time is set to 90 days. If user wants a more granular API keys, there is a link which navigates to the right section in Stack Management.
////
Other::
* Adds public list asset criticality API ({kibana-pull}186568[#186568]).
* Adds criticality column to All Users table In Explore pages ({kibana-pull}186456[#186456]).
* Adding Asset Criticality column to All Hosts tab in Explore pages ({kibana-pull}186375[#186375]).
* Create public versions of asset criticality get,upsert,delete and csv upload APIs ({kibana-pull}186169[#186169]).
* The Cases webhook connector now supports SSL certificate authentication ({kibana-pull}185925[#185925]).
* Update links for integration buttons in observability solutions ({kibana-pull}184477[#184477]).
* Adds SLO status, SLI value, error budget remaining and consumed in burn rate alert context ({kibana-pull}184471[#184471]).
* Feat(slo): prevent initial backfill ({kibana-pull}184312[#184312]).
* Update `Add data` links to use improved deep linking ({kibana-pull}184164[#184164]).
* Change navigation behavior for the o11y guide cards to pre-select the correct solution ({kibana-pull}184065[#184065]).
* The POC for creating entity-centric indices using entity definitions ({kibana-pull}183205[#183205]).
* Using alert summary widget as the history chart ({kibana-pull}181824[#181824]).
* Reveal data quality entry on the side-nav in serverless ({kibana-pull}180816[#180816]).
* Flyout Summary section ({kibana-pull}179479[#179479]).
...
* Allow users to create case using templates ({kibana-pull}187138[#187138]).
Dashboard::
* Scroll to panel on minimize ({kibana-pull}184696[#184696]).
* Redesign the "Add Panel" Experience ({kibana-pull}183764[#183764]).
* Background Color Should Match When Margins are Off ({kibana-pull}181450[#181450]).
* Simplify workflow for dashboard copy creation in both view and edit interaction modes ({kibana-pull}180938[#180938]).
Data Discovery::
* Include custom field label in CSV reports ({kibana-pull}181565[#181565]).
* Convert EuiTable to EuiDataGrid. Enable up to 500 fields per page ({kibana-pull}175787[#175787]).
Discover::
* Creates ES|QL where filters for ordinal charts ({kibana-pull}184420[#184420]).
* Expanding the time range only on demand when No results ({kibana-pull}181723[#181723]).
* Creating where clause filters from the table, sidebar and table row viewer ({kibana-pull}181399[#181399]).
* Use push flyout for Discover document flyout ({kibana-pull}166406[#166406]).
Elastic Security::
For the Elastic Security 8.15.0 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Fleet::
...
* Introduce policy_ids in package policy SO ({kibana-pull}184636[#184636]).
* Added force flag to delete agent_policies API ({kibana-pull}184419[#184419]).
* Surface option to delete diagnostics files ({kibana-pull}183690[#183690]).
...
The share menu is updated for a more streamlined user experience. Users can navigate through a tabbed modal to copy links for discover, dashboard, and lens.
////
* Remove unnecessary field definitions for custom integrations and add `logs@mappings` to log streams ({kibana-pull}178083[#178083]).
Lens & Visualizations::
* [Unified Search] [Maps] [Dashboard] Adds wildcard matching to field pickers across Kibana in *Lens* ({kibana-pull}182631[#182631]).
* Legend Statistics feature in *Lens* ({kibana-pull}182357[#182357]).
Machine Learning::
* AIOps: Adds AI Assistant contextual insight to Log Rate Analysis page in ML plugin in Observability serverless ({kibana-pull}186509[#186509]).
...
* Anomaly swim lane - adds / edits UX improvements ({kibana-pull}182586[#182586]).
...
* AIOps: Adds Pattern analysis tab in Discover ({kibana-pull}178916[#178916]).
Management::
* Siem query rule - reduce field_caps usage ({kibana-pull}184890[#184890]).
* ES Query rule loads fewer fields on query execution ({kibana-pull}183694[#183694]).
* Adds horizontal resizability to the autocomplete popup on the Dev Tools Console ({kibana-pull}180243[#180243]).
* Display results instead of errors after search timeout ({kibana-pull}179679[#179679]).
////
!!TODO!! The above PR had a lengthy release note description:
When a long-running search times out, instead of showing an error, any results that are available will be shown (along with a warning that the results are incomplete).
...
Platform::
* Adds http2 support for Kibana server ({kibana-pull}183465[#183465]).
////
!!TODO!! The above PR had a lengthy release note description:
...
////
* Rolling file appender: implement retention policy ({kibana-pull}182346[#182346]).
////
!!TODO!! The above PR had a lengthy release note description:
Kibana's `rolling-file` appender now supports more advanced retention policies. Please refer to the [Kibana logging documentation](https://www.elastic.co/guide/en/kibana/current/logging-configuration.html#rolling-file-appender) for more details
////
Presentation::
* Adds "Creator" column to dashboard listing page ({kibana-pull}182256[#182256]).
Security::
* Inited spaces API for serverless ({kibana-pull}184418[#184418]).
////
!!TODO!! The above PR had a lengthy release note description:
Enabled spaces API for serverless in preparation for custom spaces and custom roles. The ability to create or delete spaces is still guarded by the `xpack.spaces.maxSpaces` setting.
////
* Added telemetry for space solution property ({kibana-pull}184192[#184192]).
* Added optional solution property for Space in a stateful offering ({kibana-pull}183986[#183986]).
* Added optional role description field for Save/Edit Role page ({kibana-pull}183145[#183145]).
* Audit logs can be filtered by username via the `xpack.security.audit.ignore_filters.users` configuration setting ({kibana-pull}183137[#183137]).
* Added support for `remote_cluster` field in ES role definition ({kibana-pull}182377[#182377]).
* Added UI Capability to role management ({kibana-pull}181424[#181424]).
* Replace usage of Get API keys endpoint with Query API keys endpoint for API keys grid page ({kibana-pull}168970[#168970]).
////
!!TODO!! The above PR had a lengthy release note description:
Enhanced API key management to manage larger number of API keys by adding server side filtering, pagination and querying.
////
SharedUX::
* Cloud ID copy input now has a information "?" icon with a popup and a link ({kibana-pull}183388[#183388]).
* Now, when clicking on the "Endpoints and API keys" button in the header, it will open the new flyout ({kibana-pull}183236[#183236]).
...
The new *Connection details* overlay features a cleaner UI for Elasticsearch endpoint URL and Cloud ID token retrieval. As well as the new *"API key"* tab, which is a simplified interface for creating an API key with all the default settings, except the expiration time is set to 90 days. If user wants a more granular API keys, there is a link which navigates to the right section in Stack Management.
////
* The dashboard listing page now allows filtering dashboards by creators for dashboards created after version 8.14 ({kibana-pull}180147[#180147]).
* Use Data Stream for Reporting storage ({kibana-pull}176022[#176022]).
////
!!TODO!! The above PR had a lengthy release note description:
...
////
Other::
* Improve metadata autocomplete suggestions with comma and pipe ({kibana-pull}188338[#188338]).
* Automatically encapsulate index names with special chars with quotes ({kibana-pull}187899[#187899]).
* Adds a preview button to alerts inside the risk contribution panel ({kibana-pull}187148[#187148]).
* Adds public list asset criticality API ({kibana-pull}186568[#186568]).
* Adds criticality column to All Users table In Explore pages ({kibana-pull}186456[#186456]).
* Adding Asset Criticality column to All Hosts tab in Explore pages ({kibana-pull}186375[#186375]).
* Capitalize things ({kibana-pull}186340[#186340]).
* Create public versions of asset criticality get,upsert,delete and csv upload APIs ({kibana-pull}186169[#186169]).
* Enable links to Dataset Quality from Management and Logs Explorer ({kibana-pull}186143[#186143]).
* The Cases webhook connector now supports SSL certificate authentication ({kibana-pull}185925[#185925]).
* Update question contents to match new designs ({kibana-pull}184866[#184866]).
* Gemini Connector Assistant Integration ({kibana-pull}184741[#184741]).
* Editor supports integrations ({kibana-pull}184716[#184716]).
* AI settings ({kibana-pull}184678[#184678]).
* Allows to comment out the current line with the keyboard ({kibana-pull}184637[#184637]).
* Telemetry collection support for Logstash agent driven monitoring ({kibana-pull}184480[#184480]).
* Update links for integration buttons in observability solutions ({kibana-pull}184477[#184477]).
* Adds SLO status, SLI value, error budget remaining and consumed in burn rate alert context ({kibana-pull}184471[#184471]).
* Implement clone monitor action ({kibana-pull}184393[#184393]).
* Lightweight monitors allow 10 and 30 seconds schedules ({kibana-pull}184380[#184380]).
* Feat(slo): prevent initial backfill ({kibana-pull}184312[#184312]).
* Update `Add data` links to use improved deep linking ({kibana-pull}184164[#184164]).
* Render a Discover-like table in the assistant instead of a Lens chart ({kibana-pull}184106[#184106]).
* Adds UI controls to mark KB entries as public ({kibana-pull}184094[#184094]).
* Change navigation behavior for the o11y guide cards to pre-select the correct solution ({kibana-pull}184065[#184065]).
...
////
* Filter by brushing a date histogram ({kibana-pull}184012[#184012]).
* Adds `spaceId` to entities ({kibana-pull}183943[#183943]).
* Query all search connectors by default but allow user to override ({kibana-pull}183712[#183712]).
* Gemini connector integration ({kibana-pull}183668[#183668]).
* Added support for the additional info field in the ServiceNow ITOM connector ({kibana-pull}183380[#183380]).
* Adds downstream dependency service name to logs and errors to improve alert insights ({kibana-pull}183215[#183215]).
* The POC for creating entity-centric indices using entity definitions ({kibana-pull}183205[#183205]).
* Config: add support for `${VAR:default}` syntax ({kibana-pull}182139[#182139]).
////
!!TODO!! The above PR had a lengthy release note description:
...
////
* Using alert summary widget as the history chart ({kibana-pull}181824[#181824]).
* Update general settings for apm config ({kibana-pull}180944[#180944]).
* Reveal data quality entry on the side-nav in serverless ({kibana-pull}180816[#180816]).
* Flyout Summary section ({kibana-pull}179479[#179479]).
...
=== Bug Fixes
Alerting::
* Fixes kibana.alert.rule.execution.timstamp timezone and format ({kibana-pull}183905[#183905]).
Canvas::
* Fixes parent API should implement unified search interface with react embeddables ({kibana-pull}183772[#183772]).
Dashboard::
...
Design::
* A11y fixes for user profile input labels ({kibana-pull}186471[#186471]).
Discover::
...
* Error messages fix overlapping ({kibana-pull}181416[#181416]).
Elastic Security::
For the Elastic Security 8.15.0 release information, refer to {security-guide}/release-notes.html[_Elastic Security Solution Release Notes_].
Fleet::
...
* Bugfix - Show warning callout in configs tab when an error occurs ({kibana-pull}187487[#187487]).
...
* [ES|QL] Unable to load page error on edit/add ES|QL panel ({kibana-pull}188664[#188664]).
...
* Fixes datatable actions when first row is empty in *Lens* ({kibana-pull}181344[#181344]).
* Fixes yAxis scale/custom domain issues and help/error text in *Lens* ({kibana-pull}180532[#180532]).
Logs::
* Fixes log stream fly-out when embedded in APM or the Infrastructure UI ({kibana-pull}189763[#189763]).
* Fixes log entry fly-out when response is slow ({kibana-pull}187303[#187303]).
* Fixes fly-out link to the legacy Uptime app ({kibana-pull}186328[#186328]).
Machine Learning::
...
* Fixes Field statistics panel displays multiple errors if associated index is deleted and race condition when refreshes too fast ({kibana-pull}188327[#188327]).
* Hides ML embeddables from the "Add panel" flyout when ML feature isn't available ({kibana-pull}187639[#187639]).
...
Management::
* Allow selection of timestamp when some index pattern segments are unmet ({kibana-pull}189336[#189336]).
...
Platform::
* Fixes small `i18n` + `l10n` inconsistencies ({kibana-pull}181735[#181735]).
Presentation::
...
* Don't close the flyout if the user cancels save to library ({kibana-pull}188995[#188995]).
...
Querying & Filtering::
* Fixes performance issue with nested subqueries ({kibana-pull}181208[#181208]).
Security::
* Fixed `ComboBox` overflow with large chips ({kibana-pull}184722[#184722]).
* Added `disabledFeatures` back to mappings, so it can be aggregated on ({kibana-pull}184195[#184195]).
* Support read-only remote index & cluster sections with read_security access ({kibana-pull}183126[#183126]).
////
!!TODO!! The above PR had a lengthy release note description:
...
////
* Added transformation of application * privilege to all ({kibana-pull}181400[#181400]).
////
!!TODO!! The above PR had a lengthy release note description:
Added transformation of application wildcard `*` privilege to `all` to correctly filter and display roles as `superuser`.
////
SharedUX::
* Fixes case sensitivity tag search ({kibana-pull}183092[#183092]).
Sharing::
* Better error handling ({kibana-pull}185903[#185903]).
Other::
...
* Remove inaccurate values suggestions ({kibana-pull}189228[#189228]).
* Fixes showing the correct log view in the rule creation flyout ({kibana-pull}189205[#189205]).
* Allow retest on failure to be turned off ({kibana-pull}189013[#189013]).
////
!!TODO!! The above PR had a lengthy release note description:
Fixes a bug where retest on failure was not able to be turned off when creating a monitor in the Synthetics app. Retest on failure can now be turned off.
////
* Improve support for `Invoke` completion trigger kind ({kibana-pull}188877[#188877]).
* Fixed rds and aws cpu percentages ({kibana-pull}188768[#188768]).
* Bring back EuiThemeProvider to fix o11y and stack monitoring rules in stack management ({kibana-pull}188724[#188724]).
* Fixes gemini connector card in icon style ({kibana-pull}188469[#188469]).
* Allow users to save mappings with errors ({kibana-pull}188326[#188326]).
* Fixes AI Assistant Settings when plugin disabled ({kibana-pull}188160[#188160]).
* Hide ai assistant menu item when in a disabled space ({kibana-pull}188017[#188017]).
* Remove links to legacy risk scoring docs ({kibana-pull}187585[#187585]).
* Fixes x-axis timezone on alertSummaryWidget full size ({kibana-pull}187468[#187468]).
* Fixes synthetics/uptime fields alerts autocomplete for query bar ({kibana-pull}186588[#186588]).
* Fixes TLS cert view for > 3 monitors per cert ({kibana-pull}186204[#186204]).
...
* Fixes flaky e2e test (contextual insights for APM errors) ({kibana-pull}184642[#184642]).
* Accept negated index patterns ({kibana-pull}184528[#184528]).
* Set validation errors in subaction framework as user errors ({kibana-pull}184317[#184317]).
...
* UI States for reduced user privileges ({kibana-pull}183947[#183947]).
* Fix(slo): history details data ({kibana-pull}183097[#183097]).
* Decrease bucket size top_dependencies sends to get_connection_stats query ({kibana-pull}182884[#182884]).
* Feat(slo): Improve permission check on SLO pages ({kibana-pull}182609[#182609]).
