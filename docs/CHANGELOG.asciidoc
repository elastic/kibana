...
* Adds four dedicated case analytics indices that allow users to build dashboards and metrics using case data. These indices are automatically created when {kib} starts up and are updated periodically with cases, comments, attachments, and activity data ({kibana-pull}223405[#223405]).
...
* Adds the **Create alert rule** action to ES|QL dashboard panels, usable from the panel context menu or by right-clicking a data point on the visualization. This allows you to generate an alert when the data on the chart crosses a certain threshold ({kibana-pull}217719[#217719]).
* Adds collapsible sections to **Dashboard**. Collapsible sections allow you to group panels into logical groups, and to help your dashboards load faster by only loading their content when expanded ({kibana-pull}220877[#220877]).
...
* Adds "Compare to" badge for Metric charts in **Lens** ({kibana-pull}214811[#214811]).
...
Discover::
* Adds an **Attributes** tab in **Discover** when exploring OTel documents ({kibana-pull}222391[#222391]).
* When an ES|QL query times out (as a result of the `search:timeout` advanced setting), partial results that are available are now shown ({kibana-pull}219027[#219027]).
* Adds click actions for Stacktrace and Degraded Fields in Discover ({kibana-pull}214413[#214413]).
* Shows a **Load more** option instead of pagination when exploring Logs in **Discover** ({kibana-pull}211176[#211176]).

Elastic Observability solution::
* Registers UI setting for anonymization ({kibana-pull}224607[#224607]).
* Adds accordion sections and attributes tables ({kibana-pull}224185[#224185]).
* Adds the history tab view for calendar-based SLO in the SLO details page ({kibana-pull}223825[#223825]).
* Renders suggested dashboards ({kibana-pull}223424[#223424]).
* Adds primary **Add to case** button to Synthetics ({kibana-pull}223184[#223184]).
* SLOs are now easier to manage with the ability to view definitions, delete SLOs, and purge SLI data from a single page, without the need to consider instances. ({kibana-pull}222238[#222238]).
...
* Multi space monitors !! ({kibana-pull}221568[#221568]).
* Edit private locations labels and tags in Synthetics ({kibana-pull}221515[#221515]).
* Update Knowledge Base model via settings ({kibana-pull}221319[#221319]).
* Enable ELSER and e5 on EIS ({kibana-pull}220993[#220993]).
* The Data Set Quality page now supports Failure Store metrics ({kibana-pull}220874[#220874]).
* Adds monitor downtime alert on no data ({kibana-pull}220127[#220127]).
* Only show ELSER in EIS if available ({kibana-pull}220096[#220096]).
* Save group by information with dynamic mapping ({kibana-pull}219826[#219826]).
* Added compact view in monitors overview page ({kibana-pull}219060[#219060]).
* Linked dashboards in Rule and Alert pages ({kibana-pull}219019[#219019]).
* Specify embedding model during onboarding for the Knowledge Base ({kibana-pull}218448[#218448]).
* Changed embeddable view when only one monitor in one location is selected ({kibana-pull}218402[#218402]).
* Adds logical AND to monitor tags and locations filter ({kibana-pull}217985[#217985]).
* Added drilldown to synthetics stats overview embeddable ({kibana-pull}217688[#217688]).
* Investigation guide frontend ({kibana-pull}217106[#217106]).
* Enable KQL filter for TLS rules ({kibana-pull}216973[#216973]).
* Investigation Guide backend ({kibana-pull}216377[#216377]).
* Embeddable Trace Waterfall ({kibana-pull}216098[#216098]).
* Archiving conversations ({kibana-pull}216012[#216012]).
* Related alerts based on scoring !! ({kibana-pull}215673[#215673]).
* Service map support for span links ({kibana-pull}215645[#215645]).
* Adds 'Read More', 'Read Less' functionality to highlight_fields ({kibana-pull}215326[#215326]).
* Adds KQL Filter to TLS Alerting Rule ({kibana-pull}215110[#215110]).
* Adds context.grouping action variable in SLO Burn rate and ES Query rules ({kibana-pull}213550[#213550]).
* Adds context.grouping action variable in Custom threshold and APM rules ({kibana-pull}212895[#212895]).
* SLO overview should give users a clear, immediate picture into key objective data. Previously, the user would have had to scroll past static data that describes the SLO definition before seeing valuable information about their SLIs. This static data has been moved to a separate tab, making charts more easily accessible. ({kibana-pull}212826[#212826]).
* ES|QL rule type improvements - adding grouping per row ({kibana-pull}212135[#212135]).
* Share conversations ({kibana-pull}211854[#211854]).
* Duplicate conversations ({kibana-pull}208044[#208044]).
* The create rule flyout, used by solutions, uses the new and improved rule form ({kibana-pull}206685[#206685]).
* Replace the status filter with controls on the observability alerts pages ({kibana-pull}198495[#198495]).
* Adds support for `GroupStreamDefinition` to `/api/streams` endpoints [#208126]({kibana-pull}208126).
* Indicates if failure store isn't enabled for data stream [#221644]({kibana-pull}221644).
...
Search::
* Adds a section that enables adding, updating, and deleting query rules written on top of the Query Rules APIs to pin or exclude documents to a query according to criteria you set. ({kibana-pull}227226[#227226]).
...
* Shows list of keyboard shortcuts at the bottom of the editor ({kibana-pull}221331[#221331]).
...
* Adds support for `command`/`ctrl` + click to open links in a new tab, for example allowing to conduct simultaneous searches in parallel more efficiently ({kibana-pull}210982[#210982]).
...
Elastic Observability solution::
* Submit a comment in cases by pressing + Enter ({kibana-pull}228473[#228473]).
* Update SLO starter prompt ({kibana-pull}224493[#224493]).
* Integration new tail sampling settings ({kibana-pull}224479[#224479]).
* Get model ID from anonymization rules ({kibana-pull}224280[#224280]).
* Update system prompt to inform about anonymization ({kibana-pull}224211[#224211]).
* Adds investigation guide empty state ({kibana-pull}223974[#223974]).
* Anonymization support ({kibana-pull}223351[#223351]).
* Remapping `iInCircle` and `questionInCircle` and deprecating `help` icon ({kibana-pull}223142[#223142]).
* Show cases on alert detail overview ({kibana-pull}222903[#222903]).
* Remove is_correction and confidence attributes from kb entry ({kibana-pull}222814[#222814]).
* Refetch alert detail rule data on edit flyout submit ({kibana-pull}222118[#222118]).
* Update spec.max to 3.4 ({kibana-pull}221544[#221544]).
* Adds Edot logging level to central config ({kibana-pull}219722[#219722]).
* Remove metrics and logs from get_service_stats api ({kibana-pull}218346[#218346]).
* Remove double confirmation when deleting conversation ({kibana-pull}217991[#217991]).
* 790 deployment environment discrepancy ({kibana-pull}217899[#217899]).
* Embeddable Trace Waterfall Enhancements ({kibana-pull}217679[#217679]).
...
* Include spaceID in SLI documents ({kibana-pull}214278[#214278]).
////
!!TODO!! The above PR had a lengthy release note description:
SpaceId can now be referenced in document-based access filters for roles to restrict a user's access to SLI data for spaces where they do not have access.
////
* Update delete confirmation modal ({kibana-pull}212695[#212695]).
* Enable syntax highlighting for ES|QL ({kibana-pull}212669[#212669]).
* Runtime metrics show dashboards with different ingest path ({kibana-pull}211822[#211822]).
...
* Enable editing central config for EDOT Agents / SDKs ({kibana-pull}211468[#211468]).
...
* Fixes an issue causing an internal server error when renaming a search session ({kibana-pull}226757[#226757]).
...
* Fixes an issue that causes transitions from **Logs Stream** and **Logs Explorer** to **Discover** to lose some context such as the selected time range or KQL query ({kibana-pull}215867[#215867]).
...
* Inject user prompt before tool call when query actions are clicked ({kibana-pull}227462[#227462]).
* Fixed edit private location with no monitors assigned ({kibana-pull}227411[#227411]).
* Fixes missing sparklines from dependency ({kibana-pull}227211[#227211]).
* Throw tool validation error on MALFORMED_FUNCTION_CALL finish reason when processing Gemini Stream ({kibana-pull}227110[#227110]).
* Available in stateful solution view if enabled ({kibana-pull}226999[#226999]).
* Fix: product docs installation status ({kibana-pull}226919[#226919]).
...
* Collapse *query tool calls ({kibana-pull}226078[#226078]).
* Fixes broken operation page ({kibana-pull}226036[#226036]).
* Limit environment name length when creating ML job ({kibana-pull}225973[#225973]).
* Fixes schema page ({kibana-pull}225481[#225481]).
* Hide Settings from serverless navigation ({kibana-pull}225436[#225436]).
* Fixes Agent Explorer page ({kibana-pull}225071[#225071]).
* Adds query rewriting ({kibana-pull}224498[#224498]).
* Fixes SLO federated view bug when remote clusters and index name listed exceeded 4096 bytes ({kibana-pull}224478[#224478]).
* Suggested dashboards are returned only for custom threshold alerts ({kibana-pull}224458[#224458]).
* Fixes broken EDOT JVM metrics dashboard when classic agent metrics are present ({kibana-pull}224052[#224052]).
* Use bulk helper for bulk importing knowledge base entries ({kibana-pull}223526[#223526]).
* Remove run soon for sync private location task !! ({kibana-pull}222062[#222062]).
* Adjust example to ndjson format ({kibana-pull}221617[#221617]).
* Dataset quality: Make sure non-aggregatable message is not shown if no data matches ({kibana-pull}221599[#221599]).
* Delete user instruction if text is empty ({kibana-pull}221560[#221560]).
* Check for documents before starting semantic text migration ({kibana-pull}221152[#221152]).
* Hide Data set details when dataStream is coming from remote cluster ({kibana-pull}220529[#220529]).
* Make API tests more resilient ({kibana-pull}220503[#220503]).
* Remove index write blocks ({kibana-pull}220362[#220362]).
* Receive `aria-labelledby` from Elastic Charts svg ({kibana-pull}220298[#220298]).
* Query alerts using the `alert.start` field and update alerts function API test to check alert information ({kibana-pull}219651[#219651]).
* Fixes Alerts environment query follow up ({kibana-pull}219571[#219571]).
* Don't open in flyout mode on mount ({kibana-pull}219420[#219420]).
* Change the alerts query to include environment not defined value ({kibana-pull}219228[#219228]).
* Disable use logical AND when filter is removed ({kibana-pull}218910[#218910]).
* Ensure index template are created ({kibana-pull}218901[#218901]).
* Use fields instead of _source in the metadata endpoint ({kibana-pull}218869[#218869]).
* Fixes span url link when transactionId missing in Span Links ({kibana-pull}218232[#218232]).
* Fixing error - Display results and Visualize query Bedrock Error ({kibana-pull}218213[#218213]).
* Make create annotations form keyboard navigable ({kibana-pull}217918[#217918]).
* EDOT error summary fix ({kibana-pull}217885[#217885]).
* Remove direct function calling from the chat input ({kibana-pull}217359[#217359]).
* Adds error text in environment filter when input is invalid ({kibana-pull}216782[#216782]).
* Change tpm abbreviation to trace per minute for screen-readers ({kibana-pull}216282[#216282]).
...
* Use `nameTooltip` api for dependencies tables ({kibana-pull}215940[#215940]).
...
* Remove unnecessary field service.environment from top dependency spans endpoint ({kibana-pull}215321[#215321]).
* Update retrieve_elastic_doc api test ({kibana-pull}215237[#215237]).
* Fixes id overflow ({kibana-pull}215199[#215199]).
* Fixes contextual insights scoring ({kibana-pull}214259[#214259]).
* Update knowledge base installation flow ({kibana-pull}214133[#214133]).
* Show inspect configuration button always !! ({kibana-pull}213619[#213619]).
* Fixing - Failing test: Stateful Observability - Deployment-agnostic A… ({kibana-pull}213530[#213530]).
...
* Removed unnecessary breadcrumbs in profiling ({kibana-pull}211081[#211081]).
