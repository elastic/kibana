...
The 8.19.9 release includes the following fixes.
...
=== Fixes
Alerting::
* Fixes an issue that caused the Security alerts table not to update columns correctly when switching view modes ({kibana-pull}245253[#245253]).
...
* Fixes the serialization of `meta.error` in JSON layouts: if the error is an `Error` instance, only `message`, `name`, and `stack` are included. Other fields are no longer returned in the logs ({kibana-pull}244364[#244364]).
* Fixes an issue where Upgrade Assistant incorrectly warned about nodes exceeding the `storage low watermark` when the `max headroom` setting was configured ({kibana-pull}243906[#243906]).
