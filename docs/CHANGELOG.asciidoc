...
If you have alerting rules with configured snooze notifications, do not upgrade {kib} from 8.3.0 to 8.4.0.

To verify if your alerting rules have snooze notifications, open the main menu, then click **Stack Management** -> **Rules and Connectors**.
Filter the rule list by selecting **View** -> **Snoozed**.
For each space, cancel the snooze for all affected rules before you upgrade.

To identify snoozed rules in all Spaces using **Dev Tools**, run the following query and then search for the `is_snoozed_until` string:
...
When you upgrade {kib} from 8.3.0 to 8.4.0, and your alerting rules have snooze notifications, you receive the following error message:
