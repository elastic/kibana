...
An issue was discovered by Elastic whereby sensitive information is recorded in Kibana logs in the event of an error. Elastic has released Kibana 8.11.1 and 7.17.15 which resolves this issue. The error object recorded in the log may contain account credentials for the `kibana_system` user, API keys and credentials of, Kibana end-users. For more information, refer to link:https://discuss.elastic.co/t/kibana-8-10-1-security-update/343287[Security announcements].
