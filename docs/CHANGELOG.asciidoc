...
As a workaround, enable a machine learning job on at least one service in APM. {kib-issue}69238[#69238]
Reporting::
* Config setting `xpack.reporting.enabled` has a bug causing Kibana to fail
start up if it exists in `kibana.yml`, regardless of true for false value.
There is no workaround until a patch fix is released. {kib-pull}69645[#69645]
* Config setting `xpack.reporting.encryptionKey` has an unintentional breaking
change that validates the encryption key string to be a minimum of 32
characters. If upgrading from 7.7.x or below, Kibana will fail to start up in
7.8.0 if the `kibana.yml` has `xpack.reporting.encryptionKey` containing fewer
than 32 characters. Since changing the encryption key means being no longer
able to access older reports, there is no workaround until a patch fix is
released. {kib-pull}69827[#69827]
SIEM::
...
[discrete]
[[deprecation-86474]]
==== Deprecates `xpack.security.authc.saml.realm` and `xpack.security.authc.oidc.realm`

*Details* +
Deprecates the `xpack.security.authc.saml.realm` and  `xpack.security.authc.oidc.realm` settings used for {kib} SAML and OpenID Connect authentication. For more information, refer to {kibana-pull}53010[#53010].

*Impact* +
Use `xpack.security.authc.providers.saml.<provider-name>.realm` and `xpack.security.authc.providers.oidc.<provider-name>.realm`.
