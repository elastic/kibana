...
<<breaking-changes-8.0.0,8.0.0>> | <<breaking-changes-8.0.0-rc2,8.0.0-rc2>> | <<breaking-changes-8.0.0-rc1,8.0.0-rc1>> | <<breaking-changes-8.0.0-beta1,8.0.0-beta1>> | <<breaking-changes-8.0.0-alpha2,8.0.0-alpha2>> | <<breaking-changes-8.0.0-alpha1,8.0.0-alpha1>>
...
[float]
[[management-breaking-changes-8.0.0-beta1]]
==== Management

[discrete]
[[breaking-113555]]
.Removes console settings
[%collapsible]
====
*Details* +
The `console.proxyFilter` and `console.proxyConfig` settings have been removed. For more information, refer to {kibana-pull}113555[#113555].

*Impact* +
Before you upgrade to 8.0.0, remove `console.proxyFilter` and `console.proxyConfig` from kibana.yml, then use the <<settings,`server.ssl*`>> and <<elasticsearch-ssl-keyPassphrase,`elasticsearch.ssl.keyPassphrase`>> instead.
====
