...
For 7.0.0-alpha1, {kib} gives users the ability to switch between the old 6.x 
style headers and the new K7 design. To turn on the new design, go to 
*Management > Advanced Settings* and search for *K7 design*. In future versions, 
we'll remove the old setting and use the K7 design instead.
