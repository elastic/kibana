...
* Fixes bug that prevented panels in sections from URL state from being transformed ({kibana-pull}237382[#237382]).
...
* Removes unecessary `_source` from query as we use fields ({kibana-pull}239205[#239205]).
...
Management::
* Fixed a bug in component template creation flow where a new component template with `@custom` suffix in name would lead to updating mappings of all unrelated data streams and a message would display asking to rollover conflicting ones ({kibana-pull}237952[#237952]).
Search::
