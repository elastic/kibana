...
=== Fixes
Alerting::
* Fixes an issue with the interface freezing when an error occurred after deleting a connector ({kibana-pull}221958[#221958]).
Dashboards and Visualizations::
...
Data ingestion and Fleet::
* Fixes the `UnenrollInactiveAgentsTask` query in {{fleet}} to un-enroll only those agents that are inactive for longer than what is defined in the `unenroll_timeout` setting [#222592]({{kib-pull}}222592). ({kibana-pull}222592[#222592]).
...
* Fixes an issue with Uptime rules appearing on the **Create rule** modal even when the `observability:enableLegacyUptimeApp` setting is disabled ({kibana-pull}220005[#220005]).
Elastic Security solution::
