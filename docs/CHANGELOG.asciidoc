...
* Adds a MongoDB connector `datetime_conversion` (**Out-of-range date handling**) setting so syncs can clamp, convert, or error on dates outside the supported range instead of always aborting ({kibana-pull}277685[#277685]).
