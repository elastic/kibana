...
Do not upgrade Kibana from 8.3 to 8.4 if you have any alerting rules with a snooze notification configured.

You can identify snoozed rules in {kib} by navigating to **Stack Management** -> **Rules and Connectors**.
Filter the rule list by selecting **View** -> **Snoozed**.
Cancel the snooze for all affected rules before upgrading. This must be done for each Space.

You can also identify snoozed rules in any Space in {kib} **Dev Tools**.
Run the following query and searching for the string `is_snoozed_until`.
