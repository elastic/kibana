...
Any JavaScript (or TypeScript) file that imports SASS (.scss) files will automatically build with the EUI & Kibana invisibles (SASS variables, mixins, functions) from the `styling_constants.scss` file. However, any Legacy (file path includes `/legacy`) files will not.
