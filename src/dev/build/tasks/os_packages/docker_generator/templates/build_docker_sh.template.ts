...
function generator({
  ...
  docker build -t ${imageTag}${imageFlavor}${ubiImageFlavor}:${version}-${architecture} -f Dockerfile . || exit 1;

  docker save ${imageTag}${imageFlavor}${ubiImageFlavor}:${version}-${architecture} | gzip -c > ${dockerTargetFilename}
