...
export const BuildRspackBundles: Task = {
  ...
  async run(buildConfig, log, build) {
    ...
    // [rspack-transition] TODO: Implement proper bundle metrics collection for the
    // rspack optimizer (size limits, module counts, CI stats integration). For now,
    // emit an empty metrics file so post_build_kibana.sh's ship_ci_stats call does
    // not fail when it reads optimizer_bundle_metrics.json.
    await write(
