...
export function getNodeDownloadInfo(config: Config, platform: Platform) {
  ...
  const pointerCompressionPath = platform.isLinux() ? 'node-pointer-compression/' : '';
  const url = `https://us-central1-elastic-kibana-184716.cloudfunctions.net/kibana-ci-proxy-cache/${pointerCompressionPath}dist/v${version}/${downloadName}`;
