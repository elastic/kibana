...
// We own only the packages: block (derived from kibana.jsonc discovery). The
// rest of pnpm-workspace.yaml — install settings, allowBuilds, overrides — is
// authored by hand and left untouched. Keep these markers in sync with
...
async function writeWorkspaceFile(pkgs, log) {
  ...
  const block = [PACKAGES_START, 'packages:', ...dirs.map((d) => `  - '${d}'`), PACKAGES_END].join(
    '\n'
  );

  const current = await readIfExists(WORKSPACE_PATH);
  const re = new RegExp(`${PACKAGES_START}[\\s\\S]*?${PACKAGES_END}`);
  if (current === undefined || !re.test(current)) {
    throw new Error(
