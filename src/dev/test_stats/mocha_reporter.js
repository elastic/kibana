...
import { getSnapshotOfRunnableLogs } from '../mocha/log_cache';

// capture binding to Date.now() so that timer mocks don't break test times
const dateNow = Date.now.bind(Date);
const MOCHA_PKG = require('mocha/package.json');

export function setupTestStatsMochaReporter(runner, options = {}) {
  const {
  ...
  runner.on('end', () => {
    ...
    const reportPath = resolve(rootDirectory, `target/junit/test-summary.ndjson`);
    mkdirp.sync(dirname(reportPath));
    writeFileSync(
      reportPath,
      JSON.stringify({
        name: reportName,
        mocha: MOCHA_PKG.version,
        rootDirectory,
        ...
        tests: [...results, ...skippedResults].map(result => ({
          title: result.node.title,
          parents: getParentTitles(result.node).filter(Boolean),
          path: getPath(result.node),
          time: getDuration(result.node),
          systemOut: getSnapshotOfRunnableLogs(result.node) || '',
          failure: result.error ? inspect(result.error) : undefined,
