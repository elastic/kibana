...
const THIS_PATH = Path.resolve(
  REPO_ROOT,
  'src/dev/packages/test/src/functional_test_runner/lib/config/run_check_ftr_configs_cli.ts'
);
const THIS_REL = Path.relative(REPO_ROOT, THIS_PATH);

const IGNORED_PATHS = [
  THIS_PATH,
  Path.resolve(REPO_ROOT, 'src/dev/packages/test/src/jest/run_check_jest_configs_cli.ts'),
  Path.resolve(
    REPO_ROOT,
