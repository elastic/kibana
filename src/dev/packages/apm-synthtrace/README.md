...
Via the CLI, you can run scenarios, either using a fixed time range or continuously generating data. Scenarios are available in [`src/dev/packages/apm-synthtrace/src/scenarios/`](https://github.com/elastic/kibana/blob/main/src/dev/packages/apm-synthtrace/src/scenarios/).
...
```
node scripts/jest --config ./src/dev/packages/apm-synthtrace/jest.config.js
```
...
```
node scripts/type_check.js --project src/dev/packages/apm-synthtrace/tsconfig.json
```
