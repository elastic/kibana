...
import fakeNamesIndexTemplate from './fixtures/fake_names_index_template.json';
import fs from 'fs';
...
export default function (scenarioManager, request) {
  ...
  describe('_data', function () {
    ...
    it('should accept a multipart/form-data request with a csv file attached', function () {
      return request.post('/kibana/names/_data')
      .attach('csv', fixture('fake_names.csv'))
      .expect(200);
    });

    it('should also accept the raw csv data in the payload body', function () {
      const csvData = fs.readFileSync(fixture('fake_names_big.csv'), { encoding: 'utf8' });
    ...
    it('should return JSON results', function () {
      return request.post('/kibana/names/_data')
      .attach('csv', fixture('fake_names.csv'))
    ...
    it('should index one document per row in the csv', function () {
      return request.post('/kibana/names/_data')
      .attach('csv', fixture('fake_names.csv'))
      .expect(200)
      .then(() => {
    ...
    it('should stream a chunked response', function () {
      return request.post('/kibana/names/_data')
      .attach('csv', fixture('fake_names.csv'))
    ...
    it('should respond with an array of one or more "result objects"', function () {
      return request.post('/kibana/names/_data')
        .attach('csv', fixture('fake_names_big.csv'))
        .expect(200)
        .then((dataResponse) => {
    ...
    describe('result objects', function () {
      ...
      it('should include a count of created documents', function () {
        return request.post('/kibana/names/_data')
        .attach('csv', fixture('fake_names.csv'))
        .expect(200)
        .then((dataResponse) => {
      ...
      it('should report any indexing errors per document under an "errors.index" key', function () {
        return request.post('/kibana/names/_data')
          .attach('csv', fixture('fake_names_with_mapping_errors.csv'))
          .expect(200)
          .then((dataResponse) => {
            expect(dataResponse.body[0]).to.have.property('created');
      ...
      it('should use the filename and line numbers as document IDs', function () {
        return request.post('/kibana/names/_data')
          .attach('csv', fixture('fake_names_with_mapping_errors.csv'))
          .expect(200)
          .then((dataResponse) => {
      ...
      it('should report any csv parsing errors under an "errors.other" key', function () {
        return request.post('/kibana/names/_data')
          .attach('csv', fixture('fake_names_with_parse_errors.csv'))
          .expect(200)
          .then((dataResponse) => {
    ...
    describe('optional parameters', function () {
      it('should accept a custom csv_delimiter query string param for parsing the CSV', function () {
        return request.post('/kibana/names/_data?csv_delimiter=|')
        .attach('csv', fixture('fake_names_pipe_delimited.csv'))
        .expect(200)
        .then((dataResponse) => {
          expect(dataResponse.body[0]).to.have.property('created');
      ...
      it('should accept a boolean pipeline query string parameter enabling use of the index pattern\'s associated pipeline',
      function () {
