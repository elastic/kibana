...
export default function (scenarioManager, request) {
  describe('simulate', function simulatePipeline() {
    ...
    describe('compilation errors', function simulatePipeline() {
      const pipeline = {
        input: { foo: '[message]' },
        processors: [
          {
      ...
      it('should return a 200 for a compile error caused by a processor', function () {
        request.post('/kibana/ingest/simulate')
        .send(pipeline)
        .expect(200)
        .then((response) => {
      ...
      it('should only return a result for the processor that threw the error', function () {
        request.post('/kibana/ingest/simulate')
        .send(pipeline)
        .expect(200)
        .then((response) => {
    ...
    describe('data errors', function simulatePipeline() {
      const pipeline = {
        input: { foo: '[message]' },
        processors: [
          {
      ...
      it('should return 200 with non-compile error object for a processor with an invalid source_field', () => {
        return Promise.all([
          request.post('/kibana/ingest/simulate')
          .send(pipeline)
          .expect(200)
      ...
      it('should return results up to and including the erroring processor', () => {
        return Promise.all([
          request.post('/kibana/ingest/simulate')
          .send(pipeline)
          .expect(200)
