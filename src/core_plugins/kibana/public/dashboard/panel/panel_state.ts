...
function findTopLeftMostOpenSpace(
  ...
  currentPanels.forEach(panel => {
    for (
      let x = panel.gridData.x;
      x < panel.gridData.x + panel.gridData.w;
      x++
    ) {
      for (
        let y = panel.gridData.y;
        y < panel.gridData.y + panel.gridData.h;
        y++
      ) {
        grid[y][x] = 1;
      }
    ...
    for (let x = 0; x < DASHBOARD_GRID_COLUMN_COUNT; x++) {
      if (grid[y][x] === 1) {
        ...
        for (let h = y; h < Math.min(y + height, maxY); h++) {
          for (
            let w = x;
            w < Math.min(x + width, DASHBOARD_GRID_COLUMN_COUNT);
            w++
          ) {
