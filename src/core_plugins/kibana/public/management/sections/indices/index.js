...
const INDEX_PATTERN_DOM_ELEMENT_ID = 'indexPatternReact';

export function updateIndexPattern(
  $scope,
  ...
  render(
    <I18nProvider>
      {indexPatterns.length > 2 ? <IndexPatternTable indexPatterns={indexPatterns} /> : <RenderCreateIndexPatternPrompt />}
    </I18nProvider>,
    node,
  );
}
...
export function updateIndexPatternList(
  ...
  console.log('updateIndexPatternList', indexPatterns);
...
export const destroyIndexPattern = () => {
  const node = document.getElementById(INDEX_PATTERN_DOM_ELEMENT_ID);
  node && unmountComponentAtNode(node);
};
    ...
    return {
      ...
      link: async function ($scope) {
        ...
        const renderList = () => {
          ...
          updateIndexPattern($scope, indexPatternCreationOptions, $scope.defaultIndex, $scope.indexPatternList);
        };

        $scope.$on('$destroy', destroyIndexPatternList);
        //$scope.$on('$destroy', destroyIndexPattern);
