...
export const wrapWithSortProps = ({ defaults, getState, setState}) => {
  return (BaseComponent) => (props) => {
    const { sortBy = defaults.sortBy, sortAsc = defaults.sortAsc } = getState();
    const sortProps = {
      sortBy,
      sortAsc,
      changeSort: field => {
    ...
    return <BaseComponent {...props} {...sortProps}/>
  }
};
