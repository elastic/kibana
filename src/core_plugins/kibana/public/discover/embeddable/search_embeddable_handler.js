...
export class SearchEmbeddableHandler extends EmbeddableHandler {
  ...
  async getTitleFor(panelId) {
    const savedObject = await this.searchLoader.get(panelId);
    return savedObject.title;
  }

  async render(domNode, panel, container) {
    const savedObject = await this.searchLoader.get(panel.id);
    const editUrl = await this.getEditPath(panel.id);
