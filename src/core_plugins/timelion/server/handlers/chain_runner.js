...
let parseSheet = require('./lib/parse_sheet.js');
let parseDateMath = require('../lib/date_math.js');
let calculateInterval = require('../../public/lib/calculate_interval.js');

let repositionArguments = require('./lib/reposition_arguments.js');
let indexArguments = require('./lib/index_arguments.js');
let validateTime = require('./lib/validate_time.js');
...
module.exports = function (tlConfig) {
  let preprocessChain = require('./lib/preprocess_chain')(tlConfig);
  ...
  function invoke(fnName, args) {
    let functionDef = tlConfig.server.plugins.timelion.getFunction(fnName);
  ...
  function invokeChain(chainObj, result) {
    ...
    let chain = _.clone(chainObj.chain);
    ...
    if (link.type === 'chain') {
      ...
      let args = link.arguments ? result.concat(link.arguments) : result;
  ...
  function resolveChainList(chainList) {
    let seriesList = _.map(chainList, function (chain) {
      let values = invoke('first', [chain]);
      return values.then(function (args) {
        return args;
      });
    });
    return Promise.all(seriesList).then(function (args) {
      let list = _.chain(args).pluck('list').flatten().value();
      let seriesList = _.merge.apply(this, _.flatten([{}, args]));
  ...
  function preProcessSheet(sheet) {
    ...
    _.each(sheet, function (chainList, i) {
      try {
        let queriesInCell = _.mapValues(preprocessChain(chainList), function (val) {
          val.cell = i;
          return val;
        });
    ...
    let promises = _.chain(queries).values().map(function (query) {
    ...
    return Promise.settle(promises).then(function (resolvedDatasources) {
      ...
      _.each(queries, function (query, i) {
        let functionDef = tlConfig.server.plugins.timelion.getFunction(query.function);
        let resolvedDatasource = resolvedDatasources[i];
