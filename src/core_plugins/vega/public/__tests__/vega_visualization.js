...
import embeddedDataGraph from '!!raw-loader!./embedded_data_graph.json';
import embeddedDataImage256 from './embedded_data_image_256.png';
import embeddedDataImage512 from './embedded_data_image_512.png';
...
describe('VegaVisualizations', () => {
  ...
  describe('VegaVisualization - basics', () => {
    ...
    it('should show the graph and update on resize', async function () {

      let vegaVis;
      try {
        vegaVis = new VegaVisualization(domNode, vis);
        const vegaParser = new VegaParser(JSON.parse(embeddedDataGraph), new SearchCache());
        await vegaParser.parseAsync();

        await vegaVis.render(vegaParser, { data: true });
        const mismatchedPixels1 = await compareImage(embeddedDataImage512);
        ...
        const mismatchedPixels2 = await compareImage(embeddedDataImage256);
