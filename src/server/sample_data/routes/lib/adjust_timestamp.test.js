...
describe('preserve day of week and time of day', () => {
  const currentTimeMarker = new Date(Date.parse('2018-01-02T00:00:00'));
  const now = new Date(Date.parse('2018-04-25T18:24:58.650Z')); // Wednesday
  ...
  test('adjusts time to monday of the same week as now', () => {
    ...
    expect(timestamp).toBe('2018-04-23T23:50:00');
  });

  test('adjusts time to friday of the same week as now', () => {
    const originalTimestamp = '2017-12-29T23:50:00'; // Friday, same week relative to currentTimeMarker
    const timestamp = adjustTimestamp(originalTimestamp, currentTimeMarker, now, true);
    expect(timestamp).toBe('2018-04-27T23:50:00');
  });

  test('adjusts time to monday of the previous week as now', () => {
    const originalTimestamp = '2017-12-25T23:50:00'; // Monday, previous week relative to currentTimeMarker
    const timestamp = adjustTimestamp(originalTimestamp, currentTimeMarker, now, true);
    expect(timestamp).toBe('2018-04-16T23:50:00');
  });

  test('adjusts time to friday of the week after now', () => {
    const originalTimestamp = '2018-01-05T23:50:00'; // Friday, next week relative to currentTimeMarker
    const timestamp = adjustTimestamp(originalTimestamp, currentTimeMarker, now, true);
    expect(timestamp).toBe('2018-05-04T23:50:00');
  });
