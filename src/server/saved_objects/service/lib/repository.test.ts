...
describe('SavedObjectsRepository', () => {
  ...
  let getSearchDsl: sinon.SinonStub;
  let callAdminCluster: sinon.SinonStub;
  let onBeforeWrite: sinon.SinonStub;
  ...
  beforeEach(() => {
    ...
    sandbox.stub(savedObjectsRepository, 'getCurrentTime' as any).returns(mockTimestamp);
    getSearchDsl = sandbox.stub(getSearchDslNS, 'getSearchDsl').returns({});
  });
  ...
  describe('#find', () => {
    ...
    it('requires searchFields be an array if defined', async () => {
      try {
        await savedObjectsRepository.find({ searchFields: 'string' as any });
    ...
    it('requires fields be an array if defined', async () => {
      try {
        await savedObjectsRepository.find({ fields: 'string' as any });
        throw new Error('expected find() to reject');
      } catch (error) {
        sinon.assert.notCalled(callAdminCluster);
        sinon.assert.notCalled(onBeforeWrite);
    ...
    it('formats Elasticsearch response', async () => {
      ...
      searchResults.hits.hits.forEach((doc: any, i) => {
  ...
  describe('onBeforeWrite', () => {
    ...
    it('can throw es errors and have them decorated as SavedObjectsClient errors', async () => {
      ...
      const es401 = new elasticsearch.errors.AuthenticationException();
