...
export function validate(obj: any) {
  if (!isObject(obj) || obj instanceof IncomingMessage) {
    return;
  }
  ...
  while (stack.length > 0) {
    ...
    if (hasOwnProperty(value, '__proto__')) {
      throw badRequest(`__proto__ is an invalid key`);
    }

    if (hasOwnProperty(value, 'prototype') && previousKey === 'constructor') {
      throw badRequest(`constructor.prototype is an invalid key`);
    }
...
export function setupPrototypePollutionValidation(server: Server) {
  server.ext('onPostAuth', function onPostAuthPrototypePollutionValidation(req, h) {
