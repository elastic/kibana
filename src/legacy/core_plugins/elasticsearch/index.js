  ...
  return new kibana.Plugin({
    ...
    async init(server) {
      ...
      const [esConfig, adminCluster, dataCluster] = await combineLatest(
        server.newPlatform.core.elasticsearch.legacy.config$,
        server.newPlatform.core.elasticsearch.adminClient$,
        server.newPlatform.core.elasticsearch.dataClient$
      ).pipe(
        first(),
      ...
      server.expose('createCluster', (name, clientConfig = {}) => {
        ...
        const cluster = new Cluster(server.newPlatform.core.elasticsearch.createClient(name, {
          ...esConfig,
          ...clientConfig,
        }));
