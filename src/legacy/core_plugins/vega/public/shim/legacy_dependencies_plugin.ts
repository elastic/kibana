...
export class LegacyDependenciesPlugin implements Plugin<any, any> {
  public setup() {
    ...
    return {
      ...
      es: async () => (await chrome.dangerouslyGetActiveInjector()).get('es'),
      ...
      serviceSettings: () => chrome.veryDangerouslyGetActiveInjector().get('serviceSettings'),
