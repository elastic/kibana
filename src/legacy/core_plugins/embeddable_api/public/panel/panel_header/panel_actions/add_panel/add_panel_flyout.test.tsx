...
test('adds a panel to the container', async done => {
  ...
  findTestSubject(component, `createNew-${GREETING_EMBEDDABLE}`).simulate('click');

  await nextTick();

  (modalContents.props as GreetingInitializerProps).onCreate({
    firstName: 'Dany',
    lastName: 'Targaryan',
  });
  ...
  const child = container.getChild<GreetingEmbeddable>('greeting');
