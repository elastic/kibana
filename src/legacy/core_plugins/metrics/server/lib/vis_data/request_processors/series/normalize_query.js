...
const hasPipelineAggregations = (aggs = {}) => Object.keys(aggs).length > 1;
 ...
 *
 * Important: for Pipeline aggregations we cannot apply this logic
 *
 */
function removeEmptyTopLevelAggregation(doc, series) {
  const filter = get(doc, `aggs.${series.id}.filter`);

  if (isEmptyFilter(filter) && !hasPipelineAggregations(doc.aggs[series.id].aggs)) {
