...
export function initEditorDirective(app, deps) {
  app.directive('visualizeApp', function () {
    return {
      ...
      controller: function VisualizeAppController(
        $scope,
        $element,
        $route,
        AppState,
        ...
        $scope.topNavMenu = [...(visualizeCapabilities.save ? [{
          ...
          run: (anchorElement) => {
            ...
            showShareContextMenu({
              ...
              getUnhashableStates: deps.getUnhashableStates,
              objectId: savedVis.id,
              objectType: 'visualization',
              shareContextMenuExtensions: deps.shareContextMenuExtensions,
              sharingData: {
                title: savedVis.title,
              },
        ...
        $scope.$watch('state.savedQuery', newSavedQueryId => {
          ...
          if (!$scope.savedQuery || newSavedQueryId !== $scope.savedQuery.id) {
            deps.savedQueryService.getSavedQuery(newSavedQueryId).then((savedQuery) => {
              $scope.$evalAsync(() => {
                $scope.savedQuery = savedQuery;
