...
export const buildPointSeriesData = (table, dimensions) => {
  ...
  const { intervalESUnit, intervalESValue, interval, bounds } = params;
  chart.ordered = {
    interval: moment.duration(interval),
    intervalESUnit: intervalESUnit,
    intervalESValue: intervalESValue,
    date: true,
    min: isNaN(bounds.min) ? Date.parse(bounds.min) : bounds.min,
    max: isNaN(bounds.max) ? Date.parse(bounds.max) : bounds.max,
  };
