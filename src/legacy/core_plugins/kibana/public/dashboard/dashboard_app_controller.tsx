...
export class DashboardAppController {
  ...
  private savedDashboard: SavedObjectDashboard;
  private updateFiltersSubscription?: Subscription;
  private fetchFiltersSubscription?: Subscription;
  private lastReloadRequestTime: number = 0;
  ...
  constructor({
    ...
    this.savedDashboard = $scope.dash = $route.current.locals.dash;
    if (this.savedDashboard.id) {
      this.docTitle.change(this.savedDashboard.title);
    }

    this.dashboardStateManager = new DashboardStateManager({
      savedDashboard: this.savedDashboard,
      AppStateClass,
    ...
    this.appStatus = {
      dirty: !this.savedDashboard.id,
    };

    this.dashboardStateManager.registerChangeListener((status: { dirty: boolean }) => {
      this.appStatus.dirty = status.dirty || !this.savedDashboard.id;
      this.updateState();
    });
    ...
    $scope.updateQueryAndFetch = ({ query, dateRange }) => {
      ...
      if (_.isEqual(oldQuery, query)) {
        ...
        this.lastReloadRequestTime = new Date().getTime();
    ...
    $scope.onFiltersUpdated = (filters: Filter[]) => this.onFiltersUpdated(filters);
    ...
    this.updateFiltersSubscription = this.queryFilter.getUpdates$().subscribe({
      next: () => {
        this.onFiltersUpdated(this.queryFilter.getFilters());
      },
    });
    ...
    $scope.$on('$destroy', () => {
      ...
      if (this.fetchFiltersSubscription) {
        this.fetchFiltersSubscription.unsubscribe();
      }
    });
  }

  private async onFiltersUpdated(filters: Filter[]) {
    await this.queryFilter.setFilters(filters);
    this.$scope.model.filters = this.queryFilter.getFilters();
    this.refreshDashboardContainer();
    this.dashboardStateManager.applyFilters(this.$scope.model.query, filters);
  }
  ...
  private initializeTopNavActions() {
    ...
    this.navActions[TopNavIds.ADD] = () => {
      if (this.dashboardContainer && !isErrorEmbeddable(this.dashboardContainer)) {
        openAddPanelFlyout(this.dashboardContainer);
      }
    };
  ...
  private getDashboardInput(): DashboardContainerInput {
    ...
    this.dashboardStateManager.getPanels().forEach((panel: SavedDashboardPanel) => {
      embeddablesMap[panel.panelIndex] = convertSavedDashboardPanelToPanelState(
        panel,
        this.dashboardStateManager.getUseMargins()
      );
    });
    let expandedPanelId;
    const dashboardContainer = this.dashboardContainer;
    ...
    return {
      ...
      useMargins: this.dashboardStateManager.getUseMargins(),
      lastReloadRequestTime: this.lastReloadRequestTime,
    ...
    dashboardFactory
      ...
      .then((container: DashboardContainer | ErrorEmbeddable) => {
        if (!isErrorEmbeddable(container)) {
          ...
          this.outputSubscription = this.dashboardContainer.getOutput$().subscribe(() => {
            this.updateIndexPatterns();
          });

          this.inputSubscription = this.dashboardContainer.getInput$().subscribe(async () => {
            let dirty = false;
            // This has to be first because handleDashboardContainerChanges cuases
            // appState.save which will cause refreshDashboardContainer to be called.

            // Add filters modifies the object passed to it, hence the clone deep.
            if (!_.isEqual(container.getInput().filters, this.queryFilter.getFilters())) {
              await this.queryFilter.addFilters(_.cloneDeep(container.getInput().filters));
          ...
          this.dashboardStateManager.registerChangeListener(dirty => {
            if (dirty) {
              this.refreshDashboardContainer();
            }
          });
          ...
          if (this.routeParams[DashboardConstants.NEW_VISUALIZATION_ID_PARAM]) {
            container.addSavedObjectEmbeddable(
              ...
              this.routeParams[DashboardConstants.NEW_VISUALIZATION_ID_PARAM]
            );
