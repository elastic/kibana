...
describe('Saved Object', function() {
  ...
  beforeEach(
    ngMock.inject(function(es, Private, $window) {
      ...
      describe('when true', function() {
        it('requests confirmation and updates on yes response', function() {
          ...
          return createInitializedSavedObject({ type: 'dashboard', id: 'myId' }).then(
            savedObject => {
              ...
              return savedObject.save({ confirmOverwrite: true }).then(() => {
        ...
        it('does not update on no response', function() {
          ...
          return createInitializedSavedObject({ type: 'dashboard', id: 'HI' }).then(savedObject => {
            ...
            sinon
              .stub(savedObjectsClientStub, 'create')
              .returns(Bluebird.reject(mock409FetchError));
