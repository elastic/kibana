...
export function callClient(searchRequests, requestsOptions = [], { Promise, es, config, sessionId, esShardTimeout }) {
  ...
  Object.keys(searchStrategyMap).forEach(searchStrategyId => {
    ...
    const { searching, abort } = searchStrategy.search({ searchRequests: requests, es, Promise, config, sessionId, esShardTimeout });
