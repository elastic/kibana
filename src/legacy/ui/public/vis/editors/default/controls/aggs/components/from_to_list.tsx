...
function FromToList({ showValidation, onBlur, ...rest }: FromToListProps) {
  const fromToListConfig: InputListConfig = {
    ...
    getRemoveBtnAriaLabel: (item: FromToModel) =>
      i18n.translate('data.aggTypes.ipRanges.removeRangeAriaLabel', {
    ...
    renderInputRow: (item: FromToModel, index, onChangeValue) => (
      <>
        <EuiFlexItem>
          <EuiFieldText
            aria-label={i18n.translate('data.aggTypes.ipRanges.ipRangeFromAriaLabel', {
        ...
        <EuiFlexItem>
          <EuiFieldText
            aria-label={i18n.translate('data.aggTypes.ipRanges.ipRangeToAriaLabel', {
