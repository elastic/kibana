...
let _time = undefined;
timefilterSetupMock.timefilter.setTime.mockImplementation((time) => {
  _time = {
    from: time.from.toISOString(),
    to: time.to.toISOString(),
  };
});
timefilterSetupMock.timefilter.getTime.mockImplementation(() => {
  return _time;
});
const { timefilter } = timefilterSetupMock;
...
describe('brushEvent', () => {
  ...
  test('ignores event when data.xAxisField not provided', () => {
    ...
    onBrushEvent(event, $state, timefilter);
    expect($state)
