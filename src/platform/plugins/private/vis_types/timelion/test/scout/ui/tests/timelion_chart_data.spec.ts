...
test.describe('Timelion visualization - chart data', { tag: tags.stateful.classic }, () => {
  registerTimelionSuiteHooks(test, { includeLongWindowLogstash: true });
  ...
  test('should display correct data for specified index pattern and timefield', async ({
    ...
    expect(chartData).toStrictEqual([3, 5, 2, 6, 1, 6, 1, 7, 0, 0]);
  });
  ...
  test('should display correct chart data for average, min, max and cardinality aggregations', async ({
    ...
    expect(firstAreaChartData).toStrictEqual([5732.783676366217, 5721.775973559419]);
    expect(secondAreaChartData).toStrictEqual([0, 0]);
    expect(thirdAreaChartData).toStrictEqual([19985, 19986]);
    expect(forthAreaChartData).toStrictEqual([5019, 4958, 0, 0]);
  });
  ...
  test('should display correct chart data for expressions using functions', async ({
    ...
    expect(firstAreaChartData).toStrictEqual(firstAreaExpectedChartData);
    expect(secondAreaChartData).toStrictEqual(firstAreaExpectedChartData);
    expect(thirdAreaChartData).toStrictEqual(thirdAreaExpectedChartData);
    expect(forthAreaChartData).toStrictEqual(forthAreaExpectedChartData);
  });
  ...
  test('should display correct chart title, data and labels for expressions with custom labels, yaxis and offset', async ({
    ...
    expect(firstAreaChartData).toStrictEqual(firstAreaExpectedChartData);
    expect(secondAreaChartData).toStrictEqual(secondAreaExpectedChartData);
    expect(thirdAreaChartData).toStrictEqual(thirdAreaExpectedChartData);
    expect(firstAxesLabels).toStrictEqual(['12.2GB', '12.21GB']);
    expect(secondAxesLabels).toStrictEqual(['5.59KB', '5.6KB']);
  ...
  test('should display correct chart data for split expression', async ({ pageObjects }) => {
    ...
    expect(firstAreaChartData).toStrictEqual([0, 905, 910, 850, 0]);
    expect(secondAreaChartData).toStrictEqual([0, 763, 699, 825, 0]);
    expect(thirdAreaChartData).toStrictEqual([0, 423, 386, 389, 0]);
  });
