...
/** Id of the document in the annotations index that holds the comment count; never a comment id. */
...
export const routeFromLegacy = ({ pathname, url }: LegacyRoute): AnnotationRoute => {
  try {
...
export const normalizeRoute = (route: AnnotationRoute | LegacyRoute): AnnotationRoute =>
  'pageKey' in route ? route : routeFromLegacy(route);

export const newAnnotationSchema = schema.object({
  author: authorSchema,
  text: textSchema,
...
export const annotationPatchSchema = schema.object({
...
export const importBodySchema = schema.object({
  ...
  annotations: schema.arrayOf(schema.object({}, { unknowns: 'allow' }), {
    maxSize: MAX_ANNOTATIONS,
  }),
});
...
const importedAnnotationSchema = newAnnotationSchema
  .extends({
    id: idSchema,
    createdAt: timestampSchema,
...
export const parseImport = ({
  exportedAt,
  annotations,
}: TypeOf<typeof importBodySchema>): { payload: AnnotationsExport; skipped: number } => {
  const readable: Annotation[] = [];
  for (const item of annotations) {
    try {
      const { route, ...annotation } = importedAnnotationSchema.validate(item);
      readable.push({ ...annotation, route: normalizeRoute(route) });
    } catch {
      // written by an incompatible version of the layer
    }
  }
  return {
    payload: { version: 2, exportedAt, annotations: readable },
    skipped: annotations.length - readable.length,
  };
};
