...
export const ANNOTATIONS_INDEX = '.kibana-developer-toolbar-annotations';
...
export const ensureAnnotationsIndex = async (
  esClient: ElasticsearchClient,
  logger: Logger
): Promise<void> => {
  if (await esClient.indices.exists({ index: ANNOTATIONS_INDEX })) {
    return;
  }
  try {
  ...
  logger.info(`Created developer toolbar annotations index [${ANNOTATIONS_INDEX}]`);
};
