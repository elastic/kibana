...
import { registerAnnotationsRoutes } from './routes';
...
describe('annotations routes', () => {
  ...
  const client = {
    ...
    update: jest.fn(),
    importAll: jest.fn(),
  } as unknown as jest.Mocked<AnnotationsClient>;
  registerAnnotationsRoutes(router, Promise.resolve(client));
  ...
  it('turns limit violations into bad requests the user can act on', async () => {
    client.create.mockRejectedValueOnce(new AnnotationsLimitError('Store is full'));
    const response = await call('post', ANNOTATIONS_API_PATH, {});
  ...
  it('adds the records the import could not read to those it could not write', async () => {
    ...
    const response = await call('post', `${ANNOTATIONS_API_PATH}/import`, {
      version: 2,
  ...
  it('lets other failures through', async () => {
    ...
    await expect(call('patch', `${ANNOTATIONS_API_PATH}/{id}`, {})).rejects.toThrow('cluster down');
  });
});
