...
import { isSafeRelativePath, type AnnotationsHostServices } from '@kbn/ui-annotations';
import { routeFromLocation, type AnnotationRoute } from '../../common/annotations';
import { createAnnotationsApi } from './annotations_api';
import { captureViewport } from './capture_viewport';

/** Developer toolbar and `@kbn/design-tools` UI that must never be annotated (mirrors `IGNORED_SELECTOR` in `@kbn/design-tools`). */
...
export const routeOf = (
  ...
  basePath: IBasePath
): AnnotationRoute => routeFromLocation({ pathname: basePath.remove(pathname), search, hash });

export const createAnnotationsHostServices = ({
  http,
  application,
  security,
}: Pick<CoreStart, 'http' | 'application' | 'security'>): AnnotationsHostServices => ({
  api: createAnnotationsApi(http),
