...
export type ScopedFilesClient<M = unknown> = {
    [K in keyof FilesClient]: K extends 'list' ? (arg?: Omit<Parameters<FilesClient<M>[K]>[0], 'kind'>) => ReturnType<FilesClient<M>[K]> : (arg: Omit<Parameters<FilesClient<M>[K]>[0], 'kind'>) => ReturnType<FilesClient<M>[K]>;
};
/**
