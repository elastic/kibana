...
spaceTest.describe('Console output copy to clipboard', { tag: tags.deploymentAgnostic }, () => {
  ...
  spaceTest(
    'copies the selected output on a human-speed (held) click',
    async ({ page, pageObjects }) => {
      await spaceTest.step('run a request and select its output', async () => {
        await pageObjects.console.sendRequest();
        await pageObjects.console.selectOutput();
      });

      await spaceTest.step(
        'press the copy button, holding it longer than the blur-hide delay',
        async () => {
          await pageObjects.console.slowClickCopyOutput(HUMAN_CLICK_HOLD_MS);
        }
      );

      await spaceTest.step('the copy completes with a success toast', async () => {
