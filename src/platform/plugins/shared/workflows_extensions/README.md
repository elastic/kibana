...
This section provides a complete guide for contributors who want to add event-driven triggers so workflows can subscribe and run when events occur. Triggers are registered from **your plugin**, not from inside `workflows_extensions` (same ownership rule as [custom steps](#contributing-custom-step-types)).
...
- **Trigger**: An event type workflows can subscribe to; defined by a unique id and an event payload schema.
- **Trigger id**: Namespaced string with **kebab-case** namespace and **camelCase** event (e.g. `example.customTrigger`, `cases.updated`). Must match between server and public registration.
- **eventSchema**: Zod object schema for the payload; used for validation when emitting and for UI/docs.
- **emitEvent**: Function to fire an event for a trigger; subscribed workflows in the same space run (if a trigger event handler is registered).
- **Request-scoped client**: `getWorkflowsClient()` from `context.workflows`; use in route handlers. Binds space and request automatically.
- **Conditional subscription**: In workflow YAML, `triggers[].on.condition` (KQL) filters which events run the workflow (e.g. `event.severity: "high"`).
