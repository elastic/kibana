...
3. The approved list lives in `test/scout_workflows_extensions/api/fixtures/approved_trigger_definitions.ts`. That Scout suite is the **full catalog** on stateful classic: CI boots the `workflows_extensions` config set (Alerting v2, Significant Events, Nightshift) so every registered trigger is present.
...
Some triggers are only registered when the owning plugin is loaded (Kibana `enabled` config, e.g. `xpack.significantEvents.enabled`). That is boot-time config, not a runtime LaunchDarkly flag — `feature_flags.overrides` cannot load a disabled plugin.
