...
> = {
  ...
  documentation: {
    details: `The data.find step finds the first item in an array matching a KQL condition. Use \`item.field\` to reference item properties and \`index\` to access the item's position. Always returns \`{ item, index }\`. Use \`errorIfEmpty: true\` to return an error instead of null when no match is found.`,
    examples: [
