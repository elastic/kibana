...
| `APPROVED_TRIGGER_DEFINITIONS` | Add new ID + `schemaHash`; sorted by ID. Gated plugins that are not default-on go in the default Scout stateful config | test fails | CI blocks merge until updated |
...
- [ ] `APPROVED_TRIGGER_DEFINITIONS` is updated (sorted; new entry near related namespace); gated plugins that are not default-on are enabled in the default Scout stateful config; workflows-eng review requested
