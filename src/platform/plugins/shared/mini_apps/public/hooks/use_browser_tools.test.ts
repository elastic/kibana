...
import { createMiniAppBrowserTools } from './use_browser_tools';
...
describe('createMiniAppBrowserTools', () => {
  ...
  it('creates three browser tools', () => {
    const tools = createMiniAppBrowserTools({ getCode, setCode });
  ...
  it('get_code tool calls getCode', () => {
    const tools = createMiniAppBrowserTools({ getCode, setCode });
    const getCodeTool = tools.find((t) => t.id === 'mini_app_get_code')!;

    // The get_code handler takes an empty params object
    (getCodeTool.handler as (params: Record<string, never>) => void)({});
