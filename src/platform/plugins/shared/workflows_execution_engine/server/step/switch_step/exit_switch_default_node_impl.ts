...
export class ExitSwitchDefaultNodeImpl implements NodeImplementation {
  constructor(
    private node: ExitSwitchDefaultNode,
  ...
  public run(): void {
    ...
    if (exitSwitchNode) {
      ...
      throw new Error(
        `ExitSwitchDefaultNode ${this.node.id} must have an exit-switch successor, but none found.`
      );
    }
  }
}
