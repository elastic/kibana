...
describe('executeWorkflowSync', () => {
  ...
  describe('capabilities threading', () => {
    it('forwards capabilities from input onto each step handlerContext', async () => {
      const marker = { secret: 'present' };
      let capturedCapabilities: Record<string, unknown> | undefined;

      const capabilitiesStep: ServerStepDefinition = {
        ...
        outputSchema: {} as never,
        handler: async (ctx) => {
          capturedCapabilities = ctx.capabilities;
          return { output: {} };
        },
      };
      ...
      const result = await executeWorkflowSync({
        workflowDefinition: makeWorkflow({
          steps: [{ name: 'cap', type: 'test.capabilities' } as never],
        }),
        payload: {},
        maxTimeoutMs: 5000,
        getStepDefinition: (t) => (t === 'test.capabilities' ? capabilitiesStep : undefined),
        logger,
        capabilities: { marker },
      });

      expect(result.status).toBe('completed');
      expect(capturedCapabilities).toEqual({ marker });
      expect(capturedCapabilities?.marker).toBe(marker);
    });

    it('leaves capabilities undefined when not provided', async () => {
      let capturedCapabilities: Record<string, unknown> | undefined = { sentinel: true };
