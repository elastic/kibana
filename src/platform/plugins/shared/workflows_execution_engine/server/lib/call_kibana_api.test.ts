...
describe('callKibanaApi', () => {
  ...
  it('leaves the internal-caller attestation to Core, dropping any caller-supplied value', async () => {
    // The attestation is an HMAC bound to the exact credential, so Core's self client derives it
    // per attempt from whichever credential it is about to send — including on a 401 retry, where
    // a value computed here would already be stale.
    mockSelfFetch.mockResolvedValue(mockSelfResponse(createMockResponse({ body: { ok: true } })));
    ...
    expect(headers[UIAM_INTERNAL_CALLER_ATTESTATION_HEADER]).toBeUndefined();
    expect(headers[UIAM_ATTESTATION_HEADER]).toBeUndefined();
    expect(mockGetAttestationHeaders).not.toHaveBeenCalled();
    // The credential itself still goes out; only the attestation moved.
