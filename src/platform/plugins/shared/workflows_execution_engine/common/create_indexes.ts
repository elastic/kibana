...
import {
  WORKFLOWS_CONCURRENCY_SEMAPHORE_INDEX,
  WORKFLOWS_CONCURRENCY_SEMAPHORE_INDEX_MAPPINGS,
...
export async function createIndexes(options: CreateIndexesOptions): Promise<void> {
  ...
  await Promise.all([
    ...
    createIndexWithMappings({
      esClient,
      indexName: WORKFLOWS_CONCURRENCY_SEMAPHORE_INDEX,
      mappings: WORKFLOWS_CONCURRENCY_SEMAPHORE_INDEX_MAPPINGS,
      logger,
    }),
  ]);
}
