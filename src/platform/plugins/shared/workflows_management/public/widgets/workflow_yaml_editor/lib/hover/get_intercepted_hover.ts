...
export async function getInterceptedHover(
  ...
  try {
    ...
    for (const provider of hoverProviders) {
      ...
      if (hover) {
        // Clean up hover content generated by monaco-yaml:
        // - Remove "Source: ....json" suffixes
        // - Remove standalone "||" lines (empty oneOf union type descriptions)
        const trustedContents = hover.contents
          ...
          .filter((content) => {
            ...
            // Filter out empty entries and entries that are only pipes/whitespace
            // (empty oneOf union descriptions from monaco-yaml)
