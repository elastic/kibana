...
import {
  findBracketDottedKeyUsages,
  hasBracketDottedKeyUsage,
} from './alert_trigger_bracket_key_usage';
...
describe('findBracketDottedKeyUsages', () => {
  it('returns correct positions for a single match', () => {
    const yaml = "message: {{ event.alerts[0]['kibana.alert.rule.name'] }}";
    const matches = findBracketDottedKeyUsages(yaml);
    expect(matches).toHaveLength(1);
    expect(matches[0].lineNumber).toBe(1);
    expect(matches[0].text).toBe("['kibana.alert.rule.name']");
    expect(matches[0].startColumn).toBe(yaml.indexOf("['") + 1);
    expect(matches[0].endColumn).toBe(yaml.indexOf("']") + 3);
  });

  it('returns correct positions across multiple lines', () => {
    const yaml = `line one
message: {{ event.alerts[0]['kibana.alert.status'] }}
another: {{ x["service.name"] }}`;
    const matches = findBracketDottedKeyUsages(yaml);
    expect(matches).toHaveLength(2);
    expect(matches[0].lineNumber).toBe(2);
    expect(matches[0].text).toBe("['kibana.alert.status']");
    expect(matches[1].lineNumber).toBe(3);
    expect(matches[1].text).toBe('["service.name"]');
  });

  it('returns empty for no matches', () => {
    expect(
      findBracketDottedKeyUsages('message: {{ event.alerts[0].kibana.alert.status }}')
    ).toHaveLength(0);
  });
});
