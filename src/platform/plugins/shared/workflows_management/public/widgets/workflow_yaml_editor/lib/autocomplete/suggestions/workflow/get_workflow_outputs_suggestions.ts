...
export function isInWorkflowOutputWithBlock(
  ...
  // Fallback: when path is empty (cursor on blank/whitespace line inside a step),
  // use position-based detection to find which step contains the cursor
  else if (path.length === 0 && yamlDocument && typeof absolutePosition === 'number') {
    stepIndex = findStepIndexAtPosition(yamlDocument, absolutePosition);
  }

  // If we couldn't determine a step, bail out
  if (stepIndex === null) {
    return false;
  }
...
export async function getWorkflowOutputsSuggestions(
  ...
  // Primary: read from focusedStepInfo (when available)
  ...
  // Fallback: read directly from YAML document when focusedStepInfo is unavailable
  if (existingKeys.size === 0 && yamlDocument) {
    const stepIndex =
      path[0] === 'steps' && typeof path[1] === 'number'
        ? path[1]
        : findStepIndexAtPosition(yamlDocument, absoluteOffset);
    if (stepIndex !== null) {
      const withNode = yamlDocument.getIn(['steps', stepIndex, 'with'], true);
