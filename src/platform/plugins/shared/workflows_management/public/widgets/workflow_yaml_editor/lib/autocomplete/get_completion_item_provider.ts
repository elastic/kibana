    ...
    provideCompletionItems: async (model, position, completionContext) => {
      ...
      if (matchType === 'type' && autocompleteContext.path && autocompleteContext.yamlDocument) {
        ...
        const capabilityIndex = connectorTypes
          ...
          : undefined;
        if (typeof connectorId === 'string' && !connectorId.includes('{{') && capabilityIndex) {
          suggestions = suggestions.flatMap((suggestion) => {
            const key = getDeduplicationKey(suggestion);
            const classification = classifyConnectorAction(connectorId, key, capabilityIndex);
            if (!classification || classification.status === 'notConnectorAction') {
              return [suggestion];
            }
            if (classification.status === 'differentConnectorAction') {
              return [];
            }
            if (classification.status === 'available') {
              return [suggestion];
            }
            return [markConnectorActionUnavailable(suggestion, classification.connectorName)];
          });
        }
      }
