...
export function useRegisterKeyboardCommands(): UseRegisterKeyboardCommandsReturn {
  ...
  const suggestionsAction = useRef<monaco.IDisposable | null>(null);
  ...
  const registerKeyboardCommands = (params: RegisterKeyboardCommandsParams) => {
    ...
    suggestionsAction.current = editor.addAction({
      id: 'editor.action.triggerSuggest',
      label: i18n.translate('workflows.workflowDetail.yamlEditor.action.triggerSuggest', {
        defaultMessage: 'Trigger suggestions',
      }),
      // eslint-disable-next-line no-bitwise
      keybindings: [monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyI],
      run: (currentEditor: monaco.editor.IStandaloneCodeEditor) => {
        // Trigger the suggest widget manually
        currentEditor.getAction('editor.action.triggerSuggest')?.run();
      },
    });
  };
