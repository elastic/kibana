...
const validateStep = (
  ...
  const classification = classifyConnectorAction(connectorId, step.stepType, capabilityIndex);
  const subAction = step.stepType.replace(/^\./, '').split('.').slice(1).join('.');
  const typeRange = step.propInfos.type?.valueNode.range;

  if (classification?.status !== 'unavailable' || !subAction || !typeRange) {
    return undefined;
  }
  ...
  return {
    ...
    message: i18n.translate(
      'workflows.validateUnavailableConnectorActions.unsupportedActionMessage',
      {
        ...
        values: { action: subAction, connectorName: classification.connectorName },
      }
    ),
    hoverMessage: null,
