    ...
    if (isPair(ancestor)) {
      ...
      if (!childNode && index === ancestors.length - 1 && targetNode) {
        const seqIndex = ancestor.items.findIndex((item) => {
          ...
          if (item && targetNode && 'range' in item && 'range' in targetNode && 
              item.range && targetNode.range) {
            return targetNode.range[0] >= item.range[0] && 
                   targetNode.range[1] <= item.range[2];
          }
      ...
      if (childNode) {
        ...
        const seqIndex = ancestor.items.findIndex((item) => {
          ...
          if (item && childNode && 
              'range' in item && 'range' in childNode && 
              item.range && childNode.range) {
            return item.range[0] === childNode.range[0] && 
                   item.range[1] === childNode.range[1];
          }
