...
description: |
  ...
  loads sample national park data using bulk operations, 
  searches for parks by category, 
  iterates over search hits,
  ask LLM when the park was founded,
  and send the response to slack.   
enabled: true
tags: ["demo", "getting-started"]
consts:
  indexName: .alerts-national-parks
triggers:
  - type: manual
steps:
  - name: get_index
  ...
  - name: log_results
    ...
    with:
      message: |-
        ...
        Took {{ steps.search_park_data.output.took }} 
        Top results: {{ steps.search_park_data.output.hits.hits | json:2 }}  - name: elasticsearch_indices_delete_step  - name: console_step
  - name: process-results
    type: foreach
    foreach: "{{steps.search_park_data.output.hits.hits}}"
    steps:
      - name: process-item
        type: console
        with:
