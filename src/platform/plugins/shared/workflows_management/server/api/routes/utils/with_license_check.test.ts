...
function createWorkflowsContext(license: ILicense): WorkflowsRequestHandlerContext {
  return {
...
describe('withLicenseCheck', () => {
  ...
  it('calls the route handler when license is available, active, and enterprise', async () => {
    ...
    const wrapped = withLicenseCheck(handler);
    const response = httpServerMock.createResponseFactory();
    const license = enterpriseLicense();
    const context = createWorkflowsContext(license);
  ...
  it('returns forbidden when license information is not available', async () => {
    ...
    await wrapped(createWorkflowsContext(unavailableLicense()), request as never, response);
  ...
  it('returns forbidden when license is not active', async () => {
    ...
    await wrapped(createWorkflowsContext(expiredEnterpriseLicense()), request as never, response);
  ...
  it('returns forbidden when license tier is below enterprise', async () => {
    ...
    await wrapped(createWorkflowsContext(basicLicense()), request as never, response);
