...
async function getExternalResumeStepExecution(
  ...
  return execution?.stepExecutions.find(
    (stepExecution) =>
      ...
      stepExecution.stepType === 'waitForInput' &&
      getExternalApiKeyId(stepExecution.output) === apiKeyId
  );
}

function getExternalApiKeyId(output: unknown): string | undefined {
  if (output == null || typeof output !== 'object' || !('external' in output)) {
    return undefined;
  }

  const external = (output as { external?: unknown }).external;
  if (external == null || typeof external !== 'object' || !('apiKeyId' in external)) {
    return undefined;
  }

  const apiKeyId = (external as { apiKeyId?: unknown }).apiKeyId;
