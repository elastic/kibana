...
export function defineRoutes(
  ...
  router.post(
    ...
    async (context, request, response) => {
      try {
        const spaceId = spaces.getSpaceId(request);
        const createdWorkflow = await api.createWorkflow(request.body, spaceId);
  ...
  router.put(
    ...
    async (context, request, response) => {
      try {
        const { id } = request.params as { id: string };
        const spaceId = spaces.getSpaceId(request);
        const updated = await api.updateWorkflow(id, request.body, spaceId);
        if (updated === null) {
          return response.notFound();
        }
        return response.ok({
  ...
  router.delete(
    ...
    async (context, request, response) => {
      try {
        const { id } = request.params as { id: string };
        const spaceId = spaces.getSpaceId(request);
        await api.deleteWorkflows([id], spaceId);
        return response.ok();
      } catch (error) {
        return response.customError({
          statusCode: 500,
  ...
  router.delete(
    ...
    async (context, request, response) => {
      try {
        ...
        const spaceId = spaces.getSpaceId(request);
        await api.deleteWorkflows(ids, spaceId);
        return response.ok();
      } catch (error) {
        return response.customError({
          statusCode: 500,
