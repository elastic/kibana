...
import { PostTestStepRequestBody } from '../../common/model/api/workflows.gen';

export function registerPostTestStepRoute({ router, api, logger, spaces }: RouteDependencies) {
  router.versioned
    .post({
      access: 'public',
      path: WORKFLOWS_API_PATHS.TEST_STEP,
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            body: buildRouteValidationWithZod(PostTestStepRequestBody),
          },
        },
      },
