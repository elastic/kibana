...
const makeDeps = (): {
  ...
  workflowExecutionsDataAccess: { search: jest.Mock };
  logger: ReturnType<typeof loggerMock.create>;
} => {
  const storageClient = { search: jest.fn() };
  const esClient = elasticsearchServiceMock.createElasticsearchClient();
  const workflowExecutionsDataAccess = { search: jest.fn() };
  ...
  return {
    deps: {
      ...
      esClient,
      workflowExecutionsDataAccess: workflowExecutionsDataAccess as any,
    },
    storageClient,
    esClient,
...
describe('WorkflowSearchService', () => {
  ...
  describe('getWorkflows', () => {
    ...
    it('skips execution-history fetch when there are no workflows on the page', async () => {
      const { deps, storageClient, workflowExecutionsDataAccess } = makeDeps();
      storageClient.search.mockResolvedValue({ hits: { total: { value: 0 }, hits: [] } });
  ...
  describe('getWorkflowStats', () => {
    ...
    it('includes managed executions in history stats when includeManagedExecutionStats=true', async () => {
      ...
      const executionStatsQuery = workflowExecutionsDataAccess.search.mock.calls[0]?.[0]?.query;
    ...
    it('returns an empty execution-history array when the executions index is missing', async () => {
      const { deps, storageClient, workflowExecutionsDataAccess, logger } = makeDeps();
      storageClient.search.mockResolvedValue({
        aggregations: {
      ...
      workflowExecutionsDataAccess.search.mockRejectedValue(
