...
The data search service provides type-safe, Promise-based methods for executing Elasticsearch searches across various search strategies. These methods abstract away the complexity of Observable-based search execution and provide built-in pagination support.

**Available search methods:**

- `esql` - ES|QL queries for columnar data analysis
- `dsl` - Query DSL searches
- `dslPaginated` - Query DSL searches with built-in pagination
- `eql` - Event Query Language for sequence queries
- `sql` - SQL syntax queries

#### ES|QL Search

```.ts
const result = await data.search.esql(
  {
    query: 'FROM logs-* | WHERE status >= 400 | STATS count = COUNT(*) BY status',
    timeZone: 'America/New_York'
  }
);

console.log(result.rawResponse.values);
```

#### Basic DSL Search

```.ts
const result = await data.search.dsl(
  {
    index: 'logs-*',
...
console.log(result.rawResponse.hits.total);
```

#### DSL Search with Pagination
...
```.ts
const result = await data.search.eql(
  {
    index: 'logs-*',
    query: 'process where process.name == "cmd.exe"',
    fields: ['process.name', 'process.pid', '@timestamp']
  }
);

console.log(result.rawResponse.hits.events);
```

#### SQL Search

```.ts
const result = await data.search.sql(
  {
...
console.log(result.rawResponse.rows);
console.log(result.took); // Elasticsearch query execution time in ms
```

### Inspector Integration

The search methods support integration with the Inspector for debugging and monitoring search requests. This feature is **only available on the client-side**.

To enable Inspector integration, pass an `inspector` object with your search request options. The inspector object accepts the following properties:

- `adapter` (optional): A `RequestAdapter` instance from `@kbn/inspector-plugin/common`. If not provided, a new adapter will be created.
- `title`: The title displayed in the Inspector UI for this request.
- `description` (optional): Additional description shown in the Inspector.
- `id` (optional): A unique identifier for the request.

#### Example: ES|QL Search with Inspector

```.ts
import { RequestAdapter } from '@kbn/inspector-plugin/common';

// Create a RequestAdapter to collect request information
...
// The request details are now available in the Inspector UI
console.log(result.rawResponse.values);
```

The `RequestAdapter` captures request details, timing information, and responses, making them available in the Inspector panel for debugging and performance analysis.
