...
The Typed Search Service provides a type-safe, Promise-based interface for executing Elasticsearch searches across various search strategies. It abstracts away the complexity of Observable-based search execution and provides built-in pagination support.

**Available search methods:**

- `searchESQL` - ES|QL queries for columnar data analysis
- `searchDSL` - Query DSL searches
- `searchDSLPaginated` - Query DSL searches with built-in pagination
- `searchEQL` - Event Query Language for sequence queries
- `searchSQL` - SQL syntax queries

#### ES|QL Search

```.ts
const result = await data.search.typed.searchESQL(
  {
...
```.ts
const result = await data.search.typed.searchDSL(
  {
    index: 'logs-*',
...
```.ts
const result = await data.search.typed.searchDSLPaginated({
  index: 'logs-*',
...
for await (const page of result.pagination.getAllPages(10)) {
  console.log(`Page has ${page.rawResponse.hits.hits.length} hits`);
}
```
...
```.ts
const result = await data.search.typed.searchEQL(
  {
    index: 'logs-*',
...
```.ts
const result = await data.search.typed.searchSQL(
  {
