...
export declare class SearchSessionService implements ISearchSessionService {
    ...
    private create;
    get: ({ savedObjectsClient }: SearchSessionDependencies, user: AuthenticatedUser | null, sessionId: string) => Promise<SavedObject<SearchSessionSavedObjectAttributes>>;
    ...
    update: (deps: SearchSessionDependencies, user: AuthenticatedUser | null, sessionId: string, attributes: Partial<SearchSessionSavedObjectAttributes>) => Promise<import("@kbn/core/server").SavedObjectsUpdateResponse<SearchSessionSavedObjectAttributes>>;
     ...
     * @internal
     */
    trackId: (deps: SearchSessionDependencies, user: AuthenticatedUser | null, searchId: string, options: ISearchOptions) => Promise<void>;
     ...
     * @internal
     */
    getId: (deps: SearchSessionDependencies, user: AuthenticatedUser | null, searchRequest: IKibanaSearchRequest, { sessionId, isStored, isRestore, requestHash }: ISearchOptions) => Promise<string>;
    asScopedProvider: ({ security, savedObjects, elasticsearch }: CoreStart) => (request: KibanaRequest) => {
        getId: (searchRequest: IKibanaSearchRequest<any>, args_1: ISearchOptions) => Promise<string>;
        ...
        update: (sessionId: string, attributes: Partial<SearchSessionSavedObjectAttributes>) => Promise<import("@kbn/core/server").SavedObjectsUpdateResponse<SearchSessionSavedObjectAttributes>>;
