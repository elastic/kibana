...
// Define a common interface that captures the properties we need from both response types
...
export async function getSearchStatus(
  ...
  try {
    ...
    if (session.strategy === 'esql_async') {
      const apiResponse = await asUserClient.esql.asyncQueryGet({ id: asyncId }, { meta: true });
      response = apiResponse.body;
    } else {
      const apiResponse = await internalClient.asyncSearch.status({ id: asyncId }, { meta: true });
      response = apiResponse.body;
    }
