...
function createRequestHash(keys: Record<string, any>) {
  const hash = new Sha256().update(stringify(keys), 'utf8').digest('hex');
  return hash;
}
 ...
 *
 * Also, ignores sessionId for compatibility with background searches created before https://github.com/elastic/kibana/pull/237191
 */
export const createRequestHashForBackgroundSearches = (keys: Record<string, any>) => {
  ...
  const { sessionId, preference, ...rest } = keys;
