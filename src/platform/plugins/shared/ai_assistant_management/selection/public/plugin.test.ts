...
describe("AI Assistant Management Selection Plugin", () => {
    describe("serverless", () => {
        it("uses serverlessUiSettingsKey to get the correct value from uiSettings", async () => {
            ...
            const coreStart = { uiSettings: { get: jest.fn(() => AIAssistantType.Observability) } } as unknown as CoreStart
            const plugins = {
                spaces: {
    ...
    describe("stateful", () => {
        it("uses the correct setting key to get the correct value from uiSettings", async () => {
            ...
            const spacesBehaviorSubject = new BehaviorSubject({ solution: undefined } as unknown as Space)
            const coreStart = {
                uiSettings: {
            ...
            expect(collected).toEqual([AIAssistantType.Default, AIAssistantType.Default, AIAssistantType.Observability, AIAssistantType.Observability, AIAssistantType.Security])
        })
    })
})