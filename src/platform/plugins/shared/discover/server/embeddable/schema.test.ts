...
describe('by-value tab schema', () => {
  it('accepts default classic and ES|QL tabs with an omitted type', () => {
    const classicTab = parseByValueTab(classicTabInput);
    const esqlTab = parseByValueTab(esqlTabInput);

    expect(classicTab).toMatchObject({
      data_source: { type: AS_CODE_DATA_VIEW_REFERENCE_TYPE },
    });
    expect(esqlTab).toMatchObject({
      data_source: { type: AS_CODE_ESQL_DATA_SOURCE_TYPE },
    });
    expect(classicTab).toHaveProperty('type', DiscoverTabType.Default);
    expect(esqlTab).toHaveProperty('type', DiscoverTabType.Default);
  });

  it('accepts an explicit default type', () => {
    expect(parseByValueTab({ ...classicTabInput, type: DiscoverTabType.Default })).toMatchObject({
      type: DiscoverTabType.Default,
    });
    expect(parseByValueTab({ ...esqlTabInput, type: DiscoverTabType.Default })).toMatchObject({
      type: DiscoverTabType.Default,
    });
  });
