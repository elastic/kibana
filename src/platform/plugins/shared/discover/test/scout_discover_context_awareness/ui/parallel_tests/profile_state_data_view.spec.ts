...
/**
 * The data view counterpart of the ES|QL profile state suite: the same three storage lifetimes — UI
 * state for the life of the tab, persistent state in `discover.tabs` in localStorage, URL state in
...
spaceTest.describe(
  ...
  () => {
    ...
    spaceTest.beforeEach(async ({ browserAuth, pageObjects }) => {
      ...
      await pageObjects.discover.selectDataView(CONTEXT_AWARENESS_DATA_VIEWS.LOGS, {
    ...
    spaceTest(
      ...
      async ({ page, pageObjects }) => {
        ...
        const switchTo = async (dataView: string) => {
          await discover.selectDataView(dataView, { createAdHocIfMissing: false });
          await discover.waitUntilTabIsLoaded();
        };
        ...
        await switchTo(CONTEXT_AWARENESS_DATA_VIEWS.ALL);
        expect(await readRowHeight(page, dataGrid)).toStrictEqual(AUTO_ROW_HEIGHT);

        await setRowHeight(page, dataGrid, 'Custom', 2);

        await switchTo(CONTEXT_AWARENESS_DATA_VIEWS.LOGS);
        expect(await readRowHeight(page, dataGrid)).toStrictEqual(AUTO_ROW_HEIGHT);
