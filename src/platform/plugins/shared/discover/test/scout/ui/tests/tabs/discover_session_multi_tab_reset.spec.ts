...
import { test, tags, type ScoutPage, type ScoutTestFixtures } from '@kbn/scout';
...
test.describe(
  ...
  () => {
    ...
    test('should clear all tabs when starting a new session', async ({ pageObjects, page }) => {
      ...
      await test.step('create and save a multi-tab session', async () => {
      ...
      await test.step('load the saved session', async () => {
