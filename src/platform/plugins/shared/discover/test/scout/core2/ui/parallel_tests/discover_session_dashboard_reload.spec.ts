...
import type { ApiServicesFixture, Locator, PageObjects, ScoutPage } from '@kbn/scout';
...
const saveFilteredInlineDashboard = async (
  page: ScoutPage,
  { dashboard, dataGrid, filterBar }: PageObjects,
  ...
  return { expectFilteredPanels, expectEditableFilter };
};
...
spaceTest.describe(
  ...
  () => {
    ...
    spaceTest(
      'shares an unnamed inline Data View across repeated panels after reload',
      async ({ apiServices, page, pageObjects, scoutSpace }) => {
        const { dashboard } = pageObjects;
        const savedSearchName = `Inline panels repeated ${scoutSpace.id}`;

        await createInlineSession(apiServices, scoutSpace.id, savedSearchName);

        const { expectFilteredPanels, expectEditableFilter } = await saveFilteredInlineDashboard(
          page,
          pageObjects,
          savedSearchName
        );

        await spaceTest.step('add the same session again without losing the filter', async () => {
    ...
    spaceTest(
      'keeps an inline filter editable across linked and by-value panels after reload',
      async ({ apiServices, page, pageObjects, scoutSpace }) => {
        const { dashboard } = pageObjects;
        const savedSearchName = `Inline panels cloned ${scoutSpace.id}`;

        await createInlineSession(
          apiServices,
          scoutSpace.id,
          savedSearchName,
          'Shared inline logs'
        ...
        await spaceTest.step('clone the linked panel without losing the filter', async () => {
          await dashboard.ensureEditMode();
          await dashboard.clonePanel(savedSearchName);
          await dashboard.waitForPanelsToLoad(2);
          await expectFilteredPanels(2);
        ...
        await spaceTest.step('save and reload the linked panel and its clone', async () => {
          await dashboard.saveChangesToExistingDashboard();
          await page.reload();
          await dashboard.waitForPanelsToLoad(2);
          await expectFilteredPanels(2);
    ...
    spaceTest(
      'keeps a dashboard filter editable after saving its linked inline session in Discover',
      async ({ apiServices, page, pageObjects, scoutSpace }) => {
        const { dashboard, dataGrid, discover, filterBar, queryBar } = pageObjects;
        const savedSearchName = `Edited linked inline session ${scoutSpace.id}`;

        await createInlineSession(
          apiServices,
          scoutSpace.id,
          savedSearchName,
          'Editable inline logs'
        );
        ...
        const panel = dashboard.getPanelHoverActionsLocator(savedSearchName);
        const grid = panel.getByRole('grid');
        const bytesCell = panel.locator(dataGrid.getCellValue(0, 'bytes'));
        await expect(bytesCell).toHaveText(/^\s*\d[\d,]*\s*$/);
        const bytesText = (await bytesCell.innerText()).trim();
        const bytesValue = Number(bytesText.replace(/,/g, ''));
        expect(bytesValue).toBeGreaterThan(0);
        ...
        const unfilteredRowCount = await getGridRowCount(grid);

        await dataGrid.filterCell({ rowIndex: 0, columnId: 'bytes', mode: 'for' });
        await expect.poll(() => getGridRowCount(grid)).toBeLessThan(unfilteredRowCount);
        const filteredRowCount = await getGridRowCount(grid);
        expect(filteredRowCount).toBeGreaterThan(0);
        await dashboard.saveDashboard(savedSearchName);
        ...
        const expectEditableFilter = async () => {
          ...
          await expectBytesFilterEditor(page, bytesValue);
          await expect
            .poll(() => page.components.comboBox('filterOperatorList').getSelectedOptions())
            .toStrictEqual(['is']);
          await filterBar.closeFieldEditorModal();
        };

        await spaceTest.step('verify the dashboard filter before editing the session', async () => {
          await expectEditableFilter();
        });

        await spaceTest.step('change only the query and save the linked session', async () => {
          await dashboard.editLinkedDiscoverPanel(savedSearchName);
        ...
        await spaceTest.step('reload without losing the dashboard filter reference', async () => {
          await page.reload();
          await dashboard.waitForPanelsToLoad(1);
          await expect(grid).toHaveAttribute('aria-rowcount', String(filteredRowCount));
