...
spaceTest.describe('Discover — adhoc data views', { tag: tags.deploymentAgnostic }, () => {
  ...
  spaceTest(
    'preserves an API-created inline session and local edits through Content Management reloads',
    async ({ apiServices, discoverScoutSpace, page, pageObjects }) => {
      const { discover, dataGrid, filterBar, queryBar, unifiedFieldList } = pageObjects;
      const title = `Inline session CM reload ${discoverScoutSpace.id}`;
          ...
          tabs: [
            {
              ...
              filters: [{ type: 'condition', condition: { field: 'bytes', operator: 'exists' } }],
            },
          ],
        },
      ...
      const contentManagementReads: string[] = [];
      const sessionHttpRequests: string[] = [];
      page.on('request', (request) => {
        const pathname = new URL(request.url()).pathname;
        if (
          pathname.endsWith('/api/content_management/rpc/get') &&
          request.postData()?.includes(sessionId)
        ) {
          contentManagementReads.push(pathname);
        }
        if (pathname.endsWith(`/api/discover_sessions/${sessionId}`)) {
          sessionHttpRequests.push(pathname);
        }
      });

      await discover.loadSavedSearch(title);
      await discover.waitUntilTabIsLoaded();
      const dataViewId = await discover.getCurrentDataViewId();
      expect(dataViewId).toMatch(/^discover-inline-/);
      await expect(discover.unsavedChangesIndicator()).toBeHidden();

      await spaceTest.step('reload the saved API definition without changes', async () => {
        await page.reload();
        await discover.waitUntilTabIsLoaded();
        expect(await discover.getCurrentDataViewId()).toBe(dataViewId);
        await expect(discover.unsavedChangesIndicator()).toBeHidden();
      ...
      await spaceTest.step('preserve unsaved query and column edits after reload', async () => {
        await discover.writeAndSubmitKqlQuery('bytes >= 1');
        await unifiedFieldList.clickFieldListItemAdd('extension');
        await expect(discover.unsavedChangesIndicator()).toBeVisible();
        await page.reload();
        await discover.waitUntilTabIsLoaded();
        expect(await discover.getCurrentDataViewId()).toBe(dataViewId);
        await expect.poll(() => queryBar.getQuery()).toBe('bytes >= 1');
        await expect.poll(() => dataGrid.getColumnTitles()).toContain('extension');
        await expect(discover.unsavedChangesIndicator()).toBeVisible();
      });

      await spaceTest.step(
        'save through Content Management and reload a clean session',
        async () => {
      ...
      expect(contentManagementReads.length).toBeGreaterThanOrEqual(3);
      expect(sessionHttpRequests).toStrictEqual([]);
    }
  );
