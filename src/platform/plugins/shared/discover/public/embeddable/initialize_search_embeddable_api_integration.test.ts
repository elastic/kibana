...
describe('Discover embeddable inline Data View identity integration', () => {
  ...
  it('gives concurrent panels with the same ID-less spec the same Data View identity', async () => {
    ...
    const initialState = {
      serializedSearchSource: { index: cloneDeep(inlineSpecWithRuntimeField) },
    };
  ...
  it('preserves inline identity and filters through a by-value round trip', async () => {
    const initialState = {
      ...
      serializedSearchSource: {
        index: cloneDeep(inlineSpecWithRuntimeField),
        filter: [
          {
    ...
    expect(reloadedPanel.dataView.toSpec().runtimeFieldMap).toEqual(
      inlineSpecWithRuntimeField.runtimeFieldMap
    );
  ...
  it('does not report unsaved changes when loading a by-value panel with an indexless filter', async () => {
    ...
    const initialState = {
      columns: ['bytes'],
      serializedSearchSource: {
        index: { ...inlineSpecWithRuntimeField, name: 'logs-*', allowHidden: false },
        filter: [
          {
            meta: { type: 'phrase', key: 'bytes', params: { query: 100 } },
            query: { match_phrase: { bytes: 100 } },
  ...
  it.each<[string, DataViewSpec]>([
    ['index pattern', { ...inlineSpecWithRuntimeField, title: 'other-logs-*' }],
    ['time field', { ...inlineSpecWithRuntimeField, timeFieldName: 'event.created' }],
    ['name', { ...inlineSpecWithRuntimeField, name: 'Other logs' }],
    ['hidden indices setting', { ...inlineSpecWithRuntimeField, allowHidden: true }],
    ['field filters', { ...inlineSpecWithRuntimeField, sourceFilters: [{ value: 'secret.*' }] }],
    [
      'field format',
      {
        ...inlineSpecWithRuntimeField,
        fieldFormats: { bytes: { id: 'number', params: { pattern: '0.00' } } },
      },
    ],
    [
      'field label',
      {
        ...inlineSpecWithRuntimeField,
        fieldAttrs: { bytes: { customLabel: 'Bytes transferred' } },
      },
    ],
    [
    ...
    async (_difference, changedSpec) => {
      ...
      expect(original.dataView.toSpec()).toMatchObject(inlineSpecWithRuntimeField);
  ...
  it('changes and restores a panel identity without changing the other panel', async () => {
    ...
    expect(otherPanel.dataView.toSpec()).toMatchObject(inlineSpecWithRuntimeField);
  ...
  it('preserves a legacy inline ID and its filter references on a cold load', async () => {
    const initialState = {
      serializedSearchSource: {
        index: { ...inlineSpecWithRuntimeField, id: 'legacy-inline-id' },
        filter: [
          {
  ...
  it('does not merge different legacy IDs even when their inline specs match', async () => {
    ...
    const first = await initialize(
      {
        serializedSearchSource: { index: { ...inlineSpecWithRuntimeField, id: 'first-inline-id' } },
      },
      services
    );
    const second = await initialize(
      {
        serializedSearchSource: {
          index: { ...inlineSpecWithRuntimeField, id: 'second-inline-id' },
        },
      },
      services
    );
