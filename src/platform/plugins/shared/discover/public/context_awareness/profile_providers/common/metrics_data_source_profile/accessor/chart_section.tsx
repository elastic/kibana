...
const MetricsExperienceGridWrapper = (
  props: ChartSectionProps & {
    ...
    sortStateAdapter: ProfileStateAdapter<MetricsGridSort>;
  }
) => {
  const { gridSettingsStateAdapter, sortStateAdapter } = props;
  ...
  const onMetricsSortChange = useCallback(
    (next: MetricsSort) => {
      ...
      sortStateAdapter.updateState({ field, direction });
    },
    [sortStateAdapter]
  );
...
export const createChartSection =
  ...
  () => {
    ...
    const sortStateAdapter = toolkit.getStateAdapter(METRICS_GRID_SORT_STATE_DEF);
    return {
      ...prev(),
      renderChartSection: (props) => {
        return (
          <MetricsExperienceGridWrapper
            ...
            sortStateAdapter={sortStateAdapter}
          />
        );
      },
