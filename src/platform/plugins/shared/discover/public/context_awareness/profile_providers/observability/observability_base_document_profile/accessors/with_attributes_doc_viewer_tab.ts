...
>(
  ...
  profile: {
    ...provider.profile,
    getDocViewer: (prev, accessorParams) => (params) => {
      const baseImpl = provider.profile.getDocViewer?.(prev, accessorParams);
      const prevDocViewer = baseImpl?.(params) ?? prev(params);
