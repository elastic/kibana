...
describe('reconcileSessionInlineDataViewIds', () => {
  it('remaps only exact references in nested document and local filters', () => {
    const createFilter = (id: string) =>
      buildCombinedFilter(
        BooleanRelation.AND,
        [
          { meta: { index: id }, query: { term: { bytes: 100 } } },
          { meta: {}, query: { term: { bytes: 200 } } },
          buildCombinedFilter(
            BooleanRelation.OR,
            [
              { meta: { index: id }, query: { term: { bytes: 300 } } },
    ...
    const unreferencedFilter = { meta: {}, query: { term: { bytes: 600 } } };
    const filters = [createFilter('local-id'), unreferencedFilter];
    const session = createDiscoverSessionMock({
      id: 'session-id',
      tabs: [createInlineTab('inline-a', { ...inlineDataView, id: 'document-id' })],
    });
    session.tabs[0].serializedSearchSource.filter = cloneDeep(filters);
    const localTab = createLocalTab({
      id: 'inline-a',
      dataView: { ...inlineDataView, id: 'local-id' },
    });
    localTab.initialInternalState = {
      serializedSearchSource: {
        index: { ...inlineDataView, id: 'local-id' },
        filter: cloneDeep(filters),
      },
    };
    localTab.appState.filters = cloneDeep(filters);
    localTab.previousAppState.filters = cloneDeep(filters);
    localTab.globalState.filters = cloneDeep(filters);
    const input = { session, localTabs: [localTab] };
    const original = cloneDeep(input);

    const result = reconcileSessionInlineDataViewIds(input);
    const expected = [createFilter('document-id'), unreferencedFilter];

    expect(result.session.tabs[0].serializedSearchSource.filter).toStrictEqual(expected);
    expect(result.localTabs[0].initialInternalState?.serializedSearchSource?.filter).toStrictEqual(
      expected
    );
    expect(result.localTabs[0].appState.filters).toStrictEqual(expected);
    expect(result.localTabs[0].previousAppState.filters).toStrictEqual(expected);
    expect(result.localTabs[0].globalState.filters).toStrictEqual(expected);
    expect(input).toStrictEqual(original);
    const repeated = reconcileSessionInlineDataViewIds(result);
    expect(repeated.session).toStrictEqual(result.session);
    expect(repeated.localTabs).toStrictEqual(result.localTabs);
  });

  it('does not bind unreferenced children when binding an implicit search source filter', () => {
    ...
    session.tabs[0].serializedSearchSource.filter = [cloneDeep(filter)];
    const localTab = createLocalTab({ id: 'inline-a', dataView: inlineDataView });
    localTab.initialInternalState = {
      serializedSearchSource: { index: inlineDataView, filter: [cloneDeep(filter)] },
    };
    localTab.appState.filters = [cloneDeep(filter)];
    localTab.previousAppState.filters = [cloneDeep(filter)];
    localTab.globalState.filters = [cloneDeep(filter)];

    const result = reconcileSessionInlineDataViewIds({ session, localTabs: [localTab] });
    const expected = [
      { ...filter, meta: { ...filter.meta, index: generateInlineDataViewId(inlineDataView) } },
    ];
    ...
    expect(result.localTabs[0].appState.filters).toStrictEqual([filter]);
    expect(result.localTabs[0].previousAppState.filters).toStrictEqual([filter]);
    expect(result.localTabs[0].globalState.filters).toStrictEqual([filter]);
  });
  ...
  it('preserves distinct persisted IDs even when their definitions match', () => {
    ...
    const localTabs = session.tabs.map(({ id, serializedSearchSource }) =>
      getTabStateMock({
        id,
        initialInternalState: { serializedSearchSource: cloneDeep(serializedSearchSource) },
      })
    );
    const input = { session, localTabs };
    const original = cloneDeep(input);
    const result = reconcileSessionInlineDataViewIds(input);

    expect(result.session).toStrictEqual(session);
    expect(result.session).toBe(session);
    expect(result.localTabs).toStrictEqual(localTabs);
    expect(input).toStrictEqual(original);
  });
  ...
  it('binds API search source filters without binding unreferenced local filters or visContext', () => {
    ...
    expect(result.localTabs[0].initialInternalState?.serializedSearchSource?.index).toEqual({
      ...localDataView,
      id: targetId,
    });
    expect(result.localTabs[0].initialInternalState?.serializedSearchSource?.filter).toStrictEqual([
