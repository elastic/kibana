...
export const resetDiscoverSession = createInternalStateAsyncThunk(
  ...
  async (
    ...
    const allTabs = await Promise.all(
      discoverSession.tabs.map(async (tab) => {
        ...
        if (tabStateContainer) {
          const persistedTabSearchSource = await fromSavedObjectTabToSearchSource({
            tab,
            services,
          });
          const dataView = persistedTabSearchSource.getField('index');
    ...
    dispatch(
      internalStateSlice.actions.setPersistedDiscoverSessionResetTimestamp({
        timestamp: Date.now(),
      })
    );
  }
