...
export const replaceDataViewReferencesInFilters = (
  ...
  filters && replacement?.fromId !== undefined && replacement.fromId !== replacement.toId
    ? updateFilterReferences(filters, replacement.fromId, replacement.toId)
    : filters;

const bindImplicitInlineFilters = (
  filters: Filter[] | undefined,
  sourceDataView: DataViewSpec,
  dataViewId: string
): Filter[] | undefined => {
  // If the view already has an ID, leave filters without a reference alone.
  if (sourceDataView.id !== undefined) {
    return filters;
  }

  return filters?.map((filter) =>
    filter.meta.index === undefined
...
export const replaceDataViewReferencesInAppState = (
  ...
  const filters = replaceDataViewReferencesInFilters(appState.filters, replacement);
...
export const replaceDataViewReferencesInGlobalState = (
  ...
  const filters = replaceDataViewReferencesInFilters(globalState.filters, replacement);
...
const reconcileLocalTab = (tab: TabState, replacement: InlineDataViewIdReplacement): TabState => {
  ...
  const filters = bindImplicitInlineFilters(
    replaceDataViewReferencesInFilters(searchSource.filter, replacement),
    dataView,
    replacement.toId
  );
...
const reconcileSessionTab = (
  ...
  const filters = bindImplicitInlineFilters(
    replaceDataViewReferencesInFilters(tab.serializedSearchSource.filter, replacement),
    target.dataView,
    target.id
  );
  const filtersUnchanged =
    filters === tab.serializedSearchSource.filter ||
    filters?.every((filter, index) => filter === tab.serializedSearchSource.filter?.[index]);

  if (target.dataView.id === target.id && filtersUnchanged) {
    return tab;
  }
...
export const reconcileSessionInlineDataViewIds = ({
  ...
  const reconciledTabs = session.tabs.map((tab) => {
    const target = targetsByTab.get(tab.id);
    return target ? reconcileSessionTab(tab, target, scopedReplacementsByTab.get(tab.id)) : tab;
  });
  const reconciledSession = reconciledTabs.every((tab, index) => tab === session.tabs[index])
    ? session
    : { ...session, tabs: reconciledTabs };
