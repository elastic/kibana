...
describe('Discover state', () => {
  describe('Test discover state', () => {
    ...
    test('setting app state and syncing to URL', async () => {
      ...
      expect(getCurrentUrl()).toMatchInlineSnapshot(
        `"/#?_tab=(tabId:the-saved-search-id-with-timefield)&_a=(columns:!(default_column),dataSource:(dataViewId:index-pattern-with-timefield-id,type:dataView),grid:(),hideChart:!f,hideSidebar:!f,hideTable:!f,interval:auto,query:(language:kuery,query:''),sort:!(!(timestamp,desc)))&_g=(refreshInterval:(pause:!t,value:1000),time:(from:now-15m,to:now))"`
      );
    });
  ...
  describe('Test discover state with overridden state storage', () => {
    ...
    test('setting app state and syncing to URL', async () => {
      ...
      expect(history.createHref(history.location)).toMatchInlineSnapshot(
        `"/#?_a=(columns:!(default_column),dataSource:(dataViewId:index-pattern-with-timefield-id,type:dataView),grid:(),hideChart:!f,hideSidebar:!f,hideTable:!f,interval:auto,query:(language:kuery,query:''),sort:!(!(timestamp,desc)))&_tab=(tabId:the-saved-search-id-with-timefield)&_g=(refreshInterval:(pause:!t,value:1000),time:(from:now-15m,to:now))"`
      );
    });
  ...
  describe('Test discover state actions', () => {
    ...
    test('loadSavedSearch given an empty URL, no state changes', async () => {
      ...
      expect(getCurrentUrl()).toMatchInlineSnapshot(
        `"/#?_tab=(tabId:the-saved-search-id)&_g=(refreshInterval:(pause:!t,value:1000),time:(from:now-15d,to:now))&_a=(columns:!(default_column),dataSource:(dataViewId:the-data-view-id,type:dataView),grid:(),hideChart:!f,hideSidebar:!f,hideTable:!f,interval:auto,query:(language:kuery,query:''),sort:!())"`
      );
      const { hasUnsavedChanges } = selectHasUnsavedChanges(state.internalState.getState(), {
        runtimeStateManager: state.runtimeStateManager,
        services,
    ...
    test('loadSavedSearch given a URL with different interval and columns modifying the state', async () => {
      ...
      await new Promise(process.nextTick);
      expect(getCurrentUrl()).toMatchInlineSnapshot(
        `"/#?_a=(columns:!(message),dataSource:(dataViewId:the-data-view-id,type:dataView),grid:(),hideChart:!f,hideSidebar:!f,hideTable:!f,interval:month,query:(language:kuery,query:''),sort:!())&_g=(refreshInterval:(pause:!t,value:1000),time:(from:now-15d,to:now))&_tab=(tabId:the-saved-search-id)"`
      );
      const { hasUnsavedChanges } = selectHasUnsavedChanges(state.internalState.getState(), {
        runtimeStateManager: state.runtimeStateManager,
        services,
    ...
    test('resetDiscoverSession - when changing data views', async () => {
      ...
      await new Promise(process.nextTick);
      expect(getCurrentUrl()).toMatchInlineSnapshot(
        `"/#?_tab=(tabId:the-saved-search-id)&_g=(refreshInterval:(pause:!t,value:1000),time:(from:now-15d,to:now))&_a=(columns:!(),dataSource:(dataViewId:data-view-with-various-field-types-id,type:dataView),grid:(),hideChart:!f,hideSidebar:!f,hideTable:!f,interval:auto,query:(language:kuery,query:''),sort:!(!(data,desc)))"`
      );
      await waitFor(() => {
        expect(dataState.fetch).toHaveBeenCalledTimes(1);
      });
