...
describe('extractEsqlFingerprint', () => {
  it('extracts the data view ID and time field from an ES|QL chart', () => {
    ...
    expect(extractEsqlFingerprint(attributes)).toStrictEqual({
      dataViewId: 'esql-dv',
      timeField: '@timestamp',
    });
  });
  ...
  it('returns undefined when layers refer to different data views', () => {
    const attributes = createAttributes({
    ...
    expect(extractEsqlFingerprint(attributes)).toBeUndefined();
  });
  ...
  it.each([undefined, ''])('omits an absent or empty time field (%s)', (timeFieldName) => {
    ...
    expect(extractEsqlFingerprint(attributes)).toStrictEqual({ dataViewId: 'esql-dv' });
  });
  ...
  ])('returns undefined for $name', ({ attributes }) => {
    expect(extractEsqlFingerprint(attributes)).toBeUndefined();
  });
  ...
  ])('cannot extract a fingerprint: $name', ({ layers, dataViews }) => {
    expect(extractEsqlFingerprint(createAttributes({ layers, dataViews }))).toBeUndefined();
  });
});
