...
describe('search embeddable transform utils', () => {
  ...
  describe('by-value tab type state', () => {
    it('round-trips Metrics profile state between the by-value and stored formats', () => {
      ...
      expect(state.attributes.tabs[0].attributes.tabTypeState).toEqual({
        type: DiscoverTabType.Metrics,
        dimensions: ['host.name'],
        searchTerm: 'cpu',
    ...
    it('drops Metrics profile state from a stored classic tab', () => {
      ...
      const mismatchedState: StoredSearchEmbeddableByValueState = {
        ...state,
        attributes: {
          ...state.attributes,
          tabs: [
      ...
      const [resultTab] = fromStoredSearchEmbeddableByValue(mismatchedState).tabs;
