...
export type { AllowedSchemaTypes, AllowedSchemaStringTypes, AllowedSchemaBooleanTypes, AllowedSchemaNumberTypes, PossibleSchemaTypes, } from '@elastic/ebt/client';
...
export interface ICollectorSet {
     ...
     */
    makeUsageCollector: <TFetchReturn, ExtraOptions extends object = {}>(options: UsageCollectorOptions<TFetchReturn, ExtraOptions>) => Collector<TFetchReturn, ExtraOptions>;
    /**
     ...
     */
    getCollectorByType: <TFetchReturn, ExtraOptions extends object>(type: string) => Collector<TFetchReturn, ExtraOptions> | undefined;
    /**
     * Fetches the collection from all the registered collectors
     * @internal: telemetry use
     */
    bulkFetch: <TFetchReturn, ExtraOptions extends object>(esClient: ElasticsearchClient, soClient: SavedObjectsClientContract, collectors?: Map<string, Collector<TFetchReturn, ExtraOptions>>) => Promise<Array<{
        type: string;
        result: unknown;
    }>>;
    /**
 ...
 */
export type RecursiveMakeSchemaFrom<U, RequireMeta> = U extends object ? Record<string, unknown> extends U ? {
    type: 'pass_through';
    _meta: {
        description: string;
    } & SchemaMetaOptional<U>;
} | MakeSchemaFrom<U, RequireMeta> : MakeSchemaFrom<U, RequireMeta> : RequireMeta extends true ? {
...
export type MakeSchemaFrom<Base, RequireMeta = false> = {
    [Key in keyof Required<Base>]: Required<Base>[Key] extends Array<infer U> ? {
        type: 'array';
        items: RecursiveMakeSchemaFrom<U, RequireMeta>;
 ...
 */
export type CollectorFetchMethod<TReturn, ExtraOptions extends object = {}> = (this: ICollector<TReturn> & ExtraOptions, // Specify the context of `this` for this.log and others to become available
context: CollectorFetchContext) => Promise<TReturn> | TReturn;
/**
