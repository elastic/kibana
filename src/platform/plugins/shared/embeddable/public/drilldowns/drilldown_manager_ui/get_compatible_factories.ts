...
export async function getCompatibleFactories(
  ...
  await asyncForEach(entries, async ([type, getDrilldownDefinition]) => {
    ...
    const intersectsWithTriggers = supportedTriggers.some((supportedTrigger) =>
      triggers.includes(supportedTrigger)
    );
    if (isCompatible && intersectsWithTriggers) {
      factories.push({
        type,
