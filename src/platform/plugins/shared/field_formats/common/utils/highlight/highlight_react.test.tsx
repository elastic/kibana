...
describe('getHighlightReact', () => {
  it('returns plain string unchanged when no field name is provided', () => {
    expect(
      getHighlightReact('lorem ipsum', undefined, { highlight: { myField: ['lorem ipsum'] } })
    ).toBe('lorem ipsum');
  });

  it('returns plain string unchanged when no hit is provided', () => {
    expect(getHighlightReact('lorem ipsum', 'myField', undefined)).toBe('lorem ipsum');
  });

  describe('Highlight with DSL substrings', () => {
    const check = (value: string, highlights: string[], expected: string) => {
      expect(
        render(getHighlightReact(value, 'myField', { highlight: { myField: highlights } }))
      ).toBe(expected);
    };
  ...
  describe('Highlight with ES|QL inline tags', () => {
    ...
    test('returns plain string unchanged when value has no inline tags', () => {
      check('lorem ipsum', defaultTags, 'lorem ipsum');
    });

    test('returns plain string unchanged when pre or post tag is missing from config', () => {
      const hit = { inline_highlights: { myField: { preTag: '', postTag: '</em>' } } };
      expect(getHighlightReact(inline('lorem'), 'myField', hit)).toBe(inline('lorem'));
      expect(
        getHighlightReact(inline('lorem'), 'myField', {
          inline_highlights: { myField: { preTag: '<em>', postTag: '' } },
        })
      ).toBe(inline('lorem'));
    });

    test('highlights a single word at the start', () => {
      check(`${inline('lorem')} ipsum dolor`, defaultTags, `${mark('lorem')} ipsum dolor`);
    });

    test('highlights a single word in the middle', () => {
      check(`lorem ${inline('ipsum')} dolor`, defaultTags, `lorem ${mark('ipsum')} dolor`);
    });

    test('highlights a single word at the end', () => {
      check(`lorem ipsum ${inline('dolor')}`, defaultTags, `lorem ipsum ${mark('dolor')}`);
    });

    test('highlights multiple tagged spans in one value', () => {
      check(
        `lorem ${inline('ipsum')} dolor ${inline('sit')}`,
        defaultTags,
        `lorem ${mark('ipsum')} dolor ${mark('sit')}`
      );
    });
    ...
    test('highlights the entire field value', () => {
      check(inline('lorem'), defaultTags, mark('lorem'));
    });
    ...
    test('leaves text after an unclosed opening tag unchanged', () => {
      check('lorem <em>ipsum', defaultTags, 'lorem &lt;em&gt;ipsum');
    });
  });
});
