...
import { getFieldNameFromField, isColumnWithMeta, isColumnMetricWithSourceField } from '../utils';
...
const getOrderByWithAgg = ({
  ...
  if (agg.aggParams.orderBy === 'custom') {
    ...
    if (!orderMetricColumn || !isColumnMetricWithSourceField(orderMetricColumn[0])) {
      return null;
    }
    return {
      orderBy: { type: 'custom' },
