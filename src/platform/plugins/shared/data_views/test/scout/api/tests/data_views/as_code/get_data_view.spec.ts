...
import { COMMON_HEADERS, SERVICE_PATH, ID_OVER_MAX_LENGTH } from '../../../fixtures/constants';

const AS_CODE_DATA_VIEW_PATH = SERVICE_PATH;
...
apiTest.describe('GET /api/data_views/{id} - as code', { tag: tags.deploymentAgnostic }, () => {
  ...
  apiTest('can retrieve a data view', async ({ apiClient }) => {
    const getResponse = await apiClient.get(`${AS_CODE_DATA_VIEW_PATH}/${MOCK_DATA_VIEW.id}`, {
      headers: {
        ...COMMON_HEADERS,
  ...
  apiTest('returns 404 for a non-existing data view', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(`${AS_CODE_DATA_VIEW_PATH}/${nonExistingId}`, {
      headers: {
        ...COMMON_HEADERS,
        ...adminApiCredentials.apiKeyHeader,
      },
  ...
  apiTest('returns 400 when ID is too long', async ({ apiClient }) => {
    const response = await apiClient.get(`${AS_CODE_DATA_VIEW_PATH}/${ID_OVER_MAX_LENGTH}`, {
      headers: {
        ...COMMON_HEADERS,
        ...adminApiCredentials.apiKeyHeader,
      },
