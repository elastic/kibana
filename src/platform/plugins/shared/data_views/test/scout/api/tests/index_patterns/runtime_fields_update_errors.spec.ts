...
apiTest.describe(
  `POST ${DATA_VIEW_PATH_LEGACY}/{id}/runtime_field/{name} - update errors (legacy index pattern api)`,
  ...
  () => {
    ...
    apiTest('returns 404 error on non-existing index pattern', async ({ apiClient }) => {
      ...
      const response = await apiClient.post(`${DATA_VIEW_PATH_LEGACY}/${id}/runtime_field/foo`, {
