...
import {
  ...
  HAS_USER_DATA_VIEW_PATH,
  INTERNAL_COMMON_HEADERS,
  SERVICE_KEY,
} from '../../fixtures/constants';

apiTest.describe(
  `GET ${HAS_USER_DATA_VIEW_PATH} (data view api)`,
  ...
  () => {
    ...
    // sharing the deployment) we narrowly delete just the existing data views and the two
    // test indices the suite reads from.
    ...
    apiTest('returns false if no data views exist', async ({ apiClient }) => {
      const response = await apiClient.get(HAS_USER_DATA_VIEW_PATH, {
    ...
    apiTest('returns true if has data view with user data', async ({ apiClient, esArchiver }) => {
      ...
      const createResponse = await apiClient.post(DATA_VIEW_PATH, {
        ...
        body: {
          override: true,
          [SERVICE_KEY]: { title: 'basic_index' },
        },
      });
      expect(createResponse).toHaveStatusCode(200);
      createdIds.push(createResponse.body[SERVICE_KEY].id);
    ...
    apiTest('returns true if has user data view without data', async ({ apiClient }) => {
      const createResponse = await apiClient.post(DATA_VIEW_PATH, {
        headers: { ...COMMON_HEADERS, ...adminApiCredentials.apiKeyHeader },
        responseType: 'json',
        body: {
