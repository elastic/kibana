...
apiTest.describe(
  `PUT ${DATA_VIEW_PATH_LEGACY}/{id}/runtime_field - main (legacy index pattern api)`,
  ...
  () => {
    ...
    apiTest('can overwrite an existing field', async ({ apiClient }) => {
      ...
      expect(putResponse.body[SERVICE_KEY_LEGACY]).toBeDefined();

      const fooResponse = await apiClient.get(
    ...
    apiTest('can add a new runtime field', async ({ apiClient }) => {
      ...
      await apiClient.put(`${DATA_VIEW_PATH_LEGACY}/${id}/runtime_field`, {
        headers: { ...COMMON_HEADERS, ...adminApiCredentials.apiKeyHeader },
        responseType: 'json',
        body: {
          name: 'runtimeBar',
