...
apiTest.describe('DELETE /api/data_views/data_view/{id}/runtime_field/{name} - errors', { tag: tags.PLATFORM }, () => {
  ...
  apiTest.describe('legacy API', () => {
    apiTest('returns 404 for non-existent index pattern', async ({ apiClient }) => {
      // TODO: DELETE from non-existent pattern, verify 404
    });

    apiTest('returns 404 for non-existent runtime field', async ({ apiClient }) => {
      // TODO: Create pattern, DELETE non-existent field, verify 404, delete pattern
    });
  });
