...
apiTest.describe(
  'index patterns integration (legacy index pattern api)',
  ...
  () => {
    ...
    apiTest(
      ...
      async ({ apiClient }) => {
        ...
        expect(omit(recreatedIndexPattern, ['version', 'namespaces'])).toStrictEqual(
          omit(resultIndexPattern, ['version', 'namespaces'])
        );
      }
    );
  }
