...
apiTest.describe(
  `DELETE ${DATA_VIEW_PATH}/{id}/runtime_field/{name} - main (data view api)`,
  ...
  () => {
    ...
    apiTest('can delete a runtime field', async ({ apiClient }) => {
      ...
      expect(typeof getResponse.body[SERVICE_KEY].fields.runtimeBar).toBe('object');

      const deleteResponse = await apiClient.delete(
        `${DATA_VIEW_PATH}/${id}/runtime_field/runtimeBar`,
        {
          headers: {
            ...COMMON_HEADERS,
            ...adminApiCredentials.apiKeyHeader,
      ...
      const verifyResponse = await apiClient.get(`${DATA_VIEW_PATH}/${id}`, {
        headers: {
          ...COMMON_HEADERS,
          ...adminApiCredentials.apiKeyHeader,
        },
      ...
      expect(verifyResponse.body[SERVICE_KEY].fields.runtimeBar).toBeUndefined();
    });
  }
);
