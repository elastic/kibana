...
describe('sanitize', () => {
  ...
  test('returns deduplicated related items from incoming references', async () => {
    ...
    expect(result.related_items).toEqual([
      { type: 'index-pattern', type_label: 'data view', id: 'dv-1' },
      { type: 'lens', type_label: 'visualization', id: 'lens-1' },
      { type: 'tag', type_label: 'tag', id: 'tag-1' },
      { type: 'tag', type_label: 'tag', id: 'tag-2' },
    ]);
    expect(result.related_items_count).toBe(4);
  });

  test('omits related items when there are no references', async () => {
    mockedTransformDashboardIn.mockReturnValue({ attributes: storedAttributes, references: [] });
    mockedTransformDashboardOut.mockReturnValue({
      dashboardState: baseDashboardState,
      warnings: [],
    ...
    expect(result).not.toHaveProperty('related_items_count');
  });

  test('limits related items to the response schema maximum', async () => {
    mockedTransformDashboardIn.mockReturnValue({
      attributes: storedAttributes,
    ...
    expect(result.related_items).toHaveLength(100);
    expect(result.related_items_count).toBe(101);
  });
});
