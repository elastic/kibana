...
describe('CopyToDashboardModal', () => {
  ...
  test('should enable add to "new dashboard" when in saved dashboard', async () => {
    const result = render(
      <CopyToDashboardModal api={api} closeModal={closeModalMock} />
    );

    await waitFor(() => expect(result.container.querySelector('#new-dashboard-option')).toBeEnabled());
  });
  ...
  test('should disable add to "new dashboard" when in new dashboard', async () => {
    ...
    await waitFor(() => expect(result.container.querySelector('#new-dashboard-option')).toBeDisabled());
  });
  ...
  test('does not show the current dashboard in the dashboard picker', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('mockDashboardPicker idsToOmit:dashboardOne')).toBeInTheDocument());
  });
  ...
  test('should navigate to dashboard on submit', async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(navigateToWithEmbeddablePackageMock).toHaveBeenCalledWith('dashboard', {});
      }
    );
  });
});
