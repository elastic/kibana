...
describe('convertSchemaToParameters', () => {
  ...
  it('should convert a parameters object to a zod schema', () => {
    ...
    // Test that the converted schema has the same structure as the original
    expect(converted).toHaveProperty('esql');
    expect(converted).toHaveProperty('type');
    expect(converted).toHaveProperty('layers');
    expect(converted).toHaveProperty('title');

    // Test that the type field is an enum with the correct values
    expect(converted.type).toBeInstanceOf(z.ZodEnum);
    expect(converted.type._def.values).toEqual(chartTypes);

    // Test that the esql field is an object with query property
    expect(converted.esql).toBeInstanceOf(z.ZodObject);
    expect(converted.esql.shape).toHaveProperty('query');

    // Test that optional fields are properly marked as optional
    expect(converted.layers).toBeInstanceOf(z.ZodOptional);
    expect(converted.title).toBeInstanceOf(z.ZodOptional);
  });
});
