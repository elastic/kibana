...
export function initializeLayoutManager(
  ...
  const resetLayout = (state: DashboardState) => {
    const nextLayoutAndState = deserializeLayout(state.panels, state.pinned_panels);

    const layoutToApply = nextLayoutAndState.layout;
    const childStateToApply = nextLayoutAndState.childState;
