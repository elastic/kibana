...
apiTest.describe('dashboards - get', { tag: tags.DEPLOYMENT_AGNOSTIC }, () => {
  ...
  apiTest('should return 200 with an existing dashboard', async ({ apiClient }) => {
    ...
    expect(response.body.item.attributes.title).toBe('Requests');

    // Does not return unsupported options from the saved object
    expect('darkTheme' in response.body.item.attributes.options).toBe(false);
    expect('display' in response.body.item.attributes.refreshInterval).toBe(false);
  });
