...
apiTest.describe('dashboards - get', { tag: tags.DEPLOYMENT_AGNOSTIC }, () => {
  ...
  apiTest.skip('should return 200 with an existing dashboard', async ({ apiClient }) => {
    ...
    expect('darkTheme' in response.body.data.options).toBe(false);
    expect('display' in response.body.data.refreshInterval).toBe(false);
  });
