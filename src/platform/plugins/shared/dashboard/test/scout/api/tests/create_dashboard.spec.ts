...
apiTest.describe('dashboards - create', { tag: tags.deploymentAgnostic }, () => {
  ...
  apiTest('sets top level default values', async ({ apiClient }) => {
    ...
    expect(response.body.data.kibanaSavedObjectMeta.searchSource).toStrictEqual({});
    expect(response.body.data.panels).toStrictEqual([]);
    expect(response.body.data.timeRestore).toBe(false);
    expect(response.body.data.options).toStrictEqual({
  ...
  apiTest('sets panels default values', async ({ apiClient }) => {
    ...
    expect(/^[0-9a-f-]{36}$/.test(response.body.data.panels[0].uid)).toBe(true);
    expect(response.body.data.panels[0].uid).toBe(response.body.data.panels[0].grid.i);
  });
  ...
  apiTest('sets controls default values', async ({ apiClient }) => {
    ...
    expect(/^[0-9a-f-]{36}$/.test(response.body.id)).toBe(true);
    // saved object stores controls panels as an object, but the API should return as an array
    expect(Array.isArray(response.body.data.controlGroupInput.controls)).toBe(true);
    expect(response.body.data.controlGroupInput.ignoreParentSettings).toStrictEqual(
  ...
  apiTest('can create a dashboard in a defined space', async ({ apiClient }) => {
    ...
    expect(response).toHaveStatusCode(200);
    expect(response.body.data.namespaces).toStrictEqual([spaceId]);
  });
