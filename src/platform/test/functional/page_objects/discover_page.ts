...
export class DiscoverPageObject extends FtrService {
  ...
  public async loadSavedSearch(searchName: string) {
    ...
    await this.header.waitUntilLoadingHasFinished();
    // Wait for the picker flyout to close, which proves the selection was applied in both
    // classic and serverless; the previous breadcrumb-based check silently no-ops in serverless.
    await this.flyout.ensureClosed('loadSearchForm');
  }
