...
export class SharePageObject extends FtrService {
  ...
  async isShareMenuOpen() {
    return await this.testSubjects.waitForExists('shareContextMenu');
  }

  async isShareModalOpen() {
    return await this.testSubjects.waitForExists('shareContextModal');
  }

  async clickShareTopNavButton() {
    // The project header renders share as a title action revealed on header hover, outside the menu.
    if (await this.testSubjects.waitForExists(APP_HEADER_TEST_SUBJECTS.root, { timeout: 1000 })) {
