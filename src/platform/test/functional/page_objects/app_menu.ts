...
export class AppMenuPageObject extends FtrService {
  ...
  private async ensureOverflowPopoverClosed() {
    if (await this.testSubjects.waitForExists(APP_MENU_POPOVER, { timeout: 500 })) {
  ...
  async clickMenuItem(testId: string, { isInOverflowMenu }: { isInOverflowMenu?: boolean } = {}) {
    await this.retry.try(async () => {
      if (!isInOverflowMenu && (await this.testSubjects.waitForExists(testId, { timeout: 1000 }))) {
  ...
  async menuItemExists(testId: string) {
    if (await this.testSubjects.waitForExists(testId)) {
      return true;
    }

    if (!(await this.testSubjects.waitForExists(APP_MENU_OVERFLOW_BUTTON))) {
      return false;
    }

    await this.openOverflowPopover();
    const exists = await this.testSubjects.waitForExists(testId);
    await this.ensureOverflowPopoverClosed();
    return exists;
  }
