...
- **Route**: the host's `pageKey` for the page (the pin is only placed there) and the `path` at the moment of commenting, both relative to the host's origin and base path, so that comments do not depend on the deployment or space they were made in.
