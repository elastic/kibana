...
import { getDashboardSavedObjectMigrations } from './dashboard_saved_object_migrations';
import {
  createExtract,
  createInject,
} from './migrations/migrate_extract_panel_references/dashboard_container_references';
import type { DashboardDoc730ToLatest } from './migrations/migrate_to_730/types';

const embeddableSetupMock = createEmbeddableSetupMock();
const extract = createExtract(getAllEmbeddableReferenceManagers(), embeddableSetupMock);
const inject = createInject(getAllEmbeddableReferenceManagers(), embeddableSetupMock);
...
const migrations = getDashboardSavedObjectMigrations(embeddableSetupMock);
...
describe('dashboard', () => {
  ...
  describe('embeddable migrations for by value panels', () => {
    ...
    it('runs migrations on by value panels only', () => {
      ...
      const migrationsList = getDashboardSavedObjectMigrations(newEmbeddableSetupMock);
