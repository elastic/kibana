...
const changeOfBase = (a: number, b: number): number => Math.log(a) / Math.log(b);
...
export function log(a: NumberOrArray, b: number = Math.E): NumberOrArray {
  if (b <= 0) throw new Error('Base out of range');

  if (Array.isArray(a)) {
    return a.map((int) => {
      if (int < 0) throw new Error('Must be greater than 0');
      return changeOfBase(int, b);
    });
  }
