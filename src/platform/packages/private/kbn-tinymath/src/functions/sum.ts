...
const findSum = (total: number, current: number): number => total + current;
...
export function sum(...args: NumberOrArray[]): number {
  return args.reduce((total: number, current: NumberOrArray) => {
