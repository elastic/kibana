...
/**
 * Registers the command used by the PromQL `by ($1) ($0)` snippet.
 * It triggers suggestions immediately at the first placeholder (`$1`),
 * then arms a one-shot Tab handler (only in snippet mode) to move to `$0`
 * and trigger suggestions there.
 */
export const addPromqlBySnippetCommand = (editor: monaco.editor.IStandaloneCodeEditor) => {
  const promqlBySnippetActiveKey = editor.createContextKey<boolean>(
    'esqlPromqlBySnippetActive',
    false
  );
  const clearPromqlBySnippetState = () => promqlBySnippetActiveKey.reset();

  editor.addCommand(
    monaco.KeyCode.Tab,
    () => {
      editor.trigger('promql', 'jumpToNextSnippetPlaceholder', {});

      requestAnimationFrame(() => {
        editor.trigger('promql', 'editor.action.triggerSuggest', {});
      });

      clearPromqlBySnippetState();
    },
    'esqlPromqlBySnippetActive && inSnippetMode && !suggestWidgetVisible && textInputFocus'
  );

  editor.onDidBlurEditorText(clearPromqlBySnippetState);

  editor.addAction({
    id: 'esql.promql.bySnippetInserted',
    label: 'PromQL By Snippet Inserted',

    run: () => {
      promqlBySnippetActiveKey.set(true);
      editor.trigger('promql', 'editor.action.triggerSuggest', {});
    },
  });
};
