...
import { functionDefinitions as pkgFunctionDefs } from '@elastic/esql-definitions/functions';
import { operatorDefinitions as pkgOperatorDefs } from '@elastic/esql-definitions/operators';
...
import { promqlOperatorDefinitions as pkgPromqlOperatorDefs } from '@elastic/esql-definitions/promql-operators';
...
function getFunctionDefinition(ESFunctionDefinition: Record<string, any>): FunctionDefinition {
  ...
  const ret = {
    ...
    observabilityTier: ESFunctionDefinition.observabilityTier,
    locationsAvailable,
    description: ESFunctionDefinition.description,
    alias: aliasTable[ESFunctionDefinition.name],
    ignoreAsSuggestion: ESFunctionDefinition.snapshotOnly,
    preview: ESFunctionDefinition.preview,
    tsdbCompatible: ESFunctionDefinition.tsdbCompatible,
    signatures: _.uniqBy(
    ...
    .map(
      (func) =>
        `  ${func.name.toUpperCase()} = '${
          (func as any).operator?.toLowerCase() ?? func.name.toLowerCase()
        }',`
    )
    .join('\n')}
  }`;
...
`
  ...
  const promqlDefinitions: PromQLESFunctionDefinition[] = promqlFunctionDefinitions.map((fn) => ({
    type: fn.type,
    name: fn.name,
    description: promqlFunctionDocs[fn.name]?.description ?? '',
    examples: promqlFunctionDocs[fn.name]?.examples ?? [],
    signatures: fn.signatures as PromQLESFunctionDefinition['signatures'],
    preview: false,
    snapshot_only: fn.snapshotOnly,
  }));
  ...
  const allPromqlOperatorDefs: PromQLESFunctionDefinition[] = pkgPromqlOperatorDefs.map((op) => ({
    type: op.type,
    name: op.name,
    operator: op.operator,
    description: promqlOperatorDocs[op.name]?.description ?? '',
    examples: promqlOperatorDocs[op.name]?.examples ?? [],
    signatures: op.signatures as PromQLESFunctionDefinition['signatures'],
    preview: false,
    snapshot_only: op.snapshotOnly,
  }));
