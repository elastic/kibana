...
const LOCAL_FILE = Path.join(
  REPO_ROOT,
  'src/platform/packages/private/kbn-gen-ai-functional-testing/docs_credentials.json'
);

export const retrieveFromVault = async () => {
  const { stdout } = await execa(
    'vault',
    ['read', '-field=source-cluster-config', 'secret/ci/elastic-kibana/ai-infra-ci-docs'],
    {
      cwd: REPO_ROOT,
      buffer: true,
    }
...
export const uploadToVault = async () => {
  ...
  await execa(
    'vault',
    ['write', 'secret/ci/elastic-kibana/ai-infra-ci-docs', `source-cluster-config=${asB64}`],
    {
      cwd: REPO_ROOT,
      buffer: true,
    }
