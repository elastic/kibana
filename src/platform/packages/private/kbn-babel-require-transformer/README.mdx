...
The following environment variables will output transformed code to the console for inspecting and debugging:

- `KBN_BABEL_DUMP_FINAL` => Output the final transformed code
- `KBN_BABEL_DUMP_MATCH` => Only output the transformed code of a passed filename
- `KBN_BABEL_DEBUG` => Will output for which `require` statement the plugin kicks in
