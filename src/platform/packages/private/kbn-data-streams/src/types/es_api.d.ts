...
export interface ClientSearchRequest<SearchRuntimeMappings extends BaseSearchRuntimeMappings = {}> extends Omit<api.SearchRequest, 'index' | 'fields' | 'track_total_hits' | 'size'> {
...
export type ClientSearchResponse<TDocument, TSearchRequest extends Omit<api.SearchRequest, 'index'>> = api.SearchResponse<TDocument, TSearchRequest>;
export type ClientCreateRequest<TDocument> = Omit<api.BulkRequest<TDocument>, 'operations' | 'index'> & {
    /**
...
export type ClientCreate<TDocumentType> = (request: ClientCreateRequest<TDocumentType>) => Promise<ClientCreateResponse>;
export type ClientExists = () => Promise<boolean>;
export interface InternalIDataStreamClient<S extends MappingsDefinition, FullDocumentType = GetFieldsOf<S>, SRM extends BaseSearchRuntimeMappings = never> {
