...
export const VISUALIZATION_TYPES: Record<VisualizationType, { icon: string; label: string }> = {
  ...
  bar: { icon: 'chartBarVertical', label: 'Bar' },
  pie: { icon: 'chartPie', label: 'Pie' },
  metric: { icon: 'chartMetric', label: 'Metric' },
  gauge: { icon: 'chartGauge', label: 'Gauge' },
  goal: { icon: 'visGoal', label: 'Goal' },
  table: { icon: 'table', label: 'Data Table' },
  tagcloud: { icon: 'chartTagCloud', label: 'Tag Cloud' },
  markdown: { icon: 'text', label: 'Markdown' },
  vega: { icon: 'code', label: 'Vega' },
