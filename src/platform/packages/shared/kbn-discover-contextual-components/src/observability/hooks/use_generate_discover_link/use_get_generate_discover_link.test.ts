...
describe('useGetGenerateDiscoverLink', () => {
  const mockUseUnifiedDocViewerServices = useUnifiedDocViewerServices as jest.Mock;
  const mockDiscoverLocator = {
    getRedirectUrl: jest.fn(() => DISCOVER_URL),
  };

  beforeEach(() => {
    mockUseUnifiedDocViewerServices.mockReturnValue({
      data: {
        query: {
          timefilter: {
  ...
  it('returns undefined if discoverLocator is missing', () => {
    mockUseUnifiedDocViewerServices.mockReturnValueOnce({
      data: {
        query: {
          timefilter: {
            timefilter: { getAbsoluteTime: jest.fn(() => ({ from: 'now-15m', to: 'now' })) },
