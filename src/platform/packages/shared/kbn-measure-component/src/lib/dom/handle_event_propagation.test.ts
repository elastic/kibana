...
describe('handleEventPropagation', () => {
  ...
  it('should call callback when event type is click', () => {
    eventMock = { ...eventMock, type: 'click' } as unknown as MouseEvent;
  ...
  it('should call callback when target element is disabled', () => {
    (targetElementMock.hasAttribute as jest.Mock).mockImplementation(
      (attr: string) => attr === 'disabled'
    );
  ...
  it('should not call callback on pointerdown for enabled elements', () => {
    handleEventPropagation({ event: eventMock, callback: callbackMock });
