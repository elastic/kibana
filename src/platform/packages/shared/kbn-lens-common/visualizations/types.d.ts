...
import { type LENS_CATEGORY_DISPLAY, type LENS_LEGEND_DISPLAY, type LENS_NUMBER_DISPLAY, type LENS_LAYER_TYPES } from './constants';
...
import type { DatatableVisualizationState, LensPagesizeActionData, LensResizeActionData, LensSortActionData, LensToggleActionData } from './datatable/types';
import type { HeatmapVisualizationState } from './heatmap/types';
import type { SuggestionRequest, GeneralDatasourceStates, AnnotationGroups, VisualizationType, FramePublicAPI, Datasource, StateSetter, LayerAction, VisualizationConfigProps, VisualizationDimensionGroupConfig, VisualizationLayerWidgetProps, VisualizationToolbarProps, VisualizationDimensionChangeProps, DragDropOperation, GetDropPropsArgs, VisualizationLayerSettingsProps, VisualizationDimensionEditorProps, VisualizationSuggestion, DatasourceLayers, VisualizationDisplayOptions, VisualizationInfo, TableInspectorAdapter, VisualizeEditorContext, Suggestion, UserMessage } from '../types';
import type { LENS_EDIT_PAGESIZE_ACTION, LENS_EDIT_RESIZE_ACTION, LENS_EDIT_SORT_ACTION, LENS_TOGGLE_ACTION } from './datatable/constants';
...
export type LensConfiguration = XYVisualizationState | DatatableVisualizationState | LensPartitionVisualizationState | MetricVisualizationState | LegacyMetricState | GaugeVisualizationState | HeatmapVisualizationState | LensTagCloudState;
...
export type AddLayerFunction<T = unknown> = (layerType: LensLayerType, extraArg?: T, ignoreInitialValues?: boolean, seriesType?: SeriesType) => void;
...
export type TriggerEvent = BrushTriggerEvent | ClickTriggerEvent | MultiClickTriggerEvent | LensTableRowContextMenuEvent | LensAlertRulesEvent;
/**
...
export interface Visualization<T = unknown, P = T, ExtraAppendLayerArg = unknown> {
     ...
     */
    initialize: {
        (addNewLayer: () => string, nonPersistedState?: T, mainPalette?: SuggestionRequest['mainPalette'], datasourceStates?: GeneralDatasourceStates): T;
        (addNewLayer: () => string, persistedState: P, mainPalette?: SuggestionRequest['mainPalette'], datasourceStates?: GeneralDatasourceStates, annotationGroups?: AnnotationGroups, references?: Reference[]): T;
    };
    ...
    getDescription: (state: T, layerId?: string) => {
        icon?: IconType;
        label: string;
    };
    /** Visualizations can have references as well */
    getPersistableState?: (state: T, datasource?: Datasource, datasourceState?: {
        state: unknown;
    }) => {
        state: P;
    ...
    /** Reset button on each layer triggers this */
    cloneLayer?: (state: T, layerId: string, newLayerId: string, 
    /** @param contains map old -> new id **/
    clonedIDsMap: Map<string, string>) => T;
    /** Optional, if the visualization supports multiple layers */
    removeLayer?: (state: T, layerId: string) => T;
    /** Track added layers in internal state */
    appendLayer?: (state: T, layerId: string, type: LensLayerType, indexPatternId: string, extraArg?: ExtraAppendLayerArg, seriesType?: SeriesType) => T;
    /** Retrieve a list of supported layer types with initialization data */
    getSupportedLayers: (state?: T, frame?: Pick<FramePublicAPI, 'datasourceLayers' | 'activeData'>) => VisualizationLayerDescription[];
    /**
     ...
     * */
    getSupportedActionsForLayer?: (layerId: string, state: T, setState: StateSetter<T>, registerLibraryAnnotationGroup: RegisterLibraryAnnotationGroupFunction, isSaveable?: boolean) => LayerAction[];
    /**
     * This method is a clunky solution to the problem, but I'm banking on the confirm modal being removed
     * with undo/redo anyways
     */
    getCustomRemoveLayerText?: (layerId: string, state: T) => {
        title?: string;
        description?: string;
    } | undefined;
     ...
     */
    getCustomLayerHeader?: (props: VisualizationLayerWidgetProps<T>) => undefined | ReactElement<VisualizationLayerWidgetProps<T>>;
     ...
     */
    LayerPanelComponent?: (props: VisualizationLayerWidgetProps<T>) => null | ReactElement<VisualizationLayerWidgetProps<T>>;
    /**
     ...
     */
    FlyoutToolbarComponent?: (props: VisualizationToolbarProps<T> & {
        isInlineEditing: boolean;
     ...
     */
    setDimension: (props: VisualizationDimensionChangeProps<T> & {
        groupId: string;
        previousColumn?: string;
    }) => T;
    reorderDimension?: (props: VisualizationDimensionChangeProps<T> & {
        groupId: string;
    ...
    getDropProps?: (dropProps: GetDropPropsArgs) => {
        dropTypes: DropType[];
        nextLabel?: string;
    } | undefined;
    /**
    ...
    LayerSettingsComponent?: (props: VisualizationLayerSettingsProps<T> & {
     ...
     */
    DimensionEditorComponent?: (props: VisualizationDimensionEditorProps<T>) => null | ReactElement<VisualizationDimensionEditorProps<T>>;
    /**
     * Additional editor that gets rendered inside the dimension popover in an additional section below "appearance".
     * This can be used to configure dimension-specific options
     */
    DimensionEditorAdditionalSectionComponent?: (props: VisualizationDimensionEditorProps<T>) => null | ReactElement<VisualizationDimensionEditorProps<T>>;
    /**
     * Additional editor that gets rendered inside the data section.
     * This can be used to configure dimension-specific options
     */
    DimensionEditorDataExtraComponent?: (props: VisualizationDimensionEditorProps<T>) => null | ReactElement<VisualizationDimensionEditorProps<T>>;
    /**
    ...
    getAddLayerButtonComponent?: (props: AddLayerButtonProps<T>) => null | ReactElement<AddLayerButtonProps<T>>;
    /**
    ...
    toExpression: (state: T, datasourceLayers: DatasourceLayers, attributes?: Partial<{
        title: string;
        description: string;
    }>, datasourceExpressionsByLayers?: Record<string, Ast>) => ExpressionAstExpression | string | null;
    /**
     ...
     */
    toPreviewExpression?: (state: T, datasourceLayers: DatasourceLayers, datasourceExpressionsByLayers?: Record<string, Ast>) => ExpressionAstExpression | string | null;
    /**
    ...
    getSuggestionFromConvertToLensContext?: (props: VisualizationStateFromContextChangeProps<T>) => Suggestion<T> | undefined;
    isEqual?: (state1: P, references1: Reference[], state2: P, references2: Reference[], annotationGroups: AnnotationGroups) => boolean;
     ...
     */
    getExportDatatables?: (state: T, datasourceLayers?: DatasourceLayers, activeData?: TableInspectorAdapter) => Datatable[];
    /**
