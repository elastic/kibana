import type { AggregateQuery, ExecutionContextSearch, Filter, Query, TimeRange } from '@kbn/es-query';
...
import type { PublishesDataViews, PublishingSubject, SerializedTimeRange, SerializedTitles, ViewMode } from '@kbn/presentation-publishing';
import type { Action } from '@kbn/ui-actions-plugin/public';
import type { BrushTriggerEvent, ClickTriggerEvent, MultiClickTriggerEvent } from '@kbn/charts-plugin/public';
...
import type { IndexPatternMap, IndexPatternRef, LensDocument, LensInspector, SharingSavedObjectProps, TableInspectorAdapter, UserMessage, VisualizationDisplayOptions } from '../types';
...
export interface LensOverrides {
     ...
     */
    overrides?: AllowedChartOverrides | AllowedSettingsOverrides | AllowedXYOverrides | AllowedPartitionOverrides | AllowedGaugeOverrides;
}
/**
...
export interface LensPublicCallbacks extends LensApiProps {
    ...
    onLoad?: (isLoading: boolean, adapters?: Partial<DefaultInspectorAdapters>, dataLoading$?: PublishingSubject<boolean | undefined>) => void;
    onFilter?: (data: Simplify<(ClickTriggerEvent['data'] | MultiClickTriggerEvent['data']) & PreventableEvent>) => void;
    onTableRowClick?: (data: Simplify<LensTableRowContextMenuEvent['data'] & PreventableEvent>) => void;
    /**
 ...
 */
export type LensSerializedSharedState = Simplify<LensOverrides & LensWithReferences & LensUnifiedSearchContext & LensPanelProps & SerializedTitles & SerializedDrilldowns & Omit<LensSharedProps, 'noPadding'> & {
    isNewPanel?: boolean;
}>;
 ...
 */
export type LensComponentProps = Simplify<LensRequestHandlersProps & LensSharedProps & {
    /**
 ...
 */
export type LensComponentForwardedProps = Pick<LensComponentProps, 'style' | 'className' | 'noPadding' | 'abortController' | 'executionContext' | 'viewMode' | 'forceDSL'>;
/**
 ...
 * * other props from the embeddable
 */
export type LensRuntimeState = Simplify<Omit<ComponentSerializedProps, 'attributes' | 'references'> & {
    attributes: NonNullable<LensSerializedState['attributes']>;
} & Pick<LensComponentForwardedProps, 'viewMode' | 'abortController' | 'executionContext' | 'forceDSL'> & ContentManagementProps>;
...
export type LensInternalApi = Simplify<Pick<IntegrationCallbacks, 'updateAttributes' | 'updateOverrides'> & PublishesDataViews & VisualizationContextHelper & {
...
export declare const LENS_DATASOURCE_ID: {
    readonly FORM_BASED: "formBased";
    readonly TEXT_BASED: "textBased";
};
...
type TypedLensAttributes<TVisType, TVisState> = Simplify<Omit<LensDocument, 'savedObjectId' | 'state' | 'visualizationType'> & {
    visualizationType: TVisType;
    state: Simplify<Omit<LensDocument['state'], 'datasourceStates' | 'visualization'> & {
        datasourceStates: {
 ...
 */
export type TypedLensSerializedState = Simplify<Omit<LensSerializedState, 'attributes'> & {
    attributes: TypedLensAttributes<'lnsXY', XYPersistedState> | TypedLensAttributes<'lnsPie', LensPartitionVisualizationState> | TypedLensAttributes<'lnsHeatmap', HeatmapVisualizationState> | TypedLensAttributes<'lnsGauge', GaugeVisualizationState> | TypedLensAttributes<'lnsDatatable', DatatableVisualizationState> | TypedLensAttributes<'lnsLegacyMetric', LegacyMetricState> | TypedLensAttributes<'lnsMetric', MetricVisualizationState> | TypedLensAttributes<'lnsChoropleth', ChoroplethChartState> | TypedLensAttributes<'lnsTagcloud', LensTagCloudState> | TypedLensAttributes<string, unknown>;
}>;
/**
