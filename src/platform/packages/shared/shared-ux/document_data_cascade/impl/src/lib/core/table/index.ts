...
interface TableRowAdapterProps<G extends GroupNode, L extends LeafNode> {
  rowInstance: Row<G>;
}

export function useTableRowAdapter<G extends GroupNode, L extends LeafNode>({
  rowInstance,
}: TableRowAdapterProps<G, L>) {
  return useMemo(() => {
    return {
      ...
      get rowChildren() {
        return rowInstance.subRows as Row<G>[];
      },
