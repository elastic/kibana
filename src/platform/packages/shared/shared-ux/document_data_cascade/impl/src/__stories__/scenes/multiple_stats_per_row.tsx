...
import { getSiPrefixedNumber } from '../../components/helpers/number_badge/number_badge';
...
> = {
  render: function DataCascadeWrapper(args, context) {
    ...
    const initData: MockGroupData[] = new Array(100000).fill(null).map((_, index) => {
      return {
    ...
    >(({ rowData }) => {
      return [
        <EuiStat
          reverse
          title={getSiPrefixedNumber(Number(rowData.index))}
          textAlign="right"
          description={
            <FormattedMessage
              id="sharedUXPackages.data_cascade.demo.row.ordinalIndex"
              defaultMessage="<indicator>Ordinal index</indicator>"
              values={{
        ...
        <EuiFlexItem grow={false}>
          <EuiText>
            <div>
              <h3>{context.name}</h3>
              <p>
                {`Data Cascade showing multiple stats per row with an initialized data count of ${initData.length}, change the viewport to tablet to see
                how stats that exceeds the available screen space are displayed.`}
              </p>
            </div>
          </EuiText>
        </EuiFlexItem>
