...
-   The row is a `dl`, each pair a `dt`/`dd` inside a single wrapper element, so the title/value association is programmatic rather than visual. A `dl` accepts only one wrapper element around a pair, which is why the pair wrapper is also the flex item that wraps the row.
