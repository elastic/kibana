...
When the user scrolls the flyout body, the header automatically collapses to a compact row showing only the title: it drops to an `xs` heading on a single ellipsized line, with the full text available as a hover tooltip when it is a plain string. The description slides away to give the body the recovered space. The title row, the tab bar, and the divider stay pinned in both states.
