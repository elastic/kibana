...
import fs from 'fs';
import type { StorybookConfig as BaseStorybookConfig } from 'storybook-react-rsbuild';
...
export const defaultConfig: StorybookConfig = {
  ...
  framework: {
    ...
    options: { builder: { fsCache: true, lazyCompilation: true } },
  },
  typescript: {
    reactDocgen: false,
  },
  ...
  rsbuildFinal: (config) => {
    config.output = { ...config.output, overrideBrowserslist: ['last 1 chrome versions'] };
    config.resolve = {
      ...config.resolve,
      alias: { core_styles: resolve(REPO_ROOT, 'src/core/public/index.scss') },
    };
    return config;
  },
