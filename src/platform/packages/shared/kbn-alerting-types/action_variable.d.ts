 ...
 */
type FlattenKeys<T extends Record<string, unknown>, Key = keyof T> = Key extends string ? T[Key] extends Record<string, unknown> ? `${Key}.${FlattenKeys<T[Key]>}` : `${Key}` : never;
/**
 ...
 */
export type ActionContextVariablesFlatten = FlattenKeys<Omit<ActionContextVariables, 'context' | 'state' | 'params'>> | 'params' | 'rule.params';
export type SummaryActionContextVariablesFlatten = FlattenKeys<Omit<SummaryActionContextVariables, 'context' | 'state' | 'params'>> | 'params' | 'rule.params';
export {};
