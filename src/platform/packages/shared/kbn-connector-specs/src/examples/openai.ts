...
export const OpenAIConnectorExample: ConnectorSpec = {
  // ---- Metadata (required) ----
  metadata: {
    id: '.gen-ai',
    displayName: 'OpenAI',
    icon: 'logoOpenAI',
    description:
      'OpenAI, Azure OpenAI, and compatible services with streaming and function calling',
  ...
  schema: z.discriminatedUnion('method', [
    ...
    z.object({
      ...
      apiUrl: UISchemas.url('https://api.openai.com/v1').describe('API URL'),
      defaultModel: z.string().default('gpt-4o').describe('Default Model'),
    }),
    ...
    z.object({
      ...
      apiUrl: withUIMeta(UISchemas.url(), {
    ...
    z.object({
      ...
      certificateAuth: z
        .object({
          certificate: withUIMeta(z.string(), {
            widget: 'textarea',
            widgetOptions: { rows: 8 },
          })
            .optional()
            .describe('Certificate (PEM)'),
          privateKey: withUIMeta(z.string(), {
            sensitive: true,
            widget: 'textarea',
            widgetOptions: { rows: 8 },
          })
