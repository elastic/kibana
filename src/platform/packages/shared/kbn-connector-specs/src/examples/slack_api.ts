...
export const SlackApiConnectorExample: ConnectorSpec = {
  // ---- Metadata (required) ----
  metadata: {
    id: '.slack_api',
    displayName: 'Slack',
  ...
  schema: z.discriminatedUnion('method', [
    z.object({
      ...
      fields: z.object({
        ...
        Authorization: withUIMeta(z.string(), {
    ...
    z.object({
      ...
      fields: z.object({
        ...
        url: UISchemas.url('https://hooks.slack.com/services/...').describe('Webhook URL'),
        // UI derives: key-value editor (z.record())
        extraHeaders: z.record(z.string()).optional().describe('Additional Headers'),
      }),
    }),
  ]),
  ...
  actions: {
    ...
    postMessage: {
      ...
      input: z.object({
        ...
        text: withUIMeta(z.string().min(1), {
          widget: 'textarea',
