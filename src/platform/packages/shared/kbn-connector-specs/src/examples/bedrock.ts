...
export const BedrockConnectorExample: ConnectorSpec = {
  // ---- Metadata (required) ----
  metadata: {
    id: '.bedrock',
  ...
  schema: z.discriminatedUnion('method', [
    z.object({
      ...
      credentials: z.object({
        accessKey: withUIMeta(z.string(), {
          helpText: 'AWS IAM access key ID',
          placeholder: 'AKIAIOSFODNN7EXAMPLE',
        }).describe('Access Key ID'),

        secretKey: withUIMeta(UISchemas.secret(), {
  ...
  actions: {
    ...
    invokeAI: {
      ...
      input: z.object({
        ...
        temperature: z.number().min(0).max(1).optional().describe('Temperature'),

        maxTokens: z.number().int().positive().optional().default(512).describe('Max Tokens'),
