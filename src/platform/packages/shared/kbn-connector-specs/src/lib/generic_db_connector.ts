 ...
 *
 * isReadOnlySql strips leading whitespace/comments, rejects multi-statement
 * input (semicolons), and requires an allowlisted leading keyword. This matches
 * the original Snowflake and BigQuery prefix-only semantics.
 *
 * assertReadOnly is the stricter MySQL variant: it additionally rejects
 * write/DDL tokens that can hide after a read-only prefix (e.g. WITH ... INSERT)
 * and MySQL executable comments (/ *!...*\/) that bypass comment stripping.
 *
 * The semicolon check is conservative: a semicolon inside a string literal is treated
...
const WRITE_PATTERN =
  /\b(INSERT\s+INTO|REPLACE\s+INTO|UPDATE\s+|DELETE\s+|MERGE\s+INTO|CREATE\s+|DROP\s+|ALTER\s+|TRUNCATE\s+|GRANT\s+|REVOKE\s+|CALL\s+|LOAD\s+DATA|INTO\s+(OUTFILE|DUMPFILE))/i;
