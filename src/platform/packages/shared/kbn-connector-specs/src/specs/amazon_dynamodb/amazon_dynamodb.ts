...
export const AmazonDynamoDB: ConnectorSpec = {
  ...
  skill: [
    ...
    '',
    '### DynamoDB data model',
    'Every DynamoDB item is identified by a primary key. The primary key is either a single partition key, or a partition key plus a sort key (composite key). ' +
      'All attribute values in API calls use the DynamoDB typed value format: {"S": "string"}, {"N": "123"}, {"BOOL": true}, {"NULL": true}, {"L": [...]}, {"M": {...}}.',
    '',
    '### Typical workflow',
    '1. Call `listTables` to discover table names.',
    '2. Call `describeTable` to learn the primary key attributes, indexes, and attribute definitions for a table.',
    '3. Use `query` (preferred) or `scan` to find items.',
    '4. Use `getItem` to fetch a specific item by its exact primary key.',
    '',
    '### query vs. scan',
    '- Prefer `query` whenever you have the partition key — it is fast and cost-efficient.',
    '- Only use `scan` when there is no applicable key condition (e.g., searching by a non-key attribute on a table with no relevant GSI).',
    '- `scan` reads every item in the table and can be expensive for large tables.',
    '',
    '### Pagination',
    '- Both `query` and `scan` may return a `lastEvaluatedKey` when there are more results.',
    '- Pass `lastEvaluatedKey` as `exclusiveStartKey` in the next call to retrieve the next page.',
    '- `listTables` uses `lastEvaluatedTableName` / `exclusiveStartTableName` for pagination.',
    '',
    '### Expression syntax',
    '- Use expression attribute names (#alias) when an attribute name is a DynamoDB reserved word (e.g., "name", "status", "year").',
    '- Use expression attribute values (:alias) for all literal values in condition, key condition, and filter expressions.',
    '- Example key condition: "userId = :uid AND createdAt > :ts".',
    '',
    '### Conditional writes',
    '- Use `conditionExpression` in `putItem` and `deleteItem` to prevent overwriting or deleting unintended items.',
    '- "attribute_not_exists(pk)" in putItem ensures an insert-only operation.',
  ].join('\n'),
};
