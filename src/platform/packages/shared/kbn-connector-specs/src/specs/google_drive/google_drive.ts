...
export const GoogleDriveConnector: ConnectorSpec = {
  ...
  skill: [
    ...
    '',
    '## When searchFiles cannot find a file you expect to exist',
    "searchFiles relies on Google's search index, which only surfaces files shared directly with",
    'the authenticated account (or owned by it). A file reachable only because a parent folder was',
    'shared with you — not the file itself — may be missing from searchFiles results even when you',
    'search by its exact name. listFiles reads a folder by ID directly and bypasses the index, so it',
    'returns everything in a folder you can access, including such indirectly-shared files.',
    'When searchFiles returns nothing for a file the user insists exists, do not give up: use',
    'searchFiles to find the shared parent folder (or start from the root folder), then call listFiles',
    'to browse into its subfolders until you reach the file, then use its ID with downloadFile.',
    '',
