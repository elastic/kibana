...
describe('Gitlab connector', () => {
  ...
  describe('test handler', () => {
    it('calls GET /user and returns ok with username and name', async () => {
      mockGet.mockResolvedValue({ data: { username: 'jsmith', name: 'Jane Smith' } });

      const result = await Gitlab.test.handler(mockContext);

      expect(mockGet).toHaveBeenCalledWith(`${BASE}/user`);
      expect(result).toEqual({ username: 'jsmith', name: 'Jane Smith' });
    });
  });
