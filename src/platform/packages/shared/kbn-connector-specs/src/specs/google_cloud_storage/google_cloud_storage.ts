...
export const GoogleCloudStorageConnector: ConnectorSpec = {
  ...
  schema: z.object({
    projectId: z
      .string()
      .optional()
      .describe('Google Cloud project ID, required for listing buckets')
      .meta({
        label: 'Project ID',
        widget: 'text',
        placeholder: 'my-gcp-project',
        helpText:
          'Required when using the List Buckets action. Find it in the Google Cloud Console.',
      }),
  }),
  auth: {
    types: ['bearer'],
  ...
  actions: {
    listBuckets: {
      ...
      handler: async (ctx, input: ListBucketsInput) => {
        const typedConfig = (ctx.config ?? {}) as { projectId?: string };
        const { projectId } = typedConfig;

        if (!projectId) {
          throw new Error(
            'Project ID is required to list buckets. Configure it in the connector settings.'
          );
        }
