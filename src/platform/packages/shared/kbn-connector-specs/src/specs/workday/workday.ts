...
export const Workday: ConnectorSpec = {
  ...
  skill: [
    ...
    "- Time off: call `listAbsenceTypes` first to discover plan names, then `getTimeOffBalance` with the worker's own WID.",
    '- Recruiting: `listJobRequisitions` → `getJobRequisition`; use `listCandidates` with `jobRequisitionId` to scope to a role.',
    '- Learning: `listCourses` (optional `search`) → `getCourse`; `listEnrollments` requires a `workerId` — it is not optional.',
    "- Self-service: `listRequestTypes` to discover categories, then `listRequests` for the authenticated worker's submissions.",
    '- Procurement: `listPurchaseOrders` is list-only — the Workday API has no GET-by-ID for purchase orders.',
    '',
    'Gotchas:',
    '- WIDs are opaque; always obtain them from a prior search/list call.',
    '- `listInboxTasks` and `getTimeOffBalance` only return data for the currently authenticated OAuth user.',
    '- `checkBudget` is a read-only POST check — it does not create a transaction.',
