...
export const CensysConnector: ConnectorSpec = {
  ...
  skill: [
    ...
    '',
    '## Rescan flow',
    '1. Call `rescan` with `type` set to `service` (plus `ip`, `port`, `protocol`, `transportProtocol`) or `webproperty` (plus `hostname` and `port`).',
    ...
    '1. Call `censEyeCreateAnalysisJob` with `type` set to `host`, `webproperty`, or `certificate`, plus the matching identifier fields at the top level: `host` for `host`, `hostname` and `port` for `webproperty`, `certificate` for `certificate`.',
    ...
    '- Web property: two separate top-level fields, `hostname` and `port` (e.g., `hostname: "example.com"`, `port: 443`). Pass an IPv6 address as a plain hostname — the connector adds the `[...]` brackets when building the web property ID for the API.',
