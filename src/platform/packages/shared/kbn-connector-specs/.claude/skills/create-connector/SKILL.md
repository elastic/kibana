...
**Do not add `events`.** Optional inbound `ConnectorSpec.events` is allowlisted in
`connector_spec_contract.test.ts` (`SPECS_ALLOWED_EVENTS`). New outbound connectors must omit `events`
unless that allowlist explicitly includes the connector id.
