...
export async function searchDashboards(
  { hasNoReference, hasReference, options, search, size }: SearchDashboardsArgs,
...
export async function findDashboardsByIds(
  ...
  const findPromises = ids.map((id) => findDashboardById(id, contentManagementService));
