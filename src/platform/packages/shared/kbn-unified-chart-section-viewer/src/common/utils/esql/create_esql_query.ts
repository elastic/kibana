 ...
 *   of `metricItem.sourceName` so the chart's scope matches the scope METRICS_INFO scanned.
...
export function createESQLQuery({
  ...
  const { metricName, metricTypes, fieldTypes, sourceName } = metricItem;
  const index = isConcreteSingleSource(originalSource) ? originalSource : sourceName;
