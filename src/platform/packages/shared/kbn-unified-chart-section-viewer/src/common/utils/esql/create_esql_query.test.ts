...
`.trim()
  ...
  it('should override index if provided in metric', () => {
    const query = createESQLQuery({
      metricItem: { ...mockMetric, sourceName: 'custom-metrics-*' },
    });
    expect(query).toBe(
      `
TS custom-metrics-*
...
`.trim()
  ...
  describe('conflicting field types (issue #5385)', () => {
    const mockMetricWithConflictingTypes: ParsedMetricItem = {
      ...
      sourceName: 'timeseries-rich-metrics-primary',
      units: ['ms'],
      metricTypes: ['gauge'],
...
`.trim()
  ...
  describe('originalSource (issue #262360)', () => {
    ...
    const mockMetricForBackingIndex: ParsedMetricItem = {
      ...
      fieldTypes: [ES_FIELD_TYPES.LONG],
      sourceName: 'edge-case-gauge-to-counter',
      units: ['ms'],
      metricTypes: ['gauge'],
      dimensionFields: [],
    };
...
`.trim()
    ...
    it('should fall back to sourceName when originalSource contains a glob', () => {
      const query = createESQLQuery({
        metricItem: mockMetricForBackingIndex,
...
`.trim()
    ...
    it('should fall back to sourceName when originalSource is a comma list', () => {
      const query = createESQLQuery({
        metricItem: mockMetricForBackingIndex,
...
`.trim()
    ...
    it('should fall back to sourceName when originalSource is undefined', () => {
      const query = createESQLQuery({
        metricItem: mockMetricForBackingIndex,
      });
      expect(query).toBe(
...
`.trim()
    ...
    it('should fall back to sourceName when originalSource is empty', () => {
      const query = createESQLQuery({
        metricItem: mockMetricForBackingIndex,
        originalSource: '',
      });
