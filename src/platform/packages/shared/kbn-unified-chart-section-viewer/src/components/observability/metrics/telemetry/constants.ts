...
export const METRICS_ESQL_QUERY_FAILURE_EVENT_TYPE = 'metrics_esql_query_failure';

/**
 * Value of the `profile` field on `metrics_esql_query_failure`. Deliberately
 * the short profile name rather than the `metrics-data-source-profile`
 * registration ID, so the field stays stable if that ID is renamed.
 */
