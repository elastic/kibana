...
describe('parseMetricsWithTelemetry', () => {
  ...
  it('returns one metric when source_name is a single string', () => {
    const response: MetricsESQLResponse[] = [
      {
  ...
  it('returns two metrics when source_name is an array of two', () => {
    const response: MetricsESQLResponse[] = [
      {
        metric_name: 'cpu.usage',
        source_name: ['stream-a', 'stream-b'],
        unit: ['percent'],
        metric_type: 'gauge',
        field_type: ES_FIELD_TYPES.DOUBLE,
        dimension_fields: 'host.name',
  ...
  it('falls back to the legacy `data_stream` column when `source_name` is absent', () => {
    // The ES|QL `METRICS_INFO` response still returns a column literally named
    // `data_stream`. The TS surface exposes it as `source_name`, but the parser
