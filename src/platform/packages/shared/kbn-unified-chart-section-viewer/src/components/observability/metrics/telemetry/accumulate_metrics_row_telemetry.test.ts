...
describe('accumulateMetricsRowTelemetry', () => {
  ...
  it('counts a single-value metric row', () => {
    ...
    accumulateMetricsRowTelemetry(telemetry, {
      sourceNames: ['metrics-system.cpu-default'],
      units: ['percent'],
      metricTypes: ['gauge'],
  ...
  it('counts null units as none', () => {
    ...
    expect(telemetry).toEqual({
      ...
      multi_value_counts: { source_names: 0, field_types: 0, metric_types: 0, units: 1 },
    });
  });
  ...
  it('accumulates multi-value rows across multiple calls', () => {
    ...
    accumulateMetricsRowTelemetry(telemetry, {
      sourceNames: ['metrics-system.cpu-default', 'metrics-system.cpu-prod'],
      units: ['percent'],
      metricTypes: ['gauge'],
      fieldTypes: [ES_FIELD_TYPES.DOUBLE],
    });
    ...
    accumulateMetricsRowTelemetry(telemetry, {
      sourceNames: ['metrics-system.memory-default'],
      units: ['bytes', 'percent'],
    ...
    expect(telemetry).toEqual({
      ...
      multi_value_counts: { source_names: 1, field_types: 1, metric_types: 1, units: 1 },
    });
  });
});
