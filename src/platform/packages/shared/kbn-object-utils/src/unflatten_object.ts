...
export function unflattenObjectWithEscapedDots<T extends Record<string, any>>(
  source: T,
  target: Record<string, any> = {}
): DedotObject<T> {
  // eslint-disable-next-line guard-for-in
  for (const key in source) {
    ...
    let unescapedPath = key as string | string[];
    if (key.includes('\\.')) {
      // Creates a path array with escaped dots (`\\.`) turned to dots (`.`).
      unescapedPath = key
        .split(/\\\./g)
        .join('\\~')
        .split('.')
    ...
    if (Array.isArray(val)) {
      ...
      set(target, unescapedPath, unflattenedArray);
    } else if (val !== null && typeof val === 'object') {
      set(target, unescapedPath, unflattenObject(val));
    } else {
      set(target, unescapedPath, val);
    }
  }
