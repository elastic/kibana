...
describe('JSON Schema to Zod parser - Unit tests', () => {
  ...
  describe('additionalProperties', () => {
    it('retains unknown keys by default (omitted additionalProperties)', () => {
      const jsonSchema = {
        type: 'object',
        properties: {
          id: { type: 'string' },
      ...
      const result = zodSchema!.parse({ id: 'x', extra: 'kept' }) as Record<string, unknown>;
      expect(result.extra).toBe('kept');
    });

    it('retains unknown keys when additionalProperties is true', () => {
      const jsonSchema = {
        type: 'object',
        properties: {
          id: { type: 'string' },
      ...
      const result = zodSchema!.parse({ id: 'x', extra: 42 }) as Record<string, unknown>;
    ...
    it('rejects unknown keys when additionalProperties is false', () => {
      ...
      expect(() => zodSchema!.parse({ id: 'x' })).not.toThrow();
      expect(() => zodSchema!.parse({ id: 'x', extra: 'nope' })).toThrow();
    });

    it('validates unknown keys against the additionalProperties schema', () => {
      const jsonSchema = {
        type: 'object',
        properties: {
          id: { type: 'string' },
      ...
      const result = zodSchema!.parse({ id: 'x', count: 1, total: 2 }) as Record<string, unknown>;
      expect(result.count).toBe(1);
      expect(result.total).toBe(2);

      expect(() => zodSchema!.parse({ id: 'x', count: 'not-a-number' })).toThrow();
    });

    it('preserves nested open-shaped objects (regression: MCP exec tool input)', () => {
      // The elastic MCP `exec` tool declares an open `input` object, where the
      // shape varies by the sibling `id`. Without honoring `additionalProperties`
      // the converter would silently strip everything inside `input`.
      const jsonSchema = {
        type: 'object',
        properties: {
          id: { type: 'string' },
      ...
      const result = zodSchema!.parse({
        id: 'es.cat.indices',
        input: { index: 'logs-*' },
      }) as { id: string; input: Record<string, unknown> };

      expect(result.input).toEqual({ index: 'logs-*' });
    });
  });
