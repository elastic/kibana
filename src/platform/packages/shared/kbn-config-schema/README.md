...
__Options:__
  ...
  * `unknowns: 'ignore' | 'forbid'` - indicates whether unknown properties and sub-properties in nested objects should be ignored or forbidden. It is `forbid` by default unless the global validation option `stripUnknownKeys` is set to `true` when calling `validate()`.
