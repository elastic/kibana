...
- **`local`**: Tests are executed against servers started locally. Configuration is auto-generated by Scout and saved to `KIBANA_REPO_ROOT/.scout/servers/local.json`.
  ...
  > **Note:** `local.json` is validated against the same schema as the cloud configs. Because it is auto-generated when servers start, a previously generated file can become **stale** if a new required field (such as `cloudUsersFilePath`) is added to the schema afterwards. If you run with `--skip-server`/`--skip-init` against an already-running server, the stale file is reused and you'll see a validation error like `'cloudUsersFilePath' is required`. To fix it, either regenerate the file by starting servers without `--skip-server`, or add the missing field manually (for local stateful runs `cloudUsersFilePath` is not actually read, so any non-empty path works — the generator uses `KIBANA_REPO_ROOT/.ftr/role_users.json`).
