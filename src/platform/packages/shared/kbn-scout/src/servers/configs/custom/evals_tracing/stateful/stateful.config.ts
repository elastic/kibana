...
/**
 * Scout configuration for @kbn/evals with native Elasticsearch OTLP tracing.
 *
 * Sends traces directly to ES 9.x+ native OTLP endpoints (/_otlp/v1/traces)
 * using protobuf, with fallback to EDOT collector HTTP endpoint for older ES.
 *
 * Usage:
 *   node scripts/scout start-server --stateful --config-dir evals_tracing
 */
