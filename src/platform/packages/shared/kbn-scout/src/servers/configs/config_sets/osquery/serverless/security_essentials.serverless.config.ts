...
import { servers as securityEssentialsConfig } from '../../default/serverless/security_essentials.serverless.config';

/**
 * Custom Scout server configuration for Osquery tests in serverless security essentials mode.
 *
 * This configuration extends the default security essentials serverless config and adds:
...
const kbnServerArgsWithoutFleetHosts = securityEssentialsConfig.kbnTestServer.serverArgs.filter(
  (arg: string) =>
    !arg.startsWith('--xpack.fleet.fleetServerHosts=') && !arg.startsWith('--xpack.fleet.outputs=')
);

export const servers: ScoutServerConfig = {
  ...securityEssentialsConfig,

  kbnTestServer: {
    ...securityEssentialsConfig.kbnTestServer,
    serverArgs: [
      ...
      `--xpack.fleet.outputs=${JSON.stringify([
        {
          ...
          hosts: [`https://es01:${securityEssentialsConfig.servers.elasticsearch.port}`],
        },
      ])}`,
