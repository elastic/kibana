...
const handleNonFlattenedOutput = (
  ...
  if (flagsReader.boolean('save')) {
    ...
    // The saved manifest contains one entry per discovered module, with the full set of
    // `(arch, domain)` modes in `serverRunFlags`. Pipelines that need to fan modules out
    // into one Buildkite step per mode (e.g. heavy suites like streams_app/dashboard in
    // the regular Scout pipeline) apply that scheduling decision themselves, by calling
    // `splitModulesByServerRunFlags` from `.buildkite/pipeline-utils/scout/module_split.ts`.
