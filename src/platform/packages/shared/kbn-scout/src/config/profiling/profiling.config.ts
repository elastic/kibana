...
const profilingKibanaYamlFilePath = join(__dirname, './profiling_kibana.yml');
...
const kbnUrl = `${serverConfig.kibana.protocol}://${serverConfig.kibana.hostname}:${serverConfig.kibana.port}`;
...
export const servers: ScoutServerConfig = {
  ...
  kbnTestServer: {
    ...
    serverArgs: [
      `--server.port=${serverConfig.kibana.port}`,
      ...
      `--elasticsearch.hosts=${formatUrl(
        Object.fromEntries(
          Object.entries(serverConfig.elasticsearch).filter(([key]) => key.toLowerCase() !== 'auth')
        )
      )}`,
      `--elasticsearch.username=${serverConfig.elasticsearch.username}`,
      `--elasticsearch.password=${serverConfig.elasticsearch.password}`,
      ...
      // Load profiling configuration from YAML file
      `--config=${profilingKibanaYamlFilePath}`,
    ],
  },
};
