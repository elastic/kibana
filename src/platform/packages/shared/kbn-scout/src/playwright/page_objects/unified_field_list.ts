...
export class UnifiedFieldList {
  ...
  private async toggleMetaSection(): Promise<void> {
    ...
    // EuiAccordion exposes its expanded state via `aria-expanded` on the
    // toggle button — we use that rather than the EUI `euiAccordion-isOpen`
    // class so the selector stays decoupled from EUI internals.
    const sectionContainer = this.page.testSubj.locator('fieldListGroupedMetaFields');
    const toggleButton = sectionContainer.locator('button[aria-expanded]');
  ...
  async clickFieldListItemAdd(fieldName: string): Promise<void> {
    ...
    const toggle = this.page.testSubj.locator(`fieldToggle-${fieldName}`);
