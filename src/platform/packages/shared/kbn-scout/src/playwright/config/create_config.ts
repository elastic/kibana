...
export function createPlaywrightConfig(options: ScoutPlaywrightOptions): PlaywrightTestConfig {
  ...
  return defineConfig<ScoutTestOptions>({
    ...
    reporter: [
      ['html', { outputFolder: './.scout/output/reports', open: 'never' }], // HTML report configuration
      ['json', { outputFile: './.scout/output/reports/test-results.json' }], // JSON report
    ...
    outputDir: './.scout/output/test-artifacts', // For other test artifacts (screenshots, videos, traces)
