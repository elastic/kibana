...
function handleStatsByColumnLeafOperation(
  ...
  // get the column field definition from the column node
  const columnField = (columnNode.definition as ESQLColumn).args.find(
    (arg) => arg.text === columnNode.definition.name
  )!;

  let operationColumnName = columnField.name;

  if (columnField.type === 'literal' && columnField.literalType === 'param') {
    const controlVariable = esqlVariables?.find((variable) => variable.key === columnField.value);

    if (!controlVariable) {
      throw new Error(`The control variable for the "${columnField.name}" column was not found`);
    }

    operationColumnName = controlVariable.value as string;
  }
...
function handleStatsByCategorizeLeafOperation(
  ...
  if (isFunctionExpression(categorizedField)) {
    ...
    // if it is not a function, then it is a column so we look to get the column field definition from the column node
    const columnFieldNode = (categorizedField as ESQLColumn).args.find(
      (arg) => arg.text === (categorizedField as ESQLColumn).name
    )!;

    categorizedFieldName = columnFieldNode.name;

    if (columnFieldNode.type === 'literal' && columnFieldNode.literalType === 'param') {
      const controlVariable = esqlVariables?.find(
        (variable) => variable.key === columnFieldNode.value
      );

      if (!controlVariable) {
        throw new Error(
          `The control variable for the "${columnFieldNode.name}" column was not found`
        );
      }

      categorizedFieldName = controlVariable.value as string;
    }
  }
...
>(
  ...
  if (isFieldUsedInOperatingStatsCommand) {
    ...
    // get more details about the field to be filtered on
    const fieldColumnNode = (
      fieldDeclarationCommandSummary.grouping[fieldName].column as ESQLColumn
    ).args.find(
      (arg) => arg.text === fieldDeclarationCommandSummary.grouping[fieldName].column.name
    )!;

    if (fieldColumnNode.type === 'literal' && fieldColumnNode.literalType === 'param') {
      const controlVariable = esqlVariables?.find(
        (variable) => variable.key === fieldColumnNode.value
      );

      if (!controlVariable) {
        throw new Error(
          `The control variable for the "${fieldColumnNode.name}" column was not found`
        );
      }

      fieldName = controlVariable.value as string;
    }
  } else {
