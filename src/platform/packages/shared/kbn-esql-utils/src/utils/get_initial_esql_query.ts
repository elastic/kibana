...
const OVERLY_BROAD_WILDCARDS = new Set(['*', '*:*']);

/**
 * Matches every index on the local cluster (`*`) or on every remote cluster (`*:*`). TSDB field
 * metadata surfacing from such a broad resolution is not a reliable signal that the user wants a
 * time series query.
 */
const hasOverlyBroadIndexPattern = (indexPattern: string): boolean =>
  indexPattern.split(',').some((entry) => OVERLY_BROAD_WILDCARDS.has(entry.trim()));
 ...
 * `*` and `*:*` match every index, so TSDB field metadata there is not a reliable signal of time
 * series intent; and when the user has a query or filters, TS would restrict the results to the TSDB
 * indices, where the filtered fields may not exist.
