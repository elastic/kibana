...
const collectEqualitiesFromWhereExpression = (
  ...
  if (isBinaryExpression(expr)) {
    if (expr.name === 'and') {
      collectEqualitiesFromWhereExpression(expr.args[0], entitySet, out);
      collectEqualitiesFromWhereExpression(expr.args[1], entitySet, out);
      return;
    }
    if (expr.name === '==') {
      const [a, b] = expr.args;
...
const collectEntityLiteralsFromCommands = (
  branchCommands: readonly ESQLAstCommand[],
  entitySet: ReadonlySet<string>
): Map<string, unknown> => {
  const out = new Map<string, unknown>();
  for (const cmd of branchCommands) {
    if (cmd.name === 'where') {
      collectEqualitiesFromWhereExpression(cmd.args[0] as ESQLAstItem | undefined, entitySet, out);
    }
    if (cmd.name === 'eval') {
      for (const arg of cmd.args) {
        ...
        for (const item of items) {
          ...
          if (!entitySet.has(colId)) continue;
...
const forkRowLiteralMatchesCell = (cell: unknown, literalValue: unknown): boolean => {
...
export const appendEntityFiltersToChangePointLineEsql = (
  ...
  if (!entityColumnIds.length) {
    return lineEsql;
  }
  const entitySet = new Set(entityColumnIds);
  let existing = new Map<string, unknown>();
  try {
    const { root } = Parser.parse(lineEsql);
    existing = collectEntityLiteralsFromCommands(root.commands as ESQLAstCommand[], entitySet);
  } catch {
    existing = new Map();
  }
  ...
  for (const col of entityColumnIds) {
    ...
    if (existing.has(col) && forkRowLiteralMatchesCell(row[col], existing.get(col))) {
      continue;
    }
...
/**
 * Returns the BY grouping column names from a `CHANGE_POINT ... BY col1[, col2...]` command.
 * Returns `undefined` when the query has no CHANGE_POINT command, no BY clause, or fails to parse.
 *
 * For a query like `... | CHANGE_POINT total_bytes ON day BY host, response.keyword` this returns
 * `['host', 'response.keyword']`.
 */
export const getChangePointByColumns = (esql?: string): string[] | undefined => {
  ...
  try {
    ...
    const byOption = cp.args.find(
      (a) => (a as ESQLCommandOption).type === 'option' && (a as ESQLCommandOption).name === 'by'
    ) as ESQLCommandOption | undefined;
    if (!byOption) return undefined;
    const cols = byOption.args.filter(isColumn).map((c) => (c as ESQLColumn).parts.join('.'));
...
export const buildChangePointLineDataQuery = (
  ...
  try {
    ...
    if (forkIndex >= 0) {
      ...
      if (!prefixBeforeFork.some((c) => isSourceCommandName(c.name))) {
        return undefined;
      }
