...
export type WithActiveSpanAsUnion<T = unknown> =
  ...
  | ((name: string, opts: WithActiveSpanOptions, ctx: Context, cb: () => T) => T);
...
export function withActiveSpan(...args: Args) {
  ...
  const tracer = opts.tracer || tracingApi?.getDefaultTracer();

  if (!tracer || isTracingSuppressed(ctx)) {
    return cb();
  }
