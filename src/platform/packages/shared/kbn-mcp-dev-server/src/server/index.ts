...
import { semanticCodeSearchTool } from '../tools/semantic_code_search';
import { getDistinctValuesTool } from '../tools/get_distinct_values';
import { symbolAnalysisTool } from '../tools/symbol_analysis';
...
run(async () => {
  ...
  addTool(server, semanticCodeSearchTool);
  addTool(server, getDistinctValuesTool);
  addTool(server, symbolAnalysisTool);
