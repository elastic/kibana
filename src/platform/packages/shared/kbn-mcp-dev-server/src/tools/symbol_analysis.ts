...
const description = `The precision tool for the second step in a "chain of investigation." Use this *after* \`semantic_code_search\` has helped you identify a specific, concrete symbol (e.g., a class name, function name, or type alias).
...
**Workflow:**
1.  Use \`semantic_code_search\` to discover a key symbol (e.g., \`indicatorTypesSchema\`).
2.  Feed that exact symbol name into \`symbol_analysis\` to get a comprehensive, cross-referenced report of all its connections.
`;

export const symbolAnalysisTool: ToolDefinition<typeof findUsagesInputSchema> = {
