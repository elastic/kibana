 ...
 * and supplies `baseCloneDir` on the WorkspaceContext (typically something like
 * `${workspaceRoot}/base`). Individual git worktrees are then created as siblings
 * inside `${workspaceRoot}/`, not nested inside the base clone (git requires that
 * worktrees live outside the primary working tree directory).
 */
export async function ensureClonedRepo(context: WorkspaceGlobalContext): Promise<string> {
  ...
  if (!gitDirExists) {
    ...
    await shell(`git clone ${repoRoot} ${baseCloneDir}`, {
      log,
      cwd: process.cwd(),
    });
    return baseCloneDir;
