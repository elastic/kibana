...
export const IndicesAnalyzeRequest = z.object({
  ...
  char_filter: z.array(AnalysisCharFilter).describe('Array of character filters used to preprocess characters before the tokenizer.').optional().meta({ found_in: 'body' }),
  ...
  filter: z.array(AnalysisTokenFilter).describe('Array of token filters used to apply after the tokenizer.').optional().meta({ found_in: 'body' }),
  ...
  tokenizer: AnalysisTokenizer.describe('Tokenizer to use to convert text into tokens.').optional().meta({ found_in: 'body' })
