...
`
  ...
  await generateInlineCastsMapping(pathToElasticsearch);
})();
...
async function generateInlineCastsMapping(pathToElasticsearch: string) {
  ...
  const outputTsPath = join(
    __dirname,
    '../src/commands/definitions/generated/inline_casts_mapping.ts'
  );

  const tsContent = `// Do not edit this manually... generated by scripts/generate_function_definitions.ts
export const inlineCastsMapping: Record<string, string> = ${JSON.stringify(
    inlineCastsDefinition,
    null,
    2
  )};
`;
