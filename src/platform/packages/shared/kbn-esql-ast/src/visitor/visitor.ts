...
> {
  ...
  public static readonly findNodeChainAtOrBefore = (
    ast: ESQLAstQueryExpression,
    pos: number
  ): ESQLProperNode[] => {
    return new Visitor()
      .on('visitExpression', (ctx): ESQLProperNode[] => {
        const nodeLocation = ctx.node.location;
        const nodes = [...ctx.arguments()];
        ...
        if (nodeLocation && nodeLocation.max < pos) {
          const last = nodes[nodes.length - 1];
          if (last && last.location && last.location.max === nodeLocation.max) {
            return [...ctx.visitExpression(last, undefined), ctx.node];
          } else {
            return [ctx.node];
          }
        }
        ...
        for (let i = nodes.length - 1; i >= 0; i--) {
          ...
          if (!location) continue;
          const isInside = location.min <= pos && location.max >= pos;
          if (isInside) return [...ctx.visitExpression(node, undefined), ctx.node];
          const isAfter = location.max < pos;
          if (isAfter) {
            return [...ctx.visitExpression(node, undefined), ctx.node];
          }
        }

        return [ctx.node];
      })
      .on('visitCommand', (ctx): ESQLProperNode[] => {
        const nodes = [...ctx.arguments()];
        for (let i = nodes.length - 1; i >= 0; i--) {
          const node = nodes[i];
          const { location } = node;
          if (!location) continue;
          const isInside = location.min <= pos && location.max >= pos;
          if (isInside) return [...ctx.visitExpression(node), ctx.node];
          const isAfter = location.max < pos;
          if (isAfter) {
            if (ctx.node.location && ctx.node.location.max === location.max) {
              return [...ctx.visitExpression(node), ctx.node];
            }
      ...
      .on('visitQuery', (ctx): ESQLProperNode[] => {
        const nodes = [...ctx.commands()];
        for (let i = nodes.length - 1; i >= 0; i--) {
          const node = nodes[i];
          const { location } = node;
          ...
          const isInside = min <= pos && pos <= max;
          if (isInside) return [...ctx.visitCommand(node), ctx.node];
          const isAfter = location.max < pos;
          if (isAfter) return [...ctx.visitCommand(node), ctx.node];
        }
        return [];
      })
      .visitQuery(ast);
