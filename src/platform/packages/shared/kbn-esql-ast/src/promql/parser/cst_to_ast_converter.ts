...
import type * as cst from '../../antlr/promql_parser';
import {
...
} from '../../antlr/promql_parser';
import { getPosition } from '../../parser/tokens';
import { PromQLBuilder } from '../builder';
import type { AstNodeParserFields } from '../builder/types';
import type {
  PromQLAstExpression,
...
export class PromQLCstToAstConverter {
  ...
  private fromParserRuleToUnknown(ctx: antlr.ParserRuleContext): PromQLUnknownItem {
    const location = getPosition(ctx.start, ctx.stop);
  ...
  public fromExpression(ctx: cst.ExpressionContext): PromQLAstExpression | undefined {
    if (!ctx) return undefined;
    ...
    if (ctx instanceof ValueExpressionContext) {
      return this.fromValueExpression(ctx);
    }

    // Parenthesized: (expression)
    if (ctx instanceof ParenthesizedContext) {
    ...
    if (ctx instanceof SubqueryContext) {
  ...
  private fromArithmeticUnary(ctx: ArithmeticUnaryContext): PromQLAstExpression {
  ...
  private fromValueExpression(ctx: ValueExpressionContext): PromQLAstExpression | undefined {
  ...
  private fromValue(ctx: cst.ValueContext): PromQLAstExpression | undefined {
  ...
  private fromFunction(ctx: cst.FunctionContext): PromQLAstExpression {
    const identToken = ctx.IDENTIFIER();
    const name = identToken?.getText() ?? '';

    const paramsCtx = ctx.functionParams();
    const args: PromQLAstExpression[] = [];
  ...
  private fromGrouping(ctx: cst.GroupingContext): PromQLGrouping {
  ...
  private fromLabelList(ctx: cst.LabelListContext): PromQLLabelName[] {
    const labels: PromQLLabelName[] = [];
  ...
  private fromSelector(ctx: cst.SelectorContext): PromQLSelector {
    const seriesMatcherCtx = ctx.seriesMatcher();
    const { metric, labels } = this.fromSeriesMatcher(seriesMatcherCtx);

    // Range: [duration]
    const lsb = ctx.LSB();
    const durationCtx = ctx.duration();
    let range: PromQLAstExpression | undefined;
  ...
  private fromSeriesMatcher(ctx: cst.SeriesMatcherContext): {
    ...
    const labels = labelsCtx ? this.fromLabels(labelsCtx) : [];

    return { metric, labels };
  }

  private fromLabels(ctx: cst.LabelsContext): PromQLLabel[] {
  ...
  private fromLabel(ctx: cst.LabelContext): PromQLLabel | undefined {
    ...
    let operator: PromQLLabelMatchOperator = '=';
    ...
    let value: PromQLStringLiteral | undefined;
  ...
  private fromLabelName(ctx: cst.LabelNameContext): PromQLLabelName | undefined {
    const identCtx = ctx.identifier();
    if (identCtx) {
  ...
  private fromArithmeticBinary(ctx: ArithmeticBinaryContext): PromQLAstExpression {
  ...
  private toBinaryOperator(text: string): PromQLBinaryOperator {
    ...
    switch (lowerText) {
      ...
      case '<':
      case '<=':
        return lowerText as PromQLBinaryOperator;
      case 'and':
      case 'or':
      case 'unless':
        return lowerText as PromQLBinaryOperator;
  ...
  private fromModifier(ctx: cst.ModifierContext): PromQLModifier {
    ...
    let groupModifier: PromQLGroupModifier | undefined;
  ...
  private fromParenthesized(ctx: ParenthesizedContext): PromQLAstExpression {
    const exprCtx = ctx.expression();
  ...
  private fromSubquery(ctx: SubqueryContext): PromQLAstExpression {
    // The base expression is the left-hand side of the subquery
    // In the grammar, expr[range:resolution] is parsed as a postfix operation
    // So we need to get the expression from the recursion context
    const exprCtx = ctx.expression();
    ...
    let resolution: PromQLAstExpression | undefined;
  ...
  private fromEvaluation(ctx: cst.EvaluationContext): PromQLEvaluation {
  ...
  private fromDuration(ctx: cst.DurationContext): PromQLAstExpression {
  ...
  private fromConstant(ctx: cst.ConstantContext): PromQLAstExpression {
    const numberCtx = ctx.number_();
    if (numberCtx) {
      return this.fromNumber(numberCtx);
    }
  ...
  private fromNumber(ctx: cst.NumberContext): PromQLNumericLiteral {
    ...
    // Hexadecimal
    if (ctx instanceof HexLiteralContext) {
  ...
  private fromString(ctx: cst.StringContext): PromQLStringLiteral {
  ...
  private fromStringToken(token: antlr.Token): PromQLStringLiteral {
  ...
  private fromTimeValue(ctx: cst.TimeValueContext): PromQLTimeValue {
    const text = ctx.getText();
    const parserFields = this.getParserFields(ctx);
  ...
  private fromIdentifier(ctx: cst.IdentifierContext): PromQLIdentifier {
    const identToken = ctx.IDENTIFIER();
    if (identToken) {
