...
describe('PromQL Parser', () => {
  describe('basic parsing', () => {
    ...
    it('parses a metric selector with labels', () => {
      ...
      expect(selector.labels[0].labelName.name).toBe('job');
      expect(selector.labels[1].labelName.name).toBe('status');
    });
    ...
    it('parses a range vector selector', () => {
      ...
      const selector = result.root.expression as any;
      expect(selector.range).toBeDefined();
    });
  ...
  describe('static methods', () => {
    it('parseExpression returns the expression directly', () => {
      const result = PromQLParser.parseExpression('http_requests_total');

      expect(result.root.type).toBe('selector');
    });

    it('parseQuery throws on errors', () => {
      expect(() => PromQLParser.parseQuery('rate(')).toThrow();
    });

    it('parseErrors returns only errors', () => {
      const errors = PromQLParser.parseErrors('rate(');
