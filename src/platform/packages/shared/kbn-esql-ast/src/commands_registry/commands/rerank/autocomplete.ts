 ...
 */
import type { ESQLCommand, ESQLSingleAstItem, ESQLAstRerankCommand } from '../../../types';
...
const QUERY_TEXT = 'Your search query.' as const;
...
export async function autocomplete(
  ...
  switch (position) {
    ...
    case CaretPosition.ON_KEEP_OPERATOR_AFTER_TRAILING_SPACE: {
      ...
      const suggestedColumnName = callbacks.getSuggestedUserDefinedColumnName?.() || '';
      if (
        lastOnField &&
        isColumn(lastOnField) &&
        getExpressionPosition(innerText, lastOnField) === 'after_column' &&
        (rerankCommand?.targetField?.name === lastOnField.name ||
          lastOnField.name === suggestedColumnName ||
          !context?.columns?.has(lastOnField.name))
      ) {
        return [assignCompletionItem];
      }
...
async function handleOnFieldList({
  ...
  const suggestions = await handleFragment(
    ...
    (fragment: string, rangeToReplace: { start: number; end: number }) => {
      return buildNextActions({ withSpaces: true }).map((suggestion) => ({
        ...suggestion,
        filterText: fragment,
...
export function buildNextActions(options?: {
  withSpaces?: boolean;
