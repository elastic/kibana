...
function isAfterCompleteFieldWithSpace(text: string): boolean {
  ...
  const endsWithCommaSpace = getLastNonWhitespaceChar(text) === ',';

  // Also check if we're after a comma-separated field (like ", agent ")
  const afterCommaAndField = /,\s*[^=\s,]+\s+$/i.test(text);

  return (hasFieldWithSpace && !hasAssignment && !endsWithCommaSpace) || afterCommaAndField;
}
