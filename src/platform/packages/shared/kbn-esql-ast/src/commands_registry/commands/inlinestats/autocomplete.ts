...
export async function autocomplete(
  ...
  switch (pos) {
    ...
    case 'expression_complete': {
      ...
      let expressionRoot: ESQLSingleAstItem | undefined;
      if (Array.isArray(lastArg)) {
        return [];
      }

      if (isAssignment(lastArg)) {
        const rightHandAssignment = lastArg.args[lastArg.args.length - 1];
        if (Array.isArray(rightHandAssignment)) {
          expressionRoot = rightHandAssignment[0] as ESQLSingleAstItem;
        } else if (rightHandAssignment && !Array.isArray(rightHandAssignment)) {
          expressionRoot = rightHandAssignment as ESQLSingleAstItem;
        }
      } else {
        expressionRoot = lastArg as ESQLSingleAstItem;
      }
      ...
      // Align behavior with STATS: after a complete expression, still suggest
      // operators appropriate for the expression, plus continuation tokens.
      const suggestions = await suggestForExpression({
        innerText,
        getColumnsByType: callbacks?.getByType,
        expressionRoot,
      ...
      return [...suggestions, ...controlSuggestions];
    }
    ...
    case 'grouping_expression_complete': {
      ...
      return getCommaAndPipe(innerText, expressionRoot, columnExists);
    }
