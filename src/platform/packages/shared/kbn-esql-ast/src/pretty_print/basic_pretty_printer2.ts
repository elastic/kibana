...
import { ESQLAstExpressionNode } from '../visitor';
...
export class BasicPrettyPrinter {
  ...
  protected readonly reducer: Reducer<string, void> = new Reducer<string, void>({
    up: (ctx, up) => {
      const node = ctx.node;
      switch (node.type) {
        case 'identifier': {
        ...
        case 'command': {
          ...
          const cmdFormatted = `${cmdType}${cmd}${argsFormatted}`;
        ...
        case 'query': {
          ...
          const text = up.join(cmdSeparator);
  ...
  public printIdentifier(node: ESQLIdentifier): string {
    const formatted = LeafPrinter.identifier(node);
    return this.decorateWithComments(node, formatted);
  }

  public printSource(node: ESQLSource): string {
    const formatted = LeafPrinter.source(node);
    return this.decorateWithComments(node, formatted);
  }

  public print(query: ESQLAstQueryExpression) {
    return this.reducer.reduce(query, undefined);
  }

  public printCommand(command: ESQLAstCommand) {
    return this.reducer.reduce(command, undefined);
  }

  public printExpression(expression: ESQLAstExpressionNode) {
    return this.reducer.reduce(expression, undefined);
  }
}
