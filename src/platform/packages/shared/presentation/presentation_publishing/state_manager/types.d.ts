...
export type ComparatorFunction<StateType, KeyType extends keyof StateType> = (last: StateType[KeyType] | undefined, current: StateType[KeyType] | undefined, lastState?: Partial<StateType>, currentState?: Partial<StateType>) => boolean;
/**
...
export type StateComparators<StateType> = {
    [KeyType in keyof Required<StateType>]: 'referenceEquality' | 'deepEquality' | 'skip' | ComparatorFunction<StateType, KeyType>;
};
...
export type SubjectsOf<T extends object> = {
    [KeyType in keyof Required<T> as `${SnakeToCamelCase<string & KeyType>}$`]: PublishingSubject<T[KeyType]>;
};
export type SettersOf<T extends object> = {
    [KeyType in keyof Required<T> as `set${Capitalize<SnakeToCamelCase<string & KeyType>>}`]: (value: T[KeyType]) => void;
};
export interface StateManager<StateType extends object> {
    getLatestState: () => WithAllKeys<StateType>;
    reinitializeState: (newState?: Partial<StateType>, comparators?: StateComparators<StateType>) => void;
