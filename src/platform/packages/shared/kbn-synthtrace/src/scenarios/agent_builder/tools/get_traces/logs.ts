...
/**
 * SCENARIO: Logs for get_traces tool
 *
 ...
 * CLI Usage:
 * ```
 * node scripts/synthtrace src/platform/packages/shared/kbn-synthtrace/src/scenarios/agent_builder/tools/get_traces/logs.ts \
 ...
 */
function generateDefaultSequences(baseTimestamp: number): LogEvent[] {
  return [
...
/**
 * Creates a sequence of log events with shared fields.
...
const scenario: Scenario<LogDocument> = async () => ({
  ...
  generate: ({ range, clients: { logsEsClient } }) => generateLogsData({ range, logsEsClient }),
});
