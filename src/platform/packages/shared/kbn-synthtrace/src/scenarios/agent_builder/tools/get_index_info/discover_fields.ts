...
/**
 * SCENARIO: Discover Fields Test Data
 *
 * Story: Generates comprehensive observability data to test the discover_fields tool.
 ...
 * - Node: `k8s-node-01`
 *
 * This generates data with many ECS fields for the discover_fields tool to find.
 *
 * Run:
 * ```
 * node scripts/synthtrace \
 *   src/platform/packages/shared/kbn-synthtrace/src/scenarios/agent_builder/tools/discover_fields/discover_fields.ts \
 ...
 * {
 *   "tool_id": "observability.discover_fields",
 *   "tool_params": {}
 * }
 * ```
 */
 ...
 */
export function generateDiscoverFieldsData({
  range,
  infraEsClient,
  apmEsClient,
  hosts,
...
export default createCliScenario(({ range, clients: { infraEsClient, apmEsClient } }) => {
  ...
  return generateDiscoverFieldsData({ range, infraEsClient, apmEsClient, hosts });
});
