...
Control how many docs each channel emits per tick via the `volume` key (`ChannelVolume`). Each service can have a `ChannelEntry` with `rate`, `every` (emit every N-th tick), and `spikes` (burst/silence windows with a `multiplier`, optional `start`/`end` timestamps, and optional service scope).
...
- **Message content** — add strings to the pools in `log_mocks/`
...
- **New infra technologies** — add to `constants.ts`, `log_mocks/infra/`, and the service graph
...
Templates live in `log_mocks/`. Each sub-folder owns one domain:

| Folder | Domain |
|---|---|
| `log_mocks/service/` | Per-request success and error messages, outbound call pairs, stack traces |
| `log_mocks/infra/` | Per-technology infra messages (one file per dependency: `postgres.ts`, `redis.ts`, …) |
| `log_mocks/noise.ts` | Background noise messages keyed by health state and runtime |
...
2. Create `log_mocks/infra/<tech>.ts` with message pools keyed by condition.
