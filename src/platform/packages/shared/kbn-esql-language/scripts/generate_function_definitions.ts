...
const ELASTICSEARCH_PROMQL_DEFINITIONS_PATH =
  '/docs/reference/query-languages/promql/kibana/definitions';
...
function printPromQLGeneratedFile(definitions: PromQLESFunctionDefinition[]): string {
...
`
  ...
  const promqlDefinitionsDirectory = join(
    pathToElasticsearch,
    ELASTICSEARCH_PROMQL_DEFINITIONS_PATH
  );

  const promqlDefinitions: PromQLESFunctionDefinition[] = readdirSync(promqlDefinitionsDirectory)
    .filter((file) => file.endsWith('.json'))
    .map((file) => {
      const content = readFileSync(join(promqlDefinitionsDirectory, file), 'utf-8');
  ...
  await writeFile(
    ...
    printPromQLGeneratedFile(promqlDefinitions)
  );
})();
