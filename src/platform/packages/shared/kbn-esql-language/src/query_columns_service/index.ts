...
const COLUMN_CACHE_TTL_MS = 1000 * 60 * 5; // 5 minutes, matching cacheParametrizedAsyncFunction default
...
export class QueryColumns {
  private static readonly cache = new Map<string, ColumnCacheEntry>();
  ...
  static fromCache(query: string) {
    ...
    const entry = QueryColumns.cache.get(key);
    if (entry) {
      if (Date.now() - entry.timestamp > COLUMN_CACHE_TTL_MS) {
        QueryColumns.cache.delete(key);
        return undefined;
      }
      // Move it to the end to mark it as recently used
      QueryColumns.cache.delete(key);
      QueryColumns.cache.set(key, entry);
      return entry.value;
    }
    return undefined;
  }
  ...
  static setCache(key: string, value: ESQLColumnData[]) {
    ...
    QueryColumns.cache.set(normalizedKey, { value, timestamp: Date.now() });
  }
