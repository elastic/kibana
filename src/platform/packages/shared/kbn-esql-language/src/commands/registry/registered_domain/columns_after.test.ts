...
describe('REGISTERED_DOMAIN > columnsAfter', () => {
  ...
  it('adds 4 prefixed columns from REGISTERED_DOMAIN', () => {
    const result = columnsAfter(synth.cmd`REGISTERED_DOMAIN parts = host`, previousColumns);
  ...
  it('uses the target field as prefix', () => {
    const result = columnsAfter(synth.cmd`REGISTERED_DOMAIN my_prefix = host`, []);

    expect(result[0].name).toBe('my_prefix.domain');
    expect(result[1].name).toBe('my_prefix.registered_domain');
    expect(result[2].name).toBe('my_prefix.top_level_domain');
    expect(result[3].name).toBe('my_prefix.subdomain');
  });
});
