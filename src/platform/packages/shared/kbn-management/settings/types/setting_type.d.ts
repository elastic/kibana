 ...
 * @public
 */
export type StringSettingType = Extract<SettingType, 'color' | 'image' | 'json' | 'markdown' | 'select' | 'string'>;
/**
 * A narrowing type representing all {@link SettingType} values that correspond
 ...
 * @public
 */
export type KnownTypeToValue<T extends SettingType = SettingType> = T extends 'color' | 'image' | 'json' | 'markdown' | 'select' | 'string' ? string : T extends 'boolean' ? boolean : T extends 'number' | 'bigint' ? number : T extends 'array' ? Array<string | number> : T extends 'undefined' ? undefined : never;
