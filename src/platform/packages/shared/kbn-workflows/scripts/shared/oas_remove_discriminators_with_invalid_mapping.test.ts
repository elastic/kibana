...
describe('removeDiscriminatorsWithInvalidMapping', () => {
  it('should return document as-is when components.schemas is missing', () => {
    ...
    expect(removeDiscriminatorsWithInvalidMapping(doc)).toBe(doc);
  });
  ...
  it('should remove discriminator when mapping count does not match oneOf count', () => {
    ...
    removeDiscriminatorsWithInvalidMapping(doc);
  ...
  it('should preserve discriminator when mapping count matches oneOf count', () => {
    const doc = makeDocument({
      MySchema: {
        oneOf: [{ $ref: '#/components/schemas/A' }, { $ref: '#/components/schemas/B' }],
        discriminator: {
  ...
  it('should skip schemas with empty mapping (0 keys)', () => {
    const doc = makeDocument({
      MySchema: {
  ...
  it('should leave schemas without discriminator unchanged', () => {
    const doc = makeDocument({
