...
import { createPlainIndexExecutionsDal } from './create_plain_index_executions_dal';
...
describe('createPlainIndexExecutionsDal', () => {
  it('creates workflow and step DAL instances backed by their system indices', async () => {
    ...
    const { workflowExecutionsDal, stepExecutionsDal, initSetup, initStart } =
      createPlainIndexExecutionsDal({ esClient, logger });
    ...
    await expect(initSetup()).resolves.toBeUndefined();
    await expect(initStart()).resolves.toBeUndefined();
  });
});
