...
const workflow = parse(CREATE_CONVERSATION_PROPOSAL_YAML) as ParsedWorkflow;
...
describe('create-conversation-proposal workflow', () => {
  ...
  it('records a failure from the workflow-level on-failure, since HITL steps take none', () => {
    ...
    expect(fallback.map(({ type }) => type)).toContain('proposals.recordResult');
  });
  ...
  it('records the execution outcome around the action', () => {
    expect(findStep(workflow.steps, 'mark_executing')?.type).toBe('proposals.recordResult');
    expect(findStep(workflow.steps, 'record_success')?.type).toBe('proposals.recordResult');
  });
});
