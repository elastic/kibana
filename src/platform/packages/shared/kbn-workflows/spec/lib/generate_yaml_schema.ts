...
function createYamlSchemaWithoutProblematicConnectors() {
  ...
  const safeConnectors = GENERATED_KIBANA_CONNECTORS.filter((connector: any) => {
    ...
    return (
      !connectorString.includes('AlertSuppression') &&
  ...
  console.log(
    `Using ${safeConnectors.length} safe connectors out of ${GENERATED_KIBANA_CONNECTORS.length} total`
  );
...
function isSchemaValidForValidation(schema: any): boolean {
  try {
    ...
    const brokenRefs = schemaString.match(
      /"\$ref":"[^"]*\/allOf\/\d+\/allOf\/\d+\/properties\/[^"]*"/g
    );
        ...
        items: {
          ...
          properties: {
            ...
            type: {
              ...
              minLength: 1,
              description:
                'Step type (connector or built-in like console, http, if, foreach, parallel, merge, wait)',
            },
            with: {
              type: 'object',
