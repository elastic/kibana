...
function visitAbstractStep(graph: graphlib.Graph, previousStep: any, currentStep: any): any {
  ...
  if ((modifiedCurrentStep as ElasticsearchStep).type?.startsWith('elasticsearch.')) {
    return visitElasticsearchStep(graph, previousStep, modifiedCurrentStep);
  }

  if ((modifiedCurrentStep as KibanaStep).type?.startsWith('kibana.')) {
    return visitKibanaStep(graph, previousStep, modifiedCurrentStep);
  }
...
export function visitElasticsearchStep(
  ...
  if (previousStep) {
    graph.setEdge(getNodeId(previousStep), elasticsearchNode.id);
  }

  return elasticsearchNode;
}

export function visitKibanaStep(graph: graphlib.Graph, previousStep: any, currentStep: any): any {
  ...
  if (previousStep) {
    graph.setEdge(getNodeId(previousStep), kibanaNode.id);
  }
