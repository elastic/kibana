...
describe('Generated Kibana Connectors', () => {
    ...
    TEST_SAMPLES.forEach((connectorType) => {
      describe(`Connector: ${connectorType}`, () => {
        ...
        it('should have valid internal connector properties', () => {
          ...
          expect(connector.documentation === null || typeof connector.documentation === 'string').toBe(true);
        });
        ...
        it('should have query and header parameters as optional in schema', () => {
          ...
          for (const optionalParam of optionalParams) {
            ...
            if (paramSchema) {
              // These should be optional (except for required headers)
              const isOptional = paramSchema.isOptional();
        ...
        it('should have consistent parameter extraction from URL patterns', () => {
          ...
          for (const pattern of patterns) {
            ...
            const patternParams = (pattern.match(/\{([^}]+)\}/g) || [])
  ...
  describe('Nested Schema Issue Detection', () => {
    it('should detect the createCaseDefaultSpace nested schema issue', () => {
      const connector = GENERATED_KIBANA_CONNECTORS.find(c => c.type === 'kibana.createCaseDefaultSpace')!;
      expect(connector).toBeDefined();
      ...
      if (foundSuspiciousParams.length > 0) {
        console.warn(`⚠️ Detected potentially flattened nested schema in createCaseDefaultSpace: ${foundSuspiciousParams.join(', ')}`);
        console.warn('This suggests the schema explosion is not working correctly for nested objects');
      }
  ...
  it('should FAIL: detect missing alert_suppression schema dependencies', () => {
    ...
    const securityConnectors = GENERATED_KIBANA_CONNECTORS.filter(c => 
      c.type === 'kibana.PatchRule' || c.type === 'kibana.CreateRule' || c.type === 'kibana.UpdateRule'
    );
    ...
    securityConnectors.forEach(connector => {
      expect(() => {
        ...
        if ('_def' in schema && schema._def.typeName === 'ZodUnion') {
          // Check each option in the union
          schema._def.options.forEach((option: any, index: number) => {
            try {
              // Try to serialize each union option - this should expose missing dependencies
              JSON.stringify(option._def);
            } catch (error) {
              throw new Error(`Union option ${index} in ${connector.type} has serialization issues: ${error}`);
            }
          });
        }
  ...
  it('should fix Monaco JSON schema generation to restore YAML validation', () => {
    ...
    for (const refMatch of allRefs) {
      const refPath = refMatch.match(/"\$ref":"([^"]+)"/)[1];
    ...
    if (brokenRefs.length > 0) {
      ...
      const yourPattern = brokenRefs.find(ref => ref.includes('anyOf/665/properties/with/anyOf/6/allOf/1/allOf/1'));
      if (yourPattern) {
    ...
    console.log(`📊 WorkflowSchema has ${Object.keys(workflowDef.properties).length} top-level properties`);
    ...
    if (stepItems && stepItems.anyOf) {
      const kibanaConnector = stepItems.anyOf.find((item: any) => 
        item.properties && 
  ...
  it('should prove that additionalProperties: true was replaced with false', () => {
    ...
    it('should validate that complex POST endpoints have reasonable parameter counts', () => {
      const complexPostEndpoints = GENERATED_KIBANA_CONNECTORS.filter(c => 
        c.methods!.includes('POST') && c.parameterTypes!.bodyParams!.length > 10
      );
      ...
      for (const connector of complexPostEndpoints) {
        ...
        if (bodyParamCount > 20) {
          console.warn(`⚠️ Connector ${connector.type} has ${bodyParamCount} body parameters - possible over-flattening`);
        }
