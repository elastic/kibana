...
function createBenchmark(name: string, config: string) {
  return {
    ...
    name,
    beforeAll: 'node scripts/build_kibana_platform_plugins',
    run: `node scripts/functional_tests --config ${config} --kibana-install-dir ${process.env.KIBANA_BUILD_LOCATION}`,
    compare: {
      exists: 'lhs' as const,
