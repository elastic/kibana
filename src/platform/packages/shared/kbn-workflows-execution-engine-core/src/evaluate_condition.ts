...
import { safeEvaluateKql } from './safe_evaluate_kql';
...
export const evaluateCondition = (
  ...
  if (typeof renderedCondition === 'string') {
    const { matched, error } = safeEvaluateKql(renderedCondition, context);
    if (error) {
      if (error instanceof KQLSyntaxError) {
        throw new Error(`Syntax error in condition for step ${stepId}: ${String(error)}`);
      }
      throw error;
    }
    return matched;
  ...
  throw new Error(
    `Invalid condition type for step ${stepId}. ` +
      `Got type ${typeof renderedCondition}, ` +
