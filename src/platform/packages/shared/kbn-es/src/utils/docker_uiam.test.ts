...
describe(`#runUiamContainer()`, () => {
  ...
  test('fails if container never becomes healthy', async () => {
    ...
    await expect(
      ...
      `[Error: The "uiam-cosmosdb" container failed to start within 180 seconds. Last known status: running. Check the logs with docker logs -f uiam-cosmosdb]`
    );
    ...
    await expect(runUiamContainer(new ToolingLog(), uiamContainer)).rejects.toMatchInlineSnapshot(
      `[Error: The "uiam" container failed to start within 180 seconds. Last known status: running. Check the logs with docker logs -f uiam]`
    );
