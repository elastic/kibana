...
export class NativeRealm {
  ...
  async setPasswords(options: any) {
    ...
    await Promise.all([
      ...reservedUsers.map(async (user: string) => {
  ...
  // @ts-ignore
  async _autoRetry(
    opts: { attempt?: any; maxAttempts?: any },
    fn: { (): Promise<void>; (): Promise<string[]>; (): Promise<boolean>; (arg0: any): any }
  ) {
