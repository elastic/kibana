 ...
 */
export declare enum ProfilingESField {
    Timestamp = "@timestamp",
...
type DedotKey<TKey extends string | number | symbol, TValue> = TKey extends `${infer THead}.${infer TTail}` ? {
    [key in THead]: DedotKey<TTail, TValue>;
} : {
    [key in TKey]: TValue;
};
export type DedotObject<TObject extends Record<string, any>> = UnionToIntersection<ValuesType<{
    [TKey in keyof TObject]: DedotKey<TKey, TObject[TKey]>;
}>>;
export type FlattenObject<TObject extends Record<string, any>, TPrefix extends string = ''> = UnionToIntersection<ValuesType<{
    [TKey in keyof TObject & string]: TObject[TKey] extends Record<string, any> ? FlattenObject<TObject[TKey], `${TPrefix}${TKey}.`> : {
        [key in `${TPrefix}${TKey}`]: TObject[TKey];
    };
}>>;
type FlattenedKeysOf<TObject extends Record<string, any>> = keyof FlattenObject<TObject>;
export type PickFlattened<TObject extends Record<string, any>, TPickKey extends FlattenedKeysOf<TObject>> = DedotObject<Pick<FlattenObject<TObject>, TPickKey>>;
