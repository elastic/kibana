...
describe('createRegExpPatternFrom should create a regular expression starting from a string that', () => {
  ...
  it('tests positive for patterns with the data selector suffix', () => {
    expect('logs-*::data').toMatch(createRegExpPatternFrom('logs', ['data']));
    expect('logs-*::data').toMatch(createRegExpPatternFrom('logs', []));
    expect('logs-*').toMatch(createRegExpPatternFrom('logs', ['data']));
    expect('cluster1:logs-*::data,logs-*::data').toMatch(createRegExpPatternFrom('logs', ['data']));
  });

  it('tests negative for patterns with the failures selector suffix', () => {
    expect('logs-*::failures').not.toMatch(regExpPattern);
    expect('cluster1:logs-*::failures,').not.toMatch(regExpPattern);
    expect('cluster1:logs-*::failures,logs-*::failures').not.toMatch(regExpPattern);
  });

  it('tests negative for patterns with the wildcard selector suffix', () => {
    expect('logs-*::*').not.toMatch(regExpPattern);
    expect('cluster1:logs-*::*,logs-*::*').not.toMatch(regExpPattern);
  });
