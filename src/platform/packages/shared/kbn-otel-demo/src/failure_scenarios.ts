...
/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. "Elastic Source Code" is Conditions
 * are defined in the "Elastic License 2.0", the "GNU Affero General Public
 * License v3.0 only", and the "Server Side Public License v 1"; you may not
 * use this file except in compliance with, at your election, the Elastic
 * License 2.0, the GNU Affero General Public License v3.0 only, or the Server
...
/**
 * Failure scenarios that can be applied to the OTel Demo.
 ...
 */
export function getScenariosByCategory(category: 'dramatic' | 'subtle'): FailureScenario[] {
  return FAILURE_SCENARIOS.filter((s) => s.category === category);
}
...
export function listScenarioIds(): string[] {
  return FAILURE_SCENARIOS.map((s) => s.id);
}
