...
/**
 * Creates the KubePay shared ConfigMap
 */
function createKubepayConfigMap(namespace: string): object {
  return {
    apiVersion: 'v1',
    kind: 'ConfigMap',
    metadata: {
...
/**
 * Manifest generator for KubePay
 */
export const kubepayManifests: DemoManifestGenerator = {
  generate(options: ManifestOptions): string {
    ...
    // Add KubePay-specific ConfigMap
    manifests.push(createKubepayConfigMap(namespace));
    ...
    // Zipkin NodePort for tracing UI access
    manifests.push({
      apiVersion: 'v1',
      kind: 'Service',
      metadata: {
