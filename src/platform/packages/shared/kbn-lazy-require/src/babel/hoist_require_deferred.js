...
/**
 * Attempts to find a `requireDeferred` call from @kn/lazy-require and hoist both
 * the import and the call, to make sure it runs before any other modules.
 *
 * This is designed to be run in `Program.enter` to handle both ESM imports and
 * top-level `requireDeferred()` calls before other transforms run (such as ESM
 * > CJS).
 *
 * @param {babel.types} t
    ...
    ImportDeclaration(path) {
      ...
      for (const specifier of path.get('specifiers')) {
        if (
          ...
          importDeclarationNode.source = t.stringLiteral(`@kbn/lazy-require/require_deferred_auto`);
