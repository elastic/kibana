 ...
 */
const EuiFormDiv = (props: EuiFormProps) => <EuiForm component="div" {...props} />;
...
export interface RecurringScheduleFormProps {
  ...
  onRecurringScheduleChange: (newSchedule: RecurringSchedule) => void;
...
export const RecurringScheduleForm = memo(
  ({
    ...
    const [formData] = useFormData<RecurringSchedule>({
      form,
      onChange: onRecurringScheduleChange,
      watch: [
        'frequency',
        'interval',
        'ends',
    ...
    const parsedSchedule = useMemo(() => parseSchedule(formData), [formData]);

    return (
      <Form form={form} FormWrapper={EuiFormDiv}>
