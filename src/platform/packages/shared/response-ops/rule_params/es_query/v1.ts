...
const EsQueryRuleParamsSchemaProperties = {
  size: schema.number({ min: 0, max: ES_QUERY_MAX_HITS_PER_EXECUTION, meta: { description: 'The number of documents to pass to the configured actions when the threshold condition is met.'} }),
  ...
  excludeHitsFromPreviousRun: schema.boolean({ defaultValue: true,  meta: { description: 'Indicates whether to exclude matches from previous runs. If `true`, you can avoid alert duplication by excluding documents that have already been detected by the previous rule run. This option is not available when a grouping field is specified.'} }),
  ...
  threshold: schema.arrayOf(schema.number({meta: { description: 'The threshold value that is used with the `thresholdComparator`. If the `thresholdComparator` is `between` or `notBetween`, you must specify the boundary values.'}}), { minSize: 1, maxSize: 2 }),
  ...
  termField: schema.maybe(
    ...
    ],{ meta: { description: 'The names of up to four fields that are used for grouping the aggregation. This property is required when `groupBy` is `top`.'} })
  ),
  ...
  searchType: schema.oneOf(
    ...
    {
      meta: { description: 'The type of query For example: `esQuery` for Elasticsearch Query DSL or `esqlQuery` for Elasticsearch Query Language (ES|QL).'},
  ...
  searchConfiguration: schema.conditional(
    ...
    schema.never(),
    { meta: { description: 'The query definition, which uses KQL or Lucene to fetch the documents from Elasticsearch.'} }
  ),
