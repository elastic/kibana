...
describe('run_calibration.sh metadata', () => {
  ...
  it('documents campaign-only use and has no seed plumbing', () => {
    const script = readFileSync(RUN_CALIBRATION_SH, 'utf8');
    expect(script).toContain('Campaign-only');
    expect(script).toContain('Not the regular CI check entrypoint');
    expect(script).not.toContain('KIBANA_CI_WARM_START_MEMORY_SEED');
  ...
  it('prints post-GC mean and SD from a valid report', () => {
    const result = runReportValidation('aa', 0, 'observed', {
      pairCount: 4,
      meanBytes: 1.5 * 1024 * 1024,
      sampleStandardDeviationBytes: 0.75 * 1024 * 1024,
    });

    expect(result.status).toBe(0);
    expect(result.stderr).toContain('post-GC pairs=4 meanMiB=1.50 sdMiB=0.75');
  });
});
  ...
  writeFileSync(
    ...
    JSON.stringify(
      outcome === undefined
        ...
        : {
            outcome,
            ...(postForcedGcHeapUsed ? { postForcedGcHeapUsed } : {}),
          }
    )
  );
