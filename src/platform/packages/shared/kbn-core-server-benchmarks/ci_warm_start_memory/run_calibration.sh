#!/usr/bin/env bash
#
# Campaign-only calibration entrypoint for the warm-start memory benchmark.
# Download fixed kibana-on-merge distributables, verify checksums, run the
# paired warm-start memory benchmark, assert the orientation's expected
# outcome, and upload manifest/report artifacts.
#
# Not the regular CI check entrypoint. Invoke from the Flaky Test Runner
# (or locally with bk credentials) when calibrating SD / reproducibility
# after changes to the forced-GC signal or the paired decision rule.
#
# Orientation (left/right artifact pairing):
#   aa  fixed A vs fixed A (same-artifact control → outcome observed)
#   ab  fixed A vs regressed B (known regression → outcome regression)
#   ba  regressed B vs fixed A (reversed pair → outcome observed)
#
# Inputs:
#   --orientation aa|ab|ba
#     or KIBANA_CI_WARM_START_MEMORY_CALIBRATION_ORIENTATION
#
# Keep artifact metadata in sync with calibration_artifacts.ts. Pins expire
# with Buildkite artifact retention — refresh before the next campaign if
...
usage() {
  cat <<'EOF'
Usage: run_calibration.sh [--orientation aa|ab|ba] [--dry-run]

Campaign-only: runs Linux warm-start memory calibration using fixed
kibana-on-merge distributables. Writes manifest/report under
target/ci-warm-start-memory-bench/. Not part of regular CI.
EOF
}
...
validate_benchmark_result() {
  ...
  # whose outcome contradicts the orientation's expected result. Also print the
  # post-GC mean/SD so flaky-runner logs carry the calibration signal.
  report_outcome="$(
    node -e "
      const report = JSON.parse(require('fs').readFileSync(process.argv[1], 'utf8'));
      if (typeof report.outcome !== 'string') process.exit(2);
      const m = report.postForcedGcHeapUsed || {};
      const toMiB = (bytes) =>
        Number.isFinite(bytes) ? (bytes / 1048576).toFixed(2) : 'n/a';
      console.error(
        'post-GC pairs=' + (m.pairCount ?? 'n/a') +
        ' meanMiB=' + toMiB(m.meanBytes) +
        ' sdMiB=' + toMiB(m.sampleStandardDeviationBytes)
      );
...
run_benchmark() {
  echo "--- Warm-start calibration orientation=${ORIENTATION}"
