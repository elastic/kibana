import type { FormulaPublicApi, TermsIndexPatternColumn, TypedLensByValueInput } from '@kbn/lens-common';
...
export type ChartType = 'xy' | 'pie' | 'heatmap' | 'metric' | 'gauge' | 'donut' | 'mosaic' | 'regionmap' | 'table' | 'tagcloud' | 'treemap';
...
export type LensConfig = LensMetricConfig | LensGaugeConfig | LensPieConfig | LensHeatmapConfig | LensMosaicConfig | LensRegionMapConfig | LensTableConfig | LensTagCloudConfig | LensTreeMapConfig | LensXYConfig;
...
export type LensBreakdownConfig = string | Identity<(LensBreakdownTopValuesConfig | LensBreakdownIntervalsConfig | LensBreakdownFiltersConfig | LensBreakdownDateHistogramConfig) & {
    colorPalette?: string;
}>;
...
export type LensRegionMapConfig = Identity<LensBaseConfig & LensBaseLayer & LensRegionMapConfigBase>;
...
export type LensAnnotationLayer = Identity<LensBaseXYLayer & {
    type: 'annotation';
    events: Array<Identity<LensAnnotationLayerBaseProps & {
        datetime: string;
    }> | Identity<LensAnnotationLayerBaseProps & {
        field: string;
        filter: string;
    }>>;
}>;
export type LensSeriesLayer = Identity<LensBaseXYLayer & {
    type: 'series';
...
export type ChartTypeLensConfig<T extends ChartType> = T extends keyof ChartTypeLensMap ? ChartTypeLensMap[T] : never;
export {};
