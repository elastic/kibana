...
export const datasetTypeSchema = schema.oneOf(
  [
    ...
    schema.object(
      ...
      },
      {
        meta: {
          description:
            'DataView dataset - uses a Kibana data view as the data source for the metric visualization',
        },
      }
    ),
    // Index dataset type
    schema.object(
        ...
        runtime_fields: schema.maybe(
          schema.arrayOf(
            schema.object(
              ...
              {
                meta: {
                  description: 'Runtime field definition - computed field available at query time',
                },
              }
            )
          )
      ...
      },
      {
        meta: {
          description:
            'Index dataset - uses an Elasticsearch index as the data source with optional runtime fields',
        },
      }
    ),
  ],
  {
    meta: {
      description:
        'Dataset configuration - defines the data source for the metric visualization (DataView or Index)',
    },
  }
);
...
export const datasetEsqlTableTypeSchema = schema.oneOf(
  [
    ...
    schema.object(
      ...
      },
      {
        meta: {
          description:
            'ESQL dataset - uses an ESQL query as the data source for the metric visualization',
        },
      }
    ),
    // Table dataset type
    schema.object(
      ...
      },
      {
        meta: {
          description:
            'Table dataset - uses a Kibana datatable object as the data source for the metric visualization',
        },
      }
    ),
  ],
  {
    meta: {
      description:
        'ESQL/Table dataset configuration - defines advanced data sources for the metric visualization (ESQL query or Kibana datatable)',
    },
  }
);
