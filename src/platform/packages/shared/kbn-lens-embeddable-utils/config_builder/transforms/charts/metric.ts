...
import type { BuildDependencies, LensAttributes } from '../../types';
...
import { getValueColumn } from '../columns/value';
import type { MetricState, MetricStateESQL } from '../../schema';
...
function reverseBuildVisualizationState(
  ...
  if (dataset.type === 'esql' || dataset.type === 'table') {
    ...
    props = {
      metric: {
        column: esqlLayer.columns.find((c) => c.columnId == visualization.metricAccessor)!
          .fieldName,
        empty_as_null: false,
        fit: false,
      ...
      ...(visualization.secondaryMetricAccessor
        ? {
            secondary_metric: {
              column: esqlLayer.columns.find(
                (c) => c.columnId == visualization.secondaryMetricAccessor
              )!.fieldName,
            },
          }
        : {}),
      ...(visualization.maxAccessor
        ? {
            max_value: {
              column: esqlLayer.columns.find((c) => c.columnId == visualization.maxAccessor)!
                .fieldName,
            },
          }
        : {}),
      ...(visualization.breakdownByAccessor
        ? {
            breakdown_by: {
              column: esqlLayer.columns.find(
                (c) => c.columnId == visualization.breakdownByAccessor
              )!.fieldName,
            },
          }
        : {}),
...
function buildFormBasedLayer(
  ...
  if (layer.breakdown_by) {
    ...
    const breakdownColumn = fromBucketLensApiToLensState(layer.breakdown_by as LensApiBucketOperations, []);
  ...
  if (layer.secondary_metric) {
    ...
    const newColumn = fromMetricAPItoLensState(layer.secondary_metric as LensApiAllMetricOperations);
...
export function fromMetricLegacyToAPI(
  ...
  const visualizationState = reverseBuildVisualizationState(
    visualization,
    layer,
    dataView,
    formulaAPI
