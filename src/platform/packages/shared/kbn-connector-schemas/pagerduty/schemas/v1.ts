...
const EventActionSchema = lazySchema(() =>
  z.enum([EVENT_ACTION_TRIGGER, EVENT_ACTION_RESOLVE, EVENT_ACTION_ACKNOWLEDGE])
);

const PayloadSeveritySchema = lazySchema(() => z.enum(['critical', 'error', 'warning', 'info']));
const LinksSchema = lazySchema(() =>
  z.array(z.object({ href: z.string(), text: z.string() }).strict())
);
const customDetailsSchema = lazySchema(() => Coerced(z.record(z.string(), z.any())));
