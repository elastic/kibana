...
describe('DataViewSource', () => {
  it('throws when constructed with a DataView that has no id', () => {
    ...
    expect(() => new DataViewSource(dv)).toThrow(/requires a DataView with an id/);
  });
  ...
  it('builds a self-referential index-pattern reference', () => {
    const source = new DataViewSource(makeDataViewMock({ id: 'dv-7' }));
  ...
  describe('isTimeBased', () => {
    ...
    it('returns false when timeFieldName is undefined', () => {
      const source = new DataViewSource(makeDataViewMock({ timeFieldName: undefined }));
  ...
  describe('isPersisted', () => {
    it('delegates to the wrapped DataView', () => {
      const persisted = new DataViewSource(makeDataViewMock({ persisted: true }));
      const adhoc = new DataViewSource(makeDataViewMock({ persisted: false }));
  ...
  describe('serialize', () => {
    it('returns identity-only kind = "index-pattern" form', () => {
      const source = new DataViewSource(makeDataViewMock({ id: 'dv-9' }));
