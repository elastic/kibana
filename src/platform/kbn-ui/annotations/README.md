...
- **Trail**: the disclosure controls the author clicked on the page before commenting (links, buttons that are not form submits, tabs, menu items…; anchor + label each), recorded passively while comment mode is off. Form and selection controls are left out: repeating those changes data rather than revealing UI.
