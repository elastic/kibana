...
export interface AnnotationsNotice {
  type: 'success' | 'error';
  message: string;
}
...
export interface AnnotationsController {
  store: Store<AnnotationsState>;
  ...
  guideTo(annotation: Annotation): Promise<void>;
...
const droppingDraft = (state: AnnotationsState): Partial<AnnotationsState> =>
  state.pending?.saving ? {} : { pending: null };

export const createAnnotationsController = (
  services: AnnotationsHostServices,
  { initialActive = false }: AnnotationsControllerOptions = {}
  ...
  const notify = (type: AnnotationsNotice['type'], message: string) => {
  ...
  const load = async (): Promise<void> => {
    ...
    try {
      const annotations = await api.list();
      ...
      if (started && sequence === loadSequence) {
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.loadFailed', {
  ...
  const loadAuthor = async () => {
    const user = await services
      .getCurrentUser()
      .catch((): AnnotationsUser => ({ username: 'anonymous' }));
    if (!started) {
      return;
    }
  ...
  const signAs = (displayName: string): AnnotationAuthor => {
    const author = {
  ...
  const onLocationChange = () => {
    ...
    const { pageKey: previous, guideId, annotations } = store.getState();
    if (pageKey === previous) {
      return;
    }
    // A guide survives the navigation it asked for (to the comment's page), nothing else.
    const guided = annotations.find(({ id }) => id === guideId);
    store.setState({
      pageKey,
      pending: null,
      activeThreadId: null,
  ...
  return {
    ...
    async save(text, { attachScreenshot, displayName }) {
      ...
      const updateDraft = (changes: Partial<PendingAnnotation>) =>
        store.setState((state) =>
      ...
      try {
        ...
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.saveFailed', {
    ...
    async reply(id, text, displayName) {
      const sent = await mutate(
        ...
        (message) =>
          i18n.translate('kbnUI.annotations.notice.replyFailed', {
    ...
    async setResolved(id, resolved) {
      await mutate(
        ...
        (message) =>
          i18n.translate('kbnUI.annotations.notice.resolveFailed', {
    ...
    async guideTo(annotation) {
      const { id, route } = annotation;
      store.setState((state) => ({
        guideId: id,
        activeThreadId: null,
        focusPinId: null,
      ...
      if (!isSafeRelativePath(route.path)) {
        ...
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.unsafePath', {
      ...
      try {
        ...
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.navigateFailed', {
    ...
    async exportAll() {
      try {
        ...
        downloadJson(`annotations-${payload.exportedAt.slice(0, 10)}.json`, payload);
      } catch (error) {
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.exportFailed', {
    ...
    async importFile(file) {
      try {
        ...
        notify(
          ...
          i18n.translate('kbnUI.annotations.notice.imported', {
            defaultMessage:
        ...
        notify(
          'error',
          i18n.translate('kbnUI.annotations.notice.importFailed', {
