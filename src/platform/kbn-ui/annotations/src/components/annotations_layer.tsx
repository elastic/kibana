...
export const AnnotationsLayer = () => {
  ...
  const notice = useAnnotationsState((state) => state.notice);
  const guided = useAnnotationsState((state) =>
    state.guideId ? state.annotations.find(({ id }) => id === state.guideId) ?? null : null
  );
  ...
  return (
    <ResolvedAnchorsProvider>
      ...
      {guided && <GuideOverlay annotation={guided} />}
