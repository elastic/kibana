...
describe('AnnotationsLayer', () => {
  ...
  const renderLayer = async () => {
    const controller = createAnnotationsController({
      api: createInMemoryAnnotationsApi([seeded]),
  ...
  it('returns focus to where it was when comment mode ends', async () => {
    ...
    await screen.findByTestId('kbnUiAnnotationsPanel');
    escape();
  ...
  it('gives the pin focus when its thread is opened from the panel', async () => {
    const controller = await renderLayer();
    act(() => controller.setActive(true));
    const row = await screen.findByTestId('kbnUiAnnotationsPanelItem-a');

    fireEvent.click(row.querySelector('.euiPanel') ?? row);

    await waitFor(() =>
      expect(document.activeElement).toBe(screen.getByTestId('kbnUiAnnotationsPin-a'))
    );
  ...
  it('closes the panel menu with Escape without leaving comment mode', async () => {
    const controller = await renderLayer();
    act(() => controller.setActive(true));
    fireEvent.click(await screen.findByTestId('kbnUiAnnotationsPanelMenu'));
    await screen.findByTestId('kbnUiAnnotationsExport');

    escape();

    await waitFor(() => expect(screen.queryByTestId('kbnUiAnnotationsExport')).toBeNull());
  ...
  it('keeps a draft that is being saved when Escape is pressed, and discards one that is not', async () => {
    ...
    await screen.findByTestId('kbnUiAnnotationsComposer');
