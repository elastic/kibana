...
```tsx
import { Navigation } from '@kbn/ui-one-navigation';
...
```bash
yarn test:jest src/platform/kbn-ui/one-navigation              # Run all tests
yarn test:jest src/platform/kbn-ui/one-navigation --watch      # Run in watch mode
yarn test:jest src/platform/kbn-ui/one-navigation --coverage   # Generate coverage report
```
