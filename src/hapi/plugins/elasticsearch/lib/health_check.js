...
module.exports = function (plugin, server) {
  ...
  plugin.setStatus('yellow', 'Waiting for Elasticsearch');
  ...
  function waitForPong() {
    return client.ping({ requestTimeout: 1500 }).catch(function (err) {
      ...
      plugin.setStatus('red', format('Unable to connect to Elasticsearch at %s. Retrying in 2.5 seconds.', config.elasticsearch));
  ...
  function waitForShards() {
    ...
    .then(function (resp) {
      ...
      if (resp.timed_out) {
        plugin.setStatus('red', 'No existing Kibana index found');
        return;
      }
      ...
      if (resp.status === 'red') {
        plugin.setStatus('red', 'Elasticsearch is still initializing the kibana index... Trying again in 2.5 second.');
        return Promise.delay(2500).then(waitForShards);
      }

      // otherwise we are g2g
      plugin.setStatus('green', 'Kibana index ready');
    });
  }
