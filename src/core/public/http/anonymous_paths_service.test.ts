...
describe('#setup()', () => {
  describe('#register', () => {
    it(`allows paths that don't start with /`, () => {
      const anonymousPaths = new AnonymousPathsService();
      const setup = anonymousPaths.setup();
      setup.register('bar');
    });
  ...
  describe('#isAnonymous', () => {
    it('returns true for registered paths', () => {
      ...
      const setup = anonymousPaths.setup();
      setup.register('/bar');
      const start = anonymousPaths.start({ basePath });
      expect(start.isAnonymous('/foo/bar')).toBe(true);
    });
    ...
    it('when there is no basePath and calling "isAnonymous" without a starting slash, returns true for paths registered with a starting slash', () => {
      ...
      setup.register('/bar');
      const start = anonymousPaths.start({ basePath });
      expect(start.isAnonymous('bar')).toBe(true);
    });
    ...
    it('returns false for other paths', () => {
      ...
      setup.register('/bar');
      const start = anonymousPaths.start({ basePath });
      expect(start.isAnonymous('/foo/foo')).toBe(false);
    });
    ...
    it('returns false for sub-paths of registered paths', () => {
      ...
      setup.register('/bar');
      const start = anonymousPaths.start({ basePath });
      expect(start.isAnonymous('/foo/bar/baz')).toBe(false);
    });
  });
});
