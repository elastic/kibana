...
describe('#setup()', () => {
  ...
  beforeEach(() => {
    ...
    startDeps = { injectedMetadata: setupDeps.injectedMetadata };
  ...
  describe('register', () => {
    ...
    it('throws an error if an App with the same appBasePath is registered', () => {
      const { register, registerLegacyApp } = service.setup(setupDeps);

      register(Symbol(), { id: 'app1', mount } as any);

      expect(() =>
        register(Symbol(), { id: 'app2', mount, appBasePath: '/app/app1' } as any)
      ).toThrowErrorMatchingInlineSnapshot(
        `"An application is already registered with the appBasePath \\"/app/app1\\""`
      );
  ...
  describe('registerLegacyApp', () => {
    ...
    it('throws an error if a LegacyApp with the same appBasePath is registered', () => {
      const { register, registerLegacyApp } = service.setup(setupDeps);
...
describe('#start()', () => {
  ...
  describe('getUrlForApp', () => {
    ...
    it('creates URL for registered appId', async () => {
      ...
      register(Symbol(), { id: 'app1', mount } as any);
      registerLegacyApp({ id: 'legacyApp1' } as any);
      register(Symbol(), { id: 'app2', mount, appBasePath: '/custom/path' } as any);
  ...
  describe('navigateToApp', () => {
    ...
    it('changes the browser history for custom appBasePaths', async () => {
      const { register } = service.setup(setupDeps);
