...
import { CoreStatus, ServiceStatus, ServiceStatusLevel, ServiceStatusLevels } from '../../types';
...
describe('GET /api/status', () => {
  ...
  const setupServer = async ({
    allowAnonymous = true,
    coreStatus,
  }: { allowAnonymous?: boolean; coreStatus?: CoreStatus } = {}) => {
    ...
    if (coreStatus) {
      status.core$ = new BehaviorSubject(coreStatus);
    }
  ...
  describe('status level and http response code', () => {
    it('respond with a 200 when core.overall.status is available', async () => {
      await setupServer({
        coreStatus: {
          elasticsearch: createServiceStatus(),
          savedObjects: createServiceStatus(),
          overall: createServiceStatus(ServiceStatusLevels.available),
        },
      });
    ...
    it('respond with a 200 when core.overall.status is degraded', async () => {
      await setupServer({
        coreStatus: {
          elasticsearch: createServiceStatus(),
          savedObjects: createServiceStatus(),
          overall: createServiceStatus(ServiceStatusLevels.degraded),
        },
      });
      await supertest(httpSetup.server.listener).get('/api/status?v8format=true').expect(200);
    });
    it('respond with a 503 when core.overall.status is unavailable', async () => {
      await setupServer({
        coreStatus: {
          elasticsearch: createServiceStatus(),
          savedObjects: createServiceStatus(),
          overall: createServiceStatus(ServiceStatusLevels.unavailable),
        },
      });
