...
describe('BasePathProxyServer', () => {
  ...
  test('exposes route details of incoming request to a route handler', async () => {
    ...
    await server.start();
    await proxySupertest.get(`${basePath}/`).expect(200, {
      path: `${basePath}/`,
      method: 'get',
      options: {
        authRequired: true,
  ...
  describe('body options', () => {
    test('should reject the request because the Content-Type in the request is not valid', async () => {
      ...
      await server.start();
      await proxySupertest.post(`${basePath}/`).send({ test: 1 }).expect(415, {
        statusCode: 415,
    ...
    test('should reject the request because the payload is too large', async () => {
      ...
      await server.start();
      await proxySupertest.post(`${basePath}/`).send({ test: 1 }).expect(413, {
        statusCode: 413,
    ...
    test('should not parse the content in the request', async () => {
      ...
      await server.start();
      await proxySupertest.post(`${basePath}/`).send({ test: 1 }).expect(200, {
        parse: false,
        maxBytes: 1024, // hapi populates the default
        output: 'data',
      });
