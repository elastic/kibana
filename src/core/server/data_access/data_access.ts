...
export interface DataAccessCreateOptions {
  id?: string;
  overwrite?: boolean;
}
...
export class DataAccess {
   ...
   * @param {DataAccessCreateOptions} [options={}]
  ...
  async create(
    type: string,
    attributes = {},
    options: DataAccessCreateOptions = { overwrite: false }
  ) {
   ...
   */
  async bulkCreate(
    objects: Array<DataAccessObject>,
    options: DataAccessCreateOptions = { overwrite: false }
  ) {
    const { overwrite } = options;
    const time = this.getCurrentTime();
    const objectToBulkRequest = (object: DataAccessObject) => {
