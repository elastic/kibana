/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0 and the Server Side Public License, v 1; you may not use this file except
 * in compliance with, at your election, the Elastic License 2.0 or the Server
 * Side Public License, v 1.
 */

import { BootstrapTemplateInterpolator } from './render_template';

function mockParams() {
  return {
    themeTag: 'v7',
    jsDependencyPaths: ['/js-1', '/js-2'],
    styleSheetPaths: ['/style-1', '/style-2'],
    publicPathMap: '{"foo": "bar"}',
  };
}

describe('BootstrapTemplateInterpolator', () => {
  let interpolator: BootstrapTemplateInterpolator;

  beforeEach(() => {
    interpolator = new BootstrapTemplateInterpolator();
  });

  afterAll(() => {
    jest.restoreAllMocks();
  });

  describe('#interpolate()', () => {
    test('resolves to a string', async () => {
      const content = await interpolator.interpolate(mockParams());
      expect(typeof content).toEqual('string');
    });

    test('interpolates templateData into string template', async () => {
      const content = await interpolator.interpolate(mockParams());

      expect(content).toMatchSnapshot();
    });
  });
});
