...
| http.request.id | (optional) Opaque request id used to join with proxy / error logs | string |
...
To emit the **status Kibana actually sent** at the default `info` level for a specific route,
set `options.httpResponseLogLevel: 'info'` when registering the route. Core then writes a slim
record (`http.response.status_code`, `url.path`, `http.request.id`) without query strings,
headers, or client IP. The full access-log record above stays at `debug`.

Use this only for routes that on-call must query during incidents. Do not opt in high-volume
or user-input-heavy paths.

```ts
router.get(
  {
    path: '/internal/api/my_plugin/status',
    validate: false,
...
router.versioned
  ...
  .addVersion({ version: '1', validate: false }, handler);
```
