...
(including the one breaching the threshold) over the limit.

Note that only the size from the rolled files is taken into account (the active file is ignored).
