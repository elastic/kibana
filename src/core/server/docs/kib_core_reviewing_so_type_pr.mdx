...
PRs that modify Saved Object type registrations are validated by the **Check changes in Saved Objects** CI step (`node scripts/check_saved_objects`). The check compares the current type registry against a GCS baseline snapshot and posts actionable feedback on the pull request when Saved Object types are added, updated, or removed.

When the check detects migration-related changes, it fails CI and summarizes which types were affected, including reminders about two-step releases when applicable.
...
The **Check changes in Saved Objects** CI step (`node scripts/check_saved_objects`) detects this scenario.
...
The **Check changes in Saved Objects** CI step (`node scripts/check_saved_objects`) also detects those scenarios.
...
- Maintains a list of all removed types in `src/core/packages/saved-objects/server-internal/removed_types.json`
