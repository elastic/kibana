...
import type { CloneIndexParams } from '@kbn/core-saved-objects-server/src/migration/actions';
...
jest.mock('@kbn/core-saved-objects-server/src/migration/actions/clone_index', () => {
  const { setTimeout: actualTimer } = jest.requireActual('timers/promises');
  const realModule = jest.requireActual(
    '@kbn/core-saved-objects-server/src/migration/actions/clone_index'
  );
