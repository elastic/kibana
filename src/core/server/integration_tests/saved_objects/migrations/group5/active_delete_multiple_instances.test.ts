...
describe('multiple migrator instances running in parallel', () => {
  describe('when upgrading to a new stack version with matching mappings', () => {
    ...
    beforeAll(async () => {
      esServer = await startElasticsearch({ dataArchive: BASELINE_TEST_ARCHIVE_400K });
    ...
    it('will actively delete and sccessfully complete migration', async () => {
      ...
      expect(beforeCleanup.server).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K);
      expect(beforeCleanup.basic).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K);
      expect(beforeCleanup.deprecated).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K);
      expect(beforeCleanup.complex).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K);
      ...
      expect(afterCleanup.basic).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K); // we keep 'basic' SOs
      expect(afterCleanup.deprecated).not.toBeDefined(); // 'deprecated' is no longer present in nextMinor's mappings
      expect(afterCleanup.complex).toEqual(BASELINE_DOCUMENTS_PER_TYPE_400K / 2); // we excludeFromUpgrade half of them with a hook
    });
