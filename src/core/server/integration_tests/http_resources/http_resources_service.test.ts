...
function applyTestsWithDisableUnsafeEvalSetTo(disableUnsafeEval: boolean) {
  describe(`with disableUnsafeEval=${disableUnsafeEval}`, () => {
    ...
    const defaultCspRules = disableUnsafeEval
      ...
      : `script-src 'report-sample' 'self' 'unsafe-eval'; worker-src 'report-sample' 'self' blob:; style-src 'report-sample' 'self' 'unsafe-inline'; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'`;
    beforeEach(async () => {
      root = createRoot({
