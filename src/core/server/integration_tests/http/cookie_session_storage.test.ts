...
describe('Cookie based SessionStorage', () => {
  ...
  describe('#set()', () => {
    it('Should write to session storage & set cookies', async () => {
      ...
      const factory = createCookieSessionStorageFactory(logger.get(), innerServer, cookieOptions);
      await server.start();
  ...
  describe('#get()', () => {
    ...
    it('returns null for empty session', async () => {
      ...
      const response = await supertest(innerServer.server).get('/').expect(200, { value: null });
    ...
    it('returns null for invalid session (expired) & clean cookies', async () => {
      ...
      const sessionCookie = retrieveSessionCookie(cookies[0]);
      const response2 = await supertest(innerServer.server)
        .get('/')
        .set('Cookie', `${sessionCookie.key}=${sessionCookie.value}`)
    ...
    it('returns null if multiple session cookies are detected.', async () => {
      ...
      const factory = createCookieSessionStorageFactory(
        logger.get(),
        mockServer as any,
        cookieOptions
      );
    ...
    describe('#SameSite', () => {
      ...
      for (const sameSite of ['Strict', 'Lax', 'None'] as const) {
        it(`sets and parses SameSite = ${sameSite} correctly`, async () => {
          ...
          const factory = createCookieSessionStorageFactory(logger.get(), innerServer, {
            ...cookieOptions,
            isSecure: true,
            name: `sid-${sameSite}`,
            sameSite,
