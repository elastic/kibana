...
import {
  SavedObjectsBulkOperationObject,
...
export class SavedObjectsRepository {
  ...
  async bulkCreate<T extends SavedObjectAttributes = any>(
    ...
    return await this.performBulkOperation<T, SavedObjectsBulkCreateObject<T>>(
      objects,
      onInvalidType,
      asBulkOperation,
      processResponse
  ...
  private async performBulkOperation<
    H extends SavedObjectAttributes,
    T extends SavedObjectsBulkOperationObject<H>
  >(
    objects: T[],
    onInvalidType: (object: T) => any,
    asBulkOperation: (object: T) => [BulkOperation, RawSavedObjectDoc],
    processResponse: (
      requestedId: string,
      objectToSave: RawSavedObjectDoc,
      responseObject: SavedObjectDoc
    ) => SavedObject<H>
  ): Promise<SavedObjectsBulkResponse<H>> {
