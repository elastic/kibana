...
describe('#validateRetries', () => {
  ...
  describe('module calls', () => {
    ...
    test('non-empty retries', () => {
      ...
      const retry3 = createRetry({ type: 'foo', id: '3', idToOverwrite: 'a' }); // this is not a valid retry but we test it for posterity
      const retry4 = createRetry({ type: 'foo', id: '4', overwrite: true, idToOverwrite: 'b' });
      const retry5 = createRetry({ type: 'foo', id: '5', overwrite: true, idToOverwrite: 'c' });
      const retries = [retry1, retry2, retry3, retry4, retry5];
      validateRetries(retries);
      expect(getNonUniqueEntries).toHaveBeenCalledTimes(2);
      // check all retry objects for non-unique entries
      expect(getNonUniqueEntries).toHaveBeenNthCalledWith(1, retries);
      // check only retry overwrites with `overwrite` === true and `idToOverwrite` !== undefined for non-unique entries
      const retryOverwriteEntries = [
        { type: retry4.type, id: retry4.idToOverwrite },
        { type: retry5.type, id: retry5.idToOverwrite },
      ];
  ...
  describe('results', () => {
    ...
    test('throws Boom error if any retry overwrites are not unique', () => {
    ...
    test('does not throw error if retry objects and retry overwrites are unique', () => {
