 ...
 *  1. The import ID map that was generated by the `checkOriginConflicts` function should dictate the IDs of the objects we create.
...
export const createSavedObjects = async <T>(
  ...
  // use the import ID map from the `checkOriginConflicts` or `getImportIdMapForRetries` function to ensure that each object is being
  // created with the correct ID also, ensure that the `originId` is set on the created object if it did not have one
