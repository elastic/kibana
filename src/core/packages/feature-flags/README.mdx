...
There are 3 typed methods. Each is available as an RxJS observable (`get*Value$`) and, in the browser, as a React hook (`use*Value`). Both follow the current value, and update when the flag or evaluation context changes.

|  Type   | Observable                                               | React                                                |
|:-------:|:---------------------------------------------------------|:-----------------------------------------------------|
| Boolean | `core.featureFlags.getBooleanValue$(flagName, fallback)` | `core.featureFlags.useBooleanValue(flagName, fallback)` |
| String  | `core.featureFlags.getStringValue$(flagName, fallback)`  | `core.featureFlags.useStringValue(flagName, fallback)`  |
| Number  | `core.featureFlags.getNumberValue$(flagName, fallback)`  | `core.featureFlags.useNumberValue(flagName, fallback)`  |

In React, call the `use*Value` hook during render. The first render uses the synchronous evaluation, then the hook updates when the flag or the evaluation context changes:

```tsx
const enabled = core.featureFlags.useBooleanValue('myPlugin.exampleBoolean', false);
const variant = core.featureFlags.useStringValue('myPlugin.exampleString', 'default');
const limit = core.featureFlags.useNumberValue('myPlugin.exampleNumber', 1);
```

On the server (and in non-React browser code), subscribe directly:

```ts
core.featureFlags.getBooleanValue$('myPlugin.exampleBoolean', false).subscribe((enabled) => {
  // adapt to the current value
});
```

### Request handler context

In HTTP handlers, evaluate flags from the core request context:
...
> [!NOTE]
> The `PUT kbn:/internal/core/_settings` endpoint is only available in the test environments. To enable it, add the following
> to your `kibana.yml`:
>
> ```yaml
