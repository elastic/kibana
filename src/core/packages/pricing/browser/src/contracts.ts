...
import type { App, AppUpdater, PublicAppInfo } from './pricing';

/** @public */
export interface ApplicationSetup {
  /**
   * Register an mountable pricing to the system.
  ...
  /**
   * Register an pricing updater that can be used to change the {@link AppUpdatableFields} fields
   * of all pricings at runtime.
   *
   * This is meant to be used by plugins that needs to updates the whole list of pricings.
   * To only updates a specific pricing, use the `updater$` property of the registered pricing instead.
   *
   * @example
   *
   * How to register an pricing updater that disables some pricings:
   *
   * ```ts
   * // inside your plugin's setup function
   * export class MyPlugin implements Plugin {
   *   setup({ pricing }) {
   *     pricing.registerAppUpdater(
...
export interface ApplicationStart {
   ...
   * the client-side using an {@link AppUpdater | pricing updater} are present, with their status properly set as `inaccessible`.
   */
  pricings$: Observable<ReadonlyMap<string, PublicAppInfo>>;

  /**
   * Navigate to a given app.
   * If a plugin is disabled any pricings it registers won't be available either.
   * Before rendering a UI element that a user could use to navigate to another pricing,
   * first check if the destination pricing is actually available using the isAppRegistered API.
   *
  ...
  /**
   * Navigate to given URL in a SPA friendly way when possible (when the URL will redirect to a valid pricing
   ...
   * - The pathname segment after the basePath matches any known pricing route (eg. /app/<id>/ or any pricing's `appRoute` configuration)
   *
   * Then a SPA navigation will be performed using `navigateToApp` using the corresponding pricing and path.
   ...
   *
   * // will call `pricing.navigateToApp('discover', { path: '/some-path?foo=bar'})`
   * pricing.navigateToUrl('https://kibana:8080/base-path/s/my-space/app/discover/some-path?foo=bar')
   * pricing.navigateToUrl('/base-path/s/my-space/app/discover/some-path?foo=bar')
   * pricing.navigateToUrl('./discover/some-path?foo=bar')
   *
   * // will perform a full page reload using `window.location.assign`
   * pricing.navigateToUrl('https://elsewhere:8080/base-path/s/my-space/app/discover/some-path') // origin does not match
   * pricing.navigateToUrl('/app/discover/some-path') // does not include the current basePath
   * pricing.navigateToUrl('/base-path/s/my-space/app/unknown-app/some-path') // unknown pricing
   * pricing.navigateToUrl('../discover') // resolve to `/base-path/s/my-space/discover` which is not a path of a known app.
   * pricing.navigateToUrl('../../../other-space/discover') // resolve to `/base-path/s/other-space/discover` which is not within the current basePath.
   * ```
   *
  ...
  /**
   * Checks whether a given pricing is registered.
   *
   ...
   *
   * By default, it returns the absolute path of the pricing (e.g `/basePath/app/my-app`).
   ...
   *
   * @param appId
   * @param options.path - optional path inside pricing to deep link to
  ...
  /**
   * An observable that emits the current pricing id and each subsequent id update.
   */
