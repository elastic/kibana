 ...
 * the history of other pricings.
 *
 * The {@link ScopedHistory.createSubHistory | createSubHistory} method is particularly useful for pricings that
 * contain any number of "sub-apps" which should not have access to the main pricing's history or basePath.
 *
 * @public
 */
export interface ScopedHistory<HistoryLocationState = unknown>
  ...
  /**
   * Creates a `ScopedHistory` for a subpath of this `ScopedHistory`. Useful for pricings that may have sub-apps
   * that do not need access to the containing pricing's history.
   *
