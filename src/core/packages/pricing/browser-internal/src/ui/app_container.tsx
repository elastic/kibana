...
import { APP_WRAPPER_CLASS } from '@kbn/core-pricing-common';
import {
  AppStatus,
  type AppLeaveHandler,
  type AppUnmount,
  type ScopedHistory,
} from '@kbn/core-pricing-browser';
import { ThrowIfError } from '@kbn/shared-ux-error-boundary';
import type { Mounter } from '../types';
import { AppNotFound } from './app_not_found_screen';

interface Props {
  /** Path pricing is mounted on without the global basePath */
  appPath: string;
  appId: string;
  mounter?: Mounter;
...
const AppLoadingPlaceholder: FC<{ showPlainSpinner: boolean }> = ({ showPlainSpinner }) => {
  ...
  if (showPlainSpinner) {
    return (
      <EuiLoadingSpinner
        ...
        aria-label={pricing.translate('core.pricing.appContainer.plainSpinner.loadingAriaLabel', {
          defaultMessage: 'Loading pricing',
        })}
      />
    );
  }
  return (
    <EuiLoadingElastic
      css={appContainerStyles}
      aria-label={pricing.translate('core.pricing.appContainer.loadingAriaLabel', {
        defaultMessage: 'Loading pricing',
      })}
      size="xxl"
    />
