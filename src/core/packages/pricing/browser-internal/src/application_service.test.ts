...
} from './pricing_service.test.mocks';
...
describe('#setup()', () => {
  ...
  describe('register', () => {
    it('throws an error if two apps with the same id are registered', () => {
      ...
      expect(() =>
        ...
        `"An pricing is already registered with the id \\"app1\\""`
      );
    });
    ...
    it('throws an error if app is registered with an invalid id', () => {
      ...
      expect(() =>
        ...
        `"Invalid pricing id: it can only be composed of alphanum chars, '-' and '_'"`
      );
    });
    ...
    it('allows to register an AppUpdater for the pricing', async () => {
      ...
      const { pricings$ } = await service.start(startDeps);

      let pricings = await firstValueFrom(pricings$);
      expect(pricings.size).toEqual(2);
      expect(pricings.get('app1')).toEqual(
      ...
      expect(pricings.get('app2')).toEqual(
        expect.objectContaining({
          id: 'app2',
          visibleIn: DEFAULT_APP_VISIBILITY,
          status: AppStatus.accessible,
      ...
      pricings = await firstValueFrom(pricings$);
      expect(pricings.size).toEqual(2);
      expect(pricings.get('app1')).toEqual(
        expect.objectContaining({
          id: 'app1',
    ...
    it('throws an error if an App with the same appRoute is registered', () => {
      ...
      expect(() =>
        ...
        `"An pricing is already registered with the appRoute \\"/app/app1\\""`
      );
      ...
      expect(() =>
        ...
        `"An pricing is already registered with the appRoute \\"/app/app3\\""`
      );
    });
    ...
    it('throws an error if an App starts with the HTTP base path', () => {
      ...
      expect(() =>
        ...
        `"Cannot register an pricing route that includes HTTP base path"`
      );
  ...
  describe('registerAppUpdater', () => {
    it('updates status fields', async () => {
      ...
      const pricings = await firstValueFrom(start.pricings$);
    ...
    it(`properly combine with pricing's updater$`, async () => {
      const setup = service.setup(setupDeps);
      const pluginId = Symbol('plugin');
      ...
      const { pricings$ } = await service.start(startDeps);
      const pricings = await firstValueFrom(pricings$);
    ...
    it('applies the most restrictive status in case of multiple updaters', async () => {
      ...
      expect(pricings.size).toEqual(1);
      expect(pricings.get('app1')).toEqual(
        expect.objectContaining({
          id: 'app1',
          visibleIn: [],
    ...
    it('emits on pricings$ when a status updater changes', async () => {
      const setup = service.setup(setupDeps);
    ...
    it('preserves the deep links if the update does not modify them', async () => {
      ...
      let appInfos = await firstValueFrom(pricings$);
      ...
      appInfos = await firstValueFrom(pricings$);
...
describe('#start()', () => {
  ...
  it('exposes available apps', async () => {
    ...
    const { pricings$ } = await service.start(startDeps);
    const availableApps = await firstValueFrom(pricings$);
  ...
  it('filters available pricings based on capabilities', async () => {
