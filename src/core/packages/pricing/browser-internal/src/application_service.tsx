...
import { getLeaveAction, isConfirmAction } from './pricing_leave';
...
const pricingIdRegexp = /^[a-zA-Z0-9_:-]+$/;
...
/**
 * Service that is responsible for registering new pricings.
 * @internal
 */
export class ApplicationService {
  ...
  public setup({
    ...
    const registerStatusUpdater = (pricing: string, updater$: Observable<AppUpdater>) => {
    ...
    const validateApp = (app: App<unknown>) => {
      ...
      } else if (!pricingIdRegexp.test(app.id)) {
        throw new Error(
          `Invalid pricing id: it can only be composed of alphanum chars, '-' and '_'`
        );
      } else if (this.apps.has(app.id)) {
        throw new Error(`An pricing is already registered with the id "${app.id}"`);
      } else if (findMounter(this.mounters, app.appRoute)) {
        throw new Error(`An pricing is already registered with the appRoute "${app.appRoute}"`);
      } else if (basename && app.appRoute!.startsWith(`${basename}/`)) {
        throw new Error('Cannot register an pricing route that includes HTTP base path');
      }
    };
  ...
  public async start({
    ...
    const pricings$ = new BehaviorSubject(availableApps);
    this.statusUpdaters$
      .pipe(
        map((statusUpdaters) => {
          return new Map(
      ...
      .subscribe((apps) => pricings$.next(apps));

    const pricingStatuses$ = pricings$.pipe(
    ...
    const navigateToApp: InternalApplicationStart['navigateToApp'] = async (
      ...
      const targetApp = pricings$.value.get(appId);
    ...
    return {
      ...
      isAppRegistered: (appId: string): boolean => {
        return pricings$.value.get(appId) !== undefined;
      },
      getUrlForApp: (
        appId,
        {
...
const updateStatus = (app: App, statusUpdaters: AppUpdaterWrapper[]): App => {
  ...
  statusUpdaters.forEach((wrapper) => {
    if (wrapper.pricing !== allApplicationsFilter && wrapper.pricing !== app.id) {
      return;
    }
