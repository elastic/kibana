 ...
 * @public
 */
export type AppUpdatableFields = Pick<App, 'status' | 'visibleIn' | 'tooltip' | 'defaultPath' | 'deepLinks'>;
/**
 * @public
 */
...
export type AppDeepLinkLocations = 'globalSearch' | 'classicSideNav' | 'projectSideNav' | 'home' | 'kibanaOverview';
/**
 * Input type for registering secondary in-app locations for an application.
 *
 * Deep links must include at least one of `path` or `deepLinks`. A deep link that does not have a `path`
 ...
 * @public
 */
export type PublicAppInfo = Omit<App, 'mount' | 'updater$' | 'keywords' | 'deepLinks' | 'visibleIn'> & {
    status: AppStatus;
    appRoute: string;
    keywords: string[];
    deepLinks: PublicAppDeepLinkInfo[];
