...
export interface ServiceAccountOperationHandle {
   ...
   * is detached. Requires the acting user to hold the `manage_security` cluster privilege, and
   * records the binding in the space the request is scoped to.
   *
   * This is the only place a service account is named for a workload; everything below identifies
  ...
  /**
   * Removes a workload's binding, behind the same privilege as {@link attach}. Succeeds whether or
   * not a binding existed, and takes effect on a running execution at its next credential mint.
   */
  detach(
    request: KibanaRequest,
  ...
  /**
   * Returns the headers to stamp on a loopback HTTP request to Kibana's own APIs, made as the
   * workload's service account. Freshly minted per call: read them immediately before the call and
   * never persist them.
   */
