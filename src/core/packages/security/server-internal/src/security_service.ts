  ...
  implements CoreService<InternalSecurityServiceSetup, InternalSecurityServiceStart>
{
  ...
  private registerServiceAccountOperation({
    ...
    return {
      bindWorkload: async (request, params) => delegate().bindWorkload(type, request, params),
      unbindWorkload: async (request, params) => delegate().unbindWorkload(type, request, params),
