...
**Preloaded values.** By default, browser-side reads are **lazy**: the cache starts empty for a given key and the first `peek`/`get`/`get$`/`getState$` miss fires a `GET /internal/user_storage/{key}` request to hydrate it. `peek(key)` returns `undefined` (or the provided `defaultValue`) until that fetch lands; `get(key)` is `async` and resolves once it does (see [Browser API](#browser-api) for the full read/write surface).
...
| Method | Returns | Notes |
|---|---|---|
| `isAvailable()` / `isAvailable$()` | `boolean` / `Observable<boolean>` | Whether User Storage is available for the current user (`false` for anonymous / no-`profile_uid` users). Static for the lifetime of a page load. Check this — not user profile state — before offering save/delete UI. |
| `canWrite()` / `canWrite$()` | `boolean` / `Observable<boolean>` | Whether the current user can persist writes. Today identical to `isAvailable`, kept distinct so read/write access can diverge later without an API change. |
| `peek<T>(key, default?)` | `T \| undefined` | Pure, synchronous cache read; never fetches, never `await`s. Safe in React render, and the right choice for `preload: true` keys (always cached at first paint) or any best-effort snapshot where you don't want to trigger a fetch. |
| `get<T>(key, default?)` | `Promise<T \| undefined>` | Safe, resolved read: resolves once the effective value is known, awaiting a lazy fetch if needed. Rejects if the fetch fails. Use this (or `update`) as the read half of a read-modify-write. |
| `get$<T>(key, default?)` | `Observable<T \| undefined>` | Cache snapshot, then every future value for the key. The first emission may be a temporary default for a non-preloaded key — see `getState$` if you need to tell that apart from a resolved value. |
| `getState$<T>(key, default?)` | `Observable<UserStorageValue<T>>` | Load-state stream: `{status:'loading', value: default}` while a lazy key is hydrating, then `{status:'resolved', value}`, or `{status:'error', value: default, error}` if the fetch fails. Lets UI show a fallback while disabling destructive actions until resolved. |
...
| `update<T>(key, default, updater)` | `Promise<T>` | Safe read-modify-write: awaits the resolved current value, applies `updater`, and `set`s the result. If `updater` returns the exact value it was given, the write is skipped as a no-op. Last-write-wins — no concurrent-writer detection. |
...
For `preload: true` keys the cache is pre-populated at first paint, so `peek` is truly synchronous and is the natural choice in render/imperative code that can't `await`. For lazy keys, prefer `get` (or `update`) whenever you need the *true* stored value — e.g. as the base of a read-modify-write — since `peek` can return an unhydrated default before the background fetch lands.

**Read-modify-write.** Don't build a mutation on top of `peek()` for a lazy key — the cache may not be hydrated yet, and computing the next value from an unhydrated default can silently overwrite a user's existing stored data. Use `update()` instead:

```typescript
await core.userStorage.update<StoredPresets>(
  'myPlugin:date-range-presets',
  DEFAULT_STORED_PRESETS,
  (stored) => {
    if (alreadyHasPreset(stored, preset)) return stored; // no-op: same reference, skips the write
    return { ...stored, presets: [...stored.presets, preset] };
  }
);
```
...
`useUserStorage(key, defaultValue?)` returns `[value, setter, state]`, backed by `getState$` under the hood. `value` re-renders on every cache change; the setter persists via HTTP, refreshes the cache on success, and resolves to the validated value (if the HTTP write fails, the cache is unchanged, the returned promise rejects, and the error is published to `getHttpError$`). `state.status` is `'loading'` | `'resolved'` | `'error'` (with `state.error` set for the last one) — use it to tell a temporary fallback apart from the resolved value, and to keep destructive actions disabled until it settles:

```tsx
const [layout, setLayout, { status }] = useUserStorage<NavLayout>('myPlugin:nav-layout', {
  hidden: [],
  order: [],
});
...
return (
  <button
    ...
    onClick={() => setLayout({ ...layout, hidden: [...layout.hidden, 'discover'] })}
  >
    Hide Discover
  </button>
);
...
Because `myPlugin:nav-layout` is registered with `preload: true`, `layout` (and `status: 'resolved'`) are populated synchronously on the very first render — no loading state to handle in practice for preloaded keys. Lazy (non-preloaded) keys start at `status: 'loading'` with `layout` set to the caller's `defaultValue` until the background fetch resolves.
...
If your feature must work for users without a profile (anonymous pages, API-key auth), do not depend on User Storage. Check `core.userStorage.isAvailable()` (or `canWrite()`) before offering save/delete UI — for these users it's `false`, the browser cache will be empty, and writes will return `403`. Either layer over `localStorage` for those cases or design the feature so missing User Storage is acceptable (e.g. fall back to defaults).
