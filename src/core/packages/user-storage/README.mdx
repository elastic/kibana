...
`useUserStorage(key, defaultValue?)` returns `[value, setter, state]`. `value`
re-renders on every cache change; the setter persists via HTTP and refreshes the cache
on success. `state.status` is `'loading'` | `'resolved'` | `'error'` (with `state.error`
set for the last one) — use it to tell a temporary fallback apart from the resolved
value, and to keep destructive actions disabled until it settles:
