...
If your feature must work for users without a profile (anonymous pages, API-key auth), do not depend on User Storage. Check `core.userStorage.isAvailable()` before offering save/delete UI — for these users it's `false`, the browser cache will be empty, and writes will return `403`. Either layer over `localStorage` for those cases or design the feature so missing User Storage is acceptable (e.g. fall back to defaults).
