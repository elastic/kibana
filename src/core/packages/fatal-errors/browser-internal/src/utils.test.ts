...
import { formatMessage, formatStack } from './utils';
...
describe('formatMessage', () => {
  it('should prepend the `source` to the message', () => {
    expect(formatMessage('error message', 'unit_test')).toBe('unit_test: error message');
  });

  it('should handle a simple string', () => {
    expect(formatMessage('error message')).toBe('error message');
  });
  ...
  it('should read the root cause reason from elasticsearch errors', () => {
    ...
    expect(formatMessage(err, 'foo')).toBe('foo: I am the detailed message');
  });
  ...
  it('should combine the root cause reasons if elasticsearch error has more than one', () => {
    ...
    expect(formatMessage(err)).toBe('I am the detailed message 1\nI am the detailed message 2');
  });
});
