...
export const Title = React.memo(
  React.forwardRef<TitleHandle, TitleProps>(
    ({ title, titleOffset, size = 's', onEditingChange }, ref) => {
          ...
          <EuiTitle size={size}>
            <h1>
              {isEditing ? (
                ...
                // Editable read view matches non-editable geometry; editing is triggered from
                // the pencil action in the title actions cluster.
                <span css={styles.readModeTitle}>{readContent}</span>
              ) : (
