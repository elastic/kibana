...
Use `AppHeader` when the page can render its header inline in the app's React tree. This is the preferred long-term model for pages that own their title, back target, tabs, badges, and app menu locally. `AppHeader` also handles the classic `EuiPageHeader` fallback when Chrome Next is disabled.
