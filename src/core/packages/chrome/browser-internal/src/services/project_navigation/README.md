...
- **Detection Algorithm**: The active node is found by the `findActiveNodes` utility, which uses two methods:
  ...
  2.  **Longest URL Match**: As a fallback, the utility finds the most specific link by matching the longest URL prefix. If `deepLink.id` is an application id, the match uses that app's `appRoute` (no `defaultPath`). Deep links still match `url`.
