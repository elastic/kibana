...
- `available`: Whether the app is available (defaults to `true`).
...
Use `available: false` when the app depends on async checks (license, permissions).
...
// Later in start
core.chrome.sidebar.getApp('mySidebarApp').setAvailable(true);
```
