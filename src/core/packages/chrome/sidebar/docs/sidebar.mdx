...
`SidebarHeader` requires a `title` prop. It renders as an `<h2>` heading and sets the panel's `aria-label` (e.g. "Side panel: My App"). When using custom header content via `children`, `title` is still required and used for the `aria-label`:
...
If you don't use `SidebarHeader` at all (fully custom header), you must set the label manually via `useSidebarPanel`:
