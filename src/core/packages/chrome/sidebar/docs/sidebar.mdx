...
// Check this app
const isOpen = myApp.isOpen();
const isOpen$ = myApp.isOpen$();

// Check the global sidebar state
const isSidebarOpen = core.chrome.sidebar.isOpen();
...
function MyToolbarButton() {
  ...
  return (
    <>
      ...
      <button onClick={myApp.isOpen ? myApp.close : myApp.open}>
        {myApp.isOpen ? 'Close' : 'Open'}
      </button>
    </>
  );
}
