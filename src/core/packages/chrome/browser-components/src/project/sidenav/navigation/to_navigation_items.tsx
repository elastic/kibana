...
export interface NavigationItems {
  ...
  activeItemId?: string;
  /** Extension ids that opt into hide-when-empty; used by the visibility layer only. */
...
interface NavigationConversionContext {
  ...
  hideWhenEmptyExtensionIds: Set<string>;
...
const createNavigationConversionContext = (
  ...
  const ctx: NavigationConversionContext = {
    panelStateManager,
    hideWhenEmptyExtensionIds: new Set<string>(),
...
const createExtensionPointSection = (
  ...
  if (child.hideWhenEmpty) {
    hideWhenEmptyExtensionIds.add(child.extensionId);
  }
...
const convertPanelOpenerSections = (
  ...
  for (const child of children) {
    ...
    if (child.renderAs === 'extension') {
      ...
      const extensionSection = createExtensionPointSection(child, ctx.hideWhenEmptyExtensionIds);
      if (extensionSection) {
...
export const toNavigationItems = (
  ...
  return {
    ...
    hideWhenEmptyExtensionIds: ctx.hideWhenEmptyExtensionIds,
  };
};
