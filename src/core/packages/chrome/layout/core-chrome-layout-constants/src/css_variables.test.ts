...
describe('CSS Variables', () => {
  describe('layoutVarName', () => {
    ...
    it('should generate application variable names with --kbn-application-- prefix', () => {
      expect(layoutVarName('application-top-bar-height')).toBe('--kbn-application--top-bar-height');
      expect(layoutVarName('application-bottom-bar-width')).toBe(
        '--kbn-application--bottom-bar-width'
      );
      expect(layoutVarName('application-content-left')).toBe('--kbn-application--content-left');
    });

    it('should strip application- prefix correctly', () => {
      // Verify the prefix stripping logic
      expect(layoutVarName('application-top-bar-height')).not.toContain('application-top-bar');
      expect(layoutVarName('application-top-bar-height')).toContain('top-bar-height');
    });
  });
  ...
  describe('layoutVar', () => {
    ...
    it('should wrap application variables in var() syntax', () => {
      expect(layoutVar('application-top-bar-height')).toBe(
        'var(--kbn-application--top-bar-height)'
      );
    ...
    it('should include fallback values when provided', () => {
      ...
      expect(layoutVar('application-top-bar-height', '40px')).toBe(
