...
describe('resolve_tokens', () => {
  ...
  describe('getExtensions', () => {
    const ExtensionPoint = Symbol.for('test.ExtensionPoint') as ExtensionPointToken<string>;
    ...
    it('propagates errors thrown while resolving a contribution', () => {
      const container = new Container({ defaultScope: 'Singleton' });
      container.bind(ExtensionPoint).toDynamicValue(() => {
        throw new Error('broken contribution');
      });

      expect(() => getExtensions(container, ExtensionPoint)).toThrow('broken contribution');
    });
  });
});
