...
import { cacheInScope, ProvidedService } from '@kbn/core-di-internal';
...
export function loadHttp({ bind, onActivation }: ContainerModuleLoadOptions): void {
  onActivation(Route, ({ get }, route) => {
    ...
    let handler: RequestHandler = async (_context, request, response) => {
      ...
      scope.bind(ProvidedService).toConstantValue(Response);

      let routeHandler: InstanceType<typeof route>;
      try {
        routeHandler = scope.get(route, { autobind: true });
      } catch (err) {
        scope.unbindAll();
        throw err;
      }
      ...
      try {
        return await routeHandler.handle();
      } finally {
        scope.unbindAll();
      }
    };
