...
describe('PluginModule', () => {
  ...
  describe('context-less fork', () => {
    // Mirrors http.ts / application.ts, which bind short-lived values (Request,
    // Response, ApplicationParameters) on a fork created without a plugin id.
    // Such a fork is its own context, so activating its provided services must
    // not require a Context binding.
    const requestValue = Symbol.for('requestValue');

    it('activates its own provided services without a Context', () => {
      const fork = root.get(Fork)();
      fork.bind(requestValue).toConstantValue('request');
      fork.bind(ProvidedService).toConstantValue(requestValue);

      expect(() => fork.getAll(ProvidedService)).not.toThrow();
    });

    it('resolves a plugin-provided service depending on a value bound on the fork', () => {
      const plugin = root.get(Scope)(token1);
      plugin
        .bind('service')
        .toResolvedValue((value) => `served:${value}`, [requestValue])
        .inRequestScope();
      plugin.bind(ProvidedService).toConstantValue('service');
      plugin.get(Setup);
      ...
      expect(fork.get('service')).toBe('served:request');
    });
  });
