...
import { type Bind, ContainerModule, type ServiceIdentifier } from 'inversify';
...
function bindEach<T extends object>(
  bind: Bind,
  object: T,
...
>(
  ...
  return new ContainerModule(({ bind }) => {
    bindEach(bind, pluginInitializerContext, InternalPluginInitializer);
    bindEach(bind, coreSetupContext, InternalCoreSetup);
    bindEach(bind, plugins, PluginSetup);
  });
}
...
export function createStartModule<TCoreStartContext extends object, TPluginsStart extends object>(
  ...
  return new ContainerModule(({ bind }) => {
    bindEach(bind, coreStartContext, InternalCoreStart);
    bindEach(bind, plugins, PluginStart);
  });
}
