...
export const euiPushFlyoutPaddingInlineEnd = '--temp-hack-eui-push-flyout-padding-inline-end';
 ...
 */
export function useSyncPushFlyoutStyles(): SerializedStyles {
  useEffect(() => {
    ...
    const callback: MutationCallback = (mutationsList: MutationRecord[]) => {
      for (const mutation of mutationsList) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          ...
          if (paddingInlineEnd) {
            document.body.style.setProperty(euiPushFlyoutPaddingInlineEnd, paddingInlineEnd);
          } else {
            document.body.style.removeProperty(euiPushFlyoutPaddingInlineEnd);
          }
        }
      }
    };
  ...
  return css`
    padding-inline-end: var(${euiPushFlyoutPaddingInlineEnd}, 0px);
  `;
}
