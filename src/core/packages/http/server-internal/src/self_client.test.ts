...
describe('InternalHttpSelfScopedClient', () => {
  ...
  describe('internal-caller attestation', () => {
    const attestationFor = (credential: HTTPAuthorizationHeader) => ({
      [UIAM_INTERNAL_CALLER_ATTESTATION_HEADER]: `attestation-for-${credential.credentials}`,
    });

    it('stamps an attestation derived from the credential a fake request carries', async () => {
      const getInternalCallerAttestationHeaders = jest.fn(attestationFor);
      const { self } = createClient({ getInternalCallerAttestationHeaders });

      await self
        .asScoped(createFakeRequest({ authorization: 'Bearer essu_service_account' }))
        .fetch('/api/status');

      expect(sentRequest().headers.get(UIAM_INTERNAL_CALLER_ATTESTATION_HEADER)).toBe(
        'attestation-for-essu_service_account'
      );
      expect(getInternalCallerAttestationHeaders).toHaveBeenCalledTimes(1);
    });

    it('does not stamp an attestation for a user-created (external) UIAM credential', async () => {
      const getInternalCallerAttestationHeaders = jest.fn(attestationFor);
      const { self } = createClient({ getInternalCallerAttestationHeaders });
      const request = createFakeRequest({ authorization: 'ApiKey essu_user_created' });
      markExternalUiamCredential(request);

      await self.asScoped(request).fetch('/api/status');

      expect(sentRequest().headers.get(UIAM_INTERNAL_CALLER_ATTESTATION_HEADER)).toBeNull();
      expect(getInternalCallerAttestationHeaders).not.toHaveBeenCalled();
    });

    it('does not stamp an attestation for a non-UIAM credential', async () => {
      const getInternalCallerAttestationHeaders = jest.fn(attestationFor);
      const { self } = createClient({ getInternalCallerAttestationHeaders });

      await self
        .asScoped(createFakeRequest({ authorization: 'ApiKey regular-key' }))
        .fetch('/api/status');
    ...
    it('does not stamp an attestation for a real request', async () => {
      const getInternalCallerAttestationHeaders = jest.fn(attestationFor);
      const { self } = createClient({
        authHeaders: { authorization: 'Bearer essu_inbound' },
        getInternalCallerAttestationHeaders,
      });
    ...
    it('rejects a caller-supplied attestation as a protected header', async () => {
      const { self } = createClient({ getInternalCallerAttestationHeaders: attestationFor });

      await expect(
        self
          .asScoped(createFakeRequest({ authorization: 'Bearer essu_token' }))
          .fetch('/api/status', {
            headers: { [UIAM_INTERNAL_CALLER_ATTESTATION_HEADER]: 'forged' },
          })
      ).rejects.toThrow(/protected headers are not allowed/);
    });
  });
  ...
  describe('401 retry', () => {
    ...
    it('overrides a stale attestation the caller supplied through request headers', async () => {
      mockFetchResponses(unauthorizedResponse(), okResponse());
      const { self } = createClient({
        unauthorizedErrorHandler: jest.fn(async (options, toolkit) =>
          toolkit.retry({
            authHeaders: {
              ...
              [UIAM_INTERNAL_CALLER_ATTESTATION_HEADER]: 'attestation-for-essu_refreshed',
            },
          })
        ),
      });
