...
describe('CspDirectives', () => {
  describe('#addDirectiveValue', () => {
    ...
    it(`removes 'none' from object_src when other values are added`, () => {
      ...
      expect(directives.getCspHeader()).toMatchInlineSnapshot(
        `"script-src 'report-sample' 'self' 'wasm-unsafe-eval'; worker-src 'report-sample' 'self' blob:; style-src 'report-sample' 'self' 'unsafe-inline'; object-src 'report-sample' some-object_src-value; form-action 'report-sample' 'self'"`
      );
    });
    ...
    it('augments report-only directives when testing default-src none', () => {
      ...
      expect(directives.getCspHeadersByDisposition()).toMatchInlineSnapshot(`
        Object {
          "enforceHeader": "script-src 'report-sample' 'self' 'wasm-unsafe-eval'; worker-src 'report-sample' 'self' blob:; style-src 'report-sample' 'self' 'unsafe-inline'; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'; img-src 'self' img-src-value",
  ...
  describe('#fromConfig', () => {
    ...
    it('adds default value for config with directives', () => {
      ...
      expect(directives.getCspHeader()).toMatchInlineSnapshot(
        `"script-src 'report-sample' 'self' 'wasm-unsafe-eval' baz; worker-src 'report-sample' 'self' blob: foo; style-src 'report-sample' 'self' 'unsafe-inline' bar dolly; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'"`
      );
    });
    ...
    it('adds additional values for some directives without defaults', () => {
      ...
      const directives = CspDirectives.fromConfig(config);
      expect(directives.getCspHeader()).toMatchInlineSnapshot(
        `"script-src 'report-sample' 'self' 'wasm-unsafe-eval'; worker-src 'report-sample' 'self' blob:; style-src 'report-sample' 'self' 'unsafe-inline'; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'; connect-src 'self' connect-src; default-src 'self' default-src; font-src 'self' font-src; frame-src 'self' frame-src; img-src 'self' img-src; frame-ancestors 'self' frame-ancestors; report-uri report-uri; report-to report-to"`
      );
    });
    ...
    it('adds single quotes for keywords in added directives', () => {
      ...
      const directives = CspDirectives.fromConfig(config);
      expect(directives.getCspHeader()).toMatchInlineSnapshot(
        `"script-src 'report-sample' 'self' 'wasm-unsafe-eval' 'unsafe-hashes'; worker-src 'report-sample' 'self' blob:; style-src 'report-sample' 'self' 'unsafe-inline'; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'"`
      );
    });

    it('adds single quotes for wasm-unsafe-eval in script_src', () => {
      const config = cspConfig.schema.validate({
        script_src: [`wasm-unsafe-eval`],
      });
      const directives = CspDirectives.fromConfig(config);
      expect(directives.getCspHeader()).toContain(`'wasm-unsafe-eval'`);
    });
    ...
    it('merges additional CSP configs as expected', () => {
      ...
      expect(directives.getCspHeader()).toEqual(
        `script-src 'report-sample' 'self' 'wasm-unsafe-eval' cdn.host.test; worker-src 'report-sample' 'self' blob: cdn.host.test; style-src 'report-sample' 'self' 'unsafe-inline' cdn.host.test; object-src 'report-sample' 'none'; form-action 'report-sample' 'self'; connect-src 'self' *.foo.bar cdn.host.test; font-src 'self' cdn.host.test; frame-src 'self' cdn.host.test; img-src 'self' *.foo.bar cdn.host.test`
      );
    });
  });
});
