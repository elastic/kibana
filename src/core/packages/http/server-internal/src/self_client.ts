...
import type { HTTPAuthorizationHeader as HTTPAuthorizationHeaderType } from '@kbn/core-security-server';
import {
...
interface HttpSelfClientParams {
  ...
  readonly getInternalCallerAttestationHeaders?: (
    credential: HTTPAuthorizationHeaderType
...
class InternalHttpSelfScopedClient implements HttpSelfScopedClient {
  ...
  private createHeaders<TRequestBody>(
    ...
    // was built from, and over any attestation the caller stamped for the previous credential.
    addHeaders(headers, retryAuthHeaders);
    this.addInternalCallerAttestation(headers);
  ...
  /**
   * Stamps the UIAM internal-caller attestation for a credential Kibana itself holds, so the
   * receiving Kibana can re-attach the UIAM shared secret on its behalf. Derived from the
   * credential on this attempt's headers, which is what keeps the pair consistent across a retry.
   *
   * Only fake requests qualify: a real request's credential rode in over HTTP and authenticated on
   * its own, so attesting it here would change what it is allowed to do.
   */
  private addInternalCallerAttestation(headers: Headers): void {
    const { getInternalCallerAttestationHeaders } = this.params;
    if (!getInternalCallerAttestationHeaders || !this.request.isFakeRequest) {
      return;
    }

    // UIAM rejects a user-created (external) key presented with client authentication, so it must
    // never be attested.
    if (isExternalUiamCredential(this.request)) {
      return;
    }

    const credential = HTTPAuthorizationHeader.parseFromRequest({
    ...
    addHeaders(headers, getInternalCallerAttestationHeaders(credential));
  }
...
const isRetryOverridableHeader = (name: string): boolean => {
  ...
  return !(
    lowerName === 'cookie' ||
    lowerName === 'host' ||
    lowerName === 'x-client-authentication' ||
