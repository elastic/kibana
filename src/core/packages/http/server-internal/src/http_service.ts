  ...
  public async preboot(deps: PrebootDeps): Promise<InternalHttpServicePreboot> {
    ...
    prebootSetup.server.route({
      ...
      handler: (req, responseToolkit) => {
        this.log.debug(`Kibana server is not ready yet ${req.method}:${req.url.href}. For troubleshooting guidance, see ${deps.docLinks.links.server.troubleshootServerNotReady}`);
