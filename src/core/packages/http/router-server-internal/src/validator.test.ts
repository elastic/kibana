...
describe('Router validator', () => {
  ...
  it('should validate and infer the type from a zod-schema ObjectType', () => {
    ...
    expect(() => schemaValidation.getParams({})).toThrow(/Invalid input/);
    expect(() => schemaValidation.getParams(undefined)).toThrow(/Invalid input/);
    expect(() => schemaValidation.getParams({}, 'myField')).toThrow(/Invalid input/);
  });
