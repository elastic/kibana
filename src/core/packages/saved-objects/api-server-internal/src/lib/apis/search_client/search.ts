...
export const performSearch = async <T = unknown, A = unknown>(
  ...
  let { namespaces, ...esOptions } = options;
  const { types } = options;
  ...
  esOptions = {
    ...
    index: options.pit ? undefined : commonHelper.getIndicesForTypes(allowedTypes),
      namespaces,
