...
describe('collectExportedObjects', () => {
  ...
  describe('when `includeReferences` is `true`', () => {
    ...
    describe('access control', () => {
      ...
      const abc3 = createObject({
        type: 'abc',
        id: '3',
      });

      it('calls `accessControlExportTransform` once for all objects if defined', async () => {
        const accessControlExportTransform: SavedObjectsExportTransform = jest.fn();
        ...
        expect(accessControlExportTransform).toHaveBeenCalledTimes(1);
        expect(accessControlExportTransform).toHaveBeenCalledWith(
          expect.objectContaining({ request }),
          [
            expect.objectContaining({ type: 'foo', id: '1' }),
            expect.objectContaining({ type: 'abc', id: '3' }),
            expect.objectContaining({ type: 'bar', id: '2' }),
          ]
        );
      });

      it('does not call `accessControlExportTransform` if undefined', async () => {
        const accessControlExportTransform: SavedObjectsExportTransform = jest.fn();
        ...
        expect(accessControlExportTransform).not.toHaveBeenCalled();
      });
    });
  });
