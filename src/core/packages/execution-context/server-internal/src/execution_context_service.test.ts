...
describe('ExecutionContextService', () => {
  describe('setup', () => {
    ...
    describe('append', () => {
      it('appends space to existing context', async () => {
        service.set({
          type: 'type-a',
          name: 'name-a',
          id: 'id-a',
      ...
      it('does nothing if no context exists', async () => {
        service.append({ space: 'my-space' });
      ...
      it('preserves existing space value when appending without space', async () => {
        service.set({
          type: 'type-a',
          name: 'name-a',
          id: 'id-a',
      ...
      it('overwrites existing space value when provided', async () => {
        service.set({
          type: 'type-a',
          name: 'name-a',
          id: 'id-a',
        ...
        service.append({ space: 'new-space' });
      ...
      it('emits context to the logs when "append" is called', async () => {
        service.set({
          type: 'type-a',
          name: 'name-a',
          id: 'id-a',
        ...
        const logs = loggingSystemMock.collect(core.logger).debug;
        expect(logs[logs.length - 1]).toMatchInlineSnapshot(`
          Array [
            "{\\"type\\":\\"type-a\\",\\"name\\":\\"name-a\\",\\"id\\":\\"id-a\\",\\"description\\":\\"description-a\\",\\"space\\":\\"my-space\\"}",
          ]
        `);
      });
      ...
      it('can be disabled', async () => {
        ...
        disabledService.append({ space: 'my-space' });

        expect(disabledService.get()).toBeUndefined();
      });
    });
