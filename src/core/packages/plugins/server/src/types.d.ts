...
import type { ConfigPath, EnvironmentMode, PackageInfo, ConfigDeprecationProvider } from '@kbn/config';
...
export type DynamicConfigDescriptor<T> = {
    [Key in keyof T]?: T[Key] extends Maybe<any[]> ? boolean : T[Key] extends Maybe<object> ? // can be nested for objects
    DynamicConfigDescriptor<T[Key]> | boolean : boolean;
};
/**
 ...
 * @public
 */
export type PluginInitializer<TSetup = void, TStart = void, TPluginsSetup extends Record<string, any> = never, TPluginsStart extends Record<string, any> = never> = (core: PluginInitializerContext) => Promise<Plugin<TSetup, TStart, TPluginsSetup, TPluginsStart> | PrebootPlugin<TSetup, TPluginsSetup>>;
export {};
