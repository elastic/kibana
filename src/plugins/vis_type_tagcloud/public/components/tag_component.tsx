...
export type TagCloudComponentProps = TagCloudVisRenderValue & {
...
export const TagCloudChart = ({
  ...
  const metrics = visData.rows.map((data) => data[metricColumn]);
  const values = bucket ? visData.rows.map((data) => data[tagColumn]) : [];
  const maxValue = Math.max(...metrics);
  const minValue = Math.min(...metrics);

  const tagData = visData.rows.map((row, i) => {
    ...
    <EuiResizeObserver onResize={updateChartSize}>
      {(resizeRef) => (
        <div className="tgcChart__wrapper" ref={resizeRef}>
          ...
          {label && showLabel && <div className="tgcChart__label">{label}</div>}
          {warning && (
