...
const url = _interopRequireWildcard(require('url'));
...
function toURL(base, path) {
  ...
  const urlResult = new url.URL(
    `${(0, _lodash.trimEnd)(base, '/')}/${(0, _lodash.trimStart)(path, '/')}`
...
function getRequestConfig(headers, esConfig, uri, kibanaVersion, proxyConfigCollection) {
  ...
  if (kibanaVersion.major < 8) {
    ...
    if (proxyConfigCollection.hasConfig()) {
      return { ...proxyConfigCollection.configForUri(uri), headers: newHeaders };
    }
  }

  return { ...(0, _lib.getElasticsearchProxyConfig)(esConfig), headers: newHeaders };
}
...
function getProxyHeaders(req) {
  ...
  if (
    _req !== null &&
    _req !== void 0 &&
    (_req$info = _req.info) !== null &&
    _req$info !== void 0 &&
    _req$info.remotePort &&
    _req !== null &&
    _req !== void 0 &&
    (_req$info2 = _req.info) !== null &&
    _req$info2 !== void 0 &&
    _req$info2.remoteAddress
  ) {
...
const createHandler =
  ({ log, proxy: { readLegacyESConfig, pathFilters, proxyConfigCollection }, kibanaVersion }) =>
