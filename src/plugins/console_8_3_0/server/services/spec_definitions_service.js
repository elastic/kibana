...
const _lodash = _interopRequireWildcard(require('lodash'));

const _glob = _interopRequireDefault(require('glob'));
...
class SpecDefinitionsService {
  ...
  addEndpointDescription(endpoint, description = {}) {
    ...
    if (urlParamsDef) {
      description.url_params = _lodash.default.assign(
  ...
  loadJSONSpecInDir(dirname) {
    ...
    return generatedFiles.reduce((acc, file) => {
      const overrideFile = overrideFiles.find(
        (f) => (0, _path.basename)(f) === (0, _path.basename)(file)
      );
