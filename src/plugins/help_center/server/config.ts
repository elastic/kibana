...
export const configSchema = schema.object({
  ...
  mainInterval: schema.duration({ defaultValue: '2m' }), // (2min) How often to retry failed fetches, and/or check if HelpCenter items need to be refreshed from remote
  fetchInterval: schema.duration({ defaultValue: '1d' }), // (1day) How often to fetch remote and reset the last fetched time
});

export type HelpCenterConfigType = TypeOf<typeof configSchema>;
