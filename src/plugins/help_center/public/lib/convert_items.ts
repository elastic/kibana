...
import { ApiItem, HelpCenterItem } from '../types';
import { NEWSFEED_FALLBACK_LANGUAGE } from '../../common/constants';

export const convertItems = (items: ApiItem[], userLanguage: string): HelpCenterItem[] => {
  return items
...
export const localizeItem = (rawItem: ApiItem, userLanguage: string): HelpCenterItem => {
  const {
...
export const validateIntegrity = (item: Partial<HelpCenterItem>): boolean => {
  const hasMissing = [
    item.title,
    item.description,
    item.linkText,
