...
import { ApiItem, FetchResult, HelpCenterPluginBrowserConfig } from '../types';
import { IHelpCenterApiDriver } from './types';
import { convertItems } from './convert_items';
import type { HelpCenterStorage } from './storage';

type ApiConfig = HelpCenterPluginBrowserConfig['service'];

interface HelpCenterResponse {
  items: ApiItem[];
}

export class HelpCenterApiDriver implements IHelpCenterApiDriver {
  ...
  constructor(
    ...
    private readonly storage: HelpCenterStorage
  ) {
  ...
  fetchHelpCenterItems(config: ApiConfig): Rx.Observable<FetchResult> {
    ...
    return Rx.from(
      window.fetch(request).then(async (response) => {
        const { items } = (await response.json()) as HelpCenterResponse;
