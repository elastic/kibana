...
export function useStateContainer(stateContainer: GetStateReturn, filterProps: string[]) {
  const [state, setState] = useState<AppState>(stateContainer.getAppState());

  useEffect(() => {
    const unsubscribe = stateContainer.subscribe((next) => {
