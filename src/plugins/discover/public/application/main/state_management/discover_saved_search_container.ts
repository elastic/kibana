...
export function getSavedSearchContainer({
  ...
  internalStateContainer.transitions.setDiscoverSessionInitial(initialSavedSearch);
  internalStateContainer.transitions.setDiscoverSessionEdited(copySavedSearch(initialSavedSearch));
  const set = (savedSearch: SavedSearch) => {
    addLog('[savedSearch] set', savedSearch);
    internalStateContainer.transitions.setDiscoverSessionInitial(savedSearch);
    internalStateContainer.transitions.setDiscoverSessionEdited(copySavedSearch(savedSearch));
    internalStateContainer.transitions.setDiscoverSessionHasChanged(false);
    return savedSearch;
  };
  const getState = () => internalStateContainer.getState().discoverSessionEdited!;
  const getTitle = () => internalStateContainer.getState().discoverSessionEdited?.title;
  const getId = () => internalStateContainer.getState().discoverSessionEdited?.id;
  ...
  const assignNextSavedSearch = ({ nextSavedSearch }: { nextSavedSearch: SavedSearch }) => {
    const initial = internalStateContainer.getState().discoverSessionInitial!;
    const hasChanged = !isEqualSavedSearch(initial, nextSavedSearch);
    internalStateContainer.transitions.setDiscoverSessionEdited(nextSavedSearch);
    internalStateContainer.transitions.setDiscoverSessionHasChanged(hasChanged);
  };
