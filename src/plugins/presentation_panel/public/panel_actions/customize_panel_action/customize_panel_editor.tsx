...
export const CustomizePanelEditor = ({
  ...
  const [title, setTitle] = useState(getTitle(api));
  const [description, setDescription] = useState(getDescription(api));
  ...
  const save = () => {
    if (title !== api.title$?.value) api.setTitle?.(title);
          ...
          labelAppend={
            api?.defaultTitle$?.value && (
              <EuiButtonEmpty
                ...
                onClick={() => setTitle(api.defaultTitle$?.value)}
                disabled={hideTitle || title === api?.defaultTitle$?.value}
          ...
          labelAppend={
            api.defaultDescription$?.value && (
              <EuiButtonEmpty
                ...
                onClick={() => setDescription(api.defaultDescription$?.value)}
                disabled={api.defaultDescription$?.value === description}
                aria-label={i18n.translate(
