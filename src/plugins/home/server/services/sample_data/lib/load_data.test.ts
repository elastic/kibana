...
test('load flight data', async () => {
  ...
  const count = await loadData(
    './src/plugins/home/server/services/sample_data/data_sets/flights/flights.json.gz',
    bulkInsertMock
  );
...
test('load log data', async () => {
  ...
  const count = await loadData(
    './src/plugins/home/server/services/sample_data/data_sets/logs/logs.json.gz',
    bulkInsertMock
  );
...
test('load ecommerce data', async () => {
  ...
  const count = await loadData(
    './src/plugins/home/server/services/sample_data/data_sets/ecommerce/ecommerce.json.gz',
    bulkInsertMock
  );
