...
export function StacktraceContent({ hit, dataView }: { hit: DataTableRecord; dataView: DataView }) {
  const errorStackTrace = getFieldValue(hit, fieldConstants.ERROR_STACK_TRACE);
  const dataStreamTypeField = getFieldValue(hit, fieldConstants.DATASTREAM_TYPE_FIELD);
  const dataStreamDatasetField = getFieldValue(hit, fieldConstants.DATASTREAM_DATASET_FIELD);
