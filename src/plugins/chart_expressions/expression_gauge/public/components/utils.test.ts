...
describe('expression gauge utils', () => {
  describe('getValueFromAccessor', () => {
    ...
    it('returns accessor value for number', () => {
      expect(getValueFromAccessor('metric', row, state)).toEqual(2);
    });
    it('returns accessor value for array', () => {
      expect(getValueFromAccessor('min', row, state)).toEqual(20);
    });
    it('returns undefined for not number', () => {
      expect(getValueFromAccessor('max', row, state)).toEqual(undefined);
    });
  });
  describe('getMaxValue', () => {
    ...
    it('returns correct value for existing max', () => {
      ...
      expect(getMaxValue(row, { ...state, max: 'max' })).toEqual(5);
    });
    it('returns fallback value', () => {
      const localState = { ...state, max: 'max' };
