...
const _sample_dataset_schema = require('./lib/sample_dataset_schema');

const _data_sets = require('./data_sets');

const _routes = require('./routes');

const _usage = require('./usage');

const _uninstall = require('./routes/uninstall');

const _register_with_integrations = require('./lib/register_with_integrations');
...
class SampleDataRegistry {
  ...
  setup(core, usageCollections, customIntegrations) {
    ...
    const usageTracker = (0, _usage.usage)(
    ...
    if (customIntegrations && core) {
      (0, _register_with_integrations.registerSampleDatasetWithIntegration)(
        customIntegrations,
        core
      );
    }
    ...
    return {
      ...
      addAppLinksToSampleDataset: (id, appLinks) => {
        ...
        const existingAppLinks =
          (_this$appLinksMap$get = this.appLinksMap.get(id)) !== null &&
          _this$appLinksMap$get !== void 0
            ? _this$appLinksMap$get
            : [];
