...
export class VegaParser {
  ...
  async _parseAsync() {
    ...
    if (!this.parsedSpec) {
      this.parsedSpec = hjson.parse(this.spec, { legacyRoot: false });
    }
    if (!_.isPlainObject(this.parsedSpec)) {
      throw new Error(
    ...
    } else if (this.parsedSpec?.autosize === undefined) {
      // Default autosize should be fit, unless it's a map (leaflet-vega handles that)
      this.parsedSpec!.autosize = { type: 'fit', contains: 'padding' };
    }
  ...
  _compileVegaLite() {
    ...
    if (this.useMap) {
      ...
      if (this.vlspec?.config === undefined || (hasConfig && !this.vlspec.config.projection)) {
        ...
        if (
          !Array.isArray(this.parsedSpec?.projections) ||
          this.parsedSpec?.projections.length !== 1 ||
          this.parsedSpec?.projections[0].name !== 'projection'
        ) {
          throw new Error(
            i18n.translate(
  ...
  _calcSizing() {
    ...
    if (!this.useMap) {
      ...
      const autosize = this.parsedSpec?.autosize.type || this.parsedSpec?.autosize;
      if (
        autosize === 'fit' ||
        (autosize === 'none' && !this.parsedSpec?.width && !this.parsedSpec?.height)
      ) {
        this.useResize = true;
      }
    }
    ...
    if (
      this.useResize &&
      this.parsedSpec?.padding &&
      this.parsedSpec.autosize.contains !== 'padding'
    ) {
      if (typeof this.parsedSpec.padding === 'object') {
        this.paddingWidth +=
          (+this.parsedSpec.padding.left || 0) + (+this.parsedSpec.padding.right || 0);
        this.paddingHeight +=
          (+this.parsedSpec.padding.top || 0) + (+this.parsedSpec.padding.bottom || 0);
      } else {
        this.paddingWidth += 2 * (+this.parsedSpec.padding || 0);
        this.paddingHeight += 2 * (+this.parsedSpec.padding || 0);
      }
    }

    if (this.useResize && (this.parsedSpec?.width || this.parsedSpec?.height)) {
      if (this.isVegaLite) {
        delete this.parsedSpec.width;
        delete this.parsedSpec.height;
      } else {
        this._onWarning(
          i18n.translate(
  ...
  _parseConfig(): KibanaConfig | {} {
    ...
    if (this.parsedSpec?._hostConfig !== undefined) {
      result = this.parsedSpec._hostConfig;
      delete this.parsedSpec._hostConfig;
    ...
    if (_.isPlainObject(this.parsedSpec?.config) && this.parsedSpec?.config?.kibana !== undefined) {
      result = this.parsedSpec.config.kibana;
      delete this.parsedSpec.config.kibana;
      if (!_.isPlainObject(result)) {
        throw new Error(
  ...
  _parseSchema() {
    if (!this.parsedSpec?.$schema) {
      ...
      this.parsedSpec!.$schema = DEFAULT_SCHEMA;
    }

    const schema = schemaParser(this.parsedSpec!.$schema);
  ...
  async _resolveDataUrls() {
    ...
    this._findObjectDataUrls(this.parsedSpec!, (obj: Data) => {
      const url = obj.url;
      delete obj.url;
      let type = url!['%type%'];
      delete url!['%type%'];
  ...
  _setDefaultColors() {
    ...
    if (this.isVegaLite) {
      ...
      if (
        !this.parsedSpec?.config?.mark ||
        (this.parsedSpec.config.mark.color === undefined &&
          this.parsedSpec.config.mark.fill === undefined)
      ) {
