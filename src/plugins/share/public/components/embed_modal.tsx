  ...
  const renderButtons = () => {
    return (
      <EuiCopy textToCopy={url}>
        {(copy) => (
          <EuiButton fill data-test-subj="copyEmbedUrlButton" data-share-url={url} onClick={copy}>
            <FormattedMessage id="share.link.copyEmbedCodeButton" defaultMessage="Copy Embed" />
          </EuiButton>
        )}
      </EuiCopy>
    );
