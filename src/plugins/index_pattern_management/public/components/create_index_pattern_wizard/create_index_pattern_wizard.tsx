...
> {
  ...
  createIndexPattern = async (timeFieldName: string | undefined, indexPatternId: string) => {
    ...
    const { patternList, title } = this.state;
  ...
  goToTimeFieldStep = (patternList: string[], title: string, selectedTimeField?: string) => {
    this.setState({ step: 2, patternList, title, selectedTimeField });
  };
  ...
  renderContent() {
    const { allIndices, isInitiallyLoadingIndices, step, patternList, title } = this.state;
      ...
      return (
        <EuiPageContent>
          ...
          <StepIndexPattern
            ...
            initialQuery={patternList || initialQuery}
