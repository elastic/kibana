...
export const ForwardFormStateToParent = () => {
  ...
  // This pattern is useful when, for example, the form is inside a multi step wizard and the button
  // to go "next" is outside the scope of the component where the form is declared.
  const initialState = {
    isValid: true,
    validate: async () => true,
