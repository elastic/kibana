...
export interface SendRequestConfigNp {
  path: string;
  method: 'get' | 'post' | 'put' | 'delete' | 'patch';
  body?: any;
}

export interface SendRequestResponseNp {
  data: any;
  error: Error | null;
}

export interface UseRequestConfigNp extends SendRequestConfigNp {
  pollIntervalMs?: number;
  initialData?: any;
  deserializer?: (data: any) => any;
}

export interface UseRequestResponseNp {
  isInitialRequest: boolean;
  isLoading: boolean;
  error: null | unknown;
  data: any;
  sendRequestNp: (...args: any[]) => Promise<SendRequestResponseNp>;
}

export const sendRequestNp = async (
  http: HttpServiceBase,
  { path, method, body }: SendRequestConfigNp
): Promise<SendRequestResponseNp> => {
  try {
    const response = await (http as any)[method](path, body);
...
export const useRequestNp = (
  ...
  const _sendRequest = async () => {
    ...
    const response = await sendRequestNp(http, requestBody);
  ...
  return {
    ...
    error,
    data,
    sendRequestNp: _sendRequest, // Gives the user the ability to manually request data
  };
};
