...
import type { MetricsItems } from './types';
import { SanitizedFieldType } from './types';

describe('calculateLabel(metric, metrics)', () => {
  test('returns the metric.alias if set', () => {
    expect(calculateLabel({ alias: 'Example' } as MetricsItems)).toEqual('Example');
  });

  test('returns "Count" for a count metric', () => {
    expect(calculateLabel({ type: 'count' } as MetricsItems)).toEqual('Count');
  });

  test('returns "Calculation" for a bucket script metric', () => {
    expect(calculateLabel({ type: 'calculation' } as MetricsItems)).toEqual('Bucket Script');
  });

  test('returns formatted label for series_agg', () => {
    const label = calculateLabel({ type: 'series_agg', function: 'max' } as MetricsItems);

    expect(label).toEqual('Series Agg (max)');
  });

  test('returns formatted label for basic aggs', () => {
    const label = calculateLabel({ type: 'avg', field: 'memory' } as MetricsItems);

    expect(label).toEqual('Average of memory');
  });

  test('returns formatted label for pipeline aggs', () => {
    const metric = ({ id: 2, type: 'derivative', field: 1 } as unknown) as MetricsItems;
    const metrics = ([
      { id: 1, type: 'max', field: 'network.out.bytes' },
      metric,
    ] as unknown) as MetricsItems[];
  ...
  test('returns formatted label for pipeline aggs (deep)', () => {
    const metric = ({ id: 3, type: 'derivative', field: 2 } as unknown) as MetricsItems;
    const metrics = ([
      { id: 1, type: 'max', field: 'network.out.bytes' },
  ...
  test('should throw an error if field not found', () => {
    const metric = ({ id: 2, type: 'max', field: 3 } as unknown) as MetricsItems;
    const metrics = ([
      { id: 1, type: 'max', field: 'network.out.bytes', alias: 'Outbound Traffic' },
      metric,
    ] as unknown) as MetricsItems[];
