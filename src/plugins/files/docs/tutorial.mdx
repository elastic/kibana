...
<DocCallOut title="Ensure users can access their files">
  HTTP access tags define a new permission for an HTTP endpoint. **Ensure that users your users have the permissions they need to access their files!** See `KibanaFeatureConfig` for more information on how to grant user premissions access to HTTP APIs.
</DocCallOut>
