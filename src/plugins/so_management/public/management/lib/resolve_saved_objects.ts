...
async function getSavedObject(doc: any, services: SavedObjectLoader[]) {
...
async function importIndexPattern(
  ...
  if (!newId) {
    ...
    const isConfirmed = await overlays.openConfirm(
...
function groupByType(docs: any) {
  ...
  return docs.reduce((types: Record<string, any>, doc: any) => {
    switch (doc._type) {
      case 'search':
    ...
    return types;
  }, defaultDocTypes as Record<string, any>);
}
...
export async function resolveIndexPatternConflicts(
  ...
  await awaitEachItemInParallel(conflictedIndexPatterns, async ({ obj }) => {
    ...
    const filter = (obj.searchSource.getOwnField('filter') || []).map((filter2: any) => {
      if (!(filter2.meta && filter2.meta.index)) {
        return filter2;
      }

      resolution = resolutions.find(({ oldId }) => oldId === filter2.meta.index);
      return resolution
        ? { ...filter2, ...{ meta: { ...filter2.meta, index: resolution.newId } } }
        : filter2;
    });
