...
import { ProcessedImportResponse } from './process_import_response';

type FailedImport = ProcessedImportResponse['failedImports'][0];
...
export async function resolveImportErrors({
  ...
  const overwriteDecisionCache = new Map<string, boolean>();
  const replaceReferencesCache = new Map<string, any[]>();
  ...
  while (
    importFailures.some((failure: FailedImport) =>
