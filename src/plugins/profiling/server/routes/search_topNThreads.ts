...
import { TRACE_EVENTS_TOPN_THREADS_SEARCH_ROUTE_PATH } from '../../common';
import { getDocID } from './index';

export function registerTraceEventsTopNThreadsSearchRoute(router: IRouter<DataRequestHandlerContext>) {
  router.get(
    {
      path: TRACE_EVENTS_TOPN_THREADS_SEARCH_ROUTE_PATH,
      validate: {
        query: schema.object({
    ...
    async (context, request, response) => {
      ...
      try {
        ...
        return response.ok({
          body: {
            topNThreads: (resTopNStackTraces as IEsSearchResponse).rawResponse.aggregations,
