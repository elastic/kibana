...
interface Props {
  loading: boolean;
  initialTextObject: TextObject;
  inputEditorValue: string;
  setInputEditorValue: (value: string) => void;
}

export const Editor = memo(
  ({ loading, initialTextObject, inputEditorValue, setInputEditorValue }: Props) => {
    const {
      services: { storage, objectStorageClient },
    } = useServicesContext();

    const editorValueRef = useRef<TextObject | null>(null);
    const [localStorageValue, setLocalStorageValue] = useState<string | undefined>(
      initialTextObject.text
    );
    ...
    // When the editor unmounts, we update the currentTextObject
    useEffect(() => {
      return () => {
        // If the editor is empty, reset localstorage value to undefined
        // so that next time we open Console the default input is displayed
        if (!inputEditorValue) {
          debouncedUpdateLocalStorageValue(undefined);
        }
        editorDispatch({
