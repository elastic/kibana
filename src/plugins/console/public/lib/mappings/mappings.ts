...
import { retrieveSettings, SettingsToRetrieve } from './settings';
...
let perIndexTypes: Record<string, object> = {};
let perAliasIndexes: Record<string, string[]> = {};
...
export function getFields(indices: string | string[], types?: string | string[]) {
  // get fields for indices and types. Both can be a list, a string or null (meaning all).
  let ret: Field[] = [];
  indices = expandAliases(indices);

  if (typeof indices === 'string') {
    const typeDict = perIndexTypes[indices] as Record<string, unknown>;
    if (!typeDict) {
      return [];
    }
    ...
    Object.keys(perIndexTypes).forEach((index) => {
      if (!indices || indices.length === 0 || indices.includes(index)) {
        ret.push(getFields(index, types) as unknown as Field);
      }
    });
...
export function loadMappings(mappings: {
  [key: string]: { mappings?: Record<string, FieldMapping> };
}) {
  perIndexTypes = {};
...
export function loadAliases(aliases: { [key: string]: { aliases: object } }) {
  perAliasIndexes = {};
...
export async function retrieveMappings(http: HttpSetup, settingsToRetrieve: SettingsToRetrieve) {
...
export async function retrieveAliases(http: HttpSetup, settingsToRetrieve: SettingsToRetrieve) {
