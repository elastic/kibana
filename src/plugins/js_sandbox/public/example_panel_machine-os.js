function(props) {
  ...
  const data = React.useMemo(() => {
    const data = [
      ...props.data.map((d, i) => ({
        machine_os: d['machine.os'],
        count: d.count - (props.crossfilter.find(d2 => d2['machine.os'] === d['machine.os'])?.count ?? 0),
        type: '02_context'
      })),
    ...
    console.log('data machine.os', data);
    return data;
  ...
  React.useEffect(() => {
    ...
    const options = {
      ...
      init: (view) => {
        ...
        view.addSignalListener('machine_os', function(event, item) {
            console.log('item',item);
          if (item['machine_os']) {
              props.dispatch(`WHERE machine.os == "${item['machine_os'][0]}"`);
            } else {
              props.dispatch('');
            }
          })
    ...
    const click = vl.selectMulti().encodings('y').name('machine_os');
    ...
    const spec = vl.markBar({ tooltip: true })
      ...
      .encode(
        ...
        vl.x().sum("count"),
        vl.y().fieldN('machine_os').sort('-x'),
        vl.color().fieldN('type').scale({ range: ['steelblue', 'lightgrey'] }).legend({disable: true}),
        vl.tooltip([vl.fieldN("machine_os"), vl.fieldQ("count")])
      )
      .width(props.width)
      .height(props.height)
      .autosize({ type: 'fit-x'})
