...
import { from, Observable, combineLatest, defer } from 'rxjs';
...
> {
  ...
  constructor(public readonly execution: ExecutionParams) {
    ...
    this.context = {
      ...
      isPartial: () => execution.params.partial,
  ...
  public start(input: Input = null as any) {
    ...
    this.firstResultFuture = this.invokeChain(this.state.get().ast.chain, input);
    return this.firstResultFuture;
  }

  invokeChain(chainArr: ExpressionAstFunction[], input: unknown): Observable<any> {
    return defer(() =>
      (from([input]).pipe as any)(
        ...chainArr.map((link) =>
          switchMap((currentInput) => {
            ...
            return this.resolveArgs(fn, currentInput, fnArgs).pipe(
              switchMap((resolvedArgs) => {
                return this.invokeFunction(fn, currentInput, resolvedArgs);
              }),
              map((output) => {
  ...
  invokeFunction(
    ...
    const fnResult = fn.fn(normalizedInput, args, this.context);
    const observable: Observable<any> = fnResult.subscribe
      ? fnResult
      : fnResult.then
      ? from(fnResult)
      : from([fnResult]);
    ...
    return defer(() => {
      ...
      const resolveArgFns = mapValues(argAstsWithDefaults, (asts, argName) => {
        return asts.map((item: ExpressionAstExpression) => {
          return (subInput = input) => {
            return this.interpret(item, subInput).pipe(
              map((output) => {
      ...
      const resolvedArgValuesObservable = combineLatest(
        ...argNames.map((argName) => {
          ...
          if (!argDefs[argName].resolve) return from([interpretFns]);
          return combineLatest(...interpretFns.map((fn: any) => fn()));
        })
      );
