...
export class VisualizeEmbeddable
  ...
  public getQuickEditor = async () => {
    if (!this.vis.type.getQuickEditor) {
      return;
    }

    const quickEdit = await this.vis.type.getQuickEditor();

    if (quickEdit) {
      const { component: Component, onSave } = quickEdit;

      const onChange = async (savedVis: SerializedVis) => {
        const visState = savedVis;
        const vis = await createVisAsync(visState.type, visState);
        this.vis = vis;
        this.updateInput({ savedVis });
        this.updateHandler();
      };

      // On close, whether we're backing out of changes or confirming,
      // rebuild the vis state one last time and force an update
      const onClose = async () => {
        const visState = this.input.savedVis!;
        const vis = await createVisAsync(visState.type, visState);
        this.vis = vis;
      ...
      return {
        element: <Component vis={this.input.savedVis} onChange={onChange} />,
        onSave,
        onClose,
      };
    }
