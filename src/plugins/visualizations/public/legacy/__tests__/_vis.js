...
import { npStart } from '../../../../../legacy/ui/public/new_platform';
...
describe('Vis Class', function() {
  ...
  beforeEach(function() {
    vis = npStart.plugins.visualizations.createVis(indexPattern, stateFixture);
  });
  ...
  const verifyVis = function(vis) {
    ...
    expect(vis.type).to.eql(npStart.plugins.visualizations.get('pie'));
  ...
  describe('setState()', function() {
    it('should set the state to defaults', function() {
      const vis = npStart.plugins.visualizations.createVis(indexPattern);
      expect(vis).to.have.property('type');
      expect(vis.type).to.eql(npStart.plugins.visualizations.get('histogram'));
      expect(vis).to.have.property('aggs');
