...
export const showWarningMessageIfTimelionSheetWasFound = async (
  ...
  if (count > 0) {
    logger.warn(
      'Deprecated since 7.0, the Timelion app will be removed in the latest 7.x minor. To continue using your Timelion worksheets, migrate them to a dashboard. See https://www.elastic.co/guide/en/kibana/current/create-panels-with-timelion.html.'
    );
  }
};
...
export async function getDeprecations({
  ...
  if (count > 0) {
    deprecations.push({
      message: `You have ${count} Timelion worksheets. The Timelion app will be removed in in the latest 7.x minor. To continue using your Timelion worksheets, migrate them to a dashboard.`,
      documentationUrl:
