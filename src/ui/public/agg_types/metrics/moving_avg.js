  ...
  return new DerivativeAggType({
    ...
    makeLabel: function (aggConfig) {
      if (aggConfig.params.customMetric) {
        ...
        if (label.includes('Moving average of ')) {
          label = '2. moving average of ' + label.substring('Moving average of '.length);
        }
        else if (label.includes('moving average of ')) {
          label = (parseInt(label.substring(0, 1)) + 1) + label.substring(1);
        }
        else {
          label = 'Moving average of ' + label;
        }
        return label;
      }
      const metric = aggConfig.vis.aggs.find(agg => agg.id === aggConfig.params.buckets_path);
      return 'Moving average of ' + metric.makeLabel();
    },
    params: [
      {
        name: 'customMetric',
