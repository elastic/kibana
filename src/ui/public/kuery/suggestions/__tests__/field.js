...
describe('Kuery field suggestions', function () {
  ...
  describe('with one index pattern', () => {
  ...
  describe('with multiple index patterns', () => {
    beforeEach(ngMock.inject(function (Private) {
      indexPattern = Private(StubbedLogstashIndexPatternProvider);
    ...
    it('should dedup suggestions from multiple index patterns', () => {
      const prefix = '.';
      const suffix = '';
      const suggestions = getSuggestions({ prefix, suffix });
      expect(suggestions.filter(({ text }) => text === 'machine.os ').length).to.be(1);
    });
  });
});
