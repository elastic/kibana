...
import {
  getFieldValueObject, makeFiltersBoolean, isRemoveFilter,isAltKeyPressed
} from '../multiselect';
...
describe('multiselect functions', function () {
  ...
  describe('function isAltKeyPressed', function () {
    it('should return true because ctrl key pressed', function () {
      let res = isAltKeyPressed({ ctrlKey:true });
      expect(res).to.be(true);
      res = isAltKeyPressed({ keyCode:18 });
      expect(res).to.be(true);
    });

    it('should return false because CtrlKey not pressed', function () {
      const res = isAltKeyPressed({ });
      expect(res).to.be(false);
    });
