...
function generateUrls({ version, plugin }: { version: string; plugin: string }) {
  return [
    plugin,
...
export function parseMilliseconds(val: string | number) {
...
export function parse(
  command: string,
  options: { timeout?: number; quiet?: boolean; silent?: boolean; config?: string },
  kbnPackage: { version: string }
) {
  const settings = {
    ...
    workingPath: '',
    tempArchiveFile: '',
    plugins: [] as Array<{ id: string; stripPrefix: string; kibanaVersion: string }>,
  };
