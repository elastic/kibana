 ...
 */
function assertValidPackageName(plugin: { id: string }) {
...
export async function getPackData(
  settings: {
    ...
    plugins: Array<{ id: string; stripPrefix: string; kibanaVersion: string }>;
  },
  logger: Logger
) {
  let packages: Array<{ id: string; stripPrefix: string; kibanaVersion: string }> = [];
