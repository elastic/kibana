...
export function _getFilePath(filePath: string) {
...
export function _checkFilePathDeprecation(sourceUrl: string, logger: Logger) {
...
export function _downloadSingle(
  settings: { tempArchiveFile: string; timeout: number },
  logger: Logger,
  sourceUrl: string
) {
...
export function download(
  settings: { urls: string[]; tempArchiveFile: string; timeout: number },
  logger: Logger
) {
  ...
  function tryNext(): Promise<void> {
    ...
    return _downloadSingle(settings, logger, sourceUrl).catch((err: any) => {
