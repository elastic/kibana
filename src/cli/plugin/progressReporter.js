...
module.exports = function (logger) {
  ...
  function init(totalSize) {
    ...
    let totalDesc = _totalSize || 'unknown number of';
  ...
  function progress(size) {
    ...
    _runningTotal += size;
    let dotCount = Math.round(_runningTotal / _totalSize * 100 / 5);
    if (dotCount > 20) dotCount = 20;
    for (let i = 0; i < (dotCount - _oldDotCount); i++) {
      logger.log('.', true);
    }
    _oldDotCount = dotCount;
  }
