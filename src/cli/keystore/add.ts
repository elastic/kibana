...
export async function add(keystore: Keystore, key: string, options: any = {}) {
  ...
  if (options.stdin) {
    ...
    value = (await question(`Enter value for ${key}`, { mask: '*' })) as string;
  }
...
export function addCli(program: any, keystore: Keystore) {
  program
    .command('add <key>')
    .description(
