...
import { confirm, question } from './lib/utils';
import { Keystore } from './lib';

export async function create(keystore: Keystore, options: any = {}) {
  ...
  logger.log(`Created Kibana keystore in ${(keystore as any).path}`);
}

export function createCli(program: any, keystore: Keystore) {
  program
    .command('create')
