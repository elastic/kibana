...
export class EncryptionConfig {
  #config = load(readFileSync(join(getConfigDirectory(), 'kibana.yml'), 'utf8'));
  #encryptionKeyPaths = [
    'xpack.encryptedSavedObjects.encryptionKey',
    'xpack.reporting.encryptionKey',
    'xpack.security.encryptionKey',
  ];
  #encryptionMeta: Record<string, EncryptionMeta> = {
    'xpack.encryptedSavedObjects.encryptionKey': {
  ...
  _getEncryptionKey(key: string) {
    return get(this.#config as object, key);
  }

  _hasEncryptionKey(key: string) {
    return !!get(this.#config as object, key);
  }
