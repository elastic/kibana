...
- Using `@` in test titles (e.g., `@timestamp`, `@elastic`). Playwright treats `@word` in test/describe titles as tags. Do not use `@` followed by word characters in titles. This causes Scout tag validation to fail with "Unsupported tag(s) found". Rephrase the title instead (e.g., use `timestamp field` instead of `@timestamp`).
