...
- **Oversized specs**: planned Scout files that mix roles/flows or would exceed ~4–5 UI tests (warns at 8 UI / 15 API via `@kbn/eslint/scout_max_tests_per_file`). Split them in the plan, not after the fact.
