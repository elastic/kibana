...
4. **Catalog EUI component interactions**: list every EUI component the tests interact with directly (combo boxes, data grids, selectable lists, etc.) so the executor can use a published Component Object through `page.components.*`, or identify where an existing `Eui*Wrapper` class is temporarily required because no equivalent Component Object exists
