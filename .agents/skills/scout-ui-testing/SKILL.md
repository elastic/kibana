...
- **Keep spec files small**: one role + one flow per file; roughly 4–5 short tests, or 2–3 when a scenario runs longer than ~30s. `@kbn/eslint/scout_max_tests_per_file` warns above 8 UI tests. Split the file rather than disabling the rule — skipping one flake should not drop a large suite.
