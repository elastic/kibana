...
Sorted by estimated complexity (simple → complex).
...
- `<file>` — **Reason**: <why this test is no longer needed, and what coverage is lost (if any)>
...
- `<test>` — **Reason**: <what about this test requires a clean server>
...
| # | Proposed spec | From FTR file | Complexity | Effort | Notes |
|---|--------------|--------------|------------|--------|-------|
| 1 | `feature_controls_viewer.spec.ts` | `feature_controls.ts` | simple | 1h | Built-in viewer role, existing page objects |
| 2 | `preserve_url.spec.ts` | `preserve_url.ts` | simple | 1h | Pure navigation test |
...
| # | Proposed spec | From FTR file | Complexity | Effort | Notes |
|---|--------------|--------------|------------|--------|-------|
| 3 | `panel_actions.spec.ts` | `panel_actions.ts` | medium | 3h | Needs new panel page object |
| 4 | `filter_bar.spec.ts` | `filter_bar.ts` | medium | 2h | Needs `data-test-subj` added to filter pills in source |

**Batch effort**: ~5h
**Dependencies**: panel page object (created in this batch), `data-test-subj` additions to source
**Blockers**: source code changes needed for `filter_bar`

### Batch N — Complex / blocked

| # | Proposed spec | From FTR file | Complexity | Effort | Notes |
|---|--------------|--------------|------------|--------|-------|
| N | `snapshot_restore.spec.ts` | `snapshot_restore.ts` | complex | 5h | Needs custom server config for `path.repo` |
