...
| `@elastic/eui/no-nested-copy-tooltip` | [`components/copy_tooltip.md`](components/copy_tooltip.md) | No autofix — `EuiToolTip` `content` and `beforeMessage` cannot be merged automatically; pick the surviving wording. Render-prop root reached via a helper/variable/`map` is not analyzed. |
...
| `@elastic/eui/tooltip-button-icon-wrap` | [`components/tooltip_icon.md`](components/tooltip_icon.md) · [`components/copy_tooltip.md`](components/copy_tooltip.md) | `{...props}` without an explicit `title` is silently skipped — verify the spread does not omit a tooltip. No `aria-label` on the button → autofix can't run; supply both `aria-label` and `EuiToolTip` `content`. Not reported inside an `EuiCopy` with a meaningful `beforeMessage` (that tooltip comes from `EuiCopy`) — do **not** add a wrapper there. |
