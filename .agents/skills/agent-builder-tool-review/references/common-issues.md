...
All issues referenced are from `elastic/agentic-interface-program`.

## Availability / Scoping (C2)

**Cases tools not gated in Search projects (#154):** Cases tools were registered
...
**Context Engine tools registered when feature is off (#156):** Context Engine
...
**Streams tools on wrong project types (#157, #160):** Streams tools were
...
**Render directives leak to MCP clients (#148):** Tool return values included
...
**Internal identifiers in error messages (#153):** A query tool returned
...
**Workflow tool bypasses destructive controls (#145):** `execute_workflow` ran
...
**MCP bypasses confirmation dialogs (#147):** Streams write tools had
...
**Separate read and write MCP tools (#114):** A combined tool handled both read
...
**Descriptions script assistant behavior (#149):** Tool descriptions included
...
**Extraneous conversation data in parameters (#152):** A tool's parameter
...
**Duplicated schema per result (#146):** `get_connectors` repeated the same
`stepTypes` block for every connector in the response, massively inflating the
return payload.

**Full document content with no control (#155):** `product_documentation`
...
**Raw Zod validation trace (#143):** `create_partition` didn't pre-check the
...
**Missing annotations (#111):** Tools shipped without MCP annotations entirely.
