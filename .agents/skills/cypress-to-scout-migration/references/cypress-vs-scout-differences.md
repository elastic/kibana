...
| No EUI abstraction | Published EUI Component Objects through `page.components.*` (`comboBox`, `dataGrid`, `superSelect`, `globalToastList`); existing `Eui*Wrapper` classes only as compatibility fallbacks |
