...
- **Blaming a prior fix without matching signatures**: before concluding an earlier fix "didn't hold", confirm the recurrence carries the _same_ error signature (same element/assertion/call site) that fix targeted. A fix can be correct and the test still flaky at a _different_ step — that is a new flake in a too-long test, not a failed fix. In that case prefer reducing UI interactions (API-driven setup/teardown, or splitting the test) over adding the next wait.
