...
- **Counting a pre-fix Cloud build as a recurrence**: a Cloud image trails `main`, so a failure reported soon after a fix merged may have run a checkout that predates the fix. Resolve the run's `Build hash` (see `references/pipelines.md`) and confirm the fix's merge commit is an ancestor before treating the failure as a recurrence — otherwise it is propagation lag, not the fix failing.
