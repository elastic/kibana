...
- **Never recommend a retry inside shared test-framework infrastructure.** Retrying a "transient" failure inside a framework package or shared service (`kbn-test*`, `kbn-ftr-*`, the Scout framework packages) — e.g. wrapping an auth or HTTP call in a bounded retry loop — is the interaction-retry pitfall amplified: it masks the root cause for every suite that shares the code, and it will hold across any flaky-runner verification precisely because it hides the failure. Diagnose why the operation fails (an unreliable service, a missing readiness signal, a product bug) and recommend fixing that — or escalate to the framework's owning team.
- **Recommending framework internals over documented APIs**: don't propose exposing a framework's internal helpers (e.g. adding an internal tag-computation function to `@kbn/scout`'s public exports) when a documented approach exists (the exported `tags` constants). A capability the documented API lacks is a feature request for the framework's owning team, not a gap to bridge in a fix PR.
