...
- **[general]** **Shared environment**: for state the suite assumes absent or leaves behind on shared servers (fixed names/time windows, cluster-wide "no data" assertions, unreverted settings/templates, orphaned resources), see **Critical check 5 (doesn't assume or leak shared-server state)**.
