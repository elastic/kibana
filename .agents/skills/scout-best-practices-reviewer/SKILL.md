...
| 6 | **Spec file stays small enough to skip without a coverage hole** | adds or grows a `*.spec.ts` | Nested `describe`s, more than one role, or unrelated flows in one file → split (one role + one flow per file). UI: more than 5 leaf tests, or more than 3 when scenarios are clearly long (~30s+); treat more than 8 as `major`. API: more than 8 is a split candidate; more than 15 is `major` unless it is clearly one endpoint + one role. `test.step` groups structure inside a test and does not count; `test.each` that is still one behavior counts as one test. Do not re-flag `@kbn/eslint/scout_max_tests_per_file` when it is already an ESLint **error** — but this rule ships as **warn** (CI `--quiet` ignores it), so still comment with a concrete split. A file-level `eslint-disable` of this rule is a `blocker`; split instead. → `docs/extend/testing/scout-best-practices.md#organize-test-suites-by-role-and-user-flow`, `docs/extend/testing/scout-best-practices.md#keep-test-suites-independent`, `docs/extend/testing/parallelism.md` |
...
- **[general]** **Don't circumvent the linter**: CI gates lint *violations*, so don't re-flag those — but it can't see a **suppressed** rule, so read every `eslint-disable` the diff adds. A file-level `/* eslint-disable <rule> */` is a `blocker`; it silences code added later too. An undescribed `// eslint-disable-next-line` is a `major`. With a stated reason (`-- <why>`) it is acceptable only when the rule genuinely can't express the case; if a supported alternative exists, name it. Same bar for hiding a flagged pattern behind a hack, or a sanctioned escape hatch like `dispatchEvent('click')`: both need a documented justification. `@kbn/eslint/scout_max_tests_per_file` ships as **warn** (CI `--quiet` ignores it), so still comment with a split; a file-level disable of that rule is a `blocker`.
...
- **[general]** **Suite size**: one top-level suite per file; one role + one flow. See **Critical check 6**. `@kbn/eslint/scout_max_tests_per_file` warns above 8 UI / 15 API tests.
