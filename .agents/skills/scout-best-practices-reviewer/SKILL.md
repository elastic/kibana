...
- **Flake traps**: avoid `waitForTimeout()` and time-based assertions/retries; rely on auto-waiting + explicit readiness signals. Some locators are forbidden by `@kbn/eslint/scout_no_locators` (e.g. `globalLoadingIndicator`).
