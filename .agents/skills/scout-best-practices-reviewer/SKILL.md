...
| 7 | Generally, test suites should be small | adds or grows a `*.spec.ts` | If possible, split large test suites so each covers one role and one flow. Smaller suites are easier to read, mean skipping one affects fewer tests, and they're easier to balance in CI since they usually (though not always) run faster. → `docs/extend/testing/scout-best-practices.md#organize-test-suites-by-role-and-user-flow`, `docs/extend/testing/scout-best-practices.md#keep-test-suites-independent`, `docs/extend/testing/parallelism.md` |
