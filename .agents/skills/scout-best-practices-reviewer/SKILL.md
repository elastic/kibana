...
- **[general]** **Isolation**: parallel-safe data; resilient primary cleanup in `afterAll`/`afterEach`; targeted idempotent cleanup in `beforeAll` only for deterministic leftovers from interrupted runs; no broad `cleanStandardList()` in `beforeAll`; `cleanStandardList()` as an `afterAll` catch-all after domain-specific cleanup; no reliance on file ordering or shared mutable state.
