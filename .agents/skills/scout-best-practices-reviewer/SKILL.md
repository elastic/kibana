...
- **[general]** **Shared environment**: CI runs configs against shared servers, and Cloud deployments are long-lived — flag fixed literal resource names (namespace per run instead), fixed-timestamp data in a time window other suites likely use too, assertions that data is absent cluster-wide (empty prompts / "no data" redirects — belongs in a unit test), cleanup that deletes the tracking object but not the underlying resource, and unreverted behavior-changing state (settings, feature flags, index templates). See `docs/extend/testing/scout-best-practices.md#expect-a-shared-test-environment` and `#dont-leak-state-into-the-next-suite`.
