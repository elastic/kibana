...
- **[general]** **Don't circumvent the linter**: CI gates lint *violations*, so don't re-flag those — but it can't see a rule that was **suppressed**, which is exactly how bad patterns land. Read every `eslint-disable` the diff adds. Treat a file-level `/* eslint-disable <rule> */` as a `blocker`: it silences the rest of the file, including code added later, and it is never the right scope. Treat an undescribed `// eslint-disable-next-line <rule>` as a `major` and ask for the fix. A single-line disable **with** a stated reason (`-- <why>`) is acceptable only when the rule genuinely can't express the case; if a supported alternative exists, name it. The same applies to swapping a flagged pattern for a hack that hides it from the rule. Sanctioned escape hatches (e.g. `dispatchEvent('click')` for a documented app bug) are fine only with an explicit documented justification.
  - Most common offender: `playwright/no-nth-methods`. A suppressed `.first()` almost always means the locator matched more than one element — the selector is the bug; name the replacement from **Don't select elements by index** in `docs/extend/testing/ui-best-practices.md`. Only a genuinely ordered collection, or elements that are truly indistinguishable in a component the author doesn't own, justify a described `.nth()` / `.last()`, and both need a preceding `toHaveCount` to bound the index.
