...
| 6 | **Spec file stays small enough to skip without a coverage hole** | adds or grows a `*.spec.ts` | Nested `describe`s, more than one role, or unrelated flows in one file → split (one role + one flow per file). `@kbn/eslint/scout_max_tests_per_file` (8 UI / 15 API) is the upper bound; reviewer numbers are the target: UI more than 5 leaf tests, or more than 3 when scenarios are clearly long (~30s+); treat more than 8 as `major`. API: more than 8 is a split candidate; more than 15 is `major` unless it is clearly one endpoint + one role. The rule counts syntactic `test` / `apiTest` / `spaceTest` call sites, not runtime tests — a `for` loop around `test()` is one lint count. Count the loop's data array for the cap, worker balance, and skip blast radius; a 4-row loop that is still one role + one flow is the intended Playwright shape. Only push a split when the generated suite is large or the rows are genuinely different flows. A spec that only calls a shared describe helper (zero local `test()` calls; the tests live in the imported factory) is the same limitation — count the tests the helper registers, not the lint count. `test.step` groups structure inside a test and does not count. This rule ships as **warn** (CI `--quiet` ignores it), so still comment with a concrete split. A file-level `eslint-disable` of this rule is a `blocker`; split instead. → `docs/extend/testing/scout-best-practices.md#organize-test-suites-by-role-and-user-flow`, `docs/extend/testing/scout-best-practices.md#keep-test-suites-independent`, `docs/extend/testing/parallelism.md` |
