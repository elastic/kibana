...
| 5 | **Doesn't assume a clean cluster** | adds or changes a spec or fixture | The suite assumes the cluster holds only its own data, which a shared server (earlier suites, parallel workers, Cloud's out-of-the-box content) can't guarantee: asserts data is *absent* cluster-wide (empty prompt / "no data" redirect / zero-count → move to a lower-level test with mocked data when you can - e.g., RTL component test); or asserts totals or positions over a list it doesn't fully own (`toHaveCount(n)`, first/nth row → narrow the query to the suite's own data, then assert containment or by id/name). These pass locally and alone, then fail rarely and unreproducibly in CI. → `docs/extend/testing/scout-best-practices.md#expect-a-shared-test-environment` |
| 6 | **Leaves the cluster as it found it** | adds or changes a spec, fixture, or setup/teardown hook | The suite leaves state behind for the suites that run after it: uses a fixed literal resource name (index, pipeline, saved object, data view, alert, Fleet config, default UI setting, or other Kibana entity) or a fixed time window other suites likely reuse → namespace per run / per suite; deletes the tracking object but not the underlying resource (e.g. API keys, tasks); or changes settings, feature flags, or index templates without reverting them. These pass locally and alone, then break the suites that run next in CI. Do not flag `cleanStandardList()` solely because a sequential suite uses `test` and the default space. Inspect the Playwright config, fixtures, and cleanup implementation. Check that the call is needed, preserves shared data, and does not replace required domain-specific cleanup. Apply the cleanup guidance in the linked best-practices section, including its limited allowance for before-hook cleanup. Flag broad deletes of non-space-scoped data (ES documents or indices, pipelines, templates, API keys removed by type, tag, or fixed field value), including in shared helpers, since parallel consumers may depend on that data. → `docs/extend/testing/scout-best-practices.md#dont-leak-state-into-the-next-suite` |
| 7 | **Helper lives in the right place** | adds, moves, or renames a page object, UI component object, or API service class (paths listed under Inputs) | Wraps a Kibana component that two or more plugins render but sits in one plugin → belongs in `@kbn/scout` (or the solution package if only that solution renders it). Drives a screen only its own plugin's tests use but sits in `@kbn/scout` → keep it plugin-local. Same class name already exists in another Scout location → duplicate, reuse or consolidate. Renames or removes a `pageObjects.<key>` / `apiServices.<key>` → breaking for every consumer, ask for the key to stay. For consumer counts run `node scripts/scout audit`. → `docs/extend/testing/page-objects.md#scout-page-objects-placement` |
| 8 | **One role + one flow per spec** | adds or changes a `*.spec.ts` | Split a spec when it mixes roles, nested suites, or unrelated flows, or when its duration makes it a worker bottleneck or gives a skip a large blast radius; aim for about a minute per spec. A small generated table or shared describe helper can stay when it remains one flow, but assess all generated tests; `test.step()` does not add a scenario. Treat a worker bottleneck or large skip blast radius as `major`. → `docs/extend/testing/scout-best-practices.md#organize-test-suites-by-role-and-user-flow`, `docs/extend/testing/scout-best-practices.md#keep-test-suites-independent`, `docs/extend/testing/parallelism.md` |
...
- **[general]** **Reuse-first**: prefer existing `pageObjects`, fixtures, and `apiServices`; if adding helpers/page objects, place them per **Critical check 7** and `docs/extend/testing/page-objects.md#scout-page-objects-placement` (plugin vs solution vs `@kbn/scout`) and register via fixtures.
...
- **[general]** **Isolation**: parallel-safe data; resilient primary cleanup in `afterAll`/`afterEach`; use `cleanStandardList()` only when relevant and not redundant, normally in post-test cleanup; justify before-hook use; preserve shared fixtures and clean up resources and settings separately; id/name-based deletes for anything not scoped by a space, also in shared helpers; no reliance on file ordering or shared mutable state.
- **[general]** **Shared environment**: for state the suite assumes absent or leaves behind on shared servers (fixed names/time windows, cluster-wide "no data" assertions, unreverted settings/templates, orphaned resources, or shared Kibana entities like data views, alerts, Fleet config, and default UI settings), see **Critical check 5 (doesn't assume a clean cluster)** and **Critical check 6 (leaves the cluster as it found it)**.
- **[general]** **Suite size**: generally, smaller test suites, preferably following the role + one guideline per file. See **Critical check 8**.
