...
export const PillSelect = ({
  ...
  const picker = (
    <Popover id="canvas__pill-select--popover">
      ...
      <div className="canvas__pill-select--options">
        {difference(options, values)
          .filter(val => val.indexOf(search) > -1)
          .map(opt => (
            <div
              className="canvas__pill-select--option clickable"
              key={opt}
              onClick={() => addPill(opt)}
            >
              {opt}
  ...
  return (
    <div className="canvas__pill-select">
      ...
      <Overlay rootClose onHide={close} show={popover} container={this} target={target}>
        {picker}
      </Overlay>
    </div>
  );
