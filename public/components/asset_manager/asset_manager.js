    ...
    <div className="canvas__asset-manager">
      <div>
        ...
        <p>
          ...
          when you added them. Workpads are limited in size. <strong>You can reclaim space</strong>{' '}
          by deleting assets that you no longer needed. Sorry, I can't tell you which assets are
          currently in use: We're working on that, I promise.
        </p>
        <p>
          I can tell you that the current approximate size of your uncompressed assets is{' '}
          <strong>
            {Math.round(assets.reduce((total, asset) => total + asset.value.length, 0) / 1024)}KB
          </strong>
        </p>
      </div>
