...
- Target end-state for reconciliation is the decided topology model, as updated by **ingest-spec PR #32 (node-output model)**: the topology is a **graph of nodes connected by each node's `output`** (a flat list of next-node ids); **streams are emergent** (a source→destination path read off the graph, not authored or named). Pipelines split into reusable **`pipeline_definitions`** (no output) and in-graph **pipelines** (placements that reference a definition live by name and carry the `output`; metrics per-placement, never aggregated). **Conditions live on routing nodes** (`mode: exclusive | clone`), never on a plain `output`; **destinations are sinks (no `output`)** with a built-in catch-all + optional inner routing node. **Any wiring is allowed** and the routing invariant is a **non-enforced guideline** (`stream-topology-model.md`).
- **Where our prototype sits in that model:** the ADR's managed inputs (`otlp`/`async_bulk`/`prometheus_remote_write`/`syslog`) are the v1 focus and flow through the *managed processing service*. The ADR explicitly describes the **deferred direct-to-ES variants** (`es_otlp`/`bulk`/`es_prometheus_remote_write`) as "*synchronous, ES-only routing, Streamlang transpiled to ES ingest pipelines*" — exactly this prototype's execution mode. Where Streamlang runs for v1 is the **open RFC (João + Andrew C.)** this work feeds. No contradiction: we are proving the deferred direct-to-ES execution layer, and the ADR's routing-invariant downgrade ratifies our "drop the validation gates" stance.
...
- ✅ **DSL strawman drafted, conformed to PR #32** (`streams_graph_dsl.md`, v0.2 node-output model): nodes wired by `output`; **streams emergent (unnamed)**; `pipeline_definitions` (reusable) vs `pipelines` (placements, per-placement metrics); **conditions on routing nodes** (`mode: exclusive | clone`); destinations are sinks with a catch-all. Decisions: **ES path ships `exclusive`-only — `clone` fan-out is an execution-layer limitation, not a model decision** (ES indexes once; fan-out → service); **intermediate retention accepted** (node-local data streams / destination catch-all). Provisional PoC defaults: definition/placement split (now canonical), DAG cycle + soft depth guardrail, conditions on routing nodes only, per-node lifecycle defaulting to cluster default, **streams emergent (no topology name)**.
...
**Authoring model for the prototype:** the graph DSL (`streams_graph_dsl.md`, node-output model) **lowers to N per-node definitions** — each node (source, pipeline placement, routing node, or destination) becomes one graph-stream definition. A node's `output` / routing-node `conditions` lower to that definition's own `routing` edges (one reroute per edge) to arbitrary destinations; an unconditional `output` is an edge with no `where`. This reuses the existing per-stream machinery maximally; the DSL is the authoring surface, per-node definitions are what the state machine consumes. (`mode: clone` fan-out is rejected — not expressible on the ES path.)
...
| 2 | New `StreamActiveRecord` subclass | `state_management/streams/graph_stream.ts` (register in `stream_from_definition.ts`). **Do NOT replicate WiredStream's parent-cascade:** `wired_stream.ts` `doHandleUpsertChange` (≈232–348) auto-creates missing *name-derived* parents and auto-injects disabled routing entries into them, and the delete path cascades to name-children. The graph type has no name-parent — a node creates only itself + its declared routing-edge destinations; no ancestor cascade. | parallel path; `WiredStream` untouched; hierarchy not dragged back in |
...
| 4 | Pipeline generation variant | **Model on `generateClassicIngestPipelineBody` (`generate_ingest_pipeline.ts:113-131`), not the wired generator** — the classic path is already guard-less, ancestor-walk-free, and uses `field_access_pattern: 'flexible'`; it's the closest reference shape. Add the reroute hand-off; reuse `generateReroutePipeline` **via a type-generalizing adapter** (mirror wired `{destination,where,status}` routing shape), fed graph edges. | the verified make-or-break: non-child reroute works; guard-less by construction |
...
- **YAML model generation** with round-trip fidelity — a decided requirement (`canvas-design.md`, GitOps). Note the new path's explicit-per-node config (no inheritance) shapes this schema directly. **Strawman complete (conformed to PR #32): see `streams_graph_dsl.md`** (node-output DSL: `sources`/`pipeline_definitions`/`pipelines`/`routing_nodes`/`destinations`, nodes wired by `output`, conditions on routing nodes, emergent streams) — decisions: ES path ships `exclusive`-only (`clone` fan-out is an execution-layer limitation); intermediate retention accepted; five secondary PoC defaults resolved.
