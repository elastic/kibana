...
import { LegacyPatternLayout } from '../LegacyPatternLayout';
...
test('`createConfigSchema()` creates correct schema.', () => {
  const layoutSchema = LegacyPatternLayout.createConfigSchema(mockSchema);

  const validConfigWithOptional = { kind: 'legacy-pattern' };
...
test('`format()` correctly formats record with full pattern.', () => {
  const layout = new LegacyPatternLayout();
