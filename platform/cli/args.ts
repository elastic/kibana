...
export const description = 'Kibana is an open source (Apache Licensed), browser-based analytics and search dashboard for Elasticsearch.';
export const docs = 'Documentation: https://elastic.co/kibana';

export const options: { [key: string]: ArgvOptions } = {
  config: {
    alias: 'c',
    description: 'Path to the config file, can be changed with the ' +
  ...
  ssl: {
    description: 'Dev only. Specify --no-ssl to not run the dev server using HTTPS',
    type: 'boolean',
    default: true
  },
  'base-path': {
    description: 'Dev only. Specify --no-base-path to not put a proxy with a random base path in front of the dev server',
    type: 'boolean',
    default: true
  },
  watch: {
    description: 'Dev only. Specify --no-watch to prevent automatic restarts of the server in dev mode',
    type: 'boolean',
    default: true
  }
};
...
export const check = (options: { [key: string]: any }) =>
  (argv: { [key: string]: any }) => {
