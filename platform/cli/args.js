...
import type { Options as ArgvOptions } from 'yargs';
  ...
  config: {
    ...
    coerce: (arg: mixed) => {
      ...
      if (Array.isArray(arg)) {
        return arg.map((file: any) => resolve(file));
      }
      return arg;
    },
    requiresArg: true
...
function ensureConfigExists(path) {
  if (!fileExists(path)) {
...
export const check = (options: { [key: string]: any }) =>
  (argv: { [key: string]: any }) => {
    ...
    if (typeof argv.config === 'string') {
      ensureConfigExists(argv.config);
    }
    if (Array.isArray(argv.config)) {
      argv.config.map(ensureConfigExists);
    }
