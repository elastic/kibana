...
module.exports = function createDateAgg(config, tlConfig) {
  ...
  _.each(config.metric, function (metric, i) {
    ...
    if (metric[0] === 'count') {
      ...
      const field = metric[1];
      const metricName = metricType + '(' + field + ')';

      dateAgg.time_buckets.aggs[metricName] = {meta: {type: 'metric', metric: metricType, field: field}};
      dateAgg.time_buckets.aggs[metricName][metricType] = { field: field };
    } else {
