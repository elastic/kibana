...
export const demoprices = {
  ...
  args: {
    bucket: {
      types: ['string'],
      help:
        "Time interval to bucket by. Second and minute will produce a lot of rows, so maybe don't use those.",
  ...
  fn: (context, args) => {
    const getRows = bucket => {
      ...
      const mappedRows = rows.reduce(
        (acc, row, i) => {
          const adjustedTime = moment(row.time)
            .startOf(bucket)
            .toISOString();
