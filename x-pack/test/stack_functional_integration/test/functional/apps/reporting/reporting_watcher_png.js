...
export default ({ getService, getPageObjects }) => {
  ...
  describe('watcher app', () => {
    ...
    describe('PNG Reporting watch', () => {
      ...
      before(async () => {
        esClient = new Client({
          node: build(servers.elasticsearch).toString(),
        });
    ...
    it.only('should be successful and update revision', async () => {
      await PageObjects.common.sleep(200000);
      const index = '.watcher-history*';
      const historySearch = search(index, watchId);
      const searchResponse = await historySearch(esClient);
      // log.debug(`\n### historySearch ...\n${index}: \n\t${pretty(searchResponse)}`);\
      const getResponse = await esClient.get({
        index: '.watches',
        id: watchId,
      });
      await PageObjects.common.tryForTime(90000, async function tryToEmail () {
    ...
    it.skip('should be successful and update revision', function() {
...
const build = ({ protocol, auth, hostname, port }) =>
