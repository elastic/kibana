...
export default class VisualizePage {
  ...
  filterVisByNameSlowTyping(vizName) {
  ...
  openSavedVisualization(vizName) {
    const self = this;
    // this try loop is a work-around for https://github.com/elastic/kibana/issues/14688
    return PageObjects.common.tryForTime(60000, function () {
  ...
  async getLegendLabelsList() {
 	...
 	await this.remote
	.setFindTimeout(defaultFindTimeout)
	.findAllByClassName('legend-value-title')
	.getProperty('textContent')
	.then(function (texts) {
  array = texts.map(s => s.trim());
});
