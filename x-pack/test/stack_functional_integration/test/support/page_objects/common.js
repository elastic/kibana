...
export default class Common {
  ...
  navigateToApp(appName, username=config.servers.kibana.username, password=config.servers.kibana.password) {
    const self = this;
    const appUrl = getUrl.noAuth(config.servers.kibana, config.apps[appName]);
    self.debug('navigating to ' + appName + ' url: ' + appUrl + ' username: ' + username);
    ...
    function navigateTo(url) {
      return self.try(function () {
        ...
        // })
        .then(function () {
          self.debug('wait for kibanaWelcomeLogo deleted');
          return self.remote.setFindTimeout(120000).waitForDeletedByClassName('kibanaWelcomeLogo');
        })
        .then(function () {
        ...
        .then(function (currentUrl) {
          ...
          if (loginPage) {
            self.debug(`Found loginPage.  Login as username = ${username} / ${password}`);
            return PageObjects.shield.login(username, password)
            .then(function () {
              return self.remote.getCurrentUrl();
            });
          } else {
