...
export async function SessionViewCypressCliTestRunner({ getService }: FtrProviderContext) {
  ...
  await withProcRunner(log, async (procs) => {
    await procs.run('cypress', {
      cmd: 'yarn',
      args: ['cypress:run'],
      cwd: resolve(__dirname, '../../plugins/session_view'),
      env: {
        FORCE_COLOR: '1',
...
export async function SessionViewCypressCliFirefoxTestRunner({
  getService,
}: FtrProviderContext) {
  const log = getService('log');
  const config = getService('config');
...
export async function SessionViewCypressCcsTestRunner({ getService }: FtrProviderContext) {
  const log = getService('log');
...
export async function SessionViewCypressVisualTestRunner({ getService }: FtrProviderContext) {
  const log = getService('log');
  const config = getService('config');
...
export async function SessionViewCypressUpgradeCliTestRunner({
  getService,
}: FtrProviderContext) {
  const log = getService('log');
  const config = getService('config');
