...
export default function ApiTest({ getService }: FtrProviderContext) {
  ...
  const url = `/api/apm/services/opbeans-java/transactions/distribution?${qs.stringify({
  ...
  describe('Transaction distribution', () => {
    ...
    describe('when data is loaded', () => {
      ...
      it('returns transactions with some hits', () => {
        expect(response.body.buckets.some((bucket: any) => bucket.count > 0)).to.be(true);
      });

      it('returns transactions with some samples', () => {
