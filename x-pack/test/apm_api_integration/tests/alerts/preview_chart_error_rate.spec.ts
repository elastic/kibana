...
export default function ApiTest({ getService }: FtrProviderContext) {
  ...
  registry.when(`with data loaded`, { config: 'basic', archives: [archiveName] }, () => {
    ...
    it('transaction_error_rate with group by on transaction name', async () => {
      ...
      expect(response.status).to.be(200);
      expect(response.body.errorRateChartPreview.length).to.equal(12);
      expect(
        response.body.errorRateChartPreview.map(
          (item: { name: string; data: Array<{ x: number; y: number | null }> }) => item.name
        )
    ...
    it('transaction_error_rate with empty service name, transaction name, transaction type and group by on transaction name', async () => {
      ...
      expect(
        ...
        'opbeans-python_production_celery_opbeans.tasks.update_stats',
        'opbeans-ruby_production_request_Rack',
        'opbeans-node_testing_request_GET /api',
        'opbeans-go_testing_request_GET /api/products/:id',
        'opbeans-go_testing_request_GET /api/products/:id/customers',
      ]);
    });
  });
}
