  ...
  registry.when(
    ...
    () => {
      it('returns the correct data', async () => {
        ...
        expectSnapshot(roundNumber(impact)).toMatchInline(`"82.26"`);
      });
      ...
      describe('returns data with previous period', async () => {
        ...
        it('returns correct impact data', () => {
          ...
          expectSnapshot(roundNumber(currentPeriodFirstItem.impact)).toMatchInline(`"89.10"`);
          expectSnapshot(roundNumber(previousPeriodFirstItem.impact)).toMatchInline(`"44.59"`);
        });
      });
    }
  );
