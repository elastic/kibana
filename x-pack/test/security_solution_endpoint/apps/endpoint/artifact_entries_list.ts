...
export default ({ getPageObjects, getService }: FtrProviderContext) => {
  const pageObjects = getPageObjects(['common', 'blocklist', 'hostIsolationExceptions']);
  ...
  describe('For each artifact list under management', function () {
    ...
    interface ArtifactResponseType {
      _index: string;
      _id: string;
      _score: number;
    ...
    const testsData = [
      {
        ...
        pageObject: pageObjects.blocklist,
        fleetArtifact: {
      ...
      },
      {
        ...
        pageObject: pageObjects.hostIsolationExceptions,
        fleetArtifact: {
    ...
    for (const testData of testsData) {
      describe(`When on the ${testData.title} entries list`, function () {
        before(async () => {
          ...
          await testData.pageObject.navigateToList();
        });
        after(async () => {
        ...
        it(`should be able to update an existing ${testData.title} entry`, async () => {
          ...
          // Opens edit flyout
          await testData.pageObject.clickCardActionMenu();
