...
type ExpectedStatus = 'success' | 'forbidden_types' | 'forbidden';

const {
  DEFAULT: { spaceId: DEFAULT_SPACE_ID },
} = SPACES;

export interface ImportAcrossSpaceTestDefinition extends TestDefinition {
  request: Array<{ type: string; id: string; originId?: string; namespaces?: string[] }>;
  overwrite: boolean;
  createNewCopies: boolean;
}

export type ImportAcrossSpaceTestSuite = TestSuite<ImportAcrossSpaceTestDefinition>;

export interface ImportAcrossSpaceTestCase extends TestCase {
  originId?: string;
...
export const getTestCases = (
...
): Record<string, ImportAcrossSpaceTestCase> => ({
  ...CASES,
  CONFLICT_1_OBJ: Object.freeze({
    type: 'sharedtype',
    id: `${CID}1`,
    expectedNamespaces: [spaceId],
  }),
  ...
  CONFLICT_4_OBJ: Object.freeze({
    type: 'sharedtype',
    id: `${CID}4`,
    expectedNewId: `${CID}4a`,
    expectedNamespaces: [spaceId],
  }),
  NEW_SINGLE_NAMESPACE_OBJ: Object.freeze({
    type: 'dashboard',
    id: 'new-dashboard-id',
    expectedNamespaces: [spaceId],
  }),
  NEW_MULTI_NAMESPACE_OBJ: Object.freeze({
    type: 'sharedtype',
    id: 'new-sharedtype-id',
    expectedNamespaces: [spaceId],
  }),
  NEW_NAMESPACE_AGNOSTIC_OBJ: Object.freeze({
    type: 'globaltype',
 ...
 */
const createRequest = ({ type, id, originId, expectedNamespaces }: ImportAcrossSpaceTestCase) => ({
  type,
  id,
  ...(originId && { originId }),
  ...(expectedNamespaces && { namespaces: expectedNamespaces }),
});
...
export function importAcrossSpaceTestSuiteFactory(
  es: KibanaClient,
  esArchiver: any,
  ...
  const expectResponseBody = (
    testCases: ImportAcrossSpaceTestCase | ImportAcrossSpaceTestCase[],
    ...
    if (expectedStatus === 'forbidden_types') {
      ...
      for (let i = 0; i < expectedSuccesses.length; i++) {
        const { type, id, successParam, expectedNewId, expectedNamespaces } = expectedSuccesses[i];
        ...
        if (!singleRequest || overwrite || createNewCopies) {
          ...
          const { _source } = await expectResponses.successCreated(
            es,
            expectedNamespaces?.length ? expectedNamespaces[0] : spaceId,
            type,
            destinationId ?? id
          );
  ...
  const createTestDefinitions = (
  ...
  ): ImportAcrossSpaceTestDefinition[] => {
    const cases = Array.isArray(testCases) ? testCases : [testCases];
    const responseStatusCode = expectedStatus === 'success' ? 200 : 403;
    const {
      overwrite = false,
    ...
    if (!singleRequest) {
      ...
      return cases.map((x) => ({
        ...
        responseBody:
          responseBodyOverride ||
          expectResponseBody(x, expectedStatus, false, overwrite, createNewCopies, spaceId),
        overwrite,
        createNewCopies,
      }));
    }
    ...
    return [
      {
        ...
        responseBody:
          responseBodyOverride ||
          expectResponseBody(cases, expectedStatus, true, overwrite, createNewCopies, spaceId),
        overwrite,
        createNewCopies,
      },
    ];
  ...
  const makeImportAcrossSpaceTest = (describeFn: Mocha.SuiteFunction) => (
    description: string,
    definition: ImportAcrossSpaceTestSuite
  ) => {
      ...
      for (const test of tests) {
        it(`should return ${test.responseStatusCode} ${test.title}`, async () => {
          ...
          const query = test.overwrite
            ...
            : '';
          await supertest
            .post(`${getUrlPrefix(spaceId)}/api/saved_objects/_import_across_space${query}`)
  ...
  const addTests = makeImportAcrossSpaceTest(describe);
  // @ts-ignore
  addTests.only = makeImportAcrossSpaceTest(describe.only);
