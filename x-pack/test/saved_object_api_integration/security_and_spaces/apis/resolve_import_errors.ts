...
export default function ({ getService }: FtrProviderContext) {
  ...
  const createTests = (overwrite: boolean, duplicate: boolean, spaceId: string) => {
    ...
    return {
      unauthorized: [
        createTestDefinitions(group1Importable, true, overwrite, duplicate, { spaceId }),
        ...
        createTestDefinitions(group2, true, overwrite, duplicate, { spaceId, singleRequest }),
      ].flat(),
      authorized: [
        createTestDefinitions(group1All, false, overwrite, duplicate, { spaceId, singleRequest }),
        createTestDefinitions(group2, false, overwrite, duplicate, { spaceId, singleRequest }),
      ].flat(),
    };
  };
  ...
  describe('_resolve_import_errors', () => {
    getTestScenarios([
      ...
      const suffix =
        ` within the ${spaceId} space` + overwrite
          ? ' with overwrite enabled'
          : duplicate
          ? ' with duplicate enabled'
          : '';
      const { unauthorized, authorized } = createTests(overwrite, duplicate, spaceId);
