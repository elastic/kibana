...
import {
  importAcrossSpaceTestSuiteFactory,
  getTestCases,
  ImportAcrossSpaceTestDefinition,
} from '../../common/suites/import_across_space';
...
const createNewCopiesTestCases = (spaceId: string) => {
...
const createTestCases = (overwrite: boolean, spaceId: string) => {
  ...
  const group1Importable = [
    ...
    { ...CASES.SINGLE_NAMESPACE_SPACE_1, ...fail409(!overwrite) },
    { ...CASES.SINGLE_NAMESPACE_SPACE_2, ...fail409(!overwrite) },
...
export default function ({ getService }: FtrProviderContext) {
  ...
  const { addTests, createTestDefinitions } = importAcrossSpaceTestSuiteFactory(
    es,
    esArchiver,
    supertest
  );
  const createTests = (overwrite: boolean, createNewCopies: boolean, spaceId: string) => {
    ...
    if (createNewCopies) {
      const { importable, nonImportable, all } = createNewCopiesTestCases(spaceId);
      return {
        unauthorized: [
          ...createTestDefinitions(importable, 'forbidden', { createNewCopies, spaceId }),
          ...createTestDefinitions(nonImportable, 'forbidden', {
            createNewCopies,
            spaceId,
            singleRequest,
          }),
        ...
        authorized: [
          ...createTestDefinitions(all, 'success', {
            createNewCopies,
            spaceId,
            singleRequest,
          }),
    ...
    return {
      unauthorized: [
        ...createTestDefinitions(group1Importable, 'forbidden', { overwrite, spaceId }),
        ...createTestDefinitions(group1NonImportable, 'forbidden', {
          overwrite,
          spaceId,
          singleRequest,
        }),
        ...createTestDefinitions(group2, 'forbidden', { overwrite, spaceId, singleRequest }),
        ...createTestDefinitions(group3, 'forbidden', { overwrite, spaceId, singleRequest }),
        ...createTestDefinitions(group4, 'forbidden', { overwrite, spaceId, singleRequest }),
      ],
      authorized: [
        ...createTestDefinitions(group1All, 'success', { overwrite, spaceId, singleRequest }),
        ...createTestDefinitions(group2, 'success', { overwrite, spaceId, singleRequest }),
        ...createTestDefinitions(group3, 'success', { overwrite, spaceId, singleRequest }),
        ...createTestDefinitions(group4, 'success', { overwrite, spaceId, singleRequest }),
      ],
    };
  };

  describe('_import_across_space', () => {
    getTestScenarios([
      [false, false],
      [false, true],
      [true, false],
      ...
      const _addTests = (user: TestUser, tests: ImportAcrossSpaceTestDefinition[]) => {
