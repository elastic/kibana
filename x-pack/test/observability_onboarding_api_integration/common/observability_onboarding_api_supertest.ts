...
export function createObservabilityOnboardingApiClient(st: supertest.Agent) {
  return async <TEndpoint extends APIEndpoint>(
    ...
    if (res?.status !== 200) {
      throw new ObservabilityOnboardingApiError(res, endpoint);
    }
...
export type ObservabilityOnboardingApiSupertest = ReturnType<
  typeof createObservabilityOnboardingApiClient
>;

export class ObservabilityOnboardingApiError extends Error {
  res: ApiErrorResponse;

  constructor(res: request.Response, endpoint: string) {
    super(
      `Unhandled ObservabilityOnboardingApiError.
        Status: "${res.status}"
        Endpoint: "${endpoint}"
