...
const SECURITY_PACKAGES_ROUTE = `${INGEST_API_EPM_PACKAGES}/synthetics-0.0.3`;
...
export function SyntheticsPolicyProvider({ getService }: FtrProviderContext) {
  ...
  const retrieveSyntheticsPackageInfo = (() => {
    ...
    return () => {
      if (!apiRequest) {
        log.info(`Setting up call to retrieve Synthetics package from ${SECURITY_PACKAGES_ROUTE}`);
  ...
  return {
    ...
    async getEndpointPkgKey() {
      return pkgKeyFromPackageInfo((await retrieveSyntheticsPackageInfo())!);
    },
    ...
    async createPolicy(): Promise<PolicyTestResourceInfo> {
      ...
      // Retrieve the Synthetics package information
      const endpointPackageInfo = await retrieveSyntheticsPackageInfo();
    ...
    // /**
    //  * Deletes all policies within an agent policy
    //  * @param name
    //  */
    // async clearPoliciesFromAgentPolicy(name: string) {
    //   let packagePolicyList: GetPackagePoliciesResponse['items'];
    //   try {
    //     const {
    //       body: packagePoliciesResponse,
