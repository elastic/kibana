      ...
      describe('#Permissions: Read Only User - Single Host Flyout', () => {
        describe('Dashboards Tab', () => {
          before(async () => {
            ...
            const dashboardsTabSearchParams = `?_a=(dateRange:(from:%27${DATE_WITH_HOSTS_DATA_FROM}%27,to:%27${DATE_WITH_HOSTS_DATA_TO}%27),filters:!(),limit:100,panelFilters:!(),query:(language:kuery,query:%27%27))&controlPanels=(cloud.provider:(fieldName:cloud.provider,grow:!f,order:1,title:%27Cloud%20Provider%27,type:optionsListControl,width:medium),host.os.name:(fieldName:host.os.name,grow:!f,order:0,title:%27Operating%20System%27,type:optionsListControl,width:medium),service.name:(fieldName:service.name,grow:!f,order:2,title:%27Service%20Name%27,type:optionsListControl,width:medium))&tableProperties=(detailsItemId:host-1-Linux,pagination:(pageIndex:0,pageSize:10),sorting:(direction:desc,field:alertsCount))&assetDetails=(dateRange:(from:%27${DATE_WITH_HOSTS_DATA_FROM}%27,to:%27${DATE_WITH_HOSTS_DATA_TO}%27),name:host-1,tabId:dashboards)`;
