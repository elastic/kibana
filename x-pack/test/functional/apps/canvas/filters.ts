...
export default function canvasFiltersTest({ getService, getPageObjects }: FtrProviderContext) {
  ...
  describe('filters', function () {
    ...
    it('filter updates when dropdown is changed', async () => {
      ...
      const startingMatchFilters = await PageObjects.canvas.getFiltersFromDebug({ type: 'term' });
      ...
      await retry.try(async () => {
        const matchFilters = await PageObjects.canvas.getFiltersFromDebug({ type: 'term' });
    ...
    it('filter updates when time range is changed', async () => {
      ...
      const startingTimeFilters = await PageObjects.canvas.getFiltersFromDebug({ type: 'range' });
      ...
      await retry.try(async () => {
        const timeFilters = await PageObjects.canvas.getFiltersFromDebug({ type: 'range' });
