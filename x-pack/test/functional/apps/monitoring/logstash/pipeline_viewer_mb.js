...
export default function ({ getService, getPageObjects }) {
  ...
  describe('Logstash pipeline viewer mb', () => {
    ...
    it('displays pipelines inputs and ouputs', async () => {
      const { inputs, outputs } = await pipelineViewer.getPipelineDefinition();

      expect(inputs).to.eql([{ name: 'stdin', metrics: ['62.5 e/s emitted'] }]);
      expect(outputs).to.eql([
        { name: 'elasticsearch', metrics: ['0%', '94.86 ms/e', '62.5 e/s received'] },
        { name: 'stdout', metrics: ['0%', '0 ms/e', '62.5 e/s received'] },
      ]);
    });
  });
}
