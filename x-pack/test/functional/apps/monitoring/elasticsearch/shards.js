    ...
    describe('Shard Allocation Per Node', () => {
      ...
      it('master-data node with 20 indices and 38 shards', async () => {
        ...
        expect(await shards.getNodeAllocation('.monitoring-es-6-2017.10.05')).to.eql({
          ...
          shards: [
            { classification: 'shard monShard replica initializing 0', tooltip: 'Initializing' },
          ],
          status: 'yellow',
        });
        ...
        expect(await shards.getNodeAllocation('avocado-tweets-2017.09.30')).to.eql({
          ...
          shards: [
            { classification: 'shard monShard replica started 0', tooltip: 'Started' },
            { classification: 'shard monShard primary started 1', tooltip: 'Started' },
            { classification: 'shard monShard replica started 2', tooltip: 'Started' },
            { classification: 'shard monShard primary started 3', tooltip: 'Started' },
            { classification: 'shard monShard primary started 4', tooltip: 'Started' },
          ],
          status: 'green',
        });
        expect(await shards.getNodeAllocation('relocation_test')).to.eql({
          ...
          shards: [
            {
              classification: 'shard monShard primary relocating 0',
              tooltip: 'Relocating',
            },
          ],
          status: 'green',
        ...
        expect(await shards.getNodeAllocation('phone-home')).to.eql({
          ...
          shards: [
            { classification: 'shard monShard primary started 0', tooltip: 'Started' },
            { classification: 'shard monShard primary started 2', tooltip: 'Started' },
            { classification: 'shard monShard primary started 3', tooltip: 'Started' },
            { classification: 'shard monShard primary started 4', tooltip: 'Started' },
          ],
          status: 'green',
    ...
    describe('Shard Allocation Per Index', () => {
      ...
      it('green status index with full shard allocation', async () => {
        ...
        expect(await shards.getAssignedIndexAllocationByNode('jUT5KdxfRbORSCWkb5zjmA')).to.eql({
          ...
          shards: [
            { classification: 'shard monShard primary started 0', tooltip: 'Started' },
            { classification: 'shard monShard replica started 1', tooltip: 'Started' },
            { classification: 'shard monShard replica started 2', tooltip: 'Started' },
            { classification: 'shard monShard primary started 3', tooltip: 'Started' },
            { classification: 'shard monShard primary started 4', tooltip: 'Started' },
          ],
        ...
        expect(await shards.getAssignedIndexAllocationByNode('xcP6ue7eRCieNNitFTT0EA')).to.eql({
          ...
          shards: [
            ...
            { classification: 'shard monShard primary started 1', tooltip: 'Started' },
            { classification: 'shard monShard primary started 2', tooltip: 'Started' },
            { classification: 'shard monShard replica started 3', tooltip: 'Started' },
            { classification: 'shard monShard replica started 4', tooltip: 'Started' },
          ],
        });
      });
      ...
      it('yellow status index with single unallocated shard', async () => {
        ...
        expect(await shards.getUnassignedIndexAllocation()).to.eql([
          { classification: 'shard monShard replica unassigned 1', tooltip: 'Unassigned' },
        ]);
        ...
        expect(await shards.getAssignedIndexAllocationByNode('bwQWH-7IQY-mFPpfoaoFXQ')).to.eql({
          ...
          shards: [{ classification: 'shard monShard replica started 0', tooltip: 'Started' }],
        });
      });
    });
  });
