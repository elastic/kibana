...
} from '../../screens/threat_intelligence/indicator';
...
export const filterInFromFlyoutBlockItem = () => {
  cy.get(FLYOUT_OVERVIEW_HIGH_LEVEL_BLOCK_ITEM).first().realHover();
  cy.get(FLYOUT_OVERVIEW_TAB_BLOCKS_FILTER_IN_BUTTON).first().click();
};
...
export const filterOutFromFlyoutBlockItem = () => {
  cy.get(FLYOUT_OVERVIEW_HIGH_LEVEL_BLOCK_ITEM).first().realHover();
  cy.get(FLYOUT_OVERVIEW_TAB_BLOCKS_FILTER_OUT_BUTTON).first().click();
};
