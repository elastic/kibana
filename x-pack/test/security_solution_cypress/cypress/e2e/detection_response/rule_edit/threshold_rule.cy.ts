...
describe('Detection threshold rules, edit', { tags: ['@ess', '@serverless'] }, () => {
  ...
  describe('with suppression enabled', () => {
    ...
    it('displays suppress options correctly on edit form', () => {
      ...
      cy.get(ALERT_SUPPRESSION_DURATION_INPUT).eq(0).should('be.enabled').should('have.value', 360);
      cy.get(ALERT_SUPPRESSION_DURATION_INPUT).eq(1).should('be.enabled').should('have.value', 's');
