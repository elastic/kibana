...
describe(
  ...
  () => {
    describe('Reverting prebuilt rules', () => {
      ...
      it('User can revert customized prebuilt rule to current Elastic version', function () {
      ...
      it('User can view diff between current and original Elastic rule versions in flyout', function () {
        patchRule('rule_1', { name: 'Customized prebuilt rule' }); // We want to make this a customized prebuilt rule
        cy.get(RULE_NAME).contains('Customized prebuilt rule').click();
      ...
      it("Revert prebuilt rule button should be disabled when rule's base version is missing", function () {
        patchRule('rule_1', { name: 'Customized prebuilt rule' }); // We want to make this a customized prebuilt rule
      ...
      it("Revert prebuilt rule button shouldn't appear if rule is non-customzied", function () {
      ...
      it("Revert prebuilt rule button shouldn't appear if rule is not prebuilt", function () {
        createRule(
          getNewRule({
            name: 'Custom rule',
            index: getIndexPatterns(),
