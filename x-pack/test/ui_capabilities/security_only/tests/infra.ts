    ...
    UserScenarios.forEach(scenario => {
      it(`${scenario.fullName}`, async () => {
        ...
        switch (scenario.username) {
          // these users have a read/write view of Infrastructure
          case 'superuser':
          case 'all':
          ...
          case 'infrastructure_read':
            // these users have a read-only view of Infrastructure
            expect(uiCapabilities.success).to.be(true);
            expect(uiCapabilities.value).to.have.property('infrastructure');
            expect(uiCapabilities.value!.infrastructure).to.eql({
              show: true,
          ...
          case 'no_kibana_privileges':
            ...
            break;
          // all other users can't do anything with Infrastructure
          default:
            expect(uiCapabilities.success).to.be(true);
            expect(uiCapabilities.value).to.have.property('infrastructure');
            expect(uiCapabilities.value!.infrastructure).to.eql({
