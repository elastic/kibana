...
export default function listRuleTypes({ getService }: FtrProviderContext) {
  ...
  describe('internal_rule_types', () => {
    it('should return 200 with list of alert types including internal fields', async () => {
      const response = await supertest.get(
        `${getUrlPrefix(Spaces.space1.id)}/internal/alerting/_rule_types`
      );
