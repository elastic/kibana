  ...
  describe('get', () => {
    ...
    describe('internal', () => {
      ...
      it('should handle get alert request appropriately', async () => {
        ...
        const response = await supertest.get(
          `${getUrlPrefix(Spaces.space1.id)}/internal/alerting/rule/${createdAlert.id}`
        );
      ...
      it(`shouldn't find alert from another space`, async () => {
        ...
        await supertest
          .get(`${getUrlPrefix(Spaces.other.id)}/internal/alerting/rule/${createdAlert.id}`)
          .expect(404, {
            statusCode: 404,
            error: 'Not Found',
            message: `Saved object [alert/${createdAlert.id}] not found`,
      ...
      it(`should handle get alert request appropriately when alert doesn't exist`, async () => {
        await supertest
          .get(`${getUrlPrefix(Spaces.space1.id)}/internal/alerting/rule/1`)
          .expect(404, {
            statusCode: 404,
            error: 'Not Found',
            message: 'Saved object [alert/1] not found',
