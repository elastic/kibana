...
export default function deleteActionTests({ getService }: FtrProviderContext) {
  ...
  describe('delete', () => {
    ...
    it('should handle delete action request appropriately', async () => {
      ...
      await supertest
        .delete(`${getUrlPrefix(Spaces.space1.id)}/api/actions/${createdAction.id}`)
        .set('kbn-xsrf', 'foo')
        .expect(204, '');
    });
    ...
    it(`should handle delete request appropriately when action doesn't exist`, async () => {
      await supertest
        .delete(`${getUrlPrefix(Spaces.space1.id)}/api/actions/2`)
        .set('kbn-xsrf', 'foo')
        .expect(404, {
          statusCode: 404,
          error: 'Not Found',
    ...
    it(`shouldn't delete action from preconfigured list`, async () => {
      await supertest
        .delete(`${getUrlPrefix(Spaces.space1.id)}/api/actions/my-slack1`)
        .set('kbn-xsrf', 'foo')
        .expect(400, {
          statusCode: 400,
          error: 'Bad Request',
