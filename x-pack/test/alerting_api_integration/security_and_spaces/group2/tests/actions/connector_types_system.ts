      ...
      describe(scenario.id, () => {
        it('should return 200 with list of action types containing defaults', async () => {
          const response = await supertestWithoutAuth
            .get(`${getUrlPrefix(space.id)}/internal/actions/connector_types`)
        ...
        it('should return 200 with list of action types containing system connectors', async () => {
          const response = await supertestWithoutAuth
            .get(`${getUrlPrefix(space.id)}/internal/actions/connector_types`)
            .auth(user.username, user.password);
          ...
          switch (scenario.id) {
            ...
            case 'space_1_all_with_restricted_fixture at space1':
              ...
              expect(
                response.body.some(
                  createActionTypeMatcher('test.system-action', 'Test system action')
                )
              ).to.be(true);
