...
export default function emailTest({ getService }: FtrProviderContext) {
  ...
  describe('create email action', () => {
    ...
    it('should respond with a 400 Bad Request when creating an email action with a server not added to hostsAllowList', async () => {
      await supertest
        .post('/api/actions/action')
        .set('kbn-xsrf', 'foo')
        .send({
          ...
          actionTypeId: '.email',
          config: {
            service: 'gmail', // not added to hostsAllowList in the config for this test
            from: 'bob@example.com',
          },
          secrets: {
            user: 'bob',
      ...
      await supertest
        ...
        .send({
          ...
          actionTypeId: '.email',
          config: {
            host: 'stmp.gmail.com', // not added to hostsAllowList in the config for this test
            port: 666,
            from: 'bob@example.com',
          },
          secrets: {
        ...
        .then((resp: any) => {
          expect(resp.body).to.eql({
            ...
            error: 'Bad Request',
            message:
              "error validating action type config: [host] value 'stmp.gmail.com' is not in the hostsAllowList configuration",
          });
        });
    });

    it('should handle creating an email action with a server added to hostsAllowList', async () => {
      const { body: createdAction } = await supertest
        .post('/api/actions/action')
        .set('kbn-xsrf', 'foo')
        .send({
          ...
          actionTypeId: '.email',
          config: {
            host: 'some.non.existent.com', // added to hostsAllowList in the config for this test
            port: 666,
            from: 'bob@example.com',
          },
          secrets: {
