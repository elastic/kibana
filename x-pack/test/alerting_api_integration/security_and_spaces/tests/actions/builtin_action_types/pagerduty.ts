...
export default function pagerdutyTest({ getService }: FtrProviderContext) {
  ...
  describe('pagerduty action', () => {
    ...
    it('should return unsuccessfully when default pagerduty url is not present in hostsAllowList', async () => {
      await supertest
        .post('/api/actions/action')
        .set('kbn-xsrf', 'foo')
        .send({
        ...
        .then((resp: any) => {
          expect(resp.body).to.eql({
            ...
            error: 'Bad Request',
            message:
              'error validating action type config: error configuring pagerduty action: target url "https://events.pagerduty.com/v2/enqueue" is not present in the Kibana config xpack.actions.hostsAllowList',
          });
        });
    });
