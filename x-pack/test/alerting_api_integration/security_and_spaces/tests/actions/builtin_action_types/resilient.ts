...
export default function resilientTest({ getService }: FtrProviderContext) {
  ...
  describe('IBM Resilient', () => {
    ...
    describe('IBM Resilient - Action Creation', () => {
      ...
      it('should respond with a 400 Bad Request when creating a ibm resilient action with a not present in hostsAllowList apiUrl', async () => {
        await supertest
          .post('/api/actions/action')
          .set('kbn-xsrf', 'foo')
          .send({
          ...
          .then((resp: any) => {
            expect(resp.body).to.eql({
              ...
              error: 'Bad Request',
              message:
                'error validating action type config: error configuring connector action: target url "http://resilient.mynonexistent.com" is not present in the Kibana config xpack.actions.hostsAllowList',
            });
          });
      });
