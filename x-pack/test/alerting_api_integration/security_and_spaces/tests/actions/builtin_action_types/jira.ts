        ...
        await supertest
          ...
          .then((resp: any) => {
            expect(resp.body).to.eql({
              ...
              error: 'Bad Request',
              message:
                'error validating action type config: error configuring connector action: target url "http://jira.mynonexistent.com" is not present in the Kibana config xpack.actions.allowedHosts',
            });
          });
      });
