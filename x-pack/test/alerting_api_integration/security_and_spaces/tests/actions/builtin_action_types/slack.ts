...
export default function slackTest({ getService }: FtrProviderContext) {
  ...
  describe('slack action', () => {
    ...
    it('should respond with a 400 Bad Request when creating a slack action with not present in hostsAllowList webhookUrl', async () => {
      await supertest
        .post('/api/actions/action')
        .set('kbn-xsrf', 'foo')
        .send({
        ...
        .then((resp: any) => {
          expect(resp.body).to.eql({
            ...
            error: 'Bad Request',
            message:
              'error validating action type secrets: error configuring slack action: target hostname "slack.mynonexistent.com" is not present in the Kibana config xpack.actions.hostsAllowList',
          });
        });
    });
