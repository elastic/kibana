...
export default async function ({ readConfigFile }: FtrConfigProviderContext) {
  ...
  const testFiles = [require.resolve('./security_network_dashboard')];
  ...
  const apmGlobalLabels = {
    ...
    ftrConfig: `x-pack/test/performance/tests/journeys/security_network_dashboard/config.ts`,
    performancePhase: process.env.TEST_PERFORMANCE_PHASE,
    journeyName: 'security_network_dashboard',
  };
