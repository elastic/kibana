...
export default async function ({ readConfigFile }: FtrConfigProviderContext) {
  ...
  const testFiles = [require.resolve('./security_users_dashboard')];
  ...
  const apmGlobalLabels = {
    ...
    ftrConfig: `x-pack/test/performance/tests/journeys/security_users_dashboard/config.ts`,
    performancePhase: process.env.TEST_PERFORMANCE_PHASE,
    journeyName: 'security_users_dashboard',
  };
