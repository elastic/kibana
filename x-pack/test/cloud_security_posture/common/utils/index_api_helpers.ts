...
export const deleteIndices = async (es: Client, indices: string[]) => {
  ...
  export const addIndexDocsWithoutTimestamp =  async <T>(es: Client, indexDocs: T[], index: string) => {
    await Promise.all(
      indexDocs.map((findingDoc) =>
        es.index({
          index,
  ...
  export const addIndexBulkDocs =  async <T>(es: Client, indexDocs: Array<Record<string, unknown>>, indices: string[]) => {
    await es.bulk({
        refresh: true,
