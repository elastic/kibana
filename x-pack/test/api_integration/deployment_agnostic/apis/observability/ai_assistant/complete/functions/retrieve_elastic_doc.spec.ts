...
export default function ApiTest({ getService }: DeploymentAgnosticFtrProviderContext) {
  ...
  describe('tool: retrieve_elastic_doc', function () {
    ...
    // Calling `retrieve_elastic_doc` via the chat/complete endpoint
    describe('POST /internal/observability_ai_assistant/chat/complete with multilingual E5 inference ID', function () {
      let llmProxy: LlmProxy;
      let connectorId: string;
      let messageAddedEvents: MessageAddEvent[];
      let firstRequestBody: ChatCompletionStreamParams;
      ...
      before(async () => {
        ...
        await installProductDoc(supertest, '.multilingual-e5-small-elasticsearch');
      ...
      after(async () => {
        await uninstallProductDoc(supertest, '.multilingual-e5-small-elasticsearch');
        llmProxy.close();
        await observabilityAIAssistantAPIClient.deleteActionConnector({
          actionId: connectorId,
        });
