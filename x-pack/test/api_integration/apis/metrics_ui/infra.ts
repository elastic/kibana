  ...
  describe('Hosts', () => {
    ...
    describe('endpoint validations', () => {
      ...
      it('should fail when metric is invalid', async () => {
        ...
        expect(normalizeNewLine(response.body.message)).to.be(
          '[request body]: Failed to validate: in metrics/0/type: "any" does not match expected type "cpu" | "normalizedLoad1m" | "diskSpaceUsage" | "memory" | "memoryFree" | "rx" | "tx" | "rxNew" | "txNew"'
        );
      });
