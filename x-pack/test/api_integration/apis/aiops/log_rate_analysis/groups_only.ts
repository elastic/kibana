...
export default ({ getService }: FtrProviderContext) => {
  ...
  describe('POST /internal/aiops/log_rate_analysis - groups only', () => {
    (['1', '2'] as ApiVersion[]).forEach((apiVersion) => {
        ...
        describe(`with v${apiVersion} - ${testData.testName}`, () => {
          ...
          async function assertAnalysisResult(data: any[]) {
            ...
            const addSignificantItemsActions = data.filter(
              (d) =>
                d.type === (apiVersion === '1' ? 'add_significant_terms' : 'add_significant_items')
            );
            ...
            const histogramActions = data.filter(
              (d) =>
                d.type ===
                (apiVersion === '1'
                  ? 'add_significant_terms_histogram'
                  : 'add_significant_items_histogram')
            );
            ...
            const groupActions = data.filter(
              (d) =>
                d.type ===
