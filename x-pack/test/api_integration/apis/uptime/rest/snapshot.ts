...
export default function({ getService }: FtrProviderContext) {
  ...
  describe('snapshot count', () => {
    ...
    describe('when data is present', async () => {
      [true, false].forEach(async (highCardinality: boolean) => {
        const cardinality = highCardinality ? 'high' : 'low';
        const cardMultiplier = highCardinality ? 200 : 1;
        const cardSnapshotSuffix = highCardinality ? '_high_card' : '';
          ...
          [true, false].forEach(async (includeTimespan: boolean) => {
            describe(`with timespans ${includeTimespan ? 'included' : 'missing'}`, async () => {
              ...
              it('will count all statuses correctly', async () => {
                ...
                expectFixtureEql(apiResponse.body, 'snapshot' + cardSnapshotSuffix);
              });
              ...
              it('will fetch a monitor snapshot filtered by up status', async () => {
                ...
                expectFixtureEql(apiResponse.body, 'snapshot_filtered_by_up' + cardSnapshotSuffix);
              });
            });
          });
        });
