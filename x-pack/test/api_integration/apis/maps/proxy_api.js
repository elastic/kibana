...
export default function ({ getService }) {
  ...
  describe('EMS proxy', () => {
    it('should correctly rewrite url and format', async () => {
      ...
      const topoJson = worldLayer.formats.find((format) => {
        return format.type === 'topojson';
      });
      expect(topoJson.type).to.be('topojson');
      expect(topoJson.url).to.be('file?id=world_countries');
    });
  });
}
