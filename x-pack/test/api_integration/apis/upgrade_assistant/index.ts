  ...
  describe('Upgrade Assistant', function () {
    ...
    before(
      ...
      async function () {
        ...
        console.log('IN BEFORE TEST CONFIG, esVersion????', esVersion);
        console.log('IN BEFORE TEST CONFIG, kibanaVersion????', kibanaVersion);
        if (esVersion.replace('-SNAPSHOT', '') !== kibanaVersion.replace('-SNAPSHOT', '')) {
          console.log('SKIPPING THIS TEST SUITE');
          this.skip();
        }
      }
    );
