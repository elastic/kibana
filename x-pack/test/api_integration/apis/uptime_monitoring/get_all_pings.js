...
export default function ({ getService }) {
  ...
  describe('get_all_pings', () => {
    ...
    it('should get all pings stored in index', async () => {
      const { body: apiResponse } = await supertest.get('/api/uptime/monitors').expect(200);
    ...
    it('should sort pings according to timestamp', async () => {
      const { body: apiResponse } = await supertest.get('/api/uptime/monitors?sort=asc').expect(200);
    ...
    it('should return results of n length', async () => {
      const { body: apiResponse } = await supertest.get('/api/uptime/monitors?size=1').expect(200);
