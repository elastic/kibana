...
export default function ({ getService }: FtrProviderContext) {
  ...
  describe('fleet_list_agent', () => {
    ...
    it('should return the list of agents when requesting as a user superser', async () => {
    ...
    it('should not return the list of agents when requesting as a user with fleet read permissions', async () => {
      await supertestWithoutAuth
        .get(`/api/fleet/agents`)
        .auth(users.fleet_user.username, users.fleet_user.password)
        .expect(403);
    });
    it('should not return the list of agents when requesting as a user with fleet all permissions', async () => {
      await supertestWithoutAuth
        .get(`/api/fleet/agents`)
