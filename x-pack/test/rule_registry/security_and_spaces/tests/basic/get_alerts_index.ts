  ...
  describe('Alert - Get Index - RBAC - spaces', () => {
    ...
    describe('Users:', () => {
      it(`${obsOnlySpacesAll.username} should be able to access the APM alert in ${SPACE1}`, async () => {
        ...
        expect(indexNames.includes(`${APM_ALERT_INDEX}-${DEFAULT_SPACE}`)).to.eql(true); // assert this here so we can use constants in the dynamically-defined test cases below
      });
