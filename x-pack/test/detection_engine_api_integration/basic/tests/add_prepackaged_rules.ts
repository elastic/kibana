...
import { removeExceptionListServerGeneratedProperties } from '../../../lists_api_integration/utils';
    ...
    describe('endpoint exceptions list', () => {
      it('should create the endpoint exceptions list if one does not yet exist', async () => {
        ...
        const { body: getListBody } = await supertest
          .get(`${EXCEPTION_LIST_URL}?list_id=endpoint_list&namespace_type=agnostic`)
          .set('kbn-xsrf', 'xxx')
          .expect(200);

        body.data = [removeExceptionListServerGeneratedProperties(getListBody.data[0])];
        expect(getListBody).to.eql({
