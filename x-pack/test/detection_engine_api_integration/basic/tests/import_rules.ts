...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('import_rules', () => {
    ...
    describe('importing rules with an index', () => {
      ...
      // Should be able to import a lot of rules in a timely manner
      it('should be able to import 1000 rules', async () => {
        const ruleIds = new Array(1000).fill(undefined).map((_, index) => `rule-${index}`);
        const { body } = await supertest
          .post(`${DETECTION_ENGINE_RULES_URL}/_import`)
          .set('kbn-xsrf', 'true')
