...
export default ({ getService }: FtrProviderContext) => {
  ...
  describe.skip('@ess @serverless Detection rule status telemetry', async () => {
    ...
    describe.skip('"kql" rule type', () => {
      ...
      it('@skipInServerless should have zero values for "detection_rule_status.all_rules" rules that are not query based', () => {
      ...
      it('@skipInServerless should have non zero values for "index_duration"', () => {
        expect(
      ...
      it('@skipInServerless should have non zero values for "succeeded"', () => {
        expect(stats?.detection_rules.detection_rule_status.custom_rules.query.succeeded).to.eql(1);
      });

      it('@skipInServerless should have non zero values for "succeeded", "index_duration", "search_duration" and "enrichment_duration"', () => {
        expect(
          stats?.detection_rules.detection_rule_status.custom_rules.query.index_duration.max
        ).to.be.above(1);
        expect(
