...
const getValidFieldsForRuleType = <T extends keyof RuleTypeToFields>(
  ruleType: T
): { [K in RuleTypeToFields[T][number]]: SingleLineStringFieldTestValues } => {
  return (
    Object.entries(SINGLE_LINE_STRING_FIELDS_MAP) as Array<
      [SINGLE_LINE_STRING_FIELDS, SingleLineStringFieldTestValues]
    >
  )
    .filter(([field]) =>
      (RULE_TYPE_FIELD_MAPPING[ruleType] as readonly SINGLE_LINE_STRING_FIELDS[]).includes(field)
    )
...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('@ess @serverless @skipInServerlessMKI review prebuilt rules updates from package with mock rule assets', () => {
    describe('Query rule single line string fields', () => {
      beforeEach(async () => {
      ...
      (
        Object.entries(queryRuleFields) as Array<
          [
            Extract<SINGLE_LINE_STRING_FIELDS, keyof typeof queryRuleFields>,
            SingleLineStringFieldTestValues
          ]
        >
        ...
        describe(`testing field: ${field}`, () => {
          ...
          describe("when rule field doesn't have an update and has no custom value - scenario AAA", () => {
            it('should not show in the upgrade/_review API response', async () => {
              ...
              expect(
                (reviewResponse.rules[0].diff.fields as DiffFieldsForType<ThreatMatchFieldsDiff>)[
                  field
                ]
              ).toBeUndefined();
    ...
    describe('Threat match rule single line string fields', () => {
      const threatMatchRuleFields = getValidFieldsForRuleType('threat_match');
      beforeEach(async () => {
        await deleteAllRules(supertest, log);
        await deleteAllTimelines(es, log);
        await deleteAllPrebuiltRuleAssets(es, log);
      ...
      (
        Object.entries(threatMatchRuleFields) as Array<
          [
            Extract<SINGLE_LINE_STRING_FIELDS, keyof typeof threatMatchRuleFields>,
            SingleLineStringFieldTestValues
          ]
        >
      ).map(([field, testValues]) => {
        ...
        describe(`testing field: ${field}`, () => {
          const getRuleAssetSavedObjects = () => [
            createRuleAssetSavedObjectOfType('threat_match', {
              rule_id: 'rule-1',
              version: 1,
              [field]: baseValue,
            }),
          ...
          describe("when rule field doesn't have an update but has a custom value - scenario ABA", () => {
            it('should show in the upgrade/_review API response', async () => {
              ...
              expect(
                (
                  reviewResponse.rules[0].diff
                    .fields as DiffFieldsForType<ThreatMatchFieldsDiff> as DiffFieldsForType<ThreatMatchFieldsDiff>
                )[field]
              ).toEqual({
                base_version: baseValue,
                current_version: customValue,
              ...
              // console.log('HERE', JSON.stringify(reviewResponse, null, 2));
    ...
    describe('New terms rule single line string fields', () => {
      const newTermsRuleFields = getValidFieldsForRuleType('new_terms');
      beforeEach(async () => {
        await deleteAllRules(supertest, log);
        await deleteAllTimelines(es, log);
        await deleteAllPrebuiltRuleAssets(es, log);
      ...
      (
        Object.entries(newTermsRuleFields) as Array<
          [
            Extract<SINGLE_LINE_STRING_FIELDS, keyof typeof newTermsRuleFields>,
            SingleLineStringFieldTestValues
          ]
        >
      ).map(([field, testValues]) => {
        ...
        describe(`testing field: ${field}`, () => {
          const getRuleAssetSavedObjects = () => [
            createRuleAssetSavedObjectOfType('new_terms', {
              rule_id: 'rule-1',
              version: 1,
              [field]: baseValue,
            }),
          ...
          describe("when rule field doesn't have an update and has no custom value - scenario AAA", () => {
            it('should not show in the upgrade/_review API response', async () => {
              ...
              const reviewResponse = await reviewPrebuiltRulesToUpgrade(supertest);
              expect(
                (reviewResponse.rules[0].diff.fields as DiffFieldsForType<NewTermsFieldsDiff>)[
                  field
                ]
              ).toBeUndefined();
