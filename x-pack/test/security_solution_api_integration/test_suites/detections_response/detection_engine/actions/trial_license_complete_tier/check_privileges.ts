      ...
      indexTestCases.forEach((index) => {
        it(`for KQL rule with index param: ${index}`, async () => {
          ...
          expect(body?.execution_summary?.last_execution.message).to.contain(
            `This rule's API key is unable to access all indices that match the ["${index[0]}"] pattern. To learn how to update and manage API keys, refer to https://www.elastic.co/guide/en/kibana/8.19/alerting-setup.html#alerting-authorization.`
          );
