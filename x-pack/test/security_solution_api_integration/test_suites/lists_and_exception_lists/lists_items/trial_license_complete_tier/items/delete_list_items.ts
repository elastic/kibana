    ...
    describe('deleting list items', () => {
      ...
      it('should delete a single list item with a list item id', async () => {
        ...
        const { body } = await supertest
          .delete(`${LIST_ITEMS_URL}?id=${getCreateMinimalListItemSchemaMock().id}`)
          .set('kbn-xsrf', 'true')
          .expect(200);
      ...
      it('should delete a single list using an auto generated id', async () => {
        ...
        const { body } = await supertest
          .delete(`${LIST_ITEMS_URL}?id=${bodyWithCreateListItem.id}`)
          .set('kbn-xsrf', 'true')
          .expect(200);
      ...
      it('should return an error if the id does not exist when trying to delete it', async () => {
        const { body } = await supertest
          .delete(`${LIST_ITEMS_URL}?id=c1e1b359-7ac1-4e96-bc81-c683c092436f`)
          .set('kbn-xsrf', 'true')
          .expect(404);
