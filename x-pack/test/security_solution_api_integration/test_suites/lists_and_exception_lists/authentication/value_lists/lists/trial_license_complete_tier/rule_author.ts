...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('@serverless @serverlessQA rule_author value lists API behaviors', () => {
    before(async () => {
    ...
    describe('find value list', () => {
      it('should return 200 for rule_author', async () => {
        ...
        await ruleAuthor.get(`${LIST_URL}/_find`).set('kbn-xsrf', 'true').send().expect(200);
      });
    });
    ...
    describe('update value list', () => {
      it('should return 200 for rule_author', async () => {
        ...
        await ruleAuthor.put(LIST_URL).set('kbn-xsrf', 'true').send(updatedList).expect(200);
      });
    });
  });
};
