...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('@serverless @serverlessQA admin value lists API behaviors', () => {
    before(async () => {
    ...
    describe('find value list', () => {
      it('should return 200 for admin', async () => {
        ...
        await admin.get(`${LIST_URL}/_find`).set('kbn-xsrf', 'true').send().expect(200);
      });
    });
    ...
    describe('patch value list', () => {
      it('should return 200 for admin', async () => {
        ...
        await admin.patch(LIST_URL).set('kbn-xsrf', 'true').send(patchedListPayload).expect(200);
      });
    });
    ...
    describe('update value list', () => {
      it('should return 200 for admin', async () => {
        ...
        await admin.put(LIST_URL).set('kbn-xsrf', 'true').send(updatedList).expect(200);
      });
    });
  });
};
