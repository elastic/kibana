...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('@serverless @serverlessQA viewer value list items API behaviors', () => {
    before(async () => {
    ...
    describe('update value list item', () => {
      it('should return 403 for viewer', async () => {
        ...
        await viewer.put(LIST_ITEM_URL).set('kbn-xsrf', 'true').send(updatedListItem).expect(403);
      });
    });
  });
};
