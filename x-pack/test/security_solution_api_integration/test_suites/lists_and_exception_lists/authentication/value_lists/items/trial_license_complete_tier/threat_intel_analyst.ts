...
export default ({ getService }: FtrProviderContext): void => {
  ...
  let admin: TestAgent;
  let threatIntelligenceAnalyst: TestAgent;

  describe('@serverless @serverlessQA threat_intelligence_analyst value list items API behaviors', () => {
    before(async () => {
      admin = await utils.createSuperTest('admin');
      threatIntelligenceAnalyst = await utils.createSuperTest('threat_intelligence_analyst');
    });
    ...
    describe('create value list item', () => {
      it('should return 200 for threat_intelligence_analyst', async () => {
        ...
        await threatIntelligenceAnalyst
          .post(LIST_ITEM_URL)
          .set('kbn-xsrf', 'true')
