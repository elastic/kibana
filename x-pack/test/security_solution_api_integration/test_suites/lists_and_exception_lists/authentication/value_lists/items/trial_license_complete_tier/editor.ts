...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('@serverless @serverlessQA editor value list items API behaviors', () => {
    before(async () => {
    ...
    describe('update value list item', () => {
      it('should return 200 for editor', async () => {
        ...
        await editor.put(LIST_ITEM_URL).set('kbn-xsrf', 'true').send(updatedListItem).expect(200);
      });
    });
  });
};
