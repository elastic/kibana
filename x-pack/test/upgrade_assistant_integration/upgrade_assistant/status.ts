...
export default function({ getService }: FtrProviderContext) {
  ...
  describe('status and _cluster/state contract', () => {
    ...
    it('the _cluster/state endpoint is still what we expect', async () => {
      ...
      try {
        if (result.body.metadata.indices['7.0-data'].state === 'open') {
          return;
        }
      } catch (e) {
        expect().fail(
      ...
      expect().fail(
        `The response contract for _cluster/state metadata has changed. Please update Upgrade Assistant checkup.`
      );
    });
  });
}
