  ...
  describe('installs packages from direct upload', async () => {
    ...
    it('should throw an error if the archive manifest misses a mandatory field', async function () {
      ...
      const res = await supertest
        ...
        .expect(400);
      expect(res.error.text).to.equal(
        '{"statusCode":400,"error":"Bad Request","message":"Invalid top-level package manifest: one or more fields missing of name, version, description, type, title, categories, format_version, release, owner"}'
      );
    });
