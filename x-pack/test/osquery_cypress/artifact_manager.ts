...
async function getArtifact(artifact: string, urlExtractor: (data: AxiosResponse<any>, filename: string) => string, log: ToolingLog, version: string) {
  log.info(`Fetching ${version} of ${artifact}`)
  const agents = await axios(`https://artifacts-api.elastic.co/v1/versions/${version}/builds/latest`)
  const arch = archMap[process.arch] ?? process.arch
  const dirName = `${artifact}-${version}-${process.platform}-${arch}`
  const filename = dirName + '.tar.gz'
  let url = urlExtractor(agents.data, filename)
  if (!url) {
...
const fetchers = {
  'elastic-agent': getArtifact.bind(null, 'elastic-agent', (data, filename) => get(data, ['build','projects','beats','packages', filename, 'url'])),
  'fleet-server': getArtifact.bind(null, 'fleet-server', (data, filename) => get(data, ['build','projects','fleet-server','packages', filename, 'url'])),
}
...
export class ArtifactManager {
  ...
  public constructor(versions: FetchArtifactsParams, log: ToolingLog) {
  ...
  public fetchArtifacts = async () => {
    ...
    await Promise.all(
      Object.keys(this.versions).map(async (artifactName) => {
        this.artifacts[artifactName] = await fetchers[artifactName](this.log, this.versions[artifactName])
      })
    )
  }
