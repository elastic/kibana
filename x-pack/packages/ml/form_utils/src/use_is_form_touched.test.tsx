/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import React, { FC } from 'react';
import { act, renderHook } from '@testing-library/react-hooks';
import { configureStore } from '@reduxjs/toolkit';
import { useDispatch, Provider } from 'react-redux';

import { stringValidator } from './validators/string_validator';
import { createFormField } from './form_field';
import { createFormSlice } from './form_slice';
import { useIsFormTouched } from './use_is_form_touched';

const defaultFormFields = [createFormField('firstName'), createFormField('lastName')];
const formSlice = createFormSlice('test', defaultFormFields, [], { stringValidator });

const reduxStore = configureStore({
  reducer: { test: formSlice.reducer },
});

const ReduxProvider: FC = ({ children }) => {
  return <Provider store={reduxStore}>{children}</Provider>;
};

describe('useIsFormTouched', () => {
  it('should return if the form has been touched', () => {
    const { result } = renderHook(
      () => ({
        dispatch: useDispatch(),
        isFormTouched: useIsFormTouched(formSlice, defaultFormFields, []),
      }),
      {
        wrapper: ReduxProvider,
      }
    );

    expect(result.current.isFormTouched).toBe(false);

    act(() => {
      result.current.dispatch(
        formSlice.actions.setFormField({ field: 'firstName', value: 'John' })
      );
    });

    expect(result.current.isFormTouched).toBe(true);
  });
});
