...
export const getAnimationOptions = (theme: EuiThemeComputed): cytoscape.AnimationOptions => ({
  duration: 250, //theme.animation.normal
  // @ts-expect-error The cubic-bezier options here are not recognized by the cytoscape types
  easing: theme.animation.bounce,
});
...
export const getNodeHeight = (theme: EuiThemeComputed): string => theme.size.xxl;
...
const getStyle = (theme: EuiThemeComputed): cytoscape.Stylesheet[] => {
  ...
  return [
    ...
    },
    {
      ...
      style: {
        ...
        //
        // @ts-expect-error
        'target-distance-from-node': isIE11 ? undefined : theme.size.xs,
        width: 1,
...
export const getCytoscapeDivStyle = (theme: EuiThemeComputed/* , status: FETCH_STATUS */): CSSProperties => ({
...
${theme.colors.lightShade}`,
  ...
  cursor: `${/* status === FETCH_STATUS.LOADING */false ? 'wait' : 'grab'}`,
  marginTop: 0,
});
