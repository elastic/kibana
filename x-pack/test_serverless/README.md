...
To test custom roles on a project type that doesn't yet support them, use a feature-flagged test configuration. This allows testing in the Kibana CI before the feature is enabled on MKI. Once the project type officially supports custom roles, move the tests to a standard FTR config to enable execution on MKI.
