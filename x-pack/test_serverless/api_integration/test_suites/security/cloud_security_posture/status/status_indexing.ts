...
export default function (providerContext: FtrProviderContext) {
  ...
  describe('GET /internal/cloud_security_posture/status', function () {
    ...
    describe('STATUS = INDEXING TEST', () => {
      ...
      it(`Return kspm status indexing when logs-cloud_security_posture.misconfiguration_latest doesn't contain new kspm documents, but has newly connected agents`, async () => {
      ...
      it(`Return cspm status indexing when logs-cloud_security_posture.misconfiguration_latest doesn't contain new cspm documents, but has newly connected agents  `, async () => {
        await createPackagePolicy(
          supertestWithoutAuth,
          agentPolicyId,
          'cspm',
