...
export default ({ getPageObjects, getService }: FtrProviderContext) => {
  ...
  const otherUser: estypes.SecurityPutUserRequest = {
    username: 'other_user',
    password: 'changeme',
    roles: ['superuser'],
  };
  ...
  describe('Home page', function () {
    before(async () => {
      ...
      await es.security.putUser(otherUser);
    ...
    after(async () => {
      await es.security.deleteUser({ username: otherUser.username });
    ...
    describe('Readonly API key', function () {
      ...
      // Serverless tests run as elastic (superuser) so unable to test `read_security` permissions
      it.skip('should see readonly form elements', async () => {
        // Create a key to updated
