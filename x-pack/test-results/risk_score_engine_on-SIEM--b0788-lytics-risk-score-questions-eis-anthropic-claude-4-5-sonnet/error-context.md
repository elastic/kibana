...
- Name: risk_score_engine_on.spec.ts >> SIEM Entity Analytics Skill - Risk Score Tests >> entity analytics risk score questions
- Location: x-pack/solutions/security/packages/kbn-evals-suite-entity-analytics/evals/v1/risk_score_engine_on.spec.ts:74:26

# Error details

```
KbnClientRequesterError: [GET - http://localhost:5621/api/status] request failed (attempt=5/5): undefined -- Status: N/A, Cause: fetch failed -- and ran out of retries
```

```
KbnClientRequesterError: [DELETE http://localhost:5621/api/risk_score/engine/dangerously_delete_data] 400 Bad Request -- {"message":"This API is not available when Entity Store V2 is enabled. Use the Entity Store APIs instead.","status_code":400}
```

# Test source

```ts
  145 |         `${decodeURIComponent(parsed.username)}:${decodeURIComponent(parsed.password)}`
  146 |       ).toString('base64')}`;
  147 |       parsed.username = '';
  148 |       parsed.password = '';
  149 |     }
  150 |     this.urlForFetch = parsed.toString();
  151 | 
  152 |     this.dispatcher =
  153 |       parsed.protocol === 'https:'
  154 |         ? new Agent({
  155 |             connect: {
  156 |               ca: options.certificateAuthorities,
  157 |               rejectUnauthorized: false,
  158 |               timeout: FETCH_CONNECT_TIMEOUT_MS,
  159 |             },
  160 |           })
  161 |         : null;
  162 |   }
  163 | 
  164 |   public resolveUrl(relativeUrl = '/') {
  165 |     return this.resolveUrlInternal(this.url, relativeUrl);
  166 |   }
  167 | 
  168 |   private resolveUrlInternal(baseUrl: string, relativeUrl = '/') {
  169 |     return Url.resolve(
  170 |       baseUrl.endsWith('/') ? baseUrl : `${baseUrl}/`,
  171 |       relativeUrl.startsWith('/') ? relativeUrl.slice(1) : relativeUrl
  172 |     );
  173 |   }
  174 | 
  175 |   async request<T>(options: ReqOptions): Promise<KbnClientResponse<T>> {
