...
# Create the zip and generate the sha1 hash using filenames like:
...
sha1_filename = base_filename + '.sha1'
...
print('Creating ' + path.join(src_path, sha1_filename))
with open (sha1_filename, 'w') as f:
  f.write(sha1_filename(zip_filename))

runcmd('gsutil cp ' + path.join(src_path, zip_filename) + ' gs://headless_shell_staging')
runcmd('gsutil cp ' + path.join(src_path, sha1_filename) + ' gs://headless_shell_staging')
