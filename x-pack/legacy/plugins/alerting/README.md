...
|actionGroups|An explicit configuration of groups the alert type may schedule actions for, shaped as a key value where the key is the ActionGroup unique ID and the value is the human readable name for this actionGroup within this AlertType. Alert `actions` validation will use this configuartion to ensure groups are valid. To specify an actionGroup add. We highly encourage using `kbn-i18n` to translate the value of actionGroups when registering the AlertType. |Record<string,string>|
