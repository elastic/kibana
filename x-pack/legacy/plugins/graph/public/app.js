...
export function initAngularModule(moduleName, deps) {
  ...
  app.controller('graphuiPlugin', function ($scope, $route) {
    ...
    $scope.pluginDependencies = coreStart;
    ...
    $scope.topNavMenu.push({
      ...
      run: () => {
        ...
        currentSettingsFlyout = coreStart.overlays.openFlyout(<Settings observable={settingsObservable} />, {
          size: 'm',
    ...
    if ($route.current.locals.savedWorkspace) {
      ...
      $route.current.locals.SavedWorkspacesProvider.get().then(function (newWorkspace) {
        ...
        openSourceModal(coreStart, indexPattern => {
