...
export function lspRoute(
  ...
  server.route({
    ...
    async handler(req: RequestFacade, h: ResponseToolkitFacade) {
      ...
      const files = await groupFiles(req, locations);
  ...
  server.route({
    ...
    async handler(req: RequestFacade, h: ResponseToolkitFacade) {
      try {
        ...
        const title: string = await findTitleFromHover(hover, uri, position);
        const files = await groupFiles(req, response.result);
        return { title, files, uri, position };
      } catch (error) {
        log.error(error);
        if (error instanceof ResponseError) {
  ...
  async function groupFiles(req: RequestFacade, list: Location[]) {
    const files = [];
  ...
  async function findTitleFromHover(hover: ResponseMessage, uri: string, position: Position) {
    let title: string;
