...
export function initPutRolesApi(
      ...
      if (spaces.length === 1 && spaces[0] === GLOBAL_RESOURCE) {
        return {
          privileges: [
            ...
            ...(feature
              ? flatten(
                  Object.entries(feature).map(([featureName, featurePrivileges]) =>
  ...
  const transformRolesToEs = (payload, existingApplications = []) => {
    ...
    return pick(
      {
        ...
        applications: [...transformKibanaPrivilegesToEs(kibana), ...otherApplications],
      },
      identity
    );
  };
  ...
  const getKibanaSchema = () => {
    ...
    const allSpacesSchema = Joi.array()
      .length(1)
      .items(Joi.string().valid([GLOBAL_RESOURCE]));
    return Joi.array()
      .items(
        Joi.object({
          base: Joi.alternatives().when('spaces', {
            is: allSpacesSchema,
            then: Joi.array()
              .items(Joi.string().valid(Object.keys(privileges.global)))
              .empty(Joi.array().length(0)),
            otherwise: Joi.array()
              .items(Joi.string().valid(Object.keys(privileges.space)))
              .empty(Joi.array().length(0)),
          }),
          feature: Joi.object()
