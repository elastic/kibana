...
const registerPrivilegesWithClusterTest = (
  ...
  const createExpectUpdatedPrivileges = (mockServer, mockCallWithInternalUser, error) => {
    return (postPrivilegesBody, deletedPrivileges = []) => {
      ...
      expect(mockCallWithInternalUser).toHaveBeenCalledWith('shield.postPrivileges', {
      ...
      for (const deletedPrivilege of deletedPrivileges) {
        ...
        expect(mockCallWithInternalUser).toHaveBeenCalledWith('shield.deletePrivilege', {
          application,
  ...
  test(description, async () => {
    ...
    const mockCallWithInternalUser = registerMockCallWithInternalUser().mockImplementation(api => {
      switch (api) {
    ...
    assert({
      expectUpdatedPrivileges: createExpectUpdatedPrivileges(
        mockServer,
