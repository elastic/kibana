...
export async function fetchFile(params: FileRequestParams): Promise<Response> {
  const { registryUrl } = epmConfigStore.getConfig();
  const { pkgkey, filePath } = params;
  return getResponse(`${registryUrl}/package/${pkgkey}/${filePath}`);
}
