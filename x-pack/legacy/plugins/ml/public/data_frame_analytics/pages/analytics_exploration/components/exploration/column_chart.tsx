...
export const ColumnChart: FC<Props> = props => {
  const { indexPattern, columnType } = props;
  ...
  return <WithIndexPatternColumnChart {...props} />;
};

export const WithIndexPatternColumnChart: FC<Props> = ({ indexPattern, columnType, query }) => {
  const field = indexPattern.fields.find(f => f.name === columnType.name);
  ...
  const fetchCardinality = async () => {
    try {
      const resp: any = await ml.esSearch({
  ...
  const fetchChartHistogramData = async () => {
    try {
      const respStats: any = await ml.esSearch({
        index: indexPattern.title,
        size: 0,
        body: {
          query,
  ...
  }, [field && field.name, JSON.stringify(query)]);

  if (field === null || data.length === 0) {
    return null;
  }
  ...
  const getColor = d => {
    ...
    if (xScaleType === 'ordinal' && hoveredRow[field.name] === d.key) {
      return '#5782ae';
    }

    if (
      xScaleType === 'linear' &&
      hoveredRow[field.name] >= +d.key &&
      hoveredRow[field.name] < +d.key + interval
    ) {
      return '#5782ae';
    }
    ...
    if (
      xScaleType === 'time' &&
      moment(hoveredRow[field.name]).unix() * 1000 >= +d.key &&
      moment(hoveredRow[field.name]).unix() * 1000 < +d.key + interval
    ) {
      return '#5782ae';
    }
