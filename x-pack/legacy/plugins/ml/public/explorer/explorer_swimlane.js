...
export const ExplorerSwimlane = injectI18n(
  class ExplorerSwimlane extends React.Component {
    static propTypes = {
      ...
      function cellMouseover(target, laneLabel, bucketScore, index, time) {
        ...
        tooltipData.push({
          name: intl.formatMessage({
            id: 'xpack.ml.explorer.swimlane.maxAnomalyScoreLabel',
        ...
        .each(function() {
          if (swimlaneData.fieldName !== undefined) {
            d3.select(this)
              ...
              .attr(
                'aria-label',
                label => `${mlEscape(swimlaneData.fieldName)}: ${mlEscape(label)}`
              );
          }
        });
