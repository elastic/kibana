  ...
  injectObservablesAsProps(
    ...
    class Explorer extends React.Component {
      ...
      async componentDidUpdate() {
        if (
          this.props.explorer !== undefined &&
          this.props.explorer.action !== EXPLORER_ACTION.IDLE
        ) {
          ...
          if (action === EXPLORER_ACTION.JOB_SELECTION_CHANGE) {
            ...
            if (selectedJobs.length > 1) {
              this.props.appStateHandler(APP_STATE_ACTION.SAVE_SWIMLANE_VIEW_BY_FIELD_NAME, {
      ...
      async updateExplorer(stateUpdate = {}, showOverallLoadingIndicator = true) {
        ...
        if (selectionInfluencers.length === 0) {
          stateUpdate.influencers = await loadTopInfluencers(
            jobIds,
            timerange.earliestMs,
            timerange.latestMs,
            noInfluencersConfigured
        ...
        if (stateUpdate.influencers !== undefined && !noInfluencersConfigured) {
          for (const influencerName in stateUpdate.influencers) {
            if (
              stateUpdate.influencers[influencerName][0] &&
              stateUpdate.influencers[influencerName][0].influencerFieldValue
            ) {
              stateUpdate.filterPlaceHolder = i18n.translate(
                'xpack.ml.explorer.kueryBar.filterPlaceholder',
                {
                  defaultMessage: 'Filter by influencer fields… ({queryExample})',
                  values: {
                    queryExample: `${influencerName} : ${stateUpdate.influencers[influencerName][0].influencerFieldValue}`,
                  },
                }
              );
              break;
        ...
        if (
          ...
          updatedAnomalyChartRecords !== undefined
        ) {
        ...
        if (selectedCells !== null) {
          this.updateCharts(
            stateUpdate.anomalyChartRecords,
            timerange.earliestMs,
            timerange.latestMs
          );
        } else {
      ...
      swimlaneCellClick = swimlaneSelectedCells => {
        ...
        if (Object.keys(swimlaneSelectedCells).length === 0) {
          ...
          const currentShowTopFieldValues = _.get(
            this.state,
            'selectedCells.showTopFieldValues',
            false
          );
      ...
      render() {
        const { dateFormatTz, globalState, intl, jobSelectService, TimeBuckets } = this.props;
                      ...
                      <EuiFlexItem grow={false} style={{ alignSelf: 'center' }}>
                        <EuiFormRow label="&#8203;">
                          <div className="panel-sub-title">
                            ...
                            {filterActive === true && swimlaneViewByFieldName === 'job ID' && (
                              <FormattedMessage
