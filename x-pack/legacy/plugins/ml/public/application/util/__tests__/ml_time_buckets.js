...
describe('ML - time buckets', () => {
  ...
  describe('calcEsInterval', () => {
    it('returns correct interval for various durations', () => {
      expect(calcEsInterval(moment.duration(500, 'ms'))).to.eql({
        value: 500,
        unit: 'ms',
        expression: '500ms',
      });
      expect(calcEsInterval(moment.duration(1000, 'ms'))).to.eql({
        value: 1,
        unit: 's',
        expression: '1s',
      });
      expect(calcEsInterval(moment.duration(15, 's'))).to.eql({
        value: 15,
        unit: 's',
        expression: '15s',
      });
      expect(calcEsInterval(moment.duration(60, 's'))).to.eql({
        value: 1,
        unit: 'm',
        expression: '1m',
      });
      expect(calcEsInterval(moment.duration(1, 'm'))).to.eql({
        value: 1,
        unit: 'm',
        expression: '1m',
      });
      expect(calcEsInterval(moment.duration(60, 'm'))).to.eql({
        value: 1,
        unit: 'h',
        expression: '1h',
      });
      expect(calcEsInterval(moment.duration(3, 'h'))).to.eql({
        value: 3,
        unit: 'h',
        expression: '3h',
      });
      expect(calcEsInterval(moment.duration(24, 'h'))).to.eql({
        value: 1,
        unit: 'd',
        expression: '1d',
      });
      expect(calcEsInterval(moment.duration(3, 'd'))).to.eql({
        value: 3,
        unit: 'd',
        expression: '3d',
      });
      expect(calcEsInterval(moment.duration(7, 'd'))).to.eql({
        value: 1,
        unit: 'w',
        expression: '1w',
      });
      expect(calcEsInterval(moment.duration(1, 'w'))).to.eql({
        value: 1,
        unit: 'w',
        expression: '1w',
      });
      expect(calcEsInterval(moment.duration(4, 'w'))).to.eql({
        value: 28,
        unit: 'd',
        expression: '28d',
      });
      ...
      expect(calcEsInterval(moment.duration(12, 'M'))).to.eql({
        value: 1,
        unit: 'y',
        expression: '1y',
      });
      expect(calcEsInterval(moment.duration(1, 'y'))).to.eql({
        value: 1,
        unit: 'y',
        expression: '1y',
      });
