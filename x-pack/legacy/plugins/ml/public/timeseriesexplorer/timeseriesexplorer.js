...
export class TimeSeriesExplorer extends React.Component {
  ...
  contextChartSelected = selection => {
    ...
    if (
      ...
      const { criteriaFields, detectorId, entities, modelPlotEnabled } = this.state;
      ...
      getFocusData(
        ...
        this.setState({
          ...
          showModelBoundsCheckbox:
            modelPlotEnabled === true && refreshFocusData.focusChartData.length > 0,
          ...
          this.setState(
            {
              entities: entities.map(entity => {
                if (firstRec.partition_field_name === entity.fieldName) {
                  entity.fieldValues = chain(resp.records)
  ...
  refresh = (fullRefresh = true) => {
    ...
    this.setState(
      ...
      () => {
        ...
        const nonBlankEntities = filter(currentEntities, entity => {
    ...
    this.setState({ detectorId, detectors, selectedJob }, () => {
      this.updateControlsForDetector(() => {
        ...
        mlFieldFormatService
          .populateFormats([jobId], getIndexPatterns())
          .catch(err => {
