    ...
    expect(filter).toEqual({
      geo_shape: {
        location: {
          relation: 'INTERSECTS',
          shape: {
            coordinates: [[[-89, 39], [-89, 35], [-83, 35], [-83, 39], [-89, 39]]],
            type: 'Polygon',
          },
        },
      },
    ...
    expect(filter).toEqual({
      geo_shape: {
        location: {
          relation: 'INTERSECTS',
          shape: {
            coordinates: [[[-180, 39], [-180, 35], [180, 35], [180, 39], [-180, 39]]],
            type: 'Polygon',
          },
        },
      },
...
describe('convertMapExtentToPolygon', () => {
  it('should convert bounds to envelope', () => {
    ...
    expect(convertMapExtentToPolygon(bounds)).toEqual({
      type: 'Polygon',
      coordinates: [[[90, 10], [90, -10], [100, -10], [100, 10], [90, 10]]],
    });
  });
  ...
  it('should clamp longitudes to -180 to 180', () => {
    ...
    expect(convertMapExtentToPolygon(bounds)).toEqual({
      type: 'Polygon',
      coordinates: [[[-180, 10], [-180, -10], [180, -10], [180, 10], [-180, 10]]],
    });
  });
