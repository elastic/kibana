...
export function LayerSettings(props) {
  ...
  const onZoomChange = ([min, max]) => {
    props.updateMinZoom(props.layerId, Math.max(props.minZoomForData, parseInt(min, 10)));
    props.updateMaxZoom(props.layerId, Math.min(props.maxZoomForData, parseInt(max, 10)));
  };
  ...
  const renderZoomSliders = () => {
    return (
      <ValidatedDualRange
        ...
        min={props.minZoomForData}
        max={props.maxZoomForData}
