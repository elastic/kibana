...
describe('getPaginatedNodes', () => {
  ...
  it('should return a subset based on the pagination parameters', async () => {
    const nodes = await getPaginatedNodes(req, esIndexPattern, { clusterUuid },
      metricSet, pagination, sort, queryText, { clusterStats, shardStats });
  ...
  it('should return a sorted subset', async () => {
    const nodes = await getPaginatedNodes(req, esIndexPattern, { clusterUuid },
      metricSet, pagination, { ...sort, field: 'foo', direction: 'desc' }, queryText, { clusterStats, shardStats });
    expect(nodes).toEqual({
      pageOfNodes: [
        { name: 'two', uuid: 2, isOnline: false, shardCount: 5, foo: 12 },
        { name: 'one', uuid: 1, isOnline: true, shardCount: 10, foo: 10 },
  ...
  it('should return a filterd subset', async () => {
    const nodes = await getPaginatedNodes(req, esIndexPattern, { clusterUuid },
      metricSet, pagination, sort, 'tw', { clusterStats, shardStats });
    expect(nodes).toEqual({
      pageOfNodes: [
        { name: 'two', uuid: 2, isOnline: false, shardCount: 5, foo: 12 },
      ],
