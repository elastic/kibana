 ...
 */
export const init = async (monitoringPlugin, server) => {
  ...
  async function getMonitoringCluster() {
    const configs = server.config().get('xpack.monitoring.elasticsearch');
    if (configs.hosts) {
  ...
  server.plugins.alerting.setup.registerType(getLicenseExpiration(getMonitoringCluster));
};
