...
test('transformField maps field types to kibana index pattern data types', () => {
  ...
  expect(transformField({ name: 'testField', type: 'string' }).type).toBe(undefined);
...
test('transformField changes values based on other values', () => {
  ...
  expect(transformField({ name: 'testField', aggregatable: true, type: 'text' }).aggregatable).toBe(
    false
  );
  ...
  expect(transformField({ name: 'testField', doc_values: true, script: 'doc[]' }).doc_values).toBe(
    false
  );
  ...
  expect(transformField({ name: 'testField', type: 'object', enabled: false }).enabled).toBe(false);
  ...
  /* TODO: is this test useful or is logic missing in code? what if {type: binary and script: 'doc[]'} ? */
  expect(transformField({ name: 'testField', type: 'binary' }).scripted).toBe(false);
