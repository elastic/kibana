...
> Note: the workpad is loaded by `fetch`, therefore the runtime cannot be initialized on the local file system. URLs may be relative.
...
The runtime is a global library with `KbnCanvas` as the namespace. When executed, the `share` method will interpret any and all nodes that match the API. The init can be called from anywhere, in a script block at the bottom of the page or after any other initialization.
...
See [Supported Expressions](#supported-expressions)
...
To minimize the distribution size, we opted to avoid as many libraries as possible in the UI. So while Canvas uses Redux to maintain state, we opted for a React Hooks-based approach with a custom context and reducer. This code can be found in `shareable_runtime/context`.
