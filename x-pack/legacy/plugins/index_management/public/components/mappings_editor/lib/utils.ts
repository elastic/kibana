...
const stateWithValidity = ['configuration', 'fieldsJsonEditor', 'fieldForm'];

export const isStateValid = (state: { [key: string]: any }): boolean | undefined =>
  Object.entries(state)
    .filter(([key]) => stateWithValidity.includes(key))
    .reduce(
      (isValid, { 1: value }) => {
        if (value === undefined) {
          return isValid;
