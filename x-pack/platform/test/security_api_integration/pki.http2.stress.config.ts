...
/**
 * FTR config for the PKI HTTP/2 RST_STREAM stress test (kibana#258232).
 *
 * Extends pki.http2.config.ts (same ES + Kibana setup) but runs only the
 * stress test suite and adds debug-level security logging so failures are
...
export default async function ({ readConfigFile }: FtrConfigProviderContext) {
  ...
  const config = configureHTTP2({
    ...
    junit: {
      reportName: 'X-Pack Security API Integration Tests HTTP/2 PKI RST_STREAM Stress',
    },
  });
