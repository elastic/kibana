...
test.describe('Session Concurrent Limit', { tag: [...tags.stateful.classic] }, () => {
  ...
  test.describe('Cleanup', () => {
    ...
    test(
      'should properly clean up sessions that exceeded concurrent session limit',
    ...
    test(
      'should properly clean up sessions that exceeded concurrent session limit when legacy sessions are present',
      async ({ apiClient, config, esClient }) => {
        test.setTimeout(100000);
    ...
    test(
      'should not clean up session if the limit is not exceeded',
      async ({ apiClient, config, esClient }) => {
        test.setTimeout(100000);
    ...
    test(
      'should not clean up sessions of the anonymous users',
      async ({ apiClient, config, esClient }) => {
        test.setTimeout(100000);
    ...
    test(
      'should not clean up unauthenticated sessions',
      async ({ apiClient, config, esClient }) => {
        test.setTimeout(100000);
  ...
  test.describe('Global Limit', () => {
    ...
    test(
      'should properly enforce session limit with single provider',
      async ({ apiClient, config, esClient }) => {
        const cookieOne = await loginWithBasic(apiClient, TEST_USERNAME, TEST_PASSWORD);
    ...
    test(
      'should properly enforce session limit with single provider and multiple users',
      async ({ apiClient, config, esClient }) => {
    ...
    test(
      'should properly enforce session limit with multiple providers',
      async ({ apiClient, config, esClient }) => {
        const basicCookieOne = await loginWithBasic(apiClient, TEST_USERNAME, TEST_PASSWORD);
        const basicCookieTwo = await loginWithBasic(apiClient, TEST_USERNAME, TEST_PASSWORD);
        const samlCookieOne = await loginWithSAML(apiClient);
