...
export default function filterRulesWithGapsTests({ getService }: FtrProviderContext) {
  const supertest = getService('supertest');
  const supertestWithoutAuth = getService('supertestWithoutAuth');

  describe('filter rules with gaps', () => {
    ...
    for (const scenario of UserAtSpaceScenarios) {
      ...
      describe(scenario.id, () => {
        ...
        describe(`filter rules with gaps (${scenario.id})`, () => {
          beforeEach(async () => {
            await supertest
