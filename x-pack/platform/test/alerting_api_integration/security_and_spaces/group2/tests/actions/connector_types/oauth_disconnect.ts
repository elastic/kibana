...
async function performOAuthFlow(
  supertestWithoutAuth: ReturnType<FtrProviderContext['getService']>,
  {
    ...
    connectorId,
    sessionCookie,
  }: { spaceId: string; connectorId: string; sessionCookie: string }
): Promise<void> {
  const { body: startFlowResponse } = await (supertestWithoutAuth as any)
  ...
  const callbackResponse = await (supertestWithoutAuth as any)
    .get(
      `${getUrlPrefix(spaceId)}/api/actions/connector/_oauth_callback?code=fake-auth-code&state=${
  ...
  describe('OAuth _oauth_disconnect', () => {
    ...
    describe('user with actions "all" privilege', () => {
      ...
      it('should disconnect after a successful OAuth flow', async () => {
        ...
        await (supertestWithoutAuth as any)
          .post(
