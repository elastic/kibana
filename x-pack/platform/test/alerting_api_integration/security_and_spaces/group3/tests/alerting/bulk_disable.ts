...
export default ({ getService }: FtrProviderContext) => {
  ...
  describe('bulkDisableRules', () => {
    ...
    describe('internally managed rule types', () => {
      ...
      it('should throw 400 error when trying to bulk disabling an internally managed rule type using the ids param', async () => {
        const { body: createdRule1 } = await supertest
        ...
        const response = await supertest
          .patch('/internal/alerting/rules/_bulk_disable')
          .set('kbn-xsrf', 'foo')
      ...
      it('should throw 400 error when trying to bulk disabling an internally managed rule type using the filter param', async () => {
        const { body: createdRule1 } = await supertest
          .post('/api/alerts_fixture/rule/internally_managed')
          .set('kbn-xsrf', 'foo')
          .send({ ...rulePayload, tags: ['internally-managed'] })
