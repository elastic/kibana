...
export default function createGapAutoFillSchedulerTests({ getService }: FtrProviderContext) {
  ...
  describe('gap auto fill scheduler - create and schedule backfills', () => {
    ...
    for (const scenario of ScenariosToTest) {
      ...
      describe(scenario.id, () => {
        ...
        it('creates scheduler and (for authorized) eventually creates a system backfill', async () => {
          ...
          // Unauthorized scenarios are rejected regardless of the rule and gap fixtures, so
          // assert the denial without paying for them.
          switch (scenario.id) {
            case 'no_kibana_privileges at space1':
            case 'space_1_all at space2':
            case 'global_read at space1': {
              const deniedResp = await supertestWithoutAuth
                .post(
                  `${getUrlPrefix(
                    apiOptions.spaceId
              ...
              expect(deniedResp.statusCode).to.eql(403);
              return;
            }
    ...
    // Scheduler behavior and request validation do not vary by role, so they run once.
