...
export default function getRuleIdsWithGapsPart2Tests({ getService }: FtrProviderContext) {
  const supertest = getService('supertest');
  const supertestWithoutAuth = getService('supertestWithoutAuth');

  describe('get rules with gaps - part 2', () => {
    ...
    for (const scenario of UserAtSpaceScenarios) {
      ...
      describe(scenario.id, () => {
        ...
        describe(`get rules with gaps part 2 (${scenario.id})`, () => {
          beforeEach(async () => {
            await supertest
