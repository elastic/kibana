...
export default function findGapsTests({ getService }: FtrProviderContext) {
  ...
  describe('find gaps', () => {
    ...
    for (const scenario of UserAtSpaceScenarios) {
      ...
      describe(scenario.id, () => {
        ...
        describe('find gaps with request body', () => {
          it('should find gaps for a rule', async () => {
            // The unauthorized error message names the rule type and consumer, so the rule has
            // to exist even for the denied scenarios.
    ...
    // Query parameter handling and request validation do not vary by role, so they run once.
