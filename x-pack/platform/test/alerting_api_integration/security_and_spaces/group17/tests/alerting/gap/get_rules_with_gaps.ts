...
export default function getRuleIdsWithGapsTests({ getService }: FtrProviderContext) {
  ...
  describe('get rules with gaps', () => {
      ...
      describe(scenario.id, () => {
        ...
        describe(`get rules with gaps (${scenario.id})`, () => {
          ...
          it('should return empty result when no gaps exist', async () => {
            ...
            // the denial without paying for the rule fixture.
            switch (scenario.id) {
              case 'no_kibana_privileges at space1':
              case 'space_1_all at space2': {
                const deniedResponse = await supertestWithoutAuth
                  .post(
                    `${getUrlPrefix(apiOptions.spaceId)}/internal/alerting/rules/gaps/_get_rules`
                  )
                  .set('kbn-xsrf', 'foo')
    ...
    // Response-shape and validation coverage that does not vary by role, so it runs once.
