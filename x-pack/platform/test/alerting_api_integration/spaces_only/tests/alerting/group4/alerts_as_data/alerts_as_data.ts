...
export default function createAlertsAsDataInstallResourcesTest({ getService }: FtrProviderContext) {
  ...
  describe('alerts as data', function () {
    ...
    for (const enableFlapping of [false, true]) {
      ...
      it(`should write alert docs during rule execution with flapping.enabled: ${enableFlapping}`, async () => {
        ...
        expect(alertADocRun2[ALERT_FLAPPING_HISTORY]).to.eql(
          enableFlapping ? [...alertADocRun1[ALERT_FLAPPING_HISTORY]!, false] : []
        );
        ...
        expect(alertBDocRun2[ALERT_FLAPPING_HISTORY]).to.eql(
          enableFlapping ? [...alertBDocRun1[ALERT_FLAPPING_HISTORY]!, true] : []
        );
        ...
        expect(alertCDocRun2[ALERT_FLAPPING_HISTORY]).to.eql(
          enableFlapping ? [...alertCDocRun1[ALERT_FLAPPING_HISTORY]!, true] : []
        );
        // event.action set to close
        ...
        expect(alertADocRun3[ALERT_FLAPPING_HISTORY]).to.eql(
          enableFlapping ? [...alertADocRun2[ALERT_FLAPPING_HISTORY]!, false] : []
        );
        ...
        // but if flapping is enabled its flapping history should be updated
        ...
        expect(alertCDocRun3[ALERT_FLAPPING_HISTORY]).to.eql(
          enableFlapping ? [...alertCDocRun2[ALERT_FLAPPING_HISTORY]!, true] : []
        );
        // event.action should be 'open'
  ...
  async function setFlappingSettings(enabled: boolean) {
    await supertestWithoutAuth
      .post(`${getUrlPrefix(Spaces.space1.id)}/internal/alerting/rules/settings/_flapping`)
      .set('kbn-xsrf', 'foo')
      .auth('superuser', 'superuser')
      .send({ enabled, look_back_window: 20, status_change_threshold: 4 })
      .expect(200);
  }
}
