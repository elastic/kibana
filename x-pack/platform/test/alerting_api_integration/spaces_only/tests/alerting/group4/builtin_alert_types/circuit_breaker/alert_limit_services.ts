  ...
  describe('rule that hits max alerts circuit breaker', () => {
    ...
    it('short circuits rule execution if rule type lets framework handle alert limit', async () => {
      ...
      expect(executeEvent?.message).to.eql(
        'Rule reported more than the maximum number of alerts in a single run. This limit helps prevent actions being triggered excessively. Consider narrowing your rule\'s query results or adjusting the limit configuration under "xpack.alerting.rules.run.alerts.max". Alerts may be missed and recovery notifications may be delayed'
      );
