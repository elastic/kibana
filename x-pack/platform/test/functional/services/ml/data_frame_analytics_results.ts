...
export function MachineLearningDataFrameAnalyticsResultsProvider(
  ...
  return {
    ...
    async assertFeatureImportancePopoverContent() {
      ...
      if (await testSubjects.waitForExists('mlDFADecisionPathJSONViewer', { timeout: 1000 })) {
      ...
      } else if (await testSubjects.waitForExists('mlDFADecisionPathPopover', { timeout: 1000 })) {
    ...
    async expandContentSection(sectionId: string, shouldExpand: boolean) {
      ...
      const expandableContentExists = await testSubjects.waitForExists(contentSubj, {
        timeout: 1000,
      });
