...
export default function ({ getService, getPageObjects }: FtrProviderContext) {
  ...
  const createDrilldown = async () => {
    ...
    await retry.waitForWithTimeout(
      'dashboard drilldown to be saved',
      10000,
      async () => (await dashboardDrilldownPanelActions.getPanelDrilldownCount()) === 1
    );
    ...
    describe('Create & use drilldowns', () => {
      ...
      describe('test dashboard to dashboard drilldown with controls', () => {
        ...
        it('creates filter pills representing controls selections', async () => {
          ...
          const [, rangeSliderControl] = await dashboardControls.getAllControlIds();
          await dashboardControls.validateRange('value', rangeSliderControl, '1000', '15000');
