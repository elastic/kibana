...
export default ({ getService, loadTestFile, getPageObjects }: FtrProviderContext) => {
  ...
  describe('lens app - group 1', () => {
    ...
    if (config.get('esTestCluster.ccs')) {
      ...
      // Layers, chart switching and the dimension editor moved to Scout for the non-CCS run
      // (see x-pack/platform/plugins/shared/lens/test/scout/smokescreen). They stay here for
      // CCS because Scout cannot target a real remote cluster yet.
