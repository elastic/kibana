  ...
  describe('Queries API', function () {
      ...
      const queries = [
        {
          ...
          esql: { where: 'KQL("message:\'OutOfMemoryError\'")' },
        },
        {
          id: v4(),
          title: 'cluster_block_exception',
          kql: { query: "message:'cluster_block_exception'" },
          esql: { where: 'KQL("message:\'cluster_block_exception\'")' },
        },
      ];
    ...
    it('bulks insert and remove queries', async () => {
      const firstQuery = {
        ...
        title: 'first query',
        kql: { query: 'query 1' },
        esql: { where: 'KQL("query 1")' },
      };
      const secondQuery = {
        id: 'second',
        title: 'second query',
        kql: { query: 'query 2' },
        esql: { where: 'KQL("query 2")' },
      };
      const thirdQuery = {
        id: 'third',
        title: 'third query',
        kql: { query: 'query 3' },
        esql: { where: 'KQL("query 3")' },
      };
      await putStream(apiClient, STREAM_NAME, {
        stream,
        ...emptyAssets,
