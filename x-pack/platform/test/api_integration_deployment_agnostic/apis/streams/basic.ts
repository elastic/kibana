...
export default function ({ getService }: DeploymentAgnosticFtrProviderContext) {
  ...
  describe('Streams basic API', () => {
    describe('GET /api/streams list type query parameter', () => {
      const listTypeFilterClassicDataStream = 'logs-list-type-filter-default';
      let listTypeFilterApiClient: StreamsSupertestRepositoryClient;

      before(async () => {
        listTypeFilterApiClient = await createStreamsRepositoryAdminClient(roleScopedSupertest);
        // Fresh clusters can have zero classic streams; add an unmanaged classic data stream for assertions.
        await indexDocument(esClient, listTypeFilterClassicDataStream, {
          '@timestamp': new Date().toISOString(),
          message: 'stream list type filter fixture',
        });
      });

      after(async () => {
        await esClient.indices
          .deleteDataStream({ name: listTypeFilterClassicDataStream })
          .catch(() => {});
      });

      it('returns only wired streams when type=wired', async () => {
        await retry.tryForTime(
          120_000,
          async () => {
            const {
              body: { streams },
              status: listResponseStatus,
            } = await listTypeFilterApiClient.fetch('GET /api/streams 2023-10-31', {
              params: { query: { type: 'wired' } },
            });

            expect(listResponseStatus).to.eql(200);
            expect(streams).to.be.an('array');
            expect(streams.length).to.be.greaterThan(0);
            expect(streams.every((stream) => stream.type === 'wired')).to.eql(true);
          },
          undefined,
          500
        );
      ...
      it('returns only classic streams when type=classic', async () => {
        await retry.tryForTime(
          120_000,
          async () => {
            const {
              ...
              params: { query: { type: 'classic' } },
            });

            expect(listResponseStatus).to.eql(200);
            expect(streams).to.be.an('array');
            expect(streams.length).to.be.greaterThan(0);
            expect(streams.every((stream) => stream.type === 'classic')).to.eql(true);
            expect(
              streams.some((stream) => stream.name === listTypeFilterClassicDataStream)
            ).to.eql(true);
          },
          undefined,
          500
      ...
      it('returns only query streams when type=query', async () => {
        await retry.tryForTime(
          120_000,
          async () => {
            const {
            ...
            expect(listResponseStatus).to.eql(200);
            expect(streams).to.be.an('array');
            expect(streams.every((stream) => stream.type === 'query')).to.eql(true);
          },
          undefined,
          500
        );
      ...
      it('rejects invalid type query value', async () => {
        const { status: listResponseStatus } = await listTypeFilterApiClient.fetch(
          'GET /api/streams 2023-10-31',
          {
            params: { query: { type: 'invalid' } },
          }
        );
        expect(listResponseStatus).to.eql(400);
      });
    });
    ...
    describe.skip('Basic functionality', () => {
      ...
      describe('Full flow for logs.ecs', () => {
        ...
        it(`Index an Apache error log message, should goto ${rootStream}.apache.error`, async () => {
          ...
          expect((result._source as JsonObject)['stream.name']).to.eql(
            `${rootStream}.apache.error`
          );
        });
      ...
      describe('Basic setup - validation', () => {
        ...
        it('persists description-only overrides without freezing inherited mappings', async () => {
          ...
          try {
            ...
            const {
              updated_at: _parentForUpdateProcessingUpdatedAt,
              ...parentProcessingForUpdate
