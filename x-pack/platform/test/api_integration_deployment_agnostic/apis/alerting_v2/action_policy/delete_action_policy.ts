...
export default function ({ getService }: DeploymentAgnosticFtrProviderContext) {
  ...
  describe('Delete Action Policy API', function () {
    ...
    it('should delete a action policy by id', async () => {
      ...
      const response = await supertestWithoutAuth
        .delete(`${ACTION_POLICY_API_PATH}/${createdPolicyId}`)
        .set(roleAuthc.apiKeyHeader)
        .set(samlAuth.getInternalRequestHeader());
    ...
    it('should return 404 when deleting a non-existent action policy', async () => {
      const response = await supertestWithoutAuth
        .delete(`${ACTION_POLICY_API_PATH}/non-existent-id`)
        .set(roleAuthc.apiKeyHeader)
        .set(samlAuth.getInternalRequestHeader());
