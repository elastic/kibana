...
1. Next you want each instance of Kibana to run with its own UUID as that is used to identify each Kibana's owned tasks. In the file `x-pack/platform/test/functional/config.base.ts` simple change the uuid on the line `--server.uuid=` into any random UUID.
