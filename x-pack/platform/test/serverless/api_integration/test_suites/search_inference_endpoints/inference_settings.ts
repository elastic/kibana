    ...
    describe('space isolation', function () {
      ...
      before(async () => {
        await adminSupertest
          ...
          .send({ id: SPACE_A, name: 'Space A', disabledFeatures: [] })
          .expect(200);
        await adminSupertest
          .post('/api/spaces/space')
          .send({ id: SPACE_B, name: 'Space B', disabledFeatures: [] })
          .expect(200);
      });

      after(async () => {
        await adminSupertest.delete(`/api/spaces/space/${SPACE_A}`).expect(204);
        await adminSupertest.delete(`/api/spaces/space/${SPACE_B}`).expect(204);
      });

      afterEach(async () => {
        await adminSupertest.put(spaceApiPath(SPACE_A)).send({ features: [] }).expect(200);
        await adminSupertest.put(spaceApiPath(SPACE_B)).send({ features: [] }).expect(200);
      });
      ...
      it('settings saved in one space should not be visible in another', async () => {
        ...
        await adminSupertest.put(spaceApiPath(SPACE_A)).send(settingsA).expect(200);

        const { body } = await adminSupertest.get(spaceApiPath(SPACE_B)).expect(200);
      ...
      it('settings saved in one space should be readable from the same space', async () => {
        ...
        const { body } = await adminSupertest.get(spaceApiPath(SPACE_A)).expect(200);
      ...
      it('each space should maintain its own independent settings', async () => {
        ...
        await adminSupertest.put(spaceApiPath(SPACE_A)).send(settingsA).expect(200);
        await adminSupertest.put(spaceApiPath(SPACE_B)).send(settingsB).expect(200);

        const { body: bodyA } = await adminSupertest.get(spaceApiPath(SPACE_A)).expect(200);
        const { body: bodyB } = await adminSupertest.get(spaceApiPath(SPACE_B)).expect(200);
