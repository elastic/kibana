...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('Unified Osquery — CRUD with flag ON', () => {
    ...
    describe('create', () => {
      ...
      // (`common/attachments_framework_legacy/osquery.ts`); a 400 is the contract
      // regardless of feature-flag state.
      it('rejects a legacy externalReference `osquery` payload with unknown metadata keys', async () => {
        const postedCase = await createCase(supertest, postCaseReq);
        await createComment({
          supertest,
          caseId: postedCase.id,
      ...
      it('rejects a unified `osquery` payload missing a required metadata field', async () => {
        const postedCase = await createCase(supertest, postCaseReq);
        await createComment({
          supertest,
          caseId: postedCase.id,
          params: unifiedOsqueryPayload({ metadata: { agentIds: ['agent-1'] } }),
          expectedHttpCode: 400,
        });
      });

      it('rejects a unified `osquery` payload with an unknown top-level key', async () => {
        const postedCase = await createCase(supertest, postCaseReq);
        await createComment({
          supertest,
          caseId: postedCase.id,
