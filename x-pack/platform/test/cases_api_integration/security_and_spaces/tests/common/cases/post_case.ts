...
export default ({ getService }: FtrProviderContext): void => {
  ...
  describe('post_case', () => {
    ...
    describe('happy path', () => {
      ...
      it('should create a user action when creating a case generated by assistant', async () => {
        const postedCase = await createCase(supertest, getPostCaseRequest({ isAssistant: true }));
        const { userActions } = await findCaseUserActions({ supertest, caseID: postedCase.id });
        const creationUserAction = removeServerGeneratedPropertiesFromUserAction(userActions[0]);
