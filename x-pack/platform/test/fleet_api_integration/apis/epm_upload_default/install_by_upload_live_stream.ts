...
export default function (providerContext: FtrProviderContext) {
  ...
  describe('Install by upload with ownerless live-stream protection', () => {
    ...
    // 8.19 does not have the later custom-dataset template installer for
    // integration packages, so this mutation-time path is not present.
    it.skip('rejects a package policy when an uploaded package later targets an ownerless stream through a custom dataset', async () => {
      const original = await createOwnerlessLiveStream();
      const buf = await buildUploadPackageZip({
        name: MUTATE_PACKAGE,
