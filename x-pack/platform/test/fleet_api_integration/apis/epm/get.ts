  ...
  describe('EPM - get', () => {
    ...
    describe('Knowledge Base', () => {
      ...
      it('includes knowledge base information in package info assets when fetching from the info endpoint', async function () {
        ...
        // Verify knowledge base items have correct structure with generated IDs
        // Note: IDs are now generated by Elasticsearch, so we verify they exist but don't check specific values
        ...
        knowledgeBaseItems.forEach((item: any) => {
          ...
          expect(item.id).to.not.be.empty(); // ID should be a non-empty string generated by ES
        });
      });
    });
  });
