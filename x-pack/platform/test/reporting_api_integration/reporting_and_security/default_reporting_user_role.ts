...
export default function ({ getService }: FtrProviderContext) {
  ...
  describe('Default reporting_user role', () => {
    ...
    it('details of the reserved feature privileges of reporting_user', async () => {
      const role = await es.security.getRole({ name: 'reporting_user' });
      expect(role).toEqual({
        reporting_user: {
          cluster: [],
          ...
          indices: [],
          applications: [
            { application: 'kibana-*', privileges: ['reserved_reporting_user'], resources: ['*'] },
          ],
          run_as: [],
    ...
    describe('Report generation with default reporting_user role', () => {
