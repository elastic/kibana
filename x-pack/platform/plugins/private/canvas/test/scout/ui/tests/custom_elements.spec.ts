...
test.describe('Canvas custom elements', { tag: testData.CANVAS_UI_TAGS }, () => {
  ...
  test('create → add → edit → delete a custom element', async ({
    ...
    await test.step('create custom element from the first workpad element', async () => {
      ...
      await page.testSubj.locator('canvasWorkpadEditMenuButton').click({ timeout: 20_000 });
      ...
      await expect(canvas.customElementCreateSuccessToast).toBeVisible({ timeout: 30_000 });
    });
    ...
    await test.step('add the custom element to the workpad', async () => {
      ...
      await expect(canvas.workpadPageElements).toHaveCount(5, { timeout: 30_000 });
      ...
      await expect(canvas.workpadPageElements).toHaveCount(4, { timeout: 15_000 });
    });
    ...
    await test.step('edit the custom element name and description', async () => {
      ...
      await page.testSubj.locator('canvasElementCard__editButton').click({ timeout: 20_000 });
    ...
    await test.step('delete the custom element', async () => {
      ...
      await page.testSubj.locator('canvasElementCard__deleteButton').click({ timeout: 20_000 });
      await page.testSubj.locator('confirmModalConfirmButton').click({ timeout: 20_000 });

      await expect(canvas.elementCards).toHaveCount(0, { timeout: 15_000 });
