...
describe('ConfirmMigrationReadonlyFlyoutStep', () => {
  ...
  it('blocks start until all warning checkboxes are checked', () => {
    ...
    expect(screen.getByTestId('readonlyDataStreamModalTitle')).toHaveTextContent(
      'Set data stream to read-only'
    );
    expect(screen.getByTestId('dataStreamMigrationWarningsCallout')).toBeInTheDocument();
    expect(screen.getByTestId('readOnlyDsWarningCallout')).toBeInTheDocument();
