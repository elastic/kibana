 ...
 *   (2) A tighter loop that pushes each in progress reindex operation through DataStreamReindexService.processNextStep. If all
 ...
 * DataStreamReindexService and if any operation is locked longer than the DataStreamReindexService's timeout, it is assumed to have been
 ...
 */
export class DataStreamReindexWorker {
  ...
  public static create(
  ...
  ): DataStreamReindexWorker {
    if (DataStreamReindexWorker.workerSingleton) {
      log.debug(`More than one ReindexWorker cannot be created, returning existing worker.`);
    } else {
      DataStreamReindexWorker.workerSingleton = new DataStreamReindexWorker(
        client,
        credentialStore,
        clusterClient,
        log,
    ...
    return DataStreamReindexWorker.workerSingleton;
  }
  ...
  private constructor(
    ...
    this.reindexService = dataStreamReindexServiceFactory(
      callAsInternalUser,
      dataStreamReindexActionsFactory(this.client, callAsInternalUser),
      log,
      this.licensing
    );
  }
  ...
  private getCredentialScopedReindexService = (credential: Credential) => {
    ...
    const actions = dataStreamReindexActionsFactory(this.client, callAsCurrentUser);
    return dataStreamReindexServiceFactory(callAsCurrentUser, actions, this.log, this.licensing);
  };
