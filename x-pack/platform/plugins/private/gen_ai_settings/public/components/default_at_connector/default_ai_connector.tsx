import { EuiCheckbox, EuiComboBox, EuiComboBoxOptionOption, EuiFormRow } from '@elastic/eui';
...
const getOptions = (connectors: UseGenAiConnectorsResult): EuiComboBoxOptionOption<string>[] => {
    const preconfigured = connectors.connectors
    ...
    const custom = connectors.connectors
        ?.filter(connector => !connector.isPreconfigured)
        .map(connector => ({
            label: connector.name,
            value: connector.id,
    ...
    return [
        ...
        {
            label: i18n.translate(
                'xpack.gen_ai_settings.settings.defaultLLm.select.group.custom.label',
...
export const DefaultAIConnector: React.FC<Props> = ({ hasConnectorsAllPrivilege, connectors }) => {
    ...
    const selectedOptions = useMemo(() => getOptionsByValues(defaultLlmValues, options),
        ...
        <>
            ...
            >
                <EuiComboBox
                    ...
                    isDisabled={!hasConnectorsAllPrivilege || fields[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR]?.isReadOnly}
 ...
 */
function getDefaultLlmValue(unsavedChanges: Record<string, UnsavedFieldChange<UiSettingsType>>, fields: Record<string, FieldDefinition<UiSettingsType>>) {
    const defaultLlmUnsavedValue = unsavedChanges[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR]?.unsavedValue as string | undefined;
    const defaultLlmSavedValue = fields[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR]?.savedValue as string | undefined;
    const defaultLlmDefaultValue = fields[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR]?.defaultValue as string | undefined;

    const defaultLlmValue = defaultLlmUnsavedValue ??
 ...
 */
function getDefaultLlmOnlyValue(unsavedChanges: Record<string, UnsavedFieldChange<UiSettingsType>>, fields: Record<string, FieldDefinition<UiSettingsType>>): boolean {
    const defaultLlmOnlyUnsavedValue = unsavedChanges[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR_DEFAULT_ONLY]?.unsavedValue as boolean | undefined;
    const defaultLlmOnlySavedValue = fields[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR_DEFAULT_ONLY]?.savedValue as boolean | undefined;
    const defaultLlmOnlyDefaultValue = fields[GEN_AI_SETTINGS_DEFAULT_AI_CONNECTOR_DEFAULT_ONLY]?.defaultValue as boolean | undefined;

    const defaultLlmOnlyValue = defaultLlmOnlyUnsavedValue ??
