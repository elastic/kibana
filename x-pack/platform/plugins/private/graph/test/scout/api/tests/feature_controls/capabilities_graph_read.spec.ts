...
apiTest.describe(
  'POST /api/core/capabilities - user with graph:read',
  { tag: testData.GRAPH_TAGS },
  () => {
    let credentials: RoleApiCredentials;

    apiTest.beforeAll(async ({ requestAuth }) => {
      credentials = await requestAuth.getApiKeyForCustomRole(testData.GRAPH_READ_ROLE);
    });

    apiTest('exposes graph navlink with show but not save', async ({ apiClient }) => {
