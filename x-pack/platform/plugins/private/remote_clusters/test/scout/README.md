...
Test results are written relative to each Playwright config (`ui/` or `api/`): HTML/JSON reports go to `.scout/reports` and other artifacts (screenshots, traces) to `.scout/test-artifacts`.
