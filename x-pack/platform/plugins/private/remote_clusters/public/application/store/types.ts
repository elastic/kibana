...
export interface ClustersState {
  isLoading: boolean;
  /** Unvalidated HTTP error from loadClusters; consumed by clusterLoadError selector and rendered in RemoteClusterList error UI. Common shape: Kibana HttpFetchError with body.statusCode / body.error. */
...
export interface AddClusterState {
  isAdding: boolean;
  /** Unvalidated HTTP error from addCluster action; narrowed to RequestError in getAddClusterError selector. */
  error: unknown;
}
...
export interface EditClusterState {
  clusterName: string | undefined;
  isEditing: boolean;
  /** Unvalidated HTTP error from editCluster action; narrowed to RequestError in getEditClusterError selector. */
  error: unknown;
}
