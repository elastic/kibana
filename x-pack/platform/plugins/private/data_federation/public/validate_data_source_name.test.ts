...
import { isValidDataSourceName, validateIndexNameRules } from '../common';
...
describe('isValidDataSourceName', () => {
  ...
  validNames.forEach((validName) => {
    it(`accepts "${validName}"`, () => {
      expect(isValidDataSourceName(validName)).toEqual(true);
    });
  });
  ...
  invalidNames.forEach((invalidName) => {
    it(`rejects "${invalidName}"`, () => {
      expect(isValidDataSourceName(invalidName)).toEqual(false);
    });
  });
});
