/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import { isValidIndexName, validateIndexNameRules } from './valdiate_index_name';

describe('isValidIndexName', () => {
  const validNames = [
    'lowercaseletters', // Lowercase only
    '123', // Cannot include \, /, *, ?, ", <, >, |, space, , or #
    'does_not_start_with_underscores', // Cannot start with _
    'does-not-start-with-a-dash', // Cannot start with -
    'does+not+start+with+a+plus', // Cannot start with +
    'is..not..just..two..periods', // name can't be ..
    'is.not.just.one.period', // name can't be .
    'x'.repeat(255), // Cannot be longer than 255 bytes
  ];

  validNames.forEach((validName) => {
    it(`accepts "${validName}"`, () => {
      expect(isValidIndexName(validName)).toEqual(true);
    });
  });

  const invalidNames = [
    'someUpperCaseLetters', // Lowercase only
    '1\\2\\3', // Cannot include \
    '1/2/3', // Cannot include /
    '1*2*3', // Cannot include *
    '1?2?3', // Cannot include ?
    '1"2"3', // Cannot include "
    '1<2<3', // Cannot include <
    '1>2>3', // Cannot include >
    '1|2|3', // Cannot include |
    '1 2 3', // Cannot include space character
    '1,2,3', // Cannot include ,
    '1#2#3', // Cannot include #
    'a:b', // Colon is deprecated and not supported
    '_starts_with_underscores', // Cannot start with _
    '-starts-with-a-dash', // Cannot start with -
    '+starts+with+a+plus', // Cannot start with +
    '.starts_with_dot', // Can't start with .
    '..', // name can't be ..
    '.', // name can't be .
    'x'.repeat(256), // Cannot be longer than 255 bytes
    'ü'.repeat(128), // Cannot be longer than 255 bytes (using 2 byte char)
  ];

  invalidNames.forEach((invalidName) => {
    it(`rejects "${invalidName}"`, () => {
      expect(isValidIndexName(invalidName)).toEqual(false);
    });
  });
});

describe('validateIndexNameRules', () => {
  it('returns null for a valid name', () => {
    expect(validateIndexNameRules('my-ds')).toBeNull();
  });

  it('returns { error, message } for an invalid name', () => {
    expect(validateIndexNameRules('SomeUpperCaseLetters')).toMatchObject({
      error: 'must_be_lowercase',
      message: 'Name must be lowercase.',
    });
  });
});
