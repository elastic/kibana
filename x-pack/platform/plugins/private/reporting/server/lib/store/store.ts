...
export class ReportingStore {
  ...
  private async rollDataStreamIfRequired(): Promise<void> {
    ...
    if (!exists) {
      this.logger.debug(`${msgPrefix} does not exist so ${skipMessage}`);
      return;
    }
    ...
    this.logger.debug(`${msgPrefix} template version: ${templateVersion}`);
    ...
    if (mappingsArray.length === 0) {
      this.logger.debug(`${msgPrefix} has no indices so ${skipMessage}`);
      return;
    }
    ...
    const mappingsVersion =
      mappingsVersions.length === 0 ? undefined : Math.max(...mappingsVersions);
    this.logger.debug(`${msgPrefix} mappings version: ${mappingsVersion ?? '<none>'}`);

    if (mappingsVersion === undefined) {
      // no mapping version found on any indices
      this.logger.info(`${msgPrefix} has no mapping versions so ${rollMessage}`);
    } else if (mappingsVersion < templateVersion) {
      // all mappings are old
      this.logger.info(`${msgPrefix} has older mappings than the template so ${rollMessage}`);
    } else if (mappingsVersion > templateVersion) {
      ...
      this.logger.debug(`${msgPrefix} has latest mappings applied so ${skipMessage}`);
      return;
    }
    ...
    this.logger.info(
      `${msgPrefix} rolled over to pick up index template version ${templateVersion}`
    );
  }
  ...
  public async start() {
    ...
    try {
      await this.rollDataStreamIfRequired();
    } catch (e) {
