...
import { farequoteLuceneSearchTestData } from '../fixtures/expected_field_stats_random_sampler';

spaceTest.describe(
  'index based with farequote lucene saved search',
  ...
  () => {
    ...
    spaceTest('displays index details', async ({ page, pageObjects }) => {
      await runIndexDataVisualizerTests({
        page,
        pageObjects,
        testData: farequoteLuceneSearchTestData,
      });
    });
  }
);
