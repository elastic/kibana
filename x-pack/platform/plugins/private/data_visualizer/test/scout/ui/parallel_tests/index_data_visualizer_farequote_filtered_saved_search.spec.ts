...
import { farequoteKQLFiltersSearchTestData } from '../fixtures/expected_field_stats_random_sampler';

spaceTest.describe(
  'index based with farequote KQL saved search and filters',
  ...
  () => {
    ...
    spaceTest('displays index details', async ({ page, pageObjects }) => {
      await runIndexDataVisualizerTests({
        page,
        pageObjects,
        testData: farequoteKQLFiltersSearchTestData,
      });
    });
  }
);
