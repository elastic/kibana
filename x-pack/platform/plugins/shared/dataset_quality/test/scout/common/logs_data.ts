...
/**
 * Wire this as a data stream's `default_pipeline` to route documents into the
 * failure store: it throws for any `log.level` outside the accepted set, and
 * `createFailedLogRecord` deliberately sends one that isn't.
 */
export const logLevelNormalizationProcessors = [
  {
    script: {
