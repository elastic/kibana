...
const DATASET = 'dq.fd.errors';
const DATA_STREAM = buildDataStreamName({ dataset: DATASET });
const PIPELINE_NAME = 'dq-fd-errors@pipeline';
const COMPONENT_TEMPLATE_NAME = 'dq-fd-errors@mappings';
...
apiTest.describe(
  'Dataset quality - failed docs errors of a data stream',
  ...
  () => {
    ...
    apiTest(
      'returns no errors while every document passes the ingest pipeline',
      async ({ apiClient, logsSynthtraceEsClient }) => {
        ...
        const response = await apiClient.get(failedDocsErrorsUrl(), {
    ...
    apiTest(
      'returns the ingest pipeline error that sent the document to the failure store',
        ...
        await expect
          .poll(
            async () => {
              ...
              return response.body.errors as FailedDocsError[];
            },
            { timeout: 60_000, intervals: [2_000] }
          )
          .toStrictEqual([{ type: 'script_exception', message: 'runtime error' }]);
      }
    );
  }
);
