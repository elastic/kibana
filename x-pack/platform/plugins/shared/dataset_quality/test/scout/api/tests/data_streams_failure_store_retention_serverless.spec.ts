...
/**
 * Serverless-only half of the failure-store retention checks: there the cluster
 * default failure-store retention is not exposed, so the details API must not
 * report one even for an elevated user.
 *
 * Every resource is prefixed for this spec only, so no other suite can leak into it.
 */
const DATASET = 'dq.fs.svl';
const DATA_STREAM = buildDataStreamName({ dataset: DATASET });
const COMPONENT_TEMPLATE_NAME = 'dq-fs-svl@mappings';
...
apiTest.describe(
  'Dataset quality - failure store retention of a data stream on serverless',
  ...
  () => {
    ...
    apiTest(
      'omits the defaultRetentionPeriod for a disabled failure store',
