...
- **context-engine.createKi**: Index a KI document into an AI index. Inputs: \`ai_index_id\`, \`ki\`, and an optional \`verifiers\` list. When the AI index does not exist yet, it is created automatically. Output: the document \`id\` of the created KI, plus \`verification\` (\`passed\` and per-verifier \`results\`) when \`verifiers\` was given. When any verifier fails, the KI is not written and \`id\` is absent. The write is not refreshed unless \`refresh: true\` is set; set it when a later step in the same run updates or deletes the KI.
