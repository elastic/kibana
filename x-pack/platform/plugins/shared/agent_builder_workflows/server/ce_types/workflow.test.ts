...
import type { CeDocument } from '@kbn/context-engine-plugin/server';
import type { ElasticsearchClient } from '@kbn/core-elasticsearch-server';
import type { Logger } from '@kbn/logging';
import { createWorkflowCeType } from './workflow';
...
const createCeDocument = (overrides: Partial<CeDocument> = {}): CeDocument => ({
  id: 'entry-1',
  type: 'workflow',
  title: 'My Workflow',
...
describe('workflowCeType', () => {
  describe('id and fetchFrequency', () => {
    it('has id "workflow"', () => {
      const ceType = createWorkflowCeType(createMockApi());
      expect(ceType.id).toBe('workflow');
    });

    it('returns 30m fetch frequency', () => {
      const ceType = createWorkflowCeType(createMockApi());
      expect(ceType.fetchFrequency!()).toBe('30m');
    });
  });
  ...
  describe('list', () => {
    it('lists workflows across all spaces', async () => {
      ...
      const pages = [];
      for await (const page of ceType.list({
        esClient,
    ...
    it('handles ES errors gracefully and logs warning', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('CE workflow: failed to list workflows')
      );
    });
  ...
  describe('getCeData', () => {
    it('returns entry with workflow metadata', async () => {
      ...
      const result = await ceType.getCeData('workflow-abc', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('queries ES with correct parameters', async () => {
      ...
      await ceType.getCeData('workflow-abc', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('returns undefined when workflow is not found', async () => {
      ...
      const result = await ceType.getCeData('nonexistent', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('handles missing optional fields gracefully', async () => {
      ...
      const result = await ceType.getCeData('workflow-minimal', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('falls back to originId as title when name is missing', async () => {
      ...
      const result = await ceType.getCeData('workflow-no-name', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('returns undefined and logs warning on ES error', async () => {
      ...
      expect(result).toBeUndefined();
      expect(logger.warn).toHaveBeenCalledWith(
        "CE workflow: failed to get data for 'workflow-abc': search_phase_execution_exception"
      );
    });
    ...
    it('omits empty tags and triggerTypes from content', async () => {
      ...
      const result = await ceType.getCeData('workflow-empty-arrays', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
  ...
  describe('getPermissions', () => {
    it('returns the api:workflowsManagement:read privilege', () => {
      const ceType = createWorkflowCeType(createMockApi());
      const permissions = ceType.getPermissions!('workflow-abc', {
  ...
  describe('toAttachment', () => {
    it('converts workflow to workflow.yaml attachment', async () => {
      ...
      const ceType = createWorkflowCeType(api);

      const result = await ceType.toAttachment(createCeDocument(), {
        savedObjectsClient: {} as never,
        request: {} as never,
        spaceId: 'default',
      });
    ...
    it('calls api.getWorkflow with correct origin_id and spaceId', async () => {
      ...
      await ceType.toAttachment(createCeDocument({ origin_id: 'workflow-xyz' }), {
        savedObjectsClient: {} as never,
        request: {} as never,
        spaceId: 'my-space',
      });
