...
import { WORKFLOW_CE_TYPE, WORKFLOW_YAML_ATTACHMENT_TYPE } from '@kbn/workflows/common/constants';
...
export const createWorkflowCeType = (api: WorkflowsManagementApi): CeTypeDefinition => ({
  id: WORKFLOW_CE_TYPE,
  fetchFrequency: () => '30m',
  ...
  async *list(context) {
    ...
    while (hasMore) {
      try {
        ...
        context.logger.warn(`CE workflow: failed to list workflows: ${(error as Error).message}`);
        return;
      }
    }
  },
  ...
  getCeData: async (originId, context) => {
    try {
      ...
      context.logger.warn(
        `CE workflow: failed to get data for '${originId}': ${(error as Error).message}`
      );
      return undefined;
    }
  },
  ...
  /**
   * Workflow entries are gated by the Workflows Management read API privilege —
