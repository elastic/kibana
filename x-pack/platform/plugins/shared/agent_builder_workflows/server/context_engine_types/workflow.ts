...
export const createWorkflowContextEngineType = (
  ...
  async *list(context) {
    ...
    while (hasMore) {
      try {
        ...
        context.logger.warn(
          `Context Engine workflow: failed to list workflows: ${(error as Error).message}`
        );
        return;
      }
    }
  ...
  getContextEngineData: async (originId, context) => {
    try {
      ...
      context.logger.warn(
        `Context Engine workflow: failed to get data for '${originId}': ${(error as Error).message}`
      );
      return undefined;
    }
  },
