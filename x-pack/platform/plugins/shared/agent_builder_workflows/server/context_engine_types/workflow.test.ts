...
import type { ContextEngineDocument } from '@kbn/context-engine-plugin/server';
import type { ElasticsearchClient } from '@kbn/core-elasticsearch-server';
import type { Logger } from '@kbn/logging';
import { createWorkflowContextEngineType } from './workflow';
...
const createContextEngineDocument = (
  overrides: Partial<ContextEngineDocument> = {}
...
describe('workflowContextEngineType', () => {
  describe('id and fetchFrequency', () => {
    it('has id "workflow"', () => {
      const contextEngineType = createWorkflowContextEngineType(createMockApi());
      expect(contextEngineType.id).toBe('workflow');
    });

    it('returns 30m fetch frequency', () => {
      const contextEngineType = createWorkflowContextEngineType(createMockApi());
      expect(contextEngineType.fetchFrequency!()).toBe('30m');
    });
  });
  ...
  describe('list', () => {
    it('lists workflows across all spaces', async () => {
      ...
      const pages = [];
      for await (const page of contextEngineType.list({
        esClient,
    ...
    it('handles ES errors gracefully and logs warning', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Context Engine workflow: failed to list workflows')
      );
    });
  ...
  describe('getContextEngineData', () => {
    it('returns entry with workflow metadata', async () => {
      const esClient = createMockEsClient([
        {
          _id: 'workflow-abc',
          _source: {
      ...
      const result = await contextEngineType.getContextEngineData('workflow-abc', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
      ...
      const { content } = result!.entries[0];
    ...
    it('queries ES with correct parameters', async () => {
      ...
      await contextEngineType.getContextEngineData('workflow-abc', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('returns undefined when workflow is not found', async () => {
      ...
      const result = await contextEngineType.getContextEngineData('nonexistent', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('handles missing optional fields gracefully', async () => {
      ...
      const result = await contextEngineType.getContextEngineData('workflow-minimal', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
    ...
    it('falls back to originId as title when name is missing', async () => {
      ...
      const result = await contextEngineType.getContextEngineData('workflow-no-name', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });

      expect(result!.entries[0].title).toBe('workflow-no-name');
    });
    ...
    it('returns undefined and logs warning on ES error', async () => {
      ...
      expect(result).toBeUndefined();
      expect(logger.warn).toHaveBeenCalledWith(
        "Context Engine workflow: failed to get data for 'workflow-abc': search_phase_execution_exception"
      );
    });
    ...
    it('omits empty tags and triggerTypes from content', async () => {
      ...
      const result = await contextEngineType.getContextEngineData('workflow-empty-arrays', {
        esClient,
        savedObjectsClient: {} as never,
        logger: createMockLogger(),
      });
  ...
  describe('getPermissions', () => {
    it('returns the api:workflowsManagement:read privilege', () => {
      const contextEngineType = createWorkflowContextEngineType(createMockApi());
      const permissions = contextEngineType.getPermissions!('workflow-abc', {
  ...
  describe('toAttachment', () => {
    it('converts workflow to workflow.yaml attachment', async () => {
      ...
      const contextEngineType = createWorkflowContextEngineType(api);

      const result = await contextEngineType.toAttachment(createContextEngineDocument(), {
        savedObjectsClient: {} as never,
        request: {} as never,
        spaceId: 'default',
      });
    ...
    it('calls api.getWorkflow with correct origin_id and spaceId', async () => {
      ...
      await contextEngineType.toAttachment(
        createContextEngineDocument({ origin_id: 'workflow-xyz' }),
        {
          savedObjectsClient: {} as never,
          request: {} as never,
          spaceId: 'my-space',
