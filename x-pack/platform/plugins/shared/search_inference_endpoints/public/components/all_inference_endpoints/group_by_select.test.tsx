...
describe('GroupBySelect', () => {
  ...
  it('should render the filter button with "Model" when model_id is selected', () => {
    ...
    expect(getByText(/Group:/i)).toBeInTheDocument();
    expect(getByText(/^Model$/i)).toBeInTheDocument();
  });
  ...
  it('should call onChange with GroupByOptions.Model when "Model" option is selected', async () => {
    const { getByRole } = render(
      <Wrapper>
        <GroupBySelect value={GroupByOptions.None} onChange={mockOnChange} />
      </Wrapper>
