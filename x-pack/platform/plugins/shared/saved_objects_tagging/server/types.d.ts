import type { IRouter, CustomRequestHandlerContext, SavedObjectsClientContract, SavedObjectReference } from '@kbn/core/server';
...
export interface SavedObjectTaggingStart {
     ...
     *          instead.
     */
    createInternalAssignmentService: (options: CreateTagAssignmentServiceOptions) => IAssignmentService;
    convertTagNameToId: (tagName: string, allTags: Tag[]) => string | undefined;
    getTagsFromReferences: (references: SavedObjectReference[], allTags: Tag[]) => {
        tags: Tag[];
        missingRefs: SavedObjectReference[];
    };
    replaceTagReferences: (references: SavedObjectReference[], newTagIds: string[]) => SavedObjectReference[];
}
/**
 * @internal
 */
