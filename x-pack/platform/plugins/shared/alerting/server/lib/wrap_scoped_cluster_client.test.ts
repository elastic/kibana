...
describe('wrapScopedClusterClient', () => {
  ...
  describe('transport.request', () => {
    describe('ES|QL', () => {
      ...
      test('throws error when es|ql async query delete throws abort error', async () => {
        const { abortController, scopedClusterClient, childClient } = getMockClusterClients();
        ...
        expect(loggingSystemMock.collect(logger).debug[0][0]).toEqual(
          `executing ES|QL query for rule .test-rule-type:abcdefg in space my-space - {\"method\":\"DELETE\",\"path\":\"/_query/async/FjhHTHlyRVltUm5xck1tV0RFN18wREEeOUxMcnkxZ3NTd0MzOTNabm1NQW9TUTozMjY1NjQ3\"} - with options {}`
        );
        expect(logger.warn).not.toHaveBeenCalled();
      });
    });
