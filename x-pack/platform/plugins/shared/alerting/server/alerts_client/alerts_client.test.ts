...
import * as RetryTransientEsErrorsModule from '../alerts_service/lib/retry_transient_es_errors';
    ...
    describe(`using ${label} for alert indices`, () => {
      ...
      describe('initializeExecution()', () => {
        ...
        test('should call retryTransientEsErrors when querying for tracked alerts', async () => {
          const retrySpy = jest
            ...
            .mockImplementationOnce((esCall: () => Promise<any>) => esCall());
      ...
      describe('persistAlerts()', () => {
        ...
        test('should not update ongoing alerts in existing index when they are not in the processed alerts', async () => {
          ...
          expect(spy).toHaveBeenNthCalledWith(3, 'trackedRecoveredAlerts');
        ...
        test('should set tracked to false on newly recovered alerts dropped by the max-alerts cap', async () => {
          ...
          const bulkBody = (
            clusterClient.bulk.mock.calls[0][0] as { body: Array<Record<string, unknown>> }
          ).body;
