...
>(
  ...
  const logger = opts.context.alertsClientContext.logger.get('applyAlertLimit');
  logger.info(`Applying alert limit mapping function`);
  if (!opts.context.hasReachedAlertLimit) {
    logger.info(`Alert limit not reached, passing through alerts as is`);
    return opts.alerts;
  }

  logger.info(`Alert limit reached, applying alert limit logic`);
