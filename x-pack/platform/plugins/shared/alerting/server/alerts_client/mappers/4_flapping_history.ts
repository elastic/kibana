...
>(
  ...
  opts.context.alertsClientContext.logger.info(`Applying flapping history mapping function`);

  const previouslyRecoveredAlerts: Map<string, Alert<S, C, R>> = opts.context
  ...
  return compact([
    ...
    ...[...previouslyRecoveredAlertIds].map((id) => {
      const alert: Alert<S, C, R> | undefined = previouslyRecoveredAlerts.get(id);
      if (alert) {
