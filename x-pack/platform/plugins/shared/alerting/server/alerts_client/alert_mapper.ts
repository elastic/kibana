...
export const filterFor = <S extends State, C extends Context, G extends string, R extends string>(
  alerts: AlertsResult<S, C, G, R>,
  category: AlertCategory
) => alerts.filter(({ category: cat }) => category === cat);

export const filterOut = <S extends State, C extends Context, G extends string, R extends string>(
  alerts: AlertsResult<S, C, G, R>,
  category: AlertCategory
) => alerts.filter(({ category: cat }) => category !== cat);
...
export type AlertsResult<
  ...
  G extends string,
  R extends string
> = Array<CategorizedAlert<S, C, G, R>>;
...
function asyncPipe<S extends State, C extends Context, G extends string, R extends string>(
  ...
  return async (input: AlertsResult<S, C, G, R>, context: MapperContext<S, C, G, R>) => {
    let acc = input;
...
export async function mapAlerts<
  ...
  R extends string
>(
  categorizedAlerts: AlertsResult<S, C, G, R>,
