...
jest.mock('../application/maintenance_window/methods/get/get_maintenance_window', () => ({
  getMaintenanceWindow: jest.fn(),
}));
jest.mock('../application/maintenance_window/methods/update/update_maintenance_window', () => ({
  updateMaintenanceWindow: jest.fn().mockResolvedValue({ id: 'mw-1' }),
}));
jest.mock('../application/maintenance_window/methods/find/find_maintenance_windows', () => ({
  findMaintenanceWindows: jest.fn(),
}));
jest.mock('../application/maintenance_window/methods/delete/delete_maintenance_window', () => ({
  deleteMaintenanceWindow: jest.fn().mockResolvedValue({}),
}));
jest.mock('../application/maintenance_window/methods/archive/archive_maintenance_window', () => ({
  archiveMaintenanceWindow: jest.fn().mockResolvedValue({ id: 'mw-1' }),
}));
jest.mock('../application/maintenance_window/methods/finish/finish_maintenance_window', () => ({
  finishMaintenanceWindow: jest.fn().mockResolvedValue({ id: 'mw-1' }),
}));
jest.mock(
  '../application/maintenance_window/methods/get_active/get_active_maintenance_windows',
  () => ({
...
describe('MaintenanceWindowClient notifyChange', () => {
  ...
  it('does not notify when create fails', async () => {
    const { createMaintenanceWindow } = jest.requireMock(
      '../application/maintenance_window/methods/create/create_maintenance_window'
    );
