...
): RuleResponseInternalV1['actions'] => {
  return [
    ...actions.map((action) => {
      const {
        group,
...
export const transformRuleToRuleResponseInternal = <Params extends RuleParams = never>(
  rule: Rule<Params>
): RuleResponseInternalV1<RuleParamsV1> => ({
  id: rule.id,
