...
describe('RulesClientFactory', () => {
  ...
  describe('isAuthenticationInternalAPIKey()', () => {
    ...
    test('returns true when UIAM reports the key as internal', async () => {
      const constructorCall = await setupFactory();

      securityService.authc.getCurrentUser.mockReturnValueOnce({
        authentication_type: 'api_key',
        api_key: { internal: true },
      } as unknown as AuthenticatedUser);

      expect(constructorCall.isAuthenticationInternalAPIKey()).toBe(true);
    });

    test('returns false for a user-created (external) key', async () => {
      const constructorCall = await setupFactory();

      securityService.authc.getCurrentUser.mockReturnValueOnce({
        authentication_type: 'api_key',
        api_key: { internal: false },
      } as unknown as AuthenticatedUser);

      expect(constructorCall.isAuthenticationInternalAPIKey()).toBe(false);
    });

    test('returns false when internality is not reported (ES-managed key)', async () => {
      const constructorCall = await setupFactory();
    ...
    test('returns false when security is disabled', async () => {
      const factory = new RulesClientFactory();
      factory.initialize(rulesClientFactoryParams);
      await factory.create(mockRouter.createKibanaRequest(), savedObjectsService);
      const constructorCall = jest.requireMock('./rules_client').RulesClient.mock.calls[0][0];
