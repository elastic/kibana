...
import { rawRuleSchema } from './v14';

describe('rawRuleSchemaV14', () => {
  const baseRule = {
    name: 'my rule',
    enabled: true,
    consumer: 'alerts',
  ...
  it.each([...ALERT_SEVERITY_VALUES])(
    'accepts a severity_equals condition with the "%s" severity value',
    (severity) => {
      expect(() =>
  ...
  it('rejects a severity_equals condition with an unknown severity value', () => {
    expect(() =>
      rawRuleSchema.validate({
        ...baseRule,
        snoozedInstances: [
          {
            instanceId: 'alert-1',
            conditions: [{ type: 'severity_equals', value: 'not-a-severity' }],
            snoozedAt: '2024-01-02T12:00:00.000Z',
            snoozedBy: 'elastic',
          },
        ],
