...
export class AlertsService implements IAlertsService {
  ...
  > | null> {
    ...
    const { result: initialized, error } = opts.isPreview
      ? await this.getPreviewInitializationPromise()
    ...
    >({
      ...
      context: opts.isPreview ? PREVIEW_CONTEXT : opts.ruleType.alerts.context,
