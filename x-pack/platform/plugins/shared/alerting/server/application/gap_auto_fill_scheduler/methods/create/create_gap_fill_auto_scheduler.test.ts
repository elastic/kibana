...
function getParams(
  overwrites: Partial<CreateGapFillAutoSchedulerParams> = {}
...
describe('createGapFillAutoScheduler()', () => {
  ...
  test('succeeds creating, scheduling task and updating SO with task id', async () => {
    ...
    const result = await rulesClient.createGapFillAutoScheduler(params);
  ...
  test('validates params and throws on invalid', async () => {
    ...
    } as unknown as CreateGapFillAutoSchedulerParams;

    await expect(rulesClient.createGapFillAutoScheduler(invalid)).rejects.toThrowError(
  ...
  test('logs and rethrows when authorization fails', async () => {
    ...
    await expect(rulesClient.createGapFillAutoScheduler(getParams())).rejects.toThrow('no access');
