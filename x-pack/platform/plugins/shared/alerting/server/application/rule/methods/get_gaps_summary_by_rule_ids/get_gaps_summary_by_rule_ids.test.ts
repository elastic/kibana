...
describe('getGapsSummaryByRuleIds', () => {
  ...
  test('should successfully get gaps summary for rules', async () => {
    ...
    expect(eventLogClient.aggregateEventsBySavedObjectIds).toHaveBeenCalledWith('alert', ruleIds, {
      filter: `event.action: gap AND event.provider: alerting AND not kibana.alert.rule.gap.disabled:* AND kibana.alert.rule.gap.range <= "2023-11-16T09:00:00.000Z" AND kibana.alert.rule.gap.range >= "2023-11-16T08:00:00.000Z"`,
      aggs: {
        unique_rule_ids: {
          terms: {
            field: 'rule.id',
