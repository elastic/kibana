  ...
  describe('event log aggregation', () => {
    it('should aggregate events with correct parameters', async () => {
      ...
      expect(eventLogClient.aggregateEventsWithAuthFilter).toHaveBeenCalledWith(
        ...
        filter,
        expect.objectContaining({
          filter: `event.action: gap AND event.provider: alerting AND not kibana.alert.rule.gap.disabled:* AND kibana.alert.rule.gap.range <= "2024-01-02T00:00:00.000Z" AND kibana.alert.rule.gap.range >= "2024-01-01T00:00:00.000Z" AND (kibana.alert.rule.gap.status : unfilled OR kibana.alert.rule.gap.status : partially_filled)`,
          aggs: {
    ...
    it('should handle missing status filters', async () => {
      ...
      expect(eventLogClient.aggregateEventsWithAuthFilter).toHaveBeenCalledWith(
        ...
        filter,
        expect.objectContaining({
          filter: `event.action: gap AND event.provider: alerting AND not kibana.alert.rule.gap.disabled:* AND kibana.alert.rule.gap.range <= "2024-01-02T00:00:00.000Z" AND kibana.alert.rule.gap.range >= "2024-01-01T00:00:00.000Z"`,
          aggs: {
            latest_gap_timestamp: { max: { field: '@timestamp' } },
            unique_rule_ids: { terms: { field: 'rule.id', size: 10000 } },
          },
