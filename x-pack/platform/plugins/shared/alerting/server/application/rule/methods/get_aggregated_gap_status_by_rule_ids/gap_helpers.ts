 ...
 * Precedence: IN_PROGRESS > UNFILLED > FILLED (matches existing behavior)
 */
export function calculateAggregatedGapStatus(sums: GapDurationSums): AggregatedGapStatus | null {
  ...
  if (sumUnfilledMs > 0) return aggregatedGapStatus.UNFILLED;
  if (sumInProgressMs > 0) return aggregatedGapStatus.IN_PROGRESS;
  if (sumFilledMs > 0) return aggregatedGapStatus.FILLED;
  return null;
}
