...
} from '../lib/stack_alerts_only_privilege_setup';

apiTest.describe(
  'Stack alerts only privilege - all privilege',
  ...
  () => {
    ...
    for (const spec of RULE_SPECS) {
      apiTest(`can get a ${spec.ruleTypeId} rule`, async ({ apiClient }) => {
        ...
        const body = response.body as { id: string; rule_type_id: string };
        expect(body.id).toBe(rule.ruleId);
        expect(body.rule_type_id).toBe(spec.ruleTypeId);
      });
    }

    apiTest('can find rules', async ({ apiClient }) => {
      const response = await apiClient.get('api/alerting/rules/_find', {
        headers: { ...COMMON_HEADERS, ...withAllPrivilegeCreds.apiKeyHeader },
        responseType: 'json',
      });
      expect(response).toHaveStatusCode(200);
