 ...
 *
 * `flow` identifies the consumer: 'cloud_connector' for the package-policy
 * cloud connector form; 'iac_key_check' is used by the verify endpoint and
 * 'iac_upgrade_task' by the daily upgrade-check background task (both compare
 * a stored digest rather than presenting the artifact to a user).
 */
