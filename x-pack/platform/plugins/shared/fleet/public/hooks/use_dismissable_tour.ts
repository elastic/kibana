...
export function useDismissableTour(tourKey: TourKey) {
  ...
  // Check if this tour should be shown (not hidden and is the active tour or no tour manager)
  const isOpen = !isHidden && (activeTour === tourKey || activeTour === null);

  // Request to show this tour when it should be visible and no other tour is active
  React.useEffect(() => {
    if (!isHidden && activeTour === null && setActiveTour) {
      setActiveTour(tourKey);
    }
  }, [isHidden, activeTour, setActiveTour, tourKey]);
