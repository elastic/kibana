...
describe('buildKuery', () => {
  ...
  it('should include expired tokens in the inactive filter', () => {
    expect(buildKuery('', [], 'inactive', [])).toEqual(
      '(active:false or expire_at <= "now") and (not hidden:true)'
    );
  });
  ...
  it('should combine the status filter with the other filters', () => {
    expect(buildKuery('my-token', ['policy-1'], 'active', ['policy-2'])).toEqual(
      '(my-token) and (policy_id:"policy-1") and (active:true and not (expire_at <= "now")) and ' +
