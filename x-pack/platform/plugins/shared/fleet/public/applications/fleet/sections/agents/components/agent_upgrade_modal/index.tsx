...
export const AgentUpgradeAgentModal: React.FunctionComponent<AgentUpgradeAgentModalProps> = ({
  ...
  const isIARVersion = (version: string) => {
  ...
  const versionOptions: Array<EuiComboBoxOptionOption<string>> = useMemo(() => {
    ...
    const options = displayVersions.map((option) => ({
      label: option,
      value: option,
      toolTipContent: isIARVersion(option)
        ? i18n.translate('xpack.fleet.upgradeAgents.iarVersionOptionTooltip', {
            defaultMessage: 'Independent Agent Release version',
          })
        : undefined,
    }));
    if (options.length === 0) {
  ...
  const isSelectedVersionIAR = useMemo(() => {
    return isIARVersion(selectedVersion[0]?.value || '');
  }, [selectedVersion]);
        ...
        <>
          <EuiCallOut
            ...
            title={
              <FormattedMessage
                ...
                defaultMessage="Independent Agent Release version selected"
              />
            }
          ...
          >
            <FormattedMessage
              ...
              values={{
                documentationLink: (
                  <EuiLink external={true} href={docLinks.links.fleet.iarAgentVersion}>
                    <FormattedMessage
