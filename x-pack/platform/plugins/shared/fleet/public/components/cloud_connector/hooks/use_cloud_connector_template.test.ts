...
describe('useCloudConnectorTemplate', () => {
  ...
  describe('when the template is a Workload Identity template', () => {
    const WII_TEMPLATE_URL =
      'https://console.aws.amazon.com/cloudformation/home#/stacks/quickcreate?templateURL=https%3A%2F%2Fstatic.example%2Fwii-template.yml&param_ElasticIssuer=ISSUER&param_ElasticSubject=SUBJECT';
    const WII_CLOUD = { ...CLOUD, organizationId: '2070044029', csp: 'aws', region: 'eu-west-1' };

    beforeEach(() => {
      mockedUseIacProvisioner.mockReturnValue({ isIacProvisionerEnabled: true });
    });

    it('keeps the static quick-create URL instead of rendering an IaC artifact', async () => {
      const { result } = renderHook(() =>
        useCloudConnectorTemplate({
          ...HOOK_PARAMS,
          cloud: WII_CLOUD,
      ...
      const { launchButtonProps, isDisabled } = result.current;
      if (!('href' in launchButtonProps)) {
        throw new Error('expected href launch button props');
      }
      expect(launchButtonProps.href).toContain('wii-template.yml');
      expect(launchButtonProps.href).toContain(
        'param_ElasticIssuer=workload-identity-issuer.eu-west-1.aws.svc.elastic.cloud/orgs/2070044029'
      );
      expect(launchButtonProps.href).toContain(
        'param_ElasticSubject=deployment:kibana-component-id'
      );
      expect(isDisabled).toBe(false);
      expect(mockedSendRenderIacTemplate).not.toHaveBeenCalled();
    });

    it('disables the launch button when the Workload Identity parameters cannot be resolved', () => {
      const { result } = renderHook(() =>
        useCloudConnectorTemplate({
          ...HOOK_PARAMS,
          cloud: { ...WII_CLOUD, organizationId: undefined },
          iacTemplateUrl: WII_TEMPLATE_URL,
        })
      );
