...
export const isCloudConnectorReusableEnabled = (
  ...
  if (provider === AWS_PROVIDER) {
    if (templateName === 'cspm') {
      return semverGte(packageInfoVersion, CLOUD_CONNECTOR_AWS_CSPM_REUSABLE_MIN_VERSION);
    }
    ...
    if (templateName === 'cspm') {
      return semverGte(packageInfoVersion, CLOUD_CONNECTOR_AZURE_CSPM_REUSABLE_MIN_VERSION);
    }
    if (templateName === 'asset_inventory') {
      return semverGte(
        packageInfoVersion,
 ...
 * First checks package-level vars, then data_streams vars.
...
export const findVariableDef = (packageInfo: PackageInfo, key: string) => {
  // First, check package-level vars (for package-level integrations like AWS)
  const packageLevelVar = packageInfo?.vars?.find((v) => v?.name === key);
  if (packageLevelVar) {
    return packageLevelVar;
  }

  // Then check data_streams vars (for stream-level integrations like CSPM)
