...
import type { PackagePolicyConfigRecord, PackagePolicyConfigRecordEntry } from '../../../types';
import { detectStorageMode } from '../../../../common/services/cloud_connectors';
import type { CloudConnectorVarStorageMode } from '../../../../common/services/cloud_connectors';
...
/**
 * Helper to get vars from the correct location based on storage mode.
 */
const getVarsForMode = (
  mode: CloudConnectorVarStorageMode,
  input: NewPackagePolicyInput,
  newPolicy: NewPackagePolicy
): PackagePolicyConfigRecord | undefined => {
  if (mode === 'package') {
    return newPolicy.vars;
  }
  return input.streams?.find((stream) => stream.enabled)?.vars;
};

/**
 * Helper to update policy with credentials for package-level vars.
 */
const updatePackagePolicyVars = (
  policy: NewPackagePolicy,
  credentials: CloudConnectorCredentials
): NewPackagePolicy => {
  const updatedVars = updateInputVarsWithCredentials(
    policy.vars as PackagePolicyConfigRecord,
    credentials
  );
...
export const useCloudConnectorSetup = (
  ...
  // Detect storage mode from package info using the accessor
  const storageMode = useMemo(() => detectStorageMode(packageInfo), [packageInfo]);

  // State for new connection form
  const [newConnectionCredentials, setNewConnectionCredentials] =
    useState<CloudConnectorCredentials>(() => {
      // Get vars from the correct location based on storage mode
      const vars = getVarsForMode(storageMode, input, newPolicy) ?? {};
  ...
  const updatePolicyWithNewCredentials = useCallback(
    (credentials: CloudConnectorCredentials) => {
      ...
      if (storageMode === 'package') {
        // Update package-level vars
        updatedPolicy = updatePackagePolicyVars(updatedPolicy, credentials);
        updatePolicy({
          updatedPolicy: { ...updatedPolicy, cloud_connector_id: undefined },
          isValid: isNameValid ? undefined : false,
        });
      } else {
        // Update stream-level vars (original behavior)
    ...
    [input.streams, newPolicy, updatePolicy, storageMode]
  );
