...
/**
 * Schema-valid resolve body. The package name is intentionally unknown so the
 * flag-on handler 404s without calling the provisioner.
 */
 ...
 *
 * The live 200 resolve path needs a running IaC Provisioner (mTLS) and is not
 * exercised here. These specs cover route authorization, request-schema
 * validation, and the unknown-package 404.
 */
apiTest.describe(
  ...
  () => {
    ...
    apiTest('returns 400 when flow is missing', async ({ apiClient, samlAuth }) => {
      ...
      const response = await apiClient.post(testData.RESOLVE_BLUEPRINTS_PATH, {
        ...
        body: {
          provider: 'aws',
          integrations: VALID_RESOLVE_BODY.integrations,
        },
        responseType: 'json',
      });
