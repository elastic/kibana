...
describe('AgentStatusChangeTask', () => {
  ...
  describe('Task logic', () => {
    ...
    it('should correctly resolve agentless flag for agents with versioned policy_id', async () => {
      // Agents enrolled via version-specific policies have policy_id: "<uuid>#<version>" but
      // policy_base_id: "<uuid>". The agentlessPolicies list is keyed by the base id, so
      // lookups must use policy_base_id, not policy_id.
      const agents = [
        {
          id: 'agent-versioned',
          policy_id: 'agentless-policy-1#9.6', // versioned — mismatches the list entry
          policy_base_id: 'agentless-policy-1', // base id — matches the list entry
          status: 'online',
          namespaces: ['default'],
