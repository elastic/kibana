...
describe('upgrade handler', () => {
  describe('checkKibanaVersion', () => {
    it('should not throw if upgrade version is equal to kibana version', () => {
      expect(() => checkKibanaVersion('8.4.0', '8.4.0')).not.toThrow();
    });

    it('should throw if upgrade version is higher than kibana version', () => {
      expect(() => checkKibanaVersion('8.5.0', '8.4.0')).toThrow(
    ...
    it('should not throw if upgrade version is equal to kibana version with snapshot', () => {
      expect(() => checkKibanaVersion('8.4.0', '8.4.0-SNAPSHOT')).not.toThrow();
    });

    it('should not throw if force is specified and patch is newer', () => {
      expect(() => checkKibanaVersion('8.4.1', '8.4.0', true)).not.toThrow();
      expect(() => checkKibanaVersion('8.4.1-SNAPSHOT', '8.4.0', true)).not.toThrow();
    });

    it('should not throw if not force is specified and patch is newer', () => {
      expect(() => checkKibanaVersion('8.4.1', '8.4.0', false)).not.toThrow();
      expect(() => checkKibanaVersion('8.4.1-SNAPSHOT', '8.4.0', false)).not.toThrow();
    });

    it('should throw if force is specified and minor is newer', () => {
      expect(() => checkKibanaVersion('8.5.0', '8.4.0', true)).toThrow();
    });

    it('should not throw if force is specified and major and minor is newer', () => {
      expect(() => checkKibanaVersion('7.5.0', '8.4.0', true)).not.toThrow();
      expect(() => checkKibanaVersion('8.4.0', '8.4.0', true)).not.toThrow();
    });
  });
});
