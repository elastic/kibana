...
/**
 * Internal API Handler: Create Cloud Connector with Agent Policy and Package Policy
 *
 * This handler atomically creates three resources in a single request:
 * 1. Agent Policy
 * 2. Cloud Connector (with secrets handling)
 * 3. Package Policy (referencing the cloud connector)
 *
 * Designed for agentless integrations that require cloud connectors.
 * Implements comprehensive rollback on failure to prevent orphaned resources.
 */
...
> = async (context, request, response) => {
  ...
  try {
    const result = await createAgentPolicyWithCloudConnector({
      ...
      packagePolicyRequest: packagePolicyRequest as Omit<
        NewPackagePolicy,
