...
/**
 * Returns true if the data stream opts into OTel asset naming via `use_otel_suffix: true`, or any
 * of its streams effectively use the OTel collector input type, AND OTel integrations are enabled.
 * Resolves named inputs so that a stream referencing an input by name (e.g. `otel_logs`) is
 * correctly identified as OTel when its backing input has `type: otelcol`.
 *
