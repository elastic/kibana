...
import { getRegistryDataStreamAssetBaseName, isValidDataset } from '../../../../common/services';
...
const OTEL_COLLECTOR_INPUT_TYPE = 'otelcol';
const VALID_DATA_STREAM_TYPES = ['logs', 'metrics', 'traces', 'synthetics', 'profiles'];

function isValidDataStreamType(
  type: string,
  allowBlank?: boolean
  ...
  if (!VALID_DATA_STREAM_TYPES.includes(type)) {
    return {
      valid: false,
...
function dataStreamUsesOtelInput(
  pkgInfo: { policy_templates?: UploadPolicyTemplate[] },
  dataStream: { streams?: Array<{ input?: string }> }
): boolean {
  const namedOtelInputs = new Set<string>();
  for (const template of pkgInfo.policy_templates ?? []) {
  ...
  return (dataStream.streams ?? []).some(
    (stream) =>
      stream.input === OTEL_COLLECTOR_INPUT_TYPE ||
      (stream.input !== undefined && namedOtelInputs.has(stream.input))
  );
}
...
function liveDataStreamIndexPattern(
  ...
  const patternedName = isOtelInputType ? `${assetBaseName}.otel` : assetBaseName;
  return dataStream.dataset_is_prefix ? `${patternedName}.*-*` : `${patternedName}-*`;
}
...
function isOtelUploadDataStream(
  ...
  const experimentalFeatures = appContextService.getExperimentalFeatures() as
    | { enableOtelIntegrations?: boolean }
    | undefined;
  if (!experimentalFeatures?.enableOtelIntegrations) {
    return false;
  }
