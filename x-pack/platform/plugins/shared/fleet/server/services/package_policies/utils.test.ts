...
describe('removeCloudConnectorTransientVars', () => {
  it('should remove cloud_connector_name transient var from all streams', () => {
    const packagePolicy = {
      name: 'test-policy',
      inputs: [
        {
          ...
          streams: [
            {
              id: 'stream-1',
              vars: {
                cloud_connector_name: { value: 'my-connector', type: 'text' },
                role_arn: { value: 'arn:aws:iam::123:role/Test', type: 'text' },
              },
            },
            {
              id: 'stream-2',
              vars: {
                cloud_connector_name: { value: 'other-connector', type: 'text' },
                tenant_id: { value: 'tenant-123', type: 'text' },
              },
            },
          ],
        },
    ...
    const result = removeCloudConnectorTransientVars(packagePolicy);
  ...
  it('should handle streams without vars', () => {
    const packagePolicy = {
      name: 'test-policy',
      inputs: [
        {
    ...
    expect(result.inputs[0].streams[0]).toEqual({ id: 'stream-1' });
    expect(result.inputs[0].streams[1].vars).toEqual({});
  });

  it('should return unchanged policy when cloud_connector_name is not present', () => {
    const packagePolicy = {
      name: 'test-policy',
      inputs: [
        {
  ...
  it('should only remove cloud_connector_name and preserve other vars', () => {
    const packagePolicy = {
      name: 'test-policy',
      inputs: [
        {
          ...
          streams: [
            {
              ...
              vars: {
                cloud_connector_name: { value: 'my-connector', type: 'text' },
                other_temp_var: { value: 'temp-value', type: 'text' },
                role_arn: { value: 'arn:aws:iam::123:role/Test', type: 'text' },
              },
            },
          ],
  ...
  it('should not mutate original package policy', () => {
    const packagePolicy = {
      name: 'test-policy',
      inputs: [
        {
    ...
    const originalVars = { ...packagePolicy.inputs[0].streams[0].vars };
    removeCloudConnectorTransientVars(packagePolicy);

    expect(packagePolicy.inputs[0].streams[0].vars).toEqual(originalVars);
  });
});
