    ...
    ({ id, attributes: installation }) => {
      return reinstallPackageForInstallation({
        ...
        if (isNonReinstallable) {
          ...
          // The stamp can fail if the cluster is read-only (e.g. flood-stage watermark); in
          // that case we still log at warn and return so the failure is not re-thrown and does
          // not fall through to the error path.
          try {
