...
export interface HostUploadedFileMetadata {
    ...
    file: Pick<Required<BaseFileMetadata>, 'name' | 'size' | 'Status' | 'ChunkSize' | 'mime_type' | 'extension'> & Omit<BaseFileMetadata, 'name' | 'size' | 'Status' | 'ChunkSize' | 'mime_type' | 'extension'> & {
...
export interface FilesClientFactory {
    ...
    maxSizeBytes?: number) => FleetToHostFileClientInterface;
    /**
    ...
    fromHost: (
    /** The integration package name */
    packageName: string) => FleetFromHostFileClientInterface;
}
