...
describe('Package policy service', () => {
  ...
  describe('updatePackageInputs', () => {
    ...
    describe('when the package is synthetics', () => {
      // Synthetics persists only the single active input and drops the disabled ones. On upgrade
      // Fleet must not re-add the package-default inputs (which enable synthetics/browser), or the
      // policy ends up with two enabled inputs and marks agents unhealthy (regression from #229595,
      // reverted in #236104).
      it('does not re-add inputs that were stripped from the stored policy', () => {
        ...
        // Mirrors packageToPackagePolicyInputs(packageInfo): every input is present and the
        // browser input is enabled by default.
        const inputsOverride: NewPackagePolicyInput[] = [
          ...
          { type: 'synthetics/browser', policy_template: 'synthetics', enabled: true, streams: [] },
        ];
