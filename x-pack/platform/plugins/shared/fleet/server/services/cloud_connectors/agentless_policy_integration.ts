...
export async function createAndIntegrateCloudConnector(params: {
  ...
  if (existingCloudConnectorId) {
    ...
    try {
      ...
      // provider-default group), so no requester is ever exempt from this check.
      ...
      // Sampling a single usage is sufficient: group membership is enforced on every
      // attach, so all package policies on a connector belong to the same group.
      ...
      if (existingPolicies.total > 0) {
        const existingPackageName = existingPolicies.saved_objects[0].attributes.package?.name;
