...
export async function createAndIntegrateCloudConnector(params: {
   ...
   * Re-saving a policy with its already-attached connector is not a new attachment, so
   * policy-group enforcement is skipped — this grandfathers policies attached to
   * cross-group connectors before group enforcement was effective (the pre-inversion
   * allowlist lookup never matched var_groups integrations, so mixed-group attachments
   * were possible in the wild).
   */
  ...
  if (existingCloudConnectorId) {
    ...
    try {
      ...
      // Every integration resolves to a group (its isolated group if registered, otherwise the
      // provider-default group), so no requester is ever exempt from this check. The one
      // exception is re-saving a policy with its already-attached connector (update path):
      // that is not a new attachment, and rejecting it would make policies attached to
      // pre-enforcement cross-group connectors permanently un-editable.
      if (existingCloudConnectorId !== attachedCloudConnectorId) {
        const requestingGroup = getPolicyGroupForIntegration(packageInfo.name, cloudProvider);

        // Look up ALL of the connector's package policies. Pre-enforcement data may contain
        // mixed-group attachments (see attachedCloudConnectorId above), so a single-usage
        // sample would be nondeterministic — reject if ANY usage belongs to a different
        // group, matching the frontend's every-usage check in use_get_cloud_connectors.
        ...
        for (const so of existingPolicies.saved_objects) {
          const existingPackageName = so.attributes.package?.name;
          if (!existingPackageName) continue;

          const connectorGroup = getPolicyGroupForIntegration(existingPackageName, cloudProvider);

          if (connectorGroup !== requestingGroup) {
            throw new CloudConnectorCreateError(
              `Cloud connector ${existingCloudConnectorId} belongs to policy group "${connectorGroup}" (used by "${existingPackageName}") and cannot be reused by integration "${packageInfo.name}" in policy group "${requestingGroup}"`
            );
          }
        }
      }
