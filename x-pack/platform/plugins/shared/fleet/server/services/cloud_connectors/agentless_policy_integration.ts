...
export async function createAndIntegrateCloudConnector(params: {
  ...
  if (existingCloudConnectorId) {
    ...
    try {
      ...
      // Enforce policy group scope: connectors can only be reused within the same policy
      // group. Re-saving a policy with its already-attached connector is not a new
      // attachment and is exempt.
      if (existingCloudConnectorId !== attachedCloudConnectorId) {
        ...
        // Reject if ANY usage of the connector belongs to a different group, matching the
        // frontend's every-usage check in use_get_cloud_connectors.
