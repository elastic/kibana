...
export const getAvailableVersions = async ({
  ...
  // Take each version and compare to our `MINIMUM_SUPPORTED_VERSION`, also only get the version <= to the current cluster version - we
  ...
  availableVersions = uniq(
    [...availableVersions, ...apiVersions]
      .filter((v: any) => {
        ...
        return semverGte(v, MINIMUM_SUPPORTED_VERSION) && !semverGt(v, kibanaVersion);
      })
