import type { RouteMethod, VersionedRouteConfig, AddVersionOpts, IKibanaResponse, RouteConfigOptions } from '@kbn/core-http-server';
...
type FleetAuthzRouteRegistrar<Method extends RouteMethod, Context extends RequestHandlerContext = RequestHandlerContext> = (config: FleetRouteConfig<Method>) => FleetVersionedRoute<Method, Context>;
export interface FleetAuthzRouteConfig<T extends FleetAuthzRouterConfigParam = FleetAuthzRouterConfigParam> {
    fleetAuthz?: T;
}
/**
 * Internal type necessary to make access in VersionedRouteConfig optional
 */
export type FleetVersionedRouteConfig<Method extends RouteMethod> = Omit<VersionedRouteConfig<Method>, 'access'> & {
 ...
 */
export type FleetRouteConfig<Method extends RouteMethod> = FleetVersionedRouteConfig<Method> & FleetAuthzRouteConfig;
/**
 * Interface replacing the native VersionedRouter to handle fleetAuthz
 */
export interface FleetVersionedRouter<TContext extends FleetRequestHandlerContext = FleetRequestHandlerContext> {
 ...
 */
export interface FleetAuthzRouter<TContext extends FleetRequestHandlerContext = FleetRequestHandlerContext> {
...
export type FleetHandler<P, Q, B, Context extends RequestHandlerContext> = (...params: Parameters<RequestHandler<P, Q, B, Context>>) => MaybePromise<IKibanaResponse>;
/**
 * Interface that redefines Core native VersionedRoute interface to accept Fleet custom types
 */
export interface FleetVersionedRoute<Method extends RouteMethod = RouteMethod, Context extends RequestHandlerContext = RequestHandlerContext> {
    addVersion<P = unknown, Q = unknown, B = unknown>(options: FleetAddVersionOpts<P, Q, B>, handler: FleetHandler<P, Q, B, Context>): FleetVersionedRoute<Method, Context>;
}
export {};
