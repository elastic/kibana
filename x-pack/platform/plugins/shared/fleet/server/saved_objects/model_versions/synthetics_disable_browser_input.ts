...
export const disableBrowserInputWhenBothEnabled: SavedObjectModelDataBackfillFn<
  PackagePolicy,
  PackagePolicy
> = (doc) => {
  const pkg = doc.attributes.package?.name;
  if (pkg !== FLEET_SYNTHETICS_PACKAGE) return doc;

  const enabledInputs = doc.attributes.inputs.filter((i) => i.enabled);
  if (enabledInputs.length === 1) return doc;
  ...
  return {
    attributes: { ...doc.attributes, bump_agent_policy_revision: true },
  };
};
