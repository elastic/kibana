...
describe('Agent Routes - request body schemas', () => {
  ...
  describe('configuration.ai_indices', () => {
    const withConfiguration = (configuration: Record<string, unknown>) => ({
      ...createBody,
      configuration: { tools: [], ...configuration },
    });

    it('accepts ai_indices on create', () => {
      const schema = routeSchemas[`POST:${createPath}`];

      expect(() => schema.validate(createBody)).not.toThrow();
      expect(() => schema.validate(withConfiguration({ ai_indices: [] }))).not.toThrow();
      expect(() =>
        schema.validate(withConfiguration({ ai_indices: ['index-1', 'index-2'] }))
      ).not.toThrow();
    });

    it('accepts ai_indices on update', () => {
      const schema = routeSchemas[`PUT:${updatePath}`];

      expect(() => schema.validate({ configuration: { ai_indices: ['index-1'] } })).not.toThrow();
    });

    it('rejects ai_indices outside of configuration', () => {
      expect(() =>
        routeSchemas[`POST:${createPath}`].validate({ ...createBody, ai_indices: ['index-1'] })
      ).toThrow(/'ai_indices' was unexpected/);
      expect(() => routeSchemas[`PUT:${updatePath}`].validate({ ai_indices: ['index-1'] })).toThrow(
        /'ai_indices' was unexpected/
      );
    });

    it('rejects ai_indices that is not an array of strings', () => {
      const schema = routeSchemas[`POST:${createPath}`];

      expect(() => schema.validate(withConfiguration({ ai_indices: 'index-1' }))).toThrow(
        /\[configuration.ai_indices]: could not parse array value from json input/
      );
      expect(() => schema.validate(withConfiguration({ ai_indices: [1] }))).toThrow(
        /\[configuration.ai_indices.0]: expected value of type \[string]/
      );
    });

    it('rejects more than 100 ai_indices', () => {
      const aiIndices = Array.from({ length: 101 }, (_, i) => `index-${i}`);

      expect(() =>
        routeSchemas[`POST:${createPath}`].validate(withConfiguration({ ai_indices: aiIndices }))
      ).toThrow(/array size is \[101], but cannot be greater than \[100]/);
      expect(() =>
        routeSchemas[`POST:${createPath}`].validate(
          withConfiguration({ ai_indices: aiIndices.slice(0, 100) })
        )
      ).not.toThrow();
      expect(() =>
        routeSchemas[`PUT:${updatePath}`].validate({ configuration: { ai_indices: aiIndices } })
      ).toThrow(/array size is \[101], but cannot be greater than \[100]/);
    });
  });
});
