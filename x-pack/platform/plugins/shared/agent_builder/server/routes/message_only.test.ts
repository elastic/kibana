...
import type { EventChatRequestBodyPayload } from '../../common/http_api/chat';
...
describe('message-only contract', () => {
  ...
  it('assigns distinct public identities and stable callback/origin identities', async () => {
    const payload: EventChatRequestBodyPayload = {
      trigger_mode: 'never',
      input: 'hello',
    ...
    const callback = { request, spaceId: 'default', payload, messageId: 'callback-key' };
  ...
  it('limits the new schema fields to supported endpoints', () => {
    expect(eventConversePayloadSchema.validate({ input: 'hi' }).trigger_mode).toBe('always');
    ...
    expect(() => eventConversePayloadSchema.validate({ trigger_mode: 'auto' })).toThrow();
    expect(() =>
