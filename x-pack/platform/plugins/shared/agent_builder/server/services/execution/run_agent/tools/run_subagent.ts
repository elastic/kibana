...
- Only pass \`auto_approved_apis\` when the task you are delegating genuinely has to mutate state. A read-only task needs no grant.
 ...
 */
type SubagentDestructiveAccess = 'granted' | 'denied' | 'unavailable';

type SubagentApiGrant =
  | { status: 'granted'; autoApprovedApis?: AutoApprovedApi[] }
...
type SettledApiGrant =
  ...
  | {
      ...
      grantReport: { destructive_access?: SubagentDestructiveAccess };
    };
...
const resolveSubagentApiGrant = ({
  ...
  if (status === ConfirmationStatus.unprompted) {
    const intro = i18n.translate(
      ...
      {
        defaultMessage:
          'The agent is delegating "{description}" to `{agentId}`, and wants it to call these APIs without asking you again. They can modify or delete existing data.',
        values: { agentId, description },
      }
    );
    const denyExplanation = i18n.translate(
      ...
      {
        defaultMessage:
          'The task still runs either way. If you deny, `{agentId}` runs without these APIs.',
        values: { agentId },
      }
    );
    ...
    return {
      ...
      promptReturn: prompts.askForConfirmation({
        ...
        title: i18n.translate(
          ...
          {
            defaultMessage: 'Allow `{agentId}` to change your data?',
            values: { agentId },
          }
        ),
  ...
  return { status: 'granted', autoApprovedApis: pending };
};
...
export const createSubagentTool = ({
  ...
  const tool: InternalBuiltinToolDefinition<typeof schema> = {
    ...
    handler: async (
      ...
      const settleApiGrant = (): SettledApiGrant => {
        if (requestedApis.length === 0) {
          return { grantReport: {} };
        }

        const grant = resolveSubagentApiGrant({
        ...
        return {
          ...(grant.status === 'granted' && grant.autoApprovedApis
            ...
            : {}),
          grantReport: { destructive_access: grant.status },
        };
      };
