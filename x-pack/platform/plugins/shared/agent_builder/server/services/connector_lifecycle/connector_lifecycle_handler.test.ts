...
const { getWorkflowTemplatesForConnector } = jest.requireMock('@kbn/connector-specs/server');
...
describe('createConnectorLifecycleHandler', () => {
  ...
  describe('onPostSave', () => {
    it('skips updates (only handles new creations)', async () => {
      getWorkflowTemplatesForConnector.mockReturnValue([SIMPLE_WORKFLOW_YAML]);
      ...
      await handler.onPostSave(createBaseParams({ isUpdate: true }) as any);

      expect(getWorkflowTemplatesForConnector).not.toHaveBeenCalled();
    });
    ...
    it('skips unsuccessful saves', async () => {
      ...
      await handler.onPostSave(createBaseParams({ wasSuccessful: false }) as any);
    ...
    it('skips connectors without workflows', async () => {
      ...
      await handler.onPostSave(createBaseParams() as any);
    ...
    it('creates workflows and tools for connectors with workflow templates', async () => {
      ...
      expect(createCall[0].yaml).toContain('connector.test.action');
    ...
    it('does not create a tool when workflow lacks agent-builder-tool tag', async () => {
      getWorkflowTemplatesForConnector.mockReturnValue([WORKFLOW_YAML_NO_TOOL_TAG]);
      const toolRegistry = createMockToolRegistry();
      const workflowsManagement = createMockWorkflowsManagement();
      const handler = createConnectorLifecycleHandler({
        serviceManager: createMockServiceManager(toolRegistry) as any,
    ...
    it('logs error but does not throw when workflow creation fails', async () => {
      ...
      await expect(handler.onPostSave(createBaseParams() as any)).resolves.toBeUndefined();
