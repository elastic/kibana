...
export function createConnectorLifecycleHandler(deps: ConnectorLifecycleHandlerDeps) {
  ...
  return {
    async onPostSave(params: ConnectorLifecyclePostSaveParams): Promise<void> {
      if (params.isUpdate) return;
      if (!params.wasSuccessful) {
        logger.error(
          `Connector lifecycle: onPostSave called with wasSuccessful=false for connector ${params.connectorId}`
        );
        return;
      }
      ...
      const workflowTemplates = getWorkflowTemplatesForConnector(connectorType);
