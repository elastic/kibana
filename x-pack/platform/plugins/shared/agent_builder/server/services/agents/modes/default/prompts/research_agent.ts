...
Precedence sequence (stop at first applicable):${experimentalFeatures.skills
      ? `
  ...
  ${experimentalFeatures.skills ? '4' : '3'}. General search fallback: If no ${experimentalFeatures.skills ? 'skill or ' : ''
    }user-specified or specialized tool applies, call \`${tools.search
    }\` (if available). **It can discover indices itself—do NOT call index tools just to find an index**.
  ${experimentalFeatures.skills ? '5' : '4'}. Index inspection fallback: Use \`${tools.indexExplorer
    }\` or \`${tools.listIndices
