...
describe('compileConversationFilter', () => {
  ...
  describe('pre-built AST filters', () => {
    ...
    it.each(['and', 'or', 'not'])('rejects a %p node with no operands', (fn) => {
      expect(() => compileConversationFilter({ type: 'function', function: fn })).toThrow(
        expect.objectContaining({
          code: AgentBuilderErrorCode.badRequest,
          message: expect.stringContaining(`Malformed "${fn}" expression in filter`),
        })
      );
    });

    it('rejects a boolean node whose operand is missing', () => {
      expect(() =>
        compileConversationFilter({ type: 'function', function: 'and', arguments: [undefined] })
      ).toThrow(
        expect.objectContaining({
          code: AgentBuilderErrorCode.badRequest,
          message: expect.stringContaining('Unsupported expression in filter'),
  ...
  describe('metadata fields', () => {
    ...
    it('rejects a range comparison, since metadata values are stored as strings', () => {
      expect(() => compileConversationFilter('metadata.count > 5')).toThrow(
        expect.objectContaining({
          code: AgentBuilderErrorCode.badRequest,
          message: expect.stringContaining('Metadata is stored untyped'),
        })
      );
    });

    it.each(['"metadata.*": high', '"metadata.sev?rity": high'])(
      'rejects the quoted wildcard key in %p, which KQL does not expand on a flattened field',
      (filter) => {
    ...
    it('rejects a wildcard key built directly as a node', () => {
      expect(() => compileConversationFilter(nodeBuilder.is('metadata.*', 'high'))).toThrow(
        expect.objectContaining({
          code: AgentBuilderErrorCode.badRequest,
  ...
  describe('rejections', () => {
    ...
    it.each(['alert', '"alert triage"'])(
      'tells the caller a bare value in %p names no field, rather than blaming wildcards',
      (filter) => {
        expect(() => compileConversationFilter(filter)).toThrow(
          expect.objectContaining({
            code: AgentBuilderErrorCode.badRequest,
            message: expect.stringContaining('a bare value matches no field'),
          })
        );
      }
    );

    it('rejects a comparison node with no arguments at all', () => {
      expect(() =>
        compileConversationFilter({ type: 'function', function: 'is', arguments: [] })
      ).toThrow(
        expect.objectContaining({
          code: AgentBuilderErrorCode.badRequest,
          message: expect.stringContaining('a bare value matches no field'),
