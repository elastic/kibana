...
// (perm_kibana) is always present; spaces and other optional fields appear
...
/**
 * Build a `checkPrivileges` mock that handles `kibana` privilege checks.
 */
 ...
 * Pass `[]` (the default) for "no privileges".
 */
...
describe('SML query/search/authz service', () => {
  ...
  beforeEach(() => {
    ...
    // getDocumentsByIds, findByOrigin, findByOriginAcrossSpaces) run
  ...
  describe('autocompleteSml', () => {
    ...
    it('applies permission filtering when securityAuthz is present', async () => {
      ...
      const rawResult = await autocompleteSml({
        query: 'a',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    describe('pre-aggregation authz filter (MV_CONTAINS subset)', () => {
      ...
      it('treats a missing index as an empty universe (no authz clause)', async () => {
        ...
        // No authz WHERE clause is emitted when the universe is empty.
        expect(esql).not.toContain('| WHERE MV_CONTAINS(?, permissions.kibana.privileges.name)');
      });
    });
  });
  ...
  describe('checkItemsAccess', () => {
    it('grants all access when securityAuthz is absent', async () => {
      const result = await checkItemsAccess({
        ids: ['item-1', 'item-2'],
        spaceId: 'default',
        esClient: scopedClient,
        request,
  ...
  describe('findByOriginAcrossSpaces', () => {
    ...
    it('only fetches the guard-relevant fields via _source filter', async () => {
      ...
      await findByOriginAcrossSpaces({
        type: 'visualization',
        originId: 'ref-1',
        esClient: scopedClient,
        logger,
