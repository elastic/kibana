...
describe('mergeAppendMessageResponseIntoConversation', () => {
  ...
  it('appends the new user message event to the cached conversation', () => {
    ...
    const [firstEvent, secondEvent] = merged?.events ?? [];
    expect(isUserMessageEvent(firstEvent) ? firstEvent.message : undefined).toBe('Initial note');
    expect(isUserMessageEvent(secondEvent) ? secondEvent.message : undefined).toBe(
      'Follow-up note'
    );
  ...
  it('shows an optimistic human note before the server responds', () => {
    ...
    expect(
      optimistic?.events?.some(
        (event) => isUserMessageEvent(event) && event.message === 'Draft note'
      )
    ).toBe(true);
