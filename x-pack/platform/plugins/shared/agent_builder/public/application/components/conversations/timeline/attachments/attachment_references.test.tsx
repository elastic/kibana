...
import { AttachmentReferences } from './attachment_references';
...
describe('AttachmentReferences', () => {
  ...
  it('renders nothing when there are no refs or attachments', () => {
    const { container } = render(<AttachmentReferences />);
  ...
  it('renders the "Added" heading and one pill per resolved attachment', () => {
    ...
    expect(screen.getAllByTestId('agentBuilderAttachmentPill')).toHaveLength(2);
  });
  ...
  it('skips hidden attachments', () => {
    ...
    const { container } = render(
      <AttachmentReferences attachmentRefs={[makeRef('h')]} conversationAttachments={[hidden]} />
    );
    expect(container.firstChild).toBeNull();
  });
  ...
  it('uses fallbackAttachments when no attachmentRefs are provided', () => {
    render(<AttachmentReferences fallbackAttachments={[makeFallback('f1', 'Fallback Label')]} />);
