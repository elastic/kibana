...
describe('ExecutionError', () => {
  ...
  it('shows hook execution callout and skips the reasoning error panel', () => {
    ...
    renderWithIntl(<ExecutionError error={error} />);
  ...
  it('shows request aborted error inside the reasoning error panel', () => {
    ...
    expect(screen.getByTestId('agentBuilderExecutionErrorRequestAborted')).toBeInTheDocument();
    expect(screen.getByTestId('reasoningErrorPanel')).toBeInTheDocument();
    expect(screen.queryByTestId('agentBuilderGenericExecutionError')).not.toBeInTheDocument();
  });

  it('shows generic errors inside the reasoning error panel', () => {
    renderWithIntl(<ExecutionError error={new Error('boom')} />);

    expect(screen.getByTestId('agentBuilderGenericExecutionError')).toBeInTheDocument();
    expect(screen.getByTestId('reasoningErrorPanel')).toBeInTheDocument();
  });
  ...
  describe('from a persisted execution_failed error', () => {
    it('classifies a context length exceeded error', () => {
      ...
      expect(screen.getByTestId('agentBuilderExecutionErrorContextExceeded')).toBeInTheDocument();
    });
    ...
    it('shows the message and the cause chain for a generic error', () => {
      ...
      const details = screen.getByTestId('agentBuilderGenericExecutionError').textContent;
