...
const waitingForInputLabel = i18n.translate(
  'xpack.agentBuilder.conversation.thinking.waitingForInput',
  {
...
export const RoundThinkingTitle = ({
  ...
  if (isAwaitingPrompt) {
    thinkingButtonLabel = waitingForInputLabel;
  } else if (isLoading) {
  ...
  return (
    ...
    >
      <EuiFlexGroup gutterSize="s" direction="row" alignItems="center" responsive={false}>
        ...
        {hasSteps && (
          <EuiIcon type="chevronSingleRight" color="subdued" size="m" aria-hidden={true} />
        )}
      </EuiFlexGroup>
    </EuiFlexGroup>
  );
