...
apiTest.describe('Agent Builder — RBAC (stateful)', { tag: [...tags.stateful.classic] }, () => {
  ...
  apiTest('minimal_read: can GET agents, agent by id, tools, tool by id', async ({ apiClient }) => {
  ...
  apiTest('manage_agents: can CRUD agent; cannot mutate tools', async ({ apiClient }) => {
    ...
    const updateRes = await apiClient.put(`${rbacApiBase}/agents/${encodeURIComponent(agentId)}`, {
      headers: h,
  ...
  apiTest('manage_tools: can CRUD tool; cannot mutate agents', async ({ apiClient }) => {
    ...
    const deleteRes = await apiClient.delete(`${rbacApiBase}/tools/${encodeURIComponent(toolId)}`, {
      headers: h,
      responseType: 'json',
    });
    expect(deleteRes).toHaveStatusCode(200);
