...
// `manage_agents` sub-feature, so they are the "restricted" persona the
// per-space default agent feature pins to a single agent. `actions: ['read']`
// lets the chat UI read the LLM connector without erroring. `spaces: ['*']`
// keeps the role valid in the default space these UI tests run in.
const READ_ONLY_ROLE: KibanaRole = {
  elasticsearch: { cluster: [], indices: [] },
  kibana: [
    {
      base: [],
      feature: { agentBuilder: ['minimal_read'], actions: ['read'] },
      spaces: ['*'],
    },
  ],
};
