...
const ADD_EVENTS_PATH = (id: string) =>
  `${CONVERSATIONS_PATH}/${encodeURIComponent(id)}/_add_events`;
...
apiTest.describe(
  ...
  () => {
    ...
    apiTest('returns 404 for a conversation that does not exist', async ({ asAdmin }) => {
      ...
      // Sends an internal type so type-validation fires first; this test is
      // rewritten in a follow-up commit once a custom type is available to
      // send a valid payload to a non-existent conversation.
      expect(res).toHaveStatusCode(400);
    });
  }
);
