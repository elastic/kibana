...
apiTest.describe(
  'Agent Builder — agents access-control mode API',
  { tag: [...tags.stateful.classic] },
  () => {
    const createdAgentIds: string[] = [];

    const mockAgent = {
      id: 'test-agent-access-control-mode',
      name: 'Test Agent',
    ...
    apiTest('POST allows explicit private access-control mode', async ({ asAdmin }) => {
    ...
    apiTest('PUT updates access-control mode explicitly', async ({ asAdmin }) => {
