...
import {
  setupAnswerAgentCallsInvalidTool,
  apiTest,
  API_AGENT_BUILDER,
  getConversation,
  postConverse,
...
apiTest.describe(
  'Agent Builder — converse error handling API',
  ...
  () => {
    ...
    for (const mode of EXECUTION_MODES) {
      ...
      // `action: 'regenerate'` was removed server-side; the field is still accepted for
      // backward compatibility and ignored, so the request behaves like a normal converse call.
      apiTest(
        `[${mode}] ignores the deprecated regenerate action and appends a new round`,
        async ({ apiClient }) => {
          ...
          expect(conversation.rounds[1].response.message).toBe(RESPONSE_2);
        }
      );
    }
  }
