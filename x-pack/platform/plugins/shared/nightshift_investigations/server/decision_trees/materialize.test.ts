...
describe('materializeDecisionTrees', () => {
  it('writes an index and each tree file verbatim', async () => {
    ...
    expect(session.mkdirs).toHaveBeenCalledWith(['/workspace/decision-trees']);
    ...
    const written = session.writeFiles.mock.calls[0][0][1].content.toString('utf8');
  ...
  it('lists each tree by filename and symptom in the index table', async () => {
    ...
    const index = session.writeFiles.mock.calls[0][0][0].content.toString('utf8');
  ...
  it('appends a reinforced learnings section built from the tree learnings', async () => {
    ...
    const file = session.writeFiles.mock.calls[0][0][1].content.toString('utf8');
  ...
  it('skips archived trees so the agent never builds on a retired one', async () => {
    ...
    expect(session.writeFiles.mock.calls[0][0]).toHaveLength(1);
  });
  ...
  it('writes only the requested trees when treeIds is set', async () => {
    ...
    const paths = session.writeFiles.mock.calls[0][0].map((file: { path: string }) => file.path);
  ...
  it('does not write files when the hydrate signal has already aborted', async () => {
    ...
    expect(session.mkdirs).not.toHaveBeenCalled();
