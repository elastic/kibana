...
test.describe(
  'Onboarding Authenticate and Deploy step — AWS identity federation flag OFF',
  { tag: tags.stateful.classic },
  () => {
    useOnboardingFeatureFlag({ [AWS_IDENTITY_FEDERATION_ENABLED_FLAG]: false });

    test.beforeEach(async ({ page }) => {
      await mockAwsPackage(page, MOCK_AWS_PACKAGE_IDENTITY_FEDERATION_SUPPORTED);
    });

    test('offers only access keys as an authentication method', async ({ browserAuth, page }) => {
      ...
      const description = page.testSubj.locator('managedIntegrationsSection-description');
      await expect(description).toContainText('Utilize AWS Access Keys to set up');
      await expect(description).not.toContainText('Federated Identity');
