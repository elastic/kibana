...
import { isMigratedAttachmentType, isPersistableType, toUnifiedAttachmentType } from './migration_utils';
...
describe('migration_utils', () => {
  describe('isMigratedAttachmentType', () => {
    ...
    it('is true for legacy and unified Lens persistable subtype ids', () => {
      expect(isMigratedAttachmentType(LEGACY_LENS_ATTACHMENT_TYPE)).toBe(true);
      expect(isMigratedAttachmentType(LENS_ATTACHMENT_TYPE)).toBe(true);
    });
