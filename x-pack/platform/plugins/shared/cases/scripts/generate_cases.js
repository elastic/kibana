...
const fetch = async ({ options, data }) => {
...
const generateCases = async (cases) => {
  try {
    await pMap(
      cases,
      (theCase) => {
        ...
        return fetch({ options, data: theCase });
      },
      { concurrency: 100 }
    );
  } catch (error) {
