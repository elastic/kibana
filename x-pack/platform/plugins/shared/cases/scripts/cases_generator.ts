...
interface SpaceConfig {
  /** Pattern for generated space IDs — {i} is replaced with 1-based index, e.g. "analytics-{i}". */
  namePattern: string;
...
interface GeneratorConfig {
  ...
  /** Create multiple spaces before generating cases. Null = use single `space` field. */
  spaces: SpaceConfig | null;
  /**
   * Weighted owner distribution, e.g. { securitySolution: 50, observability: 30, cases: 20 }.
   * Values are relative weights (not required to sum to 100). Null = uniform random.
   */
  ownerDistribution: Record<string, number> | null;
  /**
   * Which solution owners should have analytics indices enabled via the configure API.
   * Applied to every target space after space creation. Null = don't enable analytics.
   */
...
/**
 * Picks an owner from the given list using weighted random selection.
 * Falls back to uniform random when distribution is null or all weights are zero.
 */
function weightedOwnerPick(owners: string[], distribution: Record<string, number> | null): string {
  if (!distribution || owners.length === 0) return pick(owners);
  const totalWeight = owners.reduce((sum, o) => sum + (distribution[o] ?? 0), 0);
...
/**
 * Parses an owner distribution string like "securitySolution:50,observability:30,cases:20"
 * into a weight map. Returns null if the string is empty or unparseable.
 */
function parseOwnerDistribution(str: string): Record<string, number> | null {
  ...
  for (const pair of str
    ...
    if (colonIdx > 0) {
      ...
      if (owner && !isNaN(weight)) result[owner] = weight;
    }
  }
...
function generateObservabilityAlert(alertNum: number) {
  ...
  return {
    ...
    _source: {
      ...
      // consumer must match producer so the correct observability alert renderer is used
...
/**
 * Creates a single Kibana space via POST /api/spaces/space.
 * Silently skips if the space already exists (409).
 */
...
/**
 * Creates N spaces from a SpaceConfig, returning the list of space IDs.
 * Space names are generated by replacing {i} in namePattern with 1-based index.
 */
async function createSpaces(
  ...
  for (const spaceId of spaceIds) {
    await createSpace(kbnClient, headers, spaceId);
  }
  return spaceIds;
}
...
/**
 * Enables analytics for a specific owner in a given space by patching (or creating)
 * the cases-configure saved object with `analytics_enabled: true`.
 *
 * Flow:
 *   1. GET /api/cases/configure?owner=<owner>  — find existing configure SO
 *   2. If found → PATCH /api/cases/configure/<id>  with { analytics_enabled: true, version }
 *   3. If not found → POST /api/cases/configure  with required fields + analytics_enabled: true
 */
async function enableAnalyticsForOwner({
  kbnClient,
  headers,
  space,
  owner,
  ...
  const basePath = space ? `/s/${space}` : '';
  const configurePath = `${basePath}/api/cases/configure`;
  const spaceLabel = space || 'default';

  try {
    // Try to find existing configure SO for this owner
    const { data: existing } = await kbnClient.request<Array<{ id: string; version: string }>>({
      method: 'GET',
...
/**
 * Enables analytics for the given owners across all target spaces.
 */
async function enableAnalyticsForSpaces({
  kbnClient,
  headers,
  spaces,
  owners,
  ...
  for (const space of spaces) {
    for (const owner of owners) {
      await enableAnalyticsForOwner({ kbnClient, headers, space, owner });
    }
  }
}
  ...
  await pMap(
    ...
    async (newCase, index) => {
      ...
      try {
        ...
        // observability cases do not support event attachments
        for (let i = 0; i < eventsPerCase && newCase.owner !== 'observability'; i++) {
...
async function interactiveMode(): Promise<GeneratorConfig> {
  ...
  // Step 2: Spaces
  console.log('\n── Step 2: Spaces ──\n');
  const space = await prompt(rl, 'Single space ID (empty for default)', '');
  const createMultipleStr = await prompt(rl, 'Create multiple spaces? (y/n)', 'n');
  let spaces: SpaceConfig | null = null;
  if (createMultipleStr.toLowerCase() === 'y') {
    const patternStr = await prompt(rl, 'Space name pattern ({i} = index)', 'analytics-{i}');
    const numSpacesStr = await prompt(rl, 'Number of spaces to create', '3');
    spaces = { namePattern: patternStr, count: parseInt(numSpacesStr, 10) || 1 };
  }

  // Step 3: Owners & distribution
  console.log('\n── Step 3: Owners & Distribution ──\n');
  ...
  if (configureDistStr.toLowerCase() === 'y') {
    ownerDistribution = {};
    const defaultPct = Math.floor(100 / owners.length);
    for (const owner of owners) {
      const pctStr = await prompt(rl, `  Weight for "${owner}"`, String(defaultPct));
      ownerDistribution[owner] = parseFloat(pctStr) || defaultPct;
    }
  }

  const enableAnalyticsStr = await prompt(
    rl,
    'Enable analytics indices for specific solutions? (y/n)',
    'n'
  );
  let analyticsOwners: string[] | null = null;
  if (enableAnalyticsStr.toLowerCase() === 'y') {
    const analyticsOwnersStr = await prompt(
      rl,
      'Which solutions should have analytics enabled? (comma-separated)',
      owners.join(',')
    );
    analyticsOwners = analyticsOwnersStr
      .split(',')
      .map((o) => o.trim())
      .filter(Boolean);
  }
  ...
  console.log('\n── Step 4: Case Generation ──\n');
  const countStr = await prompt(rl, 'Number of cases to generate (per space)', '10');
  ...
  console.log(`  ES:         ${node}`);
  if (spaces) {
    console.log(`  Spaces:     ${spaces.count} × pattern "${spaces.namePattern}"`);
  } else {
  ...
  if (analyticsOwners && analyticsOwners.length > 0) {
    console.log(`  Analytics:  ${analyticsOwners.join(', ')}`);
  }
  console.log(`  Cases:      ${count}${spaces ? ' per space' : ''}`);
...
async function main() {
  try {
    ...
    // Determine target spaces — create them if a SpaceConfig was provided
    const targetSpaces: string[] =
      config.spaces && config.spaces.count > 0
        ? await createSpaces(kbnClient, headers, config.spaces)
        : [config.space];

    // Enable analytics for the configured owners in every target space
    if (config.analyticsOwners && config.analyticsOwners.length > 0) {
    ...
    // Create an ES client once (reused across spaces)
    ...
    // Generate cases per space
    for (const space of targetSpaces) {
      if (targetSpaces.length > 1) {
        logger.info(`\n── Space: ${space || 'default'} ──`);
      }

      // Build case objects for this space using weighted owner distribution
      ...
      if (esClient) {
        if (config.alerts > 0) {
          alertsByOwner = await indexAlertsForOwners(esClient, cases, config.alerts, space);
        }
        if (config.events > 0) {
