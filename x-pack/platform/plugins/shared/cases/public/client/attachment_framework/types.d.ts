 ...
 */
export interface UnifiedReferenceAttachmentViewProps<Metadata = UnifiedReferenceAttachmentPayload['metadata']> extends CommonAttachmentViewProps {
 ...
 */
export interface UnifiedValueAttachmentViewProps<Data = UnifiedValueAttachmentPayload['data']> extends CommonAttachmentViewProps {
    data: Data;
    createdBy: CaseUser;
    version: string;
    rowContext: RowContext;
...
export type UnifiedReferenceAttachmentType<Metadata = UnifiedReferenceAttachmentPayload['metadata']> = AttachmentType<UnifiedReferenceAttachmentViewProps<Metadata>> & UnifiedAttachmentSchema;
export type UnifiedValueAttachmentType<Data = UnifiedValueAttachmentPayload['data']> = AttachmentType<UnifiedValueAttachmentViewProps<Data>> & UnifiedAttachmentSchema;
...
type IsReferenceSchema<S extends z.ZodType> = z.infer<S> extends {
    attachmentId: string;
} ? true : false;
/** Narrow registration type — renderers see typed `data`/`metadata` from `schema`. */
type UnifiedAttachmentTypeFromSchema<S extends z.ZodType> = IsReferenceSchema<S> extends true ? UnifiedReferenceAttachmentType<z.infer<S> extends {
    metadata?: infer M;
} ? M : never> : z.infer<S> extends {
    data: infer D;
} ? UnifiedValueAttachmentType<D> : never;
/** Broad registration type returned to callers; avoids a union (contravariant intersection on view props). */
type UnifiedAttachmentTypeForRegistry<S extends z.ZodType> = IsReferenceSchema<S> extends true ? UnifiedReferenceAttachmentType : UnifiedValueAttachmentType;
/**
