...
export interface CaseAttachmentWorkflowContextValue {
  caseId: string;
  /**
...
/**
 * Reads the enclosing case context. Returns `undefined` outside a case attachment surface, or
 * when the current user cannot run workflows through Cases. Absence is a legitimate state
 * (alerts page, flyout), not a programming error, so this does not throw.
 */
