...
describe('stringifyUrlParams', () => {
  ...
  it('encodes defaults correctly', () => {
    ...
    expect(stringifyUrlParams(urlParams)).toMatchInlineSnapshot(
      `"cases=(assignees:!(),category:!(),customFields:(my_field:!(foo,bar)),extendedFieldFilters:!(),from:now-30d,owner:!(),page:1,perPage:10,reporters:!(),search:'',searchFields:!(cases.title,cases.description,cases.incremental_id.text,cases.observables.value,cases.customFields.value,cases-comments.comment,cases-comments.alertId,cases-comments.eventId,cases.ef_all_values),severity:!(),sortField:createdAt,sortOrder:desc,status:!(),tags:!(),to:now)"`
    );
  });
