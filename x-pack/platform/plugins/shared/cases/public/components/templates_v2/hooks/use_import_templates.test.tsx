...
describe('useImportTemplates', () => {
  ...
  it('sends template metadata as template-prefixed keys and case defaults as top-level keys', async () => {
    (postTemplate as jest.Mock).mockResolvedValue({ templateId: 'new-1' });
    const template = makeTemplate({
    ...
    expect(parsedDefinition.template_name).toEqual('Test Template');
    expect(parsedDefinition.template_description).toEqual('Template metadata description');
    expect(parsedDefinition.template_tags).toEqual(['metadata-tag']);
