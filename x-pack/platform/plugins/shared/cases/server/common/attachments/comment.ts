...
): attachment is UserCommentAttachmentPayload {
  return (
    isLegacyAttachmentRequest(attachment) &&
    isLegacyCommentAttachment(attachment) &&
...
export function isUnifiedPayloadCommentAttachment(
  ...
  return isUnifiedValueAttachmentRequest(attachment) && isUnifiedCommentAttachment(attachment);
}
...
function isNewSchema(attributes: AttachmentAttributesV2): boolean {
  return isUnifiedValueAttachmentRequest(attributes) && attributes.type === COMMENT_ATTACHMENT_TYPE;
}

function isOldSchema(attributes: AttachmentAttributesV2): boolean {
  return isLegacyAttachmentRequest(attributes) && attributes.type === AttachmentType.user;
}
