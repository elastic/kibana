...
describe('buildAttachmentTypeFilter', () => {
  ...
  it('maps a unified alert type to the legacy `alert` bucket', () => {
  ...
  it('does not recognize legacy spellings (e.g. `alert`) as a unified type', () => {
    ...
    // Only added as a literal (never-matching) unified-side value; no legacy mapping applies.
    expect(filter).not.toContain('cases-comments.attributes.type');
  });

  it('matches a specific external-reference subtype id, not the whole legacy bucket', () => {
  ...
  it('treats unified-only types (no legacy form) as unified-side only', () => {
