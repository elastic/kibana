...
export class LifecycleTransform {
  ...
  public async upsert(): Promise<void> {
    try {
      ...
      this.logger.error(`[${this.transformId}] Failed to upsert lifecycle transform.`, { error });
    }
  }
  ...
  private async upsertDestinationIndex(): Promise<void> {
    ...
    try {
      await this.esClient.indices.create({
        ...
        aliases: {
          // Read alias only — the transform writes directly to the internal
          // backing index and does not use the alias as a write target.
          [this.indexAlias]: {
      ...
      // A second Kibana node may have created the index between our exists-check
      // and this create call — that is not a real error.
      if (error.body?.error?.type === 'resource_already_exists_exception') {
        this.logger.debug(
          `[${this.transformId}] Lifecycle destination index created concurrently, skipping.`
        );
        return;
      }
      throw error;
  ...
  private async upsertTransform(): Promise<void> {
    ...
    try {
      ...
      // A second Kibana node may have called putTransform between our exists-check
      // and this call.  ES stores transforms as versioned cluster-state documents,
      // so a concurrent creation produces version_conflict_engine_exception (not 409).
      if (
        error.body?.error?.type === 'version_conflict_engine_exception' ||
        error.body?.error?.type === 'resource_already_exists_exception'
      ) {
        this.logger.debug(
          `[${this.transformId}] Transform created concurrently by another node (race condition). Continuing.`
        );
        return;
      }
      throw error;
