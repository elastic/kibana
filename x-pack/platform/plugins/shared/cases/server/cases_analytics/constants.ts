...
import {
  ...
  CAI_CONTENT_SYNC_TYPE,
  getContentDestinationIndexName,
  getContentSynchronizationSourceQuery,
} from './content_index/constants';
import {
...
export type CAISyncType = typeof CAI_CONTENT_SYNC_TYPE | typeof CAI_ACTIVITY_SYNC_TYPE;

export const CAISyncTypes = [CAI_CONTENT_SYNC_TYPE, CAI_ACTIVITY_SYNC_TYPE] as const;

export const SYNCHRONIZATION_QUERIES_DICTIONARY: Record<
  string,
  (lastSyncAt: Date, spaceId: string, owner: Owner) => QueryDslQueryContainer
> = {
  [CAI_CONTENT_SYNC_TYPE]: getContentSynchronizationSourceQuery,
...
export const sourceIndexBySyncType = (syncType: CAISyncType): string => {
  switch (syncType) {
    case CAI_CONTENT_SYNC_TYPE:
      return CAI_CONTENT_SOURCE_INDEX;
...
export const destinationIndexBySyncType = (
  ...
  switch (syncType) {
    case CAI_CONTENT_SYNC_TYPE:
      return getContentDestinationIndexName(spaceId, owner);
    case CAI_ACTIVITY_SYNC_TYPE:
