  ...
  mode: z
    ...
    .describe(
      'Operation to perform. Required fields per mode:\n' +
        ...
        '- add_alerts: case_id, alerts (array of {alertId, index, rule?})\n' +
        '- add_events: case_id, events (array of {eventId, index})\n' +
        '- get_all: case_id'
    ),
...
export const attachmentsTool = (
  ...
  return {
    ...
    description:
      'Manages case attachments: add comments, attach alerts or events, and retrieve all attachments. Use the `mode` field to specify the operation.',
