...
| \`${
    ...
    }\` | \`add_comment\`, \`add_alerts\`, \`add_events\`, \`add_attachments\`, \`get_all\` |
...
- \`${
      platformCoreCasesTools.attachments
    }\` mode \`get_all\` — returns the discriminated attachments array; summarize them.
...
Cases tools return metadata only — comments and attachments are never included. To analyze discussion, call \`${
      platformCoreCasesTools.attachments
    }\` with \`mode: "get_all"\` and the case ID; filter to \`type === "user"\` for comments.
