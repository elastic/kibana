...
export type BucketKeyString = Omit<Bucket, 'key'> & {
    key: string;
};
...
export interface CasesTelemetryWithAlertsAggsByOwnerResults {
    by_owner: {
        buckets: Array<ReferencesAggregation & {
            key: string;
            doc_count: number;
        }>;
    };
}
export type FileAttachmentAggregationResults = Record<Owner, FileAttachmentAggsResult> & FileAttachmentAggsResult;
...
export type CaseAggregationResult = Record<Owner, {
    counts: Buckets;
