...
describe('toUnifiedAttributes', () => {
  ...
  // folded to `security.endpoint` it never re-emits `actions`. Reads fold it in-memory
  // to the unified shape so the client/UI can render it; the stored SO is untouched.
  describe('legacy `actions` (in-memory fold on read)', () => {
