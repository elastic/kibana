...
import { KV_ERROR_PROMPT, KV_HEADER_ERROR_PROMPT, KV_HEADER_REGEX_ERROR_PROMPT } from './prompts';
import {
  COMMON_ERRORS,
  KV_EXAMPLE_ANSWER,
  KV_HEADER_ERROR_EXAMPLE_ANSWER,
  KV_HEADER_REGEX_EXAMPLE_ANSWER,
} from './constants';
...
export async function handleHeaderRegexError({
  state,
  model,
}: HandleKVNodeParams): Promise<Partial<KVState>> {
  const outputParser = new JsonOutputParser();
  const kvHeaderErrorGraph = KV_HEADER_REGEX_ERROR_PROMPT.pipe(model).pipe(outputParser);

  const pattern = await kvHeaderErrorGraph.invoke({
    packageName: state.packageName,
    dataStreamName: state.dataStreamName,
    current_regex: JSON.stringify(state.regex, null, 2),
    errors: JSON.stringify(state.errors, null, 2),
