...
interface WrapClusterClientFactoryOpts {
...
type WrapClusterClientOpts = WrapClusterClientFactoryOpts & {
  logMetricsFn: LogSearchMetricsFn;
};

type WrapEsClientOpts = Omit<WrapClusterClientOpts, 'clusterClient'> & {
...
export interface WrappedClusterClient {
  client: () => IClusterClient;
  getMetrics: () => SearchMetrics;
}

export function createWrappedClusterClientFactory(
  opts: WrapClusterClientFactoryOpts
): WrappedClusterClient {
  ...
  const wrappedClient = wrapClusterClient({ ...opts, logMetricsFn: logMetrics });
...
class WrappedClusterClientImpl implements IClusterClient {
  #asInternalUser?: ElasticsearchClient;
  #asCurrentUser?: ElasticsearchClient;
  #asSecondaryAuthUser?: ElasticsearchClient;

  constructor(private readonly opts: WrapClusterClientOpts) {}

  public asScoped(request: ScopeableRequest): IScopedClusterClient {
    if (this.#asInternalUser === undefined) {
      const { clusterClient, ...rest } = this.opts;
    ...
    if (this.#asCurrentUser === undefined) {
      ...
      this.#asCurrentUser = wrapEsClient({
        ...rest,
        esClient: clusterClient.asScoped(request).asCurrentUser,
      });
    }
    if (this.#asSecondaryAuthUser === undefined) {
      ...
      this.#asSecondaryAuthUser = wrapEsClient({
        ...rest,
        esClient: clusterClient.asScoped(request).asSecondaryAuthUser,
      });
    }
    return {
      asInternalUser: this.#asInternalUser,
      asCurrentUser: this.#asCurrentUser,
      asSecondaryAuthUser: this.#asSecondaryAuthUser,
    };
  }
...
function wrapClusterClient(opts: WrapClusterClientOpts): IClusterClient {
  return new WrappedClusterClientImpl(opts);
}
  ...
  async function request<TResponse = unknown, TContext = unknown>(
    ...
    if (params.method === 'POST' && params.path === '/_query') {
      ...
      try {
        ...
        opts.logger.debug(
          () =>
            `executing ES|QL query for task ${opts.task.id} - ${JSON.stringify(
              params
...
function getWrappedEqlSearchFn(opts: WrapEsClientOpts) {
  ...
  async function search<TEvent = unknown>(
    ...
    try {
      ...
      opts.logger.debug(
        () =>
          `executing eql query for task ${opts.task.id} - ${JSON.stringify(
            params
...
function getWrappedSearchFn(opts: WrapEsClientOpts) {
  ...
  > {
    ...
    try {
      ...
      opts.logger.debug(
        () =>
          `executing query for task ${opts.task.id} - ${JSON.stringify(
            params
          )} - with options ${JSON.stringify(searchOptions)}${
            requestTimeout ? ` and ${requestTimeout}ms requestTimeout` : ''
          }`
      ...
      opts.logger.trace(
        () => `result of executing query for task ${opts.task.id}: ${JSON.stringify(body)}`
      );
      return result;
    } catch (e) {
      if (opts.abortController.signal.aborted) {
