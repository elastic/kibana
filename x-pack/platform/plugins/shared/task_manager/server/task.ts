...
/**
 * The maximum number of task instances a single `createBatchTaskRunner` batch
 * may contain. Bounds the ES cost of a single claim/run cycle for a batchable
 * task type.
 */
export const MAX_BATCH_SIZE = 1000;

/**
 * The run context passed into a batch task's run function as its sole argument.
 * Unlike `RunContext`, a batch run covers many task instances of the same type
 * claimed together, and does not support `fakeRequest` / `enrichRequest` since
 * those are per-instance concerns.
 */
export interface BatchRunContext {
  /**
   * The task instances that make up this batch. All share the same `taskType`.
   */
  taskInstances: ConcreteTaskInstance[];

  signal: AbortSignal;

  /**
   * A UUID generated by Task Manager for this specific execution of the batch.
   */
  executionUuid: string;
}

/**
 * The result of running a batch task, keyed by task instance id. Any task
 * instance whose id is absent from the map is treated as a failure for that
 * task, so it is retried rather than silently marked successful.
 */
export type BatchRunResult = Map<string, RunResult>;

export type BatchRunFunction = () => Promise<BatchRunResult>;
export interface CancellableBatchTask {
  run: BatchRunFunction;
  cancel?: CancelFunction;
  cleanup?: () => Promise<void>;
}

export type BatchTaskRunCreatorFunction = (context: BatchRunContext) => CancellableBatchTask;
...
export const taskDefinitionSchema = schema.object(
  {
     ...
     * type is run via `createBatchTaskRunner`. Required (and only meaningful)
     * for batchable task types; see `createBatchTaskRunner` on `TaskDefinition`.
     */
    batchSize: schema.maybe(
      schema.number({
        min: 1,
        max: MAX_BATCH_SIZE,
      })
...
export type TaskDefinition = Omit<TypeOf<typeof taskDefinitionSchema>, 'paramsSchema'> & {
   ...
   *
   * Mutually exclusive with `createBatchTaskRunner` — a task type may define
   * exactly one of the two.
   */
  createTaskRunner?: TaskRunCreatorFunction;
  /**
   * Creates an object that has a run function which performs the work for a
   * whole batch of task instances of this type (see `batchSize`) in one call,
   * returning a result per task instance. Task Manager groups task instances
   * of this type claimed within a poll cycle into batches of up to `batchSize`
   * and charges a single capacity slot per batch, rather than one slot per
   * task instance.
   *
   * Mutually exclusive with `createTaskRunner` — a task type may define
   * exactly one of the two. Requires `batchSize` to also be set.
   */
