...
export const buildTaskFakeRequest = ({
  ...
  // verdict is UIAM's own, captured when the task was scheduled and persisted on `userScope`.
  // Framework-granted UIAM keys (flag absent) keep receiving the shared secret.
  if (uiamApiKeyExternal) {
    Object.assign(headers, getExternalUiamCredentialHeaders());
  }
