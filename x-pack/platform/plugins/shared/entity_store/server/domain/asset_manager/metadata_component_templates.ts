...
export const getMetadataComponentTemplateName = (namespace: string) =>
  `${ENTITY_BASE_PREFIX}-${ENTITY_SCHEMA_VERSION_V2}-${ENTITY_METADATA}_${namespace}@platform`;

/** @deprecated Legacy Security-scoped name; used only for upgrade migration. */
export const getLegacySecurityMetadataComponentTemplateName = (namespace: string) =>
