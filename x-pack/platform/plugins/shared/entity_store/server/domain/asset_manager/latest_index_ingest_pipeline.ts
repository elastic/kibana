...
export const getLatestIndexIngestPipelineId = (namespace: string): string =>
  `.${ENTITY_BASE_PREFIX}.${ENTITY_SCHEMA_VERSION_V2}.${ENTITY_LATEST}.${namespace}_ingest_pipeline`;

/** @deprecated Legacy Security-scoped name; used only for upgrade migration. */
export const getLegacySecurityLatestIndexIngestPipelineId = (namespace: string): string =>
