...
describe('conversation proposals routes', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('should gate reads on the read privilege and decisions on the write privilege', () => {
    const { posts, gets, byPath } = registerAndCollect({});

    expect(byPath(gets, '/internal/proposals').config.security?.authz?.requiredPrivileges).toEqual([
      PROPOSALS_API_PRIVILEGE_READ,
    ]);
