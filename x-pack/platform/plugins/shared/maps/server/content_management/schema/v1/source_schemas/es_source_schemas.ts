...
const applyForceRefreshSchema = lazySchema(() =>
  z.boolean().default(true).optional().meta({
    description: `When true, results are re-fetched when page's automatic refresh fires or user clicks 'Refresh'.`,
  })
);

const narrowByGlobalSearch = lazySchema(() =>
  z
    .boolean()
    .default(true)
    .optional()
...
const narrowByGlobalTime = lazySchema(() =>
  z
    .boolean()
    .default(true)
    .optional()
...
export const BaseESSourceSchema = lazySchema(() =>
  z
    .object({
...
export const ESSearchSourceSchema = lazySchema(() =>
  BaseESSourceSchema.extend({
...
export const ESQLSourceSchema = lazySchema(() =>
  z
    .object({
      applyForceRefresh: applyForceRefreshSchema,
