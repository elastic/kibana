...
apiTest.describe('Maps - map migrations', { tag: [...tags.stateful.classic] }, () => {
  ...
  apiTest(
    'should apply saved object reference migration when importing map saved objects prior to 7.2.0',
    async ({ apiClient }) => {
      const response = await apiClient.post('api/saved_objects/map', {
  ...
  apiTest('should not fail migration with invalid attributes', async ({ apiClient }) => {
    const response = await apiClient.post('api/saved_objects/map', {
      headers: { ...PUBLIC_HEADERS, ...cookieHeader },
      body: {
        attributes: {
  ...
  apiTest('should apply embeddable migrations', async ({ apiClient }) => {
    const response = await apiClient.get(
      'api/saved_objects/dashboard/4beb0d80-c2ef-11eb-b0cb-bd162d969e6b',
      {
        headers: { ...PUBLIC_HEADERS, ...cookieHeader },
      }
    );
