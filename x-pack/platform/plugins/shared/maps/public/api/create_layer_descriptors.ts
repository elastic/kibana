...
export const createLayerDescriptors = {
  async createSecurityLayerDescriptors(
    ...
    const { createSecurityLayerDescriptors } =
      await import('../classes/layers/wizards/solution_layers/security');
  ...
  async createBasemapLayerDescriptor(): Promise<LayerDescriptor | null> {
    const { createBasemapLayerDescriptor } =
      await import('../classes/layers/create_basemap_layer_descriptor');
