...
const ALL_SAVED_OBJECT_INDICES = ['.kibana*'];
...
const getAggregatedSpaceData = (esClient: Client, objectTypes: string[]) =>
  esClient.search<unknown, { count: SpaceCountAggregate }>({
    ...
    aggs: {
      count: {
        terms: { field: 'normalized_namespace', missing: 'default', size: 10 },
