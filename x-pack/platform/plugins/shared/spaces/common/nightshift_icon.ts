...
/**
 * Inline SVG for the side nav with separate moon / star groups so CSS can
 * animate each part on hover (see `nightshift_sidenav_icon_animation.ts`).
 */
export const getNightshiftNavIconSvg = ({
  size = 16,
  startColor = NIGHTSHIFT_GRADIENT_START,
  endColor = NIGHTSHIFT_GRADIENT_END,
  solidColor,
  gradientId = GRADIENT_ID,
}: NightshiftNavIconSvgOptions = {}): string => {
  const fill = solidColor ?? `url(#${gradientId})`;
  const gradientDef = solidColor
    ? ''
    : `<defs>
    <linearGradient id="${gradientId}" x1="0.582498" y1="-0.452642" x2="13.3945" y2="11.0631" gradientUnits="userSpaceOnUse">
      <stop offset="0.168292" stop-color="${startColor}"/>
      <stop offset="0.83" stop-color="${endColor}"/>
    </linearGradient>
  </defs>`;

  return `<svg class="nightshift-nav-icon" xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="${NIGHTSHIFT_ICON_VIEWBOX}" fill="none" aria-hidden="true">
  ${gradientDef}
  <g class="nightshift-nav-icon__moon"><path d="${NIGHTSHIFT_ICON_MOON_PATH}" fill="${fill}"/></g>
  <g class="nightshift-nav-icon__star nightshift-nav-icon__star--1"><path d="${NIGHTSHIFT_ICON_STAR_1_PATH}" fill="${fill}"/></g>
  <g class="nightshift-nav-icon__star nightshift-nav-icon__star--2"><path d="${NIGHTSHIFT_ICON_STAR_2_PATH}" fill="${fill}"/></g>
</svg>`;
};
...
export function getNightshiftIconSvg({
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="${NIGHTSHIFT_ICON_VIEWBOX}" fill="none">
  ${gradientDef}
  <path d="${NIGHTSHIFT_ICON_STAR_1_PATH} ${NIGHTSHIFT_ICON_STAR_2_PATH} ${NIGHTSHIFT_ICON_MOON_PATH}" fill="${fill}"/>
</svg>`;
}
