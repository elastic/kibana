...
If the user's request does not make the scope clear, call \`ask_user_question\` alone before \`generate_dashboard\` and ask **How should I prettify this dashboard?** with two options: **Improve existing charts only** and **Improve existing charts and add useful new ones**. Do not ask when the user already specified either scope. For charts only, use \`prettifyPanelConfigs: true\` with empty \`operations\` and skip the design pass.

## Design pass

When the user requests or selects **Improve existing charts and add useful new ones**, evaluate the dashboard against a **regeneration-quality** bar — not a local cleanup mindset. Compare it to the dashboard you would create from scratch for the same purpose. If composition, coverage, hierarchy, and layout already meet that bar, do **not** invent panel additions or removals: set \`prettifyPanelConfigs: true\` with empty \`operations\` and stop. Only redesign when the existing inventory is sparse, unbalanced, redundant, or missing high-value questions for its purpose.
