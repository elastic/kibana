  ...
  it('compiles authoring, review, and skill prompt content', () => {
    expect({
      ...
      **Editing a custom content panel:**
      ...
      Hard cap: after the user answers the prettify question, call platform.dashboard.generate_dashboard **at most twice** this round (first apply, then one review retry). A third call is forbidden even if \`data.review.problems\` is still non-empty. Leftovers after the second call are informational — mention them, render the last attachment, and stop. Do not keep generating or rendering until the review is empty.
      ...
      After they answer, call platform.dashboard.generate_dashboard with \`dashboardAttachmentId\` and batched operations for only the chosen categories (or their description). Review runs on that generate. If \`data.review.problems\` is still non-empty, you may call platform.dashboard.generate_dashboard **once more** and then you must stop.
