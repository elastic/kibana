...
- Use \`add_panels\` to add panels in one batched operation. Each item declares a \`kind\`: \`markdown\` for a summary or context panel, \`visualization\` for a Lens visualization created inline from natural language, or \`panelConfig\` for a panel built from an already-resolved panel configuration (e.g. an existing visualization's config).
...
- \`add_panels\`: add panels in one batched operation. Each item declares \`kind: "markdown"\`, \`kind: "visualization"\`, or \`kind: "panelConfig"\`.
