...
- \`kind: "panelConfig"\` adds a panel from an already-resolved configuration. Supply the panel \`type\` (\`"vis"\` for a Lens visualization, \`"markdown"\` for a markdown panel) and \`config\` directly — for example, the visualization config you obtained from an existing visualization, or \`{ content }\` for markdown. The generation tool never reads an attachment or saved-object store, so all external content must already be resolved into \`config\` before you call it. Do **not** hand-build a Lens \`config\` for a new visualization here — use \`kind: "panelRequest"\` instead.
...
- If a user wants to change a dashboard panel's content, prefer \`edit_panels\` over removing and re-adding the panel. \`edit_panels\` works for ES|QL-backed Lens visualization panels (\`kind: "panelRequest"\`) and markdown panels (\`kind: "panelConfig"\`, \`type: "markdown"\`).
