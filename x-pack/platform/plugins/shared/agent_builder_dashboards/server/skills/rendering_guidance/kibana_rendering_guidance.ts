...
1. **Resolve inputs**:
   ...
   - To put an existing standalone visualization onto a dashboard, read that visualization attachment's content with \`${attachmentTools.read}\` and include its configuration in \`additionalContext\` (the generation tool never reads other attachments itself). Do NOT do this for panels already on the dashboard — the tool reads those server-side via \`dashboardAttachmentId\`; reference their panel ids in \`request\` instead.
2. **Generate** (persists automatically):
   - Call ${dashboardTools.generateDashboard} with \`dashboardAttachmentId\` set to the dashboard you are editing (omit it for a new dashboard) and your natural-language \`request\`. The tool reads the current payload from that reference, applies the requested changes, and persists the result as a \`${DASHBOARD_ATTACHMENT_TYPE}\` attachment for you.
   - It returns \`data.attachment_id\`, \`data.version\`, a compact \`data.dashboard\` summary, the inner agent's \`data.response\` text, and optional \`data.failures\`. Do **not** pass the dashboard payload back into any tool — reference \`data.attachment_id\` instead.
3. **Render**:
...
- When the user points at specific panels or sections, reference their \`id\` values from the returned \`data.dashboard\` summary in your \`request\` text. Never invent an \`attachment_id\`, panel \`id\`, or section \`id\`.
- Relay \`data.response\` before rendering, especially the \`Material decisions\` section after
  prettify/review work, so the user sees why panels or queries were materially changed or skipped.
- If the generation result includes \`data.failures\`, explain which panels remain unresolved
  after visualization generation exhausted its retries and report each returned \`type\`,
