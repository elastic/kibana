...
1. **Resolve inputs**:
   - To work with a saved dashboard, search for it with \`platform.core.ce_search\`, then attach it with \`platform.core.ce_attach\` using the exact \`entry_id\` from the search result. The attached \`${DASHBOARD_ATTACHMENT_TYPE}\` attachment is your editable working copy; pass its \`attachment_id\` to generation as \`dashboardAttachmentId\`.
...
- When a user asks what dashboards are available, search with \`platform.core.ce_search\`.
