...
import { prettifyDashboardTool } from './prettify_dashboard_tool';
...
describe('prettifyDashboardTool', () => {
  ...
  it('uses the prettify_dashboard tool id', () => {
    expect(createTool().id).toBe(dashboardTools.prettifyDashboard);
  });
  ...
  it('does not mutate when review returns no findings', async () => {
    inspectDashboardImage.mockResolvedValue([]);
    const tool = createTool();
    ...
    const dashboard = context.attachments.getAttachmentRecord('dash-1');
