...
export const panelRequestSchema = z.object({
  ...
  renderer: z
    ...
    .optional()
    .describe(
      '(optional) Which engine renders the visualization. Use "lens" (the default when omitted) for standard charts. Use "vega" for custom Vega-family visualizations (Vega-Lite by default, or allowlisted Raw Vega such as sunburst) — small multiples/faceting, layered or combination charts, scatter/bubble plots, sunburst/hierarchy, custom encodings, or when the user explicitly asks for Vega/Vega-Lite. Ignored when editing an existing panel (edits keep the existing renderer).'
    ),
  index: z
    .string()
    .max(256)
