...
export const createDashboardCeType = ({
  getDashboardClient,
}: CreateDashboardCeTypeOptions): CeTypeDefinition => ({
  id: DASHBOARD_CE_TYPE,
  fetchFrequency: () => '30m',
  ...
  getCeData: async (originId, context) => {
    try {
      ...
      context.logger.warn(
        `CE dashboard: failed to get data for '${originId}': ${(error as Error).message}`
      );
      return undefined;
    }
  },
  ...
  toAttachment: async (item, context) => {
    try {
      ...
      throw new Error(
        `CE dashboard: failed to get data for '${item.origin_id}': ${(error as Error).message}`
      );
    }
  },
});
