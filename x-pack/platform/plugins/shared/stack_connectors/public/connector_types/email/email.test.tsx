...
describe('email format edge cases with real validator', () => {
  let connectorType: ConnectorTypeModel;

  function realValidateEmails(
    addresses: string[],
    options?: ValidateEmailAddressesOptions
  ): ValidatedEmail[] {
    return validateEmailAddresses(null, addresses, options ?? {});
  }

  beforeAll(() => {
    ExperimentalFeaturesService.init({ experimentalFeatures: experimentalFeaturesMock });
    const registry = new TypeRegistry<ConnectorTypeModel>();
    registerConnectorTypes({
      connectorTypeRegistry: registry,
      services: { validateEmailAddresses: realValidateEmails },
    });
    connectorType = registry.get(CONNECTOR_TYPE_ID)!;
  });

  test('rejects address starting with @ sign', async () => {
    const result = await connectorType.validateParams(
      { to: ['@something@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toContain(
      'Email address @something@example.com is not valid.'
    );
  });

  test('rejects address with double @ sign', async () => {
    const result = await connectorType.validateParams(
      { to: ['user@@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toContain('Email address user@@example.com is not valid.');
  });

  test('rejects address with leading dot in local part', async () => {
    const result = await connectorType.validateParams(
      { to: ['.user@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
  ...
  test('rejects address with trailing dot in local part', async () => {
    const result = await connectorType.validateParams(
      { to: ['user.@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
  ...
  test('rejects address with double dots in domain', async () => {
    const result = await connectorType.validateParams(
      { to: ['user@example..com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
  ...
  test('rejects address with space in domain', async () => {
    const result = await connectorType.validateParams(
      { to: ['user@exam ple.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toContain('Email address user@exam ple.com is not valid.');
  });

  test('rejects address with single-label domain', async () => {
    const result = await connectorType.validateParams(
      { to: ['user@localhost'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toContain('Email address user@localhost is not valid.');
  });

  test('rejects address with leading hyphen in local part', async () => {
    const result = await connectorType.validateParams(
      { to: ['-user@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
  ...
  test('rejects address with leading hyphen in domain', async () => {
    const result = await connectorType.validateParams(
      { to: ['user@-example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
  ...
  test('rejects address with path traversal attempt', async () => {
    const result = await connectorType.validateParams(
      { to: ['../etc/passwd@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toContain(
      'Email address ../etc/passwd@example.com is not valid.'
    );
  });

  test('accepts valid email address with hyphens and subdomains', async () => {
    const result = await connectorType.validateParams(
      {
    ...
    expect(result.errors.to).toEqual([]);
  });

  test('accepts RFC 5322 quoted local part', async () => {
    const result = await connectorType.validateParams(
      { to: ['"quoted"@example.com'], cc: [], bcc: [], message: 'msg', subject: 'sub' },
      null
    );
    expect(result.errors.to).toEqual([]);
  });

  test('invalid edge cases in cc and bcc fields', async () => {
    const result = await connectorType.validateParams(
      {
        to: ['valid@example.com'],
        cc: ['@double@example.com'],
    ...
    expect(result.errors.to).toEqual([]);
    expect(result.errors.cc).toContain(
      'Email address @double@example.com is not valid.'
    );
    expect(result.errors.bcc).toContain('Email address user@example..com is not valid.');
  });
});
