...
describe('CreateAlert', () => {
  ...
  it('does not render the additional options by default', () => {
    ...
    expect(screen.queryByTestId('jsm-entity-row')).not.toBeInTheDocument();
  });

  it('renders the form fields by default', () => {
    render(<CreateAlert {...options} />);

    expect(screen.getByTestId('jsm-message-row')).toBeInTheDocument();
    expect(screen.getByTestId('jsm-alias-row')).toBeInTheDocument();
  ...
  it('renders the form fields with the subActionParam values', () => {
    ...
    expect(within(screen.getByTestId('jsm-tags')).getByText('super tag')).toBeInTheDocument();
    expect(
      within(screen.getByTestId('jsm-message-row')).getByDisplayValue('a message')
    ).toBeInTheDocument();
    expect(
      within(screen.getByTestId('jsm-alias-row')).getByDisplayValue('an alias')
    ).toBeInTheDocument();
    expect(
