...
describe('CloseAlert', () => {
  ...
  it('does not render the additional options by default', () => {
    ...
    expect(screen.queryByTestId('jsm-source-row')).not.toBeInTheDocument();
  });
  ...
  it('renders the additional form fields with the subActionParam values', async () => {
    ...
    expect(
      within(screen.getByTestId('jsm-source-row')).getByDisplayValue('a source')
    ).toBeInTheDocument();
    expect(
      within(screen.getByTestId('jsm-user-row')).getByDisplayValue('a user')
    ).toBeInTheDocument();
  });
