...
export const getWebhookSecretQueryParamsKeyRoute = (
  router: IRouter,
  getStartServices: StartServicesAccessor<ConnectorsPluginsStart, unknown>
) => {
  router.get(
    {
      path: `${INTERNAL_BASE_STACK_CONNECTORS_API_PATH}/{id}/secret_query_params`,
      security: {
        authz: {
          enabled: false,
          reason:
    ...
    async (
      ...
      try {
        ...
        const allowedConnectorTypes = ['.http'];
        ...
        const secretQueryParams = decryptedConnector.attributes.secrets?.secretQueryParams || {};

        return res.ok({ body: Object.keys(secretQueryParams) });
      } catch (error) {
        if (isBoom(error)) {
