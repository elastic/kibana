...
describe('catalog refresh task', () => {
  it('converts a millisecond interval into a Task Manager schedule', () => {
    expect(refreshIntervalToSchedule(60_000)).toBe('60s');
    expect(refreshIntervalToSchedule(10_000)).toBe('10s');
  });
  ...
  it('invokes refreshFromRegistry from the task runner', async () => {
    const taskManager = taskManagerMock.createSetup();
    const service = {
    ...
    expect(service.refreshFromRegistry).toHaveBeenCalledTimes(1);
  });

  it('schedules the recurring refresh task', async () => {
    const taskManager = taskManagerMock.createStart();
    await scheduleCatalogRefreshTask(taskManager, '60s', loggerMock.create());
