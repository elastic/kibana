...
apiTest.describe('Stream lifecycle - retention API', { tag: ['@ess', '@svlOblt'] }, () => {
  ...
  apiTest('should update stream with DSL retention of 7 days', async ({ apiClient, samlAuth }) => {
    const { cookieHeader } = await samlAuth.asStreamsAdmin();
  ...
  apiTest('should update stream with DSL retention of 30 days', async ({ apiClient, samlAuth }) => {
    const { cookieHeader } = await samlAuth.asStreamsAdmin();
  ...
  apiTest('should update stream with DSL retention of 90 days', async ({ apiClient, samlAuth }) => {
    const { cookieHeader } = await samlAuth.asStreamsAdmin();
  ...
  apiTest('should switch from DSL back to inherited lifecycle', async ({ apiClient, samlAuth }) => {
    const { cookieHeader } = await samlAuth.asStreamsAdmin();
  ...
  apiTest(
    ...
    async ({ apiClient, samlAuth }) => {
      ...
      expect(verifyResult.success, verifyResult.error).toBe(true);
      expect(verifyResult.stream!.stream.ingest.lifecycle.dsl.data_retention).toBe('30d');
      expect(verifyResult.stream!.stream.ingest.lifecycle.dsl.downsample).toBe([
        { after: '0d', fixed_interval: '1d' },
        { after: '10d', fixed_interval: '10d' },
      ]);
    }
