...
const bulkQueriesRoute = createServerRoute({
  ...
  handler: async ({
    ...
    // collected in typedOperations and applied atomically via syncQueries.
    const validationErrors: Array<{ id: string; message: string }> = [];
    const indexOperations = new Map<string, StreamQuery>();
    const deleteOperationIds = new Set<string>();
    ...
    for (const operation of operations) {
      if ('index' in operation && operation.index) {
        ...
        deleteOperationIds.add(operation.delete.id);
      }
    }
    ...
    // syncQueries is set-based: build the merged "next" state from current
    // links minus deletes, plus index ops (overriding existing entries).
    const { [streamName]: currentLinks } = await kiClient.getStreamToQueryLinksMap([streamName]);
    const currentIds = new Set(currentLinks.map((link) => link.query.id));

    const nextQueries: StreamQuery[] = [
      ...currentLinks
        .filter((link) => !deleteOperationIds.has(link.query.id))
        .map((link) => indexOperations.get(link.query.id) ?? link.query),
      ...Array.from(indexOperations.values()).filter((q) => !currentIds.has(q.id)),
    ];

    await kiClient.syncQueries(definition, nextQueries);
