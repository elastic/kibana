...
export const identifyFeaturesRoute = createServerRoute({
  ...
  handler: async ({
    ...
    const task = await taskClient.get<IdentifyFeaturesResult>(
      `streams_feature_identification_${params.path.name}`
    );
    ...
    if (task.status === 'not_started' || params.query.force) {
      await taskClient.schedule<FeatureIdentificationTaskParams>({
        task: {
          ...
          id: `streams_feature_identification_${params.path.name}`,
          space: '*',
          stream: params.path.name,
        },
        params: {
          name,
          connectorId,
