...
export async function generateSignificantEventQueriesUsingDescription(
  ...
  const { output: zeroShotQueries } = await inferenceClient.output({
    ...
    connectorId,
    input: `You are an expert log analyst tasked with generating KQL (Kibana Query Language) queries to identify significant events in logs generated by {{identified_system}}.
...
- Field name: ${categorizationField}
...
Based on your deep expertise of the {{identified_system}}, generate Elasticsearch KQL (Kibana Query Language) queries following the KQL Guide to detect the most significant events for this type of system.
...
Remember: focus exclusively on queries that are specific to the {{identified_system}}.
`,
    schema: {
      type: 'object',
      properties: {
