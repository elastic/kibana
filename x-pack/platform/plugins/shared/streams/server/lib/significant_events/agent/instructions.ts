...
export const SIGNIFICANT_EVENTS_AGENT_RESEARCH_INSTRUCTIONS = dedent(`
  ...
  **When did it happen? (incident windows)**
  - **cluster_by_time**: Identifies **Incident Windows** (the Arena). Requires **time_range** (from, to) and **rule_ids** (e.g. changed rules from find_changed_queries, or new/regression from compare_to_baseline). Only the given rules are clustered (stable/noisy rules are not included). Returns cluster_id, start, end, alert_count, rule_ids with **rule name and query** per rule, peak_rate. Use after step 1 (find_changed_queries or compare_to_baseline); then use group_within_window within a chosen window.
  ...
  **Filter** (what's new/changed vs baseline) → **When** (cluster_by_time) → **Where** (group_within_window, describe_cluster) → **Evidence** (sample_cluster, context_expansion) → **Correlate** (entity timeline, topology) → **Validate** (compare_to_baseline) → **Emit**. Tool outputs include rule names, queries, and scope stats (e.g. total_alert_count, changed_count, new_count) so you stay grounded without a separate context-gathering step.
  ...
  2. **When did it happen?** Use cluster_by_time to get Incident Windows (start/end/alert_count). Pass **rule_ids** from step 1 (changed rules from find_changed_queries, or e.g. new + regression from compare_to_baseline). Outputs include rule metadata per window and peak_rate. Prioritize windows by alert_count or peak_rate.
