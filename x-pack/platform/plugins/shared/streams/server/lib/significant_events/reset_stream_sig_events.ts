...
export interface StreamsSignificantEventsResetDeletedCounts {
  queries: number;
  features: number;
  rules: number;
  alerts_v1: number;
...
export const emptySignificantEventsResetDeletedCounts =
  (): StreamsSignificantEventsResetDeletedCounts => ({
    queries: 0,
    features: 0,
    rules: 0,
    alerts_v1: 0,
...
const sumDeletedCounts = (
  totals: StreamsSignificantEventsResetDeletedCounts,
  streamCounts: StreamsSignificantEventsResetDeletedCounts
): void => {
...
const collectResetSnapshot = async (kiClient: KnowledgeIndicatorClient): Promise<ResetSnapshot> => {
  ...
  const byStream: Record<string, StreamsSignificantEventsResetDeletedCounts> = {};
...
}: ResetSignificantEventsDeps): Promise<StreamsSignificantEventsResetResult> => {
