...
export class WiredStream extends StreamActiveRecord<Streams.WiredStream.Definition> {
  ...
  protected async doValidateUpsertion(
    ...
    if (this._changes.processing && this._definition.ingest.processing.steps.length > 0) {
      const streamlangValidation = validateStreamlang(this._definition.ingest.processing, 'wired');
      if (!streamlangValidation.isValid) {
        throw new MalformedStreamError(
          `Invalid processing Streamlang DSL "${
            this._definition.name
          }": ${streamlangValidation.errors.join(', ')}`
        );
      }
    }
