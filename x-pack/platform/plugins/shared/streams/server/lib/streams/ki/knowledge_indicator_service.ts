...
type CoreContext = CoreSetup<StreamsPluginStartDependencies> | CoreStart;

function isCoreSetup(core: CoreContext): core is CoreSetup<StreamsPluginStartDependencies> {
  return 'getStartServices' in core;
}

export class KnowledgeIndicatorService {
  constructor(private readonly core: CoreContext, private readonly logger: Logger) {}

  private async getCoreStart(): Promise<CoreStart> {
    if (isCoreSetup(this.core)) {
