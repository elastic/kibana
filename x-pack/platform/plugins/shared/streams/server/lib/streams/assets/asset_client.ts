...
type StoredQueryLink = Omit<QueryLink, 'query'> & {
  [QUERY_TITLE]: string;
  [QUERY_DSL_BODY]?: QueryDslQueryContainer;
  [QUERY_ESQL_BODY]?: string;
};
  ...
  if (link['asset.type'] === 'query') {
    return {
      ...
      query: {
        ...
        ...(link['query.esql.query']
          ? {
              esql: {
                query: link['query.esql.query'],
              },
            }
          : {
              dsl: {
                query: link['query.dsl.query']!,
              },
            }),
      },
    } satisfies QueryLink;
...
function toStorage(name: string, request: AssetLinkRequest): StoredAssetLink {
  ...
  if (link['asset.type'] === 'query') {
    ...
    return {
      ...
      'query.dsl.query': 'dsl' in query ? query.dsl.query : undefined,
      'query.esql.query': 'esql' in query ? query.esql.query : undefined,
    };
  }
