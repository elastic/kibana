  ...
  describe('asTree', () => {
    ...
    it('allows nested includeAll', () => {
      ...
      const child1Nested = testContentPackEntry({
        ...
        queries: [{ id: 'keep', title: 'keep query', esql: { where: 'KQL("keep")' } }],
      });
      ...
      expect(tree.children[0].children[0].request.queries).toEqual([
        { id: 'keep', title: 'keep query', esql: { where: 'KQL("keep")' } },
      ]);
    });
    ...
    it('filters streams and queries according to include spec', () => {
      const root = testContentPackEntry({
        ...
        queries: [
          { id: 'keep', title: 'keep query', esql: { where: 'KQL("keep")' } },
          { id: 'drop', title: 'drop query', esql: { where: 'KQL("drop")' } },
        ],
      });
    ...
    it('throws on duplicate query', () => {
      const existing = asTree({
        ...
        streams: [
          testContentPackEntry({
            name: 'logs',
            queries: [{ id: 'one', title: 'title', esql: { where: 'KQL("qty: one")' } }],
          }),
        ],
        include: { objects: { all: {} } },
      });
      ...
      const incoming = asTree({
        ...
        streams: [
          testContentPackEntry({
            name: 'logs',
            queries: [{ id: 'one', title: 'title', esql: { where: 'KQL("qty: two")' } }],
          }),
        ],
        include: { objects: { all: {} } },
      });
