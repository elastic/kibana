...
const assertYamlContains = (expected: string, label: string) => {
...
describe('continuous_extraction_workflow.yaml stays in sync with constants', () => {
  it('uses the correct step type', () => {
    assertYamlContains(KI_SELECT_STREAMS_STEP_TYPE, 'step type');
  });

  it('uses the correct timeout', () => {
    assertYamlContains(`timeout: "${COORDINATOR_INTERVAL_MINUTES - 1}m"`, 'timeout');
  });

  it('uses the correct coordinator interval', () => {
    assertYamlContains(`every: "${COORDINATOR_INTERVAL_MINUTES}m"`, 'coordinator interval');
  });
  ...
  it('uses the correct max poll iterations', () => {
    assertYamlContains(`max-iterations: ${MAX_POLL_ITERATIONS}`, 'max poll iterations');
  });

  it('uses the correct poll delay duration', () => {
    assertYamlContains(`duration: "${POLL_DELAY_SECONDS}s"`, 'poll delay duration');
  });
});
