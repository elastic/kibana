...
describe('StreamsKIsOnboardingClient', () => {
  ...
  describe('getStatuses', () => {
    ...
    it('maps each execution to a status summary and fills missing streams with NotStarted', async () => {
      ...
      expect(result).toEqual({
        'logs.nginx': { status: StreamsKIsOnboardingStatus.InProgress },
        'logs.apache': { status: StreamsKIsOnboardingStatus.Completed },
        'logs.haproxy': { status: StreamsKIsOnboardingStatus.Failed, error: 'boom' },
        'logs.envoy': { status: StreamsKIsOnboardingStatus.NotStarted },
      });
    });
    ...
    it('does not fetch the completed output for completed executions', async () => {
      ...
      expect(result).toEqual({
        'logs.nginx': { status: StreamsKIsOnboardingStatus.Completed },
      });
    ...
    it('ignores executions whose concurrency group key is unknown or unrequested', async () => {
      ...
      expect(result).toEqual({
        'logs.nginx': { status: StreamsKIsOnboardingStatus.NotStarted },
      });
    });
  });
