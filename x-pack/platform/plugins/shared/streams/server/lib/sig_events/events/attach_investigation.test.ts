...
const createInvestigation = (
  ...
  completed_date: '2026-01-01T01:00:00.000Z',
  ...overrides,
});

const createEventClient = (hits: SigEvent[]) => {
...
describe('attachInvestigationToEvent', () => {
  ...
  it('upserts by workflow_execution_id: replaces an existing entry for the same execution', async () => {
    const oldInvestigation = createInvestigation({ result_status: 'failed' });
    const existing = createEvent({ event_id: 'event-1', investigations: [oldInvestigation] });
    const { client, dataStreamClient } = createEventClient([existing]);

    const updated = createInvestigation({ result_status: 'success' });
    const result = await attachInvestigationToEvent({
      eventClient: client,
      eventId: 'event-1',
    ...
    // Only one entry — the old one was replaced, not duplicated
    expect(written.investigations).toHaveLength(1);
    expect(written.investigations![0].result_status).toBe('success');
  });
  ...
  it('carries forward the previous_event_id lineage', async () => {
    ...
    expect(result.updated).toBe(1);
    // The returned event_id is the newly-created version, which has previous_event_id = 'event-3'
