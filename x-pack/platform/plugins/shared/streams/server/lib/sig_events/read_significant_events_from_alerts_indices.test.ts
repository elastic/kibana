...
const makeQueryLink = (id = 'q1', overrides: Partial<QueryLink> = {}): QueryLink => ({
  query: {
    id,
    type: 'match',
    title: 'Test query',
...
describe('readSignificantEventsFromAlertsIndices', () => {
  ...
  it('groups ES|QL rows into per-rule occurrences with gap filling', async () => {
    const linkA = makeQueryLink('qa', { rule_id: 'rule-a' });
    const linkB = makeQueryLink('qb', { rule_id: 'rule-b' });
  ...
  it('emits an empty occurrences array (not a zero-filled series) for rules absent from the result', async () => {
    const linkFiring = makeQueryLink('qa', { rule_id: 'rule-a' });
    const linkSilent = makeQueryLink('qb', { rule_id: 'rule-b' });
  ...
  it('returns an empty response when the alerts index is missing (verification_exception)', async () => {
    const link = makeQueryLink('qa', { rule_id: 'rule-a' });
