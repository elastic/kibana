...
1. **Merge duplicates**: If two pages cover the same topic, merge them into one and delete the other using \`platform.streams.memory.delete\`.
2. **Remove stale entries**: Delete pages that are outdated, no longer relevant, or contain only trivial information using \`platform.streams.memory.delete\`.
...
- **Read before acting**: Always read a page's full content before deciding to modify or delete it. Use \`platform.streams.memory.read\` before writing.
...
1. Use \`platform.streams.memory.list\` to see all current pages
2. Identify candidates for merging, deletion, or improvement
3. Use \`platform.streams.memory.read\` to read pages before acting
4. Use \`platform.streams.memory.write\` or \`platform.streams.memory.patch\` to merge/improve pages
5. Use \`platform.streams.memory.delete\` to remove duplicates or stale pages`,
    getInlineTools: () =>
