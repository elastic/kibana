...
export const streamsManagementSkill = defineSkillType({
  ...
  content: dedent(`
    ...
    Write operations: "set retention" → ${UPDATE_STREAM} with changes.lifecycle. "create a partition" → ${CREATE_PARTITION}. "delete stream X" → ${DELETE_STREAM}. "map fields" → ${UPDATE_STREAM} with changes.fields. "create a query stream" / "save this ES|QL as a stream" → ${UPDATE_STREAM} with changes.query = {esql, field_descriptions?} (creates the query stream if it does not exist, updates its ES|QL if it does). Multiple ingest change types can be combined in a single ${UPDATE_STREAM} call, but changes.query is exclusive — query streams have no ingest configuration.
