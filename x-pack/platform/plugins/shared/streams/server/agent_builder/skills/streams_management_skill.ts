...
export const streamsManagementSkill = defineSkillType({
  ...
  content: dedent(`
    ...
    - ${DESIGN_PIPELINE} results: include \`steps\` (full apply-ready pipeline), \`existing_steps\` (the pipeline as it was), and \`step_changes\` (per-step diff: \`added\` / \`unchanged\` / \`removed\`). \`status: "proposal_not_applied"\` means the proposal still needs the user's approval — apply it via ${UPDATE_STREAM}, which renders a platform confirmation dialog. The dialog is a single Apply / Cancel for the entire pipeline; partial selection at the dialog is not supported. Use the confidence check and step-count rule (see "Pipeline changes" above): chain only when confident AND step count ≤ 2 AND no \`removed\` entries in \`step_changes\` the user did not explicitly ask for; otherwise present existing + proposed in chat for review. When chaining, build \`confirmation_body\` from success_rate, field_changes, warnings, hints, and a per-step list that shows additions vs unchanged. If simulation.mode is "partial", note that existing steps were not re-simulated. When extract_fields: true was used, hints will name the heuristic that produced the seed parsing step. Any warning starting with "The proposed pipeline REMOVES…" is a hard stop on auto-chaining — list each removal in chat and confirm. If hints mention fields with key=value prefixes (e.g. "Field X has values like key=val — all share a key= prefix"), present this to the user and offer to refine the extraction — do NOT silently add anything. If the user confirms, call ${DESIGN_PIPELINE} with extract_fields: false and a **complete** description of ALL fields to extract from the stream (not just the ones with prefixes). The extract_fields: false path builds a full pipeline from scratch — a partial instruction like "strip the prefix from X" will fail because it misses timestamp handling, other fields, etc. Use the field_changes from the previous result to build the full instruction. Example: "Parse body.text extracting source IP, URL path, HTTP method, status code, response size, user agent, process ID, service name (without the service= prefix), user ID (without the user= prefix), severity level, and timestamp". This produces a single optimized grok that captures all fields cleanly without separate cleanup steps. When the refinement succeeds (simulation passes), chain directly to ${UPDATE_STREAM} with the platform confirmation dialog — the user already expressed intent to apply when they asked to refine, so do not ask again in chat.
    - Simulation failure handling for ${DESIGN_PIPELINE}: when \`simulation.success_rate\` is 0%, low, or null, do NOT invent an explanation for the cause. Quote the relevant entries from the \`warnings\` array verbatim (in a quoted block or code fence) — they contain the actual Elasticsearch error. Treat any 0% or low success rate as a hard stop on auto-chaining: present the verbatim warnings to the user and ask how to proceed. Do NOT call ${UPDATE_STREAM} until the user explicitly confirms after seeing the warnings.
