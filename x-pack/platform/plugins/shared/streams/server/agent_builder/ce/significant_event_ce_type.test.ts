...
describe('createSignificantEventCeType', () => {
  beforeEach(() => {
  ...
  it('lists significant events for CE indexing', async () => {
  ...
  it('indexes a significant event entry', async () => {
    ...
    const result = await ceType.getCeData('payment-outage', {
      esClient: {} as never,
      savedObjectsClient: {} as never,
      logger: loggingSystemMock.createLogger(),
    });
      ...
      ceType.toAttachment(
        {
          ...
          origin: { uri: `${SIGNIFICANT_EVENT_CE_TYPE}://payment-outage` },
