...
2. Use follow-up \`add_panels\` with per-panel \`sectionId\` only when you need to target an existing section returned by an earlier tool result.
...
- Use \`add_panels\` to add markdown panels, panels from existing visualization attachments, or inline Lens visualization panels from natural language.
...
- Use \`add_panels\` to add markdown panels, panels from existing visualization attachments, or inline Lens visualization panels.
...
- Use \`set_metadata\` to update dashboard text metadata, or \`add_panels\` with \`source: "markdown"\` to add dashboard text panels.

Supported operations:
- \`set_metadata\`: set or update dashboard title and description.
- \`add_panels\`: add one or more panels. Each item requires \`source: "markdown" | "attachment" | "inline_visualization"\`, explicit \`grid\` coordinates, and optional \`sectionId\`.
- For \`source: "markdown"\`, provide \`content\`.
- For \`source: "attachment"\`, provide \`attachmentId\` for an existing visualization attachment.
- For \`source: "inline_visualization"\`, provide \`query\` and optional \`chartType\`, \`index\`, and \`esql\`.
- Batch multiple panel additions into the same \`add_panels\` operation whenever they can be planned together, even when they target different \`sectionId\` values.
...
- \`add_section\`: create a new section with its own \`grid.y\`, and optionally create that section's initial panels with \`panels\`. Those nested panel grids are section-relative and do not need a \`sectionId\`.
...
- \`add_panels\` with \`source: "attachment"\` consumes visualization attachment IDs and turns them into dashboard panels.
- \`add_panels\` with \`source: "inline_visualization"\` and \`edit_visualization_panels\` work directly on dashboard panels and do not create standalone visualization attachments.
