...
- Use \`set_metadata\` or \`add_panels\` with \`kind: "markdown"\` to update dashboard text.
...
- \`add_panels\` with \`kind: "visualization"\` and \`edit_visualization_panels\` work directly on dashboard panels and do not create standalone visualization attachments.
