...
import { resolveLensConfig, filterVisualizationIds } from './utils';
...
describe('resolveLensConfig', () => {
  ...
  it('should throw when referenced result is not a visualization type', () => {
    const results = new Map([['abc123', { type: ToolResultType.other, data: { someData: true } }]]);
  ...
  it('should throw when visualization config is not an object', () => {
    const results = new Map([
      ['abc123', { type: ToolResultType.visualization, data: { visualization: 'not-an-object' } }],
    ]);
    const resultStore = createMockResultStore(results);
  ...
  it('should return the visualization config when valid', () => {
    const results = new Map([
      ['abc123', { type: ToolResultType.visualization, data: { visualization: validLensConfig } }],
    ]);
    const resultStore = createMockResultStore(results);
