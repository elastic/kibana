...
/**
 * Allowed type keys for the runtime-imposed `constraints` parameter in Context Engine search.
 * Extend this enum when adding new constrainable Context Engine types.
 */
export enum ContextEngineSearchFilterType {
  connector = 'connector',
}

/**
 * Runtime-imposed, per-type id-allowlist constraints for Context Engine search.
 *
 ...
 *
 * Keys must be values of {@link ContextEngineSearchFilterType}.
 *
 ...
 */
export type ContextEngineSearchConstraints = Partial<
  Record<ContextEngineSearchFilterType, { ids?: string[] }>
>;

/**
 * Agent-discoverable refinements for Context Engine search.
 *
 * Exposed in the LLM tool input schema; the agent picks which (if any) to
 * supply. Combined with {@link ContextEngineSearchConstraints} server-side — agent filters
 * never widen the runtime-imposed constraints.
 */
export interface ContextEngineSearchFilters {
  /** Restrict to one or more Context Engine types (ANY semantics; matches if `type` is in the list). */
  types?: string[];
 ...
 */
export const CONTEXT_ENGINE_HTTP_SEARCH_QUERY_MAX_LENGTH = 512;

/**
 * Response body for `POST /internal/agent_context_layer/sml/_search`.
 */
export interface ContextEngineSearchHttpResponse {
  results: ContextEngineSearchHttpResultItem[];
}
 ...
 */
export interface ContextEngineSearchHttpResultItem {
  id: string;
  type: string;
  origin: { uri: string };
  title: string;
...
/**
 * Wire representation of a single Context Engine object.
 *
 * Mirrors the server-side `ContextEngineDocument` shape used by the storage layer.
 */
export interface ContextEngineHttpItem {
  id: string;
  type: string;
  title: string;
  origin: { uri: string };
 ...
 */
export interface ContextEngineGetHttpResponse {
  items: ContextEngineHttpItem[];
}
 ...
 */
export const CONTEXT_ENGINE_HTTP_LIST_PER_PAGE_DEFAULT = 20;
export const CONTEXT_ENGINE_HTTP_LIST_PER_PAGE_MAX = 1000;
export const CONTEXT_ENGINE_HTTP_LIST_PAGE_DEFAULT = 1;

/**
 * Response body for `GET /internal/agent_context_layer/sml`.
 */
export interface ContextEngineListHttpResponse {
  page: number;
  per_page: number;
  items: ContextEngineHttpItem[];
}
 ...
 */
export interface ContextEngineUpsertHttpResponse {
  items: ContextEngineHttpItem[];
 ...
 */
export interface ContextEngineDeleteHttpResponse {
  origin_id: string;
  deleted: boolean;
}
 ...
 */
export const CONTEXT_ENGINE_HTTP_AUTOCOMPLETE_QUERY_MAX_LENGTH = 256;

/**
 * Response body for `POST /internal/agent_context_layer/sml/_autocomplete`.
 */
export interface ContextEngineAutocompleteHttpResponse {
  results: ContextEngineAutocompleteHttpResultItem[];
}
 ...
 */
export interface ContextEngineAutocompleteHttpResultItem {
  id: string;
  type: string;
  origin: { uri: string };
  title: string;
   ...
   */
  matched_discovery_labels?: ContextEngineMatchedDiscoveryLabel[];
}

export interface ContextEngineMatchedDiscoveryLabel {
  value: string;
  kind: string;
  /**
