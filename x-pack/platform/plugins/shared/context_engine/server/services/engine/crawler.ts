...
import type { ContextEngineIndexer } from './indexer';
import {
  createContextEngineCrawlerStateStorage,
  type ContextEngineCrawlerStateStorage,
} from './crawler_state_storage';
import {
  createContextEngineStorage,
  contextEngineIndexName,
  type ContextEngineStorage,
} from './storage';

export type { ContextEngineCrawler };

export interface ContextEngineCrawlerDeps {
  indexer: ContextEngineIndexer;
  logger: Logger;
}

export class ContextEngineCrawlerImpl implements ContextEngineCrawler {
  private readonly indexer: ContextEngineIndexer;
  private readonly logger: Logger;

  constructor({ indexer, logger }: ContextEngineCrawlerDeps) {
  ...
  async crawl({
    ...
    const crawlerStateStorage = createContextEngineCrawlerStateStorage({
      logger: this.logger,
      esClient,
    });
    ...
    this.logger.debug(
      `Context Engine crawler: starting crawl for type '${definition.id}' across all spaces`
    );
    ...
    try {
      for await (const page of definition.list(context)) {
        if (abortSignal?.aborted) {
          this.logger.info(
            `Context Engine crawler: crawl aborted for type '${definition.id}' after ${totalItems} item(s)`
          );
          return;
        }
      ...
      this.logger.error(
        `Context Engine crawler: failed to list items for type '${definition.id}': ${
    ...
    this.logger.debug(
      `Context Engine crawler: enumerated ${totalItems} item(s) for type '${definition.id}': ${newCount} new, ${updatedCount} updated, ${unchangedCount} unchanged`
    );
    ...
    if (deletedCount > 0) {
      this.logger.info(
        `Context Engine crawler: marked ${deletedCount} item(s) as deleted for type '${definition.id}'`
      );
    }

    if (newCount === 0 && updatedCount === 0 && deletedCount === 0) {
      this.logger.debug(
        `Context Engine crawler: no state changes needed for type '${definition.id}'`
      );
    }
  ...
  private async applyMappingsOrRebuild({
    ...
    try {
      ...
      this.logger.warn(
        `Context Engine crawler: mapping update failed (${error.statusCode} ${errorType}) — dropping index '${contextEngineIndexName}' and re-crawling immediately`
      );
      try {
   ...
   * Returns true if state has docs but the Context Engine data index is empty.
   */
  private async checkDataIntegrity({
    ...
    esClient: ElasticsearchClient;
    stateClient: ReturnType<ContextEngineCrawlerStateStorage['getClient']>;
    attachmentType: string;
  }): Promise<boolean> {
    const stateCount = await this.countStateDocs({ stateClient, attachmentType });
    if (stateCount === 0) return false;

    const contextEngineDataCount = await this.countContextEngineDocuments({
      esClient,
      attachmentType,
    });
    this.logger.debug(
      `Context Engine crawler: integrity check for type '${attachmentType}' — ${stateCount} state doc(s), ${contextEngineDataCount} Context Engine data doc(s) in index '${contextEngineIndexName}'`
    );
    if (contextEngineDataCount === 0) {
      this.logger.warn(
        `Context Engine crawler: data integrity mismatch for type '${attachmentType}' — ` +
          `${stateCount} state doc(s) but 0 documents in Context Engine data index. ` +
  ...
  private async processPage({
    ...
    const stateMap = integrityResetNeeded
      ? new Map<string, ContextEngineCrawlerStateDocument>()
      : await this.batchLookupState({ stateClient, attachmentType, ids: page.map((i) => i.id) });

    let newItems = 0;
    let updatedItems = 0;
    let unchangedItems = 0;
    const operations: Array<{ id: string; document: ContextEngineCrawlerStateDocument }> = [];

    for (const item of page) {
      const existing = stateMap.get(item.id);
      let updateAction: ContextEngineCrawlerStateDocument['update_action'];
  ...
  private async processQueue({
    ...
    while (hasMore) {
      ...
      if (pendingItems.length === 0) {
        if (processedCount === 0) {
          this.logger.debug(
            `Context Engine crawler: no pending actions to process for type '${attachmentType}'`
          );
        }
        break;
      }
      ...
      this.logger.debug(
        `Context Engine crawler: processing batch of ${pendingItems.length} pending action(s) for type '${attachmentType}'`
      );

      const stateAckOps: Array<
        | { index: { _id: string; document: ContextEngineCrawlerStateDocument } }
        | { delete: { _id: string } }
      > = [];

      const validHits = pendingItems.filter((hit) => {
        if (!hit._id) {
          this.logger.warn('Context Engine crawler: skipping hit without _id');
          return false;
        }
        const doc = hit._source;
        if (!doc || !doc.update_action) {
          this.logger.debug(
            `Context Engine crawler: skipping hit '${hit._id}' — no source or update_action`
          );
          return false;
        }
        return true;
      ...
      const indexPromises = validHits.map((hit) => {
        ...
        if (action !== 'delete' && manualOriginUris.has(`${attachmentType}://${doc.origin_id}`)) {
          this.logger.debug(
            `Context Engine crawler: skipping '${action}' for origin '${doc.origin_id}' (type: ${doc.type_id}) — manual entry exists`
          );
          stateAckOps.push({
            index: {
              _id: hit._id!,
        ...
        return limit(async () => {
          try {
            this.logger.debug(
              `Context Engine crawler: processing '${action}' for origin '${doc.origin_id}' (type: ${doc.type_id})`
            );
            ...
            errorCount++;
            this.logger.error(
              `Context Engine crawler: failed to process action '${action}' for '${
                doc.origin_id
              }': ${(error as Error).message}`
            );
          }
      ...
      if (stateAckOps.length > 0) {
        try {
          ...
          this.logger.error(
            `Context Engine crawler: failed to ACK state updates for type '${attachmentType}': ${
              (error as Error).message
            }`
          );
        }
    ...
    if (processedCount > 0 || errorCount > 0) {
      this.logger.info(
        `Context Engine crawler: queue processing complete for type '${attachmentType}': ${processedCount} succeeded, ${errorCount} failed`
      );
    }
  }

  /**
   * Return the subset of `originIds` that already have at least one entry with
   * `ingestion_method: 'manual'` in the Context Engine data index. Used to skip dispatching
  ...
  private async findManualOriginUris({
    ...
    try {
      const response = await esClient.search<{ origin?: { uri?: string } }>({
        ...
        // manual entries exist for the same origin.
      ...
      this.logger.warn(
        `Context Engine crawler: failed to look up manual origin ids: ${(error as Error).message}`
      );
    }
  ...
  /**
   * Count how many documents exist in the Context Engine data index for a given attachment type.
   * Returns 0 if the index doesn't exist yet.
   */
  private async countContextEngineDocuments({
    esClient,
    attachmentType,
  }: {
    esClient: ElasticsearchClient;
  ...
  private async batchLookupState({
    ...
    attachmentType: string;
    ids: string[];
  }): Promise<Map<string, ContextEngineCrawlerStateDocument>> {
    const result = new Map<string, ContextEngineCrawlerStateDocument>();
    ...
    try {
      ...
      this.logger.error(
        `Context Engine crawler: failed to batch-lookup state for type '${attachmentType}': ${
          (error as Error).message
        }`
      );
      throw error;
  ...
  private async writeStateOperations({
    ...
    stateClient: ReturnType<ContextEngineCrawlerStateStorage['getClient']>;
    operations: Array<{ id: string; document: ContextEngineCrawlerStateDocument }>;
    attachmentType: string;
  }): Promise<void> {
    try {
      const bulkResponse = await stateClient.bulk({
      ...
      this.logger.error(
        `Context Engine crawler: failed to update state for type '${attachmentType}': ${
          (error as Error).message
        }`
      );
      throw error;
