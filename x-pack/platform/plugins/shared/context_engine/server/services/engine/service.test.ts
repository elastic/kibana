...
import { ContextEngineSearchFilterType } from '../../../common/http_api/context_engine';
import { createContextEngineService, isNotFoundError } from './service';
import {
  ContextEngineResultWindowExceededError,
  ContextEngineAuthzEnumerationIncompleteError,
  ContextEngineCorpusTooLargeError,
} from './errors';
import { contextEngineIndexName } from './storage';
...
jest.mock('./storage', () => {
  ...
  return {
    ...actual,
    createContextEngineStorage: jest.fn(),
  };
});
...
// Column order produced by buildContextEngineEsqlQuery. The permission name fields
...
describe('createContextEngineService', () => {
  describe('lifecycle', () => {
    it('setup() returns registerType', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('Registered Context Engine type: dashboard');
    });

    it('start() returns the ContextEngineService with registered types accessible', () => {
      const service = createContextEngineService();
      const logger = createMockLogger();
      const setup = service.setup({ logger });
      ...
      const contextEngineService = service.start({ logger });

      expect(contextEngineService.search).toBeDefined();
      expect(contextEngineService.autocomplete).toBeDefined();
      expect(contextEngineService.checkItemsAccess).toBeDefined();
      expect(contextEngineService.getDocuments).toBeDefined();
      expect(contextEngineService.findByOrigin).toBeDefined();
      expect(contextEngineService.findByOriginAcrossSpaces).toBeDefined();
      expect(contextEngineService.indexAttachment).toBeDefined();
      expect(contextEngineService.deleteAttachment).toBeDefined();
      expect(contextEngineService.getTypeDefinition).toBeDefined();
      expect(contextEngineService.listTypeDefinitions).toBeDefined();
      expect(contextEngineService.getCrawler).toBeDefined();
      expect(contextEngineService.getCrawler()).toBeDefined();
      expect(contextEngineService.getTypeDefinition('dashboard')).toBe(def);
      expect(contextEngineService.listTypeDefinitions()).toContain(def);
    });
  });
});
...
describe('ContextEngineService', () => {
  ...
  describe('search', () => {
    it('issues an ES|QL FORK+FUSE hybrid query with MV_CONTAINS space filter', async () => {
      ...
      await contextEngineService.search({
        query: 'foo bar',
        size: 10,
        spaceId: 'default',
    ...
    it('threads constraints and agent filters as WHERE clauses with positional params', async () => {
      ...
      await contextEngineService.search({
        ...
        esClient: scopedClient,
        request,
        constraints: { [ContextEngineSearchFilterType.connector]: { ids: ['gh-1'] } },
    ...
    it('returns baseline fields only when no fields param is provided', async () => {
      ...
      const result = await contextEngineService.search({
        query: 'viz',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    it('returns empty results when index does not exist (404)', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        'Context Engine index does not exist yet — returning empty results'
      );
    });
    ...
    it('throws on non-404 errors', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('Context Engine search failed: Connection refused');
    });
    ...
    it('pushes an MV_CONTAINS authz filter into the query when securityAuthz is present', async () => {
      ...
      const service = createContextEngineService();
      service.setup({ logger });
      const contextEngineService = service.start({ logger, securityAuthz });
  ...
  describe('autocomplete', () => {
    it('builds a single nested discovery_labels query (with inner_hits) and a space filter', async () => {
      ...
      await contextEngineService.autocomplete({
        query: 'git',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    it('threads per-type constraints through buildConstraintsFilter into the ES filter clauses', async () => {
      ...
      await contextEngineService.autocomplete({
        ...
        esClient: scopedClient,
        request,
        constraints: { [ContextEngineSearchFilterType.connector]: { ids: ['gh-1', 'jira-1'] } },
      });
    ...
    it('maps inner_hits onto matched_discovery_labels', async () => {
      ...
      const result = await contextEngineService.autocomplete({
        query: 'git',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    describe('pre-aggregation authz filter (MV_CONTAINS subset)', () => {
      ...
      it('pushes the authorized ES index subset as an MV_CONTAINS filter', async () => {
        ...
        const contextEngineService = startWithAuthz(securityAuthz, {
      ...
      it('fails closed when _terms_enum returns complete=false', async () => {
        ...
        ).rejects.toBeInstanceOf(ContextEngineAuthzEnumerationIncompleteError);
        // The search query is never issued when authz enumeration is incomplete.
        expect(esqlQueryMock).not.toHaveBeenCalled();
      });

      it('fails closed (ContextEngineCorpusTooLargeError) when distinct values exceed the ceiling', async () => {
        const securityAuthz = createMockSecurityAuthz(['saved_object:lens/get']);
        ...
        ).rejects.toBeInstanceOf(ContextEngineCorpusTooLargeError);
      });
  ...
  describe('checkItemsAccess', () => {
    it('grants all access when securityAuthz is absent', async () => {
      ...
      const result = await contextEngineService.checkItemsAccess({
        ids: ['item-1', 'item-2'],
        spaceId: 'default',
        esClient: scopedClient,
        request,
    ...
    it('calls ES search with correct query for checkItemsAccess', async () => {
      ...
      await contextEngineService.checkItemsAccess({
        ids: ['id-1'],
        spaceId: 'my-space',
        esClient: scopedClient,
        request,
  ...
  describe('getDocuments', () => {
    it('fetches documents from ES and returns Map', async () => {
      ...
      const result = await contextEngineService.getDocuments({
        ids: ['doc-1', 'doc-2'],
        spaceId: 'default',
        esClient: scopedClient,
      });
    ...
    it('handles other errors gracefully', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        'Context Engine getDocuments failed: Connection timeout'
      );
    });
    ...
    it('calls ES search with correct query', async () => {
      ...
      await contextEngineService.getDocuments({
        ids: ['id-1', 'id-2'],
        spaceId: 'my-space',
        esClient: scopedClient,
      });
  ...
  describe('listDocuments', () => {
    ...
    it('passes pagination params to ES with sorted by updated_at desc', async () => {
      ...
      await contextEngineService.listDocuments({
        spaceId: 'default',
        esClient: scopedClient,
        page: 3,
        perPage: 25,
    ...
    it('returns empty results when index does not exist', async () => {
      ...
      const result = await contextEngineService.listDocuments({
        spaceId: 'default',
        esClient: scopedClient,
      });
    ...
    it('throws on non-404 errors', async () => {
      ...
      await expect(
        contextEngineService.listDocuments({
          spaceId: 'default',
          esClient: scopedClient,
        })
      ).rejects.toThrow('Connection refused');
      expect(logger.warn).toHaveBeenCalledWith(
        'Context Engine listDocuments failed: Connection refused'
      );
    });

    it('throws ContextEngineResultWindowExceededError when ES rejects with result-window error', async () => {
      const service = createContextEngineService();
      service.setup({ logger });
      const contextEngineService = service.start({ logger });
      ...
      ).rejects.toBeInstanceOf(ContextEngineResultWindowExceededError);
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(
        expect.stringContaining('Context Engine listDocuments failed')
      );
    });
    ...
    it('does not translate unrelated 400 ES errors', async () => {
      ...
      ).rejects.not.toBeInstanceOf(ContextEngineResultWindowExceededError);
    });
  });
  ...
  describe('findByOrigin', () => {
    it('returns every entry matching (type, originId) that is visible in the caller space', async () => {
      ...
      const result = await contextEngineService.findByOrigin({
        type: 'visualization',
        originId: 'ref-1',
        spaceId: 'default',
        esClient: scopedClient,
    ...
    it('throws on non-404 errors and logs', async () => {
      ...
      await expect(
        contextEngineService.findByOrigin({
          type: 'visualization',
          originId: 'ref-1',
          spaceId: 'default',
          esClient: scopedClient,
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Context Engine findByOrigin failed')
      );
    });
    ...
    it('logs a warning when total entries exceed MAX_ENTRIES_PER_ORIGIN', async () => {
      ...
      await contextEngineService.findByOrigin({
        type: 'visualization',
        originId: 'overfull',
        spaceId: 'default',
        esClient: scopedClient,
  ...
  describe('findByOriginAcrossSpaces', () => {
    it('returns matching entries regardless of which space they are in', async () => {
      ...
      const result = await contextEngineService.findByOriginAcrossSpaces({
        type: 'visualization',
        originId: 'ref-1',
        esClient: scopedClient,
      });
    ...
    it('throws on non-404 errors and logs', async () => {
      ...
      await expect(
        contextEngineService.findByOriginAcrossSpaces({
          type: 'visualization',
          originId: 'ref-1',
          esClient: scopedClient,
        })
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Context Engine findByOriginAcrossSpaces failed')
      );
    });
    ...
    it('only fetches the guard-relevant fields via _source filter', async () => {
      ...
      await contextEngineService.findByOriginAcrossSpaces({
        type: 'visualization',
        originId: 'ref-1',
        esClient: scopedClient,
      });
    ...
    it('throws ContextEngineCorpusTooLargeError (fail-closed) when total entries exceed MAX_ENTRIES_PER_ORIGIN', async () => {
      ...
      ).rejects.toThrow(expect.objectContaining({ name: 'ContextEngineCorpusTooLargeError' }));
