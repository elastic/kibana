...
import { buildAgentFilters, buildConstraintsFilter } from './service';

// buildConstraintsFilter is designed to combine clauses from multiple
// ContextEngineSearchFilterType values. When the enum grows beyond 'connector', add
...
describe('buildConstraintsFilter', () => {
  ...
  it('returns undefined when type criteria has no ids', () => {
    expect(
      buildConstraintsFilter({ [ContextEngineSearchFilterType.connector]: {} })
    ).toBeUndefined();
  });

  it('excludes the type entirely when ids is empty', () => {
    const filter = buildConstraintsFilter({
      [ContextEngineSearchFilterType.connector]: { ids: [] },
    });
  ...
  it('returns a filter for a single type with ids', () => {
    const filter = buildConstraintsFilter({
      [ContextEngineSearchFilterType.connector]: { ids: ['conn-1', 'conn-2'] },
    });
    expect(filter).toEqual({
      bool: {
        should: [
