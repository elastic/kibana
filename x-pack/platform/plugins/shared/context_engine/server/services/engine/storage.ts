...
import type { ContextEngineDocument } from './types';

export const contextEngineIndexName = chatSystemIndex('sml-data');
...
/**
 * Single source of truth for Context Engine data index field mappings (storage + Elasticsearch).
 *
 ...
 */
const contextEngineStorageSchemaProperties = {
  id: types.keyword({}),
  type: types.keyword({}),
...
export const storageSettings = {
  name: contextEngineIndexName,
  schema: {
    properties: contextEngineStorageSchemaProperties,
  },
} satisfies IndexStorageSettings;

/**
 * Elasticsearch `mappings` block for the Context Engine data index (e.g. integration tests, tooling).
 * Field definitions match `contextEngineStorageSchemaProperties` / `storageSettings`.
 */
export const contextEngineElasticsearchIndexMappings = {
  dynamic: 'strict' as const,
  properties: contextEngineStorageSchemaProperties,
};

export type ContextEngineStorageSettings = typeof storageSettings;

export type ContextEngineStorage = StorageIndexAdapter<
  ContextEngineStorageSettings,
  ContextEngineDocument
>;

export const createContextEngineStorage = ({
  logger,
  esClient,
}: {
  logger: Logger;
  ...
  return new StorageIndexAdapter<ContextEngineStorageSettings, ContextEngineDocument>(
    esClient,
    logger,
    storageSettings
  );
