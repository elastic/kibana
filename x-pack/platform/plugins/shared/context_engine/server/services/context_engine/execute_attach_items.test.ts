...
import type { ContextEngineDocument, ContextEngineService } from './types';
import { resolveAttachItems } from './execute_attach_items';
...
const createContextEngineService = (): ContextEngineService =>
  ({
...
const baseParams = {
  contextEngine: createContextEngineService(),
...
describe('resolveAttachItems', () => {
  ...
  it('returns access denied when checkItemsAccess denies the entry', async () => {
    mockCheckItemsAccess.mockResolvedValue(new Map([['entry-1', false]]));
    mockGetDocuments.mockResolvedValue(new Map());
    const results = await resolveAttachItems({
      ...baseParams,
      entryIds: ['entry-1'],
    });
  ...
  it('returns a generic text attachment when getTypeDefinition is undefined (unregistered Context Engine type)', async () => {
    const contextEngineDoc = createContextEngineDoc({
      type: 'orphan-type',
      title: 'Ad-hoc note',
    ...
    mockCheckItemsAccess.mockResolvedValue(new Map([['entry-1', true]]));
    mockGetDocuments.mockResolvedValue(new Map([['entry-1', contextEngineDoc]]));
  ...
  it('uses toAttachment description when provided', async () => {
    const contextEngineDoc = createContextEngineDoc({ origin_id: 'so-1', origin: { uri: 'so-1' } });
    mockCheckItemsAccess.mockResolvedValue(new Map([['entry-1', true]]));
    mockGetDocuments.mockResolvedValue(new Map([['entry-1', contextEngineDoc]]));
    mockGetTypeDefinition.mockReturnValue({
      id: 'visualization',
  ...
  it('falls back to contextEngineDoc type/title when toAttachment omits description', async () => {
    const contextEngineDoc = createContextEngineDoc({
      type: 'connector',
      title: 'My Drive',
      origin_id: 'so-1',
  ...
  it('uses contextEngineDoc.origin.uri when converted attachment has no origin', async () => {
    const contextEngineDoc = createContextEngineDoc({
  ...
  it('returns failure and logs when toAttachment throws', async () => {
    ...
    expect(results[0].success).toBe(false);
    if (!results[0].success) {
      expect(results[0].message).toContain("Failed to convert Context Engine item 'entry-1'");
    }
  ...
  it('processes multiple entry ids independently', async () => {
    const docOk = createContextEngineDoc({
      id: 'entry-ok',
      origin_id: 'r-ok',
      origin: { uri: 'r-ok' },
    });
