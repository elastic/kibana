...
import { createContextEngineCrawlerStateStorage } from './crawler_state_storage';
import { createContextEngineStorage } from './storage';
import { ContextEngineCrawlerImpl } from './crawler';
import type { ContextEngineTypeDefinition, ContextEngineListItem } from './types';

jest.mock('./crawler_state_storage', () => {
  const client = {
    search: jest.fn(),
  ...
  return {
    contextEngineCrawlerStateIndexName: '.test-sml-crawler-state',
    createContextEngineCrawlerStateStorage: jest.fn().mockReturnValue({
...
const getMockContextEngineClient = () => mockContextEngineClient;

const getMockStateClient = () =>
  (createContextEngineCrawlerStateStorage as jest.Mock)({ logger: {}, esClient: {} }).getClient();

async function* yieldPages(
  ...pages: ContextEngineListItem[][]
): AsyncIterable<ContextEngineListItem[]> {
...
const createMockDefinition = (
  ...
  getContextEngineData: jest.fn().mockResolvedValue({ entries: [] }),
...
const createMockEsClient = (): jest.Mocked<ElasticsearchClient> => {
  ...
  return {
    ...
    // findManualOriginIds (in crawler.ts) calls search on the Context Engine data index.
...
describe('ContextEngineCrawlerImpl', () => {
  ...
  beforeEach(() => {
    ...
    mockContextEngineClient.existsIndex.mockResolvedValue(false);
    mockContextEngineClient.clean.mockResolvedValue({ acknowledged: true });
    mockUpdateMappingsIfNeeded.mockResolvedValue(undefined);
    (createContextEngineStorage as jest.Mock).mockReturnValue({
      getClient: jest.fn().mockReturnValue(getMockContextEngineClient()),
    });
  });
  ...
  describe('new items detected', () => {
    it('when list yields items not in state, writes state docs with update_action create', async () => {
      ...
      const crawler = new ContextEngineCrawlerImpl({ indexer: mockIndexer, logger });
  ...
  describe('processQueue', () => {
    ...
    it('skips hits without _id and logs warning', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('Context Engine crawler: skipping hit without _id');
  ...
  describe('data integrity check', () => {
    it('when state has items but countContextEngineDocuments returns 0, forces re-index of all items', async () => {
      const items = [{ id: 'a', updatedAt: '2024-01-01', spaces: ['default'] }];
      const definition = createMockDefinition({
        list: jest.fn().mockReturnValue(yieldPages(items)),
      });
      ...
      mockStateClient.search
        ...
        // sweepStaleState returns empty
        .mockResolvedValue({ hits: { hits: [] } });
      // Context Engine data index is empty
  ...
  describe('schema version check', () => {
    it('mapping update failure: drops index and forces full re-index', async () => {
      ...
      expect(mockContextEngineClient.clean).toHaveBeenCalledTimes(1);
    ...
    it('non-response error propagates immediately without retrying', async () => {
      ...
      expect(mockUpdateMappingsIfNeeded).toHaveBeenCalledTimes(1);
      expect(mockContextEngineClient.clean).not.toHaveBeenCalled();
    });
  ...
  describe('list() failure', () => {
    it('logs error and returns without processing', async () => {
      async function* failingList(): AsyncIterable<ContextEngineListItem[]> {
