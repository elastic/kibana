...
import { createContextEngineIndexer } from './indexer';
...
jest.mock('./storage', () => ({
  contextEngineIndexName: '.test-sml-data',
  createContextEngineStorage: jest.fn().mockReturnValue({
...
const createMockRegistry = (definition?: ContextEngineTypeDefinition) => ({
...
describe('createContextEngineIndexer', () => {
  beforeEach(() => {
    mockUuidCounter = 0;
  });

  describe('indexAttachment', () => {
    it('delete action: calls deleteByQuery and does NOT call getContextEngineData', async () => {
      const getContextEngineData = jest.fn();
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({ id: 'lens', getContextEngineData })
      );
      const logger = createMockLogger();
      const esClient = createMockEsClient();
      const indexer = createContextEngineIndexer({ registry, logger });
      ...
      expect(getContextEngineData).not.toHaveBeenCalled();
    });

    it('create action: calls getContextEngineData, deletes existing entries, bulk indexes new ones with permissions from getPermissions hook', async () => {
      const bulkMock = jest.fn().mockResolvedValue({ errors: false, items: [] });
      const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
      (createContextEngineStorage as jest.Mock).mockReturnValue({ getClient: getClientMock });
      ...
      const getContextEngineData = jest.fn().mockResolvedValue(contextEngineData);
      const getPermissions = jest.fn().mockReturnValue({
        kibana: { privileges: [{ name: 'perm1' }] },
        elasticsearch: { indices: [] },
      });
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({ id: 'lens', getContextEngineData, getPermissions })
      );
      const logger = createMockLogger();
      const esClient = createMockEsClient();
      ...
      expect(getContextEngineData).toHaveBeenCalledTimes(1);
      expect(getContextEngineData).toHaveBeenCalledWith('att-2', {
        esClient,
        savedObjectsClient: {},
        logger: contextLogger,
      });
    ...
    it('create action: round-trips all new schema fields (tags, discovery_labels, extended_attrs, references, description, user_id)', async () => {
      ...
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({
          id: 'dashboard',
    ...
    it('unknown type in origin mode: throws ContextEngineUnregisteredTypeError without touching ES', async () => {
      ...
      ).rejects.toBeInstanceOf(ContextEngineUnregisteredTypeError);
    ...
    it('getContextEngineData returns undefined: deletes existing entries and does not index', async () => {
      const bulkMock = jest.fn();
      const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
      (createContextEngineStorage as jest.Mock).mockReturnValue({ getClient: getClientMock });

      const getContextEngineData = jest.fn().mockResolvedValue(undefined);
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({ id: 'lens', getContextEngineData })
      );
      const logger = createMockLogger();
    ...
    it('getContextEngineData returns empty entries: deletes existing entries and does not index', async () => {
      const bulkMock = jest.fn();
      const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
      (createContextEngineStorage as jest.Mock).mockReturnValue({ getClient: getClientMock });

      const getContextEngineData = jest.fn().mockResolvedValue({ entries: [] });
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({ id: 'lens', getContextEngineData })
      );
      const logger = createMockLogger();
    ...
    it('deleteEntries handles 404 gracefully', async () => {
      ...
      const registry = createMockRegistry(
        createMockContextEngineTypeDefinition({ id: 'lens', getContextEngineData: jest.fn() })
      );
      const logger = createMockLogger();
      const indexer = createContextEngineIndexer({ registry, logger });
    ...
    it('bulk index throws on failure', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('failed to index Context Engine data')
      );
    });

    describe('manual-entry protection (origin mode)', () => {
      it('skips getContextEngineData and write when a manual entry already exists', async () => {
        const bulkMock = jest.fn();
        const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
        (createContextEngineStorage as jest.Mock).mockReturnValue({ getClient: getClientMock });
    ...
    describe('content mode (manual)', () => {
      ...
      it('content-mode getPermissions throw: propagates the throw and leaves existing entries intact', async () => {
        ...
        const registry = createMockRegistry(
          createMockContextEngineTypeDefinition({ id: 'lens', getPermissions })
        );
        const logger = createMockLogger();
        const esClient = createMockEsClient();
        const indexer = createContextEngineIndexer({ registry, logger });
      ...
      it('stamps empty permissions when content-mode type has no getPermissions hook', async () => {
        ...
        const registry = createMockRegistry(
          createMockContextEngineTypeDefinition({ id: 'lens' /* no getPermissions */ })
        );
        const logger = createMockLogger();
        const esClient = createMockEsClient();
        const indexer = createContextEngineIndexer({ registry, logger });
      ...
      it('content mode for an unregistered type writes entries with empty permissions and warns once per type', async () => {
        ...
        // ContextEngineTypeDefinition. The trade-off is intentionally surfaced:
        // (a) the entry has no permission gate (empty ContextEnginePermissions),
      ...
      it('content mode with empty entries deletes existing and writes nothing', async () => {
        ...
        const registry = createMockRegistry(createMockContextEngineTypeDefinition({ id: 'lens' }));
        const logger = createMockLogger();
        const esClient = createMockEsClient();
        const indexer = createContextEngineIndexer({ registry, logger });
      ...
      it.each<{
        ...
        expectedPermissions: ContextEnginePermissions;
      }>([
        {
        ...
        async ({ registered, requestedPermissions, expectedPermissions }) => {
          ...
          const registry = registered
            ? createMockRegistry(createMockContextEngineTypeDefinition({ id: 'corpus_entry' }))
      ...
      it('content mode: getPermissions hook exists + requestedPermissions supplied → throws ContextEnginePermissionsConflictError before any ES mutation', async () => {
        const bulkMock = jest.fn().mockResolvedValue({ errors: false, items: [] });
        const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
        (createContextEngineStorage as jest.Mock).mockReturnValue({ getClient: getClientMock });
        ...
        ).rejects.toBeInstanceOf(ContextEnginePermissionsConflictError);
    ...
    describe('getPermissions hook', () => {
      it('origin-mode: stamps fully-shaped empty permissions when type has no getPermissions hook', async () => {
        ...
        const registry = createMockRegistry(
          createMockContextEngineTypeDefinition({
            id: 'lens',
            getContextEngineData /* no getPermissions */,
          })
        );
        const logger = createMockLogger();
        const esClient = createMockEsClient();
