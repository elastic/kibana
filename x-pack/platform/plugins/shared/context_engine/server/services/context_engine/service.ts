...
import type {
  ...
  ContextEngineAutocompleteResult,
...
import { createContextEngineIndexer, type ContextEngineIndexer } from './indexer';
import { ContextEngineCrawlerImpl } from './crawler';
import type { ContextEngineCrawler } from './types';
...
//   through `ContextEngineIndexer` so type-registration enforcement, permission derivation, and storage
//   shape stay consistent. There are no document-write paths in this file.

export interface ContextEngineServiceSetup {
  /**
   * Register an Context Engine type definition.
   * Should be called during plugin setup.
   */
  registerType: (definition: ContextEngineTypeDefinition) => void;
}

export interface ContextEngineServiceStartDeps {
  logger: Logger;
  securityAuthz?: AuthorizationServiceSetup;
}

export interface ContextEngineServiceInstance {
  setup: (deps: { logger: Logger }) => ContextEngineServiceSetup;
  start: (deps: ContextEngineServiceStartDeps) => ContextEngineService;
}

export const createContextEngineService = (): ContextEngineServiceInstance => {
  return new ContextEngineServiceImpl();
};

class ContextEngineServiceImpl implements ContextEngineServiceInstance {
  private registry: ContextEngineTypeRegistry;
  private indexer?: ContextEngineIndexer;
  private crawler?: ContextEngineCrawler;
  private securityAuthz?: AuthorizationServiceSetup;

  constructor() {
    this.registry = createContextEngineTypeRegistry();
  }

  setup({ logger }: { logger: Logger }): ContextEngineServiceSetup {
    return {
      registerType: (definition: ContextEngineTypeDefinition) => {
        this.registry.register(definition);
        logger.info(`Registered Context Engine type: ${definition.id}`);
      },
    };
  }

  start({ logger, securityAuthz }: ContextEngineServiceStartDeps): ContextEngineService {
    this.securityAuthz = securityAuthz;
    if (!securityAuthz) {
      logger.warn(
        'Context Engine service started without security authorization — permission checks are disabled (open access)'
      );
    }
    this.indexer = createContextEngineIndexer({
      registry: this.registry,
      logger: logger.get('indexer'),
    });
    this.crawler = new ContextEngineCrawlerImpl({
  ...
  private getIndexer(): ContextEngineIndexer {
    if (!this.indexer) {
      throw new Error('Context Engine indexer not initialized — call start() first');
    }
    return this.indexer;
  }
}
 ...
 */
const emptyPermissions = (): ContextEngineDocument['permissions'] => ({
  kibana: { privileges: [] },
...
/**
 * Combined privilege check for Context Engine entries. In a single ES `_has_privileges`
 ...
 * patterns. Context Engine entries store concrete names by construction.
 *
...
const getAuthorizedPrivileges = async ({
  ...
  try {
    ...
    logger.warn(
      `Context Engine privilege check failed; failing closed: ${(error as Error).message}`
    );
    return { authorizedPerms: new Set(), authorizedIndices: new Set() };
  }
};
 ...
 * (see storage.ts) — the same paths the ES|QL authz filter references.
 */
...
/**
 * Enumerate every distinct value of a keyword permission field across the Context Engine
 ...
 * the Context Engine system index (we always read as the internal user).
 *
  ...
  try {
    for (let page = 0; page < maxPages; page++) {
      ...
      if (!response.complete) {
        ...
        throw new ContextEngineAuthzEnumerationIncompleteError(
    ...
    if (error instanceof ContextEngineAuthzEnumerationIncompleteError) {
      throw error;
    }
    if (isNotFoundError(error)) {
      logger.debug(`Context Engine index does not exist yet — '${field}' universe is empty`);
      return [];
    }
    throw error;
  }

  throw new ContextEngineCorpusTooLargeError(
 ...
 */
const filterPageByPermissions = async <T extends { permissions: ContextEnginePermissions }>(
  items: T[],
  {
    request,
    securityAuthz,
 ...
 */
const filterResultsByPermissions = async <T extends { permissions: ContextEnginePermissions }>({
  searchResult,
  request,
  securityAuthz,
  logger,
 ...
 *     `_has_privileges`.
 *
 * Entries without any kibana privileges and without any elasticsearch
...
const checkItemsAccess = async ({
  ...
  try {
    const response = await esClient.asInternalUser.search<
      Pick<ContextEngineDocument, 'id' | 'permissions'>
    >({
      index: contextEngineIndexName,
      size: ids.length,
    ...
    logger.warn(`Context Engine items access check failed: ${(error as Error).message}`);
    for (const id of ids) {
      accessMap.set(id, false);
    }
    return accessMap;
...
const CONTEXT_ENGINE_BM25_FIELDS = ['title', 'description', 'content'] as const;

/** Semantic multi-fields searched in the second FORK branch. */
const CONTEXT_ENGINE_SEMANTIC_FIELDS = [
  'title.semantic',
...
/**
 * Build an ES|QL query string + positional params array for the Context Engine search path.
 *
 ...
 */
const buildContextEngineEsqlQuery = ({
  query,
  size,
  fields,
  spaceId,
  ...
  const lines: string[] = [`FROM ${contextEngineIndexName} METADATA _id, _index, _score`];
  ...
  if (trimmed === '' || trimmed === '*') {
    ...
    lines.push('| FORK');
    const bm25Conditions = CONTEXT_ENGINE_BM25_FIELDS.map((field) => {
      params.push(trimmed);
      return `MATCH(${field}, ?)`;
    }).join(' OR ');
    lines.push(
    ...
    const semanticConditions = CONTEXT_ENGINE_SEMANTIC_FIELDS.map((field) => {
      params.push(trimmed);
      return `MATCH(${field}, ?)`;
    }).join(' OR ');
    lines.push(
...
export const buildConstraintsFilter = (
  constraints: ContextEngineSearchConstraints | undefined
...
export const buildAgentFilters = (
  filters: ContextEngineSearchFilters | undefined
...
/**
 * Returns true for ES|QL errors that indicate the Context Engine index does not exist yet.
...
/**
 * Search the Context Engine index using ES|QL FORK + FUSE hybrid retrieval, with
...
const searchCe = async ({
  ...
  logger.debug(
    `Context Engine search: query=${JSON.stringify(query)}, size=${size}, spaceId='${spaceId}'`
  );
  ...
  if (securityAuthz) {
    ...
    logger.debug(
      `Context Engine search authz: actions=${authz.authorizedActions.length}, indices=${authz.authorizedIndices.length}`
    );
  }

  const { esql, params } = buildContextEngineEsqlQuery({
    query,
    size,
    fields,
    spaceId,
  ...
  try {
    ...
    if (isNotFoundError(error) || isEsqlIndexMissingError(error)) {
      logger.debug('Context Engine index does not exist yet — returning empty results');
      return { results: [] };
    }
    logger.warn(`Context Engine search failed: ${(error as Error).message}`);
    throw error;
  }
  ...
  type ContextEngineSearchResultInternal = ContextEngineSearchResult & {
    permissions: ContextEnginePermissions;
  };

  const allResults: ContextEngineSearchResultInternal[] = response.values.map((row) => {
    const result: ContextEngineSearchResultInternal = {
  ...
  logger.debug(`Context Engine search: returned=${response.values.length}, size=${size}`);
 ...
 */
const buildContextEngineAutocompleteQuery = (query: string): Record<string, unknown> => {
  const trimmed = query.trim();
  if (trimmed === '' || trimmed === '*') {
    return { match_all: {} };
  }
...
const autocompleteCe = async ({
  ...
  logger.debug(
    `Context Engine autocomplete: query=${JSON.stringify(
      query
    )}, size=${size}, spaceId='${spaceId}', index='${contextEngineIndexName}'`
  );

  try {
    const contextEngineQuery = buildContextEngineAutocompleteQuery(query);
    ...
    const response = await esClient.asInternalUser.search<ContextEngineDocument>({
      index: contextEngineIndexName,
      size,
      allow_no_indices: true,
      ignore_unavailable: true,
    ...
    const results: ContextEngineAutocompleteResult[] = response.hits.hits
      .filter((hit) => hit._source != null)
      .map((hit) => {
        const source = hit._source!;
        const result: ContextEngineAutocompleteResult = {
          id: source.id ?? '',
          type: source.type ?? '',
          title: source.title ?? '',
    ...
    logger.debug(`Context Engine autocomplete: returned ${results.length} result(s)`);

    return { results };
  } catch (error) {
    if (isNotFoundError(error)) {
      logger.debug(
        'Context Engine index does not exist yet — returning empty autocomplete results'
      );
      return { results: [] };
    }
    logger.warn(`Context Engine autocomplete failed: ${(error as Error).message}`);
    throw error;
  }
};
...
const getDocumentsByIds = async ({
  ...
  const docMap = new Map<string, ContextEngineDocument>();
  ...
  try {
    ...
    if (!isNotFoundError(error)) {
      logger.warn(`Context Engine getDocuments failed: ${(error as Error).message}`);
    }
  }
...
/**
 * Compose the canonical `origin.uri` from the Context Engine `type` and bare
 ...
 * content mode and `getContextEngineData` in origin mode can both produce >1
 ...
 * because bare `originId` is not unique across Context Engine types (a lens id and
...
const findByOrigin = async ({
  ...
  try {
    ...
    if (total > MAX_ENTRIES_PER_ORIGIN) {
      logger.warn(
        `Context Engine findByOrigin: origin '${originUri}' has ${total} entries in space '${spaceId}' but only the first ${MAX_ENTRIES_PER_ORIGIN} are returned. Producer is likely misbehaving — investigate before the cross-space guard becomes unreliable.`
      );
    }
    ...
    logger.warn(`Context Engine findByOrigin failed: ${(error as Error).message}`);
    throw error;
  }
};
 ...
 * see it.
 */
const FIND_ACROSS_SPACES_SOURCE_FIELDS: ReadonlyArray<keyof ContextEngineDocument> = [
  'id',
  'type',
  'spaces',
  'origin',
 ...
 * "no Context Engine index yet" is a normal first-write state.
 *
 ...
 * {@link ContextEngineCorpusTooLargeError} (fail-closed). The limit is generous
 ...
 * 1 entry per origin).
 *
 * Returned `ContextEngineDocument`s carry only the fields in
...
const findByOriginAcrossSpaces = async ({
  ...
  try {
    ...
    if (total > MAX_ENTRIES_PER_ORIGIN) {
      throw new ContextEngineCorpusTooLargeError(
        `Context Engine origin '${originUri}' has ${total} entries, which exceeds the ${MAX_ENTRIES_PER_ORIGIN}-entry cross-space guard limit. The write is rejected to avoid acting on a partial cross-space view. Reduce the entry count for this origin before retrying.`
      );
    }
    ...
    // ContextEngineCorpusTooLargeError is an intentional fail-closed signal — let it
    // propagate without the generic "failed" warn, which would be redundant.
    if (!(error instanceof ContextEngineCorpusTooLargeError)) {
      logger.warn(`Context Engine findByOriginAcrossSpaces failed: ${(error as Error).message}`);
    }
    throw error;
  }
};
...
/**
 * Project an ES `_source` payload into the canonical `ContextEngineDocument`
 ...
 */
const hydrateDocument = (source: ContextEngineDocument): ContextEngineDocument => {
  const originUri = source.origin?.uri ?? '';
  const doc: ContextEngineDocument = {
    id: source.id ?? '',
    type: source.type ?? '',
    title: source.title ?? '',
...
const listDocuments = async ({
  ...
  try {
    ...
    const results: ContextEngineDocument[] = response.hits.hits
      .filter((hit) => hit._source != null)
      .map((hit) => hydrateDocument(hit._source!));
    ...
    if (isResultWindowExceededError(error)) {
      ...
      throw new ContextEngineResultWindowExceededError(reason);
    }
    logger.warn(`Context Engine listDocuments failed: ${(error as Error).message}`);
    throw error;
  }
};
