...
import { CeSearchFilterType } from '../../../common/http_api/ce';
import { createCeService, isNotFoundError } from './ce_service';
import {
...
jest.mock('./ce_storage', () => {
  const actual = jest.requireActual('./ce_storage');
  return {
    ...actual,
...
// Column order produced by buildCeEsqlQuery. The permission name fields
...
describe('createCeService', () => {
  describe('lifecycle', () => {
    it('setup() returns registerType', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('Registered CE type: dashboard');
    });

    it('start() returns the CeService with registered types accessible', () => {
      const service = createCeService();
      const logger = createMockLogger();
      const setup = service.setup({ logger });
      ...
      expect(ceService.search).toBeDefined();
      ...
      expect(ceService.getCrawler()).toBeDefined();
      expect(ceService.getTypeDefinition('dashboard')).toBe(def);
      expect(ceService.listTypeDefinitions()).toContain(def);
    });
  });
});
...
describe('CeService', () => {
  ...
  describe('search', () => {
    ...
    it('threads constraints and agent filters as WHERE clauses with positional params', async () => {
      ...
      await ceService.search({
        ...
        esClient: scopedClient,
        request,
        constraints: { [CeSearchFilterType.connector]: { ids: ['gh-1'] } },
    ...
    it('returns baseline fields only when no fields param is provided', async () => {
      ...
      const result = await ceService.search({
        query: 'viz',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    it('returns empty results when index does not exist (404)', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        'CE index does not exist yet — returning empty results'
      );
    });
    ...
    it('throws on non-404 errors', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('CE search failed: Connection refused');
    });
    ...
    it('pushes an MV_CONTAINS authz filter into the query when securityAuthz is present', async () => {
      ...
      const service = createCeService();
      service.setup({ logger });
      const ceService = service.start({ logger, securityAuthz });
  ...
  describe('autocomplete', () => {
    ...
    it('threads per-type constraints through buildConstraintsFilter into the ES filter clauses', async () => {
      ...
      await ceService.autocomplete({
        ...
        esClient: scopedClient,
        request,
        constraints: { [CeSearchFilterType.connector]: { ids: ['gh-1', 'jira-1'] } },
      });
    ...
    it('maps inner_hits onto matched_discovery_labels', async () => {
      ...
      const result = await ceService.autocomplete({
        query: 'git',
        size: 10,
        spaceId: 'default',
        esClient: scopedClient,
    ...
    describe('pre-aggregation authz filter (MV_CONTAINS subset)', () => {
      ...
      it('pushes the authorized ES index subset as an MV_CONTAINS filter', async () => {
        ...
        const ceService = startWithAuthz(securityAuthz, {
      ...
      it('fails closed when _terms_enum returns complete=false', async () => {
        ...
        ).rejects.toBeInstanceOf(CeAuthzEnumerationIncompleteError);
        // The search query is never issued when authz enumeration is incomplete.
        expect(esqlQueryMock).not.toHaveBeenCalled();
      });

      it('fails closed (CeCorpusTooLargeError) when distinct values exceed the ceiling', async () => {
        const securityAuthz = createMockSecurityAuthz(['saved_object:lens/get']);
        ...
        ).rejects.toBeInstanceOf(CeCorpusTooLargeError);
      });
  ...
  describe('checkItemsAccess', () => {
    it('grants all access when securityAuthz is absent', async () => {
      ...
      const result = await ceService.checkItemsAccess({
        ids: ['item-1', 'item-2'],
        spaceId: 'default',
        esClient: scopedClient,
        request,
    ...
    it('calls ES search with correct query for checkItemsAccess', async () => {
      ...
      await ceService.checkItemsAccess({
        ids: ['id-1'],
        spaceId: 'my-space',
        esClient: scopedClient,
        request,
  ...
  describe('getDocuments', () => {
    it('fetches documents from ES and returns Map', async () => {
      ...
      const result = await ceService.getDocuments({
        ids: ['doc-1', 'doc-2'],
        spaceId: 'default',
        esClient: scopedClient,
      });
    ...
    it('handles other errors gracefully', async () => {
      ...
      expect(result.size).toBe(0);
      expect(logger.warn).toHaveBeenCalledWith('CE getDocuments failed: Connection timeout');
    });
  ...
  describe('listDocuments', () => {
    ...
    it('returns empty results when index does not exist', async () => {
      ...
      const result = await ceService.listDocuments({
        spaceId: 'default',
        esClient: scopedClient,
      });
    ...
    it('throws on non-404 errors', async () => {
      ...
      await expect(
        ceService.listDocuments({
          spaceId: 'default',
          esClient: scopedClient,
        })
      ).rejects.toThrow('Connection refused');
      expect(logger.warn).toHaveBeenCalledWith('CE listDocuments failed: Connection refused');
    });

    it('throws CeResultWindowExceededError when ES rejects with result-window error', async () => {
      const service = createCeService();
      service.setup({ logger });
      const ceService = service.start({ logger });
      ...
      ).rejects.toBeInstanceOf(CeResultWindowExceededError);
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(
        expect.stringContaining('CE listDocuments failed')
      );
    });
    ...
    it('does not translate unrelated 400 ES errors', async () => {
      ...
      ).rejects.not.toBeInstanceOf(CeResultWindowExceededError);
    });
  });
