...
export interface CeServiceStartDeps {
  logger: Logger;
  securityAuthz?: AuthorizationServiceSetup;
}

export interface CeServiceInstance {
  setup: (deps: { logger: Logger }) => CeServiceSetup;
  start: (deps: CeServiceStartDeps) => CeService;
}

export const createCeService = (): CeServiceInstance => {
  return new CeServiceImpl();
};

class CeServiceImpl implements CeServiceInstance {
  private registry: CeTypeRegistry;
  private indexer?: CeIndexer;
  private crawler?: CeCrawler;
  private securityAuthz?: AuthorizationServiceSetup;

  constructor() {
    this.registry = createCeTypeRegistry();
  }

  setup({ logger }: { logger: Logger }): CeServiceSetup {
    return {
      registerType: (definition: CeTypeDefinition) => {
        this.registry.register(definition);
        logger.info(`Registered CE type: ${definition.id}`);
      },
    };
  }

  start({ logger, securityAuthz }: CeServiceStartDeps): CeService {
    this.securityAuthz = securityAuthz;
    if (!securityAuthz) {
      logger.warn(
        'CE service started without security authorization — permission checks are disabled (open access)'
      );
    }
    this.indexer = createCeIndexer({ registry: this.registry, logger: logger.get('indexer') });
    this.crawler = new CeCrawlerImpl({
  ...
  private getIndexer(): CeIndexer {
    if (!this.indexer) {
      throw new Error('CE indexer not initialized — call start() first');
    }
    return this.indexer;
  }
}
...
/**
 * Combined privilege check for CE entries. In a single ES `_has_privileges`
 ...
 * patterns. CE entries store concrete names by construction.
 *
...
const getAuthorizedPrivileges = async ({
  ...
  try {
    ...
    logger.warn(`CE privilege check failed; failing closed: ${(error as Error).message}`);
    return { authorizedPerms: new Set(), authorizedIndices: new Set() };
  }
};
 ...
 * (see ce_storage.ts) — the same paths the ES|QL authz filter references.
 */
...
/**
 * Enumerate every distinct value of a keyword permission field across the CE
 ...
 * the CE system index (we always read as the internal user).
 *
  ...
  try {
    for (let page = 0; page < maxPages; page++) {
      ...
      if (!response.complete) {
        ...
        throw new CeAuthzEnumerationIncompleteError(
    ...
    if (error instanceof CeAuthzEnumerationIncompleteError) {
      throw error;
    }
    if (isNotFoundError(error)) {
      logger.debug(`CE index does not exist yet — '${field}' universe is empty`);
      return [];
    }
    throw error;
  }
 ...
 */
const filterPageByPermissions = async <T extends { permissions: CePermissions }>(
  items: T[],
  {
    request,
    securityAuthz,
 ...
 */
const filterResultsByPermissions = async <T extends { permissions: CePermissions }>({
  searchResult,
  request,
  securityAuthz,
  logger,
...
const checkItemsAccess = async ({
  ...
  try {
    const response = await esClient.asInternalUser.search<Pick<CeDocument, 'id' | 'permissions'>>({
      index: ceIndexName,
      size: ids.length,
    ...
    logger.warn(`CE items access check failed: ${(error as Error).message}`);
    for (const id of ids) {
      accessMap.set(id, false);
    }
    return accessMap;
...
const CE_BM25_FIELDS = ['title', 'description', 'content'] as const;

/** Semantic multi-fields searched in the second FORK branch. */
const CE_SEMANTIC_FIELDS = ['title.semantic', 'description.semantic', 'content.semantic'] as const;

/**
 * Build an ES|QL query string + positional params array for the CE search path.
 *
...
const buildCeEsqlQuery = ({
  ...
  const lines: string[] = [`FROM ${ceIndexName} METADATA _id, _index, _score`];
  ...
  if (trimmed === '' || trimmed === '*') {
    ...
    lines.push('| FORK');
    const bm25Conditions = CE_BM25_FIELDS.map((field) => {
      params.push(trimmed);
      return `MATCH(${field}, ?)`;
    }).join(' OR ');
    lines.push(
    ...
    const semanticConditions = CE_SEMANTIC_FIELDS.map((field) => {
      params.push(trimmed);
      return `MATCH(${field}, ?)`;
    }).join(' OR ');
    lines.push(
...
export const buildConstraintsFilter = (
  constraints: CeSearchConstraints | undefined
...
export const buildAgentFilters = (
  filters: CeSearchFilters | undefined
...
/**
 * Returns true for ES|QL errors that indicate the CE index does not exist yet.
...
/**
 * Search the CE index using ES|QL FORK + FUSE hybrid retrieval, with
...
const searchCe = async ({
  ...
  logger.debug(`CE search: query=${JSON.stringify(query)}, size=${size}, spaceId='${spaceId}'`);
  ...
  if (securityAuthz) {
    ...
    logger.debug(
      `CE search authz: actions=${authz.authorizedActions.length}, indices=${authz.authorizedIndices.length}`
    );
  }

  const { esql, params } = buildCeEsqlQuery({
    query,
    size,
    fields,
    spaceId,
  ...
  try {
    ...
    if (isNotFoundError(error) || isEsqlIndexMissingError(error)) {
      logger.debug('CE index does not exist yet — returning empty results');
      return { results: [] };
    }
    logger.warn(`CE search failed: ${(error as Error).message}`);
    throw error;
  }
  ...
  type CeSearchResultInternal = CeSearchResult & { permissions: CePermissions };

  const allResults: CeSearchResultInternal[] = response.values.map((row) => {
    const result: CeSearchResultInternal = {
  ...
  logger.debug(`CE search: returned=${response.values.length}, size=${size}`);
 ...
 */
const buildCeAutocompleteQuery = (query: string): Record<string, unknown> => {
  const trimmed = query.trim();
  if (trimmed === '' || trimmed === '*') {
    return { match_all: {} };
  }
...
/**
 * Autocomplete the CE index. Prefix-only, with per-row provenance for the @ menu.
 */
const autocompleteCe = async ({
  ...
  logger.debug(
    `CE autocomplete: query=${JSON.stringify(
      query
    )}, size=${size}, spaceId='${spaceId}', index='${ceIndexName}'`
  );

  try {
    const ceQuery = buildCeAutocompleteQuery(query);
    ...
    const results: CeAutocompleteResult[] = response.hits.hits
      .filter((hit) => hit._source != null)
      .map((hit) => {
        const source = hit._source!;
        const result: CeAutocompleteResult = {
          id: source.id ?? '',
          type: source.type ?? '',
          title: source.title ?? '',
    ...
    logger.debug(`CE autocomplete: returned ${results.length} result(s)`);

    return { results };
  } catch (error) {
    if (isNotFoundError(error)) {
      logger.debug('CE index does not exist yet — returning empty autocomplete results');
      return { results: [] };
    }
    logger.warn(`CE autocomplete failed: ${(error as Error).message}`);
    throw error;
  }
};
...
const getDocumentsByIds = async ({
  ...
  const docMap = new Map<string, CeDocument>();
  ...
  try {
    ...
    if (!isNotFoundError(error)) {
      logger.warn(`CE getDocuments failed: ${(error as Error).message}`);
    }
  }
