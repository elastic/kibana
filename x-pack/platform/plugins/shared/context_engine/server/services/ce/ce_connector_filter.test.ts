...
import { buildAgentFilters, buildConstraintsFilter } from './ce_service';

// buildConstraintsFilter is designed to combine clauses from multiple
// CeSearchFilterType values. When the enum grows beyond 'connector', add
...
describe('buildConstraintsFilter', () => {
  ...
  it('returns undefined when type criteria has no ids', () => {
    expect(buildConstraintsFilter({ [CeSearchFilterType.connector]: {} })).toBeUndefined();
  });

  it('excludes the type entirely when ids is empty', () => {
    const filter = buildConstraintsFilter({ [CeSearchFilterType.connector]: { ids: [] } });
  ...
  it('returns a filter for a single type with ids', () => {
    const filter = buildConstraintsFilter({
      [CeSearchFilterType.connector]: { ids: ['conn-1', 'conn-2'] },
    });
    expect(filter).toEqual({
      bool: {
        should: [
