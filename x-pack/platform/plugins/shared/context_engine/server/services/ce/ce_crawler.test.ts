...
import { createCeCrawlerStateStorage } from './ce_crawler_state_storage';
import { createCeStorage } from './ce_storage';
import { CeCrawlerImpl } from './ce_crawler';
import type { CeTypeDefinition, CeListItem } from './types';

jest.mock('./ce_crawler_state_storage', () => {
  const client = {
    search: jest.fn(),
  ...
  return {
    ceCrawlerStateIndexName: '.test-sml-crawler-state',
    createCeCrawlerStateStorage: jest.fn().mockReturnValue({
...
const getMockStateClient = () =>
  (createCeCrawlerStateStorage as jest.Mock)({ logger: {}, esClient: {} }).getClient();

async function* yieldPages(...pages: CeListItem[][]): AsyncIterable<CeListItem[]> {
...
const createMockDefinition = (overrides: Partial<CeTypeDefinition> = {}): CeTypeDefinition => ({
  ...
  getCeData: jest.fn().mockResolvedValue({ entries: [] }),
...
const createMockEsClient = (): jest.Mocked<ElasticsearchClient> => {
  ...
  return {
    ...
    // findManualOriginIds (in ce_crawler.ts) calls search on the CE data index.
...
describe('CeCrawlerImpl', () => {
  ...
  describe('processQueue', () => {
    ...
    it('skips hits without _id and logs warning', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('CE crawler: skipping hit without _id');
  ...
  describe('data integrity check', () => {
    it('when state has items but countCeDocuments returns 0, forces re-index of all items', async () => {
      const items = [{ id: 'a', updatedAt: '2024-01-01', spaces: ['default'] }];
      const definition = createMockDefinition({
        list: jest.fn().mockReturnValue(yieldPages(items)),
      });
  ...
  describe('list() failure', () => {
    it('logs error and returns without processing', async () => {
      async function* failingList(): AsyncIterable<CeListItem[]> {
