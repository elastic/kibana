...
import { createCeStorage, ceIndexName, type CeStorage } from './ce_storage';
...
export class CeCrawlerImpl implements CeCrawler {
  ...
  private async applyMappingsOrRebuild({ storage }: { storage: CeStorage }): Promise<boolean> {
    try {
      ...
      this.logger.warn(
        `CE crawler: mapping update failed (${error.statusCode} ${errorType}) — dropping index '${ceIndexName}' and re-crawling immediately`
      );
      try {
