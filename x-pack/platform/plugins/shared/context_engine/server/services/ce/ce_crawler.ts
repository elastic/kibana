...
import type { CeIndexer } from './ce_indexer';
import {
  createCeCrawlerStateStorage,
  type CeCrawlerStateStorage,
} from './ce_crawler_state_storage';
import { createCeStorage, ceIndexName, storageSettings } from './ce_storage';
import { isNotFoundError } from './ce_service';

export type { CeCrawler };

export interface CeCrawlerDeps {
  indexer: CeIndexer;
  logger: Logger;
}

export class CeCrawlerImpl implements CeCrawler {
  private readonly indexer: CeIndexer;
  private readonly logger: Logger;

  constructor({ indexer, logger }: CeCrawlerDeps) {
  ...
  async crawl({
    ...
    const crawlerStateStorage = createCeCrawlerStateStorage({
      logger: this.logger,
      esClient,
    });
    ...
    this.logger.debug(`CE crawler: starting crawl for type '${definition.id}' across all spaces`);

    const indexRecreated = await this.recreateIndexIfMappingsChanged({ esClient });

    // Data integrity check: if the CE data index is empty for this type but
    ...
    try {
      for await (const page of definition.list(context)) {
        if (abortSignal?.aborted) {
          this.logger.info(
            `CE crawler: crawl aborted for type '${definition.id}' after ${totalItems} item(s)`
          );
          return;
        }
      ...
      this.logger.error(
        `CE crawler: failed to list items for type '${definition.id}': ${(error as Error).message}`
      );
      return;
    }

    this.logger.debug(
      `CE crawler: enumerated ${totalItems} item(s) for type '${definition.id}': ${newCount} new, ${updatedCount} updated, ${unchangedCount} unchanged`
    );
    ...
    if (deletedCount > 0) {
      this.logger.info(
        `CE crawler: marked ${deletedCount} item(s) as deleted for type '${definition.id}'`
      );
    }

    if (newCount === 0 && updatedCount === 0 && deletedCount === 0) {
      this.logger.debug(`CE crawler: no state changes needed for type '${definition.id}'`);
    }
   ...
   * Returns true if state has docs but the CE data index is empty.
   */
  private async checkDataIntegrity({
    ...
    esClient: ElasticsearchClient;
    stateClient: ReturnType<CeCrawlerStateStorage['getClient']>;
    attachmentType: string;
  }): Promise<boolean> {
    const stateCount = await this.countStateDocs({ stateClient, attachmentType });
    if (stateCount === 0) return false;

    const ceDataCount = await this.countCeDocuments({ esClient, attachmentType });
    this.logger.debug(
      `CE crawler: integrity check for type '${attachmentType}' — ${stateCount} state doc(s), ${ceDataCount} CE data doc(s) in index '${ceIndexName}'`
    );
    if (ceDataCount === 0) {
      this.logger.warn(
        `CE crawler: data integrity mismatch for type '${attachmentType}' — ` +
          `${stateCount} state doc(s) but 0 documents in CE data index. ` +
  ...
  private async recreateIndexIfMappingsChanged({
    ...
    const indexExists = await ceClient.existsIndex();
    if (!indexExists) {
      return false;
    }
    ...
    try {
      ...
      const response = await esClient.indices.getMapping({ index: ceIndexName });
      ...
      if (existingVersion !== expectedVersion) {
        this.logger.warn(
          `CE crawler: schema version mismatch (existing='${existingVersion}', expected='${expectedVersion}') — deleting index '${ceIndexName}' and recreating with current schema`
        );
        await ceClient.clean();
        return true;
      }
    } catch (error) {
  ...
  private async processPage({
    ...
    const stateMap = integrityResetNeeded
      ? new Map<string, CeCrawlerStateDocument>()
      : await this.batchLookupState({ stateClient, attachmentType, ids: page.map((i) => i.id) });

    let newItems = 0;
    let updatedItems = 0;
    let unchangedItems = 0;
    const operations: Array<{ id: string; document: CeCrawlerStateDocument }> = [];

    for (const item of page) {
      const existing = stateMap.get(item.id);
      let updateAction: CeCrawlerStateDocument['update_action'];
  ...
  private async processQueue({
    ...
    while (hasMore) {
      ...
      if (pendingItems.length === 0) {
        if (processedCount === 0) {
          this.logger.debug(
            `CE crawler: no pending actions to process for type '${attachmentType}'`
          );
        }
        break;
      }
      ...
      this.logger.debug(
        `CE crawler: processing batch of ${pendingItems.length} pending action(s) for type '${attachmentType}'`
      );

      const stateAckOps: Array<
        { index: { _id: string; document: CeCrawlerStateDocument } } | { delete: { _id: string } }
      > = [];

      const validHits = pendingItems.filter((hit) => {
        if (!hit._id) {
          this.logger.warn('CE crawler: skipping hit without _id');
          return false;
        }
        const doc = hit._source;
        if (!doc || !doc.update_action) {
          this.logger.debug(`CE crawler: skipping hit '${hit._id}' — no source or update_action`);
          return false;
        }
        return true;
      });
      ...
      const indexPromises = validHits.map((hit) => {
        ...
        if (action !== 'delete' && manualOriginUris.has(`${attachmentType}://${doc.origin_id}`)) {
          this.logger.debug(
            `CE crawler: skipping '${action}' for origin '${doc.origin_id}' (type: ${doc.type_id}) — manual entry exists`
          );
          stateAckOps.push({
            index: {
              _id: hit._id!,
        ...
        return limit(async () => {
          try {
            this.logger.debug(
              `CE crawler: processing '${action}' for origin '${doc.origin_id}' (type: ${doc.type_id})`
            );
            ...
            errorCount++;
            this.logger.error(
              `CE crawler: failed to process action '${action}' for '${doc.origin_id}': ${
      ...
      if (stateAckOps.length > 0) {
        try {
          ...
          this.logger.error(
            `CE crawler: failed to ACK state updates for type '${attachmentType}': ${
              (error as Error).message
            }`
          );
        }
    ...
    if (processedCount > 0 || errorCount > 0) {
      this.logger.info(
        `CE crawler: queue processing complete for type '${attachmentType}': ${processedCount} succeeded, ${errorCount} failed`
      );
    }
  }
   ...
   * `ingestion_method: 'manual'` in the CE data index. Used to skip dispatching
  ...
  private async findManualOriginUris({
    ...
    try {
      ...
      this.logger.warn(
        `CE crawler: failed to look up manual origin ids: ${(error as Error).message}`
      );
    }
  ...
  /**
   * Count how many documents exist in the CE data index for a given attachment type.
   * Returns 0 if the index doesn't exist yet.
   */
  private async countCeDocuments({
    esClient,
    attachmentType,
  }: {
    esClient: ElasticsearchClient;
  ...
  private async batchLookupState({
    ...
    attachmentType: string;
    ids: string[];
  }): Promise<Map<string, CeCrawlerStateDocument>> {
    const result = new Map<string, CeCrawlerStateDocument>();
    ...
    try {
      ...
      this.logger.error(
        `CE crawler: failed to batch-lookup state for type '${attachmentType}': ${
          (error as Error).message
        }`
      );
      throw error;
  ...
  private async writeStateOperations({
    ...
    stateClient: ReturnType<CeCrawlerStateStorage['getClient']>;
    operations: Array<{ id: string; document: CeCrawlerStateDocument }>;
    attachmentType: string;
  }): Promise<void> {
    try {
      const bulkResponse = await stateClient.bulk({
      ...
      this.logger.error(
        `CE crawler: failed to update state for type '${attachmentType}': ${
          (error as Error).message
        }`
      );
      throw error;
