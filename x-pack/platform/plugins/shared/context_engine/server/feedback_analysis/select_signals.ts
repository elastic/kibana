...
export interface SelectSignalsOptions {
  ...
  /** Space of the AI index being analyzed; only this space's signals are read. */
  spaceId: string;
...
export interface SelectSignalsResult {
  ...
  /** The space the signals came from: the AI index's own, or none when the window held none. */
  spaces: string[];
...
export const selectSignals = async (
  ...
  { destValue, sources, spaceId, signalTimeRange, signalFilter, now }: SelectSignalsOptions
  ...
  const index = buildSignalsIndexName(spaceId);
  ...
  return {
    ...
    // The search reads one space's index, so anything it matched came from that space.
    spaces: signalCount > 0 ? [spaceId] : [],
    signalCount,
    window,
  };
};
