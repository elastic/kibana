...
/**
 * Neutral CE type for ad-hoc / eval corpus documents written directly via the
 ...
 *
 * There is no crawler integration: `list` yields nothing and `getCeData`
 ...
 * skips `getCeData`). Unlike solution-owned types (dashboard, significant
 ...
 * stored CE document. So `toAttachment` builds the attachment directly from
 * the indexed `title` / `content` / `description`.
 */
export const corpusEntryCeType: CeTypeDefinition = {
  id: CORPUS_ENTRY_CE_TYPE,

  async *list() {},

  getCeData: async () => undefined,
