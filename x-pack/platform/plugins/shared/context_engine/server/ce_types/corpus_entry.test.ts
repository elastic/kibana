...
import type { CeContext, CeDocument, CeToAttachmentContext } from '../services/ce/types';
import { corpusEntryCeType, CORPUS_ENTRY_CE_TYPE } from './corpus_entry';

const mockCeContext = {} as CeContext;
const mockAttachmentContext = {} as CeToAttachmentContext;

const baseItem: CeDocument = {
  id: 'item-1',
...
describe('corpusEntryCeType', () => {
  describe('id', () => {
    it('is corpus_entry', () => {
      expect(corpusEntryCeType.id).toBe('corpus_entry');
    });
  });
  ...
  describe('list', () => {
    it('yields nothing', async () => {
      const items: unknown[] = [];
      for await (const item of corpusEntryCeType.list(mockCeContext)) {
        items.push(item);
      }
  ...
  describe('getCeData', () => {
    it('returns undefined', async () => {
      await expect(corpusEntryCeType.getCeData('origin-1', mockCeContext)).resolves.toBeUndefined();
    });
  });

  describe('toAttachment', () => {
    it('returns undefined when all text fields are empty', async () => {
      const result = await corpusEntryCeType.toAttachment!(
