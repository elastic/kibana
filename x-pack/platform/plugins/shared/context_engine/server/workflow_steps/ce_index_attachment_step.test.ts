...
import { CeIndexAttachmentInputSchema } from '../../common/workflow_steps/ce_index_attachment_step';
...
import { createCeIndexer } from '../services/ce/ce_indexer';
import { createCeTypeRegistry } from '../services/ce/ce_type_registry';
import { kibanaSavedObjectPermissions } from '../services/ce/permissions/kibana_saved_object';
import type { CeTypeDefinition } from '../services/ce/types';
// Imported as a namespace so we can spy on `createCeStorage` without
// adding a `__mock__` shim. `jest.spyOn(target, method)` needs a
// concrete object reference, which is what the `* as` form gives us.
import * as ceStorage from '../services/ce/ce_storage';
...
type StepInput = z.infer<typeof CeIndexAttachmentInputSchema>;
...
describe('createContextEngineAddEntryStepDefinition', () => {
  ...
  it('forwards caller-supplied entries as content-mode write for upsert', async () => {
    ...
    expect(startContract.indexAttachment).toHaveBeenCalledWith({
      ...
      action: 'create',
      // Workflow writes always go through content-mode → manual entries.
  ...
  it('strips optional entry fields when not provided', async () => {
    const startContract = buildStartContract();
    ...
    const context = buildHandlerContext({
      ...
      attachmentType: 'custom',
      action: 'upsert',
      entries: [{ type: 'custom', title: 't', content: 'c' }],
    });
  ...
  it('preserves optional entry fields when provided (no permissions field — derived by indexer)', async () => {
    const startContract = buildStartContract();
    const definition = buildDefinition({ getStartContract: () => startContract });
    ...
    // The entry forwarded to the start contract never carries `permissions`.
    // The indexer stamps them from `CeTypeDefinition.getPermissions`, so
    // the workflow step cannot spoof them either via the input schema or
    // via the forwarded entry.
  ...
  it('forwards a caller-supplied permissions object to indexAttachment for upsert', async () => {
    ...
    const context = buildHandlerContext({
      ...
      action: 'upsert',
      entries: [{ type: 'corpus_entry', title: 't', content: 'c' }],
      permissions: {
  ...
  it('surfaces CePermissionsConflictError from the start contract as a step error result', async () => {
    const startContract = buildStartContract();
  ...
  it('handles delete by calling deleteAttachment with ingestionMethod="all" and reports requestedEntryCount = 0', async () => {
    const startContract = buildStartContract();
    const definition = buildDefinition({ getStartContract: () => startContract });
    ...
    expect(startContract.deleteAttachment).toHaveBeenCalledWith({
      ...
      // and remove every entry for the origin (manual + crawled).
      ingestionMethod: 'all',
    });
  ...
  it('allows delete to proceed when the attachment type is not registered', async () => {
    ...
    // type is disabled — otherwise stale entries become unreachable from
  ...
  it('upserts under an unregistered attachment type by handing the call to the indexer', async () => {
    ...
    const context = buildHandlerContext({
      ...
      action: 'upsert',
      entries: [{ type: 'my_notes', title: 't', content: 'c' }],
    });
  ...
  describe('contextEngine:write privilege check', () => {
    ...
    it('rejects upsert with an error and never calls indexAttachment when the caller lacks the privilege', async () => {
      ...
      expect(result.output).toBeUndefined();
      expect(result.error).toBeInstanceOf(Error);
      expect((result.error as Error).message).toContain(apiPrivileges.writeContextEngine);
  ...
  describe('end-to-end: real indexer derives permissions from getPermissions', () => {
    // Wires a real `CeIndexer` (with a registry holding the given type)
    ...
    const buildEndToEndHarness = (registeredType: CeTypeDefinition) => {
      ...
      const realIndexer = createCeIndexer({ registry, logger });
    ...
    it('stamps `saved_object:lens/get` on content-mode entries for a visualization-like type', async () => {
      ...
      const context = buildHandlerContext({
        ...
        attachmentType: 'lens',
        action: 'upsert',
        entries: [{ type: 'lens', title: 'My Viz', content: 'content' }],
      });
    ...
    it('end-to-end: no-hook type + workflow-supplied permissions are stamped on the indexed document', async () => {
      ...
      const context = buildHandlerContext({
        ...
        attachmentType: 'corpus_entry',
        action: 'upsert',
        entries: [{ type: 'corpus_entry', title: 'My KI', content: 'content' }],
        permissions: {
  ...
  describe('input-schema permission spoofing guard', () => {
    // The entry schema is `strict()` so an unknown `permissions` field is
    ...
    it('rejects a entry that carries a permissions field (schema is strict)', () => {
      const parsed = CeIndexAttachmentInputSchema.safeParse({
        originId: 'doc-1',
        attachmentType: 'custom',
        action: 'upsert',
        entries: [
