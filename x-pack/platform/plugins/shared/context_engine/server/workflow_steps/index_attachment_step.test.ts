...
import { ContextEngineIndexAttachmentInputSchema } from '../../common/workflow_steps/index_attachment_step';
...
type StepInput = z.infer<typeof ContextEngineIndexAttachmentInputSchema>;
...
describe('createContextEngineAddEntryStepDefinition', () => {
  ...
  it('preserves optional entry fields when provided (no permissions field — derived by indexer)', async () => {
    ...
    // The indexer stamps them from `ContextEngineTypeDefinition.getPermissions`, so
  ...
  it('surfaces ContextEnginePermissionsConflictError from the start contract as a step error result', async () => {
    const startContract = buildStartContract();
  ...
  it('handles delete by calling deleteAttachment with ingestionMethod="all" and reports requestedEntryCount = 0', async () => {
    ...
    expect(startContract.deleteAttachment).toHaveBeenCalledWith({
      ...
      // 'all' tells the Context Engine indexer to skip the ingestion_method filter
  ...
  describe('end-to-end: real indexer derives permissions from getPermissions', () => {
    // Wires a real `ContextEngineIndexer` (with a registry holding the given type)
    ...
    const buildEndToEndHarness = (registeredType: ContextEngineTypeDefinition) => {
      const bulkMock = jest.fn().mockResolvedValue({ errors: false, items: [] });
      const getClientMock = jest.fn().mockReturnValue({ bulk: bulkMock });
      jest
        .spyOn(contextEngineStorage, 'createContextEngineStorage')
        .mockReturnValue({ getClient: getClientMock } as any);

      const registry = createContextEngineTypeRegistry();
      ...
      const realIndexer = createContextEngineIndexer({ registry, logger });
