...
/**
 * Factory for the Context Engine "index attachment" workflow step.
 *
 * Workflow writes always go through the **content-mode** path on the Context Engine
 ...
 * the Context Engine contract, distinct from `indexAttachment({ action: 'delete' })`)
 ...
 *    gates every HTTP route via `withContextEngineFeatureFlag` and the Context Engine crawler
 ...
 *    rest of the Context Engine surface so a deployment with the flag off cannot
 *    silently mutate the Context Engine index via a workflow.
 ...
 *    plugin → open access" convention used by the Context Engine read path.
 *
 ...
 * privileges without registering a Context Engine type. `delete` calls
 * `deleteAttachment` directly so cleanup keeps working even after the
 * registering plugin is disabled.
 *
 * The handler defers resolving the Context Engine start contract until execution
...
export const createContextEngineAddEntryStepDefinition = ({
   ...
   * via `withContextEngineFeatureFlag` and to the Context Engine crawler task. The check is
    ...
    handler: async (context) => {
      try {
        ...
        if (action === 'delete') {
          ...
          const entries: ContextEngineEntry[] = input.entries.map((entry) => ({
            type: entry.type,
            title: entry.title,
          ...
          // arrays to match the fully-shaped `ContextEnginePermissions` the contract expects.
