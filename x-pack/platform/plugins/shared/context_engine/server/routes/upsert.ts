...
import type { CeDocumentInput, CeService } from '../services/ce/types';
  ...
  router.put(
    ...
    withCeFeatureFlag(async (ctx, request, response) => {
      try {
        ...
        const result = await ce.upsertDocument({
          id,
          spaceId,
          document: request.body as CeDocumentInput,
          esClient,
        });
        if (!result) {
        ...
        const body: CeUpsertHttpResponse = {
          item: toCeHttpItem(result.document),
