...
describe('registerGetRoute', () => {
  ...
  it('returns 404 when feature flag is disabled', async () => {
    ...
    expect(mockCeService.getDocuments).not.toHaveBeenCalled();
  });

  it('returns 404 when the document is not found', async () => {
    mockCeService.getDocuments.mockResolvedValue(new Map());
  ...
  it('returns 404 when access check denies the item', async () => {
    mockCeService.getDocuments.mockResolvedValue(new Map([['entry-1', sampleDocument]]));
  ...
  it('returns 200 with the document when found and authorized', async () => {
    ...
    const response = await callHandler({ id: 'entry-1' });
    expect(mockCeService.getDocuments).toHaveBeenCalledWith({
      ids: ['entry-1'],
      spaceId: 'test-space',
  ...
  it('propagates errors from ce.getDocuments', async () => {
    mockCeService.getDocuments.mockRejectedValue(new Error('ES connection failed'));
    await expect(callHandler({ id: 'entry-1' })).rejects.toThrow('ES connection failed');
