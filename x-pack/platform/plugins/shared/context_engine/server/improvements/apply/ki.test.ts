...
describe('addKi', () => {
  ...
  it('keeps attributes the agent provided', async () => {
    ...
    await addKi({
      esClient,
      dest: plainIndex,
      ki: { type: 'document', title: 'Refund window', attributes: { source: 'feedback_loop' } },
      now: NOW,
    });
  ...
  it('omits fields the agent left unset rather than writing undefined', async () => {
    ...
    await addKi({
      esClient,
      dest: plainIndex,
      ki: { type: 'document', title: 'Refund window', description: undefined, tags: undefined },
      now: NOW,
    });

    expect(esClient.index).toHaveBeenCalledWith(
      expect.objectContaining({
        document: { '@timestamp': NOW, type: 'document', title: 'Refund window' },
      })
    );
  });
  ...
  it('refuses to write to an index pattern', async () => {
    ...
    await expect(
      addKi({
        esClient,
        dest: indexPattern,
        ki: { type: 'document', title: 'Refund window' },
        now: NOW,
      })
