...
export function registerMCPRoutes({ router, getInternalServices, logger }: RouteDependencies) {
  ...
  router.versioned
    .post({
      ...
      summary: 'MCP server',
      description:
        'WARNING: This endpoint is designed for MCP clients (such as Claude Desktop, Cursor, VS Code extensions, etc.) and should not be used directly via REST APIs. Use MCP Inspector or native MCP clients to interact with this endpoint. Communicate with the MCP server via JSON-RPC 2.0. MCP is designed for AI clients to access your Elastic tools. This endpoint requires JSON-RPC formatting and will not work from the Dev Tools Console.',
      options: {
