...
import { OnechatEsqlParam } from '../../types';

const PARAM_REGEX = /\?(\w+)/g;
const PARAM_VALIDATOR = 'esql-param-validator';
const ADD_PARAM_COMMAND = 'esql.addParam';
...
const highlightMissingParams = ({
  ...
  const paramMatches = code.matchAll(PARAM_REGEX);
  ...
  monaco.editor.setModelMarkers(model, PARAM_VALIDATOR, markers);
};

const setupSyntaxHighlighter = ({
  model,
  params,
}: {
  model: monaco.editor.ITextModel;
  ...
  return {
    dispose: () => {
      monaco.editor.setModelMarkers(model, PARAM_VALIDATOR, []);
...
export const useEsqlEditorParams = (
  params: OnechatEsqlParam[] | undefined,
