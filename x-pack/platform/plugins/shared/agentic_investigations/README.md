...
Impact has no privilege of its own yet. Reads and writes require the investigations sub-feature privilege, `manage_investigations`, which `includeIn: 'all'` joins to the base All level. A dedicated impact privilege can be split out later if read and write need to diverge. Escalations are a separate sub-feature with `includeIn: 'none'`, so `all` and `read` do not grant them. Follow the sub-feature pattern for any new entity that is not intrinsic to an investigation.
