...
export const listProposalsQuerySchema = z.object({
  ...
  /** Drops proposals that have been replaced by a newer attempt. */
  excludeSuperseded: z.stringbool().default(false),
  /** Filters to proposals in the given action category. */
  category: proposalCategorySchema.optional(),
  /**
   * When set, filters to proposals whose `decidedAt` is within the last N hours.
   * Used by the closed-proposals endpoint (72 h) to exclude stale decisions.
   */
