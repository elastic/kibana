...
export const escalationQueryKeys = {
  escalations: {
    ...
    list: (status?: string, page?: number, perPage?: number, search?: string) =>
      [
        ...escalationQueryKeys.escalations.all,
        'list',
        status ?? 'open',
        page ?? 1,
        perPage ?? null,
  ...
  userProfiles: (uids: readonly string[]) =>
    [...escalationQueryKeys.escalations.all, 'profiles', uids] as const,
  suggestUsers: (term: string) => [...escalationQueryKeys.escalations.all, 'suggest', term] as const,
};
