...
describe('createImpactPrivilegesChecker', () => {
  ...
  it('should resolve when the principal holds manage_impact', async () => {
    const { security, checkPrivileges } = createSecurity(true);
    const { checker } = createChecker(security);
    ...
    expect(checkPrivileges).toHaveBeenCalledWith({
      kibana: [`api:${IMPACT_API_PRIVILEGE_MANAGE}`],
    });
  });

  it('should throw when the principal does not hold manage_impact', async () => {
    const { security } = createSecurity(false);
    const { checker } = createChecker(security);
