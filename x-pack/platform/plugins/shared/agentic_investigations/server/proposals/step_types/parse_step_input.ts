 ...
 * validation only.
 *
 * That matters most for `optionalStepInput`, whose entire job is to turn the
 * `''` that Liquid renders for an absent input back into an omission. Without
 * this call every optional input arrives as an empty string, so `??` defaults
 * never fire and `''` is stored in place of a value.
 */
