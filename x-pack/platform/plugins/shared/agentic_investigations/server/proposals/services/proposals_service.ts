...
export class ProposalsService {
  ...
  async create(
    ...
    // primary sort key is always populated even for a non-action proposal whose
   ...
   * Performs no writes at all: with nothing durable written here there is
   * nothing to roll back when a resume fails, and the decision reaches the
   * record by exactly one path regardless of which surface released the gate.
   *
  ...
  async update(params: UpdateProposalParams, spaceId: string): Promise<Proposal> {
    ...
    const updated: StoredProposalRecord = {
      ...
      status,
      decision,
      // Stamped here rather than accepted as a parameter, so the moment a
      // decision was recorded is always the moment it was written.
      ...(params.decision !== undefined
        ? { decidedBy: params.decidedBy ?? proposal.decidedBy, decidedAt: new Date().toISOString() }
        : {}),
