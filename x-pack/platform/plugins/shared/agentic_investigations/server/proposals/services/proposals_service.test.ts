...
describe('ProposalsService', () => {
  ...
  describe('list', () => {
    ...
    it('should filter by category when provided', async () => {
      ...
      await service.list(listQuery({ category: 'respond' }), SPACE_ID);

      const [[searchArgs]] = storage.search.mock.calls;
      expect(searchArgs.query.bool.filter).toEqual(
        expect.arrayContaining([{ term: { category: 'respond' } }])
      );
    });

    it('should not add a category filter when category is absent', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
      ...
      const [[searchArgs]] = storage.search.mock.calls;
      const filterStr = JSON.stringify(searchArgs.query.bool.filter);
      expect(filterStr).not.toContain('"category"');
    });

    it('should filter by decidedWithinHours when provided', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
    ...
    it('should not add a decidedAt filter when decidedWithinHours is absent', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
      ...
      const [[searchArgs]] = storage.search.mock.calls;
      const filterStr = JSON.stringify(searchArgs.query.bool.filter);
      expect(filterStr).not.toContain('decidedAt');
    });

    it('should add a must_not exists filter when excludeSuperseded is true', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
      ...
      const [[searchArgs]] = storage.search.mock.calls;
      const filterStr = JSON.stringify(searchArgs.query.bool.filter);
      expect(filterStr).toContain('"supersededBy"');
      expect(filterStr).toContain('"must_not"');
    });

    it('should not add a supersededBy filter when excludeSuperseded is false', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);

      await service.list(listQuery({ excludeSuperseded: false }), SPACE_ID);

      const [[searchArgs]] = storage.search.mock.calls;
      const filterStr = JSON.stringify(searchArgs.query.bool.filter);
      expect(filterStr).not.toContain('"supersededBy"');
    });

    it('should use the caller-supplied sort when provided', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
      const customSort = [{ createdAt: { order: 'desc' as const } }];

      await service.list(listQuery(), SPACE_ID, customSort);

      const [[searchArgs]] = storage.search.mock.calls;
      expect(searchArgs.sort).toEqual(customSort);
    });

    it('should default to impact-based sort when no sort override is given', async () => {
      const storage = createStorage(baseDocument());
      const { service } = createService(storage);
      ...
      const [[searchArgs]] = storage.search.mock.calls;
      expect(searchArgs.sort[0]).toEqual({ impactRank: { order: 'asc' } });
    });
  });
