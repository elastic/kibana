...
export type ResolveUser = (request: KibanaRequest) => Promise<User | undefined>;
 ...
 * or an API key whose creator has no activated profile. Falling back to `authc`
 * still yields a username, so attribution degrades rather than disappearing.
 */
export const createUserResolver =
  ({
    ...
    security,
    logger,
  }: Pick<CoreStart, 'userProfile' | 'security'> & { logger: Logger }): ResolveUser =>
  async (request) => {
    try {
