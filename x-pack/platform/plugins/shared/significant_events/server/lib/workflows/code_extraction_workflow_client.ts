...
interface CodeExtractionWorkflowInputPayload {
  /** Connector for the `scs.code_researcher` agent step. Omitted -> YAML default. */
 ...
 * uses an `ai.agent` step (`scs.code_researcher`) to enumerate the deployable
 * services across every SCS-indexed repository, then fans out to the per-service
...
export class SignificantEventsCodeExtractionClient {
  ...
  async run({ request, spaceId, inputs }: CodeExtractionRunParams): Promise<{
    executionId: string;
    isNew: boolean;
  }> {
