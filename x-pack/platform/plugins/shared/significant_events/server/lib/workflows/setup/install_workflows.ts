...
const WORKFLOWS_TO_INSTALL: Array<{
  ...
  // Run-quota enforcement — installed globally, and excluded from pause so the
  // control loop can still resume engines afterwards.
  ...ALWAYS_ON_WORKFLOW_IDS.map((workflowId) => ({
    workflowId,
    spaceId: GLOBAL_WORKFLOW_SPACE_ID,
  })),
