...
const WORKFLOWS_TO_INSTALL: Array<{
  ...
  // Installed disabled in the default space; enabled on demand by
  // CleanupWorkflowService.ensureEnabled from the extraction path, which
  // schedules its trigger. Restorable + `enabled: false` YAML => installed disabled.
  {
    workflowId: SIGNIFICANT_EVENTS_CLEANUP_WORKFLOW_ID,
    spaceId: DEFAULT_SPACE_ID,
  },
];
