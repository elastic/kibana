...
/** Memory synthesis stays owned by this plugin: it runs against knowledge indicators. */
export const MEMORY_WORKFLOW_IDS = [SIGNIFICANT_EVENTS_MEMORY_SYNTHESIS_WORKFLOW_ID] as const;

/**
 * Curation workflows owned by the agent_memory plugin.
 *
 * This plugin does not install them, but it does sweep them when Significant
 * Events is paused — the pause banner promises that memory activity stops too.
 * They are therefore in the maintenance sets and deliberately absent from
 * {@link ALL_INSTALLABLE_WORKFLOW_IDS}.
 */
export const AGENT_MEMORY_WORKFLOW_IDS = [
  AGENT_MEMORY_CONSOLIDATION_WORKFLOW_ID,
  AGENT_MEMORY_CONVERSATION_SCRAPER_WORKFLOW_ID,
  AGENT_MEMORY_GAP_DETECTION_WORKFLOW_ID,
] as const;
