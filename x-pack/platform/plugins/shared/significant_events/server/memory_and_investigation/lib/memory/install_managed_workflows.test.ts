...
const MEMORY_WORKFLOW_IDS = [SIGNIFICANT_EVENTS_MEMORY_SYNTHESIS_WORKFLOW_ID] as const;
...
describe('installMemoryWorkflows', () => {
  ...
  it('names the failed workflow when an install rejects', async () => {
    const client = createClientMock();
    client.install.mockRejectedValueOnce(new Error('synthesis boom'));
    ...
    await expect(
      ...
      `Failed to install memory workflows: [${SIGNIFICANT_EVENTS_MEMORY_SYNTHESIS_WORKFLOW_ID} (synthesis boom)]`
    );
