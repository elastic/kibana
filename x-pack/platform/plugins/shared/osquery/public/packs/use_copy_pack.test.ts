...
describe('useCopyPack', () => {
  ...
  it('calls the copy API and navigates to the new pack edit page on success', async () => {
    ...
    expect(mockHttp.post).toHaveBeenCalledWith('/api/osquery/packs/source-id/copy', {
      version: '2023-10-31',
    });

    expect(mockNavigateToApp).toHaveBeenCalledWith('osquery', {
      path: '/packs/new-pack-id/edit',
    });

    expect(mockToasts.addSuccess).toHaveBeenCalledWith('Pack duplicated successfully');
  });

  it('invalidates the packs query cache on success', async () => {
    mockHttp.post.mockResolvedValue({
      data: { saved_object_id: 'new-pack-id', name: 'my-pack_copy' },
    });
    ...
    expect(invalidateSpy).toHaveBeenCalledWith([PACKS_ID]);
  });

  it('shows an error toast on failure', async () => {
    const error = { body: { error: 'Not Found', message: 'Pack not found' } };
    mockHttp.post.mockRejectedValue(error);

    const { result } = renderHook(() => useCopyPack({ packId: 'bad-id' }), {
      wrapper: createWrapper(queryClient),
    });
