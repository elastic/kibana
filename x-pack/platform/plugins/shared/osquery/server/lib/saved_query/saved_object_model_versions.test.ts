...
describe('Pack saved object model version 3 forward compatibility', () => {
  ...
  it('accepts a synthetic future pack SO with fictitious future root fields', () => {
    const syntheticFutureDoc = {
      name: 'forward-compat-pack',
      description: 'pretends to come from a future migration',
      enabled: true,
      created_at: '2026-05-01T00:00:00.000Z',
      created_by: 'elastic',
      updated_at: '2026-05-01T00:00:00.000Z',
    ...
    expect(() => (forwardCompatibility as ObjectType).validate(syntheticFutureDoc)).not.toThrow();
  });

  // V3's unknowns:'allow' per-query schema passes schedule_id/start_date
  // through on a rollback read rather than stripping them.
  it('passes the V3-minted per-query `schedule_id`/`start_date` through on rollback read', () => {
    const v3StampedDoc = {
      name: 'test-pack-1',
    ...
    const out = (forwardCompatibility as ObjectType).validate(v3StampedDoc) as {
...
describe('Pack saved object model version 3 — schedule_id/start_date/id backfill', () => {
  ...
  const dataBackfillChange = packSavedObjectModelVersion3.changes.find(
  ...
  it('registers a single data_backfill change (no mappings_addition for schedule_id)', () => {
    expect(packSavedObjectModelVersion3.changes).toHaveLength(1);
    expect(packSavedObjectModelVersion3.changes[0].type).toBe('data_backfill');
  });
  ...
  // effectiveKey '0'. V3 would otherwise stamp two rows the same id/schedule_id.
  ...
  // An empty-string schedule_id is malformed, not a real durable identity. V3
  ...
  it('(d) backfills unconditionally (no flag input exists)', () => {
  ...
  it('(e) forward-compat — a rolled-back node reads a V3-migrated doc without throwing', () => {
    const forwardCompatibility = packSavedObjectModelVersion3.schemas?.forwardCompatibility;
    expect(forwardCompatibility).toBeDefined();
