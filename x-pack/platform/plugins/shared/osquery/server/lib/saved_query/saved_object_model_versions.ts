...
// Must never change once V3 has shipped (feeds the deterministic UUIDv5).
 ...
 * them. MUST be deterministic: data_backfill also runs on the read path (no
 * write-back), so `uuidv4()`/`new Date()` would drift across reads/nodes and
 * sever the schedule_id history join. Existing values are preserved as-is.
 */
