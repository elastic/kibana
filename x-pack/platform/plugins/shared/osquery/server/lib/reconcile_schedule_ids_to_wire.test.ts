...
import { packSavedObjectModelVersion3 } from './saved_query/saved_object_model_versions';

// Version-agnostic: V3 mints deterministic UUIDv5 schedule_ids, and any
// pre-existing value is preserved as-is, so match any valid UUID version.
...
describe('reconcileScheduleIdsToWire', () => {
  ...
  describe('Fleet wire fields on reconcile', () => {
    test('legacy interval pack — legacy per-query shape plus space_id and schedule_id', async () => {
      const scopedClient = createMockScopedClient();
      const packagePolicyUpdate = jest.fn().mockResolvedValue({});
      const packagePolicyList = mockFetchAllItems([
      ...
      expect(packBlock.queries.q1.schedule_id).toBe('sched-q1');
      expect(packBlock.queries.q1.space_id).toBe('default');
    });
  });

  // End-to-end: legacy SO → real V3 backfill → reconciler → Fleet wire.
  describe('integration — legacy SO → model version V3 → reconciler → Fleet wire', () => {
    const backfillFn = (
      packSavedObjectModelVersion3.changes.find(
    ...
    test('legacy queries gain schedule_id via V3, and the reconciler carries them to the wire', async () => {
      const legacyQueries = [
