...
export interface PackResultCounts extends ResultCounts {
  queriesWithResults: number;
  queriesTotal: number;
}

export type ResultCountsMap = Map<string, ResultCounts>;
...
export async function getResultCountsForActions(
...
async function fetchResultCountsBatch(
  esClient: ElasticsearchClient,
  actionIds: string[],
  namespace: string
): Promise<ResultCountsMap> {
...
export async function getPackResultCounts(
  esClient: ElasticsearchClient,
  queryActionIds: string[],
  namespace = 'default'
): Promise<PackResultCounts> {
  const countsMap = await getResultCountsForActions(esClient, queryActionIds, namespace);

  let totalRows = 0;
  let respondedAgents = 0;
  ...
  for (const counts of countsMap.values()) {
    ...
    respondedAgents += counts.respondedAgents;
  ...
  return {
    ...
    errorAgents,
    queriesWithResults,
    queriesTotal: queryActionIds.length,
  };
}
