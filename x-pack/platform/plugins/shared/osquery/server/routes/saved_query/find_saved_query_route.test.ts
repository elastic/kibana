...
describe('findSavedQueryRoute', () => {
  ...
  it('sanitizes special characters from search term', async () => {
    setupRoute();
    ...
    const findArgs = mockSavedObjectsClient.find.mock.calls[0][0];
    expect(findArgs.filter).toContain('osquery-saved-query.attributes.id: testscript*');
    expect(findArgs.filter).not.toContain('<');
    expect(findArgs.filter).not.toContain('>');
  });
