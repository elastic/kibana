...
apiTest.describe('Osquery saved queries - viewer', { tag: tags.deploymentAgnostic }, () => {
  ...
  apiTest.beforeAll(async ({ requestAuth, apiServices }) => {
    ...
    const response = await apiServices.osquery.savedQueries.create(testData.getMinimalSavedQuery());
    savedObjectId = (response.data as Record<string, Record<string, string>>).data.saved_object_id;
  });
