...
test.describe.skip('App Features for Endpoint Complete PLI', { tag: ['@svlSecurity'] }, () => {
  ...
  test('response actions should be available', async ({ page }) => {
    ...
    // TODO: Check that osquery response actions are available in Endpoint Complete tier
    // This would typically involve checking for the presence of response action UI elements
  });
});
