...
test.describe('Alert Event Details - Response Actions Form', { tag: ['@ess', '@svlSecurity'] }, () => {
  ...
  test('adds response actions with osquery with proper validation and form values', async ({
    ...
    await expect(page.getByText('Response actions are run on each rule execution.').first()).toBeVisible();
    ...
    await expect(responseAction2.getByText('Query is a required field').first()).not.toBeVisible();
    ...
    const osqueryColumnSelect = responseAction2.locator('[data-test-subj="osqueryColumnValueSelect"]');
    ...
    await expect(
      responseAction2.getByText('Custom key/value pairs. e.g. {"application":"foo-bar","env":"production"}')
    ).toBeVisible();
    ...
    await newResponseAction0.locator('[data-test-subj="comboBoxInput"]').pressSequentially(packName);
    await page.getByRole('option', { name: packName }).click();
    ...
    await expect(
      newResponseAction1.getByText('Custom key/value pairs. e.g. {"application":"foo-bar","env":"production"}')
    ).toBeVisible();
    ...
    await expect(finalResponseAction0.locator('[data-test-subj="comboBoxSearchInput"]')).toHaveValue(packName);
    ...
    await expect(
      finalResponseAction1.getByText('Custom key/value pairs. e.g. {"application":"foo-bar","env":"production"}')
    ).toBeVisible();
