...
test.describe(
  ...
  () => {
    ...
    test('should run query and enable ecs mapping', async ({ page, pageObjects }) => {
      ...
      await expect(
        page.testSubj.locator('osqueryResultsTable').locator('[data-test-subj="dataGridFullScreenButton"]')
      ).toBeVisible({ timeout: 30_000 });
      await expect(
        page.locator(
    ...
    test('should run customized saved query', async ({ page, pageObjects }) => {
      ...
      await expect(page.testSubj.locator('kibanaCodeEditor')).toContainText('select * from uptime', { timeout: 15_000 });
