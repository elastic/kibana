...
describe('Data Streams tab', () => {
  ...
  afterEach(() => {
    jest.useRealTimers();
    // Reset singleton services to ensure clean state between tests
    // This is critical for test isolation when tests run together
    httpService.setup(httpServiceMock.createSetupContract());
    // Don't reset modules - it can break test isolation by resetting module state
    // while leaving singleton services in inconsistent states
    // jest.resetModules();
  });
