...
// Security MKI projects enforce a project-level default retention (elastic/kibana#241105).
// That default only exists on real Cloud projects, so this suite is Cloud-only; the generic
// Get-shape assertions (shared across project types) live in data_streams.spec.ts instead
...
apiTest.describe(
  ...
  () => {
    ...
    apiTest.beforeAll(async ({ requestAuth, config }) => {
      ...
      apiTest.skip(
        !config.isCloud,
        'The Security MKI default retention is provisioned at the Cloud project level'
      );
      // skip() marks the tests as skipped in reports; it doesn't stop this hook from running, so
      // the early return avoids fetching credentials/stats that no test will use.
      if (!config.isCloud) {
        return;
      }
