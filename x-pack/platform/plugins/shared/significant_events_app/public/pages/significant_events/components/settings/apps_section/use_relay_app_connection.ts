...
export function useRelayAppConnection(): UseRelayAppConnection {
  ...
  const connectMutation = useMutation<SlackAppConnectResponse, Error>({
    ...
    onError: (error) => {
      notifications.toasts.addError(getFormattedError(error), {
        title: i18n.translate('xpack.significantEventsApp.settings.apps.connectError', {
  ...
  const disconnectMutation = useMutation<SlackAppDisconnectResponse, Error>({
    ...
    onError: (error) => {
      notifications.toasts.addError(getFormattedError(error), {
        title: i18n.translate('xpack.significantEventsApp.settings.apps.disconnectError', {
