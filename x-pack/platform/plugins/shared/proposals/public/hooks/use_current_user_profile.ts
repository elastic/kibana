...
//     a reverse-proxy hiccup. Caching it as `null` would permanently block the approval card's
//     "who's approving" label for the rest of the session. Let react-query retry on 401.
...
export const useCurrentUserProfile = () => {
  ...
  return useQuery<UserProfileWithAvatar | null>({
    queryKey: queryKeys.userProfiles.current(),
    queryFn: async () => {
      try {
