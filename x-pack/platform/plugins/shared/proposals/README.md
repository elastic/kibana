...
1. **Recompute the remaining time** from the proposal's fixed `expiresAt`, so a failed attempt never extends the deadline. One `data.set`: Liquid cannot read a variable written by the same step, so `now` is negated and added rather than stored and subtracted.
2. **Settle and break** if the deadline has passed (`expired`).
3. **Park on the gate**, under a step-level `on-failure: continue` so a timeout stays inside the loop instead of reaching the workflow-level handler — which is what makes an unanswered proposal end the run as `completed` with an output, instead of `failed`. The key is honoured by the engine but unmodelled by `WaitForApprovalStepSchema` — see "Known limitations" and [#19315](https://github.com/elastic/security-team/issues/19315).
...
5. **Check the resumer's privilege**, and `loop.continue` when denied. Nothing has been written at this point. Skipped unless someone actually answered: the autonomy path has no resumer, and neither does a gate that timed out.
6. **Adopt the chain's live head**, once. Everything below writes, a revision appended during the park would have superseded the carried id, and `updateProposal` refuses a superseded row. Placement is the whole invariant — after the privilege check so a denied resumer re-parks before this reads the chain, and before every branch that writes.
7. **Re-read the clock**, then settle `expired` in two cases: the gate ran out (`handle_gate_timeout`), or a decision arrived after the deadline (`settle_expired_after_gate`). The timeout branch comes first, because the gate's timeout *is* the time left on the deadline — so every timeout also satisfies the late-decision condition, and checking that first would record that a decision arrived when none did. Both come before the dismissal branch: a timed-out gate answers blank, which the dismissal branch would otherwise record as a decision nobody made. The late-decision case exists only because the HTTP routes are not the only way in — a resume through the platform resume API or the Inbox does not refuse an expired decision.
8. **Record the decision**, together with the status it implies — never on its own, because `approved` + `pending` is not a legal pair, and a decision-only write would leave the record claiming an approval with no outcome.
9. On an action failure, **clone** the proposal, adopt the new id, and loop; the clone inherits the deadline so a chain of retries cannot outlive it. Cloning a proposal that already carries `supersededBy` is refused, because overwriting the pointer would orphan the first clone.

The loop adopts in exactly two places — step 1 for the settle that runs before the gate, step 6 for everything that runs after it. Every step that writes is downstream of one of them.

Three invariants the loop depends on:

- **`max-iterations` does not settle the record.** A `while` is in `flowControlStepTypes`, which `handleWorkflowLevelOnFailure` skips, so the error `on-limit: fail` throws would reach no handler at all — hence `on-limit: continue`, which exits the loop and falls through to the output step. The deadline is what settles: every park is bounded by the time left on it, so the expiry check is reached on the first iteration after it passes. The iteration limit is a runaway guard and nothing more — see "Known limitations".
...
- **Three steps rely on an `on-failure` their schema does not model** ([#19315](https://github.com/elastic/security-team/issues/19315)). Neither `WaitForApprovalStepSchema`, `WorkflowExecuteStepSchema` nor `IfStepSchema` merges `StepWithOnFailureSchema`, unlike the connector-derived schema every custom step gets — so zod drops the key on any path that validates steps against the full schema. The engine honours it on all three: a HITL wait fails through the ordinary `failStep` path with a `TimeoutError`, and `handleStepLevelOnFailure` wraps any step declaring the key with no exclusion by type. All three are load-bearing here — the gate's settles an unanswered proposal, the action's keeps a failed action inside the loop to be cloned, and `settle_expired`'s re-resolves the live head when a revision wins the race against its write — and they reach the engine only because managed workflows install under `lightweightValidation`, which does not validate steps. The schema-parity test beside the definition pins exactly these three so a fourth cannot appear unnoticed, and so the list shrinks when #19315 lands.
- **An exhausted iteration budget leaves the record `pending`.** `max-iterations` is 200 with `on-limit: continue`, so hitting it exits the loop and reports `final_status: pending` rather than settling. The deadline settles the record on its own, so reaching the limit means something spun — a resumer who keeps failing the decide check, or a long chain of action retries. The proposal still reads as expired everywhere it is shown, since `expired` is computed from `expiresAt` on every read, but it stays in an open queue bucket because grouping keys off `status`. Settling it would need a sweep that runs with its own privileges, which the plugin does not have.
- **A post-adopt write can still lose a race.** Every write after the gate is preceded by one adopt, but a revision landing in the window between them makes the write conflict. Only the top-of-loop settle retries; the post-gate writes do not, so a conflict there reaches the workflow-level handler — which adopts and settles the live head rather than stranding it. Losing a decision to a millisecond race is bad but bounded, and one retry per write would undo the single-adopt simplification. If it ever matters, the fix is one retry around the whole post-adopt section, not one per write.
