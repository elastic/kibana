...
interface ConnectorContextEngineTypeDeps {
  /**
...
/**
 * Creates the Context Engine type definition for connectors.
 *
 * Connectors are indexed into the Context Engine exclusively via event-driven calls
 ...
 */
export const createConnectorContextEngineType = (
  deps: ConnectorContextEngineTypeDeps
  ...
  return {
    ...
    getContextEngineData: async (originId, context) => {
      try {
        ...
        context.logger.warn(
          `Context Engine connector: failed to get data for '${originId}': ${
    ...
    toAttachment: async (item, context) => {
      try {
        ...
        logger.warn(
          `Context Engine connector: failed to convert '${item.origin_id}' to attachment: ${
            (error as Error).message
          }`
        );
        return undefined;
