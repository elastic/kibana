...
import { visualizationContextEngineType } from './visualization';
...
describe('visualizationContextEngineType', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('id', () => {
    it('equals visualization', () => {
      expect(visualizationContextEngineType.id).toBe('visualization');
    });
  });
  ...
  describe('list', () => {
    it('calls createPointInTimeFinder with type lens, namespaces [*], fields [title]', async () => {
      ...
      await collectPages(visualizationContextEngineType.list(createContext() as never));
    ...
    it('returns items with id, updatedAt from so.updated_at, spaces from so.namespaces', async () => {
      ...
      const result = await collectPages(
        visualizationContextEngineType.list(createContext() as never)
      );
  ...
  describe('getContextEngineData', () => {
    it('returns entry with correct type, title, content (permissions are handled by getPermissions)', async () => {
      ...
      const result = await visualizationContextEngineType.getContextEngineData!(
        'viz-1',
  ...
  describe('getPermissions', () => {
    it('returns the saved_object:lens/get privilege (via kibanaSavedObjectPermissions helper)', () => {
      const permissions = visualizationContextEngineType.getPermissions!(
        'viz-1',
        createContext() as never
      );
  ...
  describe('toAttachment', () => {
    it('calls resolve with lens and item.origin_id', async () => {
      ...
      await visualizationContextEngineType.toAttachment!(
    ...
    it('returns visualization attachment with correct shape', async () => {
      ...
      const result = await visualizationContextEngineType.toAttachment!(
        { origin_id: 'viz-1' } as never,
        createContext() as never
      );
    ...
    it('propagates errors from resolve to the caller', async () => {
      ...
      await expect(
        visualizationContextEngineType.toAttachment!(
          { origin_id: 'viz-1' } as never,
          createContext() as never
        )
