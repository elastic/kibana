...
- \`attributes.user.id\` / \`attributes.user.hash\` / \`attributes.user.name\` — real user id and username when \`agentBuilder:tracing:includeUserData\` is on; otherwise a stable \`user.hash\` and no username.
