...
- \`attributes.user.id\` / \`attributes.user.hash\` — user id when \`agentBuilder:tracing:includeRealIds\` is on; otherwise a stable \`user.hash\`.
- \`attributes.user.name\` — username; omitted unless \`agentBuilder:tracing:includeRealNames\` is on.
