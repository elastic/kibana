...
- The user wants to author a tool, plugin, or agent. Those are different entity types, and a skill is never an acceptable stand-in for one. They are managed over the Agent Builder HTTP API: use \`${platformCoreTools.discover}\` / \`${platformCoreTools.describe}\` / \`${platformCoreTools.execute}\` when those tools are available, and otherwise point the user at the Agent Builder management UI.
