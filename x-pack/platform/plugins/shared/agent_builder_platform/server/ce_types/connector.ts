...
const CONNECTOR_CE_TYPE = 'connector';

interface ConnectorCeTypeDeps {
  /**
...
/**
 * Creates the CE type definition for connectors.
 *
 * Connectors are indexed into the CE exclusively via event-driven calls
 ...
 */
export const createConnectorCeType = (deps: ConnectorCeTypeDeps): CeTypeDefinition => {
  const { getActionSavedObjectsClient, logger } = deps;

  return {
    id: CONNECTOR_CE_TYPE,
    ...
    getCeData: async (originId, context) => {
      try {
        ...
        context.logger.warn(
          `CE connector: failed to get data for '${originId}': ${(error as Error).message}`
        );
        return undefined;
      }
    },
    ...
    toAttachment: async (item, context) => {
      try {
        ...
        logger.warn(
          `CE connector: failed to convert '${item.origin_id}' to attachment: ${
