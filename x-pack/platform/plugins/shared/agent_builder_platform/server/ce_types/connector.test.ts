...
import type { CeListItem } from '@kbn/context-engine-plugin/server';
import { AttachmentType } from '@kbn/agent-builder-common/attachments';
import { createConnectorCeType } from './connector';
...
async function collectPages(iterable: AsyncIterable<CeListItem[]>): Promise<CeListItem[]> {
  const items: CeListItem[] = [];
...
describe('connectorCeType', () => {
  ...
  describe('id', () => {
    it('equals connector', () => {
      expect(connectorCeType.id).toBe('connector');
    });
  });

  describe('list', () => {
    it('yields nothing — connector indexing is event-driven only', async () => {
      const result = await collectPages(connectorCeType.list(createContext() as never));
  ...
  describe('getCeData', () => {
    it('returns entry with connector name and description in content', async () => {
      ...
      const result = await connectorCeType.getCeData!('conn-1', createContext() as never);
    ...
    it('returns undefined on error and logs warning', async () => {
      ...
      const result = await connectorCeType.getCeData!('missing-conn', context as never);
  ...
  describe('getPermissions', () => {
    it('returns the saved_object:action/get Kibana privilege', () => {
      ...
      const permissions = connectorCeType.getPermissions!('conn-1', createContext() as never);
  ...
  describe('toAttachment', () => {
    it('returns connector attachment data', async () => {
      ...
      const result = await connectorCeType.toAttachment!(
