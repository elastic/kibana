...
import { visualizationCeType } from './visualization';
...
describe('visualizationCeType', () => {
  ...
  describe('id', () => {
    it('equals visualization', () => {
      expect(visualizationCeType.id).toBe('visualization');
    });
  });
  ...
  describe('list', () => {
    it('calls createPointInTimeFinder with type lens, namespaces [*], fields [title]', async () => {
      ...
      await collectPages(visualizationCeType.list(createContext() as never));
    ...
    it('returns items with id, updatedAt from so.updated_at, spaces from so.namespaces', async () => {
      ...
      const result = await collectPages(visualizationCeType.list(createContext() as never));
  ...
  describe('getCeData', () => {
    it('returns entry with correct type, title, content (permissions are handled by getPermissions)', async () => {
      ...
      const result = await visualizationCeType.getCeData!('viz-1', createContext() as never);
    ...
    it('content includes title, description, chartType, esql joined by newline', async () => {
      ...
      const result = await visualizationCeType.getCeData!('viz-2', createContext() as never);
    ...
    it('returns undefined on error when savedObjectsClient.get throws', async () => {
      ...
      const result = await visualizationCeType.getCeData!('missing-viz', context as never);
    ...
    it('handles missing optional fields: no description, no esql', async () => {
      ...
      const result = await visualizationCeType.getCeData!('viz-minimal', createContext() as never);
  ...
  describe('toAttachment', () => {
    it('calls resolve with lens and item.origin_id', async () => {
      ...
      await visualizationCeType.toAttachment!(
    ...
    it('returns visualization attachment with correct shape', async () => {
      ...
      const result = await visualizationCeType.toAttachment!(
        { origin_id: 'viz-1' } as never,
        createContext() as never
      );
    ...
    it('propagates errors from resolve to the caller', async () => {
      ...
      await expect(
        visualizationCeType.toAttachment!({ origin_id: 'viz-1' } as never, createContext() as never)
