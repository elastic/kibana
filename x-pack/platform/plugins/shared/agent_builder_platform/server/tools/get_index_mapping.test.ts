  ...
  describe('resources without fields', () => {
    beforeEach(() => {
      getIndexFieldsMock.mockResolvedValue({
        'logs-aged-out': { type: 'alias', fields: [] },
      });
    });

    it('explains that an empty field list can mean every index behind the alias is on the frozen tier', async () => {
    ...
    it('does not blame the frozen tier for an empty data stream, whose fields come from its template', async () => {
      getIndexFieldsMock.mockResolvedValue({
        'logs-empty': { type: 'dataStream', fields: [] },
      });

      const result = await runTool(['logs-empty'], false);
