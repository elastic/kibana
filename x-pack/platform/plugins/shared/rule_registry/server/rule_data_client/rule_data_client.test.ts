  ...
  for (const isUsingDataStreams of [false, true]) {
    ...
    describe(`using ${label} for alert indices`, () => {
      ...
      describe('bulk()', () => {
        ...
        test('does not log an error when the only per-item failures are ignorable version conflicts', async () => {
          scopedClusterClient.bulk.mockResponseOnce({
            took: 0,
            errors: true,
            items: [
          ...
          // The full response is still returned to the caller unchanged.
          expect(bulkWriteResponse?.body.errors).toBe(true);
          // But it must not be logged as an error - only a debug line is emitted.
          expect(logger.error).not.toHaveBeenCalled();
          expect(logger.debug).toHaveBeenCalledWith(
            expect.stringContaining('completed with only ignorable per-item errors')
          );
          expect(ruleDataClient.isWriteEnabled()).toBe(true);
        });

        test('logs an error when a non-ignorable failure accompanies ignorable version conflicts', async () => {
          scopedClusterClient.bulk.mockResponseOnce({
            took: 0,
            errors: true,
            items: [
