  ...
  for (const isUsingDataStreams of [false, true]) {
    ...
    describe(`using ${label} for alert indices`, () => {
      ...
      describe('bulk()', () => {
        ...
        test('does not log an error when the only failures are benign version conflicts (409)', async () => {
          scopedClusterClient.bulk.mockResponseOnce({
            took: 12,
            errors: true,
            items: [
          ...
          // The raw (unsanitized) response is returned unchanged and nothing is logged.
          expect(bulkWriteResponse?.body.errors).toBe(true);
          expect(logger.error).not.toHaveBeenCalled();
          expect(ruleDataClient.isWriteEnabled()).toBe(true);
        });

        test('logs an error when real failures are mixed with benign version conflicts (409)', async () => {
          scopedClusterClient.bulk.mockResponseOnce({
            took: 12,
            errors: true,
            items: [
