...
import { productDocIndexPattern, getAIClientToolsIndexName } from '@kbn/product-doc-common';
...
describe('getIndicesForProductNames', () => {
  ...
  it('returns individual index names when product names are specified', () => {
    expect(getIndicesForProductNames(['kibana', 'elasticsearch'])).toEqual([
      getAIClientToolsIndexName('kibana'),
      getAIClientToolsIndexName('elasticsearch'),
    ]);
  });
  ...
  it('returns individual index names when ELSER is specified', () => {
    ...
    ).toEqual([getAIClientToolsIndexName('kibana'), getAIClientToolsIndexName('elasticsearch')]);
  });
