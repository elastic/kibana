...
├── processing/ui/     12 specs   data processing / enrichment pipeline
├── routing/ui/         9 specs   data routing / partitioning (incl. AI suggestions)
├── lifecycle/ui/       7 specs   data retention
├── data_quality/ui/    1 spec    data quality tab
├── schema/ui/          3 specs   data mapping / schema editor
├── query_streams/ui/   7 specs   query streams (stateful-classic only)
├── core/ui/            9 specs   enablement, layout shell, list view, stream detail chrome
└── integrations/ui/    3 specs   cross-app surfaces (Discover, ES|QL, TSDB)
```
...
(e.g. `lifecycle/ui/fixtures/retention_helpers.ts`) live next to the specs
that use them.
