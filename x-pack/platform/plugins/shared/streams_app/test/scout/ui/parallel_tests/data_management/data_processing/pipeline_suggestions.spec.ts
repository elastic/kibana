...
import { expect, type ScoutPage, type TestInfo } from '@kbn/scout';
import {
  test,
 ...
 */
function setupMockPipelineResponse(page: ScoutPage, streamName: string) {
  return page.route(
    `**/internal/streams/${streamName}/_suggest_processing_pipeline`,
    async (route) => {
...
test.describe('Stream data processing - Pipeline suggestions', { tag: ['@ess'] }, () => {
  ...
  test.beforeAll(async ({ apiServices, logsSynthtraceEsClient, log }, testInfo: TestInfo) => {
    // Generate unique stream name for this worker
    testStream = getUniqueClassicStreamName(testInfo, 'pipeline-suggestions');
  ...
  test('should generate suggestion and allow accept/reject', async ({ page, pageObjects }) => {
    ...
    await expect(page.getByText('Extract useful fields from your data')).toBeVisible();
    await setupMockPipelineResponse(page, testStream);
