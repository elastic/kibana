...
test.describe('Stream data quality', { tag: ['@ess', '@svlOblt'] }, () => {
  test.beforeAll(async ({ apiServices, logsSynthtraceEsClient }, testInfo: TestInfo) => {
    ...
    testStream = getUniqueClassicStreamName(testInfo, 'nginx-default');
    wiredStream = getUniqueStreamName(testInfo, 'nginx');

    // Clean up any existing streams from previous runs
    await safeDeleteStream(apiServices, wiredStream);
  ...
  test.beforeEach(async ({ browserAuth, pageObjects }) => {
    ...
    await pageObjects.streams.gotoDataQualityTab(testStream);
  });
  ...
  test('date picker should show same time range as Streams Main page', async ({ pageObjects }) => {
    ...
    await pageObjects.streams.clickStreamNameLink(wiredStream);
