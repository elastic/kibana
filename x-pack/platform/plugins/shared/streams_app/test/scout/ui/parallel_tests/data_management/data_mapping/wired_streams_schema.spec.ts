...
// Store stream names for cleanup - will be set per worker
let parentStreamName: string;
let childStreamName: string;
...
test.describe(
  ...
  () => {
    test.beforeAll(async ({ apiServices, logsSynthtraceEsClient }, testInfo: TestInfo) => {
      ...
      parentStreamName = getUniqueStreamName(testInfo, 'schema-parent');
      childStreamName = getUniqueNestedStreamName(testInfo, 'schema-parent', 'child');

      // Clean up any existing streams from previous runs
      await safeDeleteStream(apiServices, childStreamName);
      await safeDeleteStream(apiServices, parentStreamName);
    ...
    test.beforeEach(async ({ apiServices, browserAuth, pageObjects }) => {
      ...
      await safeClearStreamMappings(apiServices, parentStreamName);
      await safeClearStreamMappings(apiServices, childStreamName);

      await pageObjects.streams.gotoSchemaEditorTab(childStreamName);
    ...
    test('inherited fields display with parent links', async ({ page, pageObjects }) => {
      ...
      await pageObjects.streams.gotoSchemaEditorTab(parentStreamName);
      ...
      await expect(page.getByTestId('streamsAppFieldParentLink')).toContainText(parentStreamName);
    });
