...
test.describe('Stream data mapping - schema editor', { tag: ['@ess', '@svlOblt'] }, () => {
  ...
  test.beforeAll(async ({ apiServices, logsSynthtraceEsClient }, testInfo) => {
    // Generate unique stream name for this worker
    testStreamName = getUniqueStreamName(testInfo, 'mapping');
  ...
  test.beforeEach(async ({ apiServices, browserAuth, pageObjects }) => {
    ...
    await safeClearStreamMappings(apiServices, testStreamName);

    await pageObjects.streams.gotoSchemaEditorTab(testStreamName);
  });
