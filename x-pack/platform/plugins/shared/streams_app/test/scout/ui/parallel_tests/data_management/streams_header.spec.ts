...
test.describe('Stream detail header', { tag: ['@ess', '@svlOblt'] }, () => {
  ...
  test('shows correct badges', async ({ pageObjects, config }) => {
    ...
    await test.step('verify Discover badge link', async () => {
      await pageObjects.streams.verifyDiscoverButtonLink(testStreamName);
    });
  ...
  test('transitions data quality from Good -> Degraded -> Poor for stream', async ({
    ...
    await pageObjects.streams.verifyDataQuality(testStreamName, 'Good');
    ...
    await pageObjects.streams.verifyDataQuality(testStreamName, 'Degraded');
    ...
    await pageObjects.streams.verifyDataQuality(testStreamName, 'Poor');
  });
});
