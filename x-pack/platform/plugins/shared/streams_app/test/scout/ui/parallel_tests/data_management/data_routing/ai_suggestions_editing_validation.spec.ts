...
test.describe('Stream data routing - AI suggestions editing validation', { tag: ['@ess'] }, () => {
  ...
  test.afterEach(async ({ apiServices }) => {
    // Clean up any streams created during this test
    if (existingStream) {
  ...
  test('should show error when editing suggestion with duplicate name', async ({
    ...
    // Create a unique existing stream for this worker
    existingStream = getUniqueStreamName(testInfo, 'existing');
    await safeDeleteStream(apiServices, existingStream);
