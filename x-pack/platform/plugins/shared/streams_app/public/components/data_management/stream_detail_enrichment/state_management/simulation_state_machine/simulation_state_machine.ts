...
const hasAnyValidSteps = (steps: StreamlangStepWithUIAttributesWithCustomIdentifier[]) =>
  steps.some(isValidStep);
    ...
    runningSimulation: {
      invoke: {
        ...
        input: ({ context }) => ({
          ...
          steps: context.steps.filter((step) => {
