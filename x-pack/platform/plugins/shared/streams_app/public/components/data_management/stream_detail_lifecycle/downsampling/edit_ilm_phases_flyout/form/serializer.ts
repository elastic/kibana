  ...
  return (data: IlmPhasesFlyoutFormInternal): IlmPolicyPhases => {
    ...
    if (!meta.hot?.enabled) {
      ...
      next.hot = {
        ...previous,
        name: 'hot',
        size_in_bytes: meta.hot.sizeInBytes ?? previous.size_in_bytes ?? 0,
    ...
    if (!meta.warm?.enabled) {
      ...
      next.warm = {
        ...previous,
        name: 'warm',
        size_in_bytes: meta.warm.sizeInBytes ?? previous.size_in_bytes ?? 0,
        min_age: minAge,
      };
    ...
    if (!meta.cold?.enabled) {
      ...
      next.cold = {
        ...previous,
        name: 'cold',
        size_in_bytes: meta.cold.sizeInBytes ?? previous.size_in_bytes ?? 0,
        min_age: minAge,
      };
    ...
    if (!meta.frozen?.enabled) {
      ...
      next.frozen = {
        ...previous,
        name: 'frozen',
        size_in_bytes: previous.size_in_bytes ?? 0,
        min_age: minAge,
      };
    ...
    if (!meta.delete?.enabled) {
      ...
      next.delete = {
        ...previous,
        name: 'delete',
        // delete.min_age is required by the public type; submission is gated by form validity.
