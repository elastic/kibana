...
describe('AutocompleteSelector', () => {
  ...
  describe('Field Type Icons', () => {
    ...
    it('hides icons for suggestions when icon is not present', async () => {
      ...
      expect(screen.getByTestId('field-icon-date')).toBe(false);
      expect(screen.getByTestId('field-icon-keyword')).toBe(false);
      expect(screen.getByTestId('field-icon-text')).toBe(false);
      expect(screen.getByTestId('field-icon-unknown')).toBe(false);
    });
