...
export const streamEnrichmentMachine = setup({
  ...
  actions: {
    ...
    /* Save state tracking - prevents stream.received from interrupting saves */
    setSaving: assign({ isSaving: true }),
    clearSaving: assign({ isSaving: false }),
  ...
  guards: {
    ...
    // Prevents stream.received from interrupting an in-progress save operation
    isNotSaving: ({ context }) => !context.isSaving,
  },
}).createMachine({
  ...
  states: {
    ...
    ready: {
      ...
      on: {
        'stream.received': {
          // Guard prevents interrupting an in-progress save operation
          guard: 'isNotSaving',
          target: '#ready',
          actions: [
