...
import type { AnyStateMachine, MachineImplementationsFrom } from 'xstate';
...
// Keep setup typing for implementation contracts while exporting a widened machine
// to avoid TS7056 when serializing the full machine type.
const streamEnrichmentMachineSetup = setup({
  types: {
...
type StreamEnrichmentMachineInternal = ReturnType<
  typeof streamEnrichmentMachineSetup.createMachine
>;

const streamEnrichmentMachineInternal = streamEnrichmentMachineSetup.createMachine({
...
}) as unknown as StreamEnrichmentMachineInternal;

export type StreamEnrichmentMachine = StreamEnrichmentMachineInternal;

export const streamEnrichmentMachine: AnyStateMachine = streamEnrichmentMachineInternal;
...
export const createStreamEnrichmentMachineImplementations = ({
  ...
  telemetryClient,
}: StreamEnrichmentServiceDependencies): MachineImplementationsFrom<StreamEnrichmentMachine> => ({
  actors: {
