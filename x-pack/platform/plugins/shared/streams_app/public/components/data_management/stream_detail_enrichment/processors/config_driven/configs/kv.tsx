...
> = {
  ...
  convertFormStateToConfig: (formState) => {
    const { field } = formState;
    return {
      kv: {
        if: formState.if,
        field,
        field_split: formState.field_split,
        value_split: formState.value_split,
        target_field: isEmpty(formState.target_field) ? undefined : formState.target_field,
        include_keys: isEmpty(formState.include_keys) ? undefined : formState.include_keys,
        exclude_keys: isEmpty(formState.exclude_keys) ? undefined : formState.exclude_keys,
        prefix: isEmpty(formState.prefix) ? undefined : formState.prefix,
        trim_key: isEmpty(formState.trim_key) ? undefined : formState.trim_key,
        trim_value: isEmpty(formState.trim_value) ? undefined : formState.trim_value,
        strip_brackets: formState.strip_brackets,
  ...
  convertProcessorToFormState: (processor) => {
    ...
    return structuredClone({
      ...kv,
      type: 'kv',
      include_keys: kv.include_keys ?? [],
      exclude_keys: kv.exclude_keys ?? [],
    });
  },
  fieldConfigurations: [
    {
