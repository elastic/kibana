...
const ProcessorConfigurationListItem = ({
  ...
  const previousProcessor =
    idx >= 1
      ? processorsRefs.filter(
          (ref) => ref.getSnapshot().context.processor.whereParentId === processor.whereParentId
        )[idx - 1]
      : undefined;
  const comesAfterWhereProcessor =
    previousProcessor?.getSnapshot().context.processor.type === 'where';
...
const SubList = ({ parentId }: { parentId: string; grandparentId?: string }) => {
  const processorsRefs = useStreamEnrichmentSelector((state) => state.context.processorsRefs);
  const relevantProcessorsRefs = processorsRefs.filter(
    (processorRef) => processorRef.getSnapshot().context.processor.whereParentId === parentId
  );
  ...
  return (
    <EuiFlexGroup direction="column" gutterSize="s" responsive={false}>
      <SortableList onDragItem={handlerItemDrag} subId={parentId}>
        {relevantProcessorsRefs.map((processorRef, idx) => (
...
const WhereProcessorForm = () => {
  return <>Nothing to do here</>;
};

// function WhereStep(props: {
//   step: ProcessorDefinition;
//   onChange: (step: ProcessorDefinition) => void;
// }) {
...
//       selectPreviewRecords(getEnrichmentState().context.simulatorRef.getSnapshot().context),
//       { grokCollection },
//       processorConverter.toUIDefinition(step)
//     )
//   );
...
const getProcessorDescription = (processor: ProcessorDefinitionWithUIAttributes) => {
  ...
  } else if (processor.type === 'where' && 'where' in processor) {
    return JSON.stringify(processor.where.if);
  }
