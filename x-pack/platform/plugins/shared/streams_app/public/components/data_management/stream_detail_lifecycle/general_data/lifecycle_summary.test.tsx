...
describe('LifecycleSummary', () => {
  ...
  describe('DSL Lifecycle', () => {
    ...
    it('should open the override template modal when deleting a downsample step that is inherited from a template', () => {
      const definition = createDslDefinition('60d', [
        { after: '10d', fixed_interval: '1h' },
        { after: '30d', fixed_interval: '1d' },
      ]);
      ...
      fireEvent.click(screen.getByTestId('downsamplingPopover-step1-deleteButton'));

      expect(screen.getByTestId('createPolicyModalTitle')).toBeInTheDocument();
    });

    it('should save directly when deleting a downsample step that is not inherited from a template', async () => {
      const ingestLifecycle: IngestStreamLifecycle = {
        dsl: {
          data_retention: '60d',
          downsample: [
            { after: '10d', fixed_interval: '1h' },
