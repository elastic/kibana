...
export const interactiveModeMachine = setup({
  ...
  actions: {
    ...
    addProcessor: assign(
      (
        ...
        const convertedProcessor: PipelineStepWithUIAttributes = {
          ...processorDefinition,
          customIdentifier:
            typeof processorDefinition.customIdentifier === 'string'
              ? processorDefinition.customIdentifier
              : createId(),
          parentId: conversionOptions.parentId ?? null,
          branch: conversionOptions.branch,
        };
        ...
        const insertIndex = findInsertIndex(
          ...
          conversionOptions.parentId ?? null,
    ...
    addCondition: assign(
      (
        ...
        const conversionOptions = options ?? { parentId: null };
        const convertedCondition: PipelineStepWithUIAttributes = {
          ...condition,
          customIdentifier: condition.customIdentifier ?? createId(),
          parentId: conversionOptions.parentId ?? null,
          branch: conversionOptions.branch,
        };
    ...
    reorderStepsByDragDrop: enqueueActions(
      (
        ...
        let newBranch: PipelineStepBranch;
    ...
    syncToDefinition: ({ context }) => {
      context.parentRef.send({
        type: 'mode.definitionUpdated',
        definition: {
          steps: context.stepRefs.map((stepRef) => stepRef.getSnapshot().context.step),
        },
      });
    },
    ...
    overwriteSteps: assign(
      (assignArgs, params: { steps: PipelineProcessorsUiDefinition['steps'] }) => {
        ...
        const stepRefs = params.steps.map((step) => {
          return spawnStep(
            step,
            parentRef,
            assignArgs.spawn as StepSpawner,
    ...
    storeSuggestedPipeline: assign((_, params: { pipeline: PipelineProcessorsUiDefinition }) => ({
  ...
  context: ({ input, spawn, self }) => {
    const uiSteps = input.definition.steps;
    const parentRef: StepParentActor = self;
    const stepRefs = uiSteps.map((step) => {
      // When switching from JSON mode new steps will be denoted in the input. These steps should be marked as
      // "new" and also "updated" (to mimic being fully configured in interactive mode). "new" alone would just
      // denote a draft state.
      const shouldBeMarkedAsNewAndUpdated = Boolean(
        step.customIdentifier && input.newStepIds.includes(step.customIdentifier)
      );
  ...
  states: {
    pipelineSuggestion: {
      ...
      states: {
        ...
        viewingSuggestion: {
          entry: [{ type: 'syncToDefinition' }, { type: 'sendStepsToSimulator' }],
          on: {
            'suggestion.accept': {
              target: 'idle',
              actions: [{ type: 'syncToDefinition' }, { type: 'clearSuggestion' }],
            },
            'suggestion.dismiss': {
              target: 'idle',
              actions: [
      ...
      states: {
        idle: {
          ...
          on: {
            ...
            'step.reorderByDragDrop': {
              ...
              target: 'idle',
              reenter: true,
              // Re-enter to trigger syncToDefinition for sibling reordering.
        ...
        creating: {
          ...
          on: {
            ...
            'step.save': {
              target: 'idle',
              actions: [{ type: 'reassignSteps' }, { type: 'syncToDefinition' }],
            },
            'step.filterByCondition': {
              actions: [
                { type: 'storeConditionFilter', params: ({ event }) => event },
        ...
        editing: {
          ...
          on: {
            'step.change': {
              actions: [{ type: 'syncToDefinition' }, { type: 'sendStepsToSimulator' }],
            },
            'step.cancel': {
              target: 'idle',
              actions: [{ type: 'clearAutoConditionFilter' }],
