...
interface LegacyAncestorProcessing {
  steps: unknown[];
}
...
interface DraftSampleSource {
  ...
  ancestorProcessing: LegacyAncestorProcessing;
}
...
export async function resolveDraftSampleSource(
  ...
  let ancestorProcessing: LegacyAncestorProcessing = { steps: [] };
  if (parentIsDraft) {
...
async function collectDraftAncestorProcessing(
  streamsRepositoryClient: StreamsRepositoryClient,
  streamName: string
): Promise<LegacyAncestorProcessing> {
