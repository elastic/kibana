...
export const DocViewerContext = React.createContext<DocViewerContextValue | undefined>(undefined);

export const useDocViewerContext = (): DocViewerContextValue => {
...
export function getDocViewerDiffStrings({
  originalSample,
  flattenedHit,
}: {
  originalSample?: Record<string, unknown>;
  ...
  const originalValue = originalSample
    ? JSON.stringify(orderObjectKeys(flattenObjectNestedLast(originalSample)), null, 2)
    : '';
  const newValue = JSON.stringify(orderObjectKeys(flattenedHit), null, 2);
...
function JsonDiffViewer({ hit, decreaseAvailableHeightBy }: DocViewRenderProps) {
  ...
  const { originalValue, newValue } = useMemo(
    () =>
      getDocViewerDiffStrings({
        originalSample,
        flattenedHit: hit.flattened,
      }),
    [originalSample, hit.flattened]
  );
