...
const sanitiseStep = ({ customIdentifier, ...restOfStep }: PipelineStepWithUIAttributes) =>
  omitBy(restOfStep, isUndefined);

const isStepUpdated = (
  step: PipelineStepWithUIAttributes,
  originalStep: PipelineStepWithUIAttributes
  ...
  actions: {
    changeProcessor: assign(
      (
        ...
        const nextStep: PipelineStepWithUIAttributes = {
          ...params.step,
    ...
    changeDescription: assign(({ context }, { description }: { description?: string }) => {
      if (!isPipelineProcessorStep(context.step)) {
        return {};
      }
    ...
    changeParent: assign(
      (
        { context },
        { parentId, branch }: { parentId: string | null; branch?: PipelineStepBranch }
      ) => {
        const updatedStep = {
          ...context.step,
          parentId,
