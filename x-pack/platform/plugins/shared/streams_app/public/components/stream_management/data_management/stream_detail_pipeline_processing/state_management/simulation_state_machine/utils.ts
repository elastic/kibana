...
export function getSourceFields(processor: ProcessorFormState): string[] {
  const sourceFields = ((): Array<string | undefined> => {
    switch (processor.action) {
      ...
      case 'trim':
      case 'join':
        return [asString(processor.field)];
      case 'network_direction':
        return [asString(processor.source_ip), asString(processor.destination_ip)];
      case 'set':
        // `set` reads from a field only when copying; literal values have no source.
        return asString(processor.copy_from) ? [asString(processor.copy_from)] : [];
      default:
        // append, enrich, remove, drop: no individual source column.
        return [];
    }
  })();
...
export function getDestinationField(processor: ProcessorFormState): string | undefined {
  const destinationField = ((): string | undefined => {
    switch (processor.action) {
      ...
      case 'user_agent':
      case 'uri_parts':
        return asString(processor.target_field);
      case 'set':
      case 'append':
        return asString(processor.field);
      case 'network_direction':
...
export function collectDescendantProcessorIdsForCondition(
  ...
  return steps
    .filter((step) =>
      step.customIdentifier ? descendantStepIds.has(step.customIdentifier) : false
    )
    .filter((step) => isPipelineProcessorStep(step))
