...
import type { GrokFormState, PipelineProcessorDefinitionWithUIAttributes } from '../../../../types';
...
export function useGrokPatternSuggestion(abortController: ReturnType<typeof useAbortController>) {
  ...
  async function suggestGrokPattern(
    ...
    try {
      ...
      const nativeGrokProcessor: GrokFormState = {
        ...grokProcessor,
        action: 'grok',
        field: grokProcessor.from,
        patterns: grokProcessor.patterns.map((pattern) => ({ value: pattern })),
      };
      ...
      return { grokProcessor: nativeGrokProcessor, simulationResult };
    } catch (error) {
