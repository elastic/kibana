...
export const DateProcessorForm = () => {
  ...
  const applySuggestions = async ({ field }: { field: string }) => {
    ...
    try {
      ...
      const suggestedFormats = suggestions.formats ?? [];
      if (!isEmpty(suggestedFormats)) {
        // Merge the suggested formats with the existing ones
        const prevFormats = form.getValues('formats');
        form.setValue('formats', uniq([...prevFormats, ...suggestedFormats]), {
          shouldValidate: true,
        });
        form.clearErrors();
      }
  ...
  return (
    <>
      <ProcessorFieldSelector fieldKey="field" helpText="" onChange={handleProcessorFieldChange} />
