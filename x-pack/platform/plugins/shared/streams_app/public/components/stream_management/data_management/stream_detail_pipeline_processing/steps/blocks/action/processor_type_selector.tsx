...
import type { PipelineProcessorType, ProcessorFormState } from '../../../types';
...
type TAvailableProcessors = Partial<Record<PipelineProcessorType, TAvailableProcessor>>;
...
export const ProcessorTypeSelector = ({ disabled = false }: { disabled?: boolean }) => {
  ...
  const processorType = useWatch<{ action: PipelineProcessorType }>({ name: 'action' });
  ...
  const handleChange = (selectedOptions: Array<EuiComboBoxOptionOption<PipelineProcessorType>>) => {
  ...
  const groupedOptions = useMemo(() => getProcessorTypeSelectorOptions(), []);
...
const getAvailableProcessors = (): Partial<TAvailableProcessors> => ({
  date: {
    type: 'date',
    inputDisplay: 'Date',
    getDocUrl: () => (
...
const getProcessorDescription = (docLinks: DocLinksStart) => (type: PipelineProcessorType) => {
  return getAvailableProcessors()[type]?.getDocUrl(docLinks);
};

const getProcessorTypeSelectorOptions = (): Array<{
  label: string;
  options: Array<EuiComboBoxOptionOption<PipelineProcessorType>>;
}> => {
  const availableProcessors = getAvailableProcessors();

  // Define processor groups
  const groups = {
    remove: [] as Array<EuiComboBoxOptionOption<PipelineProcessorType>>,
    extract: [] as Array<EuiComboBoxOptionOption<PipelineProcessorType>>,
    convert: [] as Array<EuiComboBoxOptionOption<PipelineProcessorType>>,
    set: [] as Array<EuiComboBoxOptionOption<PipelineProcessorType>>,
    other: [] as Array<EuiComboBoxOptionOption<PipelineProcessorType>>,
  };
