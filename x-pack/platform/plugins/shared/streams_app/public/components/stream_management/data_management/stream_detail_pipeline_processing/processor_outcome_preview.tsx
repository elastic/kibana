...
import type { GrokPatternField } from './types';
...
const OutcomePreviewTable = ({ previewDocuments }: { previewDocuments: FlattenRecord[] }) => {
  ...
  const { currentProcessorSourceFields, currentProcessorDestinationField, currentStepId, stepIds } =
    useStreamEnrichmentSelector((state) => {
      ...
      for (const stepRef of stepRefs) {
        ...
        if ('action' in step && isStepUnderEdit(snapshot)) {
          return {
  ...
  // Get grok patterns from the draft processor. The form uses `{ value }`
  // wrappers for useFieldArray, but grok-ui expects raw pattern strings.
  const grokPatterns =
    draftProcessor?.processor && isGrokDraftProcessor(draftProcessor.processor)
      ? (draftProcessor.processor.patterns ?? []).map((pattern: string | GrokPatternField) =>
  ...
  const isGrokProcessorActive =
    ...
    isGrokDraftProcessor(draftProcessor.processor) &&
    !isEmpty(draftProcessor.processor.field);

  // Get the source field for the grok processor
  const grokSourceField =
    isGrokProcessorActive && isGrokDraftProcessor(draftProcessor?.processor)
      ? draftProcessor.processor.field
      : undefined;
