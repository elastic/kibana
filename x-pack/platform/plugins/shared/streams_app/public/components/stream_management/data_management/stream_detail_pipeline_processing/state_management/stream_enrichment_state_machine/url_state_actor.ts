...
// URL syncing is temporarily parked until the processing editor is wired into
// the new flyout deeplink model. Keep the implementation below intact so it can
// be re-enabled when that work lands.
const PROCESSING_FLYOUT_URL_SYNC_ENABLED = false;
...
export function createUrlInitializerActor({
  ...
  return fromCallback(({ sendBack }) => {
    if (!PROCESSING_FLYOUT_URL_SYNC_ENABLED) {
      return sendBack({
