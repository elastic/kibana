...
const enableActionPolicyParamsSchema = z.object({
  id: z
    ...
    .max(ID_MAX_LENGTH)
    .describe(
      'The ID of the action policy to enable. Copy it from the response when you create a policy, fetch one policy, or fetch the policy list.'
    ),
});
...
export class EnableActionPolicyRoute extends BaseAlertingRoute {
  ...
  static schemas = {
    request: {
      params: enableActionPolicyParamsSchema,
    },
    response: {
      200: {
  ...
  constructor(
    ...
    private readonly request: KibanaRequest<
      z.infer<typeof enableActionPolicyParamsSchema>,
      unknown,
      unknown
    >,
