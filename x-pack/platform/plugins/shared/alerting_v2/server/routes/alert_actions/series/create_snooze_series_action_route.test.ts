...
import { createSnoozeSeriesActionBodySchema } from '@kbn/alerting-v2-schemas';
...
import { CreateSnoozeSeriesActionRoute } from './create_snooze_series_action_route';
import { createRouteDependencies } from '../../test_utils';

describe('CreateSnoozeSeriesActionRoute', () => {
  ...
  it('injects action_type and returns noContent', async () => {
    ...
    const route = new CreateSnoozeSeriesActionRoute(
      ctx,
      request,
...
describe('createSnoozeSeriesActionBodySchema', () => {
  it('accepts empty payload without action_type', () => {
    expect(createSnoozeSeriesActionBodySchema.safeParse({}).success).toBe(true);
  });

  it('accepts payload with expiry', () => {
    expect(
      createSnoozeSeriesActionBodySchema.safeParse({ expiry: '2026-01-28T16:03:00.000Z' }).success
    ).toBe(true);
  });

  it('rejects payload with action_type', () => {
    expect(createSnoozeSeriesActionBodySchema.safeParse({ action_type: 'snooze' }).success).toBe(
      false
    );
  });
});
