...
import { createAckEpisodeActionBodySchema } from '@kbn/alerting-v2-schemas';
...
import { CreateAckEpisodeActionRoute } from './create_ack_episode_action_route';
import { createRouteDependencies } from '../../test_utils';

describe('CreateAckEpisodeActionRoute', () => {
  ...
  it('injects action_type and returns noContent', async () => {
    ...
    const route = new CreateAckEpisodeActionRoute(
      ctx,
      request,
...
describe('createAckEpisodeActionBodySchema', () => {
  it('accepts an empty payload', () => {
    expect(createAckEpisodeActionBodySchema.safeParse({}).success).toBe(true);
  });

  it('rejects payload with episode_id', () => {
    expect(createAckEpisodeActionBodySchema.safeParse({ episode_id: 'ep-1' }).success).toBe(false);
  });

  it('rejects payload with action_type', () => {
    expect(createAckEpisodeActionBodySchema.safeParse({ action_type: 'ack' }).success).toBe(false);
  });
});
