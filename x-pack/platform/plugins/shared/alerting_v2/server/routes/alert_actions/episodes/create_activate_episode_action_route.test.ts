...
import { createActivateEpisodeActionBodySchema } from '@kbn/alerting-v2-schemas';
...
import { CreateActivateEpisodeActionRoute } from './create_activate_episode_action_route';
import { createRouteDependencies } from '../../test_utils';

describe('CreateActivateEpisodeActionRoute', () => {
  ...
  it('injects action_type and returns noContent', async () => {
    ...
    const route = new CreateActivateEpisodeActionRoute(
      ctx,
      request,
...
describe('createActivateEpisodeActionBodySchema', () => {
  it('accepts payload without action_type', () => {
    expect(createActivateEpisodeActionBodySchema.safeParse({ reason: 'x' }).success).toBe(true);
  });

  it('rejects payload with action_type', () => {
    expect(
      createActivateEpisodeActionBodySchema.safeParse({ action_type: 'activate', reason: 'x' })
        .success
    ).toBe(false);
  });
});
