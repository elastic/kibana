...
@injectable()
export class GetRuleExecutionKpiRoute extends BaseAlertingRoute {
  static method = 'get' as const;
  static path = `${ALERTING_V2_RULE_API_PATH}/{id}/_execution_kpi`;
  ...
  static routeOptions = {
    summary: 'Get rule execution KPI',
  } as const;
  static validate = {
    request: {
      params: buildRouteValidationWithZod(ruleIdParamsSchema),
      query: buildRouteValidationWithZod(executionKpiQuerySchema),
    },
    response: {
      200: {
        body: () => executionKpiResponseSchema,
        description: 'Indicates a successful call.',
      },
    },
  };

  protected readonly routeName = 'get rule execution kpi';
  ...
  protected async execute() {
    ...
    const { date_start, date_end } = this.request.query;

    const result = await this.executionLogService.getExecutionKpi({
      ruleId: id,
