...
export class BulkUpdateApiKeyRoute extends BaseAlertingRoute {
  ...
  static path = `${ALERTING_V2_RULE_API_PATH}/_bulk_update_api_key`;
  ...
  static schemas = {
    ...
    response: {
      200: {
        ...
        description: 'Returns the result of the bulk update API key operation.',
      },
      400: {
  ...
  protected readonly routeName = 'bulk update rules api key';
  ...
  protected async execute() {
    const result = await this.rulesClient.bulkUpdateApiKey({ ids: this.request.body.ids });
