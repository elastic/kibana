 ...
 * 2. Both model-version keys ('8', '9') are present in ruleModelVersions.
...
describe('ruleModelVersions fold lines', () => {
  it("contains key '8' for security.detection.query v1", () => {
    expect(ruleModelVersions).toHaveProperty('8');
  });

  it("contains key '9' for security.detection.threshold v1", () => {
  ...
  it('dense version sequence runs from 1 to 11 with no gaps', () => {
    // Updated from 10 to 11 by step 6.4: model version 11 makes `query` optional
