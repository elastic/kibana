...
export const ruleModelVersions: SavedObjectsModelVersionMap = {
  ...
  //
  // The five entries below ('6' through '10') were individually designed on this
  // POC branch, each carrying its own mappings or backfill and a schema step
  // ('10' was the exception: changes: [] — schema step only, no migration changes).
  // They are squashed here into a single '6' purely to satisfy the saved-objects
  ...
  // this squashed entry, re-introduce the matching schema imports (V4, V8) that
  ...
  //     10.6.0.json (keys '10.5.0'/'10.6.0') must become 10.10.0.json with keys
  //     '10.9.0'/'10.10.0', because the fixture is addressed by the highest model
  //     version in the PR ('10' in production, '6' in this squashed form).
  //
  // Changes carried by this squashed '6', in migration order:
  //   1. mappings_addition — metadata.builder_fields as flattened (was '6')
  //   2. fromBuilderManifest(securityDetectionQueryManifest, 1).changes
  //      — typed sub-fields for security.detection.query v1 (was '7')
  //   3. fromBuilderManifest(securityDetectionThresholdManifest, 1).changes
  //      — typed sub-fields for security.detection.threshold v1 (was '8')
  //   4. mappings_addition — framework field families: signature_id, source,
  //      ownership, builder_type (was '9')
  //   5. data_backfill — stamps all four field families on migrating rules (was '9')
  //   (was '10': changes: [] — no changes; only advanced the schema to V9)
  //
  ...
  //
  // The schemas block is '10's, verbatim (ruleSavedObjectAttributesSchemaV9),
  ...
  '6': {
    changes: [
      // was '6': introduce metadata.builder_fields as flattened
      {
        type: 'mappings_addition',
        addedMappings: {
          metadata: {
      ...
      // was '7': security.detection.query v1 typed sub-fields
      ...fromBuilderManifest(securityDetectionQueryManifest, 1).changes,
      // was '8': security.detection.threshold v1 typed sub-fields
      ...fromBuilderManifest(securityDetectionThresholdManifest, 1).changes,
      // was '9': framework field families (identity, versions, source, ownership, builder_type)
      {
        type: 'mappings_addition',
        addedMappings: {
          metadata: {
      ...
      // was '9': backfill stamps all four field families
      {
        type: 'data_backfill',
        backfillFn: (doc) => {
    ...
    // schemas from '10' (the final schema step in the POC sequence)
    schemas: {
  ...
  // They were squashed into the single '6' above only to satisfy the saved-objects
  ...
  //   2. Remove the squashed '6' above.
  //   3. Re-add the imports for ruleSavedObjectAttributesSchemaV4 and
  //      ruleSavedObjectAttributesSchemaV8 (removed from the import list above
  ...
  //        alerting_rule/10.6.0.json (keys '10.5.0'/'10.6.0')
  //      must become 10.10.0.json with top-level keys '10.9.0'/'10.10.0'.
  //      The checker resolves the fixture by the PR's highest model version
  //      (10.10.0 in production, 10.6.0 in this squashed form) and requires
