...
/**
 * v5 collapses the two `query` encodings into one and moves the lifecycle
 ...
 *   `recovering`.
 *
 * The pre-collapse keys are still accepted because model version 7 leaves them
 * on disk for the rollback window: model version 6's schema requires
 * `query.format` and a present `query.breach`. Nothing reads or writes them.
 * Model version 8 removes them, at which point `query` reduces to `base` plus
