...
export interface AlertingServerSetup {
  ...
  /**
   * Registers a rule builder type owned by the calling plugin. Rules carrying a
   * matching `metadata.builder_type` have their `metadata.builder_fields`
   * validated against `builderFieldsSchema` and their query generated by
   * `generateQuery` on every create and update. Unlike artifact types, an
   * unregistered builder type is rejected — a builder-authored rule the server
   * cannot generate a query for would have no valid query at all.
   *
   * Registering here is enough to author rules of this type over the API; see
