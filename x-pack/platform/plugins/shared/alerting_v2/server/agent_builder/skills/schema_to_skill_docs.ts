...
export const generateMatcherContextDoc = (): string => {
  ...
  return [
    ...
    ...rows,
    '',
    'An empty matcher is a catch-all that matches all episodes in the space.',
  ].join('\n');
};
...
export const generateSingleRuleActionPolicyDoc = (): string =>
  [
    ...
    '3. `set_matcher`: omit (leave as catch-all). The policy name and description already',
    '   express the intent. Use `tags: ["<unique-tag>"]` only when the rule carries a tag',
    '   that uniquely identifies it — `rule.id` is no longer available as a KQL field.',
...
export const generateMultiRuleActionPolicyDoc = (): string =>
  [
    ...
    '  rules created later.',
    '- **A family of rules by tag**: `tags: ["production"]`. Matched against the rule\'s tags.',
