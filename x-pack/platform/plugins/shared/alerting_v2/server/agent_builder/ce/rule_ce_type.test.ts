...
import { RULE_ATTACHMENT_TYPE, RULE_CE_TYPE } from '@kbn/alerting-v2-schemas';
import type { RulesClient } from '../../lib/rules_client';
import { RULE_SAVED_OBJECT_TYPE, type RuleSavedObjectAttributes } from '../../saved_objects';
import { ALERTING_V2_API_PRIVILEGES } from '../../lib/security/privileges';
import { createRuleCeType } from './rule_ce_type';
...
const buildCeContext = (logger = loggingSystemMock.createLogger()) => ({
...
describe('createRuleCeType', () => {
  ...
  describe('id and fetchFrequency', () => {
    it('uses the shared RULE_CE_TYPE constant', () => {
      expect(buildDefinition().id).toBe(RULE_CE_TYPE);
    });
  ...
  describe('list', () => {
    const drainList = async () => {
      ...
      for await (const page of def.list(buildCeContext())) {
        items.push(...page);
      }
      return items;
    };
  ...
  describe('getCeData', () => {
    it('returns a single entry built from rule metadata + query', async () => {
      ...
      const result = await buildDefinition().getCeData('rule-1', buildCeContext());
    ...
    it('falls back to originId for title when metadata.name is missing', async () => {
      ...
      const result = await buildDefinition().getCeData('rule-bare', buildCeContext());
    ...
    it('returns undefined and logs a warning when the saved object lookup throws', async () => {
      ...
      const result = await buildDefinition().getCeData('rule-missing', buildCeContext(logger));

      expect(result).toBeUndefined();
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("CE rule: failed to get data for 'rule-missing'")
      );
    });
  });
  ...
  describe('getPermissions', () => {
    it('returns the rules-read API privilege', () => {
      const permissions = buildDefinition().getPermissions!('rule-1', buildCeContext());
  ...
  describe('toAttachment', () => {
    const buildCeDocument = (overrides: Partial<{ origin_id: string }> = {}) => {
      ...
      return {
        ...
        title: 'High CPU',
        origin_id: originId,
        origin: { uri: `${RULE_CE_TYPE}://${originId}` },
        content: '',
    ...
    it('returns undefined when getRule throws', async () => {
      ...
      const result = await buildDefinition().toAttachment(
        buildCeDocument({ origin_id: 'rule-missing' }),
        buildToAttachmentContext()
      );
    ...
    it('returns undefined when the response fails schema parsing', async () => {
      ...
      const result = await buildDefinition().toAttachment(
        buildCeDocument({ origin_id: 'rule-broken' }),
        buildToAttachmentContext()
      );
