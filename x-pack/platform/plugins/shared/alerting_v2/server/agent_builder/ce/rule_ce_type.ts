...
export const createRuleCeType = ({
  ...
  getCeData: async (originId, context) => {
    try {
      ...
      context.logger.warn(
        `CE rule: failed to get data for '${originId}': ${(error as Error).message}`
      );
      return undefined;
    }
  },
