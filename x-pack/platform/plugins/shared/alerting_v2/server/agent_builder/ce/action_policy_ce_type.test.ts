...
import { ACTION_POLICY_ATTACHMENT_TYPE, ACTION_POLICY_CE_TYPE } from '@kbn/alerting-v2-schemas';
...
import { createActionPolicyCeType } from './action_policy_ce_type';
...
describe('createActionPolicyCeType', () => {
  ...
  describe('id and fetchFrequency', () => {
    it('uses the shared ACTION_POLICY_CE_TYPE constant', () => {
      expect(buildDefinition().id).toBe(ACTION_POLICY_CE_TYPE);
    });

    it('returns "1m" as fetch frequency', () => {
      // Pinned to '1m' to match rule_ce_type — bumping should be a deliberate decision.
  ...
  describe('getCeData', () => {
    it('returns a single entry built from action policy metadata', async () => {
      ...
      const result = await buildDefinition().getCeData('policy-1', buildCeContext());
    ...
    it('falls back to originId for title when attributes.name is missing', async () => {
      ...
      const result = await buildDefinition().getCeData('policy-bare', buildCeContext());
    ...
    it('returns undefined and logs a warning when the saved object lookup throws', async () => {
      // getCeData is called by the crawler per-origin — a single
      ...
      const result = await buildDefinition().getCeData('policy-missing', buildCeContext(logger));

      expect(result).toBeUndefined();
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("CE action policy: failed to get data for 'policy-missing'")
      );
    });
  });
  ...
  describe('getPermissions', () => {
    it('returns the action-policies-read API privilege', () => {
      ...
      // the CE entry MUST stamp the same privilege so a user without
      // it cannot see policy entries in agent context.
      //
      // Regression history: prior iterations of analogous CE types
      ...
      const permissions = buildDefinition().getPermissions!('policy-1', buildCeContext());
  ...
  describe('toAttachment', () => {
    const buildCeDocument = (overrides: Partial<{ origin_id: string }> = {}) => {
      ...
      return {
        ...
        title: 'Critical alerts → Slack',
        origin_id: originId,
        origin: { uri: `${ACTION_POLICY_CE_TYPE}://${originId}` },
        content: '',
    ...
    it('returns undefined when getActionPolicy throws', async () => {
      ...
      const result = await buildDefinition().toAttachment(
        buildCeDocument({ origin_id: 'policy-missing' }),
        buildToAttachmentContext()
      );
    ...
    it('uses an empty string when the CE document has no origin_id', async () => {
      // Defensive contract: `toAttachment` is invoked via the CE
      ...
      const document = buildCeDocument();
