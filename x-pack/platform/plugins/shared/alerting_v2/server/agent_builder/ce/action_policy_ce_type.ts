...
interface CreateActionPolicyCeTypeOptions {
  getScopedActionPolicyClient: (request: KibanaRequest) => ActionPolicyClient;
  getInternalRepository: () => ISavedObjectsRepository;
}

export const createActionPolicyCeType = ({
  getScopedActionPolicyClient,
  getInternalRepository,
}: CreateActionPolicyCeTypeOptions): CeTypeDefinition => ({
  id: ACTION_POLICY_CE_TYPE,
  fetchFrequency: () => '1m',
  ...
  getCeData: async (originId, context) => {
    try {
      ...
      context.logger.warn(
        `CE action policy: failed to get data for '${originId}': ${(error as Error).message}`
      );
      return undefined;
    }
  },
