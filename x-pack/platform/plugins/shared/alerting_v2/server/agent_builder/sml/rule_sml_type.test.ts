...
describe('createRuleSmlType', () => {
  ...
  describe('getContextEngineData', () => {
    it('returns a single entry built from rule metadata + query', async () => {
      ...
      const result = await buildDefinition().getContextEngineData('rule-1', buildSmlContext());
    ...
    it('falls back to originId for title when metadata.name is missing', async () => {
      ...
      const result = await buildDefinition().getContextEngineData('rule-bare', buildSmlContext());

      expect(result?.entries[0].title).toBe('rule-bare');
    });
