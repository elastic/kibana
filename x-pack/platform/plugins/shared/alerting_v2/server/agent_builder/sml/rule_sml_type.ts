...
import type { ContextEngineTypeDefinition } from '@kbn/context-engine-plugin/server';
import {
...
export const createRuleSmlType = ({
...
}: CreateRuleSmlTypeOptions): ContextEngineTypeDefinition => ({
  id: RULE_SML_TYPE,
  fetchFrequency: () => '1m',
  ...
  getContextEngineData: async (originId, context) => {
    try {
      const repository = getInternalRepository();
