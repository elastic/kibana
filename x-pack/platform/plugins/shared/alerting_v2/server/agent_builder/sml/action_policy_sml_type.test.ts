...
describe('createActionPolicySmlType', () => {
  ...
  describe('getContextEngineData', () => {
    it('returns a single entry built from action policy metadata', async () => {
      ...
      const result = await buildDefinition().getContextEngineData('policy-1', buildSmlContext());
    ...
    it('falls back to originId for title when attributes.name is missing', async () => {
      ...
      const result = await buildDefinition().getContextEngineData('policy-bare', buildSmlContext());

      expect(result?.entries[0].title).toBe('policy-bare');
    });

    it('returns undefined and logs a warning when the saved object lookup throws', async () => {
      // getContextEngineData is called by the crawler per-origin — a single
      ...
      const result = await buildDefinition().getContextEngineData(
        'policy-missing',
        buildSmlContext(logger)
      );
