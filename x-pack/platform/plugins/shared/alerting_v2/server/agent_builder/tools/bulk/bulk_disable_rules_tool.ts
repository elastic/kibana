...
import { ALERTING_V2_BULK_DISABLE_RULES_TOOL_ID, ALERTING_V2_TOOL_TAGS } from '../../tool_ids';
...
const schema = z
  .object({
    ids: z
      ...
      .min(1)
      .optional()
      .describe('Array of rule IDs to disable. Provide either ids or filter, not both.'),
    filter: z
      .string()
      .optional()
      .describe(
        'KQL filter to select rules for disabling. Provide either filter or ids, not both.'
      ),
  })
...
export const createBulkDisableRulesTool = ({
  ...
  handler: async ({ ids, filter }, { request }) => {
    try {
      ...
      const result = await rulesClient.bulkDisableRules(params);
