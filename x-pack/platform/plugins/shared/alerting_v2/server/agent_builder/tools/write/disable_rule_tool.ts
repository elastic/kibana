...
import { ALERTING_V2_DISABLE_RULE_TOOL_ID, ALERTING_V2_TOOL_TAGS } from '../../tool_ids';

const schema = z.object({
  rule_id: z.string().describe('The ID of the rule to disable.'),
});

export const createDisableRuleTool = ({
  getScopedRulesClient,
}: {
  getScopedRulesClient: GetScopedRulesClient;
}): BuiltinToolDefinition<typeof schema> => ({
  id: ALERTING_V2_DISABLE_RULE_TOOL_ID,
  type: ToolType.builtin,
  description: 'Disables an Alerting v2 rule, pausing its scheduled execution.',
  ...
  handler: async ({ rule_id }, { request }) => {
    try {
      ...
      const rule = await rulesClient.disableRule({ id: rule_id });
