...
Known limitation: any wildcard `DROP` (including patterns that cannot match the metadata, such as `DROP labels.*`) makes a rule ineligible, so it runs without deduplication. See `hasColumnMatching` in `deduplication_query.ts`.
