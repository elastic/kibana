...
| 7 | `FilterDuplicateEventsStep` | Drop breached events whose deterministic `_id` already exists in `.rule-events`. |
| 8 | `DirectorStep` | Enrich alert-type events with episode state. |
| 9 | `StoreAlertEventsStep` | Persist the batch into `.rule-events`, using the deterministic `_id` where one applies. |
