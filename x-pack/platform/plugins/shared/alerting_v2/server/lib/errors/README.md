...
| `INVALID_RULE_QUERY_CONFIG`         | 400    | An update would leave `recovery` unusable against the merged `query` (a `condition` strategy without `query.breach`, or a segment that does not compose onto `query.base`) | `{ rule_id }`                              |
| `INVALID_STATE_TRANSITION_CONFIG`   | 400    | A recovery delay (`state_transition.recovering`) is set while `recovery.strategy` is `manual`, where it is inert                    | `{ rule_id }`                              |
