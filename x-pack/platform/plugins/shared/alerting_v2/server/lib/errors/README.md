...
| `MAX_SCHEDULES_PER_MINUTE_EXCEEDED` | 400    | Scheduling the rule (or the enabled subset of a bulk create) would exceed `xpack.alerting_v2.rules.maxScheduledPerMinute`           | `{ interval?, maxScheduledPerMinute }`     |
...
| Code                               | Status               | When                                                                                                                                                                                                                                                     | `details`                                                 |
| ---------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `ALERT_EVENT_NOT_FOUND`            | 404                  | No alert event matches the supplied `group_hash` (+ optional `alert_id`). Single-action route.                                                                                                                                                          | `{ group_hash, alert_id? }`                               |
...
| `ALERT_NOT_FOUND`                  | 200 (per-item)       | Bulk-only. The item's `group_hash` resolved, but its `alert_id` did not match the group's latest alert (superseded). Reported per item in the `errors[]` array.                                                                                         | `{ group_hash, alert_id }`                                |
| `INVALID_ALERT_STATE_TRANSITION`   | 400 / 200 (per-item) | The requested action is a no-op against the current lifecycle state: `activate` of an already-`active` alert, or `deactivate` of an already-`inactive` alert. Every other transition is allowed. Single route returns 400; bulk reports it per item.    | `{ group_hash, alert_id, alert_status, action_type }`     |
