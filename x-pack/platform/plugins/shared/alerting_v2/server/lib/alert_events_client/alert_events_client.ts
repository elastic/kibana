...
import { internalCreateAlertEventDataSchema } from '@kbn/alerting-v2-schemas';
...
export class AlertEventsClient {
  ...
  public async createAlertEvent(
    ...
    // Validate input on the plugin-client path. This catches missing identity
    // fields, oversized `data`, etc., while allowing `elastic*` sources so
    // first-party plugins can use their natural source names.
    const parsed = internalCreateAlertEventDataSchema.parse(event);

    const { source } = parsed;
    const groupHash = getGroupHash(parsed, this.spaceId);

    const episodeStatus = parsed.alert_status ?? alertEpisodeStatus.active;
    const episodeId = await this.resolveEpisodeId(groupHash, episodeStatus, abortSignal);

    const atTimestamp = parsed.timestamp ?? new Date().toISOString();
