...
export class MaintenanceWindowService implements MaintenanceWindowServiceContract {
  ...
  public async getEnabledMaintenanceWindows(): Promise<ActiveMaintenanceWindow[]> {
    ...
    try {
        ...
        for (const doc of response.saved_objects) {
          ...
          if (!Array.isArray(doc.attributes.events)) {
            this.logger.warn({
              ...
              code: ALERTING_V2_LOG_CODES.MAINTENANCE_WINDOW_DOCUMENT_INVALID,
            });
            continue;
          }
      ...
      this.logger.error({
        error,
        code: ALERTING_V2_LOG_CODES.MAINTENANCE_WINDOW_FETCH_FAILED,
      });
      return [];
    } finally {
      try {
        ...
        this.logger.warn({
          ...
          code: ALERTING_V2_LOG_CODES.MAINTENANCE_WINDOW_PIT_CLOSE_FAILED,
        });
      }
    }
