 ...
 * ({@link InternalRule}, the public API shape plus the version counter) plus
 * the envelope fields that are not part of the rule itself (`spaceId`,
 ...
 * uses the full `rule` as its snapshot and its `version` as the sequence.
 */
export interface RuleEventPayload {
  /** Rule id. Always present, even when the full `rule` could not be resolved. */
  readonly ruleId: string;
  /** Kibana space the rule lives in. Not part of {@link InternalRule}. */
