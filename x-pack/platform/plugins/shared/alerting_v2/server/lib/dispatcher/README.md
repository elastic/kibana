...
- A group whose every destination fails gets **no** `fire`/`notified` record (previously it was recorded as fired despite delivery failing). Its episodes are re-fetched and re-dispatched on subsequent ticks while they remain in the scan window, then drop out of scope; each failed attempt emits `dispatch_failed` history events. This is deliberate: transient failures (e.g. Workflow Engine unavailable) get free retries via the overlap re-read, at the cost of one retry per tick — and repeated `dispatch_failed` events — for persistently failing destinations (e.g. a disabled or deleted workflow) until the events age out of the scan window. Possible follow-ups if that retry noise becomes a problem (none implemented):
  - **Terminal record after N attempts** — count failed attempts per group (e.g. from prior failure records) and write a terminal `.alert-actions` record once a retry budget is exhausted, so the `last_fired` dedup mark moves past the group.
  - **Classify failure reasons** — permanent reasons (`workflow_not_found`, `workflow_disabled`, `missing_api_key`) write a terminal record immediately; only transient reasons (`schedule_error`) are retried.
  - **Retry backoff** — persist a `dispatch_failed`-style action record with the attempt timestamp and skip re-dispatch until a backoff interval has elapsed, instead of retrying every tick.
  - **Per-destination fire records** — record delivery per destination rather than per group, so a partially failed group retries only its failed destinations instead of relying on the first-success group-level record.
