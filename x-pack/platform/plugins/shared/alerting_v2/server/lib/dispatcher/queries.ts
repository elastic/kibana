...
export const getDispatchableAlertEventsQuery = (): EsqlRequest => {
  ...
  return esql`FROM ${ALERT_EVENTS_DATA_STREAM},${ALERT_ACTIONS_DATA_STREAM} METADATA _index, _source
      | WHERE (TO_STRING(_index) LIKE ${ALERT_ACTIONS_BACKING_INDEX}) OR (TO_STRING(_index) LIKE ${ALERT_EVENTS_BACKING_INDEX} and type == ${alertEventType})
      | EVAL
      ...
      | STATS
          last_event_timestamp = MAX(@timestamp) WHERE TO_STRING(_index) LIKE ${ALERT_EVENTS_BACKING_INDEX},
          last_episode_status = LAST(episode_status, @timestamp) WHERE TO_STRING(_index) LIKE ${ALERT_EVENTS_BACKING_INDEX},
          data_json = LAST(data_json, @timestamp) WHERE TO_STRING(_index) LIKE ${ALERT_EVENTS_BACKING_INDEX}
