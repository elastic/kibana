...
export const CreateNewWorkflowSubform = ({ value, onChange }: CreateNewWorkflowSubformProps) => {
  ...
  const handleTypeChange = (nextTypeId: SingleStepWorkflowTypeId) => {
    if (nextTypeId === value.typeId) return;
    const nextType = getSingleStepWorkflowType(nextTypeId);
    if (!nextType) return;
    onChange({ ...value, typeId: nextTypeId, connectorId: null, params: nextType.paramsTemplate });
  };

  const handleConnectorChange = (connectorId: string | null) => {
