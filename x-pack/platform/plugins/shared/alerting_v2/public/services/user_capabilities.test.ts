...
describe('UserCapabilities', () => {
  describe('can', () => {
    it('returns true when the capability key is granted', () => {
      const caps = buildService({ [ALERTING_V2_FEATURE_IDS.rules]: { read: true, all: false } });
    ...
    it('returns false when the capability key is missing from the feature block', () => {
      const caps = buildService({ [ALERTING_V2_FEATURE_IDS.rules]: {} });
  ...
  describe('canRead', () => {
    it('maps to the top-level read capability for a feature', () => {
      const caps = buildService({ [ALERTING_V2_FEATURE_IDS.alerts]: { read: true, all: false } });
      expect(caps.canRead('alerts')).toBe(true);
    });

    it('returns true when only the all capability is granted (all implies read)', () => {
      const caps = buildService({ [ALERTING_V2_FEATURE_IDS.alerts]: { read: false, all: true } });
      expect(caps.canRead('alerts')).toBe(true);
    });

    it('returns false when neither read nor all is granted', () => {
      const caps = buildService({ [ALERTING_V2_FEATURE_IDS.alerts]: { read: false, all: false } });
