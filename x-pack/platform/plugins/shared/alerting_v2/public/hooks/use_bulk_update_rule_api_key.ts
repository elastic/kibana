...
import { RulesApi, type BulkByIdsParams } from '../services/rules_api';
import { addBulkMutationDangerToast } from './bulk_mutation_toasts';
import { ruleKeys } from './query_key_factory';

/**
 * Rotates the executor task API key for the given rules to one derived from the
 * current user's credentials. Backs both the single-rule (details page) and
 * bulk (rules list) "Update API key" actions — the single case passes a
 * one-element `ids` array.
 */
export const useBulkUpdateRuleApiKey = () => {
  ...
  return useMutation({
    mutationFn: (params: BulkByIdsParams) => rulesApi.bulkUpdateRuleApiKey(params),
