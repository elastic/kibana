...
export const useDisableNotificationPolicy = () => {
  ...
  return useMutation<NotificationPolicyResponse, Error, string>({
    mutationKey: notificationPolicyKeys.disable(),
    mutationFn: (id) => notificationPoliciesApi.disableNotificationPolicy(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: notificationPolicyKeys.lists(), exact: false });
      toasts.addSuccess(
        i18n.translate('xpack.alertingV2.notificationPolicy.disableSuccess', {
          defaultMessage: 'Notification policy disabled',
        })
      );
    },
    onError: (error) => {
      toasts.addError(error, {
        title: i18n.translate('xpack.alertingV2.notificationPolicy.disableError', {
          defaultMessage: 'Failed to disable notification policy',
        }),
      });
    },
  });
