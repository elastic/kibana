...
describe('useEnableRule', () => {
  ...
  it('should enable a rule, show a success toast, and invalidate the query cache', async () => {
    ...
    const { result } = renderHook(() => useEnableRule(), { wrapper: createWrapper() });
  ...
  it('should show an error toast when enabling fails', async () => {
  ...
  it('should pass the correct rule id through to the API', async () => {
    ...
    result.current.mutate({ id: 'custom-id-456' });
