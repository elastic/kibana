...
import { useEnableRule } from './use_enable_rule';
...
describe('useEnableRule', () => {
  ...
  beforeEach(() => {
    ...
    mockUseService.mockImplementation((service: unknown) => {
      if (service === RulesApi) {
        return { enableRule: mockEnableRule } as any;
      }
  ...
  it('should enable a rule, show a success toast, and invalidate the query cache', async () => {
  ...
  it('should show an error toast and not invalidate queries when enabling fails', async () => {
    const error = new Error('enable failed');
    mockEnableRule.mockRejectedValue(error);
    const { result } = renderHook(() => useEnableRule());
  ...
  it('should pass the correct rule id through to the API and query key', async () => {
    ...
    await result.current.enableRule('custom-id-456');

    expect(mockEnableRule).toHaveBeenCalledWith('custom-id-456');
    expect(mockInvalidateQueries).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: ['rule', 'custom-id-456'] })
    );
  });
});
