...
describe('RulesListPage', () => {
  ...
  it('renders loading state', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId(CONTENT_LIST_TEST_SUBJECTS.tableSkeleton)).toBeInTheDocument();
    });
  ...
  it('uses title ascending as the default sort', async () => {
    renderPage();
    await waitForRules();

    // Column.Name sorts by `title`; the data source maps that to API `name`.
    expect(lastFindItemsArgs().sort).toEqual({ field: 'title', direction: 'asc' });
  });
  ...
  it('toggles name sort direction when the Name header is clicked', async () => {
    ...
    await waitFor(() => {
      expect(lastFindItemsArgs().sort).toEqual({ field: 'title', direction: 'desc' });
    });
  });
  ...
  describe('when the user only has read privilege', () => {
    ...
    it('hides row selection, quick edit, and actions menu affordances', async () => {
      ...
      expect(screen.queryByTestId('checkboxSelectAll')).not.toBeInTheDocument();
      expect(
        screen.queryByTestId('checkboxSelectRow-content-list-table-row-rule-1')
