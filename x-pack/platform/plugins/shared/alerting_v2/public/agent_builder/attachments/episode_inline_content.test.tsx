...
describe('createEpisodeAttachmentDefinition', () => {
  it('uses the episode label as the label', () => {
    const definition = createEpisodeAttachmentDefinition();
    expect(
      definition.getLabel({
        ...createAttachment(),
        data: { ...createAttachment().data, 'episode.label': 'Host CPU high alert' },
      } as any)
    ).toBe('Host CPU high alert');
  });

  it('falls back to "Alert episode" when no episode label is available', () => {
    const definition = createEpisodeAttachmentDefinition();
