...
const defineRuleExecutorSuite = (responseFormat: EsqlResponseFormat) => {
  ...
  apiTest.describe(
    ...
    () => {
      ...
      apiTest(
        'groups multiple documents into the same group correctly',
        async ({ apiServices }) => {
          await apiServices.alertingV2.sourceIndex.indexDocs({
            index: SOURCE_INDEX,
            docs: [
      ...
      apiTest(
        ...
        async ({ apiServices }) => {
          ...
          const hashesByHost = new Map<string, string>(
            breachedEventsBefore.map((event) => [
      ...
      apiTest(
        'writes zero events when the ES|QL query returns no rows',
        async ({ apiServices }) => {
          const rule = await apiServices.alertingV2.rules.create(
            buildCreateRuleData({
...
defineRuleExecutorSuite('json');
defineRuleExecutorSuite('arrow');
