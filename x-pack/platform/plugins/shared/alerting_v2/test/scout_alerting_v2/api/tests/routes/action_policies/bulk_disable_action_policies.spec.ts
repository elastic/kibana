...
apiTest.describe('Bulk disable action policies API', { tag: '@local-stateful-classic' }, () => {
  ...
  apiTest('disable: disables enabled policies', async ({ apiClient, apiServices }) => {
    ...
    const response = await apiClient.post(getBulkDisableActionPoliciesUrl(), {
  ...
  apiTest(
    'disable: reports a per-item error for a non-existent id',
    async ({ apiClient, apiServices }) => {
      const existing = await apiServices.alertingV2.actionPolicies.create(
        buildCreateActionPolicyData({ name: 'bulk-disable-partial' })
      );
  ...
  apiTest(
    ...
    async ({ apiClient, apiServices }) => {
      const created = await apiServices.alertingV2.actionPolicies.create(
        buildCreateActionPolicyData({ name: 'writer-bulk-disable' })
      );
  ...
  apiTest(
    ...
    async ({ apiClient, apiServices, requestAuth }) => {
      ...
      const created = await apiServices.alertingV2.actionPolicies.create(
        buildCreateActionPolicyData({ name: 'reader-bulk-disable' })
      );
  ...
  apiTest(
    ...
    async ({ apiClient, apiServices, requestAuth }) => {
      ...
      const created = await apiServices.alertingV2.actionPolicies.create(
        buildCreateActionPolicyData({ name: 'no-access-bulk-disable' })
      );
