...
apiTest.describe('Update rule API', { tag: '@local-stateful-classic' }, () => {
  ...
  apiTest(
    'validation: should reject body with unknown keys inside schedule (strict schema)',
    async ({ apiClient, apiServices }) => {
      const created = await apiServices.alertingV2.rules.create(
        buildCreateRuleData({ metadata: { name: 'rule-strict-schedule' } })
      );
      const response = await apiClient.patch(getRuleUrl(created.id), {
        headers: writerHeaders,
        body: { schedule: { every: '5m', extra: true } },
      });
      expect(response).toHaveStatusCode(400);
    }
  );
