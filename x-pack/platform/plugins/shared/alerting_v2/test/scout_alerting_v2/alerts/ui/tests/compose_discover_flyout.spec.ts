...
const TIMESTAMP_ONLY_BASE_QUERY = `FROM ${TIMESTAMP_ONLY_INDEX} | STATS count = COUNT(*) BY Carrier`;
const TIMESTAMP_ONLY_BREACH_SEGMENT = 'WHERE count > 100';
// An independent recovery query is the one shape the visual builder cannot author.
const TIMESTAMP_ONLY_RECOVERY_QUERY = `${TIMESTAMP_ONLY_BASE_QUERY} | WHERE count <= 100`;
...
const YAML_ONLY_RULE_NAME = 'scout-compose-discover-yaml-only-recovery-query';
...
test.describe(
  ...
  () => {
    ...
    test('edit flow (sad path): broken composed alert can select timestamp and save', async ({
      pageObjects,
      apiServices,
    }) => {
      let ruleId: string;

      await test.step('seed a composed alert with an invalid stored time field', async () => {
        const rule = await apiServices.alertingV2.rules.create(
          buildCreateRuleData({
            kind: 'alert',
            query: {
    ...
    test('edit flow: alert + recovery query opens YAML-only', async ({
      pageObjects,
      apiServices,
    }) => {
      let ruleId: string;

      await test.step('seed an alert rule with an independent recovery query via API', async () => {
        const rule = await apiServices.alertingV2.rules.create(
          buildCreateRuleData({
            kind: 'alert',
            query: { base: `${TIMESTAMP_ONLY_BASE_QUERY} | ${TIMESTAMP_ONLY_BREACH_SEGMENT}` },
            recovery: { strategy: 'query', query: TIMESTAMP_ONLY_RECOVERY_QUERY },
            time_field: 'timestamp',
            grouping: { fields: ['Carrier'] },
            metadata: { name: YAML_ONLY_RULE_NAME },
          })
        );
        ruleId = rule.id;
      });
    ...
    test('edit flow (YAML-only): removing all tags in the editor persists', async ({
      ...
      await test.step('seed an alert rule with tags and a recovery query (opens YAML-only)', async () => {
        const rule = await apiServices.alertingV2.rules.create(
          buildCreateRuleData({
            kind: 'alert',
            query: { base: `${TIMESTAMP_ONLY_BASE_QUERY} | ${TIMESTAMP_ONLY_BREACH_SEGMENT}` },
    ...
    test('step validation: Next disabled without query, name validation blocks advancement', async ({
      ...
      await test.step('Next is now enabled; advance through Outcome to the Details step', async () => {
        await expect(pageObjects.composeDiscover.nextButton).toBeEnabled();
        await pageObjects.composeDiscover.clickNext(); // Outcome
        await pageObjects.composeDiscover.clickNext(); // Details
      });
    ...
    test('alert condition validation: base-only query shows the no-alert-condition callout and keeps Next enabled', async ({
      ...
      await test.step('Next stays enabled — form progression is not gated on a condition', async () => {
        await expect(pageObjects.composeDiscover.nextButton).toBeEnabled();
      });
    });
