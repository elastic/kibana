...
apiTest.describe(
  'open anomaly detector job with spaces',
  ...
  () => {
    ...
    apiTest(
      'ML poweruser can open a job from the correct space',
      async ({ apiClient, samlAuth, apiServices }) => {
        const jobId = 'fq_single_open_correct_space';
        ...
        const res = await apiClient.post(
          `s/${SPACE_1}/internal/ml/anomaly_detectors/${jobId}/_open`,
    ...
    apiTest(
      'ML poweruser cannot open a job from the wrong space',
      async ({ apiClient, samlAuth, apiServices }) => {
        const jobId = 'fq_single_open_wrong_space';
        const { cookieHeader } = await samlAuth.asMlPoweruser();
        ...
        const res = await apiClient.post(
          `s/${SPACE_2}/internal/ml/anomaly_detectors/${jobId}/_open`,
          { headers: { ...INTERNAL_API_HEADERS, ...cookieHeader } }
        );

        expect(res).toHaveStatusCode(404);

        // the rejected request must leave the job closed, not open it as a side effect
