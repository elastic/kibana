...
apiTest.describe(
  ...
  () => {
    ...
    apiTest('GET job by ID from correct space returns the job', async ({ apiClient, samlAuth }) => {
      ...
      const res = await apiClient.get(`s/${SPACE_1}/internal/ml/anomaly_detectors/${JOB_ID}`, {
    ...
    apiTest('GET job by ID from wrong space returns 404', async ({ apiClient, samlAuth }) => {
      ...
      const res = await apiClient.get(`s/${SPACE_2}/internal/ml/anomaly_detectors/${JOB_ID}`, {
        headers: { ...INTERNAL_API_HEADERS, ...cookieHeader },
        responseType: 'json',
      });
    ...
    apiTest(
      ...
      async ({ apiClient, samlAuth }) => {
        ...
        const res = await apiClient.get(
          `s/${SPACE_1}/internal/ml/anomaly_detectors/fq_single_get_space*`,
          {
            headers: { ...INTERNAL_API_HEADERS, ...cookieHeader },
            responseType: 'json',
          }
    ...
    apiTest(
      'GET job by wildcard from wrong space returns empty results',
      async ({ apiClient, samlAuth }) => {
        const { cookieHeader } = await samlAuth.asMlPoweruser();

        const res = await apiClient.get(
          `s/${SPACE_2}/internal/ml/anomaly_detectors/fq_single_get_space*`,
          {
            headers: { ...INTERNAL_API_HEADERS, ...cookieHeader },
            responseType: 'json',
          }
    ...
    apiTest(
      ...
      async ({ apiClient, samlAuth }) => {
        ...
        const res = await apiClient.get(`s/${SPACE_1}/internal/ml/anomaly_detectors/${GROUP_ID}`, {
          headers: { ...INTERNAL_API_HEADERS, ...cookieHeader },
          responseType: 'json',
        });
    ...
    apiTest(
      'GET jobs by group from wrong space returns no results for that job',
      async ({ apiClient, samlAuth }) => {
        const { cookieHeader } = await samlAuth.asMlPoweruser();

        const res = await apiClient.get(`s/${SPACE_2}/internal/ml/anomaly_detectors/${GROUP_ID}`, {
          headers: { ...INTERNAL_API_HEADERS, ...cookieHeader },
          responseType: 'json',
        });
