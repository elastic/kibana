...
test.describe(
  ...
  () => {
    ...
    // Destination index and data view are cleaned up per run
    ...
    test(
      'iowa house prices outlier detection: full creation journey',
      async ({
        page,
        browserAuth,
        ...
        await test.step('navigates through the wizard and sets all needed fields', async () => {
          ...
          // Source data preview
          await expect(
            page.testSubj.locator('mlAnalyticsCreationDataGrid loaded')
          ...
          for (const chart of testData.expected.histogramCharts) {
            await expect(
              page.testSubj.locator(`mlDataGridChart-${chart.id}`)
            ).toBeVisible({ timeout: 10_000 });
          }

          // Include fields selection
          await expect(
            page.testSubj.locator('mlAnalyticsCreateJobWizardIncludesTable')
          ).toBeVisible({ timeout: 8_000 });

          // Field stats flyout from include field trigger (field '1stFlrSF')
          await page.testSubj.locator('mlAnalyticsCreateJobWizardIncludesSelect').scrollIntoViewIfNeeded();
          await page.testSubj
            .locator('~mlAnalyticsCreateJobWizardIncludesSelect > ~mlInspectFieldStatsButton-1stFlrSF')
            .click();
          await expect(
            page.testSubj.locator('~mlFieldStatsFlyoutContent')
          ...
          await expect(
            page.testSubj.locator('mlAnalyticsCreateJobFlyoutJobIdInput')
          ).toBeVisible();
          await dataFrameAnalytics.setJobId(testData.jobId);

          await expect(
            page.testSubj.locator('mlDFAnalyticsJobCreationJobDescription')
          ).toBeVisible();
          ...
          expect(await sameAsIdSwitch.getAttribute('aria-checked')).toBe('true');
          ...
          expect(await createDataViewSwitch.getAttribute('aria-checked')).toBe('true');
          ...
          await expect(page.testSubj.locator('~mlValidationCallout').first()).toBeVisible();
          ...
          expect(
            await page.testSubj
              .locator('mlAnalyticsCreateJobWizardStartJobSwitch')
        ...
        await test.step(
          ...
          async () => {
            ...
            await expect
              .poll(
                async () => (await dataFrameAnalytics.getRowData(testData.jobId)).status,
            ...
            await row
              .locator('[data-test-subj="mlAnalyticsTableRowDetailsToggle"]')
              .click();

            const details = page.testSubj.locator(
              `mlAnalyticsTableRowDetails-${testData.jobId}`
            );
            await expect(details).toBeVisible();

            // Verify 3 detail tabs exist
            await expect(
              page.testSubj.locator('~mlAnalyticsTableRowDetailsTab')
            ).toHaveCount(3);
            ...
            for (const section of ['state', 'stats', 'counts', 'progress', 'analysisStats']) {
              await expect(
                details.locator(`[data-test-subj~="mlAnalyticsTableRowDetailsSection"][data-test-subj~="${section}"]`)
              ).toBeVisible({ timeout: 5_000 });
            }

            // State section should report 'stopped'
            await expect(
              details.locator('[data-test-subj~="mlAnalyticsTableRowDetailsSection"][data-test-subj~="state"]')
            ).toContainText('stopped');

            // Counts section should show expected training document count
            await expect(
              details.locator('[data-test-subj~="mlAnalyticsTableRowDetailsSection"][data-test-subj~="counts"]')
            ).toContainText('1460');
        ...
        await test.step(
          ...
          async () => {
            ...
            await expect(
              page.testSubj.locator('mlAnalyticsEditFlyoutDescriptionInput')
            ).toBeVisible();
            ...
            await expect(
              page.testSubj.locator('mlDFExpandableSection-results')
            ).toBeVisible();
            await expect(
              page.testSubj.locator('mlExplorationDataGrid loaded')
            ).toBeVisible({ timeout: 10_000 });
            ...
            await page.testSubj
              .locator('mlScatterplotMatrixSampleSizeSelect')
              .selectOption('10000');
            const scatterSwitch = page.testSubj.locator(
        ...
        await test.step('displays the analytics job in the map view', async () => {
          ...
          await expect(
            page.testSubj.locator('mlPageDataFrameAnalyticsMap')
          ).toBeVisible();
          await expect(
            page.testSubj.locator('mlPageDataFrameAnalyticsMapLegend')
          ).toBeVisible();
          await expect(
            page.testSubj.locator('mlPageDataFrameAnalyticsMapGraph')
          ).toBeVisible();
