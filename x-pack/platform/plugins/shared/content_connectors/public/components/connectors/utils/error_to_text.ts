...
export const errorToText = (error?: HttpError): string | undefined => {
  ...
  if (error.body?.attributes?.error_code === ErrorCode.INDEX_ALREADY_EXISTS) {
    return i18n.translate(
      'xpack.contentConnectors.content.newIndex.steps.buildConnector.error.indexAlreadyExists',
      {
  ...
  if (error.body?.attributes?.error_code === ErrorCode.CONNECTOR_DOCUMENT_ALREADY_EXISTS) {
    return i18n.translate(
      'xpack.contentConnectors.content.newIndex.steps.buildConnector.error.connectorAlreadyExists',
      {
  ...
  if (error?.body?.statusCode === 403) {
    return i18n.translate(
      'xpack.contentConnectors.content.newIndex.steps.buildConnector.error.unauthorizedError',
      {
    ...
    return i18n.translate(
      'xpack.contentConnectors.content.newIndex.steps.buildConnector.error.genericError',
      {
