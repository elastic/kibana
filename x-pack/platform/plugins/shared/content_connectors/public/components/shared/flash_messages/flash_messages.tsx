...
const flashCalloutComponents: Record<
  FlashMessageTypes,
  | typeof KbnInfoCallout
  | typeof KbnSuccessCallout
  | typeof KbnWarningCallout
  | typeof KbnDangerCallout
> = {
  ...
  return (
    <div aria-live="polite" data-test-subj="FlashMessages">
      {messages.map(({ type, message, description }, index) => {
        const CalloutComponent = flashCalloutComponents[type] ?? KbnInfoCallout;
        return (
          <Fragment key={index}>
            <CalloutComponent title={message} text={description} />
            <EuiSpacer />
          </Fragment>
        );
      })}
