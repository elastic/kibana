...
const requestSchema = z
  ...
  .superRefine((request, context) => {
    ...
    const hostTemplates = [request.url, request.baseUrl]
      .filter((value): value is string => typeof value === 'string')
      .flatMap((value) =>
        [...value.matchAll(/\{\{\s*([a-zA-Z0-9_.]+)\s*\}\}/g)].map((match) => match[1])
      );
    for (const templatePath of hostTemplates) {
      if (!templatePath.startsWith('config.')) {
        context.addIssue({
          code: 'custom',
          message: 'url and baseUrl templates may only reference config.*',
          path: request.url !== undefined ? ['url'] : ['baseUrl'],
        });
      }
    }
  });
...
export const parseCatalogContract = (raw: string): CatalogContractSpec => {
  let parsed: unknown;
  try {
  ...
  try {
    return connectorSpecSchema.parse(parsed) as CatalogContractSpec;
  } catch (error) {
...
/** @deprecated Use parseCatalogContract. */
export const parseDeclarativeConnectorSpec = parseCatalogContract;
