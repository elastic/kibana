...
describe('executeDeclarativeRequest', () => {
  ...
  it('percent-encodes each templated path value as one segment', async () => {
    const requestMock = jest.fn().mockResolvedValue(response({ ok: true }));
    const request: DeclarativeRequest = {
      method: 'GET',
      baseUrl: '{{ config.baseUrl }}',
      path: '/check/{{ input.ip }}/detail',
    };
    ...
    await executeDeclarativeRequest({
      ...
      connector,
      request,
      input: { ip: '192.0.2.1/x?y#z' },
    });

    expect(requestMock).toHaveBeenCalledWith(
      expect.objectContaining({
        url: 'https://example.test/check/192.0.2.1%2Fx%3Fy%23z/detail',
      })
    );
  });
});
