...
const renderPathSegmentTemplates = (path: string, context: TemplateContext): string => {
  const exactMatch = path.match(EXACT_TEMPLATE);
  if (exactMatch) {
    const resolved = resolveTemplatePath(exactMatch[1], context);
    if (resolved === undefined || resolved === null) {
      return '';
    }
    return encodeURIComponent(String(resolved));
  }
  return path.replace(EMBEDDED_TEMPLATE, (_, templatePath: string) =>
    encodeURIComponent(String(resolveRequiredTemplatePath(templatePath, context)))
  );
};
  ...
  if (typeof renderedUrl === 'string') {
    ...
    const renderedPath =
      typeof request.path === 'string'
        ? renderPathSegmentTemplates(request.path, templateContext)
        : undefined;
