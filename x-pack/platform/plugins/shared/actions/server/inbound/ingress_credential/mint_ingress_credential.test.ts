...
describe('mintIngressCredential', () => {
  ...
  it('deletes the previous credential for the connector before minting', async () => {
    unsecuredSavedObjectsClient.find.mockResolvedValue({
      saved_objects: [
        {
          id: 'old-cred',
  ...
  it('does not create when deleting the previous credential fails', async () => {
    unsecuredSavedObjectsClient.find.mockResolvedValue({
      saved_objects: [
        {
          id: 'old-cred',
