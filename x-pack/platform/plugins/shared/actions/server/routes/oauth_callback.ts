    ...
    router.handleLegacyErrors(
      verifyAccessAndContext(licenseState, async function (context, req, res) {
        ...
        try {
          ...
          // Get saved objects client for reading/writing OAuth state and tokens
          const unsecuredSavedObjectsClient = core.savedObjects.getClient({
            includedHiddenTypes: ['oauth_state', 'connector_token'],
          });
          ...
          // Store tokens
          const connectorTokenEncryptedClient = encryptedSavedObjects.getClient({
            includedHiddenTypes: ['connector_token'],
          });

          const connectorTokenClient = new ConnectorTokenClient({
            encryptedSavedObjectsClient: connectorTokenEncryptedClient,
            unsecuredSavedObjectsClient,
            logger: routeLogger,
          });
