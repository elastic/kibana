...
describe('UserConnectorTokenClient', () => {
  describe('create()', () => {
    ...
    test('persists userCloudId when provided to create', async () => {
      const expiresAt = new Date().toISOString();
      ...
      await userClient.create({
        userIdentifiers: { profileUid: 'user-profile-123', userCloudId: 'cloud-user-123' },
        connectorId: '123',
        credentials: {
          accessToken: 'testtokenvalue',
        },
  ...
  describe('createWithRefreshToken()', () => {
    ...
    test('persists userCloudId on attributes when provided with profileUid', async () => {
      const expiresAt = new Date(Date.now() + 3600 * 1000).toISOString();
      ...
      await userClient.createWithRefreshToken({
        userIdentifiers: { profileUid: 'user-profile-123', userCloudId: 'cloud-abc' },
        connectorId: '123',
        accessToken: 'Bearer testtokenvalue',
        refreshToken: 'testrefreshtoken',
        expiresIn: 3600,
  ...
  describe('deleteConnectorTokens()', () => {
    ...
    test('does not delete when only userCloudId is provided without profileUid', async () => {
      await userClient.deleteConnectorTokens({
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'Skipping deleteConnectorTokens for user_connector_token: userCloudId without profileUid is not supported for scoped delete'
        )
      );
    });
