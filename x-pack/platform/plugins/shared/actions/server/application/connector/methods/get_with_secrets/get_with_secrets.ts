...
export async function getWithSecrets({
  ...
  // First read: space-scoped via unsecuredSavedObjectsClient. This enforces the space
  // boundary check that getDecryptedAsInternalUser (below) bypasses, because AsInternalUser
  // ignores the caller's namespace. Do not remove this read — doing so would allow a connector
  // in space A to be read from space B.
  const result = await getConnectorSo({
    unsecuredSavedObjectsClient: context.unsecuredSavedObjectsClient,
    id,
  });
  const authMode = getAuthMode(
    result.attributes.authMode as ConnectorWithSecrets['authMode'] | undefined
  );
  ...
  // Second read: decrypts secrets. AsInternalUser is required because the secrets field is
  // encrypted and not readable via the normal saved-object client. The space check above
  // ensures this call is scoped to the correct namespace.
  const namespace =
