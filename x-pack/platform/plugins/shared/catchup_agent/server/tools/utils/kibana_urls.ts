...
function buildFullUrl(
  ...
  if (publicBaseUrl) {
    ...
    return `${publicBaseUrl}${spacePrefix}${serverBasePath}${path}`;
  }
  ...
  const spacePrefix = spaceId !== 'default' ? `/s/${spaceId}` : '';
  const fullPath = `${spacePrefix}${serverBasePath}${path}`;
...
export function getAttackDiscoveryUrl(
  ...
  try {
    const spaceId = getSpaceId(request);
    const path = `${APP_ROUTES.security}/attack_discovery?id=${encodeURIComponent(
      attackDiscoveryId
    )}`;
    return buildFullUrl(request, core, spaceId, path);
  } catch (error) {
...
export function getAlertUrl(
  ...
  try {
    const spaceId = getSpaceId(request);
    const path = `${APP_ROUTES.security}/alerts/redirect/${encodeURIComponent(
      alertId
...
export function getRuleUrl(request: KibanaRequest, core: CoreStart, ruleId: string): string | null {
  try {
    const spaceId = getSpaceId(request);
    const path = `${APP_ROUTES.security}/rules/id/${encodeURIComponent(ruleId)}`;
    return buildFullUrl(request, core, spaceId, path);
  } catch (error) {
    return null;
  }
...
export function getAlertsPageUrl(
  ...
  try {
    ...
    const path = `${APP_ROUTES.security}/alerts?timerange=${encodeURIComponent(timerange)}`;
    return buildFullUrl(request, core, spaceId, path);
  } catch (error) {
    return null;
  }
