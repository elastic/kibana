...
describe('buildSecurityApi', () => {
  ...
  describe('workload bindings', () => {
    ...
    it('delegates getLoopbackAuthHeaders', async () => {
      await api.serviceAccounts.getWorkloadLoopbackAuthHeaders('alerting_rule', WORKLOAD);

      expect(serviceAccounts!.workloads.getLoopbackAuthHeaders).toHaveBeenCalledWith(
        'alerting_rule',
        WORKLOAD
      );
    });
    ...
    it.each([
      ...
      [
        ...
        () => api.serviceAccounts.getWorkloadLoopbackAuthHeaders('alerting_rule', WORKLOAD),
      ],
