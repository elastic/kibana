...
describe('ServiceAccountsService', () => {
  ...
  describe('#start', () => {
    ...
    it('selects the UIAM backend when UIAM and project context are available', async () => {
      ...
      await expect(start.getLoopbackAuthHeaders('service-account-id')).resolves.toEqual(
        expect.objectContaining({ authorization: 'Bearer essu_token' })
      );
    ...
    it('selects the Elasticsearch backend off serverless', async () => {
      ...
      await expect(start.getLoopbackAuthHeaders('service-account-id')).rejects.toMatchObject({
