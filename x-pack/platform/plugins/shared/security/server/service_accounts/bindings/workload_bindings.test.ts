...
describe('ServiceAccountWorkloadBindings', () => {
  ...
  beforeEach(() => {
    ...
    backend = {
      ...
      getLoopbackAuthHeaders: jest
        .fn()
        .mockResolvedValue({ authorization: 'Bearer essu_loopback' }),
  ...
  describe('#getLoopbackAuthHeaders', () => {
    it('verifies the binding, then mints for the bound service account', async () => {
      await expect(bindings.getLoopbackAuthHeaders(OPERATION_TYPE, WORKLOAD)).resolves.toEqual({
        authorization: 'Bearer essu_loopback',
      });

      expect(store.getVerified).toHaveBeenCalledWith(COORDINATES);
      expect(backend.getLoopbackAuthHeaders).toHaveBeenCalledWith('service-account-id');
    });

    it('refuses when the workload has no binding', async () => {
      store.getVerified.mockResolvedValue(null);

      await expect(bindings.getLoopbackAuthHeaders(OPERATION_TYPE, WORKLOAD)).rejects.toMatchObject(
        { output: { statusCode: 404 } }
      );
      expect(backend.getLoopbackAuthHeaders).not.toHaveBeenCalled();
    });
  });
  ...
  describe('availability', () => {
    it.each([
      ...
      [
        ...
        (api: ServiceAccountWorkloadBindings) =>
          api.getLoopbackAuthHeaders(OPERATION_TYPE, WORKLOAD),
      ],
...
describe('createNotImplementedWorkloadBindings', () => {
  ...
  it.each([
    ...
    ['getLoopbackAuthHeaders', () => api.getLoopbackAuthHeaders(OPERATION_TYPE, WORKLOAD)],
