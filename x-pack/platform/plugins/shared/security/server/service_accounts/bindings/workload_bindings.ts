...
export class ServiceAccountWorkloadBindings implements ServiceAccountWorkloadBindingsApi {
  ...
  async getLoopbackAuthHeaders(
    operationType: string,
    params: ServiceAccountWorkloadCoordinates
  ): Promise<Record<string, string>> {
    this.ensureAvailable();

    const binding = await this.requireBinding(this.toCoordinates(operationType, params));
    return await this.backend.getLoopbackAuthHeaders(binding.serviceAccountId);
  }
...
export const createNotImplementedWorkloadBindings = (): ServiceAccountWorkloadBindingsApi => {
  ...
  return {
    ...
    getLoopbackAuthHeaders: async () => notImplemented(),
  };
};
