...
test.describe('Field Level Security', { tag: tags.stateful.classic }, () => {
  ...
  test.describe('role creation with FLS', () => {
    ...
    test('should add new role a_viewssnrole', async ({ pageObjects }) => {
    ...
    test('should add new role a_view_no_ssn_role', async ({ pageObjects }) => {
      await pageObjects.securityRoles.goto();
    ...
    test('should add new user customer1', async ({ pageObjects }) => {
    ...
    test('should add new user customer2', async ({ pageObjects }) => {
      await pageObjects.securityUsers.createUser({
        username: 'customer2',
        password: 'changeme',
        confirm_password: 'changeme',
    ...
    test('should support case-sensitive fields', async ({ pageObjects, esClient }) => {
      await pageObjects.securityRoles.goto();
      ...
      expect(indexEntry?.field_security?.except).toEqual(
        expect.arrayContaining(['customer_region', 'Customer_region'])
      );
    });
  });

  test.describe('data access with FLS restrictions', () => {
