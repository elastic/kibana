...
apiTest.describe('Audit log — OTel field shape', { tag: tags.serverless.security.complete }, () => {
  ...
  apiTest('user_login failure: outcome=failure, no user fields', async ({ apiClient, config }) => {
  ...
  apiTest(
    ...
    async ({ apiClient, samlAuth }) => {
      ...
      const e = await receiver.waitForLogRecord((attrs) => attrs['event.action'] === 'user_logout');
