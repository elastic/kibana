...
import {
  resolveMemorySearchMode,
  type MemoryCategoryNode,
...
import { MEMORY_RRF_RANK_CONSTANT, MEMORY_SEMANTIC_MIN_SCORE } from './search_tuning';
import { bulkCreateWithInferenceFallback } from '../es/bulk_with_inference_fallback';
import type {
  CreateMemoryParams,
  UpdateMemoryParams,
  SearchMemoryParams,
 ...
 *   - .agent-memory-pages  (pages, latest version resolved via field collapse)
 *   - .agent-memory-history  (version history, append-only)
 *
...
export class MemoryServiceImpl implements MemoryService {
  ...
  async search(params: SearchMemoryParams): Promise<MemorySearchResult[]> {
    ...
    const mode = resolveMemorySearchMode(params.mode);
  ...
  private async _executePhase3(
    ...
    requestedMode: MemorySearchMode | undefined,
    query: string,
    ...
    const minScore = MEMORY_SEMANTIC_MIN_SCORE;
    const rankConstant = MEMORY_RRF_RANK_CONSTANT;
