...
export function registerElasticsearchFunction({
  ...
  functions.registerInstruction(({ availableFunctionNames }) => {
    if (availableFunctionNames.includes(ELASTICSEARCH_FUNCTION_NAME)) {
      ...
      You are only allowed to perform GET requests (Some exampples for GET requests are: Retrieving cluster information, cluster license, cluster health, indices stats, index stats, etc.) and GET/POST requests for the \`/_search\` endpoint (for search operations).
      ...
      For POST \`/_search\` operations, if there a request body is needed, make sure the request body is valid object.`;
    }
    return '';
  });
