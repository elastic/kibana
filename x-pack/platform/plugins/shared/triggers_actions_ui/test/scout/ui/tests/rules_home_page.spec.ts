...
test.describe('Rules home page', { tag: tags.stateful.classic }, () => {
  ...
  test('renders the rules list with a newly-created rule visible', async ({
    ...
    await expect(
      page.testSubj.locator(RULES_LIST_SUBJ).locator(`[data-test-subj="rulesListTableRowName-${ruleName}"]`)
    ).toBeVisible();
  });
  ...
  test('navigates to the rule details page when clicking a rule in the list', async ({
    ...
    await page.testSubj.locator(RULES_LIST_SUBJ).locator(`[data-test-subj="rulesListTableRowName-${ruleName}"]`).click();
