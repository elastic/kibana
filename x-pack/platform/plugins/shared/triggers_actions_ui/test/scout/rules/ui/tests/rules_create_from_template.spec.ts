...
test.describe('Create rule from template', { tag: tags.stateful.classic }, () => {
  test('stays on the host mount when selecting a template', async ({ browserAuth, page }) => {
    ...
    test.skip(!hasTemplates, 'No Templates tab available in this config; gap covered by unit test');
