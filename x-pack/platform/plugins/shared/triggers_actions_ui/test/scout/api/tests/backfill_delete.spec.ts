 ...
 *
 * The original FTR suite remains in place for the full security-and-spaces
 * matrix; this Scout API suite provides endpoint-level coverage close to the
 * owning plugin.
 */
...
apiTest.describe('backfill delete API', { tag: tags.stateful.classic }, () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'deletes backfills and verifies they are no longer accessible',
    async ({ esClient, kbnClient }) => {
      ...
      const taskRecord1 = await getScheduledTask(esClient, backfillId1);
      expect(taskRecord1.type).toBe('task');
      expect(taskRecord1.task.taskType).toBe('ad_hoc_run-backfill');
      expect(taskRecord1.task.enabled).toBe(true);
      expect(JSON.parse(taskRecord1.task.params)).toStrictEqual({
        adHocRunParamsId: backfillId1,
        spaceId: 'default',
      });

      const taskRecord2 = await getScheduledTask(esClient, backfillId2);
      expect(taskRecord2.type).toBe('task');
      expect(taskRecord2.task.taskType).toBe('ad_hoc_run-backfill');
      expect(taskRecord2.task.enabled).toBe(true);
      expect(JSON.parse(taskRecord2.task.params)).toStrictEqual({
      ...
      expect(await getScheduledTaskStatusCode(esClient, backfillId1)).toBe(404);
      expect(await getScheduledTaskStatusCode(esClient, backfillId2)).toBe(404);
    }
  );
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 404 when deleting a non-existent backfill ID',
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 404 when deleting a backfill from a different space',
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 403 when a no-privileges user attempts to delete a backfill',
    async ({ apiClient, kbnClient, samlAuth }) => {
      ...
      const response = await apiClient.delete(`internal/alerting/rules/backfill/${backfillId}`, {
        headers: { 'kbn-xsrf': 'scout', 'x-elastic-internal-origin': 'scout', ...cookieHeader },
      });
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 403 when a read-only user attempts to delete a backfill',
    async ({ apiClient, kbnClient, samlAuth }) => {
      const ruleRes = await kbnClient.request<{ id: string }>({
        method: 'POST',
        path: RULE_CREATE_PATH,
