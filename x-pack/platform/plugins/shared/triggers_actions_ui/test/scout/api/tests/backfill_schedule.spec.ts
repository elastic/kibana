 ...
 * matrix plus ad_hoc_run saved object, task-manager, and AAD assertions.
 */

import { tags } from '@kbn/scout';
import { expect } from '@kbn/scout/api';
import { apiTest, makeBackfillRule, makeEsQueryRule } from '../fixtures';
...
apiTest.describe('backfill schedule API', { tag: tags.stateful.classic }, () => {
  const createdRuleIds: string[] = [];
  const createdBackfillIds: string[] = [];

  apiTest.beforeAll(async ({ apiClient, kbnClient }) => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('schedules backfill for two rules successfully', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('schedules multiple ranges for a single rule', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('returns 400 for invalid schedule parameters', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // No matching rule
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('returns 400 when no rules match the given IDs', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns mixed success and error results for a batch request',
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'includes warning for non-summary actions when run_actions is true',
    async ({ apiClient, kbnClient }) => {
      expect(apiClient).toBeDefined();
      const start = daysAgoIso(14);
      const end = daysAgoIso(5);
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 403 for a user with no Kibana privileges',
    async ({ apiClient, kbnClient, samlAuth }) => {
      ...
      const response = await apiClient.post(BACKFILL_SCHEDULE_PATH.slice(1), {
        headers: { 'kbn-xsrf': 'scout', 'x-elastic-internal-origin': 'scout', ...cookieHeader },
        body: [{ rule_id: ruleId, ranges: [{ start: daysAgoIso(7), end: daysAgoIso(1) }] }],
      });
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'returns 403 for a read-only viewer trying to schedule backfill',
    async ({ apiClient, kbnClient, samlAuth }) => {
      const { cookieHeader } = await samlAuth.asInteractiveUser({
        elasticsearch: { cluster: [], indices: [] },
