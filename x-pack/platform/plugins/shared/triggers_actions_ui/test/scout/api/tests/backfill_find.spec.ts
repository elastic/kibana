 ...
 * pagination + sorting, and RBAC (no-privileges denied).
 *
 * The original FTR suite remains in place for invalid query params and the full
 * security-and-spaces matrix.
 */
...
apiTest.describe('backfill find API', { tag: tags.stateful.classic }, () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('finds backfills for a single rule by rule_id', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'finds backfills for multiple rules in a single query',
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('filters backfills by start date', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
    const { ruleId1, ruleId2, backfillId1, backfillId2 } = await setupTwoBackfills(kbnClient);
  ...
  // Filter by end date
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('filters backfills by end date', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
    const { ruleId1, ruleId2, backfillId1, backfillId2 } = await setupTwoBackfills(kbnClient);
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest('filters backfills by initiator=user', async ({ apiClient, kbnClient }) => {
    expect(apiClient).toBeDefined();
  ...
  // ────────────────────────────────────────────────────────────────────────────
  apiTest(
    'supports pagination and sorting of backfill results',
    async ({ apiClient, kbnClient }) => {
      expect(apiClient).toBeDefined();
      ...
      const page1 = await kbnClient.request<{
        total: number;
        data: Array<Record<string, unknown>>;
      }>({
        method: 'POST',
      ...
      const page2 = await kbnClient.request<{
        total: number;
        data: Array<Record<string, unknown>>;
      }>({
        method: 'POST',
  ...
  apiTest(
    ...
    async ({ apiClient, kbnClient, samlAuth }) => {
      ...
      const response = await apiClient.post(`${BACKFILL_FIND_PATH.slice(1)}?rule_ids=${ruleId}`, {
