...
export const GroupByExpression = ({
  ...
  const hasTermField = Boolean(termField && termField.length > 0);
  const hasTermSize = Boolean(termSize);
  const isExpressionIncomplete = groupBy !== 'all' && !(hasTermSize && hasTermField);
  const termFieldErrors = (errors.termField ?? []) as string[];
  const termSizeErrors = (errors.termSize ?? []) as string[];
  const hasValidationErrors = termFieldErrors.length > 0 || termSizeErrors.length > 0;
  const isExpressionInvalid = isExpressionIncomplete || hasValidationErrors;

  let expressionErrors: string[] = [];
  if (hasValidationErrors) {
    expressionErrors = [...termFieldErrors, ...termSizeErrors];
  } else if (isExpressionIncomplete) {
    if (!hasTermField) {
      expressionErrors.push(i18nTranslations.TERM_FIELD_REQUIRED);
    }
    if (!hasTermSize) {
      expressionErrors.push(i18nTranslations.TERM_SIZE_REQUIRED);
    }
  }
  ...
  return (
    <>
      ...
      {isExpressionInvalid && expressionErrors.length > 0 ? (
        <EuiFormErrorText data-test-subj="groupByExpressionErrorText">
