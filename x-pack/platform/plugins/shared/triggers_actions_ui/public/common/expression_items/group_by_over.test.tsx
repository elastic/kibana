...
describe('group by expression', () => {
  ...
  it('shows error text below the expression when group by is incomplete', () => {
    render(
      <IntlProvider locale="en">
        <GroupByExpression
          errors={{ termSize: [], termField: [] }}
    ...
    expect(screen.getByTestId('groupByExpressionErrorText')).toHaveTextContent(
      'Term field is required.'
    );
  });

  it('shows validation error text below the expression when provided', () => {
    render(
      <IntlProvider locale="en">
        <GroupByExpression
          errors={{ termSize: [], termField: ['Cannot select more than 3 terms'] }}
          fields={[
            {
              normalizedType: 'number',
              name: 'field1',
    ...
    expect(screen.getByTestId('groupByExpressionErrorText')).toHaveTextContent(
      'Cannot select more than 3 terms'
    );
  });

  it('does not show error text when group by is valid', () => {
    render(
      <IntlProvider locale="en">
        <GroupByExpression
          errors={{ termSize: [], termField: [] }}
    ...
    expect(screen.queryByTestId('groupByExpressionErrorText')).not.toBeInTheDocument();
  });
});
