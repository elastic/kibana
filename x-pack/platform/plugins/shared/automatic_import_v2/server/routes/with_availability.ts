...
>(
  handler: RequestHandler<P, Q, B, AutomaticImportV2PluginRequestHandlerContext, Method>
): RequestHandler<P, Q, B, AutomaticImportV2PluginRequestHandlerContext, Method> => {
  return async (context, req, res) => {
    const { isAvailable } = await context.automaticImportv2;
    if (!isAvailable()) {
      return res.notFound({
