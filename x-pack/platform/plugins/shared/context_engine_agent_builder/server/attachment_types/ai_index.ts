...
> => ({
  ...
  getAgentDescription: () =>
    [
      ...
      'When existing automations are listed, call `platform.workflows.get_workflow` (metadata only',
      'first) and `ask_user_question` so the user chooses to edit one or create a new automation',
      'before drafting; load full YAML with `includeYaml: true` only for the workflow being edited',
      'or when the user explicitly asks to review/compare definitions.',
