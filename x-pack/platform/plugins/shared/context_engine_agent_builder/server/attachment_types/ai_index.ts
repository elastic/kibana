...
> => ({
  ...
  getAgentDescription: () =>
    [
      ...
      '`workflowYaml`, in the same assistant turn but a separate model step after the account is',
      'visible. Never re-generate the definition the subagent returned; saving anything other than the',
      'YAML that was piloted throws away the only evidence the automation works.',
      ...
      'Once it is saved, offer to run it over the full corpus with `ask_user_question`; the pilot',
      'covered a handful of documents, and the first real run is its own decision.',
    ].join(' '),
