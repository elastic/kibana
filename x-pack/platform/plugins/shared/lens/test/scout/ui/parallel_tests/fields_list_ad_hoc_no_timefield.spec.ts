...
spaceTest.describe(
  ...
  () => {
    ...
    spaceTest.beforeEach(async ({ browserAuth, page, pageObjects }) => {
      ...
      // Switch to ad-hoc data view without time field
      ...
      // Deselect time field for ad-hoc data view without time field
      const timeFieldSelector = page.testSubj.locator('timestampField');
      await expect(timeFieldSelector).toBeVisible();
      await timeFieldSelector.selectOption({
        label: "--- I don't want to use the time filter ---",
      });
      await expect(page.testSubj.locator('fieldListLoading')).toBeHidden({ timeout: 30_000 });
    });

    spaceTest.afterAll(async ({ scoutSpace }) => {
      await scoutSpace.uiSettings.unset('defaultIndex', 'dateFormat:tz');
