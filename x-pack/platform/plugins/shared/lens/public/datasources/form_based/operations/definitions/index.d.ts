...
import type { ExpressionAstExpressionBuilder, ExpressionAstFunction } from '@kbn/expressions-plugin/public';
import type { DataPublicPluginStart } from '@kbn/data-plugin/public';
import type { FieldFormatsStart } from '@kbn/field-formats-plugin/public';
import type { DataViewsPublicPluginStart } from '@kbn/data-views-plugin/public';
import type { EuiThemeComputed } from '@elastic/eui';
import type { BaseIndexPatternColumn, ColumnBuildHints, DateRange, FormBasedLayer, GenericIndexPatternColumn, IncompleteColumn, LensLayerType, OperationMetadata, ReferenceBasedIndexPatternColumn, FramePublicAPI, IndexPattern, IndexPatternField, ParamEditorCustomProps, UserMessage } from '@kbn/lens-common';
...
export { minOperation, averageOperation, sumOperation, maxOperation, standardDeviationOperation, } from './metrics';
...
export { cumulativeSumOperation, counterRateOperation, derivativeOperation, movingAverageOperation, overallSumOperation, overallAverageOperation, overallMaxOperation, overallMinOperation, timeScaleOperation, } from './calculations';
 ...
 */
export interface ParamEditorProps<C, U = FormBasedLayer | ((prevLayer: FormBasedLayer) => FormBasedLayer)> {
    currentColumn: C;
...
interface BaseOperationDefinitionProps<C extends BaseIndexPatternColumn, AR extends boolean, P = {}> {
     ...
     */
    getDefaultLabel: (column: C, columns: Record<string, GenericIndexPatternColumn>, indexPattern?: IndexPattern, uiSettings?: IUiSettingsClient, dateRange?: DateRange) => string;
    getSerializedFormat?: (column: C, targetColumn: C, indexPattern?: IndexPattern, uiSettings?: IUiSettingsClient, dateRange?: DateRange) => Record<string, unknown>;
    /**
     ...
     */
    allowAsReference?: AR;
    paramEditor?: React.ComponentType<AR extends true ? ParamEditorProps<C, GenericIndexPatternColumn> : ParamEditorProps<C>>;
    getAdvancedOptions?: (params: ParamEditorProps<C> & {
     ...
     * for a layer
     */
    isTransferable: (column: C, newIndexPattern: IndexPattern, operationDefinitionMap: Record<string, GenericOperationDefinition>) => boolean;
    /**
     ...
     */
    getDisabledStatus?: (indexPattern: IndexPattern, layer: FormBasedLayer, layerType?: LensLayerType) => string | undefined;
    /**
     ...
     * - Missing references
     */
    getErrorMessage?: (layer: FormBasedLayer, columnId: string, indexPattern: IndexPattern, dateRange?: DateRange, operationDefinitionMap?: Record<string, GenericOperationDefinition>, targetBars?: number) => FieldBasedOperationErrorMessage[];
     ...
     */
    getDefaultVisualSettings?: (column: C) => {
     ...
     */
    optimizeEsAggs?: (aggs: ExpressionAstExpressionBuilder[], esAggsIdMap: Record<string, OriginalColumn[]>, aggExpressionToEsAggsIdMap: Map<ExpressionAstExpressionBuilder, string>) => {
    ...
    toESQL?: (column: C, columnId: string, indexPattern: IndexPattern, layer: FormBasedLayer, uiSettings: IUiSettingsClient, dateRange: DateRange) => ESQLExpressionWithParams | undefined;
}
...
export interface FieldBasedOperationErrorMessage {
    ...
    fixAction?: {
        label: string;
        newState: (data: DataPublicPluginStart, core: CoreStart, frame: FramePublicAPI, layerId: string) => Promise<FormBasedLayer>;
    };
}
interface FieldlessOperationDefinition<C extends BaseIndexPatternColumn, P = {}> {
     ...
     */
    toEsAggsFn: (column: C, columnId: string, indexPattern: IndexPattern, layer: FormBasedLayer, uiSettings: IUiSettingsClient) => ExpressionAstFunction;
}
interface FieldBasedOperationDefinition<C extends BaseIndexPatternColumn, P = {}> {
     ...
     * together with the agg configs returned from other columns.
     */
    toEsAggsFn: (column: C, columnId: string, indexPattern: IndexPattern, layer: FormBasedLayer, uiSettings: IUiSettingsClient, orderedColumnIds: string[], operationDefinitionMap?: Record<string, GenericOperationDefinition>) => ExpressionAstFunction;
    /**
     * Validate that the operation has the right preconditions in the state. For example:
     *
     * - Requires a date histogram operation somewhere before it in order
     * - Missing references
     */
    getErrorMessage?: (layer: FormBasedLayer, columnId: string, indexPattern: IndexPattern, operationDefinitionMap?: Record<string, GenericOperationDefinition>) => FieldBasedOperationErrorMessage[];
}
export interface RequiredReference {
    input: Array<GenericOperationDefinition['input']>;
    validateMetadata: (metadata: OperationMetadata, operation?: OperationType, field?: string) => boolean;
...
interface FullReferenceOperationDefinition<C extends BaseIndexPatternColumn> {
    ...
    buildColumn: (arg: BaseBuildColumnArgs & {
        ...
        shift?: string;
    } & FilterParams) => ReferenceBasedIndexPatternColumn & C;
    /**
     * Returns the meta data of the operation if applied. Undefined
     ...
     */
    toExpression: (layer: FormBasedLayer, columnId: string, indexPattern: IndexPattern) => ExpressionAstFunction[];
}
interface ManagedReferenceOperationDefinition<C extends BaseIndexPatternColumn> {
    ...
    buildColumn: (arg: BaseBuildColumnArgs & {
        ...
        reducedTimeRange?: string;
    }, operationDefinitionMap?: Record<string, GenericOperationDefinition>) => ReferenceBasedIndexPatternColumn & C;
    /**
     * Returns the meta data of the operation if applied. Undefined
     * if the operation can't be added with these fields.
     */
     ...
     * A chain of expression functions which will transform the table
     */
    toExpression: (layer: FormBasedLayer, columnId: string, indexPattern: IndexPattern, context?: {
        dateRange?: DateRange;
        now?: Date;
        targetBars?: number;
     ...
     * root level
     */
    createCopy: (layers: Record<string, FormBasedLayer>, source: DataViewDragDropOperation, target: DataViewDragDropOperation, operationDefinitionMap: Record<string, GenericOperationDefinition>) => Record<string, FormBasedLayer>;
    /**
 ...
 */
export type OperationDefinition<C extends BaseIndexPatternColumn, Input extends keyof OperationDefinitionMap<C>, P = {}, AR extends boolean = false> = BaseOperationDefinitionProps<C, AR> & OperationDefinitionMap<C, P>[Input];
/**
 ...
 * column types.
 */
export type GenericOperationDefinition<ColumnType extends BaseIndexPatternColumn = BaseIndexPatternColumn> = OperationDefinition<ColumnType, 'field'> | OperationDefinition<ColumnType, 'none'> | OperationDefinition<ColumnType, 'fullReference'> | OperationDefinition<ColumnType, 'managedReference'>;
/**
 * List of all available operation definitions
 */
export declare const operationDefinitions: GenericOperationDefinition[];
/**
 ...
 */
export declare const operationDefinitionMap: Record<string, GenericOperationDefinition>;
/**
 * Cannot map the prev names, but can guarantee the new names are matching up using the type system
 */
export declare const renameOperationsMapping: Record<string, GenericOperationDefinition['type']>;
