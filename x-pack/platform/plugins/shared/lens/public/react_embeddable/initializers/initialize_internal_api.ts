...
export function initializeInternalApi(
  ...
  const [expressionAbortController$] = buildObservableVariable<AbortController | undefined>(
    apiHasAbortController(parentApi) ? parentApi.abortController : undefined
  );

  const [renderCount$] = buildObservableVariable<number>(0);

  const [attributes$] = buildObservableVariable<LensRuntimeState['attributes']>(
    initialState.attributes || createEmptyLensState().attributes
  );
  const [overrides$] = buildObservableVariable(initialState.overrides);
  const [disableTriggers$] = buildObservableVariable(initialState.disableTriggers);
  ...
  const [validationMessages$] = buildObservableVariable<UserMessage[]>([]);
  // This other set of messages is for non-blocking messages that can be displayed in the UI
  const [messages$] = buildObservableVariable<UserMessage[]>([]);
  ...
  const [isNewlyCreated$] = buildObservableVariable<boolean>(initialState.isNewPanel || false);

  const [blockingError$] = buildObservableVariable<Error | undefined>(undefined);
  const [visualizationContext$] = buildObservableVariable<VisualizationContext>({
