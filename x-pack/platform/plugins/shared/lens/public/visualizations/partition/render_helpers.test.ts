...
describe('render helpers', () => {
  ...
  describe('#getLegendStats', () => {
    it('should firstly read the state value', () => {
      expect(
        getLegendStats({ legendStats: [LegendValue.Value] } as LensPartitionLayerState , PieChartTypes.WAFFLE)
      ).toEqual([LegendValue.Value]);

      expect(
        getLegendStats({ legendStats: [] as LegendValue[] } as LensPartitionLayerState , PieChartTypes.WAFFLE)
      ).toEqual([]);
    });
