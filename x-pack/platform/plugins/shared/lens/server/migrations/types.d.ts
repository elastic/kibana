...
import type { LensLayerType as LayerType, PersistableFilter, RowHeightMode, ValueLabelConfig } from '@kbn/lens-common';
export type CustomVisualizationMigrations = Record<string, () => MigrateFunctionsObject>;
export type OperationTypePre712 = 'avg' | 'cardinality' | 'derivative' | 'filters' | 'terms' | 'date_histogram' | 'min' | 'max' | 'sum' | 'median' | 'percentile' | 'last_value' | 'count' | 'range' | 'cumulative_sum' | 'counter_rate' | 'moving_average';
export type OperationTypePost712 = Exclude<OperationTypePre712 | 'average' | 'unique_count' | 'differences', 'avg' | 'cardinality' | 'derivative'>;
...
export type LensDocShape713 = Omit<LensDocShapePost712, 'state'> & {
    state: Omit<LensDocShapePost712['state'], 'datasourceStates'> & {
        datasourceStates: {
            indexpattern: Omit<LensDocShapePost712['state']['datasourceStates']['indexpattern'], 'layers'> & {
                layers: Record<string, Omit<LensDocShapePost712['state']['datasourceStates']['indexpattern']['layers'][string], 'columns'> & {
                    columns: Record<string, {
                        operationType: OperationTypePost712;
                    } | {
...
export type VisStatePre715 = LayerPre715 | {
    layers: LayerPre715[];
};
interface LayerPost715 extends LayerPre715 {
    layerType: LayerType;
}
export type VisStatePost715 = LayerPost715 | {
    layers: LayerPost715[];
};
...
export type LensDocShape810<VisualizationState = unknown> = Omit<LensDocShape715<VisualizationState>, 'filters' | 'state'> & {
    filters: Filter[];
    state: Omit<LensDocShape715<VisualizationState>['state'], 'datasourceStates'> & {
        datasourceStates: {
            indexpattern: Omit<LensDocShape715['state']['datasourceStates']['indexpattern'], 'layers'> & {
                layers: Record<string, Omit<LensDocShape715['state']['datasourceStates']['indexpattern']['layers'][string], 'columns'> & {
                    columns: Record<string, {
                        operationType: 'terms';
                        params: {
...
export type LensDocShape860<VisualizationState = unknown> = Omit<LensDocShape850<VisualizationState>, 'state'> & {
