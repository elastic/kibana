...
/**
 * Handles transforming old lens SO in dashboard to the latest Lens SO
 */
function migrateAttributes(attributes: LensByValueSerializedState['attributes']): LensAttributes {
  ...
  if (isLensAttributesV0(newAttributes) || isLensAttributesV1(newAttributes)) {
    ...
    const v2Attributes = transformToV2LensItemAttributes(v1Attributes);

    return transformToV3LensItemAttributes(v2Attributes) as LensAttributes;
  }

  if (isLensAttributesV2(newAttributes)) {
    return transformToV3LensItemAttributes(newAttributes) as LensAttributes;
  }
