...
- **Experiments** — list, detail, scores, and statistical comparison of two experiments. Dataset-level example summaries use fixed 25-example pages; input/output previews are bounded to 4,096 characters, and complete input, output, and evaluator details are retrieved for one example repetition on demand.
