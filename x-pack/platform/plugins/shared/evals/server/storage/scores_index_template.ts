...
export const EVALUATIONS_DATA_STREAM_ALIAS = 'kibana-evaluations';
export const EVALUATIONS_DATA_STREAM_WILDCARD = 'kibana-evaluations*';
export const EVALUATIONS_DATA_STREAM_TEMPLATE = 'kibana-evaluations-template';
export const EVALUATIONS_DEFAULT_ILM_POLICY = 'kibana-evaluations-default-policy';

interface ScoresIndexTemplateOptions {
  lifecyclePolicyName?: string;
}

export const getScoresIndexTemplate = ({
  lifecyclePolicyName,
}: ScoresIndexTemplateOptions = {}) => ({
  index_patterns: [EVALUATIONS_DATA_STREAM_WILDCARD],
  data_stream: {},
  template: {
    settings: {
      refresh_interval: '5s',
      ...(lifecyclePolicyName ? { index: { lifecycle: { name: lifecyclePolicyName } } } : {}),
    },
    mappings: {
      properties: {
        '@timestamp': { type: 'date' },
