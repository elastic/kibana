...
export class ExplorationWorkflowExecutor {
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`[AESOP] Exploration completed successfully`, {
      ...
      this.logger.error(`[AESOP] Exploration failed`, { execution_id: executionId, error });
  ...
  private async executePhase(
    ...
    this.logger.info(`[AESOP] Starting Phase ${phaseNumber}: ${phaseName}`, {
    ...
    try {
      ...
      const durationMs = Date.now() - start;
      this.logger.error(`[AESOP] Phase ${phaseNumber} failed after ${durationMs}ms`, { error });
      throw error;
    }
  }
  ...
  private async phaseSkillSynthesis(): Promise<void> {
    ...
    if (
      ...
      try {
        ...
        if (agentSkills.length > 0) {
          // Map agent output directly to ProposedSkill — no re-serialization
    ...
    if (!agentOrchestrationSucceeded && useLLM) {
      ...
      for (const rel of this.relationships.filter((r) => r.confidence > 0.3).slice(0, 5)) {
        const skill = this.synthesizeRelationshipSkill(rel);
        if (skill) {
          this.skills.push(skill);
        }
      }
...
If no improvements are warranted, return an empty array: []`;
  ...
  // ---------------------------------------------------------------------------
  // Skill Synthesis Helpers
  // ---------------------------------------------------------------------------

  private synthesizeSkill(pattern: PatternInfo): ProposedSkill | null {
    const { analystRole, roleDescription } = this.config;

    if (pattern.patternName.startsWith('Alert Rule:')) {
      const ruleName = pattern.patternName.replace('Alert Rule: ', '');
      return {
        skillId: `skill-${pattern.patternId}`,
        name: `Triage: ${ruleName}`,
        description: `Automated triage workflow for "${ruleName}" alerts. Gathers context from correlated indices, checks for known false positive patterns, and recommends next steps.`,
        confidence: pattern.confidence,
        markdown: [
          `# Triage: ${ruleName}`,
          '',
          `## Purpose`,
          `Automate initial triage for "${ruleName}" alerts, which fire ${pattern.frequency} times per week.`,
          `Tailored for ${roleDescription}.`,
          '',
          `## Steps`,
          `1. Retrieve the latest alerts matching this rule`,
          `2. Enrich with host and user context from correlated indices`,
          `3. Check for known false positive patterns`,
          `4. Recommend escalation or closure`,
          '',
          `## Example Query`,
          '```esql',
    ...
    if (pattern.patternName.startsWith('Frequent Event:')) {
      const eventAction = pattern.patternName.replace('Frequent Event: ', '');
      return {
        skillId: `skill-${pattern.patternId}`,
        name: `Monitor: ${eventAction} events`,
        description: `Monitors "${eventAction}" events for anomalous spikes or drops, providing proactive alerting before security incidents escalate.`,
        confidence: pattern.confidence * 0.8,
        markdown: [
          `# Monitor: ${eventAction} Events`,
          '',
          `## Purpose`,
          `Track "${eventAction}" events (${pattern.frequency}/week) for anomalous behavior.`,
          '',
          `## Detection Logic`,
          `1. Baseline normal frequency per host`,
          `2. Alert on >2 standard deviation spikes`,
          `3. Alert on unexpected gaps (>4 hours with no events)`,
          '',
          `## Example Query`,
          '```esql',
          pattern.exampleQueries[0] || '',
    ...
    if (pattern.patternName.startsWith('Log Source:')) {
      const source = pattern.patternName.replace('Log Source: ', '');
      return {
        skillId: `skill-${pattern.patternId}`,
        name: `Analyze: ${source} logs`,
        description: `Structured analysis of ${source} log data (${pattern.frequency} events/week). Identifies top talkers, error patterns, and security-relevant anomalies.`,
        confidence: pattern.confidence * 0.7,
        markdown: [
          `# Analyze: ${source} Logs`,
          '',
          `## Purpose`,
          `Provide structured analysis of ${source} data for ${roleDescription}.`,
          '',
          `## Analysis`,
          `1. Top hosts by event volume`,
          `2. Error rate trending`,
          `3. Unusual source IPs or user agents`,
          '',
          `## Example Query`,
          '```esql',
          pattern.exampleQueries[0] || '',
  ...
  private synthesizeRelationshipSkill(rel: RelationshipInfo): ProposedSkill | null {
    // Resolve backing index names to their data stream / alias names.
    // Skills must never reference internal backing indices like
    // `.internal.alerts-security.alerts-default-000001` — those roll over.
    const resolvedFrom = suggestCorrectIndexName(rel.from) ?? rel.from;
    const resolvedTo = suggestCorrectIndexName(rel.to) ?? rel.to;

    const shortFrom = resolvedFrom.split('-')[0] || resolvedFrom;
    const shortTo = resolvedTo.split('-')[0] || resolvedTo;
    const patternId = `rel-${shortHash(`${resolvedFrom}-${resolvedTo}-${rel.via}`)}`;
    ...
    return {
      ...
      markdown: [
        ...
        `2. Build a timeline of events across both sources`,
        `3. Identify gaps or contradictions in the event chain`,
        '',
        `## Example Query`,
        '```esql',
      ...
      source: {
        ...
        rationale: `Field "${rel.via}" is shared between ${resolvedFrom} and ${resolvedTo} with ${rel.sharedValueCount} common values, enabling cross-index investigation.`,
      },
    };
  }

  // ---------------------------------------------------------------------------
  // LLM-Powered Skill Synthesis
  // ---------------------------------------------------------------------------

  private async synthesizeSkillsWithLLM(actionsClient: any, connectorId: string): Promise<void> {
    const context = this.buildLLMContext();

    const prompt = `You are an expert Agent Builder skill designer for Elastic Security.
Based on the discovery context below, generate high-quality, actionable skills.

${context}

Generate skills as a JSON array. Each skill must have:
- "name": Short, descriptive name
- "description": 1-2 sentence description of what the skill does
- "markdown": Complete skill document in markdown with:
  - Purpose section
  - Detailed investigation/analysis steps with field names
  - Multiple ES|QL example queries using ONLY data stream names (never backing indices like .ds-* or .internal.*)
...
- Include time-bounded queries (e.g., WHERE @timestamp > NOW() - 24h)
- Reference specific fields discovered in the schemas
- Generate 3-8 skills total, focusing on the highest-value patterns
...
Respond with ONLY a JSON array (no markdown fences): [{ "name": "...", ... }, ...]`;
    ...
    try {
      ...
      const rawResponse = this.extractLLMText(result.data);
      const skills = this.parseLLMSkills(rawResponse);
