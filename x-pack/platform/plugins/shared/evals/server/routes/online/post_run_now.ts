...
const RunOnlineSuiteNowRequestBody = z.object({
...
export const registerRunOnlineSuiteNowRoute = ({
  router,
  logger,
  onlineSuiteRegistry,
  workflowsManagement,
  ...
  router.versioned
    .post({
      path: '/internal/evals/online/runs/run_now',
      ...
      summary: 'Start an online evaluation run (run now)',
    })
    .addVersion(
      {
        ...
        validate: {
          request: {
            body: buildRouteValidationWithZod(RunOnlineSuiteNowRequestBody),
          },
        },
      },
      async (_context, request, response) => {
        try {
          ...
          logger.error(`Failed to start online evaluation run (run now): ${error}`);
          return response.customError({
            statusCode: 500,
