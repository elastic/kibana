...
const RunOnlineSuiteRequestBody = z.object({
  workflow_id: z.string().min(1),
  suite_id: z.string().min(1),
  task_connector_id: z.string().min(1),
  judge_connector_id: z.string().min(1),
  suite_params: z.record(z.unknown()).optional(),
});

export const registerRunOnlineSuiteRoute = ({
  router,
  logger,
  onlineSuiteRegistry,
  workflowsManagement,
  ...
  router.versioned
    .post({
      ...
      summary: 'Start an online evaluation run',
    })
    .addVersion(
      {
        ...
        validate: {
          request: {
            body: buildRouteValidationWithZod(RunOnlineSuiteRequestBody),
          },
        },
      },
      async (context, request, response) => {
        try {
          ...
          if (!onlineSuiteRegistry) {
            return response.customError({
              statusCode: 500,
              body: { message: 'Online suite registry is not available' },
            });
          }
          ...
          logger.error(`Failed to start online evaluation run: ${error}`);
          return response.customError({
            statusCode: 500,
            body: { message: 'Failed to start online evaluation run' },
          });
        }
      }
    );
