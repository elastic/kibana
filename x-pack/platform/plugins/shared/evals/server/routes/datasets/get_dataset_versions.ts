...
const GetDatasetVersionsRequestParams = z.object({
  datasetId: z.string(),
});

export const registerGetDatasetVersionsRoute = ({ router, logger }: RouteDependencies) => {
  router.versioned
    .get({
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            params: buildRouteValidationWithZod(GetDatasetVersionsRequestParams),
          },
        },
      },
      async (context, request, response) => {
        try {
          ...
          logger.error(`Failed to get dataset versions: ${error}`);
          return response.customError({
            statusCode: 500,
            body: { message: 'Failed to get dataset versions' },
          });
        }
      }
    );
