...
import type { ReplaySeededEvaluator, ReplaySeededOutput } from './types';
...
export interface EventOutcomeResult {
  satisfiedEntries: ReplayExpectedEvent[];
  unsatisfiedEntries: ReplayExpectedEvent[];
...
export const computeEventOutcome = (
  output: ReplaySeededOutput | undefined,
  expected:
    | {
  ...
  const satisfiedEntries: ReplayExpectedEvent[] = [];
  const unsatisfiedEntries: ReplayExpectedEvent[] = [];
 ...
 */
export const eventOutcomeEvaluator: ReplaySeededEvaluator = {
  name: 'event_outcome',
  kind: 'CODE',
