...
import { extractTokensDangerouslySlow } from './extract_tokens';
...
describe('extractTokensDangerouslySlow', () => {
  it('generates a consistent root template structure', async () => {
    const tokens = await extractTokensDangerouslySlow([
  ...
  it('detects and handles special patterns correctly', async () => {
    ...
    expect(tokens.map(({ pattern }) => pattern)).toContain('IPV4');
  });

  it('detects and handles quoted strings correctly', async () => {
    const tokens = await extractTokensDangerouslySlow(['Foo "Quoted string"']);
  ...
  it('prevents greedy patterns from capturing parts of subsequent tokens', async () => {
    ...
    const patterns = tokens.map(({ pattern }) => pattern);
  ...
  it('finds a pattern no matter how inconsistent the logs are', async () => {
    ...
    expect(tokens.map(({ pattern }) => pattern)).toEqual(['GREEDYDATA']);
  });
  ...
  describe('with LogHub sample data', () => {
    ...
    it('processes HealthApp logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(healthAppLogs);
    ...
    it('processes android logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(androidLogs);
    ...
    it('processes thunderbird logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(thunderbirdLogs);
    ...
    it('processes zookeeper logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(zookeeperLogs);
    ...
    it('processes mac logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(macLogs);
    ...
    it('processes OpenStack logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(openstackLogs);
    ...
    it('processes Proxifier logs correctly', async () => {
      ...
      const tokens = await extractTokensDangerouslySlow(proxifierLogs);
