...
describe('extractTemplate', () => {
  ...
  it('detects and handles quoted strings correctly', async () => {
    const { roots, delimiter } = await syncExtractTemplate(['Foo "Quoted string"']);
  ...
  describe('with LogHub sample data', () => {
    ...
    it('processes android logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(androidLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes thunderbird logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(thunderbirdLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes zookeeper logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(zookeeperLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes mac logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(macLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes OpenStack logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(openstackLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes Proxifier logs correctly', async () => {
      ...
      const { roots, delimiter } = await syncExtractTemplate(proxifierLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
