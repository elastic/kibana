...
describe('extractTemplate', () => {
  ...
  it('generates a consistent root template structure', async () => {
    ...
    expect(usefulTokens.map(([pattern]) => pattern)).toEqual([
      'TIMESTAMP_ISO8601',
      '\\s',
      'LOGLEVEL',
      '\\s+',
  ...
  it('detects and handles special patterns correctly', async () => {
    ...
    expect(usefulTokens.map(([pattern]) => pattern)).toContain('IPV4');
  });

  it('detects and handles quoted strings correctly', async () => {
    const { roots, delimiter } = await extractTemplate(['Foo "Quoted string"']);
    const { columns } = getUsefulTokens(roots, delimiter);
  ...
  it('prevents greedy patterns from capturing parts of subsequent tokens', async () => {
    ...
    const { usefulTokens } = getUsefulTokens(roots, delimiter);
    const patterns = usefulTokens.map(([pattern]) => pattern);
  ...
  describe('with LogHub sample data', () => {
    it('processes HealthApp logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(healthAppLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes android logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(androidLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes thunderbird logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(thunderbirdLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes zookeeper logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(zookeeperLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes mac logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(macLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes OpenStack logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(openstackLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
    ...
    it('processes Proxifier logs correctly', async () => {
      ...
      const { roots, delimiter } = await extractTemplate(proxifierLogs);
      const { usefulTokens } = getUsefulTokens(roots, delimiter);
