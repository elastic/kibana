...
import type { InferenceInferenceEndpointInfo, MlInferenceConfigCreateContainer } from '@elastic/elasticsearch/lib/api/types';
import type { ModelDefinitionResponse, ModelState, TrainedModelType } from '@kbn/ml-trained-models-utils';
import type { DataFrameAnalyticsConfig, FeatureImportanceBaseline, TotalFeatureImportance } from '@kbn/ml-data-frame-analytics-utils';
...
export interface TrainedModelStat {
    ...
    ingest?: {
        ...
        pipelines: Record<string, IngestStats & {
            processors: Array<Record<string, {
                type: string;
                stats: IngestStats;
            }>>;
        }>;
...
export type ModelDownloadItem = BaseNLPModelItem & Omit<ModelDefinitionResponse, 'version' | 'config'> & {
    putModelConfig?: object;
    softwareLicense?: string;
};
/** Trained NLP model, i.e. pytorch model returned by the trained_models API */
export type NLPModelItem = BaseNLPModelItem & TrainedModelItem & {
    stats: Stats & {
        deployment_stats: TrainedModelDeploymentStatsResponse[];
    };
    /**
...
export declare function isBaseNLPModelItem(item: unknown): item is BaseNLPModelItem;
export declare function isNLPModelItem(item: unknown): item is NLPModelItem;
export declare const isElasticModel: (item: TrainedModelConfigResponse) => boolean;
...
export type TrainedModelItem = ExistingModelBase & {
    stats: Stats;
};
/** Trained DFA model */
export type DFAModelItem = Omit<TrainedModelItem, 'inference_config'> & {
    origin_job_exists?: boolean;
    inference_config?: Pick<NonNullable<MlInferenceConfigCreateContainer>, 'classification' | 'regression'>;
    metadata?: estypes.MlTrainedModelConfig['metadata'] & {
        analytics_config: DataFrameAnalyticsConfig;
        input: unknown;
        total_feature_importance?: TotalFeatureImportance[];
...
export declare function isExistingModel(item: unknown): item is TrainedModelItem;
export declare function isDFAModelItem(item: unknown): item is DFAModelItem;
export declare function isModelDownloadItem(item: TrainedModelUIItem): item is ModelDownloadItem;
export declare function isSearchOnlyModel(item: TrainedModelUIItem): boolean;
/**
 * Identifies rerank (text_similarity) trained models.
 */
export declare function isRerankModelItem(item: TrainedModelUIItem): boolean;
export declare const isBuiltInModel: (item: TrainedModelConfigResponse | TrainedModelUIItem) => boolean;
/**
