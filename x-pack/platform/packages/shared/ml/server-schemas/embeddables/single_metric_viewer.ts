...
export const singleMetricViewerEmbeddableStateSchema = z
  .object({
    ...
    job_ids: z.array(z.string().min(1).max(MAX_STRING_LENGTH)).min(1).max(1).meta({
    ...
    function_description: z.string().max(MAX_STRING_LENGTH).optional().meta({
      description:
    ...
    forecast_id: z.string().max(MAX_STRING_LENGTH).optional().meta({
