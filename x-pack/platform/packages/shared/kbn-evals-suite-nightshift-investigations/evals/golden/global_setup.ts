...
import { GOLDEN_SOURCE_DATASET, readGoldenDataset } from './datasets';
...
async function setupGoldenDataset(): Promise<(() => void) | undefined> {
  ...
  const source = await new EvalsClient(kbnClient, log).getDatasetByName(GOLDEN_SOURCE_DATASET);
  if (!source)
    throw new Error(
      `Approved dataset ${GOLDEN_SOURCE_DATASET} was not found on the selected cluster.`
    );
