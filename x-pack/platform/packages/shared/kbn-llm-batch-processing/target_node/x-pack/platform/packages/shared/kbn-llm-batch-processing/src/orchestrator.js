...
async function batchProcess(config) {
    ...
    if (config.splitStrategy === 'token-based') {
        ...
        batches = (0, split_1.tokenBasedSplit)(config.input, config.maxTokensPerBatch, config.tokenEstimator);
    }
        ...
        batches = (0, split_1.itemBasedSplit)(config.input, config.maxItemsPerBatch);
    }
    ...
    for (let i = 0; i < batches.length; i += maxConcurrent) {
        ...
        const chunkResults = await Promise.all(chunk.map(async (batch, idx) => {
    ...
    const finalOutput = await (0, merge_1.hierarchicalMerge)(batchResults, config.mergeFn);
