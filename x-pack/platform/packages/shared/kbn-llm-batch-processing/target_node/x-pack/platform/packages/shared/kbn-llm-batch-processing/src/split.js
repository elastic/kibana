...
exports.tokenBasedSplit = tokenBasedSplit;
exports.itemBasedSplit = itemBasedSplit;
/**
 ...
 */
function tokenBasedSplit(items, maxTokensPerBatch, tokenEstimator) {
    if (items.length === 0) {
        return [];
    }
    const batches = [];
 ...
 * @returns Array of batches
 */
function itemBasedSplit(items, maxItemsPerBatch) {
    if (items.length === 0) {
        return [];
    }
    const batches = [];
