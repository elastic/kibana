...
describe('tokenBasedSplit', () => {
    it('should split items to stay under max tokens per batch', () => {
        ...
        const estimator = (item) => item.length;
        const maxTokens = 5;
        const batches = (0, split_1.tokenBasedSplit)(items, maxTokens, estimator);
    ...
    it('should return empty array for empty input', () => {
        const batches = (0, split_1.tokenBasedSplit)([], 100, () => 1);
        expect(batches).toEqual([]);
    });
});
describe('itemBasedSplit', () => {
    it('should split into batches of fixed size', () => {
        ...
        const batches = (0, split_1.itemBasedSplit)(items, 3);
        expect(batches).toEqual([[1, 2, 3], [4, 5, 6], [7]]);
    });
    it('should handle exact divisions', () => {
        const items = [1, 2, 3, 4, 5, 6];
        const batches = (0, split_1.itemBasedSplit)(items, 2);
        expect(batches).toEqual([[1, 2], [3, 4], [5, 6]]);
    });
    it('should return empty array for empty input', () => {
        const batches = (0, split_1.itemBasedSplit)([], 10);
        expect(batches).toEqual([]);
    });
});
