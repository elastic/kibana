...
- `sequence` (Optional `long` — monotonic version from the primary store for ordering; see [Ordering and versioning](#ordering-and-versioning)).
...
| `object.sequence`       | `long`      | Optional monotonic version from the primary store for ordering (see [Ordering and versioning](#ordering-and-versioning)). |
...
After a successful primary write, copy your domain's monotonic version into `sequence`:
