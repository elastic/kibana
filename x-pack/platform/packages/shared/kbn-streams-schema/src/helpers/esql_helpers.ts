...
function getAssignmentName(node: ESQLAstItem | undefined): string | null {
  if (
    !node ||
    Array.isArray(node) ||
    node.type !== 'function' ||
...
function getDivisionDenominatorColumns(commands: ESQLCommand[]): Set<string> {
  ...
  walk(commands, {
    visitFunction: (node) => {
      ...
      walk(denominator as WalkerAstNode, {
        visitColumn: (column) => columns.add(column.name),
      });
    },
  });
  return columns;
...
function extractStatsDetectionPredicates(
  statsCmd: ESQLCommand,
  commandsAfterStats: ESQLCommand[]
): string[] {
  const denominatorColumns = getDivisionDenominatorColumns(commandsAfterStats);
  const predicates: Array<{ aggregation: string | null; printed: string }> = [];
  for (const arg of statsCmd.args) {
    if (Array.isArray(arg) || !('type' in arg)) continue;
    if (arg.type !== 'function' || arg.name !== 'where' || arg.subtype !== 'binary-expression') {
      continue;
    }
    const rawAggregation = (arg as ESQLFunction).args[0];
    const aggregation = Array.isArray(rawAggregation) ? rawAggregation[0] : rawAggregation;
    const rawPred = (arg as ESQLFunction).args[1];
    const predNode = Array.isArray(rawPred) ? rawPred[0] : rawPred;
    if (!predNode || Array.isArray(predNode) || !('type' in predNode)) continue;
    predicates.push({
      aggregation: getAssignmentName(aggregation),
      printed: BasicPrettyPrinter.expression(predNode as ESQLSingleAstItem),
    });
  }

  const withoutDenominators = predicates.filter(
    ({ aggregation }) => aggregation === null || !denominatorColumns.has(aggregation)
  );
  const nonNullPredicates = withoutDenominators.filter(
    ({ printed }) => !/\bIS\s+NOT\s+NULL\b/i.test(printed)
  );
  return (nonNullPredicates.length > 0 ? nonNullPredicates : withoutDenominators).map(
    ({ printed }) => printed
  );
}

/**
 * Builds short-circuiting existence probes for a candidate query, or `null` if parsing fails.
 */
export function buildExistenceProbeQueries(esql: string): string[] | null {
  const { root, parsed } = tryParseEsql(esql);
  ...
  const statsIdx = commands.findIndex((cmd) => cmd.name === 'stats');
  if (statsIdx === -1) {
    const printed = BasicPrettyPrinter.print(Builder.expression.query(commands));
    return [`${printed}\n| LIMIT 1\n| STATS COUNT(*)`];
  }

  const kept = commands.slice(0, statsIdx);
  if (kept.length === 0) return null;

  const detectionPredicates = extractStatsDetectionPredicates(
    commands[statsIdx],
    commands.slice(statsIdx + 1)
  );
  const printed = BasicPrettyPrinter.print(Builder.expression.query(kept));
  if (detectionPredicates.length === 0) {
    return [`${printed}\n| LIMIT 1\n| STATS COUNT(*)`];
  }
  return detectionPredicates.map(
    (predicate) => `${printed}\n| WHERE ${predicate}\n| LIMIT 1\n| STATS COUNT(*)`
  );
}
