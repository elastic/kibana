...
import type { TrimProcessor } from '../../../..';
import { conditionToESQLAst } from '../condition_to_esql';
import { buildIgnoreMissingFilter } from './common';

export function convertTrimProcessorToESQL(processor: TrimProcessor): ESQLAstCommand[] {
  ...
  const trimFunction = Builder.expression.func.call('TRIM', [fromColumn]);

  // Check if this is conditional or unconditional trim
  if ('where' in processor && processor.where && !('always' in processor.where)) {
    // Conditional replacement: use EVAL with CASE
    // EVAL target_field = CASE(<condition>, TRIM(to), fromColumn ?? nil)
    ...
    // Unconditional trim: use EVAL with TRIM() function
    const evalCommand = Builder.command({
      name: 'EVAL',
      args: [Builder.expression.func.binary('=', [toColumn, trimFunction])],
    });
    commands.push(evalCommand);
  }
