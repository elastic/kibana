...
export const convertStepsForUI = (
  dsl: StreamlangDSL
): StreamlangStepWithUIAttributesWithCustomIdentifier[] => {
  const result: StreamlangStepWithUIAttributesWithCustomIdentifier[] = [];
  ...
  function unnestSteps(steps: StreamlangStep[], parentId: string | null = null) {
    for (const step of steps) {
      ...
      if (isWhereBlock(step) && Array.isArray(step.where.steps)) {
        ...
        unnestSteps(step.where.steps, id);
      } else {
        // Add non-where steps
...
export const convertUIStepsToDSL = (
  ...
  for (const step of Object.values(idToStep)) {
    const { customIdentifier, parentId } = step;
