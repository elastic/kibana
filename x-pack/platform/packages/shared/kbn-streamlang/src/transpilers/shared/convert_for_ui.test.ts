...
import type { StreamlangStepWithUIAttributesWithCustomIdentifier } from '../../../types/ui';
...
describe('convertUIStepsToDSL', () => {
  it('converts a flat list of steps to DSL', () => {
    const uiSteps: StreamlangStepWithUIAttributesWithCustomIdentifier[] = [
    ...
    expect(dsl.steps[0].action).toBe('set');
    expect(dsl.steps[1].action).toBe('append');
  });
