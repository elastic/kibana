    ...
    for (const field of fieldsUsed) {
      ...
      if (expectedTypes) {
        ...
        if (actualType && actualType !== 'unknown' && !expectedTypes.includes(actualType)) {
          errors.push({
            type: 'type_mismatch',
            message: `Processor "${processorId}" expects field "${field}" to be of type ${expectedTypes.join(
              ' or '
      ...
      if (outputType !== 'unknown') {
        ...
        if (isConditional) {
          ...
          if (existingPotentialTypes.size > 1) {
            errors.push({
              type: 'mixed_type',
              message: `Field "${field}" has mixed types due to conditional processor "${processorId}". It can be ${Array.from(
                existingPotentialTypes
              ).join(' or ')}, which makes downstream usage ambiguous.`,
              processorId,
              field,
      ...
      if (nonNamespacedFields.length > 0) {
        for (const field of nonNamespacedFields) {
          errors.push({
            ...
            message: `The field "${field}" generated by processor "${processorId}" does not match the streams recommended schema - put custom fields into attributes, body.structured or resource.attributes`,
            processorId,
            field,
          });
        }
    ...
    if (reservedFields.length > 0) {
      ...
      if (reservedFieldViolations.length > 0) {
        for (const field of reservedFieldViolations) {
          errors.push({
            type: 'reserved_field',
            message: `Processor "${processorId}" is trying to modify reserved field "${field}"`,
            processorId,
            field,
          });
        }
