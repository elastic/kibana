...
const provenance = {
  ...
  methodologyNotes: CAVEAT_STATS_PATH
    ? (() => {
        ...
        return [
          ...
          `Judge of record is google-gemini-3.1-pro, applied as a single unified judge across every row of the 21 persona columns, so those comparisons share one instrument (the three columns noted above are the documented exception). It is ALSO a contestant: the Gemini 3.1 Pro row is self-judged and is disclosed here rather than hidden. Its measured self-preference on this rubric is +0.4 on a 0-100 rescale; it lands mid-field, so it does not drive the conclusion.`,
    ...
    : undefined,
  note: 'Judged evaluators recomputed by the unified judge; trace-based evaluators carried forward.',
};
