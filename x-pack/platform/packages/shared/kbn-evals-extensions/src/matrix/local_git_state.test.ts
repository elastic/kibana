...
// The source calls execFileSync with `encoding: 'utf8'`, so it reads a string
// back; the fully-typed signature resolves to the Buffer overload instead.
...
describe('readLocalGitState', () => {
  ...
  it('ignores untracked files when deciding dirtiness', () => {
    // Guards the --untracked-files=no flag: scratch files in a worktree must not
    // mark every local run as dirty.
    mockGit({
      'rev-parse HEAD': 'abc123\n',
      'status --porcelain --untracked-files=no': '',
    });
