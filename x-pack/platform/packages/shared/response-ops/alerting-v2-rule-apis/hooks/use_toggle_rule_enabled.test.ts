...
describe('useToggleRuleEnabled', () => {
  ...
  it('should enable a rule and show a success toast', async () => {
    (services.http.patch as jest.Mock).mockResolvedValue({ id: 'rule-1', enabled: true });

    const { result } = renderHook(() => useToggleRuleEnabled(services), {
    ...
    await waitFor(() => {
      expect(services.http.patch).toHaveBeenCalledWith('/internal/alerting/v2/rule/rule-1', {
        body: JSON.stringify({ enabled: true }),
      });
      expect(services.notifications.toasts.addSuccess).toHaveBeenCalledWith('Rule enabled');
  ...
  it('should disable a rule and show a success toast', async () => {
    (services.http.patch as jest.Mock).mockResolvedValue({ id: 'rule-1', enabled: false });
    ...
    await waitFor(() => {
      ...
      expect(services.notifications.toasts.addSuccess).toHaveBeenCalledWith('Rule disabled');
    });
  });

  it('should show a danger toast when the update fails', async () => {
    (services.http.patch as jest.Mock).mockRejectedValue(new Error('update failed'));
