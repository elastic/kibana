...
export const anomalyParamsSchema = schema.object(
  {
    serviceName: schema.maybe(
      schema.string({ meta: { description: 'The service name from APM.' } })
    ),
  ...
  {
    meta: {
      id: 'apm-anomaly-rule-params',
      title: 'APM Anomaly Rule Params',
      description:
        'The parameters for the APM anomaly rule. These parameters are appropriate when `rule_type_id` is `apm.anomaly`.',
    },
  }
);
