...
describe('buildEpisodeActionsQuery', () => {
  it('filters by episode ids and aggregates last ack action per episode', () => {
    ...
    expect(queryString).toContain('action_type IN ("ack", "unack")');
    expect(queryString).toContain('last_ack_action = LAST(action_type');
