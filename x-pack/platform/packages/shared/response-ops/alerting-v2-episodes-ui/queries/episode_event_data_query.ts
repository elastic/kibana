 ...
 * `.rule-events` document for the given episode.
 *
 ...
 * that carried actual alert evaluation data.
 */
export const buildEpisodeEventDataQuery = (episodeId: string) => {
  ...
  return esql.from([ALERT_EVENTS_DATA_STREAM], ['_source'])
    ...
    .pipe`INLINE STATS last_data = LAST(extracted_data, @timestamp) WHERE extracted_data != "{}" BY \`episode.id\``;
};
