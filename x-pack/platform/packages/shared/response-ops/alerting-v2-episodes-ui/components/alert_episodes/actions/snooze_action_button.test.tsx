...
describe('SnoozeActionButton', () => {
  ...
  it('renders Snooze with bell when not snoozed (after unsnooze)', () => {
    render(
      <SnoozeActionButton lastSnoozeAction={ALERT_EPISODE_ACTION_TYPE.UNSNOOZE} http={mockHttp} />
    );
    expect(screen.getByText('Snooze')).toBeInTheDocument();
  });

  it('renders Snooze with bell when previous action is undefined', () => {
    render(<SnoozeActionButton lastSnoozeAction={undefined} http={mockHttp} />);
  ...
  it('renders Unsnooze with bellSlash when snoozed', () => {
    render(
      <SnoozeActionButton lastSnoozeAction={ALERT_EPISODE_ACTION_TYPE.SNOOZE} http={mockHttp} />
    );
  ...
  it('opens popover with snooze form content on click', async () => {
    ...
    render(<SnoozeActionButton lastSnoozeAction={null} groupHash="gh-1" http={mockHttp} />);
