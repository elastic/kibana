...
export const createEpisodeActions = (deps: EpisodeActionsDeps): EpisodeAction[] => {
  ...
  const ext = (id: string) => additionalDataSource?.actionExtensions?.find((e) => e.actionId === id);
  ...
  return [
    ...
    createSnoozeAction(deps, ext('ALERTING_V2_SNOOZE_EPISODE')),
    ...
    createEditTagsAction(
      ...
      ext('ALERTING_V2_EDIT_EPISODE_TAGS')
    ),
