...
export const fetchClassicAlertsAsEpisodes = async ({
  ...
  const episodes = response.hits.hits.flatMap((hit) =>
    isClassicAlertSource(hit._source) && hit._index
      ? [mapClassicAlertToEpisode(hit._source, hit._index)]
      : []
  );
...
const enrichWithSnoozeState = async (
  ...
  return episodes.map((ep) => {
    const ctx = isClassicAlertActionContext(ep.source_action_context)
      ? ep.source_action_context
      : null;
    ...
    const isMuted = mutedByRule.get(ruleId)?.has(instanceId) ?? false;
