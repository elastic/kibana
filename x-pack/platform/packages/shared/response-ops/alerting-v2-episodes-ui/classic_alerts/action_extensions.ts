...
import {
  ...
  isClassicAlertActionContext,
} from './utils/map_alert';

const getActionContext = (episode: {
  source_action_context?: unknown;
}): ClassicAlertActionContext | null => {
  return isClassicAlertActionContext(episode.source_action_context)
    ? episode.source_action_context
    : null;
};
...
const updateWorkflowTags = async (
  ...
  for (const ep of episodes) {
    const ctx = getActionContext(ep);
    if (!ctx) continue;
    for (const tag of ctx.workflowTags) {
...
export const classicActionExtensions: Array<EpisodeActionExtension<any>> = [
  {
    ...
    isCompatible: (ep) => {
      const ctx = getActionContext(ep);
      if (!ctx) return false;
      return ctx.workflowStatus === 'open' || ctx.workflowStatus == null;
    },
      ...
      if (epsToUnsnooze.length > 0) {
        const results = await Promise.allSettled(
          epsToUnsnooze.map((ep) => {
            const ctx = getActionContext(ep);
            if (!ctx?.instanceId) return Promise.reject(new Error('No instance ID'));
