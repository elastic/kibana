/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import type { HttpStart } from '@kbn/core-http-browser';
import { useMutation, useQueryClient } from '@kbn/react-query';
import type { BulkCreateAlertActionBody } from '@kbn/alerting-v2-schemas';
import { ALERTING_V2_ALERT_API_PATH } from '@kbn/alerting-v2-constants';
import { queryKeys } from '../query_keys';

interface BulkCreateAlertActionsResponse {
  processed: number;
  total: number;
}

export const useBulkCreateAlertActions = (http: HttpStart) => {
  const queryClient = useQueryClient();

  return useMutation<BulkCreateAlertActionsResponse, Error, BulkCreateAlertActionBody>({
    mutationFn: (items) =>
      http.post<BulkCreateAlertActionsResponse>(`${ALERTING_V2_ALERT_API_PATH}/action/_bulk`, {
        body: JSON.stringify(items),
      }),
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.actionsAll() }),
        queryClient.invalidateQueries({ queryKey: queryKeys.groupActionsAll() }),
        queryClient.invalidateQueries({ queryKey: queryKeys.alertActionTagSuggestions() }),
      ]);
    },
  });
};
