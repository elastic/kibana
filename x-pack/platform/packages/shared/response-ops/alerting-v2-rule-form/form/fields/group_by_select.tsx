...
export const GroupFieldSelect: React.FC<GroupBySelectProps> = ({ services }) => {
  ...
  useEffect(() => {
    if (currentGroupingFields && currentGroupingFields.length > 0 && columnNames.size > 0) {
      const validValues = currentGroupingFields.filter((val: string) => columnNames.has(val));
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState: { error } }) => {
        const selectedOptions = (field.value ?? []).map((val: string) => ({ label: val }));
          ...
          >
            <EuiComboBox
              ...
              onCreateOption={(searchValue) => {
                field.onChange([...((field.value as string[]) ?? []), searchValue]);
              }}
              isClearable={true}
              isInvalid={!!error}
            />
