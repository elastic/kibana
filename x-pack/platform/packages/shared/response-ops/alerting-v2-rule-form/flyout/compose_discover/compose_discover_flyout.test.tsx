    ...
    it('shows "Edit {name}" in edit mode when the rule has a name', () => {
      renderFlyout({
        ...
        rule: {
          ...
          kind: 'alert',
          enabled: true,
          metadata: { name: 'CPU high', signature_id: 'test-sig-id', version: 1, tags: [] },
    ...
    it('stays on alert condition step when validateStep fails', async () => {
      renderFlyout({
        ...
        rule: {
          ...
          kind: 'signal',
          enabled: true,
          metadata: { name: 'Signal rule', signature_id: 'test-sig-id', version: 1, tags: [] },
          time_field: '@timestamp',
          schedule: { every: '1m', lookback: '5m' },
          query: { format: 'standalone', breach: { query: '' } },
          created_by: { profile_uid: 'test' },
    ...
    it('runs heuristic split and commits the result in edit + alert unified editor', () => {
      renderFlyout({
        ...
        rule: {
          ...
          kind: 'alert',
          enabled: true,
          metadata: { name: 'Edit rule', signature_id: 'test-sig-id', version: 1, tags: [] },
          time_field: '@timestamp',
          schedule: { every: '1m', lookback: '5m' },
          query: {
            format: 'composed',
