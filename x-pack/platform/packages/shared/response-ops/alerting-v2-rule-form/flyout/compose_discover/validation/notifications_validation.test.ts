...
import { isNotificationsStepValid, validateNotifications } from './notifications_validation';
...
describe('notifications validation', () => {
  ...
  describe('validateNotifications', () => {
    it('returns true when notifications are valid', () => {
      expect(validateNotifications(undefined)).toBe(true);
      expect(
    ...
    it('returns an error string for incomplete actions', () => {
      const result = validateNotifications({
        workflows: [{ id: 'item-1', source: 'existing', workflowId: null }],
      });
      expect(typeof result).toBe('string');
      expect(result).toMatch(/incomplete actions/i);
    });
  });
});
