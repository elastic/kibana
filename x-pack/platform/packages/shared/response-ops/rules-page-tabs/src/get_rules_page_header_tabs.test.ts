...
describe('getRulesPageHeaderTabs', () => {
  ...
  it('omits the v1 tab when showV1Tab is false', () => {
    const tabs = getRulesPageHeaderTabs({
      selectedTab: RULES_PAGE_TAB_IDS.v2,
      prepend,
      showV1Tab: false,
    });

    expect(tabs.map((tab) => tab.id)).toEqual([RULES_PAGE_TAB_IDS.v2]);
  });

  it('omits the v2 tab when showV2Tab is false', () => {
    const tabs = getRulesPageHeaderTabs({
    ...
    expect(tabs.map((tab) => tab.id)).toEqual([RULES_PAGE_TAB_IDS.v1]);
  });
});
