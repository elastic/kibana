 ...
 * - `code`: derived only from source code (e.g. Semantic Code Search).
 * - `logs`: derived only from log/data analysis (the historical default).
 * - `both`: corroborated by both code and logs.
 */
export type KnowledgeIndicatorSource = 'code' | 'logs' | 'both';

export const knowledgeIndicatorSourceSchema = z.enum(['code', 'logs', 'both']);
