...
describe('deriveKnowledgeIndicatorSource', () => {
  it('returns logs when there is no evidence', () => {
    expect(deriveKnowledgeIndicatorSource()).toBe('logs');
    expect(deriveKnowledgeIndicatorSource([])).toBe('logs');
  });

  it('returns code when all evidence is code-prefixed', () => {
    expect(deriveKnowledgeIndicatorSource(['code: acme/checkout pay.go error("boom")'])).toBe(
      'code'
    );
  });

  it('returns logs when no evidence is code-prefixed', () => {
    expect(deriveKnowledgeIndicatorSource(['logs: observed pattern', 'sampled 42 docs'])).toBe(
      'logs'
    );
  });
