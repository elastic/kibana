...
// TODO: props like isEdit, enforceAdaptiveAllocations, isPreconfigured, currentSolution, providerInContext can be in a config object
// Defaults can live in this package and we can check providerId on load to set default values
export const InferenceServiceFormFields: React.FC<InferenceServicesProps> = ({
  ...
  // TODO: do we need to set provider watch conditionally?
  ...
  useEffect(
    function setDefaultProviderSchema() {
      if (providerId && updatedProviders) {
        providerSetHandler(providerId);
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [providerId, updatedProviders]
  );
  ...
  const providerSetHandler = useCallback(
    (provider?: string) => {
      ...
      const providerDefaults =
        SERVICE_PROVIDERS[(provider ?? newProvider?.service) as ServiceProviderKeys].defaults;
      ...
      newProviderSchema.forEach((fieldConfig) => {
        if (!fieldConfig.sensitive) {
          ...
          } else if (providerDefaults && providerDefaults[fieldConfig.key]) {
            defaultProviderConfig[fieldConfig.key] = providerDefaults[fieldConfig.key];
          } else {
  ...
  return !isLoading ? (
    <>
      <ProviderSelect
        ...
        toggleAndApplyFilter={toggleAndApplyFilter}
