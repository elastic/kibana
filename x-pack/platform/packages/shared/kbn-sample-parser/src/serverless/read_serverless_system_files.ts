...
import { listServerlessSystemFiles } from './list_serverless_system_files';
...
export async function readServerlessSystemFiles({
  ...
  const files = await listServerlessSystemFiles({ log });

  const limiter = pLimit(50);

  const systems = await Promise.all(
    files.flatMap(async (file): Promise<ServerlessSystem[]> => {
      const fileContents = await limiter(() => getFileOrThrow(Path.join(dirName, file)));
