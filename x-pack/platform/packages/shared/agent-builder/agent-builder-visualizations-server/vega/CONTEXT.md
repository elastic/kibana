...
**Dialect gate**:
The allowlist of Raw Vega chart intents (starting with Sunburst). Selection is performed by a small classifier model call that returns a catalog id or `none`; when `none`, authoring stays within Vega-Lite (unsupported diagrams still get an honest refusal).
_Avoid_: model-chosen free-form dialect, keyword-only routing

**Catalog id**:
A stable identifier for a curated Raw Vega (or later shared) chart intent returned by the Dialect gate classifier (e.g. `sunburst`), or `none` when no allowlisted intent matches.
...
**Disclosed fallback**:
When Sunburst is selected but a usable Parent–child table cannot be produced, do not silently claim success as a sunburst: produce the closest Vega-Lite approximation and tell the user sunburst was not possible.
_Avoid_: silent VL fallback, hard fail with no alternative chart

**Allowlist refusal**:
For Raw Vega diagram intents not yet in the Dialect gate allowlist (Sankey, radar, network, …), do not author Raw Vega: refuse honestly and offer Vega-Lite / Lens / multi-chart alternatives. Widen the allowlist chart-by-chart.
