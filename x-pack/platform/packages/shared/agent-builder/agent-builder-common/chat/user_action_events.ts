...
interface BaseUserActionEvent extends BaseTimelineEvent<'user_action'> {
  user: UserIdAndName;
  action: UserActionType;
}

export interface FieldChangedUserActionEvent extends BaseUserActionEvent {
  action: UserActionType.field_changed;
  payload: FieldChangedUserActionPayload;
}

export interface AttachmentAddedUserActionEvent extends BaseUserActionEvent {
  action: UserActionType.attachment_added;
  payload: AttachmentAddedUserActionPayload;
}

export type UserActionEvent = FieldChangedUserActionEvent | AttachmentAddedUserActionEvent;
...
export const createFieldChangedUserActionEvents = ({
  ...
  for (const field of keys) {
    ...
    events.push({
      ...
      payload: {
        field,
        previous_value: serializeAuditValue(previousValue),
...
export const createAttachmentAddedUserActionEvent = ({
  ...
  user: UserIdAndName;
  timestamp?: string;
}): AttachmentAddedUserActionEvent => ({
  id: uuidv4(),
  timestamp,
  type: TimelineEventType.user_action,
  user,
