...
import type { ToolEventEmitter, ModelProvider, ScopedRunner, ToolProvider, ToolResultStore, SkillsStore, ToolPromptManager, ToolStateManager, ToolManager, RunContext } from '../runner';
 ...
 */
export type ToolHandlerResult<TResult extends ToolResult = ToolResult> = Omit<TResult, 'tool_result_id'> & {
 ...
 * Return value for {@link ToolHandlerFn} / {@link BuiltinToolDefinition}
 */
export type ToolHandlerReturn<TResult extends ToolResult = ToolResult> = ToolHandlerStandardReturn<TResult> | ToolHandlerPromptReturn;
export declare const isToolHandlerInterruptReturn: (toolReturn: ToolHandlerReturn) => toolReturn is ToolHandlerPromptReturn;
export declare const isToolHandlerStandardReturn: (toolReturn: ToolHandlerReturn) => toolReturn is ToolHandlerStandardReturn;
/**
 * Tool handler function for {@link BuiltinToolDefinition} handlers.
 */
export type ToolHandlerFn<TParams extends Record<string, unknown> = Record<string, unknown>, TResult extends ToolResult = ToolResult> = (args: TParams, context: ToolHandlerContext) => MaybePromise<ToolHandlerReturn<TResult>>;
