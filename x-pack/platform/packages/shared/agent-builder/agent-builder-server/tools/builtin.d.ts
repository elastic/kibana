...
import type { ToolCallWithResult, ToolDefinition, ToolType, ToolConfirmationPolicy } from '@kbn/agent-builder-common';
 ...
 */
export type ToolAvailabilityHandler = (context: ToolAvailabilityContext) => MaybePromise<ToolAvailabilityResult>;
...
export interface BuiltInToolConfirmationPolicy<TParams extends Record<string, unknown> = Record<string, unknown>> extends ToolConfirmationPolicy {
    /**
...
export interface BuiltInToolSpecificConfig<TParams extends Record<string, unknown> = Record<string, unknown>> {
    /**
 ...
 */
export type ToolReturnSummarizerFn = (toolReturn: ToolCallWithResult) => ToolCallWithResult['results'] | undefined;
/**
 * Built-in tool, as registered as static tool.
 */
export interface BuiltinToolDefinition<RunInput extends ZodObject<any> = ZodObject<any>, TResult extends ToolResult = ToolResult> extends Omit<ToolDefinition, 'type' | 'readonly' | 'configuration' | 'experimental' | 'confirmation'>, BuiltInToolSpecificConfig<z.infer<RunInput>> {
    /**
     * built-in tool types
     */
...
type StaticToolRegistrationMixin<T extends ToolDefinition> = Omit<T, 'readonly' | 'experimental'> & BuiltInToolSpecificConfig;
...
export type StaticToolRegistration<RunInput extends ZodObject<any> = ZodObject<any>, TResult extends ToolResult = ToolResult> = BuiltinToolDefinition<RunInput, TResult> | StaticEsqlTool | StaticIndexSearchTool | StaticWorkflowTool;
export {};
