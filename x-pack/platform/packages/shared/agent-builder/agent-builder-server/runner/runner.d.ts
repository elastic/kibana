 ...
 */
export type ScopedRunToolFn = <TParams = Record<string, unknown>>(params: ScopedRunnerRunToolsParams<TParams>) => Promise<RunToolReturn>;
/**
 * Public agentBuilder API to execute a tools.
 */
export type ScopedRunInternalToolFn = <TParams = Record<string, unknown>>(params: ScopedRunnerRunInternalToolParams<TParams>) => Promise<RunToolReturn>;
/**
...
export type RunInternalToolParams<TParams = Record<string, unknown>> = Omit<RunToolParams<TParams>, 'toolId'> & {
    tool: InternalToolDefinition<ToolType, any, any>;
};
/**
 * Params for {@link ScopedRunner.runTool}
 */
export type ScopedRunnerRunToolsParams<TParams = Record<string, unknown>> = Omit<RunToolParams<TParams>, 'request' | 'promptState'>;
export type ScopedRunnerRunInternalToolParams<TParams = Record<string, unknown>> = Omit<RunInternalToolParams<TParams>, 'request' | 'promptState'>;
/**
 * Public agentBuilder API to execute a tools.
 */
export type RunToolFn = <TParams = Record<string, unknown>>(params: RunToolParams<TParams>) => Promise<RunToolReturn>;
export type RunInternalToolFn = <TParams = Record<string, unknown>>(params: RunInternalToolParams<TParams>) => Promise<RunToolReturn>;
