 ...
 */
export interface ExecutableTool<TConfig extends object = {}, TSchema extends ZodObject<any> = ZodObject<any>> extends ToolDefinition<ToolType, TConfig> {
    /**
...
export type LlmDescriptionHandler<TConfig extends object = {}> = (params: LLmDescriptionHandlerParams<TConfig>) => MaybePromise<string>;
/**
 * Param type for {@link ExecutableToolHandlerFn}
 */
export type ExecutableToolHandlerParams<TParams = Record<string, unknown>> = Omit<ScopedRunnerRunToolsParams<TParams>, 'toolId'>;
/**
 * Execution handler for {@link ExecutableTool}
 */
export type ExecutableToolHandlerFn<TParams = Record<string, unknown>> = (params: ExecutableToolHandlerParams<TParams>) => Promise<RunToolReturn>;
/**
