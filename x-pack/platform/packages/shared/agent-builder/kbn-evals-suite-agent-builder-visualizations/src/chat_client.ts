...
interface ConverseStep {
  ...
  // trace-based evaluators / visualization extractor can introspect them
  // without modelling the full Agent Builder step union here.
  [k: string]: unknown;
}
 ...
 * standalone visualization generation suite.
 *
 * The platform-side `AgentBuilderEvaluationChatClient`
 * (`@kbn/evals-suite-agent-builder`) and the dashboards suite ship near
 * identical wrappers, but each lives in a sibling `functional-tests` module
 * with `visibility: "private"` and no public entry point, so Kibana's
 * module-visibility rules forbid importing across suites. We keep a local
 * copy with the same retry policy, default agent id, and fallback response
 * shape.
 */
export class VisualizationAgentEvaluationChatClient {
  constructor(
