 ...
 * `@kbn/evals-suite-security-esql-generation-regression` (module-visibility
 * rules forbid importing across sibling `functional-tests` suites).
 *
...
>(config: {
  ...
  return {
    ...
    evaluate: async ({ output, expected }): Promise<EvaluationResult> => {
      ...
      // Normalize then substitute `?_tstart` / `?_tend` before execution.
      // Redundant `@timestamp` bind-param WHERE bounds are stripped so
