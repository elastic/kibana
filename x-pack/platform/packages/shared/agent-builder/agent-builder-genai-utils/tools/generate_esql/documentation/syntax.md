...
The Elasticsearch Query Language (ES|QL) provides a powerful and flexible way to query, filter, transform, and analyze data stored in Elasticsearch.

An ES|QL query is composed of a source command followed by a series of processing commands, separated by a pipe character: |.
...
- LOOKUP JOIN: adds fields from the lookup index as new columns to your results table based on matching values in the join field
- WHERE: Filters rows based on a boolean condition. WHERE supports the same functions as EVAL
- RERANK: uses an inference model to compute a new relevance score for an initial set of documents
- COMPLETION: send prompts and context to an LLM
...
- (preview) FUSE: merges rows from multiple result sets and assigns new relevance scores. Used together with `FORK`
...
- CLAMP: Limits the values of all samples to have a lower limit of min and an upper limit of max
- CLAMP_MIN: Limits all input sample values to a lower bound of min
- CLAMP_MAX: Limits all input sample values to an upper bound of max


## Search functions

- DECAY: Calculates a relevance score that decays based on the distance of a numeric, spatial or date type value
...
- MATCH_PHRASE: perform a match_phrase on the specified field
- QSTR: perform a Lucene query string query
- SCORE: Scores an expression
- TOP_SNIPPETS: Extract the best snippets for a given query string from a text field
...
- INCREASE: Calculates the absolute increase in a time window
...
- PRESENT_OVER_TIME: Calculates the presence of a field in the output result over time range
...
Timespan literals represent datetime intervals and are expressed as a combination of a number and a temporal unit (e.g., `1 day`, `24h`, `7 weeks`). 

They are not whitespace-sensitive:
```esql
 1day
 1 day
 1       day
