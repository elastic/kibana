...
streamlangApiTest.describe(
  ...
  { tag: ['@ess', '@svlOblt'] },
  () => {
    streamlangApiTest('should append a value to an existing field', async ({ testBed, esql }) => {
    ...
    streamlangApiTest(
      'should append a value to a non-existent field',
      async ({ testBed, esql }) => {
    ...
    streamlangApiTest(
      ...
      async ({ testBed, esql }) => {
        ...
        await testBed.ingest(indexName, docs);
        const esqlResult = await esql.queryOnIndex(indexName, query);
        expect(esqlResult.documents[0].tags).toEqual(['existing_tag', 'existing_tag']);
      }
    );

    streamlangApiTest(
      'should not append a value when where is false',
      async ({ testBed, esql }) => {
    ...
    streamlangApiTest('should support nested where with not', async ({ testBed, esql }) => {
