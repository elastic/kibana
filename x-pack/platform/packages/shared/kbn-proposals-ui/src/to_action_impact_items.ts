 ...
 *
 * Message ids stay under `xpack.proposals.*` from when this lived in that plugin:
...
export const toActionImpactItems = (proposal: ApprovalProposal): ActionImpactItem[] => {
  ...
  items.push({
    id: 'impact',
    iconType: 'warning',
    text: i18n.translate('xpack.proposals.proposals.actionImpact.impactText', {
  ...
  if (proposal.action?.reversible !== undefined) {
    items.push({
      ...
      text: proposal.action.reversible
        ? i18n.translate('xpack.proposals.proposals.actionImpact.reversibleText', {
            defaultMessage: 'Reversible',
          })
        : i18n.translate('xpack.proposals.proposals.actionImpact.notReversibleText', {
            defaultMessage: 'Not reversible',
          }),
      status: {
        label: proposal.action.reversible
          ? i18n.translate('xpack.proposals.proposals.actionImpact.reversibleYes', {
              defaultMessage: 'Yes',
            })
          : i18n.translate('xpack.proposals.proposals.actionImpact.reversibleNo', {
              defaultMessage: 'No',
            }),
  ...
  if (proposal.expiresAt) {
    ...
    const label = proposal.expired
      ? i18n.translate('xpack.proposals.proposals.actionImpact.expiredLabel', {
    ...
    items.push({
      id: 'expires',
      iconType: 'clock',
      text: i18n.translate('xpack.proposals.proposals.actionImpact.decisionDeadlineText', {
