...
export const useFocusedStepOutline = (
  ...
  useEffect(() => {
    if (!editor || !jsonLineMap) {
      // Clear focused step info when editor or jsonLineMap is not available
    ...
    const updateFocusedStep = () => {
      ...
      const isSuggestWidgetVisible =
        suggestController?.model?.state !== undefined && suggestController.model.state !== 0;
      ...
      const matchingSteps = Object.entries(jsonLineMap).filter(([, lineInfo]) => {
  ...
  useEffect(() => {
    ...
    for (
      ...
      let className = 'ingest-pipeline-step-selected-middle';
      if (isSingleLine) {
        className = 'ingest-pipeline-step-selected-single';
      } else if (isFirstLine) {
        className = 'ingest-pipeline-step-selected-first';
      } else if (isLastLine) {
        className = 'ingest-pipeline-step-selected-last';
      }
