...
- **ES|QL Execution Validity** (`CODE`, from `@kbn/evals`) — AST parse + execute against real sample data and (per example) return rows. This is the tier that surfaces the fast-model regressions that motivated the suite.
- **ES|QL Functional Equivalence** (`LLM` calibrated judge, from `@kbn/evals`) — three-point rubric (`equivalent` / `equivalent_with_caveats` / `not_equivalent`) with cosmetic allow-lists for aliases, bucketing, and `?_tstart`/`?_tend` bounds.
...
A standalone ES|QL Validity evaluator also lives in `@kbn/evals` (`createEsqlValidityEvaluator`) but is not in the default set here — execution already covers AST validation.
...
This keeps gold and candidate structurally parallel so the equivalence evaluators measure real differences instead of cosmetic ones. The `?_tstart` / `?_tend` bind params substitute to a **now-relative** window (`getDefaultTimeBounds` / `substituteEsqlBindParams` in `@kbn/evals`), which brackets both `kibana_sample_data_logs` and the GCS snapshot replay data (whose timestamps are shifted to end at `now` by the replay pipeline).
