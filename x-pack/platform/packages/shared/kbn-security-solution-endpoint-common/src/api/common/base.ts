...
import { SUPPORTED_HOST_OS_TYPE } from '../../os_types';
import { RESPONSE_ACTION_AGENT_TYPE } from '../../constants';

/**
 * Maximum length of the `comment` field on a response action request.
 *
 * Deliberately declared here rather than shared with security_solution's MAX_COMMENT_LENGTH,
 * which is related to comments in the rule exceptions and event filters UI.
 */
export const MAX_RESPONSE_ACTION_COMMENT_LENGTH = 30000 as const;
...
export const BaseActionRequestSchema = {
  ...
  comment: schema.maybe(schema.string({ maxLength: MAX_RESPONSE_ACTION_COMMENT_LENGTH })),
