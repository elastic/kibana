...
// Now-relative window: wide enough to bracket kibana_sample_data_logs (install-time) and the
// synthtrace host-load fixture. A 100-year span collapsed auto-bucket queries to 1 row, so
// moderate bounds keep bucketing meaningful. Evaluated per call so the window tracks `now`.
