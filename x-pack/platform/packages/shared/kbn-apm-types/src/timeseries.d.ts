import type { AccessorFn, AreaSeriesStyle, Fit, FitConfig, LineSeriesStyle, RecursivePartial, SeriesColorAccessorFn, SeriesColorsArray } from '@elastic/charts';
...
export type TimeSeries<TCoordinate extends {
    x: number;
} = Coordinate | RectCoordinate | BandCoordinate> = APMChartSpec<TCoordinate>;
export interface APMChartSpec<TCoordinate extends {
    x: number;
} = Coordinate | RectCoordinate | BandCoordinate> {
    title: string;
    id?: string;
    titleShort?: string;
    hideLegend?: boolean;
