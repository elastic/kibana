...
export interface APMUsage {
    ...
    retainment: Record<'span' | 'transaction' | 'error' | 'metric' | 'onboarding', {
        ms: number;
    }>;
    integrations: {
        ml: {
    ...
    tasks: Record<'aggregated_transactions' | 'cloud' | 'host' | 'processor_events' | 'agent_configuration' | 'global_labels' | 'services' | 'versions' | 'groupings' | 'integrations' | 'agents' | 'indices_stats' | 'cardinality' | 'environments' | 'service_groups' | 'custom_dashboards' | 'per_service' | 'top_traces' | 'per_agent_config_settings', {
        took: {
            ms: number;
        };
    }>;
}
export type MetricRollupIntervals = RollupInterval.OneMinute | RollupInterval.TenMinutes | RollupInterval.SixtyMinutes;
export type MetricSupportingRollUp = 'service_destination' | 'transaction' | 'service_summary' | 'service_transaction' | 'span_breakdown';
