...
describe('createConnectorFixture', () => {
  ...
  describe('with a .gen-ai connector', () => {
    ...
    it('routes through the generic Actions path identically to other stack connectors', async () => {
      mockFetch.mockResolvedValueOnce({ is_preconfigured: false }).mockResolvedValueOnce(undefined);
      ...
      // Same call sequence as the `.email` case: preconfigured check, then Actions create.
      expect(mockFetch).toHaveBeenCalledTimes(2);
      expect(mockFetch).toHaveBeenNthCalledWith(1, {
      ...
      // No inference endpoint calls at all
      const inferenceCalls = mockFetch.mock.calls.filter(([arg]: [{ path: string }]) =>
        arg.path.startsWith('/internal/_inference')
      );
