...
function withId(example: EvaluationExample, id: string): PhoenixExampleWithId {
  return { ...(example as unknown as PhoenixExampleWithId), id };
}

describe('diffExamples', () => {
  const baseExample: EvaluationExample = {
    input: { prompt: 'hello' },
    output: { completion: 'world' },
    metadata: { split: 'train' },
  };

  const secondExample: EvaluationExample = {
    input: { prompt: 'foo' },
  ...
  it('identifies a new example to add', () => {
    const stored: PhoenixExampleWithId[] = [];
    const next = [baseExample];
