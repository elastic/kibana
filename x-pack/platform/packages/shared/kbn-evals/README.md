...
Evaluation scores are stored in the `.evaluation-scores` data stream. Each document represents one evaluator score for one example (and repetition):
...
```json
{
  ...
  "example": {
    "id": "example-1",
    "index": 0,
    "dataset": { "id": "dataset_id", "name": "my-dataset" }
  },
  "task": {
