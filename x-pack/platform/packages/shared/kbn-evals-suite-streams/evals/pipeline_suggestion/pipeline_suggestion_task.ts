...
/**
 * Schedule and poll the background pipeline suggestion task until completion.
 */
const waitForPipelineSuggestionResult = async (params: {
  kbnClient: KbnClient;
  streamName: string;
  connectorId: string;
  ...
  await kbnClient.request({
    method: 'POST',
    path: `/internal/streams/${streamName}/_pipeline_suggestion/_task`,
    body: {
  ...
  while (true) {
    const statusResponse = await kbnClient.request({
      method: 'GET',
      path: `/internal/streams/${streamName}/_pipeline_suggestion/_status`,
    });
    ...
    if (result.status === TaskStatus.Failed) {
      throw new Error((result as { error?: string }).error ?? 'Pipeline suggestion task failed');
    }
