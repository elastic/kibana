...
const DATASET_MARKER = '  // 🔧 NEW DATASETS GO HERE - Added by create_dataset_from_clipboard.ts';
...
async function insertDatasetEntry(
  ...
  if (datasetType === 'pipeline') {
    ...
    if (markerPosition !== -1) {
      ...
      console.log(
        chalk.green(`✅ Dataset entry inserted into ${datasetConstant} section in ${filePath}`)
      );
      return;
    }
  ...
  await writeFile(filePath, newContent, 'utf-8');
  console.log(chalk.green(`✅ Dataset entry added to ${filePath}`));
}

async function main() {
  console.log(chalk.bold('📋 Create Dataset from Clipboard\n'));
  console.log(chalk.cyan('💡 To use this script, run:'));
  console.log(
    chalk.gray(
      '   pbpaste | node --require ./src/setup_node_env/ ./x-pack/platform/packages/shared/kbn-evals-suite-streams/scripts/create_dataset_from_clipboard.ts'
    )
  ...
  console.log();
  console.log(chalk.bold('📖 Reading from stdin...\n'));
  ...
  try {
    ...
    if (!inputText || inputText.trim().length === 0) {
      ...
      console.error(chalk.yellow('\n💡 Did you forget to pipe the clipboard? Try:'));
      console.error(
        chalk.gray(
          '   pbpaste | node --require ./src/setup_node_env/ ./x-pack/platform/packages/shared/kbn-evals-suite-streams/scripts/create_dataset_from_clipboard.ts'
        )
    ...
    console.error(chalk.red(`❌ Error reading stdin: ${error}`));
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(chalk.red('❌ Input does not contain valid JSON'));
  ...
  if (!data.suggestionType || !data.raw_samples || !data.suggestion) {
    console.error(chalk.red('❌ Invalid clipboard data structure'));
    console.error(chalk.gray('Expected: { suggestionType, raw_samples, suggestion }'));
    console.error(chalk.gray('Got:'), Object.keys(data));
    process.exit(1);
  }

  console.log(chalk.blue(`📊 Suggestion type: ${data.suggestionType}`));
  console.log(chalk.blue(`📝 Raw samples: ${data.raw_samples.length}`));
  console.log();

  const streamName = generateStreamName(data.raw_samples);
  console.log(chalk.blue(`🏷️  Inferred stream name: ${streamName}\n`));
  ...
  try {
    switch (data.suggestionType) {
      ...
      default:
        console.error(chalk.red(`❌ Unknown suggestion type: ${data.suggestionType}`));
        process.exit(1);
    }

    console.log();
    console.log(chalk.green('✨ Dataset entry created successfully!'));
    console.log();
    ...
    console.error(chalk.red('❌ Error creating dataset entry:'), error);
    process.exit(1);
  }
}
