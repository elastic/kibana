 ...
 * Format: agentBuilder/<directory>/<dataset> -> { directory: "directory", dataset: "dataset" }
 */
function parseAgentBuilderDatasetName(datasetName: string): { directory: string; dataset: string } {
  // Remove "agentBuilder/" prefix
  const withoutPrefix = datasetName.startsWith('agentBuilder/')
    ? datasetName.slice('agentBuilder/'.length)
    : datasetName;

  const parts = withoutPrefix.split('/');
  if (parts.length !== 2) {
    throw new Error(
      `Invalid AgentBuilder dataset format: '${datasetName}'. Expected format: agentBuilder/<directory>/<dataset>, got ${datasetName}.`
    );
  }
 ...
 * Expected format: agentBuilder/<directory>/<dataset>
 */
export function isAgentBuilderDataset(datasetName: string): boolean {
  return datasetName.startsWith('agentBuilder/') && datasetName.split('/').length === 3;
}
...
export async function listAgentBuilderDatasets(
  ...
  const mappings = await getAgentBuilderIndexMappings(directory, accessToken, logger);
  return Array.from(mappings.keys()).map((name) => `agentBuilder/${directory}/${name}`);
}
