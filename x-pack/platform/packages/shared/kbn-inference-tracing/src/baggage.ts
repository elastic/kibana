...
/**
 * W3C baggage key used by eval runs to tag all inference spans with the eval run ID.
 *
 ...
 */
export const EVAL_RUN_ID_BAGGAGE_KEY = 'kibana.evals.eval_run_id';

/**
 * W3C baggage key used by eval runs to tag all inference spans with the experiment ID.
 *
 * Unlike eval_run_id (which is per-worker/suite-run), this changes per experiment
