...
describe('pruneContentReferences', () => {
  ...
  it('prunes content references correctly from content and store', async () => {
    ...
    )} example ${contentReferenceBlock(alertsPageReference2)} {reference(a54d4aa3-40f8-4c51-ad58-f1f1bde57_3c7)} {reference(1234)}`;
    ...
    expect(prunedContentReferences.prunedContent).toEqual(`Example {reference(${alertsPageReference1.id})} example {reference(${alertsPageReference2.id})}  `); 
  });
});
