...
function setupTest({
    ...
    const utils = render(
        <>
            <DefaultAIConnector connectors={mockConnectors} uiSetting={settings} toast={{} as IToasts} application={{
                getUrlForApp: jest.fn()
            } as unknown as ApplicationStart} docLinks={{} as DocLinksStart} />
        </>,
        {
            wrapper: ({ children }) => (
                <I18nProvider>
...
describe('DefaultAIConnector', () => {
    ...
    describe('combobox interaction', () => {
        ...
        it('updates selection when connector is chosen', async () => {
            ...
            expect(handleFieldChange).toHaveBeenCalledWith("genAiSettings:defaultAIConnector", { "type": "string", "unsavedValue": "custom1" })
        });
    });
    ...
    describe('checkbox interaction', () => {
        it('updates checkbox state when clicked', async () => {
            ...
            expect(handleFieldChange).toHaveBeenCalledWith("genAiSettings:defaultAIConnectorOnly", { "type": "boolean", "unsavedValue": true });
        });
    });
});