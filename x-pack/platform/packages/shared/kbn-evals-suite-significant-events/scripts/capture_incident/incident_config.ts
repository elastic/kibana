...
import { OVERVIEW_API_KEY_ENV } from './constants';
...
export function readIncidentConfig(configPath: string): IncidentConfig {
  ...
  if (!result.success) {
    const issues = result.error.issues
      ...
      .join('\n');
    throw new Error(`Invalid incident config "${absolutePath}":\n${issues}`);
  }
...
export function resolveIncidentConfig(config: IncidentConfig): ResolvedIncidentConfig {
  const resolvedApiKey = process.env[OVERVIEW_API_KEY_ENV] || config.source.apiKey || '';
  if (!resolvedApiKey) {
    throw new Error(
      `Missing source API key. Set the ${OVERVIEW_API_KEY_ENV} environment variable ` +
        `(preferred) or add "source.apiKey" to the config file.`
    );
  }

  const gcsBasePath =
    config.snapshot?.gcsBasePath ?? `customer0-incidents/incident-${config.incident.id}`;
...
/**
 * Builds the reindex source query — the broad "snapshot" filter over the
 * incident's time range. With no filter, only the time range is applied. This is
 * what gets reindexed and snapshotted (Query DSL; remote `_reindex` can't run ES|QL).
 */
export function buildSnapshotQuery(config: ResolvedIncidentConfig): QueryDslQueryContainer {
  return buildFilterQuery(config.query.timeRange, resolveSnapshotFilter(config));
}
