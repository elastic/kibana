...
import { splitClusterAlias } from './incident_utils';
...
const incidentConfigSchema = z.object({
  ...
  query: z.object({
    ...
    // The evidence-only Query DSL query that LOCATES and CONFIRMS the incident —
    // error clauses only (message / log.level / …), no entity scoping. NOT
    // snapshotted; stored so it can be replayed against the restored index to isolate
    // the error lines. A plain DSL query, same as `snapshot`.
    symptom: queryDslSchema.optional(),
    // The broad Query DSL query that is actually reindexed and snapshotted (noise
    // included). In auto mode it is built deterministically from where the symptom's
    // hits land — the concentrated entity, else the symptom's datasets. Remote
    // `_reindex` accepts Query DSL only, not ES|QL. Omit it (or `{}`) to capture the
    // whole dataset within `timeRange`.
...
export function buildIncidentConfigYaml(config: IncidentConfig): string {
  ...
  const header = [
    ...
    '# -----------------------------------------------------------------------------',
    '# Auto-generated by `capture_incident_snapshot.js --incident-id`. The evidence-only',
    '# query.symptom is derived by the Overview Agent Builder; the broad query.snapshot',
    '# is built deterministically from where its hits land (the concentrated entity,',
    '# else the symptom datasets) and confirmed by a probe against the Overview source.',
    '# Two queries, both plain Query DSL:',
...
export function resolveIncidentConfig(config: IncidentConfig): ResolvedIncidentConfig {
  ...
  const excludes = (config.source.exclude ?? []).map((pattern) => {
    const [prefix, bare] = splitClusterAlias(pattern);
    return `${prefix}-${bare}`;
  });
 ...
 *
 * `timeRange` defaults to the config's full window, but callers can pass a narrower
 * sub-window (the reindex is chunked by time so each scroll stays small and retryable).
 */
export function buildSnapshotQuery(
  config: ResolvedIncidentConfig,
  timeRange: { gte: string; lt: string } = config.query.timeRange
): QueryDslQueryContainer {
  ...
  return buildFilterQuery(timeRange, scope);
}
