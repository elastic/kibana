 ...
 */
export function createToolTrajectoryEvaluator(): Evaluator {
  return createTrajectoryEvaluator({
    orderWeight: 0,
    coverageWeight: 1,
    extractToolCalls: (output) => extractToolCallIds((output as StepsOutput)?.steps ?? []),
    goldenPathExtractor: (expected) =>
      (expected as ExpectedTools)?.expectedTools ?? DEFAULT_DISCOVERY_TOOLS,
  });
}
 ...
 */
export function createExecuteEsqlGroundingEvaluator(): Evaluator {
  return {
    name: 'execute_esql_grounding',
    kind: 'CODE',
    evaluate: ({ output }) => {
      const { calls, callsWithRows } = summarizeEsqlGrounding((output as StepsOutput)?.steps ?? []);
