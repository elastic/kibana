...
const STATUS_DECISION_RUBRIC = [
  ...
  '',
  'Status gates:',
  '- `open`: a current failure, material degradation, or sensitive-data exposure has a `confirms` verdict, or an `inconclusive`/`not_checked` verdict leaves one plausible. A concrete non-benign error in a found `off_topic` row directly confirms a separate observed-error event; judge that event from the row’s error signature and impact.',
