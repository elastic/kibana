...
import { BENIGN_AUTH_DISCOVERY, LEDGER_DB_CASCADE_DISCOVERY } from './discovery';

/**
 * End-to-end pipeline scenarios: replayed logs -> seeded canonical KI queries -> synthesized
 ...
 */
export const e2e: DatasetConfig['e2e'] = [
  {
    input: {
      scenario_id: 'ledger-db-disconnect',
      stream_name: 'logs',
    },
    canonical_queries: e2eCanonicalQueries,
    live: {
  ...
  },
  {
    ...
    output: {
      ...
      criteria: [
        {
          id: 'e2e-baseline-no-open-events',
          text: 'No significant event ends the run with status=open — healthy banking traffic must not raise an incident.',
          score: 3,
        },
        {
          id: 'e2e-baseline-benign-dismissed',
