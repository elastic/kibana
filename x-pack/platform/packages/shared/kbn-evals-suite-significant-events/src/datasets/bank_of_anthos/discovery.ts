 ...
 * and stamp `confirmed: true` itself before promoting. Every signal here is seeded by one of the
 * cascade detections, so the canonical input and this expected answer stay self-consistent.
 */
      ...
      criteria: [
        {
          ...
          text: 'Sets status=open with severity=critical for the cascade discovery: active database-connectivity failures cascading to user-facing connection-refused errors across balancereader, transactionhistory, and ledgerwriter warrant immediate on-call action.',
          score: 3,
        },
        {
