...
import type { E2ELiveConfig, E2EScenario } from '../../src/datasets';
import { createE2ELiveEvaluators } from '../../src/evaluators/e2e_live';
import { buildAvailableSnapshotsBySource } from '../shared';

/**
 * Opt-in gate: the live spec runs the REAL pipeline — LLM onboarding (minutes), a 1x-paced
 * incident-tail stream (10-15 minutes of wall clock) while real alerting rules fire, and the
 * full orchestrator. Expect roughly 30-45 minutes per scenario. Set SIGEVENTS_E2E_LIVE=true to
 * run it; it is skipped otherwise.
 */
const E2E_LIVE_ENABLED = process.env.SIGEVENTS_E2E_LIVE === 'true';
...
interface LiveScenario extends E2EScenario {
  live: E2ELiveConfig;
}

const isLiveScenario = (scenario: E2EScenario): scenario is LiveScenario =>
  scenario.live !== undefined;

evaluate.describe(
  'Significant Events - Live end-to-end pipeline',
  ...
  () => {
    ...
    evaluate.beforeAll(async ({ esClient, kbnClient, log }) => {
      if (!E2E_LIVE_ENABLED) {
        log.info('SIGEVENTS_E2E_LIVE is not set — skipping live end-to-end pipeline eval');
        evaluate.skip();
        return;
      }
      ...
      const snapshots = await buildAvailableSnapshotsBySource(
        activeDatasets,
        (dataset) => dataset.e2e.filter(isLiveScenario),
        esClient,
        log
      );
    ...
    for (const dataset of activeDatasets) {
      const liveScenarios = dataset.e2e.filter(isLiveScenario);
      ...
      evaluate.describe(dataset.id, () => {
        ...
        evaluate.beforeAll(async ({ log }) => {
          ...
          if (collectedExamples.length === 0) {
            log.info(`No live e2e scenarios available for dataset "${dataset.id}" — skipping`);
            evaluate.skip();
          }
        });
              ...
              await executorClient.runExperiment(
                {
                  datasets: [
                    {
                      name: `sigevents: Live end-to-end pipeline (${dataset.id})`,
                  ...
                  task: async ({ input }: { input: E2EScenario['input'] }) => {
                    ...
                    log.info(
                      `Live pipeline finished: ${generatedQueries.length} generated quer(ies), ` +
                        `${tailStats.created} streamed doc(s), ${detections.length} detection(s), ` +
                        `${discoveries.length} discover(ies), events: ${
                          significantEvents
                ...
                // live_* usage evaluators inside createE2ELiveEvaluators cover tokens, LLM
                // calls, tool calls, and duration deterministically instead.
                createE2ELiveEvaluators({
        ...
        evaluate.afterAll(async ({ esClient, kbnClient, apiServices, log }) => {
          log.debug('Cleaning up live e2e pipeline test data');
