    ...
    services: {
      ...
      processing: {
        simulate: async (streamName, { processor, samples }) => {
              ...
              .map(({ sample, error }) => {
                if (variant.type === 'grok' || variant.type === 'dissect') {
                  return {
                    ...
                    source: pick(sample, variant.value.field),
                  };
                }
          ...
          const messageField =
            variant.type === 'grok' || variant.type === 'dissect' ? variant.value.field : undefined;
          ...
          return {
            ...
            result: {
              ...
              [ignoreFailure ? 'ignored_errors' : 'errors']: errors.map(({ error, source }) => {
                ...
                if (variant.type === 'dissect' && error.message.includes(DISSECT_NO_MATCH_ERROR)) {
                  failure = findDissectMatchFailure(variant.value.pattern, message);
                } else if (variant.type === 'grok' && error.message.includes(GROK_NO_MATCH_ERROR)) {
                  failure =
                    variant.value.patterns.flatMap((pattern) => {
                      const failureForCurrentPattern = findGrokMatchFailure(
                        pattern,
                        message,
                        variant.value.pattern_definitions ?? {}
                      );
