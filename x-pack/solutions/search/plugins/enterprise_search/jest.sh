...
then
  ...
  if [[ "$TARGET" == *".ts"* ]]; then
    ...
    TARGET="${TARGET} --collectCoverageFrom='<rootDir>/x-pack/solutions/search/plugins/enterprise_search/${PATH_WITHOUT_EXTENSION}.{ts,tsx}'"
  # If this is a folder
  else
    TARGET=${TARGET%/} # Strip any trailing slash
    TARGET="${TARGET}/ --collectCoverageFrom='<rootDir>/x-pack/solutions/search/plugins/enterprise_search/${TARGET}/**/*.{ts,tsx}'"
  fi
else
  TARGET=''
fi
...
TZ="Etc/UTC" pnpm test:jest $TARGET $ARGS
