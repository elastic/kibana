  ...
  const columns: Array<EuiBasicTableColumn<SchemaField>> = [
    {
      render: ({ type }: SchemaField) => {
        ...
        return <EuiIcon type="error" color="danger" aria-label={i18n.translate('xpack.enterpriseSearch.searchApplications.searchApplication.schema.conflictIndicator', { defaultMessage: 'Field type conflict' })} />;
      },
      width: '24px',
    },
    {
      ...
      render: ({ name, type }: SchemaField) => (
        <EuiFlexGroup gutterSize="s" alignItems="center">
          {name.includes('.') && <EuiIcon aria-hidden={true} type="sortRight" color="subdued" />}
