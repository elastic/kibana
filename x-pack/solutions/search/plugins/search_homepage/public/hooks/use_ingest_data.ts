...
export function useIngestSampleData() {
  ...
  const {
    ...
    isLoading,
    data,
  } = useMutation<InstallingResponse | InstalledResponse, ServerError, void>(
      ...
      onSuccess: () => {
        ...
        toasts.addSuccess(
          i18n.translate('xpack.searchHomepage.sampleData.successNotification', {
            defaultMessage: 'The Sample Data installation started.',
          })
        );
      },
      onError: (error) => {
  ...
  return { ingestSampleData, isLoading: isLoading || data?.status === 'installing' };
}
