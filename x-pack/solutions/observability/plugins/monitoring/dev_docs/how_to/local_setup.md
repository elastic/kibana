...
docker run --name metricbeat \
  ...
  --hostname=metricbeat \
  --publish=5066:5066 \
  --volume="$(pwd)/x-pack/solutions/observability/plugins/monitoring/dev_docs/reference/metricbeat.yarn.yml:/usr/share/metricbeat/metricbeat.yml:ro" \
...
docker run --name filebeat \
  ...
  --publish=5067:5067 \
  --volume="$(pwd)/.es:/es:ro" \
  --volume="$(pwd)/x-pack/solutions/observability/plugins/monitoring/dev_docs/reference/filebeat.yarn.yml:/usr/share/filebeat/filebeat.yml:ro" \
...
docker run --name logstash \
  ...
  --hostname=logstash \
  --publish=9600:9600 \
  --volume="$(pwd)/x-pack/solutions/observability/plugins/monitoring/dev_docs/reference/logstash.yml:/usr/share/logstash/config/logstash.yml:ro" \
  --volume="$(pwd)/x-pack/solutions/observability/plugins/monitoring/dev_docs/reference/pipelines.yml:/usr/share/logstash/config/pipelines.yml:ro" \
