...
import type { InventoryTsvbType, LensConfigWithId, MetricsUIAggregation, SnapshotMetricType, TSVBMetricModelCreator } from '../../types';
export declare enum DataSchemaFormatEnum {
    ECS = "ecs",
    SEMCONV = "semconv"
}
...
/** helper types */
export type SchemaWrappedEntry<T> = T extends AggregationConfig ? SchemaBasedAggregations : T extends LensBaseLayer ? SchemaBasedFormula : never;
export type MetricConfigEntry<T extends RawConfig = RawConfig> = MetricsUIAggregation | LensBaseLayer | SchemaWrappedEntry<T>;
export type UnwrapMetricConfig<T> = T extends SchemaBasedFormula ? LensBaseLayer : T extends SchemaBasedAggregations ? MetricsUIAggregation : T;
export type ResolvedMetricMap<T extends MetricConfigMap> = {
    [K in keyof T]: UnwrapMetricConfig<T[K]>;
};
export type UnwrapRawConfig<T extends MetricConfigMap, K extends keyof T> = UnwrapMetricConfig<T[K]>;
/** catalog types */
export interface BaseMetricsCatalog<TConfigMap extends MetricConfigMap> {
    get schema(): DataSchemaFormat;
    get<K extends keyof ResolvedMetricMap<TConfigMap>>(key: K): UnwrapRawConfig<TConfigMap, keyof TConfigMap>;
    get(key: string): UnwrapRawConfig<TConfigMap, keyof TConfigMap> | undefined;
    getAll(): ResolvedMetricMap<TConfigMap>;
}
export type AggregationsCatalog<TConfigMap extends AggregationConfigMap> = BaseMetricsCatalog<TConfigMap>;
...
export interface InventoryMetricsConfigWithLens<TFormulas extends FormulasConfigMap, TCharts extends ChartsConfigMap> {
    getFormulas: (args?: {
        schema?: DataSchemaFormat;
    }) => Promise<FormulasCatalog<TFormulas>>;
    getCharts: (args?: {
        schema?: DataSchemaFormat;
    }) => Promise<TCharts>;
}
export type InventoryMetricsConfig<TAggregations extends AggregationConfigMap = AggregationConfigMap, TFormulas extends FormulasConfigMap | undefined = undefined, TCharts extends ChartsConfigMap | undefined = undefined> = BaseInventoryMetricsConfig<TAggregations> & (TFormulas extends undefined ? TCharts extends undefined ? InventoryTsvbMetrics : never : TCharts extends undefined ? never : InventoryMetricsConfigWithLens<NonNullable<TFormulas>, NonNullable<TCharts>>);
