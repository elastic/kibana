...
describe('Metric Threshold Alerts Executor', () => {
  ...
  describe('with 10K plus docs', () => {
    beforeAll(() => esArchiver.load(TEN_THOUSAND_PLUS_ARCHIVE));
    afterAll(() => esArchiver.unload(TEN_THOUSAND_PLUS_ARCHIVE));
  ...
  describe('with gauge data', () => {
    beforeAll(() => esArchiver.load(ALERTS_TEST_DATA_ARCHIVE));
    afterAll(() => esArchiver.unload(ALERTS_TEST_DATA_ARCHIVE));
    ...
    describe('querying the entire infrastructure', () => {
      for (const aggType of aggs) {
        it(`should work with the ${aggType} aggregator`, async () => {
          ...
          if (aggType !== 'count') {
            expect(result.aggregations).toBeTruthy();
          }
        });
      }
