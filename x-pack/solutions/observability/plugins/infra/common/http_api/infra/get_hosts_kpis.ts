...
// Memory Usage, Disk Usage) above the table. Previously each tile was a Lens
...
// by the legacy host endpoint. This endpoint collapses those four DSL
...
// - No `BUCKET` / `date_histogram` — there is no trendline. Without time
//   bucketing the ES cell count is `O(hosts × per-state slices)` ≈
//   single-digit MB on realistic fleets (5000 hosts × ~4 states), well
//   under any realistic Serverless circuit-breaker budget.
