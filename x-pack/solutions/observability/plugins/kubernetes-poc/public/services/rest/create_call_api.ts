/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import type { CoreSetup, CoreStart } from '@kbn/core/public';
import type {
  ClientRequestParamsOf,
  ReturnOf,
  RouteRepositoryClient,
} from '@kbn/server-route-repository';
import { formatRequest } from '@kbn/server-route-repository-utils';
import type { APIEndpoint, KubernetesPocServerRouteRepository } from '../../../server/routes';
import type { CallApi } from './call_api';
import { callApi } from './call_api';

export type KubernetesPocClient = RouteRepositoryClient<
  KubernetesPocServerRouteRepository,
  { signal: AbortSignal | null }
>['fetch'];

export type APIReturnType<TEndpoint extends APIEndpoint> = ReturnOf<
  KubernetesPocServerRouteRepository,
  TEndpoint
>;

export type APIClientRequestParamsOf<TEndpoint extends APIEndpoint> = ClientRequestParamsOf<
  KubernetesPocServerRouteRepository,
  TEndpoint
>;

export let callKubernetesPocApi: KubernetesPocClient = () => {
  throw new Error('callKubernetesPocApi must be initialized first');
};

export function createCallApi(core: CoreStart | CoreSetup) {
  callKubernetesPocApi = ((endpoint, options) => {
    const { params } = options as unknown as {
      params?: Partial<Record<string, any>>;
    };

    const { method, pathname } = formatRequest(endpoint, params?.path);

    return callApi(core, {
      ...options,
      method,
      pathname,
      body: params?.body,
      query: params?.query,
    } as unknown as Parameters<CallApi>[1]);
  }) as KubernetesPocClient;
}
