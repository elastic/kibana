...
/**
 * ES|QL query for network inbound/outbound traffic rate over time.
 * Uses TS command with RATE() function to calculate the rate of change for counter metrics.
 */
...
export const NetworkTrafficChart: React.FC<NetworkTrafficChartProps> = ({
  ...
  const esql = useMemo(() => getNetworkTrafficEsql(clusterName), [clusterName]);
  ...
  return (
    <div style={{ paddingTop: '8px', paddingLeft: '8px' }}>
      <EuiText size="s" style={{ fontWeight: 700 }}>
        {i18n.translate('xpack.kubernetesPoc.clusterDetailFlyout.networkTrafficTitle', {
