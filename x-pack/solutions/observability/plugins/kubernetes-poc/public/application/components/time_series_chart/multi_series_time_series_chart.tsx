...
export interface SeriesConfig {
  /** Unique key for the series (must match data field name) */
  key: string;
...
export interface MultiSeriesTimeSeriesChartProps {
  /** Unique identifier for the chart */
  id: string;
  /** Chart title displayed above the visualization */
  title: string;
  /** Time series data points with multiple series */
  data: MultiSeriesDataPoint[];
  /** Configuration for each series */
  series: SeriesConfig[];
  /** Change points to display as annotations (keyed by series key) */
...
/**
 * Multi-series time series chart component with change point annotations
 *
 * This component renders a line or area chart with multiple series using @elastic/charts
 * and overlays change point annotations detected via the CHANGE_POINT ES|QL function.
 */
export function MultiSeriesTimeSeriesChart({
  ...
  const allChangePoints = useMemo(() => {
    return Object.entries(changePoints).flatMap(([seriesKey, points]) =>
      points.map((point, index) => ({
        ...point,
        seriesKey,
        uniqueId: `${seriesKey}-${index}`,
      }))
    );
  }, [changePoints]);
      ...
      <div style={{ paddingTop: '8px', paddingLeft: '8px' }}>
        ...
        >
          <EuiIcon
            ...
            size="xl"
            color="subdued"
            aria-label={i18n.translate('xpack.kubernetesPoc.multiSeriesChart.noDataLabel', {
      ...
      <Chart size={{ width: '100%', height: height - 32 }}>
        ...
        <Settings
          ...
          noResults={
            <EuiIcon
              type="visLine"
              aria-label={i18n.translate('xpack.kubernetesPoc.multiSeriesChart.noResultsLabel', {
        ...
        {series.map((seriesConfig) => (
          <SeriesComponent
            ...
            id={`${id}-${seriesConfig.key}`}
            name={seriesConfig.name}
            xScaleType={ScaleType.Time}
            yScaleType={ScaleType.Linear}
            xAccessor="timestamp"
            yAccessors={[seriesConfig.key]}
            data={data}
            curve={CurveType.CURVE_MONOTONE_X}
            color={seriesConfig.color ? [seriesConfig.color] : undefined}
            yNice
          />
        ))}
