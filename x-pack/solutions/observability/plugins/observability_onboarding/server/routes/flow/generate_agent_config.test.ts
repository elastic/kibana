...
describe('generateAgentConfigTar', () => {
  ...
  it('never includes _write_to_logs_streams in the output config', () => {
    const tarBuffer = generateAgentConfigTar(mockOutput, mockIntegrations);
    const contents = extractTarContents(tarBuffer);
  ...
  it('preserves the output configuration produced by transformOutputToFullPolicyOutput', () => {
    const tarBuffer = generateAgentConfigTar(mockOutput, mockIntegrations);
    const contents = extractTarContents(tarBuffer);
  ...
  it('handles empty integrations list', () => {
    const tarBuffer = generateAgentConfigTar(mockOutput, []);
    const contents = extractTarContents(tarBuffer);
