...
describe('ElasticAgentAppInstrumentationStep', () => {
  it('selects "No, infrastructure only" by default', () => {
    ...
    expect(
      screen.getByText('Add APM agents or the OTel SDK to capture traces and application metrics.')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Skip app instrumentation and collect only logs and infra metrics.')
    ).toBeInTheDocument();
    expect(
  ...
  it('reveals the APM Server URL input when Yes is selected', async () => {
    ...
    expect(apmServerUrlInput).toHaveAttribute('placeholder', 'https://my-deployment.apm.io:8200');
  ...
  it('reveals the language selector when Yes is selected', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'APM endpoint' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Select language agent' })).toBeInTheDocument();
    expect(
      screen.getByTestId(
        'observabilityOnboardingKubernetesV2ElasticAgentAppInstrumentationLanguageSelector'
      )
  ...
  it('updates the documentation link when the selected language changes', async () => {
    ...
    expect(docsLink).toHaveTextContent('Elastic APM nodejs agent docs');
    ...
    expect(docsLink).toHaveTextContent('Elastic APM java agent docs');
    ...
    expect(docsLink).toHaveTextContent('Elastic APM python agent docs');
  });
