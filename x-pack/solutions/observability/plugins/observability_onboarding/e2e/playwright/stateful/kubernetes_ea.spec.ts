...
test('Kubernetes EA', async ({ page }) => {
  assertEnv(process.env.ARTIFACTS_FOLDER, 'ARTIFACTS_FOLDER is not defined.');

  const kubernetesEAFlowPage = new KubernetesEAFlowPage(page);
  const kubernetesOverviewDashboardPage = new KubernetesOverviewDashboardPage(page);
