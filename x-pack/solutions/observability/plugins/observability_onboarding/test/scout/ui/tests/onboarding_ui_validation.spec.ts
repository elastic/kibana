...
test.describe('Onboarding UI Validation', () => {
  ...
  test.describe(
    'Main Tiles Visibility - Feature Flag Dependent',
    { tag: ['@ess', '@svlOblt'] },
    () => {
      ...
      test('should show Application tile only in complete deployment mode', async ({
        pageObjects,
      }) => {
        const isApplicationTileVisible =
  ...
  test.describe('Sub-section Tiles Visibility', { tag: ['@ess', '@svlOblt'] }, () => {
    test('should show Host sub-section tiles when Host tile is selected', async ({
      pageObjects,
    }) => {
    ...
    test('should show Kubernetes sub-section tiles when Kubernetes tile is selected', async ({
      pageObjects,
    }) => {
    ...
    test('should show Cloud sub-section tiles when Cloud tile is selected', async ({
      pageObjects,
    }) => {
    ...
    test('should show Application sub-section tiles when Application tile is selected (complete mode only)', async ({
      pageObjects,
    }) => {
      const isApplicationTileVisible =
        await pageObjects.onboarding.applicationUseCaseTile.isVisible();
  ...
  test.describe('Navigation Validation', { tag: ['@ess', '@svlOblt'] }, () => {
    test('should navigate correctly when Host integration cards are clicked', async ({
      page,
      pageObjects,
    }) => {
      await pageObjects.onboarding.selectHostUseCase();
    ...
    test('should navigate correctly when Kubernetes integration cards are clicked', async ({
      page,
      pageObjects,
    }) => {
      await pageObjects.onboarding.selectKubernetesUseCase();
    ...
    test('should navigate correctly when OTel integration cards are clicked', async ({
      page,
      pageObjects,
    }) => {
      await pageObjects.onboarding.selectHostUseCase();
    ...
    test('should maintain proper URL state when switching between use cases', async ({
      page,
      pageObjects,
    }) => {
      await pageObjects.onboarding.selectHostUseCase();
  ...
  test.describe(
    'UI Stability - External Component Changes Protection',
    { tag: ['@ess', '@svlOblt'] },
    () => {
      ...
      test('should maintain consistent integration card structure after selection', async ({
        pageObjects,
      }) => {
        await pageObjects.onboarding.selectHostUseCase();
        await pageObjects.onboarding.waitForIntegrationCards();
