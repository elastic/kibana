/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import type { SloToolDeps } from './deps';
import { SloToolValidationError } from './errors';

export const assertPlatinumLicenseForTools = async (
  getLicensing: SloToolDeps['getLicensing']
): Promise<void> => {
  const licensing = await getLicensing();
  const hasCorrectLicense = (await licensing.getLicense()).hasAtLeast('platinum');

  if (!hasCorrectLicense) {
    throw new SloToolValidationError(
      'Platinum license or higher is needed to make use of this feature.'
    );
  }
};
