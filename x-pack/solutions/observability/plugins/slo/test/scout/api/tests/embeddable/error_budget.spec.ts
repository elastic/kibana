...
apiTest.describe(
  ...
  () => {
    ...
    apiTest(
      'creates a dashboard with an SLO error budget embeddable panel via REST API',
      async ({ apiClient }) => {
        ...
        expect(createdPanel.config.title).toBe('Error Budget Chart');
      }
    );

    apiTest(
      'rejects invalid error budget panel config when slo_id is missing',
      async ({ apiClient }) => {
