...
export const getPathForFeedback = (path: string) => {
  ...
  // We're falling back to the default value when no path exists in the URL in those edge cases to avoid crashes
  const sanitizedPath = typeof path !== 'string' ? '/' : path;
