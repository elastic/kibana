...
export function useTransactionDetailFlyoutDistributionChartData({
    ...
    if (isErrorMessage(overallLatencyError)) {
      notifications.toasts.addDanger({
        title: i18n.translate(
          'xpack.apm.transactionDetailFlyout.distribution.latencyDistributionErrorTitle',
          {
  ...
  useEffect(() => {
    if (isErrorMessage(errorHistogramError)) {
      notifications.toasts.addDanger({
        title: i18n.translate(
          'xpack.apm.transactionDetailFlyout.distribution.failedTransactionsLatencyDistributionErrorTitle',
          {
            defaultMessage:
  ...
  const chartData = getTransactionDistributionChartData({
    euiTheme,
