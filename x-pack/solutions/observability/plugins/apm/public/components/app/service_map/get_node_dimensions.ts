 ...
 * when the node has not been measured yet (e.g. on first render, before layout settles).
 */
export const getNodeWidth = (node: ServiceMapNode): number =>
  node.measured?.width ?? node.width ?? NODE_WIDTH;

/** Height of a node, with the same measured→default fallback as {@link getNodeWidth}. */
