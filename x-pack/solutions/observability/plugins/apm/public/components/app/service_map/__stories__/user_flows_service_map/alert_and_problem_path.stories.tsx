...
const MOCK_ALERTS = [
  ...
  {
    id: '3',
    title: 'Connection timeouts to order-service',
    severity: 'critical',
...
function createProblemEdgeStyle(failedCount: number, isDependencyEdge: boolean = false) {
  ...
  const label =
    failedCount > 0 ? `${failedCount} failed ${noun}${failedCount === 1 ? '' : 's'}` : '';
  return {
...
export const AlertAndProblemPath: StoryFn = () => {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      >
        <p>
          <strong>payment-service</strong> has a red &quot;5 alerts&quot; badge. Click a red node or
          edge to explore. Toggle &quot;Show only problem path&quot; to hide other connections. Data
          is mocked.
        </p>
      </EuiCallOut>
      <EuiSpacer size="s" />
      ...
      {!selectedNodeId && !selectedEdgeId && (
        <EuiText size="s" color="subdued">
          <p>
            Click a red node or edge to explore; the same popover as the main map is used, with
            alerts and failed requests.
          </p>
        </EuiText>
      )}
      <EuiSpacer size="s" />
