...
export interface CollapsibleServiceMapContextValue {
  ...
  /** Total hidden service neighbors (expand affordance) */
  getHiddenServiceCount: (nodeId: string) => number;
  /** Hidden neighbors with alerts */
  getHiddenAttentionCount: (nodeId: string) => number;
  /** Hidden neighbors matching active filters (full query) */
  getHiddenMatchingServiceCount: (nodeId: string) => number;
}
