...
export const ServiceNode = memo(
  ({ data, selected, sourcePosition, targetPosition }: NodeProps<ServiceNodeType>) => {
    ...
    const borderColor = useMemo(() => {
      if (data.combinedHealthStatus) {
        return getServiceHealthStatusColor(euiTheme, data.combinedHealthStatus);
      }
      if (selected) {
        return euiTheme.colors.primary;
      }
      return euiTheme.colors.mediumShade;
    }, [data.combinedHealthStatus, selected, euiTheme]);

    const borderWidth = useMemo(() => {
      const status = data.combinedHealthStatus;
      if (status === ServiceHealthStatus.critical) return euiTheme.size.xs;
      return euiTheme.size.xxs;
    }, [data.combinedHealthStatus, euiTheme.size.xxs, euiTheme.size.xs]);
    ...
    const ariaLabel = useMemo(() => {
      ...
      if (data.combinedHealthStatus) {
        parts.push(
          i18n.translate('xpack.apm.serviceMap.serviceNode.healthInfo', {
            ...
            values: {
              status: getServiceHealthStatusLabel(data.combinedHealthStatus).toLowerCase(),
            },
          })
        );
      }
      ...
      return parts.join('. ');
    }, [data.label, data.agentName, data.combinedHealthStatus, selected]);
