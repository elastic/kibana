 ...
 * given zoom (used when the graph is too large to fit without clamping to {@link MIN_ZOOM}).
 */
 ...
 * graphs (e.g. many disconnected dependencies spread far apart) the bounding-box center is empty
 * space, so the user lands on nothing. In that case we instead center on the **median** node position
 * (the dense cluster of services) at {@link LARGE_MAP_OVERVIEW_ZOOM}. Normal-sized graphs that fit
 * within {@link MIN_ZOOM} keep using `fitView`.
 */
export function getServiceMapViewTarget({
  ...
  // Approximate the zoom fitView would use (accounting for padding). If the graph fits at or above
  // MIN_ZOOM, fitView shows it nicely; otherwise it would clamp and center on empty space.
  const fitZoom =
