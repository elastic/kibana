...
export const ServiceNodeWithCollapseAffordance = memo((props: NodeProps<ServiceNodeType>) => {
  ...
  const hiddenServiceCount = ctx.getHiddenServiceCount(nodeId);
  const hiddenMatchingCount = ctx.getHiddenMatchingServiceCount(nodeId);
  const hiddenAttentionCount = ctx.getHiddenAttentionCount(nodeId);
  const badgeCount = ctx.hasActiveFilters ? hiddenMatchingCount : hiddenServiceCount;
  const isExpanded = ctx.expandedNodeIds.has(nodeId);
  const canExpand = hiddenServiceCount > 0;
        ...
        <div css={affordanceColumnStyles}>
          <EuiFlexGroup gutterSize="xs" alignItems="center" responsive={false}>
                ...
                <EuiToolTip
                  content={
                    ctx.hasActiveFilters
                      ? i18n.translate(
                          'xpack.apm.serviceMap.exploration.expandHiddenMatchingFilter',
                          {
                            defaultMessage:
                              'Show {total, plural, one {# hidden dependency} other {# hidden dependencies}} ({matching} match current filters)',
                            values: {
                              total: hiddenServiceCount,
                              matching: hiddenMatchingCount,
                            },
                          }
                        )
                      : i18n.translate('xpack.apm.serviceMap.exploration.expandHidden', {
                          defaultMessage:
                          ...
                          values: {
                            ...
                            attention:
                              hiddenAttentionCount > 0
                                ? ` (${hiddenAttentionCount} need attention)`
                                : '',
                          },
                        })
                  }
                >
                  <EuiButtonIcon
                    ...
                    aria-label={i18n.translate(
                      'xpack.apm.serviceMap.exploration.expandHiddenAria',
                      {
            ...
            {isExpanded && (
              <EuiFlexItem grow={false}>
                <EuiToolTip
                  content={i18n.translate('xpack.apm.serviceMap.exploration.collapseExpanded', {
                ...
                >
                  <EuiButtonIcon
                    ...
                    aria-label={i18n.translate(
                      'xpack.apm.serviceMap.exploration.collapseExpandedAria',
                      {
