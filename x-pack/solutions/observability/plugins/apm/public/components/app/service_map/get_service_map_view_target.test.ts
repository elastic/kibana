...
describe('getServiceMapViewTarget', () => {
  ...
  it('centers on the median node for a large sprawling graph', () => {
    // A dense cluster near the origin plus a far-away outlier that blows up the bounding box so the
    // graph can no longer fit at MIN_ZOOM.
    const nodes = [
      makeNode(0, 0),
      makeNode(100, 0),
      makeNode(200, 0),
    ...
    if (target.kind === 'center') {
      ...
      // Median lands on the dense cluster (node center = position + size/2), not the empty middle.
