...
describe('getServiceMapViewTarget', () => {
  it('fits when there are no visible nodes', () => {
    expect(
      getServiceMapViewTarget({ ...VIEWPORT, visibleNodes: [], bounds: EMPTY_BOUNDS })
    ).toEqual({ kind: 'fit' });
  });
  ...
  it('fits a small graph that comfortably fits the viewport', () => {
    const visibleNodes = [makeNode(0, 0), makeNode(300, 0), makeNode(0, 200), makeNode(300, 200)];
    const bounds: Rect = { x: 0, y: 0, width: 500, height: 280 };
    expect(getServiceMapViewTarget({ ...VIEWPORT, visibleNodes, bounds })).toEqual({ kind: 'fit' });
  });
  ...
  it('centers on the median node for a large sprawling graph', () => {
    ...
    const target = getServiceMapViewTarget({ ...VIEWPORT, visibleNodes, bounds });
