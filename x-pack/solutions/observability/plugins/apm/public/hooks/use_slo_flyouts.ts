...
import { getService } from '@kbn/core-di';
...
/**
 * Resolves SLO flyout components from the DI container via service tokens.
 *
...
export const useSloFlyouts = (): SloFlyouts => {
  ...
  return useMemo(() => {
    ...
    return {
      CreateSLOFormFlyout: container
        ? getService(container, SloCreateFlyoutToken, { optional: true })
        : undefined,
      SLODetailsFlyout: container
        ? getService(container, SloDetailsFlyoutToken, { optional: true })
        : undefined,
    };
  }, [injection]);
};
