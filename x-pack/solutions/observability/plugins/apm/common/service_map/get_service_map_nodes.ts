...
export function getAllNodes(services: ServicesResponse[], connections: Connection[]) {
  ...
  // from the services data query. Merge with existing nodes to preserve enriched data.
  services.forEach((service) => {
    const id = service[SERVICE_NAME];
    if (!FORBIDDEN_SERVICE_NAMES.includes(service[SERVICE_NAME])) {
      const existingNode = allNodesMap.get(id);
      // Merge service data with existing node, prioritizing service data
      allNodesMap.set(id, { ...existingNode, ...service, id });
    }
  });
