 ...
 * with an overlap period where both instrumentations send data simultaneously.
 *
 * Timeline (given a --from/--to range):
 *
 *   |--- classic APM only ---|--- overlap ---|--- OTel only ---|
 *   from                    25%             75%                to
 *
 * Classic APM runs from `from` to 75% of the range.
 * OTel runs from 25% of the range to `to`.
 * The middle 50% has both.
 *
 * Run:
 *   node scripts/synthtrace apm_service_overlapping_otel_metrics --from now-2d --to now --clean
 *
 * Scenario options:
 * - serviceName (string, default: "overlap-svc"): the shared service name
...
const OTEL_INDEX = 'metrics-otel_overlap.otel-default';
const OTEL_TEMPLATE_NAME = 'metrics-otel_overlap.otel';
...
async function ensureOtelTemplate(esClient: Client) {
  ...
  await esClient.indices.putIndexTemplate({
    ...
    index_patterns: ['metrics-otel_overlap.otel-*'],
    data_stream: {},
    priority: 500,
    template: {
...
const scenario: Scenario<ApmOtelFields> = async ({ from, to, logger, ...runOptions }) => {
  ...
  const classicEnd = from + totalRange * 0.75;
  const otelStart = from + totalRange * 0.25;

  return {
    bootstrap: async (_synthtraceClients, _kibanaClient, esClient) => {
      logger.info(`Indexing classic APM data for "${serviceName}" (from start to 75%)...`);
      const classicDocs = generateClassicApmDocs(serviceName, from, classicEnd, rpm);
      ...
      logger.info(`Indexing OTel JVM metrics for "${serviceName}" (from 25% to end)...`);
      const otelDocs = generateOtelJvmDocs(serviceName, otelStart, to);
    ...
    generate: ({ range, clients: { apmEsClient } }) => {
      ...
      const otelTraceStart = new Date(Math.max(rangeFromMs, otelStart));

      if (otelTraceStart >= range.to) {
        return [];
      }

      const otelRange = timerange(otelTraceStart, range.to);
