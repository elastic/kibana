...
function createAtLeastOneOfKeysValidation<K extends string>(
  requiredKeys: readonly K[],
  codecName = 'AtLeastOneOfKeys'
) {
  return new t.Type<Record<string, unknown>, Record<string, unknown>, unknown>(
    codecName,
    (input): input is Record<string, unknown> => {
      if (typeof input !== 'object' || input === null) return false;
      const obj = input as Record<string, unknown>;
      return requiredKeys.some((key) => typeof obj[key] === 'string' && obj[key] !== '');
    },
    (input, context) => {
      if (typeof input !== 'object' || input === null) {
        return t.failure(input, context, 'Expected an object');
      }

      const obj = input as Record<string, unknown>;
      const hasAtLeastOne = requiredKeys.some(
      ...
      if (!hasAtLeastOne) {
        return t.failure(
          input,
          context,
          `At least one of the following parameters must be provided: ${requiredKeys.join(', ')}`
        );
      }
...
export const serviceNameOrTraceIdRt = createAtLeastOneOfKeysValidation([
  'serviceName',
  'traceId',
] as const);
