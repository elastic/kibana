...
describe('getNCT', () => {
  ...
  const buildClientSpy = () =>
    jest.fn().mockResolvedValueOnce({
      aggregations: {
        netConnectionTypes: { buckets: [] },
      },
    });

  it('queries span documents when no errorType is provided', async () => {
    const clientSpy = buildClientSpy();
    await getNCT({ ...commonParams, apmEventClient: { search: clientSpy } as any });

    const query = clientSpy.mock.calls[0][1];
    expect(query.apm.sources[0].documentType).toEqual(ApmDocumentType.SpanEvent);
