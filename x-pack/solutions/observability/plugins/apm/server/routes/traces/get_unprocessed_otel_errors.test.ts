...
describe('getUnprocessedOtelErrors', () => {
  ...
  it('skips a document without fields instead of throwing', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining('[get_unprocessed_otel_errors] Skipping document with id [error-1]')
    );
  });

  it.each([SPAN_ID, ID, SERVICE_NAME, AT_TIMESTAMP])(
    ...
    async (missingField) => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining(missingField));
    }
  );
    ...
    const errors = await callGetUnprocessedOtelErrors([
      {
        _id: 'error-1',
        _index: 'logs-generic.otel-default',
        fields: omit(validFields, SPAN_ID),
      },
      {
        _id: 'error-2',
        _index: 'logs-generic.otel-default',
