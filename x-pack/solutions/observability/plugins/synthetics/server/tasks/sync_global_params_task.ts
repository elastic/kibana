...
export class SyncGlobalParamsPrivateLocationsTask {
  ...
  public async runTask({
    ...
    const paramsSpacesToSync = taskInstance.state.paramsSpacesToSync || [];
    ...
    try {
      ...
      if (paramsSpacesToSync.length > 0 && allPrivateLocations.length > 0) {
        await this.deployPackagePolicies.syncPackagePolicies({
          allPrivateLocations,
          soClient,
          encryptedSavedObjects,
          spaceIds: paramsSpacesToSync,
        });

        this.debugLog(`Sync of global params succeeded`);
      }
    } catch (error) {
