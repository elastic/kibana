...
describe('tlsRuleExecutor', () => {
  ...
  const commonFilter =
    'synthetics-monitor-multiple.attributes.alert.tls.enabled: true and (synthetics-monitor-multiple.attributes.type: http or synthetics-monitor-multiple.attributes.type: tcp)';
  ...
  describe('getMonitors', () => {
    it('should filter monitors based on monitor ids', async () => {
      ...
      expect(getAllMock).toHaveBeenCalledWith({
        filter: `${commonFilter} AND synthetics-monitor-multiple.attributes.id:(\"${monitorId}\")`,
      });
    });
    ...
    it('should filter monitors based on tags', async () => {
      ...
      expect(getAllMock).toHaveBeenCalledWith({
        filter: `${commonFilter} AND synthetics-monitor-multiple.attributes.tags:(\"${tag}\")`,
      });
    });
    ...
    it('should filter monitors based on monitor types', async () => {
      ...
      expect(getAllMock).toHaveBeenCalledWith({
        filter: `${commonFilter} AND synthetics-monitor-multiple.attributes.type:(\"${monitorType}\")`,
      });
    });
