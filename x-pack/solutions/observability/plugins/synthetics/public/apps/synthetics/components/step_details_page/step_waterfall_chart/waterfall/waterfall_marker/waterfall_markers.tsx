...
export function WaterfallChartMarkers() {
  ...
  const annotations = useMemo(() => {
    ...
    const recognizedMarkerItemsByOffset = Array.from(markerItemsByOffset.entries()).reduce(
      (acc, [offset, items]) => {
        ...
        const vitalMarkers = showCustomMarks
          ? items
          : items.filter(
              ({ id }) => getMarkersInfo(id, theme.euiTheme.colors.mediumShade) !== undefined
            );
    ...
    return Array.from(recognizedMarkerItemsByOffset.entries()).map(([offset, items]) => {
      ...
      const label = uniqueIds
        .map((id) => getMarkersInfo(id, theme.euiTheme.colors.mediumShade)?.label ?? id)
        .join(' / ');
      const id = uniqueIds[0];
      const markersInfo = getMarkersInfo(id, theme.euiTheme.colors.mediumShade);
      ...
      return {
        ...
        color: markersInfo?.color ?? theme.euiTheme.colors.mediumShade,
      ...
      {annotations.map(({ id, offset, label, field, color, strokeWidth, dash }) => {
        ...
        return (
          <LineAnnotation
            ...
            zIndex={Number(theme.euiTheme.levels.content)}
          />
        );
      })}
    </Wrapper>
...
function getMarkersInfo(id: string, euiColorMediumShade: string) {
  switch (id) {
