...
describe('EUI & React Router Component Helpers', () => {
  ...
  it('renders', () => {
    ...
    expect(link.getAttribute('data-test-subj')).toEqual('syntheticsReactRouterEuiLink');
  });
  ...
  it('passes down all ...rest props', () => {
    ...
    expect(link.getAttribute('data-test-subj')).toEqual('foo');
    expect(link.getAttribute('href')).toEqual('/enterprise_search/');
  ...
  it('renders with the correct href', () => {
    ...
    const link = getByRole('link');
    expect(link).toBeInTheDocument();
    expect(link.getAttribute('href')).toEqual('/enterprise_search/foo/bar');
