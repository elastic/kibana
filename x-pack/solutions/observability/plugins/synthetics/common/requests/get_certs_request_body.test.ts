...
describe('getCertsRequestBody - cross-cluster search (CCS) scoping', () => {
  const ruleParams: GetCertsParams = {
    pageIndex: 0,
    size: 1000,
    sortBy: 'common_name',
  ...
  it('uses match_none for the local branch when no SO ids are passed', () => {
    // When CCS is on but there are no local enabled monitors, the local branch
    // must match nothing rather than all local pings, so disabled/deleted monitors
    // do not surface while the remote branch admits CCS pings.
    const body = getCertsRequestBody(
    ...
    expect(local).toEqual([{ match_none: {} }]);
  });
});
