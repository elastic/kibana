...
describe('getCertsRequestBody', () => {
  ...
  describe('cross-cluster search (CCS) scoping', () => {
    ...
    it('omits the local branch entirely when no SO ids are passed', () => {
      // When CCS is enabled but the enabled SO list is empty there are no local
      // monitors to gate against, so the local branch is dropped entirely.
      // Keeping an unfiltered local branch would surface stale data from
      // previously-disabled/deleted monitors — exactly the problem this change
      // avoids.
      const body = getCertsRequestBody(
      ...
      const scopingClause = findScopingClause(body);
      expect(scopingClause).toBeDefined();
      const should = scopingClause?.bool?.should as estypes.QueryDslQueryContainer[];
      // Only the remote branch — no local branch when there are no enabled monitors.
      expect(should).toHaveLength(1);
      const remoteBranch = should[0];
