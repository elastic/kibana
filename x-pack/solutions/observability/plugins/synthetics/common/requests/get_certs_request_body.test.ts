...
describe('getCertsRequestBody', () => {
  ...
  describe('cross-cluster search (CCS) scoping', () => {
    ...
    // the only filter clause whose `should` contains only `bool.filter` branches.
    ...
    it('produces a remote-only scoping clause when CCS is enabled and no local monitor ids are passed', () => {
      // When CCS is on but there are no local enabled monitors, the local branch
      // is omitted entirely so CCS-enabled + empty SO list does not surface stale
      // local certs from previously-disabled/deleted monitors.
      const body = getCertsRequestBody(
      ...
      const branches = scoping?.bool?.should as estypes.QueryDslQueryContainer[];
      // Only one branch — the remote branch — when there are no local monitor ids.
      expect(branches).toHaveLength(1);
      const [remoteBranch] = branches;
      expect(remoteBranch?.bool?.filter).toEqual(
        expect.arrayContaining([{ wildcard: { _index: '*:*' } }])
      );
      // No local-ping must_not clause on the sole branch.
      const remoteFilters = remoteBranch?.bool?.filter as estypes.QueryDslQueryContainer[];
      expect(remoteFilters.find((f) => f?.bool?.must_not !== undefined)).toBeUndefined();
    });
  });
});
