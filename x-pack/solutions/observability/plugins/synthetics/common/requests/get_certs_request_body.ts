...
export const buildMonitorScopingFilter = ({
  ...
  // When there are no local monitor IDs (CCS enabled but empty SO list), use
  // match_none for the local branch so disabled/deleted local monitors do not
  // surface unexpectedly while the remote branch handles CCS pings.
  const localBranchFilters: estypes.QueryDslQueryContainer[] = hasMonitorIds
    ? [
