 ...
 *
 * Both monitors use the Elastic-managed `dev` public location from the fixture,
 * so no private location is required.
 */
apiTest.describe(
  'RunTestManually',
  {
  ...
  () => {
    ...
    apiTest('runs test manually', async ({ apiClient }) => {
      const createRes = await addMonitor(apiClient, editorHeaders, httpMonitorFixture);
    ...
    apiTest('works in non default space', async ({ apiClient, kbnClient }) => {
      ...
      const createRes = await apiClient.post(`s/${SPACE_ID}/api/synthetics/monitors`, {
        ...
        body: { ...httpMonitorFixture, spaces: [] },
        responseType: 'json',
      });
