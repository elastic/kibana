    ...
    >
{
  ...
  public setup(
    ...
    if (plugins.searchInferenceEndpoints) {
      ...
      if (failures.length) {
        this.logger.warn(
          `Failed to register inference features for Observability AI Insights: ${failures.join(
            '; '
          )}`
        );
      } else {
        this.logger.debug('Registered Observability AI Insights inference features');
      }
    }
