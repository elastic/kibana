...
type TraceType = 'min' | 'max' | 'sum' | 'count' | 'avg' | 'p95' | 'p99';
...
function getTraceAggregation(aggregation: AggregationConfig | undefined): {
  ...
  if (['min', 'max', 'sum', 'avg'].includes(aggregation.type)) {
    return {
      ...
      } as Record<Exclude<TraceType, 'count' | 'p95' | 'p99'>, { field: string }>,
      buckets_path: 'metric',
    };
  }
  ...
  return {
    ...
    buckets_path: `metric[${percentile}]`,
  };
}
...
async function getTraceChangePoints({
  index,
  start,
  end,
  kqlFilter: kuery,
  ...
  const { agg: traceAgg, buckets_path: bucketsPathTrace } = getTraceAggregation(aggregation);
...
export async function getToolHandler({
  ...
  const traceChangePoints = await getTraceChangePoints({
    esClient,
    index,
    start,
    end,
  ...
  return orderBy(traceChangePoints.flat(), [(item) => item.changes.p_value]).slice(0, 25);
}
