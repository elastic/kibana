...
const METRICS_ESQL_RECOMMENDED_QUERIES = [
  {
    ...
    query: `TS ${METRICS_INDEX_PATTERN} | STATS SUM(RATE(system.network.out.bytes)) BY BUCKET(@timestamp, 100, ?_tstart, ?_tend)`,
    description: i18n.translate(
  ...
  },
  {
    ...
    query: `TS ${METRICS_INDEX_PATTERN} | STATS SUM(RATE(system.network.out.bytes)) BY BUCKET(@timestamp, 100, ?_tstart, ?_tend), host.name`,
    description: i18n.translate(
  ...
  },
  {
    ...
    query: `TS ${METRICS_INDEX_PATTERN} | STATS AVG(system.cpu.total.norm.pct) BY BUCKET(@timestamp, 100, ?_tstart, ?_tend)`,
  ...
  },
  {
    ...
    query: `TS ${METRICS_INDEX_PATTERN} | STATS AVG(system.cpu.total.norm.pct) BY BUCKET(@timestamp, 100, ?_tstart, ?_tend), host.name`,
    description: i18n.translate(
