...
import { ObservabilityRouteHandlerResources } from '../routes/types';

export interface DashboardsClient {
  getAllDashboards: () => Promise<Array<SavedObjectsFindResult<DashboardSavedObjectAttributes>>>;
  getDashboardsById: (ids: string[]) => Promise<Array<SavedObject<DashboardSavedObjectAttributes>>>;
}

export async function createDashboardsClient(
  resources: Pick<ObservabilityRouteHandlerResources, 'request' | 'context'>
): Promise<DashboardsClient> {
  const soClient = (await resources.context.core).savedObjects.client;
  async function getDashboards(
    page: number
  ): Promise<Array<SavedObjectsFindResult<DashboardSavedObjectAttributes>>> {
    const perPage = 1000;

    const response = await soClient.find<DashboardSavedObjectAttributes>({
      type: 'dashboard',
      perPage,
      page,
    });
  ...
  return {
    getDashboardsById: async (ids: string[]) => {
      ...
      return response.saved_objects as Array<SavedObject<DashboardSavedObjectAttributes>>;
    },
