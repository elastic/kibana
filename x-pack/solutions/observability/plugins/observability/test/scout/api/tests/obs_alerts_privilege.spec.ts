...
apiTest.describe(
  ...
  () => {
    ...
    let withPrivilegeCreds: RoleApiCredentials;
    ...
    apiTest.beforeAll(async ({ apiClient, requestAuth, samlAuth }) => {
      ...
      withPrivilegeCreds = await requestAuth.getApiKeyForCustomRole({
    ...
    for (const spec of RULE_SPECS) {
      apiTest(`with privilege: cannot create a ${spec.ruleTypeId} rule`, async ({ apiClient }) => {
        const response = await apiClient.post('api/alerting/rule', {
    ...
    apiTest('with privilege: cannot update a rule', async ({ apiClient }) => {
    ...
    apiTest('with privilege: cannot delete a rule', async ({ apiClient }) => {
      const rule = createdRules[0];
      if (!rule) return;
    ...
    apiTest('with privilege: cannot mute all alerts on a rule', async ({ apiClient }) => {
      const rule = createdRules[0];
      if (!rule) return;
    ...
    apiTest('with privilege: cannot snooze a rule', async ({ apiClient }) => {
      const rule = createdRules[0];
      if (!rule) return;
    ...
    for (const spec of RULE_SPECS) {
      apiTest(
        `with privilege: can mute an alert instance for ${spec.ruleTypeId}`,
        async ({ apiClient }) => {
      ...
      apiTest(
        `with privilege: can unmute an alert instance for ${spec.ruleTypeId}`,
        async ({ apiClient }) => {
          const rule = createdRules.find((r) => r.ruleTypeId === spec.ruleTypeId);
          if (!rule) return;
          const response = await apiClient.post(
    ...
    apiTest('with privilege: can find alerts via RAC', async ({ apiClient }) => {
    ...
    apiTest(
      'with privilege: can acknowledge a real alert via bulk update',
      async ({ apiClient }) => {
        if (!realAlertId) {
          return;
        }
    ...
    for (const spec of RULE_SPECS) {
      apiTest(
        `without privilege: cannot mute an alert instance for ${spec.ruleTypeId}`,
        async ({ apiClient }) => {
          const rule = createdRules.find((r) => r.ruleTypeId === spec.ruleTypeId);
          if (!rule) return;
          const response = await apiClient.post(
      ...
      apiTest(
        `without privilege: cannot unmute an alert instance for ${spec.ruleTypeId}`,
        async ({ apiClient }) => {
          const rule = createdRules.find((r) => r.ruleTypeId === spec.ruleTypeId);
          if (!rule) return;
          const response = await apiClient.post(
    ...
    apiTest('without privilege: cannot find alerts via RAC', async ({ apiClient }) => {
      const response = await apiClient.post('internal/rac/alerts/find', {
        headers: { ...KIBANA_HEADERS, ...withoutPrivilegeCreds.apiKeyHeader },
        body: {
          rule_type_ids: ['.es-query'],
    ...
    apiTest(
      'without privilege: cannot acknowledge an alert via bulk update',
      async ({ apiClient }) => {
        const response = await apiClient.post('internal/rac/alerts/bulk_update', {
          headers: { ...KIBANA_HEADERS, ...withoutPrivilegeCreds.apiKeyHeader },
          body: {
