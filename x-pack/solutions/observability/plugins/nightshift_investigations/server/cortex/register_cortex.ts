...
export const runCortexOptimize = async ({
  ...
  // Only the Nightshift investigation agent writes to Cortex: it is the one agent whose
  // post-execution hook runs this workflow, and other agents' rounds must not edit the wiki. An
  // unidentified caller is refused rather than trusted — the optimize workflow has a manual
  // trigger, so it can be run without an agent id.
  if (agentId !== NIGHTSHIFT_INVESTIGATION_AGENT_ID) {
    logger.debug(
      'Cortex optimizer skipped — round was not produced by the Nightshift investigation agent'
    );
    return;
  }
