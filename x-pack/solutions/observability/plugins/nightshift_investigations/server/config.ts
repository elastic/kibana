...
const cortexConfigSchema = schema.object({
  // Governs Cortex end to end: the hydrate/optimize hooks on the Nightshift investigation agent, the
...
const decisionTreesConfigSchema = schema.object({
  ...
  // Nightshift investigation agent, the hydrate step that materializes trees into the sandbox, the
  // agent's own tools, the decision-tree AI index, and the Decision Trees tab in the significant
  // events app.
