...
{"create":{}}
{"@timestamp":"${HOUR_AGO}","kind":"discovery","event_id":"evt-008","processed":true,"severity":"60-high","confidence":0.84,"stream_names":["logs.web-frontend"],"title":"Search API — elevated empty-result rate","summary":"Empty-result share on search-api rose after a catalog-service deploy while browse traffic on web-frontend remained high. Product search and recommendations look stale to shoppers.","symptom_hypothesis":"Catalog index lag after deploy causes search-api to return empty facets for newly published SKUs.","causal_features":[{"feature_id":"web-frontend","type":"entity","subtype":"service","name":"web-frontend","stream_name":"logs.web-frontend"}],"signals":[{"type":"detection","stream_name":"logs.web-frontend","confirmed":true,"collected_at":"${HOUR_AGO}","description":"Testing: whether search-api empty-result rate spiked after the catalog deploy. Expected if true: sustained empty-result share above baseline. Found: empty results climbed on browse paths tied to search-api. Why: timing aligns with catalog-service rollout. Verdict: confirms — search quality degraded for web users.","evidence":{"esql_query":"FROM logs.web-frontend\n| WHERE service.name == \"web-frontend\"\n  AND message LIKE \"*empty search*\"\n| STATS empty_results = COUNT(*)\n  BY minute = BUCKET(@timestamp, 1 minute)\n| SORT minute DESC","result":"found"},"metadata":{"detection_id":"det-012","rule_uuid":"rule-uuid-012","rule_name":"search-empty-rate","change_point_type":"step_change","p_value":0.04}}],"kibana.space_ids":["default"]}
{"create":{}}
