...
describe('url_params', () => {
  it('builds a share URL with the eventId query param', () => {
    ...
    expect(buildNightshiftEventFlyoutShareUrl('evt-001')).toBe(
      `${window.location.origin}/app/observability/nightshift?${BLAST_RADIUS_QUERY_PARAM}=logs.web&${NIGHTSHIFT_EVENT_ID_QUERY_PARAM}=evt-001`
    );
  });
  ...
  it('replaces an existing eventId when building a share URL', () => {
    window.history.pushState(
      {},
      '',
      `/app/observability/nightshift?${NIGHTSHIFT_EVENT_ID_QUERY_PARAM}=old-id&${BLAST_RADIUS_QUERY_PARAM}=logs.web`
    );

    expect(buildNightshiftEventFlyoutShareUrl('evt-001')).toBe(
      `${window.location.origin}/app/observability/nightshift?${NIGHTSHIFT_EVENT_ID_QUERY_PARAM}=evt-001&${BLAST_RADIUS_QUERY_PARAM}=logs.web`
    );
  });
