...
import { EuiIcon, EuiText, EuiFlexGroup, EuiFlexItem, useEuiTheme } from '@elastic/eui';
import { css } from '@emotion/react';
import type { SourceNodeData, NodeHealth } from '../../mock_data';
...
const SHIPPER_ICONS: Record<string, string> = {
  'Elastic Agent': 'agentApp',
  Filebeat: 'filebeatApp',
  'Fluent Bit': 'logstashApp',
  Kubernetes: 'kubernetesPodApp',
  OpenTelemetry: 'opentelemetryApp',
  AWS: 'logoAWS',
};

type SourceNodeType = Node<SourceNodeData, 'source'>;

export const SourceNode = ({ data, selected }: NodeProps<SourceNodeType>) => {
  const { euiTheme } = useEuiTheme();
  const health = data.health as NodeHealth;
  const healthColor = HEALTH_COLORS[health];
  const iconType = SHIPPER_ICONS[data.shipperType] ?? 'dot';
