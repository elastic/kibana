...
export default function ApiTest({ getService, getPageObjects }: FtrProviderContext) {
  ...
  describe('Conversations', () => {
    ...
    describe('without a connector', () => {
      ...
      describe('after clicking on the setup models button', () => {
        before(async () => {
          await testSubjects.click(ui.pages.conversations.setupGenAiConnectorsButtonSelector);
        });

        it('opens the inference flyout', async () => {
          await testSubjects.existOrFail(ui.pages.inferenceFlyout.flyout);
        });
      ...
      describe('after creating a model', () => {
        before(async () => {
        ...
        describe('and sending over some text', () => {
          ...
          describe('and adding another prompt', () => {
            ...
            it('appends to the existing one', async () => {
              ...
              const [userReply, contextRequest, contextResponse, assistantResponse] = messages.map(
                (msg) => msg.message
              );
          ...
          describe('and choosing to send feedback', () => {
            ...
            it('emits a telemetry event that captures the conversation', async () => {
              ...
              const { feedback, conversation } = events[0].properties as unknown as ChatFeedback;
        ...
        describe('and opening an old conversation', () => {
          ...
          describe('and sending another prompt', () => {
            before(async () => {
              ...
              await testSubjects.setValue(ui.pages.conversations.chatInput, 'And what are SLIs?');
              await testSubjects.pressEnter(ui.pages.conversations.chatInput);
            ...
            describe('and choosing to send feedback', () => {
              ...
              it('emits a telemetry event that captures the conversation', async () => {
                ...
                const { feedback, conversation } = events[1].properties as unknown as ChatFeedback;
