 ...
 * The implementation must rely on resolved target['service.name'] from linked traces.
 */
...
const REDIS_DEPENDENCY = {
  resource: 'redis',
  spanType: 'db',
  spanSubtype: 'redis',
};

const KAFKA_DEPENDENCY = {
  resource: 'kafka',
  spanType: 'messaging',
  spanSubtype: 'kafka',
};
  ...
  describe(`tool: ${OBSERVABILITY_GET_SERVICE_TOPOLOGY_TOOL_ID}`, function () {
    ...
    describe('when fetching service topology', () => {
      ...
      it('returns connections to multiple downstream services', () => {
        ...
        // Note: checkout-service is linked (resolves to service.name), recommendation-service is separate (external)
        const connectionToCheckout = resultData.connections.find(
          (c) =>
            c.source['service.name'] === FRONTEND_SERVICE.name &&
            (('service.name' in c.target && c.target['service.name'] === CHECKOUT_SERVICE.name) ||
              ('span.destination.service.resource' in c.target &&
                c.target['span.destination.service.resource'] ===
                  CHECKOUT_SERVICE_DEPENDENCY.resource))
        );
        ...
        const connectionToRecommendation = resultData.connections.find(
          (c) =>
            ...
            (('service.name' in c.target &&
              c.target['service.name'] === RECOMMENDATION_SERVICE.name) ||
              ('span.destination.service.resource' in c.target &&
                c.target['span.destination.service.resource'] ===
      ...
      it('returns the correct source and target details', () => {
        ...
        // Verify target is either a resolved service or external dependency
        if ('service.name' in connectionToCheckout!.target) {
          expect(connectionToCheckout?.target['service.name']).to.be(CHECKOUT_SERVICE.name);
        } else {
    ...
    describe('when fetching upstream topology (direction: upstream)', () => {
      ...
      it('shows the queried service as the target', () => {
        ...
        // Target may be resolved as service.name or span.destination.service.resource
        if ('service.name' in upstreamFromFrontend!.target) {
          expect(upstreamFromFrontend?.target['service.name']).to.be(CHECKOUT_SERVICE.name);
        } else {
    ...
    describe('downstream queries exclude sibling dependencies', () => {
      ...
      it('includes direct dependencies of the queried service', () => {
        ...
        const toRedis = resultData.connections.find(
          (c) =>
            c.source['service.name'] === CHECKOUT_SERVICE.name &&
            'span.destination.service.resource' in c.target &&
            c.target['span.destination.service.resource'] === REDIS_DEPENDENCY.resource
        );
        const toKafka = resultData.connections.find(
          (c) =>
            c.source['service.name'] === CHECKOUT_SERVICE.name &&
            'span.destination.service.resource' in c.target &&
            c.target['span.destination.service.resource'] === KAFKA_DEPENDENCY.resource
        );
      ...
      it('excludes parent connections (frontend calls)', () => {
        ...
        const fromFrontend = resultData.connections.find(
          (c) => c.source['service.name'] === FRONTEND_SERVICE.name
        );
     ...
     *
     * The synthtrace scenario generates linked traces:
     *   frontend → checkout-service → postgres (linked via trace.id)
     *   recommendation-service → postgres (separate trace)
     */
    describe('upstream queries include full upstream chain (multi-hop)', () => {
      ...
      before(async () => {
        ...
        // frontend → checkout-service (indirect, via linked trace)
      ...
      it('includes direct callers of the queried service', () => {
        ...
        const checkoutToPostgres = resultData.connections.find(
          (c) =>
            c.source['service.name'] === CHECKOUT_SERVICE.name &&
            'span.destination.service.resource' in c.target &&
            c.target['span.destination.service.resource'] === POSTGRES_DEPENDENCY.resource
      ...
      it('includes indirect upstream callers via linked traces (multi-hop)', () => {
        ...
        const frontendToCheckout = resultData.connections.find(
          (c) =>
            c.source['service.name'] === FRONTEND_SERVICE.name &&
            'service.name' in c.target &&
            c.target['service.name'] === CHECKOUT_SERVICE.name
        );
      ...
      it('includes callers from separate traces', () => {
        ...
        const recommendationToPostgres = resultData.connections.find(
          (c) =>
            c.source['service.name'] === RECOMMENDATION_SERVICE.name &&
            'span.destination.service.resource' in c.target &&
            c.target['span.destination.service.resource'] === POSTGRES_DEPENDENCY.resource
        );
      ...
      it('traverses the entire upstream path', () => {
        ...
        // - frontend (indirect caller via checkout-service, linked trace)
        // - recommendation-service (direct caller of postgres, separate trace)
        expect(sourceServices.has(CHECKOUT_SERVICE.name)).to.be(true);
        expect(sourceServices.has(FRONTEND_SERVICE.name)).to.be(true);
        expect(sourceServices.has(RECOMMENDATION_SERVICE.name)).to.be(true);
      });
