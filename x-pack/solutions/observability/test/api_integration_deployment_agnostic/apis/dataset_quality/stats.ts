...
export default function ({ getService }: DeploymentAgnosticFtrProviderContext) {
  ...
  describe('Stats', function () {
    ...
    describe('creation date with many data streams', () => {
      const dataStreamCount = 20;
      let supertestDatasetQualityMonitorWithCookieCredentials: SupertestWithRoleScopeType;
      let roleAuthc: RoleCredentials;
      ...
      before(async () => {
        ...
        await synthtraceLogsEsClient.index(
          Array.from({ length: dataStreamCount }, (_, i) =>
            timerange('2023-11-20T15:00:00.000Z', '2023-11-20T15:01:00.000Z')
              .interval('1m')
              .rate(1)
              .generator((timestamp) =>
                log
                  ...
                  .message('This is a log message')
                  .timestamp(timestamp)
                  .dataset(`chunking.test.dataset.${i.toString().padStart(2, '0')}`)
                  .defaults({
                    'log.file.path': '/my-service.log',
                  })
              )
      ...
      it('returns creation date for all data streams when includeCreationDate is true', async () => {
        const stats = await callApiAs(
          supertestDatasetQualityMonitorWithCookieCredentials,
          ['logs'],
          true
        ...
        const chunkingTestDataStreams = stats.body.dataStreamsStats.filter((ds: { name: string }) =>
          ds.name.includes('chunking.test.dataset')
        );

        expect(chunkingTestDataStreams.length).to.be(dataStreamCount);
        for (const ds of chunkingTestDataStreams) {
          expect(ds.creationDate).to.be.a('number');
          expect(ds.creationDate).greaterThan(0);
        }
      });
    });
  });
