...
import { apm, timerange, Instance } from '@kbn/apm-synthtrace-client';

const MAX_DEPENDENCIES = 10000;
const MAX_DEPENDENCIES_PER_SERVICE = 500;
const MAX_SERVICES = 20;

export async function generateManyDependencies({
  from,
  to,
  ...
  const instanceDependencies = (instance: Instance, startIndex: number) => {
    return timerange(new Date(from), new Date(to))
      .interval('1m')
      .rate(60)
        ...
        const span = instance
          ...
          .children(
            instance
              ...
              .destination(currentIndex === 0 ? 'elasticsearch' : `elasticsearch/${destination}`)
              .timestamp(timestamp)
              .duration(200)
              .success()
          );
  ...
  const data = instances.flatMap((instance, index) =>
