  ...
  registry.when('Api Key privileges check', { config: 'basic' }, () => {
    describe('index privileges', function () {
      ...
      it('returns user authorization as false for noAccessUser', async () => {
        ...
        expect(resp.body.datasetUserPrivileges.datasetsPrivilages.logs.canMonitor).to.be(false);
      ...
      it('get list of privileged data streams for datasetQualityMonitorUser', async () => {
        ...
        expect(resp.body.datasetUserPrivileges.datasetsPrivilages.logs.canMonitor).to.be(true);
        expect(
