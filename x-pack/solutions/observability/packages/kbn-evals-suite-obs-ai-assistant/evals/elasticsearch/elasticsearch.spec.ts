...
evaluate.describe('Elasticsearch function', { tag: '@svlOblt' }, () => {
  ...
  evaluate.describe('index management', () => {
    ...
    evaluate.describe('existing index', () => {
      ...
      evaluate(
        'returns the count of docs in the KB',
        async ({ evaluateDataset: evaluateDataset }) => {
          await evaluateDataset({
            dataset: {
      ...
      evaluate(
        'returns store stats of an index',
        async ({ evaluateDataset: evaluateElasticsearchDataset }) => {
      ...
      evaluate(
        'returns refresh stats of an index',
        async ({ evaluateDataset: evaluateElasticsearchDataset }) => {
          await evaluateElasticsearchDataset({
            dataset: {
  ...
  evaluate('returns cluster license', async ({ evaluateDataset: evaluateElasticsearchDataset }) => {
    await evaluateElasticsearchDataset({
      dataset: {
