...
import type { EvaluateDatasetFn } from '../create_evaluate';
...
export const paymentUnreachableScenario = (
  evaluate: {
    describe: Function;
    beforeAll: Function;
    afterAll: Function;
    ...
    name: string,
    fn: (fixtures: { evaluateDataset: EvaluateDatasetFn }) => Promise<void>
  ) => void),
  agentLabel: string
) => {
  evaluate.describe(
    `${agentLabel} - Payment Unreachable Investigation`,
    { tag: tags.serverless.observability.complete },
    () => {
      let replayResult: LoadResult;

      evaluate.beforeAll(async ({ esClient, log }: any) => {
      ...
      evaluate.afterAll(async ({ esClient, log }: any) => {
