...
- `obs-ai-datasets/otel-demo/product-catalog` — product catalog service failure on product OLJCESPC7Z (via the `productCatalogFailure` feature flag in the OTel demo)
- `obs-ai-datasets/otel-demo/ad-high-cpu` — ad service high CPU load causing latency spikes (via the `adHighCpu` feature flag in the OTel demo)
