...
describe('RootCauseCard', () => {
  ...
  describe('esql_query drill-down (new — not in the original streams_app version)', () => {
    const evidenceWithQuery: Evidence = {
      rule_name: 'auth_5xx_spike',
      esql_query: 'FROM logs.auth | WHERE status_code >= 500 | STATS count = COUNT(*)',
    };

    it('does not render a Query section when esql_query is absent', () => {
      renderWithI18n(
        <RootCauseCard event={{ ...baseEvent, evidences: [{ rule_name: 'no-query' }] }} />
      );

      fireEvent.click(screen.getByText('no-query'));

      expect(screen.queryByText('Query')).not.toBeInTheDocument();
    });

    it('renders the raw esql_query text when present', () => {
      renderWithI18n(<RootCauseCard event={{ ...baseEvent, evidences: [evidenceWithQuery] }} />);

      fireEvent.click(screen.getByText('auth_5xx_spike'));

      expect(screen.getByText('Query')).toBeInTheDocument();
      expect(screen.getByText(evidenceWithQuery.esql_query!)).toBeInTheDocument();
    });

    it('does not render "Open in Discover" when onOpenInDiscover is not provided', () => {
      renderWithI18n(<RootCauseCard event={{ ...baseEvent, evidences: [evidenceWithQuery] }} />);

      fireEvent.click(screen.getByText('auth_5xx_spike'));

      expect(screen.queryByText('Open in Discover')).not.toBeInTheDocument();
    });

    it('renders "Open in Discover" and calls the callback with the evidence when clicked', () => {
      const onOpenInDiscover = jest.fn();
      renderWithI18n(
        <RootCauseCard
          event={{ ...baseEvent, evidences: [evidenceWithQuery] }}
          onOpenInDiscover={onOpenInDiscover}
        />
      );

      fireEvent.click(screen.getByText('auth_5xx_spike'));
      fireEvent.click(screen.getByText('Open in Discover'));

      expect(onOpenInDiscover).toHaveBeenCalledWith(evidenceWithQuery);
    });
  });
