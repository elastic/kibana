...
import { NightshiftOverview } from './nightshift_overview';
...
describe('NightshiftOverview', () => {
  ...
  it('renders when state is critical', () => {
    renderWithIntl(<NightshiftOverview {...defaultProps} state="critical" />);
    expect(screen.getByTestId('sigeventsOverview')).toBeInTheDocument();
  });

  it('renders with default state (critical)', () => {
    renderWithIntl(<NightshiftOverview {...defaultProps} />);
    expect(screen.getByTestId('sigeventsOverview')).toBeInTheDocument();
  });

  it('renders the healthy/no-critical view when state is warning', () => {
    renderWithIntl(<NightshiftOverview {...defaultProps} state="warning" />);
    expect(screen.getByTestId('sigeventsOverview')).toBeInTheDocument();
  ...
  describe('healthy variant', () => {
    it('renders the StatusHeader (no-critical variant) and the 5 default healthy metric cards', () => {
      renderWithIntl(<NightshiftOverview {...defaultProps} state="healthy" />);
  ...
  it('passes blast radius score to the MainSignificantEvent', () => {
    renderWithIntl(<NightshiftOverview {...defaultProps} state="critical" blastRadiusScore={95} />);
    expect(screen.getByText('95')).toBeInTheDocument();
  });

  it('passes a custom main event title', () => {
    renderWithIntl(
      <NightshiftOverview {...defaultProps} state="critical" mainEventTitle="Custom event title" />
    );
  ...
  it('does not render the impacted cards section when empty', () => {
    renderWithIntl(<NightshiftOverview {...defaultProps} state="critical" impactedCards={[]} />);
    expect(screen.queryByTestId('sigeventsOverviewImpactedCards')).not.toBeInTheDocument();
  });
