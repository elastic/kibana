...
import { executeGetLogs } from '../../src/tool_client';
 ...
 *
 * Both arms call `observability.get_logs` through the tool execution API, so the
 * ranking is deterministic and the only variable is whether the semantic path is
 * used. Token cost and latency are not measured here; they belong to the agent
