...
export const TrustedDeviceCommonFieldsEntry = lazySchema(() =>
  z.object({
    ...
    field: z
      ...
      .describe('Device field to match against (available on all OS)'),
    /**
     * Entry match type
     */
    type: z.enum(['match', 'wildcard', 'match_any']).describe('Entry match type'),
...
export const TrustedDeviceUsernameEntry = lazySchema(() =>
  z.object({
    ...
    field: z
      .literal('user.name')
      .describe('Username field (Windows-only, requires os_types to be exclusively Windows)'),
    /**
     * Entry match type
     */
    type: z.enum(['match', 'wildcard', 'match_any']).describe('Entry match type'),
...
export const BlocklistWindowsCodeSignatureEntry = lazySchema(() =>
  z.object({
     ...
     */
    field: z.literal('file.Ext.code_signature').describe('Windows code signature field'),
    /**
     * Must be nested for Windows code signature
     */
    type: z.literal('nested').describe('Must be nested for Windows code signature'),
    /**
    ...
    entries: z
      .array(
        z.object({
           ...
           */
          field: z.literal('subject_name').describe('Certificate subject name'),
          /**
           * Match type for subject name
           */
          type: z.enum(['match', 'match_any']).describe('Match type for subject name'),
          value: z.union([z.string(), z.array(z.string()).min(1)]),
          /**
           * Must be the value "included"
           */
      ...
      .min(1)
      .describe('Nested subject_name entries'),
  })
);
...
export const TrustedAppHashEntry = lazySchema(() =>
  z.object({
     ...
     */
    field: z
      .enum(['process.hash.md5', 'process.hash.sha1', 'process.hash.sha256'])
      .describe('Process hash field'),
    /**
     * Hash entries only support match type
     */
    type: z.literal('match').describe('Hash entries only support match type'),
    /**
     * Hash value (MD5, SHA1, or SHA256)
     */
    value: z.string().describe('Hash value (MD5, SHA1, or SHA256)'),
...
export const TrustedAppPathEntry = lazySchema(() =>
  z.object({
     ...
     */
    field: z.literal('process.executable.caseless').describe('Process executable path field'),
    /**
     * Path supports both match and wildcard types
     */
    type: z.enum(['match', 'wildcard']).describe('Path supports both match and wildcard types'),
    /**
     * Executable path
     */
    value: z.string().describe('Executable path'),
    operator: z.literal('included'),
  })
);
...
export const TrustedAppWindowsCodeSignatureEntry = lazySchema(() =>
  z.object({
     ...
     * Windows code signature field
     */
    field: z.literal('process.Ext.code_signature').describe('Windows code signature field'),
    ...
    entries: z
      .array(
        z.union([
          z.object({
             ...
             * Certificate subject name
             */
            value: z.string().describe('Certificate subject name'),
            operator: z.literal('included'),
          }),
          z.object({
             ...
             */
            value: z.literal('true').describe("Must be the string 'true'"),
            operator: z.literal('included'),
          }),
        ])
      )
      .min(2)
      .max(2)
      .describe('Must include exactly 2 entries - one for subject_name and one for trusted'),
  })
);
...
export const TrustedAppMacCodeSignatureEntry = lazySchema(() =>
  z.object({
     ...
     */
    field: z.literal('process.code_signature').describe('macOS code signature field'),
    type: z.literal('nested'),
    /**
     * Must include exactly 2 entries - one for subject_name and one for trusted
     */
...
export const ExceptionListItemComment = lazySchema(() =>
  z.object({
    ...
    updated_at: z
      ...
      .datetime()
      .optional()
      .describe('Autogenerated date of last object update.'),
...
**Fully flexible:** Supports any field name for maximum compatibility with detection rules. No field restrictions are enforced.
      ...
      */
    entries: ExceptionListItemEntryArray.optional().describe(
      'Exception entries for endpoint security exceptions (used to prevent detection rule alerts).\n\n**Fully flexible:** Supports any field name for maximum compatibility with detection rules. No field restrictions are enforced.\n'
    ),
...
export const TrustedAppsWindowsProperties = lazySchema(() =>
  z.object({
     ...
     */
    os_types: z
      .array(z.literal('windows'))
      .min(1)
      .max(1)
      .optional()
      .describe('Must be Windows only'),
    /**
    ...
    entries: z
      ...
      .min(1)
      .optional()
      .describe('Process hash, executable path, or code signature entries'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
export const TrustedAppsMacProperties = lazySchema(() =>
  z.object({
     ...
     * Must be macOS only
     */
    os_types: z.array(z.literal('macos')).min(1).max(1).optional().describe('Must be macOS only'),
    /**
     * Process hash, executable path, or code signature entries
     */
    entries: z
...
export const TrustedAppsLinuxProperties = lazySchema(() =>
  z.object({
     ...
     * Must be Linux only
     */
    os_types: z.array(z.literal('linux')).min(1).max(1).optional().describe('Must be Linux only'),
    /**
    ...
    entries: z
      ...
      .min(1)
      .optional()
      .describe('Process hash or executable path entries (code signature not supported on Linux)'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
export const TrustedDevicesWindowsProperties = lazySchema(() =>
  z.object({
      ...
      .array(
        z.object({
          ...
          field: z
            ...
            .describe('Device field to match against (user.name is Windows-only)'),
          /**
           * Entry match type
           */
          type: z.enum(['match', 'wildcard', 'match_any']).describe('Entry match type'),
    ...
    os_types: z
      ...
      .max(1)
      .optional()
      .describe('Must be Windows-only to allow username field'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
export const TrustedDevicesMacProperties = lazySchema(() =>
  z.object({
      ...
      .array(
        z.object({
          ...
          field: z
            ...
            .describe('Device field to match against'),
          /**
           * Entry match type
           */
          type: z.enum(['match', 'wildcard', 'match_any']).describe('Entry match type'),
     ...
     * macOS-only
     */
    os_types: z.array(z.literal('macos')).min(1).max(1).optional().describe('macOS-only'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
export const TrustedDevicesWindowsMacProperties = lazySchema(() =>
  z.object({
      ...
      .array(
        z.object({
          ...
          field: z
            ...
            .describe('Device field to match against (username not available for multi-OS)'),
          /**
           * Entry match type
           */
          type: z.enum(['match', 'wildcard', 'match_any']).describe('Entry match type'),
    ...
    os_types: z
      ...
      .max(2)
      .optional()
      .describe('Must include both Windows and macOS (username field not allowed)'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
export const HostIsolationProperties = lazySchema(() =>
  z.object({
    ...
    entries: z
      .array(
        z.object({
           ...
           */
          field: z.literal('destination.ip').describe('Must be destination.ip'),
          /**
           * Must be match
           */
          type: z.literal('match').describe('Must be match'),
          /**
           * Valid IPv4 address or CIDR notation (e.g., "192.168.1.1" or "10.0.0.0/8")
           */
          value: z
            .string()
            .describe('Valid IPv4 address or CIDR notation (e.g., "192.168.1.1" or "10.0.0.0/8")'),
          /**
           * Must be the value "included"
           */
          operator: z.literal('included').describe('Must be the value "included"'),
      ...
      .max(1)
      .optional()
      .describe('Exactly one entry allowed for host isolation exceptions'),
    /**
    ...
    os_types: z
      ...
      .max(3)
      .optional()
      .describe('Must include all three operating systems (windows, linux, macos)'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
* Code signature entry: only 1 allowed
     ...
     * Windows-only
     */
    os_types: z.array(z.literal('windows')).min(1).max(1).optional().describe('Windows-only'),
    tags: EndpointArtifactTags.optional(),
  })
);
...
* Path entry: only 1 allowed
     ...
     * Linux-only
     */
    os_types: z.array(z.literal('linux')).min(1).max(1).optional().describe('Linux-only'),
    tags: EndpointArtifactTags.optional(),
  })
);
