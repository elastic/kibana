...
**Feature id (current):** `securitySolutionRulesV4` (`RULES_FEATURE_ID_V4`, `RULES_FEATURE_LATEST`)
...
- **`securitySolutionRulesV2`:** Deprecated; display name was “Rules, Alerts, and Exceptions.” Privileges were split so that **`securitySolutionRulesV4` + `securitySolutionAlertsV1`** replace the older combined model (`replacedBy` mappings in the v2 config).
- **`securitySolutionRulesV3`:** Deprecated; replaced by **`securitySolutionRulesV4`** (`replacedBy` mappings in the v3 config).
