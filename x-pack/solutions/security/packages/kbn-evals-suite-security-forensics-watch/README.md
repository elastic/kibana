# @kbn/evals-suite-security-forensics-watch

Coverage for **Forensics Watch** — the specialist tier that receives an
...
`workflow.execute`-sequenced and has no router surface), Forensics Watch is invoked
...
| --- | --- | --- | --- |
| **L0 — routing smoke** | `evals/routing_smoke.spec.ts` | Live LLM (Scout) | The Agent Builder router invokes the Forensics Watch skill for the natural-language turns that should reach it (`skillInvoked` + correct tool). |
...
node scripts/jest --config \
  x-pack/solutions/security/packages/kbn-evals-suite-security-forensics-watch/jest.config.js
...
node scripts/evals run --suite security-forensics-watch \
