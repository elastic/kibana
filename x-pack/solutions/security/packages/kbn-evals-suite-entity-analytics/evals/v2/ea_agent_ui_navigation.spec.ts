 ...
 *   2. Calls the generic `platform.core.redirect_user_to` tool with the matching
...
const LINK_TOOL_ID = 'platform.core.redirect_user_to';
      ...
      async ({ evaluateDataset }) => {
        await evaluateDataset({
          dataset: {
            ...
            description:
              'Enabling/disabling Entity Analytics and clearing all entity data are global controls at the top of the management page — the agent redirects there (bare management page, no tab subpath) via platform.core.redirect_user_to with the management-page path.',
            examples: [
              {
      ...
      async ({ evaluateDataset }) => {
        await evaluateDataset({
          dataset: {
            ...
            description:
              'Risk-scoring configuration and re-score-now intents redirect to the Risk Score management tab via platform.core.redirect_user_to with the risk_score tab path.',
            examples: [
              {
      ...
      async ({ evaluateDataset }) => {
        await evaluateDataset({
          dataset: {
            ...
            description:
              'Asset criticality bulk / CSV upload intents redirect to the Asset Criticality management tab via platform.core.redirect_user_to with the asset_criticality tab path.',
            examples: [
              {
      ...
      async ({ evaluateDataset }) => {
        await evaluateDataset({
          dataset: {
            ...
            description:
              'Bulk-linking entities to resolution targets via a CSV import redirects to the Entity Resolution management tab via platform.core.redirect_user_to with the entity_resolution tab path. (Single-entity resolution opens a per-entity flyout and is not covered here.)',
            examples: [
              {
                input: {
      ...
      async ({ evaluateDataset }) => {
        await evaluateDataset({
          dataset: {
            ...
            description:
              'Entity store / engine status questions redirect to the Status management tab via platform.core.redirect_user_to with the status tab path.',
            examples: [
              {
