...
import type { DetectionEngineeringChatClient, ErrorResponse, Step, Messages } from './chat_client';
...
const evaluateToolCallAssertion = async (
  ...
  return {
    ...
    explanation: `Tool "${toolCallAssertion.id}" was called. ${
      toolCriteriaResult.explanation ?? ''
    }`,
  };
};
...
const createCriteriaEvaluator = ({
  ...
  evaluate: async ({ expected, ...rest }: EvaluateOpts) => {
    ...
    if (criteria.length === 0) {
      return { score: 1, label: 'PASS', explanation: 'No main criteria specified.' };
    }
...
const createToolCallsEvaluator = ({
  ...
  evaluate: async ({ input, output, expected, metadata }: EvaluateOpts) => {
    ...
    if (toolCalls.length === 0) {
      return { score: 1, label: 'PASS', explanation: 'No tool call assertions specified.' };
    }
