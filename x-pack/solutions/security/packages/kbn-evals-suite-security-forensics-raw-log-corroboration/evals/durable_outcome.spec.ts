 ...
 *
 * EXPECTED STATE AT THIS HEAD: this gate fails, and that failure is the
 * measurement. Nothing in the tree under test writes `.pnd-investigations` — the
 * `threat-hunting` skill registers ES|QL, search, index discovery and cases, and
 * the probed `emit_corroboration` / `recordDeepWatch` ids have no implementation
 * — so the worker genuinely has no durable outcome yet, which is exactly what an
 * L4 gate is supposed to report. The spec deliberately does NOT skip on a
 * pre-run index probe: index existence is state, not evidence that a producer is
 * registered, and a skip would suppress the first write that creates the index
 * in the first place. It goes green when a producer lands.
 */
