...
/** Minimal shape the parser needs from an anonymized alert document. */
export interface AnonymizedAlertInput {
  id?: string;
  page_content: string;
}
...
/**
 * Build a lookup of parsed alert fields keyed by the real ES `_id` (which is NOT
 * anonymized). No ES call: the CSV is already in-hand as a workflow input.
 */
export const parseAnonymizedAlertsCsv = (
  anonymizedAlerts: AnonymizedAlertInput[]
