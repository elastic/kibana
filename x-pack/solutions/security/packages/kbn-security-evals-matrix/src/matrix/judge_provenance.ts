...
/** Family patterns, most-specific first (`nous` before `meta`: Hermes ids also contain "Llama"). */
...
/** Whether a judge id names an EIS-backed connector (`eis-` prefix or vendor-canonical id). */
...
export interface JudgePolicy {
  ...
  /** Reject a judge from the candidate's own family. Default false. */
...
/** Audit a matrix's judge pairings, counting docs per policy violation. */
...
export interface DerivedJudgeProvenance {
  /** A single judge id when every admitted run shares one, otherwise a `mixed: ...` summary. */
  judgeModelId: string;
  judgeBreakdown: JudgeShare[];
}

/** Derive which judge graded the admitted runs, counted from the aggregated scores. */
export function deriveJudgeProvenance(aggregated: AggregatedModelScores[]): DerivedJudgeProvenance {
  ...
  for (const model of aggregated) {
    for (const suite of model.suites ?? []) {
      // Suites split into separate experiments list every judge in `judgeModelIds`.
