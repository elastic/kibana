...
/** Where the numbers came from: run, branch, and window that produced the matrix. */
export interface MatrixProvenance {
  ...
  commitSha?: string;
  /** True when the generator's working tree had uncommitted changes. */
...
export const renderMatrix = (
  ...
  // A plain line rather than an HTML comment so it survives into the published docs.
