...
const inlineMd = (s: string): string =>
  s
    ...
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, text: string, url: string) =>
      // Untrusted model output: allow only whitespace-free http(s) URLs so no scheme or attribute can be injected.
...
/** Minimal markdown → HTML; input is untrusted, so every text run is escaped before inline markdown. */
const mdToHtml = (md: string): string => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Fenced code stays verbatim so ES|QL pipes are not parsed as tables.
    if (trimmed.startsWith('```')) {
      ...
      // Indented sub-bullets open a nested <ul> inside the previous <li>.
...
/** Worst-case spread across repetitions as a `±` badge; empty for single-rep cells. */
...
/** Per-prompt score from the trace's own evaluator scores, or '' when the trace has none. */
const variantScore = (
          ...
          return cards
            .map(({ label, trace }) => {
              ...
              if (trace?.answer) {
                ...
                // Scores only, no conversation: the suite is code-evaluated without an agent.
                body +=
