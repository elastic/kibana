...
/** Converts direct example trace keys (the only ones carrying repTrails) into reliability cells. */
...
const reliabilityHtml = (agreement: ReliabilityRow, tied: boolean): string => {
  ...
  return (
    ...
    )} path similarity${orderOnly}${answer}</small>${hotspot}${tiedNote}`
  );
};
...
/** Renders the reliability view as a standalone page, separate from matrix.html. */
export const renderReliabilityHtml = (
  ...
  // A row is tied when its interval overlaps any other measured row's.
