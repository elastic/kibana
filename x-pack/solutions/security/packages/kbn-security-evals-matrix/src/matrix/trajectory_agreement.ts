 ...
 * Keyed on `tool_id`, never `tool_call_id`: provider invocation ids are unique per call.
 * A cell with one repetition is `unmeasured`, not 0 or 1.
 */
...
export interface TrajectoryCell {
  ...
  trails: string[][];
  /** True when the example is an open-ended capability probe with no repeatable path. */
  probe?: boolean;
...
export interface TrajectoryAgreement {
  ...
  repetitions: number;
  /** Pairwise order-preserving exact-match rate of tool_id lists in [0, 1]. */
  identicalRate?: number;
  /** Pairwise same-tool-set rate in [0, 1], ignoring order and repetition; always >= identicalRate. */
  toolSetRate?: number;
  ...
  pairs?: number;
  /** Mean pairwise similarity of final answers in [0, 1], independent of path agreement. */
...
export interface ReliabilityRow {
  ...
  identicalRate?: number;
  /** Pooled same-tool-set rate. */
  toolSetRate?: number;
  sequenceSimilarity?: number;
  /** Total pairs pooled across this model's measured cells. */
  pairs?: number;
  /** Wilson 95% interval on `identicalRate`. */
...
/** Wilson score interval at 95%, which stays within [0, 1] for small samples near 0. */
...
/** Index and tool of the first differing step between two trails, or undefined when identical. */
...
export const rowAgreement = (cells: TrajectoryCell[], modelId: string): ReliabilityRow => {
  // Probes have no repeatable path by contract, so they are excluded.
  ...
  // Pool pairs rather than averaging per-cell rates so the sample size is preserved.
...
/**
 * @deprecated Fallback probe classification for score documents that lack
 * `example.metadata.pathContract`.
 */
...
/** Resolves whether an example is a probe, preferring the declared contract, and reports the source. */
...
/**
 * Per-repetition tool_id sequences; docs sharing a repetition_index are the same run and the first wins.
 */
