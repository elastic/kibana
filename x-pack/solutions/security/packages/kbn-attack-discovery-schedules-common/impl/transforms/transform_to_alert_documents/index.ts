...
export const generateAttackDiscoveryAlertHash = ({
   ...
   * - `generationSource` - optional producer identity so independent writers to the shared
   *               Attack Discovery index do not deduplicate against each other. Omitted for
   *               existing Kibana AD runs so their hashes stay byte-identical.
   */
  const alertIds = attackDiscovery.alertIds;
  const originalAlertIds = getOriginalAlertIds({ alertIds, replacements });
  const sourceSuffix = generationSource == null ? '' : `|generation_source=${generationSource}`;
  const hashInput =
    [...originalAlertIds].sort().join() + connectorId + ownerId + spaceId + sourceSuffix;
