/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

/*
 * NOTICE: Do not edit this file manually.
 * This file is automatically generated by the OpenAPI Generator, @kbn/openapi-generator.
 *
 * info:
 *   title: Post Generate Internal API endpoint
 *   version: 2023-10-31
 */

import { z, lazySchema } from '@kbn/zod/v4';

import { Replacements, ApiConfig } from '../../../common_attributes.gen';

export const PostGenerateRequestBody = lazySchema(() =>
  z.object({
    /**
     * Alerts index pattern for retrieval
     */
    alerts_index_pattern: z.string().max(1024).describe('Alerts index pattern for retrieval'),
    /**
     * Anonymization replacements map
     */
    replacements: Replacements.optional().describe('Anonymization replacements map'),
    /**
     * Connector configuration
     */
    api_config: ApiConfig.describe('Connector configuration'),
    /**
     * End time for alert retrieval
     */
    end: z.string().max(1024).optional().describe('End time for alert retrieval'),
    /**
     * Query filter for alert retrieval
     */
    filter: z
      .object({})
      .catchall(z.unknown())
      .optional()
      .describe('Query filter for alert retrieval'),
    /**
     * Maximum number of alerts to retrieve
     */
    size: z.number().int().optional().default(10).describe('Maximum number of alerts to retrieve'),
    /**
     * Start time for alert retrieval
     */
    start: z.string().max(1024).optional().describe('Start time for alert retrieval'),
    /**
     * How this generation was triggered
     */
    source: z
      .enum(['action', 'interactive', 'scheduled'])
      .optional()
      .default('interactive')
      .describe('How this generation was triggered'),
    /**
     * Metadata about the trigger source (e.g. rule details for action/scheduled)
     */
    source_metadata: z
      .object({
        /**
         * UUID of the alerting action execution
         */
        action_execution_uuid: z
          .string()
          .max(1024)
          .optional()
          .describe('UUID of the alerting action execution'),
        /**
         * ID of the alerting rule that triggered this generation
         */
        rule_id: z
          .string()
          .max(1024)
          .optional()
          .describe('ID of the alerting rule that triggered this generation'),
        /**
         * Name of the alerting rule that triggered this generation
         */
        rule_name: z
          .string()
          .max(1024)
          .optional()
          .describe('Name of the alerting rule that triggered this generation'),
      })
      .optional()
      .describe('Metadata about the trigger source (e.g. rule details for action/scheduled)'),
    /**
     * Optional composite workflow configuration for alert retrieval and validation. Three independent retrieval toggles (skill_enabled, default_retrieval_enabled, alert_retrieval_workflows_enabled) compose the alert set; at least one must be enabled.
     */
    workflow_config: z
      .object({
        /**
         * Query mode for the built-in default alert retrieval workflow. Only meaningful when default_retrieval_enabled is true.
         */
        alert_retrieval_mode: z
          .enum(['custom_query', 'esql'])
          .optional()
          .default('custom_query')
          .describe(
            'Query mode for the built-in default alert retrieval workflow. Only meaningful when default_retrieval_enabled is true.'
          ),
        /**
         * Array of user-created alert retrieval workflow IDs to execute. Only meaningful when alert_retrieval_workflows_enabled is true.
         */
        alert_retrieval_workflow_ids: z
          .array(z.string().max(1024))
          .optional()
          .default([])
          .describe(
            'Array of user-created alert retrieval workflow IDs to execute. Only meaningful when alert_retrieval_workflows_enabled is true.'
          ),
        /**
         * Toggle 3 - whether the user-created alert retrieval workflows run.
         */
        alert_retrieval_workflows_enabled: z
          .boolean()
          .optional()
          .default(false)
          .describe('Toggle 3 - whether the user-created alert retrieval workflows run.'),
        /**
         * Toggle 2 - whether the built-in default alert retrieval workflow runs.
         */
        default_retrieval_enabled: z
          .boolean()
          .optional()
          .default(false)
          .describe('Toggle 2 - whether the built-in default alert retrieval workflow runs.'),
        /**
         * ES|QL query for alert retrieval (required when default_retrieval_enabled is true and alert_retrieval_mode is 'esql').
         */
        esql_query: z
          .string()
          .max(10000)
          .optional()
          .describe(
            "ES|QL query for alert retrieval (required when default_retrieval_enabled is true and alert_retrieval_mode is 'esql')."
          ),
        /**
         * Toggle 1 - whether the attack discovery skill performs its own additional alert retrieval.
         */
        skill_enabled: z
          .boolean()
          .optional()
          .default(true)
          .describe(
            'Toggle 1 - whether the attack discovery skill performs its own additional alert retrieval.'
          ),
        /**
         * ID of the validation workflow to use (or 'default' for built-in)
         */
        validation_workflow_id: z
          .string()
          .max(1024)
          .optional()
          .default('default')
          .describe("ID of the validation workflow to use (or 'default' for built-in)"),
      })
      .optional()
      .describe(
        'Optional composite workflow configuration for alert retrieval and validation. Three independent retrieval toggles (skill_enabled, default_retrieval_enabled, alert_retrieval_workflows_enabled) compose the alert set; at least one must be enabled.'
      ),
  })
);
export type PostGenerateRequestBody = z.infer<typeof PostGenerateRequestBody>;
export type PostGenerateRequestBodyInput = z.input<typeof PostGenerateRequestBody>;

export const PostGenerateResponse = lazySchema(() =>
  z.object({
    /**
     * Unique identifier for this generation execution
     */
    execution_uuid: z.string().uuid().describe('Unique identifier for this generation execution'),
  })
);
export type PostGenerateResponse = z.infer<typeof PostGenerateResponse>;
