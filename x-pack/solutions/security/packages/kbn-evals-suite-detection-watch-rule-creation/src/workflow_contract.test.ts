...
// The registry is the single source for both the yaml and the version, so this asserts
// exactly what the alertzero plugin installs — not a copy that could drift from it.
const RULE_CREATION_DEFINITION = getManagedWorkflowDefinition(ALERTZERO_RULE_CREATION_WORKFLOW_ID);
if (!RULE_CREATION_DEFINITION || typeof RULE_CREATION_DEFINITION.yaml !== 'string') {
  throw new Error(
    `Managed workflow "${ALERTZERO_RULE_CREATION_WORKFLOW_ID}" is not registered with an inline yaml definition`
  );
}
const RULE_CREATION_YAML: string = RULE_CREATION_DEFINITION.yaml;
...
describe('system-security-rule-creation contract', () => {
  ...
  it('bumps the definition version so versionStrategy:auto reinstalls the change', () => {
    ...
    expect(RULE_CREATION_DEFINITION.version).toBeGreaterThanOrEqual(3);
  });
});
