...
/**
 * Asserts the managed rule-creation workflow the alert_zero plugin installs at start is present, and
 * returns its yaml.
 *
  ...
  try {
    ...
    throw new Error(
      `Managed workflow "${RULE_CREATION_WORKFLOW_ID}" is not installed. It is installed at ` +
        `plugin start by the alert_zero plugin (installStatic / ALERT_ZERO_WATCH_WORKFLOW_IDS), so this ` +
        `usually means the alert_zero plugin is disabled or the Workflows feature is unavailable. ` +
