...
/**
 * Ported verbatim from `kbn-evals-suite-detection-watch-rule-creation`, with one
 * adaptation: the sibling aliases its per-suite evaluator type as `RuleEvaluator`
 * in `./dataset_evaluator`. This suite's evaluators are plain `@kbn/evals`
 * evaluators, so the alias is declared here and the body below is unchanged.
 */
type RuleEvaluator = Evaluator;
