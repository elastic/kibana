...
/**
 * Solution navigation for the Alert Zero app, shared by ESS and serverless so the two cannot drift.
 *
 * Nodes are omitted from the rendered tree when `xpack.alertzero.enabled` is false, because the deep links
 * they reference are only registered by the Alert Zero plugin when it is enabled.
 */

/**
 * The Alert Zero nodes that sit directly in the solution navigation body, in display order.
 *
 * Returned as a list rather than a single node because Alert Zero contributes several top-level entries that
...
/** Alert Zero nodes that follow the platform Discover / Dashboards entries. */
