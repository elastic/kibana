...
/**
 * Titles of the sample conversations these proposals hang off, keyed by conversation id.
 * A conversation is an Agent Builder conversation, so there is no AlertZero-owned object to
 * read them from — the titles the mock list API resolves live here.
 */
const CONVERSATION_TITLES = new Map([
  ['inv-officer-impossible-travel-001', 'Impossible travel — exec account (cfo@corp)'],
  ['inv-officer-sales-nas-002', 'Ransomware encryption — Sales file server'],
  ['inv-floor-domain-admins-003', 'Unauthorized Domain Admins elevation'],
  ['inv-floor-findb-staged-005', 'Staged archive — FIN-DB-02'],
  ['inv-floor-phishing-url-006', 'Phishing — invoice lure (Finance)'],
  ['inv-floor-oauth-tune-007', 'OAuth token abuse · app-salesforce-sync'],
  ['inv-hunt-mailbox-auto-008', 'Mailbox forwarding rule removed — j.reyes'],
]);
...
/**
 * Mock proposals in the shape the proposals API returns. Every `conversationId` is a key of
 * `CONVERSATION_TITLES`, so the samples describe one set of incidents and a card title is
 * derived from that link rather than restated alongside each proposal.
 *
...
export const MOCK_PROPOSALS: MockProposal[] = PROPOSALS.map((proposal) => ({
  ...proposal,
  conversationTitle: CONVERSATION_TITLES.get(proposal.conversationId),
}));
