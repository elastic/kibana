...
/**
 * Technologies the MVP resolves an index scope for (plan.md Phase 2). Extend only alongside a TECHNOLOGY_INDEX_MAP entry.
 */
...
/**
 * A2's output contract (plan.md:245). `required`/`optional` list every pattern checked for the technology; `missing` lists every pattern from either list that resolved to zero indices.
 */
...
/**
 * Tier 1's raw statuses (plan.md:259). The hit/clean collapse for evidence[].last_hunt_status happens in the managed-workflow evidence step, not here.
 */
...
export const HuntForThreatResult = lazySchema(() =>
  z.object({
    ...
    /**
     * A confirmed event match in a required index inside the window (hunt-watch-implementation.md:42). Never set by an optional-index or out-of-window match.
     */
    hasConfirmedHit: z
      .boolean()
      .describe(
