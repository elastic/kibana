...
import { PciEvalChatClient } from './chat_client';
import type { EvaluatePciDataset } from './evaluate_dataset';
import { createEvaluatePciDataset } from './evaluate_dataset';

export const evaluate = base.extend<
  {},
  {
    chatClient: PciEvalChatClient;
    evaluateDataset: EvaluatePciDataset;
    internalEsClient: Client;
  }
>({
  chatClient: [
    async ({ fetch, log, connector }, use) => {
      const chatClient = new PciEvalChatClient(fetch, log, connector.id);
