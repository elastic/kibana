...
/**
 * Distance from a handle to the shared vertical riser.
 * Outgoing edges from the same node turn here so their verticals coincide.
 */
...
export const getGraphEdgePath = ({
  ...
  const shareSourceRiser = stepOffset === 0;
  const stub = GRAPH_EDGE_STEP_OFFSET;
  const goingRight = aligned.targetX >= aligned.sourceX;
  const sharedStepX = goingRight ? aligned.sourceX + stub : aligned.sourceX - stub;
  ...
  const [path] = getSmoothStepPath({
    ...
    offset: shareSourceRiser ? stub : stepOffset,
    ...(shareSourceRiser ? { centerX: sharedStepX } : {}),
  });
