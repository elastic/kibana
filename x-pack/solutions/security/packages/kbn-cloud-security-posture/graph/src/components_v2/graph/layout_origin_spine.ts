...
import { isConnectorNode, isEntityNode, isRelationshipNode, isStackNode } from '../utils';
...
): string[] => (g[direction](nodeId) ?? []).map((id) => id.toString());

/**
 * Moves non-spine nodes so 1:1 connector ↔ entity hops share a handle band.
 * Spine nodes stay put so the origin investigation path remains a straight line.
 */
export const alignHandleBandsInPlace = (
  g: Dagre.graphlib.Graph,
  nodesById: Record<string, Node<NodeViewModel>>,
  edges: Array<Edge<EdgeViewModel>>,
  stackedNodeIds: Set<string>
  ...
  const trySetCenterY = (nodeId: string, y: number): void => {
    if (!spineIds.has(nodeId)) {
      setNodeCenterY(g, nodeId, snapped(y));
    }
  };

  for (const nodeId of g.nodes()) {
    const data = nodesById[nodeId]?.data;

    if (!data || !isConnectorNode(data)) {
      continue;
    }

    const predecessors = neighborIds(g, nodeId, 'predecessors');
    const successors = neighborIds(g, nodeId, 'successors');
    const exclusiveSuccessor =
      successors.length === 1 && neighborIds(g, successors[0], 'predecessors').length === 1
        ? successors[0]
        : undefined;
    const exclusivePredecessor =
      predecessors.length === 1 && neighborIds(g, predecessors[0], 'successors').length === 1
        ? predecessors[0]
        : undefined;

    if (exclusiveSuccessor) {
      trySetCenterY(nodeId, getNodeCenterY(g, exclusiveSuccessor));
    } else if (successors.length >= 2) {
      const successorYs = successors.map((id) => getNodeCenterY(g, id));
      trySetCenterY(nodeId, (Math.min(...successorYs) + Math.max(...successorYs)) / 2);
    } else if (exclusivePredecessor) {
      trySetCenterY(nodeId, getNodeCenterY(g, exclusivePredecessor));
    }
  }
  ...
  for (const nodeId of g.nodes()) {
    ...
    if (!data || !isEntityNode(data) || spineIds.has(nodeId)) {
      continue;
    }

    const successors = neighborIds(g, nodeId, 'successors');

    if (successors.length === 1 && neighborIds(g, successors[0], 'predecessors').length === 1) {
      trySetCenterY(nodeId, getNodeCenterY(g, successors[0]));
    }
  }
};

/** Converts a laid-out node position to its connection-point Y (handle band). */
export const getLayoutNodeCenterY = (node: Node<NodeViewModel>): number => {
  if (node.type === 'label' || node.type === 'relationship' || isConnectorNode(node.data)) {
    return node.position.y + EVENT_PILL_HEIGHT / 2;
  }

  if (isEntityNode(node.data)) {
    return (
      node.position.y + (node.measured?.height ?? node.height ?? CARD_NODE_DEFAULT_HEIGHT) / 2
    );
  }
