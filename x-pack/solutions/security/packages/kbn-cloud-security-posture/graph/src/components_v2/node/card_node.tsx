...
/** Risk severity bands aligned with Entity Analytics. */
type EntityRiskLevel = 'critical' | 'high' | 'moderate' | 'low' | 'unknown';

const EA_CRITICALITY_TITLE: Record<EaCriticalityLevel, string> = {
...
const COUNT_LEVEL_TO_EA: Record<CriticalityLevel, EaCriticalityLevel> = {
  extreme: 'extreme_impact',
  high: 'high_impact',
...
const GroupStackTab = styled.div<{
  ...
  border-left: 1px solid ${({ defaultBorderColor }) => defaultBorderColor};
  border-right: 1px solid ${({ defaultBorderColor }) => defaultBorderColor};
  border-bottom: 1px solid ${({ defaultBorderColor }) => defaultBorderColor};
...
/**
 * Light EUI badge fills used by Entity Analytics Entities table (`RiskScoreCell`).
 */
const getRiskScoreBadgeColors = (
  level: EntityRiskLevel,
  colors: {
    backgroundBaseSubdued: string;
  ...
  switch (level) {
    case 'critical':
      return { background: colors.backgroundLightDanger, text: colors.textDanger };
    case 'high':
      return { background: colors.backgroundLightRisk, text: colors.textRisk };
    case 'moderate':
      return { background: colors.backgroundLightWarning, text: colors.textWarning };
    case 'low':
      return { background: colors.backgroundBaseNeutral, text: colors.textNeutral };
    case 'unknown':
    default:
      return { background: colors.backgroundBaseSubdued, text: colors.textSubdued };
  }
};

/**
 * Entity type icon + container fill, aligned with risk-score severity.
 * Icon uses Borders/Strong/{severity}; container uses Backgrounds/Base/{severity}.
 * Unknown icon uses Datavis/Utilities/Avatar Text (`vis.euiColorVisGrey2`).
 */
const getEntityIconColors = (
  level: EntityRiskLevel,
  colors: {
    ...
    vis: { euiColorVisGrey2: string };
  }
): { icon: string; background: string } => {
  switch (level) {
    case 'critical':
      return { icon: colors.borderStrongDanger, background: colors.backgroundBaseDanger };
    case 'high':
      return { icon: colors.borderStrongRisk, background: colors.backgroundBaseRisk };
    case 'moderate':
      return { icon: colors.borderStrongWarning, background: colors.backgroundBaseWarning };
    case 'low':
      return { icon: colors.borderStrongNeutral, background: colors.backgroundBaseNeutral };
    case 'unknown':
    default:
      return { icon: colors.vis.euiColorVisGrey2, background: colors.backgroundBaseSubdued };
  }
};

/** Match Entity Analytics `formatRiskScore` (two decimal places). */
const formatRiskScore = (riskScore: number) => (Math.round(riskScore * 100) / 100).toFixed(2);
...
const parseEaCriticalityLevel = (value?: string): EaCriticalityLevel => {
  const normalized = (value ?? '').toLowerCase().replace(/\s+/g, '_');
  if (normalized === 'extreme_impact' || normalized.includes('extreme')) return 'extreme_impact';
  if (normalized === 'high_impact' || normalized.includes('high')) return 'high_impact';
  if (normalized === 'medium_impact' || normalized.includes('medium')) return 'medium_impact';
  if (normalized === 'low_impact' || normalized.includes('low')) return 'low_impact';
  return 'unassigned';
};

const getEaCriticalityColor = (
  level: EaCriticalityLevel,
  severity: { danger: string; risk: string; warning: string; neutral: string; unknown: string }
): string => {
  switch (level) {
    case 'extreme_impact':
...
const AssetCriticalityHealthBadge = ({
  level,
  label,
}: {
  level: EaCriticalityLevel;
  ...
  return (
    <EuiHealth
      color={getEaCriticalityColor(level, euiTheme.colors.severity)}
      textSize="xs"
      css={metadataTextCss}
    >
      {label ?? EA_CRITICALITY_TITLE[level]}
    </EuiHealth>
  );
};
...
const CriticalityCountsGrid = ({
  ...
  <CriticalityGrid>
    {(['extreme', 'high', 'medium', 'low'] as const).flatMap((level) => {
      ...
      if (!val) return [];
      const eaLevel = COUNT_LEVEL_TO_EA[level];
      return [
        <AssetCriticalityHealthBadge
          key={level}
          level={eaLevel}
          label={`${val} ${EA_CRITICALITY_TITLE[eaLevel]}`}
        />,
      ];
    })}
  </CriticalityGrid>
...
const RiskScoreBadge = ({ score }: { score: number }) => {
  const { euiTheme } = useEuiTheme();
  const colors = getRiskScoreBadgeColors(getEntityRiskLevel(score), euiTheme.colors);
 ...
 * plain white card, primary-blue icon, title, type + Entities-table risk badge.
 */
...
export const CardNode = memo<NodeProps>((props: NodeProps) => {
  ...
  const { euiTheme } = useEuiTheme();
  // Figma Graph viz (13969:1176) — X-small Level 2 at rest, one step up on hover.
  ...
  }, [clearHoverCloseTimer, closeEntityActions]);
  ...
  const iconColors = getEntityIconColors(getEntityRiskLevel(displayRiskScore), euiTheme.colors);
  const iconBg = iconColors.background;
          ...
          <CardShellClip>
            ...
            {hasBody && (
              <CardBody data-test-subj={GRAPH_ENTITY_NODE_DETAILS_ID}>
                {(showCriticality || showDataSource) && (
                  <MetadataRow>
                    {showCriticality && (
                      <MetadataField>
                        <FieldLabel>
                        ...
                        <MetadataValueRow>
                          {assetCriticality && !assetCriticalityCounts && (
                            <AssetCriticalityHealthBadge
                              level={parseEaCriticalityLevel(assetCriticality)}
                            />
                          )}
                          {assetCriticalityCounts && (
                    ...
                    {showDataSource && (
                      <MetadataField>
                        <FieldLabel>
                          {i18n.translate(
                            'securitySolutionPackages.csp.graph.node.card.label.dataSource',
                            { defaultMessage: 'Data source' }
                          )}
                        </FieldLabel>
                        <MetadataValueRow>
                          <FieldValue truncate>{dataSource}</FieldValue>
                        </MetadataValueRow>
                      </MetadataField>
                    )}
                  </MetadataRow>
