...
describe('CountryFlags', () => {
  test('renders null when input is unexpectedly undefined', () => {
  ...
  test('renders badge with only country flags when below the limit', async () => {
    ...
    fireEvent.mouseOver(screen.getByTestId(TEST_SUBJ_BADGE) as Element);
  ...
  test('renders badge with counter, tooltip and two first country flags when over the limit and all repeated the same amount', async () => {
    ...
    expect(tooltipContent).toHaveTextContent('🇺🇸 United States');
  ...
  test('renders badge with counter, tooltip and two most repeated country flags when over the limit', async () => {
    render(<CountryFlags countryCodes={['US', 'FR', 'DE', 'FR', 'DE']} />);
    // France and Germany are both tied with 2 codes
    // but France's first code appears before Germany's first code
    // so France is first
    expect(screen.queryByTestId(TEST_SUBJ_BADGE)).toHaveTextContent('🇫🇷🇩🇪');
    expect(screen.queryByTestId(TEST_SUBJ_TOOLTIP_CONTENT)).not.toBeInTheDocument();
