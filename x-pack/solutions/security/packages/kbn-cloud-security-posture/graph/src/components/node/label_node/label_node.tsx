...
export const LabelNode = memo<NodeProps>((props: NodeProps) => {
  ...
  const shouldShowTooltip =
    isTextTruncated || analysis.totalEvents > BADGES_LIMIT || analysis.totalAlerts > BADGES_LIMIT;
  ...
  return (
    <>
            ...
            >
              ...
              >
                <EuiTextTruncate truncation="end" text={text}>
                  {(truncatedText) => {
                    setTextTruncated(truncatedText.length !== text.length);
      ...
      {/* TODO Remove type-casting when `ips` and `countryCodes` exist in schema */}
      <LabelNodeDetails ips={ips as string[]} countryCodes={countryCodes as string[]} />
    </>
  );
});
