...
export type MockScenario = 'single-actor' | 'grouped-actor' | 'grouped-target';

const getMockDataForScenario = (scenario: MockScenario = 'single-actor') => {
  if (scenario === 'grouped-actor') {
...
export const useFetchGraphData = (params: UseFetchGraphDataParams) => {
  ...
  return useMemo(
    () => ({
      ...
      isError: false,
      data: getMockDataForScenario(useFetchGraphDataMock?.scenario),
