...
`scout/index.ts` exposes the generated clients as worker fixtures named after the FTR services (`detectionsApi`, `exceptionsApi`, `listsApi`, `timelinesApi`, `entityAnalyticsApi`, `endpointManagementApi`, `endpointExceptionsApi`, `osqueryApi`). Merge them into your module's test type:
