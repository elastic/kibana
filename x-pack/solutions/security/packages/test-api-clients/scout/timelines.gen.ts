...
import type {
  ...
  DeleteNoteResponse,
} from '@kbn/security-solution-plugin/common/api/timeline/delete_note/delete_note_route.gen';
import type {
...
> If the user already has a draft Timeline, the existing draft Timeline is cleared and returned.
  ...
  async copyTimeline<TResponseType extends ScoutResponseType = 'json'>(
    ...
    return apiClient.get<ScoutResponseBody<TResponseType, CopyTimelineResponse>>(path, {
      headers: {
        'kbn-xsrf': 'true',
        [ELASTIC_HTTP_VERSION_HEADER]: '2023-10-31',
        [X_ELASTIC_INTERNAL_ORIGIN_REQUEST]: 'kibana',
  ...
  async deleteNote<TResponseType extends ScoutResponseType = 'json'>(
    ...
    options: ScoutApiRequestOptions<TResponseType> = {}
  ): Promise<ApiClientResponse<ScoutResponseBody<TResponseType, DeleteNoteResponse>>> {
    const basePath =
      options.kibanaSpace && options.kibanaSpace !== 'default' ? `/s/${options.kibanaSpace}` : '';
    const path = `${basePath}/api/note`;

    return apiClient.delete<ScoutResponseBody<TResponseType, DeleteNoteResponse>>(path, {
      headers: {
        'kbn-xsrf': 'true',
        [ELASTIC_HTTP_VERSION_HEADER]: '2023-10-31',
        [X_ELASTIC_INTERNAL_ORIGIN_REQUEST]: 'kibana',
