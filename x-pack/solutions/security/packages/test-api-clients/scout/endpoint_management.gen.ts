...
const securitySolutionScoutApiServiceFactory = (apiClient: ApiClientFixture) => ({
  ...
  async createWorkflowInsight<TResponseType extends ScoutResponseType = 'json'>(
    props: CreateWorkflowInsightProps,
    options: ScoutApiRequestOptions<TResponseType> = {}
  ): Promise<ApiClientResponse<ScoutResponseBody<TResponseType, CreateWorkflowInsightResponse>>> {
    const basePath =
      options.kibanaSpace && options.kibanaSpace !== 'default' ? `/s/${options.kibanaSpace}` : '';
    const path = `${basePath}/internal/api/endpoint/workflow_insights`;

    return apiClient.post<ScoutResponseBody<TResponseType, CreateWorkflowInsightResponse>>(path, {
      headers: {
        'kbn-xsrf': 'true',
...
> Files retrieved from third-party-protected hosts require a different password. Refer to [Third-party response actions](https://www.elastic.co/docs/solutions/security/endpoint-response-actions/third-party-response-actions) for your system's password.
  ...
  async getPendingWorkflowInsights<TResponseType extends ScoutResponseType = 'json'>(
    props: GetPendingWorkflowInsightsProps,
    options: ScoutApiRequestOptions<TResponseType> = {}
  ): Promise<
    ApiClientResponse<ScoutResponseBody<TResponseType, GetPendingWorkflowInsightsResponse>>
  > {
    const basePath =
      options.kibanaSpace && options.kibanaSpace !== 'default' ? `/s/${options.kibanaSpace}` : '';
    const path = `${basePath}/internal/api/endpoint/workflow_insights/pending`;

    return apiClient.get<ScoutResponseBody<TResponseType, GetPendingWorkflowInsightsResponse>>(
      `${path}?${stringifyQuery(props.query, { arrayFormat: 'none' })}`,
      {
        headers: {
          'kbn-xsrf': 'true',
