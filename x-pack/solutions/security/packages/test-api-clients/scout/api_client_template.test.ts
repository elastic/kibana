...
import type { FindRulesResponse } from '@kbn/security-solution-plugin/common/api/detection_engine/rule_management/find_rules/find_rules_route.gen';
...
describe('generated Scout API client (api_client_scout template)', () => {
  ...
  it('types the body after the requested response type', async () => {
    const ndjson = '{"id":"a"}\n{"id":"b"}\n';
    const apiClient = createApiClientMock();
    apiClient.get.mockResolvedValue({ ...response, body: ndjson });
    const detectionsApi = createDetectionsApi(apiClient);
    const props = { query: { page: 1 } };

    const textResponse = await detectionsApi.findRules(props, { responseType: 'text' });
    const text: string = textResponse.body;

    expect(text).toBe(ndjson);

    apiClient.get.mockResolvedValue({ ...response, body: Buffer.from(ndjson) });
    const bufferResponse = await detectionsApi.findRules(props, { responseType: 'buffer' });
    const buffer: Buffer = bufferResponse.body;

    expect(buffer.toString('utf8')).toBe(ndjson);

    apiClient.get.mockResolvedValue({ ...response, body: { data: [] } });
    const jsonResponse = await detectionsApi.findRules(props);
    const data: FindRulesResponse['data'] = jsonResponse.body.data;
  ...
  it('lets the exported options type describe a reusable text download', async () => {
    ...
    await detectionsApi.exportRules(
      { query: { exclude_export_details: true }, body: { objects: [{ rule_id: 'rule-1' }] } },
      downloadOptions
    );
  ...
  it('percent-encodes reserved characters in path params', async () => {
    ...
    await osqueryApi.osqueryGetSavedQueryDetails({ params: { id: 'team/alice?x#y' } });

    const [url] = apiClient.get.mock.calls[0];

    expect(url).toBe('/api/osquery/saved_queries/team%2Falice%3Fx%23y');
  });
