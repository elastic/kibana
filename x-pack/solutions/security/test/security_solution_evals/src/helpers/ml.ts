...
import { fetchKibanaStatus } from '@kbn/security-solution-plugin/common/endpoint/utils/kibana_status';
...
export const installIntegrationAndCreatePolicy = async ({
  ...
  const stackVersion = kbnStatus.version.number.replace(/-SNAPSHOT$/, '');

  // Ensure the integration we install matches the currently running stack version.
  // This prevents tests from accidentally pulling a newer integration package (e.g. 9.3.0) than the stack (e.g. 9.2.4).
  await installIntegration(kbnClient, integrationName, stackVersion);
