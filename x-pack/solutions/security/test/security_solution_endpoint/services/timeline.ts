...
export function TimelineTestServiceProvider({ getService }: FtrProviderContext) {
  ...
  return new (class TimelineTestService {
    ...
    async createTimeline(title: string): Promise<PatchTimelineResponse> {
      ...
      log.info('Draft timeline:');
      log.indent(4, () => {
        log.info(JSON.stringify(createdTimeline));
      });
      ...
      // Update the timeline
      const updatedTimelineResponse = await this.updateTimeline(
        timelineId,
        timelineUpdate,
        version
      );
      ...
      log.indent(4, () => {
        log.info(JSON.stringify(updatedTimelineResponse));
      });
