...
test.describe(
  ...
  () => {
    ...
    test('should only show Security SOC Management sub-privilege', async ({
      ...
      await page.getByRole('option', { name: /All Spaces/i }).first().click();
      await pageObjects.ai4dsoc.securityCategory.first().click();
      await expect(
        page.locator('button.euiAccordion__arrow').first()
      ).toBeVisible();
    ...
    test('should not show Timelines and Notes features', async ({
      pageObjects,
      page,
    }) => {
      await pageObjects.ai4dsoc.assignToSpaceButton.first().click();
    ...
    test('should not show Siem Migration feature', async ({
      pageObjects,
      page,
    }) => {
      await pageObjects.ai4dsoc.assignToSpaceButton.first().click();
      ...
      await page.getByRole('option', { name: /All Spaces/i }).first().click();
      await expect(
        pageObjects.ai4dsoc.siemMigrationsFeatureLocator().first()
      ).not.toBeAttached();
    });
