...
test.describe(
  ...
  () => {
    ...
    test('uploads a valid CSV file', async ({ page, pageObjects }) => {
      ...
      await page.getByTestId('privileged-user-monitoring-file-picker').first().setInputFiles({
        name: 'valid_file.txt',
        mimeType: 'text/plain',
    ...
    test('it validates an invalid CSV file', async ({ page, pageObjects }) => {
      ...
      await expect(
        pageObjects.entityAnalyticsPrivMon.validationStep.first()
      ).toContainText("1 row is invalid and won't be added");
    });
  }
);
