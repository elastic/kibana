...
test.describe(
  'Alert suppression - indicator match rule',
  {
  ...
  () => {
    ...
    test('Creates indicator match rule with per-execution suppression and verifies details', async ({
      page,
      pageObjects,
      kbnClient,
    }) => {
    ...
    test('Creates indicator match rule with time interval suppression and verifies details', async ({
      page,
      pageObjects,
      kbnClient,
    }) => {
      ...
      const definitionDetails = page.testSubj.locator('definitionRule');
      await expect(definitionDetails).toBeVisible();
      await expect(definitionDetails.getByText(SUPPRESS_BY_FIELDS[0])).toBeVisible();
