...
test.describe(
  'Alert details expandable flyout left panel response',
  ...
  () => {
    test.beforeEach(async ({ browserAuth, page, apiServices }) => {
      ...
      await page.testSubj.locator('securitySolutionFlyoutResponseTab').click();
    });
    ...
    test('should display empty response message', async ({ page }) => {
      ...
      await expect(page.testSubj.locator('securitySolutionFlyoutResponseDetails')).toContainText(
        'Responses'
      );

      await expect(page.testSubj.locator('securitySolutionFlyoutResponseNoData')).toBeVisible();
    });
  }
);
