...
test.describe('AI Assistant Prompts', { tag: ['@ess', '@svlSecurity'] }, () => {
  test.beforeEach(async ({ browserAuth, apiServices, browserScopedApis }) => {
    // Login
    ...
    await browserScopedApis.assistant.createConversation({
      id: `${mockConvo1.id}-${timestamp}`,
      title: mockConvo1.title,
    });
    await browserScopedApis.assistant.createConversation({
      id: `${mockConvo2.id}-${timestamp}`,
      title: mockConvo2.title,
    });
  });

  test.describe('System Prompts', () => {
    test.beforeEach(async ({ browserScopedApis }) => {
    ...
    test('Deselecting a system prompt prevents prompt from being sent. When conversation is then cleared, the prompt remains cleared.', async ({
      page,
      pageObjects,
    }) => {
      await page.gotoApp('security', { path: '/get_started' });
    ...
    test('Add prompt from system prompt selector and set multiple conversations (including current) as default conversation', async ({
      page,
      pageObjects,
    }) => {
      await page.gotoApp('security', { path: '/get_started' });
  ...
  test.describe('Quick Prompts', () => {
    test('Add a quick prompt and send it in the conversation', async ({ page, pageObjects }) => {
      await page.gotoApp('security', { path: '/get_started' });
      await pageObjects.assistantPage.open();
    ...
    test('Add a quick prompt with context and it is only available in the selected context', async ({
      page,
      pageObjects,
      apiServices,
    }) => {
      ...
      await pageObjects.assistantPage.createQuickPrompt(
        testPrompt.name,
        testPrompt.content,
        ['Alert (from view)']
      );
