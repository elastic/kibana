...
import {
  ...
  DOCUMENT_DETAILS_FLYOUT_FOOTER_ADD_TO_CASE_SUBMIT,
...
export const openTakeActionButtonAndSelectItem = (option: string) => {
  ...
  if (
    option === DOCUMENT_DETAILS_FLYOUT_FOOTER_ADD_TO_NEW_CASE ||
    option === DOCUMENT_DETAILS_FLYOUT_FOOTER_ADD_TO_EXISTING_CASE
  ) {
    selectTakeActionItem(DOCUMENT_DETAILS_FLYOUT_FOOTER_ADD_TO_CASE);
    selectTakeActionItem(option);
    cy.get(DOCUMENT_DETAILS_FLYOUT_FOOTER_ADD_TO_CASE_SUBMIT).click();
    return;
  }
  selectTakeActionItem(option);
};
