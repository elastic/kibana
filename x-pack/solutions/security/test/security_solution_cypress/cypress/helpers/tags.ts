...
/**
 * Asserts that a rule tags element's rendered text matches `expectedText`, ignoring the
 * environment-specific "Missing Elastic Cloud API Key" tag (see MISSING_UIAM_API_KEY_TAG) so the
 * assertion holds on both ESS/local and MKI.
 */
export const shouldHaveTagsText = (
  chainable: Cypress.Chainable<JQuery<HTMLElement>>,
  expectedText: string | undefined
): void => {
  chainable.invoke('text').then((text) => {
    cy.wrap(text.replace(MISSING_UIAM_API_KEY_TAG, '')).should('equal', expectedText);
  });
};
