...
export async function enableEntityStore(
  ...
  { extraIndexPatterns = [], entityTypes }: Options = {}
): Promise<void> {
  ...
  const RETRIES = 5;
  const RETRY_DELAY_MS = 5000; // Wait between retries to let tasks complete
  let success: boolean = false;

  for (let attempt = 0; attempt < RETRIES; attempt++) {
    log.info(`Enabling Entity Store (attempt ${attempt + 1}/${RETRIES})...`);

    const requestBody: { indexPattern: string; entityTypes?: EntityType[] } = {
      indexPattern,
    };
    if (entityTypes) {
      requestBody.entityTypes = entityTypes;
    }
