...
export default ({ getService }: FtrProviderContext) => {
  ...
  describe('@ess @serverless @serverlessQA Bulk disable', () => {
    before(async () => {
    ...
    describe('Happy path', () => {
      it('should bulk disable schedules by `id`', async () => {
        const apis = getAttackDiscoverySchedulesApis({ supertest });
        const firstSchedule = await apis.create({
          schedule: getSimpleAttackDiscoverySchedule({ enabled: true }),
        });
        const secondSchedule = await apis.create({
          schedule: getSimpleAttackDiscoverySchedule({ enabled: true }),
        });

        const result = await apis.bulkDisable({ ids: [firstSchedule.id, secondSchedule.id] });

        expect(result.ids.sort()).toEqual([firstSchedule.id, secondSchedule.id].sort());
        expect(result.total).toEqual(2);
        expect(result.errors).toEqual([]);
        await checkIfScheduleDisabled({ getService, id: firstSchedule.id });
        await checkIfScheduleDisabled({ getService, id: secondSchedule.id });
      });
    });
    ...
    describe('Partial results', () => {
      it('should exclude missing schedules from successful ids', async () => {
        ...
        const result = await apis.bulkDisable({ ids: [createdSchedule.id, 'missing-schedule'] });
