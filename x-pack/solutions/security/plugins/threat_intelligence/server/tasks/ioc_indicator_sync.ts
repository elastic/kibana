...
import type { ThreatIntelligenceStartDependencies } from '../types';
import {
...
const buildBulkBody = (reports: ReportHit[], now: string): IocIndicatorDoc[] => {
  ...
  for (const report of reports) {
    ...
    for (const ioc of iocs) {
      if (!ioc.value || !isIocType(ioc.type)) continue;
...
export const registerIocIndicatorSyncTask = ({
  ...
  coreSetup: CoreSetup<ThreatIntelligenceStartDependencies>;
  logger: Logger;
}): void => {
