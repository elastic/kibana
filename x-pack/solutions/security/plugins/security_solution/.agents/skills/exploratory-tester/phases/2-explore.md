...
2b. Knowledge approval already happened in Phase 0 Step 0g, **for this mode too** — this step only reads that result, it never prompts the user again (a duplicated, independently-answered prompt here was a real bug in an earlier revision of this skill). Read `config.json → knowledge_file`. If `approved` is `true`, `path` and `sha256` are the exact full repo-relative path and hash the user already confirmed — pass both to every sub-agent prompt in steps 3 and 7. If `approved` is `false` (declined, or no knowledge file exists), omit the knowledge file lines from all sub-agent prompts entirely. Never re-derive `path` from `area_slug` yourself and never substitute a guessed hash — both come only from `config.json → knowledge_file`.
2c. Run `create-flow-spaces.py` unconditionally, before dispatching Wave 1:
   ```bash
   python3 x-pack/solutions/security/plugins/security_solution/.agents/skills/exploratory-tester/scripts/create-flow-spaces.py \
     --session-dir "$SESSION_DIR"
   ```
   Phase 1 already runs this for the normal (non-resumed) path, so this is usually a no-op — flows that already have a `space_id` are reconciled (HTTP 409 → reuse), not recreated. It is not redundant for a *resumed* session (`Session-dir:` provided): `phases/0-setup.md`'s Resume path skips all of Phase 1 unconditionally, so a session that crashed after Phase 0 wrote `config.json` (with every flow's `space_id` still `null`) but before Phase 1 finished creating spaces would otherwise dispatch Wave 1 with `flow.space_id` unset, and every sub-agent would construct an invalid `/s/null/...` navigation URL. Running it here closes that gap the same way step 6b already does for Wave 2 and `phases/2-investigation.md` already does for parallel investigation flows.
3. Dispatch sub-agents concurrently via the Agent tool. **Read `templates/subagent-prompt.md` and use it verbatim as each sub-agent prompt, substituting the placeholders (`<flow object as JSON>`, `<value of $SESSION_DIR>`, `<N>`, `<knowledge file path, or omitted entirely>`, `<knowledge file sha256, or omitted entirely>`) with actual values. Do not construct the prompt yourself.** Use the exact knowledge path and sha256 read from `config.json → knowledge_file` in step 2b, or omit both placeholder lines from the prompt if `approved` is `false` — never substitute a guessed path or hash.
...
6b. If any investigation flows were created, run `create-flow-spaces.py` again before dispatching Wave 2:
   ```bash
   python3 x-pack/solutions/security/plugins/security_solution/.agents/skills/exploratory-tester/scripts/create-flow-spaces.py \
     --session-dir "$SESSION_DIR"
   ```
   This is safe to rerun — flows from Wave 1 that already have a `space_id` are reconciled (HTTP 409 → reuse), only the newly appended investigation flows get a space created. Without this, Wave 2 sub-agents would read an unset `space_id` and construct an invalid `/s/null/...` navigation URL.
