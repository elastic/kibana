...
() => {
  ...
  document
    .querySelectorAll('[class*="euiToast--danger"], [data-test-subj*="toastErrorMessage"]')
    .forEach((el) =>
  ...
  document
    .querySelectorAll('[data-test-subj="globalBannerList"] [class*="danger"]')
    .forEach((el) =>
  ...
  if (embeddableErrors.length > 0)
    r.level2.push({
      type: 'embeddable_error',
      count: embeddableErrors.length,
      text: Array.from(embeddableErrors)
  ...
  document
    .querySelectorAll('[class*="euiCallOut--danger"]')
    .forEach((el) =>
  ...
  if (notRendered.length > 0)
    r.level2.push({
      type: 'panels_not_rendered',
      count: notRendered.length,
  ...
  document
    .querySelectorAll('[data-test-subj="searchResponseWarningsBadgeToogleButton"]')
    .forEach((el) =>
      r.level2.push({
        type: 'search_response_warning_badge',
        text:
          'Search response warning badge present (' +
          el.textContent.trim() +
          ') — click it to read the full message before concluding no warning exists',
      })
    );
  ...
  document.querySelectorAll('[data-test-subj="searchResponseWarningsCallout"]').forEach((el) =>
    r.level2.push({
      type: 'search_response_warning_callout',
