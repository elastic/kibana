...
- When suggesting a split of an oversized spec, keep the new files in the same `test/scout/<namespace>/` directory as the original. Apply the general one-role-one-flow check from `scout-best-practices-reviewer`.
