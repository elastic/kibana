...
- [ ] Response actions history assertions are scoped to hosts this test created — no global row counts — and Fleet setup used the worker space
...
- Indexing endpoint hosts with the default-space `kbnClient` and then opening response actions history in the worker space — the rows stay hidden unless Fleet setup uses that space
