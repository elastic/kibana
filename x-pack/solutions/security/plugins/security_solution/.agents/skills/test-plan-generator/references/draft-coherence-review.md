...
| D6 | *Known Limitations* ↔ *Sources Summary* | Every ⚠️ source referenced in *Known Limitations* (inaccessible Figma, no PR linked, external doc not fetched, Figma canvas / oversized section narrowed by user selection, Figma screenshot budget reached mid-draft) has a corresponding *Sources Summary* row, and vice versa. Statuses do not have to be identical — a partial-coverage case can pair `✅ Metadata read (M of N children catalogued — narrowed by user selection)` in *Sources Summary* with a `⚠️` KL entry — but every KL warning must be traceable to a *Sources Summary* row and vice versa | KL mentions Figma was inaccessible but Sources Summary lists Figma as `✅ Metadata read` with no qualifier; or a `✅ Metadata read (M of N children catalogued — narrowed by user selection)` row has no corresponding `⚠️` KL entry naming the un-catalogued children |
