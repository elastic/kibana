...
| D6 | *Known Limitations* ↔ *Sources Summary* | Every ⚠️ source referenced in *Known Limitations* (inaccessible Figma, no PR linked, external doc not fetched, Figma container narrowed by user selection, Figma MCP quota exhausted mid-fetch) has a corresponding *Sources Summary* row, and vice versa. Statuses do not have to be identical — a partial-coverage case can pair `✅ Read (M of N — user-selected subset)` in *Sources Summary* with a `⚠️` KL entry — but every KL warning must be traceable to a *Sources Summary* row and vice versa | KL mentions Figma was inaccessible but Sources Summary lists Figma as `✅ Read` with no qualifier; or a `✅ Read (M of N — user-selected subset)` row has no corresponding `⚠️` KL entry naming the un-inspected children |
