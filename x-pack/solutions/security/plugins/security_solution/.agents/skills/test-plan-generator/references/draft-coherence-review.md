...
| D6 | *Known Limitations* ↔ *Sources Summary* | Every ⚠️ source referenced in *Known Limitations* has a corresponding *Sources Summary* row, and vice versa. The pairing rule works from the *Sources Summary* row's status class, using the vocabulary defined in [`output-formats.md`](output-formats.md#sources-summary) *Figma links* section: **(a) Partial-coverage / narrowed rows** (`✅ Metadata read (M of N children catalogued — narrowed by user selection)`) always pair with a `⚠️` KL entry naming the un-catalogued children. **(b) Name-mismatch rows** (a plain `✅ Metadata read + K opened PNGs …` where an opened PNG revealed a name-vs-content mismatch) always pair with a `⚠️` KL entry naming the mismatch — the fetch worked, but the metadata is misleading. **(c) Make rows** (`✅ Design context read (Figma Make — code-only; no screenshot support)`) pair with a `⚠️` KL entry **only** when the Make link produced scenarios needing visual / layout / name-anchored verification (`get_screenshot` is Make-unsupported); a Make link whose scenarios are all code-only has no KL entry. **(d) All five canonical declined-cap rows** — `⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only)` (Design-file screenshot miss), `⚠️ Screenshot budget reached (Slides link catalogued without screenshot; no Figma-derived context)` (Slides screenshot miss — no metadata fallback available), `⚠️ get_metadata budget reached (3 fetched — remaining Figma links not inspected; scenarios derived from non-Figma sources)`, `⚠️ get_design_context budget reached (2 fetched — remaining escape-hatch calls skipped; affected scenarios fall back to metadata and any screenshot already gathered)` (Design escape-hatch miss), and `⚠️ get_design_context budget reached (2 fetched — remaining Make links catalogued without design context)` (Make miss with no fallback) — **always** pair with a `⚠️` KL entry naming the scenarios or links that could not be verified against the design; these rows are the declined / user-unavailable branch of [`gathering-context.md`](gathering-context.md#step-5--session-budget) Step 5. **(e) Raised-cap rows** — the explicit `get_screenshot` raised-cap indicators (`✅ Metadata read + K opened PNGs for visual verification (cap raised from 8 by user)` for Design links, `✅ K opened PNGs (Slides — <node name>) (cap raised from 8 by user)` for Slides links), or the implicit `get_metadata` / `get_design_context` raised-cap case (extra `✅ Metadata read` or `✅ Design context read` rows beyond the default cap of 3 / 2) — are approved overages and constitute complete coverage; they must **not** have a paired KL entry. Every KL warning must be traceable to a *Sources Summary* row and vice versa. | KL mentions Figma was inaccessible but Sources Summary lists Figma as `✅ Metadata read` with no qualifier; or a `✅ Metadata read (M of N children catalogued — narrowed by user selection)` row has no corresponding `⚠️` KL entry naming the un-catalogued children; or a screenshot revealed a name-vs-content mismatch but no KL entry names the mismatch; or a Make link produced a layout-anchored scenario but no KL entry names the missing visual verification; or a `⚠️ Screenshot budget reached (Slides link catalogued without screenshot; …)` row has no KL entry naming the un-inspected Slides link; or a `⚠️ get_metadata budget reached (…)` row has no KL entry naming the un-inspected links; or a `⚠️ get_design_context budget reached (…)` Make row has no KL entry; or an approved-overage row (any raised-cap indicator, or the implicit metadata/design-context raised-cap case) has a paired `⚠️` KL entry as if coverage were incomplete |
