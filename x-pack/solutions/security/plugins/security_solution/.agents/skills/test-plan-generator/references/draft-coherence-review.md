...
| D6 | *Known Limitations* ↔ *Sources Summary* | Every ⚠️ source referenced in *Known Limitations* (inaccessible Figma, no PR linked, external doc not fetched, Figma canvas / oversized section narrowed by user selection, Figma screenshot budget reached mid-draft — declined-or-user-unavailable branch, **Figma name-vs-content mismatch surfaced by an opened PNG**, **Figma Make link with scenarios needing visual verification** — `get_screenshot` is Make-unsupported per [`gathering-context.md`](gathering-context.md#step-4--escape-hatch-get_design_context) *Figma Make branch*) has a corresponding *Sources Summary* row, and vice versa. Statuses do not have to be identical — a partial-coverage case can pair `✅ Metadata read (M of N children catalogued — narrowed by user selection)` in *Sources Summary* with a `⚠️` KL entry; a name-mismatch case pairs a plain `✅ Metadata read + K opened PNGs for visual verification` row (the fetch itself worked) with a `⚠️` KL entry that names the mismatch; a Make link with layout/name-anchored scenarios pairs a plain `✅ Design context read (Figma Make — code-only; no screenshot support)` row with a `⚠️` KL entry that names the un-verifiable scenarios. An approved screenshot overage (raised cap) is complete coverage — the raised-cap Sources Summary row must **not** have a paired KL entry. Every KL warning must be traceable to a *Sources Summary* row and vice versa. | KL mentions Figma was inaccessible but Sources Summary lists Figma as `✅ Metadata read` with no qualifier; or a `✅ Metadata read (M of N children catalogued — narrowed by user selection)` row has no corresponding `⚠️` KL entry naming the un-catalogued children; or a screenshot revealed a node whose name did not match its rendered content but no KL entry names the mismatch; or a Make link produced a layout-anchored scenario but no KL entry names the missing visual verification; or an approved (raised-cap) `get_screenshot` overage still has a paired `⚠️` KL entry as if coverage were incomplete |
