...
| D6 | *Known Limitations* ↔ *Sources Summary* | Every ⚠️ source referenced in *Known Limitations* (inaccessible Figma, no PR linked, external doc not fetched, Figma canvas / oversized section narrowed by user selection, Figma screenshot budget reached mid-draft, **Figma name-vs-content mismatch surfaced by an opened PNG** — per Figma Step 3 in [`gathering-context.md`](gathering-context.md#step-3--add-visual-verification-with-get_screenshot-only-where-needed)) has a corresponding *Sources Summary* row, and vice versa. Statuses do not have to be identical — a partial-coverage case can pair `✅ Metadata read (M of N children catalogued — narrowed by user selection)` in *Sources Summary* with a `⚠️` KL entry, and a name-mismatch case pairs a plain `✅ Metadata read + K opened PNGs for visual verification` row (the fetch itself worked) with a `⚠️` KL entry that names the mismatch — but every KL warning must be traceable to a *Sources Summary* row and vice versa | KL mentions Figma was inaccessible but Sources Summary lists Figma as `✅ Metadata read` with no qualifier; or a `✅ Metadata read (M of N children catalogued — narrowed by user selection)` row has no corresponding `⚠️` KL entry naming the un-catalogued children; or a screenshot revealed a node whose name did not match its rendered content but no KL entry names the mismatch |
