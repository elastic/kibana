...
8. Output the Sources Summary as defined in [`output-formats.md`](output-formats.md). Include one row per linked PR showing whether it was re-read or skipped. **Render the refreshed Issue Clarity Assessment block in the chat after the Sources Summary**, identical to the one written into the draft. **Then render the token usage line** using the values from Step 6 — on success emit `**Token usage:** input=X, output=Y, cache_read=Z, cache_create=W, **total=T**` (commas between fields, bold label and total, `cache_read` before `cache_create`); on failure emit `**Token usage:** not available for this session`. Never substitute `total=0`. See [`output-formats.md`](output-formats.md#token-usage-marker) for the exact format.
