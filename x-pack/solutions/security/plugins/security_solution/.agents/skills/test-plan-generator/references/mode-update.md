...
6. **Apply only the identified changes.** Do not rewrite accurate sections.
   ...
   **Special handling: skill-rule migrations.** If `SKILL_RULE_MIGRATIONS` (from Step 5) is non-empty, treat each failing check as an identified change and apply its remedy on the updated draft. The remedy for every check is documented in the *What to verify* / *Common failure* columns of the two tables in [`draft-coherence-review.md`](draft-coherence-review.md#document-as-whole-coherence) — read the row for the failing check ID and fix the draft accordingly. Examples (both currently caught by `S6`): missing upgrade coverage → add the Upgrade scenarios template from [`optional-scenarios.md`](optional-scenarios.md#upgrade-scenarios), or add an *Out of scope* bullet with a one-clause reason; stale hardcoded upgrade versions → resolve fresh values via the placeholder rules in [`optional-scenarios.md`](optional-scenarios.md#upgrade-scenarios) and replace them. Do **not** rewrite sections that are not flagged — the migration is scoped to the failing checks only.
