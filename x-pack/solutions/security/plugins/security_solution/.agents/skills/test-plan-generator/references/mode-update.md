...
6. **Apply only the identified changes.** Do not rewrite accurate sections.
   ...
   **Compute the token usage marker, then save the updated draft.** Invoke `x-pack/solutions/security/plugins/security_solution/.agents/scripts/session-token-usage.py`. If the script exits 0, capture its output — space-separated `input=… output=… cache_create=… cache_read=… total=…` — as `TOKEN_LINE`. If it exits non-zero, leave `TOKEN_LINE` unset and skip marker prepending. Then save to `x-pack/solutions/security/plugins/security_solution/.agents/tmp/test-plan-#<issue_number>.md`, overwriting any existing draft at that path. This save is unconditional — it applies whether the changes involved new scenarios, edits to existing scenarios, Pending-work migrations, an assessment refresh, or only metadata updates (Known Limitations, AC list, feature-flag names, etc.). **If `TOKEN_LINE` was captured**, prepend `<!-- tokens: input=X output=Y cache_create=W cache_read=Z total=T -->` (same field order as the script output) as the **first line** of the updated draft. Overwriting the draft file naturally replaces any prior tokens marker from the previous publish — the marker always reflects the run that produced the current comment. The publish step will PATCH the existing GitHub comment with this file's contents. Skip the Sources Summary sub-step inside [Saving the draft](../SKILL.md#saving-the-draft) — the update-mode version with PR re-read info is emitted in step 8 below. See [`output-formats.md`](output-formats.md#token-usage-marker) for the marker spec and the fallback rule.
...
8. Output the Sources Summary as defined in [`output-formats.md`](output-formats.md). Include one row per linked PR showing whether it was re-read or skipped. **Render the refreshed Issue Clarity Assessment block in the chat after the Sources Summary**, identical to the one written into the draft. **Then render the token usage line.** If `TOKEN_LINE` was captured in Step 6, reuse it. **If Step 5's empty-change-list branch jumped here directly** (Steps 6 and 7 were skipped and `TOKEN_LINE` is unset), invoke `x-pack/solutions/security/plugins/security_solution/.agents/scripts/session-token-usage.py` here to capture it — the check itself has a token cost worth reporting. On success emit `**Token usage:** input=X, output=Y, cache_create=W, cache_read=Z, **total=T**` (commas between fields, bold label and total); on failure emit `**Token usage:** not available for this session`. Never substitute `total=0`. See [`output-formats.md`](output-formats.md#token-usage-marker) for the exact format.
