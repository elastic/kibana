...
| Figma — <file or node name> | ✅ Metadata read (N fetchable children catalogued) / ✅ Metadata read + K opened PNGs for visual verification / ✅ Metadata read (M of N children catalogued — narrowed by user selection) / ⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only) / ⚠️ Read with errors / ⚠️ Node not found in file / ⛔ Inaccessible |
...
- For **Figma links**, the status cell reflects the metadata-first flow defined in [`gathering-context.md`](gathering-context.md#figma):
  - Metadata only (no screenshot opened): `✅ Metadata read (N fetchable children catalogued)`, replacing N with the count of `frame` / `instance` / nested `section` / `component` / `component_set` layers under the root (the same fetchable-element filter defined in [`gathering-context.md`](gathering-context.md#figma) Step 2 — include `component` and `component_set` so a direct component link reports the right child count, not zero).
  - Metadata + targeted screenshots (typical case for UI features): `✅ Metadata read + K opened PNGs for visual verification`, replacing K with the number of PNGs actually **opened** (i.e. downloaded and inspected — the vision-token cost the session budget is measured in). `get_screenshot` calls that only returned a URL and were never opened do not count. Include the returned screenshot URLs inline in the status cell so a reader can preview them — Figma expires them after ~15 minutes, so treat them as a preview, not a stable reference.
  ...
  - Session screenshot budget reached mid-draft: `⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only)`, using the same "opened PNGs" unit as the budget itself. Also add a Known Limitations entry naming which scenarios lacked visual verification.
