...
- For **Figma links**, the status cell reflects the metadata-first flow defined in [`gathering-context.md`](gathering-context.md#figma):
  ...
  - **Session `get_screenshot` cap reached** — declined / user-unavailable branch of [Step 5](gathering-context.md#step-5--session-budget). Use `⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only)`, using the same "opened PNGs" unit as the budget itself. Also add a Known Limitations entry naming which scenarios lacked visual verification. This is valid only when the affected links already have metadata inventories built.
  - **Session `get_metadata` cap reached** — declined / user-unavailable branch. Use `⚠️ get_metadata budget reached (3 fetched — remaining Figma links not inspected; scenarios derived from non-Figma sources)` on every un-catalogued Figma link (one row per link — not one row for the session). Metadata-only fallback is **not** valid here because the un-catalogued links have no metadata at all; scenarios must come from non-Figma sources (issue text, PR diff, other Figma links whose inventory was built before the cap fired). Add a Known Limitations entry naming the un-inspected links.
  - **Session `get_design_context` cap reached** — declined / user-unavailable branch. Two variants depending on which link tripped the cap:
    - Design-file escape-hatch miss (the link has metadata + screenshot; only the escape hatch could not run): `⚠️ get_design_context budget reached (2 fetched — remaining escape-hatch calls skipped; scenarios verified from metadata + screenshot only)`. Add a KL entry naming the scenarios that would have benefited from the pixel-precise identifiers.
