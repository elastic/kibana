...
| Figma — <file or node name> | ✅ Metadata read (N fetchable elements catalogued) / ✅ Metadata read + K opened PNGs for visual verification / ✅ Metadata read (M of N children catalogued — narrowed by user selection) / ✅ Design context read (Figma Make — metadata-first flow skipped) / ⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only) / ⚠️ Read with errors / ⚠️ Node not found in file / ⛔ Inaccessible |
...
- For **Figma links**, the status cell reflects the metadata-first flow defined in [`gathering-context.md`](gathering-context.md#figma):
  ...
  - Figma Make link (`/make/` URL — Step 2 metadata step is unsupported, so the flow routes directly to `get_design_context` per [`gathering-context.md`](gathering-context.md#figma) Step 1): `✅ Design context read (Figma Make — metadata-first flow skipped)`. Do not add a Known Limitations entry for this case by itself — Make routing is by design, not a source gap; only add a ⚠️ entry when `get_design_context` itself failed or returned insufficient detail.
