...
4. **If the user declines, or is not present to answer** (async run, dry-run, batch generation), stop calling immediately and switch to metadata-only reasoning for the remaining scenarios. Emit an appropriate ⚠️ status in the Sources Summary: for the `get_screenshot` cap use the canonical `⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only)` defined in [`output-formats.md`](output-formats.md#sources-summary); for the rarer `get_metadata` (cap 3) and `get_design_context` (cap 2) hits, adapt the same `⚠️ <tool> budget reached (…)` shape and name the tool that hit — e.g. `⚠️ get_design_context budget reached (2 fetched — remaining Make links catalogued without design context)`. In every case, add a Known Limitations entry naming the specific scenarios that could not be verified. This is the only branch that produces a `⚠️` for a budget hit — an approved overage is not a source gap.
