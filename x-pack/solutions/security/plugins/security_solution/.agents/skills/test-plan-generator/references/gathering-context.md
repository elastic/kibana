...
4. **If the user declines, or is not present to answer** (async run, dry-run, batch generation), stop calling immediately and switch to metadata-only reasoning for the remaining scenarios. Emit the budget-reached status defined in [`output-formats.md`](output-formats.md#sources-summary) (`⚠️ Screenshot budget reached (K PNGs opened — remaining scenarios verified from metadata only)` or the `get_design_context` / `get_metadata` analogue) **and** add a Known Limitations entry naming the specific scenarios that could not be visually / structurally verified. This is the only branch that produces a `⚠️` for a budget hit — an approved overage is not a source gap.
...
- **Known Limitations.** Only add a ⚠️ entry when coverage is genuinely incomplete:
  ...
  - The session budget cap fired mid-draft and remaining scenarios could not be visually verified (declined / user-unavailable branch of Step 5 — approved overages are complete coverage and do **not** get a KL entry).
