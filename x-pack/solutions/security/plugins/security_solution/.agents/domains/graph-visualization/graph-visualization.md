...
- **DSL clauses must handle both single-value and array shapes** — Elasticsearch DSL allows bool clauses (`must_not`, `must`, `should`, `filter`) to be either a single object or an array. Normalize to an array before iterating; a `?? []` default only covers `undefined`, not the single-object shape. ([PR #289467](https://github.com/elastic/kibana/pull/289467) · @niros1 · `search_filters.ts`)
