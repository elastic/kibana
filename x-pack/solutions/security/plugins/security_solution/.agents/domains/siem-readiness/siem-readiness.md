...
- **Required-field coverage checks must use `ruleQueryIndices`, never `indexToRules`.** `indexToRules` includes auxiliary index sources (e.g., `threat_match` indicator indices from `threatIndex`) that share a rule entry for blast-radius purposes but whose schema `required_fields` do NOT describe. Checking required-field mapping against `indexToRules` produces false `partial` findings (e.g., event fields flagged as unmapped in `logs-ti*`). Use `ruleQueryIndices` (event/query indices only) and `ruleNames` for any field-mapping coverage check; keep `indexToRules` for blast-radius enrichment only.
...
- **The attachment schema, formatter, and agent description must all agree on which fields exist.** `validate` and `format` both call `.safeParse` on the raw data, which silently strips any key not declared in the schema. A field mentioned in `getAgentDescription()` but absent from the dimension schema (e.g., `siemReadinessQualityDataSchema`) will be silently dropped before the agent sees it. Whenever you add a field to a payload type (`QualityPayload`, `ContinuityPayload`, etc.) or reference it in the description, also add it to the corresponding `siemReadiness*DataSchema` and render it in the corresponding `format*ForAgent` function. The agent tool returning `ToolResultType.other` bypasses this schema, so tool tests will pass even when the schema is out of sync — always cross-check against the attachment file explicitly.
...
**Attachment schema/description mismatch silently drops fields via `.safeParse`:** The quality attachment schema was missing `type` on `actionableFindingSchema` and the entire `missingFieldsByRule` array — both of which `getAgentDescription()` promised the agent. Because the agent tool returns `ToolResultType.other` (bypassing `.safeParse`), all tool tests passed. The bug only manifests when a quality attachment is created from the payload and routed through `validate`/`format`. A generic reviewer would miss this because the tool path works correctly and nothing fails at compile time.
