...
test.describe.skip('AI Assistant - Basic License', { tag: [...tags.stateful.classic] }, () => {
  test('user with Basic license should not be able to use assistant', async ({
    ...
    await pageObjects.assistant.titleHeading.click();
    await expect(pageObjects.assistant.conversationTitle.locator('input')).not.toBeVisible();
    await expect(page.testSubj.locator('addNewConnectorButton')).toBeDisabled();
    await expect(page.testSubj.locator('chat-context-menu')).toBeDisabled();
    await expect(pageObjects.assistant.flyoutNavToggle).toBeDisabled();
    await expect(pageObjects.assistant.newChatButton).toBeDisabled();
  });
});
