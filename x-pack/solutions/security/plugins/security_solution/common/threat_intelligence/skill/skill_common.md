...
`logs-vulnerability.*`, `logs-aws.*`, `logs-okta.*`, `logs-kubernetes.*`,
`logs-github.*`, `logs-network_traffic.*`) for a report's IOCs and ATT&CK
technique IDs. Returns the top matching documents AND an `affected_assets`
aggregation (unique hosts + users). Body: `{ report_id?, iocs?, techniques?,
