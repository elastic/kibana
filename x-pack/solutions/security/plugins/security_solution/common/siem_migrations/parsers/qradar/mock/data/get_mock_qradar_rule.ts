...
const getMockQRadarRuleDataXml = (ruleName: string, isBuildingBlock: boolean = false) => {
  ...
  return {
    normal: `<rule buildingBlock="${isBuildingBlock}" enabled="true" id="${id}" name="BB:CategoryDefinition: Authentication Success" origin="SYSTEM" owner="admin" roleDefinition="false" scope="LOCAL" type="EVENT">
  <name>${ruleName}</name>
...
`,
    sanitized: `<rule buildingBlock="${isBuildingBlock}" enabled="true" id="${id}" name="BB:CategoryDefinition: Authentication Success" origin="SYSTEM" owner="admin" roleDefinition="false" scope="LOCAL" type="EVENT">
  <name>${ruleName}</name>
  <notes>Edit this BB to include all events that indicate successful attempts to access the network.</notes>
  <testDefinitions>
    <test group="Event Property Tests" id="21" name="com.q1labs.sem.semces.cre.tests.EventCategory_Test" uid="0">
