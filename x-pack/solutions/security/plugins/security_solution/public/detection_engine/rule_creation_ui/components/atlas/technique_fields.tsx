...
import { MitreAtlasSubtechniqueFields } from './subtechnique_fields';
import { useAtlasThreatOptions } from './use_atlas_threat_options';

const hasSubtechniqueOptions = (
  subtechniquesOptions: Array<{ techniqueId: string }>,
...
export const MitreAtlasTechniqueFields: React.FC<AddTechniqueProps> = ({
  field,
  idAria,
  isDisabled,
  threatIndex,
  ...
  const addAtlasTechnique = useCallback(() => {
    const threats = [...(field.value as Threats)];
    threats[threatIndex] = {
      ...threats[threatIndex],
      technique: [
  ...
  const getSelectTechnique = useCallback(
    (
      ...
      const options = techniquesOptions.filter((t) => t.tacticIds.includes(parentTactic.id));
      return (
        <EuiSuperSelect
          id="mitreAtlasTechnique"
          options={[
            ...(technique.name === 'none'
              ? [
                  {
          ...
          data-test-subj="mitreAtlasTechnique"
          disabled={disabled}
  ...
  return (
    <TechniqueContainer>
      ...
      <MyAddItemButton
        data-test-subj="addMitreAtlasTechnique"
        onClick={addAtlasTechnique}
        isDisabled={isDisabled}
      >
        {i18n.ADD_ATLAS_TECHNIQUE}
      </MyAddItemButton>
    </TechniqueContainer>
  );
};
