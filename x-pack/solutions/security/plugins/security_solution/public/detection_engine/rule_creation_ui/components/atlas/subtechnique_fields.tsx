...
export const MitreAtlasSubtechniqueFields: React.FC<AddSubtechniqueProps> = ({
  field,
  idAria,
  isDisabled,
  threatIndex,
  ...
  const { subtechniques: subtechniquesOptions } = useAtlasThreatOptions();
  ...
  const addAtlasSubtechnique = useCallback(() => {
    const threats = [...(field.value as Threats)];
    const subtechniques = technique[techniqueIndex].subtechnique;
    ...
    (index: number, disabled: boolean, subtechnique: ThreatSubtechnique) => {
      ...
      return (
        <EuiSuperSelect
          ...
          data-test-subj="mitreAtlasSubtechnique"
          disabled={disabled}
  ...
  return (
    <SubtechniqueContainer>
      ...
      <MyAddItemButton
        data-test-subj="addMitreAtlasSubtechnique"
        onClick={addAtlasSubtechnique}
        isDisabled={isDisabled}
      >
        {i18n.ADD_ATLAS_SUBTECHNIQUE}
      </MyAddItemButton>
    </SubtechniqueContainer>
  );
};
