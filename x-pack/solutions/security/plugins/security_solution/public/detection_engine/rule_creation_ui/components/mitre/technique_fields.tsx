...
export const MitreAttackTechniqueFields: React.FC<AddTechniqueProps> = ({
  ...
  const tacticsOptions = useMemo(() => mitreConfig?.tactics ?? [], [mitreConfig]);
  const techniquesOptions = useMemo(() => mitreConfig?.techniques ?? [], [mitreConfig]);
  const subtechniquesOptions = useMemo(() => mitreConfig?.subtechniques ?? [], [mitreConfig]);
