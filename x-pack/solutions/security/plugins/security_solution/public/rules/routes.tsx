...
import { BlogPostPrebuiltRuleCustomizationCallout } from '../detection_engine/rule_management_ui/components/blog_post_prebuilt_rule_customization_callout';
...
const RULES_MANAGEMENT_TAB_PATH = /^\/rules\/(management|monitoring|updates)(\/|$)/;

const RulesContainerComponent: React.FC = () => {
  useReadonlyHeader(i18n.READ_ONLY_BADGE_TOOLTIP);
  const { capabilities } = useKibana().services.application;
  const { pathname } = useLocation();
  const isRulesManagementTab = RULES_MANAGEMENT_TAB_PATH.test(pathname);
  ...
  return (
    <>
      {isRulesManagementTab && <BlogPostPrebuiltRuleCustomizationCallout />}
