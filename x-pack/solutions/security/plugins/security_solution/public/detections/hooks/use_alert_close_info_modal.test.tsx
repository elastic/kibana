...
describe('hasAlertsInSuppressionWindow', () => {
  ...
  type BuildDescribeBodyParams = { matchingAlerts: number, expectedResult: boolean }
  ...
  describe('when a list of ids is passed', () => {
    ...
    const buildDescribeBody = ({ matchingAlerts, expectedResult }: BuildDescribeBodyParams) => {
      ...
      it('should only use the list of ids and attempt to search through the alerts with the right query', () => {
        expect(fetchQueryAlertsMock).toHaveBeenCalledWith(expect.objectContaining({
          query: {
            query: {
              bool: {
                filter: expect.arrayContaining([
                  { ids: { values: ['id1', 'id2'] } },
                  { exists: { field: 'kibana.alert.rule.parameters.alert_suppression.duration.value' } },
                ])
              }
            },
            size: 0
  ...
  it('should throw an error if neither query nor ids are provided', async () => {
    await expect(
      hasAlertsInSuppressionWindow({})
    ).rejects.toThrow('either query or a non empty list of alert ids must be defined');
  });
})
