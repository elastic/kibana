...
AlertContextMenu.displayName = 'AlertContextMenu';
 ...
 */
export const AddExceptionFlyoutWrapper: React.FC<AddExceptionFlyoutWrapperProps> = memo(
  ({
    ruleId,
    ruleRuleId,
    ruleIndices,
    ...
    const queryAlertsParams = useMemo(() => {
      return {
        query: buildGetAlertByIdQuery(eventId),
        indexName: signalIndexName,
        queryName: ALERTS_QUERY_NAMES.ADD_EXCEPTION_FLYOUT,
      };
    }, [eventId, signalIndexName]);

    const { loading: isLoadingAlertData, data } = useQueryAlerts<EcsHit, {}>(queryAlertsParams);
...
AddExceptionFlyoutWrapper.displayName = 'AddExceptionFlyoutWrapper';
