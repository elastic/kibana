...
import type { InitializationFlowState } from '../../../../common/components/initialization';
import {
...
const mockInitState = (
  overrides?: Partial<
    Record<typeof INITIALIZATION_FLOW_CREATE_LIST_INDICES, InitializationFlowState<null>>
  >
) => ({
  [INITIALIZATION_FLOW_CREATE_LIST_INDICES]: {
    loading: false,
...
describe('useListsConfig', () => {
  ...
  describe('when initialization is loading', () => {
    beforeEach(() => {
      useSecuritySolutionInitializationMock.mockReturnValue(
        mockInitState({
          [INITIALIZATION_FLOW_CREATE_LIST_INDICES]: { loading: true, result: null },
        })
      );
    });
    ...
    beforeEach(() => {
      useSecuritySolutionInitializationMock.mockReturnValue(
        mockInitState({
          [INITIALIZATION_FLOW_CREATE_LIST_INDICES]: {
            loading: false,
            result: { status: INITIALIZATION_FLOW_STATUS_ERROR, error: 'Index creation failed' },
          },
        })
      );
    });
  ...
  it('calls useSecuritySolutionInitialization with create-list-indices', () => {
    ...
    expect(useSecuritySolutionInitializationMock).toHaveBeenCalledWith([
      INITIALIZATION_FLOW_CREATE_LIST_INDICES,
    ]);
  });
});
