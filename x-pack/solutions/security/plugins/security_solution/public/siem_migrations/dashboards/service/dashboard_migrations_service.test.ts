...
describe('SiemDashboardMigrationsService', () => {
  ...
  describe('addDashboardsToMigration performance', () => {
    it('should demonstrate reduced event loop blocking with Promise.all vs sequential await', async () => {
      // Create a large number of dashboards to make the difference measurable
      const largeDashboardArray = new Array(CREATE_MIGRATION_BODY_BATCH_SIZE * 3).fill({
        result: { id: 'dashboard', title: 'Test Dashboard', 'eai:data': '<empty/>' },
      }) as CreateDashboardMigrationDashboardsRequestBody;

      // Mock the API to simulate network delay
      const networkDelay = 10; // 10ms per request
      mockAddDashboardsToDashboardMigration.mockImplementation(
        () => new Promise((resolve) => setTimeout(resolve, networkDelay))
      );

      // Measure event loop blocking time for the current Promise.all implementation
      const startTime = process.hrtime.bigint();

      await service.addDashboardsToMigration('mig-1', largeDashboardArray);

      const endTime = process.hrtime.bigint();
      const totalTime = Number(endTime - startTime) / 1_000_000; // Convert to milliseconds

      // The current implementation should complete in roughly the time of the longest batch
      // Since we have 3 batches of 50 each, and each batch takes 10ms, total should be ~10ms
      // (all batches run in parallel)
      expect(totalTime).toBeLessThan(50); // Should be much less than 30ms (3 * 10ms)
      expect(mockAddDashboardsToDashboardMigration).toHaveBeenCalledTimes(3);
    });

    it('should simulate sequential await performance for comparison', async () => {
      ...
      // Simulate the old sequential implementation
      const simulateSequentialImplementation = async (
        migrationId: string,
      ...
      await simulateSequentialImplementation('mig-1', largeDashboardArray);

      const endTime = process.hrtime.bigint();
      const totalTime = Number(endTime - startTime) / 1_000_000; // Convert to milliseconds

      // The sequential implementation should take roughly 30ms (3 batches * 10ms each)
      expect(totalTime).toBeGreaterThan(25); // Should be close to 30ms
      expect(mockAddDashboardsToDashboardMigration).toHaveBeenCalledTimes(3);
    });

    it('should demonstrate that Promise.all reduces total execution time', async () => {
      const largeDashboardArray = new Array(CREATE_MIGRATION_BODY_BATCH_SIZE * 4).fill({
        result: { id: 'dashboard', title: 'Test Dashboard', 'eai:data': '<empty/>' },
      }) as CreateDashboardMigrationDashboardsRequestBody;

      const networkDelay = 20; // 20ms per request
      mockAddDashboardsToDashboardMigration.mockImplementation(
        () => new Promise((resolve) => setTimeout(resolve, networkDelay))
      );

      // Test current Promise.all implementation
      const startParallel = process.hrtime.bigint();
      await service.addDashboardsToMigration('mig-1', largeDashboardArray);
      const endParallel = process.hrtime.bigint();
      const parallelTime = Number(endParallel - startParallel) / 1_000_000;

      // Reset mocks
      mockAddDashboardsToDashboardMigration.mockClear();
      mockAddDashboardsToDashboardMigration.mockImplementation(
        () => new Promise((resolve) => setTimeout(resolve, networkDelay))
      );
      ...
      const startSequential = process.hrtime.bigint();
      await simulateSequential('mig-1', largeDashboardArray);
      const endSequential = process.hrtime.bigint();
      const sequentialTime = Number(endSequential - startSequential) / 1_000_000;

      // Parallel should be significantly faster than sequential
      expect(parallelTime).toBeLessThan(sequentialTime);

      // Parallel should be close to the time of a single batch (5ms)
      expect(parallelTime).toBeLessThan(25);

      // Sequential should be close to the sum of all batches (4 * 20ms = 80ms)
      expect(sequentialTime).toBeGreaterThan(75);
