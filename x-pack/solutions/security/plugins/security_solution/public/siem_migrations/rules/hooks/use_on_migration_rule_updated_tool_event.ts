 ...
 * update_translated_rule tool completes successfully in the active conversation.
 */
export const useOnMigrationRuleUpdatedToolEvent = (
  callback: (data: SiemMigrationRuleUpdatedToolEventData) => void
): void => {
  ...
  useEffect(() => {
    ...
    const subscription = agentBuilder.events.ui.activeConversation$
      ...
      .subscribe((event) => {
        if (
          isToolUiEvent<
            typeof SIEM_MIGRATION_RULE_UPDATED_TOOL_EVENT,
            SiemMigrationRuleUpdatedToolEventData
          >(event, SIEM_MIGRATION_RULE_UPDATED_TOOL_EVENT)
        ) {
