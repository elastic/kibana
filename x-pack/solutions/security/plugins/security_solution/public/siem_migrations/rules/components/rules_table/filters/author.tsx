...
import { Author } from '../../rules_table_columns';

const AUTHOR_FILTER_POPOVER_WIDTH = 200;
...
export const AuthorFilterButton: React.FC<AuthorFilterButtonProps> = React.memo(
  ({ author, onAuthorChanged }) => {
    ...
    const renderOption = useCallback<
      NonNullable<EuiSelectableProps<{ author: AuthorFilter }>['renderOption']>
    >((option) => {
      console.log(`[TEST] option: ${JSON.stringify(option)}`);
      return <Author isPrebuiltRule={option.author === AuthorFilter.ELASTIC} />;
    }, []);
