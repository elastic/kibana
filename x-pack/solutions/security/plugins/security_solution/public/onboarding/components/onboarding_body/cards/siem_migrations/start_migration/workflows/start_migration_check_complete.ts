...
const COMPLETE_BADGE_TEXT = (migrationsCount: number) =>
  i18n.translate(
    'xpack.securitySolution.onboarding.siemMigrations.workflows.startMigration.completeBadge',
    {
      defaultMessage:
...
> = async ({ siemMigrations }) => {
  const missingCapabilities = siemMigrations.workflows
  ...
  if (siemMigrations.workflows.isAvailable()) {
    const migrationsStats = await siemMigrations.workflows.getMigrationsStats();
