...
export const getResolvedEntities = (
  ...
  let rows = buildResolvedEntities();
  if (query) {
  ...
  if (!isPageFiltersEmpty(pageFilters)) {
    rows = rows.filter((row) => {
      if (row.isUnresolved) {
        const record = RAW_RECORDS.find((entry) => entry.id === row.id);
        return record ? recordMatchesPageFilters(record, pageFilters) : false;
      }
      const identity = IDENTITY_BY_ID[row.id];
      return identity ? identityMatchesPageFilters(identity, pageFilters) : false;
    });
  }
...
export const getRawRecords = (
  ...
  let rows = RAW_RECORDS.map(rawRecordRowFromRecord);
  if (query) {
    const visible = new Set(
      filterHitsByEsQuery(buildAllEntityStoreHits(), query)
