...
/**
 * v.6 metrics v.4 — 30-day sparklines. Most keep a clear direction; Risk movers
 * and New anomalies use a spike-then-drop shape (unclear trend).
 */
const V4_CARD_TRENDS: Record<SignalCardId, number[]> = {
...
const V4_CARD_DELTAS: Partial<Record<SignalCardId, number>> = {
  untriagedHighRisk: 3,
  newToCritical: 2,
  riskMovers: 4,
  newAndAlerting: 1,
...
const V4_CARD_TITLES: Partial<Record<SignalCardId, string>> = {
  newToCritical: 'New to critical',
};

/** Subtitle / description overrides. */
const V4_CARD_DESCRIPTIONS: Partial<Record<SignalCardId, string>> = {
  untriagedHighRisk: 'High or critical risk with uncased alerts',
  ...
  hiddenRisk: 'Low or moderate risk hiding a high-risk record',
};

const displayTitleFor = (card: SignalCardData): string =>
  V4_CARD_TITLES[card.id] ?? card.title;

const displayDescriptionFor = (card: SignalCardData): string =>
  V4_CARD_DESCRIPTIONS[card.id] ?? card.description;
...
const SignalMetricCard: React.FC<SignalMetricCardProps> = ({
  ...
  const delta = V4_CARD_DELTAS[card.id] ?? card.delta;
  ...
  const trendKeyframes = V4_CARD_TRENDS[card.id] ?? card.trend;
