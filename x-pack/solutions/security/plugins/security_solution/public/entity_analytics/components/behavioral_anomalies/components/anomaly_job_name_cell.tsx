...
import { useAnomalySingleMetricViewerUrl } from '../hooks/use_anomaly_single_metric_viewer_url';
import type { BehavioralAnomalyTableRow } from '../types';
import { BEHAVIORAL_ANOMALIES_TABLE_JOB_NAME_LINK_TEST_ID } from '../test_ids';
...
interface AnomalyJobNameCellProps {
  row: BehavioralAnomalyTableRow;
}

/**
 * Renders the ML job name as a link that opens the row's anomaly in the
 * ML Single Metric Viewer in a new tab. Falls back to a plain label when the
 * ML plugin is unavailable.
 */
export const AnomalyJobNameCell: React.FC<AnomalyJobNameCellProps> = ({ row }) => {
    ...
    <EuiToolTip content={row.jobDisplayName} anchorProps={{ css: truncatedAnchorCss }}>
      <EuiText size="xs" component="span">
        <EuiLink
          ...
          target="_blank"
          external={false}
          data-test-subj={BEHAVIORAL_ANOMALIES_TABLE_JOB_NAME_LINK_TEST_ID}
        >
          {row.jobDisplayName}
        </EuiLink>
      </EuiText>
