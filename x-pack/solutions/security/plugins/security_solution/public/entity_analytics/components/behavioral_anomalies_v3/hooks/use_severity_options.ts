...
/**
 * BA-v.3 local copy of `useSeverityOptions` from the ML Anomaly Explorer
 * (x-pack/platform/plugins/shared/ml/public/application/explorer/hooks/use_severity_options.ts).
 *
 * Kept here so the v3 tab is self-contained and can be deleted independently.
 */

export interface SeverityOptionV3 {
  val: number;
  display: string;
  rangeDisplay: string;
  color: string;
...
const lowLabel = i18n.translate(
  'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severity.lowLabel',
  { defaultMessage: 'low' }
);
const warningLabel = i18n.translate(
  'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severity.warningLabel',
  { defaultMessage: 'warning' }
);
const minorLabel = i18n.translate(
  'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severity.minorLabel',
  { defaultMessage: 'minor' }
);
const majorLabel = i18n.translate(
  'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severity.majorLabel',
  { defaultMessage: 'major' }
);
const criticalLabel = i18n.translate(
  'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severity.criticalLabel',
  { defaultMessage: 'critical' }
);

export const getSeverityRangeDisplayV3 = (val: number): string => {
  switch (val) {
...
export const useSeverityOptionsV3 = (): SeverityOptionV3[] => {
    ...
    () => [
      {
        ...
        display: lowLabel,
        rangeDisplay: getSeverityRangeDisplayV3(ML_ANOMALY_THRESHOLD.LOW),
      ...
      },
      {
        ...
        rangeDisplay: getSeverityRangeDisplayV3(ML_ANOMALY_THRESHOLD.CRITICAL),
