...
import { ANOMALIES_TABLE_ROW_ACTIONS_ARIA_LABEL } from '../translations';
import {
...
export const AnomalyRowActionsMenu: React.FC<AnomalyRowActionsMenuProps> = ({ row }) => {
  const { euiTheme } = useEuiTheme();
  const [isOpen, setIsOpen] = useState(false);
  const closePopover = useCallback(() => setIsOpen(false), []);
  const togglePopover = useCallback(() => setIsOpen((value) => !value), []);

  const { actions, addToChat } = useBehavioralAnomalyRowActions({ row, closePopover });
  ...
  const panels = useMemo<EuiContextMenuPanelDescriptor[]>(
    ...
    [actions, addToChat, euiTheme.size.m, euiTheme.size.s]
  );

  const button = (
    <EuiButtonIcon
      iconType="boxesVertical"
      aria-label={ANOMALIES_TABLE_ROW_ACTIONS_ARIA_LABEL}
      onClick={togglePopover}
      color={isOpen ? 'primary' : 'text'}
      data-test-subj={BEHAVIORAL_ANOMALIES_TABLE_ROW_ACTIONS_BUTTON_TEST_ID}
    />
  );
  ...
  return (
    ...
    >
      <EuiContextMenu
        ...
        panels={panels}
        size="s"
        data-test-subj={BEHAVIORAL_ANOMALIES_TABLE_ROW_ACTIONS_MENU_TEST_ID}
      />
    </EuiPopover>
  );
};
