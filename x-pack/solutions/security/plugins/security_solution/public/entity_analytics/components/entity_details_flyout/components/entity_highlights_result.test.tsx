...
describe('EntityHighlightsResult', () => {
  ...
  it('falls back to the raw username when no profile UID is available', () => {
    const generatedAt = new Date('2024-01-15T10:30:00Z').getTime();
    const generatedBy = '2036929221';
    ...
    expect(screen.getByText(generatedBy)).toBeInTheDocument();
    expect(mockUseBulkGetUserProfiles).toHaveBeenCalledWith({ uids: new Set() });
  });

  it('resolves and displays the user display name when a profile UID is available', () => {
    const generatedAt = new Date('2024-01-15T10:30:00Z').getTime();
    const mockUser = {
      username: '2036929221',
    ...
    render(
      <EntityHighlightsResult
        ...
        showAnonymizedValues={false}
        generatedAt={generatedAt}
        generatedBy={mockUser.username}
        authorProfileUid={mockUser.uid}
        onRefresh={mockOnRefresh}
      />,
      { wrapper: TestProviders }
    );
    ...
    expect(mockUseBulkGetUserProfiles).toHaveBeenCalledWith({ uids: new Set([mockUser.uid]) });
  });
