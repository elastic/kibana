...
/**
 * v.6 metrics v.5 — 30-day sparklines. Most keep a clear direction; Risk movers
 * and New anomalies use a spike-then-drop shape (unclear trend).
 */
const V5_CARD_TRENDS: Record<SignalCardId, number[]> = {
...
const V5_CARD_DELTAS: Partial<Record<SignalCardId, number>> = {
  untriagedHighRisk: 3,
  newToCritical: 2,
  riskMovers: 4,
  newAndAlerting: 1,
...
const V5_CARD_TITLES: Partial<Record<SignalCardId, string>> = {
...
const displayTitleFor = (card: SignalCardData): string =>
  V5_CARD_TITLES[card.id] ?? card.title;
...
const SignalMetricCard: React.FC<SignalMetricCardProps> = ({
  ...
  const delta = V5_CARD_DELTAS[card.id] ?? card.delta;
  ...
  const trendKeyframes = V5_CARD_TRENDS[card.id] ?? card.trend;
