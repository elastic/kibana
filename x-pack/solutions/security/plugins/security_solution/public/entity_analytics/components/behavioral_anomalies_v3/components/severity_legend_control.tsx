...
import { getSeverityRangeDisplayV3 } from '../hooks/use_severity_options';
import { BEHAVIORAL_ANOMALIES_V3_SEVERITY_LEGEND_TEST_ID } from '../test_ids';

/**
 * BA-v.3 local copy of ML Anomaly Explorer's `SeverityLegendControl`
...
export interface SeverityLegendControlV3Props {
  allSeverityOptions: SeverityOptionV3[];
  selectedSeverities: SeverityOptionV3[];
  onChange: (selectedSeverities: SeverityOptionV3[]) => void;
  dataTestSubj?: string;
}

export const SeverityLegendControlV3: FC<SeverityLegendControlV3Props> = ({
  allSeverityOptions,
  selectedSeverities,
  onChange,
  dataTestSubj = BEHAVIORAL_ANOMALIES_V3_SEVERITY_LEGEND_TEST_ID,
}) => {
  ...
  const handleSeverityClick = useCallback(
    (clickedSeverity: SeverityOptionV3) => {
      ...
      let newSelectedSeverities: SeverityOptionV3[];
          ...
          <EuiFlexItem key={severity.val} grow={false}>
            <EuiButtonEmpty
              ...
              aria-label={i18n.translate(
                'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severityLegendControl.anomalyScoreButtonLabel',
                {
                  defaultMessage: 'Anomaly score {range}',
                  values: { range: getSeverityRangeDisplayV3(severity.val) },
                }
              )}
              ...
              <EuiFlexGroup gutterSize="xs" alignItems="center" responsive={false}>
                ...
                <EuiFlexItem grow={false}>
                  <EuiText size="s" css={severityTextCss(isSelected)}>
                    {getSeverityRangeDisplayV3(severity.val)}
                  </EuiText>
                </EuiFlexItem>
              </EuiFlexGroup>
            </EuiButtonEmpty>
  ...
  return (
    <EuiFormControlLayout
      ...
      prepend={i18n.translate(
        'xpack.securitySolution.entityAnalytics.behavioralAnomaliesV3.severityLegendControl.anomalyScoreLabel',
