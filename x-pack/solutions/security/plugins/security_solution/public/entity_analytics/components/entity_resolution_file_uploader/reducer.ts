...
import type { ValidatedFile, ResolutionCsvUploadResponse } from './types';
...
export interface ResultStepState {
  ...
  uploadResponse?: ResolutionCsvUploadResponse;
  uploadError?: string;
...
export type ReducerAction =
  ...
  | { type: 'resetState' }
  | { type: 'fileValidated'; payload: { validatedFile: ValidatedFile } }
  | { type: 'fileError'; payload: { message: string } }
  | { type: 'uploadingFile' }
  | {
      type: 'fileUploaded';
      payload: { response?: ResolutionCsvUploadResponse; errorMessage?: string };
    };
  ...
  switch (action.type) {
    ...
    case 'loadingFile':
      if (isFilePickerStep(state)) {
        return { ...state, isLoading: true, fileName: action.payload.fileName };
      }
      break;
