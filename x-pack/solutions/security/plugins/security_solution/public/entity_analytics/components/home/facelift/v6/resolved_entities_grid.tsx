  ...
  const styles = useMemo(
    () => ({
      ...
      row: css`
        display: flex;
        flex-wrap: nowrap;
        /* Stretch so NestedView can fill the row and keep the trunk seamless. */
        align-items: stretch;
      ...
      `,
      /**
       * Streams NestedView draws the connector; raw names sit on the branch
       * under the parent resolved-entity name (no repeated root label).
       * Overflow must stay visible so border/::before lines are not clipped.
       */
      nameCell: css`
        min-inline-size: 0;
        overflow: visible;
            ...
            {columnIds.map((columnId, index) => {
              ...
              return (
                <div
                  css={[
                    styles.cell,
                    columnId === 'name' && styles.nameColumn,
                    numericColumnIds.has(columnId) && styles.numericCell,
                  ]}
                  key={columnId}
