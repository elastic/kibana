...
import { EMPTY_PAGE_FILTERS, filterIdentities, filterRawRecords } from './data';
...
const useHeaderCellWidths = (
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const firstRowCells = container.querySelector('.euiDataGridRow > div:first-child');
    ...
    const observed = container.querySelector('.euiDataGridRow > div:first-child');
    observed
...
export const ResolvedEntitiesGrid: React.FC<ResolvedEntitiesGridProps> = ({
  ...
  const rows = useMemo(() => {
    ...
    // The row builders and `filterIdentities` / `filterRawRecords` both read
    // the active preset window — for the period columns and for card
    // membership — so the rows change with it even though the range isn't
    // referenced directly here, which is why the linter thinks it is unused.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isResolvedView, query, pageFilters, activeFilter, sortingColumns, timeRange]);
  ...
  const leadingControlColumns = useMemo<EuiDataGridControlColumn[]>(() => {
    ...
    const actions: EuiDataGridControlColumn = {
      ...
      rowCellRender: ({ rowIndex }) => {
        const row = rows[rowIndex];
        if (!row) return null;
        return <EntityRowActions row={row} canUseTimeline={canUseTimeline} />;
      },
    };
