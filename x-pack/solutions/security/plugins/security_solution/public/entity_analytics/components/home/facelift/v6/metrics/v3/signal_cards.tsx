...
/** Decorative area sparkline behind the value — fill + 2px peak line. */
const Sparkline: React.FC<{ values: number[]; fill: string; stroke: string }> = ({
  values,
  fill,
  stroke,
}) => {
    ...
    <Chart size={['100%', '100%']}>
      ...
      <AreaSeries
        ...
        areaSeriesStyle={{
          ...
          line: { strokeWidth: 2, visible: true, stroke },
          point: { visible: 'never' },
        }}
      />
    </Chart>
...
const SignalMetricCard: React.FC<SignalMetricCardProps> = ({
  ...
  // backgrounds/base/{danger|success} fill + borders/base/{danger|success} line.
  const sparklineFill =
    showDelta && delta! > 0
    ...
    >
      {showTrend ? (
        <div
          ...
          `}
        >
          <Sparkline values={trendKeyframes!} fill={sparklineFill} stroke={sparklineStroke} />
        </div>
      ) : null}
