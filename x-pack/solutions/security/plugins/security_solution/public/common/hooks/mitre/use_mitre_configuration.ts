...
export const useMitreConfiguration = (types?: MitreEntityType[]): MitreConfiguration => {
  ...
  const managedQuery = useFetchMitreEntitiesQuery(
    { types },
    {
      enabled: isEnabled,
      onError: (error) => {
  ...
  const legacyQuery = useFetchLegacyMitreQuery(types, {
    enabled: !isEnabled,
    onError: (error) => {
      addError(error, { title: i18n.MITRE_CONFIGURATION_FETCH_ERROR });
    },
