...
import { CellActions as DocumentDetailsCellActions } from '../../../../../flyout/document_details/shared/components/cell_actions';
import { CellActions as AttackDetailsCellActions } from '../../../../../flyout/attack_details/components/cell_actions';
...
import { EntityAlertsDetails } from './entity_alerts_details';
...
import { USER_PREVIEW_BANNER } from '../../../main/components/user_entity_overview';
import type { NarrowDateRange } from '../../../../../common/components/ml/types';
import { MisconfigurationsInsight } from '../../../main/components/misconfiguration_insight';
import { AlertCountInsight } from '../../../main/components/alert_count_insight';
...
export interface UserDetailsProps {
   ...
   * Only effective when useLegacyExpandableFlyout is true.
   */
  isAttackDetails?: boolean;
  /**
   * Renderer for cell actions when the legacy expandable flyout links are disabled.
   */
  renderCellActions?: CellActionRenderer;
  /**
   * When true, render legacy expandable-flyout features: header preview link, PreviewLink in
   * related-hosts columns, and CSP left-panel navigation from insight tiles. Defaults to false (v2 mode).
   */
  useLegacyExpandableFlyout?: boolean;
}

/**
 * User details panel used in both Flyout v2 (document tools flyout) and the legacy expandable
 * flyout left panel. Behavior is identical across both surfaces except for legacy-only features
 * gated by `useLegacyExpandableFlyout`: header preview link, PreviewLink-wrapped related-hosts
 * columns, and CSP left-panel routing from insight tiles.
 */
export const UserDetailsView: React.FC<UserDetailsProps> = ({
  userName,
  entityId,
  timestamp,
  scopeId,
  ...
  const onShowAlert = useCallback(
    ...
    [defaultFlyoutProperties, history, renderCellActions, services, store]
  );
  ...
  const onShowAlertCountDetailsV2 = useCallback(() => {
    services.overlays.openSystemFlyout(
      flyoutProviders({
        services,
        store,
        ...
        children: (
          <EntityAlertsDetails
            title={i18n.translate('xpack.securitySolution.flyout.entities.userAlertsTitle', {
      ...
      { ...defaultToolsFlyoutProperties, size: 's', session: 'inherit' }
    );
  }, [services, store, history, userName, entityId, onShowAlert]);
  ...
  const userLink = useMemo(
    () =>
      useLegacyExpandableFlyout
        ...
        : undefined,
    [openUserPreview, useLegacyExpandableFlyout]
  );

  const onShowAlertCountDetails = useLegacyExpandableFlyout
    ? () =>
        openDetailsPanel({
          tab: EntityDetailsLeftPanelTab.CSP_INSIGHTS,
          subTab: CspInsightLeftPanelSubTab.ALERTS,
        })
    : onShowAlertCountDetailsV2;

  const onShowMisconfigurationsDetails = useLegacyExpandableFlyout
    ? () =>
        openDetailsPanel({
          tab: EntityDetailsLeftPanelTab.CSP_INSIGHTS,
