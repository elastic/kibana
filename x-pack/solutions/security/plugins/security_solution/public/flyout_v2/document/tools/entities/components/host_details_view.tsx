...
import { VulnerabilitiesInsight } from '../../../main/components/vulnerabilities_insight';
...
export interface HostDetailsProps {
  ...
  /**
   * When provided (e.g. from EntitiesDetailsView), use this for entity/risk so the left panel
   ...
   * Set for attack flyout entity panels; omit in document details flyout. Only effective when
   * useLegacyExpandableFlyout is true.
   */
   ...
   * related-users columns, and CSP left-panel navigation from insight tiles. Defaults to false (v2 mode).
   */
  useLegacyExpandableFlyout?: boolean;
}

/**
 * Host details panel used in both Flyout v2 (document tools flyout) and the legacy expandable
 * flyout left panel. Behavior is identical across both surfaces except for legacy-only features
 * gated by `useLegacyExpandableFlyout`: header preview link, PreviewLink-wrapped related-users
 * columns, and CSP left-panel routing from insight tiles.
 */
export const HostDetailsView: React.FC<HostDetailsProps> = ({
  hostName,
  entityId,
  timestamp,
  scopeId,
  ...
  const onShowAlertCountDetailsV2 = useCallback(() => {
    services.overlays.openSystemFlyout(
      flyoutProviders({
        ...
        children: (
          <EntityAlertsDetails
            title={i18n.translate('xpack.securitySolution.flyout.entities.hostAlertsTitle', {
  ...
  }, [services, store, history, hostName, entityId, onShowAlert]);
  ...
  const hostRiskData =
    riskScoreStateFromApi.data && riskScoreStateFromApi.data.length > 0
      ...
      : undefined;
  // useObservedHost always queries security default indices; duplicating useHostDetails against
  // sourcerer patterns yields sparse hostDetails that still suppress real data.
  ...
  const hostLink = useMemo(
    () =>
      useLegacyExpandableFlyout
        ...
        : undefined,
    [openHostPreview, useLegacyExpandableFlyout]
  );
  ...
  const onShowVulnerabilitiesDetails = useLegacyExpandableFlyout
    ? () =>
        openDetailsPanel({
          tab: EntityDetailsLeftPanelTab.CSP_INSIGHTS,
