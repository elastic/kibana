...
const SUPPRESSED_ALERTS_COUNT_TECHNICAL_PREVIEW = i18n.translate(
  'xpack.securitySolution.flyout.document.overview.insights.suppressedAlertsCountTechnicalPreview',
  {
...
export const SuppressedAlerts: React.VFC<SuppressedAlertsProps> = ({
  ...
  const text = useMemo(
    () => (
      <FormattedMessage
        id="xpack.securitySolution.flyout.document.insights.correlations.suppressedAlertsLabel"
      ...
      {ruleType && isSuppressionRuleInGA(ruleType) ? null : (
        <EuiFlexItem grow={false}>
          <EuiBetaBadge
            ...
            tooltipContent={
              <FormattedMessage
                id="xpack.securitySolution.flyout.document.insights.entities.suppressedAlertTechnicalPreviewTooltip"
