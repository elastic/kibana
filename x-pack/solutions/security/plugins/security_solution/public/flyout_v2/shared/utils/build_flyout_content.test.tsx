...
describe('buildFlyoutContent', () => {
  ...
  it('should return a User element for a user.name field', () => {
    const result = buildFlyoutContent(USER_NAME_FIELD_NAME, 'my-user');

    expect(result).not.toBeNull();

    const { getByTestId } = render(result!);
    expect(getByTestId('mockUser')).toHaveTextContent('my-user');
  });

  it('should pass hit to User element when provided', () => {
    const mockHit = {
      id: 'test-doc-id',
      raw: { _id: 'test-doc-id', _index: 'test-index' },
    ...
    const { getByTestId } = render(result!);
    expect(getByTestId('mockUser')).toHaveAttribute('data-has-hit', 'true');
  });
