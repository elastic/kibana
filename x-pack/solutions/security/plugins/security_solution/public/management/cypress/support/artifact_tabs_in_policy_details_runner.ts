...
export const getArtifactTabsTests = (artifactsData: ArtifactsFixtureType[]) => () => {
  ...
  for (const testData of artifactsData) {
    describe(`${testData.title} tab`, () => {
      ...
      context(`Given there are no ${testData.title} entries`, () => {
        ...
        it(`[ALL] User can add ${testData.title} artifact`, () => {
          ...
          cy.get('[data-test-subj$="-perPolicy"]').click();
          ...
          clickArtifactTab(testData.nextTabId);
