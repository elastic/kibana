...
describe('ScriptsLibrary', () => {
  ...
  describe('Page elements', () => {
    ...
    it('should show the upload button when user has `canWriteScriptsLibrary` privileges', () => {
      useUserPrivilegesMock.mockReturnValue({
        endpointPrivileges: {
      ...
      expect(uploadButton.textContent).toEqual('Upload script');
    });
  ...
  describe('Interactions (with data)', () => {
    ...
    it('should not show edit flyout when loaded via URL without `canWriteScriptsLibrary` privilege', async () => {
      ...
      act(() => {
        history.push(`${SCRIPTS_LIBRARY_PATH}?selectedScriptId=${scriptsList[0].id}&show=edit`);
      });
      render();
      const { queryByTestId } = renderResult;

      const flyout = queryByTestId('test-endpointScriptFlyout-edit');
