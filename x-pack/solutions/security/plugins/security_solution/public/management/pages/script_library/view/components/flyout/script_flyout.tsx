...
import {
  type ScriptLibraryUrlParams,
  useScriptLibraryUrlParams,
} from '../script_library_url_params';
...
export interface EndpointScriptFlyoutProps {
  ...
  scriptItem?: EndpointScript;
  show: Exclude<Required<ScriptLibraryUrlParams>['show'], 'delete'>;
...
export const EndpointScriptFlyout = memo<EndpointScriptFlyoutProps>(
  ({
    ...
    const { selectedScriptId } = useScriptLibraryUrlParams();
    ...
    const useSubmitScript = useWithScriptSubmit(
      show as Extract<Required<ScriptLibraryUrlParams>['show'], 'edit' | 'create'>
    );
