...
export interface ShowEventButtonProps {
  ...
  eventId: string;
  index: string;
  /** @deprecated Events use the flyout hook directly; kept for alert attachments passed via cases */
  onShowEventDetails?: (eventId: string, index: string) => void;
}

const ShowEventButtonComponent = ({
  id,
  eventId,
  index,
  onShowEventDetails,
}: ShowEventButtonProps) => {
  ...
  const onClick = useCallback(() => {
    ...
    } else if (onShowEventDetails) {
      onShowEventDetails(eventId, index);
    } else {
      navigateToCaseView({ detailName, tabId: CASE_VIEW_PAGE_TABS.EVENTS });
    }
  }, [eventId, index, openFlyout, telemetry, detailName, navigateToCaseView, onShowEventDetails]);
