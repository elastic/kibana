...
export const BULK_DISABLE_ATTACK_DISCOVERY_SCHEDULES_MUTATION_KEY = [
  'POST',
  ATTACK_DISCOVERY_SCHEDULES_BULK_DISABLE,
];

interface BulkDisableAttackDiscoverySchedulesParams {
  ids: string[];
}

export const useBulkDisableAttackDiscoverySchedules = () => {
  const {
    services: { telemetry },
  } = useKibana();
  ...
  return useMutation<
    ...
    Error,
    BulkDisableAttackDiscoverySchedulesParams
  >(({ ids }) => bulkDisableAttackDiscoverySchedules({ ids }), {
    mutationKey: BULK_DISABLE_ATTACK_DISCOVERY_SCHEDULES_MUTATION_KEY,
    onSuccess: ({ ids }) => {
