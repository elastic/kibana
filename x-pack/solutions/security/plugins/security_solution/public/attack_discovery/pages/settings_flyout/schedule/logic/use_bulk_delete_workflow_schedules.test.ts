...
describe('useBulkDeleteWorkflowSchedules', () => {
  ...
  beforeEach(() => {
    ...
    deleteWorkflowScheduleMock.mockResolvedValue(
      {} as unknown as Awaited<ReturnType<typeof deleteWorkflowSchedule>>
    );
  ...
  it('fans out to the internal per-id delete endpoint for every id', async () => {
    const result = await renderMutation(() => useBulkDeleteWorkflowSchedules());

    await act(async () => {
      await result.mutateAsync({ ids });
      expect(deleteWorkflowScheduleMock).toHaveBeenCalledTimes(2);
      expect(deleteWorkflowScheduleMock).toHaveBeenCalledWith({ id: 'test-0' });
      expect(deleteWorkflowScheduleMock).toHaveBeenCalledWith({ id: 'test-1' });
    });
  });
  ...
  it('reports only the succeeded ids when some per-id calls fail (partial success)', async () => {
    deleteWorkflowScheduleMock.mockImplementation(({ id }) =>
      id === 'test-1'
        ? Promise.reject(new Error('no'))
        : Promise.resolve({} as unknown as Awaited<ReturnType<typeof deleteWorkflowSchedule>>)
    );
