...
const OPEN_SCHEDULE_DETAILS_LABEL = i18n.translate(
  'xpack.securitySolution.attackDetailsFlyout.header.openScheduleDetailsLabel',
  {
...
export const HeaderTitle = memo(() => {
  ...
  const { attackId, attack, isPreviewMode } = useAttackDetailsContext();
  ...
  const scheduleIdForDetails = useMemo(() => {
  ...
  const showScheduleTitleLink = scheduleIdForDetails != null && !isPreviewMode;

  const openScheduleDetails = useCallback(() => {
    if (scheduleIdForDetails) {
      setScheduleDetailsId(scheduleIdForDetails);
    }
  }, [scheduleIdForDetails]);

  const onCloseScheduleDetails = useCallback(() => setScheduleDetailsId(undefined), []);

  const onScheduleTitleClick = useCallback(
