...
export const stixAdapter: FetchAdapter = {
  ...
  async run(source, context: AdapterRunContext) {
    ...
    if (response.status >= 400) {
      throw new Error(
        `STIX fetch ${redactUrl(url)} failed: HTTP ${response.status} ${response.statusText}`
      );
    }

    const bundle = safeParseJson(response.body);
    if (bundle == null) {
      throw new Error(`STIX response at ${redactUrl(url)} was not valid JSON`);
    }
    ...
    for (const { object } of sdos) {
      ...
      if (object.type === 'indicator') {
        ...
        if (iocs.length > 0) {
          // Parseable indicator pattern → structured at ingest; enrichment skips re-extraction.
          lineage = {
