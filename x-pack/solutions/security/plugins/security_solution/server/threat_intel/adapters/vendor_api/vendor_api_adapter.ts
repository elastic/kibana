...
/** Vendor API adapter (rss delegate or json_list fetch). */
export const vendorApiAdapter: FetchAdapter = {
  ...
  async run(source, context: AdapterRunContext) {
    ...
    if (handler.kind === 'rss') {
      // Reuse the RSS adapter and rewrite source.type / adapter_id to vendor_api.
...
const runJsonList = async (
  ...
  if (parsed == null) {
    throw new Error(`Vendor API response at ${redactUrl(url)} was not valid JSON`);
  }
