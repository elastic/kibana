...
export const textIndicatorListAdapter: FetchAdapter = {
  ...
  async run(source: SourceHit, context: AdapterRunContext): Promise<NormalizedReport[]> {
    ...
    if (response.status >= 400) {
      throw new Error(
        `text_indicator_list fetch ${url} failed: HTTP ${response.status} ${response.statusText}`
      );
    }

    const blocks = parseIndicatorList(response.body);
    if (blocks.length === 0) {
      log.warn(`text_indicator_list at ${url} produced 0 blocks for source ${source._id}`);
      return [];
    }
    ...
    if (dedupedIocs.size === 0) {
      log.warn(
        `text_indicator_list at ${url} had blocks but 0 parseable IOCs for source ${source._id}`
      );
      return [];
    }
    ...
    log.info(
      `text_indicator_list: ${url} → ${dedupedIocs.size} deduped IOCs across ${reports.length} chunk(s) for source ${source._id}`
    );
