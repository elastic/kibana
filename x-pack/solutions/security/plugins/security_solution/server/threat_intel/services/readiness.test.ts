...
): ReadinessDeps & { esClient: ReturnType<typeof elasticsearchServiceMock.createElasticsearchClient> } => {
...
describe('getThreatIntelReadiness', () => {
  ...
  it('returns degraded with embedding_endpoint_unavailable when inference.get fails', async () => {
    const deps = buildDefaultDeps();
    deps.esClient.inference.get.mockImplementation(async ({ inference_id: id }) => {
