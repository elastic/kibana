...
/**
 * Resolve the endpoint an operator picked for this feature in Stack Management >
 * Model Settings, falling back to the feature's `recommendedEndpoints`. Returns
 * undefined (rather than throwing) so the caller can drop to the genAi default.
 */
const resolveFeatureEndpointId = async ({
  ...
  if (!searchInferenceEndpoints) return undefined;
...
export const resolveScopedModel = async ({
  ...
  const featureEndpointId = await resolveFeatureEndpointId({
    searchInferenceEndpoints,
    featureId,
    request,
    logger,
  });
  if (featureEndpointId) {
