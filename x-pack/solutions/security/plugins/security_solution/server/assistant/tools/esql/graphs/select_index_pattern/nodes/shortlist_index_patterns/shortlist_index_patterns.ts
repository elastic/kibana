...
const getStructuredOutputSchema = (state: typeof SelectIndexPatternAnnotation.State) => {
  return z
    .object({
      shortlistedIndexPatterns: z.array(z.string()).describe('Shortlisted index patterns').superRefine((value, ctx) => {
        ...
        const hallucinated = value.filter((indexPattern) => !state.indexPatterns.includes(indexPattern));
        if (hallucinated.length > 0) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: `The following index patterns may have typos or are not valid: ${hallucinated.join(', ')}. These are the valid options: ${state.indexPatterns.join(', ')}`,
          });
        }
      }),
    })
...
export const getShortlistIndexPatterns = ({
  ...
  return async (state: typeof SelectIndexPatternAnnotation.State) => {
    ...
    const systemMessage = new SystemMessage({
      content: "You are a security analyst who is an expert in Elasticsearch and particularly writing Elastic Search queries. " +
        ...
        "Respond only in the following schema.\n\n"+JSON.stringify(zodToJsonSchema(getStructuredOutputSchema(state))),
    });
    ...
    try {
      const result = await llm
        .withStructuredOutput(getStructuredOutputSchema(state), { name: 'shortlistedIndexPatterns' })
        .withRetry({
          stopAfterAttempt: 3,
        })
