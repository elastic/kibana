...
describe("nl to esql agent", () => {
    ...
    it("calls naturalLanguageToEsql with the correct parameters", async () => {
        ...
        (naturalLanguageToEsql as unknown as jest.Mock).mockReturnValue(new Observable(subscriber => {
            const result: ChatCompletionMessageEvent = { content: "Hello, World!", "type": ChatCompletionEventType.ChatCompletionMessage, toolCalls: [] };
            subscriber.next(result);
            subscriber.complete();
        }));

        const result = await agent({
            ...sampleState, messages: sampleMessageLog
        })

        expect(naturalLanguageToEsql).toHaveBeenCalledTimes(1);
        expect(naturalLanguageToEsql).toHaveBeenCalledWith(expect.objectContaining({
