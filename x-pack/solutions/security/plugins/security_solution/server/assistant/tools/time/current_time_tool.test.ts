...
describe('CurrentTimeTool', () => {
    ...
    it('description', () => {
        expect(CURRENT_TIME_TOOL.description).toContain('Call this to get the current local time of the user')
    })
    ...
    describe('tool', () => {
        ...
        ])('gets correct time if dateFormat:tz is %s', async (timezone: string | undefined, expectedResult: string) => {
            defaultArgs.core.uiSettings.client.get = jest.fn().mockImplementation(async (key: string) => {
