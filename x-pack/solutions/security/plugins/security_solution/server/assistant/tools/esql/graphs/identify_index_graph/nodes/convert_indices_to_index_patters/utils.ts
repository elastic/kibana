 ...
 * @returns 
 */
export const getIndexPatterns = (tree: IndexNamePartNode, options?: Options): {
    indexPatterns: string[];
    remainingIndices: string[];
} => {
    // This function will traverse the tree and generate index patterns.
    const stack: { node: IndexNamePartNode; prefix: string, indexPatternAdded: boolean }[] = [{ node: tree, prefix: '', indexPatternAdded: false }];
    ...
    while (stack.length > 0) {
        ...
        const { node, prefix, indexPatternAdded: parendIndexPatternAdded } = stack.pop()!;

        if (node.children && node.children.length > 1 && node.value !== '' && (ignoreDigitParts ? !isNumber(node.value.replace(".", "").replace("-", "")) : true)) {
        ...
        if (node.children && node.children.length > 0) {
            for (const child of node.children) {
                stack.push({ node: child, prefix: `${prefix}${node.value}`, indexPatternAdded: parendIndexPatternAdded || indexPatternAdded });
            }
        } else {
