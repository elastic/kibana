...
/**
 * Converts a createLlmInstance result to a ScopedModel for use with agent-builder utilities
 */
...
export const exploreIndices = ({
  ...
  return async (state: typeof EnhancedIndexExplorerAnnotation.State) => {
    ...
    // Convert createLlmInstance result to ScopedModel for agent-builder compatibility
    const model = await convertToScopedModel(createLlmInstance, inference, request, connectorId);

    // Use the indexExplorer utility from agent-builder
