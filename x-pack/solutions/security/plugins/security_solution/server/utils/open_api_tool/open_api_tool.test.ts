...
class TestableOpenApiTool extends OpenApiTool<{}> {
    ...
    protected getInternalNode(args: { tools: Promise<StructuredToolInterface[]>; name: string; description: string; }): Promise<StructuredToolInterface> {
        throw new Error("Method not implemented.");
    }
    ...
    static async createTestableOpenApiTool() {
        const yamlOpenApiSpec = await fs.promises.readFile(path.join(__dirname, 'sample_open_api_spec.yml'), 'utf8');
        ...
        return new this({ dereferencedOas: dereferencedOas });
    }
}
