...
describe('getLeadToolAvailability', () => {
  ...
  it('returns available when all checks pass (space OK, FF enabled, Enterprise license)', async () => {
    const result = await getLeadToolAvailability({
      core: mockCore,
      request: mockRequest,
      logger: mockLogger,
  ...
  it('returns unavailable when leadGenerationEnabled FF is false', async () => {
    const result = await getLeadToolAvailability({
      core: mockCore,
      request: mockRequest,
      logger: mockLogger,
    ...
    expect(result.status).toBe('unavailable');
    expect(result.reason).toContain('Lead generation');
  });

  it('returns unavailable when license is below Enterprise', async () => {
