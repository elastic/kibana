...
describe('automaticTroubleshootingGenerateInsightTool', () => {
  ...
  describe('tool definition', () => {
    it('returns a valid builtin tool definition', () => {
      const tool = automaticTroubleshootingGenerateInsightTool();

      expect(tool).toBeDefined();
      expect(tool.type).toBe(ToolType.builtin);
      expect(tool.id).toBe(AUTOMATIC_TROUBLESHOOTING_GENERATE_INSIGHT_TOOL_ID);
    ...
    it('has correct tool id format', () => {
      expect(AUTOMATIC_TROUBLESHOOTING_GENERATE_INSIGHT_TOOL_ID).toBe(
        'security.automatic_troubleshooting.generate_insight'
      );
    });
  });
  ...
  describe('tool metadata', () => {
    ...
    it('includes elastic_defend tag', () => {
      expect(tool.tags).toContain('elastic_defend');
    });

    it('includes automatic_troubleshooting tag', () => {
      expect(tool.tags).toContain('automatic_troubleshooting');
    });
  });
});
