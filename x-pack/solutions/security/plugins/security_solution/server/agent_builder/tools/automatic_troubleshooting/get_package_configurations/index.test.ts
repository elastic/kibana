...
describe('automaticTroubleshootingGetPackageConfigurationsTool', () => {
  ...
  describe('tool definition', () => {
    it('returns a valid builtin tool definition', () => {
      const tool = automaticTroubleshootingGetPackageConfigurationsTool(
        mockEndpointAppContextService
      );

      expect(tool).toBeDefined();
      expect(tool.type).toBe(ToolType.builtin);
      expect(tool.id).toBe(AUTOMATIC_TROUBLESHOOTING_GET_PACKAGE_CONFIGURATIONS_TOOL_ID);
      expect(tool.description).toContain('Fetches Elastic Defend package configurations');
      expect(tool.tags).toEqual(['elastic_defend', 'automatic_troubleshooting']);
    });

    it('has correct tool id format', () => {
      expect(AUTOMATIC_TROUBLESHOOTING_GET_PACKAGE_CONFIGURATIONS_TOOL_ID).toBe(
        'security.automatic_troubleshooting.get_package_configurations'
      );
    });
  });

  describe('handler', () => {
    let tool: ReturnType<typeof automaticTroubleshootingGetPackageConfigurationsTool>;

    beforeEach(() => {
      tool = automaticTroubleshootingGetPackageConfigurationsTool(mockEndpointAppContextService);
    });
