...
describe('entityRelationshipHistoryTool', () => {
  ...
  const mockCreateRelationshipsClient = jest.fn(() => ({
  ...
  beforeEach(() => {
    ...
    mockCore.getStartServices.mockResolvedValue([
      mockCoreStart,
      { entityStore: { createRelationshipsClient: mockCreateRelationshipsClient } },
      {},
    ]);
  ...
  describe('handler — params forwarding', () => {
    ...
    it('uses spaceId as the relationships client namespace', async () => {
      await tool.handler(
        { entityId: ENTITY_ID },
        createToolHandlerContext(mockRequest, mockEsClient, mockLogger, { spaceId: 'my-space' })
      );

      expect(mockCreateRelationshipsClient).toHaveBeenCalledWith(expect.anything(), 'my-space');
    });

    it('passes esClient.asCurrentUser to the relationships client', async () => {
      await tool.handler({ entityId: ENTITY_ID }, ctx());

      expect(mockCreateRelationshipsClient).toHaveBeenCalledWith(
