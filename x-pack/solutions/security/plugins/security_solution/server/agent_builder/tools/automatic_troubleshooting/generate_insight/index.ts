...
import { automaticTroubleshootingToolName } from '../utils';
import { createGenerateInsightGraph } from './graph';

const TOOL_ID = 'generate_insight';
export const AUTOMATIC_TROUBLESHOOTING_GENERATE_INSIGHT_TOOL_ID =
  automaticTroubleshootingToolName(TOOL_ID);
...
export const automaticTroubleshootingGenerateInsightTool = (): BuiltinToolDefinition<
  typeof generateInsightSchema
> => {
  return {
    id: AUTOMATIC_TROUBLESHOOTING_GENERATE_INSIGHT_TOOL_ID,
...
- When a conclusion has been reached`,
    ...
    handler: async (
      ...
      try {
        ...
        logger.error(
          `Error in ${AUTOMATIC_TROUBLESHOOTING_GENERATE_INSIGHT_TOOL_ID} tool: ${error.message}`
        );
        return {
          results: [
            {
