...
export const searchEntitiesTool = (
  ...
  return {
    ...
    If the user wants a **list** or **table** of matching entities (including when they expect **zero or many** results), use security.get_entity per row and attachments.add with type "security.entity_list" — **even when only one** entity matches (see entity-analytics skill). If they name **multiple entity kinds in one question** (e.g. riskiest **hosts and users**), run one or more searches (split entityTypes or multi-value entityTypes as appropriate), get_entity for every included row, then a **single** "security.entity_list" attachment with all rows. If they want **one particular** entity, **details**, or an **entity card** (not list framing), set maxResults: 1 when appropriate and use type "security.entity_card" after security.get_entity.
