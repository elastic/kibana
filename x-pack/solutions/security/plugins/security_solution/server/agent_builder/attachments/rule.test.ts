  ...
  describe('resolve', () => {
    it('fetches the rule by rule_id and stores it as attachment content', async () => {
      ...
      expect(content).toEqual(
        expect.objectContaining({
          origin: 'static-rule-id',
          attachmentLabel: 'Resolved Rule',
          // ruleId must be set to the saved-object UUID so downstream detection-rule-edit
          // treats this as an update (PATCH) rather than a create (POST).
          ruleId: 'rule-so-id-1',
        })
      );
