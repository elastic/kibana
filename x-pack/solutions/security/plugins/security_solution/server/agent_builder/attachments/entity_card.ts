...
> => ({
  ...
  getAgentDescription: () =>
    `**Use this (not security.entity_list)** when the user wants **one** primary entity: they say **entity card**, **card**, **the** riskiest / highest-risk **host** (singular), **the** top **user**, **#1** entity, or any question whose rich UI should be the **single-entity flyout-style Canvas**. After \`security.search_entities\` with \`maxResults: 1\` (or picking one winner), call \`security.get_entity\`, then \`attachments.add\` with this type. A security entity card attachment captures one investigated entity (host, user, service, or generic) with EUID, optional risk summary, asset criticality, watchlists, resolution context, and insight summaries. In the UI it renders as a compact inline card with Canvas mirroring the entity flyout (summary, risk, resolution, insights). Summarize in prose and reference the attachment id. Create with attachments.add, type "${SecurityAgentBuilderAttachments.entityCard}", JSON matching the schema (entity_type, entity_id, plus optional fields from security.get_entity: entity_name, data_source, watchlist_names, criticality, field_rows, first_seen, last_activity, risk_score_norm, risk_level, risk_note, risk_inputs, resolution, insights).`,
});
