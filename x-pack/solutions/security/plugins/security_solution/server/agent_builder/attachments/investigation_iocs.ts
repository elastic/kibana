  ...
  return {
    ...
    format: (attachment) => {
      return {
        getRepresentation: () => ({
          type: 'text',
          value: formatIocsForAgent(attachment.data as InvestigationIocsAttachmentData),
        }),
      };
    },
    getAgentDescription: () => {
...
Present the indicators grouped by category, one row per indicator. Every value is already verified against telemetry — cite them verbatim and never widen, defang, or normalize them.`;
    },
  };
};
