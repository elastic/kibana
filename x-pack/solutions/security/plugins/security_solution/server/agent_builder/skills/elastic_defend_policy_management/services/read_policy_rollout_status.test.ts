...
const responseZero = (
  ...
  extras: Partial<
    Pick<
      CurrentRevisionResponses,
      'upstreamUnclassifiedHosts' | 'truncated' | 'responseCoverageIncomplete'
    >
  > = {}
): CurrentRevisionResponses => ({
...
describe('readPolicyRolloutStatus', () => {
  ...
  it('marks incomplete United coverage when the empty agent-ID set skips the policy-response search', async () => {
    const harness = await createEndpointUsageAccess();
    mockSearches(harness.esClient, {
      united: unitedSearchResult({
        agents: [],
        clusters: { partial: 1, successful: 1, total: 2 },
      }),
    });
    ...
    expect(result.currentRevisionResponses).toEqual(
      responseZero('united_agent_id_set_empty', {
  ...
  it('returns policy_response_index_missing with strict local flags and preserved revision coverage', async () => {
    const harness = await createEndpointUsageAccess();
    ...
    expect(responseRequest.allow_no_indices).toBe(false);
    expect(responseRequest.ignore_unavailable).toBe(false);
    expect(harness.esClient.search).toHaveBeenCalledTimes(2);
    expect(result.revisionCoverage.source).toBe('united_metadata_tuple_aggregation');
    expect(result.currentRevisionResponses).toEqual(
      responseZero('policy_response_index_missing', {
        upstreamUnclassifiedHosts: 7,
        truncated: true,
        responseCoverageIncomplete: true,
      })
    );
  ...
  it.each([
    {
      name: 'distributed structured index_not_found_exception',
      ccsEnabled: true,
      fault: () =>
        Object.assign(new Error('index missing'), {
          meta: { body: { error: { type: 'index_not_found_exception' } } },
        }),
    },
    {
      name: 'local non-index search fault',
      ccsEnabled: false,
      fault: () => new Error('search_phase_execution_exception'),
    },
  ])('propagates a $name from the policy-response search', async ({ ccsEnabled: ccs, fault }) => {
    const harness = await createEndpointUsageAccess();
    harness.endpointAppContextService.isCcsEnabled.mockResolvedValue(ccs);
    const responseFault = fault();
    harness.esClient.search.mockImplementation(async (request) => {
      if (isUnitedSearch(request)) {
        return unitedSearchResult({
          agents: [{ key: 'agent-1' }],
