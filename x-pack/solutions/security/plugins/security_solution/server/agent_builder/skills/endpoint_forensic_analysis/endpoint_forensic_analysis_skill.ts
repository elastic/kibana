...
- **Dispatch first, poll once, in one pass.** Call \`osquery.run_live_query\` for every guidance-block query (and \`secondary_query\`) up front, back-to-back, before waiting on any of them — never wait for one category's rows before dispatching the next category's query. Keep every returned \`action_id\` in a scratch list keyed by indicator type as you go.
- **Never re-dispatch a query that already has a pending \`action_id\`.** If a category's rows haven't arrived yet, call \`osquery.get_live_query_results\` again on its *existing* \`action_id\` — do not call \`osquery.run_live_query\` a second time for the same category. Re-running an already-dispatched query wastes a full poll cycle and can push the conversation past the platform's 20-minute execution ceiling for no new signal.
- The mutex \`winbaseobj\` query is the slowest of the seven and commonly takes **longer than 180s** to complete — do **not** conclude "the agent did not respond" after only 1-2 polls. But poll it with **short, frequent** checks instead of long blocking waits: call \`osquery.get_live_query_results\` with \`wait_seconds: 30\` and retry up to 10 times (≈300s total budget), moving on to poll a *different* pending category's \`action_id\` on each retry rather than re-polling the same one back-to-back — this interleaves the wait across all seven categories instead of burning the full budget on mutex alone before even checking the others. Only report a row as unavailable after its own action's \`status\` is confirmed as a genuine timeout/failure (not just one slow poll) — an in-progress \`status\` is not a terminal result.
- If, after interleaved polling, some categories still haven't returned rows within the overall budget, present the ones that *did* complete and mark the rest "— (pending, live query still in flight)" rather than continuing to poll indefinitely — a partial live-host cross-check delivered promptly is more useful than a complete one that risks hitting the execution timeout.
