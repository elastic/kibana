...
export const siemReadinessSkill = defineSkillType({
  ...
  description:
    'SIEM Readiness assessment: check data coverage, pipeline health, ECS data quality, ' +
    ...
    'retention compliance, integration coverage, or MITRE ATT&CK rule coverage. ' +
...
Reports ingest pipeline health — failure rates and document counts per pipeline.
...
Create **one attachment** containing all pipelines. Shows a table with pipeline name (linked to the pipeline management page), status, and failure rate.
