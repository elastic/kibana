...
describe('createAssessPolicyChangeTool', () => {
  ...
  it('preserves all fitting blocker-only rows without a truncation marker', async () => {
    const globalBlockers = [
      { reason: 'global_manifest_version_too_old' },
      { reason: 'policy_revision_conflict' },
    ];
    mockedAssessChange.mockResolvedValue(
      createDto({
        changes: [],
    ...
    const result = await getResult(VALID_PARAMS);
    const presented = result.data as { globalBlockers: unknown[] } & Record<string, unknown>;

    expect(presented.globalBlockers).toEqual(globalBlockers);
    expect(presented).not.toHaveProperty('global_blockers_value_truncated');
    expect(presented).not.toHaveProperty('global_blockers_value_total');
  });
