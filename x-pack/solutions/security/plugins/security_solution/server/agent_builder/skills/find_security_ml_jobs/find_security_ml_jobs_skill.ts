...
- Provide this context to the 'platform.core.generate_esql' tool:
  ...
  You **MUST ALWAYS** return required fields. Use this command: "KEEP ${getAnomalyKeepFields().join(
    ', '
  )}"."
...
5. Use the 'find.security.ml.jobs.extract_euid' tool to extract EUIDs from the combined anomaly records returned by the 'platform.core.execute_esql' tool.
6. Use the ~/skills/security/entities/entity-analytics skill and the 'security.get_entity' to look up the entity store profiles using the extracted EUIDs
7. Summarize the findings in a table format and provide key insights based on the anomalies and entities found.
  Table MUST include the EUID column. If multiple EUIDs are extracted for a single anomaly record, include all EUIDs in the table (e.g. separate by comma or list them in the same cell).`,
    ...
    getInlineTools: () => [findSecurityMlJobsTool(ctx), extractEuidFromMlDataTool()],
  });
