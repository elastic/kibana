...
describe('detection-coverage', () => {
  ...
  it('names the free-text search limit for Step 1 and the capability for Step 2', () => {
    expect(skill.content).toMatch(/filter.*only matches names, index patterns, and MITRE/s);
    // The result includes description and query — the model should read both for matching.
    expect(skill.content).toMatch(/every rule result includes its description and query/);
