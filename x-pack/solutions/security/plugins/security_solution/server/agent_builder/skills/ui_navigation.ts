...
/**
 * Builds the `flyout` template (with placeholder ids) that opens a specific entity's detail panel
 * directly on its Resolution ("Add entities to resolution group") tab. The name-derived fields only
 * set the flyout header title; the skill instructs the model to omit them when `entity.name` is
 * unknown (the flyout still resolves from the EUID).
 */
