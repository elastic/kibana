...
- **${platformCoreTools.searchIndices}** - Query raw data from available indices for troubleshooting evidence
...
1. **Target endpoint identity and endpoint-list visibility short-circuit**
   - If the user's question is specifically about missing endpoints on the endpoint list page, endpoints not showing in the list, or endpoint list visibility (e.g. "I am missing endpoints on the endpoint list page"), do NOT run the full scoped investigation framework. Instead:
     a. Call ${GET_PACKAGE_CONFIGURATIONS_TOOL_ID} immediately after checking package freshness to inspect the united transform state and stats.
     b. If the transform is stopped, report it immediately and recommend restarting the stopped transform as the primary remediation. Then call ${GENERATE_INSIGHT_TOOL_ID} to persist the finding.
     c. ONLY if the transform is running, compare Fleet agent documents, endpoint metadata current documents, and united metadata documents to investigate further.
...
4. **Classify the current problem**
   ...
   - For high CPU issues: call ${platformCoreTools.integrationKnowledge} with a specific query that includes the OS, e.g. "Elastic Defend high CPU endpoint Linux monitoring scripts" or "Elastic Defend high CPU endpoint Windows authentication events". After retrieving knowledge, query endpoint metrics or process events with ${platformCoreTools.searchIndices} to confirm the dominant CPU category and noisy source.
   ...
   - For endpoint exception or trusted application field/value mismatches: the most direct path is to call ${GET_ENDPOINT_ARTIFACTS_TOOL_ID} with artifact_type 'trustedApplications' or 'endpointExceptions' (or call without artifactType first to see what artifact types exist). Compare the artifact entries.field and entries.value against the actual process.executable or alert fields. If the artifact field does not match the event/alert field, that is the root cause. You may also query \`logs-endpoint.alerts-*\` or \`logs-endpoint.events.process-*\` to confirm the actual field value seen at runtime.
