...
export const getSearchAlertsByHostTool = () => ({
  id: 'security.fix-false-positive-alerts.search-alerts-by-host',
  type: ToolType.builtin,
  description:
    'Search alerts for a detection rule aggregated by host.name, process.parent.name, and process.name. ' +
    'Returns ranked breakdowns of hosts, parent processes, and processes by alert count, plus sample alert documents. ' +
  ...
  schema: z.object({
    ...
    hostName: z
      ...
      .optional()
      .describe(
        'Optional: filter to a specific host name. Omit to see all hosts ranked by alert count.'
      ),
    size: z
      .number()
      .min(1)
  ...
  handler: async (
    { ruleId, hostName, size }: { ruleId: string; hostName?: string; size: number },
    context: {
      spaceId: string;
    ...
    try {
      ...
      if (hostName) {
        filters.push({ term: { 'host.name': String(hostName) } });
      }
      ...
      console.log(`[search-alerts-by-host] ES query:`, JSON.stringify(searchQuery, null, 2));
      ...
      const hostBuckets = toBucketList(searchResult.aggregations?.hosts);
      ...
      const topHost = hostBuckets[0];
      const topParent = parentProcessBuckets[0];
      let assessment =
        hostBuckets.length > 0 && topHost
          ? `Host "${topHost.value}" leads with ${topHost.alertCount} of ${total} total alerts. ` +
            `${hostBuckets.length} distinct host(s) found.`
          : `No host data found in alerts for rule ${ruleId}.`;
      if (topParent) {
        assessment +=
          ` Top parent process: "${topParent.value}" (${topParent.alertCount} alerts).` +
          ` Parent processes are often the root cause of false positives — consider excluding by parent process rather than host.`;
      }

      console.log(`[search-alerts-by-host] ${assessment}`);
        ...
        results: [
          {
            ...
            data: {
              message: `Failed to search alerts by host for rule ${ruleId}: ${
