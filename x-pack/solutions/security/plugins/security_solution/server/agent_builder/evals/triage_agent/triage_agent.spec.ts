 ...
 */
evaluate.describe('Triage Agent', { tag: tags.serverless.security.complete }, () => {
  evaluate(
  ...
  evaluate('verdict includes confidence score between 0.0 and 1.0', async ({ evaluateDataset }) => {
    await evaluateDataset({
      dataset: {
  ...
  evaluate('verdict includes recommended_action field', async ({ evaluateDataset }) => {
    await evaluateDataset({
      dataset: {
