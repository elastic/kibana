 ...
 */
evaluate.describe('Correlator Agent', { tag: tags.serverless.security.complete }, () => {
  evaluate(
  ...
  evaluate('should map attack chain to MITRE ATT&CK techniques', async ({ evaluateDataset }) => {
    await evaluateDataset({
      dataset: {
  ...
  evaluate('should identify entity clusters (host, user, network)', async ({ evaluateDataset }) => {
    await evaluateDataset({
      dataset: {
