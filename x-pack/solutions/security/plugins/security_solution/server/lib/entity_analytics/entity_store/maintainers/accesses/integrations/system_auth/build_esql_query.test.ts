...
describe('System Auth buildEsqlQuery', () => {
  describe('namespace handling', () => {
    it('uses the namespace to form the index pattern', () => {
      ...
      expect(query).toContain('FROM logs-system.auth-default');
    });

    it('uses a custom namespace', () => {
      const query = buildEsqlQuery('production');
      expect(query).toContain('FROM logs-system.auth-production');
    });
  });
  ...
  describe('query structure', () => {
    ...
    it('filters for ssh_login action', () => {
      const query = buildEsqlQuery('default');
      expect(query).toContain('event.action == "ssh_login"');
    });
