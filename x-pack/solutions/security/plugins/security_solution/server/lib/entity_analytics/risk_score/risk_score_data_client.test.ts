...
describe('RiskScoreDataClient', () => {
  ...
  describe('getDailyAverageRiskScoreNormSeries', () => {
    ...
    it('filters on risk.id_field === "entity.id" and aggregates by risk.id_value', async () => {
      ...
      expect(filters).toEqual(
        expect.arrayContaining([
          ...
          { terms: { 'risk.id_value': ['host:InnoDB'] } },
        ])
      );

      const aggs = searchCall.aggs as { by_entity: { terms: { field: string } } };
      expect(aggs.by_entity.terms.field).toBe('risk.id_value');
    ...
    it('returns V2-written documents (risk.id_field === "entity.id")', async () => {
      esClient.search.mockResolvedValueOnce(
    ...
    it('excludes legacy-written documents — `risk.id_field === "entity.id"` filter eliminates raw-name docs', async () => {
      // Simulate Elasticsearch's perspective: the legacy doc has
      // risk.id_field === 'host.name', so the filter rejects it at the query
