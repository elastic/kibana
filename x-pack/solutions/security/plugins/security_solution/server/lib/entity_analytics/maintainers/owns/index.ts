...
import { buildOwnsConfigs } from './configs';
...
export const ownsMaintainer: RegisterEntityMaintainerConfig = {
  ...
  run: async ({
    ...
    logger.info(
      `[owns] Completed run: ${result.totalBuckets} buckets, ${result.totalRecords} records, ${result.totalWritten} entities written, ${result.totalDroppedTargets} targets dropped, ${result.totalMetadataDocsApplied} metadata docs appended, ${result.totalMetadataDocsFailed} metadata docs failed`
    );
