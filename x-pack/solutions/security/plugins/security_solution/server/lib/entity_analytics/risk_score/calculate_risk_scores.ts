...
const calculateWithESQL = async (
  ...
  logger.info(
    `STEP ONE: Received ESQL Risk Score composite response:\n${JSON.stringify(response)}`
  );
    ...
    ([entityType, { buckets, after_key: afterKey }]) => {
      ...
      return esClient.esql
        ...
        .catch((error) => {
          ...
          console.log(query);
          return [entityType, { afterKey, scores: [] }] as const;
        });
    }
  );
...
export const calculateRiskScores = async (
  ...
  withSecuritySpan('calculateRiskScores', async () => {
    ...
    console.log('esql', esqlResults);
    // const afterKeysESQL = Object.fromEntries(
    //   esqlResults.map(([entityType, { afterKey }]) => [entityType, afterKey])
    // );
    // const scores = esqlResults.reduce((acc, [entityType, { scores }]) => {
    //   acc[entityType] = scores;
    //   return acc;
