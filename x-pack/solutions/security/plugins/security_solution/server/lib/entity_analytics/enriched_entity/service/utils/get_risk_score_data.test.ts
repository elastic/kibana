...
describe('getRiskScoreData', () => {
  ...
  describe('error handling', () => {
    it('rejects when search throws', async () => {
      ...
      jest.mocked(esClient.search).mockRejectedValue(error);
    ...
    it('rejects when any search in a parallel batch throws', async () => {
      const error = new Error('search failed for bob');
      jest
        .mocked(esClient.search)
      ...
      ).rejects.toThrow('search failed for bob');
    });

    it('rejects when mget throws', async () => {
      const riskScore = makeRiskScore({ inputs: [makeInput('alert-1')] });
      jest.mocked(esClient.search).mockResolvedValue(makeSearchResponse(riskScore));
