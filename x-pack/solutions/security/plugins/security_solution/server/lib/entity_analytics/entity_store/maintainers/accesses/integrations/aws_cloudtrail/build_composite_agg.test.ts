...
describe('AWS CloudTrail buildCompositeAggQuery', () => {
  ...
  describe('query structure', () => {
    ...
    it('filters for AWS module', () => {
      const query = buildCompositeAggQuery();
      const filters = query.query.bool.filter;
      expect(filters).toContainEqual({ term: { 'event.module': 'aws' } });
    });

    it('filters for StartSession action', () => {
      const query = buildCompositeAggQuery();
      const filters = query.query.bool.filter;
      expect(filters).toContainEqual({ term: { 'event.action': 'StartSession' } });
    });
...
describe('AWS CloudTrail buildBucketUserFilter', () => {
