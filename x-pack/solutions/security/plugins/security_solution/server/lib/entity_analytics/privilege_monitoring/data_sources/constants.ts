...
export const INTEGRATION_MATCHERS_DETAILED: Partial<Record<IntegrationType, Matcher>> = {
...
export const getMatchersFor = (integration: IntegrationType): Matcher[] =>
  INTEGRATION_MATCHERS_DETAILED[integration]
    ? [INTEGRATION_MATCHERS_DETAILED[integration] as Matcher]
    : []; // There should be no matchers for okta_entity as this is for sync markers only
...
export const INTEGRATION_TYPES = ['okta', 'okta_entity', 'ad'] as const;
...
export const STREAM_INDEX_PATTERNS: Record<IntegrationType, (namespace: string) => string> = {
  ...
  okta_entity: (namespace) => `logs-entityanalytics_okta.entity-${namespace}`,
};
