...
const enrichWithHealth = ({
  ...
  indexHealth: Record<string, IndexHealthEntry>;
  ...
  for (const idx of pipeline.indices) {
  ...
  const threshold = thresholds.length > 0 ? Math.max(...thresholds) : SILENCE_THRESHOLD_DEFAULT_MS;
...
export const fetchPipelines = async ({
  ...
  const indexHealth = await fetchIndexHealth({ esClient, isServerless, logger });
  ...
  let rawPipelines: PipelineStats[];

  if (isServerless) {
    rawPipelines = Object.keys(pipelineToIndices).map((name) => ({
      name,
