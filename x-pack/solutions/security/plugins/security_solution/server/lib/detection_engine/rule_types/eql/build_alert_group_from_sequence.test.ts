...
describe('buildAlert', () => {
  ...
  // we didn't find it
  test('it builds an alert correctly with some missing events in the sequence', () => {
    const eqlSequence: EqlHitsSequence<SignalSource> = {
      join_keys: [],
      events: [
        sampleDocNoSortId('d5e8eb51-a6a0-456d-8a15-4b79bfec3d71'),
        {
          // Not just a lazy mock, this is actually what the EQL search API returns for missing events
          _index: '',
          _id: '',
          _source: {},
          missing: true,
