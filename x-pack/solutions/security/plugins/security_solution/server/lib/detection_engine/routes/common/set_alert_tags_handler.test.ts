...
import { setAlertTagsHandler } from './set_alert_tags_handler';
import { responseAdapter } from '../__mocks__/test_adapters';

describe('set alert tags handler', () => {
  ...
  describe('update alert tags by IDs', () => {
    test('returns 200 when updating tags by IDs', async () => {
    ...
    test('handles adding and removing tags', async () => {
      const mockedResponse = responseMock.create();
      const request = requestMock.create({
        method: 'post',
        body: {
      ...
      const script =
        call.script && typeof call.script === 'object' && 'params' in call.script
          ? call.script
          : null;
      expect(script?.params?.tagsToAdd).toEqual(['new-tag']);
      expect(script?.params?.tagsToRemove).toEqual(['old-tag']);
    });
    ...
    test('returns 404 when validateSiemClient returns false', async () => {
      const mockedResponse = responseMock.create();
      const request = requestMock.create({
        method: 'post',
        body: {
      ...
      await setAlertTagsHandler({
        ...
        response: mockedResponse,
        getIndexPattern: () => Promise.resolve('.alerts-security.alerts-default'),
        validateSiemClient: async () => false,
      });
      const response = responseAdapter(mockedResponse);
