...
import { isMachineLearningParams, isEsqlParams, isCorrelationParams } from '../utils/utils';
...
export const extractReferences = <TParams extends RuleParams>({
  ...
  if (!isMachineLearningParams(params) && !isEsqlParams(params) && !isCorrelationParams(params)) {
    returnReferences = [
      ...returnReferences,
      ...extractDataView({
