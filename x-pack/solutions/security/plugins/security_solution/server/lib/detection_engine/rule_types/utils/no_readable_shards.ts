...
export const getNoReadableShardsWarning = ({
  ...
  return `The events search returned no shards for index pattern(s) ${indexPatterns}, so no alerts can be created. Matching indices were found while validating the rule, which means the rule owner can see the index metadata but has no "read" index privilege on the indices. Grant the rule owner the "read" index privilege on these index patterns, or re-save the rule as a user who has it.`;
};
...
export const reportMissingAggregations = ({
  ...
  if (hasZeroShards(searchResult)) {
    const warning = getNoReadableShardsWarning({ inputIndex });
