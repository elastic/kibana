...
describe('searchAfterAndBulkCreate', () => {
  ...
  describe('reduced event fields request', () => {
    test('should request an explicit fields list when the reducedEventFieldsRequestEnabled experimental feature is on', async () => {
      ruleServices.scopedClusterClient.asCurrentUser.search.mockResolvedValueOnce(
        sampleEmptyDocSearchResults()
      );
      ...
      await searchAfterAndBulkCreate({
        sharedParams: {
          ...
          experimentalFeatures: {
            ...sharedParams.experimentalFeatures,
            reducedEventFieldsRequestEnabled: true,
          },
          mergeStrategy: 'missingFields',
          constantKeywordFields: ['data_stream.dataset'],
        },
        services: ruleServices,
        eventsTelemetry: undefined,
        filter: defaultFilter,
      ...
      expect(ruleServices.scopedClusterClient.asCurrentUser.search).toHaveBeenCalledWith(
        expect.objectContaining({
          fields: [
            { field: '@timestamp', include_unmapped: true },
            { field: 'data_stream.dataset', include_unmapped: true },
    ...
    test('should request the fields wildcard when the reducedEventFieldsRequestEnabled experimental feature is off', async () => {
      ruleServices.scopedClusterClient.asCurrentUser.search.mockResolvedValueOnce(
        sampleEmptyDocSearchResults()
      );
