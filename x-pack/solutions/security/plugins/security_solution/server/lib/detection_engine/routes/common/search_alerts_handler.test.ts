...
import { requestContextMock, responseMock } from '../__mocks__';
import { searchAlertsHandler } from './search_alerts_handler';
import { responseAdapter } from '../__mocks__/test_adapters';

describe('search for alerts', () => {
  ...
  describe('search and aggregate on signals index', () => {
    test('returns 200 when using single search', async () => {
      ...
      await searchAlertsHandler({
        ...
        request: getSignalsQueryRequest(),
        response: mockedResponse,
        getIndexPattern: () => Promise.resolve(undefined),
      });
    ...
    test('returns 200 when using single agg', async () => {
      ...
      await searchAlertsHandler({
        context: requestContextMock.convertContext(context),
        request: getSignalsAggsQueryRequest(),
        response: mockedResponse,
        getIndexPattern: () => Promise.resolve(undefined),
      });
      const response = responseAdapter(mockedResponse);
    ...
    test('returns 200 when using aggs and search together', async () => {
      ...
      await searchAlertsHandler({
        context: requestContextMock.convertContext(context),
        request: getSignalsAggsAndQueryRequest(),
        response: mockedResponse,
        getIndexPattern: () => Promise.resolve(undefined),
      });
      const response = responseAdapter(mockedResponse);
