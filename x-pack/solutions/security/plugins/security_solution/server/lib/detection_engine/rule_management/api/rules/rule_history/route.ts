...
import {
  RuleHistoryRequestQuery,
  RestoreRuleRequestQuery,
...
export const patchRuleHistoryRoute = (router: SecuritySolutionPluginRouter, logger: Logger) => {
  router.versioned
    .patch({
      access: 'public',
      path: DETECTION_ENGINE_RULES_URL_HISTORY,
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            query: buildRouteValidationWithZod(RestoreRuleRequestQuery),
          },
        },
      },
      async (context, request, response): Promise<IKibanaResponse> => {
        ...
        const { id: ruleId, changeId } = request.query;
        ...
        try {
          ...
          const rule = await detectionRulesClient.restoreRule({ ruleId, changeId });
          const body = { ok: true, rule };
