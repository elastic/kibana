...
// A technique unknown to the legacy blob — used to validate that
// retrieve-then-choose can surface and select techniques the LLM has no prior
// knowledge of.
...
const setupChainMock = (llmResponse: {
  tactics: string[];
  techniques: Array<{ id: string; subtechnique?: string[] }>;
}) => {
...
describe('formatMitreMapping', () => {
  const lookups = buildLegacyLookups();

  it('formats a tactic with a matching technique and subtechnique', async () => {
    const result = await formatMitreMapping(
      {
        tactics: ['TA0001'],
  ...
  it('outputs an empty technique array when no selected technique belongs to the tactic', async () => {
  ...
  it('tolerates missing arrays in a malformed model response', async () => {
    await expect(
      formatMitreMapping({} as Parameters<typeof formatMitreMapping>[0], lookups)
    ).resolves.toEqual([]);
  });
});
...
describe('addMitreMappingsNode', () => {
  ...
  it('uses MitreAttackDataClient when provided to validate IDs', async () => {
    ...
    const node = addMitreMappingsNode({
      model: {} as never,
      mitreAttackDataClient: mockClient({ getById, list, search }),
    });
  ...
  it('retrieves candidates from the managed index and lets the LLM pick a technique unknown to legacy data', async () => {
    const { capturedInvokes } = setupChainMock({
      tactics: ['TA0003'],
      techniques: [{ id: 'T1671' }],
    });
  ...
  it('drops techniques whose tactics do not match the requested tactic', async () => {
    ...
    expect(result.rule?.threat?.[0]?.technique).toEqual([]);
  });
});
