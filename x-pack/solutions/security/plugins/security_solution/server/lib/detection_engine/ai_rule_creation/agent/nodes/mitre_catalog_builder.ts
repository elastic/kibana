...
import type { MitreTactic, MitreTechnique } from '../../../../../../common/detection_engine/mitre/types';
...
function buildCatalogText(
  ...
  allTechniques.forEach((tech) => {
    tech.tactics.forEach((tacticValue) => {
      ...
      if (tactic) {
        const existing = techniqueMap.get(tactic.id) ?? [];
        existing.push(tech);
        techniqueMap.set(tactic.id, existing);
      }
    });
  });
  ...
  return targetIds
    .map((id) => {
      ...
      const techs = techniqueMap.get(id) ?? [];
