...
Every bucket of the phase 3 response carries a full source document, so the response can exceed `elasticsearch.maxResponseSize` (100 MB by default on Serverless) when the documents are large or when a single document holds an array with many new terms (the same document is returned once per term). When that happens the terms are split into smaller batches and the request is retried, halving the batch size until it fits. For a single new terms field all new terms of a page are fetched at once and halved on failure down to a single term. For multiple new terms fields the composite aggregation chunk size (500) is halved on failure down to a single term, the same way it is halved down to 125 on `max_clause_count` errors. In both cases processing resumes from the failed chunk, so alerts of already processed chunks are never created or suppressed twice.
