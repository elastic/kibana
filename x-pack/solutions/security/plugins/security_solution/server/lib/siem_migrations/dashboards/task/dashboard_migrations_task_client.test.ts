...
import type { DashboardMigrationsRunning } from './dashboard_migrations_task_client';
import { DashboardMigrationsTaskClient } from './dashboard_migrations_task_client';
import {
  SiemMigrationStatus,
...
import type { DashboardMigrationTaskStartParams } from './types';
...
import type { SiemMigrationDataStats } from '../../common/data/types';

jest.mock('./dashboard_migrations_task_runner', () => {
  return {
    DashboardMigrationTaskRunner: jest.fn().mockImplementation(() => {
      return {
...
describe('DashboardMigrationsTaskClient', () => {
  let migrationsRunning: DashboardMigrationsRunning;
  let logger: MockedLogger;
  let data: ReturnType<typeof createDashboardMigrationsDataClientMock>;
  const params: DashboardMigrationTaskStartParams = {
    migrationId,
  ...
  beforeEach(() => {
    ...
    DashboardMigrationsTaskClient.migrationsLastError = new Map();
    jest.clearAllMocks();
  });
  ...
  describe('start', () => {
    it('should not start if migration is already running', async () => {
      ...
      migrationsRunning.set(migrationId, {} as DashboardMigrationTaskRunner);
      const client = new DashboardMigrationsTaskClient(
        migrationsRunning,
        logger,
        data,
        currentUser,
    ...
    it('should not start if there are no dashboards to migrate (total = 0)', async () => {
    ...
    it('should not start if there are no pending dashboards', async () => {
      data.items.getStats.mockResolvedValue({
    ...
    it('should start migration successfully', async () => {
      ...
      (DashboardMigrationTaskRunner as jest.Mock).mockImplementationOnce(
        () => mockedRunnerInstance
      );
      ...
      expect(DashboardMigrationsTaskClient.migrationsLastError.has(migrationId)).toBe(false);
    });
    ...
    it('should throw error if a race condition occurs after setup', async () => {
      ...
      (DashboardMigrationTaskRunner as jest.Mock).mockImplementation(() => mockedRunnerInstance);
  ...
  describe('updateToRetry', () => {
    it('should not update if migration is currently running', async () => {
      ...
      const filter: DashboardMigrationFilters = { fullyTranslated: true };
  ...
  describe('getStats', () => {
    it('should return RUNNING status if migration is running', async () => {
      migrationsRunning.set(migrationId, {} as DashboardMigrationTaskRunner); // migration is running
      data.items.getStats.mockResolvedValue({
      ...
      data.migrations.get.mockResolvedValue({
        id: migrationId,
      } as unknown as StoredDashboardMigration);
  ...
  describe('getAllStats', () => {
    it('should return combined stats for all migrations', async () => {
      ...
      const migrations = [{ id: 'm1' }, { id: 'm2' }] as unknown as StoredDashboardMigration[];
      ...
      migrationsRunning.set('m1', {} as DashboardMigrationTaskRunner);
      const client = new DashboardMigrationsTaskClient(
        migrationsRunning,
        logger,
        data,
  ...
  describe('stop', () => {
    it('should stop a running migration', async () => {
      ...
      } as unknown as DashboardMigrationTaskRunner;
