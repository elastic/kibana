...
const mockGetDashboardsClient = jest.fn();
...
const mockSecuritySolutionContext = {
  siemMigrations: {
    getDashboardsClient: mockGetDashboardsClient,
  },
};
...
const makeRequest = (body: Record<string, unknown> = {}): KibanaRequest =>
  ({
    body: {
      ...
      input: {
        id: 'dashboard-1',
        original_dashboard: { title: 'Test dashboard' },
      },
      ...body,
    },
...
const mockRouter = { versioned: mockVersioned } as unknown as Parameters<
  typeof registerSiemDashboardMigrationsInvokeRoute
>[0];

// ── Tests ─────────────────────────────────────────────────────────────────────

describe('registerSiemDashboardMigrationsInvokeRoute', () => {
  let logger: Logger;

  beforeEach(() => {
    jest.clearAllMocks();
    logger = loggerMock.create();
    mockGetDashboardsClient.mockReturnValue(mockDashboardMigrationsClient);
    mockCreateInvoker.mockResolvedValue({ execute: mockExecute });

    registerSiemDashboardMigrationsInvokeRoute(mockRouter, logger);
  });
  ...
  describe('handler', () => {
    it('returns 200 with output on success', async () => {
      const output = { status: 'completed', elastic_dashboard: {} };
    ...
    it('forwards config to invoker.execute when provided', async () => {
      ...
      const config = { configurable: { skipPrebuiltDashboardsMatching: true } };
