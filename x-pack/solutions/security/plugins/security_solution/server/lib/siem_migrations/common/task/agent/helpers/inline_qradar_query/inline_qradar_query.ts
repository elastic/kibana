...
export interface GetInlineQradarQueryParams {
  model: ChatModel;
  logger: Logger;
}
export interface InlineQradarQueryInput {
  query: string;
  resources: MigrationResources;
}
export interface InlineQradarQueryOutput {
...
export const getInlineSplQuery: NodeHelperCreator<
  GetInlineQradarQueryParams,
  InlineQradarQueryInput,
  InlineQradarQueryOutput
> = ({ model, logger }) => {
  return async ({ query, resources }) => {
    // Early check to avoid unnecessary LLM calls
    // TODO : vendor specific unsupported checks
