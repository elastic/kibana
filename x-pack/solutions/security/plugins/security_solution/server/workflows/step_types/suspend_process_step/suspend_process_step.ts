...
/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor name agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */
...
export const createSuspendProcessStepDefinition = (
  ...
  createPollServerStepDefinition({
    ...
    start: async (context) => {
      ...
      // via `withEndpointAuthz({ all: ['canSuspendProcess'] })`. Assert the
      ...
      if (!authz.canSuspendProcess) {
        throw new ExecutionError({
          type: 'PermissionError',
          message: 'The workflow user does not have the canSuspendProcess endpoint privilege.',
        });
      }
      ...
      return {
        output: {
          ...
          message: details.wasSuccessful
            ? `Suspend-process action completed successfully. Action ID: ${details.id}`
            : `Suspend-process action failed. Action ID: ${details.id}`,
        },
      };
    },
  });
