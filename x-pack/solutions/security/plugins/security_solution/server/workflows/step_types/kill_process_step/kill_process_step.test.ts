...
import { createKillProcessStepDefinition } from './kill_process_step';
...
type StepDef = ReturnType<typeof createKillProcessStepDefinition>;
...
describe('createKillProcessStepDefinition', () => {
  let mockEndpointService: ReturnType<typeof createMockEndpointAppContextService>;
  let mockKillProcess: jest.Mock;
  let stepDefinition: ReturnType<typeof createKillProcessStepDefinition>;

  beforeEach(() => {
    jest.clearAllMocks();
    mockEndpointService = createMockEndpointAppContextService();
    mockKillProcess = jest.fn().mockResolvedValue({ id: ACTION_ID });
    (mockEndpointService.getInternalResponseActionsClient as jest.Mock).mockReturnValue({
      killProcess: mockKillProcess,
    });
    stepDefinition = createKillProcessStepDefinition(mockEndpointService);
  });

  describe('start', () => {
    it('dispatches the kill-process action by PID and returns action_id as state', async () => {
      const result = await stepDefinition.start!(
        createStartContext({
          endpoint_ids: ['agent-1'],
          parameters: { pid: 1234 },
          comment: 'terminate malicious process',
        })
      );
    ...
    it('dispatches the kill-process action by entity_id', async () => {
      await stepDefinition.start!(
        createStartContext({
          endpoint_ids: ['agent-1'],
          parameters: { entity_id: 'entity-abc' },
        })
      );

      expect(mockKillProcess).toHaveBeenCalledWith(
        expect.objectContaining({ parameters: { entity_id: 'entity-abc' } })
      );
    });
    ...
    it('uses an empty string comment when comment is omitted', async () => {
      ...
      expect(mockKillProcess).toHaveBeenCalledWith(expect.objectContaining({ comment: '' }));
    });

    it('throws ExecutionError when the user lacks canKillProcess privilege', async () => {
