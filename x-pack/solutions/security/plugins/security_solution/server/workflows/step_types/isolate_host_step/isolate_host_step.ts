...
export const createIsolateHostStepDefinition = (
  ...
  createPollServerStepDefinition({
    ...
    start: async (context) => {
      const { endpoint_ids: endpointIds, comment } = context.input;
      ...
      if (!authz.canIsolateHost) {
        throw new ExecutionError({
          ...
          message: 'The workflow user does not have the canIsolateHost endpoint privilege.',
        });
      }
      ...
      const dispatched = await client.isolate({
      ...
      return {
        output: {
          ...
          message: details.wasSuccessful
            ? `Host isolation completed successfully. Action ID: ${details.id}`
            : `Host isolation failed. Action ID: ${details.id}`,
        },
      };
    },
  });
