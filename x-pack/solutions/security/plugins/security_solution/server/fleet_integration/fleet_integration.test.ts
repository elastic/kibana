...
describe('Fleet integrations', () => {
  ...
  describe('protected artifact policy settings gate', () => {
    ...
    describe('create callback', () => {
      ...
      const buildCreateCallback = () => {
        const manifestManager = buildManifestManagerMock();
        return getPackagePolicyCreateCallback(
          logger,
          manifestManager,
          securitySolutionRequestContextFactory,
          endpointAppContextStartContract.alerting,
      ...
      beforeEach(async () => {
        ...
        (reqContextMock.getEndpointService as jest.Mock).mockReturnValue(
          endpointAppContextServiceMock
        );
        securitySolutionRequestContextFactory.create.mockReturnValue(
          Promise.resolve(reqContextMock)
        );
      });

      it('throws 403 when non-superuser supplies artifacts.global.public_key on create', async () => {
        (endpointAppContextServiceMock.getEndpointAuthz as jest.Mock).mockResolvedValue(
          getEndpointAuthzInitialStateMock({ canWriteAdminData: false })
        );
        const callback = buildCreateCallback();
        const policy = createNewPackagePolicyMock();
        policy.inputs = [
          {
            type: 'endpoint',
            enabled: true,
            ...
            config: {
              policy: {
                value: {
                  windows: { advanced: { artifacts: { global: { public_key: 'attacker-key' } } } },
                } as unknown,
              },
            },
          },
      ...
      it('allows superuser to supply artifacts.global.public_key on create', async () => {
        (endpointAppContextServiceMock.getEndpointAuthz as jest.Mock).mockResolvedValue(
          getEndpointAuthzInitialStateMock({ canWriteAdminData: true })
        );
        const callback = buildCreateCallback();
        ...
        policy.inputs = [
          {
            ...
            config: {
              policy: {
                value: {
                  windows: { advanced: { artifacts: { global: { public_key: 'my-key' } } } },
                } as unknown,
              },
            },
          },
