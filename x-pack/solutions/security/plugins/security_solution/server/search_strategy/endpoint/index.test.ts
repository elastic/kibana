...
describe('endpointSearchStrategyProvider', () => {
  ...
  it('rejects with a 403 when the caller cannot read Security Solution', async () => {
    const { provider, search } = buildProvider({ canReadSecuritySolution: false });
  ...
  it('runs the query when the caller can read Security Solution', async () => {
    const { provider, search } = buildProvider({ canReadSecuritySolution: true });
