    ...
    for (const vmName of targets) {
      ...
      await log.indent(2, async () => {
        ...
        if (options.installFirefox) {
          ...
          const hasFirefox =
            (
              await vm.exec('command -v firefox >/dev/null 2>&1 && echo 0 || echo 1', {
                silent: true,
              })
            ).stdout.trim() === '0';
          if (!hasFirefox) {
            const hasSnap =
              (
                await vm.exec('command -v snap >/dev/null 2>&1 && echo 0 || echo 1', {
                  silent: true,
                })
              ).stdout.trim() === '0';
            if (hasSnap) {
        ...
        if (options.installChrome) {
          if (arch === 'x86_64' || arch === 'amd64') {
            ...
            const hasChromium =
              (
                await vm.exec('command -v chromium >/dev/null 2>&1 && echo 0 || echo 1', {
                  silent: true,
                })
              ).stdout.trim() === '0';
            if (!hasChromium) {
