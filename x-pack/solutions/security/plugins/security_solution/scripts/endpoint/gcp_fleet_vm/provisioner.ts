...
const createEnrollmentApiKey = async (
    kbnClient: KbnClient,
    policyId: string
): Promise<string> => {
...
const ensureOsqueryIntegrationOnPolicy = async (
    ...
    const hasOsquery = existingIntegrations.items?.some(
        (p) => p.package?.name === 'osquery_manager'
    );
...
const validateFleetServerVmHealthy = async (cfg: GcpFleetVmConfig, log: ToolingLog): Promise<boolean> => {
    try {
...
export const provisionGcpFleetVm = async (
    ...
    const agentVersion = config.agentVersion || (await getAgentVersionMatchingCurrentStack(kbnClient, log));
    ...
    const calderaUrl =
        config.enableCaldera
            ? config.calderaUrl ||
            `http://${localTsHostname || localTsIp}:8888`
            : undefined;
    ...
    if (config.fleetServerMode === 'local-docker') {
        ...
        if (exists) {
            ...
            if (healthy) {
                log.info(`Fleet Server VM [${config.fleetServerName}] is healthy; reusing (skipping create).`);
            } else {
                log.warning(`Recreating Fleet Server VM [${config.fleetServerName}] due to failed health check`);
    ...
    if (config.fleetServerMode === 'gcp') {
        try {
            ...
            const candidates = Array.from(
                new Set([config.fleetServerName, remoteHostnameShort, remoteHostname].map((s) => s.trim()).filter(Boolean))
            );
