...
const fmt = (v: unknown) => (typeof v === 'string' ? v : v != null ? JSON.stringify(v, null, 2) : '');
...
const printLinkLogs = ({
    ...
    log.info(`[caldera][link] ability="${abilityName}" paw=${paw} executor=${executor} status=${status}`);
    ...
    const cmd = link?.command ?? link?.ability?.executors?.[0]?.command ?? link?.display?.command ?? '';
    if (cmd) {
...
const runOp: RunFn = async ({ log, flags }) => {
    ...
    const restoreGroup = async () => {
        try {
            ...
            log.warning(`[caldera] failed to restore agent ${targetPaw} group back to ${originalGroup}: ${e}`);
        }
    };
        ...
        if (operationId) {
            ...
            if (abilityIds.length && chainLen >= abilityIds.length) {
                ...
                if (links.length >= abilityIds.length && finishedLinks.length >= abilityIds.length) {
                    ...
                    log.info(`[caldera] fetching link results for operation ${operationId} (${links.length} link(s))`);
                    ...
                    for (const link of sorted) {
                        ...
                        if (isBrowserVisit(link?.ability?.name ?? link?.ability)) {
                            log.info(`[caldera][browser] tip: check History DB + osquery custom_data_dir on the target host`);
                        }
                    }
                    return;
                }
