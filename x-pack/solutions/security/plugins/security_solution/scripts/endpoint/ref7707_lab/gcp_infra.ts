...
import {
  ...
  gcloudSsh,
  GCP_REQUIRED_LABELS,
} from '../gcp_fleet_vm/gcloud';
import {
  REF7707_DOMAINS as REF7707_LAB_DOMAINS,
  DEFAULT_WEB_PORT as REF7707_WEB_PORT,
} from './constants';
...
export const provisionRef7707GcpInfra = async ({
  ...
  if (!namePrefix)
    throw new Error(`namePrefix is required (must match the prefix used by run_gcp_fleet_vm)`);
  ...
  const webExists = await gcloudInstanceExists({
    log,
    project: gcpProject,
    zone: gcpZone,
    instance: webVmName,
  });
  const dnsExists = await gcloudInstanceExists({
    log,
    project: gcpProject,
    zone: gcpZone,
    instance: dnsVmName,
  ...
  const webTsIp = await fetchTailscaleIpv4({
    log,
    project: gcpProject,
    zone: gcpZone,
    instance: webVmName,
  });
  const dnsTsIp = await fetchTailscaleIpv4({
    log,
    project: gcpProject,
    zone: gcpZone,
    instance: dnsVmName,
