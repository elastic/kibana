...
const runEnableDns: RunFn = async ({ log, flags }) => {
    ...
    try {
        ...
        if (!match?.id) {
            const known = (list.items ?? []).slice(0, 20).map((p) => p.name).join(', ');
            throw new Error(`Unable to find agent policy named [${policyName}]. Known policies (first 20): ${known}`);
        }
...
`,
        flags: {
            string: ['kibanaUrl', 'elasticUrl', 'username', 'password', 'apiKey', 'spaceId', 'policyName', 'logLevel'],
            default: {
