...
import { getRuntimeServices, startRuntimeServices, stopRuntimeServices } from '../endpoint_agent_runner/runtime';
...
const configureDnsmasq = async ({
    ...
    await dnsVm.exec(
        `sudo tee /etc/dnsmasq.d/ref7707.conf >/dev/null <<'EOF'\n${confLines}\nEOF`,
        { shell: true }
    );
...
const runBenignChainOnHost = async ({
    ...
    await vm.exec(
        `curl -fsS -o /var/tmp/ref7707/config.ini http://${domain}:${webPort}/config.ini`,
        { shell: true }
    );
    await vm.exec(
...
const setupSshLateral = async ({
    ...
    const pubKey = (await initiator.exec(`cat /home/ubuntu/.ssh/id_ed25519.pub`, { shell: true }))
        .stdout.trim();

    await target.exec(`mkdir -p /home/ubuntu/.ssh && chmod 700 /home/ubuntu/.ssh`, { shell: true });
    await target.exec(
        `grep -qF "${pubKey.replaceAll('"', '\\"')}" /home/ubuntu/.ssh/authorized_keys 2>/dev/null || echo "${pubKey.replaceAll(
            '"',
            '\\"'
...
export const runRef7707Lab = async (options: RunRef7707LabOptions): Promise<void> => {
    ...
    try {
        ...
        if (dnsTelemetrySource === 'network_packet_capture') {
            ...
            if (dnsTelemetrySource === 'packetbeat') {
                try {
        ...
        await configureVictimDnsResolver({ victimVmName: victimVmName, dnsServerIp: dnsIp });
        ...
        if (orchestrator === 'caldera') {
            ...
            await deploySandcatToMultipassUbuntuVm({ vmName: initiatorVmName, calderaUrl: options.calderaUrl, log });
            await deploySandcatToMultipassUbuntuVm({ vmName: victimVmName, calderaUrl: options.calderaUrl, log });
            ...
            const caldera = new CalderaClient({ calderaUrl: options.calderaUrl, apiKey: options.calderaApiKey });
            ...
            while (Date.now() - start < waitMs) {
                ...
                log.info(`[caldera] operation state=${state} chain=${chainLen}${expectedLinks ? `/${expectedLinks}` : ''}`);
            ...
            await runSshLateral({ initiatorVmName, targetVmName: victimVmName, domain, webPort: DEFAULT_WEB_PORT });
        }

        log.info(`Lab complete.\nDNS VM: ${dnsVmName}\nWEB VM: ${webVmName}\nINIT VM: ${initiatorVmName}\nVICT VM: ${victimVmName}`);
