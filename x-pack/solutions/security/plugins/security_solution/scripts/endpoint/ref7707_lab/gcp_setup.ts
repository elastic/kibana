...
const runSetup: RunFn = async ({ log, flags }) => {
  ...
  const localTailscaleHostname =
    (flags.localTailscaleHostname as string) || process.env.TS_HOSTNAME || '';
  ...
  const enableDnsTelemetry =
    flags.enableDnsTelemetry !== undefined ? Boolean(flags.enableDnsTelemetry) : true;
  ...
  try {
    ...
    if (enableDnsTelemetry) {
      ...
      log.info(
        `[ref7707] DNS telemetry enabled. Wait ~30-90s for agents to receive policy updates.`
      );
    } else {
