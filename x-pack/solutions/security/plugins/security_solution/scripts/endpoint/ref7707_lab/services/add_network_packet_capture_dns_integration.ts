...
export const addNetworkPacketCaptureDnsIntegrationToAgentPolicy = async ({
  ...
  if (!force) {
    log.debug(
      `Checking if agent policy [${agentPolicyId}] already includes a network capture integration`
    );
    ...
    for (const integrationPolicy of integrationPolicies) {
      ...
      if (pkg && candidatePackageNames.includes(pkg)) {
        log.debug(
          `Returning existing network capture Integration Policy [${pkg}] in agent policy [${agentPolicyId}]`
        );
  ...
  for (const packageName of candidatePackageNames) {
    try {
      ...
      const inputTemplatesPath = epmRouteService.getInputsTemplatesPath(
        packageName,
        packageVersion
      );
      ...
      if (!inputs.length) {
        throw new Error(
          `EPM did not return any input templates for [${packageName}@${packageVersion}].`
        );
      }
      ...
      lastError = e;
      log.warning(
        `[ref7707] unable to install/configure ${packageName} for DNS-only: ${
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(
        `Unable to install/configure any network packet capture integration for DNS-only streams`
      );
};
