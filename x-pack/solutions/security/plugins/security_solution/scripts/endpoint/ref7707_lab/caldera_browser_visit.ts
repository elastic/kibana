...
const replaceTokens = (
  cmd: string,
  vars: { domain: string; webPort: number; dnsIp?: string; webIp?: string }
) => {
  return cmd
...
const runBrowserVisit: RunFn = async ({ log, flags }) => {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    try {
      ...
      log.info(
        `[caldera][browser] created operation: ${opName}${operationId ? ` (${operationId})` : ''}`
      );
      if (operationId) {
        log.info(
          `[caldera][browser] dashboard: ${trimSlash(calderaUrl)}/#/operations/${operationId}`
        );
      }
      ...
      const linkDetails = linkId
        ? await caldera.getOperationLink(operationId, linkId).catch(() => undefined)
        : undefined;

      log.info(
        `[caldera][browser] link status=${linkDetails?.status ?? link?.status} finish=${
          linkDetails?.finish ?? link?.finish
        }`
      );
