...
const installChromiumOrChrome = async (hostVm: HostVm, log: ToolingLog): Promise<void> => {
    ...
    if (isArm64) {
        ...
        logger.info('Installing Chromium for ARM64 architecture (Chrome not available for ARM64 Linux)');
        await hostVm.exec('sudo apt-get update', { silent: true });
        ...
        await hostVm.exec('wget -q -O /tmp/google-chrome-key.gpg https://dl.google.com/linux/linux_signing_key.pub', { silent: true });
        await hostVm.exec('sudo gpg --dearmor -o /usr/share/keyrings/google-chrome-keyring.gpg /tmp/google-chrome-key.gpg', { silent: true });
...
const ensureFirefox = async (hostVm: HostVm, log: ToolingLog): Promise<void> => {
    ...
    if (Boolean(checkResult.stdout?.trim())) {
...
const injectChromeHistory = async (
    ...
    const vmUsername = (username === 'patryk' || username === 'elastic' || !username) ? 'ubuntu' : username;
    ...
    if (Boolean(chromiumCheck.stdout?.trim())) {
        isChromium = true;
    } else if (Boolean(chromeCheck.stdout?.trim())) {
        isChromium = false;
    } else {
