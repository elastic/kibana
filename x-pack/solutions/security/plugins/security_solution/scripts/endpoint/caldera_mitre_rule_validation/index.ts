...
const runInvokeAtomicOnGcpLinux = async ({
    ...
    const inner = [
        ...
        `(sudo -n true >/dev/null 2>&1 && sudo -n pwsh -NoProfile -NonInteractive -Command ${JSON.stringify(ps)} || pwsh -NoProfile -NonInteractive -Command ${JSON.stringify(ps)})`,
    ].join('; ');

    log.info(`[invoke-atomic] running ${techniqueId} on GCP VM ${instanceName} (${gcpProject}/${gcpZone})`);
    await gcloudSsh({ log, project: gcpProject, zone: gcpZone, instance: instanceName, command: `bash -lc ${JSON.stringify(inner)}` });
...
const pickRandomAbilityWithInstalledRuleCoverage = async ({
    ...
    const isStableAbility = (ab: any): boolean => {
        ...
        const cmd = execs.map((e: any) => String(e?.command ?? '')).join('\n').toLowerCase();
    ...
    const usableAbilities = abilities
        ...
        .filter((ab: any) => String(ab?.technique_id ?? ab?.technique?.attack_id ?? '').trim().length > 0)
        .filter((ab: any) => {
            const execs = Array.isArray(ab?.executors) ? ab.executors : [];
            return execs.some((ex: any) => String(ex?.command ?? '').trim().length > 0);
        });
    const filteredByStability = stableOnly ? usableAbilities.filter(isStableAbility) : usableAbilities;

    ok(filteredByStability.length > 0, `[caldera] no usable${stableOnly ? ' stable' : ''} abilities with technique id found for the selected platform/OS`);
    ...
    const isEndpointTelemetryRule = (r: any): boolean => {
        ...
        const idx = indices.map((i: any) => String(i)).join(' ').toLowerCase();
        return (
    ...
    const collectTechniqueIds = (predicate: (r: any) => boolean, onlySatisfiable: boolean): Set<string> => {
    ...
    ok(candidates.length > 0, `[caldera] no abilities found that map to an installed Elastic rule (by technique id) for os=${targetOs}`);

    const picked = candidates[Math.floor(Math.random() * candidates.length)];
    log.info(
        `[caldera] picked ability with installed rule coverage: name="${String(picked?.name ?? '')}" technique=${String(
            picked?.technique_id ?? picked?.technique?.attack_id ?? ''
        )}`
    );
    return picked;
};
...
const isFleetServerAgent = (agent: any): boolean => {
    ...
    if (components.some((c: any) => String(c?.name ?? '').toLowerCase() === 'fleet-server')) return true;
...
const osMatches = (targetOs: TargetOs, agentOsFamily: string): boolean => {
    ...
    if (targetOs === 'macos') return agentOsFamily.includes('darwin') || agentOsFamily.includes('mac') || agentOsFamily.includes('osx');
    ...
    const isMac = agentOsFamily.includes('darwin') || agentOsFamily.includes('mac') || agentOsFamily.includes('osx');
...
const findMitreTechniqueOrSubtechnique = (
    id: string
): { id: string; name: string; reference: string; parent?: { id: string; name: string } } | undefined => {
    for (const t of mitreTechniques as any[]) {
        ...
        if (sub) return { id: sub.id, name: sub.name, reference: sub.reference, parent: { id: t.id, name: t.name } };
    }
    return undefined;
};
...
const ruleSupportsOs = (rule: any, targetOs: TargetOs): { supported: boolean; reason?: string } => {
    ...
    const windows = normalized.some((t) => t === 'os: windows' || t === 'windows' || t.includes('os: windows'));
    const linux = normalized.some((t) => t === 'os: linux' || t === 'linux' || t.includes('os: linux'));
    const macos = normalized.some((t) => t === 'os: macos' || t === 'macos' || t.includes('os: macos'));

    const hasAnyOsTag = windows || linux || macos;
    if (!hasAnyOsTag) {
        // best-effort fallback: treat as any, but note ambiguity
        return { supported: true, reason: 'Rule has no explicit OS tags; treating as OS-agnostic (best-effort).' };
    }

    if (targetOs === 'windows') return { supported: windows, reason: windows ? undefined : 'Rule is not tagged for Windows.' };
    if (targetOs === 'linux') return { supported: linux, reason: linux ? undefined : 'Rule is not tagged for Linux.' };
...
const inferPrereqPackages = (rule: any): string[] => {
    ...
    const indexStr = indices.map((i: any) => String(i)).join(' ').toLowerCase();

    if (indexStr.includes('logs-endpoint') || indexStr.includes('metrics-endpoint') || indexStr.includes('endpoint.')) {
        pkgs.add('endpoint');
    }
    if (indexStr.includes('packetbeat')) pkgs.add('packetbeat');
    if (indexStr.includes('logs-osquery') || indexStr.includes('osquery')) pkgs.add('osquery_manager');
...
const runCli: RunFn = async ({ log, flags }) => {
    ...
    const useInvokeAtomic =
        Boolean(flags.useInvokeAtomic) || (execute && Boolean(techniqueId) && !randomAbility && !abilityId && !abilityName);
    const invokeAtomicCleanup = flags.invokeAtomicCleanup !== undefined ? Boolean(flags.invokeAtomicCleanup) : true;
    ...
    ok(targetOs === 'windows' || targetOs === 'linux' || targetOs === 'macos' || targetOs === 'any', 'Invalid --targetOs');
    ok(!execute || Boolean(fleetAgentId || autoSelectAgent), '--execute requires --fleetAgentId (single-target) or --autoSelectAgent');
    ok(!execute || useInvokeAtomic || Boolean(calderaUrl), '--execute requires --calderaUrl (unless using Invoke-Atomic)');
    ok(!execute || useInvokeAtomic || Boolean(calderaApiKey), '--execute requires --calderaApiKey (unless using Invoke-Atomic)');

    const kbnClient = createKbnClient({ log, url: kibanaUrl, username, password, apiKey, spaceId: spaceId || undefined });
    ...
    if (techniqueId) {
        ...
        log.info(
            `[mitre] technique: ${techniqueId}${tt ? ` (${tt.name})` : ''}${tt?.parent ? ` parent=${tt.parent.id} (${tt.parent.name})` : ''}${tt ? ` ${tt.reference}` : ''
            }`
        );
    }
    ...
    for (const a of candidates) {
        ...
        log.info(`  - ${a.id} host=${hostname || '<unknown>'} os=${osFamily || '<unknown>'} status=${a.status ?? '<unknown>'}`);
    }
    ...
    if (effectiveTacticId || effectiveTechniqueId) {
        ...
        if (matchingRules.length === 0 && installPrebuilt) {
            ...
            await installPrebuiltRules(kbnClient).catch((e: any) => {
                log.warning(`[rules] prebuilt installation failed (continuing): ${String(e?.message ?? e)}`);
            });
            matchingRules = await findAndFilterRules({ effectiveTacticId, effectiveTechniqueId });
        }
        ...
        if (matchingRules.length === 0 && !execute) {
            throw new Error(
                `[rules] no matching rules found for tactic=${tacticId || '<none>'} technique=${techniqueId || '<none>'} os=${targetOs}`
            );
        }
    }
    ...
    if (matchingRules.length > 0) {
        ...
        for (const r of matchingRules.slice(0, 25)) {
            ...
            log.info(`  - ${r.id} enabled=${Boolean(r.enabled)} name="${r.name}"${osInfo ? ` note="${osInfo}"` : ''}`);
        }
        ...
        if (enabledRules.length === 0) {
            if (enableMatchingRules) {
                log.warning(`[rules] enabling ${matchingRules.length} matching rules (explicit flag enabled)`);
                await enableRulesById(kbnClient, matchingRules.map((r: any) => r.id));
            } else {
                throw new Error(`[rules] matching rules exist but none are enabled. Re-run with --enableRules to enable them.`);
            }
        }
    } else {
        log.warning(`[rules] skipping MITRE rule coverage check in plan (no tactic/technique provided)`);
    }
    ...
    if (fleetAgentId && remediatePrereqs && matchingRules.length > 0) {
        ...
        log.info(`[prereqs] inferred packages for rule "${matchingRules[0].name}": ${prereqs.length ? prereqs.join(', ') : '<none>'}`);
        ...
        if (skipped.length) log.warning(`[prereqs] could not auto-attach (unsupported generic config): ${skipped.join(', ')}`);
    }
    ...
    if (useInvokeAtomic) {
        ...
        if (!selectedFleetAgentId && autoSelectAgent) {
            ...
            log.info(`[autoSelect] selected Fleet agent: ${selectedFleetAgentId} host=${getFleetHostname(candidates[0])}`);
        }
        ...
        const execTargetOs: TargetOs = targetOs === 'any' ? inferTargetOsFromFleetOsFamily(getFleetOsFamily(target)) : targetOs;
        ok(execTargetOs === 'linux', `[invoke-atomic] currently supports linux only (got targetOs=${execTargetOs})`);

        // Best-effort prereq remediation for the selected host
        if (remediatePrereqs && matchingRules.length > 0) {
            const policyId = String(target?.policy_id ?? '');
            if (!policyId) throw new Error(`[fleet] selected agent has no policy_id: ${selectedFleetAgentId}`);
            const policy = await fetchAgentPolicy(kbnClient, policyId);
            const prereqs = inferPrereqPackages(matchingRules[0]);
            log.info(
                `[prereqs] inferred packages for rule "${matchingRules[0].name}": ${prereqs.length ? prereqs.join(', ') : '<none>'}`
            );
            log.info(`[prereqs] target policy: ${policy.name} (${policy.id})`);
            const { attached, skipped } = await attachKnownPrereqsToPolicy({
                kbnClient,
        ...
        ok(Boolean(ruleIdForAlerts), `[invoke-atomic] no matching rule id available to wait for alerts`);
        ...
        if (waitForAlertsEnabled) {
            log.info(`[alerts] waiting up to ${waitForAlertsMs}ms for alerts for rule ${ruleIdForAlerts}...`);
    ...
    if (listAbilities) {
        ...
        for (const ab of abilities.slice(0, 50)) {
            const execs = Array.isArray(ab?.executors) ? ab.executors : [];
            const platforms = [...new Set(execs.map((e: any) => String(e?.platform ?? '')).filter(Boolean))].sort();
            log.info(
                `  - ${ab?.ability_id ?? ab?.id ?? '<no-id>'} name="${String(ab?.name ?? '')}" tactic=${String(ab?.tactic ?? '')} technique=${String(
    ...
    if (!selectedFleetAgentId && autoSelectAgent) {
        ...
        ok(Boolean(match), `[autoSelect] no Fleet candidate matched any Caldera paw (sandcat -paw %H expected)`);
        selectedFleetAgentId = match.id;
        log.info(`[autoSelect] selected Fleet agent: ${selectedFleetAgentId} host=${getFleetHostname(match)}`);
    }
    ...
    ok(Boolean(calderaAgent), `[caldera] no sandcat agent found matching paw=${hostname} (deploy sandcat with -paw %H)`);
    ...
    if (abilityId || abilityName) {
        ...
        ok(Boolean(found), `[caldera] ability not found (abilityId="${abilityId || '<none>'}" abilityName="${abilityName || '<none>'}")`);
        ...
        log.info(
            `[caldera] ability selected explicitly: name="${String(found?.name ?? '')}" tactic=${effectiveTacticId || '<none>'} technique=${effectiveTechniqueId || '<none>'}`
        );
        ...
        log.info(
            `[caldera] random ability picked (prebuilt-first): name="${String(picked?.name ?? '')}" tactic=${effectiveTacticId || '<none>'} technique=${effectiveTechniqueId || '<none>'}`
        );
    } else {
    ...
    ok(filteredAbilities.length > 0, `[caldera] no abilities found matching requested selection for platform=${platform}`);
    ...
    if (effectiveTacticId || effectiveTechniqueId) {
        ...
        matchingRules = await findAndFilterRules({ effectiveTacticId: '', effectiveTechniqueId }).then((rules) =>
            // Apply OS gating using the execution OS (not "any") when possible
            rules.filter((r: any) => ruleSupportsOs(r, execTargetOs).supported)
        );
        if (matchingRules.length === 0 && installPrebuilt) {
            log.warning(`[rules] no matching rules for selected ability; attempting prebuilt rules installation...`);
            await installPrebuiltRules(kbnClient).catch((e: any) => {
                log.warning(`[rules] prebuilt installation failed (continuing): ${String(e?.message ?? e)}`);
            });
            matchingRules = await findAndFilterRules({ effectiveTacticId: '', effectiveTechniqueId }).then((rules) =>
    ...
    if (matchingRules.length > 1) {
        ...
        if (scored[0]?.score > 0) {
            ...
            log.info(`[rules] selected best-matching rule by query/token overlap: ${matchingRules[0].id} "${matchingRules[0].name}"`);
        } else {
            log.info(`[rules] multiple rules match technique, but none mention executor tokens; keeping first match: ${matchingRules[0].id} "${matchingRules[0].name}"`);
        }
    }
    ...
    if (matchingRules.length === 0 && createRuleIfMissing) {
        ...
        throw new Error(
            `[rules] no matching rules found for selected ability mapping tactic=${effectiveTacticId || '<none>'} technique=${effectiveTechniqueId || '<none>'} os=${targetOs}`
        );
    } else {
    ...
    const adversary = await caldera.createAdversary({
        name: adversaryName,
        description: `Temporary adversary for MITRE test (tactic=${effectiveTacticId || 'n/a'} technique=${effectiveTechniqueId || 'n/a'})`,
    ...
    ok(Boolean(group), `[caldera] no --calderaGroup provided and agent has no group; cannot target operation`);
