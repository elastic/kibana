 ...
 *
 * The target is either a known list (`target`), or resolved from the uploaded
 * file name (`resolveTarget`), which is the path the import route takes when no
 ...
 * value is written.
 */
/** The list an import writes to: its access name and its id (part of every item id). */
export interface LookupImportTarget {
  index: string;
  listId: string;
}
...
export const importLookupItemsToStream = ({
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const values: string[] = [];
    let targetPromise: Promise<LookupImportTarget> | undefined =
      target != null ? Promise.resolve(target) : undefined;

    readBuffer.on('fileName', (fileNameEmitted: string) => {
      if (targetPromise != null || resolveTarget == null) return;
      readBuffer.pause();
      targetPromise = resolveTarget(decodeURIComponent(fileNameEmitted));
