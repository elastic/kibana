...
/**
 * A rule appears once per reason in a scan; count each rule once, keeping the entry that
 * carries the key check (`canRead`) over one that does not.
 */
const uniqueRules = (rules: ValueListReferencingRule[]): ValueListReferencingRule[] => {
  const byRule = new Map<string, ValueListReferencingRule>();
  for (const rule of rules) {
    const existing = byRule.get(rule.id);
    if (existing == null || (existing.canRead === undefined && rule.canRead !== undefined)) {
      byRule.set(rule.id, rule);
    }
  }
...
export const restrictListWithChecks = async ({
  ...
  if (alias == null) {
    // The locator already addresses the concrete index. The client still drops an alias
    // a previous interrupted restrict left behind, so a rerun completes the restrict.
    const { changed } = dryRun ? { changed: false } : await lists.restrictList({ id });
    return {
      body: { ...base, access: 'restricted', changed, dryRun, message: null },
      statusCode: 200,
    };
  }

  const rules = uniqueRules(referencingRules.rules ?? []);
