...
/** The gate an analyst has opened the approval or dismiss modal for. */
...
export const ConversationsPage: React.FC = () => {
  ...
  const onRequestApproval = useCallback((proposal: PndProposalRow) => {
    ...
    setPending({ decision: 'approve', hasResponded: false, proposal });
  }, []);

  const onDismiss = useCallback((proposal: PndProposalRow) => {
    setDecisionError(undefined);
    setPending({ decision: 'dismiss', hasResponded: false, proposal });
  }, []);
  ...
  return (
    <PndPageSection>
      ...
      {pending != null && pending.answer == null ? (
        <ApprovalModal
          data-test-subj="pndApprovalModal"
