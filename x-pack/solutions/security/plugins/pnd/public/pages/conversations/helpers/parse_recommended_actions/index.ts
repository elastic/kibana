...
/**
 * The label `watch_floor.yaml`'s `reason_incident_contained` step writes immediately before the
 * staged containment actions it rendered with liquid's `| json` — the same label-anchored contract
 * `parse_tuning_proposal` reads, applied to the Phase-3 containment gate.
 *
 ...
 * every Phase-3 row back to the fixed decision form, where approving carries no `approved_actions`
 ...
 */
export const RECOMMENDED_ACTIONS_LABEL = 'Staged containment actions JSON:';
 ...
 * `priority` is **not** enforced here, because nothing this reader returns is trusted: the form
 * only ever toggles an action whose `execution` reads exactly `kibana_api`, and the workflow's
 * own allow-list of executable action types is the boundary, not this reader.
 */
const isStagedAction = (value: unknown): value is AttackDiscoveryRecommendedAction => {
...
/**
 * The containment actions the Watch Floor staged behind {@link RECOMMENDED_ACTIONS_LABEL} in the
 * `incident_contained` gate's reasoning summary.
 *
 ...
 * fixed decision form, where nothing can be toggled on and therefore nothing executes. An empty
 * array is **not** a failure: it means the recommended-actions skill staged nothing, and the form
 * says so rather than pretending the row is degraded.
 */
