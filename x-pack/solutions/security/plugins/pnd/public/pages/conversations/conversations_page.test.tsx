...
const APPROVAL_MODAL = 'pndApprovalModal';
...
/**
 * Answers the first row on screen the way an analyst does: open ApprovalModal from the primary
 * action (approve) or the overflow dismiss item, give a reason, submit.
 *
 * Card-click opens the details flyout, not this modal — register #60.
 */
const answerGate = async ({
  ...
  if (decision === 'dismiss') {
    ...
    fireEvent.click(screen.getByTestId('pndQueueRowDismiss'));
  } else {
    fireEvent.click((await screen.findAllByTestId('pndQueueRowPrimaryAction'))[0]);
  }
  typeRationale(rationale);
  fireEvent.click(screen.getByTestId(`${APPROVAL_MODAL}-confirm`));
};

const typeRationale = (rationale: string) => {
  fireEvent.change(screen.getByTestId(`${APPROVAL_MODAL}-rationale`), {
...
describe('ConversationsPage — answering a gate', () => {
  ...
  it('opens the details flyout for the row that was activated', async () => {
    ...
    expect(screen.queryByTestId(APPROVAL_MODAL)).not.toBeInTheDocument();
  });
  ...
  it('opens the approval card from the card’s own primary action', async () => {
    ...
    expect(screen.getByTestId(APPROVAL_MODAL)).toBeInTheDocument();
  });
  ...
  it('closes the approval card once the decision lands', async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId('pndApprovalModal')).not.toBeInTheDocument());
  });
  ...
  it('opens no approval card when the overflow menu is reached for', async () => {
    ...
    expect(screen.queryByTestId('pndApprovalModal')).not.toBeInTheDocument();
  });
});
