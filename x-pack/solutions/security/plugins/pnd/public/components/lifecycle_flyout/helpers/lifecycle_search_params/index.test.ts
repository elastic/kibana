...
describe('LIFECYCLE_TAB_IDS', () => {
  it('is the three tabs the details flyout renders, in order', () => {
    expect(LIFECYCLE_TAB_IDS).toEqual(['overview', 'attachments', 'timeline']);
  });
});
...
describe('readLifecycleTabId', () => {
  ...
  it('reads the Attachments tab, which is a first-class tab again', () => {
    expect(readLifecycleTabId('?lifecycle=ad-1&lifecycleTab=attachments')).toBe('attachments');
  });

  /**
   * The retired tab ids that remain sections inside Overview. A link a colleague pasted
   * before the 2026-08-17 sync must land on the tab that holds that content now.
   */
  it.each(['tuning', 'lifecycle'])(
