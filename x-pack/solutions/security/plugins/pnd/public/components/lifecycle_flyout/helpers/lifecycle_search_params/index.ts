...
/**
 * The overlay's three tabs, in render order: Overview, Attachments, Timeline.
 *
 * Main's `ConversationDetailsFlyout` names those three. Attachments also remains a **section**
 * inside Overview (decision 1 of the 2026-08-17 sync), so a stale `?lifecycleTab=attachments`
 * now lands on the Attachments tab rather than falling back. `tuning` and `lifecycle` stay
 * retired and still resolve to Overview through {@link readLifecycleTabId}.
 *
 ...
 */
export const LIFECYCLE_TAB_IDS = ['overview', 'attachments', 'timeline'] as const;
