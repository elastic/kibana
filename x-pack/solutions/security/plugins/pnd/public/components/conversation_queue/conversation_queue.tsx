...
export interface ConversationQueueProps {
  ...
  /** Opens the dismiss confirmation, which still `_respond`s with decision: dismiss. */
  onDismiss: (proposal: PndProposalRow) => void;
...
export const ConversationQueue: React.FC<ConversationQueueProps> = ({
  ...
  const onSelectEvent = useCallback(
    (event: QueueEvent) => {
      ...
      if (proposal != null && proposal.correlationId.length > 0) {
        onViewLifecycle(proposal.correlationId);
      }
    },
    [onViewLifecycle, pendingBySourceId]
  );
  ...
  const onOpenChatEvent = useCallback(
    (event: QueueEvent) => {
      ...
      openConversation(event.threadConversationId);
    },
    [openConversation]
  );
  ...
  const onSelectChild = useCallback(
    (eventId: string) => {
      ...
      if (pending != null && pending.correlationId.length > 0) {
        onViewLifecycle(pending.correlationId);
        return;
      }
    ...
    [onViewLifecycle, pendingBySourceId, resolvedBySourceId]
  );
              ...
              ? proposals.map((proposal) => {
                  ...
                  return (
                    <QueueRow
                      ...
                      onRequestApproval={(selected) => {
                        const pending = pendingBySourceId.get(selected.id);
