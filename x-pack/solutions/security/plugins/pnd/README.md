...
| Managed Watch **install** | Not called (`initializeManagedWorkflows` skipped) |
...
installs **only static helpers** at boot (rule workers + Detection + auto-approver) at `spaceId: '*'`, then uninstalls leftover
**catalog** watches still sitting at `*`. Catalog watches (`lifecycle: 'dynamic'`) install per-space when an analyst enables them or saves settings. YAML default `enabled: false` on catalog watches; Detection stays `enabled: true` so `execute()` works.

⛔ `PND_INSTALLABLE_WORKFLOW_IDS` (what PND boot-installs) is deliberately **not**
`PND_WATCH_WORKFLOW_IDS` / `SYSTEM_SECURITY_WATCH_IDS` (what `_respond` / `_auto_respond` may resume).
Installing a workflow grants nothing; resuming one runs its remaining steps under the
...
`managed_workflow_drift.test.ts` pins the relationship in both directions.

A local cluster that still has leftover `system-security-watch-floor` (etc.) documents at `*` from the
pre-settings-plane POC: the first *successful* boot deletes them. If init degrades (incomplete static
install or ≥1000 managed docs), leftovers can survive that boot — delete those global watch docs or
wipe the local workflows index, then re-enable Floor. Kibana start never fails on cleanup.
...
| 36 | **`PATCH /internal/pnd/watches/{watchId}` still rejects `autonomyLevel`.** Settings (including autonomy) persist as per-space managed-workflow template values. Autonomy has exactly one writer: `PUT /internal/pnd/autonomy`, gated on `pnd_manage_autonomy`. | The PATCH route carries `pnd_write`, and route-level `requiredPrivileges` cannot be declared per field, so honouring `autonomyLevel` there would launder an autonomy raise past the privilege that exists to gate it. GET/PUT/`_auto_respond` all read the same template-value store; an uninstalled watch GET-returns default `manual` without installing. PUT is install-on-save and does not enable. The field stays in `UpdateWatchRequestBody` so the refusal is explicit. |
...
| 43 | ✅ **Per-space catalog enablement.** Catalog watches are `lifecycle: 'dynamic'` and are **not** installed at boot. [`initializeManagedWorkflows`](server/managed_workflows/initialize_managed_workflows.ts) installs static helpers (rule workers, #283488 Detection, auto-approver) at `*`, uninstalls leftover global catalog watches, then `ready()` when that pass is complete. Enable or save settings in a space installs `system-security-watch-floor-{spaceId}` (YAML `enabled: false`, then `updateWorkflow({ enabled: true })` if the analyst enabled it). | **Decision 8 is implemented.** The AlertZero loop still needs four gates: `xpack.pnd.enabled`, the AD 2.0 feature flag, the per-space Advanced Setting `securitySolution:enableAttackDiscoveryWorkflows` (defaults **false**), and enabling Floor in that space. PUT autonomy is install-on-save of a **disabled** per-space doc; it does not subscribe to Attack Discovery. |
...
```bash
# ⛔ Returns {"total": 0} for catalog watches that are not yet enabled in this space.
# Catalog watches install per-space as `system-security-watch-floor-{spaceId}`. Static helpers
# (Detection, auto-approver, rule workers) still live at GLOBAL_WORKFLOW_SPACE_ID = '*' and
# the space-scoped API cannot see those global documents.
