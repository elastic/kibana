 ...
 *
 * Disables a detection rule by its object id.  The client scope-checks the
 * rule first: a missing id and a rule outside the API's scope both answer 404
 ...
 *   - Re-disabling an already-disabled rule is NOT short-circuited.  A
 *     redundant disable still rewrites the rule and moves `updated_at` /
 *     `updated_by`.
 *   - `revision` never moves — disable is not a meaningful rule edit.
 *   - Toggling a rule into the state it already has is not an error.
 *
...
/** `POST /api/detection_engine/v2/rules/{id}/_disable` */
export const DETECTION_ENGINE_V2_RULE_DISABLE_PATH =
  `${DETECTION_ENGINE_V2_RULES_PATH}/{id}/_disable` as const;
...
const ruleIdParamsSchema = z.object({
  id: z.string().min(1).describe('The object id of the detection rule to disable.'),
});
...
/**
 * Registers `POST /api/detection_engine/v2/rules/{id}/_disable`.
 *
 ...
 * allowed, and delegates to `DetectionRulesClient.disableRule`.
 *
 ...
 */
export function registerDisableRuleRoute(
  ...
  router.versioned
    .post({
      access: 'public',
      path: DETECTION_ENGINE_V2_RULE_DISABLE_PATH,
      security: {
        authz: {
    ...
    .addVersion(
      ...
      async (
        ...
        try {
          ...
          const rule = await client.disableRule(
