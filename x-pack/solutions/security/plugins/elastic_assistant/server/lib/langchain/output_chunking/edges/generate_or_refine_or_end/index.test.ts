...
describe('getGenerateOrRefineOrEndEdge', () => {
  ...
  it('returns "end" when there are zero alerts', () => {
    const withZeroAlerts: AttackDiscoveryGraphState = {
      ...graphState,
      anonymizedDocuments: [], // <-- zero alerts
    };
  ...
  it('returns "end" when max hallucination failures are reached', () => {
    const withMaxHallucinationFailures: AttackDiscoveryGraphState = {
      ...graphState,
  ...
  it('returns "end" when max retries are reached', () => {
    const withMaxRetries: AttackDiscoveryGraphState = {
      ...graphState,
  ...
  it('returns refine when there are unrefined results', () => {
    const withUnrefinedResults: AttackDiscoveryGraphState = {
      ...graphState,
      unrefinedResults: [
        {
          alertIds: [],
