...
additional prompt`),
    ...
    it("produces correct prompt when citations are disabled", async ()=>{
        const result = await chatPromptFactory(DEFAULT_ASSISTANT_GRAPH_PROMPT_TEMPLATE, {...baseInputs, contentReferencesStore: newContentReferencesStore({ disabled: true })})
        expect(result.messages).toEqual(
            expect.arrayContaining([
...
additional prompt`),
    ...
    it("produces correct prompt when there is no user prompt", async ()=>{
        const result = await chatPromptFactory(DEFAULT_ASSISTANT_GRAPH_PROMPT_TEMPLATE, {...baseInputs, llmType: undefined, connectorId: "test-connector-id"})
        expect(result.messages).toEqual(
            expect.arrayContaining([
                new SystemMessage(`system prompt 
