...
import { getDurationNanoseconds } from '../../public/post/get_duration_nanoseconds';
...
export const postAttackDiscoveryInternalRoute = (
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            body: buildRouteValidationWithZod(AttackDiscoveryPostInternalRequestBody),
          },
          response: {
            200: {
              body: { custom: buildRouteValidationWithZod(AttackDiscoveryPostInternalResponse) },
            },
          },
        },
      },
      async (
        ...
        request,
        response
      ): Promise<IKibanaResponse<AttackDiscoveryPostInternalResponse>> => {
        ...
        try {
          ...
          generateAndUpdateAttackDiscoveries({
            actionsClient,
            enableFieldRendering: true, // the internal (deprecated) post attack discovery API always passes true for this value. It's still possible for clients who read the generated discoveries to specify false when retrieving them.
            executionUuid,
            authenticatedUser,
            config: request.body,
            dataClient,
            ...
            savedObjectsClient,
            telemetry,
            withReplacements: false, // the internal (deprecated) post attack discovery API never applies replacements when generating discoveries
          })
            .then(async (result) => {
              const end = new Date();
