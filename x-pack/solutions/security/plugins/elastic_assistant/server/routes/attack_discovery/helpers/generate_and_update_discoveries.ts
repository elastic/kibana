...
export const generateAndUpdateAttackDiscoveries = async ({
  ...
  try {
    ...
    const generatedDiscoveriesCount = attackDiscoveries?.length ?? 0;
    logger.error(
      `Attack discovery graph generated ${generatedDiscoveriesCount} discovery/discoveries from ${anonymizedAlerts.length} anonymized alert(s) for execution UUID ${executionUuid}`
    );

    if (generatedDiscoveriesCount === 0) {
      logger.error(
        `No attack discoveries were generated by the LLM graph for execution UUID ${executionUuid}. This could indicate: 1) The alerts do not contain attack patterns, 2) The LLM did not detect any patterns, or 3) An issue with the graph execution.`
      );
    }
    ...
    const dedupedCount = dedupedDiscoveries.length;
    if (generatedDiscoveriesCount > 0 && dedupedCount === 0) {
      logger.error(
        `All ${generatedDiscoveriesCount} attack discovery/discoveries were filtered out during deduplication for execution UUID ${executionUuid}. This means all generated discoveries already exist in the index.`
      );
    } else if (generatedDiscoveriesCount > dedupedCount) {
      logger.error(
        `Deduplication filtered out ${generatedDiscoveriesCount - dedupedCount} of ${generatedDiscoveriesCount} attack discovery/discoveries for execution UUID ${executionUuid}, ${dedupedCount} new discovery/discoveries remain`
      );
    } else if (generatedDiscoveriesCount > 0) {
      logger.error(
        `No deduplication needed: all ${generatedDiscoveriesCount} attack discovery/discoveries are new for execution UUID ${executionUuid}`
      );
    }
