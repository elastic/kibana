...
import { bulkEnableAttackDiscoverySchedulesRequest } from '../../../../../__mocks__/request';
...
describe('bulkEnableAttackDiscoverySchedulesRoute', () => {
  ...
  it('should handle successful request', async () => {
    const response = await server.inject(
      bulkEnableAttackDiscoverySchedulesRequest({ ids: ['schedule-1', 'schedule-2'] }),
  ...
  it('should handle missing data client', async () => {
    ...
    const response = await server.inject(
      bulkEnableAttackDiscoverySchedulesRequest({ ids: ['schedule-1'] }),
      requestContextMock.convertContext(context)
    );
  ...
  it('should handle `dataClient.bulkEnableSchedules` error', async () => {
    bulkEnableAttackDiscoverySchedules.mockRejectedValue(new Error('Oh no!'));
    const response = await server.inject(
      bulkEnableAttackDiscoverySchedulesRequest({ ids: ['schedule-1'] }),
      requestContextMock.convertContext(context)
    );
