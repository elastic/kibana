...
import type { UpdateAttackDiscoverySchedulesInternalRequestBody } from '@kbn/elastic-assistant-common';
import { OpenAiProviderType } from '@kbn/stack-connectors-plugin/common/openai/constants';

import { updateAttackDiscoverySchedulesInternalRoute } from './update';
import { serverMock } from '../../../../../__mocks__/server';
import { requestContextMock } from '../../../../../__mocks__/request_context';
import { updateAttackDiscoverySchedulesInternalRequest } from '../../../../../__mocks__/request';
...
const mockRequestBody: UpdateAttackDiscoverySchedulesInternalRequestBody = {
...
describe('updateAttackDiscoverySchedulesInternalRoute', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    updateAttackDiscoverySchedulesInternalRoute(server.router);
  ...
  it('should handle successful request', async () => {
    const response = await server.inject(
      updateAttackDiscoverySchedulesInternalRequest('schedule-1', mockRequestBody),
  ...
  it('should handle missing data client', async () => {
    ...
    const response = await server.inject(
      updateAttackDiscoverySchedulesInternalRequest('schedule-2', mockRequestBody),
      requestContextMock.convertContext(context)
    );
  ...
  it('should handle `dataClient.updateSchedule` error', async () => {
    ...
    const response = await server.inject(
      updateAttackDiscoverySchedulesInternalRequest('schedule-3', mockRequestBody),
      requestContextMock.convertContext(context)
    );
    expect(response.status).toEqual(500);
    expect(response.body).toEqual({
