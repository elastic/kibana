...
import type { AttackDiscoveryFindInternalResponse } from '@kbn/elastic-assistant-common';

import { findAttackDiscoveriesInternalRoute } from './find_attack_discoveries_internal';
...
describe('findAttackDiscoveriesInternalRoute', () => {
  ...
  it('throws if response validation fails', async () => {
    mockDataClient.findAttackDiscoveryAlerts.mockResolvedValueOnce({ data: [], invalid: true });
  ...
  it('always calls findAttackDiscoveryAlerts with withReplacements set to false for this internal API', async () => {
    await getHandler(mockContext, mockRequest, mockResponse);

    expect(mockDataClient.findAttackDiscoveryAlerts).toHaveBeenCalledWith(
      expect.objectContaining({
        findAttackDiscoveryAlertsParams: expect.objectContaining({
  ...
  it('always calls findAttackDiscoveryAlerts with enableFieldRendering set to true for this internal API', async () => {
    await getHandler(mockContext, mockRequest, mockResponse);
