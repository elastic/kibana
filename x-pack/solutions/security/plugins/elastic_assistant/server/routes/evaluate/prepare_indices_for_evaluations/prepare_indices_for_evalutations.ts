...
export class PrepareIndicesForEvaluations {
    ...
    async setup() {
        ...
        await Promise.allSettled(this.indicesCreateRequests.map((index) => this.esClient.indices.create(index)))
        this.isSetup = true
    }
    ...
    async cleanup() {
        ...
        await Promise.allSettled(this.indicesCreateRequests.map(index => this.esClient.indices.delete({ index: index.index })))
        this.isSetup = false
    }
}