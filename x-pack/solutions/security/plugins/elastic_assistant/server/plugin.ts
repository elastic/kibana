    ...
    >
{
  ...
  public setup(
    ...
    const featureFlagDefinitions: FeatureFlagDefinition[] = [
      {
        featureFlagName: ELASTIC_AI_ASSISTANT_SUMMARIZATION_ENABLED_FEATURE_FLAG,
        fallbackValue: false,
        fn: (conversationSummarizationEnabled) => {
          this.assistantService?.setIsConversationSummarizationEnabled(
            conversationSummarizationEnabled
          );
          return !conversationSummarizationEnabled; // keep subscription active while the feature flag is disabled.
        },
      },
    ];
