...
| Run tool (`security.attack-discovery.run`), sync mode | Soft deadline | **90s** | `ATTACK_DISCOVERY_RUN_SOFT_DEADLINE_MS` — [`run_step/constants.ts`](server/workflows/steps/run_step/constants.ts) | **Not a kill**: on deadline the call returns `execution_uuid` and the pipeline keeps running in the background for slow-path resume. Applies to the Agent Builder tool only — the run **step** awaits the pipeline and is bounded by its caller's timeout. |
...
**Decision.** Agent Builder integrations call `security.attack-discovery.run` in **sync mode**. The Agent Builder run tool races the generation pipeline against a hard-coded `ATTACK_DISCOVERY_RUN_SOFT_DEADLINE_MS = 90s` soft deadline (≈30s of headroom under the 120s AB ceiling). If the pipeline finishes first, the tool returns the full sync output (`attack_discoveries`, `execution_uuid`, `alerts_context_count`, `discovery_count`). If the soft deadline wins, the tool returns `{ execution_uuid }` only and lets the underlying pipeline keep running in the background. The agent skill exposes a dedicated `security.attack-discovery.get_status` tool so the user can resume by `execution_uuid` on a subsequent prompt.

The deadline is the **tool's**, not the step's. The workflow run step used to race the same deadline, which meant every non-AB caller — a scheduled workflow with a 35m timeout, say — also lost its result at 90s and got an unusable `status: 'pending'`. Sync mode now awaits the pipeline; a caller that needs a shorter bound sets its own step timeout.
...
### ADR-015 — Deliberate fork of the Attack Discovery alert field map

**Context.** The AD alerts-as-data field map exists in two copies: the canonical `attackDiscoveryAlertFieldMap` in [`@kbn/attack-discovery-schedules-common`](../../packages/kbn-attack-discovery-schedules-common/impl/fields/field_map.ts) (consumed by `elastic_assistant/server/plugin.ts`) and a fork in [`@kbn/discoveries`](../../packages/kbn-discoveries/impl/attack_discovery/alert_fields/alert_field_map.ts) (consumed by this plugin's `server/plugin.ts`). The same is true of the `ATTACK_DISCOVERY_ALERTS_CONTEXT` constant and the `ALERT_ATTACK_DISCOVERY_*` field-name constants. Both plugins call `ruleRegistry.ruleDataService.initializeIndex(...)` with the same `registrationContext` / `dataset` / `additionalPrefix`, so both install the **same** component template, `.adhoc.alerts-security.attack.discovery.alerts-mappings`. Every other field of the two `IndexOptions` objects comes from a shared source, so the field map and the context constant are the only values that can diverge.

**Decision.** Keep the fork. It is a **step toward** — not an achievement of — `@kbn/discoveries` becoming a standalone package: the package still imports `@kbn/attack-discovery-schedules-common` at runtime today (for example in [`deduplicate_scheduled_discoveries`](../../packages/kbn-discoveries/impl/attack_discovery/generation/deduplicate_scheduled_discoveries/index.ts), which uses `deduplicateAttackDiscoveries` / `getScheduledIndexPattern` / `normalizeAttackDiscovery`), and this plugin will keep depending on it for `AttackDiscoveryScheduleDataClient` regardless. Forking the alert-document schema removes one of the edges that would have to go away first. The **invariant**: the two field maps and the `ATTACK_DISCOVERY_ALERTS_CONTEXT` constant must remain identical. Enforcement is [`server/attack_discovery_index_parity.test.ts`](server/attack_discovery_index_parity.test.ts), which asserts deep equality of the two field maps and strict equality of the context string. Header comments on both copies point at each other and at the test.

**Consequence.** Adding or changing an AD alert field is a two-file change, and forgetting the second file fails the parity test with the offending key named rather than producing an install-order-dependent component template in production. The cost is accepted duplication; do **not** "fix" a parity failure by merging the two maps. **Exit condition:** when `@kbn/discoveries` genuinely separates from `@kbn/attack-discovery-schedules-common` (no runtime import remaining, and this plugin no longer installs the same component template as `elastic_assistant`), the shared-template invariant disappears and the parity test should be deleted along with the cross-referencing header comments.
