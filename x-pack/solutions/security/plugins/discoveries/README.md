...
| Run **tool** (`security.attack-discovery.run`), sync mode | Soft deadline | **90s** | `ATTACK_DISCOVERY_RUN_SOFT_DEADLINE_MS` — [`run_step/constants.ts`](server/workflows/steps/run_step/constants.ts) | **Not a kill**: on deadline the Agent Builder run tool returns `execution_uuid` and the pipeline keeps running in the background for slow-path resume. |
| Run **step** (`security.attack-discovery.run`), sync mode | Step `timeout` | per-workflow | workflow YAML `timeout:` | **No soft deadline**: sync mode awaits completion, bounded by the step's own `timeout`; slow generations return their discoveries inline once complete. |
| Agent Builder workflow-tool wrapper | Wait-for-completion ceiling | **120s** | `WAIT_FOR_COMPLETION_TIMEOUT_SEC` — `@kbn/agent-builder-common` | External AB ceiling the run tool's 90s soft deadline sits safely under. |
