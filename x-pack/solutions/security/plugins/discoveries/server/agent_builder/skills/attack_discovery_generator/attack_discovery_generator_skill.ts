...
- **Marker/tag live-retrieval is single-shot.** When the user asks you to run AD on alerts matching a marker, tag, or label (live-retrieval), the retrieval path is exactly one call: get the baseline via \`${GET_DEFAULT_ESQL_QUERY_TOOL_ID}\`, adapt its filter to the marker by editing the query text, and pass it to \`${RUN_ATTACK_DISCOVERY_TOOL_ID}\` in \`esql\` mode. Do NOT load \`attack-discovery-alert-retrieval-builder\` — it is for building new retrieval artifacts, not for this task. Do NOT call \`platform.core.generate_esql\` to rephrase or re-derive a query that already filters on the marker; if the adapted baseline returns rows, those rows are the alert set — analyze them, never re-query.
