...
Worker definitions are `dynamic` + `auto` + `restorable`. They are installed on enable or a settings save with `workflowIdSuffix: spaceId`, so every space owns an independent copy. Disable changes enablement in place. Each Worker is a `yamlTemplate` whose template values mirror the settings API (shared fields flat, Worker-specific fields under `extras`) and are re-used during definition upgrades. Persisted values are validated against the Worker's current declaration on every read and write. Extras keys that are absent on an older document are filled from that Worker's defaults at startup, before `ready()` re-renders the workflow, and again on read so the panel stays usable if that rewrite has not been persisted yet. A present value that fails validation is not replaced (see [Pre-customer state](#pre-customer-state)).
...
3. Add the Worker's `WorkerSettingsDeclaration` in `@kbn/alertzero-common` (`impl/worker_settings/<watch>.ts`) and list it in `WORKER_SETTINGS_DECLARATIONS`: `workerId`, `settingsVersion: 1`, `allowedAutonomyLevels` (1–3 of the shared scale, ascending), optional `scheduleInterval: { defaultValue }`, optional `extras: { schema, defaultValue }`. On the server add the Worker to the `RegisteredWorkerId` union in `server/managed_workflows/workers/worker_settings.ts`; `workers/index.ts` then registers it from the catalog through the shared registration, which reads the declaration. Settings migrations are described in `impl/worker_settings/README.md`.
...
5. Defaults from the declaration are used for a fresh per-space install and for extras keys missing on an older document. Persisted values are untrusted: on read and write they are parsed by the Worker's complete schema (`workerId` literal, `autonomy` restricted to the allowed levels, `scheduleInterval` only if declared, `extras` only if declared, unknown keys rejected). A stored document that still fails after that fill projects the Worker as `unavailable`.
...
Nothing changes in the API schema or the UI: `scheduleInterval` is already optional on `WorkerSettings` and `WorkerSettingsWrite`, and the interval control renders purely off the field's presence in the read body. A document persisted before the interval existed receives `defaultValue`. Renames and other shape changes are covered in `kbn-alertzero-common/impl/worker_settings/README.md`.
...
AlertZero is not live. How a stored document is brought onto the current declaration — default fill, per-worker `settingsVersion` steps, and shared field renames — is in `kbn-alertzero-common/impl/worker_settings/README.md`. A document that is still invalid after that migration stays unavailable until it is repaired or the affected per-space Worker is reset with the Watch team.
