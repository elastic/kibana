...
5. Defaults from the declaration are used for a fresh per-space install and for any declared field a stored document omits. Persisted values are otherwise untrusted: on read and write they are parsed by the Worker's complete schema (`workerId` literal, `autonomy` restricted to the allowed levels, `scheduleInterval` only if declared, `extras` only if declared, unknown keys rejected). A wrong type, an unknown key, or a settings version mismatch projects the Worker as `unavailable`.
...
3. **`impl/worker_settings/<watch>.ts`** (in `@kbn/alertzero-common`) — add `scheduleInterval: { defaultValue: '<interval>' }` to the Worker's declaration. The interval string lives in `kbn-workflows/managed/definitions/alertzero/worker_settings_defaults.ts` (the template fills from the same constant) and the declaration imports it. Presence is the opt-in: it drives fresh-install defaults, the projected settings, and whether an interval PATCH is accepted or rejected with a 400 naming `scheduleInterval`.

Nothing changes in the API schema or the UI: `scheduleInterval` is already optional on `WorkerSettings` and `WorkerSettingsWrite`, and the interval control renders purely off the field's presence in the read body. A document persisted before the field existed is read and rendered with that default; see [Pre-customer state](#pre-customer-state).
...
2. **Declaration** — add its fresh-install default in `kbn-workflows/managed/definitions/alertzero/worker_settings_defaults.ts` and include it in `extras.defaultValue` in `impl/worker_settings/<watch>.ts`, which imports that constant. A stored document that lacks the field is read with that default filled in.
3. **Template** — forward the field in the Worker's `yamlTemplate` renderer and YAML and bump the definition `version`. Adding a field with a declaration default is additive: a stored document that lacks it is read and rendered with that default, and no reset is required. A rename, removal, or retype is breaking; before customers the exit is a coordinated reset ([Pre-customer state](#pre-customer-state)). The setting is done only when the saved value reaches the run.
...
AlertZero is not live. An additive change — a new settings field with a declaration default — needs no reset and no migration. A stored document that omits the field is read and rendered with that default; the raw stored values are left as they are until the next save. A deliberate breaking change (a rename, removal, retype, or settings version bump) has no migration path. The exit is a coordinated reset of the affected per-space Worker documents, with the Watch teams.
