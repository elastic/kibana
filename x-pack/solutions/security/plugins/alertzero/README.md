...
5. Treat stored values as untrusted old data. `migrate` validates the shape and returns the complete current value set; a stored key the Worker does not project is rejected rather than dropped, because the projection is what the next install writes back. Reads and PATCHes run `migrate()`; startup does not enumerate documents before `ready()`.
...
8. Add settings-module tests for defaults, patches, and that projected keys are not stripped. Add managed-definition tests for valid rendered YAML and registry tests for catalog/settings wiring, including that each Worker YAML's `consts.worker_settings` block declares no key the template values do not carry. Imported YAML changes require an explicit managed-definition version decision.
