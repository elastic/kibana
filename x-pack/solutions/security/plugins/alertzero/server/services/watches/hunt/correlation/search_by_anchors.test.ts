...
describe('searchByAnchors', () => {
  ...
  it('excludes self-match when source_report_id provided', async () => {
    ...
    expect(callArg.query.bool.must_not).toEqual([{ term: { _id: 'rpt-self' } }]);
  });

  it('scopes the source report lookup to the acting space and returns empty when it is not there', async () => {
    const esClient = elasticsearchServiceMock.createElasticsearchClient();
    esClient.search.mockResolvedValue(emptySearchResponse);
    const result = await searchByAnchors(esClient, logger, 'hunt-a', {
      source_report_id: 'rpt-in-another-space',
    });
    const fetchArg = (esClient.search as unknown as jest.Mock).mock.calls[0][0];
    expect(JSON.stringify(fetchArg.query.bool.filter)).toContain('hunt-a');
    expect(result.hits).toHaveLength(0);
  });

  it('does not run the correlation query when the source report is missing', async () => {
    const esClient = elasticsearchServiceMock.createElasticsearchClient();
    esClient.search.mockResolvedValue(emptySearchResponse);
    await searchByAnchors(esClient, logger, 'hunt-a', {
      source_report_id: 'rpt-in-another-space',
    });
    expect(esClient.search as unknown as jest.Mock).toHaveBeenCalledTimes(1);
  });
