...
/** Matches `huntCorrelationAttachmentDataSchema`'s `anchors.max(50)`; the attachment rejects more. */
const MAX_ATTACHMENT_ANCHORS = 50;
...
export const runCorrelationEngine = async (
  ...
  return {
    ...
    attachment_data: {
      ...
      anchors: status === 'unavailable' ? [] : anchorItems.slice(0, MAX_ATTACHMENT_ANCHORS),
      diamond_scores: [],
      thresholds: {
      ...
      self_match_excluded: true,
      // The attachment schema requires a non-empty revision when present, so an
      // unknown revision is omitted rather than written as an empty string.
      ...(reportRevision ? { report_revision: reportRevision } : {}),
    },
  };
};
