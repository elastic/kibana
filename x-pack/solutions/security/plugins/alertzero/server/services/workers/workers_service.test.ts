...
describe('WorkersService', () => {
  ...
  it('lists a Worker whose stored settings contain a key the schema no longer knows as unavailable', async () => {
    const harness = createPersistentHarness();
    const service = harness.createService();
  ...
  it('lists a Rule Tuning Worker that omits extras as available, with the defaults filled', async () => {
    const harness = createPersistentHarness();
    const service = harness.createService();
    await service.update(RULE_TUNING, { enabled: true }, SPACE, request);
    const document = harness.documents.get(`${RULE_TUNING}-${SPACE}`);
    if (!document) throw new Error('Expected the Rule Tuning document to be installed');
    document.values = { settingsVersion: 1, autonomyLevel: 'manual', scheduleInterval: '2h' };
