...
describe('reviseProposalTool', () => {
  it('checks the manage privilege before revising', async () => {
    ...
    await run(agenticWith({ assertCanManage, revise }), {
  ...
  it('returns the new revision id, revision number, and the supersedes pointer', async () => {
  ...
  it('never calls revise() when the privilege check rejects', async () => {
    ...
    const result = await run(agenticWith({ assertCanManage, revise }), {
      proposalId: 'proposal-1',
    });
