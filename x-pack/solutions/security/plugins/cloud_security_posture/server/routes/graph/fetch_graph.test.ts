...
describe('fetchGraph', () => {
  ...
  it('should include entity enrichment when isEnrichPolicyExists is true', async () => {
    ...
    expect(query).toContain(`WITH actorEntityName = entity.name, actorEntityType = entity.type, actorSourceId = doc_id, actorSourceIndex = doc_index`);
    expect(query).toContain(`ENRICH ${getEnrichPolicyId()} ON target.entity.id`);
    expect(query).toContain(`WITH targetEntityName = entity.name, targetEntityType = entity.type, targetSourceId = doc_id, targetSourceIndex = doc_index`);
