...
import { defineGraphRoute } from './route';

describe('defineGraphRoute', () => {
  it('registers the route with the securitySolution privilege', () => {
    const router = httpServiceMock.createRouter();

    defineGraphRoute(router as any);

    const [config] = router.versioned.post.mock.calls[0];
    const authz = config.security?.authz;

    expect(config.path).toBe('/internal/cloud_security_posture/graph');
