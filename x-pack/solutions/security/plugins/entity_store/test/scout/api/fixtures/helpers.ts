...
export const triggerMaintainerRun = async (
  ...
  // `sync=true` runs the maintainer lifecycle in-process (runSync) and persists
  // the maintainer state (e.g. the watermark) BEFORE returning. The default
  // async path (runSoon) returns immediately and the state is persisted later,
  // so tests that depend on a settled watermark must use `sync: true`.
  const runUrl = sync
