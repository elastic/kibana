...
export function registerResolutionRulesDisable(router: EntityStorePluginRouter) {
  router.versioned
    .put({
      ...
      description: 'Disable an OOTB resolution rule by ID. Requires an enterprise license.',
      options: {
    ...
    .addVersion(
      ...
      wrapMiddlewares(
        async (ctx, req, res): Promise<IKibanaResponse> => {
          ...
          logger.debug(`Resolution rules disable API called for rule: ${id}`);
          ...
          await resolutionRulesClient.setRuleEnabled(id, false);
