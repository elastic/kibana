...
export const hostEntityDefinition: EntityDefinitionWithoutId = {
  ...
  fields: [
    ...
    collect({ source: 'host.boot.id' }),
    newestValue({
      source: 'host.cpu.usage',
      mapping: { type: 'scaled_float', scaling_factor: 1000 },
    }),
    collect({ source: 'host.disk.read.bytes', mapping: { type: 'long' } }),
    collect({ source: 'host.disk.write.bytes', mapping: { type: 'long' } }),
    collect({ source: 'host.network.egress.bytes', mapping: { type: 'long' } }),
    collect({ source: 'host.network.egress.packets', mapping: { type: 'long' } }),
    collect({ source: 'host.network.ingress.bytes', mapping: { type: 'long' } }),
    collect({ source: 'host.network.ingress.packets', mapping: { type: 'long' } }),
    collect({ source: 'host.pid_ns_ino' }),
    collect({ source: 'host.uptime', mapping: { type: 'long' } }),
    collect({ source: 'host.os.family' }),
    collect({ source: 'host.os.full' }),
    collect({ source: 'host.os.kernel' }),
    collect({ source: 'host.os.platform' }),
    collect({ source: 'host.os.version' }),
    collect({ source: 'host.geo.city_name' }),
    collect({ source: 'host.geo.continent_code' }),
    collect({ source: 'host.geo.continent_name' }),
    collect({ source: 'host.geo.country_iso_code' }),
    collect({ source: 'host.geo.country_name' }),
    // collect({ source: 'host.geo.location', mapping: { type: 'geo_point' } }),
