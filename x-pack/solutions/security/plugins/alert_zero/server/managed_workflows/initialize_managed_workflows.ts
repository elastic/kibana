...
import { ALERT_ZERO_MANAGED_WORKFLOW_OWNER_ID } from '../../common/constants';
...
export const initializeManagedWorkflows = async ({
  ...
  const client = await workflowsExtensions.initManagedWorkflowsClient(
    ALERT_ZERO_MANAGED_WORKFLOW_OWNER_ID
  );
  ...
  if (canReconcile) {
    try {
      await client.ready();
      logger.info('Alert Zero managed workflows initialized');
    } catch (error) {
      logger.warn(
        `Alert Zero managed workflow reconciliation failed: ${
