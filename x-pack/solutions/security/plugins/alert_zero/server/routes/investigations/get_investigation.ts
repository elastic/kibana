...
import type { GetInvestigationResponse } from '@kbn/alert-zero-common';
import { buildRouteValidationWithZod } from '@kbn/zod-helpers/v4';
import { getMockInvestigationById } from '@kbn/alert-zero-common';
...
export const registerGetInvestigationRoute = ({ router, logger, config }: RouteDependencies) => {
  router.versioned
    .get({
      path: ALERT_ZERO_INVESTIGATION_URL_TEMPLATE,
      access: INTERNAL_API_ACCESS,
      security: {
        authz: { requiredPrivileges: [ALERT_ZERO_API_PRIVILEGE_READ] },
      },
      summary: 'Get a Alert Zero investigation by id',
    })
    .addVersion(
      {
