    ...
    >
{
  ...
  setup(
    ...
    if (!this.config.enabled) {
      this.logger.info('Alert Zero plugin is disabled');
      return {};
    }

    this.logger.info('Setting up Alert Zero plugin');
    ...
    features.registerKibanaFeature({
      id: ALERT_ZERO_FEATURE_ID,
      name: ALERT_ZERO_PLUGIN_NAME,
      order: 1101,
      category: DEFAULT_APP_CATEGORIES.security,
      app: ['kibana', ALERT_ZERO_FEATURE_ID],
      privileges: {
        all: {
          app: ['kibana', ALERT_ZERO_FEATURE_ID],
          api: [ALERT_ZERO_API_PRIVILEGE_READ, ALERT_ZERO_API_PRIVILEGE_WRITE],
          savedObject: { all: [], read: [] },
          ui: ['show', 'write'],
        },
        read: {
          app: ['kibana', ALERT_ZERO_FEATURE_ID],
          api: [ALERT_ZERO_API_PRIVILEGE_READ],
          savedObject: { all: [], read: [] },
          ui: ['show'],
        },
      },
  ...
  start(_core: CoreStart, plugins: AlertZeroStartDependencies): AlertZeroPluginStart {
    ...
    const managedWorkflows = initializeManagedWorkflows({
      ...
      this.logger.error(
        `Alert Zero managed workflow initialization failed: ${
  ...
  private requireWatchesService(): WatchesService {
    if (!this.watchesService) {
      throw new Error('Watches service is not available until the Alert Zero plugin has started');
    }
  ...
  private requireWorkersService(): WorkersService {
    if (!this.workersService) {
      throw new Error('Workers service is not available until the Alert Zero plugin has started');
    }
