...
import { ALERT_ZERO_API_PRIVILEGE_READ, ALERT_ZERO_API_PRIVILEGE_WRITE } from '../common/constants';
...
jest.mock('./agent', () => ({
  agentType: { id: 'mock-alert-zero-type', baseConfiguration: {} },
...
describe('AlertZeroPlugin feature-flag gating', () => {
  ...
  describe('when xpack.alertzero.enabled is true', () => {
          ...
          privileges: expect.objectContaining({
            all: expect.objectContaining({
              api: expect.arrayContaining([
                ALERT_ZERO_API_PRIVILEGE_READ,
                ALERT_ZERO_API_PRIVILEGE_WRITE,
              ]),
              ui: expect.arrayContaining(['write']),
            }),
            read: expect.objectContaining({ api: [ALERT_ZERO_API_PRIVILEGE_READ] }),
          }),
        })
      );
    ...
    it('registers the Alert Zero thin agent type when Agent Builder is available at setup', () => {
      const plugin = new AlertZeroPlugin(createContext(createConfig({ enabled: true })));
      const coreSetup = coreMock.createSetup();
      const features = { registerKibanaFeature: jest.fn() };
      const workflowsExtensions = { registerManagedWorkflowOwner: jest.fn() };
