...
**High-confidence creation gate:** this is the rule that is easiest to miss during testing. A document in an IdP namespace does **not** automatically create a high-confidence user entity. It must also have `event.kind = asset`. That is the only gate; no other event kind, category, or type creates a high-confidence user entity.

If that gate fails, the document may still look "IdP-like", but it does **not** create a high-confidence user entity by itself. It can still enrich a high-confidence entity that already exists, because the post-aggregation filter also keeps rows whose `entity.id` was matched by the lookup against the latest index.
...
| IdP/account-looking event **without** `event.kind = asset` | **No high-confidence user entity created.** It falls to the local path if it has `user.name` + `host.id`, and creates nothing otherwise | Namespace alone is not enough |
...
| --------- | ---------- | --------- | ------- |
| Source maps to a known IdP/account namespace **and** the document has `event.kind = asset` | High | Mapped namespace such as `okta`, `entra_id`, or `active_directory` | Build a high-confidence EUID |
...
- Why it qualifies: the namespace maps to `okta`, and the document is an asset event
...
- Outcome: **no high-confidence user entity is created from this document**. The same is true for IdP lifecycle documents such as `event.category = iam` with `event.type = user`: they can enrich `user:jane@acme.com@okta` if it already exists, but they never create it.
...
- **IdP post-aggregation filter**: High-confidence entities are only created from documents with `event.kind = asset`. Every other event kind, including `iam` lifecycle events, can enrich a high-confidence entity that already exists but cannot create one.
...
- **Document `_id`**: SHA-256 hash of the EUID string (not the EUID itself), via `hashEuid`
