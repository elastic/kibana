...
import { TELEMETRY_QUERY_SOURCE, INDEX_PATTERN_LOGSTASH_MONITORING } from '../../common/constants';
import {
  HITS_SIZE,
  Counter,
...
function processStatsResults(
  ...
  currHits.forEach((hit) => {
    ...
    const clusterUuid = hit._source!.logstash?.elasticsearch?.cluster?.id || monitoringClusterUuid;
 ...
 * @param {string} monitoringClusterUuid - monitoring cluster UUID
 */
function processLogstashStateResults(
 ...
 */
async function fetchLogstashStats(
    ...
    body: {
      query: createQuery({
        ...
        filters: [
          {
            bool: {
              should: [{ term: { 'metricset.name': 'node_stats' } }],
            },
          },
        ],
  ...
  if (hitsLength > 0) {
    ...
    processStatsResults(results, options, monitoringClusterUuid);
  }
 ...
 * @param {Object} options - additional processing required options
 */
async function fetchLogstashState(
  callCluster: ElasticsearchClient,
  monitoringClusterUuid: string,
  ephemeralIds: string[],
  start: string,
    ...
    body: {
      query: createQuery({
        ...
        filters: [
          ...
          {
            bool: {
              should: [{ term: { 'metricset.name': 'node' } }],
            },
          },
        ],
      }) as estypes.QueryDslQueryContainer,
  ...
  if (hitsLength > 0) {
    // further augment the clusters object with more stats
    processLogstashStateResults(results, options, monitoringClusterUuid);
  }
  return Promise.resolve();
}
 ...
 */
export async function collectLogstashMetricbeatMonitoringMetrics(
  callCluster: ElasticsearchClient,
  monitoringClusterUuid: string,
  start: string,
  end: string,
  ...
  await fetchLogstashStats(callCluster, monitoringClusterUuid, start, end, options);
