...
async function fetchLogstashState(
  ...
  for (const pluginType of ['input', 'codec', 'filter', 'output']) {
    const params: estypes.SearchRequest = {
 ...
 */
export async function collectLogstashAgentMonitoringMetrics(
  callCluster: ElasticsearchClient,
  monitoringClusterUuid: string,
  start: string,
  end: string,
  ...
  if (allHostIds.length > 0) {
    await fetchLogstashState(callCluster, allHostIds, monitoringClusterUuid, start, end, options);
  }
