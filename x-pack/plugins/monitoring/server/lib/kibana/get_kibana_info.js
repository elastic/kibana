...
export function handleResponse(resp) {
  const source = get(
    resp,
    'hits.hits[0]._source.kibana.stats',
    get(resp, 'hits.hits[0]._source.kibana_stats')
  );
...
export function getKibanaInfo(req, kbnIndexPattern, { clusterUuid, kibanaUuid }) {
  ...
  const params = {
    ...
    filterPath: [
      ...
      'hits.hits._source.kibana.stats.kibana',
      ...
      'hits.hits._source.kibana.stats.process.uptime_in_millis',
      'hits.hits._source.kibana_stats.timestamp',
      'hits.hits._source.kibana.stats.timestamp',
    ],
    body: {
      query: {
        bool: {
