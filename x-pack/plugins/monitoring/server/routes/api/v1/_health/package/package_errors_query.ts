 ...
 */
interface PackageErrorsQueryOptions extends QueryOptions {
  products: MonitoredProduct[];
}

export const packageErrorsQuery = ({ timeRange, timeout, products }: PackageErrorsQueryOptions) => {
  if (!timeRange) throw new Error('packageErrorsQuery: missing timeRange parameter');
  return {
    timeout: `${timeout}s`,
    query: {
      bool: {
