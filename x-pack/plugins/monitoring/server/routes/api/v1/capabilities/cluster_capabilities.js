...
export function clusterCapabilitiesRoute(server) {
  /*
   * Monitoring Home
  ...
  server.route({
    method: 'POST',
    path: '/api/monitoring/v1/capabilities/{clusterUuid}',
    config: {
      validate: {
        params: Joi.object({
    ...
    handler: async (req) => {
      ...
      try {
        ...
        setupCapabilities = await getSetupCapabilities(req, indexPatterns, req.params.clusterUuid);
      } catch (err) {
