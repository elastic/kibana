...
export function nodeSetupStatusRoute(server: MonitoringCore) {
  ...
  server.route({
    ...
    validate: {
      params: createValidationFunction(postNodeSetupStatusRequestParamsRT),
      query: createValidationFunction(postNodeSetupStatusRequestQueryRT),
      body: createValidationFunction(postNodeSetupStatusRequestPayloadRT),
    },
    handler: async (req) => {
