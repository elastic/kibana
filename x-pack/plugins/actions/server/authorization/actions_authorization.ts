...
import { ACTION_SAVED_OBJECT_TYPE } from '../saved_objects';
...
const operationAlias: Record<string, string> = {
  execute: 'get',
  list: 'get',
};
  ...
  public async ensureAuthorized(operation: string, actionTypeId?: string) {
    ...
    if (authorization) {
      ...
      const { hasAllRequested, username } = await checkPrivileges(
        authorization.actions.savedObject.get(
          ACTION_SAVED_OBJECT_TYPE,
          operationAlias[operation] ?? operation
        )
      );
      if (hasAllRequested) {
