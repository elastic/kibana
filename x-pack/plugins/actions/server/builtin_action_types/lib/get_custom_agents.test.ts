...
const TestUrl = 'https://elastic.co';

describe('getCustomAgents', () => {
  const configurationUtilities = actionsConfigMock.create();
  test('get agents for valid proxy URL', async () => {
    configurationUtilities.getProxySettings.mockReturnValue({
      proxyUrl: 'https://someproxyhost',
      proxyRejectUnauthorizedCertificates: false,
    });
    const { httpAgent, httpsAgent } = await getCustomAgents(
  ...
  test('return default agents for invalid proxy URL', async () => {
    configurationUtilities.getProxySettings.mockReturnValue({
  ...
  test('return default agents for undefined proxy options', async () => {
    const { httpAgent, httpsAgent } = await getCustomAgents(
      configurationUtilities,
      logger,
      TestUrl
