...
describe('config validation', () => {
  ...
  test('config validation handles email host in allowedHosts', () => {
    ...
    expect(() => {
      validateConfig(actionType, notAllowedHosts1, { configurationUtilities: configUtilities });
    }).toThrowErrorMatchingInlineSnapshot(
    ...
    expect(() => {
      validateConfig(actionType, notAllowedHosts2, { configurationUtilities: configUtilities });
    }).toThrowErrorMatchingInlineSnapshot(
  ...
  test('config validation for emails calls validateEmailAddresses', async () => {
    configurationUtilities.validateEmailAddresses.mockImplementationOnce(
      validateEmailAddressesImpl
    );
