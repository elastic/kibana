...
import { getTokenCountFromInvokeStream, InvokeBody, parseGeminiStreamForUsageMetadata } from './get_token_count_from_invoke_stream';
...
export const getGenAiTokenTracking = async ({
  ...
  if (validatedParams.subAction === 'invokeStream' && result.data instanceof Readable  && actionTypeId==".gemini") {
    try {
      const { totalTokenCount, promptTokenCount, candidatesTokenCount } = await parseGeminiStreamForUsageMetadata({
