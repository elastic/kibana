        ...
        streams: [
          {
            ...
            dataset: 'foo',
            enabled: true,
            pkg_variables: { 'var-name': { value: 'test_yaml: value', type: 'yaml' } },
          },
        ],
      },
      {
        ...
        streams: [
          ...
          },
          {
            ...
            dataset: 'bar2',
            enabled: true,
            pkg_variables: { 'var-name': { value: undefined, type: 'text' } },
          },
        ],
      },
      {
        ...
        streams: [
          {
            ...
            dataset: 'foo',
            enabled: true,
            pkg_variables: { 'var-name': { value: 'invalidyaml: test\n foo bar:', type: 'yaml' } },
          },
        ],
      },
      {
  ...
  const noErrorsValidationResults = {
    ...
    inputs: {
      foo: {
        ...
        streams: { 'foo-foo': { pkg_variables: { 'var-name': null } } },
      },
      bar: {
        pkg_variables: { 'bar-input-var-name': null, 'bar-input2-var-name': null },
        streams: {
          'bar-bar': { pkg_variables: { 'var-name': null } },
          'bar-bar2': { pkg_variables: { 'var-name': null } },
        },
      },
      'with-disabled-streams': {
        streams: { 'with-disabled-streams-disabled': { pkg_variables: { 'var-name': null } } },
      },
    },
  };
    ...
    expect(validateDatasource(invalidDatasource, mockPackage)).toEqual({
      ...
      inputs: {
        foo: {
          ...
          streams: { 'foo-foo': { pkg_variables: { 'var-name': ['Invalid YAML format'] } } },
        },
        bar: {
          pkg_variables: {
          ...
          streams: {
            'bar-bar': { pkg_variables: { 'var-name': ['var-name is required'] } },
            'bar-bar2': { pkg_variables: { 'var-name': null } },
          },
        },
        'with-disabled-streams': {
...
describe('Ingest Manager - validationHasErrors()', () => {
  it('returns true for stream validation results with errors', () => {
    expect(
      validationHasErrors({
        pkg_variables: { foo: ['foo error'], bar: null },
      })
    ).toBe(true);
  });
  ...
  it('returns false for stream validation results with no errors', () => {
    expect(
      validationHasErrors({
        pkg_variables: { foo: null, bar: null },
      })
    ).toBe(false);
  });
  ...
  it('returns true for input validation results with errors', () => {
    expect(
      validationHasErrors({
        pkg_variables: { foo: ['foo error'], bar: null },
        streams: { stream1: { pkg_variables: { foo: null, bar: null } } },
      })
    ).toBe(true);
    expect(
      validationHasErrors({
        pkg_variables: { foo: null, bar: null },
        streams: { stream1: { pkg_variables: { foo: ['foo error'], bar: null } } },
      })
    ).toBe(true);
  });
