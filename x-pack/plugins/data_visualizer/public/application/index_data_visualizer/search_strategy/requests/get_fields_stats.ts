...
export const getFieldsStats = (
  ...
  switch (fieldType) {
    ...
    case JOB_FIELD_TYPES.IP:
      return fetchStringFieldsStats(dataPlugin, params, fields, options, fakeFailure);
  }
};
...
export const fetchStringFieldsStats = (
  ...
  unfakeFields: Field[],
  options: ISearchOptions,
  // @todo: clean up fakeFailure
  fakeFailure?: boolean
): Observable<StringFieldStats[] | FieldStatsError> => {
  const fields = fakeFailure ? unfakeFields.map((f) => f.fieldName) : unfakeFields;
