...
describe('CasesOracleService', () => {
  ...
  describe('upsertRecord', () => {
    ...
    const createPayload = { cases, rules, grouping };
    ...
    const updateDatePayload = { ...oracleSO.attributes, version: oracleSO.version };

    beforeEach(() => {
      unsecuredSavedObjectsClient.update.mockResolvedValue(oracleSO);
    });

    it('creates a record correctly', async () => {
      const record = await service.upsertRecord({
        recordId: oracleSO.id,
        createPayload,
        updateDatePayload,
      });
  ...
  describe('bulkUpsertRecord', () => {
    ...
    beforeEach(() => {
      unsecuredSavedObjectsClient.update.mockResolvedValueOnce(oracleSO);
      unsecuredSavedObjectsClient.update.mockResolvedValueOnce({
        ...oracleSO,
        id: 'so-id-2',
        version: 'so-version-2',
        attributes: { ...oracleSO.attributes, counter: 3 },
      });
    });

    it('creates a record correctly', async () => {
      const records = await service.bulkUpsertRecord([
        {
          recordId: oracleSO.id,
          createPayload,
          updateDatePayload,
        },
        {
          recordId: 'so-id-2',
          createPayload,
          updateDatePayload: { ...updateDatePayload, version: 'so-version-2', counter: 3 },
        },
      ]);

      expect(records).toEqual([
        { ...oracleSO.attributes, id: 'so-id', version: 'so-version' },
        { ...oracleSO.attributes, id: 'so-id-2', version: 'so-version-2', counter: 3 },
      ]);
    });

    it('calls the unsecuredSavedObjectsClient.create method correctly', async () => {
      await service.bulkUpsertRecord([
        {
          recordId: oracleSO.id,
          createPayload,
          updateDatePayload,
      ...
      expect(unsecuredSavedObjectsClient.update).nthCalledWith(
        1,
        'cases-oracle',
        'so-id',
        updateDatePayload,
      ...
      expect(unsecuredSavedObjectsClient.update).nthCalledWith(
        ...
        'cases-oracle',
        'so-id-2',
        { ...updateDatePayload, version: 'so-version-2', counter: 3 },
        {
          upsert: {
            cases,
            counter: 1,
