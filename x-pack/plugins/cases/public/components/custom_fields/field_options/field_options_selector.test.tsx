...
describe('FieldOptionsSelector', () => {
  ...
  it('toggles checkbox options correctly', async () => {
    appMockRender.render(
      <FieldOptionsSelector {...{ ...props, selectedType: 'Text' as CustomFieldTypesUI }} />
    );

    userEvent.click(screen.getByText(i18n.FIELD_OPTION_REQUIRED));

    expect(props.field.setValue).toHaveBeenCalledWith({ required_option: true });

    userEvent.click(screen.getByText(i18n.FIELD_OPTION_REQUIRED));

    expect(props.field.setValue).toHaveBeenCalledWith({ required_option: false });
  });
});
