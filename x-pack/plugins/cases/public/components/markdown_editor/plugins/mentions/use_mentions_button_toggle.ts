...
interface UseMentionsButtonToggleProps {
...
export const useMentionsButtonToggle = ({
  astRef,
  editorRef,
  uiPlugins,
  value,
  ...
  const enableLensButton = useCallback(() => {
    if (editorRef?.current?.textarea && editorRef.current?.toolbar) {
      const mentionsPluginButton = editorRef.current?.toolbar?.querySelector(
        `[aria-label="${MENTION}"]`
      );

      if (mentionsPluginButton) {
