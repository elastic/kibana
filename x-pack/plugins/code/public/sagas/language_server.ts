...
import {
  installLanguageServerFailed,
  installLanguageServerSuccess,
  loadLanguageServers,
...
function* handleInstallLanguageServer(action: Action<string>) {
  try {
    ...
    yield put(installLanguageServerSuccess(action.payload!));
  } catch (err) {
    yield put(installLanguageServerFailed(err));
  }
}
...
export function* watchInstallLanguageServer() {
  yield takeEvery(String(installLanguageServer), handleInstallLanguageServer);
}
