...
export function registerTelemetryRoute({
  ...
  router.put(
    {
      path: '/api/workplace_search/telemetry',
      validate: {
        body: schema.object({
          action: schema.oneOf([
    ...
    async (ctx, request, response) => {
      ...
      try {
        ...
        log.error(
          `Workplace Search UI telemetry error: ${e instanceof Error ? e.stack : e.toString()}`
        );
        return response.internalError({ body: 'Workplace Search UI telemetry failed' });
      }
    }
  );
}
