...
import { RoleMappingsLogic, RoleMappingsActions, RoleMappingsValues } from './role_mappings_logic';
import { generateRoleMappingPath } from './utils';

export const RoleMappings: React.FC = () => {
  const { initializeRoleMappings, handleResetMappings, resetState } = useActions(
    RoleMappingsLogic
  ) as RoleMappingsActions;

  const { roleMappings, multipleAuthProvidersConfig, dataLoading } = useValues(
    RoleMappingsLogic
  ) as RoleMappingsValues;
