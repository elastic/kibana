...
export interface ConnectorContentSchedulingProps {
  type: 'full' | 'incremental'; // TODO EFE once DLS Pr merged change with SYNCJOBTYPE
}

export const ConnectorContentScheduling: React.FC<ConnectorContentSchedulingProps> = ({ type }) => {
  ...
  const [simpleCron, setSimpleCron] = useState<{
    ...
    expression: schedulingInput?.[type].interval ?? '',
    frequency: schedulingInput?.[type].interval
      ? cronToFrequency(schedulingInput[type].interval)
      : 'HOUR',
  });
        ...
        <EuiFlexGroup direction="column">
          ...
          <EuiFlexItem>
            <CronEditor
              ...
              disabled={!scheduling[type].enabled}
              ...
              onChange={({
                ...
                setScheduling({
                  // TODO EFE- check if there is a better way ?
                  ...scheduling,
                  ...{ [type]: { enabled: scheduling[type].enabled, interval: expression } },
                });
                setHasChanges(true);
              }}
