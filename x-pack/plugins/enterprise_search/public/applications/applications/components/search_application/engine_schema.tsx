  ...
  const columns: Array<EuiBasicTableColumn<SchemaField['indices'][0]>> = [
    {
      ...
      render: (name: string) => (
        <EuiLinkTo
          to={`${ENTERPRISE_SEARCH_CONTENT_PLUGIN.URL}/${generateEncodedPath(
      ...
      actions: [
        {
          ...
          onClick: (item: SchemaField['indices'][0]) => {
            navigateToUrl(
              `${ENTERPRISE_SEARCH_CONTENT_PLUGIN.URL}/${generateEncodedPath(
                SEARCH_INDEX_TAB_PATH,
                {
                  indexName: item.name,
                  tabId: 'index_mappings',
...
export const EngineSchema: React.FC = () => {
  ...
  return (
    <EnterpriseSearchEnginesPageTemplate
      ...
      pageViewTelemetry={SearchApplicationViewTabs.SCHEMA}
