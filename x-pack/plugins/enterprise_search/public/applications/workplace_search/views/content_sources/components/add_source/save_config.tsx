...
export const SaveConfig: React.FC<SaveConfigProps> = ({
  ...
  const { sourceData, sourceConfigData, buttonLoading, configuredFields } =
    useValues(AddSourceLogic);

  const { name, accountContextOnly, serviceType, configurableFields = [] } = sourceConfigData;
  ...
  const {
    configuration: { documentationUrl, applicationPortalUrl, applicationLinkTitle },
  } = sourceData;

  const isPublicKey =
    configurableFields.find((field) => field.key === 'public_key') &&
    configurableFields.find((field) => field.key === 'consumer_key');
