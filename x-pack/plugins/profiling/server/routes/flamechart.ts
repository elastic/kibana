  ...
  router.get(
    ...
    async (context, request, response) => {
      ...
      try {
        ...
        logger.info('grouping stack frame metadata by stacktrace');
        const frameMetadataForTraces = await withProfilingSpan('group_metadata', async () => {
          return groupStackFrameMetadataByStackTrace(stackTraces, stackFrames, executables);
        });

        logger.info('creating intermediate caller-callee graph');
        const intermediateRoot = await withProfilingSpan('create_intermediate_graph', async () => {
          const rootFrame = createStackFrameMetadata();
          return createCallerCalleeIntermediateRoot(
            rootFrame,
            stackTraceEvents,
        ...
        logger.info('creating caller-callee graph');
        const root = await withProfilingSpan('create_caller_callee_graph', async () => {
          return fromCallerCalleeIntermediateNode(intermediateRoot);
        });

        logger.info('creating columnar caller-callee graph');
        const columnar = await withProfilingSpan('create_columnar_caller_callee', async () => {
          return createColumnarCallerCallee(root);
        });

        logger.info('creating flamegraph');
        const flamegraph = await withProfilingSpan('create_flamegraph', async () => {
          return createFlameGraph(columnar);
        });
