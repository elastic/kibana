...
describe('getRangeInfo', () => {
  ...
  it('should handle no dateStart, dateEnd or interval specified', async () => {
    ...
    const closeToDigitPrecision = 1;

    const info = getDateRangeInfo(BaseRangeQuery);
    expect(sloppyMilliDiff(nowM5, Date.parse(info.dateStart))).toBeCloseTo(
      0,
      closeToDigitPrecision
    );
    expect(sloppyMilliDiff(nowM0, Date.parse(info.dateEnd))).toBeCloseTo(0, closeToDigitPrecision);
