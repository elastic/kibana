...
describe('Security Plugin', () => {
  ...
  describe('#applyServerlessUiOverrides', () => {
    ...
    it('should override `roleManagementEnabled` if `config.serverlessOverrides.ui.roleManagementEnabled` is `true` and type is serverless', () => {
      const config = {
        ui: { roleManagementEnabled: false },
        serverlessOverrides: { ui: { roleManagementEnabled: true } },
      };

      const plugin = new SecurityPlugin(
        coreMock.createPluginInitializerContext(config, { buildFlavor: 'serverless' })
      );
      ...
      expect(setupManagementServiceMock).toHaveBeenCalledWith({
        ...paramsResult,
        uiConfig: { roleManagementEnabled: true },
      });
    });

    it('should override `roleManagementEnabled` if `config.serverlessOverrides.ui.roleManagementEnabled` is `false` and type is serverless', () => {
      const config = {
        ui: { roleManagementEnabled: true },
        serverlessOverrides: { ui: { roleManagementEnabled: false } },
      };
      ...
      expect(setupManagementServiceMock).toHaveBeenCalledWith({
        ...paramsResult,
        uiConfig: { roleManagementEnabled: false },
      });
    });

    it('should not override if `config.serverlessOverrides.ui.roleManagementEnabled` is undefined and type is serverless', () => {
      const config = { ui: { roleManagementEnabled: true } };
    ...
    it('should not override `roleManagementEnabled` if type is traditional', () => {
      const config = {
        ui: { roleManagementEnabled: false },
        serverlessOverrides: { ui: { roleManagementEnabled: true } },
      };
      ...
      const plugin = new SecurityPlugin(
        coreMock.createPluginInitializerContext(config, { buildFlavor: 'traditional' })
      );
    ...
    it('should not return anything if neither is defined and buildFlavor `serverless`', () => {
      const plugin = new SecurityPlugin(
        coreMock.createPluginInitializerContext({}, { buildFlavor: 'serverless' })
      );
      ...
      expect(setupManagementServiceMock).toHaveBeenCalledTimes(1);
      expect(setupManagementServiceMock).toHaveBeenCalledWith(paramsResult);
    });

    it('should not return anything if neither is defined and buildFlavor `traditional`', () => {
      const plugin = new SecurityPlugin(
        coreMock.createPluginInitializerContext({}, { buildFlavor: 'traditional' })
      );
