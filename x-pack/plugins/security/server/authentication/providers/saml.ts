...
export class SAMLAuthenticationProvider extends BaseAuthenticationProvider {
  ...
  private async authenticateViaRefreshToken(
    request: KibanaRequest,
    { username, refreshToken, realm }: ProviderState
  ) {
    ...
    try {
      ...
      return AuthenticationResult.succeeded(user, {
        authHeaders,
        state: { username, realm: this.realm || realm, ...refreshedTokenPair },
      });
    } catch (err) {
      this.logger.debug(
  ...
  private async authenticateViaHandshake(request: KibanaRequest, redirectURL: string) {
    ...
    try {
      ...
      const { id: requestId, redirect, realm } = await this.options.client.callAsInternalUser(
        'shield.samlPrepare',
        {
          body: preparePayload,
        }
