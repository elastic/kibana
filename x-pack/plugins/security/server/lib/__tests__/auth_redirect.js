...
describe('lib/auth_redirect', function () {
  ...
  describe('when server.plugins.security.shouldAuthenticate() is true', () => {
    beforeEach(() => {
      server.plugins.security.shouldAuthenticate.returns(true);
    });
  ...
  describe('when server.plugins.security.shouldAuthenticate() is false', () => {
    ...
    it('replies with no credentials', async () => {
      await authenticate(request, h);
    ...
    it(`doesn't call server.security.authenticate`, async () => {
      await authenticate(request, h);

      sinon.assert.notCalled(server.plugins.security.authenticate);
    });
  });
});
