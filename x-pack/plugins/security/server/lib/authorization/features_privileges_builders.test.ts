...
test('specifies key for each feature', () => {
  ...
  const result = builder.build(features);
...
test('includes app actions when specified', () => {
  ...
  expect(result).toEqual({
    foo: {
      bar: [actions.login, actions.version, actions.app.get('foo-app'), actions.app.get('bar-app')],
    },
  });
});
  ...
  expect(result).toEqual({
    foo: {
      bar: [
        actions.login,
        actions.version,
        actions.ui.get('foo-ui-capability'),
        actions.ui.get('bar-ui-capability'),
      ],
    },
  });
});
