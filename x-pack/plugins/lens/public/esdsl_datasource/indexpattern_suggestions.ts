...
type IndexPatternSugestion = DatasourceSuggestion<EsDSLPrivateState>;
...
function buildSuggestion({
  ...
  updatedLayer?: EsDSLLayer;
  label?: string;
}): DatasourceSuggestion<EsDSLPrivateState> {
...
function getEmptyLayerSuggestionsForField(
  ...
  let newLayer: EsDSLLayer | undefined;
...
export function getDatasourceSuggestionsFromCurrentState(
  state: EsDSLPrivateState
): Array<DatasourceSuggestion<EsDSLPrivateState>> {
  ...
  return _.flatten(
    Object.entries(state.layers || {})
      ...
      .map(([layerId, layer]) => {
        ...
        const suggestions: Array<DatasourceSuggestion<EsDSLPrivateState>> = [];
        if (metrics.length === 0) {
...
function createChangedNestingSuggestion(state: EsDSLPrivateState, layerId: string) {
  const layer = state.layers[layerId];
...
function createSimplifiedTableSuggestions(state: EsDSLPrivateState, layerId: string) {
  const layer = state.layers[layerId];
...
function getMetricSuggestionTitle(layer: EsDSLLayer, onlyMetric: boolean) {
...
function separateBucketColumns(layer: EsDSLLayer) {
