...
import { IndexPattern, ExpressionBasedPrivateState, IndexPatternField, IndexPatternRef } from './types';
...
export type Props = DatasourceDataPanelProps<ExpressionBasedPrivateState> & {
...
export function ExpressionBasedDataPanel({
  setState,
  state,
  dragDropContext,
  core,
                    ...
                    const responses = await Promise.all(
                      Object.entries(localState.layers).map(([id, layer]) => {
                        const ast = {
                          type: 'expression',
                          chain: [
                            buildExpressionFunction<any>('expressionbased', {
                              query: layer.query,
                            }).toAst(),
                          ],
                        };
...
export function ExpressionBasedHorizontalDataPanel({
  setState,
  ...
  const onSubmit = async () => {
    try {
      const responses = await Promise.all(
        Object.entries(localState.layers).map(([id, layer]) => {
          return expressions.run(layer.query, null).toPromise();
        })
      );
      const cachedFieldList: Record<
        string,
