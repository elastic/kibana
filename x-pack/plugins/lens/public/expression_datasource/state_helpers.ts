...
): ExpressionBasedLayer {
  const keptColumns: ExpressionBasedLayer['columns'] = _.pick(layer.columns, column =>
    isColumnTransferable(column, newIndexPattern)
  );
  const newColumns: ExpressionBasedLayer['columns'] = _.mapValues(keptColumns, column => {
