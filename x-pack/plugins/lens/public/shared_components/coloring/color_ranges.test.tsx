...
describe('Color Ranges component', () => {
  ...
  it('should display all the color ranges passed', () => {
    const component = mount(<ColorRanges {...props} />);
    expect(
      component.find('input[data-test-subj^="my-test_dynamicColoring_range_value_"]')
    ).toHaveLength(4);
  });
  ...
  it('should disable "add new" button if there is maxStops configured', () => {
    ...
    expect(
      component
        .find('[data-test-subj="my-test_dynamicColoring_addColorRange"]')
        .first()
        .prop('disabled')
    ).toBe(false);
  ...
  it('should add a new range with default color and reasonable distance from last one', () => {
    ...
    expect(
      component.find('input[data-test-subj="my-test_dynamicColoring_range_value_3"]').prop('value')
    ).toBe(80); // 60-40 + 60
    expect(
      component
        // workaround for https://github.com/elastic/eui/issues/4792
        .find('[data-test-subj="my-test_dynamicColoring_range_color_3"]')
  ...
  it('should restore previous color when abandoning the field with an empty color', () => {
    ...
    expect(
      component
        .find('[data-test-subj="my-test_dynamicColoring_range_row_0"]')
        .first()
        .find(EuiColorPicker)
        .first()
        .prop('color')
    ...
    act(() => {
      component
        .find('[data-test-subj="my-test_dynamicColoring_range_color_0"]')
        .first()
        .simulate('blur');
    });
    component = component.update();
  ...
  it('should sort ranges value on whole component blur', () => {
    ...
    expect(
      component
        .find('[data-test-subj="my-test_dynamicColoring_range_value_3"] input[type="number"]')
        .prop('value')
    ).toBe(90);
  });
  ...
  it('should show current max/min value when user use auto detect min/max value', () => {
    ...
    const addDetectMinValueButton = component
      .find('[data-test-subj="my-test_dynamicColoring_autoDetect_minimum"]')
      .first();
    act(() => {
  ...
  it('should reverse colors when user click "reverse"', () => {
    ...
    const reverseColorsButton = component
      .find('[data-test-subj="my-test_dynamicColoring_reverseColors"]')
      .first();
    act(() => {
  ...
  it('should distribute equally ranges when use click on "Distribute equally" button', () => {
    ...
    const distributeEquallyButton = component
      .find('[data-test-subj="my-test_dynamicColoring_distributeEqually"]')
      .first();
    act(() => {
