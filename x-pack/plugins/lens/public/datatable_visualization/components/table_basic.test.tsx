...
describe('DatatableComponent', () => {
  ...
  test('it does compute a gradientHelper for each numeric column only when colorMode is enabled', () => {
    const { data, args } = sampleArgs();
    ...
    expect(wrapper.find(DataContext.Provider).prop('value').gradientHelpers).toEqual({
      c: expect.any(Function),
    });
  });

  test('it does not compute a gradientHelper for a numeric column if colorMode is "none"', () => {
    const { data, args } = sampleArgs();
