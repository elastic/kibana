...
export class TimeseriesChart extends React.Component {
  ...
  renderChart() {
    ...
    if (modelPlotEnabled === true ||
      ...
      const combinedData = contextForecastData === undefined ?
  ...
  renderFocusChart() {
    ...
    focusChart.select('.x.axis')
      .call(this.focusXAxis.ticks(numTicksForDateFormat(this.vizWidth), xAxisTickFormat)
        .tickFormat((d) => {
          return moment(d).format(xAxisTickFormat);
        }));
    focusChart.select('.y.axis')
      .call(this.focusYAxis);
    ...
    dots.attr('cx', (d) => { return this.focusXScale(d.date); })
      .attr('cy', (d) => { return this.focusYScale(d.value); })
      .attr('class', (d) => {
    ...
    // Update all markers to new positions.
    scheduledEventMarkers.attr('x', (d) => this.focusXScale(d.date) - LINE_CHART_ANOMALY_RADIUS)
    ...
    if (focusForecastData !== undefined) {
      ...
      // Update all dots to new positions.
      forecastDots.attr('cx', (d) => { return this.focusXScale(d.date); })
        .attr('cy', (d) => { return this.focusYScale(d.value); })
  ...
  drawContextElements(cxtGroup, cxtWidth, cxtChartHeight, swlHeight) {
    ...
    this.contextYScale = d3.scale.linear().range([cxtChartHeight, contextChartLineTopMargin])
    ...
    timeBuckets.setBounds(bounds);
    const xAxisTickFormat = timeBuckets.getScaledDateFormat();
    const xAxis = d3.svg.axis().scale(this.contextXScale)
      .orient('top')
    ...
    const contextBoundsArea = d3.svg.area()
      .x((d) => { return this.contextXScale(d.date); })
      .y0((d) => { return this.contextYScale(Math.min(chartLimits.max, Math.max(d.lower, chartLimits.min))); })
      .y1((d) => { return this.contextYScale(Math.max(chartLimits.min, Math.min(d.upper, chartLimits.max))); })
      .defined(d => (d.lower !== null && d.upper !== null));
    ...
    const contextValuesLine = d3.svg.line()
      .x((d) => { return this.contextXScale(d.date); })
      .y((d) => { return this.contextYScale(d.value); })
      .defined(d => d.value !== null);
