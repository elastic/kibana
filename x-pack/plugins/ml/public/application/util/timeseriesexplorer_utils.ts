...
import type { MlForecastService } from '../services/forecast_service_provider';
import type { MlTimeSeriesSeachService } from '../timeseriesexplorer/timeseriesexplorer_utils/timeseries_search_service_provider';

export function timeSeriesExplorerProvider(
  uiSettings: IUiSettingsClient,
  mlAnnotationsApiService: MlApiServices['annotations'],
  mlResultsApiService: MlApiServices['results'],
  ...
  return {
    getAutoZoomDuration(selectedJob: Job) {
    ...
    processRecordScoreResults(scoreData: any) {
    ...
    processMetricPlotResults(metricPlotData: any, modelPlotEnabled: any) {
    ...
    processForecastResults(forecastData: any) {
    ...
    // specified time.
    findChartPointForAnomalyTime(chartData: any, anomalyTime: any, aggregationInterval: any) {
      let chartPoint;
    ...
    processDataForFocusAnomalies(
      ...
      anomalyRecords.forEach((record: MlAnomalyRecordDoc) => {
        ...
        const chartPoint = this.findChartPointForAnomalyTime(
          chartData,
          recordTime,
          aggregationInterval
        );
    ...
    findChartPointForScheduledEvent(chartData: any, eventTime: any) {
      let chartPoint;
      if (chartData === undefined) {
        return chartPoint;
      }
    ...
    processScheduledEventsForChart(
      ...
      if (scheduledEvents !== undefined) {
        ...
        each(scheduledEvents, (events, time) => {
          const exactChartPoint = this.findChartPointForScheduledEvent(chartData, +time);
    ...
    getFocusData(
      ...
      return forkJoin([
        ...
        mlResultsApiService.getAnomalyRecords$(
          [selectedJob.job_id],
          criteriaFields,
          0,
          searchBounds.min.valueOf(),
        ...
        map(
          ([
            ...
            let focusChartData = this.processMetricPlotResults(
              metricData.results,
              modelPlotEnabled
            );
            // Tell the results container directives to render the focus chart.
            focusChartData = this.processDataForFocusAnomalies(
              focusChartData,
              anomalyRecords,
            ...
            focusChartData = this.processScheduledEventsForChart(
              focusChartData,
              scheduledEvents,
              focusAggregationInterval
            );
            ...
            if (forecastData) {
              refreshFocusData.focusForecastData = this.processForecastResults(
