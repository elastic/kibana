...
export function EmbeddedMapComponent({ geoPoints }) {
  ...
  useEffect(() => {
    if (embeddable && !isErrorEmbeddable(embeddable) && Array.isArray(geoPoints)) {
      layerList.current = [...layerList.current, getGeoPointsLayer(geoPoints!, 'red')];
      embeddable.setLayerList(layerList.current);
    }
  }, [embeddable, geoPoints]);
