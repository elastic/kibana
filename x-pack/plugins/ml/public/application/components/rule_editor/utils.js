...
export function buildRuleDescription(rule) {
  ...
  actions.forEach((action, i) => {
    ...
    switch (action) {
      case ML_DETECTUR_RULE_ACTION.SKIP_RESULT:
        ...
        break;
      case ML_DETECTUR_RULE_ACTION.SKIP_MODEL_UPDATE:
...
export function filterTypeToText(filterType) {
  switch (filterType) {
    case ML_DETECTUR_RULE_FILTER_TYPE.INCLUDE:
      return i18n.translate('xpack.ml.ruleEditor.includeFilterTypeText', { defaultMessage: 'in' });
    case ML_DETECTUR_RULE_FILTER_TYPE.EXCLUDE:
