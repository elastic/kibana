...
export class ChartLoader {
  ...
  private newJobLineChart = memoizeOne(ml.jobs.newJobLineChart, eq);
  private newJobPopulationsChart = memoizeOne(ml.jobs.newJobPopulationsChart, eq);
  private getEventRateData = memoizeOne(mlResultsService.getEventRateData, eq);
  private getCategoryFields = memoizeOne(getCategoryFieldsOrig, eq);
  ...
  async loadLineCharts(
    ...
    if (this._timeFieldName !== '') {
      ...
      const resp = await this.newJobLineChart(
  ...
  async loadPopulationCharts(
    ...
    if (this._timeFieldName !== '') {
      ...
      const resp = await this.newJobPopulationsChart(
        this._indexPatternTitle,
        this._timeFieldName,
        start,
        end,
  ...
  async loadEventRateChart(
    ...
    if (this._timeFieldName !== '') {
      const resp = await this.getEventRateData(
        this._indexPatternTitle,
        this._query,
        this._timeFieldName,
        start,
  ...
  async loadFieldExampleValues(
    ...
    const { results } = await this.getCategoryFields(
      this._indexPatternTitle,
      field.name,
      10,
      this._query,
