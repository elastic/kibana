...
export const fieldTypeAssociatedAriaLabels = {
  ML_JOB_FIELD_TYPES: {
...
export const getFieldTypeAssociatedAriaLabel = (storage: string, keyword: string) => {
  const correctConstant = isKeywordFieldType(storage, keyword, true);
  if (!correctConstant) {
    return null;
  }
  return fieldTypeAssociatedAriaLabels[storage][correctConstant] || null;
};

export const isKeywordFieldType: boolean | void = (
  storage: string,
  keyword: string,
  getConstant: boolean = false
) => {
  const filtered = Object.keys(types[storage])
    .map(constantName => ({
      constantName,
      matched: !!(types[storage][constantName] === keyword),
    }))
    .filter(item => item.matched);
  const filteredConstant = filtered.length ? filtered[0].constantName : null;
  return getConstant ? filteredConstant : !!filtered.length;
};
