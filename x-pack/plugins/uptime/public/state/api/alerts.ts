...
export const createAlert = async ({
  ...
  defaultActions.forEach((aId) => {
    actions.push({
      ...
      params: {
        message:
          '{{state.monitor}} with url {{{state.monitorUrl}}} is down from {{state.observerLocation}}. The latest error message: {{state.latestErrorMessage}}',
      },
    });
  });
  ...
  const data = {
    ...
    tags: [monitorId, UPTIME_AUTO_ALERT],
