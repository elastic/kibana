...
import { commonStateTranslations, commonMonitorStateI18 } from './translations';
...
export const statusCheckAlertFactory: UptimeAlertTypeFactory = (_server, libs) => ({
  ...
  async executor(options: AlertExecutorOptions) {
    ...
    if (monitorsByLocation.length || availabilityResults.length) {
      if (rawParams.isAutoGenerated) {
        for (const monitorLoc of monitorsByLocation) {
          ...
          const alertInstance = options.services.alertInstanceFactory(
            MONITOR_STATUS.id + monitorLoc.location
          );
