...
export const getStatusMessage = (
  ...
  let statusMessage = '';
  if (downMonInfo) {
    console.log(downMonInfo, '!!!!!!!!!!downMonInfooooooooo');
    statusMessage = DOWN_LABEL('test', downMonInfo.monitorCount || 0, 30, 'days', 1);
  }
  ...
  async executor({
    ...
    if (isAutoGenerated) {
      for (const monitorLoc of downMonitorsByLocation) {
        const monitorCount = monitorLoc?.count;
        const monitorInfo = {
          ...monitorLoc.monitorInfo,
          monitorCount,
        };
    ...
    mergedIdsByLoc.forEach((monIdByLoc) => {
      ...
      const statusMessage = getStatusMessage(
        { ...downMonInfo!, monitorCount: downMonCount },
        availMonInfo!,
        availability
      );
      const monitorSummary = getMonitorSummary(monitorInfo, statusMessage);
