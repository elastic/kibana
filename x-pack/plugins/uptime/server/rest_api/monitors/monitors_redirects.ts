...
export const createGetMonitorRedirectsRoute: UMRestApiRouteFactory = (libs: UMServerLibs) => ({
  method: 'GET',
  path: API_URLS.MONITOR_REDIRECTS,
  validate: {
    query: schema.object({
  ...
  handler: async ({ callES, dynamicSettings }, _context, request, response): Promise<any> => {
    ...
    return response.ok({
      body: {
        ...(await libs.requests.getMonitorRedirects({
          callES,
          dynamicSettings,
          monitorId,
          dateStart,
