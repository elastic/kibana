...
MigrationApis.getUpgradeCheckupStatus = jest.fn();
...
describe('reindex template API', () => {
  ...
  describe('GET /api/upgrade_checkup/reindex/{indexName}.json', () => {
    it('returns a template', async () => {
      MigrationApis.getUpgradeCheckupStatus.mockResolvedValue({
        cluster: [],
        indices: [],
        nodes: [],
      });
      const resp = await server.inject({
        method: 'GET',
        url: '/api/upgrade_checkup/status',
      });
    ...
    it('returns an 403 error if it throws forbidden', async () => {
      ...
      MigrationApis.getUpgradeCheckupStatus.mockRejectedValue(e);
      const resp = await server.inject({
        method: 'GET',
        url: '/api/upgrade_checkup/status',
      });
    ...
    it('returns an 500 error if it throws', async () => {
      MigrationApis.getUpgradeCheckupStatus.mockRejectedValue(new Error(`scary error!`));
      const resp = await server.inject({
        method: 'GET',
        url: '/api/upgrade_checkup/status',
      });
