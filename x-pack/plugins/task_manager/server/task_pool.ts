...
export class TaskPool {
  ...
  /**
   * Gets # of tasks to fill available workers and queue.
   */
  public get numOfTasksToClaim() {
    ...
    return this.maxQueuedTasks - this.tasksInQueue.length + this.availableWorkers;
  }
  ...
  /**
   * For each available worker, take a task from the queue and occupy a worker.
   */
  private fillAvailableWorkersFromQueue() {
    // Move tasks from tasksInQueue to tasksRunning synchronously
    const tasksToRun =
    ...
    // Mark them as running and start running them asynchronously
    (async () => {
      await Promise.all(
  ...
  /**
   * Resets the tasksInQueue object based on when a task is scheduled to run
   */
  private resortQueue() {
    this.tasksInQueue = this.tasksInQueue.sort(
  ...
  public run = async (tasks: TaskRunner[]): Promise<TaskPoolRunResult> => {
    if (tasks.length) {
      tasks
        // Tasks in queue will keep showing up until they run, let's filter them out if the task is the same
        .filter(
          (taskRunner) =>
      ...
      this.resortQueue();
      this.fillAvailableWorkersFromQueue();
    }
