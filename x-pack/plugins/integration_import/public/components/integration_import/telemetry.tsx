...
import type {
  ...
  IntegrationSettings,
} from './integration_auto_import/types';
...
type ReportImportOpen = () => void;
type ReportAutoImportStepComplete = (params: { step: number }) => void;
type ReportAutoImportGenerationComplete = (params: {
  connector: AIConnector;
  integrationSettings: IntegrationSettings;
  durationMs: number;
  error?: string;
}) => void;
type ReportAutoImportComplete = (params: {
  integrationName: string;
  integrationSettings: IntegrationSettings;
  connector: AIConnector;
  error?: string;
...
interface TelemetryContextProps {
  ...
  reportAutoImportOpen: ReportImportOpen;
  reportAutoImportStepComplete: ReportAutoImportStepComplete;
  reportAutoImportGenerationComplete: ReportAutoImportGenerationComplete;
  reportAutoImportComplete: ReportAutoImportComplete;
}
...
export const TelemetryContextProvider = React.memo<PropsWithChildren<{}>>(({ children }) => {
  ...
  const reportImportOpen = useCallback<ReportImportOpen>(() => {
    const sessionId = uuidV4();
    sessionData.current = { sessionId, startedAt: Date.now() };
    stepsData.current = { startedAt: Date.now() };
    telemetry.reportEvent(TelemetryEventType.IntegrationAutoImportOpen, {
      sessionId,
    });
  }, [telemetry]);

  const reportAutoImportStepComplete = useCallback<ReportAutoImportStepComplete>(
    ({ step }) => {
      telemetry.reportEvent(TelemetryEventType.IntegrationAutoImportStepComplete, {
  ...
  const reportAutoImportGenerationComplete = useCallback<ReportAutoImportGenerationComplete>(
    ({ connector, integrationSettings, durationMs, error }) => {
      telemetry.reportEvent(TelemetryEventType.IntegrationAutoImportGenerationComplete, {
        sessionId: sessionData.current.sessionId,
  ...
  const reportAutoImportComplete = useCallback<ReportAutoImportComplete>(
    ({ integrationName, integrationSettings, connector, error }) => {
      telemetry.reportEvent(TelemetryEventType.IntegrationAutoImportComplete, {
        sessionId: sessionData.current.sessionId,
        integrationName,
  ...
  const value = useMemo<TelemetryContextProps>(
    () => ({
      ...
      reportAutoImportOpen: reportImportOpen,
      reportAutoImportStepComplete,
      reportAutoImportGenerationComplete,
      reportAutoImportComplete,
    }),
    [
      reportUploadZipIntegrationComplete,
      reportImportOpen,
