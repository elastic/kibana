...
import { IntegrationImport } from './integration_import';
...
jest.mock('./integration_import_landing', () => ({
  IntegrationImportLanding: jest.fn(() => <div data-test-subj="landingMock" />),
}));
jest.mock('./integration_import_upload', () => ({
  IntegrationImportUpload: jest.fn(() => <div data-test-subj="uploadMock" />),
}));
jest.mock('./integration_auto_import', () => ({
  IntegrationAutoImport: jest.fn(() => <div data-test-subj="autoImportMock" />),
}));
...
const getElement = () => <IntegrationImport services={mockServices} />;

describe('CreateIntegration', () => {
  describe('when url is /import', () => {
    let wrapper: React.ComponentType;
    beforeEach(() => {
      wrapper = getWrapper('/import');
    });
  ...
  describe('when url is /import/automatic', () => {
    let wrapper: React.ComponentType;

    beforeEach(() => {
      wrapper = getWrapper('/import/automatic');
    });

    it('should render the autoImport page', () => {
      const result = render(getElement(), { wrapper });
      expect(result.queryByTestId('autoImportMock')).toBeInTheDocument();
    });
    ...
    describe('and user is not authorized', () => {
      ...
      it('should render the landing page', () => {
        const result = render(getElement(), { wrapper });
        expect(result.queryByTestId('autoImportMock')).not.toBeInTheDocument();
        expect(result.queryByTestId('landingMock')).toBeInTheDocument();
      });
    });
  ...
  describe('when url is /import/upload', () => {
    let wrapper: React.ComponentType;

    beforeEach(() => {
      wrapper = getWrapper('/import/upload');
    });
  ...
  describe('when url is not exact', () => {
    ...
    beforeEach(() => {
      wrapper = getWrapper('/import/something_else');
    });
