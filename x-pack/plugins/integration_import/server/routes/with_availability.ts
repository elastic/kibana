...
import { IntegrationImportRouteHandlerContext } from '../plugin';
...
>(
  handler: RequestHandler<P, Q, B, IntegrationImportRouteHandlerContext, Method>
): RequestHandler<P, Q, B, IntegrationImportRouteHandlerContext, Method> => {
  return async (context, req, res) => {
    const { isAvailable } = await context.integrationImport;
    if (!isAvailable()) {
      return res.notFound({
