...
export const QueryBuilderMode: React.FC = () => {
  ...
  const updateFields = (index: string, fieldName: string, checked: boolean) => {
    ...
    usageTracker?.count(AnalyticsEvents.queryBuilderFieldsUpdated, currentIndexFields.length);
  };

  useEffect(() => {
    usageTracker?.load(AnalyticsEvents.queryBuilderModeLoaded);
  }, [usageTracker]);
