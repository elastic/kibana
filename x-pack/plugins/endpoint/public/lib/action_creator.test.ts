 ...
 */
import { createActions, createActionFactory } from './action_creator';

describe('action creator', () => {
  describe('#createActions', () => {
    test('it creates multiple action creators from Array', () => {
      const actions = createActions(['actionOne', 'actionTwo']);
    ...
    test('it creates multiple action creators from Set', () => {
      const actions = createActions(new Set(['actionOne', 'actionTwo']));
      expect(actions).toEqual({
        actionOne: expect.any(Function),
        actionTwo: expect.any(Function),
      });
  ...
  describe('#actionCreatorFactory', () => {
    ...
    test('action creator returns expected object', () => {
      const getAction = createActionFactory<'actionOne', PayloadWithOneArg>('actionOne');
      expect(getAction({ data: { one: 1, two: 2 } })).toEqual({
        type: 'actionOne',
        payload: [{ data: { one: 1, two: 2 } }],
      });

      const getActionWithMultiPayload = createActionFactory<'actionOne', PayloadWithTwoArgs>(
        'actionOne'
      );
