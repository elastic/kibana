...
function widthsOfProcessSubtrees(indexedProcessTree: IndexedProcessTree): ProcessWidths {
  const widths = new Map<EndpointEvent, number>();
...
function processPositions(
  ...
  const positions = new Map<EndpointEvent, Vector2>();
  /**
   ...
   * reset the counters.
   */
  let lastProcessedParentNode: EndpointEvent | undefined;
  /**
...
export const processNodePositionsAndEdgeLineSegments = createSelector(
  ...
  function processNodePositionsAndEdgeLineSegments(
    ...
    const transformedPositions = new Map<EndpointEvent, Vector2>();
