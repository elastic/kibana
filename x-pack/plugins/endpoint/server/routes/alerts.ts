...
import { AlertRequestParams, EndpointAppContext, AlertDetailsRequestParams } from '../types';
...
export function registerAlertRoutes(router: IRouter, endpointAppContext: EndpointAppContext) {
  ...
  const alertDetailHandler: RequestHandler<AlertDetailsRequestParams> = async (ctx, req, res) => {
    try {
...
function mapToAlertResultList(
  ...
  const hits: AlertDataWrapper[] = searchResponse?.hits?.hits;
