...
export const StepCreateForm: FC = () => {
  ...
  const timeFieldName = useWizardSelector(
  ...
  const started = useWizardSelector((s) => s.stepCreate.started);
  const dataViewId = useWizardSelector((s) => s.stepCreate.dataViewId);

  const { setCreated, setStarted, setDataViewId } = useWizardActions();
              ...
              {started === true && createDataView === true && dataViewId === undefined && (
                <EuiFlexItem style={PANEL_ITEM_STYLE} grow={false}>
                  <EuiPanel style={{ position: 'relative' }}>
                    ...
                    <EuiText color="subdued" size="s">
                      <p>
                        {i18n.translate('xpack.transform.stepCreateForm.creatingDataViewMessage', {
