...
export type SummarizedShareToSpaceResult =
  | SuccessfulResponse
  | UnsuccessfulResponse
  | ProcessingResponse;

export function summarizeShareResult(
  savedObject: SavedObjectsManagementRecord,
  shareResult: ProcessedImportResponse | undefined,
  includeRelated: boolean
): SummarizedShareToSpaceResult {
  const successful = Boolean(shareResult && shareResult.failedImports.length === 0);

  const conflicts = shareResult
    ? shareResult.failedImports.filter(failed => failed.error.type === 'conflict')
    : [];

  const unresolvableErrors = shareResult
    ? shareResult.failedImports.filter(failed => failed.error.type !== 'conflict')
    : [];
  ...
  const hasUnresolvableErrors = Boolean(
    shareResult && shareResult.failedImports.some(failed => failed.error.type !== 'conflict')
  );
  ...
  if (typeof shareResult === 'undefined') {
    return {
      processing: true,
