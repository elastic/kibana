...
import { SummarizedShareToSpaceResult, SummarizedSavedObjectResult } from '..';
...
export const ShareStatusIndicator = (props: Props) => {
  const { summarizedShareResult, conflictResolutionInProgress } = props;
  if (summarizedShareResult.processing || conflictResolutionInProgress) {
    return <EuiLoadingSpinner />;
  }

  const objectResult = summarizedShareResult.objects.find(
  ...
  if (successful) {
    const message = props.overwritePending ? (
      <FormattedMessage
        id="xpack.spaces.management.shareToSpace.shareStatus.pendingOverwriteMessage"
        defaultMessage="Saved object will be overwritten. Click 'Skip' to cancel this operation."
      />
    ) : (
      <FormattedMessage
        id="xpack.spaces.management.shareToSpace.shareStatus.successMessage"
      ...
      <EuiIconTip
        ...
        content={
          <EuiText>
            <p>
              <FormattedMessage
                id="xpack.spaces.management.shareToSpace.shareStatus.conflictsMessage"
            ...
            <p>
              <FormattedMessage
                id="xpack.spaces.management.shareToSpace.shareStatus.conflictsOverwriteMessage"
