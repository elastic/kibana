...
describe('#hasGlobalAllPrivilegesForObjectType', () => {
  const type = 'foo';
  ...
  describe(`authorization is null`, () => {
    test(`returns true`, async () => {
      ...
      const result = await client.hasGlobalAllPrivilegesForObjectType(type);
  ...
  describe('useRbacForRequest is true', () => {
    test(`returns false if user is not authorized to enumerate spaces`, async () => {
      ...
      const { mockAuthorization, mockCheckPrivilegesGlobally } = createMockAuthorization();
      mockAuthorization.mode.useRbacForRequest.mockReturnValue(true);
      mockCheckPrivilegesGlobally.mockReturnValue({ hasAllRequested: false });
      const request = Symbol() as any;
      ...
      expect(mockCheckPrivilegesGlobally).toHaveBeenCalledWith({
        kibana: [
          mockAuthorization.actions.savedObject.get(type, 'create'),
          mockAuthorization.actions.savedObject.get(type, 'get'),
          mockAuthorization.actions.savedObject.get(type, 'update'),
          mockAuthorization.actions.savedObject.get(type, 'delete'),
        ],
      });
    });
    ...
    test(`returns true if user is authorized to enumerate spaces`, async () => {
      ...
      const { mockAuthorization, mockCheckPrivilegesGlobally } = createMockAuthorization();
      mockAuthorization.mode.useRbacForRequest.mockReturnValue(true);
      mockCheckPrivilegesGlobally.mockReturnValue({ hasAllRequested: true });
      const request = Symbol() as any;
