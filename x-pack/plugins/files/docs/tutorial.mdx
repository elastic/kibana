...
Consumers of the file service are able to decide the shape of their metadata. The file service provides the ability to search custom data which is useful for tagging and filtering your files &mdash; all metadata must be serializable to JSON.

_File metadata should be treated as external state that needs to be migrated_. Plugins should always be compatible with past version of their own custom metadata. Learn more about how to specify custom metadata below.
...
<DocCallOut title="File contents are immutable">
  ...
  If you need to update the contents of a file, you should create a new file with the new contents.
</DocCallOut>
