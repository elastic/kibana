...
import { DATA_QUALITY_ID } from '../constants';
...
export async function fetchEsQuery({
  ...
  logger.debug(
    `es query rule ${DATA_QUALITY_ID}:${ruleId} "${name}" query - ${JSON.stringify(sortedQuery)}`
  );

  const { body: searchResult } = await esClient.search(sortedQuery, { meta: true });

  logger.debug(
    ` es query rule ${DATA_QUALITY_ID}:${ruleId} "${name}" result - ${JSON.stringify(searchResult)}`
  );
