...
import type { SearchRequest, WriteResponseBase } from '@elastic/elasticsearch/lib/api/types';
...
import { ScoreTaskResult, ScoreByPolicyTemplateBucket, VulnSeverityAggs } from './types';
import {
...
export async function scheduleScoreStatsTask(
  taskManager: TaskManagerStartContract,
  logger: Logger
) {
...
const aggregateLatestVulnerabilities = async (esClient: ElasticsearchClient) => {
  ...
  return vulnStatsTrendDocIndexingPromises;
};

const aggregateEnabledMisconfigurations = async (
  esClient: ElasticsearchClient,
  ...
  return findingsCustomScoresDocIndexingPromises;
};

const aggregateLatestMisconfigurations = async (esClient: ElasticsearchClient) => {
  ...
  return findingsFullScoresDocIndexingPromises;
};

const doesIndexHaveData = async (esClient: ElasticsearchClient, index: string) => {
...
export const aggregateLatestFindings = async (
  ...
  try {
    let findingsFullScoresDocIndexingPromises;
    let vulnStatsTrendDocIndexingPromises;
    let findingsCustomScoresDocIndexingPromises: Array<Promise<WriteResponseBase>> = [];
    const startAggTime = performance.now();

    const hasFindingsData = await doesIndexHaveData(esClient, LATEST_FINDINGS_INDEX_DEFAULT_NS);

    if (hasFindingsData) {
      findingsFullScoresDocIndexingPromises = await aggregateLatestMisconfigurations(esClient);
      findingsCustomScoresDocIndexingPromises = await aggregateEnabledMisconfigurations(
        esClient,
        encryptedSoClient
      );
    }

    const hasVulnData = await doesIndexHaveData(esClient, LATEST_VULNERABILITIES_INDEX_DEFAULT_NS);
    if (hasVulnData)
      vulnStatsTrendDocIndexingPromises = await aggregateLatestVulnerabilities(esClient);
...
export function taskRunner(coreStartServices: CspServerPluginStartServices, logger: Logger) {
  return ({ taskInstance }: RunContext) => {
    ...
    return {
      async run(): Promise<ScoreTaskResult> {
        try {
...
export function setupScoreTask(
