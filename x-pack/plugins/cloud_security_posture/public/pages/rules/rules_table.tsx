...
export const RulesTable = ({
  ...
  const euiPagination: EuiBasicTableProps<CspRule>['pagination'] = {
    pageIndex: page,
    pageSize,
  ...
  const onTableChange = ({ page: pagination }: Criteria<CspRule>) => {
    if (!pagination) return;
    setPagination({ page: pagination.index, perPage: pagination.size });
  };

  const rowProps = (row: CspRule) => ({
    ['data-test-subj']: TEST_SUBJECTS.getCspRuleTableRowItemTestId(row.metadata.id),
    style: {
...
const getColumns = ({
  setSelectedRuleId,
}: GetColumnProps): Array<EuiTableFieldDataColumnType<CspRule>> => [
  {
