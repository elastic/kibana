...
describe('Reporting server createConfig$', () => {
  ...
  it('creates random encryption key and default config using host, protocol, and port from server info', async () => {
    ...
    expect((mockLogger.warn as any).mock.calls[0]).toMatchObject([
      'Generating a random key for xpack.reporting.encryptionKey. To prevent sessions from being invalidated on restart, please set xpack.reporting.encryptionKey in kibana.yml in kibana.yml manually or by using bin/kibana-encryption-key',
    ]);
  });
