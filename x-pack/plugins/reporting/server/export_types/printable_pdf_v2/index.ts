    ...
    ExportType<PdfExportTypeSetupDeps, PdfExportTypeStartDeps, JobParamsPDFV2, TaskPayloadPDFV2>
{
  ...
  private async getSpaceId(request: KibanaRequest, logger = this.logger) {
  ...
  private async getUiSettingsServiceFactory(savedObjectsClient: SavedObjectsClientContract) {
    const { uiSettings: uiSettingsService } = this.startDeps;
    const scopedUiSettingsService = uiSettingsService.asScopedToClient(savedObjectsClient);
    return scopedUiSettingsService;
  }

  private async getUiSettingsClient(request: KibanaRequest, logger = this.logger) {
    const spacesService = this.setupDeps.spaces?.spacesService;
   ...
   */
  private getServerInfo(): ReportingServerInfo {
