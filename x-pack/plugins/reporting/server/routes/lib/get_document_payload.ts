...
const getTitle = (exportType: PdfExportType, title?: string): string =>
  `${title || DEFAULT_TITLE}.${exportType.jobContentExtension}`;

const getReportingHeaders = (output: TaskRunResult, exportType: PdfExportType) => {
...
export function getDocumentPayloadFactory(reporting: ReportingCore) {
  ...
  async function getCompleted({
    ...
    const exportType = exportTypesRegistry.get((item: PdfExportType) => item.jobType === jobType);
