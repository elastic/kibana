/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

import { schema, TypeOf } from '@kbn/config-schema';
import { ReportTaskParams, ScheduledReportTaskParams } from '../';

export { getNextRun } from './get_next_run';

export type ScheduleIntervalSchemaType = TypeOf<typeof ScheduleIntervalSchema>;

export const ScheduleIntervalSchema = schema.oneOf([
  schema.object({ minutes: schema.number({ min: 1, max: 59 }) }),
  schema.object({ hours: schema.number({ min: 1, max: 23 }) }),
  schema.object({ days: schema.number({ min: 1, max: 31 }) }),
]);

export const ScheduleReportByIdSchema = schema.object({
  report_id: schema.string(),
  schedule: ScheduleIntervalSchema,
  api_key: schema.object({
    name: schema.string(),
    base64: schema.string(),
  }),
});

export function isScheduled(
  task: ReportTaskParams | ScheduledReportTaskParams
): task is ScheduledReportTaskParams {
  return (
    (task as ReportTaskParams).id == null && (task as ScheduledReportTaskParams).schedule != null
  );
}
