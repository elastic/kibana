...
function reportFromTask(task: ReportTaskParams | ScheduledReportTaskParams) {
      ...
      return {
        ...
        run: async () => {
          ...
          if (isScheduled(params)) {
            ...
            nextRun = getNextRun(params.schedule);
          } else {
            task = params;
          }
          ...
          try {
            ...
            report = await this._claimJob(task); // FIXME: defineTask() should define a `processing` task, so we could skip this if we called this.defineTask
            jobId = report._id;
          ...
          try {
            ...
            if (nextRun) {
              this.logger.info(`Scheduling the next recurrence....`);
              const next = await this.scheduleTask(params, taskInstance.scheduledAt, nextRun); // FIXME: preserve the task.scheduledAt, should be the same for all reports created under this schedule
              this.logger.info(
                `Scheduled [${next.taskType}] task [${next.id}] ` +
                  `to run at [${next.runAt.toISOString()}]`
              );
            }
