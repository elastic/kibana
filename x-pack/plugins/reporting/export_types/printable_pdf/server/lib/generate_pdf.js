...
function generatePdfObservableFn(server) {
  ...
  const urlScreenshotsObservable = (urls, headers, layout, browserTimezone, cancellationToken) => {
    return Rx.from(urls).pipe(
      mergeMap(url => screenshotsObservable(url, headers, layout, browserTimezone, cancellationToken),
        (outer, inner) => inner,
        captureConcurrency
      )
    );
  ...
  return function generatePdfObservable(title, urls, browserTimezone, headers, layoutParams, logo, cancellationToken) {

    const layout = createLayout(server, layoutParams);

    const screenshots$ = urlScreenshotsObservable(urls, headers, layout, browserTimezone, cancellationToken);
