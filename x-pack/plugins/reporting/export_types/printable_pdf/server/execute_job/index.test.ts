...
describe('headers', () => {
  test(`fails if it can't decrypt headers`, async () => {
    ...
    await expect(
      executeJob({ objects: [], timeRange: {} }, cancellationToken)
    ).rejects.toBeDefined();
  });
  ...
  describe('conditions', () => {
    test(`uses hostname from reporting config if set`, async () => {
      ...
      expect(mockServer.uiSettingsServiceFactory().get).toBeCalledWith(
...
test('uses basePath from job when creating saved object service', async () => {
  ...
  await executeJob(
    { objects: [], headers: encryptedHeaders, basePath: jobBasePath },
    cancellationToken
  );
...
test(`adds forceNow to hash's query, if it exists`, async () => {
  ...
  await executeJob(
    { objects: [{ relativeUrl: '/app/kibana#/something' }], forceNow, headers: encryptedHeaders },
    cancellationToken
  );
...
test(`doesn't append forceNow query to url, if it doesn't exists`, async () => {
  ...
  await executeJob(
    { objects: [{ relativeUrl: '/app/kibana#/something' }], headers: encryptedHeaders },
    cancellationToken
  );
