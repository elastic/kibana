...
export const openAddFilterPopover = () => {
  ...
  cy.get(GLOBAL_SEARCH_BAR_ADD_FILTER).should('be.visible').click();
};

export const fillAddFilterForm = ({ key, value }: SearchBarFilter) => {
  cy.get(ADD_FILTER_FORM_FIELD_INPUT).should('exist');
  cy.get(ADD_FILTER_FORM_FIELD_INPUT).should('be.visible').type(key).click();
  ...
  cy.get(ADD_FILTER_FORM_SAVE_BUTTON).should('be.enabled').click();
