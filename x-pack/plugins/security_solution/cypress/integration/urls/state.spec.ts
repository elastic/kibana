...
describe('url state', () => {
  ...
  describe('filters parameter', () => {
    it('sets the url state when filters are set', () => {
      loginAndWaitForPageWithoutDateRange(ABSOLUTE_DATE_RANGE.url);
      openAddFilterPopover();
      fillAddFilterForm({ key: 'host.ip', value: '1.1.1.1' });

      cy.url().should(
        'include',
        'filters=!((%27$state%27:(store:appState),meta:(alias:!n,disabled:!f,key:host.ip,negate:!f,params:(query:%271.1.1.1%27),type:phrase),query:(match_phrase:(host.ip:%271.1.1.1%27))))'
      );
    });

    it('reads the url state for filters', () => {
      loginAndWaitForPageWithoutDateRange(ABSOLUTE_DATE_RANGE.urlHostWithSearchFilter);
      cy.get(GLOBAL_SEARCH_BAR_FILTER_ITEM).should('have.text', 'host.ip: 0.0.0.0');
    });
  });
  ...
  describe('savedQuery parameter', () => {
    it('should write and read url state for the saved query', () => {
      ...
      cy.wait('@savedQuery').then(({ response }) => {
        ...
        cy.get(KQL_INPUT).should('have.text', 'host.ip: "1.1.1.1"');
      });
    });
  });
