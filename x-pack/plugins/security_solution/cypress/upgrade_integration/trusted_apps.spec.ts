...
import { TRUSTED_APPS_URL } from '../urls/navigation';

describe('Check trusted apps after upgrade', () => {
  before(() => {
    loginAndWaitForPageWithoutDateRange(TRUSTED_APPS_URL);
  });
  ...
  it('Displays trusted apps list with a result from automation', () => {
    cy.get(TRUSTED_APPLICATIONS_LIST_COUNTER).should('have.text', 'Showing 1 trusted application');
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_TITLE)
      .first()
      .should('have.text', 'Trusted app from automation');
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_CRITERIA_CONDITION)
      .first()
      .should('have.text', 'AND process.executable.caselessIS testPath');
  });

  it('Assigns trusted app to a policy', () => {
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_MENU_BUTTON).click();
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_EDIT_OPTION).click();
    cy.get(TRUSTED_APPLICATIONS_FLYOUT_BY_POLICY_OPTION).click();
    cy.get(TRUSTED_APPLICATIONS_FLYOUT_BY_POLICY_OPTIONS_LIST_BOX)
      .contains('Endpoint integration')
      .click();
    cy.get(TRUSTED_APPLICATIONS_FLYOUT_SAVE_BUTTON).click();
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_EFFECT_SCOPE_MENU_BUTTON).click();
    cy.get('div.euiProgress', { timeout: 12000 }).should('not.exist');
    cy.get(TRUSTED_APPLICATIONS_LIST_ITEM_EFFECT_SCOPE_MENU)
      .contains('Endpoint integration')
