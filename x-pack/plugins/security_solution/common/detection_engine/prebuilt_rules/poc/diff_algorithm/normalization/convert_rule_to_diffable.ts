...
import { convertLegacyVersionToSemantic } from '../../content_model/semantic_version';
import type {
 ...
 */
export const convertRuleToDiffable = (rule: RuleResponse | PrebuiltRuleContent): DiffableRule => {
...
const extractDiffableCommonFields = (
  ...
  return {
    ...
    rule_content_version: extractDiffableRuleContentVersion(rule),
    stack_version_min: extractDiffableStackVersionMin(rule),
    stack_version_max: extractDiffableStackVersionMax(rule),
    meta: rule.meta ?? {},
...
const extractDiffableRuleContentVersion = (
  rule: RuleResponse | PrebuiltRuleContent
): SemanticVersion => {
  if ('rule_content_version' in rule) {
    return rule.rule_content_version;
  }
  if ('version' in rule) {
    return convertLegacyVersionToSemantic(rule.version);
  }
  return '';
};

const extractDiffableStackVersionMin = (
  rule: RuleResponse | PrebuiltRuleContent
): SemanticVersion => {
  if ('stack_version_min' in rule) {
    return rule.stack_version_min;
  }
  return '';
};

const extractDiffableStackVersionMax = (
  rule: RuleResponse | PrebuiltRuleContent
): SemanticVersion => {
  if ('stack_version_max' in rule) {
    return rule.stack_version_max;
  }
  return '';
};
