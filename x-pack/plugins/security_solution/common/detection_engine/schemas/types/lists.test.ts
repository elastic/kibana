...
describe('Lists', () => {
  ...
  describe('listArray', () => {
    ...
    test('it should not validate when unexpected type found in array', () => {
      ...
      expect(getPaths(left(message.errors))).toEqual([
        'Invalid value "1" supplied to "Array<{| id: NonEmptyString, list_id: NonEmptyString, type: "detection" | "endpoint" | "events", namespace_type: "agnostic" | "single" |}>"',
      ]);
      expect(message.schema).toEqual({});
    });
  });
  ...
  describe('listArrayOrUndefined', () => {
    ...
    test('it should not allow an item that is not of type "list" in array', () => {
      ...
      expect(getPaths(left(message.errors))).toEqual([
        'Invalid value "1" supplied to "(Array<{| id: NonEmptyString, list_id: NonEmptyString, type: "detection" | "endpoint" | "events", namespace_type: "agnostic" | "single" |}> | undefined)"',
        'Invalid value "[1]" supplied to "(Array<{| id: NonEmptyString, list_id: NonEmptyString, type: "detection" | "endpoint" | "events", namespace_type: "agnostic" | "single" |}> | undefined)"',
      ]);
      expect(message.schema).toEqual({});
    });
