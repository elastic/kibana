...
describe('Rule response schema', () => {
  ...
  describe('investigation_fields', () => {
    test('it should validate rule with "investigation_fields"', () => {
      const payload = getRulesSchemaMock();
      payload.investigation_fields = { fields: ['foo', 'bar'] };
    ...
    test('it should NOT validate an empty array for "investigation_fields.fields"', () => {
      const payload: RuleResponse = {
        ...getRulesSchemaMock(),
      ...
      expect(getPaths(left(message.errors))).toEqual([
        'Invalid value "[]" supplied to "investigation_fields"',
      ]);
      expect(message.schema).toEqual({});
    });
