...
import {
  ...
  getDataFromFieldsHitsSafety,
} from './helpers';

describe('Events Details Helpers', () => {
  it('#getDataFromFieldsHits', () => {
    ...
    const result = getDataFromFieldsHits(response.fields);
  ...
  it('#getDataFromFieldsHitsSafety', async () => {
    ...
    const result = await getDataFromFieldsHitsSafety(response.fields);
    expect(result).toEqual([
      {
        category: 'event',
        field: 'event.category',
