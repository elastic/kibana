...
export class CrowdstrikeActionsClient extends ResponseActionsClientImpl {
  ...
  private async sendAction(
    ...
    return Promise.resolve(actionSendResponse);
  }
  ...
  // actionResponse?.data.combined?.resources[doc.agent.id].stdout
  private async completeCrowdstrikeAction(
    actionResponse: Array<ActionTypeExecutorResult<CrowdstrikeBaseApiResponse>>,
    doc: LogsEndpointAction
  ): Promise<void> {
    const stdout = actionResponse?.data.map((res) => res.resources[0].stdout).join('\n');
    const stderr = actionResponse?.data.map((res) => res.resources[0].stderr).join('\n');
    const options = {
    ...
    return this.writeActionResponseToEndpointIndex(options);
  }
  ...
  private async completeInitAction(
    ...
    const generateStdout = (scripts: Script[]): string => {
      return scripts
        .map((script) => {
          const { command, description } = script;

          return `${command.replace('\n', '').padEnd(30)}${description}`;
        })
        .join('\n');
    };
  ...
  async shell(
    ...
    // };
    // return result;
    const fetchedDetails = await this.fetchActionDetails(
