...
describe('TotalsPagination', () => {
  ...
  describe('cursor', () => {
    ...
    it('does not build a cursor when all events are present', () => {
      expect(TotalsPaginationBuilder.buildCursor(0, events)).toBeNull();
    });

    it('creates a cursor when not all events are present', () => {
      expect(TotalsPaginationBuilder.buildCursor(events.length + 1, events)).not.toBeNull();
    });

    it('creates a cursor with the right information', () => {
      const cursor = TotalsPaginationBuilder.buildCursor(events.length + 1, events);
      expect(cursor).not.toBeNull();
      // we are guaranteed that the cursor won't be null from the check above
      const builder = TotalsPaginationBuilder.createBuilder(0, cursor!);
  ...
  describe('pagination builder', () => {
    it('does not include the search after information when no cursor is provided', () => {
      const builder = TotalsPaginationBuilder.createBuilder(100);
    ...
    it('returns no results when the aggregation does not exist in the response', () => {
      expect(TotalsPaginationBuilder.getTotals()).toStrictEqual({});
    });
    ...
    it('constructs the totals from the aggregation results', () => {
      ...
      expect(TotalsPaginationBuilder.getTotals(agg)).toStrictEqual({ awesome: 5, soup: 1 });
    });
  });
});
