...
export function registerHostIsolationRoutes(
  ...
  router.post(
    {
      path: ENDPOINT_RESPONSE_ACTION_ROUTE,
      validate: EndpointResponseActionRequestSchema,
      options: { authRequired: true, tags: ['access:securitySolution'] },
    },
    withEndpointAuthz(
      { all: ['canIsolateHost'] },
      logger,
      endpointResponseActionRequestHandler(endpointContext)
    )
  );
}
...
export const endpointResponseActionRequestHandler = function (
  ...
  unknown,
  unknown,
  TypeOf<typeof EndpointResponseActionRequestSchema.body>,
  SecuritySolutionRequestHandlerContext
> {
  return async (context, req, res) => {
    endpointContext.service.getFeatureUsageService().notifyUsage('RESPONSE_ACTIONS');
