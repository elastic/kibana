...
export const savedObjectsToCreate = [
  {
    attributes: {
      fieldAttrs: '{}',
      fields: '[]',
...
export const createDashboards = async ({ request }: { request: FrameworkRequest }) => {
  const savedObjectsClient = request.context.core.savedObjects.client;

  const createSO = await savedObjectsClient.bulkCreate(savedObjectsToCreate, {
    overwrite: true,
  });
