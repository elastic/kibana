...
import { InstallMigrationRulesRequestBody } from '../../../../../../common/siem_migrations/model/api/rules/rule_migration.gen';
import type { SecuritySolutionPluginRouter } from '../../../../../types';
import { withLicense } from '../util/with_license';
import { installRules } from './helpers';

export const registerInstallMigrationRulesRoute = (
      ...
      withLicense(
        async (context, req, res): Promise<IKibanaResponse<InstallMigrationRulesResponse>> => {
          ...
          try {
            ...
            const rulesToInstall = migrationRules.filter(
              (rule) => !rule.elastic_rule.id && canInstallMigrationRule(rule)
            );
