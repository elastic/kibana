...
import {cloudOnlyLogger, getPreviousDiagTaskTimestamp } from '../helpers';
  ...
  return {
    ...
    runTask: async (
      ...
      if (!Array.isArray(hits) || !hits.length) {
        logger.info('no diagnostic alerts retrieved');
        return 0;
      }
      logger.info(`Received ${hits.length} diagnostic alerts`);
