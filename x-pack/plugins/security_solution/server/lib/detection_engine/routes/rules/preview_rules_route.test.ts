...
import { previewRulesRoute } from './preview_rules_route';
import { getPreviewRulesSchemaMock } from '../../../../../common/detection_engine/schemas/request/rule_schemas.mock';
// eslint-disable-next-line @kbn/eslint/no-restricted-paths
import { elasticsearchClientMock } from 'src/core/server/elasticsearch/client/mocks';
import { getQueryRuleParams } from '../../schemas/rule_schemas.mock';
import { PreviewRuleOptions } from '../../rule_types/types';
import { createRuleTypeMocks } from '../../rule_types/__mocks__/rule_type';
...
describe.each([
  ...
  beforeEach(() => {
    ...
    previewRulesRoute(server.router, ml, mockPreviewRuleOptions);
  });

  describe('status codes with actionClient and alertClient', () => {
    test('returns 200 when creating a single rule with a valid actionClient and alertClient', async () => {
      const response = await server.inject(getPreviewRequest(), context);
    ...
    test('returns 404 if siem client is unavailable', async () => {
      ...
      // @ts-expect-error
      const response = await server.inject(getPreviewRequest(), contextWithoutSecuritySolution);
      expect(response.status).toEqual(404);
      expect(response.body).toEqual({ message: 'Not Found', status_code: 404 });
    });
  ...
  describe('request validation', () => {
    ...
    test('allows rule type of query and custom from and interval', async () => {
      const request = requestMock.create({
        method: 'post',
        path: DETECTION_ENGINE_RULES_URL,
        body: { from: 'now-7m', interval: '5m', ...getPreviewRulesSchemaMock() },
      });
      const result = server.validate(request);
    ...
    test('disallows invalid invocation count', async () => {
      const request = requestMock.create({
        method: 'post',
        path: DETECTION_ENGINE_RULES_URL,
        body: {
