...
export const createRulesBulkRoute = (
  ...
  const handleRequestFactory = <TSchema extends CreateRulesSchema>() => async (
    ...
    request: KibanaRequest<{}, {}, CreateRulesBulkSchema>,
    ...
    const rules = await Promise.all(
      ruleDefinitions
        ...
        .map(async (payloadRule) => {
          ...
          const internalRule = convertCreateAPIToInternalSchema<CreateRulesSchema>( // TODO: support RAC
            payloadRule,
            siemClient,
            isRuleRegistryEnabled
          );
          ...
          try {
            ...
            return transformValidateBulkError(internalRule.params.ruleId, createdRule, ruleActions);
          } catch (err) {
