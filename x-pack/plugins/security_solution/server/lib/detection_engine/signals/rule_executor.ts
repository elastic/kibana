...
import { RuleAlertAttributes, EqlSignalSearchResponse, WrappedSignalHit } from './types';
import {
  getGapBetweenRuns,
  getListsClient,
  getExceptions,
...
type RuleExecutorParams = AlertExecutorOptions<
  RuleTypeParams,
  AlertTypeState,
  AlertInstanceState,
  AlertInstanceContext,
...
export const ruleExecutor = async ({
  previousStartedAt,
  startedAt,
  alertId,
  services,
  ...
  lists,
  eventsTelemetry,
}: RuleExecutorParams): Promise<AlertTypeState | void> => {
  const {
    anomalyThreshold,
    from,
    ruleId,
  ...
  const savedObject = await services.savedObjectsClient.get<RuleAlertAttributes>('alert', alertId);
  const {
    actions,
    name,
    tags,
