...
type LegacyMigrateByRulesClientParams = Omit<LegacyMigrateParams, 'rulesClient'> & {
  update: RulesClient['update'];
  find: RulesClient['find'];
  delete: RulesClient['delete'];
};

export const legacyMigrateByRulesClient = async ({
  rule,
  savedObjectsClient,
  find,
  update,
  delete: deleteRule,
}: LegacyMigrateByRulesClientParams): Promise<void> =>
  withSecuritySpan('legacyMigrate', async () => {
    if (rule == null || rule.id == null) {
      return;
    }
    ...
    // migration and cleanup are needed
    if (siemNotificationsExist) {
      await deleteRule({ id: siemNotification.data[0].id });
    }
    // If legacy notification sidecar ("siem-detection-engine-rule-actions")
    // exist, migration and cleanup are needed
    if (legacyRuleNotificationSOsExist) {
...
//  * and returns necessary pieces for the updated rule
//  */
// export const legacyMigrateForRulesClient = async ({
//   savedObjectsClient,
//   rule,
// }: {
//   rule: SanitizedRule;
//   savedObjectsClient: SavedObjectsClientContract;
// }): Promise<SanitizedRule | null | undefined> =>
//   withSecuritySpan('legacyMigrate', async () => {
//     if (rule == null || rule.id == null) {
//       return rule;
//     }
...
//     // find it using the references array, not params.ruleAlertId
//     const [siemNotification, legacyRuleActionsSO] = await Promise.all([
//       savedObjectsClient.find<RuleAlertType>({
//         type: 'alert',
//         filter: 'alert.alertTypeId:(siem.notifications)',
//         hasReference: {
//           type: 'alert',
//           id: rule.id,
//         },
...
//     const siemNotificationsExist =
//       siemNotification != null && siemNotification.saved_objects.length > 0;
//     const legacyRuleNotificationSOsExist =
//       legacyRuleActionsSO != null && legacyRuleActionsSO.saved_objects.length > 0;
...
//     // migration and cleanup are needed
//     if (siemNotificationsExist) {
//       await savedObjectsClient.delete('alert', siemNotification.saved_objects[0].id);
//     }
//     // If legacy notification sidecar ("siem-detection-engine-rule-actions")
//     // exist, migration and cleanup are needed
//     if (legacyRuleNotificationSOsExist) {
