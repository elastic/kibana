...
describe('AssetCriticalityDataClient', () => {
  ...
  describe('#isIndexMappingsOutdated()', () => {
    let esClientMock: MockInternalEsClient;
    let loggerMock: ReturnType<typeof loggingSystemMock.createLogger>;
    let subject: AssetCriticalityDataClient;
    ...
    it('returns true if index mappings are missing', async () => {
      esClientMock.indices.getMapping.mockResolvedValue({});

      const result = await subject.isIndexMappingsOutdated();

      expect(result).toBe(true);
    });

    it('returns true if index mappings are outdated', async () => {
      esClientMock.indices.getMapping.mockResolvedValue({
    ...
    it('returns false if index mappings are up to date', async () => {
      esClientMock.indices.getMapping.mockResolvedValue({
        '.asset-criticality.asset-criticality-default': {
          mappings: {
            properties: {
