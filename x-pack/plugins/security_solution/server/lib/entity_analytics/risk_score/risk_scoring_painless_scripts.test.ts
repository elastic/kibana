...
describe('risk_scoring_painless_scripts', () => {
  // the elasrticsearch team have made an exception for our scripted_metric
  // aggregation but we must keep the scripts the same
  test('The scripts should not have changed', () => {
    expect(RISK_SCORING_INIT_SCRIPT).toMatchInlineSnapshot(`
      "
      state.inputs = []
      "
    `);
    expect(RISK_SCORING_MAP_SCRIPT).toMatchInlineSnapshot(`
      "
      ...
      "
    `);
    expect(RISK_SCORING_COMBINE_SCRIPT).toMatchInlineSnapshot(`
      "
      return state;
      "
    `);
    expect(RISK_SCORING_REDUCE_SCRIPT).toMatchInlineSnapshot(`
      "
