  ...
  .case(updateTableActivePage, (state, { activePage, uebaType, tableType }) => ({
    ...
    [uebaType]: {
      ...
      queries: {
        ...state[uebaType].queries,
        [tableType]: {
          // TO DO Steph/ueba fix active page/limit on ueba tables. is broken because
          // @ts-ignore
  ...
  .case(updateTableLimit, (state, { limit, uebaType, tableType }) => ({
    ...
    [uebaType]: {
      ...
      queries: {
        ...state[uebaType].queries,
        [tableType]: {
          // TO DO Steph/ueba fix active page/limit on ueba tables. is broken because multiple UebaTableType.userRules tables
          // @ts-ignore
          ...state[uebaType].queries[tableType],
          limit,
        },
