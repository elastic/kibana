...
import { useExceptionsAndValueListsCapability } from '../use_exceptions_and_value_lists_capability';
...
export const useListDetailsView = (exceptionListId: string) => {
  ...
  return {
    ...
    invalidListId,
    isReadOnly: !!(
      (!canUserCRUD && canUserREAD) ||
      (!canWriteExceptionsAndValueLists && canAccessExceptionsAndValueLists)
    ),
    list,
    listName: list?.name,
