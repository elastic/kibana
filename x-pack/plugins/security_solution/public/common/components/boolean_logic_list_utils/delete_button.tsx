...
interface EntryDeleteButtonProps<T> {
  entries: T[];
  isOnlyItem: boolean;
  entryIndex: number;
  itemIndex: number;
...
export const EntryDeleteButtonComponent = React.memo<EntryDeleteButtonProps>(
  ({ entries, nestedParentIndex, isOnlyItem, entryIndex, itemIndex, onDelete }) => {
    ...
    if (entryIndex === 0 && itemIndex === 0 && nestedParentIndex == null) {
