...
jest.mock('react-router-dom', () => {
  ...
  return {
    ...actual,
    useParams: jest.fn(() => ({ detailName: '', tabName: 'dns' })),
    useLocation: jest.fn(() => {
...
describe('VisualizationActions', () => {
  ...
  const mockGetCreateCaseFlyout = jest.fn(() => (
    <div data-test-subj="createCaseFlyout">{'createCaseFlyout'}</div>
  ));
  const mockGetAllCasesSelectorModal = jest.fn(() => (
    <div data-test-subj="getAllCasesSelectorModal">{'getAllCasesSelectorModal'}</div>
  ));
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('Should render Open in Lens button in viz action menu', () => {
    const { container } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
  ...
  test('Should render Save button in viz action menu', () => {
    const { container } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
  ...
  test('Should render Inspect button in viz action menu', () => {
    const { container } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
  ...
  test('Should render Inspect Modal after clicking the inspect button', () => {
    const { baseElement, container, debug } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
    );
  ...
  test('Should render Add to new Case button in viz action menu', () => {
    const { container } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
  ...
  test('Should render Add to new Case Modal after clicking on Add to new Case button', () => {
    ...
    expect(baseElement.querySelector('[data-test-subj="createCaseFlyout"]')).toBeInTheDocument();
  });

  test('Should render Add to existing Case button in viz action menu', () => {
    const { container } = render(
      <TestProviders store={store}>
        <VisualizationActions {...props} />
      </TestProviders>
  ...
  test('Should render Add to existing Case Modal after clicking on Add to Existing Case button', () => {
    ...
    expect(
      baseElement.querySelector('[data-test-subj="getAllCasesSelectorModal"]')
    ).toBeInTheDocument();
  });
});
