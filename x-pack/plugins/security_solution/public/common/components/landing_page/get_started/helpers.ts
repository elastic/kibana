        ...
        (acc, section) => {
          const activeCards =
            section.cards?.reduce((accCards, card) => {
              const activeSteps = getActiveSteps(card.steps, activeProducts, getStartedSteps);
...
export const updateActiveSections = ({
  ...
  const steps = getStepsByActiveProduct({ activeProducts, cardId, sectionId, getStartedSteps });
