...
const PickEventTypeComponents: React.FC<PickEventTypeProps> = ({
  ...
  const onChangeEventTypeAndIndexesName = useCallback(
    ({
      id,
      newEventType,
      indexNames,
  ...
  const handleSaveIndices = useCallback(() => {
    onChangeEventTypeAndIndexesName({
      id: timelineId,
      newEventType: filterEventType,
      indexNames: selectedOptions.map((so) => so.label),
    });
    setPopover(false);
  }, [filterEventType, onChangeEventTypeAndIndexesName, selectedOptions, timelineId]);
