...
describe('useTimelineManager', () => {
  ...
  it('getIndexToAddById', async () => {
    await act(async () => {
      ...
      await waitForNextUpdate();
      const index = result.current.getIndexToAddById(testId);
      expect(index).toEqual(timelineDefaults.indexToAdd);
    });
  });
  it('setIndexToAdd', async () => {
    await act(async () => {
      ...
      const index = result.current.getIndexToAddById(testId);
      expect(index).toEqual(indexToAddArgs.indexToAdd);
    });
  });
  ...
  it('getTimelineFilterManager undefined on uninitialized', async () => {
    await act(async () => {
      ...
      await waitForNextUpdate();
      const index = result.current.getTimelineFilterManager(testId);
  ...
  it('getTimelineFilterManager defined at initialize', async () => {
    await act(async () => {
      ...
      const index = result.current.getTimelineFilterManager(testId);
      expect(index).toEqual(mockFilterManager);
    });
  });
});
