...
export const useHostIsolationAction = ({
  ...
  const { data: hostDetails, isFetching: loadingHostIsolationStatus } =
  ...
  const isolationSupported = useMemo(() => {
    return isEndpointAlert
      ? isIsolationSupported({
          ...
          capabilities: hostDetails?.capabilities,
        })
      : false;
  }, [agentVersion, hostDetails?.capabilities, hostOsFamily, isEndpointAlert]);

  const isIsolationAllowed = useUserPrivileges().endpointPrivileges.canIsolateHost;

  const isolateHostHandler = useCallback(() => {
    closePopover();
    if (hostDetails?.isIsolated === false) {
      onAddIsolationStatusClick('isolateHost');
    } else {
      onAddIsolationStatusClick('unisolateHost');
    }
  }, [closePopover, hostDetails, onAddIsolationStatusClick]);

  const isolateHostTitle = hostDetails?.isIsolated === false ? ISOLATE_HOST : UNISOLATE_HOST;

  return isIsolationAllowed &&
    isEndpointAlert &&
    isolationSupported &&
    ...
    ? [
        <EuiContextMenuItem
          ...
          disabled={hostDetails?.agentStatus === HostStatus.UNENROLLED}
