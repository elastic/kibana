...
const THREAT_NAMESPACE = 'threat';
const replaceKibanaAlertKey = (ruleKey: string) =>
  ruleKey.replace(ALERT_RULE_THREAT_NAMESPACE, THREAT_NAMESPACE);

export const buildThreatObjectFromFieldsApi = (data: TimelineEventsDetailsItem[] | null) => {
  const threatFrameworks = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_FRAMEWORK), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  ...
  const tacticIdValues = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_TACTIC_ID), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  const tacticNameValues = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_TACTIC_NAME), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  const tacticReferenceValues = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_TACTIC_REFERENCE), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  ...
  const techniqueIdValues = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_ID), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  const techniqueNameValues = [
    ...(find(
      { field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_NAME), category: THREAT_NAMESPACE },
      data
    )?.values ?? []),
  ];
  const techniqueReferenceValues = [
    ...(find(
      {
        field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_REFERENCE),
        category: THREAT_NAMESPACE,
      },
      data
    )?.values ?? []),
  ];
  ...
  const subTechniqueIdValues = [
    ...(find(
      {
        field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_SUBTECHNIQUE_ID),
        category: THREAT_NAMESPACE,
      },
      data
    )?.values ?? []),
  ...
  const subTechniqueNameValues = [
    ...(find(
      {
        field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_SUBTECHNIQUE_NAME),
        category: THREAT_NAMESPACE,
      },
      data
    )?.values ?? []),
  ...
  const subTechniqueReferenceValues = [
    ...(find(
      {
        field: replaceKibanaAlertKey(ALERT_THREAT_TECHNIQUE_SUBTECHNIQUE_REFERENCE),
        category: THREAT_NAMESPACE,
      },
      data
    )?.values ?? []),
