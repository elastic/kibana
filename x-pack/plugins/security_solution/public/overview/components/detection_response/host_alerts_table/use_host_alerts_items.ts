  ...
  useEffect(() => {
    if (data == null || !data.aggregations) {
      ...
      setPaginationData((p) => ({
        ...p,
        pageCount: Math.ceil(
          // While there could be more than 100 hosts we only want to show 25 pages of results,
          // and the host count cardinality result will always be the total count
          Math.min(data.aggregations?.host_count.value || 0, MAX_ALLOWED_RESULTS) / ITEMS_PER_PAGE
        ),
      }));
    }
