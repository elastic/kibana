  ...
  this.mergeGraph = function (newData) {
    ...
    dedupedNodes.forEach((dedupedNode) => {
      ...
      const node = {
        ...
        icon: isNewIcon(dedupedNode.icon) ? dedupedNode.icon : convertToNewIcon(dedupedNode.icon),
        data: dedupedNode,
      };
      ...
      data.vertices.forEach((node) => {
        targetFields.some((fieldDef) => {
          if (node.field === fieldDef.name) {
            ...
            node.icon = isNewIcon(fieldDef.icon) ? fieldDef.icon : convertToNewIcon(fieldDef.icon);
