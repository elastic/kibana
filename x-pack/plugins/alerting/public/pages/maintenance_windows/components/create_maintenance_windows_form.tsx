...
const recurringFieldComponentProps = {
...
export const CreateMaintenanceWindowForm = React.memo<CreateMaintenanceWindowFormProps>(
  (props: CreateMaintenanceWindowFormProps) => {
    ...
    const [{ recurring, timezone, categoryIds }, mounted] = useFormData<FormProps>({
      form,
    ...
    const onConfirm = useCallback(() => {
      ...
      if (maintenanceWindowId) {
        archiveMaintenanceWindow({ maintenanceWindowId, archive: true }, { onSuccess });
      }
    }, [archiveMaintenanceWindow, closeModal, maintenanceWindowId, onSuccess]);
    ...
    const modal = useMemo(() => {
      ...
      return m;
    }, [isModalVisible, closeModal, onConfirm]);
    ...
    useEffect(() => {
      ...
      setFieldValue('categoryIds', [
        ...new Set(validRuleTypes.map((ruleType) => ruleType.category)),
      ]);
    ...
    return (
      <Form form={form} data-test-subj="createMaintenanceWindowForm">
        <EuiFlexGroup direction="column" responsive={false}>
          <EuiFlexItem>
            <UseField path="title" componentProps={titleFieldComponentProps} />
          </EuiFlexItem>
          <EuiSpacer size="xs" />
          <EuiFlexItem>
            <EuiText size="s">
          ...
          <EuiFlexItem>
            <UseField path="recurring" componentProps={recurringFieldComponentProps} />
          </EuiFlexItem>
          {isRecurring ? (
            <EuiFlexItem>
