...
const generateAlertOpts = ({ action, group, state, id }: GeneratorParams = {}) => {
  ...
  switch (action) {
    case EVENT_LOG_ACTIONS.newInstance:
      message = `test:1: 'test-rule' created new alert: '${id}'`;
      break;
    case EVENT_LOG_ACTIONS.activeInstance:
      message = `test:1: 'test-rule' active alert: '${id}' in actionGroup: '${group}'`;
      break;
    case EVENT_LOG_ACTIONS.recoveredInstance:
      message = `test:1: 'test-rule' alert '${id}' has recovered`;
      break;
  }
  return {
    action,
...
describe('Alerts Client', () => {
  ...
  describe('initialize()', () => {
    ...
    test('should log warning and return if previousExecutionUuid is not provided', async () => {
      await alertsClient.initialize({ previousRuleExecutionUuid: undefined, rule: ruleData });
