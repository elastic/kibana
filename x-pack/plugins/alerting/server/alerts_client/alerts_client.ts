...
export class AlertsClient {
  ...
  private async extractReferences<Params extends AlertTypeParams>(
    ...
    const extractedRefsAndParams = ruleType?.extract ? ruleType.extract(ruleParams) : null;
    const extractedReferences = extractedRefsAndParams?.references ?? [];
    const params = (extractedRefsAndParams?.state as Params) ?? ruleParams;
  ...
  private injectReferencesIntoParams<Params extends AlertTypeParams>(
    ...
    try {
      ...
      return ruleParams && ruleType?.inject
        ? (ruleType.inject(ruleParams, paramReferences) as Params)
