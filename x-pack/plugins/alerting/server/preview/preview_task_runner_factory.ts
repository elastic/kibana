...
import { PreviewTaskRunner } from './preview_task_runner';
...
export class PreviewTaskRunnerFactory {
  private isInitialized = false;
  private taskRunnerContext?: TaskRunnerContext;

  public initialize(taskRunnerContext: TaskRunnerContext) {
    if (this.isInitialized) {
      throw new Error('PreviewTaskRunnerFactory already initialized');
    }
  ...
  public create({ taskInstance }: RunContext) {
    if (!this.isInitialized) {
      throw new Error('PreviewTaskRunnerFactory not initialized');
    }

    return new PreviewTaskRunner(taskInstance, this.taskRunnerContext!);
  }
}
