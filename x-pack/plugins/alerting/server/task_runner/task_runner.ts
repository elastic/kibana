...
> {
  ...
  async executeAlerts(
    ...
    const recoveryAlertIds = getRecoveredAlertIds(alerts, originalAlertIds);
    for (const recoveryAlertId of recoveryAlertIds) {
      ...
      } else if (alerts[recoveryAlertId]) {
        this.logger.debug(`Setting recovery context for recovered alert ${recoveryAlertId}`);
        alerts[recoveryAlertId].setContext(recoveryContext[recoveryAlertId]);
      }
    }
