...
import { MaintenanceWindowService } from './maintenance_window';
...
> {
  ...
  private async runRule({
    ...
    const { queryDelaySettings, flappingSettings } =
      await this.context.rulesSettingsService.getSettings(fakeRequest);
    ...
    const alertsClient = await withAlertingSpan('alerting:initialize-alerts-client', () =>
      ...
      >({
        ...
        maintenanceWindowService: this.maintenanceWindowService,
        rule: {
          id: rule.id,
          name: rule.name,
          tags: rule.tags,
  ...
  private async prepareToRun(): Promise<RunRuleParams<Params>> {
    return await this.timer.runWithTimer(TaskRunnerTimerSpan.PrepareRule, async () => {
      ...
      this.maintenanceWindowService.initialize({
