...
export const resolveRuleRoute = (
  ...
  router.get(
    {
      path: `${BASE_ALERTING_API_PATH}/rule/resolve/{id}`,
      validate: {
        params: paramSchema,
      },
    },
    router.handleLegacyErrors(
      verifyAccessAndContext(licenseState, async function (context, req, res) {
        ...
        const { id } = req.params;
        const resolvedRule = await rulesClient.resolve({ id });
        return res.ok({
          body: {
            ...resolvedRule,
            savedObject: rewriteBodyRes(resolvedRule.saved_object?.attributes),
          },
        });
      })
    )
