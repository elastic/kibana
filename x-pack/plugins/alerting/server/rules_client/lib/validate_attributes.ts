...
import { KqlNode, KqlFunctionNode, KqlLiteralNode, nodeTypes } from '@kbn/es-query';
...
export const validateOperationOnAttributes = (
  astFilter: KqlFunctionNode | null,
...
export const iterateFilterKureyNode = ({
  ...
  astFilter.arguments.forEach((ast: KqlNode, index: number) => {
    if (hasNestedKey && nodeTypes.literal.isNode(ast) && ast.value != null) {
      localNestedKeys = `${ast.value}`;
    } else if (nodeTypes.literal.isNode(ast) && ast.value && typeof ast.value === 'string') {
    ...
    if (nodeTypes.function.isNode(ast)) {
    ...
    if (localStoreValue && nodeTypes.literal.isNode(ast)) {
