...
import { ServiceInstanceComparisonStatisticsParams } from '.';
...
export async function getServiceInstanceSystemMetricStats({
  ...
  serviceNodeIds,
}: ServiceInstanceComparisonStatisticsParams) {
      ...
      response.aggregations?.[SERVICE_NODE_NAME].buckets.map(
        (serviceNodeBucket) => {
          ...
          return {
            ...
            cpuUsage: serviceNodeBucket.cpu_usage.timeseries.buckets.map(
              (dateBucket) => ({
                x: dateBucket.key,
                y: dateBucket.avg.value,
              })
            ),
            memoryUsage: serviceNodeBucket[
              memoryMetricsKey
