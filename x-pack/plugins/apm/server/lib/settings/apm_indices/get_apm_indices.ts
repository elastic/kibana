...
async function getApmIndicesSavedObject(savedObjectsClient: ISavedObjectsClient) {
...
export async function getApmIndexSettings({
  ...
  const apmIndices = Object.keys(config.indices) as Array<keyof typeof config.indices>;
