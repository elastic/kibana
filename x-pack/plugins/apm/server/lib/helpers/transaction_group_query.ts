...
export function prepareTransactionGroups({
  ...
  const results = buckets.map((bucket: ITransactionGroupBucket) => {
    ...
    const sample: ITransactionGroupSample | {} = get(
      bucket,
    ...
    return {
      name: bucket.key,
      serviceName: get(sample, SERVICE_NAME, 'n/a'),
      id: get(sample, TRANSACTION_ID, 'n/a'),
      p95: bucket.p95.values['95.0'],
      averageResponseTime,
      transactionsPerMinute,
      impact,
      transactionType: get(sample, TRANSACTION_TYPE, 'n/a')
    };
  });
