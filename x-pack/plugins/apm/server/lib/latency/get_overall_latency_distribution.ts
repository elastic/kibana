...
} from '../search_strategies/correlations/queries/query_histogram_range_steps';
import { getTransactionDurationRangesRequest } from '../search_strategies/correlations/queries/query_ranges';
...
export async function getOverallLatencyDistribution(
  ...
  return withApmSpan('get_overall_latency_distribution', async () => {
    ...
    const { body: histogramIntervalRequestBody } = getHistogramIntervalRequest(
      params
    );
    ...
    overallLatencyDistribution.overallHistogram = transactionDurationRangesResponse.aggregations.logspace_ranges.buckets
      .map((d) => ({
        key: d.from,
