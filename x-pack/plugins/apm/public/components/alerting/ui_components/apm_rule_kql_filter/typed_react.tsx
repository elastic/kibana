...
export type RendererFunction<RenderArgs, Result = RendererResult> = (
  args: RenderArgs
) => Result;
...
> =>
  hoc(
    class ChildFunctionRenderer extends React.Component<
    ...
    > {
      ...
      public render() {
        return (
          this.props
            .children as ChildFunctionRendererProps<InjectedProps>['children']
...
export type PropsOfContainer<Container> =
  Container extends InferableComponentEnhancerWithProps<
    infer InjectedProps,
    any
  >
    ? InjectedProps
...
export function composeStateUpdaters<State, Props>(
  ...updaters: Array<StateUpdater<State, Props>>
) {
  return (state: State, props: Props) =>
    updaters.reduce(
      (currentState, updater) => updater(currentState, props) || currentState,
      state
    );
}
