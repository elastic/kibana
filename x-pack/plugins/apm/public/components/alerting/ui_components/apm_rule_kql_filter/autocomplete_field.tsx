...
import { composeStateUpdaters } from './typed_react';
...
> {
      ...
      <EuiOutsideClickDetector onOutsideClick={this.handleBlur}>
        <AutocompleteContainer>
          <EuiFieldSearch
            ...
            value={value}
            aria-label={ariaLabel}
            data-test-subj="apmSearchField"
          />
          {areSuggestionsVisible &&
          !isLoadingSuggestions &&
          suggestions.length > 0 ? (
            <SuggestionsPanel data-test-subj="apmSuggestionsPanel">
        ...
        if (suggestions.length > 0) {
          this.setState(
            composeStateUpdaters(
              withSuggestionsVisible,
              withPreviousSuggestionSelected
            )
          );
        }
        break;
  ...
  private updateSuggestions = () => {
    const inputCursorPosition = this.inputElement
...
const withPreviousSuggestionSelected = (
  ...
  selectedIndex:
    props.suggestions.length === 0
      ...
      ? (state.selectedIndex + props.suggestions.length - 1) %
