...
export function MobileStats({ start, end, kuery }: { start: string; end: string; kuery: string }) {
  ...
  const getComparisonValueFormatter = useCallback(
    (value) => {
      return <span>{value && comparisonEnabled ? `${previousPeriodLabel}: ${value}` : null}</span>;
    },
  ...
  const metrics: MetricDatum[] = [
    ...
    },
    {
      ...
      extra: getComparisonValueFormatter(data?.previousPeriod.launchTimes?.value?.toFixed(1)),
      trendShape: MetricTrendShape.Area,
    },
    {
      color: euiTheme.eui.euiColorLightestShade,
