...
export const useUrlState = <State>({
  ...
  const queryString = history?.location
    ? getQueryStringFromLocation(history.location)
    : '';
...
const getQueryStringFromLocation = (location: Location) =>
...
const replaceQueryStringInLocation = (
  location: Location,
  queryString: string
): Location => {
