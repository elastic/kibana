...
export const datemathStringRT = new rt.Type<string, string, unknown>(
  ...
  (value, context) =>
    pipe(
      ...
      chain((stringValue) =>
        isValidDatemath(stringValue)
          ? rt.success(stringValue)
          : rt.failure(stringValue, context)
      )
    ),
  String
);
...
function extendRelativeDatemath(
  ...
  const { unit: normalizedUnit, amount: normalizedAmount } = normalizeDate(
    newAmount,
    newUnit
  );
  ...
  const diffAmount = Math.abs(
    normalizedAmount - convertDate(parsedAmount, unit, normalizedUnit)
  );
...
function extendAbsoluteDatemath(
  ...
  const normalizedTimestampDiff = convertDate(
    normalizedDiff.amount,
    normalizedDiff.unit,
    'ms'
  );
...
function isDateInRange(date: string | number): boolean {
  try {
    const epoch =
      typeof date === 'string' ? datemathToEpochMillis(date) ?? -1 : date;
