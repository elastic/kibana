...
describe('TelemetryEventsSender', () => {
  ...
  beforeEach(() => {
    ...
    sender['fetchClusterInfo'] = jest.fn(
      async () =>
        ({
          cluster_uuid: '1',
          cluster_name: 'name',
  ...
  describe('queueTelemetryEvents', () => {
    it('queues two events', () => {
      sender.queueTelemetryEvents('osquery-upgrades', [
        {
          packageName: 'system',
          currentVersion: '0.3',
          newVersion: '1.0',
      ...
      expect(sender['queuesPerChannel']['osquery-upgrades']).toBeDefined();
    });
    ...
    it('should send events when due', async () => {
      ...
      expect(sender['sendEvents']).toHaveBeenCalledWith(
        'https://telemetry-staging.elastic.co/v3/send/osquery-upgrades',
