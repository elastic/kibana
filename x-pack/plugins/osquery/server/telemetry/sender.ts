...
import type { OsqueryTelemetryChannel, OsqueryTelemetryChannelEvents } from './types';

/**
 * Simplified version of https://github.com/elastic/kibana/blob/main/x-pack/plugins/security_solution/server/lib/telemetry/sender.ts
...
export class TelemetryEventsSender {
  ...
  public queueTelemetryEvents<T extends OsqueryTelemetryChannel>(
    channel: T,
    events: Array<OsqueryTelemetryChannelEvents[T]>
  ) {
    if (!this.queuesPerChannel[channel]) {
      this.queuesPerChannel[channel] = new TelemetryQueue<OsqueryTelemetryChannelEvents[T]>();
    }
