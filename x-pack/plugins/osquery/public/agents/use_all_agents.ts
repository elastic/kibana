  ...
  return useQuery<{
    ...
    queryFn: () => {
      ...
      if (osqueryPolicies?.length) {
        ...
        if (searchValue) {
          kuery += ` and (local_metadata.host.hostname.keyword:*${searchValue}* or local_metadata.elastic.agent.id:*${searchValue}* or policy_id: *${searchValue}* or local_metadata.os.platform: *${searchValue}*) `;
        } else {
