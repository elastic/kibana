...
import { registerActionsRoute } from './actions';
import {
  KibanaRequest,
...
describe('actions', () => {
  ...
  it('returns the existing actions', async () => {
    ...
    registerActionsRoute({ router, config: kibanaStatsConfig, overallStatus$, getMetrics });
  ...
  it('returns the an empty object if there are no actions', async () => {
    const router = httpServiceMock.createRouter();
