...
const bodySchema = schema.object({}, { allowUnknowns: true });
...
const handler: RequestHandler<ParamsSchema, unknown, BodySchema> = async (
  ctx,
  request,
  response
) => {
  const { id } = request.params;
  const { type, isNew, ...watchConfig } = request.body as any;
