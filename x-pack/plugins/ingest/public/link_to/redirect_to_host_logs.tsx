...
import { replaceLogFilterInQueryString } from '../logs/containers/logs/with_log_filter';
import { replaceLogPositionInQueryString } from '../logs/containers/logs/with_log_position';
import { WithSource } from '../logs/containers/with_source';
...
export const getHostLogsUrl = ({ hostname, time }: { hostname: string; time?: number }) =>
  ['#/logs/link-to/host-logs/', hostname, ...(time ? [`?time=${time}`] : [])].join('');
