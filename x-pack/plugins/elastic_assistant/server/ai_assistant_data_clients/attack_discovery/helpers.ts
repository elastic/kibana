...
import { UpdateAttackDiscoverySchema } from './update_attack_discovery';

export const getUpdateScript = ({
  attackDiscovery,
  isPatch,
}: {
  attackDiscovery: UpdateAttackDiscoverySchema;
  isPatch?: boolean;
}) => {
  return {
    source: `
    if (params.containsKey('api_config')) {
    ...
    if (params.assignEmpty == true || params.containsKey('exclude_from_last_attackDiscovery_storage')) {
      ctx._source.exclude_from_last_attackDiscovery_storage = params.exclude_from_last_attackDiscovery_storage;
    }
  ...
  `,
    lang: 'painless',
    params: {
      ...attackDiscovery, // when assigning undefined in painless, it will remove property and wil set it to null
