...
export const findAnonimizationFieldsRoute = (
  ...
  router.versioned
    .get({
      access: 'public',
      path: ELASTIC_AI_ASSISTANT_ANONIMIZATION_FIELDS_URL_FIND,
      options: {
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            query: buildRouteValidationWithZod(FindAnonimizationFieldsRequestQuery),
          },
        },
      },
      async (
        ...
        request,
        response
      ): Promise<IKibanaResponse<FindAnonimizationFieldsResponse>> => {
        ...
        try {
          ...
          const result = await dataClient?.findAnonimizationFields({
