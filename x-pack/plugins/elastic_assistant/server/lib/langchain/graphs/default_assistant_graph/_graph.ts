...
//       logger.error(`graph:shouldContinue:state\n${JSON.stringify(state, null, 2)}`);
...
//       logger.error(`graph:executeTools:state\n${JSON.stringify(state, null, 2)}`);
...
//     workflow.addNode('listIndicesEsDb', async (state: AgentState) => {
//       console.error('listIndicesEsDb, state', state);
//       const response = await esClient.indices.get({ index: '*' });

//       console.error('response', JSON.stringify(response, null, 2));

//       function transformInputToOutput(input: Record<string, any>) {
//         const output: Record<string, any> = {};

//         forOwn(input, (value, _key) => {
...
//       return {
//         ...state,
//         indices: transformInputToOutput(response),
//       };
//     });

//     workflow.addNode('generateESQLQuery', async (state: AgentState) => {
//       const ecsDuplicatesPrompt = ECS_DUPLICATES_PROMPT;
//       const outputParser = new JsonOutputParser();
//       const ecsDuplicatesGraph = ecsDuplicatesPrompt.pipe(model)
...
//     workflow.addNode('getColumns', async (state: AgentState) => {
//       const columns = getESQLQueryColumns({ esqlQuery: state.query, search });
...
//     workflow.addEdge(START, 'listIndicesEsDb');
//     workflow.addEdge('listIndicesEsDb', 'generateESQLQuery');
//     workflow.addEdge('generateESQLQuery', 'getColumns');
//     workflow.addEdge('getColumns', END);
