...
export interface AssistantDataWriter {
  bulk: (params: BulkParams) => Promise<WriterBulkResponse>;
}

interface AssistantDataWriterOptions {
...
export class AssistantDataWriter implements AssistantDataWriter {
  constructor(private readonly options: AssistantDataWriterOptions) {}

  public bulk = async (params: BulkParams) => {
    try {
      if (!params.conversations?.length) {
  ...
  private buildBulkOperations = (params: BulkParams): BulkOperationContainer[] => {
    const conversationBody =
      params.conversations?.flatMap((conversation) => [
        { create: { _index: this.options.index } },
        // this.scoreToEcs(score, 'host'),
      ]) ?? [];

    return conversationBody as BulkOperationContainer[];
  };

  /* private scoreToEcs = (score: unknown, identifierType: string): unknown => {
