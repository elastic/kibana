    ...
    describe('getUrlTemplateWithMeta', () => {
      ...
      it('Should only include required props', async () => {
        ...
        expect(urlTemplateWithMeta.urlTemplate).toBe(
          `rootdir/api/maps/mvt/getTile/{z}/{x}/{y}.pbf?geometryFieldName=bar&index=foobar-title-*&requestBody=(foobar:ES_DSL_PLACEHOLDER,params:('0':('0':index,'1':(fields:(),title:'foobar-title-*')),'1':('0':size,'1':1000),'2':('0':filter,'1':!()),'3':('0':query),'4':('0':index,'1':(fields:(),title:'foobar-title-*')),'5':('0':query,'1':(language:KQL,query:'tooltipField: foobar')),'6':('0':trackTotalHits,'1':1001),'7':('0':fieldsFromSource,'1':!(tooltipField,styleField)),'8':('0':source,'1':!(tooltipField,styleField))))`
        );
      });
    });
  });
