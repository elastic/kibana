...
export class ESGeoLineSource extends AbstractESAggSource {
  ...
  async getGeoJsonWithMeta(
    ...
    const entityResp = await this._runEsQuery({
      ...
      executionContext: mergeExecutionContext({ description: 'es_geo_line:entities' }, searchFilters.executionContext),
    ...
    const tracksResp = await this._runEsQuery({
      ...
      searchSessionId: searchFilters.searchSessionId,
      executionContext: mergeExecutionContext({ description: 'es_geo_line:tracks' }, searchFilters.executionContext),
      requestsAdapter: inspectorAdapters.requests,
    });
