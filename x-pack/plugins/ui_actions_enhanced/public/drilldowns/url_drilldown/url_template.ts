...
let handlebarsPromise: Promise<typeof Handlebars>;
async function getHandlebars(): Promise<typeof Handlebars> {
  if (handlebarsPromise) return handlebarsPromise;
  return (handlebarsPromise = import('handlebars').then((handlebarsLib) => {
...
export async function compile(
  ...
  const handlebarsTemplate = (await getHandlebars()).compile(urlTemplate, {
    strict: true,
    noEscape: true,
  });
