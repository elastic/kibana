...
import { sha1 } from './download/checksum';
...
export async function install(
  ...
  const binaryChecksum = await sha1(binaryPath).catch(() => 'MISSING');
  ...
  const downloadedBinaryChecksum = await sha1(binaryPath).catch(() => 'MISSING');
