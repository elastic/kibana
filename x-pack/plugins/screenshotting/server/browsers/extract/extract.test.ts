...
import { unlink, rmdir, mkdir, stat } from 'fs/promises';
...
async function cleanup() {
  await ignoreErrorCodes(['ENOENT', 'EACCES'], unlink(EXTRACT_TARGET_FILE));
  await ignoreErrorCodes(['ENOENT'], rmdir(EXTRACT_TARGET_FOLDER));
}
...
describe('extract', () => {
  ...
  describe('zip()', () => {
    ...
    it('successfully extracts a valid zip file to the given target', async () => {
      ...
      const stats = await stat(EXTRACT_TARGET_FILE);
    ...
    if (isWindows) {
      ...
      it(`throws an ExtractError with cause.code of EACESS when target is un-writeable`, async () => {
        await mkdir(EXTRACT_TARGET_FOLDER, 0o444);
