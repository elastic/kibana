 ...
 */
export async function ensureDownloaded(browsers: ChromiumArchivePaths[], logger?: Logger) {
  await Promise.all(
    browsers.map(async (paths) => {
      ...
      await Promise.all(
        paths.packages.map(async (path) => {
          ...
          try {
            const downloadedChecksum = await download(url, resolvedPath, logger);
