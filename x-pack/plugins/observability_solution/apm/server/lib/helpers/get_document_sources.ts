...
export async function getDocumentSources({
  ...
  const hasAnySourceDocPreviousRange = documentTypesInfo.some(
    (source) => source.hasDocsPreviousRange
  );

  return [
    ...mapToSources(documentTypesInfo, hasAnySourceDocPreviousRange),
    {
...
const mapToSources = (sources: DocumentTypeData[], hasAnySourceDocPreviousRange: boolean) => {
  return sources.map((source) => {
    ...
    // versions which only ships transactionMetric docs.
    const hasRequiredDocAvailability =
      documentType === ApmDocumentType.ServiceTransactionMetric
    ...
    // events.
    const isDataSourceAvailable = hasAnySourceDocPreviousRange
      ? hasDocsPreviousRange
