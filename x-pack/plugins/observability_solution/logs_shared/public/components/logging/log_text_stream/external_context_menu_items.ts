...
import { logEntryContextMenuTriggerId } from '../../../../common';

export type ObservabilityActionContextMenuItemProps = EuiContextMenuItemProps & {
  children: React.ReactElement;
};

export function getExternalContextMenuItemsFromActions({
  uiActions,
  context,
}: {
  ...
  return uiActions
    .getTriggerCompatibleActions(logEntryContextMenuTriggerId, context)
    .then((actions) => {
      return buildContextMenuForActions({
        actions: actions.map((action) => ({
          action,
          trigger: {
            id: logEntryContextMenuTriggerId,
          },
          context,
        })),
      });
