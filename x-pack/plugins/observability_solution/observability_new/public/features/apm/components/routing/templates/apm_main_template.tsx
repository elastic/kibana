...
import { ObservabilityPluginStartDependencies } from '../../../../../types';
...
export function ApmMainTemplate({
  ...
  const { services } = useKibana<ObservabilityPluginStartDependencies>();
  const kibanaEnvironment = useContext(KibanaEnvironmentContext);
  const { http, docLinks, application } = services;
