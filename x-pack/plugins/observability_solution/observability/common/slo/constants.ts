...
export const SLO_RESOURCES_VERSION = 4;
export const SLO_SUPPORTED_RESOURCE_VERSIONS = [3, 4];
...
export const SLO_SUMMARY_DESTINATION_SUPPORTED_INDEX_PATTERNS = SLO_SUPPORTED_RESOURCE_VERSIONS.map(
  (version) => `.slo-observability.summary-v${version}*`
).join(',');
export const SLO_DESTINATION_SUPPORTED_INDEX_PATTERNS = SLO_SUPPORTED_RESOURCE_VERSIONS.map(
  (version) => `.slo-observability.sli-v${version}*`
).join(',');
