...
import { GenericMetric, RuleConditionChartExpressions } from './rule_condition_chart';
...
): Array<typeof LensFieldFormat[keyof typeof LensFieldFormat]> => {
  const res: Array<typeof LensFieldFormat[keyof typeof LensFieldFormat]> = [];
  metricExpression.forEach((metricExp) => {
    if (metricExp.metrics.length < 1 || !metricExp.metrics[0].field)
      res.push(LensFieldFormat.NUMBER);
    else {
      const firstMetricField = metricExp.metrics[0].field;
