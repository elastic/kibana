...
export const registerFunctions: RegistrationCallback = async ({
  ...
  return client.getKnowledgeBaseStatus().then((response) => {
    ...
    functions.registerInstruction(({ availableFunctionNames }) => {
      ...
      if (isReady) {
        instructions.push(`You can use the "summarize" functions to store new information you have learned in a knowledge database. Once you have established that you did not know the answer to a question, and the user gave you this information, it's important that you create a summarisation of what you have learned and store it in the knowledge database. Don't create a new summarization if you see a similar summarization in the conversation, instead, update the existing one by re-using its ID.
