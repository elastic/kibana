...
export function LogicalParseView({ goBack, entity }: { goBack: () => void; entity: Entity }) {
  const {
    core: { http },
    dependencies: {
  ...
  const logsQuery = useMemo(() => {
    ...
    const whereClauses = (Array.isArray(identityFields) ? identityFields : [identityFields]).map(
      (field: string) => `${field} == "${entity.properties[field]}"`
    );
    return `FROM logs-* | WHERE @timestamp <= NOW() AND @timestamp >= NOW() - 60 minutes | WHERE ${whereClauses.join(
      ' AND '
  ...
  async function sendPlan() {
    setLoading(true);
    const result = await http.post('/api/apply_per_entity_change/plan', {
      body: JSON.stringify({
        entity,
