...
export function DataStreamManagementView() {
  ...
  if (entityType !== 'data_stream') {
    return (
      <EuiText>
        {i18n.translate(
          'xpack.inventory.dataStreamManagementView.nonphysicalManagementPageTextLabel',
          { defaultMessage: 'Non-physical management page' }
        )}
      </EuiText>
    );
  }
  ...
  if (view === 'parse') {
    return <DatasetManagementParseView goBack={goBack} />;
  }
...
export function StorageDetails(props: { details: any }) {
  return (
    <EuiPanel hasBorder>
      <EuiFlexGroup direction="column" gutterSize="s">
        <EuiFlexGroup alignItems="center" gutterSize="xs" responsive={false}>
...
function RoutingDetails(props: { id: string }) {
  ...
  const onSelect = useCallback(
    (node: any) => {
      navigateToUrl(`/app/entities/data_stream/${node.data('id')}/management`);
    },
    [navigateToUrl]
  );
