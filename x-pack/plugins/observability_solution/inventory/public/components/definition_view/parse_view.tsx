...
export function DefinitionParseView({
  goBack,
  entityDefinition,
}: {
  goBack: () => void;
  ...
  const logsQuery = useMemo(() => {
    ...
    const whereClauses = identityFields.map((field) => `WHERE ${field.field} IS NOT NULL`);
    return `FROM logs-* | WHERE @timestamp <= NOW() AND @timestamp >= NOW() - 60 minutes | ${whereClauses.join(
      ' AND '
    )} | SORT @timestamp DESC | LIMIT 100`;
  }, [entityDefinition.identityFields]);
  ...
  async function sendPlan() {
    setLoading(true);
    const result = await http.post('/api/apply_per_entity_definition_change/plan', {
      body: JSON.stringify({
        entityDefinition,
