...
import { InfraServerPluginSetupDeps } from './adapters/framework';

export const getUiSettings = (plugins: InfraServerPluginSetupDeps) => {
  const shouldRegisterLogsStreamSetting = !plugins.serverless;

  const uiSettings: Record<string, UiSettingsParams> = {};

  if (shouldRegisterLogsStreamSetting) {
    uiSettings[OBSERVABILITY_ENABLE_LOGS_STREAM] = {
