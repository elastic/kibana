...
import { NAMESPACE_SELECT_OPTIONS, PERIODS_SELECT_OPTIONS } from '../types';
...
export const  KubernetesInsightComp = ({
    client,
  }: {
    client?: any;
  }) => {
    ...
    const [fetchNodeMemAnalysis, setFetchNodeMemAnalysis] = useState<boolean>(false);
    const [nodeMemAnalysis, setNodeMemAnalysis] = useState('');
    const [fetchNodeCpuAnalysis, setFetchNodeCpuAnalysis] = useState<boolean>(false);
    ...
    useEffect(() => {
      console.log("CALLED DUE TO CHANGE in fetchNodeMemAnalysis");
      if (nodesMem.length !== 0 && fetchNodeMemAnalysis) {
        var content = JSON.stringify(nodesMem);
        ...
        client.analyze(content, assistant).then(result => {
          ...
          setIsNodesMemFlyoutOpen(true);
          setFetchNodeMemAnalysis(false);
          console.log("The response for nodes memory is here");
    ...
    }, [fetchNodeMemAnalysis]); // *** Note the dependency
    ...
    useEffect(() => {
      console.log("CALLED DUE TO CHANGE in fetchNodeCpuAnalysis");
      if (nodesCpu.length !== 0 && fetchNodeCpuAnalysis) {
        var content = JSON.stringify(nodesCpu);
        ...
        client.analyze(content, assistant).then(result => {
          ...
          setIsNodesCpuFlyoutOpen(true);
          setFetchNodeCpuAnalysis(false);
          console.log("The response for nodes cpu is here");
          console.log(result.response)
          })
          .catch(error => {
              console.log(error)
    ...
    }, [fetchNodeCpuAnalysis]); // *** Note the dependency
        ...
        <EuiFlexGroup alignItems="center" direction="column">
          ...
          <EuiFlexItem>
              ...
              <EuiFlexGroup justifyContent="spaceBetween" alignItems="flexEnd">
                ...
                <div style={{ width: 900 }}>
                  <EuiButton
                          onClick={() => setFetchNodeMemAnalysis(true)}
                          isDisabled={nodesMem.length===0}
                          size={'s'}
                          color="primary"
                        >
                          <FormattedMessage
                            id="xpack.aiops.changePointDetection.viewNodeMemButton"
          ...
          <EuiFlexItem>
              ...
              <EuiFlexGroup justifyContent="spaceBetween" alignItems="flexEnd">
                ...
                <div style={{ width: 900 }}>
                  <EuiButton
                           onClick={() => setFetchNodeCpuAnalysis(true)}
                           isDisabled={nodesCpu.length===0}
                          size={'s'}
                          color="primary"
                        >
                          <FormattedMessage
