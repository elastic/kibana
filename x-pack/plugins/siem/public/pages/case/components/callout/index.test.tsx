...
jest.mock('../../../../containers/local_storage/use_messages_storage');

const useSiemLocalStorageMock = useMessagesStorage as jest.Mock;
const siemLocalStorageMock = {
  getMessages: jest.fn(() => []),
  addMessage: jest.fn(),
};
...
describe('CaseCallOut ', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useSiemLocalStorageMock.mockImplementation(() => siemLocalStorageMock);
  });
  ...
  it('persist the callout of type primary when dismissed', () => {
    ...
    const id = createCalloutId(['message-one']);
    expect(siemLocalStorageMock.getMessages).toHaveBeenCalledWith('case');
    wrapper.find(`[data-test-subj="callout-dismiss-${id}"]`).last().simulate('click');
    expect(siemLocalStorageMock.addMessage).toHaveBeenCalledWith('case', id);
  });
  ...
  it('do not show the callout if is in the localStorage', () => {
    ...
    useSiemLocalStorageMock.mockImplementation(() => ({
  ...
  it('do not persist a callout of type danger', () => {
    ...
    wrapper.update();
    expect(siemLocalStorageMock.addMessage).not.toHaveBeenCalled();
  });
