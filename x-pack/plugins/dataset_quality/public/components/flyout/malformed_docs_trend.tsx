...
import { useEuiTheme, EuiSkeletonRectangle, EuiFlexGroup } from '@elastic/eui';

import { TimeRangeConfig } from '../../state_machines/dataset_quality_controller';
import { DEFAULT_LOGS_DATA_VIEW } from '../../../common/constants';
import { flyoutMalformedDocsTrend } from '../../../common/translations';
import { useCreateDataView } from '../../hooks';
import { useKibanaContextForPlugin } from '../../utils';
import { getLensAttributes } from './lens_attributes';

const CHART_HEIGHT = 180;
const DISABLED_ACTIONS = ['ACTION_CUSTOMIZE_PANEL', 'ACTION_CONFIGURE_IN_LENS'];

export function MalformedDocsTrend({
  dataStream,
  timeRange,
  lastReloadTime,
}: {
  ...
  const { dataView } = useCreateDataView({
    indexPatternString: DEFAULT_LOGS_DATA_VIEW,
  });
  ...
  return (
    ...
    >
      <EuiSkeletonRectangle
        ...
        contentAriaLabel={flyoutMalformedDocsTrend}
        width="100%"
        height={CHART_HEIGHT}
        borderRadius="s"
      >
        <lens.EmbeddableComponent
          id="datasetQualityFlyoutMalformedDocsTrend"
