...
import { DatasetQualityUsername, users } from './authentication';
...
export async function createDatasetQualityUsers({
  kibana,
  elasticsearch,
}: {
  kibana: Kibana;
  ...
  const datasetQualityUsers = Object.values(DatasetQualityUsername);
  await asyncForEach(datasetQualityUsers, async (username) => {
