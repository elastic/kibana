  ...
  useEffect(() => {
    ...
    if (subActionParamsState[defaultedSubAction] == null) {
      setSubActionParamsState((previousState) => {
        const stateCopy = cloneDeep(previousState);
        console.log(
          'sub action params when params state was null',
          JSON.stringify(subActionParams, null, 2)
        );
      ...
      /**
       * When a user switches the event action between create and close alert, we want to have separate configured fields
       * for each action. We also only want to post the fields that are applicable for that action type. So to achieve this
       * we will cache the state and reset the subActionParams object to only include the applicable fields.
       *
