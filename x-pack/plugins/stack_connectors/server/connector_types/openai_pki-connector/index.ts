...
export const configValidator = (configObject: Config, validatorServices: ValidatorServices) => {
  try {
    ...
    if (configObject.certPath && !configObject.certPath.endsWith('.pem')) {
      throw new Error('Invalid certificate file format. Must be a .pem file');
    }
    if (configObject.keyPath && !configObject.keyPath.endsWith('.pem')) {
      throw new Error('Invalid private key file format. Must be a .pem file');
    }
