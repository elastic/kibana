...
import { getChartData, generateVegaChartSpec, generateChartSvg, svg2png } from './vega_chart';
  ...
  const postMessage = async ({
    ...
    try {
      ...
      if (chartData) {
        ...
        const svg = await generateChartSvg(spec);
        const png = await svg2png(svg);

        const result: AxiosResponse<UploadResponse> = await request({
          axios: axiosInstance,
          method: 'post',
          url: `${SLACK_URL}files.upload`,
          logger,
          data: {
        ...
        // console.log(`resultUpload: ${JSON.stringify(result.data, null, 4)}`);
        if (result?.data?.error) {
          throw new Error(`image upload failed: ${result?.data?.error}`);
        }
