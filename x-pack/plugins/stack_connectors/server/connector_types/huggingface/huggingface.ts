...
import { removeEndpointFromUrl } from './lib/huggingface_utils';
import {
...
} from '../../../common/huggingFace/schema';
import type {
  Config,
  Secrets,
  RunActionParams,
...
export class HuggingFaceConnector extends SubActionConnector<Config, Secrets> {
  private url;
  private provider;
  private key;
  private huggingFace;
  ...
  public async invokeAsyncIterator(body: InvokeAIActionParams): Promise<{
    ...
    try {
      ...
      const requestBody: ChatCompletionCreateParamsStreaming = {
        ...
        model:
          rest.model ??
          ('defaultModel' in this.config ? this.config.defaultModel : DEFAULT_HUGGINGFACE_MODEL),
      };
      const stream = await this.huggingFace.chat.completions.create(requestBody, {
        signal,
        timeout, // do not default if not provided
      });
