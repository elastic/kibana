...
describe('The custom threshold alert type', () => {
  ...
  describe('querying with multiple criteria', () => {
    ...
    test('sends all criteria to the action context', async () => {
      ...
      expect(reasons).toBe(
        'Average test.metric.1 is 1, above or equal the threshold of 1; Average test.metric.2 is 3, above the threshold of 3. (duration: 1 min, data view: mockedDataViewName)'
      );
    });
  });
