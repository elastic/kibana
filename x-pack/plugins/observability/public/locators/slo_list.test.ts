...
describe('SloListLocator', () => {
  ...
  it("returns the correct url with the default search state when no 'kqlQuery' provided", async () => {
    ...
    expect(location.path).toMatchInlineSnapshot(
      `"/slos?search=(compact:!t,groupBy:ungrouped, filters:!(),kqlQuery:'',lastRefresh:0,page:0,perPage:25,sort:(by:status,direction:desc),view:cardView)"`
    );
  });
  ...
  it("returns the correct url with the 'kqlQuery' provided", async () => {
    ...
    expect(location.app).toEqual('observability');
    expect(location.path).toMatchInlineSnapshot(
      `"/slos?search=(compact:!t, groupBy:ungrouped, filters:!(),kqlQuery:'slo.name:%20%22Service%20Availability%22%20and%20slo.indicator.type%20:%20%22sli.kql.custom%22',lastRefresh:0,page:0,perPage:25,sort:(by:status,direction:desc),view:cardView)"`
    );
  });
});
