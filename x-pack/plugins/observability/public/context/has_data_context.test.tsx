...
function wrapperWhenRouterIsOnOverview({ children }: { children: React.ReactElement }) {
...
function wrapperWhenRouterIsNotOnOverview({ children }: { children: React.ReactElement }) {
...
describe('HasDataContextProvider', () => {
  ...
  describe('when on the Overview page', () => {
    describe('when no plugin has registered', () => {
      it('hasAnyData returns undefined and all apps return undefined', async () => {
        const { result, waitForNextUpdate } = renderHook(() => useHasData(), {
          wrapper: wrapperWhenRouterIsOnOverview,
        });
