 ...
 * We could not use shouldUnregister because of a bug when submitting the form
 * which was unmounting the components and therefore unregistering the associated values.
 */
export function useUnregisterFields() {
  ...
  useEffect(() => {
    if (indicatorType !== indicatorTypeState) {
  ...
  }, [indicatorType, indicatorTypeState, unregister, reset, resetField]);
}
