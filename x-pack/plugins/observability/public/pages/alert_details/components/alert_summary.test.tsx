...
describe('Alert summary', () => {
    ...
    it('should show alert data', async () => {
        const alertSummary = render(
            <AlertSummary alert={alertWithTags} />
        );

        expect(alertSummary.queryByTestId("alertId")).toHaveTextContent("756240e5-92fb-452f-b08e-cd3e0dc51738");
        ...
        expect(alertSummary.queryByText(moment("2021-09-02T12:54:09.674Z").format(DEFAULT_DATE_FORMAT))).toBeInTheDocument();
    ...
    it('should show empty "-" for fields when no data available', async () => {
        const alertSummary = render(
            <AlertSummary alert={alertWithNoData} />
        );
