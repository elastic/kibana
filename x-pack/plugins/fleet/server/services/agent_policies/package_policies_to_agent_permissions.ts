...
export async function storedPackagePoliciesToAgentPermissions(
  ...
  const getPermissionEntry = async (packagePolicy: PackagePolicy) => {
  ...
  for (const packagePolicy of packagePolicies as PackagePolicy[]) {
    permissionEntries.push(await getPermissionEntry(packagePolicy));
  }
