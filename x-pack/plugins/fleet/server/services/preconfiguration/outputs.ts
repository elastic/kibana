...
async function isSecretDifferent(
  ...
  if (hasHash(existingSecret) && typeof preconfiguredValue === 'string') {
    // verifying the hash tells us if the value has changed
    const hashedPreconfiguredValue = await hash(preconfiguredValue);
    return hashedPreconfiguredValue !== existingSecret.hash;
  } else {
