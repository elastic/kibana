...
describe('createAgentPolicyWithPackages', () => {
  ...
  beforeEach(() => {
    ...
    mockedAgentPolicyService.create.mockImplementation((soRepo, esClient, newPolicy, options) =>
      Promise.resolve({
        ...newPolicy,
    ...
    mockedPackagePolicyService.buildPackagePolicyFromPackage.mockImplementation(
      (soRepo, packageToInstall) => Promise.resolve(getPackagePolicy(packageToInstall))
    );
    mockIncrementPackageName.mockImplementation((soRepo: any, pkg: string) =>
      Promise.resolve(`${pkg}-1`)
    );
    mockedPackagePolicyService.create.mockImplementation((soRepo, esClient, newPolicy) =>
      Promise.resolve({
        ...newPolicy,
      } as PackagePolicy)
    );
  ...
  it('should roll back agent policy if package policy creation failed', async () => {
    mockedPackagePolicyService.buildPackagePolicyFromPackage.mockImplementationOnce(
      (soRepo, packageToInstall) => Promise.reject(new Error('error'))
    );
    let error;
    try {
