...
export async function cleanUpUnusedKibanaAssetsStep(context: InstallContext) {
  const { logger, installedPkg, packageInstallContext, spaceId, packageAssetRefs } = context;
  const { packageInfo } = packageInstallContext;
  ...
  const nextAssetRefKeys = new Set(packageAssetRefs.map((asset) => `${asset.id}-${asset.type}`));
