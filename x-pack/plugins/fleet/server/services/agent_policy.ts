...
class AgentPolicyService {
  ...
  public async ensurePreconfiguredAgentPolicy(
    ...
    return await this.ensureAgentPolicy(soRepo, esClient, newAgentPolicy, id as string);
  }
  ...
  private async ensureAgentPolicy(
    ...
    const agentPolicy = await this.get(soRepo, id, false).catch(() => null);
    if (!agentPolicy) {
      return {
        created: true,
        policy: await this.create(soRepo, esClient, newAgentPolicy, { id }),
      };
    }
    return {
      created: false,
  ...
  public async copy(
    ...
    await this.deployPolicy(soRepo, newAgentPolicy.id);
  ...
  public async deployPolicy(soRepo: ISavedObjectsRepository, agentPolicyId: string) {
