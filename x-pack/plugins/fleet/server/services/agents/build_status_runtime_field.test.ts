    ...
    expect(runtimeField).toMatchInlineSnapshot(`
      Object {
        "status": Object {
          "script": Object {
            "lang": "painless",
            "source": "long lastCheckinMillis = doc['last_checkin'].size() > 0? doc['last_checkin'].value.toInstant().toEpochMilli(): -1;if (doc['unenrolled_at'].size() > 0) {  emit('unenrolled');  } else if (lastCheckinMillis > 0 && doc['policy_id'].size() > 0 && (((doc['policy_id'].value == 'policy-1') && lastCheckinMillis < 1234567590123L) || lastCheckinMillis < 1234567290123L)) {  emit('inactive');} else if (lastCheckinMillis > 0  && lastCheckinMillis  < (1234567590123L)) {emit('offline');  } else if (  doc['policy_revision_idx'].size() == 0 || (doc['upgrade_started_at'].size() > 0 &&doc['upgraded_at'].size() == 0  )) {emit('updating');  } else if (doc['last_checkin'].size() == 0) {  emit('enrolling');  } else if (doc['unenrollment_started_at'].size() > 0) {  emit('unenrolling');  } else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'error') {  emit('error');} else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'degraded') {emit('degraded');} else {emit('online');  }",
          },
          "type": "keyword",
        },
      }
    ...
    expect(runtimeField).toMatchInlineSnapshot(`
      Object {
        "status": Object {
          "script": Object {
            "lang": "painless",
            "source": "long lastCheckinMillis = doc['last_checkin'].size() > 0? doc['last_checkin'].value.toInstant().toEpochMilli(): -1;if (doc['unenrolled_at'].size() > 0) {  emit('unenrolled');  } else if (lastCheckinMillis > 0 && doc['policy_id'].size() > 0 && (((doc['policy_id'].value == 'policy-1' || doc['policy_id'].value == 'policy-2') && lastCheckinMillis < 1234567590123L) || lastCheckinMillis < 1234567290123L)) {  emit('inactive');} else if (lastCheckinMillis > 0  && lastCheckinMillis  < (1234567590123L)) {emit('offline');  } else if (  doc['policy_revision_idx'].size() == 0 || (doc['upgrade_started_at'].size() > 0 &&doc['upgraded_at'].size() == 0  )) {emit('updating');  } else if (doc['last_checkin'].size() == 0) {  emit('enrolling');  } else if (doc['unenrollment_started_at'].size() > 0) {  emit('unenrolling');  } else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'error') {  emit('error');} else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'degraded') {emit('degraded');} else {emit('online');  }",
          },
          "type": "keyword",
        },
      }
    ...
    expect(runtimeField).toMatchInlineSnapshot(`
      Object {
        "status": Object {
          "script": Object {
            "lang": "painless",
            "source": "long lastCheckinMillis = doc['last_checkin'].size() > 0? doc['last_checkin'].value.toInstant().toEpochMilli(): -1;if (doc['unenrolled_at'].size() > 0) {  emit('unenrolled');  } else if (lastCheckinMillis > 0 && doc['policy_id'].size() > 0 && (((doc['policy_id'].value == 'policy-1' || doc['policy_id'].value == 'policy-2') && lastCheckinMillis < 1234567590123L || (doc['policy_id'].value == 'policy-3') && lastCheckinMillis < 1234567490123L) || lastCheckinMillis < 1234567290123L)) {  emit('inactive');} else if (lastCheckinMillis > 0  && lastCheckinMillis  < (1234567590123L)) {emit('offline');  } else if (  doc['policy_revision_idx'].size() == 0 || (doc['upgrade_started_at'].size() > 0 &&doc['upgraded_at'].size() == 0  )) {emit('updating');  } else if (doc['last_checkin'].size() == 0) {  emit('enrolling');  } else if (doc['unenrollment_started_at'].size() > 0) {  emit('unenrolling');  } else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'error') {  emit('error');} else if (  doc['last_checkin_status'].size() > 0 &&  doc['last_checkin_status'].value.toLowerCase() == 'degraded') {emit('degraded');} else {emit('online');  }",
          },
          "type": "keyword",
        },
      }
