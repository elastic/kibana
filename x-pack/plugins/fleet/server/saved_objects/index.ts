...
export function registerEncryptedSavedObjects(
  ...
  encryptedSavedObjects.registerType({
    ...
    attributesToIncludeInAAD: new Set([
      // exclude output_id: { type: 'keyword', index: false },
      // exclude name: { type: 'keyword' },
      // exclude type: { type: 'keyword' },
      // exclude is_default: { type: 'boolean' },
      // exclude is_default_monitoring: { type: 'boolean' },
      // exclude hosts: { type: 'keyword' },
      // exclude ca_sha256: { type: 'keyword', index: false },
      // exclude ca_trusted_fingerprint: { type: 'keyword', index: false },
      // exclude config: { type: 'flattened' },
      // exclude config_yaml: { type: 'text' },
      // exclude is_preconfigured: { type: 'boolean', index: false },
      // include ssl: { type: 'binary' },
      'ssl',
      // exclude proxy_id: { type: 'keyword' },
      // include shipper: {
      //   dynamic: false, // we aren't querying or aggregating over this data, so we don't need to specify any fields
      //   properties: {},
      // },
      'shipper',
      // include allow_edit: { enabled: false },
      'allow_edit',
    ]),
  });
  ...
  encryptedSavedObjects.registerType({
    ...
    // ToDo: Inlcude all in AAD - there are none?
  });
  encryptedSavedObjects.registerType({
