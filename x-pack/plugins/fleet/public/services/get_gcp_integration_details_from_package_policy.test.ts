...
const undefinedAllValue = {"gcpAccountType": undefined, "gcpOrganizationId": undefined, "gcpProjectId": undefined}
    ...
    test('returns undefined when packagePolicy is undefined', () => {
      ...
      it('should return the first gcp integration details when available', () => {
        const packagePolicy = {
              inputs: [
                { enabled: false, streams: [ {} ] },
                { enabled: true, streams: [ { vars : {'gcp.account_type': {value : 'account_type_test_1'}, 'gcp.project_id': {value: 'project_id_1'}, 'gcp.organization_id': {value: 'organization_id_1'}}} ] },
                { enabled: true, streams: [ { vars : {'gcp.account_type': {value : 'account_type_test_2'}, 'gcp.project_id': {value: 'project_id_2'}, 'gcp.organization_id': {value: 'organization_id_2'}}} ] },
              ],
        };
        // @ts-expect-error
        const result = getGcpIntegrationDetailsFromPackagePolicy(packagePolicy);
