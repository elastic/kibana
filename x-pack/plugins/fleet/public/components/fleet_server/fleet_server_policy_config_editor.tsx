...
class ConfigMutator {
  ...
  public applyMutations(newConfig: Record<string, any>, oldConfig: Record<string, any>, currentPath: string[] = []): ReturnType<ConfigMutation> {
    ...
    for (const key of Object.keys(newConfig)) {
      ...
      } else if (this.fieldMutators[currentPath.join('.')] && value.value !== oldConfig[key].value) {
...
const versionToMutator: MutatorMap = {
  '>=1.1.1': new ConfigMutator({
    "vars.max_connections": function toMaxAgents({value}: {value: string}): MutationResult {
      return {max_agents: {value: Math.ceil(parseInt(value) / 2), type: "integer"}};
    },
    "vars.max_agents": function toMaxConnections({value}: {value: string}): MutationResult {
      return {max_connections: {value: 2 * parseInt(value), type: "integer"}};
    }
  })
}
...
export const FleetServerPackagePolicyConfigExtension = React.memo<
  PackagePolicyEditExtensionComponentProps
>(({ onChange, validationResults, packageInfo, newPolicy }) => {
  ...
  const configMutator = useMemo(
    () => Object.keys(versionToMutator).reduce(
      (mu: ConfigMutator | null, versionRange: string) => mu 
        ? mu 
        : semver.satisfies(packageInfo.version, versionRange) 
          ? versionToMutator[versionRange] 
          : null
      , null) ?? identityMutator,
    [packageInfo.version]
  )
  ...
  const packageInputStreams = packageInfo == null ? [] : getStreamsForInputType(
    'fleet-server',
    packageInfo,
  ...
  const inputValidationResults = useMemo(() => validationResults.inputs!['fleet-server'] ?? {}, [validationResults])
      ...
      <Fragment>
        <PackagePolicyInputConfig
          ...
          updatePackagePolicyInput={(updatedInput: Partial<NewPackagePolicyInput>) => {
            const mutationValues = configMutator.applyMutations(updatedInput, packagePolicyInput)
