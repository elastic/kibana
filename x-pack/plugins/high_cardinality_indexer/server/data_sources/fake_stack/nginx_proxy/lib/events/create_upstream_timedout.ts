...
export const createUpstreamTimeout = (timestamp: Moment, method: string, path: string, domain: string, hostWithPort: string, userId: string) => {
  const host = sample(NGINX_PROXY_HOSTS) as string;
  const connectionCode = faker.random.numeric(3);
  return [{
    namespace: NGINX_PROXY,
    '@timestamp': timestamp.toISOString(),
    message: `[${createNginxTimestamp(timestamp)}] *${connectionCode} upstream timed out (110: Connection timed out) while connecting to upstream, client: ${getClientIp(userId)}, server: ${domain}, request: "${method} ${path} HTTP/1.1", upstream: "http://${hostWithPort}${path}", host: "${domain}"`,
