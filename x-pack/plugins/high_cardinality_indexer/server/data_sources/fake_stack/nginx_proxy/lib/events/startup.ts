...
export const createStartupEvents: EventFunction = (_schedule, timestamp) => {
  return NGINX_PROXY_HOSTS.map((name) => {
    const pid = faker.random.numeric(4);
    const threadId = faker.random.numeric(4);
    return [
      `[${createNginxTimestamp(timestamp)}] [notice] ${pid}#${threadId}: using the "epoll" event method`,
      `[${createNginxTimestamp(timestamp)}] [notice] ${pid}#${threadId}: nginx/1.18.0 (Ubuntu)`,
      `[${createNginxTimestamp(timestamp)}] [notice] ${pid}#${threadId}: built by gcc 7.5.0 (Ubuntu 7.5.0-3ubuntu1~18.04)`,
      `[${createNginxTimestamp(timestamp)}] [notice] ${pid}#${threadId}: OS: Linux 5.4.0-72-generic`,
      `[${createNginxTimestamp(timestamp)}] [notice] ${pid}#${threadId}: getrlimit(RLIMIT_NOFILE): 1024:4096`,
