...
export const createStartupEvents: EventFunction = (_schedule, timestamp) => MONGODB_HOSTS.map((host) => [
  `MongoDB starting : pid=${faker.random.numeric(4)} port=27017 dbpath=/data/db 64-bit host=${host} db version v5.0.0`,
...
].map((message) => createBaseEvent(timestamp, host, 'initandlisten', 'CONTROL', message, 'INFO'))).flat();
