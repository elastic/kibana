...
export const badHost = (all = false): EventFunction => (_schedule, timestamp) => {
  ...
  return [createBaseEvent(timestamp, 'INFO', host, message, sum(latency.counts), processed, latency, 'failure')];
};
