    ...
    'aws': {
      'ec2': {
        'network': {
          ...
          'out': {
            'packets': generateCounterData(`packets-out-${index}`, networkPacketsPerSecOut, interval),
        ...
        'diskio': {
          'read': {
            ...
            'bytes': generateCounterData(`diskio-bytes-read-${index}`, diskBytesPerSecRead, interval),
            'count': generateCounterData(`diskio-count-read-${index}`, diskCountPerSecRead, interval)
          },
          'write': {
            'bytes_per_sec': diskBytesPerSecWrite,
            'count_per_sec': diskCountPerSecWrite,
            'bytes': generateCounterData(`diskio-bytes-write-${index}`, diskBytesPerSecWrite, interval),
            'count': generateCounterData(`diskio-count-write-${index}`, diskCountPerSecWrite, interval)
          }
        },
        'instance': {
          'core': {
