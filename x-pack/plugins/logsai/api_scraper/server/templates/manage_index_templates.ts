...
import { apiScraperBaseComponentTemplateConfig } from './components/base_latest';
import { apiScraperEventComponentTemplateConfig } from './components/event';
...
export const installApiScraperTemplates = async ({
  esClient,
  logger,
}: {
  esClient: ElasticsearchClient;
  ...
  await Promise.all([
    upsertComponent({
      esClient,
      logger,
      component: apiScraperBaseComponentTemplateConfig,
    }),
    upsertComponent({
      esClient,
      logger,
      component: apiScraperEventComponentTemplateConfig,
    }),
  ]);
};
