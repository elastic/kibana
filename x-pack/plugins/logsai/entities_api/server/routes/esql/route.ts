...
const queryEsqlRoute = createEntitiesAPIServerRoute({
  endpoint: 'POST /internal/entities_api/esql',
  params: t.type({
    body: t.intersection([
      t.type({
        query: t.string,
  ...
  handler: async ({ context, logger, params }): Promise<ESQLSearchResponse> => {
    const esClient = await createEntitiesAPIEsClient({ context, logger });
