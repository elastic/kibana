...
export function replaceFieldInGrokPatternByName(
  grokPattern: string,
  fieldName: string,
  originalName: string
) {
  ...
  const qq = splitGrok(grokPattern)
    .map((d) => {
      const { valid, type, name } = getGrokField(d);
      if (valid) {
...
export function removeFieldInGrokPattern(grokPattern: string, originalName: string) {
  if (isAliasPattern(grokPattern)) {
    return grokPattern;
  }
  ...
  return splitGrok(grokPattern)
    .map((d) => {
      const { valid, name } = getGrokField(d);
      if (valid) {
        if (name === originalName) {
          return `.*?`;
        }
