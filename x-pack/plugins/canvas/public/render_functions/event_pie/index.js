...
export const eventPie = () => ({
  name: 'eventPie',
  displayName: 'Event Pie chart',
  help: 'Render a pie chart from data',
  reuseDomNode: false,
  render(domNode, config, handlers) {
    console.log('Rendering event Pie, config is ', config);
          ...
          config.events.forEach(event => {
            if (event.trigger === 'onClick') {
              $(domNode).bind('plotclick', (e, pos, item) => {
                runInterpreter(fromExpression(event.action), {
                  type: 'pointSeriesFilter',
                  value: { filter: item },
                });
              });
