...
} = require('../expression_explorer/constants');
...
run(
  async ({ log, flags }) => {
    ...
    const webpackConfig = path.resolve(EXPRESSION_EXPLORER_SRC, 'webpack.config.js');

    const clean = () => {
      log.info('Deleting previous build.');
      del.sync([EXPRESSION_EXPLORER_FILE], { force: true });
    };
    ...
    clean();
    log.info('Building Element Explorer...');
    execa.sync(
      'yarn',
      [
        'webpack',
  ...
  {
    description: `
      Build script for the Expression Explorer.
    `,
    flags: {
