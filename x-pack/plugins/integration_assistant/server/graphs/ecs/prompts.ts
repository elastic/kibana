...
export const ECS_MAIN_PROMPT = ChatPromptTemplate.fromMessages([
  ...
  3. Replace the value with a new object, and set the nested key "target" to be the full path of the ECS field name. If no confident match is found, the value should always be replaced with null. Also set the nested key "type" to be either "string", "boolean", "number" or "date" depending on what was detected as the example value.
  4. If the type "date" is used, then set date_format to be an array of one or more of the equivilant JAVA date formats that fits the example value. If the type is not date then date_format should be set to an empty array [].
  5. For each key that you set a target ECS field, also score the confidence you have in that the target field is correct, use a float between 0.0 and 1.0 and set the value in the nested "confidence" key.
  6. When you want to use an ECS field as a value for a target, but another field already has the same ECS field as its target, try to find another fitting ECS field. If none is found then the least confident key/value should be null instead.
  7. If you are not confident for a specific field, you should always set the value to null.
  8. These {package_name} log samples are based on source and destination type data, prioritize these compared to other related ECS fields like host.* and observer.*.
