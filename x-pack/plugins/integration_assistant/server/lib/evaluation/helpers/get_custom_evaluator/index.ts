...
import { getExampleEcsResults } from '../get_example_results';
import { getRunEcsResults } from '../get_run_results';
...
export const getCustomEvaluator =
  ...
  async (rootRun, example) => {
    ...
    const exampleEcsResults = getExampleEcsResults(example);
    const runEcsResults = getRunEcsResults(rootRun);
    ...
    const res = await chain.evaluateStrings({
      ...
      prediction: JSON.stringify(runEcsResults, null, 2),
      reference: JSON.stringify(exampleEcsResults, null, 2),
    });
