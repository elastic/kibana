...
function RoutingExplorer({
  ...
  const packageName = 'teleport';
  const dataStreamName = 'audit';
  const rawSamples = [
  ...
  // TODO: All these basic functions and UI is only here for testing purposes
    ...
    try {
      ...
      if (!isFetchError(ecsResponse)) {
        if (Object.keys(ecsResponse?.results).length > 0) {
          ...
          console.log('finished ecs graph successfully');
        } else {
          console.log('finished ecs graph without errors, but no results');
        }
      }
    } catch (e) {
      console.log(e);
  ...
  return (
    <EuiPageTemplate>
      ...
      <EuiPageTemplate.Section>
        <EuiFlexGroup>
          <EuiFlexItem>
            <EuiButton onClick={() => onEcsButtonClick()}>Run ECS Graph</EuiButton>
          </EuiFlexItem>
          <EuiFlexItem>
            <EuiButton onClick={() => onCategorizationButtonClick()}>
              Run Categorization Graph
            </EuiButton>
          </EuiFlexItem>
          <EuiFlexItem>
            <EuiButton onClick={() => onRelatedButtonClick()}>Run Related Graph</EuiButton>
          </EuiFlexItem>
        </EuiFlexGroup>
        <EuiFlexGroup>
          <EuiFlexItem>
