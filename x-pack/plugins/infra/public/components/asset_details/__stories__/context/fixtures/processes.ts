...
export const processesHttpResponse = {
  ...
  onlyProcesses: () => Promise.resolve({ ...processes, summary: {} }),
  onlySummary: () => Promise.resolve({ summary, processList: [] }),
  loading: () => new Promise(() => {}),
  noData: () => Promise.resolve({ summary: {}, processList: [] }),
  error: () =>
    new Promise(() => {
      throw new Error('err');
    }),
...
export type ProcessesHttpMocks = keyof typeof processesHttpResponse;
