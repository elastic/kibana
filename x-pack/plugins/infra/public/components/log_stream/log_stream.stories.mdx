...
### With a `flyoutEnabled` prop

The component can render a button on the row that opens a flyout, via the `flyoutEnabled` prop. The flyout will display the log details for that each entry
