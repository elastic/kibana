...
### Render props for overwritting states
...
By default the `<LogStream />` component will always show the column headers. We can control that by using the `hasColumnHeaders` prop:
