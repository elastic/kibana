...
### Render props for overwriting loading and empty states

By default the `<LogStream />` component will provide default empty and loading states. But if you were to need your own you can use the following props in order to overwrite it:
...
<LogStream
  ...
  renderEmptyState={({ handleReload }) => (
    <div>
      <p>No entries.</p>
      <button>Click here to reload</button>
    </div>
  )}
/>
```
...
### Hidding column headers

By default the `<LogStream />` component will always show the column headers. We can control that by using the `showColumnHeaders` prop:
...
<Canvas>
  <Story name="Hide column headers" args={{ startTimestamp, endTimestamp, showColumnHeaders: false }}>
    {Template.bind({})}
  </Story>
</Canvas>
