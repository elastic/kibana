  ...
  getAttributes(dataView: DataView, options: LensOptions, formula?: FormulaPublicApi) {
    const getLayers = (): PersistedIndexPatternLayer => {
        ...
        columns: {
          hosts_aggs_breakdown: {
            ...
            params: {
              ...
              orderAgg: {
                label: 'Last value of system.network.out.bytes',
                dataType: 'number',
                operationType: 'last_value',
                isBucketed: false,
                scale: 'ratio',
                sourceField: 'system.network.out.bytes',
                filter: {
                  query: 'system.network.out.bytes: *',
                  language: 'kuery',
                },
                params: {
                  sortField: '@timestamp',
      ...
      const dataLayer = formula.insertOrReplaceFormulaColumn(
        ...
        {
          formula:
            "counter_rate(max(system.network.out.bytes), kql='system.network.out.bytes >= 0') * 8",
          timeScale: 's',
          format: {
            id: 'bits',
            params: {
