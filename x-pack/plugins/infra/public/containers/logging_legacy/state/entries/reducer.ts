...
const entriesStartCurrentProgressReducer = reducerWithInitialState(
  ...
  .cases(
    [replaceEntries.started, extendEntriesStart.started],
    createRunningProgressReducer()
  )
  .cases(
    [
...
const entriesStartLastResultReducer = reducerWithInitialState(
  initialEntriesState.start.last
)
  .case(
    replaceEntries.done,
    createSuccessResultReducer(
      ({ count }, { logEntriesBefore }) => logEntriesBefore.length < count
    )
  )
  .case(
    extendEntriesStart.done,
    createSuccessResultReducer(
      ({ count }, { logEntries }) => logEntries.length < count
    )
  )
  .cases(
    [replaceEntries.failed, extendEntriesStart.failed],
    createFailureResultReducer()
  );

const entriesStartPolicyReducer = reducerWithInitialState(
  initialEntriesState.start.policy
);
...
const entriesEndCurrentProgressReducer = reducerWithInitialState(
  initialEntriesState.end.current
)
  .cases(
    [replaceEntries.started, extendEntriesEnd.started],
    createRunningProgressReducer()
  )
  .cases(
    [
...
const entriesEndLastResultReducer = reducerWithInitialState(
  initialEntriesState.end.last
)
  .case(
    replaceEntries.done,
    createSuccessResultReducer(
      ({ count }, { logEntriesAfter }) => logEntriesAfter.length < count
    )
  )
  .case(
    extendEntriesEnd.done,
    createSuccessResultReducer(
      ({ count }, { logEntries }) => logEntries.length < count - 1
    )
  )
  .cases(
    [replaceEntries.failed, extendEntriesEnd.failed],
    createFailureResultReducer()
  );

const entriesEndPolicyReducer = reducerWithInitialState(
  initialEntriesState.end.policy
)
...
const entriesEntriesReducer = reducerWithInitialState(
  initialEntriesState.entries
)
  .case(
    replaceEntries.started,
    (state, { clearEagerly }) => (clearEagerly ? [] : state)
  )
  .case(
    replaceEntries.done,
    (
      ...
      {
        params: { clearEagerly },
        result: { logEntriesBefore, logEntriesAfter },
      }
    ) =>
      clearEagerly
        ? [...logEntriesBefore, ...logEntriesAfter]
        : [...logEntriesBefore, ...logEntriesAfter].map(logEntry => {
            const oldLogEntryIndex = getIndexOfLogEntry(
              state,
  ...
  .case(extendEntriesStart.done, (state, { result: { logEntries } }) => {
    if (logEntries.length > 0) {
      ...
      const prependAtIndex = getIndexNearLogEntry(
        state,
  ...
  .case(extendEntriesEnd.done, (state, { result: { logEntries } }) => {
    if (logEntries.length > 0) {
      ...
      const appendAtIndex = getIndexNearLogEntry(
        state,
        getLogEntryKey(firstLogEntry)
      );
...
const entriesCrossSliceReducer = reducerWithoutInitialState<
  EntriesState
>().case(consolidateEntries, (state, { after, before, target }) => {
...
const entriesVisibleReducer = reducerWithInitialState(
  initialEntriesState.visible
).case(reportVisibleEntries, (state, { startKey, middleKey, endKey }) => ({
  endKey,
  middleKey,
  startKey,
}));
...
export const entriesReducer = (
  ...
  const updatedCrossSliceState = entriesCrossSliceReducer(
    updatedSliceState,
    action
  );
