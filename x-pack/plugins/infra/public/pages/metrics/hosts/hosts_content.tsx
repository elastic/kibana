...
export const HostsContent: React.FunctionComponent<Props> = ({ sourceMetricAlias }: Props) => {
  const {
    services: { lens, dataViews },
  } = useKibana<InfraClientStartDeps>();
  const [dateRange, setDateRange] = useState<TimeRange>({ from: 'now-15m', to: 'now' });
  const [dataView, setDataView] = useState<DataView>();
  const [lensTableAttrs, setLensTableAttrs] = useState<TypedLensByValueInput['attributes'] | null>(
    null
  );
  ...
  useEffect(() => {
    (async () => {
      let metricsDataView = (await dataViews.find(sourceMetricAlias, 1))[0];
      if (!metricsDataView) {
        metricsDataView = await dataViews.createAndSave({
          title: sourceMetricAlias,
          timeFieldName: '@timestamp',
        });
      }
      if (metricsDataView.id) {
  ...
  }, [dataViews, dataView, sourceMetricAlias]);

  const onQuerySubmit = useCallback(
    (payload: { dateRange: TimeRange; query?: Query }) => {
      if (lensTableAttrs && payload.query) {
        setDateRange(payload.dateRange);
        setLensTableAttrs({
          ...lensTableAttrs,
          state: {
            ...lensTableAttrs.state,
            query: payload.query,
          },
        });
      }
    ...
    <div>
      {dataView && lensTableAttrs ? (
        <>
          ...
          />
          <LensComponent id="hostsView" timeRange={dateRange} attributes={lensTableAttrs} />
        </>
      ) : (
        <InfraLoadingPanel
          height="100vh"
