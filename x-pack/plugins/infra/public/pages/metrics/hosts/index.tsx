...
export const HostsPage = () => {
  const { hasFailedLoadingSource, isLoading, loadSourceFailureMessage, loadSource, source } =
    useSourceContext();
  ...
  const settingsCtaButton = (
    <EuiButton data-test-subj="infraHostsPageGoToSettings" {...settingLinkProps}>
      <FormattedMessage
  ...
  if (!remoteClustersExist) {
    return (
      <NoIndices
        title="Missing remote cluster"
        message="Oh no, the remote cluster is not configured"
        actions={[settingsCtaButton]}
      />
    );
  }
  ...
  if (!metricIndicesExist) {
    return (
      <NoIndices
        title="No index found for this config"
        message="Looks like this index does not exists"
        actions={[settingsCtaButton]}
      />
    );
  }
