...
describe('RedirectToLogs component', () => {
  it('renders a redirect with the correct position', () => {
    ...
    expect(component).toMatchInlineSnapshot(`
      <Redirect
        to="/logs/stream?sourceId=default&logPosition=(position:(tiebreaker:0,time:1550671089404))&logFilter=(expression:'',kind:kuery)"
      />
    `);
  });
  ...
  it('renders a redirect with the correct user-defined filter', () => {
    ...
    expect(component).toMatchInlineSnapshot(`
      <Redirect
        to="/logs/stream?sourceId=default&logPosition=(position:(tiebreaker:0,time:1550671089404))&logFilter=(expression:'FILTER_FIELD:FILTER_VALUE',kind:kuery)"
      />
    `);
  });
