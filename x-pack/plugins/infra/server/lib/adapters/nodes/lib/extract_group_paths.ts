...
export function extractGroupPaths(
  ...
  const paths: InfraNode[] = node.path_0.buckets.reduce(
    (acc: InfraNode[], bucket: InfraBucket, index: number): InfraNode[] => {
        ...
        return acc.concat(
          bucket.path_1.buckets.map(
            (b: InfraBucket): InfraNode => {
              ...
              const nodePath = [
                { value: bucket.key.toString(), displayName: bucket.key.toString() },
                { value: b.key.toString(), displayName: b.key.toString() },
      ...
      const path = [{ value: key, displayName: key }].concat(nodeItem.path);
      return acc.concat({
        ...nodeItem,
        path,
      });
