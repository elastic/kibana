...
import { alertRuleEvent, AlertRuleAction } from './audit_events';
...
export class AlertsClient {
  ...
  private async updateApiKeyWithOCC({ id }: { id: string }) {
    ...
    try {
      ...
      this.auditLogger?.log(
        alertRuleEvent({
          action: AlertRuleAction.UPDATE_API_KEY,
          savedObject: { type: 'alert', id },
          error,
        })
      );
  ...
  private async disableWithOCC({ id }: { id: string }) {
    ...
    try {
      ...
      this.auditLogger?.log(
        alertRuleEvent({
          action: AlertRuleAction.DISABLE,
          savedObject: { type: 'alert', id },
          error,
        })
      );
  ...
  private async muteInstanceWithOCC({ alertId, alertInstanceId }: MuteOptions) {
    ...
    try {
      ...
      this.auditLogger?.log(
        alertRuleEvent({
          action: AlertRuleAction.MUTE_INSTANCE,
  ...
  private async unmuteInstanceWithOCC({
    ...
    try {
      ...
      this.auditLogger?.log(
        alertRuleEvent({
          action: AlertRuleAction.UNMUTE_INSTANCE,
          savedObject: { type: 'alert', id: alertId },
          error,
        })
      );
