...
const querySchema = schema.object({
  ...
  instances_muted: schema.maybe(schema.boolean()),
});

export const findAlertsWithInstancesSummaryRoute = (
  router: IRouter,
  licenseState: ILicenseState
) => {
  router.get(
    {
      path: `${BASE_ALERT_API_PATH}/_find_alerts_with_instances_summary`,
      validate: {
        query: querySchema,
      },
    },
    router.handleLegacyErrors(async function (
      ...
      const renameMap = {
        ...
        instances_muted: 'instancesMuted',
      };
