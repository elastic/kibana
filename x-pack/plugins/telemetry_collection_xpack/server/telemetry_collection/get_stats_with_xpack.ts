...
export const getStatsWithXpack: StatsGetter<{}, TelemetryAggregatedStats> = async function (
  ...
  const { callCluster, esClient } = config;
  const clustersLocalStats = await getLocalStats(clustersDetails, config, context);
  const xpack = await getXPackUsage(callCluster, esClient).catch(() => undefined); // We want to still report something (and do not lose the license) even when this method fails.
