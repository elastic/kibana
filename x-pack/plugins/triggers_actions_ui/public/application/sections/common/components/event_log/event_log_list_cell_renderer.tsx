...
export const EventLogListCellRenderer = (props: EventLogListCellRendererProps) => {
  ...
  if (['task_mem_p50', 'task_mem_p95', 'task_mem_p99'].includes(columnId)) {
    return <>{formatMemorySize(Number(value))}</>;
  }

  if (['task_cpu_p50', 'task_cpu_p95', 'task_cpu_p99'].includes(columnId)) {
...
function formatMemorySize(bytes: number, dp = 1) {
