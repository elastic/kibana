...
function useTimefilterService() {
  const { services } = useKibana();
  return services.data.query.timefilter.timefilter;
}

function useUrlStateSyncEffect(stateContainer: StateContainer) {
...
function setupUrlStateSync(stateContainer: StateContainer, stateStorage: IKbnUrlStateStorage) {
  // This handles filling the state when an incomplete URL set is provided
  const setWithDefaults = (changedState: Partial<ContainerState> | null) => {
...
function syncUrlStateWithInitialContainerState(
  ...
  const urlState = urlStateStorage.get<Partial<ContainerState>>('_a');
