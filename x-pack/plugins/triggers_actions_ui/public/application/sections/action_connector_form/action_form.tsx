    ...
    try {
      ...
      const emptyConnectorsActionTypes = actions.reduce(
        (emptyActionTypes: Set<string>, actionItem: AlertAction) => {
          const actionConnector = actionsResponse.find(field => field.id === actionItem.id);
      ...
      if (emptyConnectorsActionTypes.size > 0) {
        toastNotifications!.addDanger({
          title: i18n.translate(
            'xpack.triggersActionsUI.sections.alertForm.unableToLoadConnector',
            {
              defaultMessage: 'No {actionTypes} connector selected.',
