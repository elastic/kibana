...
interface ScoutPlaywrightConfigEntry {
  path: string;
  serverRunFlags: string[];
  usesParallelWorkers?: boolean;
}

interface ScoutPlaywrightConfigGroup {
  name: string;
  group: string;
  type: string;
  configs: ScoutPlaywrightConfigEntry[];
}

let scoutConfigByPath:
  | Map<
      string,
      { readonly serverRunFlags: readonly string[]; readonly usesParallelWorkers?: boolean }
    >
  | undefined;

function normalizeScoutConfigPath(configPath: string): string {
  return configPath.replace(/^[.][/]/, '');
}

function mergeUniqueOrderedStrings(
...
function getScoutConfigManifestPath(): string {
  const rootJson = 'scout_playwright_configs.json';
  if (fs.existsSync(rootJson)) return rootJson;

  const generatedJson = '.scout/test_configs/scout_playwright_configs.json';
  if (fs.existsSync(generatedJson)) return generatedJson;

  throw new Error(
    `Scout Playwright config manifest not found. Expected "${rootJson}" or "${generatedJson}". ` +
      `This file is generated by "node scripts/scout discover-playwright-configs --include-custom-servers --save".`
  );
}

function ensureScoutConfigManifestLoaded() {
  if (scoutConfigByPath) return;

  const manifestPath = getScoutConfigManifestPath();
  const raw = fs.readFileSync(manifestPath, 'utf8');
  const parsed = JSON.parse(raw) as ScoutPlaywrightConfigGroup[];

  if (!Array.isArray(parsed)) {
    throw new Error(`Invalid scout config manifest at "${manifestPath}": expected top-level array`);
  }
  ...
  for (const group of parsed) {
    if (!group || typeof group !== 'object' || !Array.isArray((group as any).configs)) {
      continue;
    }

    for (const cfg of (group as any).configs as ScoutPlaywrightConfigEntry[]) {
      if (!cfg || typeof cfg !== 'object' || typeof (cfg as any).path !== 'string') {
        continue;
      }

      const path = (cfg as any).path as string;
      const serverRunFlags = (cfg as any).serverRunFlags;
      if (!Array.isArray(serverRunFlags) || !serverRunFlags.every((v) => typeof v === 'string')) {
        throw new Error(
          `Invalid "serverRunFlags" for scout config "${path}" in "${manifestPath}". ` +
            `Expected string[].`
        );
      }

      const usesParallelWorkers =
        typeof (cfg as any).usesParallelWorkers === 'boolean'
          ? ((cfg as any).usesParallelWorkers as boolean)
          : undefined;

      // A single Playwright config `path` can appear multiple times across different modules
      // (e.g. `streams_app` is intentionally split by `serverRunFlags`).
      // For flaky-runner, if the user selects the config by path, we merge all discovered run modes.
      const normalizedPath = normalizeScoutConfigPath(path);
      const existing = map.get(normalizedPath);
      if (!existing) {
        map.set(normalizedPath, { serverRunFlags, usesParallelWorkers });
        continue;
      }

      const mergedServerRunFlags = mergeUniqueOrderedStrings(
        existing.serverRunFlags,
        serverRunFlags
      );

      if (
        typeof existing.usesParallelWorkers === 'boolean' &&
        typeof usesParallelWorkers === 'boolean' &&
        existing.usesParallelWorkers !== usesParallelWorkers
      ) {
        throw new Error(
          `Conflicting "usesParallelWorkers" values for scout config "${normalizedPath}" in "${manifestPath}".`
        );
      }

      map.set(normalizedPath, {
        serverRunFlags: mergedServerRunFlags,
        usesParallelWorkers: existing.usesParallelWorkers ?? usesParallelWorkers,
      });
    }
  }

  scoutConfigByPath = map;
}

function getScoutConfigInfo(configPath: string): {
  readonly serverRunFlags: readonly string[];
  readonly usesParallelWorkers?: boolean;
} {
  ensureScoutConfigManifestLoaded();

  const normalizedPath = normalizeScoutConfigPath(configPath);
  const info = scoutConfigByPath!.get(normalizedPath);
  if (!info) {
    const manifestPath = getScoutConfigManifestPath();
    throw new Error(
      `Unable to find scout config "${normalizedPath}" in "${manifestPath}". ` +
        `Make sure the manifest was generated from the current checkout.`
    );
  }
  return info;
}
...
for (const testSuite of testSuites) {
  ...
  if (testSuite.type === 'scoutConfig') {
    const scoutConfigInfo = getScoutConfigInfo(testSuite.scoutConfig);
    const usesParallelWorkers =
      scoutConfigInfo.usesParallelWorkers ??
      testSuite.scoutConfig.endsWith('parallel.playwright.config.ts');
    ...
    steps.push({
      ...
      env: {
        ...
        SCOUT_SERVER_RUN_FLAGS: scoutConfigInfo.serverRunFlags.join('\n'),
      },
