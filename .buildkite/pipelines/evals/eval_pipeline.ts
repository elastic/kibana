...
export interface EvalsSuiteMetadataEntry {
  ...
  /**
   * Buildkite timeout (minutes) for the per-connector fanout steps generated by
   * run_suite.sh (its default is 120). Bump for suites whose healthy runs get close
   * to that limit, so a slow model doesn't burn the full timeout and retry from scratch.
   */
  ...
  const suiteSteps = selectedSuites
    .map((suite) => {
      ...
      const evalServerConfigSetEnv = suite.serverConfigSet
        ...
        : null;
      const { stepTimeoutInMinutes } = suite;
      const stepTimeoutEnv =
        typeof stepTimeoutInMinutes === 'number' &&
        Number.isInteger(stepTimeoutInMinutes) &&
        stepTimeoutInMinutes > 0
          ? `          EVAL_STEP_TIMEOUT_IN_MINUTES: '${stepTimeoutInMinutes}'`
          : null;
      return [
        `      - label: '${label}'`,
        `        key: ${key}`,
