...
(async () => {
  ...
  try {
    ...
    // Register build_scout_tests for cancel-on-gate-failure. This step is defined in base.yml
    // but base.yml is not loaded with cancelOnGateFailure (it contains gate steps themselves),
    // so we register it explicitly.
    execFileSync('buildkite-agent', [
      'meta-data',
      'set',
      'cancel_on_gate_failure:build_scout_tests',
      'true',
    ]);
    ...
    if (
      ...
      pipeline.push(
        getPipeline('.buildkite/pipelines/pull_request/synthetics_plugin.yml', cancelable)
      );
      pipeline.push(getPipeline('.buildkite/pipelines/pull_request/uptime_plugin.yml', cancelable));
      pipeline.push(
        getPipeline('.buildkite/pipelines/pull_request/exploratory_view_plugin.yml', cancelable)
      );
      pipeline.push(getPipeline('.buildkite/pipelines/pull_request/ux_plugin_e2e.yml', cancelable));
    }
