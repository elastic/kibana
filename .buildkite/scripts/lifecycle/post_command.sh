...
if [[ "$IS_TEST_EXECUTION_STEP" == "true" ]]; then
  ...
  buildkite-agent artifact upload 'target/kibana-security-solution/**/*.{png,tar.gz}'
  buildkite-agent artifact upload 'target/kibana-security-solution/**/merged-mochawesome-report.json'
