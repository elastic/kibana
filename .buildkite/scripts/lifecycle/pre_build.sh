...
if [[ ! "${ES_SNAPSHOT_MANIFEST:-}" ]]; then
  # no-op-1 BUCKET=$(curl -s "https://storage.googleapis.com/kibana-ci-es-snapshots-daily/$(cat package.json | jq -r .version)/manifest-latest-verified.json" | jq -r .bucket)
