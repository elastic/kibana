...
# Sourced before the release branch checkout below so the helpers come from the build commit.
...
# For minor bumps targeting main, package.json must advance to the next dev version
# (e.g. NEW_VERSION=9.5.0 → TARGET_VERSION=9.6.0) so main doesn't stay pinned to
# the released version after the branch is cut.
if [[ "${WORKFLOW:-}" == "minor" && "$branch_to_merge_into" == "main" ]]; then
...
update_package_json_version() {
  ...
  sed -i "s/\"version\": \"[0-9]\+\.[0-9]\+\.[0-9]\+\"/\"version\": \"${TARGET_VERSION}\"/g" package.json
  sed -i "s/\"version\": \"[0-9]\+\.[0-9]\+\.[0-9]\+\"/\"version\": \"${TARGET_VERSION}\"/g" x-pack/package.json
}
...
update_initial_app_data() {
  ...
  if [[ -f "$target" ]]; then
    sed -i "s/kibanaVersion: '[0-9]\+\.[0-9]\+\.[0-9]\+'/kibanaVersion: '${TARGET_VERSION}'/" "$target"
  else
...
update_kibana_migrator_utils() {
  ...
  sed -i "s/BASELINE_ELASTICSEARCH_VERSION = '[0-9]\+\.[0-9]\+\.[0-9]\+'/BASELINE_ELASTICSEARCH_VERSION = '${TARGET_VERSION}'/" "$target"
}
...
git commit -m "[version bump] Bump version to ${TARGET_VERSION}"
# The Buildkite App token lacks the workflows permission, so GitHub diffs .github/workflows on
# every push and rejects it if that check times out, which it does on this push (the regenerated
# baseline archives). The kibanamachine token has the permission, so GitHub skips the check.
GITHUB_PR_OWNER=elastic GITHUB_PR_REPO=kibana GITHUB_PR_BRANCH="$head_branch" \
  push_as_github_token

prUrl=$(gh pr create --repo elastic/kibana --base "$branch_to_merge_into" --head "$head_branch" --title "[bump version] $(date +%F) Bump package.json versions to ${TARGET_VERSION}" --body "Generated by ${BUILDKITE_BUILD_URL}" --label "release_note:skip" --label "backport:skip")
