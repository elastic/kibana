...
BASE_COSMOSDB_REPO=mcr.microsoft.com/cosmosdb/linux/azure-cosmos-emulator
TARGET_IMAGE=docker.elastic.co/kibana-ci/uiam-azure-cosmos-emulator:latest-verified

SOURCE_IMAGE_OR_TAG=${1:-}
if [[ -z "$SOURCE_IMAGE_OR_TAG" ]]; then
  echo "Usage: $0 <image-or-tag>"
  echo "Example: $0 mcr.microsoft.com/cosmosdb/linux/azure-cosmos-emulator:vnext-preview"
  echo "Example: $0 vnext-preview"
  exit 1
fi
...
if [[ $SOURCE_IMAGE_OR_TAG =~ :[a-zA-Z0-9_.-]+$ ]]; then
  ...
  SOURCE_IMAGE="$BASE_COSMOSDB_REPO:$SOURCE_IMAGE_OR_TAG"
fi
...
UIAM_COSMOSDB_COMMIT_HASH=$(echo "$ORIG_IMG_DATA" | jq -r '.[].Config.Labels["com.visualstudio.msdata.image.build.sourceversion"] // "unknown"')
...
cat << EOT | buildkite-agent annotate --style "success"
  ...
  <br/>UIAM Cosmos DB Emulator commit: $UIAM_COSMOSDB_COMMIT_HASH
EOT

cat << EOF | buildkite-agent pipeline upload
steps:
  - label: "Update cache for UIAM Cosmos DB Emulator image"
