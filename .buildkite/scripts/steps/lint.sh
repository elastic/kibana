...
if is_pr && ! is_auto_commit_disabled; then
  git ls-files | grep -E '\.(js|mjs|ts|tsx)$' | xargs -n 250 -P 4 npx @biomejs/biome ci . || true && node scripts/eslint --no-cache --fix
else
  git ls-files | grep -E '\.(js|mjs|ts|tsx)$' | xargs -n 250 -P 4 npx @biomejs/biome ci . || true && node scripts/eslint --no-cache
fi
