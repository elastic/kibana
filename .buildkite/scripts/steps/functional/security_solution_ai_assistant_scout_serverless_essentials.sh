...
export JOB=kibana-security-solution-ai-assistant-scout-serverless-essentials

SCOUT_CONFIG="x-pack/solutions/security/plugins/security_solution/test/scout/ui/ai_assistant.playwright.config.ts"

echo "--- Running AI Assistant Scout tests (serverless security essentials): $SCOUT_CONFIG"
...
if [[ "${SCOUT_REPORTER_ENABLED:-}" =~ ^(1|true)$ ]]; then
  if [ -d ".scout/reports" ] && [ "$(ls -A .scout/reports 2>/dev/null)" ]; then
    echo "--- Upload Scout reporter events for AI Assistant serverless security essentials tests"
