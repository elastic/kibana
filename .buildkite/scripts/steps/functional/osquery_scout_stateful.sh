...
export JOB=kibana-osquery-scout-stateful

echo "--- Osquery Scout (Playwright) Stateful tests"
...
if [[ "${SCOUT_REPORTER_ENABLED:-}" =~ ^(1|true)$ ]]; then
  if [ -d ".scout/reports" ] && [ "$(ls -A .scout/reports 2>/dev/null)" ]; then
    echo "--- Upload Scout reporter events for Osquery Scout Stateful tests"
