...
deploy() {
  ...
  PROJECT_VAULT_NAME=$PROJECT_NAME
  ...
  if [ -z "${PROJECT_ID}" ] || [ "$PROJECT_ID" = 'null' ]; then
    ...
    if [[ "$IS_LEGACY_VAULT_ADDR" == "true" ]]; then
      vault_set "cloud-deploy/$PROJECT_VAULT_NAME" username="$PROJECT_USERNAME" password="$PROJECT_PASSWORD" id="$PROJECT_ID"
    else
      vault_kv_set "cloud-deploy/$PROJECT_VAULT_NAME" username="$PROJECT_USERNAME" password="$PROJECT_PASSWORD" id="$PROJECT_ID"
    fi
  ...
  # TODO: remove after https://github.com/elastic/kibana-operations/issues/15 is done
  if [[ "$IS_LEGACY_VAULT_ADDR" == "true" ]]; then
    VAULT_READ_COMMAND="vault read $VAULT_PATH_PREFIX/cloud-deploy/$PROJECT_VAULT_NAME"
  else
    VAULT_READ_COMMAND="vault kv get $VAULT_KV_PREFIX/cloud-deploy/$PROJECT_VAULT_NAME"
  fi
