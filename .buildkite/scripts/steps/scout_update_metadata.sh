...
if [[ "$existing_pr_title" == "$PR_TITLE" ]]; then
  ...
  existing_pr_url="https://github.com/elastic/kibana/pull/$existing_pr_number"
  echo "Updated existing PR: $existing_pr_url"
  buildkite-agent annotate "Updated PR: $existing_pr_url" --style info --context scout-metadata-pr
  exit 0
fi
...
buildkite-agent annotate "Created PR: $pr_url" --style info --context scout-metadata-pr
