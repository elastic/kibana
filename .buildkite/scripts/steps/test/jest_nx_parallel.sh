...
while read -r config; do
  # find containing foler of jest.config.js
  configDir=$(dirname "$config")
  # find project.json in the same folder
  projectJson="$configDir/project.json"
  # find the project name in project.json
  projectName=$(jq -r '.name' "$projectJson")

  if [[ "$projectName" == "" ]]; then
    echo "[*!*] unable to determine project name for $config" >&2
  fi

#  echo "--- $ node scripts/jest --config $config"
  echo "--- $ nx jest $projectName"
  ...
  cmd=$cmd" yarn nx run jest $projectName --coverage=false --passWithNoTests"
