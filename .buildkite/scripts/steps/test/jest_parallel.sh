...
while read -r config; do
  echo "--- $ node --expose-gc scripts/jest --config $config"
  ...
  cmd=$cmd"\" node --expose-gc ./scripts/jest  --config=\"$config\" $parallelism --logHeapUsage --coverage=false --passWithNoTests"
