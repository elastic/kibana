...
function getDashboardForNonProd(commit: string) {
  return link(
    'GPCTL Deployment Status dashboard for nonprod',
...
function getDashboardForProd(commit: string) {
  return link(
    'GPCTL Deployment Status dashboard for prod',
...
<div>:link: ${link(
    'Commits contained in deploy',
    `https://github.com/elastic/kibana/compare/${data.previousCommitHash}...${data.selectedCommitHash}`
  )}</div>
<div>:link: ${link(
    'Argo Workflow',
    `https://argo-workflows.us-central1.gcp.qa.cld.elstc.co/workflows?label=hash%3D${data.selectedCommitHash}`
  )}</div>
<div>:link: ${getDashboardForNonProd(data.selectedCommitHash)}</div>
<div>:link: ${getDashboardForProd(data.selectedCommitHash)}</div>
<div>:link: ${link(
    'Quality Gate pipeline',
    'https://buildkite.com/elastic/kibana-tests/builds?branch=main'
  )}</div>
`;
}
