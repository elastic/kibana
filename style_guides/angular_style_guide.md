...
Kibana is written in Angular, and these are the conventions we follow. Most of the decisions here come directly from the [John Papa Angular 1 Style Guide](https://github.com/johnpapa/angular-styleguide/blob/master/a1/README.md).
...
### Rule of 1
  ...
  - Define 1 component per file, recommended to be less than 400 lines of code. This promotes easier unit testing and mocking, and makes code easier to read.
...
### Small Functions
  ...
  - Define small functions, no more than 75 LOC (less is better). Small functions promote reuse, are easier to read, and are easier to test, especially when they do one thing and serve one purpose.
...
### controllerAs View Syntax
  ...
  - Use the [`controllerAs`](http://www.johnpapa.net/do-you-like-your-angular-controllers-with-or-without-sugar/) syntax over the classic controller with `$scope` syntax. Controllers are constructed, "newed" up, and provide a single new instance, and the `controllerAs` syntax is closer to that of a JavaScript constructor than the classic `$scope` syntax.
...
### controllerAs Controller Syntax
  ...
  - Use the `controllerAs` syntax over the classic controller with `$scope` syntax. `controllerAs` is syntactic sugar over `$scope` and uses `this` inside controllers which gets bound to `$scope`. This way, you can still bind to the View and still access `$scope` methods.
...
### controllerAs and `this`

  - Use arrow functions to avoid the need to alias `this`. 

  ```javascript
  /* avoid */
  function CustomerController() {
      var self = this;
      self.sendMessage = function() { return self.name; };
  }
  ```
  ...
  /* recommended */
  function CustomerController() {
      this.sendMessage = () => { return this.name; };
  }
  ```

### Bindable Members Up Top

  - Place bindable members at the top of the controller, alphabetized, and not spread through the controller code. Doing so makes it easy to read and helps you instantly identify which members of the controller can be bound and used in the View
...
### Defer Controller Logic to Services
  ...
  - Defer logic in a controller by delegating to factories. This way, logic may be reused by multiple controllers, and more easily tested. 
...
### Return a service instance from factories
  
  - Expose the callable members of the service (its interface) via a class. Define private methods outside of the class if needed, but keep their scope small. 

  ```javascript
  /* avoid object literals and the revealing module pattern */
