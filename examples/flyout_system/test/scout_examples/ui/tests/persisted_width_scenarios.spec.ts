...
const EUI_SIBLING_CLAMP_FIX = 'https://github.com/elastic/eui/pull/10075';
const NO_WIDTH = '(none)';
...
test.describe(
  ...
  () => {
    ...
    // eslint-disable-next-line playwright/no-skipped-test -- fails until elastic/eui#10075 ships in Kibana
    test.skip(
      'second system push flyout opens at the width stored by resizing the first',
      {
        annotation: {
          type: 'skip',
          description: `Fails on Kibana main: EUI clamps the new main flyout against the previous main as if it were a sibling, so Q opens narrower than the stored width. Fixed by ${EUI_SIBLING_CLAMP_FIX}.`,
        },
      },
      async ({ page }) => {
