...
const ParentFlyoutContent: React.FC<{ overlays: OverlayStart; onClose: () => void }> = ({
  ...
  const openChildFlyout = (name: string) => {
    overlays.openSystemFlyout(<ChildFlyoutContent />, {
      id: `child-flyout-demo-${name}`,
      title: `Child Flyout (session: inherit) - ${name}`,
      session: 'inherit',
      size: 's',
      type: 'overlay',
      outsideClickCloses: false,
  ...
  return (
    <>
      ...
      <EuiFlyoutBody>
        ...
        <EuiSpacer size="m" />
        <EuiFlexGroup>
          <EuiButton onClick={() => openChildFlyout('1')} iconType="arrowRight">
            Open Child Flyout
          </EuiButton>
          <EuiButton onClick={() => openChildFlyout('2')} iconType="arrowRight">
            Open another Child Flyout
          </EuiButton>
        </EuiFlexGroup>
        <EuiSpacer size="l" />
