...
| --- | --- |
| Create service account (Accounts tab, or Attach → Create and attach) | **Core browser** `core.security.serviceAccounts.create` |
...
| Run as service account | **Core** `withScopedRequest` (ES `_security/_authenticate` + `getCurrentUser`) **and** `getLoopbackAuthHeaders` (loopback HTTP to this plugin's `whoami`). Tokens never reach the browser. `lastRun` is persisted on the job. |
| Inspector → Create via server Core API | **Core server** `core.security.serviceAccounts.create` |
...
3. **Run as service account**. Compare the three identity cards: **You** (this session), **Scoped request**, **Loopback HTTP**.
...
5. `withScopedRequest` / `getLoopbackAuthHeaders` to execute — never mint or store tokens itself
