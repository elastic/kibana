    ...
    <>
      ...
      <EuiFlexGroup>
        ...
        <EuiFlexItem>
          <Card
            title="Loopback HTTP"
            subtitle="handle.getLoopbackAuthHeaders → this plugin's whoami"
          >
            {!lastRun && (
              <EuiText size="s" color="subdued">
                <p>Run the job to populate this card.</p>
            ...
            {lastRun?.loopback.error && (
              <EuiBadge color="danger">{lastRun.loopback.error}</EuiBadge>
            )}
            {lastRun && !lastRun.loopback.error && (
              <>
                {lastRun.loopback.status != null && (
                  <EuiBadge>{`HTTP ${lastRun.loopback.status}`}</EuiBadge>
                )}
                <EuiSpacer size="s" />
                <EuiCodeBlock language="json" overflowHeight={200} isCopyable>
                  {JSON.stringify(lastRun.loopback.whoami, null, 2)}
                </EuiCodeBlock>
              </>
            )}
          </Card>
