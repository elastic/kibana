...
export const AccountsDirectory = ({ coreStart, onLogged, busy }: Props) => {
  ...
  const [roleAssignmentsText, setRoleAssignmentsText] = useState('{}');
  ...
  const parseRoleAssignments = (): Record<string, unknown> | undefined => {
    try {
      const parsed = JSON.parse(roleAssignmentsText) as unknown;
  ...
  const onCreate = async () => {
    const roleAssignments = parseRoleAssignments();
    if (!roleAssignments) {
      setListError('role_assignments must be a JSON object');
      return;
    }
    await onLogged('core.security.serviceAccounts.create (browser)', async () => {
      const created = await security.serviceAccounts.create({
        name,
  ...
  return (
    <EuiPanel paddingSize="m">
      ...
      <EuiForm>
        ...
        <EuiAccordion id="sa-example-advanced-roles" buttonContent="Advanced: role_assignments">
          <EuiFormRow helpText="JSON object. Default {} until the schema is specified.">
            <EuiTextArea
              value={roleAssignmentsText}
              onChange={(event) => setRoleAssignmentsText(event.target.value)}
              rows={4}
            />
          </EuiFormRow>
        </EuiAccordion>
