...
import { loopbackGet, type LoopbackHttpResult } from './loopback';
...
const createAccountBodySchema = schema.object({
  ...
  role_assignments: schema.recordOf(
    schema.string({ minLength: 1, maxLength: ID_MAX_LENGTH }),
    schema.any()
  ),
});
...
const redactedHeaderSummary = (headers: Record<string, string>) => ({
  ...
  router.post(
    ...
    async (context, request, response) => {
      try {
        ...
        let loopbackResult: JobLastRun['loopback'];
        try {
          const headers = await operationHandle.getLoopbackAuthHeaders(coords);
          const result: LoopbackHttpResult = await loopback({
            httpSetup: http,
            path: WHOAMI_PATH,
            headers: {
              ...headers,
          ...
          loopbackResult = {
            ...
            whoami: result.body,
            loopbackAuthHeaders: redactedHeaderSummary(headers),
          };
        } catch (error) {
          loopbackResult = { error: error instanceof Error ? error.message : String(error) };
        }
