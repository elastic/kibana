...
describe('example job routes', () => {
  const setup = () => {
    ...
    return { router, soClient, operationHandle, loopback, context, handlerFor, coreStart };
  };
  ...
  it('runs both execution modes, persists lastRun, and never returns tokens', async () => {
    const { handlerFor, context, operationHandle, loopback, soClient } = setup();
    operationHandle.getBinding.mockResolvedValue(binding);
    operationHandle.withScopedRequest.mockImplementation(async (_coords, fn) =>
      fn(httpServerMock.createFakeKibanaRequest({ headers: {} }))
    );
    operationHandle.getLoopbackAuthHeaders.mockResolvedValue({
      authorization: 'Bearer essu_secret-token',
      'x-elastic-internal-origin': 'Kibana',
    });
    loopback.mockResolvedValue({
      url: 'http://127.0.0.1/internal/service_accounts_example/whoami',
      status: 200,
      body: { currentUser: { username: 'sa-user' } },
    });
      ...
      expect.objectContaining({
        lastRun: expect.objectContaining({
          ...
          loopback: expect.objectContaining({
            status: 200,
            loopbackAuthHeaders: {
              names: ['authorization', 'x-elastic-internal-origin'].sort(),
    ...
    expect(response.payload.job.lastRun.loopback.loopbackAuthHeaders.hasAuthorization).toBe(true);
  });
