...
interface QuantitativeHistogramProps {
  field: string;
  width: number;
  height: number;
}

export const QuantitativeHistogram: FC<QuantitativeHistogramProps> = (props) => {
  const { field } = props;
  ...
  const esqlWithFilters = useMemo(() => {
    if (esql === '' || range === undefined) return null;

    const els = esql.split('|').map((d) => d.trim());

    if (Object.values(panelFilters).length === 0) {
      els.push(
        `STATS count = COUNT(*), context = COUNT(*) WHERE ${field}==${
          range[0] - 1
        }, total = COUNT(*) BY bucket = BUCKET(${field}, 20, ${range[0]}, ${range[1]})`
      );
    } else {
      const filter = Object.values(panelFilters).join(' AND ');
      els.push(
  ...
  React.useEffect(() => {
    ...
    const spec = vl
      ...
      .encode(
        ...
        vl.x().fieldQ('name').bin({ maxbins: 20 }).axis({ title: '' }),
        vl
          .color()
          .fieldN('type')
