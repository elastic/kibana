...
export const plugin = async () => {
  const { DiGlobalAlphaPlugin } = await import('./plugin');
  return new DiGlobalAlphaPlugin();
};
 ...
 */
export const services = declare<IAlphaService>(({ bind, provide }) => {
  provide(AlphaServiceToken, (start) => start);
