...
const authorRole = (extraIndices = []) => ({
  ...
  indices: [
    { names: ['.lists-*', '.items-*', ...extraIndices], privileges: ['read', 'write', 'view_index_metadata'] },
    { names: ['poc-src', '.alerts-security.alerts-*', '.internal.alerts-*'], privileges: ['read', 'write', 'view_index_metadata', 'manage'] },
  ],
});
...
const cleanup = async () => {
  const rules = await kbn('GET', `/api/detection_engine/rules/_find?filter=${encodeURIComponent(`alert.attributes.name: "${RULE_NAME}"`)}`);
  for (const rule of rules.json?.data ?? []) await kbn('DELETE', `/api/detection_engine/rules?id=${rule.id}`);
  ...
  for (const u of [WILDCARD_USER, CONCRETE_USER, AUTHOR_USER]) await es('DELETE', `/_security/user/${u}`);
  for (const r of [WILDCARD_ROLE, CONCRETE_ROLE, AUTHOR_ROLE]) await es('DELETE', `/_security/role/${r}`);
};
...
const main = async () => {
  ...
  await es('PUT', `/_security/role/${WILDCARD_ROLE}`, {
    applications: [{ application: 'kibana-.kibana', privileges: ['all'], resources: ['*'] }],
    indices: [{ names: ['.lists-*', '.items-*'], privileges: ['manage', 'write', 'read', 'view_index_metadata'] }],
  });
  await es('PUT', `/_security/role/${CONCRETE_ROLE}`, { indices: [{ names: ['.lists-*', INDEX], privileges: ['read', 'view_index_metadata'] }] });
  await es('PUT', `/_security/user/${WILDCARD_USER}`, { password: PASSWORD, roles: [WILDCARD_ROLE] });
  await es('PUT', `/_security/user/${CONCRETE_USER}`, { password: PASSWORD, roles: [CONCRETE_ROLE] });

  const created = await kbn('POST', '/api/lists', { id: LIST, name: LIST, description: 'restrict poc', type: 'ip' });
  if (created.status >= 400) throw new Error(`create failed: ${created.status} ${JSON.stringify(created.json)}`);
  check('locator records concrete index and alias', created.json.storage?.locator?.index === INDEX && created.json.storage?.locator?.alias === ALIAS, JSON.stringify(created.json.storage));
  ...
  const ownCreate = await kbn('POST', '/api/lists', { id: ownList, name: ownList, description: 'x', type: 'ip' }, wh);
  check('wildcard-role user creates a lookup list', ownCreate.status === 200 && ownCreate.json.storage?.locator?.alias === `.items-${SPACE}-${ownList}`, `${ownCreate.status} ${JSON.stringify(ownCreate.json.message ?? ownCreate.json.storage)}`);
  const ownItem = await kbn('POST', '/api/lists/items', { list_id: ownList, value: '10.55.55.55' }, wh);
  ...
  check('wildcard-role user deletes the list', ownDelete.status === 200 && (await es('HEAD', `/.value-list-v2-${SPACE}-${ownList}`)).status === 404);

  log('\n=== collision ===');
  const collide = await kbn('POST', '/api/lists', { id: COLLIDE, name: 'collide', description: 'x', type: 'ip' });
  check('case-variant id rejected with 409', collide.status === 409, `${collide.status} ${collide.json?.message ?? ''}`);
  ...
  check('dry run returns 200 with access still shared', dry.status === 200 && dry.json.access === 'shared' && dry.json.changed === false);
  ...
  const exc = await kbn('POST', '/api/exception_lists', { list_id: EXC_LIST, name: EXC_LIST, description: 'x', type: 'detection', namespace_type: 'single' }, ah);
  if (exc.status >= 400) throw new Error(`author exception list failed: ${exc.status} ${JSON.stringify(exc.json)}`);
  const excItem = await kbn('POST', '/api/exception_lists/items', {
    list_id: EXC_LIST, name: 'in list', description: 'x', type: 'simple', namespace_type: 'single',
    entries: [{ field: 'destination.ip', type: 'list', operator: 'included', list: { id: LIST, type: 'ip' } }],
  }, ah);
  if (excItem.status >= 400) throw new Error(`author exception item failed: ${excItem.status} ${JSON.stringify(excItem.json)}`);
  const rule = await kbn('POST', '/api/detection_engine/rules', {
    name: RULE_NAME, description: 'x', risk_score: 1, severity: 'low', type: 'query', query: '*:*', index: ['poc-src'],
    from: 'now-1h', interval: '1h', enabled: true,
    exceptions_list: [{ id: exc.json.id, list_id: EXC_LIST, type: 'detection', namespace_type: 'single' }],
  }, ah);
  if (rule.status >= 400) throw new Error(`author rule failed: ${rule.status} ${JSON.stringify(rule.json)}`);
  await sleep(1500);
  ...
  check('dry run finds the author rule through its exception', entry?.reason === 'exception', JSON.stringify(entry));
  check('dry run reports the rule key cannot read the concrete index', entry?.canRead === false && entry?.apiKeyOwner === AUTHOR_USER);
  const blocked = await kbn('POST', '/internal/lists/_restrict', { id: LIST }, ih);
  check('restrict without force is blocked with 409', blocked.status === 409 && blocked.json.attributes?.access === 'shared' && String(blocked.json.message).includes('blocked'), String(blocked.json.message).slice(0, 160));
  ...
  check('force restricts anyway', forced.status === 200 && forced.json.access === 'restricted' && forced.json.changed === true);
  const backToShared = await kbn('POST', '/internal/lists/_unrestrict', { id: LIST }, ih);
  check('unrestrict after the forced restrict', backToShared.status === 200 && backToShared.json.alias === ALIAS);
  ...
  const saved = await kbn('PATCH', '/api/detection_engine/rules', { id: rule.json.id, description: 'saved after role grant' }, ah);
  ...
  check('after saving the rule its key reads the concrete index', freshEntry?.canRead === true, JSON.stringify(freshEntry));
  check('dry run no longer reports a blocker', typeof freshDry.json.message === 'string' && !freshDry.json.message.includes('blocked'));

  log('\n=== restrict ===');
  const restricted = await kbn('POST', '/internal/lists/_restrict', { id: LIST }, ih);
  log(`  response: ${JSON.stringify(restricted.json)}`);
  check('restrict returns 200 and access restricted', restricted.status === 200 && restricted.json.access === 'restricted' && restricted.json.changed === true);
  const afterMeta = await kbn('GET', `/api/lists?id=${LIST}`);
  check('locator dropped the alias', afterMeta.json.storage?.locator?.alias == null && afterMeta.json.storage?.locator?.index === INDEX, JSON.stringify(afterMeta.json.storage));
  check('alias removed in Elasticsearch', (await es('HEAD', `/_alias/${ALIAS}`)).status === 404);
  check('wildcard role lost access', !(await canCount(WILDCARD_USER, ALIAS)) && !(await canCount(WILDCARD_USER, INDEX)));
  check('concrete role keeps access', await canCount(CONCRETE_USER, INDEX));
  const exported = await fetch(`${KBN}/api/lists/items/_export?list_id=${LIST}`, { method: 'POST', headers: kh }).then((r) => r.text());
  ...
  check('unrestrict returns 200 with the alias', un.status === 200 && un.json.alias === ALIAS && un.json.changed === true);
