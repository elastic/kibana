...
In order to keep track of their stability, the #kibana-operations team has developed a special set of
...
The machine has these [specs](https://www.hetzner.com/dedicated-rootserver/ex100).

Since the tests are run on a local machine, there is also realistic throttling applied to the network to 
simulate real life internet connection. This means that all requests have a 100 ms latency and 6 mb/s bandwidth.
...
Please contact the #kibana-operations team if you need more help visualising and tracking the results.
