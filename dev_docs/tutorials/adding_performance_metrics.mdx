...
# What can be defined as metric?

What kind of metrics we consider? How much time it takes to start Kibana server, create saved objects or render dashboard with multiple visualizations
just to name a few.


#How do we collect metrics?
We use event-based telemetry (EBT) to report metrics to Elastic 'stack-telemetry' Staging cluster.
...
In order to better understand the performance of Kibana features, e.g. how much  monitor performance of Kibana The process can be divided into 2 part
