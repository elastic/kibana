...
Once we have the time measurment, we can use the `reportPerformanceMetricEvent` API to report it.
...
It we also be mapped into an additional index, dedicated to performance metrics.
