...
On serverless, Kibana cannot have any downtime &mdash; even during data migrations. In order to accomplish this we rollout new versions while old versions are still running. As soon as new versions are ready to start serving traffic requests will be directed to them.
...
<DocCallOut color="warning" title="What happens when you skip step 4?">
  Not waiting until your `data_backfill` has been released means that none of your SO documents will have the field populated with the value you provided in the `data_backfill` function.
</DocCallOut>
