...
### How can I do a "Runtime" migration?

Over time, you may find you need to adjust a saved objects' schema. While these changes have historically been introduced through migrations, there's a limit as to how much a schema can change without introducing a breaking change. Another option, which is the prefered way, is to make the change within your domain-specific API layer. 

There are 2 options, depending on the amount of felxibility you need:
For complete control over yuor HTTP APIs and custom handling, declare your type as `hidden`, as shown in the example. 
