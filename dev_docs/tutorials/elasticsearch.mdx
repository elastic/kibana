...
2. If the index already exists (or creation succeeds), wait until the index status is **yellow** before proceeding.
...
- After creation (or if the index already exists), always wait for the index to reach **yellow** status before indexing or searching. This ensures all primary shards are available, preventing failed operations.
