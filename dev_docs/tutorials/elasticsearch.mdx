...
## Example: Searching as the Current User

Here is a minimal example of searching an index as the current user from a route handler:
...
router.get(
  ...
  async (context, request, response) => {
    ...
    const result = await esClient.search({
      index: 'my-index',
      body: { query: { match_all: {} } }
      allow_partial_search_results: true // Optional: allows partial results if some shards are unavailable
    });
    return response.ok({ body: result });
  }
);
...
## Example: Initializing, Indexing, and Searching an Index
...
### Example: Safe Index Initialization and Usage in the Plugin Start Lifecycle
