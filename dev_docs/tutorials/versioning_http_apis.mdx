...
#### Minimal changes to make the above endpoint versionable
...
You may be accepting unexpected values as input to your HTTP APIs. For example
...
router.post(
  {
    path: '/api/myobject',
    validate: { body: schema.object({ duration: schema.number() }) },
  },
  (ctx, req, res) => {
    const { savedObjects } = await ctx.core;
...
This HTTP API currently accepts all numbers as input which allows for unexpected inputs like negative numbers to be provided and persisted. This may break existing or future migrations or integrations that assume the number will non-negative.

```ts
...
    validate: { body: schema.object({ duration: schema.number({ min: 0 }) }) },
...
```

Adding this validation to `schema.number` we remove the risk of unexpected values. It is not necessary to use `@kbn/config-schema`, as long as your validation mechanism provides finer grained controls than "number" or "string".
