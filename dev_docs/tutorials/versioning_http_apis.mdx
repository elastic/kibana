...
All Kibana HTTP API developers and maintainers must ensure that past versions of HTTP APIs remain available and usable according to backwards-compatibilty practices.
...
<DocAccordion buttonContent="FAQ: What is the difference between an internal and a public HTTP API?">
  <DocCallOut >
    We only declare HTTP APIs as public that are intended for 3rd party integration. All other HTTP APIs should be internal. Public HTTP APIs have minimum deprecation windows of 18 months and must go through breaking changes committee before they can be modified in a breaking way. New API versions to public HTTP APIs cannot be created without prior breaking changes committee approval, because a new API version implies a breaking change. Internal HTTP APIs may change more frequently, as long as they adhere to the versioning principles outlined in the rest of this doc.
  </DocCallOut>
</DocAccordion>
...
Public APIs can have one of the following statuses: `experimental`, `beta` or `stable`. This can be configured when creating the route using the router interface:
...
**`experimental`** - subject to breaking changes (or removal) with no notice period

**`beta`** - subject to shortened deprecation windows

**`stable`** - subject to a minimum 18 month deprecation window and is recommended for 3rd party integrations

### Creating versionable HTTP APIs in Kibana

Versioned HTTP APIs should hold to the following requirements. **Note:** how you meet these requirements is up to you. Use the examples provided as a guide.
