...
<DocCallOut title="A note on internal vs public HTTP APIs">
  ...
  The exact number of past APIs and the length of time they are kept available will vary per use case. Generally, the timeframe will be shorter for internal HTTP APIs than for public HTTP APIs.
</DocCallOut>
...
You may be accepting unexpected values as input to your HTTP APIs.

```ts
router.post(
  {
    path: '/api/myobject',
    validate: { body: schema.object({ num: schema.number() }) },
  },
  (ctx, req, res) => {
    const { savedObjects } = await ctx.core;
    const myObject = await savedObjects.client.create<MyObjectAttributes>('myObject', req.params.id)
    return res.ok({
      body: { ...myObject.attributes }, // <- Directly exposing SO attributes
    })
  }
