...
> [!IMPORTANT]
> **Version all public endpoints**: All Kibana's public endpoints must be versioned using the format described below.
...
> [!WARNING]
> **Keep internal versions to a minimum**: To keep maintenance light it is **highly** recommended to reduce the number of versions you have for internal endpoints. In your code it is possible to centrally define and share internal versions through code that is `common` to your browser- and server-side plugin code.
