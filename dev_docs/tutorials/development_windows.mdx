...
<DocCallOut title="Prerequisites">
  ...
  the browser to run and/or debug functional tests. It's highly advisable to make sure you install the prerequisites for a default usage listed below. 
  
  ### Default Experience
  For a default experience and a completion of the entire guide which will make you able to run GUI apps as suggested bellow you must be running **Windows 11 (Build 22000 and higher)**.
...
4) Install support for Google Chrome by running the following inside WSL shell (**Default Experience Only - Windows 11 Build 22000+ and vGPU driver**)
