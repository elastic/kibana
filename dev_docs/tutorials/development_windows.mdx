...
<DocCallOut title="Prerequisites">
  ...
  This guide allows you to achieve two different kind of experiences (Basic and Default each with their specifics).
  However a common prerequisite for both is enabling the Hardware Virtualization on Windows. 
  
  That will highly depend on your hardware, as it should be enabled first on your UEFI (or BIOS). Please check the official Microsoft documentation on 
