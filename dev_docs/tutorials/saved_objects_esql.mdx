...
- **With `_source` (via `metadata: ['_id', '_source']`):** The full document in `_source` contains all attributes needed for AAD (Additional Authenticated Data) reconstruction. Encrypted attributes are **decrypted** in `_source` using the same path as `find` and `search`. If decryption fails (e.g., key rotation), encrypted attributes are stripped from `_source` instead.
