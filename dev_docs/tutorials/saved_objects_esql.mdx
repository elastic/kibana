...
// esql.col() produces a correctly-quoted ES|QL identifier.
// Field names that contain dots (e.g. "my_type.title") must be backtick-quoted
// in ES|QL — esql.col() handles that automatically.
...
The pipeline executes as the `kibana_system` Elasticsearch user, which has elevated privileges. Always construct the pipeline server-side; never let user input determine the pipeline structure. For example, a user who controls the pipeline could use `ENRICH` to join against policies whose source data they are not authorised to see directly.

Using `ENRICH` in your own pipeline is fine — the risk is only when the user can influence which policy is used or the pipeline structure.
