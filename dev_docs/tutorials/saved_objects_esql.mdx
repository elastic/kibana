...
When interpolating user input into ES|QL queries, **never** use string concatenation. Instead, use the `esql` tagged template from `@kbn/esql-language` with the **named param syntax**.
...
Use `.print()` to extract the query string and `.getParams()` to extract the named params:
...
The query sent to Elasticsearch will be `FROM .kibana | WHERE type == "my-type" | WHERE my-type.title LIKE ?searchTerm | LIMIT 100` — with `searchTerm` as a separate parameter, never interpolated into the query string.
...
### Index is NOT controlled server-side

Unlike the `search` method (which passes an explicit `index` parameter), ES|QL does not support an explicit index parameter. The data source is specified in the `FROM` clause of your query string. Security is enforced via the `filter` parameter, which injects namespace and type restrictions — not by controlling which index is accessed.
