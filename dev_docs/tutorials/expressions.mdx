...
Same expression could also be constructed in the code:
...
On top of that in the browser there are two additional ways to run expressions and render the results:
...
Adding new functions to expressions is easy, just call `registerFunction` method on expressions service setup contract with your function definition:
