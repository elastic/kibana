...
<DocCallOut title="Design code-first APIs for OAS first" color="warning">
  Code-first API schemas must be designed carefully to produce clear OpenAPI 3.0 output. Prefer simple `@kbn/config-schema` types and keep request/response shapes narrow and explicit.
...
#### `@kbn/config-schema` types and patterns that do not map cleanly to OAS 3.0
