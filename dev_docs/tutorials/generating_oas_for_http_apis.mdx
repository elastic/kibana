...
2. `@kbn/config-schema` request and response schemas
...
Defining and organizing schemas is up to each team. In this example we show how you can organize HTTP API schemas
to prepare them for being versioned and shared both on the server and client side of your plugin:

```typescript
// In server/schemas/v1.ts
import { schema, TypeOf } from '@kbn/config-schema';
export const fooResource = schema.object({ name: schema.string() });
...
Launch Kibana and issue the following request:

```bash
curl -s -uelastic:changeme http://localhost:5601/api/oas\?pathStartsWith\=/api/foo
```

The value returned should contain the OpenAPI specification for your route.
...
<DocCallOut title="Under construction" color="warning">
  At the time of writing we only the OAS for a subset of Kibana's HTTP APIs to give teams the chance to improve the quality of OAS.
...
It's possible to generate OpenAPI specification for `access: 'internal'` routes but it is not required. The benefit will largely be for your team's internal reference and for other teams to discover your APIs. If you follow the practices outlined below it should be simple to generate OAS for internal routes as well.