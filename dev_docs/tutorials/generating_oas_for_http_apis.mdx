...
#### IDs avoid repeated inline schemas

If the same request or response schema appears in multiple routes, versions, or status codes, give it an **ID**. In this context, the ID is the OpenAPI component name for that schema. Once a schema has one, the generator can emit a `$ref` to `#/components/schemas/<id>` instead of inlining the full schema every time it is used.

Use `meta.id` with `@kbn/config-schema`:
...
export const addressSchema = schema.object(
  ...
  },
  {
    meta: {
      id: 'Address',
      description: 'Mailing address for a foo resource.',
    },
  }
);
```

Use `.meta({ id })` with `@kbn/zod/v4`:
...
With either library, an ID lets repeated uses of `addressSchema` point to the shared component:
...
For Zod v4, attach the ID at definition time. `.meta()` is immutable and returns a new schema, so calling it later does not update the schema that your route already references.

If your API uses tagged unions, prefer `schema.discriminatedUnion('type', [...])` or `z.discriminatedUnion('type', [...])` over a plain union. For Zod v4, when each variant also has its own `.meta({ id })`, Kibana can generate an OpenAPI discriminator mapping automatically.
