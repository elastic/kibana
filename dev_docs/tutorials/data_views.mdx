...
#### Get list of saved data view titles and ids

```
const idsAndTitles = await data.indexPatterns.getSavedIdsWithTitle();
idsAndTitles.forEach(({id, title}) => console.log(`Data view id: ${id} title: ${title}`));
```

#### Get list of cached data view titles and ids

```
const idsAndTitles = await data.indexPatterns.getCachedIdsWithTitle();
idsAndTitles.forEach(({id, title}) => console.log(`Data view id: ${id} title: ${title}`));
```
