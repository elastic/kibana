...
In a great majority of the use cases where we need to build a Kibana distributable, we just need the archives containng the 
...
To specify just a single installation package or docker iamges to build instead of all of them you can add rpm, deb or docker-images as an argument:
