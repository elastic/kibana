...
- Install the current version by running `pnpm add -W puppeteer@23.3.1` if we assume for the sake of this guide that the latest version is `23.3.1`. Run `pnpm kbn bootstrap` after installing.
...
- It is likely that you will not have permissions within the VM to upload the build artifacts into the destination bucket. In this case, you will see a permission error at the end of the script, like this example from the older `gsutil`-based build script:
  ```
...
- Upload the build artifacts and the sha256 files to the GCP storage buckets:
  ```
  gcloud storage cp ./chromium-fe621c5-locales-linux_* gs://headless_shell_staging
  gcloud storage cp ./chromium-fe621c5-locales-linux_* gs://headless_shell
  ```
