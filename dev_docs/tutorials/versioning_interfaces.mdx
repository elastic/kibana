...
The following is a practical example of this strategy. We will introduce this strategy to a pre-existing domain that is not versioned.

### An example: Weather insights unversioned
Consider a fictional Kibana plugin called "Weather insights". It allows users to easily create and curate a dashboards that house visualisations and metrics for weather data. Inside of this plugin are the following concepts:
...
All that is left to do now is to update our `latest.ts` and `index.ts` files:
