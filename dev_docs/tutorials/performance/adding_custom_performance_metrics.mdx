...
We use event-based telemetry (EBT) to report metrics as events. It is strongly suggested to use EBT only for client-side metrics.
If you need to add one for server code, please discuss it with Core Team.
...
Once executed during journeys run, the metric will be delivered to the `telemetry-v2-staging` cluster, alongside with the event's context.
