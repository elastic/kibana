...
Run `pnpm es snapshot` from a bootstrapped Kibana source checkout. Use either a checkout on `main` or the worktree for the revision being tested. If the Elasticsearch snapshot configuration matters for the scenario, use the matching baseline or candidate worktree for that run.
...
pnpm es snapshot -E path.data=/tmp/kibana-memory-profile/es-data/baseline-round-1
```
