...
- Types: `['index-pattern', 'dashboard', 'customIndex', 'multiNamespaceTypeCustomIndex']`
- Namespaces: `['foo-namespace', 'bar-namespace']`

But they are only authorized to:
- `index-pattern` in `foo-namespace`
- `dashboard` globally (all requested namespaces)
- `customIndex` in `bar-namespace`
- `multiNamespaceTypeCustomIndex` with no find permissions

A query like this is generated:
        ...
        "bool": {
          ...
          "should": [
            ...
            },
            {
              "bool": {
                "minimum_should_match": 1,
                "must": [{ "term": { "type": "dashboard" } }],
                "must_not": [{ "exists": { "field": "namespaces" } }],
                "should": [{ "terms": { "namespace": ["foo-namespace", "bar-namespace"] } }]
              }
            },
            {
              "bool": {
                "minimum_should_match": 1,
                "must": [{ "term": { "type": "customIndex" } }],
                "must_not": [{ "exists": { "field": "namespaces" } }],
                "should": [{ "terms": { "namespace": ["bar-namespace"] } }]
              }
            },
            {
              "bool": {
                "must": [
                  { "term": { "type": "multiNamespaceTypeCustomIndex" } },
...
The `search` method returns raw Elasticsearch search results.
...
const result = await savedObjectsClient.search({
  ...
  query: { match_all: {} },
  runtime_mappings: {
    // ❌ This accesses encrypted secrets before the search method can redact them
    exposed_api_key: {
      type: 'keyword',
      script: {
        source: `
  ...
  // ❌ The encrypted value is returned in the response, bypassing redaction
