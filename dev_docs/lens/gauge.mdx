...
const gaugeConfig: LensConfig = {
  ...
  dataset: {
    esql: 'from myindex | stats avgCpuUtilization = avg(cpu_utilization)',
  },
...
This example demonstrates how to create a gauge visualization using the `LensGaugeConfig`. It sets up a gauge to display the average CPU utilization, with specified minimum and maximum values to define the scale, and a goal value to highlight the target CPU utilization level. The `shape` property is set to `'arc'`, giving the gauge a semi-circular appearance.