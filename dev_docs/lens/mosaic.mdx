...
const mosaicConfig: LensConfig = {
  chartType: 'mosaic',
  title: 'Product Sales Distribution by Region and Category',
  dataset: {
    esql: 'from sales_data | group by region, category | stats sum(sales) as totalSales',
  },
  breakdown: {
    type: 'topValues',
    field: 'region',
    size: 5, // Top 5 regions
  },
  xAxis: {
    type: 'topValues',
    field: 'category',
