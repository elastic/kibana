...
const xyConfig: LensConfig = {
  ...
  dataset: {
    esql: 'from sales_data | group by month | stats sum(sales) as totalSales',
  },
  layers: [
    {
      type: 'series',
...
This example demonstrates how to configure an XY chart to display the trend of monthly sales. It uses a line chart series layer to represent the total sales over time, with sales data aggregated by month. The `xAxis` configuration uses a `dateHistogram` to ensure that sales data is plotted over time accurately. The `legend` and `axisTitleVisibility` options are used to enhance the chart's readability, making it easy to interpret the sales trend. This configuration exemplifies how to leverage the XY chart type for time series data analysis, showcasing trends and patterns in sales over the specified period.