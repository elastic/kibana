...
No matter which index your saved objects are stored in, we recommend no more than 10,000 saved
objects of any given type exist at a given time. This is because the `SavedObjectsClient.find` function does not at the moment
support paging past 10k saved-objects.
