...
Below is the call to reigster the type.
...
### Registering the type as an Encrytped Saved Object
...
The `rule` attribute contains values related to the encrtypted attribute `apiKeyToUse`, or that will never change:
...
of AAD hierarchical inclusion when structuring attributes for your saved objects. You can also utilize more granular keys when speficying which attributes to include in AAD,
...
Additionally, the owning team implemeted a type to help manage partial updates. This is a great addition to ensure changes to the ESOs do not render them un-decryptable.
