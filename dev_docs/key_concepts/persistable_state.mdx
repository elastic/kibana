...
"Persistable State" is developer defined part of state or configuration that can be(is ment to be) persisted by other
 plugin that the one providing it. Persistable State needs to be serializable and owner can/should provide utilities
 to migrate it, extract and inject references from/to it as well as collect telemetry.
 
## exposing state that can be persisted

Any plugin that exposes state or configuration that another plugin might persist should implement `PersistableStateService`
interface on their `setup` contract. This will allow plugins persisting the state to easily access migrations and other utilities.

example: Data plugin allows you to generate filters. Those filters can be persisted by applications in their saved 
objects or in the URL. In order to allow apps migrating the filters in case the structure changes in the future Data 
plugin implements `PersistableStateService` on `data.query.filterManager`.
...
For migrations saved object definition `migration` key should be a function, which returns a list of migrations. `getAllMirations`
function of `PersistableStateService` should be called in there to get an object with migrations which should be merged with
any migrations provided by the plugin.
...
When storing persistable stae as part of URL you must make sure your URL is versioned. When loading the state `migrateToLatest` method
...
To support persisting your state in saved objects owned by other plugin `extract` and `inject` method of Persistable State interface should be implemented.
