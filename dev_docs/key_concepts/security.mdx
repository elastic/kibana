...
1. **Default Configuration (applies to versions without specific authorization)**:
   - Requires `<privilege_1>`.

2. **Version 1**:
   - Requires **both** `<privilege_1>` and `<privilege_2>` privileges.

3. **Version 2**:
   - Inherits the default authorization configuration, requiring `<privilege_1>`.
