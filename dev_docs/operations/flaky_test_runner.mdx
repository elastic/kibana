...
The fan-out happens after the discovery step (`Discover Scout Playwright configs`) produces the manifest of modes per config. The follow-up `Plan Scout flaky steps` step reads that manifest and uploads the per-mode steps dynamically — you'll see them appear in the build once that step runs.
...
If you need more repetitions for a specific failure mode, run the Flaky Test Runner multiple times — the BK platform also caps total jobs per build at 500, and the planner refuses to upload more than that across all selected configs and modes.
