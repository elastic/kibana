...
We also have to remember that we can't assume a test passing locally will mean it will pass in CI. The two environments are different. There is a lot we could mention, but at a high level, most functional tests are run on 4 core 16gb machines, and these machines are virtualized, which means neighors can cause modest, but variable levels of performance. Additionally, end-to-end tests in CI are run against {kib} distributions, while we run the tests under the `--dev` flag locally.
