...
```
GET    /api/resources/{id}         # Read - retrieve an existing resource
POST   /api/resources              # Create - create a new resource
PUT    /api/resources/{id}         # Update - update an existing resource
DELETE /api/resources/{id}         # Delete - remove an existing resource
GET    /api/resources              # List - retrieve all resources (with pagination)
```
...
When a field that was once a string suddenly becomes an array or object, it forces breaking changes that disrupt existing user configurations.
...
If poorly defined structure or types are unavoidable, JSON strings provide a workaround—but use them sparingly:
