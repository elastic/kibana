...
1. **Is there already another dependency that offers similar functionality?** If so, adding another dependency may not be necessary.
...
3. **Does this dependency have a license that's compatible with Kibana's?** Most common open source licenses such as BSD, MIT,
and Apache 2.0/1.1 are okay to use with Kibana. Others may not be, or may require special attribution.
4. **Will this dependency need to be prebuilt?** Due to our build process, native module dependencies should include at least a prebuild
step so at install time it simply downloads instead of building from source. This allows us to optimize bootstrap times.
5. **Am I committed to maintaining this dependency?** Once you add a dependency to the `package.json`, someone else isn't going to
