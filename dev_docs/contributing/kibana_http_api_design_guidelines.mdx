...
**For public APIs, prefix with `/api/<domain>`**
...
* `PATCH`, `PUT` and `DELETE` should have side-effects (alter some resource)
* `POST` should have side-effects (like creating some resource, read on!)

**Same method, same result?**

This is sometimes referred to as _idempotency_. It is related to side-effects, but not quite the same thing.
...
If you expect you need to support a lot of input for searching or listing resources it might be good to reserve `POST` for this purposes.
...
Accepting very long strings (in excess of 1000 characters depending on other parameters) in path or query parameters can cause issues for HTTP servers that limit the byte length of certain parts of requests. HTTP servers may limit request header sections to 4096 bytes.
