...
`PUT` is a bit special in that it should _create_ the resource if it does not exist. It should expect the **full** resource in the body of the call.

`PATCH` is used for partial updates and should never create a resource that does not already exist. It should expect a **partial** resource in the body of the call.
