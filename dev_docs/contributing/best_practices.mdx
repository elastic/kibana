...
We are currently working to fully leverage change-based tasks across the repository and those function best when the dependency tree is full of focused packages around a single responsibility. 
Having that said we do not think that doing that from a theoretical perspective and over engineering from the beginning of your packages is the way to do it. Instead, breaking those up is expected 
to be seen as an available tool if the build performance is being compromised or the dependency chain touched by a chain on a given package is too heavy.
