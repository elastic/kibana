...
<DocCallOut title="Internal only">
  ...
  Kibana maintains accessibility quality using a layered, automated approach:
  ...
  - **CI tooling:** running `axe-core` check in `FTR` and `Cypress` tests (`Scout` integration pending...)
