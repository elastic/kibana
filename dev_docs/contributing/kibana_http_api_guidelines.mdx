...
Imagine we were to run requests the following methods 1000x over:

* `GET` always results in the same _server state_
* `PUT` always results in the same _server state_
* `DELETE` always results in the same _server state_ (maybe returns 404 after the first request)
