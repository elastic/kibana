...
Kibana's public HTTP APIs are the foundation for developers, partners, and customers to build powerful extensions, automations, and integrations with Kibana.
...
<DocCallOut title="Some APIs need to be Terraform-friendly">
  As an extension to the guidance in this document refer to the [Guidelines for Terraform friendly HTTP APIs](/kibana-dev-docs/contributing/http-api-tf-guidelines).
</DocCallOut>
...
The important distinction here is to create a great _API_ user experience, as opposed to optimizing for _UI_ use cases as well. Front-ends have very different needs and constraints from public HTTP API clients. Our public HTTP APIs should be primitives that are simple to use and compose.
...
<DocCallOut title="Internal vs Public APIs">
  We categorise Kibana APIs as either **internal** or **public**. See the [Internal vs Public APIs](#internal-vs-public-apis) section for more details.
</DocCallOut>
