...
```
GET /api/resources?search_after="pitId.valueA.valueB",size=10
```
...
POST /api/resources
{
  search_after: "pitId.valueA.valueB"
  size: 10,
}
```

Where `pitId` is the PIT ID  and `valueA` and `valueB` are the sort values that identify the last hit from ES ([docs](https://www.elastic.co/docs/reference/elasticsearch/rest-apis/paginate-search-results#search-after)).
...
```json
{
  "items": [...],
  "total": 100,
  "search_after": "pitId.valueA1.valueB1", // The next search_after value
  "size": 10
}
```
