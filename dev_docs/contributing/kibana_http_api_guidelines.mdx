...
The List operation is critical for Terraform's data source and import functionality.

#### Implementation considerations

1. **Consistent response structures**: Ensure GET and POST/PUT responses return the same structure with identical fields.

2. **Idempotent operations**: POST for creation and PUT for updates should be idempotent - running the same request multiple times should result in the same state.
