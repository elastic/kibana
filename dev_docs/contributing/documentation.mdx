...
In order for it to be rendered inside the Developer Guide, it must be an `mdx` file with the appropriate <DocLink id="docsSyntax" section="frontmatter" text="frontmatter"/>. Read about the syntax <DocLink id="docsSyntax" text="here"/>. There is a separate file that describes how the navigation menu should look. In order for your link to appear, the file [kibana/nav-kibana-dev.docnav.json](https://github.com/elastic/kibana/blob/main/nav-kibana-dev.docnav.json) must be edited. Read <DocLink id="kibDevDocsBestPractices" text="these instructions" /> for more details on how to add new content and test locally.
...
Where information is specific to a particular plugin, developers may also choose to keep that information along side the code.
...
[api_doc_suite.test.ts](https://github.com/elastic/kibana/blob/master/packages/kbn-docs-utils/src/api_docs/tests/api_doc_suite.test.ts) is a pretty comprehensive test suite that builds the test docs inside the [**fixtures** folder](https://github.com/elastic/kibana/tree/master/packages/kbn-docs-utils/src/api_docs/tests/__fixtures__/src).
