...
When building a plugin in Kibana, there are a handful of architetural "building blocks" you can use. Some of these building blocks are "higher-level", 
...
with many built-in capabilities, require less maintenance, and evolve new feature sets over time with little to no
 manual changes required. When developers use high-level building blocks, new features are exposed consistently, across all of Kibana, at the same time.
  On the downside, they are not as flexible as our low-level building blocks. 
 
 Low-level building blocks
 provide greater flexibility, but require more code to stitch together into a meaningful UX. This results in higher maintenance costs and greater UI/UX variability
 across Kibana.
...
Check out the Lens Embeddable if you wish to show users visualizations with aggregated data. It's built on top of the
 ...
 and <DocLink id="kibBuildingBlocks" section="ui-actions--triggers" text="UI Actions"/>.
