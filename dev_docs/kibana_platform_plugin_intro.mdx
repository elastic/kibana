...
<DocCallOut title="Internal Developer FAQ: Should I put my code in a plugin or a package?" color="warning">
If it's stateful, it has to go in a plugin, but with stateless utilities, It Depends. Stateless code exported publicly from a plugin will increase the bundle size, however, the packages folder is far away from the plugins folder so having a part of your code in a plugin and the rest in a package may make it hard to find, leading to duplication.
...
Until then, consider whether it makes sense to logically separate the code, and consider the size of the exports, when determining whether you should put it in a package or a plugin.

</DocCallOut>

<DocCallOut title="What is the difference between services provided by plugins and those by Core?">
