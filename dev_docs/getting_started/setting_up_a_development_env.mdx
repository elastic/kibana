...
**Running Elasticsearch from source** — like `yarn es snapshot`, `yarn es source` defaults to a `trial` license and auto-configures the SAML Mock IdP realm, so passwordless SAML login works out-of-the-box:
...
To opt out of the SAML Mock IdP auto-configuration (for example when using a `basic` license, which does not support SAML), start Kibana with `--mockIdpPlugin.enabled=false`:

```sh
yarn es source --license basic
yarn start --mockIdpPlugin.enabled=false
```
