...
## Using the Kibana devcontainer (optional)

Kibana also supports using a [devcontainer](https://containers.dev/) which can integrate with various editors and tools [(supported tools)](https://containers.dev/supporting). The devcontainer provides a consistent development environment across different machines and setups. The only prerequisite is having [Docker](https://www.docker.com/) installed locally.