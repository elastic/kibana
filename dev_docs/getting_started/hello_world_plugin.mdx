...
In one terminal window, run `pnpm es snapshot --license trial` to boot up Elasticsearch.

In another terminal window, run `pnpm start --run-examples` to boot up Kibana and include the example plugins. Your example plugin should show up in the navigation at the very bottom.
