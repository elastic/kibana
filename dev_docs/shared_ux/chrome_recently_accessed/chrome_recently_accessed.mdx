...
- Update the link when the state of the resource changes. For example, if a user changes the time range of a dashboard, update the link to reflect the latest state where possible.
