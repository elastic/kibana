...
const DEMO_RULES = [
  {
    ...
    description: 'Detects suspicious PowerShell process execution using -EncodedCommand flag on Windows endpoints',
    ...
    threat: [{
      ...
      technique: [{ id: 'T1059', name: 'Command and Scripting Interpreter', reference: 'https://attack.mitre.org/techniques/T1059/' }],
    }],
  },
  {
    ...
    threat: [{
      framework: 'MITRE ATT&CK',
      tactic: { id: 'TA0008', name: 'Lateral Movement', reference: 'https://attack.mitre.org/tactics/TA0008/' },
      technique: [{ id: 'T1021', name: 'Remote Services', reference: 'https://attack.mitre.org/techniques/T1021/' }],
    }],
  },
];
...
async function kibanaRequest(opts, method, path, body) {
  const base = opts.space === 'default'
    ? opts.url
    : `${opts.url}/s/${opts.space}`;
...
async function findDemoRules(opts) {
  const res = await kibanaRequest(opts, 'GET', `${RULES_URL}/_find?per_page=100&filter=alert.attributes.tags:DEMO`);
...
async function seed(opts) {
  ...
  console.log('  gap_description: "Suspicious PowerShell with -EncodedCommand flag on Windows endpoints"');
  ...
  console.log('  gap_description: "SSH brute-force login attempts from external IPs against Linux servers"');
  console.log('  technique: T1110\n');
  console.log('Scenario D — prebuilt_available (pauses at "Install rule" approval):');
  console.log('  Requires: security_detection_engine package installed with at least one rule not yet activated.');
