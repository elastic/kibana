...
There are two distinctive ways of adding items to the navigation, to add navigation items that would be visible across all consumers of this package
...
In the example above assuming the `visualize` is a valid registered management app ID, this definition would show up as a navigation card under the predefined category `content`. It's also worth pointing out that there's also 
