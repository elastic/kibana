...
export type LensConfig = (LensMetricConfig | LensGaugeConfig | LensPieConfig | LensHeatmapConfig | LensMosaicConfig | LensRegionMapConfig | LensTableConfig | LensTagCloudConfig | LensTreeMapConfig | LensXYConfig) & LensBaseConfig;
...
export type LensBreakdownConfig = string | Identity<(LensBreakdownTopValuesConfig | LensBreakdownIntervalsConfig | LensBreakdownFiltersConfig | LensBreakdownDateHistogramConfig) & { colorPalette?: string }>;

export interface LensMetricConfig {
    chartType: 'metric';
    layers: [Identity<LensBaseLayer & { 	// must be a single layer!
...
export interface LensPieConfig {
    ...
    layers: [Identity<LensBaseLayer & { 	// must be a single layer!
        /** field name to apply breakdown based on field type or full breakdown configuration */
        breakdown: Array<LensBreakdownConfig>;
    }>];
...
export interface LensXYConfig {
    chartType: 'xy';
    layers: Array<(Identity<LensBaseLayer & {
        type: 'series';
        breakdown?: LensBreakdownConfig;
        xAxis: LensBreakdownConfig;
        seriesType: 'line' | 'bar' | 'area';
    }>) | LensAnnotationLayer | LensReferenceLineLayer >;
    legend?: Identity<LensLegendConfig>;
}
