...
export const calculateUserEvent = (
  e: Event,
  {
    runtimeSettings$,
    interactionEvent$,
  ...
  }: GridLayoutStateManager,
  {
    ...
    pointerClientPosition: React.MutableRefObject<{ x: number; y: number }>;
    lastRequestedPanelPosition: React.MutableRefObject<GridPanelData | undefined>;
  }
) => {
  ...
  const previewRect = {
    left: isResize ? panelRect.left : pointerClientPixel.x - interactionEvent.pointerOffsets.left,
  ...
  if (!isTouchEvent(e)) {
    ...
    if (startScrollingUp || startScrollingDown) {
      if (scrollInterval && !scrollInterval.current) {
...
export const useGridLayoutEvents = ({
  ...
  useEffect(() => {
    ...
    document.addEventListener('scroll', onPointerMove, { passive: true });
...
export function getPointerClientPosition(e: Event) {
  if (isTouchEvent(e)) {
