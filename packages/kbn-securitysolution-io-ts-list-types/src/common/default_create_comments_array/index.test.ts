...
describe('default_comments_array', () => {
  ...
  test('it should fail validation when supplied an array of strings', () => {
    ...
    expect(getPaths(left(message.errors))).toEqual([
      'Invalid value "some string" supplied to "Array<({| comment: NonEmptyString, created_at: string, created_by: string, id: NonEmptyString |} & Partial<{| updated_at: string, updated_by: string, meta: (object | undefined) |}>)>"',
    ]);
    expect(message.schema).toEqual({});
  });
