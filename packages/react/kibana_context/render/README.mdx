...
description: This context provider is used to render a new component tree _without_ the initialization of EUI or Emotion.  This provider is typically used with `root.render()` calls.
...
const root = createRoot(document.getElementById('some_node'));
root.render(<App />);
```