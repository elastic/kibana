...
export const diGlobalsRule = PackageRule.create('diGlobals', {
  async check({ pkg }) {
    ...
    const changed =
      !compareArrays(nextValues.services.provides, manifestValues.services.provides) ||
      !compareArrays(nextValues.services.consumes, manifestValues.services.consumes) ||
      !compareArrays(nextValues.extensionPoints.hosts, manifestValues.extensionPoints.hosts) ||
      !compareArrays(
        nextValues.extensionPoints.contributes,
    ...
    if (!compareArrays(nextValues.services.provides, manifestValues.services.provides)) {
      missing.push('plugin.globals.services');
    }
    if (!compareArrays(nextValues.services.consumes, manifestValues.services.consumes)) {
      missing.push('plugin.globals.services');
    }
    if (!compareArrays(nextValues.extensionPoints.hosts, manifestValues.extensionPoints.hosts)) {
      missing.push('plugin.globals.extensionPoints');
    }
    if (
      !compareArrays(
        nextValues.extensionPoints.contributes,
    ...
    this.err(`plugin.globals is out of sync (${[...new Set(missing)].join(', ')})`, {
      'kibana.jsonc': (source) => setGlobals(source, nextValues),
    });
  },
});
