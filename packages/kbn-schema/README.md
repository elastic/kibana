# `@kbn/schema` — The Kibana JSON schema parsing library

## Table of Contents

- [`@kbn/schema` — The Kibana JSON schema parsing library](#kbnschema--the-kibana-json-schema-parsing-library)
  - [Table of Contents](#table-of-contents)
  - [Why `@kbn/schema`?](#why-kbnschema)
...
* **Limited API surface** - having a feature rich library is awesome, but it's a really hard task to audit such library and make sure everything is sane and secure enough. As everyone knows complexity is the enemy of security and hence we'd like to have a full control over what exactly we expose and commit to maintain.
...
Every schema instance has a `validate` method that is used to perform a validation of the data according to the schema. This method accepts two arguments:
...
const valueSchema = schema.object({
  ...
  env: schema.oneOf([schema.literal('dev'), schema.literal('staging'), schema.literal('prod')]),
});
...
expect(valueSchema.validate({ isEnabled: true })).toEqual({
  isEnabled: true,
  env: 'staging',
});
...
expect(() =>
  valueSchema.validate({ isEnabled: 'non-bool' })
).toThrowError(
...
expect(() =>
  valueSchema.validate({ isEnabled: 'non-bool' }, 'configuration')
).toThrowError(
...
__Options:__
  * `defaultValue: string | (() => string)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: number | (() => number)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: boolean | (() => boolean)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TLiteral | (() => TLiteral)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TBuffer | (() => TBuffer)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TStream | (() => TStream)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TValue[] | (() => TValue[])` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TObject | (() => TObject)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: Record<TKey, TValue> | (() => Record<TKey, TValue>)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: Map<TKey, TValue> | (() => Map<TKey, TValue>)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: TUnion | (() => TUnion)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: any | (() => any)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: ByteSizeValue | string | number | (() => ByteSizeValue | string | number)` - defines a default value, see [Default values](#default-values) section for more details.
...
__Options:__
  * `defaultValue: moment.Duration | string | number | (() => moment.Duration | string | number)` - defines a default value, see [Default values](#default-values) section for more details.
...
For these cases `@kbn/schema` provides a way to specify a custom validation function for almost any schema building block through the `validate` option.

For example `@kbn/schema` doesn't have a dedicated primitive for the `RegExp` based validation currently, but you can easily do that with a custom `validate` function:
...
deal with "defined or undefined"-like checks all over the place in your code. You have two options to provide a default value for almost any schema primitive:
