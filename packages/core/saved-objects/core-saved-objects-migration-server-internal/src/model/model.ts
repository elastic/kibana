...
export const model = (currentState: State, resW: ResponseType<AllActionStates>): State => {
  ...
  if (stateP.controlState === 'INIT') {
    ...
    if (Either.isRight(res)) {
      ...
      // Thus, another instance's migrator is ahead of us. We simply abort and retry
      return {
        ...stateP,
        controlState: 'FATAL',
        reason: `The ${stateP.currentAlias} migrator is attempting to lock the [${res.left.index}] index. Another migrator is likely ahead of us in the process, aborting.`,
      };
