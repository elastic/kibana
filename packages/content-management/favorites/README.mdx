...
Internally the favorites list is backed by a saved object. A saved object of type "favorite" is created for each user (user profile id) and space (space id) combination when a user for the first time favorites an object. The saved object contains a list of favorite object ids grouped by favorite type.

```
{
  "_index": ".kibana_8.16.0_001",
  "_id": "spaceid:favorite:u_profile_id",
  "_source": {
    "favorite": {
      "userId": "u_profile_id",
      "favorites": {
        "dashboard": [{id: "favorite-dashboard-id-1"}, {id: "favorite-dashboard-id-2"}],
        "visualization": [{id: "favorite-visualization-id-1"}],
      }
    },
    "type": "favorite",
    "references": [],
