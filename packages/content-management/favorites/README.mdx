...
Internally the favorites list is backed by a saved object. A saved object of type "favorite" is created for each user (user profile id) and space (space id) combination when a user for the first time favorites an object. The saved object contains a list of favorite objects.
