...
const __1 = require("..");
const { duration, object, contextRef, siblingRef } = __1.schema;
...
describe('#defaultValue', () => {
    test('can be a moment.Duration', () => {
        expect(duration({
            defaultValue: moment_1.duration(1, 'hour'),
    ...
    test('can be a function that returns compatible type', () => {
        expect(duration({
            defaultValue: () => 600,
        }).validate(undefined)).toMatchInlineSnapshot(`"PT0.6S"`);
        expect(duration({
            defaultValue: () => '1h',
        }).validate(undefined)).toMatchInlineSnapshot(`"PT1H"`);
        expect(duration({
            defaultValue: () => moment_1.duration(1, 'hour'),
        }).validate(undefined)).toMatchInlineSnapshot(`"PT1H"`);
    });
    test('can be a reference to a moment.Duration', () => {
        ...
        }).validate(undefined, { val: moment_1.duration(700, 'ms') })).toMatchInlineSnapshot(`
Object {
  "fromContext": "PT0.7S",
  "source": "PT0.6S",
  "target": "PT0.6S",
...
`);
        expect(object({
            ...
            target: duration({ defaultValue: siblingRef('source') }),
            fromContext: duration({ defaultValue: contextRef('val') }),
        }).validate(undefined, { val: moment_1.duration(2, 'hour') })).toMatchInlineSnapshot(`
Object {
  "fromContext": "PT2H",
  "source": "PT1H",
  "target": "PT1H",
...
`);
        expect(object({
            source: duration({ defaultValue: moment_1.duration(1, 'hour') }),
            target: duration({ defaultValue: siblingRef('source') }),
            fromContext: duration({ defaultValue: contextRef('val') }),
        }).validate(undefined, { val: moment_1.duration(2, 'hour') })).toMatchInlineSnapshot(`
Object {
