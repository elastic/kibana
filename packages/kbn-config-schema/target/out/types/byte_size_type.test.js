...
describe('#defaultValue', () => {
    test('can be a ByteSizeValue', () => {
        expect(byteSize({
            defaultValue: byte_size_value_1.ByteSizeValue.parse('1kb'),
        }).validate(undefined)).toMatchSnapshot();
    });
...
describe('#min', () => {
    test('returns value when larger', () => {
        expect(byteSize({
            min: '1b',
        }).validate('1kb')).toMatchSnapshot();
    });
    test('returns error when smaller', () => {
        expect(() => byteSize({
            min: '1kb',
        }).validate('1b')).toThrowErrorMatchingSnapshot();
    });
});
describe('#max', () => {
