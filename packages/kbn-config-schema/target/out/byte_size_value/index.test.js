...
describe('parsing units', () => {
    ...
    test('throws an error when no unit specified', () => {
        expect(() => _1.ByteSizeValue.parse('123')).toThrowError('could not parse byte size value');
    });
    test('throws an error when unsupported unit specified', () => {
        expect(() => _1.ByteSizeValue.parse('1tb')).toThrowError('could not parse byte size value');
    });
});
describe('#constructor', () => {
    test('throws if number of bytes is negative', () => {
        expect(() => new _1.ByteSizeValue(-1024)).toThrowErrorMatchingSnapshot();
    });
    test('throws if number of bytes is not safe', () => {
        expect(() => new _1.ByteSizeValue(NaN)).toThrowErrorMatchingSnapshot();
        expect(() => new _1.ByteSizeValue(Infinity)).toThrowErrorMatchingSnapshot();
        expect(() => new _1.ByteSizeValue(Math.pow(2, 53))).toThrowErrorMatchingSnapshot();
    });
    test('accepts 0', () => {
