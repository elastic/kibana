...
import { PainlessCompletionResult, PainlessCompletionItem } from '../types';
...
export const getPainlessClassesToAutocomplete = (): PainlessCompletionResult => {
  const painlessClasses: PainlessCompletionItem[] = context.classes.map(({ name }) => {
...
export const getPainlessConstructorsToAutocomplete = (): PainlessCompletionResult => {
  ...
  const constructors: PainlessCompletionItem[] = painlessConstructors
    ...
    .filter((constructor, index, constructorArray) => {
      return (
        constructorArray.findIndex(({ declaring }) => declaring === constructor.declaring) === index
      );
    })
    .map(({ declaring }) => {
...
export const getPainlessClassToAutocomplete = (className: string): PainlessCompletionResult => {
  ...
  const staticFieldsAutocomplete: PainlessCompletionItem[] = staticFields.map(({ name, type }) => {
    return {
      label: name,
      kind: 'property',
