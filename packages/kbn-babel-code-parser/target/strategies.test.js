...
describe('Code Parser Strategies', () => {
  it('should calculate the top level dependencies correctly', () => {
    ...
    expect((0, _strategies._calculateTopLevelDependency)(plainDep)).toEqual('dep1');
    expect((0, _strategies._calculateTopLevelDependency)(foldedDep)).toEqual('@kbn/es');
    expect((0, _strategies._calculateTopLevelDependency)(otherFoldedDep)).toEqual('@kbn/es');
  });
  it('should exclude native modules', async () => {
    _fs.readFile.mockImplementationOnce((path, options, cb) => {
      cb(null, `require('fs')`);
    });

    const results = [];
    await (0, _strategies.dependenciesParseStrategy)(mockCwd, _code_parser.parseSingleFile, 'dep1/file.js', {}, results);
  ...
  it('should return a dep from_modules', async () => {
    ...
    _can_require.canRequire.mockImplementation((mockCwd, entry) => {
    ...
    const results = await (0, _strategies.dependenciesParseStrategy)(mockCwd, _code_parser.parseSingleFile, 'dep1/file.js', {}, {});
