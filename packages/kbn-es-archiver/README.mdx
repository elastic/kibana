...
This parameter is optional, with defaults:
 ...
 - HighWaterMark (stream batch size): 5000
...
```typescript
await esArchiver.load('SOME-ESARCHIVER-ARCHIVE', {
  performance: {
    highWaterMark: SOME-POSITIVE-INTEGER,
    concurrency: SOME-POSITIVE-INTEGER,
  },
});
```
...
```
According to our benchmarks, in a servless environment, this archive went from
loading in around 2 and half minutes, to under 18 seconds.
This was with a batch size of 5000 and concurrency of 4, as shown above.