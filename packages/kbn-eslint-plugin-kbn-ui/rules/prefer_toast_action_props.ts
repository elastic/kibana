...
// i18n components whose children are inline translated text.
// E.g. any EuiLink inside them is an inline hyperlink within a sentence,
// not a standalone action, and must not be flagged.
const I18N_INLINE_ELEMENTS = new Set(['FormattedMessage', 'FormattedHTMLMessage', 'Trans']);
  ...
  const traverse = (node: TSESTree.Node): void => {
    switch (node.type) {
      case 'JSXElement': {
        ...
        // Stop traversal. Content is either I18N_INLINE_ELEMENTS, unknown custom components or and unresolvable member expressions
