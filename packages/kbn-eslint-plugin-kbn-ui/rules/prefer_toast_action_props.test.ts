...
tester.run('prefer_toast_action_props', PreferToastActionProps, {
  valid: [
    ...
    ...MOUNT_WRAPPERS.flatMap(({ fn, wrap }) => [
      ...
      {
        name: `EuiLink as render-function value in FormattedMessage inside ${fn} is not flagged`,
        code: `toasts.addSuccess({ title: 'Done', text: ${wrap(
          `<FormattedMessage
            id="x"
      ...
      {
        name: `EuiLink as direct JSX value in FormattedMessage inside ${fn} is not flagged`,
        code: `toasts.addSuccess({ title: 'Done', text: ${wrap(
          `<FormattedMessage
            id="x"
      ...
      {
        name: `EuiLink inside a paragraph element in ${fn} is not flagged`,
        code: `toasts.addSuccess({ title: 'Done', text: ${wrap(
          `<p>Visit the <EuiLink href="/docs">documentation</EuiLink> to learn more.</p>`
        )} });`,
      },
      {
        name: `EuiLink inside a container alongside FormattedMessage in ${fn} is not flagged`,
        code: `toasts.addSuccess({ title: 'Done', text: ${wrap(
          `<div>
            <p>
              <FormattedMessage
  ...
  invalid: [
    ...
    ...MOUNT_WRAPPERS.map(({ fn, wrap }) => ({
      name: `standalone EuiLink CTA after body text in ${fn} is flagged`,
      code: `
        toasts.addWarning({
      ...
      errors: [
        {
          messageId: 'actionElementInMountContent',
          data: { elementName: 'EuiLink', method: 'addWarning', mountFn: fn },
        },
      ],
    })),
  ],
