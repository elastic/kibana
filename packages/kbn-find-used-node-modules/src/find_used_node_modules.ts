...
export async function findUsedNodeModules(options: Options) {
  ...
  const promise = Rx.lastValueFrom(
    path$.pipe(
      ...
      Rx.mergeMap(async (path) => {
        ...
        for (const req of reqs) {
          ...
          // no need to scan node_modules unless they're packages
          if (
            !result.nodeModule ||
    ...
    // when no entry survives the extension filter nothing ever flows through the pipeline
  ...
  // `inputs` is updated synchronously by `next()`, so if it is still 0 here the pipeline will
  // never emit and must be completed explicitly, otherwise the promise never settles and the
  // process exits silently once the event loop drains (e.g. a node_module entered only through
  // its package.json).
  if (inputs === 0) {
    path$.complete();
  }
