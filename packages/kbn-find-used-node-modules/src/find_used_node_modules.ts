...
function getPeerDeps(thisNodeModule: string) {
  let pkgPath;
  try {
    pkgPath = require.resolve(`${thisNodeModule}/package.json`);
  } catch (err) {
    // If `thisNodeModule` is an ECMAScript module, resolving its `package.json`
    // will most like fail with the following error:
    //
    //     Package subpath './package.json' is not defined by "exports"
    //
    // Let's fall back to a more hackish, less stable approach:
    if (err.code === 'ERR_PACKAGE_PATH_NOT_EXPORTED') {
      pkgPath = require
        .resolve(thisNodeModule)
        .replace(new RegExp(`${thisNodeModule}.*`), `${thisNodeModule}/package.json`);
    } else {
      throw err;
    }
  }
