...
tests by file and tells, for every file, whether an open `failed-test` issue already tracks it.
Read-only: nothing is filed or edited. Filing issues for the untracked suites is a separate step
that does not exist yet. A suite is:

- **tracked** when an open issue titled `Flaky <framework> test suite: <file>` exists;
- **related** when open per-test issues filed by `report_failed_tests` are about it. An issue
  matches a flaky **test** by the Scout test id, or by the test name together with the file (for
  Jest, the directory in the classname); a **moved** match has the same test and file name at
  another location; a **file** match only names the same file, for another test;
- **untracked** otherwise.
...
pipeline runs both commands daily and annotates the build with the tracked, related and untracked
suites.
