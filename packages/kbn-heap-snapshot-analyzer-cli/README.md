...
## Comparing serverless images

For a before/after comparison:

1. Resolve the `git-<sha>` image tags immediately before and after the
   change.
2. Capture both images with the same solution, Elasticsearch image,
   environment, idle period, and GC procedure.
3. Save distinct snapshots and analyzer output, for example
   `kibana-oblt-before.heapsnapshot` and `kibana-oblt-after.heapsnapshot`.
4. Run the additive source comparison:

   ```sh
   node --max-old-space-size=8192 scripts/heap_snapshot_analyzer.js \
     kibana-oblt-after.heapsnapshot \
     --compare=kibana-oblt-before.heapsnapshot \
     --json kibana-oblt-source-diff.json
   ```

The comparison processes each snapshot in a separate, sequential worker so
multi-GB snapshots are never resident in one process together. Tracked nodes
are grouped by the first package-bearing source frame above their deepest
allocator frame, walking through transparent loader instrumentation such as
`require-in-the-middle`. This attributes Node.js loader allocations to the
package whose loading triggered them while preserving the allocator in each row.
Nodes created before allocation tracking started fall back to direct package
evidence, inferred dominator ownership, or an explicit unattributed bucket.
Every node belongs to exactly one group, and the analyzer fails if the sum of
row deltas does not equal the total self-size delta.

Human-readable output includes both an additive rollup by triggering package
and the detailed source/allocator/node-type rows. JSON output includes the same
views as `packages` and `rows`.

This source diff is additive. Retained and counterfactual package values remain
useful graph views, but they must not be subtracted from total heap size because
their retention boundaries can overlap.

Reuse the Elasticsearch container between runs to reduce variation.

## Capturing without allocation tracking

For a plain source-build snapshot, skip the preload and use Node's built-in
signal handler:
...
For a plain serverless Docker snapshot, remove the preload mount and
`HEAP_TRACK_FORCE` from the `docker run` command, and use:

```sh
-e NODE_OPTIONS="--inspect=0.0.0.0:9229"
```

Then capture through CDP as above. The allocation-site tables are omitted
when tracking data is absent.
