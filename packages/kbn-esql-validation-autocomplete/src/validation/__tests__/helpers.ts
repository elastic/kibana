...
export interface WithErrorsCollectedReturn {
  setup: Setup;
...
export const withErrorsCollected = (_setup: Setup, filename: string): WithErrorsCollectedReturn => {
  ...
  return {
    setup: async (...args: Parameters<Setup>) => {
      ...
      return {
        ...
        expectErrors: async (...params: Parameters<ExpectErrors>) => {
          ...
          testCases.push({ query, error, warning });
    ...
    writeFixtureFile: async () => {
      const dir = path.join(__dirname, '..', '__fixtures__');
      const file = path.join(dir, `autogenerated.testSuite.${filename}.json`);
      const suite: ValidationTestSuite = { testCases };
