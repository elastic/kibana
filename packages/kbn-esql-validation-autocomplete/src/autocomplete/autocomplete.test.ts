...
import { ESQLAstItem, getAstAndSyntaxErrors } from '@kbn/esql-ast';
import {
  FunctionParameter,
  isFieldType,
  isSupportedDataType,
...
describe('autocomplete', () => {
  ...
  describe('sort', () => {
    ...
    testSuggestions('from a | sort keywordField desc ', ['NULLS FIRST ', 'NULLS LAST ', ',', '| ']);
    // @TODO: improve here
  ...
  describe('enrich', () => {
    ...
    for (const prevCommand of [
      ...
      testSuggestions(`from a ${prevCommand}| enrich policy on b with var0 = keywordField, var1 `, [
        '= $0',
        ',',
        '| ',
      ]);
  ...
  describe.only('eval', () => {
    ...
    testSuggestions.skip('from a | eval a=keywordField =~ ', [
      ...getFieldNamesByType(ESQL_STRING_TYPES),
      ...getFunctionSignaturesByReturnType('eval', ESQL_STRING_TYPES, { scalar: true }),
    ]);
    testSuggestions(
      'from a | eval a=round()',
      [
    ...
    for (const fn of evalFunctionDefinitions) {
      ...
      if (['substring'].includes(fn.name)) {
        ...
        for (let signatureIndex = 0; signatureIndex < fn.signatures.length; signatureIndex++) {
          const signature = fn.signatures[signatureIndex];
          // @ts-ignore Partial
          ...
          // @todo: add for empty case
          signature.params.forEach((param, i) => {
            // Generate all possible cases from the signature param
            ...
            if (i < signature.params.length) {
              ...
              const requiresMoreArgs = typesToSuggestNext.length - 1 > 0;
              ...
              console.log(
                `--@@typesToSuggestNext`,
                JSON.stringify(
                  typesToSuggestNext.map((d) => `${d.type}-${d.optional ? 'optional' : 'required'}`)
                )
              );
  ...
  describe('Invoke trigger kind (all commands)', () => {
    ...
    // SORT field order
    testSuggestions(
      'FROM index1 | SORT keywordField a',
      ['ASC ', 'DESC ', ',', '| '],
      undefined,
      33
    );
    ...
    testSuggestions(
      'FROM index1 | SORT keywordField ASC n',
  ...
  describe('advancing the cursor and opening the suggestion menu automatically ✨', () => {
    ...
    describe('function arguments', () => {
      ...
      testSuggestions(
        'FROM a | EVAL REPLACE(keywordField, keywordField, )',
        [
