...
This rule blocks imports of quarantined packages outside their allowlisted files. Config lives in `@kbn/dependency-quarantine` (`packages/kbn-dependency-quarantine/configs/`). Expanding an allowlist requires review from `@elastic/kibana-security`. `eslint-disable` comments do not apply to this rule.
...
Set `preserveFileExtensions: true` for native Node.js TypeScript packages that require explicit relative file extensions. This also preserves `/index.{ext}`. The rule is auto-fixable.
