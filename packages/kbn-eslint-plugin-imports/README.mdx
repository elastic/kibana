...
This rule blocks imports of quarantined packages outside their allowlisted files. Config lives in `@kbn/dependency-quarantine` (`packages/kbn-dependency-quarantine/configs/`). Expanding an allowlist requires review from `@elastic/kibana-security`. This rule cannot be disabled with `eslint-disable`.
