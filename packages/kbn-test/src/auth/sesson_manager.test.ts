...
const supportedServerlessRoles = ['admin', 'editor', 'viewer'];
...
const readSvlRolesFromResourcesMock = jest.spyOn(helper, 'readSvlRolesFromResources');
...
describe('SamlSessionManager', () => {
  describe('for local session', () => {
    beforeEach(() => {
      ...
      readSvlRolesFromResourcesMock.mockReturnValue(supportedServerlessRoles);
    });
    ...
    test(`throws error when role is not defined in yml file`, async () => {
      const nonExistingRole = 'tester';
      const expectedErrorMessage = `Role '${nonExistingRole}' is not defined in the supported list: ${supportedServerlessRoles.join(
        ', '
