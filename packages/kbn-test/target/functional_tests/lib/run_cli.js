...
async function runCli(getHelpText, run) {
  try {
    const userOptions = (0, _getopts.default)(process.argv.slice(2)) || {};
    ...
    if (!(error instanceof Error)) {
      error = new Error(`${(0, _util.inspect)(error)} thrown!`);
    }

    console.log();
    console.log(_chalk.default.red(error.message)); // CliError is a special error class that indicates that the error is produced as a part
