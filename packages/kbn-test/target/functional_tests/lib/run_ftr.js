...
exports.assertNoneExcluded = assertNoneExcluded;
...
async function createFtr({
  ...
  const config = await (0, _functional_test_runner.readConfigFile)(log, configPath);
  return new _functional_test_runner.FunctionalTestRunner(log, configPath, {
    mochaOpts: {
      bail: !!bail,
      grep
    },
...
async function assertNoneExcluded({
  configPath,
  options
}) {
  const ftr = await createFtr({
    configPath,
    options
  });
...
async function runFtr({
  ...
  if (failureCount > 0) {
    throw new _run_cli.CliError(`${failureCount} functional test ${failureCount === 1 ? 'failure' : 'failures'}`);
  }
}

async function hasTests({
  configPath,
  options
}) {
  const ftr = await createFtr({
