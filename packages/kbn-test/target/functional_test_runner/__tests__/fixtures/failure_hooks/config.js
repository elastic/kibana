...
function _default() {
  return {
    testFiles: [require.resolve('./tests/before_hook'), require.resolve('./tests/it'), require.resolve('./tests/after_hook')],
    services: {
      hookIntoLIfecycle({
        getService
      }) {
        const log = getService('log');
        getService('lifecycle').on('testFailure', async (err, test) => {
          log.info('testFailure %s %s', err.message, test.fullTitle());
          await (0, _bluebird.delay)(10);
