...
exports.filterSuitesByTags = filterSuitesByTags;
 ...
 */
function filterSuitesByTags({
  log,
  mocha,
  include,
  exclude
}) {
  mocha.excludedTests = []; // collect all the tests from some suite, including it's children

  const collectTests = suite => suite.suites.reduce((acc, s) => acc.concat(collectTests(s)), suite.tests); // if include tags were provided, filter the tree once to
