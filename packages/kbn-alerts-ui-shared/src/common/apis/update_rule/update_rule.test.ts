...
describe('updateRule', () => {
  ...
  test('should call rule update API with notifyWhen at rule level', async () => {
    const updatedRule = {
      params: {
        aggType: 'count',
        termSize: 5,
    ...
    expect(http.put).toHaveBeenCalledWith('/api/alerting/rule/12%2F3', {
      body: '{"name":"test-update","tags":["foo","bar"],"schedule":{"interval":"5m"},"params":{},"throttle":"5m","notify_when":"onThrottleInterval","actions":[{"group":"default","id":"2","params":{},"use_alert_data_for_template":false}],"alert_delay":{"active":50},"flapping":{"look_back_window":10,"status_change_threshold":10}}',
    });
