...
import { rewriteCreateBodyRequest, rewriteUpdateBodyRequest, UPDATE_FIELDS } from '../../apis';
import { useRuleFormSelector } from '../../hooks';
import { selectRuleForSave } from '../../store';
import { RuleFormRule } from '../../types';
import { useRuleType } from '../../contexts';

const stringify = (rule: RuleFormRule, edit: boolean): string => {
  try {
    const request = edit
      ? rewriteUpdateBodyRequest(pick(rule, UPDATE_FIELDS))
      : rewriteCreateBodyRequest(omit(rule, 'id'));
...
export const ShowRequestModal: React.FC<ShowRequestModalProps> = ({ onClose, isEdit = false }) => {
  const rule = useRuleFormSelector(selectRuleForSave);
  const { id: ruleTypeId } = useRuleType();
  const formattedRequest = useMemo(
    () => stringify({ ...rule, ruleTypeId }, isEdit),
    [rule, isEdit, ruleTypeId]
  );
