 ...
 */
import { entity } from '@kbn/apm-synthtrace-client';
import { HistoryEntityDocument } from '@kbn/apm-synthtrace-client/src/lib/entities/history_entity';
...
const scenario: Scenario<Partial<HistoryEntityDocument>> = async (runOptions) => {
  ...
  return {
    generate: ({
      ...
      const serviceEntity = entity.serviceEntity({
        serviceName: 'foo',
        agentName: 'nodeJs',
      ...
      const historyEntries = entityHistoryTimestamps.generator((timestamp) => {
        return serviceEntity.timestamp(timestamp).metrics({
