...
import { SignalGenerator } from '../lib/streaming/signal_generator';
...
export class Interval implements Iterable<number> {
  ...
  generator<TField>(
    map: (timestamp: number, index?: number) => Signal<TField> | Array<Signal<TField>>
  ): SignalIterable<TField> {
    return new SignalGenerator(this, function* (i) {
      let index = 0;
      for (const x of i) {
