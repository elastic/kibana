/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0 and the Server Side Public License, v 1; you may not use this file except
 * in compliance with, at your election, the Elastic License 2.0 or the Server
 * Side Public License, v 1.
 */

import { Interval } from '../../dsl/interval';
import { MergedSignalsStream } from './merged_signals_stream';
import { SignalIterable } from './signal_iterable';
import { Signal } from '../../dsl/signal';

export class SignalGenerator<TField> implements SignalIterable<TField> {
  private readonly _gen: () => Generator<Signal<TField>>;
  constructor(
    private readonly interval: Interval,
    dataGenerator: (interval: Interval) => Generator<Signal<TField>>
  ) {
    this._order = interval.from > interval.to ? 'desc' : 'asc';

    const generator = dataGenerator(this.interval);
    const peek = generator.next();
    const value = peek.value;

    let callCount = 0;
    this._gen = function* () {
      if (callCount === 0) {
        callCount++;
        yield value;
        yield* generator;
      } else {
        yield* dataGenerator(this.interval);
      }
    };

    const peekedNumberOfEvents = peek.done ? 0 : peek.value.yieldSignals().length;
    this._ratePerMinute = interval.estimatedRatePerMinute() * peekedNumberOfEvents;
  }

  private readonly _order: 'desc' | 'asc';
  order() {
    return this._order;
  }

  toArray(): TField[] {
    return Array.from(this).map((s) => s.fields);
  }

  merge(...iterables: Array<SignalIterable<TField>>): MergedSignalsStream<TField> {
    return new MergedSignalsStream([this, ...iterables]);
  }

  private readonly _ratePerMinute: number;
  estimatedRatePerMinute() {
    return this._ratePerMinute;
  }

  *[Symbol.iterator]() {
    for (const span of this._gen()) {
      for (const fields of span.yieldSignals()) {
        yield fields;
      }
    }
  }

  async *[Symbol.asyncIterator]() {
    for (const span of this._gen()) {
      for (const fields of span.yieldSignals()) {
        yield fields;
      }
    }
  }
}
