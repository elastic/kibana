...
describe('generic', () => {
  ...
  describe('.insertCommandArgument()', () => {
    it('can insert at the end of the list', () => {
      const src = 'FROM index | LIMIT 10';
      const { root } = parse(src);
      const command = generic.findCommandByName(root, 'from', 0);

      generic.insertCommandArgument(
        command!,
  ...
  describe('.appendCommandArgument()', () => {
    it('can append and argument', () => {
      const src = 'FROM index METADATA _id | LIMIT 10';
      const { root } = parse(src);
      const command = generic.findCommandByName(root, 'from', 0);

      generic.appendCommandArgument(
        command!,
        Builder.expression.source({ name: 'test', sourceType: 'index' })
      );
