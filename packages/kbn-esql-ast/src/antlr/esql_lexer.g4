...
/*
 * Before modifying this file, please read the section above as changes here have significant impact
 * in the ANTLR generated code and its consumption upstream (including Kibana).
 * 
 * A. To add a development token (only available behind in snapshot/dev builds)
 * 
 * Since the tokens/modes are in development, simply define them under the "// in development
 * section" and follow the section comments in that section. That is use the DEV_ prefix and use the
 * {this.isDevVersion()}? conditional. They are defined at the end of the file, to minimize the
 * impact on the existing token types.
 * 
 * B. To add a new (production-ready) token
 * 
 * Be sure to go through step A (add a development token). Make sure to remove the prefix and
 * conditional before promoting the tokens in production. Since tokens types (numbers) are generated
 * by ANTLR in a continuous fashion, it is desirable to avoid changing these values hence where
 * possible, add add them at the end of their respective section. Note that the use of lexing modes
 * prevents this since any addition to a mode (regardless where it occurs) shifts all the
 * declarations that follow in other modes.
 * 
 * C. Renaming a token
 * 
 * Avoid renaming the token. But if you really have to, please check with the Kibana team as they
 * might be using the generated ANTLR "dictionary".
 * 
 * D. To remove a token
 * 
 * If the tokens haven't made it to production (and make sure to double check), simply remove them
 * from the grammar. If the tokens get promoted to release, check with the Kibana team the impact
...
// in development
// 
// Before adding a new in-development command, to sandbox the behavior when running in production
// environments
// 
// For example: to add myCommand use the following declaration: DEV_MYCOMMAND :
// {this.isDevVersion()}? 'mycommand' -> ...
// 
// Once the command has been stabilized, remove the DEV_ prefix and the {}? conditional and move the
// command to the main section while preserving alphabetical order: MYCOMMAND : 'mycommand' -> ...
DEV_INLINESTATS:
	{this.isDevVersion()}? 'inlinestats' -> pushMode(EXPRESSION_MODE);
DEV_LOOKUP:
	{this.isDevVersion()}? 'lookup' -> pushMode(LOOKUP_MODE);
DEV_METRICS:
	{this.isDevVersion()}? 'metrics' -> pushMode(METRICS_MODE);
...
EXPR_MULTILINE_COMMENT_START: MULTILINE_COMMENT_START -> channel(HIDDEN);
EXPR_MULTILINE_COMMENT_END: MULTILINE_COMMENT_END -> channel(HIDDEN);
...
EXPLAIN_MULTILINE_COMMENT_START: MULTILINE_COMMENT_START -> channel(HIDDEN);
EXPLAIN_MULTILINE_COMMENT_END: MULTILINE_COMMENT_END -> channel(HIDDEN);
...
// in 8.14 ` were not allowed this has been relaxed in 8.15 since " is used for quoting
...
FROM_MULTILINE_COMMENT_START: MULTILINE_COMMENT_START -> channel(HIDDEN);
FROM_MULTILINE_COMMENT_END: MULTILINE_COMMENT_END -> channel(HIDDEN);
...
PROJECT_NAMED_OR_POSITIONAL_PARAM:
	{this.isDevVersion()}? NAMED_OR_POSITIONAL_PARAM -> type(NAMED_OR_POSITIONAL_PARAM);
...
LOOKUP_UNQUOTED_SOURCE:
	UNQUOTED_SOURCE -> type(UNQUOTED_SOURCE);
LOOKUP_QUOTED_SOURCE: QUOTED_STRING -> type(QUOTED_STRING);

LOOKUP_LINE_COMMENT: LINE_COMMENT -> channel(HIDDEN);

LOOKUP_MULTILINE_COMMENT: MULTILINE_COMMENT -> channel(HIDDEN);

LOOKUP_WS: WS -> channel(HIDDEN);
...
LOOKUP_FIELD_ID_PATTERN: ID_PATTERN -> type(ID_PATTERN);

LOOKUP_FIELD_LINE_COMMENT: LINE_COMMENT -> channel(HIDDEN);

LOOKUP_FIELD_MULTILINE_COMMENT:
	MULTILINE_COMMENT -> channel(HIDDEN);

LOOKUP_FIELD_WS: WS -> channel(HIDDEN);
...
METRICS_UNQUOTED_SOURCE:
	UNQUOTED_SOURCE -> type(UNQUOTED_SOURCE), popMode, pushMode(CLOSING_METRICS_MODE);
METRICS_QUOTED_SOURCE:
	QUOTED_STRING -> type(QUOTED_STRING), popMode, pushMode(CLOSING_METRICS_MODE);

METRICS_LINE_COMMENT: LINE_COMMENT -> channel(HIDDEN);

METRICS_MULTILINE_COMMENT:
	MULTILINE_COMMENT -> channel(HIDDEN);

METRICS_WS: WS -> channel(HIDDEN);
...
CLOSING_METRICS_LINE_COMMENT: LINE_COMMENT -> channel(HIDDEN);

CLOSING_METRICS_MULTILINE_COMMENT:
	MULTILINE_COMMENT -> channel(HIDDEN);

CLOSING_METRICS_WS: WS -> channel(HIDDEN);
...
CLOSING_METRICS_PIPE: PIPE -> type(PIPE), popMode;