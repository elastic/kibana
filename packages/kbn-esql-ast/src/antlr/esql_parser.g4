...
sourceCommand:
	...
	| showCommand
	// in development
	| {this.devVersion()}? metricsCommand;
...
processingCommand:
	...
	| mvExpandCommand
	// in development
	| {this.devVersion()}? inlinestatsCommand
	| {this.devVersion()}? lookupCommand
	| {this.devVersion()}? matchCommand;

whereCommand: WHERE booleanExpression;
...
booleanExpression:
	...
	| valueExpression (NOT)? IN LP valueExpression (
...
operatorExpression:
	...
	| left = operatorExpression operator = (
		ASTERISK
		| SLASH
		| PERCENT
	) right = operatorExpression														# arithmeticBinary
...
functionExpression:
	identifier LP (
		ASTERISK
		| (booleanExpression (COMMA booleanExpression)*)
	)? RP;

dataType: identifier # toDataType;

rowCommand: ROW fields;
...
fromCommand: FROM indexPattern (COMMA indexPattern)* metadata?;

indexPattern: clusterString COLON indexString | indexString;

clusterString: UNQUOTED_SOURCE;

indexString: UNQUOTED_SOURCE | QUOTED_STRING;
...
metricsCommand:
	METRICS indexPattern (COMMA indexPattern)* aggregates = fields? (
		BY grouping = fields
	)?;

evalCommand: EVAL fields;

statsCommand: STATS stats = fields? (BY grouping = fields)?;
...
limitCommand: LIMIT INTEGER_LITERAL;

sortCommand: SORT orderExpression (COMMA orderExpression)*;

orderExpression:
	booleanExpression ordering = (ASC | DESC)? (
		NULLS nullOrdering = (FIRST | LAST)
	)?;

keepCommand: KEEP qualifiedNamePatterns;

dropCommand: DROP qualifiedNamePatterns;

renameCommand: RENAME renameClause (COMMA renameClause)*;
...
grokCommand: GROK primaryExpression string;

mvExpandCommand: MV_EXPAND qualifiedName;

commandOptions: commandOption (COMMA commandOption)*;

commandOption: identifier ASSIGN constant;

booleanValue: TRUE | FALSE;

numericValue: decimalValue | integerValue;

decimalValue: (PLUS | MINUS)? DECIMAL_LITERAL;

integerValue: (PLUS | MINUS)? INTEGER_LITERAL;

string: QUOTED_STRING;

comparisonOperator: EQ | NEQ | LT | LTE | GT | GTE;

explainCommand: EXPLAIN subqueryExpression;

subqueryExpression: OPENING_BRACKET query CLOSING_BRACKET;

showCommand: SHOW INFO # showInfo;

metaCommand: META FUNCTIONS # metaFunctions;

enrichCommand:
	ENRICH policyName = ENRICH_POLICY_NAME (
		ON matchField = qualifiedNamePattern
	)? (WITH enrichWithClause (COMMA enrichWithClause)*)?;

enrichWithClause: (newName = qualifiedNamePattern ASSIGN)? enrichField = qualifiedNamePattern;
...
matchCommand: MATCH matchQuery;

matchQuery: QUOTED_STRING;