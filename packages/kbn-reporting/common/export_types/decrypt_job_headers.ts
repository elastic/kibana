...
export const decryptJobHeaders = async (
  ...
  try {
    if (typeof headers !== 'string') {
      throw new Error(
        i18n.translate('reporting.common.exportTypes.missingJobHeadersErrorMessage', {
    ...
    throw new Error(
      i18n.translate('reporting.common.exportTypes.failedToDecryptReportJobDataErrorMessage', {
        defaultMessage:
          'Failed to decrypt report job data. Please ensure that {encryptionKey} is set and re-generate this report. {err}',
        values: { encryptionKey: 'reporting.encryptionKey', err: err.toString() },
      })
    );
  }
};
