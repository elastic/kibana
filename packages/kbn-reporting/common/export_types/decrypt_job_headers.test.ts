...
describe('headers', () => {
  test(`fails if it can't decrypt headers`, async () => {
    ...
    await expect(getDecryptedHeaders()).rejects.toMatchInlineSnapshot(
      `[Error: Failed to decrypt report job data. Please ensure that reporting.encryptionKey is set and re-generate this report. TypeError: Invalid initialization vector]`
    );
  });
