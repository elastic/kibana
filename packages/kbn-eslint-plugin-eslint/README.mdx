...
An ESLint and Oxlint plugin exposing custom rules used and built specifically for development within Kibana. Fourteen custom rules run in Oxlint using its `createOnce` alternative API and are wrapped with `eslintCompatPlugin` so existing ESLint configuration remains compatible. `module_migration` remains ESLint-only until its RegExp-bearing options have an equivalent Oxlint configuration, and `no_unsafe_hash` remains ESLint-only to preserve protected disable-directive enforcement.
