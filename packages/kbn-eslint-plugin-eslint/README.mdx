...
An ESLint and Oxlint plugin exposing custom rules used and built specifically for development within Kibana. Thirty-two custom rules run in Oxlint using its `createOnce` alternative API and are wrapped with `eslintCompatPlugin` so existing ESLint configuration remains compatible. Only `no_unsafe_hash` remains ESLint-only to preserve protected disable-directive enforcement.

Oxlint loads the plugin from `.oxlintrc.json` with the existing rule namespace preserved:

```json
{
  "jsPlugins": [
    { "name": "@kbn/eslint", "specifier": "./packages/kbn-eslint-plugin-eslint/oxlint_plugin.js" }
  ]
}
```

Rule configuration uses the existing `@kbn/eslint/<rule>` names in both linters.
...
For Oxlint JSON configuration, serialize `include` and `exclude` regular expressions as `{ "source": "...", "flags": "..." }`. JavaScript ESLint configurations can continue to use `RegExp` values.
