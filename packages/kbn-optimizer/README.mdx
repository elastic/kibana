...
The `@kbn/optimizer` is automatically executed from the dev cli, the Kibana build scripts, and in CI. If you're running Kibana locally in some other way you might need to build the plugins manually, which you can do by running `node scripts/build_kibana_platform_plugins.mts` (pass `--help` for options).
...
This is essentially what we're doing in [`scripts/build_kibana_platform_plugins.mts`][Cli] and the new [build system task][BuildTask].
