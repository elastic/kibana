...
async function build() {
  if (!needsRebuild()) {
    console.log('@kbn/transpile-packages-cli: already built, skipping');
    return;
  }

  console.log('@kbn/transpile-packages-cli: building...');
  ...
  try {
    // Use esbuild for fast compilation (bundled with Vite)
    ...
    console.log('@kbn/transpile-packages-cli: build complete');
  } catch (error) {
    console.error('@kbn/transpile-packages-cli: build failed:', error.message);
    process.exit(1);
  }
}
