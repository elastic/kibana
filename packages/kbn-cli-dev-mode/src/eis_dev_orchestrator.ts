...
/**
 * EIS connector discovery for `yarn start --eis` and `yarn serverless-* --eis`.
 *
 * Two-phase flow:
 *
 ...
 * by `yarn es snapshot --eis` or `yarn es serverless --eis`.
 *
 ...
 *                           with `--ssl` (e.g. `yarn es snapshot --eis --ssl`).
...
const discoverEisEndpoints = async (
  ...
  throw new Error(
    [
      ...
      'Make sure Elasticsearch was started with `yarn es snapshot --eis` or `yarn es serverless --eis` and that the CCM API key was set successfully.',
    ].join('\n')
  );
};
