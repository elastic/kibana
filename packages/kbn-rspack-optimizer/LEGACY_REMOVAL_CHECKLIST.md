...
- [ ] `src/platform/packages/shared/kbn-test/src/functional_tests/run_tests/build_kibana_platform_plugins.ts`: Remove the `[rspack-transition]` conditional; keep only the Rspack build script reference (or its successor name after the package rename).
