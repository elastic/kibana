...
export async function generateRenovateCodeowners(
  ...
  const driftCount = report.ruleDrift.length;
  ...
  if (driftCount > 0) {
    ...
    log.info(`🔧 Reviewer drift detected (first ${preview.length} rules):`);
    ...
    if (driftCount > preview.length) {
      log.info(`   ... and ${driftCount - preview.length} more`);
    }
    log.write('\n');
  }
  ...
  if (mode === 'check' && (driftCount > 0 || missingCount > 0)) {
    // Non-zero exit in check mode if we have actionable drift or missing coverage.
    process.exitCode = 1;
  }
}
