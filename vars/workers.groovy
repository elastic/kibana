...
def functional(name, Closure setup, Map processes) {
  return {
    parallelProcesses(name: name, setup: setup, processes: processes, delayBetweenProcesses: 20, label: 'tests-xxl')
  }
}

def functional(Map params) {
  def config = [name: 'functional-worker', delayBetweenProcesses: 20, label: 'tests-xxl'] + params

  return {
    parallelProcesses(config)
  }
}
