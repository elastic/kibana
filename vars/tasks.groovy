...
def test() {
  tasks([
    ...
    kibanaPipeline.scriptTaskDocker('Karma Tests', 'test/scripts/test/karma_ci.sh'),
...
def functionalXpack(Map params = [:]) {
  def config = params ?: [
    ...
    savedObjectsFieldMetrics: false,
  ...
  task {
    ...
    whenChanged(['x-pack/plugins/siem/', 'x-pack/test/siem_cypress/']) {
      task(kibanaPipeline.functionalTestProcess('xpack-siemCypress', './test/scripts/jenkins_siem_cypress.sh'))
    }
  }
}
