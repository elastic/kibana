...
- Reuse existing components before building custom ones. Prefer Kibana UI components from `src/platform/kbn-ui` over direct `@elastic/eui` equivalents when available because they enforce stricter Kibana conventions; otherwise prefer EUI over custom implementations.
...
- Avoid overriding internal styles of EUI and Kibana UI components. If the supported APIs cannot meet a product requirement, align the custom behavior with product and design first, then use a CSS override only as a last resort.
