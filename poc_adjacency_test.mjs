...
const kbn = (m, p, b) =>
  fetch(`${KBN}${p}`, { method: m, headers: H, body: b ? JSON.stringify(b) : undefined }).then(
...
const run = async () => {
  ...
  for (const value of ['10.0.0.0-10.0.0.255', '10.0.1.0-10.0.1.255']) {
    ...
    if (r.status >= 400)
      throw new Error(`add ${value} failed: ${r.status} ${JSON.stringify(r.json)}`);
  }

  // Wait for the async coalesce job to converge: the state must be clean at the source
  // version of the last write, since one interval alone can be the first write's result.
  let coalesced = [];
  for (let i = 0; i < 60; i++) {
    await sleep(1000);
    await es('POST', `/${INDEX}/_refresh`);
    const res = await es('POST', `/${INDEX}/_search`, {
      size: 50,
      query: { term: { kind: 'coalesced' } },
      _source: ['range_start', 'range_end'],
    });
    coalesced = (res.json.hits?.hits ?? [])
      .map((h) => `${h._source.range_start}-${h._source.range_end}`)
      .sort();
    const state = (await es('GET', `/${INDEX}/_doc/__state`)).json?._source;
    if (
      state?.status === 'clean' &&
      state.coalesced_version === state.source_version &&
  ...
  console.log(
    pass
      ? 'PASS: adjacent ranges merged into one interval'
      : 'FAIL: expected ["10.0.0.0-10.0.1.255"]'
  );

  // Also confirm both source docs are retained (adjacency merge must not touch sources).
  const src = await es('POST', `/${INDEX}/_search`, {
    size: 0,
  ...
  console.log(
    nSrc === 2 ? 'PASS: both source docs retained' : `FAIL: expected 2 sources, got ${nSrc}`
  );
