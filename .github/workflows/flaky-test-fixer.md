...
3. Apply the smallest test-side patch that addresses the root cause. A retried interaction is not a root-cause fix: never wrap a click/type/navigation in `retry()` / `retry.tryForTime` so a "missed" interaction registers on a later attempt (it hides an actionability bug a real user would hit) — see the `flaky-test-investigator` skill's retry pitfall. Don't add explanatory code comments to the patch by default — a good test-side fix is self-explanatory. Add one only when the fix is particularly involved or non-obvious, and keep it to 1–2 sentences; a simple change like a timeout bump never warrants a comment.
