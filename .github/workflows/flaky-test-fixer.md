...
7. Run the [Pre-open self-check](#pre-open-self-check) against the patch you are about to ship. It is the last chance to catch the failure modes that most often cost a reviewer a commit on this PR; resolve every item before continuing.
8. Decide the backport strategy and open the PR (see "PR format" and "Backport label" below). If the fix has to land on a version branch rather than `main`, don't open a PR at all — hand it over in the outcome comment instead (see "Fixes that must target a version branch").
9. Post the outcome comment on the issue (see "Outcome comment" below). Do this in every run, whether or not you opened a PR.
10. Remove the `ai:fix-flaky` label from the issue via the `remove-labels` safe output. Do this in **every** run once you have a result — whether you opened a PR, found an existing one, or opened none.
11. **Only if you opened a PR in step 8**, call the `link_fix_pr` tool with `confirm: true`. It runs after the PR and your comment exist and replaces the `%%FIX_PR_URL%%` and `%%FIX_PR_BADGE%%` placeholders in your outcome comment with the PR link and a live PR-state badge. You cannot know the PR number while running (the PR is created afterwards), so leave the placeholders in place and never write the URL, number, or badge yourself — this tool is how they get filled.
12. **Only if you opened a PR in step 8 and confidently identified a real, non-bot introducing PR author** (the same person you `cc`'d on the `Fixes` line), call the `request_fix_review` tool with their GitHub login in `author` (no leading `@`) to request them as a reviewer on the fix PR. Skip this otherwise — you couldn't identify the author, or it's a bot (includes `kibanamachine`). Like `link_fix_pr` it runs after the PR is created.
...
Before you open the PR, answer each of these about the patch as it stands. They are the failure modes that most often cost a reviewer a commit on a fixer PR, and re-reading the guardrails is not an answer — name the specific code.

1. **Terminal signal.** Which assertion produced the reported error, and what does your patch wait on? If those are not the same object, say why the thing you wait on cannot be satisfied before the thing the assertion reads.
2. **Retry idempotency.** For every retry your patch adds *or relies on*, list the actions inside the loop body and confirm each is safe to repeat. A body that isn't idempotent has to be made idempotent here.
3. **Precedent.** For every helper, wait, or prior PR you cite as justification: you opened it and confirmed it works — the wait has an explicit timeout, the selector matches something real, the earlier fix held.
4. **Coverage held.** What did the test assert and exercise before, and what does it assert and exercise now? Anything narrower or more lenient needs a stated reason in the PR body, or it isn't a fix. Rewrites lose things quietly — a second keypress, one case of a mapping, an intermediate step — so diff the behavior, not just the assertions.
5. **Blast radius.** If you touched a page object, service, fixture, or `kbn-*` package: you listed its other call sites and rejected the local alternative for a reason. If you changed a signature — a new parameter, a different return — read each call site and confirm it still compiles: CI is the only type check on this patch, so a type error here costs a reviewer a commit.
6. **Leftovers.** Nothing that suppresses this test survives the patch:

   ```bash
   grep -rnE 'describe\.skip|it\.skip|test\.skip|FLAKY:|Failing: See' <changed files and the failing spec>
   ```

   Any hit referencing this issue must be gone before you open.
7. **Contested remedy.** If two remedies were defensible, you found prior art in the repo, or you disclosed the choice as the codeowner's call in the Context section.

Anything you cannot resolve either blocks the PR or goes into the body as a single line under "Not verified locally". Don't add a self-check section to the PR body — the existing Context and Verification blocks carry whatever came out of this.
