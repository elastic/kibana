...
- **`test-environment`**: test code is fine, but its surroundings are problematic (e.g., leaked state from prior tests, flaky fixture init, missing `data-test-subj` the test relies on, parallel-slot interference, or genuinely inherent multi-node/eventual-consistency latency with a usable signal the test should wait on — a config poll, cluster or entity-store sync). If instead a read comes back stale where the product should be consistent, or no convergence signal exists at all, that is `application`, not `test-environment`.
