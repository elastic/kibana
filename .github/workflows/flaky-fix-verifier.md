...
3. **Screen the patch against the Fix guardrails.** Check `pr-diff.txt` against the [Fix guardrails](#fix-guardrails) before spending any runs: a guardrail-violating fix must never be verified as-is, because a masking patch holds across every flaky run precisely because it hides the root cause. Look in particular for a retry or error-tolerance loop anywhere (test, framework, or application code); a framework internal newly exposed to enable the fix; a retry the patch *relies on* whose body cannot safely run twice; a wait or selector added to a shared page object, service, or `kbn-*` package where the calling test would do; a weakened or narrowed assertion; and a contested remedy (timeout bump, delete-vs-stabilize, relax-vs-correct) chosen without prior art or disclosure. Derive a compliant fix, push it (see [Pushing a revised fix](#pushing-a-revised-fix)), and verify that revision instead. If you cannot derive a compliant fix, add `flaky-fix-check:failed`, post a failed comment naming the violated guardrail, and open the PR for review (see [Opening the PR for review](#opening-the-pr-for-review)).
...
- Before you push, run the same pre-open self-check the fixer runs on its patch (§ "Pre-open self-check" in `.github/workflows/flaky-test-fixer.md`) against your revision. A revision is agent output too, and it gets the same bar: name the terminal signal you wait on, confirm any retry body you add or lean on is idempotent, verify every helper you cite actually works, keep the assertion at least as strong, justify a shared-code edit over a local one, and leave no `describe.skip` / `it.skip` / `FLAKY:` referencing this issue behind.
