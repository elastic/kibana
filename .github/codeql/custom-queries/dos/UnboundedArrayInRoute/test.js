...
const badBodySchema = schema.arrayOf(
  schema.object({
    // $ Alert
    name: schema.string(),
  })
...
router.post(
  {
    ...
    validate: {
      body: schema.arrayOf(schema.arrayOf(schema.string(), { maxSize: 10 }), { maxSize: 10 }),
    },
  },
  handler
);
...
const goodBodySchema = schema.arrayOf(
  schema.object({
    name: schema.string(),
  }),
  { maxSize: 100 }
...
const reusableSchema = schema.arrayOf(
  schema.object({
    // $ Alert
    id: schema.string(),
    name: schema.string(),
