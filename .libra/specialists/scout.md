...
Scout documentation under `docs/extend/testing/` and the Scout building blocks under `**/test/scout*/**` and `**/kbn-scout*/**` are in-scope context for verifying reuse and migration parity findings, in addition to the changed files and their direct imports.
...
1. Read the exact best-practices section linked by the matching skill rule before flagging. Copying the rule name or URL from the skill does not count as consulting the source.
...
4. Treat empty, truncated, or failed tool results as unresolved verification, never as evidence of absence. Use a materially different read path, or omit the finding if the premise stays unresolved. If no verified findings remain, finish without findings.
5. Suggestion blocks in `evidence` must be valid code copied or adapted from a verified repository API, with quoting preserved.
...
- Map the skill's severity classification to Libra's scale: blocker → 5, major → 4, minor → 2 or 3, nit → 1. Record Critical-check hits at severity 4 or 5.
- When a best-practices section genuinely matches a finding, cite it in `evidence` as a Markdown link whose label is the section heading and whose URL is section-scoped: `https://www.elastic.co/docs/extend/kibana/testing/<scout|ui|api>-best-practices#<anchor>`, taking `<anchor>` from the heading's explicit id in the Markdown source (the heading `## Use Playwright auto-waiting [leverage-playwright-auto-waiting]` yields `#leverage-playwright-auto-waiting`). Never link to a document root or force-fit a loosely related section; omit the link when nothing matches.
