...
**Reading time:** \~4 minutes | Concise reference. [The short version](?tab=t.8ts5kr6m0uek) | [Full RFC](?tab=t.0) | [PoC PR](https://github.com/elastic/kibana/pull/268994)
...
The public start contract gains **`invokeHook(triggerId, payload, capabilities?)`** — the single entry point for firing a synchronous hook. An optional `capabilities` map carries call-scoped objects (e.g. `AnonymizationContext`) to step handlers without exposing them in YAML. `invokeHook` runs all registered in-process handlers in registration order — no task manager, no background jobs — and returns a `HookResult` (`completed`, `pass_through`, or `failed`). For the inline YAML path, `capabilities` is forwarded as a direct function argument into `executeWorkflowSync`, which places it on each step's `handlerContext.capabilities`. Its lifetime is exactly the call stack of that single execution — no shared map, no session-keyed lookup, no concurrency race.
...
Want more details? see [the short version](?tab=t.8ts5kr6m0uek) or the full [RFC](?tab=t.0)