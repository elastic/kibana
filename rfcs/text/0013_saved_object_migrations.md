...
11.  Mark the migration as complete. This is done as a single atomic
   ...
   10. Check that `.kibana` alias is still pointing to the source index
   11. Point the `.kibana_7.10.0` and `.kibana` aliases to the target index.
   12. Remove the temporary index `.kibana_7.10.0_reindex_temp`
   13. If this fails with a "required alias [.kibana] does not exist" error or "index_not_found_exception" for the temporary index, fetch `.kibana` again:
