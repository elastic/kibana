...
While running Kibana from source in development, the proxy server will ensure that client-side code for plugins is compiled and available. This is handled by the [optimizerMixin](https://github.com/elastic/kibana/blob/master/src/optimize/optimize_mixin.ts), where routes for the client-side assets are created. When code is updated, the routes will pause requests while @kbn/optimizer re-bundles the plugins. With Bazel, we will utilize [iBazel](https://github.com/bazelbuild/bazel-watche) to watch for file changes and re-build the plugin targets when necessary. The watcher will emit [events](https://github.com/bazelbuild/bazel-watcher#remote-events) that we will use to block requests and provide feedback to the logs.
