...
const createExceptionRule = async (ruleId, listId) => {
  await kbn('POST', '/api/exception_lists', {
    list_id: EXC_CONTAINER, name: EXC_CONTAINER, description: EXC_CONTAINER, type: 'detection', namespace_type: 'single',
  });
  await kbn('POST', '/api/exception_lists/items', {
    list_id: EXC_CONTAINER, name: 'ip in list', description: 'ip in list', type: 'simple', namespace_type: 'single',
    entries: [{ field: 'source.ip', type: 'list', operator: 'included', list: { id: listId, type: 'ip' } }],
  });
  const container = await kbn('GET', `/api/exception_lists?list_id=${EXC_CONTAINER}&namespace_type=single`);
  return kbn('POST', '/api/detection_engine/rules', {
    // enabled, so the rule holds an API key for the scan to check
    rule_id: ruleId, name: ruleId, description: ruleId, type: 'query', enabled: true,
    risk_score: 1, severity: 'low', from: 'now-1h', interval: '5m',
    index: ['nonexistent-*'], query: '*', language: 'kuery',
    exceptions_list: [{ id: container.json.id, list_id: EXC_CONTAINER, type: 'detection', namespace_type: 'single' }],
  });
};
...
const cleanup = async () => {
  ...
  for (const id of [MIG_LIST, MAYBE_LIST, EXC_LIST]) await kbn('DELETE', `/api/lists?id=${id}&deleteReferences=true`);
...
const main = async () => {
  ...
  check('a "maybe" finding warns but does not block (200 without force)', migMaybe.status === 200 && migMaybe.json?.warningLevel === 'maybe');
  check('the warning names the rule that reads .items', typeof migMaybe.json?.warning === 'string' && migMaybe.json.warning.includes(ruleId));
  ...
  if (excRule.status >= 400) throw new Error(`create exception rule failed: ${excRule.status} ${JSON.stringify(excRule.json)}`);
  await sleep(1500);
  ...
  check('an exception rule whose key can read the alias does not block (200 without force)', migExc.status === 200);
  check('the level is not referenced: exception rules do not set it', excReport?.level === 'none' || excReport?.level === 'maybe');
  check('the exception rule is reported with reason exception and canRead true', (excReport?.rules ?? []).some((r) => r.id === excRule.json.id && r.reason === 'exception' && r.canRead === true));
