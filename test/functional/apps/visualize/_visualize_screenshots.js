  ...
  describe('visualize screenshots', function describeIndexTests() {
    ...
    */
    ...
    it('should compare visualization screenshot for bar chart with count metric agg and date histogram with terms', async function () {
      ...
      await compareScreenshot('area_chart_bar', expectedSavedVizName);
    });
    ...
    async function compareScreenshot(folder, expectedSavedVizName, threshold = 0.065) {
      // //
      //     //const path = require('path');
      //     //path.exists()
      //       // check if we already have BASELINE_DIRECTORY/${expectedSavedVizName}.png
      //       // else
      //       const BASELINE_DIRECTORY = resolve(config.get('screenshots.directory'), 'baseline');
      //       // download(from gcs/expectedSavedVizName, BASELINE_DIRECTORY)
      //       //const gcsPath = 'https://storage.googleapis.com/kibana-ci-screenshots/visualize';
      //       //area_chart_bar/screenshot_area_chart_bar.png
      //       const timeout = 10000; // milliseconds?
      //       //const url = `${gcsPath}/${folder}/${expectedSavedVizName}.png`
      //       log.debug(`>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ${url}`);
      //       //await download(log, url, BASELINE_DIRECTORY, timeout);
      //       log.debug('we got the path');
