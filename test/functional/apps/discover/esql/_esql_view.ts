...
export default function ({ getService, getPageObjects }: FtrProviderContext) {
  ...
  describe('discover esql view', async function () {
    ...
    describe('ES|QL in Discover', () => {
      ...
      it('should render the histogram for indices with no @timestamp field when the ?_t_start, ?_t_end params are in the query', async function () {
        await PageObjects.discover.selectTextBaseLang();
        await PageObjects.unifiedFieldList.waitUntilSidebarHasLoaded();

        const testQuery = `from kibana_sample_data_flights | limit 10 | where timestamp >= ?_t_start and timestamp <= ?_t_end`;
