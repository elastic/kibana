...
export default function ({ getService, getPageObjects }: FtrProviderContext) {
  ...
  const tearDownDiscoverPage = async () => {
    await kibanaServer.savedObjects.clean({ types: ['search', 'index-pattern'] });
  };

  describe('discover test', function describeIndexTests() {
    describe('discover accessibility tests', () => {
      before(setUpDiscoverPage);
      ...
      it('should give focus to the saved search title h1 on navigate', async () => {
        ...
        expect(await skipButton.getAttribute('data-test-subj')).to.eql(
    ...
    describe('discover functionality tests', () => {
      ...
      describe('query', function () {
        ...
        it('should modify the time range when the histogram is brushed', async function () {
          ...
          await retry.waitFor(
            'doc table containing the documents of the brushed range',
            async () => {
              const rowData = await PageObjects.discover.getDocTableField(1);
