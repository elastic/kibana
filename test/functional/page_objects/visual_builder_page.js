  ...
  class VisualBuilderPage {
    ...
    async enterMarkdown(markdown) {
      ...
      if (process.platform === 'darwin') {
        await input.pressKeys([browser.getKeys().COMMAND, 'a']); // Select all Mac
      } else {
        await input.pressKeys([browser.getKeys().CONTROL, 'a']); // Select all for everything else
      }
      await input.pressKeys(browser.getKeys().NULL); // Release modifier keys
      await input.pressKeys(browser.getKeys().BACK_SPACE); // Delete all content
