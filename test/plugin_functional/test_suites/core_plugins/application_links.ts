...
export default function({ getService, getPageObjects }: PluginFunctionalProviderContext) {
  ...
  describe('cross-app links', () => {
    ...
    it('can be disabled by adding the `disableCoreNavigation` class on the link', async () => {
      await testSubjects.click('applink-prevention-test');

      expect(await testSubjects.exists('app-applink_end')).to.eql(true);
      expect(await browser.getCurrentUrl()).to.eql(getKibanaUrl('/app/applink_end/path'));
      expect(await wasReloaded()).to.eql(true);
    });

    it('can be disabled by adding the `disableCoreNavigation` class on any parent', async () => {
      await testSubjects.click('applink-parent-prevention-test');

      expect(await testSubjects.exists('app-applink_end')).to.eql(true);
      expect(await browser.getCurrentUrl()).to.eql(getKibanaUrl('/app/applink_end/other'));
      expect(await wasReloaded()).to.eql(true);
    });
  });
}
