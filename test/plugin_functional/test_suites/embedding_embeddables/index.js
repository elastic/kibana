...
export default function ({ getService, getPageObjects, loadTestFile }) {
  ...
  describe('embedding embeddables', function () {
    before(async () => {
      await esArchiver.loadIfNeeded('../functional/fixtures/es_archiver/logstash_functional');
      await esArchiver.load('../functional/fixtures/es_archiver/embedding_embeddables');
      await kibanaServer.uiSettings.replace({ 'dateFormat:tz': 'Australia/North', 'defaultIndex': 'logstash-*' });
      await browser.setWindowSize(1300, 900);
      await PageObjects.common.navigateToApp('settings');
      await PageObjects.header.clickGlobalNavigationLink('Embedding Embeddables');
    });
