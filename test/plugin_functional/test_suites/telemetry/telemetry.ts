  ...
  describe('Telemetry service', () => {
    ...
    describe('Opt-in/out banners', function () {
      ...
      it('shows the banner in the default configuration', async () => {
        await PageObjects.common.navigateToApp('home');
        expect(await find.existsByLinkText('Enable it here.')).to.eql(true);
        expect(await find.existsByLinkText('Disable it here.')).to.eql(false);
      });
      ...
      it('does not show the banner if opted-in', async () => {
        ...
        await PageObjects.common.navigateToApp('home');
        expect(await find.existsByLinkText('Enable it here.')).to.eql(false);
        expect(await find.existsByLinkText('Disable it here.')).to.eql(false);
      });
