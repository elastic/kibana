...
if [[ -z "$CODE_COVERAGE" ]] ; then
  ...
  cd "$XPACK_DIR"
  checks-reporter-with-killswitch "X-Pack Jest" node --max-old-space-size=8192 scripts/jest --ci --verbose --detectOpenHandles
  echo ""
  echo ""
  ...
  node --max-old-space-size=8192 scripts/jest --ci --verbose --detectOpenHandles --coverage
