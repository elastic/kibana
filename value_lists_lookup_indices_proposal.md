...
**One accuracy note, and the production form.** Counting lookup lists by distinct `.value-list-*` index misses an empty lookup list, because an index with no documents contributes nothing to an `_index` aggregation. That is acceptable for an adoption signal, where empty lists are not the point. The precise form is a single terms aggregation on the `storage.type` field, which the container now carries, splitting legacy versus lookup exactly and not missing empty lists. The telemetry task still ships the index based counts today, so the adoption series is continuous; moving the breakdown to `storage.type` is a drop-in that keeps the same payload field names.
