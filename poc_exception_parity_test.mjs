...
const nameOf = (c) => `${P}-${c.kind}-${c.storage}-${c.size}-${c.op}${c.idCase === 'mixed' ? '-mixed' : ''}`;
// the list id: as the name, except the mixed case cells, whose id has capitals and a space
const listIdOf = (c) => (c.idCase === 'mixed' ? `Poc-Parity_${c.kind}_${c.storage}_MIXED Case` : nameOf(c));
// mirrors normalizeListId in services/lookup/get_lookup_index.ts
const normalizedIdOf = (id) => id.toLowerCase().replace(/[^a-z0-9._-]/g, '-').replace(/^[-_+.]+/, '');
...
const importValues = async (listId, type, values) => {
  ...
  if (res.status >= 400) throw new Error(`import into ${listId} failed: ${res.status} ${await res.text()}`);
};

const largeValues = () => Array.from({ length: LARGE_COUNT }, (_, i) => `10.${20 + Math.floor(i / 65536)}.${Math.floor(i / 256) % 256}.${i % 256}`);
...
const writeEvents = async () => {
  const bulk = [IN_SMALL, IN_LARGE, OUT]
    .map((ip) => `${JSON.stringify({ index: {} })}\n${JSON.stringify({ '@timestamp': now(), host: { name: P }, source: { ip } })}`)
    .join('\n') + '\n';
  await es('PUT', `/${SRC_INDEX}`, { mappings: { properties: { '@timestamp': { type: 'date' }, host: { properties: { name: { type: 'keyword' } } }, source: { properties: { ip: { type: 'ip' } } } } } });
...
const checkMixedCaseId = async (cell, created, check) => {
  ...
  check(`${name}: list id kept as authored on create ("${created.json?.id}")`, created.json?.id === listId);
  if (cell.storage === 'lookup') {
    const { index, alias } = created.json?.storage?.locator ?? {};
    check(`${name}: locator index is the normalized name (${index})`, index === lookupIndexOf(listId));
    check(`${name}: locator alias is the normalized name (${alias})`, alias === lookupAliasOf(listId));
  ...
  check(`${name}: list read by authored id`, read.status === 200 && read.json?.id === listId, `status=${read.status}`);
  const item = await kbn('GET', `/api/lists/items?list_id=${encodeURIComponent(listId)}&value=${IN_SMALL}`);
  const found = Array.isArray(item.json) ? item.json[0] : item.json;
  check(`${name}: item read by value through authored id`, item.status === 200 && found?.value === IN_SMALL && found?.list_id === listId, `status=${item.status}`);
};
...
const createList = async (cell, check) => {
  ...
  for (let i = 0; i < values.length; i += IMPORT_CHUNK) await importValues(listId, type, values.slice(i, i + IMPORT_CHUNK));
  let count = 0;
  for (let i = 0; i < 90 && count < LARGE_COUNT; i++) { await sleep(2000); count = await itemCount(cell, listId); }
  check(`${name}: ${LARGE_COUNT} items imported (post-filter path)`, count === LARGE_COUNT, `count=${count}`);
};

const createExceptionAndRule = async (cell) => {
  const name = nameOf(cell);
  const container = await kbn('POST', '/api/exception_lists', { list_id: name, name, description: name, type: 'detection', namespace_type: 'single' });
  if (container.status >= 400) throw new Error(`create exception list ${name}: ${container.status} ${JSON.stringify(container.json)}`);
  const item = await kbn('POST', '/api/exception_lists/items', {
    list_id: name, name, description: name, type: 'simple', namespace_type: 'single',
    // the exception references the list id as authored, mixed case included
    entries: [{ field: 'source.ip', type: 'list', operator: cell.op, list: { id: listIdOf(cell), type: cell.kind === 'range' ? 'ip_range' : 'ip' } }],
  });
  if (item.status >= 400) throw new Error(`create exception item ${name}: ${item.status} ${JSON.stringify(item.json)}`);
  const rule = await kbn('POST', '/api/detection_engine/rules', {
    rule_id: name, name, description: name, type: 'query', enabled: true,
    risk_score: 1, severity: 'low', from: 'now-1h', interval: '1m',
    index: [SRC_INDEX], query: `host.name: "${P}"`, language: 'kuery',
    exceptions_list: [{ id: container.json.id, list_id: name, type: 'detection', namespace_type: 'single' }],
  });
  if (rule.status >= 400) throw new Error(`create rule ${name}: ${rule.status} ${JSON.stringify(rule.json)}`);
  return rule.json.id;
};
...
const alertsFor = async (name) => {
  const r = await es('POST', `/${ALERTS_INDEX}/_search?ignore_unavailable=true`, {
    _source: false, fields: ['source.ip'], size: 100, query: { term: { 'kibana.alert.rule.name': name } },
  });
  return [...new Set((r.json.hits?.hits ?? []).map((h) => h.fields?.['source.ip']?.[0]))].filter(Boolean).sort();
};
...
const main = async () => {
  ...
  const since = now();
  await sleep(1000);
  for (const id of Object.values(ruleIds)) await kbn('POST', `/internal/alerting/rule/${id}/_run_soon`);
  const observed = {};
  for (const cell of CELLS) {
    const name = nameOf(cell);
    const last = await waitForExecution(name, since);
    check(`${name}: executed (${last?.status ?? 'no execution'})`, last?.status === 'succeeded', last?.status === 'succeeded' ? '' : String(last?.message ?? '').slice(0, 200));
  }
  ...
  for (const cell of CELLS) {
    ...
    check(`${name}: alerts ${JSON.stringify(observed[name])} match expected ${JSON.stringify(expected)}`, JSON.stringify(observed[name]) === JSON.stringify(expected));
  }
  ...
  for (const cell of CELLS.filter((c) => c.storage === 'lookup')) {
    ...
    check(`${cell.kind}/${cell.size}/${cell.op}${cell.idCase === 'mixed' ? '/mixed case id' : ''}: lookup alerts equal legacy alerts`, JSON.stringify(observed[nameOf(cell)]) === JSON.stringify(observed[twin]), `${JSON.stringify(observed[nameOf(cell)])} vs ${JSON.stringify(observed[twin])}`);
  }
